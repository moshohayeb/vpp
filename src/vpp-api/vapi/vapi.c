...
void
vapi_store_request(vapi_ctx_t ctx, u32 context, bool is_dump, vapi_cb_t callback, void *callback_ctx)
{
...
vapi_ctx_alloc(vapi_ctx_t *result)
{
    ...
    memset(ctx->vapi_msg_id_t_to_vl_msg_id, ~0, __vapi_metadata.count * sizeof(*ctx->vapi_msg_id_t_to_vl_msg_id));
...
vapi_error_e
vapi_connect(vapi_ctx_t ctx, const char *name, const char *chroot_prefix, int max_outstanding_requests,
             int response_queue_size, vapi_mode_e mode, bool handle_keepalives)
{
    ...
    for (i = 0; i < __vapi_metadata.count; ++i) {
        ...
        if (VAPI_INVALID_MSG_ID != id) {
            ...
            if (id > ctx->vl_msg_id_max) {
                vapi_msg_id_t *tmp =
                    realloc(ctx->vl_msg_id_to_vapi_msg_t, sizeof(*ctx->vl_msg_id_to_vapi_msg_t) * (id + 1));
                if (!tmp) {
                    rv = VAPI_ENOMEM;
                    goto fail;
                }
...
vapi_send(vapi_ctx_t ctx, void *msg)
{
    ...
    if (msgid <= ctx->vl_msg_id_max) {
        ...
        if (id < __vapi_metadata.count) {
            VAPI_DBG("send msg@%p:%u[%s]", msg, msgid, __vapi_metadata.msgs[id]->name);
        } else {
...
vapi_error_e
vapi_recv(vapi_ctx_t ctx, void **msg, size_t *msg_size, svm_q_conditional_wait_t cond, u32 time)
{
    ...
    if (tmp == 0) {
        ...
        *msg_size = ntohl(msgbuf->data_len);
        ...
        if (msgid <= ctx->vl_msg_id_max) {
            vapi_msg_id_t id = ctx->vl_msg_id_to_vapi_msg_t[msgid];
            if (id < __vapi_metadata.count) {
                VAPI_DBG("recv msg@%p:%u[%s]", *msg, msgid, __vapi_metadata.msgs[id]->name);
            } else {
...
static vapi_error_e
vapi_dispatch_response(vapi_ctx_t ctx, vapi_msg_id_t id, u32 context, void *msg)
{
    int mrv;
    ...
    VAPI_DBG("dispatch, search from %d, %s at %d", ctx->requests_start,
             ctx->requests[tmp].context == context ? "matched" : "stopped", tmp);
    vapi_error_e rv = VAPI_OK;
    if (ctx->requests[tmp].context == context) {
        while (ctx->requests_start != tmp) {
            ...
            ctx->requests[ctx->requests_start].callback(ctx, ctx->requests[ctx->requests_start].callback_ctx,
                                                        VAPI_ENORESP, true, NULL);
            memset(&ctx->requests[ctx->requests_start], 0, sizeof(ctx->requests[ctx->requests_start]));
            ++ctx->requests_start;
            --ctx->requests_count;
        ...
        if (payload_offset != -1) {
             ...
             */
            rv = ((vapi_error_e(*)(vapi_ctx_t, void *, vapi_error_e, bool)) ctx->requests[tmp]
                      .callback)(ctx, ctx->requests[tmp].callback_ctx, VAPI_OK, is_last);
        }
        if (is_last) {
            memset(&ctx->requests[ctx->requests_start], 0, sizeof(ctx->requests[ctx->requests_start]));
            ++ctx->requests_start;
        ...
        VAPI_DBG("after dispatch, req start = %d, end = %d, count = %d", ctx->requests_start, requests_end,
                 ctx->requests_count);
    }
...
vapi_dispatch_event(vapi_ctx_t ctx, vapi_msg_id_t id, void *msg)
{
    if (ctx->event_cbs[id].cb) {
        ...
        VAPI_DBG("No handler/generic handler for msg id %u[%s], message ignored", (unsigned) id,
...
vapi_dispatch_one(vapi_ctx_t ctx)
{
    ...
    if (vpp_id > ctx->vl_msg_id_max) {
        VAPI_ERR("Unknown msg ID received, id `%u', out of range <0,%u>", (unsigned) vpp_id,
                 (unsigned) ctx->vl_msg_id_max);
        vapi_msg_free(ctx, msg);
        return VAPI_EINVAL;
    }
    ...
    if (size < expect_size) {
        VAPI_ERR("Invalid msg received, unexpected size `%zu' < expected min `%zu'", size, expect_size);
        vapi_msg_free(ctx, msg);
        return VAPI_EINVAL;
    }
    u32 context;
...
void
vapi_set_event_cb(vapi_ctx_t ctx, vapi_msg_id_t id, vapi_event_cb callback, void *callback_ctx)
{
...
void
vapi_set_generic_event_cb(vapi_ctx_t ctx, vapi_generic_event_cb callback, void *callback_ctx)
{
