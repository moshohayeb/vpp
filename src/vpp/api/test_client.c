...
vl_api_sw_interface_details_t_handler(vl_api_sw_interface_details_t *mp)
{
    ...
    fformat(stdout,
            "details: %s sw_if_index %d sup_sw_if_index %d "
            "link_duplex %s link_speed %s",
            mp->interface_name, ntohl(mp->sw_if_index), ntohl(mp->sup_sw_if_index), duplex, speed);
...
static void
vl_api_ip_neighbor_add_del_reply_t_handler(vl_api_ip_neighbor_add_del_reply_t *mp)
{
...
vl_api_vnet_ip4_fib_counters_t_handler(vl_api_vnet_ip4_fib_counters_t *mp)
{
    ...
    for (i = 0; i < count; i++) {
        fformat(stdout, "%U: %lld packets, %lld bytes\n", format_ip4_address_and_length, &ctrp->address,
                (u32) ctrp->address_length, clib_net_to_host_u64(ctrp->packets), clib_net_to_host_u64(ctrp->bytes));
        ctrp++;
    }
}
...
{
    fformat(stdout, "OAM: %U now %s\n", format_ip4_address, &mp->dst_address, mp->state == 1 ? "alive" : "dead");
}
...
main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        case 'z':
            ...
            break;
        case 't':
            set_interface_table(tm, 0 /* is_ipv6 */, 11 /* my amp goes to 11 */);
            break;
        case 'T':
            set_interface_table(tm, 1 /* is_ipv6 */, 12 /* my amp goes to 12 */);
            break;
