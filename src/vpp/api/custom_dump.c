...
static void *
vl_api_create_loopback_instance_t_print(vl_api_create_loopback_instance_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_set_flags_t_print(vl_api_sw_interface_set_flags_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_set_rx_placement_t_print(vl_api_sw_interface_set_rx_placement_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_rx_placement_dump_t_print(vl_api_sw_interface_rx_placement_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_event_t_print(vl_api_sw_interface_event_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_add_del_address_t_print(vl_api_sw_interface_add_del_address_t *mp, void *handle)
{
    u8 *s;
    ...
    if (mp->is_ipv6)
        s = format(s, "%U/%d ", format_ip6_address, (ip6_address_t *) mp->address, mp->address_length);
    else
        s = format(s, "%U/%d ", format_ip4_address, (ip4_address_t *) mp->address, mp->address_length);
...
static void *
vl_api_sw_interface_set_table_t_print(vl_api_sw_interface_set_table_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_set_mpls_enable_t_print(vl_api_sw_interface_set_mpls_enable_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_set_vpath_t_print(vl_api_sw_interface_set_vpath_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_set_vxlan_bypass_t_print(vl_api_sw_interface_set_vxlan_bypass_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_set_geneve_bypass_t_print(vl_api_sw_interface_set_geneve_bypass_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_set_l2_xconnect_t_print(vl_api_sw_interface_set_l2_xconnect_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_set_l2_bridge_t_print(vl_api_sw_interface_set_l2_bridge_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_bridge_domain_add_del_t_print(vl_api_bridge_domain_add_del_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_bridge_domain_set_mac_age_t_print(vl_api_bridge_domain_set_mac_age_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_bridge_domain_dump_t_print(vl_api_bridge_domain_dump_t *mp, void *handle)
{
    u8 *s;
...
vl_api_bd_ip_mac_add_del_t_print(vl_api_bd_ip_mac_add_del_t *mp, void *handle)
{
    ...
    if (mp->is_ipv6)
        s = format(s, "%U ", format_ip6_address, (ip6_address_t *) mp->ip_address);
    else
        s = format(s, "%U ", format_ip4_address, (ip4_address_t *) mp->ip_address);
...
static void *
vl_api_tap_connect_t_print(vl_api_tap_connect_t *mp, void *handle)
{
    u8 *s;
    u8 null_mac[6];
    ...
    if (mp->ip4_address_set)
        s = format(s, "address %U/%d ", format_ip4_address, mp->ip4_address, mp->ip4_mask_width);
    if (mp->ip6_address_set)
        s = format(s, "address %U/%d ", format_ip6_address, mp->ip6_address, mp->ip6_mask_width);
    FINISH;
}

static void *
vl_api_tap_modify_t_print(vl_api_tap_modify_t *mp, void *handle)
{
    u8 *s;
    u8 null_mac[6];
...
static void *
vl_api_tap_delete_t_print(vl_api_tap_delete_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_tap_dump_t_print(vl_api_sw_interface_tap_dump_t *mp, void *handle)
{
    u8 *s;
...
vl_api_tap_create_v2_t_print(vl_api_tap_create_v2_t *mp, void *handle)
{
    ...
    if (memcmp(mp->mac_address, null_mac, 6))
        s = format(s, "mac-address %U ", format_ethernet_address, mp->mac_address);
    if (memcmp(mp->host_mac_addr, null_mac, 6))
        s = format(s, "host-mac-addr %U ", format_ethernet_address, mp->host_mac_addr);
    ...
    if (mp->host_ip4_addr_set)
        s = format(s, "host-ip4-addr %U/%d ", format_ip4_address, mp->host_ip4_addr, mp->host_ip4_prefix_len);
    if (mp->host_ip6_addr_set)
        s = format(s, "host-ip6-addr %U/%d ", format_ip6_address, mp->host_ip6_addr, mp->host_ip6_prefix_len);
...
static void *
vl_api_sw_interface_tap_v2_dump_t_print(vl_api_sw_interface_tap_v2_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_bond_enslave_t_print(vl_api_bond_enslave_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_bond_detach_slave_t_print(vl_api_bond_detach_slave_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_bond_dump_t_print(vl_api_sw_interface_bond_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_slave_dump_t_print(vl_api_sw_interface_slave_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_ip_add_del_route_t_print(vl_api_ip_add_del_route_t *mp, void *handle)
{
    u8 *s;
    ...
    if (mp->is_ipv6)
        s = format(s, "%U/%d ", format_ip6_address, mp->dst_address, mp->dst_address_length);
    else
        s = format(s, "%U/%d ", format_ip4_address, mp->dst_address, mp->dst_address_length);
    ...
    if (mp->next_hop_n_out_labels) {
        ...
        for (i = 0; i < mp->next_hop_n_out_labels; i++) {
            s = format(s, "out-label %d ", ntohl(mp->next_hop_out_label_stack[i].label));
        }
    }
...
static void *
vl_api_mpls_route_add_del_t_print(vl_api_mpls_route_add_del_t *mp, void *handle)
{
    u8 *s;
    ...
    if (mp->mr_next_hop_proto == DPO_PROTO_IP4) {
        ...
        if (memcmp(mp->mr_next_hop, &ip6_null, sizeof(ip6_null)))
            ...
            s = format(s, "via lookup-in-ip6-table %d ", ntohl(mp->mr_next_hop_table_id));
        ...
        if (mp->mr_next_hop_via_label != htonl(MPLS_LABEL_INVALID))
            s = format(s, "via via-label %d ", ntohl(mp->mr_next_hop_via_label));
        else
            s = format(s, "via next-hop-table %d ", ntohl(mp->mr_next_hop_table_id));
    }
    ...
    if (mp->mr_next_hop_n_out_labels) {
        ...
        for (i = 0; i < mp->mr_next_hop_n_out_labels; i++) {
            s = format(s, "out-label %d ", ntohl(mp->mr_next_hop_out_label_stack[i].label));
        }
    }
...
static void *
vl_api_mpls_table_add_del_t_print(vl_api_mpls_table_add_del_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_proxy_arp_add_del_t_print(vl_api_proxy_arp_add_del_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: proxy_arp_add_del ");

    s = format(s, "%U - %U ", format_ip4_address, mp->proxy.low_address, format_ip4_address, mp->proxy.hi_address);
...
static void *
vl_api_proxy_arp_intfc_enable_disable_t_print(vl_api_proxy_arp_intfc_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_mpls_tunnel_add_del_t_print(vl_api_mpls_tunnel_add_del_t *mp, void *handle)
{
    u8 *s;
    ...
    if (mp->mt_next_hop_n_out_labels) {
        ...
        for (i = 0; i < mp->mt_next_hop_n_out_labels; i++) {
            s = format(s, "out-label %d ", ntohl(mp->mt_next_hop_out_label_stack[i].label));
        }
    }
...
static void *
vl_api_sr_mpls_policy_add_t_print(vl_api_sr_mpls_policy_add_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sr_mpls_policy_del_t_print(vl_api_sr_mpls_policy_del_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_set_unnumbered_t_print(vl_api_sw_interface_set_unnumbered_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_ip_neighbor_add_del_t_print(vl_api_ip_neighbor_add_del_t *mp, void *handle)
{
    u8 *s;
    u8 null_mac[6];
...
static void *
vl_api_oam_add_del_t_print(vl_api_oam_add_del_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_dhcp_proxy_config_t_print(vl_api_dhcp_proxy_config_t *mp, void *handle)
{
    u8 *s;
    ...
    if (mp->is_ipv6) {
        s = format(s, "svr %U ", format_ip6_address, (ip6_address_t *) mp->dhcp_server);
        s = format(s, "src %U ", format_ip6_address, (ip6_address_t *) mp->dhcp_src_address);
    } else {
        s = format(s, "svr %U ", format_ip4_address, (ip4_address_t *) mp->dhcp_server);
        s = format(s, "src %U ", format_ip4_address, (ip4_address_t *) mp->dhcp_src_address);
    }
    if (mp->is_add == 0)
        s = format(s, "del ");
...
static void *
vl_api_dhcp_proxy_set_vss_t_print(vl_api_dhcp_proxy_set_vss_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_dhcp_client_config_t_print(vl_api_dhcp_client_config_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_ip6_set_link_local_address_t_print(vl_api_sw_interface_ip6_set_link_local_address_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_ip6nd_ra_prefix_t_print(vl_api_sw_interface_ip6nd_ra_prefix_t *mp, void *handle)
{
    u8 *s;
    ...
    s = format(s, "%U/%d ", format_ip6_address, mp->address, mp->address_length);
...
static void *
vl_api_sw_interface_ip6nd_ra_config_t_print(vl_api_sw_interface_ip6nd_ra_config_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_set_arp_neighbor_limit_t_print(vl_api_set_arp_neighbor_limit_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sr_localsid_add_del_t_print(vl_api_sr_localsid_add_del_t *mp, void *handle)
{
    ...
    case SR_BEHAVIOR_END:
        s = format(s, "Address: %U\nBehavior: End", format_ip6_address, (ip6_address_t *) mp->localsid.addr);
    ...
    case SR_BEHAVIOR_X:
        s = format(s,
                   ...
                   "\nIface: %U\nNext hop: %U",
                   format_ip6_address, (ip6_address_t *) mp->localsid.addr, format_vnet_sw_if_index_name, vnm,
                   ntohl(mp->sw_if_index), format_ip6_address, (ip6_address_t *) mp->nh_addr6);
        s = format(s, (mp->end_psp ? "End.PSP: True" : "End.PSP: False"));
        break;
    case SR_BEHAVIOR_DX4:
        s = format(s,
                   ...
                   "\nIface: %U\nNext hop: %U",
                   format_ip6_address, (ip6_address_t *) mp->localsid.addr, format_vnet_sw_if_index_name, vnm,
                   ntohl(mp->sw_if_index), format_ip4_address, (ip4_address_t *) mp->nh_addr4);
        break;
    ...
    case SR_BEHAVIOR_DT6:
        s = format(s,
                   ...
                   "\nTable: %u",
                   format_ip6_address, (ip6_address_t *) mp->localsid.addr, ntohl(mp->fib_table));
        break;
    ...
    default:
        if (mp->behavior >= SR_BEHAVIOR_LAST) {
            s = format(s, "Address: %U\n Behavior: %u", format_ip6_address, (ip6_address_t *) mp->localsid.addr,
                       mp->behavior);
        } else
...
static void *
vl_api_sr_steering_add_del_t_print(vl_api_sr_steering_add_del_t *mp, void *handle)
{
    u8 *s;
    ...
    case SR_STEER_L2:
        ...
        break;
    case SR_STEER_IPV4:
        s = format(s, "Traffic type: IPv4 %U/%u", format_ip4_address, (ip4_address_t *) mp->prefix_addr,
                   ntohl(mp->mask_width));
        break;
    case SR_STEER_IPV6:
        s = format(s, "Traffic type: IPv6 %U/%u", format_ip6_address, (ip6_address_t *) mp->prefix_addr,
                   ntohl(mp->mask_width));
        break;
    default:
    ...
    s = format(s, "BindingSID: %U", format_ip6_address, (ip6_address_t *) mp->bsid_addr);
...
vl_api_sr_policy_add_t_print(vl_api_sr_policy_add_t *mp, void *handle)
{
    ...
    s = format(s, "BSID: %U", format_ip6_address, (ip6_address_t *) mp->bsid_addr);
...
static void *
vl_api_classify_add_del_table_t_print(vl_api_classify_add_del_table_t *mp, void *handle)
{
    u8 *s;
    int i;
    ...
    if (mp->is_add == 0) {
        ...
        if (mp->current_data_flag)
            s = format(s, "current-data-offset %d ", ntohl(mp->current_data_offset));
        s = format(s, "mask hex ");
...
static void *
vl_api_classify_add_del_session_t_print(vl_api_classify_add_del_session_t *mp, void *handle)
{
    u8 *s;
    int i, limit = 0;
...
static void *
vl_api_classify_set_interface_ip_table_t_print(vl_api_classify_set_interface_ip_table_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_classify_set_interface_l2_tables_t_print(vl_api_classify_set_interface_l2_tables_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_l2tpv3_create_tunnel_t_print(vl_api_l2tpv3_create_tunnel_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: l2tpv3_create_tunnel ");

    s = format(s, "client_address %U our_address %U ", format_ip6_address, (ip6_address_t *) (mp->client_address),
    ...
    s = format(s, "local_cookie %lld ", clib_net_to_host_u64(mp->local_cookie));
    s = format(s, "remote_cookie %lld ", clib_net_to_host_u64(mp->remote_cookie));
...
static void *
vl_api_l2tpv3_set_tunnel_cookies_t_print(vl_api_l2tpv3_set_tunnel_cookies_t *mp, void *handle)
{
    u8 *s;
    ...
    s = format(s, "new_local_cookie %llu ", clib_net_to_host_u64(mp->new_local_cookie));

    s = format(s, "new_remote_cookie %llu ", clib_net_to_host_u64(mp->new_remote_cookie));

    FINISH;
}

static void *
vl_api_l2tpv3_interface_enable_disable_t_print(vl_api_l2tpv3_interface_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_l2tpv3_set_lookup_key_t_print(vl_api_l2tpv3_set_lookup_key_t *mp, void *handle)
{
    u8 *s;
    char *str = "unknown";
...
static void *
vl_api_sw_if_l2tpv3_tunnel_dump_t_print(vl_api_sw_if_l2tpv3_tunnel_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_vxlan_add_del_tunnel_t_print(vl_api_vxlan_add_del_tunnel_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_geneve_add_del_tunnel_t_print(vl_api_geneve_add_del_tunnel_t *mp, void *handle)
{
    u8 *s;
    ...
    s = format(s, "%s %U ", remote_name, format_ip46_address, &remote, IP46_TYPE_ANY);
...
static void *
vl_api_geneve_tunnel_dump_t_print(vl_api_geneve_tunnel_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_gre_add_del_tunnel_t_print(vl_api_gre_add_del_tunnel_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: gre_add_del_tunnel ");

    s = format(s, "dst %U ", format_ip46_address, (ip46_address_t *) &(mp->dst_address),
               mp->is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4);

    s = format(s, "src %U ", format_ip46_address, (ip46_address_t *) &(mp->src_address),
               mp->is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4);
...
static void *
vl_api_gre_tunnel_dump_t_print(vl_api_gre_tunnel_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_l2_fib_clear_table_t_print(vl_api_l2_fib_clear_table_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_l2_interface_efp_filter_t_print(vl_api_l2_interface_efp_filter_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_l2_interface_vlan_tag_rewrite_t_print(vl_api_l2_interface_vlan_tag_rewrite_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_create_vhost_user_if_t_print(vl_api_create_vhost_user_if_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_modify_vhost_user_if_t_print(vl_api_modify_vhost_user_if_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_delete_vhost_user_if_t_print(vl_api_delete_vhost_user_if_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_vhost_user_dump_t_print(vl_api_sw_interface_vhost_user_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_want_interface_events_t_print(vl_api_want_interface_events_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: want_interface_events pid %d enable %d ", ntohl(mp->pid), ntohl(mp->enable_disable));
...
static void *
vl_api_vxlan_gpe_add_del_tunnel_t_print(vl_api_vxlan_gpe_add_del_tunnel_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_vxlan_gpe_tunnel_dump_t_print(vl_api_vxlan_gpe_tunnel_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_interface_name_renumber_t_print(vl_api_interface_name_renumber_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_ip_probe_neighbor_t_print(vl_api_ip_probe_neighbor_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_ip_scan_neighbor_enable_disable_t_print(vl_api_ip_scan_neighbor_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_want_ip4_arp_events_t_print(vl_api_want_ip4_arp_events_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: want_ip4_arp_events ");
    s = format(s, "pid %d address %U ", ntohl(mp->pid), format_ip4_address, &mp->address);
    if (mp->enable_disable == 0)
        s = format(s, "del ");
...
static void *
vl_api_want_ip6_nd_events_t_print(vl_api_want_ip6_nd_events_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: want_ip6_nd_events ");
    s = format(s, "pid %d address %U ", ntohl(mp->pid), format_ip6_address, mp->address);
    if (mp->enable_disable == 0)
        s = format(s, "del ");
...
static void *
vl_api_want_l2_macs_events_t_print(vl_api_want_l2_macs_events_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_input_acl_set_interface_t_print(vl_api_input_acl_set_interface_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_output_acl_set_interface_t_print(vl_api_output_acl_set_interface_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_cop_interface_enable_disable_t_print(vl_api_cop_interface_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_cop_whitelist_enable_disable_t_print(vl_api_cop_whitelist_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
vl_api_policer_add_del_t_print(vl_api_policer_add_del_t *mp, void *handle)
{
    ...
    s = format(s, "conform_action %U ", format_policer_action, mp->conform_action_type, mp->conform_dscp);
    s = format(s, "exceed_action %U ", format_policer_action, mp->exceed_action_type, mp->exceed_dscp);
    s = format(s, "violate_action %U ", format_policer_action, mp->violate_action_type, mp->violate_dscp);
...
static void *
vl_api_policer_classify_set_interface_t_print(vl_api_policer_classify_set_interface_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_policer_classify_dump_t_print(vl_api_policer_classify_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_clear_stats_t_print(vl_api_sw_interface_clear_stats_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_mpls_fib_dump_t_print(vl_api_mpls_fib_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_ip_fib_dump_t_print(vl_api_ip_fib_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_ip6_fib_dump_t_print(vl_api_ip6_fib_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_classify_table_ids_t_print(vl_api_classify_table_ids_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_classify_table_by_interface_t_print(vl_api_classify_table_by_interface_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_classify_table_info_t_print(vl_api_classify_table_info_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_classify_session_dump_t_print(vl_api_classify_session_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_set_ipfix_exporter_t_print(vl_api_set_ipfix_exporter_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: set_ipfix_exporter ");

    s = format(s, "collector-address %U ", format_ip4_address, (ip4_address_t *) mp->collector_address);
    s = format(s, "collector-port %d ", ntohs(mp->collector_port));
    s = format(s, "src-address %U ", format_ip4_address, (ip4_address_t *) mp->src_address);
...
static void *
vl_api_ipfix_exporter_dump_t_print(vl_api_ipfix_exporter_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_set_ipfix_classify_stream_t_print(vl_api_set_ipfix_classify_stream_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_ipfix_classify_stream_dump_t_print(vl_api_ipfix_classify_stream_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_ipfix_classify_table_add_del_t_print(vl_api_ipfix_classify_table_add_del_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_ipfix_classify_table_dump_t_print(vl_api_ipfix_classify_table_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_span_enable_disable_t_print(vl_api_sw_interface_span_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_span_dump_t_print(vl_api_sw_interface_span_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_pg_create_interface_t_print(vl_api_pg_create_interface_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_ip_source_and_port_range_check_add_del_t_print(vl_api_ip_source_and_port_range_check_add_del_t *mp, void *handle)
{
    u8 *s;
    int i;
    ...
    if (mp->is_ipv6)
        s = format(s, "%U/%d ", format_ip6_address, mp->address, mp->mask_length);
    else
        s = format(s, "%U/%d ", format_ip4_address, mp->address, mp->mask_length);
...
static void *
vl_api_lisp_enable_disable_t_print(vl_api_lisp_enable_disable_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: lisp_enable_disable %s", mp->is_en ? "enable" : "disable");

    FINISH;
}

static void *
vl_api_gpe_add_del_iface_t_print(vl_api_gpe_add_del_iface_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_pitr_set_locator_set_t_print(vl_api_lisp_pitr_set_locator_set_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_add_del_remote_mapping_t_print(vl_api_lisp_add_del_remote_mapping_t *mp, void *handle)
{
    u8 *s;
    u32 rloc_num = 0;
    ...
    s = format(s, "eid %U ", format_lisp_flat_eid, mp->eid_type, mp->eid, mp->eid_len);

    if (mp->is_src_dst) {
        s = format(s, "seid %U ", format_lisp_flat_eid, mp->eid_type, mp->seid, mp->seid_len);
    }
...
static void *
vl_api_lisp_add_del_adjacency_t_print(vl_api_lisp_add_del_adjacency_t *mp, void *handle)
{
    u8 *s;
    ...
    s = format(s, "%s ", mp->is_add ? "add" : "del");
    s = format(s, "vni %d ", clib_net_to_host_u32(mp->vni));
    s = format(s, "reid %U leid %U ", format_lisp_flat_eid, mp->eid_type, mp->reid, mp->reid_len, format_lisp_flat_eid,
...
static void *
vl_api_lisp_add_del_map_request_itr_rlocs_t_print(vl_api_lisp_add_del_map_request_itr_rlocs_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_eid_table_add_del_map_t_print(vl_api_lisp_eid_table_add_del_map_t *mp, void *handle)
{
    u8 *s;
    ...
    s = format(s, "vni %d ", clib_net_to_host_u32(mp->vni));
    s = format(s, "%s %d ", mp->is_l2 ? "bd_index" : "vrf", clib_net_to_host_u32(mp->dp_table));
    FINISH;
}

static void *
vl_api_lisp_add_del_local_eid_t_print(vl_api_lisp_add_del_local_eid_t *mp, void *handle)
{
    u8 *s;
    ...
    s = format(s, "vni %d ", clib_net_to_host_u32(mp->vni));
    s = format(s, "eid %U ", format_lisp_flat_eid, mp->eid_type, mp->eid, mp->prefix_len);
...
static void *
vl_api_gpe_add_del_fwd_entry_t_print(vl_api_gpe_add_del_fwd_entry_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_add_del_map_resolver_t_print(vl_api_lisp_add_del_map_resolver_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_gpe_enable_disable_t_print(vl_api_gpe_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_add_del_locator_set_t_print(vl_api_lisp_add_del_locator_set_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_add_del_locator_t_print(vl_api_lisp_add_del_locator_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_locator_set_dump_t_print(vl_api_lisp_locator_set_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_locator_dump_t_print(vl_api_lisp_locator_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_map_request_mode_t_print(vl_api_lisp_map_request_mode_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_eid_table_dump_t_print(vl_api_lisp_eid_table_dump_t *mp, void *handle)
{
    u8 *s;
    ...
    if (mp->eid_set) {
        s = format(s, "vni %d ", clib_net_to_host_u32(mp->vni));
        s = format(s, "eid %U ", format_lisp_flat_eid, mp->eid_type, mp->eid, mp->prefix_length);
        switch (mp->filter) {
        case 1:
            s = format(s, "local ");
            break;
...
static void *
vl_api_lisp_rloc_probe_enable_disable_t_print(vl_api_lisp_rloc_probe_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_map_register_enable_disable_t_print(vl_api_lisp_map_register_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_adjacencies_get_t_print(vl_api_lisp_adjacencies_get_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_lisp_eid_table_map_dump_t_print(vl_api_lisp_eid_table_map_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_ipsec_gre_add_del_tunnel_t_print(vl_api_ipsec_gre_add_del_tunnel_t *mp, void *handle)
{
    u8 *s;
    ...
    s = format(s, "src %U ", format_ip4_address, (ip4_address_t *) &(mp->src_address));
...
static void *
vl_api_ipsec_gre_tunnel_dump_t_print(vl_api_ipsec_gre_tunnel_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_l2_interface_pbb_tag_rewrite_t_print(vl_api_l2_interface_pbb_tag_rewrite_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_flow_classify_set_interface_t_print(vl_api_flow_classify_set_interface_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_flow_classify_dump_t_print(vl_api_flow_classify_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_feature_enable_disable_t_print(vl_api_feature_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_tag_add_del_t_print(vl_api_sw_interface_tag_add_del_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_hw_interface_set_mtu_t_print(vl_api_hw_interface_set_mtu_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_tcp_configure_src_addresses_t_print(vl_api_tcp_configure_src_addresses_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: tcp_configure_src_addresses ");
    if (mp->is_ipv6)
        s = format(s, "%U - %U ", format_ip6_address, (ip6_address_t *) mp->first_address, format_ip6_address,
                   (ip6_address_t *) mp->last_address);
    else
        s = format(s, "%U - %U ", format_ip4_address, (ip4_address_t *) mp->first_address, format_ip4_address,
                   (ip4_address_t *) mp->last_address);
...
static void *
vl_api_app_namespace_add_del_t_print(vl_api_app_namespace_add_del_t *mp, void *handle)
{
    u8 *s, *ns_id = 0;
    u8 len                = clib_min(mp->namespace_id_len, ARRAY_LEN(mp->namespace_id) - 1);
    mp->namespace_id[len] = 0;
    s                     = format(0, "SCRIPT: app_namespace_add_del ");
    s                     = format(s,
               "ns-id %s secret %lu sw_if_index %d ipv4_fib_id %d "
               "ipv6_fib_id %d",
               (char *) mp->namespace_id, mp->secret, clib_net_to_host_u32(mp->sw_if_index),
               clib_net_to_host_u32(mp->ip4_fib_id), clib_net_to_host_u32(mp->ip6_fib_id));
    FINISH;
}

static void *
vl_api_lldp_config_t_print(vl_api_lldp_config_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_dns_enable_disable_t_print(vl_api_dns_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_sw_interface_set_lldp_t_print(vl_api_sw_interface_set_lldp_t *mp, void *handle)
{
    u8 *s;
    u8 null_data[256];
...
static void *
vl_api_dns_name_server_add_del_t_print(vl_api_dns_name_server_add_del_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: dns_name_server_add_del ");
    if (mp->is_ip6)
        s = format(s, "%U ", format_ip6_address, (ip6_address_t *) mp->server_address);
    else
        s = format(s, "%U ", format_ip4_address, (ip4_address_t *) mp->server_address);
...
static void *
vl_api_dns_resolve_name_t_print(vl_api_dns_resolve_name_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_dns_resolve_ip_t_print(vl_api_dns_resolve_ip_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_session_rule_add_del_t_print(vl_api_session_rule_add_del_t *mp, void *handle)
{
    u8 *s;
    ...
    if (mp->is_ip4)
        s = format(s, "appns %d scope %d %s %U/%d %d %U/%d %d action %u tag %s", mp->appns_index, mp->scope, proto,
                   format_ip4_address, (ip4_address_t *) mp->lcl_ip, mp->lcl_plen, format_ip4_address,
                   (ip4_address_t *) mp->rmt_ip, mp->rmt_plen, mp->action_index, mp->tag);
    else
        s = format(s, "appns %d scope %d %s %U/%d %d %U/%d %d action %u tag %s", mp->appns_index, mp->scope, proto,
                   format_ip6_address, (ip6_address_t *) mp->lcl_ip, mp->lcl_plen, format_ip6_address,
                   (ip6_address_t *) mp->rmt_ip, mp->rmt_plen, mp->action_index, mp->tag);
    FINISH;
}

static void *
vl_api_ip_container_proxy_add_del_t_print(vl_api_ip_container_proxy_add_del_t *mp, void *handle)
{
    u8 *s;
    s = format(0, "SCRIPT: ip_container_proxy_add_del ");
    if (mp->is_ip4)
        s = format(s, "is_add %d address %U/%d sw_if_index %d", mp->is_add, format_ip4_address,
                   (ip4_address_t *) mp->ip, mp->plen, mp->sw_if_index);
    else
        s = format(s, "is_add %d address %U/%d sw_if_index %d", mp->is_add, format_ip6_address,
                   (ip6_address_t *) mp->ip, mp->plen, mp->sw_if_index);
    FINISH;
}

static void *
vl_api_qos_record_enable_disable_t_print(vl_api_qos_record_enable_disable_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: qos_record_enable_disable ");
    s = format(s, "sw_if_index %d ", ntohl(mp->sw_if_index));
    s = format(s, "input_source %U ", format_qos_source, ntohl(mp->input_source));
...
#define foreach_custom_print_function                                                                                  \
    ...
    _(SW_INTERFACE_IP6_SET_LINK_LOCAL_ADDRESS, sw_interface_ip6_set_link_local_address)                                \
    ...
    _(IP_SOURCE_AND_PORT_RANGE_CHECK_ADD_DEL, ip_source_and_port_range_check_add_del)                                  \
    _(IP_SOURCE_AND_PORT_RANGE_CHECK_INTERFACE_ADD_DEL, ip_source_and_port_range_check_interface_add_del)              \
    ...
    _(LISP_ADD_DEL_MAP_REQUEST_ITR_RLOCS, lisp_add_del_map_request_itr_rlocs)                                          \
...
{
#define _(n, f) am->msg_print_handlers[VL_API_##n] = (void *) vl_api_##f##_t_print;
