...
get_thread_data(vl_api_thread_data_t *td, int index)
{
    ...
    if (w->registration)
        strncpy((char *) td->type, (char *) w->registration->name, ARRAY_LEN(td->type) - 1);
    td->pid        = htonl(w->lwp);
...
vl_api_get_node_index_t_handler(vl_api_get_node_index_t *mp)
{
    ...
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_GET_NODE_INDEX_REPLY, ({ rmp->node_index = ntohl(node_index); }));
    /* *INDENT-ON* */
}
...
out:
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_GET_NEXT_INDEX_REPLY, ({ rmp->next_index = ntohl(next_index); }));
    /* *INDENT-ON* */
}
...
out:
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_GET_NODE_INDEX_REPLY, ({ rmp->next_index = ntohl(next_index); }));
    /* *INDENT-ON* */
}
...
#undef _
    ...
    am->api_trace_cfg[VL_API_CLASSIFY_ADD_DEL_SESSION].size += 5 * sizeof(u32x4);
...
api_segment_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "prefix %s", &chroot_path)) {
            ...
            if (rv < 0) {
                e = clib_error_return_code(0, rv, CLIB_ERROR_ERRNO_VALID | CLIB_ERROR_FATAL, "cannot fetch username %s",
                                           s);
                vec_free(s);
                vec_free(buf);
                return e;
...
static clib_error_t *
show_ip_arp_nd_events_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vpe_api_main_t *am = &vpe_api_main;
    ...
    if (pool_elts(am->arp_events) == 0 && pool_elts(am->nd_events) == 0 &&
        pool_elts(am->wc_ip4_arp_events_registrations) == 0 && pool_elts(am->wc_ip6_nd_events_registrations) == 0) {
        vlib_cli_output(vm, "No active arp or nd event registrations");
        return 0;
    }

    /* *INDENT-OFF* */
    pool_foreach(arp_event, am->arp_events, ({ vlib_cli_output(vm, "%U", format_arp_event, arp_event); }));

    vpe_client_registration_t *reg;
    pool_foreach(reg, am->wc_ip4_arp_events_registrations,
                 ({ vlib_cli_output(vm, "pid %d: bd mac/ip4 binding events", ntohl(reg->client_pid)); }));

    pool_foreach(nd_event, am->nd_events, ({ vlib_cli_output(vm, "%U", format_nd_event, nd_event); }));

    pool_foreach(reg, am->wc_ip6_nd_events_registrations,
                 ({ vlib_cli_output(vm, "pid %d: bd mac/ip6 binding events", ntohl(reg->client_pid)); }));
    /* *INDENT-ON* */
