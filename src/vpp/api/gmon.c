...
get_significant_errors(gmon_main_t *gm)
{
    ...
    clib_bitmap_foreach(code, gm->sig_error_bitmap, ({
                            for (vm_index = 0; vm_index < vec_len(gm->my_vlib_mains); vm_index++) {
                                ...
                                significant_errors +=
                                    em->counters[code] -
                                    ((vec_len(em->counters_last_clear) > code) ? em->counters_last_clear[code] : 0);
                            }
                        }));
    /* *INDENT-ON* */
...
gmon_init(vlib_main_t *vm)
{
    ...
    gm->sig_error_rate_ptr =
        svmdb_local_get_variable_reference(gm->svmdb_client, SVMDB_NAMESPACE_VEC, "vpp_sig_error_rate");
...
static clib_error_t *
set_significant_error_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u32 index;
    int enable = 1;
    int rv;
