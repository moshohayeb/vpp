...
static void
vl_api_vnet_get_summary_stats_reply_t_handler(vl_api_vnet_get_summary_stats_reply_t *mp)
{
    ...
    vec_reset_length(sb);
    sb = format(sb, "%v,%.0f,%llu,%llu,%llu,%llu\n%c", tm->display_name, mp->vector_rate,
                (unsigned long long) mp->total_pkts[0], (unsigned long long) mp->total_bytes[0],
                (unsigned long long) mp->total_pkts[1], (unsigned long long) mp->total_bytes[1], 0);

    n = sendto(tm->send_data_socket, sb, vec_len(sb), 0, (struct sockaddr *) &tm->send_data_addr,
...
main(int argc, char **argv)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "collector-ip %U", unformat_ip4_address, &collector_ip))
            ;
    ...
    if (display_name == 0) {
        display_name = format(0, "vpe-to-%d.%d.%d.%d", collector_ip.as_u8[0], collector_ip.as_u8[1],
