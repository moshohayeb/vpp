...
send_naws(clib_socket_t *s)
{
    ...
    clib_socket_tx_add_formatted(s,
                                 ...
                                 "%c%c%c%c"
                                 "%c%c",
                                 IAC, SB, TELOPT_NAWS, ws.ws_col >> 8, ws.ws_col & 0xff, ws.ws_row >> 8,
...
#if DEBUG
                clib_warning("SB %s\n  %U", TELOPT(opt), format_hexdump, sb, vec_len(sb));
#endif
                vec_free(sb);
                i += 2;
                if (opt == TELOPT_TTYPE) {
...
#if DEBUG
                clib_warning("IAC at %d, IAC %s %s", i, TELCMD(s->rx_buffer[i + 1]), TELOPT(s->rx_buffer[i + 2]));
#endif
                i += 3;
            }
        } else
