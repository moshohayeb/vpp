...
vlib_stats_pop_heap(void *cm_arg, void *oldheap, stat_directory_type_t type)
{
    ...
    u32 vector_index = lookup_or_create_hash_index(oldheap, stat_segment_name, next_vector_index);
    /* Back to stats segment */
    clib_mem_set_heap(sm->heap); /* Re-enter stat segment */


    /* Update the vector */
    if (vector_index == next_vector_index) { /* New */
    ...
    u64 *offset_vector = ep->offset_vector ? stat_segment_pointer(shared_header, ep->offset_vector) : 0;
    ...
    sm->directory_vector[vector_index].offset = stat_segment_offset(shared_header, cm->counters);
...
stat_validate_counter_vector(stat_segment_directory_entry_t *ep, u32 max)
{
    ...
    for (i = 0; i < tm->n_vlib_mains; i++) {
        ...
        vec_add1(offset_vector, stat_segment_offset(shared_header, counters[i]));
    }
...
vlib_map_stat_segment_init(void)
{
    ...
    void *heap;
#if USE_DLMALLOC == 0
    heap = mheap_alloc_with_flags(((u8 *) memaddr) + getpagesize(), memory_size - getpagesize(),
...
{
    stat_segment_directory_entry_t *ep = va_arg(*args, stat_segment_directory_entry_t *);
    char *type_name;
    char *format_string;
...
static clib_error_t *
show_stat_segment_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    stat_segment_main_t *sm = &stat_segment_main;
    counter_t *counter;
    hash_pair_t *p;
...
update_node_counters(stat_segment_main_t *sm)
{
    ...
    vlib_node_get_nodes(0 /* vm, for barrier sync */, (u32) ~0 /* all threads */, 1 /* include stats */,
    ...
    if (l > no_max_nodes) {
        ...
        stat_validate_counter_vector(&sm->directory_vector[STAT_COUNTER_NODE_CLOCKS], l);
        stat_validate_counter_vector(&sm->directory_vector[STAT_COUNTER_NODE_VECTORS], l);
        stat_validate_counter_vector(&sm->directory_vector[STAT_COUNTER_NODE_CALLS], l);
        stat_validate_counter_vector(&sm->directory_vector[STAT_COUNTER_NODE_SUSPENDS], l);
    ...
    for (j = 0; j < vec_len(node_dups); j++) {
        ...
        for (i = 0; i < vec_len(nodes); i++) {
            ...
            counters    = stat_segment_pointer(shared_header, sm->directory_vector[STAT_COUNTER_NODE_CLOCKS].offset);
            c           = counters[j];
            c[n->index] = n->stats_total.clocks - n->stats_last_clear.clocks;

            counters    = stat_segment_pointer(shared_header, sm->directory_vector[STAT_COUNTER_NODE_VECTORS].offset);
            c           = counters[j];
            c[n->index] = n->stats_total.vectors - n->stats_last_clear.vectors;

            counters    = stat_segment_pointer(shared_header, sm->directory_vector[STAT_COUNTER_NODE_CALLS].offset);
            c           = counters[j];
            c[n->index] = n->stats_total.calls - n->stats_last_clear.calls;

            counters    = stat_segment_pointer(shared_header, sm->directory_vector[STAT_COUNTER_NODE_SUSPENDS].offset);
            c           = counters[j];
...
do_stat_segment_updates(stat_segment_main_t *sm)
{
    ...
    sm->directory_vector[STAT_COUNTER_VECTOR_RATE].value = vector_rate / ((f64)(vec_len(vlib_mains) - start));
    ...
    sm->directory_vector[STAT_COUNTER_INPUT_RATE].value  = (f64)(input_packets - sm->last_input_packets) / dt;
    sm->directory_vector[STAT_COUNTER_LAST_UPDATE].value = now;
    sm->last_input_packets                               = input_packets;
    sm->directory_vector[STAT_COUNTER_LAST_STATS_CLEAR].value = vm->node_main.time_last_runtime_stats_clear;
...
stats_segment_socket_init(void)
{
    ...
    s->flags =
        CLIB_SOCKET_F_IS_SERVER | CLIB_SOCKET_F_SEQPACKET | CLIB_SOCKET_F_ALLOW_GROUP_WRITE | CLIB_SOCKET_F_PASSCRED;
    ...
    template.description     = format(0, "stats segment listener %s", STAT_SEGMENT_SOCKET_FILE);
...
static uword
stat_segment_collector_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    stat_segment_main_t *sm = &stat_segment_main;
