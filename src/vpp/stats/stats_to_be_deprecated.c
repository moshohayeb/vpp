...
format_vnet_interface_combined_counters(u8 *s, va_list *args)
{
    ...
    vl_api_vnet_interface_combined_counters_t *mp = va_arg(*args, vl_api_vnet_interface_combined_counters_t *);
    ...
    for (i = 0; i < count; i++) {
        ...
        vp++;
        s = format(s, "%U.%s.packets %lld\n", format_vnet_sw_if_index_name, sm->vnet_main, sw_if_index, counter_name,
                   packets);
...
{
    stats_main_t *sm                            = &stats_main;
    vl_api_vnet_interface_simple_counters_t *mp = va_arg(*args, vl_api_vnet_interface_simple_counters_t *);
    char *counter_name;
    u32 count, sw_if_index;
    count       = ntohl(mp->count);
    sw_if_index = ntohl(mp->first_sw_if_index);
    ...
    for (i = 0; i < count; i++) {
        ...
        vp++;
        s = format(s, "%U.%s %lld\n", format_vnet_sw_if_index_name, sm->vnet_main, sw_if_index, counter_name, v);
        sw_if_index++;
    }
...
set_client_for_stat(u32 reg, u32 item, vpe_client_registration_t *client)
{
    ...
    if (!p) {
        ...
        hash_set(sm->stats_registration_hash[reg], item, registration - sm->stats_registrations[reg]);
    } else {
        registration = pool_elt_at_index(sm->stats_registrations[reg], p[0]);
    }
...
do_simple_interface_counters(stats_main_t *sm)
{
    ...
    vec_foreach(cm, im->sw_if_counters)
    {
        ...
        for (i = 0; i < n_counts; i++) {
            if (mp == 0) {
                items_this_message = clib_min(SIMPLE_COUNTER_BATCH_SIZE, n_counts - i);

                mp                    = vl_msg_api_alloc_as_if_client(sizeof(*mp) + items_this_message * sizeof(v));
...
void
handle_client_registration(vpe_client_registration_t *client, u32 stat, u32 item, int enable_disable)
{
    stats_main_t *sm = &stats_main;
    ...
    if (enable_disable == 0) {
        ...
        {
            clib_warning("pid %d: already disabled for stats...", client->client_pid);
            return;
        }
        sm->enable_poller = clear_client_for_stat(stat, item, client->client_index);
        return;
    }
    /* Enable case */
    if (!rp) {
...
vl_api_want_interface_combined_stats_t_handler(vl_api_want_interface_combined_stats_t *mp)
{
    ...
    handle_client_registration(&rp, IDX_PER_INTERFACE_COMBINED_COUNTERS, swif, mp->enable_disable);

reply:
    reg = vl_api_client_index_to_registration(mp->client_index);
    if (!reg) {
        sm->enable_poller = clear_client_for_stat(IDX_PER_INTERFACE_COMBINED_COUNTERS, swif, mp->client_index);
        return;
    }
...
do_combined_interface_counters(stats_main_t *sm)
{
    ...
    vec_foreach(cm, im->combined_sw_if_counters)
    {
        ...
        for (i = 0; i < n_counts; i++) {
            if (mp == 0) {
                items_this_message = clib_min(COMBINED_COUNTER_BATCH_SIZE, n_counts - i);
            ...
            clib_mem_unaligned(&vp->packets, u64) = clib_host_to_net_u64(v.packets);
...
static void
vl_api_want_per_interface_combined_stats_t_handler(vl_api_want_per_interface_combined_stats_t *mp)
{
    stats_main_t *sm = &stats_main;
    vpe_client_registration_t rp;
    ...
    for (i = 0; i < num; i++) {
        ...
        rp.client_index = mp->client_index;
        rp.client_pid   = mp->pid;
        handle_client_registration(&rp, IDX_PER_INTERFACE_COMBINED_COUNTERS, swif, ntohl(mp->enable_disable));
    }
...
do_combined_per_interface_counters(stats_main_t *sm)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(reg, sm->stats_registrations[IDX_PER_INTERFACE_COMBINED_COUNTERS], ({ vec_add1(sm->regs_tmp, reg); }));
    /* *INDENT-ON* */
    ...
    for (i = 0; i < vec_len(sm->regs_tmp); i++) {
        ...
        /* *INDENT-OFF* */
        pool_foreach(client, reg->clients, ({ vec_add1(sm->clients_tmp, client); }));
        /* *INDENT-ON* */
        ...
        for (j = 0; j < vec_len(sm->clients_tmp); j++) {
            ...
            if (!vl_reg) {
                sm->enable_poller =
                    clear_client_for_stat(IDX_PER_INTERFACE_COMBINED_COUNTERS, reg->item, client->client_index);
                continue;
            }
...
#define _(X, x)                                                                                                        \
    ...
    clib_mem_unaligned(&vp->x##_packets, u64) = clib_host_to_net_u64(v.packets);                                       \
    clib_mem_unaligned(&vp->x##_bytes, u64)   = clib_host_to_net_u64(v.bytes);
...
/* Request from client registering interfaces it wants */
static void
vl_api_want_per_interface_simple_stats_t_handler(vl_api_want_per_interface_simple_stats_t *mp)
{
    stats_main_t *sm = &stats_main;
    vpe_client_registration_t rp;
    ...
    for (i = 0; i < num; i++) {
        ...
        rp.client_index = mp->client_index;
        rp.client_pid   = mp->pid;
        handle_client_registration(&rp, IDX_PER_INTERFACE_SIMPLE_COUNTERS, swif, ntohl(mp->enable_disable));
    }
...
reply:
    ...
    if (!reg) {
        for (i = 0; i < num; i++) {
            swif              = ntohl(mp->sw_ifs[i]);
            sm->enable_poller = clear_client_for_stat(IDX_PER_INTERFACE_SIMPLE_COUNTERS, swif, mp->client_index);
        }
...
do_simple_per_interface_counters(stats_main_t *sm)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(reg, sm->stats_registrations[IDX_PER_INTERFACE_SIMPLE_COUNTERS], ({ vec_add1(sm->regs_tmp, reg); }));
    /* *INDENT-ON* */
    ...
    for (i = 0; i < vec_len(sm->regs_tmp); i++) {
        ...
        for (j = 0; j < vec_len(sm->clients_tmp); j++) {
            ...
            if (!vl_reg) {
                sm->enable_poller =
                    clear_client_for_stat(IDX_PER_INTERFACE_SIMPLE_COUNTERS, reg->item, client->client_index);
                continue;
            }
...
ip4_nbr_ship(stats_main_t *sm, ip4_nbr_stats_ctx_t *ctx)
{
    ...
    while (0 != vec_len(ctx->counters)) {
        u32 n_items = MIN(vec_len(ctx->counters), IP4_FIB_COUNTER_BATCH_SIZE);
        u8 pause    = 0;

        dslock(sm, 0 /* release hint */, 1 /* tag */);

        mp              = vl_msg_api_alloc_as_if_client(sizeof(*mp) + (n_items * sizeof(vl_api_ip4_nbr_counter_t)));
         ...
         */
        clib_memcpy(mp->c, &ctx->counters[vec_len(ctx->counters) - n_items], n_items * sizeof(*ctx->counters));
...
do_ip4_nbr_counters(stats_main_t *sm)
{
    ...
    pool_foreach(si, im->sw_interfaces, ({
                      ...
                      */
                     dslock(sm, 0 /* release hint */, 1 /* tag */);
                     adj_nbr_walk(si->sw_if_index, FIB_PROTOCOL_IP4, ip4_nbr_stats_cb, &ctx);
                     dsunlock(sm);
...
static adj_walk_rc_t
ip6_nbr_stats_cb(adj_index_t ai, void *arg)
{
...
static void
ip6_nbr_ship(stats_main_t *sm, ip6_nbr_stats_ctx_t *ctx)
{
    api_main_t *am                     = sm->api_main;
    vl_shmem_hdr_t *shmem_hdr          = am->shmem_hdr;
    svm_queue_t *q                     = shmem_hdr->vl_input_queue;
     ...
     */
    while (0 != vec_len(ctx->counters)) {
        u32 n_items = MIN(vec_len(ctx->counters), IP6_FIB_COUNTER_BATCH_SIZE);
        u8 pause    = 0;

        dslock(sm, 0 /* release hint */, 1 /* tag */);

        mp              = vl_msg_api_alloc_as_if_client(sizeof(*mp) + (n_items * sizeof(vl_api_ip6_nbr_counter_t)));
...
do_ip6_nbr_counters(stats_main_t *sm)
{
    ...
    pool_foreach(si, im->sw_interfaces, ({
                      ...
                      */
                     dslock(sm, 0 /* release hint */, 1 /* tag */);
                     adj_nbr_walk(si->sw_if_index, FIB_PROTOCOL_IP6, ip6_nbr_stats_cb, &ctx);
                     dsunlock(sm);
...
again:
    ...
    /* *INDENT-OFF* */
    pool_foreach(fib, im4->fibs, ({ vec_add1(do_fibs->fibs, fib); }));
    ...
    for (j = 0; j < vec_len(do_fibs->fibs); j++) {
        ...
        if (mp == 0) {
            ...
            mp = vl_msg_api_alloc_as_if_client(sizeof(*mp) + items_this_message * sizeof(vl_api_ip4_fib_counter_t));
        ...
        for (i = 0; i < ARRAY_LEN(v4_fib->fib_entry_by_dst_address); i++) {
            ...
            hash_foreach_pair(p, hash, ({ vec_add1(do_fibs->pvec, p); }));
        ...
        vec_foreach(r, do_fibs->ip4routes)
        {
            ...
            vlib_get_combined_counter(&load_balance_main.lbm_to_counters, index, &c);
            /*
             * If it has actually
...
do_ip4_mfib_counters(stats_main_t *sm)
{
    ...
    for (j = 0; j < vec_len(do_fibs->mfibs); j++) {
        ...
        if (mp == 0) {
            ...
            mp = vl_msg_api_alloc_as_if_client(sizeof(*mp) + items_this_message * sizeof(vl_api_ip4_mfib_counter_t));
        ...
        mfib_table_walk(mfib->mft_index, FIB_PROTOCOL_IP4, mfib_table_stats_walk_cb, sm);
        dsunlock(sm);
        ...
        vec_foreach(pfx, do_fibs->mroutes)
        {
            ...
            vlib_get_combined_counter(&replicate_main.repm_counters, dpo_id->dpoi_index, &c);
            /*
...
do_ip6_mfib_counters(stats_main_t *sm)
{
    ...
    for (j = 0; j < vec_len(do_fibs->mfibs); j++) {
        ...
        if (mp == 0) {
            ...
            mp = vl_msg_api_alloc_as_if_client(sizeof(*mp) + items_this_message * sizeof(vl_api_ip6_mfib_counter_t));
        ...
        mfib_table_walk(mfib->mft_index, FIB_PROTOCOL_IP6, mfib_table_stats_walk_cb, sm);
        dsunlock(sm);
...
again:
    vec_reset_length(do_fibs->fibs);
    /* *INDENT-OFF* */
    pool_foreach(fib, im6->fibs, ({ vec_add1(do_fibs->fibs, fib); }));
    /* *INDENT-ON* */
    ...
    for (i = 0; i < vec_len(do_fibs->fibs); i++) {
        ...
        if (mp == 0) {
            ...
            mp = vl_msg_api_alloc_as_if_client(sizeof(*mp) + items_this_message * sizeof(vl_api_ip6_fib_counter_t));
        ...
        vec_foreach(r, do_fibs->ip6routes)
        {
            ...
            vlib_get_combined_counter(&load_balance_main.lbm_to_counters, r->index, &c);
            /*
             * If it has actually
             * seen at least one packet, send it.
             */
...
udp_encap_ship(udp_encap_stats_walk_t *ctx)
{
    ...
    while (0 != vec_len(ctx->stats)) {
        u32 n_items = MIN(vec_len(ctx->stats), UDP_ENCAP_COUNTER_BATCH_SIZE);
        u8 pause    = 0;

        dslock(sm, 0 /* release hint */, 1 /* tag */);

        mp             = vl_msg_api_alloc_as_if_client(sizeof(*mp) + (n_items * sizeof(vl_api_udp_encap_counter_t)));
         ...
         * The order we push the stats to the caller is not important.
         */
        clib_memcpy(mp->c, &ctx->stats[vec_len(ctx->stats) - n_items], n_items * sizeof(*ctx->stats));
...
bier_neighbor_ship(bier_neighbor_stats_walk_t *ctx)
{
     ...
     */
    while (0 != vec_len(ctx->stats)) {
        u32 n_items = MIN(vec_len(ctx->stats), BIER_NEIGHBOR_COUNTER_BATCH_SIZE);
        u8 pause    = 0;

        dslock(sm, 0 /* release hint */, 1 /* tag */);

        mp = vl_msg_api_alloc_as_if_client(sizeof(*mp) + (n_items * sizeof(vl_api_bier_neighbor_counter_t)));
...
stats_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "interval %u", &sec)) {
            ...
            if (rv) {
                return clib_error_return(0, "`stats_set_poller_delay' API call failed, rv=%d:%U", (int) rv,
...
static void
vl_api_stats_get_poller_delay_t_handler(vl_api_stats_get_poller_delay_t *mp)
{
    stats_main_t *sm = &stats_main;
    vl_api_registration_t *reg;
    reg = vl_api_client_index_to_registration(mp->client_index);
...
stats_thread_fn(void *arg)
{
    ...
    if (vec_len(tm->thread_prefix))
        vlib_set_thread_name((char *) format(0, "%v_stats%c", tm->thread_prefix, '\0'));

    while (1) {
        ip46_fib_stats_delay(sm, sm->stats_poll_interval_in_seconds, 0 /* nsec */);
...
vl_api_vnet_interface_simple_counters_t_handler(vl_api_vnet_interface_simple_counters_t *mp)
{
    ...
    clients = get_clients_for_stat(IDX_PER_INTERFACE_SIMPLE_COUNTERS, ~0 /*flag for all */);
    ...
    for (i = 0; i < vec_len(clients); i++) {
        ...
        if (reg) {
            ...
            sm->enable_poller = clear_client_for_stat(IDX_PER_INTERFACE_SIMPLE_COUNTERS, ~0, client.client_index);
            continue;
        }
    }
    vec_free(clients);
...
vl_api_vnet_ip4_fib_counters_t_handler(vl_api_vnet_ip4_fib_counters_t *mp)
{
    ...
    mp_size = sizeof(*mp_copy) + ntohl(mp->count) * sizeof(vl_api_ip4_fib_counter_t);

    clients = get_clients_for_stat(IDX_IP4_FIB_COUNTERS, ~0 /*flag for all */);
    ...
    for (i = 0; i < vec_len(clients); i++) {
        ...
        if (reg) {
            ...
            sm->enable_poller = clear_client_for_stat(IDX_IP4_FIB_COUNTERS, ~0, client.client_index);
            continue;
        }
    }
    vec_free(clients);
...
vl_api_vnet_ip4_nbr_counters_t_handler(vl_api_vnet_ip4_nbr_counters_t *mp)
{
    ...
    mp_size = sizeof(*mp_copy) + ntohl(mp->count) * sizeof(vl_api_ip4_nbr_counter_t);

    clients = get_clients_for_stat(IDX_IP4_NBR_COUNTERS, ~0 /*flag for all */);
    ...
    for (i = 0; i < vec_len(clients); i++) {
        ...
        if (reg) {
            ...
            sm->enable_poller = clear_client_for_stat(IDX_IP4_NBR_COUNTERS, ~0, client.client_index);
            continue;
        }
    }
    vec_free(clients);
...
vl_api_vnet_ip6_fib_counters_t_handler(vl_api_vnet_ip6_fib_counters_t *mp)
{
    ...
    mp_size = sizeof(*mp_copy) + ntohl(mp->count) * sizeof(vl_api_ip6_fib_counter_t);

    clients = get_clients_for_stat(IDX_IP6_FIB_COUNTERS, ~0 /*flag for all */);
    ...
    for (i = 0; i < vec_len(clients); i++) {
        ...
        if (reg) {
            ...
            sm->enable_poller = clear_client_for_stat(IDX_IP6_FIB_COUNTERS, ~0, client.client_index);
            continue;
        }
    }
    vec_free(clients);
...
vl_api_vnet_ip6_nbr_counters_t_handler(vl_api_vnet_ip6_nbr_counters_t *mp)
{
    ...
    mp_size = sizeof(*mp_copy) + ntohl(mp->count) * sizeof(vl_api_ip6_nbr_counter_t);

    clients = get_clients_for_stat(IDX_IP6_NBR_COUNTERS, ~0 /*flag for all */);
    ...
    for (i = 0; i < vec_len(clients); i++) {
        ...
        if (reg) {
            ...
            sm->enable_poller = clear_client_for_stat(IDX_IP6_NBR_COUNTERS, ~0, client.client_index);
            continue;
        }
    }
    vec_free(clients);
...
reply:
    ...
    if (!reg) {
        sm->enable_poller = clear_client_for_stat(IDX_UDP_ENCAP_COUNTERS, fib, mp->client_index);
        return;
    }
...
static void
vl_api_want_bier_neighbor_stats_t_handler(vl_api_want_bier_neighbor_stats_t *mp)
{
    stats_main_t *sm = &stats_main;
    vpe_client_registration_t rp;
    ...
    handle_client_registration(&rp, IDX_BIER_NEIGHBOR_COUNTERS, fib, mp->enable);

reply:
    reg = vl_api_client_index_to_registration(mp->client_index);

    if (!reg) {
        sm->enable_poller = clear_client_for_stat(IDX_BIER_NEIGHBOR_COUNTERS, fib, mp->client_index);
        return;
    }
...
vl_api_want_stats_t_handler(vl_api_want_stats_t *mp)
{
    ...
    handle_client_registration(&rp, IDX_PER_INTERFACE_SIMPLE_COUNTERS, item, mp->enable_disable);

    handle_client_registration(&rp, IDX_PER_INTERFACE_COMBINED_COUNTERS, item, mp->enable_disable);

    handle_client_registration(&rp, IDX_IP4_FIB_COUNTERS, item, mp->enable_disable);

    handle_client_registration(&rp, IDX_IP4_NBR_COUNTERS, item, mp->enable_disable);

    handle_client_registration(&rp, IDX_IP6_FIB_COUNTERS, item, mp->enable_disable);

    handle_client_registration(&rp, IDX_IP6_NBR_COUNTERS, item, mp->enable_disable);
...
vl_api_want_interface_simple_stats_t_handler(vl_api_want_interface_simple_stats_t *mp)
{
    ...
    handle_client_registration(&rp, IDX_PER_INTERFACE_SIMPLE_COUNTERS, swif, mp->enable_disable);
...
vl_api_want_ip4_fib_stats_t_handler(vl_api_want_ip4_fib_stats_t *mp)
{
    ...
    handle_client_registration(&rp, IDX_IP4_FIB_COUNTERS, fib, mp->enable_disable);

reply:
    reg = vl_api_client_index_to_registration(mp->client_index);

    if (!reg) {
        sm->enable_poller = clear_client_for_stat(IDX_IP4_FIB_COUNTERS, fib, mp->client_index);
        return;
    }
...
vl_api_want_ip4_mfib_stats_t_handler(vl_api_want_ip4_mfib_stats_t *mp)
{
    ...
    handle_client_registration(&rp, IDX_IP4_MFIB_COUNTERS, mfib, mp->enable_disable);

reply:
    reg = vl_api_client_index_to_registration(mp->client_index);
    if (!reg) {
        sm->enable_poller = clear_client_for_stat(IDX_IP4_MFIB_COUNTERS, mfib, mp->client_index);
        return;
    }
...
vl_api_want_ip6_fib_stats_t_handler(vl_api_want_ip6_fib_stats_t *mp)
{
    ...
    handle_client_registration(&rp, IDX_IP6_FIB_COUNTERS, fib, mp->enable_disable);

reply:
    reg = vl_api_client_index_to_registration(mp->client_index);
    if (!reg) {
        sm->enable_poller = clear_client_for_stat(IDX_IP6_FIB_COUNTERS, fib, mp->client_index);
        return;
    }
...
vl_api_want_ip6_mfib_stats_t_handler(vl_api_want_ip6_mfib_stats_t *mp)
{
    ...
    handle_client_registration(&rp, IDX_IP6_MFIB_COUNTERS, mfib, mp->enable_disable);

reply:
    reg = vl_api_client_index_to_registration(mp->client_index);
    if (!reg) {
        sm->enable_poller = clear_client_for_stat(IDX_IP6_MFIB_COUNTERS, mfib, mp->client_index);
        return;
    }
...
vl_api_vnet_get_summary_stats_t_handler(vl_api_vnet_get_summary_stats_t *mp)
{
    ...
    rmp->vector_rate = clib_host_to_net_u64(vlib_last_vector_length_per_node(sm->vlib_main));
...
stats_init(vlib_main_t *vm)
{
    ...
    sm->data_structure_lock            = clib_mem_alloc_aligned(sizeof(data_structure_lock_t), CLIB_CACHE_LINE_BYTES);
