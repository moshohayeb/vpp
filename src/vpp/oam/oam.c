...
init_oam_packet_template(oam_main_t *om, oam_target_t *t)
{
    ...
    sum  = ip_incremental_checksum(0, &h->icmp, sizeof(h->icmp) + sizeof(h->id) + sizeof(h->seq) + sizeof(h->data));
    csum = ~ip_csum_fold(sum);
    h->icmp.checksum = csum;
}

int
vpe_oam_add_del_target(ip4_address_t *src_address, ip4_address_t *dst_address, u32 fib_id, int is_add)
{
    u64 key;
    uword *p;
...
static clib_error_t *
oam_add_del_target_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    int is_add = -1;
        ...
        pool_foreach(t, om->targets, ({
                         ...
                         if ((t->seq - t->last_heard_seq) >= om->misses_allowed) {
                             if (t->state == OAM_STATE_ALIVE) {
                                 if (CLIB_DEBUG > 0)
                                     clib_warning("oam target %U now DEAD", format_ip4_address, &t->dst_address);
                                 t->state = OAM_STATE_DEAD;
                                 send_oam_event(t);
                             }
                         } else {
                             if (t->state == OAM_STATE_DEAD) {
                                 if (CLIB_DEBUG > 0)
                                     clib_warning("oam target %U now ALIVE", format_ip4_address, &t->dst_address);
                         ...
                         sum0              = ip_csum_update(sum0, 0 /* old seq */, new_seq, oam_template_t, seq);
                         h0->seq           = new_seq;
...
format_oam_target(u8 *s, va_list *args)
{
    ...
    if (t == 0)
        return format(s, "%=6s%=14s%=14s%=12s%=10s", "Fib", "Src", "Dst", "Last Heard", "State");

    s = format(s, "%=6d%=14U%=14U%=12.2f%=10s", t->fib_id, format_ip4_address, &t->src_address, format_ip4_address,
               &t->dst_address, t->last_heard_time, (t->state == OAM_STATE_ALIVE) ? "alive" : "dead");
    if (verbose)
...
static clib_error_t *
show_oam_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    oam_main_t *om = &oam_main;
    oam_target_t *t;
    int verbose = 0;
    ...
    /* *INDENT-OFF* */
    pool_foreach(t, om->targets, ({ vlib_cli_output(vm, "%U", format_oam_target, t, verbose); }));
    /* *INDENT-ON* */
...
format_swap_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "OAM: rx from address %U, target index %d", format_ip4_address, &t->address, t->target_pool_index);
    return s;
}
...
static uword
oam_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
