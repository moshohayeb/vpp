...
sync_config_and_reply(vcl_test_server_conn_t *conn, vcl_test_cfg_t *rx_cfg)
{
    ...
    (void) vcl_test_write(conn->fd, (uint8_t *) &conn->cfg, sizeof(conn->cfg), NULL, conn->cfg.verbose);
}

static void
vts_server_start_stop(vcl_test_server_worker_t *wrk, vcl_test_server_conn_t *conn, vcl_test_cfg_t *rx_cfg)
{
    ...
    if (rx_cfg->ctrl_handle == conn->fd) {
        for (i = 0; i < wrk->conn_pool_size; i++) {
            ...
            if (conn->cfg.verbose) {
                ...
                vcl_test_stats_dump(buf, &tc->stats, 1 /* show_rx */, is_bi /* show tx */, conn->cfg.verbose);
            }
        }
...
vts_server_rx(vcl_test_server_conn_t *conn, int rx_bytes)
{
    ...
    if (conn->cfg.test == VCL_TEST_TYPE_BI) {
        if (vsm->use_ds) {
            (void) vcl_test_write(client_fd, conn->ds[0].data, conn->ds[0].len, &conn->stats, conn->cfg.verbose);
            if (conn->ds[1].len)
                (void) vcl_test_write(client_fd, conn->ds[1].data, conn->ds[1].len, &conn->stats, conn->cfg.verbose);
        } else
            (void) vcl_test_write(client_fd, conn->buf, rx_bytes, &conn->stats, conn->cfg.verbose);
    }
...
vts_server_echo(vcl_test_server_conn_t *conn, int rx_bytes)
{
    ...
    tx_bytes = vcl_test_write(conn->fd, conn->buf, nbytes, &conn->stats, conn->cfg.verbose);
    if (tx_bytes >= 0)
...
static void
vcl_test_server_process_opts(vcl_test_server_main_t *vsm, int argc, char **argv)
{
    int v, c;
...
int
vts_handle_cfg(vcl_test_server_worker_t *wrk, vcl_test_cfg_t *rx_cfg, vcl_test_server_conn_t *conn, int rx_bytes)
{
    if (rx_cfg->verbose) {
    ...
    if (rx_bytes != sizeof(*rx_cfg)) {
        ...
        vcl_test_write(conn->fd, (uint8_t *) &conn->cfg, sizeof(conn->cfg), NULL, conn->cfg.verbose);
        return -1;
    }
...
vts_worker_init(vcl_test_server_worker_t *wrk)
{
    ...
    wrk->listen_fd = vppcom_session_create(vsm->cfg.proto, 0 /* is_nonblocking */);
    if (wrk->listen_fd < 0)
...
vts_worker_loop(void *arg)
{
    ...
    while (1) {
        num_ev = vppcom_epoll_wait(wrk->epfd, wrk->wait_events, VCL_TEST_CFG_MAX_EPOLL_EVENTS, 60000.0);
        if (num_ev < 0) {
        ...
        for (i = 0; i < num_ev; i++) {
            ...
            read_again:
                ...
                if ((conn->cfg.test == VCL_TEST_TYPE_UNI) || (conn->cfg.test == VCL_TEST_TYPE_BI)) {
                    vts_server_rx(conn, rx_bytes);
                    if (vppcom_session_attr(conn->fd, VPPCOM_ATTR_GET_NREAD, 0, 0) > 0)
                        goto read_again;
                    continue;
                }
