...
{
    int rv = snprintf(ldp->app_name, LDP_APP_NAME_MAX, "ldp-%d-%s", getpid(), app_name);
...
{
    return (ldp_fd_is_sid(fd) ? ((u32) fd & ldp->sid_bit_mask) : INVALID_SESSION_ID);
}
...
ldp_init(void)
{
    ...
    if (env_var_str) {
        ...
        if (sscanf(env_var_str, "%u", &tmp) != 1)
            ...
            ldp->debug = tmp;
            LDBG(0, "LDP<%d>: configured LDP debug level (%u) from env var " LDP_ENV_DEBUG "!", getpid(), ldp->debug);
        }
    }
    ...
    if (env_var_str) {
        ...
        LDBG(0, "LDP<%d>: configured LDP app name (%s) from the env var " LDP_ENV_APP_NAME "!", getpid(), ldp->app_name);
    }
    ...
    if (env_var_str) {
        ...
        if (sscanf(env_var_str, "%u", &sb) != 1) {
            clib_warning("LDP<%d>: WARNING: Invalid LDP sid bit specified in"
                         ...
                         "value %d (0x%x)",
                         getpid(), env_var_str, ldp->sid_bit_val, ldp->sid_bit_val);
            ...
            clib_warning("LDP<%d>: WARNING: LDP sid bit (%u) specified in the"
                         ...
                         getpid(), sb, env_var_str, LDP_SID_BIT_MIN, ldp->sid_bit_val, ldp->sid_bit_val);
            ...
            clib_warning("LDP<%d>: WARNING: LDP sid bit (%u) specified in the"
                         ...
                         getpid(), sb, env_var_str, LDP_SID_BIT_MAX, ldp->sid_bit_val, ldp->sid_bit_val);
        } else {
            ldp->sid_bit_val  = (1 << sb);
            ldp->sid_bit_mask = ldp->sid_bit_val - 1;

            LDBG(0, "LDP<%d>: configured LDP sid bit (%u) from " LDP_ENV_SID_BIT "!  sid bit value %d (0x%x)", getpid(),
...
close(int fd)
{
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        if (epfd > 0) {
            ...
            if (LDP_DEBUG > 0)
                clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): epfd %u (0x%x)", getpid(), fd, fd, func_str, epfd,
                             epfd);
            ...
            if (rv < 0) {
                ...
                (void) vppcom_session_attr(sid, VPPCOM_ATTR_SET_LIBC_EPFD, &epfd, &size);
            }
        ...
        if (LDP_DEBUG > 0)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): sid %u (0x%x)", getpid(), fd, fd, func_str, sid, sid);
        ...
        if (LDP_DEBUG > 0)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s()", getpid(), fd, fd, func_str);
    ...
    if (LDP_DEBUG > 0) {
        if (rv < 0) {
            ...
            clib_warning("LDP<%d>: ERROR: fd %d (0x%x): %s() failed! "
                         "rv %d, errno = %d",
                         getpid(), fd, fd, func_str, rv, errno_val);
            errno = errno_val;
        } else
            clib_warning("LDP<%d>: fd %d (0x%x): returning %d (0x%x)", getpid(), fd, fd, rv, rv);
    }
    return rv;
}
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        if (LDP_DEBUG > 2)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                         "sid %u (0x%x), buf %p, nbytes %u",
                         getpid(), fd, fd, func_str, sid, sid, buf, nbytes);
        ...
        if (LDP_DEBUG > 2)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                         "buf %p, nbytes %u",
                         getpid(), fd, fd, func_str, buf, nbytes);
            ...
            for (i = 0; i < iovcnt; ++i) {
                if (LDP_DEBUG > 2)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s() [%d]: "
                                 ...
                                 getpid(), fd, fd, func_str, i, sid, sid, iov, iovcnt, total);
                    ...
                    if (rv < iov[i].iov_len) {
                        if (LDP_DEBUG > 2)
                            clib_warning("LDP<%d>: fd %d (0x%x): "
                                         ...
                                         getpid(), fd, fd, rv, i, iov[i].iov_len);
                        break;
                    }
                }
            }
    ...
    if (sid != INVALID_SESSION_ID) {
            ...
            for (i = 0; i < iovcnt; ++i) {
                if (LDP_DEBUG > 4)
                    printf("%s:%d: LDP<%d>: fd %d (0x%x): calling %s() [%d]: "
                           ...
                           __func__, __LINE__, getpid(), fd, fd, func_str, i, sid, sid, iov[i].iov_base, iov[i].iov_len,
                           total);

                rv = vppcom_session_write(sid, iov[i].iov_base, iov[i].iov_len);
                if (rv < 0)
                    break;
                else {
                    total += rv;
                    if (rv < iov[i].iov_len) {
                        if (LDP_DEBUG > 4)
                            printf("%s:%d: LDP<%d>: fd %d (0x%x): "
                                   ...
                                   __func__, __LINE__, getpid(), fd, fd, rv, i, iov[i].iov_len);
                        break;
                    }
                }
            }
        ...
        if (LDP_DEBUG > 4)
            printf("%s:%d: LDP<%d>: fd %d (0x%x): calling %s(): "
                   "iov %p, iovcnt %d\n",
                   __func__, __LINE__, getpid(), fd, fd, func_str, iov, iovcnt);
    ...
    if (LDP_DEBUG > 4) {
        if (size < 0) {
            ...
            fprintf(stderr,
                    ...
                    "rv %ld, errno = %d\n",
                    __func__, __LINE__, getpid(), fd, fd, func_str, size, errno_val);
            errno = errno_val;
        } else
...
fcntl(int fd, int cmd, ...)
{
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        case F_SETFL:
            ...
            if (LDP_DEBUG > 2)
                clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                             ...
                             getpid(), fd, fd, func_str, sid, sid, flags, flags, size);

            rv = vppcom_session_attr(sid, VPPCOM_ATTR_SET_FLAGS, &flags, &size);
            break;
        ...
        case F_GETFL:
            ...
            rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_FLAGS, &flags, &size);
            if (rv == VPPCOM_OK) {
                if (LDP_DEBUG > 2)
        ...
        if (LDP_DEBUG > 2)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): cmd %d", getpid(), fd, fd, func_str, cmd);
...
int
ldp_pselect(int nfds, fd_set *__restrict readfds, fd_set *__restrict writefds, fd_set *__restrict exceptfds,
            const struct timespec *__restrict timeout, const __sigset_t *__restrict sigmask)
{
    int rv;
    char *func_str = "##";
    f64 time_out;
    ...
    if (timeout) {
        time_out = (timeout->tv_sec == 0 && timeout->tv_nsec == 0) ?
                       (f64) 0 :
                       (f64) timeout->tv_sec + (f64) timeout->tv_nsec / (f64) 1000000000;

        /* select as fine grained sleep */
        if (!nfds) {
            if (LDP_DEBUG > 3)
                clib_warning("LDP<%d>: sleeping for %.02f seconds", getpid(), time_out);
    ...
    if (nfds <= ldp->sid_bit_val) {
        ...
        if (LDP_DEBUG > 3)
            clib_warning("LDP<%d>: calling %s(): nfds %d, readfds %p, writefds %p, "
                         "exceptfds %p, timeout %p, sigmask %p",
                         getpid(), func_str, nfds, readfds, writefds, exceptfds, timeout, sigmask);
    ...
    if (PREDICT_FALSE(ldp->sid_bit_val > FD_SETSIZE / 2)) {
        clib_warning("LDP<%d>: ERROR: LDP sid bit value %d (0x%x) > "
                     "FD_SETSIZE/2 %d (0x%x)!",
                     getpid(), ldp->sid_bit_val, ldp->sid_bit_val, FD_SETSIZE / 2, FD_SETSIZE / 2);
        errno = EOVERFLOW;
        return -1;
    }
    ...
    if (readfds) {
        ...
        clib_memcpy(ldp->rd_bitmap, readfds, vec_len(ldp->rd_bitmap) * sizeof(clib_bitmap_t));
        FD_ZERO(readfds);
        ...
        clib_bitmap_foreach(fd, ldp->rd_bitmap, ({
                                ...
                                if (LDP_DEBUG > 3)
                                    clib_warning("LDP<%d>: readfds: fd %d (0x%x), sid %u (0x%x)", getpid(), fd, fd, sid,
                                                 sid);
                                if (sid == INVALID_SESSION_ID)
        ...
        if (LDP_DEBUG > 3)
            clib_warning("LDP<%d>: readfds: sid_bits_set %d, sid_bits %d, "
                         ...
                         getpid(), sid_bits_set, sid_bits, libc_bits_set, libc_bits);
    }
    if (writefds) {
        ...
        clib_memcpy(ldp->wr_bitmap, writefds, vec_len(ldp->wr_bitmap) * sizeof(clib_bitmap_t));
        FD_ZERO(writefds);
        ...
        clib_bitmap_foreach(fd, ldp->wr_bitmap, ({
                                sid = ldp_sid_from_fd(fd);
                                if (LDP_DEBUG > 3)
                                    clib_warning("LDP<%d>: writefds: fd %d (0x%x), sid %u (0x%x)", getpid(), fd, fd,
                                                 sid, sid);
                                if (sid == INVALID_SESSION_ID)
    ...
    if (exceptfds) {
        ...
        clib_memcpy(ldp->ex_bitmap, exceptfds, vec_len(ldp->ex_bitmap) * sizeof(clib_bitmap_t));
        FD_ZERO(exceptfds);
        ...
        clib_bitmap_foreach(fd, ldp->ex_bitmap, ({
                                sid = ldp_sid_from_fd(fd);
                                if (LDP_DEBUG > 3)
                                    clib_warning("LDP<%d>: exceptfds: fd %d (0x%x), sid %u (0x%x)", getpid(), fd, fd,
                                                 sid, sid);
                                if (sid == INVALID_SESSION_ID)
    ...
    do {
        if (sid_bits) {
            if (!ldp->select_vcl) {
                ...
                if (readfds)
                    clib_memcpy(ldp->rd_bitmap, ldp->sid_rd_bitmap, vec_len(ldp->rd_bitmap) * sizeof(clib_bitmap_t));
                if (writefds)
                    clib_memcpy(ldp->wr_bitmap, ldp->sid_wr_bitmap, vec_len(ldp->wr_bitmap) * sizeof(clib_bitmap_t));
                if (exceptfds)
                    clib_memcpy(ldp->ex_bitmap, ldp->sid_ex_bitmap, vec_len(ldp->ex_bitmap) * sizeof(clib_bitmap_t));

                rv = vppcom_select(sid_bits, readfds ? ldp->rd_bitmap : NULL, writefds ? ldp->wr_bitmap : NULL,
        ...
        if (libc_bits) {
            ...
            if (readfds)
                clib_memcpy(readfds, ldp->libc_rd_bitmap, vec_len(ldp->rd_bitmap) * sizeof(clib_bitmap_t));
            if (writefds)
                clib_memcpy(writefds, ldp->libc_wr_bitmap, vec_len(ldp->wr_bitmap) * sizeof(clib_bitmap_t));
            if (exceptfds)
                clib_memcpy(exceptfds, ldp->libc_ex_bitmap, vec_len(ldp->ex_bitmap) * sizeof(clib_bitmap_t));
            tspec.tv_sec = tspec.tv_nsec = 0;
            rv                           = libc_pselect(libc_bits, readfds ? readfds : NULL, writefds ? writefds : NULL,
...
int
select(int nfds, fd_set *__restrict readfds, fd_set *__restrict writefds, fd_set *__restrict exceptfds,
...
#ifdef __USE_XOPEN2K
int
pselect(int nfds, fd_set *__restrict readfds, fd_set *__restrict writefds, fd_set *__restrict exceptfds,
        const struct timespec *__restrict timeout, const __sigset_t *__restrict sigmask)
{
...
socket(int domain, int type, int protocol)
{
    ...
    if (((domain == AF_INET) || (domain == AF_INET6)) && ((sock_type == SOCK_STREAM) || (sock_type == SOCK_DGRAM))) {
        int sid;
        u8 proto = ((sock_type == SOCK_DGRAM) ? VPPCOM_PROTO_UDP : VPPCOM_PROTO_TCP);

        func_str = "vppcom_session_create";

        LDBG(0, "LDP<%d>: : calling %s(): proto %u (%s), is_nonblocking %u", getpid(), func_str, proto,
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        case AF_INET:
            if (len != sizeof(struct sockaddr_in)) {
                clib_warning("LDP<%d>: ERROR: fd %d (0x%x): sid %u (0x%x): Invalid "
                             "AF_INET addr len %u!",
                             getpid(), fd, fd, sid, sid, len);
                errno = EINVAL;
                rv    = -1;
                goto done;
            }
...
static inline int
ldp_copy_ep_to_sockaddr(__SOCKADDR_ARG addr, socklen_t *__restrict len, vppcom_endpt_t *ep)
{
    int rv = 0;
    int sa_len, copy_len;
    ...
    if (addr && len && ep) {
        ...
        case AF_INET6:
            ...
            copy_len = *len - sa_len;
            if (copy_len > 0)
                memcpy(((struct sockaddr_in6 *) addr)->sin6_addr.__in6_u.__u6_addr8, ep->ip, copy_len);
            break;
...
connect(int fd, __CONST_SOCKADDR_ARG addr, socklen_t len)
{
    ...
    if (!addr) {
        clib_warning("LDP<%d>: ERROR: fd %d (0x%x): NULL addr, len %u", getpid(), fd, fd, len);
        errno = EINVAL;
        rv    = -1;
        goto done;
    }
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        case AF_INET:
            if (len != sizeof(struct sockaddr_in)) {
                clib_warning("LDP<%d>: fd %d (0x%x): ERROR sid %u (0x%x): Invalid "
                             "AF_INET addr len %u!",
                             getpid(), fd, fd, sid, sid, len);
                errno = EINVAL;
                rv    = -1;
...
sendfile(int out_fd, int in_fd, off_t *offset, size_t len)
{
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        func_str = "vppcom_session_attr[GET_FLAGS]";
        rv       = vppcom_session_attr(sid, VPPCOM_ATTR_GET_FLAGS, &flags, &flags_len);
        if (PREDICT_FALSE(rv != VPPCOM_OK)) {
            clib_warning("LDP<%d>: ERROR: out fd %d (0x%x): %s(): "
                         "sid %u (0x%x), returned %d (%s)!",
                         getpid(), out_fd, out_fd, func_str, sid, sid, rv, vppcom_retval_str(rv));
        ...
        if (offset) {
            ...
            if (PREDICT_FALSE(off == -1)) {
                ...
                clib_warning("LDP<%d>: ERROR: out fd %d (0x%x): %s(): "
                             ...
                             "*offset %ld, rv %ld, errno %d",
                             getpid(), out_fd, out_fd, in_fd, offset, *offset, off, errno_val);
                errno = errno_val;
                size  = -1;
                goto done;
            }
        ...
        do {
            ...
            if (size < 0) {
                clib_warning("LDP<%d>: ERROR: fd %d (0x%x): %s(): sid %u (0x%x), "
                             "returned %d (%s)!",
                             getpid(), out_fd, out_fd, func_str, sid, sid, size, vppcom_retval_str(size));
                vec_reset_length(ldp->io_buffer);
                errno = -size;
                size  = -1;
                goto done;
            ...
            bytes_to_read = size;
            if (LDP_DEBUG > 2)
                clib_warning("LDP<%d>: fd %d (0x%x): called %s(): sid %u (0x%x), "
                             "results %ld, n_bytes_left %lu, bytes_to_read %lu",
                             getpid(), out_fd, out_fd, func_str, sid, sid, results, n_bytes_left, bytes_to_read);
            ...
            if (nbytes < 0) {
                ...
                clib_warning("LDP<%d>: ERROR: fd %d (0x%x): %s(): in_fd (%d), "
                             ...
                             "errno %d",
                             getpid(), out_fd, out_fd, func_str, in_fd, ldp->io_buffer, bytes_to_read, nbytes, errno_val);
                errno = errno_val;
            ...
            if (LDP_DEBUG > 2)
                clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): sid %u (0x%x), "
                             "buf %p, nbytes %u: results %d, n_bytes_left %d",
                             getpid(), out_fd, out_fd, func_str, sid, sid, ldp->io_buffer, nbytes, results, n_bytes_left);
            ...
            if (size < 0) {
                if (size == VPPCOM_EAGAIN) {
                    ...
                    clib_warning("LDP<%d>: ERROR: fd %d (0x%x): %s():"
                                 ...
                                 "returned %d (%s)",
                                 getpid(), out_fd, out_fd, func_str, sid, ldp->io_buffer, nbytes, size,
                                 vppcom_retval_str(size));
                }
                if (results == 0) {
                    vec_reset_length(ldp->io_buffer);
                    errno = -size;
    ...
    update_offset:
        ...
        if (offset) {
            ...
            if (PREDICT_FALSE(off == -1)) {
                ...
                clib_warning("LDP<%d>: ERROR: %s(): SEEK_SET failed: "
                             ...
                             "rv %ld, errno %d",
                             getpid(), in_fd, offset, *offset, off, errno_val);
                errno = errno_val;
                size  = -1;
                goto done;
            }
        ...
        if (LDP_DEBUG > 2)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                         ...
                         getpid(), out_fd, out_fd, func_str, in_fd, offset, len);
    ...
    if (LDP_DEBUG > 2) {
        if (size < 0) {
            ...
            clib_warning("LDP<%d>: ERROR: fd %d (0x%x): %s() failed! "
                         "rv %d, errno = %d",
                         getpid(), out_fd, out_fd, func_str, size, errno_val);
            errno = errno_val;
        } else
            clib_warning("LDP<%d>: fd %d (0x%x): returning %d (0x%x)", getpid(), out_fd, out_fd, size, size);
    }
    return size;
}
...
recv(int fd, void *buf, size_t n, int flags)
{
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        if (LDP_DEBUG > 2)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                         "sid %u (0x%x), buf %p, n %u, flags 0x%x",
                         getpid(), fd, fd, func_str, sid, sid, buf, n, flags);
...
ssize_t
sendto(int fd, const void *buf, size_t n, int flags, __CONST_SOCKADDR_ARG addr, socklen_t addr_len)
{
    ssize_t size;
    const char *func_str = __func__;
    u32 sid              = ldp_sid_from_fd(fd);
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        if (LDP_DEBUG > 2)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                         ...
                         getpid(), fd, fd, func_str, sid, sid, buf, n, flags, ep);
        ...
        if (LDP_DEBUG > 2)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                         ...
                         getpid(), fd, fd, func_str, buf, n, flags, addr, addr_len);
...
#ifdef USE_GNU
int
recvmmsg(int fd, struct mmsghdr *vmessages, unsigned int vlen, int flags, struct timespec *tmo)
{
    ssize_t size;
    const char *func_str;
    u32 sid = ldp_sid_from_fd(fd);
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        if (LDP_DEBUG > 2)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                         ...
                         getpid(), fd, fd, func_str, vmessages, vlen, flags, tmo);
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        case SOL_TCP:
            ...
            case TCP_NODELAY:
                ...
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_TCP_NODELAY, optval, optlen);
                break;
            case TCP_MAXSEG:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_TCP_USER_MSS, optval, optlen);
                break;
            case TCP_KEEPIDLE:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_TCP_KEEPIDLE, optval, optlen);
                break;
            case TCP_KEEPINTVL:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x), SOL_TCP",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_TCP_KEEPINTVL, optval, optlen);
                break;
            case TCP_INFO:
                if (optval && optlen && (*optlen == sizeof(struct tcp_info))) {
                    if (LDP_DEBUG > 1)
                        clib_warning("LDP<%d>: fd %d (0x%x): sid %u (0x%x), "
                                     ...
                                     "optlen %d: #LDP-NOP#",
                                     getpid(), fd, fd, sid, sid, optval, *optlen);
            ...
            case IPV6_V6ONLY:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_V6ONLY, optval, optlen);
                break;
            default:
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: ERROR: fd %d (0x%x): %s(): "
        ...
        case SOL_SOCKET:
            ...
            case SO_ACCEPTCONN:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_LISTEN, optval, optlen);
                break;
            case SO_KEEPALIVE:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_KEEPALIVE, optval, optlen);
                break;
            case SO_PROTOCOL:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv              = vppcom_session_attr(sid, VPPCOM_ATTR_GET_PROTOCOL, optval, optlen);
            ...
            case SO_SNDBUF:
                ...
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_TX_FIFO_LEN, optval, optlen);
                break;
            case SO_RCVBUF:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x), optlen %d",
                                 getpid(), fd, fd, func_str, sid, sid, buflen);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_RX_FIFO_LEN, optval, optlen);
                break;
            case SO_REUSEADDR:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_REUSEADDR, optval, optlen);
                break;
            case SO_BROADCAST:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_BROADCAST, optval, optlen);
                break;
            case SO_ERROR:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_GET_ERROR, optval, optlen);
                break;
            default:
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: ERROR: fd %d (0x%x): %s(): "
        ...
        if (LDP_DEBUG > 1)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): level %d, "
                         ...
                         getpid(), fd, fd, func_str, level, optname, optval, optlen);
            ...
            case TCP_NODELAY:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_SET_TCP_NODELAY, (void *) optval, &optlen);
                break;
            case TCP_MAXSEG:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_SET_TCP_USER_MSS, (void *) optval, &optlen);
                break;
            case TCP_KEEPIDLE:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_SET_TCP_KEEPIDLE, (void *) optval, &optlen);
                break;
            case TCP_KEEPINTVL:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x), SOL_TCP",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_SET_TCP_KEEPINTVL, (void *) optval, &optlen);
                break;
            default:
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: ERROR: fd %d (0x%x): %s(): "
            ...
            case IPV6_V6ONLY:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_SET_V6ONLY, (void *) optval, &optlen);
                break;
            default:
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: ERROR: fd %d (0x%x): %s(): "
            ...
            case SO_KEEPALIVE:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_SET_KEEPALIVE, (void *) optval, &optlen);
                break;
            case SO_REUSEADDR:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_SET_REUSEADDR, (void *) optval, &optlen);
                break;
            case SO_BROADCAST:
                ...
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): "
                                 "sid %u (0x%x)",
                                 getpid(), fd, fd, func_str, sid, sid);
                rv = vppcom_session_attr(sid, VPPCOM_ATTR_SET_BROADCAST, (void *) optval, &optlen);
                break;
            default:
                if (LDP_DEBUG > 1)
                    clib_warning("LDP<%d>: ERROR: fd %d (0x%x): %s(): "
...
listen(int fd, int n)
{
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        LDBG(0, "LDP<%d>: fd %d (0x%x): calling %s(): sid %u (0x%x), n %d", getpid(), fd, fd, func_str, sid, sid, n);
        ...
        LDBG(0, "LDP<%d>: fd %d (0x%x): calling %s(): n %d", getpid(), fd, fd, func_str, n);
...
static inline int
ldp_accept4(int listen_fd, __SOCKADDR_ARG addr, socklen_t *__restrict addr_len, int flags)
{
    int rv;
    const char *func_str;
    ...
    if (listen_sid != INVALID_SESSION_ID) {
        ...
        if (LDP_DEBUG > 0)
            clib_warning("LDP<%d>: listen fd %d (0x%x): calling %s(): "
                         ...
                         getpid(), listen_fd, listen_fd, func_str, listen_sid, listen_sid, ep, flags);
            ...
            if (rv != VPPCOM_OK) {
                ...
                if (LDP_DEBUG > 0)
                    clib_warning("LDP<%d>: listen fd %d (0x%x): calling %s(): "
                                 ...
                                 getpid(), listen_fd, listen_fd, func_str, accept_sid, accept_sid, ep, flags);
        ...
        if (LDP_DEBUG > 0)
            clib_warning("LDP<%d>: listen fd %d (0x%x): calling %s(): "
                         ...
                         getpid(), listen_fd, listen_fd, func_str, addr, addr_len, flags);
    ...
    if (LDP_DEBUG > 0) {
        if (rv < 0) {
            ...
            clib_warning("LDP<%d>: ERROR: listen fd %d (0x%x): %s() failed! "
                         "rv %d, errno = %d",
                         getpid(), listen_fd, listen_fd, func_str, rv, errno_val);
            errno = errno_val;
        } else
            clib_warning("LDP<%d>: listen fd %d (0x%x): returning %d (0x%x)", getpid(), listen_fd, listen_fd, rv, rv);
    }
    return rv;
}

int
accept4(int fd, __SOCKADDR_ARG addr, socklen_t *__restrict addr_len, int flags)
{
...
shutdown(int fd, int how)
{
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        if (LDP_DEBUG > 1)
            clib_warning("LDP<%d>: fd %d (0x%x): calling %s(): how %d", getpid(), fd, fd, func_str, how);
...
epoll_ctl(int epfd, int op, int fd, struct epoll_event *event)
{
    ...
    if (PREDICT_FALSE(vep_idx == INVALID_SESSION_ID)) {
        ...
        LDBG(1,
             "LDP<%d>: epfd %d (0x%x): calling %s(): op %d, fd %d (0x%x),"
             " event %p",
    ...
    LDBG(0, "LDP<%d>: epfd %d (0x%x), vep_idx %d (0x%x), sid %d (0x%x)", getpid(), epfd, epfd, vep_idx, vep_idx, sid,
         sid);
    ...
    if (sid != INVALID_SESSION_ID) {
        ...
        LDBG(1,
             "LDP<%d>: epfd %d (0x%x): calling %s(): vep_idx %d (0x%x),"
             " op %d, sid %u (0x%x), event %p",
             getpid(), epfd, epfd, func_str, vep_idx, vep_idx, sid, sid, event);
        ...
        LDBG(1,
             "LDP<%d>: epfd %d (0x%x), vep_idx %d (0x%x): %s() "
             "returned libc_epfd %d (0x%x)",
             getpid(), epfd, epfd, vep_idx, vep_idx, func_str, libc_epfd, libc_epfd);

        if (!libc_epfd) {
            func_str = "libc_epoll_create1";

            LDBG(1,
                 "LDP<%d>: epfd %d (0x%x), vep_idx %d (0x%x): "
                 "calling %s(): EPOLL_CLOEXEC",
                 getpid(), epfd, epfd, vep_idx, vep_idx, func_str);
            ...
            LDBG(1,
                 ...
                 getpid(), epfd, epfd, func_str, vep_idx, vep_idx, libc_epfd, libc_epfd, size);

            rv = vppcom_session_attr(vep_idx, VPPCOM_ATTR_SET_LIBC_EPFD, &libc_epfd, &size);
            if (rv < 0) {
                errno = -rv;
                rv    = -1;
                goto done;
        ...
        LDBG(1,
             "LDP<%d>: epfd %d (0x%x): calling %s(): libc_epfd %d (0x%x), "
             "op %d, fd %d (0x%x), event %p",
             getpid(), epfd, epfd, func_str, libc_epfd, libc_epfd, op, fd, fd, event);
...
ldp_epoll_pwait(int epfd, struct epoll_event *events, int maxevents, int timeout, const sigset_t *sigmask)
{
    ...
    if (PREDICT_FALSE(vep_idx == INVALID_SESSION_ID)) {
        clib_warning("LDP<%d>: ERROR: epfd %d (0x%x): bad vep_idx %d (0x%x)!", getpid(), epfd, epfd, vep_idx, vep_idx);
        errno = EBADFD;
        return -1;
    }
    ...
    LDBG(2,
         "LDP<%d>: epfd %d (0x%x): vep_idx %d (0x%x), libc_epfd %d (0x%x), "
         "events %p, maxevents %d, timeout %d, sigmask %p: time_to_wait %.02f",
         getpid(), epfd, epfd, vep_idx, vep_idx, libc_epfd, libc_epfd, events, maxevents, timeout, sigmask,
    ...
    do {
        if (!ldp->epoll_wait_vcl) {
            ...
            LDBG(3,
                 "LDP<%d>: epfd %d (0x%x): calling %s(): vep_idx %d (0x%x),"
                 " events %p, maxevents %d",
                 getpid(), epfd, epfd, func_str, vep_idx, vep_idx, events, maxevents);
        ...
        if (libc_epfd > 0) {
            ...
            LDBG(3,
                 "LDP<%d>: epfd %d (0x%x): calling %s(): libc_epfd %d "
                 "(0x%x), events %p, maxevents %d, sigmask %p",
                 getpid(), epfd, epfd, func_str, libc_epfd, libc_epfd, events, maxevents, sigmask);
    ...
    if (LDP_DEBUG > 3) {
        ...
        if (rv < 0) {
            ...
            clib_warning("LDP<%d>: ERROR: epfd %d (0x%x): %s() failed! "
                         "rv %d, errno = %d",
                         getpid(), epfd, epfd, func_str, rv, errno_val);
            errno = errno_val;
        } else
            clib_warning("LDP<%d>: epfd %d (0x%x): returning %d (0x%x)", getpid(), epfd, epfd, rv, rv);
    }
    return rv;
}
...
poll(struct pollfd *fds, nfds_t nfds, int timeout)
{
    ...
    if (LDP_DEBUG > 3)
        clib_warning("LDP<%d>: fds %p, nfds %d, timeout %d", getpid(), fds, nfds, timeout);
        ...
        if (fds[i].fd >= 0) {
            if (LDP_DEBUG > 3)
                clib_warning("LDP<%d>: fds[%d].fd %d (0x%0x), .events = 0x%x, "
                             ".revents = 0x%x",
                             getpid(), i, fds[i].fd, fds[i].fd, fds[i].events, fds[i].revents);
    ...
    do {
        if (vec_len(ldp->vcl_poll)) {
            ...
            if (LDP_DEBUG > 3)
                clib_warning("LDP<%d>: calling %s(): "
                             ...
                             "n_libc_fds %u",
                             getpid(), func_str, ldp->vcl_poll, vec_len(ldp->vcl_poll), vec_len(ldp->vcl_poll),
                             n_libc_fds);
        ...
        if (n_libc_fds) {
            ...
            if (LDP_DEBUG > 3)
                clib_warning("LDP<%d>: calling %s(): fds %p, nfds %u: n_sids %u", getpid(), fds, nfds,
    ...
    } while ((wait_for_time == -1) || (clib_time_now(&ldp->clib_time) < wait_for_time));
    rv = 0;
...
#ifdef __USE_XOPEN2K
        if ((fds[vp->fds_ndx].revents & POLLIN) && (fds[vp->fds_ndx].events & POLLRDNORM))
            fds[vp->fds_ndx].revents |= POLLRDNORM;
        if ((fds[vp->fds_ndx].revents & POLLOUT) && (fds[vp->fds_ndx].events & POLLWRNORM))
    ...
    if (LDP_DEBUG > 3) {
        if (rv < 0) {
            ...
            clib_warning("LDP<%d>: returning %d (0x%x): n_sids %u, "
                         "n_libc_fds %d",
                         getpid(), rv, rv, vec_len(ldp->vcl_poll), n_libc_fds);
