...
vtc_cfg_sync(vcl_test_session_t *ts)
{
    ...
    tx_bytes = vcl_test_write(ts->fd, (uint8_t *) &ts->cfg, sizeof(ts->cfg), NULL, ts->cfg.verbose);
    if (tx_bytes < 0) {
        vtwrn("(fd %d): write test cfg failed (%d)!", ts->fd, tx_bytes);
        return tx_bytes;
    }

    rx_bytes = vcl_test_read(ts->fd, (uint8_t *) ts->rxbuf, sizeof(vcl_test_cfg_t), NULL);
    if (rx_bytes < 0)
        return rx_bytes;
    ...
    if ((rx_bytes != sizeof(vcl_test_cfg_t)) || !vcl_test_cfg_verify(rx_cfg, &ts->cfg)) {
        vtwrn("(fd %d): Invalid config received from server!", ts->fd);
        if (rx_bytes != sizeof(vcl_test_cfg_t)) {
            vtinf("\tRx bytes %d != cfg size %lu", rx_bytes, sizeof(vcl_test_cfg_t));
        } else {
...
static void
vtc_accumulate_stats(vcl_test_client_worker_t *wrk, vcl_test_session_t *ctrl)
{
    vcl_test_session_t *ts;
    static char buf[64];
    int i, show_rx = 0;
    ...
    if (ctrl->cfg.test == VCL_TEST_TYPE_BI || ctrl->cfg.test == VCL_TEST_TYPE_ECHO)
        show_rx = 1;
...
vtc_worker_sessions_exit(vcl_test_client_worker_t *wrk)
{
    ...
    for (i = 0; i < wrk->cfg.num_test_sessions; i++) {
        ...
        (void) vcl_test_write(ts->fd, (uint8_t *) &ts->cfg, sizeof(ts->cfg), &ts->stats, verbose);
    }
    wrk->n_sessions = 0;
}
...
vtc_worker_loop(void *arg)
{
    ...
    while (n_active_sessions) {
        ...
        rv = vppcom_select(wrk->max_fd_index, (uint64_t *) rfdset, (uint64_t *) wfdset, NULL, 0);
        if (rv < 0) {
        ...
        for (i = 0; i < wrk->cfg.num_test_sessions; i++) {
            ts = &wrk->sessions[i];
            if (!((ts->stats.stop.tv_sec == 0) && (ts->stats.stop.tv_nsec == 0)))
                continue;

            if (FD_ISSET(vppcom_session_index(ts->fd), rfdset) && ts->stats.rx_bytes < ts->cfg.total_bytes) {
                (void) vcl_test_read(ts->fd, (uint8_t *) ts->rxbuf, ts->rxbuf_size, &ts->stats);
            }
            ...
            if (FD_ISSET(vppcom_session_index(ts->fd), wfdset) && ts->stats.tx_bytes < ts->cfg.total_bytes) {
                ...
                rv = vcl_test_write(ts->fd, (uint8_t *) ts->txbuf, n_bytes, &ts->stats, ts->cfg.verbose);
                if (rv < 0) {
...
vtc_print_stats(vcl_test_session_t *ctrl)
{
    ...
    if (is_echo)
        ...
        sprintf(buf, "%s-directional Stream", ctrl->cfg.test == VCL_TEST_TYPE_BI ? "Bi" : "Uni");
}
...
vtc_stream_client(vcl_test_client_main_t *vcm)
{
    ...
    vtinf("%s-directional Stream Test Starting!", ctrl->cfg.test == VCL_TEST_TYPE_BI ? "Bi" : "Uni");
    ...
    for (i = 1; i < vcm->n_workers; i++) {
        wrk = &vcm->workers[i];
        pthread_create(&wrk->thread_handle, NULL, vtc_worker_loop, (void *) wrk);
    }
...
{
    fprintf(stderr, "vcl_test_client [OPTIONS] <ipaddr> <port>\n"
                    "  OPTIONS\n"
    ...
    while ((c = getopt(argc, argv, "chn:w:XE:I:N:R:T:UBV6D")) != -1)
        ...
        case 'I':
            ...
            if (ctrl->cfg.num_test_sessions > VCL_TEST_CFG_MAX_TEST_SESS) {
                vtwrn("value greater than max number test sessions (%d)!", VCL_TEST_CFG_MAX_TEST_SESS);
                print_usage_and_exit();
            }
            break;
...
vtc_ctrl_session_exit(void)
{
    ...
    (void) vcl_test_write(ctrl->fd, (uint8_t *) &ctrl->cfg, sizeof(ctrl->cfg), &ctrl->stats, verbose);
    sleep(1);
}
