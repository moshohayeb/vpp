 ...
 */
static vppcom_main_t _vppcom_main = {.debug = VPPCOM_DEBUG_INIT, .is_init = 0, .my_client_index = ~0};
...
vppcom_cfg_heapsize(char *conf_fname)
{
    ...
    if (fp == NULL) {
        VCFG_DBG(0, "VCL<%d>: using default heapsize %lu (0x%lx)", getpid(), vcl_cfg->heapsize, vcl_cfg->heapsize);
        goto defaulted;
    }
    ...
    if (tmp == NULL) {
        VCFG_DBG(0,
                 "VCL<%d>: realloc failed, using default heapsize %ld "
                 "(0x%lx)",
                 getpid(), vcl_cfg->heapsize, vcl_cfg->heapsize);
        goto defaulted;
    }
        ...
        if (!strncmp(argv[i], "heapsize", 8)) {
            ...
            if (size == 0) {
                VCFG_DBG(0,
                         ...
                         "heapsize %ld (0x%lx)",
                         getpid(), argv[i], argv[i + 1], vcl_cfg->heapsize, vcl_cfg->heapsize);
                goto defaulted;
            }
...
defaulted:
    ...
    VCFG_DBG(0, "VCL<%d>: allocated VCL heap = %p, size %ld (0x%lx)", getpid(), heap, vcl_cfg->heapsize,
             vcl_cfg->heapsize);
}
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (vc_cfg_input) {
            ...
            if (unformat(line_input, "max-workers %u", &vcl_cfg->max_workers)) {
                ...
                vcl_cfg->vpp_api_filename = format(0, "/%s-vpe-api%c", chroot_path, 0);
                vl_set_memory_root_path((char *) chroot_path);

                VCFG_DBG(0,
                         "VCL<%d>: configured api-prefix (%s) and api "
                         "filename (%s)",
                         getpid(), chroot_path, vcl_cfg->vpp_api_filename);
                chroot_path = 0; /* Don't vec_free() it! */
            } else if (unformat(line_input, "api-socket-name %s", &vcl_cfg->vpp_api_socket_name)) {
                vec_terminate_c_string(vcl_cfg->vpp_api_socket_name);
                VCFG_DBG(0, "VCL<%d>: configured api-socket-name (%s)", getpid(), vcl_cfg->vpp_api_socket_name);
                ...
                if (q_len < vcl_cfg->vpp_api_q_length) {
                    ...
                    VCFG_DBG(0, "VCL<%d>: configured vpp-api-q-length %u", getpid(), vcl_cfg->vpp_api_q_length);
                }
            ...
            } else if (unformat(line_input, "segment-baseva 0x%x", &vcl_cfg->segment_baseva)) {
                VCFG_DBG(0, "VCL<%d>: configured segment_baseva 0x%lx", getpid(), vcl_cfg->segment_baseva);
            } else if (unformat(line_input, "segment-size 0x%x", &vcl_cfg->segment_size)) {
                VCFG_DBG(0, "VCL<%d>: configured segment_size 0x%x (%d)", getpid(), vcl_cfg->segment_size,
                         vcl_cfg->segment_size);
            } else if (unformat(line_input, "segment-size %d", &vcl_cfg->segment_size)) {
                VCFG_DBG(0, "VCL<%d>: configured segment_size %d (0x%x)", getpid(), vcl_cfg->segment_size,
                         vcl_cfg->segment_size);
            } else if (unformat(line_input, "add-segment-size 0x%x", &vcl_cfg->add_segment_size)) {
                VCFG_DBG(0, "VCL<%d>: configured add_segment_size 0x%x (%d)", getpid(), vcl_cfg->add_segment_size,
                         vcl_cfg->add_segment_size);
            } else if (unformat(line_input, "add-segment-size %d", &vcl_cfg->add_segment_size)) {
                VCFG_DBG(0, "VCL<%d>: configured add_segment_size %d (0x%x)", getpid(), vcl_cfg->add_segment_size,
                         vcl_cfg->add_segment_size);
            } else if (unformat(line_input, "preallocated-fifo-pairs %d", &vcl_cfg->preallocated_fifo_pairs)) {
                VCFG_DBG(0,
                         "VCL<%d>: configured preallocated_fifo_pairs %d "
                         "(0x%x)",
                         getpid(), vcl_cfg->preallocated_fifo_pairs, vcl_cfg->preallocated_fifo_pairs);
            } else if (unformat(line_input, "rx-fifo-size 0x%lx", &vcl_cfg->rx_fifo_size)) {
                VCFG_DBG(0, "VCL<%d>: configured rx_fifo_size 0x%x (%d)", getpid(), vcl_cfg->rx_fifo_size,
                         vcl_cfg->rx_fifo_size);
            } else if (unformat(line_input, "rx-fifo-size %d", &vcl_cfg->rx_fifo_size)) {
                VCFG_DBG(0, "VCL<%d>: configured rx_fifo_size %d (0x%x)", getpid(), vcl_cfg->rx_fifo_size,
                         vcl_cfg->rx_fifo_size);
            } else if (unformat(line_input, "tx-fifo-size 0x%lx", &vcl_cfg->tx_fifo_size)) {
                ...
                VCFG_DBG(0, "VCL<%d>: configured tx_fifo_size %d (0x%x)", getpid(), vcl_cfg->tx_fifo_size,
                         vcl_cfg->tx_fifo_size);
            } else if (unformat(line_input, "event-queue-size 0x%lx", &vcl_cfg->event_queue_size)) {
                VCFG_DBG(0, "VCL<%d>: configured event_queue_size 0x%x (%d)", getpid(), vcl_cfg->event_queue_size,
                         vcl_cfg->event_queue_size);
            } else if (unformat(line_input, "event-queue-size %ld", &vcl_cfg->event_queue_size)) {
                VCFG_DBG(0, "VCL<%d>: configured event_queue_size %d (0x%x)", getpid(), vcl_cfg->event_queue_size,
                         vcl_cfg->event_queue_size);
            } else if (unformat(line_input, "listen-queue-size 0x%lx", &vcl_cfg->listen_queue_size)) {
            ...
            } else if (unformat(line_input, "accept-timeout %f", &vcl_cfg->accept_timeout)) {
                VCFG_DBG(0, "VCL<%d>: configured accept_timeout %f", getpid(), vcl_cfg->accept_timeout);
                ...
                VCFG_DBG(0, "VCL<%d>: configured namespace_secret %lu (0x%lx)", getpid(), vcl_cfg->namespace_secret,
...
vppcom_cfg(vppcom_cfg_t *vcl_cfg)
{
    ...
    if (env_var_str) {
        ...
        VCFG_DBG(0,
                 "VCL<%d>: configured api prefix (%s) and filename (%s) "
                 "from " VPPCOM_ENV_API_PREFIX "!",
    ...
    if (env_var_str) {
        ...
        VCFG_DBG(0, "VCL<%d>: configured namespace_id (%s) from " VPPCOM_ENV_APP_NAMESPACE_ID "!", getpid(),
    ...
    if (env_var_str) {
        ...
        if (sscanf(env_var_str, "%lu", &tmp) != 1) {
            ...
            VCFG_DBG(0, "VCL<%d>: configured namespace secret (%lu) from " VPPCOM_ENV_APP_NAMESPACE_SECRET "!",
                     getpid(), vcm->cfg.namespace_secret);
        }
    }
    if (getenv(VPPCOM_ENV_APP_PROXY_TRANSPORT_TCP)) {
        vcm->cfg.app_proxy_transport_tcp = 1;
        VCFG_DBG(0, "VCL<%d>: configured app_proxy_transport_tcp (%u) from " VPPCOM_ENV_APP_PROXY_TRANSPORT_TCP "!",
                 getpid(), vcm->cfg.app_proxy_transport_tcp);
    }
    if (getenv(VPPCOM_ENV_APP_PROXY_TRANSPORT_UDP)) {
        vcm->cfg.app_proxy_transport_udp = 1;
        VCFG_DBG(0, "VCL<%d>: configured app_proxy_transport_udp (%u) from " VPPCOM_ENV_APP_PROXY_TRANSPORT_UDP "!",
                 getpid(), vcm->cfg.app_proxy_transport_udp);
    }
    if (getenv(VPPCOM_ENV_APP_SCOPE_LOCAL)) {
        vcm->cfg.app_scope_local = 1;
        VCFG_DBG(0, "VCL<%d>: configured app_scope_local (%u) from " VPPCOM_ENV_APP_SCOPE_LOCAL "!", getpid(),
                 vcm->cfg.app_scope_local);
    }
    if (getenv(VPPCOM_ENV_APP_SCOPE_GLOBAL)) {
        vcm->cfg.app_scope_global = 1;
        VCFG_DBG(0, "VCL<%d>: configured app_scope_global (%u) from " VPPCOM_ENV_APP_SCOPE_GLOBAL "!", getpid(),
