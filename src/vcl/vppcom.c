...
static void
vcl_send_session_accepted_reply(svm_msg_q_t *mq, u32 context, session_handle_t handle, int retval)
{
...
static void
vcl_send_session_disconnected_reply(svm_msg_q_t *mq, u32 context, session_handle_t handle, int retval)
{
    app_session_evt_t _app_evt, *app_evt = &_app_evt;
    session_disconnected_reply_msg_t *rmp;
    app_alloc_ctrl_evt_to_vpp(mq, app_evt, SESSION_CTRL_EVT_DISCONNECTED_REPLY);
...
static void
vcl_send_session_reset_reply(svm_msg_q_t *mq, u32 context, session_handle_t handle, int retval)
{
    app_session_evt_t _app_evt, *app_evt = &_app_evt;
...
vcl_session_accepted_handler(vcl_worker_t *wrk, session_accepted_msg_t *mp)
{
    ...
    listen_session = vcl_session_table_lookup_listener(wrk, mp->listener_handle);
    if (!listen_session) {
        svm_msg_q_t *evt_q;
    ...
    if (mp->server_event_queue_address) {
        session->vpp_evt_q = uword_to_pointer(mp->client_event_queue_address, svm_msg_q_t *);
        session->our_evt_q = uword_to_pointer(mp->server_event_queue_address, svm_msg_q_t *);
        ...
        session->vpp_evt_q            = uword_to_pointer(mp->vpp_event_queue_address, svm_msg_q_t *);
    ...
    VDBG(1,
         "VCL<%d>: vpp handle 0x%llx, sid %u: client accept request from %s"
         " address %U port %d queue %p!",
         getpid(), mp->handle, session->session_index, mp->is_ip4 ? "IPv4" : "IPv6", format_ip46_address, &mp->ip,
         mp->is_ip4 ? IP46_TYPE_IP4 : IP46_TYPE_IP6, clib_net_to_host_u16(mp->port), session->vpp_evt_q);
...
vcl_session_connected_handler(vcl_worker_t *wrk, session_connected_msg_t *mp)
{
    ...
    if (mp->retval) {
        clib_warning("VCL<%d>: ERROR: sid %u: connect failed! %U", getpid(), session_index, format_api_error,
                     ntohl(mp->retval));
    ...
    if (mp->client_event_queue_address) {
        session->vpp_evt_q = uword_to_pointer(mp->server_event_queue_address, svm_msg_q_t *);
        session->our_evt_q = uword_to_pointer(mp->client_event_queue_address, svm_msg_q_t *);
    ...
    session->vpp_handle       = mp->handle;
    session->transport.is_ip4 = mp->is_ip4;
    clib_memcpy(&session->transport.lcl_ip, mp->lcl_ip, sizeof(session->transport.lcl_ip));
    ...
    VDBG(1,
         "VCL<%d>: vpp handle 0x%llx, sid %u: connect succeeded! "
         "session_rx_fifo %p, refcnt %d, session_tx_fifo %p, refcnt %d",
         getpid(), mp->handle, session_index, session->rx_fifo, session->rx_fifo->refcnt, session->tx_fifo,
         session->tx_fifo->refcnt);
...
vcl_session_reset_handler(vcl_worker_t *wrk, session_reset_msg_t *reset_msg)
{
    ...
    vcl_send_session_reset_reply(vcl_session_vpp_evt_q(wrk, session), vcm->my_client_index, reset_msg->handle, 0);
    return sid;
}
...
vcl_session_bound_handler(vcl_worker_t *wrk, session_bound_msg_t *mp)
{
    ...
    session = vcl_session_get(wrk, sid);
    if (mp->retval) {
        VDBG(0, "VCL<%d>: ERROR: vpp handle 0x%llx, sid %u: bind failed: %U", getpid(), mp->handle, sid,
    ...
    clib_memcpy(&session->transport.lcl_ip, mp->lcl_ip, sizeof(ip46_address_t));
    session->transport.lcl_port = mp->lcl_port;
    ...
    VDBG(1, "VCL<%d>: vpp handle 0x%llx, sid %u: bind succeeded!", getpid(), mp->handle, sid);
    return sid;
}
...
vcl_handle_mq_event(vcl_worker_t *wrk, session_event_t *e)
{
    ...
    case SESSION_CTRL_EVT_CONNECTED:
        vcl_session_connected_handler(wrk, (session_connected_msg_t *) e->data);
        break;
    case SESSION_CTRL_EVT_DISCONNECTED:
        ...
        session          = vcl_session_get(wrk, sid);
        if (!session) {
            VDBG(0, "request to disconnect unknown handle 0x%llx", disconnected_msg->handle);
            break;
        }
        session->session_state = STATE_DISCONNECT;
        VDBG(0, "disconnected handle 0x%llx, sid %u", disconnected_msg->handle, sid);
        break;
...
static inline int
vppcom_wait_for_session_state_change(u32 session_index, session_state_t state, f64 wait_for_time)
{
    ...
    VDBG(0, "VCL<%d>: timeout waiting for state 0x%x (%s)", getpid(), state, vppcom_session_state_str(state));
...
vppcom_app_session_enable(void)
{
    ...
    if (vcm->app_state != STATE_APP_ENABLED) {
        ...
        if (PREDICT_FALSE(rv)) {
            VDBG(0,
                 "VCL<%d>: application session enable timed out! "
                 "returning %d (%s)",
...
vppcom_app_attach(void)
{
    ...
    if (PREDICT_FALSE(rv)) {
        VDBG(0, "VCL<%d>: application attach timed out! returning %d (%s)", getpid(), rv, vppcom_retval_str(rv));
        return rv;
    }
...
vppcom_session_unbind(u32 session_handle)
{
    ...
    VDBG(1,
         "VCL<%d>: vpp handle 0x%llx, sid %u: sending unbind msg! new state"
         " 0x%x (%s)",
         getpid(), vpp_handle, session_handle, STATE_DISCONNECT, vppcom_session_state_str(STATE_DISCONNECT));
...
vppcom_session_disconnect(u32 session_handle)
{
    ...
    VDBG(1, "VCL<%d>: vpp handle 0x%llx, sid %u state 0x%x (%s)", getpid(), vpp_handle, session_handle, state,
    ...
    if (state & STATE_CLOSE_ON_EMPTY) {
        ...
        vcl_send_session_disconnected_reply(vpp_evt_q, vcm->my_client_index, vpp_handle, 0);
        VDBG(1,
             "VCL<%d>: vpp handle 0x%llx, sid %u: sending disconnect "
             "REPLY...",
             getpid(), vpp_handle, session_handle);
    } else {
        VDBG(1, "VCL<%d>: vpp handle 0x%llx, sid %u: sending disconnect...", getpid(), vpp_handle, session_handle);
...
vppcom_app_create(char *app_name)
{
    ...
    if (!vcm->is_init) {
        ...
        svm_fifo_segment_main_init(vcl_cfg->segment_baseva, 20 /* timeout in secs */);
    ...
    if (vcm->my_client_index == ~0) {
        ...
        if (rv) {
            clib_warning("VCL<%d>: ERROR: couldn't connect to VPP!", getpid());
            return rv;
        }
        ...
        if (rv) {
            clib_warning("VCL<%d>: ERROR: vppcom_app_attach() failed!", getpid());
            return rv;
        }

        VDBG(0, "VCL<%d>: app_name '%s', my_client_index %d (0x%x)", getpid(), app_name, vcm->my_client_index,
             vcm->my_client_index);
    }
...
vppcom_app_destroy(void)
{
    ...
    VDBG(0, "VCL<%d>: detaching from VPP, my_client_index %d (0x%x)", getpid(), vcm->my_client_index,
         vcm->my_client_index);
    ...
    if (PREDICT_FALSE(rv))
        VDBG(0, "VCL<%d>: application detach timed out! returning %d (%s)", getpid(), rv, vppcom_retval_str(rv));
...
vppcom_session_close(uint32_t session_handle)
{
    ...
    if (is_vep) {
        while (next_sh != ~0) {
            ...
            if (PREDICT_FALSE(rv < 0))
                VDBG(0,
                     "VCL<%d>: vpp handle 0x%llx, sid %u: EPOLL_CTL_DEL "
                     "vep_idx %u failed! rv %d (%s)",
                     getpid(), vpp_handle, next_sh, vep_sh, rv, vppcom_retval_str(rv));
        ...
        if (is_vep_session) {
            ...
            if (rv < 0)
                VDBG(0,
                     "VCL<%d>: vpp handle 0x%llx, sid %u: EPOLL_CTL_DEL "
                     "vep_idx %u failed! rv %d (%s)",
                     getpid(), vpp_handle, session_handle, vep_sh, rv, vppcom_retval_str(rv));
        }
        ...
        if (state & STATE_LISTEN) {
            ...
            if (PREDICT_FALSE(rv < 0))
                VDBG(0,
                     "VCL<%d>: vpp handle 0x%llx, sid %u: listener unbind "
                     "failed! rv %d (%s)",
                     getpid(), vpp_handle, session_handle, rv, vppcom_retval_str(rv));
    ...
    if (vcl_session_is_ct(session)) {
        ...
        VDBG(0, "Removing ct registration %u", vcl_ct_registration_index(wrk, ctr));
    ...
    if (VPPCOM_DEBUG > 0) {
        if (is_vep)
            clib_warning("VCL<%d>: vep_idx %u / sid %u: epoll session removed.", getpid(), session_handle,
                         session_handle);
        else
            clib_warning("VCL<%d>: vpp handle 0x%llx, sid %u: session removed.", getpid(), vpp_handle, session_handle);
    }
...
vppcom_session_bind(uint32_t session_handle, vppcom_endpt_t *ep)
{
    ...
    if (ep->is_ip4)
        clib_memcpy(&session->transport.lcl_ip.ip4, ep->ip, sizeof(ip4_address_t));
    else
        clib_memcpy(&session->transport.lcl_ip.ip6, ep->ip, sizeof(ip6_address_t));
    session->transport.lcl_port = ep->port;

    VDBG(0,
         "VCL<%d>: sid %u: binding to local %s address %U port %u, "
         "proto %s",
         getpid(), session_handle, session->transport.is_ip4 ? "IPv4" : "IPv6", format_ip46_address,
         &session->transport.lcl_ip, session->transport.is_ip4 ? IP46_TYPE_IP4 : IP46_TYPE_IP6,
         clib_net_to_host_u16(session->transport.lcl_port), session->session_type ? "UDP" : "TCP");
...
vppcom_session_listen(uint32_t listen_sh, uint32_t q_len)
{
    ...
    if (listen_session->session_state & STATE_LISTEN) {
        VDBG(0, "VCL<%d>: vpp handle 0x%llx, sid %u: already in listen state!", getpid(), listen_vpp_handle, listen_sh);
        return VPPCOM_OK;
    }

    VDBG(0,
         "VCL<%d>: vpp handle 0x%llx, sid %u: sending VPP bind+listen "
         "request...",
    ...
    if (PREDICT_FALSE(rv)) {
        ...
        VDBG(0,
             ...
             "returning %d (%s)",
             getpid(), listen_session->vpp_handle, listen_sh, rv, vppcom_retval_str(rv));
        return rv;
    }
...
static int
validate_args_session_accept_(vcl_worker_t *wrk, vcl_session_t *listen_session)
{
    /* Input validation - expects spinlock on sessions_lockp */
    if (listen_session->is_vep) {
        clib_warning("VCL<%d>: ERROR: sid %u: cannot accept on an "
                     "epoll session!",
                     getpid(), listen_session->session_index);
        return VPPCOM_EBADFD;
    }

    if (listen_session->session_state != STATE_LISTEN) {
        clib_warning("VCL<%d>: ERROR: vpp handle 0x%llx, sid %u: "
                     "not in listen state! state 0x%x (%s)",
                     getpid(), listen_session->vpp_handle, listen_session->session_index, listen_session->session_state,
...
int
vppcom_session_accept(uint32_t listen_session_handle, vppcom_endpt_t *ep, uint32_t flags)
{
    ...
    is_nonblocking = VCL_SESS_ATTR_TEST(listen_session->attr, VCL_SESS_ATTR_NONBLOCK);
...
handle:
    ...
    listen_vpp_handle = listen_session->vpp_handle;
    VDBG(0,
         "VCL<%d>: vpp handle 0x%llx, sid %u: Got a client request! "
         "vpp handle 0x%llx, sid %u, flags %d, is_nonblocking %u",
         getpid(), listen_vpp_handle, listen_session_handle, client_session->vpp_handle, client_session_index, flags,
    ...
    if (ep) {
        ...
        if (client_session->transport.is_ip4)
            clib_memcpy(ep->ip, &client_session->transport.rmt_ip.ip4, sizeof(ip4_address_t));
        else
            clib_memcpy(ep->ip, &client_session->transport.rmt_ip.ip6, sizeof(ip6_address_t));
    }

    if (accepted_msg.server_event_queue_address)
        vpp_evt_q = uword_to_pointer(accepted_msg.vpp_event_queue_address, svm_msg_q_t *);
    else
        vpp_evt_q = client_session->vpp_evt_q;

    vcl_send_session_accepted_reply(vpp_evt_q, client_session->client_context, client_session->vpp_handle, 0);

    VDBG(0,
         "VCL<%d>: vpp handle 0x%llx, sid %u: accepted vpp handle 0x%llx, "
         "sid %u connection from peer %s address %U port %u to local %s "
         "address %U port %u",
         getpid(), listen_vpp_handle, listen_session_handle, client_session->vpp_handle, client_session_index,
         client_session->transport.is_ip4 ? "IPv4" : "IPv6", format_ip46_address, &client_session->transport.rmt_ip,
         client_session->transport.is_ip4 ? IP46_TYPE_IP4 : IP46_TYPE_IP6,
         clib_net_to_host_u16(client_session->transport.rmt_port), client_session->transport.is_ip4 ? "IPv4" : "IPv6",
...
vppcom_session_connect(uint32_t session_handle, vppcom_endpt_t *server_ep)
{
    ...
    if (PREDICT_FALSE(session->session_state & CLIENT_STATE_OPEN)) {
        VDBG(0,
             "VCL<%d>: vpp handle 0x%llx, sid %u: session already "
             "connected to %s %U port %d proto %s, state 0x%x (%s)",
             getpid(), session->vpp_handle, session_handle, session->transport.is_ip4 ? "IPv4" : "IPv6",
             format_ip46_address, &session->transport.rmt_ip, session->transport.is_ip4 ? IP46_TYPE_IP4 : IP46_TYPE_IP6,
             clib_net_to_host_u16(session->transport.rmt_port), session->session_type ? "UDP" : "TCP",
             session->session_state, vppcom_session_state_str(session->session_state));
        return VPPCOM_OK;
    }

    session->transport.is_ip4 = server_ep->is_ip4;
    if (session->transport.is_ip4)
        clib_memcpy(&session->transport.rmt_ip.ip4, server_ep->ip, sizeof(ip4_address_t));
    else
        clib_memcpy(&session->transport.rmt_ip.ip6, server_ep->ip, sizeof(ip6_address_t));
    session->transport.rmt_port = server_ep->port;

    VDBG(0,
         "VCL<%d>: vpp handle 0x%llx, sid %u: connecting to server %s %U "
         "port %d proto %s",
         getpid(), session->vpp_handle, session_handle, session->transport.is_ip4 ? "IPv4" : "IPv6",
         format_ip46_address, &session->transport.rmt_ip, session->transport.is_ip4 ? IP46_TYPE_IP4 : IP46_TYPE_IP6,
         clib_net_to_host_u16(session->transport.rmt_port), session->session_type ? "UDP" : "TCP");
        ...
        if (VPPCOM_DEBUG > 0) {
            if (session)
                clib_warning("VCL<%d>: vpp handle 0x%llx, sid %u: connect "
                             "failed! returning %d (%s)",
                             getpid(), session->vpp_handle, session_handle, rv, vppcom_retval_str(rv));
            else
                clib_warning("VCL<%d>: no session for sid %u: connect failed! "
                             "returning %d (%s)",
                             getpid(), session_handle, rv, vppcom_retval_str(rv));
        }
    } else
        VDBG(0, "VCL<%d>: vpp handle 0x%llx, sid %u: connected!", getpid(), session->vpp_handle, session_handle);
...
{
    if (!is_ct)
        return (e->event_type == FIFO_EVENT_APP_RX && e->fifo->client_session_index == sid);
    else
...
{
    return ((s->session_state & STATE_OPEN) || (s->session_state == STATE_LISTEN && s->session_type == VPPCOM_PROTO_UDP));
}
...
vppcom_session_read_internal(uint32_t session_handle, void *buf, int n, u8 peek)
{
    ...
    if (PREDICT_FALSE(!vcl_session_is_readable(s))) {
        ...
        VDBG(0,
             "VCL<%d>: vpp handle 0x%llx, sid %u: %s session is not open! "
             "state 0x%x (%s), returning %d (%s)",
             getpid(), s->vpp_handle, session_handle, state, vppcom_session_state_str(state), rv, vppcom_retval_str(rv));
        return rv;
    }
    ...
    if (is_ct && svm_fifo_want_tx_evt(rx_fifo)) {
        ...
        app_send_io_evt_to_vpp(s->vpp_evt_q, s->rx_fifo, SESSION_IO_EVT_CT_RX, SVM_Q_WAIT);
    }

    VDBG(2, "VCL<%d>: vpp handle 0x%llx, sid %u: read %d bytes from (%p)", getpid(), s->vpp_handle, session_handle,
         n_read, rx_fifo);
...
int
vppcom_session_read_segments(uint32_t session_handle, vppcom_data_segments_t ds)
{
    vcl_worker_t *wrk = vcl_worker_get_current();
    int n_read        = 0, rv, is_nonblocking;
    vcl_session_t *s  = 0;
...
void
vppcom_session_free_segments(uint32_t session_handle, vppcom_data_segments_t ds)
{
    vcl_worker_t *wrk = vcl_worker_get_current();
    vcl_session_t *s;
...
vppcom_session_read_ready(vcl_session_t *session)
{
    ...
    if (PREDICT_FALSE(!(session->session_state & (STATE_OPEN | STATE_LISTEN)))) {
        ...
        VDBG(1,
             "VCL<%d>: vpp handle 0x%llx, sid %u: session is not open!"
             " state 0x%x (%s), returning %d (%s)",
             getpid(), session->vpp_handle, session->session_index, state, vppcom_session_state_str(state), rv,
...
vppcom_data_segment_copy(void *buf, vppcom_data_segments_t ds, u32 max_bytes)
{
    ...
    if (first_copy < max_bytes) {
        clib_memcpy(buf + first_copy, ds[1].data, clib_min(ds[1].len, max_bytes - first_copy));
    }
    return 0;
}
...
{
    if (!is_ct)
        return (e->event_type == FIFO_EVENT_APP_TX && e->fifo->client_session_index == sid);
    else
...
vppcom_session_write(uint32_t session_handle, void *buf, size_t n)
{
    ...
    if (PREDICT_FALSE(!(s->session_state & STATE_OPEN))) {
        ...
        VDBG(1,
             "VCL<%d>: vpp handle 0x%llx, sid %u: session is not open! "
             "state 0x%x (%s)",
             getpid(), s->vpp_handle, session_handle, state, vppcom_session_state_str(state));
        return rv;
    }
    ...
    if (s->is_dgram)
        n_write = app_send_dgram_raw(tx_fifo, &s->transport, s->vpp_evt_q, buf, n, et, SVM_Q_WAIT);
    else
        n_write = app_send_stream_raw(tx_fifo, s->vpp_evt_q, buf, n, et, SVM_Q_WAIT);

    ASSERT(n_write > 0);

    VDBG(2, "VCL<%d>: vpp handle 0x%llx, sid %u: wrote %d bytes", getpid(), s->vpp_handle, session_handle, n_write);
...
vppcom_session_write_ready(vcl_session_t *session)
{
    ...
    VDBG(3, "VCL<%d>: vpp handle 0x%llx, sid %u: peek %s (%p), ready = %d", getpid(), session->vpp_handle,
         session->session_index, session->tx_fifo, svm_fifo_max_enqueue(session->tx_fifo));
...
static void
vcl_select_handle_mq_event(vcl_worker_t *wrk, session_event_t *e, unsigned long n_bits, unsigned long *read_map,
                           unsigned long *write_map, unsigned long *except_map, u32 *bits_set)
{
    session_disconnected_msg_t *disconnected_msg;
...
static int
vcl_select_handle_mq(vcl_worker_t *wrk, svm_msg_q_t *mq, unsigned long n_bits, unsigned long *read_map,
                     unsigned long *write_map, unsigned long *except_map, double time_to_wait, u32 *bits_set)
{
    svm_msg_q_msg_t *msg;
    session_event_t *e;
    u32 i;
...
static int
vppcom_select_condvar(vcl_worker_t *wrk, unsigned long n_bits, unsigned long *read_map, unsigned long *write_map,
                      unsigned long *except_map, double time_to_wait, u32 *bits_set)
{
    ...
    do {
        ...
        pool_foreach(cr, wrk->cut_through_registrations,
                     ({ vcl_select_handle_mq(wrk, cr->mq, n_bits, read_map, write_map, except_map, 0, bits_set); }));
        /* *INDENT-ON* */
        vcl_ct_registration_unlock(wrk);

        vcl_select_handle_mq(wrk, wrk->app_event_queue, n_bits, read_map, write_map, except_map, time_to_wait, bits_set);
...
static int
vppcom_select_eventfd(vcl_worker_t *wrk, unsigned long n_bits, unsigned long *read_map, unsigned long *write_map,
                      unsigned long *except_map, double time_to_wait, u32 *bits_set)
{
...
int
vppcom_select(unsigned long n_bits, unsigned long *read_map, unsigned long *write_map, unsigned long *except_map,
              double time_to_wait)
{
    ...
    if (n_bits && read_map) {
        ...
        clib_memcpy(wrk->rd_bitmap, read_map, vec_len(wrk->rd_bitmap) * sizeof(clib_bitmap_t));
        memset(read_map, 0, vec_len(wrk->rd_bitmap) * sizeof(clib_bitmap_t));
    }
    if (n_bits && write_map) {
        clib_bitmap_validate(wrk->wr_bitmap, minbits);
        clib_memcpy(wrk->wr_bitmap, write_map, vec_len(wrk->wr_bitmap) * sizeof(clib_bitmap_t));
        memset(write_map, 0, vec_len(wrk->wr_bitmap) * sizeof(clib_bitmap_t));
    }
    if (n_bits && except_map) {
        clib_bitmap_validate(wrk->ex_bitmap, minbits);
        clib_memcpy(wrk->ex_bitmap, except_map, vec_len(wrk->ex_bitmap) * sizeof(clib_bitmap_t));
        memset(except_map, 0, vec_len(wrk->ex_bitmap) * sizeof(clib_bitmap_t));
    }
    ...
    clib_bitmap_foreach(sid, wrk->wr_bitmap, ({
                            if (!(session = vcl_session_get(wrk, sid))) {
                                VDBG(0, "VCL<%d>: session %d specified in write_map is closed.", getpid(), sid);
                                return VPPCOM_EBADFD;
                            }
...
check_mq:
    ...
    for (i = 0; i < vec_len(wrk->unhandled_evts_vector); i++) {
        vcl_select_handle_mq_event(wrk, &wrk->unhandled_evts_vector[i], n_bits, read_map, write_map, except_map,
                                   &bits_set);
    }
...
vep_verify_epoll_chain(vcl_worker_t *wrk, u32 vep_idx)
{
    ...
    if (PREDICT_FALSE(!session)) {
        clib_warning("VCL<%d>: ERROR: Invalid vep_idx (%u)!", getpid(), vep_idx);
        goto done;
    }
    if (PREDICT_FALSE(!session->is_vep)) {
        clib_warning("VCL<%d>: ERROR: vep_idx (%u) is not a vep!", getpid(), vep_idx);
        goto done;
    }
    vep = &session->vep;
    clib_warning("VCL<%d>: vep_idx (%u): Dumping epoll chain\n"
                 ...
                 "}\n",
                 getpid(), vep_idx, session->is_vep, session->is_vep_session, vep->next_sh, vep->next_sh,
    ...
    for (sid = vep->next_sh; sid != ~0; sid = vep->next_sh) {
        ...
        if (PREDICT_FALSE(session->is_vep))
            clib_warning("VCL<%d>: ERROR: sid (%u) is a vep!", getpid(), vep_idx);
        ...
        if (PREDICT_FALSE(vep->vep_sh != vep_idx))
            clib_warning("VCL<%d>: ERROR: session (%u) vep_idx (%u) != "
                         "vep_idx (%u)!",
                         getpid(), sid, session->vep.vep_sh, vep_idx);
        if (session->is_vep_session) {
            clib_warning("vep_idx[%u]: sid 0x%x (%u)\n"
                         ...
                         "   et_mask        = 0x%x\n"
                         "}\n",
                         vep_idx, sid, sid, vep->next_sh, vep->next_sh, vep->prev_sh, vep->prev_sh, vep->vep_sh,
                         vep->vep_sh, vep->ev.events, vep->ev.data.u64, vep->et_mask);
        }
    }
...
vppcom_epoll_create(void)
{
    ...
    VDBG(0, "VCL<%d>: Created vep_idx %u / sid %u!", getpid(), vep_session->session_index, vep_session->session_index);
...
vppcom_epoll_ctl(uint32_t vep_handle, int op, uint32_t session_handle, struct epoll_event *event)
{
    ...
    if (vep_handle == session_handle) {
        clib_warning("VCL<%d>: ERROR: vep_idx == session_index (%u)!", getpid(), vep_handle);
        return VPPCOM_EINVAL;
    }
    ...
    if (PREDICT_FALSE(!vep_session->is_vep)) {
        clib_warning("VCL<%d>: ERROR: vep_idx (%u) is not a vep!", getpid(), vep_handle);
        return VPPCOM_EINVAL;
    }
    ...
    session = vcl_session_get_w_handle(wrk, session_handle);
    if (PREDICT_FALSE(!session)) {
        VDBG(0, "VCL<%d>: ERROR: Invalid session_handle (%u)!", getpid(), session_handle);
        return VPPCOM_EBADFD;
    }
    if (PREDICT_FALSE(session->is_vep)) {
    ...
    case EPOLL_CTL_ADD:
        ...
        VDBG(1,
             "VCL<%d>: EPOLL_CTL_ADD: vep_idx %u, sid %u, events 0x%x, "
             "data 0x%llx!",
             getpid(), vep_handle, session_handle, event->events, event->data.u64);
    ...
    case EPOLL_CTL_MOD:
        if (PREDICT_FALSE(!event)) {
            ...
            clib_warning("VCL<%d>: ERROR: sid %u EPOLL_CTL_MOD: "
                         "vep_idx (%u) != vep_idx (%u)!",
                         getpid(), session_handle, session->vep.vep_sh, vep_handle);
            rv = VPPCOM_EINVAL;
            goto done;
        }
        session->vep.et_mask = VEP_DEFAULT_ET_MASK;
    ...
    case EPOLL_CTL_DEL:
        ...
        vep_session->wait_cont_idx =
            (vep_session->wait_cont_idx == session_handle) ? session->vep.next_sh : vep_session->wait_cont_idx;
        ...
        VDBG(1, "VCL<%d>: EPOLL_CTL_DEL: vep_idx %u, sid %u!", getpid(), vep_handle, session_handle);
...
static inline void
vcl_epoll_wait_handle_mq_event(vcl_worker_t *wrk, session_event_t *e, struct epoll_event *events, u32 *num_ev)
{
    session_disconnected_msg_t *disconnected_msg;
    session_connected_msg_t *connected_msg;
    session_accepted_msg_t *accepted_msg;
...
static int
vcl_epoll_wait_handle_mq(vcl_worker_t *wrk, svm_msg_q_t *mq, struct epoll_event *events, u32 maxevents,
...
static int
vppcom_epoll_wait_condvar(vcl_worker_t *wrk, struct epoll_event *events, int maxevents, u32 n_evts, double wait_for_time)
{
    vcl_cut_through_registration_t *cr;
    double total_wait = 0, wait_slice;
    int rv;
    ...
    do {
        ...
        rv = vcl_epoll_wait_handle_mq(wrk, wrk->app_event_queue, events, maxevents, n_evts ? 0 : wait_slice, &n_evts);
        if (rv)
            total_wait += wait_slice;
        if (n_evts)
            return n_evts;
...
static int
vppcom_epoll_wait_eventfd(vcl_worker_t *wrk, struct epoll_event *events, int maxevents, u32 n_evts, double wait_for_time)
{
    vcl_mq_evt_conn_t *mqc;
    int __clib_unused n_read;
    int n_mq_evts, i;
    ...
    vec_validate(wrk->mq_events, pool_elts(wrk->mq_evt_conns));
again:
    n_mq_evts = epoll_wait(wrk->mqs_epfd, wrk->mq_events, vec_len(wrk->mq_events), wait_for_time);
    for (i = 0; i < n_mq_evts; i++) {
        mqc    = vcl_mq_evt_conn_get(wrk, wrk->mq_events[i].data.u32);
        n_read = read(mqc->mq_fd, &buf, sizeof(buf));
...
int
vppcom_epoll_wait(uint32_t vep_handle, struct epoll_event *events, int maxevents, double wait_for_time)
{
    vcl_worker_t *wrk = vcl_worker_get_current();
    vcl_session_t *vep_session;
    u32 n_evts = 0;
    ...
    if (PREDICT_FALSE(maxevents <= 0)) {
        clib_warning("VCL<%d>: ERROR: Invalid maxevents (%d)!", getpid(), maxevents);
        return VPPCOM_EINVAL;
    }
    ...
    if (vcm->cfg.use_mq_eventfd)
        return vppcom_epoll_wait_eventfd(wrk, events, maxevents, n_evts, wait_for_time);

    return vppcom_epoll_wait_condvar(wrk, events, maxevents, n_evts, wait_for_time);
}
...
vppcom_session_attr(uint32_t session_handle, uint32_t op, void *buffer, uint32_t *buflen)
{
    ...
    case VPPCOM_ATTR_GET_NREAD:
        rv = vppcom_session_read_ready(session);
        VDBG(2, "VCL<%d>: VPPCOM_ATTR_GET_NREAD: sid %u, nread = %d", getpid(), rv);
        break;

    case VPPCOM_ATTR_GET_NWRITE:
        rv = vppcom_session_write_ready(session);
        VDBG(2, "VCL<%d>: VPPCOM_ATTR_GET_NWRITE: sid %u, nwrite = %d", getpid(), session_handle, rv);
        break;
    ...
    case VPPCOM_ATTR_GET_FLAGS:
        if (PREDICT_TRUE(buffer && buflen && (*buflen >= sizeof(*flags)))) {
            ...
            *buflen = sizeof(*flags);
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_FLAGS: sid %u, flags = 0x%08x, "
                 "is_nonblocking = %u",
                 getpid(), session_handle, *flags, VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_NONBLOCK));
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_SET_FLAGS:
        if (PREDICT_TRUE(buffer && buflen && (*buflen == sizeof(*flags)))) {
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_FLAGS: sid %u, flags = 0x%08x,"
                 " is_nonblocking = %u",
                 getpid(), session_handle, *flags, VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_NONBLOCK));
        } else
            rv = VPPCOM_EINVAL;
    ...
    case VPPCOM_ATTR_GET_PEER_ADDR:
        if (PREDICT_TRUE(buffer && buflen && (*buflen >= sizeof(*ep)) && ep->ip)) {
            ...
            if (session->transport.is_ip4)
                clib_memcpy(ep->ip, &session->transport.rmt_ip.ip4, sizeof(ip4_address_t));
            else
                clib_memcpy(ep->ip, &session->transport.rmt_ip.ip6, sizeof(ip6_address_t));
            *buflen = sizeof(*ep);
            VDBG(1,
                 "VCL<%d>: VPPCOM_ATTR_GET_PEER_ADDR: sid %u, is_ip4 = %u, "
                 "addr = %U, port %u",
                 getpid(), session_handle, ep->is_ip4, format_ip46_address, &session->transport.rmt_ip,
    ...
    case VPPCOM_ATTR_GET_LCL_ADDR:
        if (PREDICT_TRUE(buffer && buflen && (*buflen >= sizeof(*ep)) && ep->ip)) {
            ...
            if (session->transport.is_ip4)
                clib_memcpy(ep->ip, &session->transport.lcl_ip.ip4, sizeof(ip4_address_t));
            else
                clib_memcpy(ep->ip, &session->transport.lcl_ip.ip6, sizeof(ip6_address_t));
            *buflen = sizeof(*ep);
            VDBG(1,
                 "VCL<%d>: VPPCOM_ATTR_GET_LCL_ADDR: sid %u, is_ip4 = %u,"
                 " addr = %U port %d",
                 getpid(), session_handle, ep->is_ip4, format_ip46_address, &session->transport.lcl_ip,
                 ep->is_ip4 ? IP46_TYPE_IP4 : IP46_TYPE_IP6, clib_net_to_host_u16(ep->port));
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_LIBC_EPFD:
        ...
        VDBG(2, "VCL<%d>: VPPCOM_ATTR_GET_LIBC_EPFD: libc_epfd %d", getpid(), rv);
        break;
    ...
    case VPPCOM_ATTR_SET_LIBC_EPFD:
        if (PREDICT_TRUE(buffer && buflen && (*buflen == sizeof(session->libc_epfd)))) {
            ...
            *buflen            = sizeof(session->libc_epfd);
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_LIBC_EPFD: libc_epfd %d, "
                 "buflen %d",
                 getpid(), session->libc_epfd, *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_PROTOCOL:
        if (buffer && buflen && (*buflen >= sizeof(int))) {
            ...
            *buflen         = sizeof(int);
            ...
            VDBG(2, "VCL<%d>: VPPCOM_ATTR_GET_PROTOCOL: %d (%s), buflen %d", getpid(), *(int *) buffer,
    ...
    case VPPCOM_ATTR_GET_LISTEN:
        if (buffer && buflen && (*buflen >= sizeof(int))) {
            *(int *) buffer = VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_LISTEN);
            *buflen         = sizeof(int);

            VDBG(2, "VCL<%d>: VPPCOM_ATTR_GET_LISTEN: %d, buflen %d", getpid(), *(int *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_ERROR:
        if (buffer && buflen && (*buflen >= sizeof(int))) {
            ...
            *buflen         = sizeof(int);
            ...
            VDBG(2, "VCL<%d>: VPPCOM_ATTR_GET_ERROR: %d, buflen %d, #VPP-TBD#", getpid(), *(int *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
        ...
        if (buffer && buflen && (*buflen >= sizeof(u32))) {
            ...
            *(size_t *) buffer =
                (session->sndbuf_size ? session->sndbuf_size :
                                        session->tx_fifo ? session->tx_fifo->nitems : vcm->cfg.tx_fifo_size);
            *buflen = sizeof(u32);

            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_TX_FIFO_LEN: %u (0x%x), "
                 "buflen %d, #VPP-TBD#",
                 getpid(), *(size_t *) buffer, *(size_t *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_SET_TX_FIFO_LEN:
        if (buffer && buflen && (*buflen == sizeof(u32))) {
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_TX_FIFO_LEN: %u (0x%x), "
                 "buflen %d, #VPP-TBD#",
                 getpid(), session->sndbuf_size, session->sndbuf_size, *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_RX_FIFO_LEN:
        if (buffer && buflen && (*buflen >= sizeof(u32))) {
            ...
            /* VPP-TBD */
            *(size_t *) buffer =
                (session->rcvbuf_size ? session->rcvbuf_size :
                                        session->rx_fifo ? session->rx_fifo->nitems : vcm->cfg.rx_fifo_size);
            *buflen = sizeof(u32);

            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_RX_FIFO_LEN: %u (0x%x), "
                 "buflen %d, #VPP-TBD#",
                 getpid(), *(size_t *) buffer, *(size_t *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
    ...
    case VPPCOM_ATTR_SET_RX_FIFO_LEN:
        if (buffer && buflen && (*buflen == sizeof(u32))) {
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_RX_FIFO_LEN: %u (0x%x), "
                 "buflen %d, #VPP-TBD#",
                 getpid(), session->sndbuf_size, session->sndbuf_size, *buflen);
        } else
            rv = VPPCOM_EINVAL;
    ...
    case VPPCOM_ATTR_GET_REUSEADDR:
        if (buffer && buflen && (*buflen >= sizeof(int))) {
            /* VPP-TBD */
            *(int *) buffer = VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_REUSEADDR);
            *buflen         = sizeof(int);

            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_REUSEADDR: %d, "
                 "buflen %d, #VPP-TBD#",
    ...
    case VPPCOM_ATTR_SET_REUSEADDR:
        if (buffer && buflen && (*buflen == sizeof(int)) && !VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_LISTEN)) {
            /* VPP-TBD */
            if (*(int *) buffer)
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_REUSEADDR: %d, buflen %d,"
                 " #VPP-TBD#",
                 getpid(), VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_REUSEADDR), *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_REUSEPORT:
        if (buffer && buflen && (*buflen >= sizeof(int))) {
            /* VPP-TBD */
            *(int *) buffer = VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_REUSEPORT);
            *buflen         = sizeof(int);

            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_REUSEPORT: %d, buflen %d,"
                 " #VPP-TBD#",
                 getpid(), *(int *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
    ...
    case VPPCOM_ATTR_SET_REUSEPORT:
        if (buffer && buflen && (*buflen == sizeof(int)) && !VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_LISTEN)) {
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_REUSEPORT: %d, buflen %d,"
                 " #VPP-TBD#",
                 getpid(), VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_REUSEPORT), *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_BROADCAST:
        if (buffer && buflen && (*buflen >= sizeof(int))) {
            /* VPP-TBD */
            *(int *) buffer = VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_BROADCAST);
            *buflen         = sizeof(int);

            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_BROADCAST: %d, buflen %d,"
                 " #VPP-TBD#",
                 getpid(), *(int *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
    ...
    case VPPCOM_ATTR_SET_BROADCAST:
        if (buffer && buflen && (*buflen == sizeof(int))) {
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_BROADCAST: %d, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_BROADCAST), *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_V6ONLY:
        if (buffer && buflen && (*buflen >= sizeof(int))) {
            /* VPP-TBD */
            *(int *) buffer = VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_V6ONLY);
            *buflen         = sizeof(int);

            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_V6ONLY: %d, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), *(int *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
    ...
    case VPPCOM_ATTR_SET_V6ONLY:
        if (buffer && buflen && (*buflen == sizeof(int))) {
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_V6ONLY: %d, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_V6ONLY), *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_KEEPALIVE:
        if (buffer && buflen && (*buflen >= sizeof(int))) {
            /* VPP-TBD */
            *(int *) buffer = VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_KEEPALIVE);
            *buflen         = sizeof(int);

            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_KEEPALIVE: %d, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), *(int *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
    ...
    case VPPCOM_ATTR_SET_KEEPALIVE:
        if (buffer && buflen && (*buflen == sizeof(int))) {
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_KEEPALIVE: %d, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_KEEPALIVE), *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_TCP_NODELAY:
        if (buffer && buflen && (*buflen >= sizeof(int))) {
            /* VPP-TBD */
            *(int *) buffer = VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_TCP_NODELAY);
            *buflen         = sizeof(int);

            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_TCP_NODELAY: %d, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), *(int *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
    ...
    case VPPCOM_ATTR_SET_TCP_NODELAY:
        if (buffer && buflen && (*buflen == sizeof(int))) {
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_TCP_NODELAY: %d, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_TCP_NODELAY), *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_TCP_KEEPIDLE:
        if (buffer && buflen && (*buflen >= sizeof(int))) {
            /* VPP-TBD */
            *(int *) buffer = VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_TCP_KEEPIDLE);
            *buflen         = sizeof(int);

            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_TCP_KEEPIDLE: %d, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), *(int *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
    ...
    case VPPCOM_ATTR_SET_TCP_KEEPIDLE:
        if (buffer && buflen && (*buflen == sizeof(int))) {
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_TCP_KEEPIDLE: %d, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_TCP_KEEPIDLE), *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_TCP_KEEPINTVL:
        if (buffer && buflen && (*buflen >= sizeof(int))) {
            /* VPP-TBD */
            *(int *) buffer = VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_TCP_KEEPINTVL);
            *buflen         = sizeof(int);

            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_TCP_KEEPINTVL: %d, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), *(int *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
    ...
    case VPPCOM_ATTR_SET_TCP_KEEPINTVL:
        if (buffer && buflen && (*buflen == sizeof(int))) {
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_TCP_KEEPINTVL: %d, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_TCP_KEEPINTVL), *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
    ...
    case VPPCOM_ATTR_GET_TCP_USER_MSS:
        if (buffer && buflen && (*buflen >= sizeof(u32))) {
            ...
            *buflen         = sizeof(int);
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_GET_TCP_USER_MSS: %d, buflen %d,"
                 " #VPP-TBD#",
                 getpid(), *(int *) buffer, *buflen);
        } else
            rv = VPPCOM_EINVAL;
    ...
    case VPPCOM_ATTR_SET_TCP_USER_MSS:
        if (buffer && buflen && (*buflen == sizeof(u32))) {
            ...
            VDBG(2,
                 "VCL<%d>: VPPCOM_ATTR_SET_TCP_USER_MSS: %u, buflen %d, "
                 "#VPP-TBD#",
                 getpid(), session->user_mss, *buflen);
        } else
            rv = VPPCOM_EINVAL;
        break;
...
int
vppcom_session_recvfrom(uint32_t session_handle, void *buffer, uint32_t buflen, int flags, vppcom_endpt_t *ep)
{
    vcl_worker_t *wrk      = vcl_worker_get_current();
    int rv                 = VPPCOM_OK;
    vcl_session_t *session = 0;
    ...
    if (ep) {
        session = vcl_session_get_w_handle(wrk, session_handle);
        if (PREDICT_FALSE(!session)) {
            VDBG(0, "VCL<%d>: invalid session, sid (%u) has been closed!", getpid(), session_handle);
            return VPPCOM_EBADFD;
        }
        ep->is_ip4 = session->transport.is_ip4;
        ep->port   = session->transport.rmt_port;
    ...
    if (flags == 0)
        ...
        clib_warning("VCL<%d>: Unsupport flags for recvfrom %d", getpid(), flags);
...
int
vppcom_session_sendto(uint32_t session_handle, void *buffer, uint32_t buflen, int flags, vppcom_endpt_t *ep)
{
    if (!buffer)
        return VPPCOM_EINVAL;
    ...
    if (flags) {
        ...
        VDBG(2, "VCL<%d>: handling flags 0x%u (%d) not implemented yet.", getpid(), flags, flags);
    }
...
vppcom_poll(vcl_poll_t *vp, uint32_t n_sids, double wait_for_time)
{
    ...
    VDBG(3, "VCL<%d>: vp %p, nsids %u, wait_for_time %f", getpid(), vp, n_sids, wait_for_time);
    ...
    if (VPPCOM_DEBUG > 3) {
        ...
        for (i = 0; i < n_sids; i++) {
            clib_warning("VCL<%d>: vp[%d].sid %d (0x%x), .events 0x%x, "
                         ".revents 0x%x",
                         getpid(), i, vp[i].sid, vp[i].sid, vp[i].events, *vp[i].revents);
        }
    }
    return num_ev;
}
