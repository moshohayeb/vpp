...
static inline int
sock_test_write(int fd, uint8_t *buf, uint32_t nbytes, vcl_test_stats_t *stats, uint32_t verbose)
{
    int tx_bytes    = 0;
    ...
    if (tx_bytes < 0) {
        ...
        fprintf(stderr,
                "SOCK_TEST: ERROR: socket write failed "
                "(errno = %d)!\n",
                errno_val);
    } else if (stats)
