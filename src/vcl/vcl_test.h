...
vcl_test_cfg_verify(vcl_test_cfg_t *cfg, vcl_test_cfg_t *valid_cfg)
{
    ...
    return ((cfg->magic == valid_cfg->magic) && (cfg->test == valid_cfg->test) &&
            ...
            (cfg->num_writes == valid_cfg->num_writes) && (cfg->total_bytes == valid_cfg->total_bytes));
}

static inline void
vcl_test_buf_alloc(vcl_test_cfg_t *cfg, uint8_t is_rxbuf, uint8_t **buf, uint32_t *bufsize)
{
...
vcl_test_session_buf_alloc(vcl_test_session_t *socket)
{
    ...
    vcl_test_buf_alloc(&socket->cfg, 0 /* is_rxbuf */, (uint8_t **) &socket->txbuf, &socket->txbuf_size);
    vcl_test_buf_alloc(&socket->cfg, 1 /* is_rxbuf */, (uint8_t **) &socket->rxbuf, &socket->rxbuf_size);
}
...
vcl_test_cfg_dump(vcl_test_cfg_t *cfg, uint8_t is_client)
{
    ...
    printf("  test config (%p):\n" VCL_TEST_SEPARATOR_STRING "                 magic:  0x%08x\n"
           ...
           is_client && (cfg->test == VCL_TEST_TYPE_UNI) ?
               ...
               is_client && (cfg->test == VCL_TEST_TYPE_BI) ? "'" VCL_TEST_TOKEN_RUN_BI "'" : spc,
           vcl_test_type_str(cfg->test), cfg->test, cfg->ctrl_handle, cfg->ctrl_handle,
           is_client ? "'" VCL_TEST_TOKEN_NUM_TEST_SESS "'" : spc, cfg->num_test_sessions, cfg->num_test_sessions,
           is_client ? "'" VCL_TEST_TOKEN_VERBOSE "'" : spc, cfg->verbose ? "on" : "off", cfg->verbose,
           is_client ? "'" VCL_TEST_TOKEN_RXBUF_SIZE "'" : spc, cfg->rxbuf_size, cfg->rxbuf_size,
           is_client ? "'" VCL_TEST_TOKEN_TXBUF_SIZE "'" : spc, cfg->txbuf_size, cfg->txbuf_size,
           is_client ? "'" VCL_TEST_TOKEN_NUM_WRITES "'" : spc, cfg->num_writes, cfg->num_writes, cfg->total_bytes,
           cfg->total_bytes);
}

static inline void
vcl_test_stats_dump(char *header, vcl_test_stats_t *stats, uint8_t show_rx, uint8_t show_tx, uint8_t verbose)
{
    ...
    printf("\n%s: Streamed %lu bytes\n"
           ...
           header, total_bytes, duration, rate, (show_rx && show_tx) ? "full" : "half");

    if (show_tx) {
        printf(VCL_TEST_SEPARATOR_STRING "  tx stats (0x%p):\n" VCL_TEST_SEPARATOR_STRING
                                         "         writes:  %lu (0x%08lx)\n"
                                         "       tx bytes:  %lu (0x%08lx)\n"
                                         "      tx eagain:  %u (0x%08x)\n"
                                         "  tx incomplete:  %u (0x%08x)\n",
               (void *) stats, stats->tx_xacts, stats->tx_xacts, stats->tx_bytes, stats->tx_bytes, stats->tx_eagain,
               stats->tx_eagain, stats->tx_incomp, stats->tx_incomp);
    }
    if (show_rx) {
        printf(VCL_TEST_SEPARATOR_STRING "  rx stats (0x%p):\n" VCL_TEST_SEPARATOR_STRING
                                         "          reads:  %lu (0x%08lx)\n"
                                         "       rx bytes:  %lu (0x%08lx)\n"
                                         "      rx eagain:  %u (0x%08x)\n"
                                         "  rx incomplete:  %u (0x%08x)\n",
               (void *) stats, stats->rx_xacts, stats->rx_xacts, stats->rx_bytes, stats->rx_bytes, stats->rx_eagain,
    ...
    if (verbose)
        printf("   start.tv_sec:  %ld\n"
               ...
               "    stop.tv_sec:  %ld\n"
               "   stop.tv_nsec:  %ld\n",
               stats->start.tv_sec, stats->start.tv_nsec, stats->stop.tv_sec, stats->stop.tv_nsec);
...
static inline int
vcl_test_read(int fd, uint8_t *buf, uint32_t nbytes, vcl_test_stats_t *stats)
{
...
static inline int
vcl_test_write(int fd, uint8_t *buf, uint32_t nbytes, vcl_test_stats_t *stats, uint32_t verbose)
{
