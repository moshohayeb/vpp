...
vcl_wait_for_app_state_change(app_state_t app_state)
{
    ...
    VDBG(0, "VCL<%d>: timeout waiting for state %s (%d)", getpid(), vppcom_app_state_str(app_state), app_state);
...
u32
vcl_ct_registration_index(vcl_worker_t *wrk, vcl_cut_through_registration_t *ctr)
{
...
void
vcl_ct_registration_del(vcl_worker_t *wrk, vcl_cut_through_registration_t *ctr)
{
