...
vl_api_application_attach_reply_t_handler(vl_api_application_attach_reply_t *mp)
{
    ...
    if (mp->retval) {
        clib_warning("VCL<%d>: attach failed: %U", getpid(), format_api_error, ntohl(mp->retval));
        return;
    }

    wrk->app_event_queue = uword_to_pointer(mp->app_event_queue_address, svm_msg_q_t *);
    if (mp->n_fds) {
        vec_validate(fds, mp->n_fds);
        vl_socket_client_recv_fd_msg(fds, mp->n_fds, 5);

        if (mp->fd_flags & SESSION_FD_F_VPP_MQ_SEGMENT)
            if (ssvm_segment_attach("vpp-mq-seg", SSVM_SEGMENT_MEMFD, fds[n_fds++]))
                return;
...
static void
vl_api_app_worker_add_del_reply_t_handler(vl_api_app_worker_add_del_reply_t *mp)
{
    int n_fds = 0, *fds = 0;
    vcl_worker_t *wrk;
    u32 wrk_index;
    ...
    if (mp->context != wrk_index) {
        clib_warning("VCL<%d>: wrk numbering doesn't match ours: %u, vpp: %u", getpid(), mp->context, wrk_index);
        goto failed;
    }
    if (!mp->is_add)
        return;
    ...
    if (mp->n_fds) {
        ...
        if (mp->fd_flags & SESSION_FD_F_VPP_MQ_SEGMENT)
            if (ssvm_segment_attach("vpp-worker-seg", SSVM_SEGMENT_MEMFD, fds[n_fds++]))
                goto failed;
...
{
    if (mp->retval)
        clib_warning("VCL<%d>: detach failed: %U", getpid(), format_api_error, ntohl(mp->retval));
...
vl_api_map_another_segment_t_handler(vl_api_map_another_segment_t *mp)
{
    ...
    if (PREDICT_FALSE(ssvm_segment_attach((char *) mp->segment_name, seg_type, fd))) {
        clib_warning("VCL<%d>: svm_fifo_segment_attach ('%s') failed", getpid(), mp->segment_name);
        return;
    }

    VDBG(1, "VCL<%d>: mapped new segment '%s' size %d", getpid(), mp->segment_name, mp->segment_size);
...
vl_api_app_cut_through_registration_add_t_handler(vl_api_app_cut_through_registration_add_t *mp)
{
    ...
    vcl_ct_registration_lookup_add(wrk, mp->evt_q_address, vcl_ct_registration_index(wrk, ctr));
...
vl_api_bind_sock_reply_t_handler(vl_api_bind_sock_reply_t *mp)
{
    ...
    VDBG(1, "VCL<%d>: bapi msg vpp handle 0x%llx, sid %u: bind retval: %u!", getpid(), mp->handle, mp->context,
         mp->retval);
}
...
{
    if (mp->retval)
        clib_warning("VCL<%d>: ERROR: sid %u: unbind failed: %U", getpid(), mp->context, format_api_error,
                     ntohl(mp->retval));
...
{
    if (mp->retval)
        clib_warning("VCL<%d>: ERROR: sid %u: disconnect failed: %U", getpid(), mp->context, format_api_error,
                     ntohl(mp->retval));
}
...
{
    if (mp->retval)
        clib_warning("VCL<%d>: ERROR: sid %u: connect failed: %U", getpid(), mp->context, format_api_error,
                     ntohl(mp->retval));
}
...
vppcom_app_send_attach(void)
{
    ...
    bmp->client_index               = vcm->my_client_index;
    bmp->context                    = htonl(0xfeedface);
    bmp->options[APP_OPTIONS_FLAGS] = APP_OPTIONS_FLAGS_ACCEPT_REDIRECT | APP_OPTIONS_FLAGS_ADD_SEGMENT |
    ...
    bmp->options[APP_OPTIONS_PROXY_TRANSPORT] = (u64)((vcm->cfg.app_proxy_transport_tcp ? 1 << TRANSPORT_PROTO_TCP : 0) |
...
vppcom_connect_to_vpp(char *app_name)
{
    ...
    if (vcl_cfg->vpp_api_socket_name) {
        if (vl_socket_client_connect((char *) vcl_cfg->vpp_api_socket_name, app_name, 0 /* default rx/tx buffer */)) {
            clib_warning("VCL<%d>: app (%s) socket connect failed!", getpid(), app_name);
            return VPPCOM_ECONNREFUSED;
        }

        if (vl_socket_client_init_shm(0)) {
            clib_warning("VCL<%d>: app (%s) init shm failed!", getpid(), app_name);
            return VPPCOM_ECONNREFUSED;
        }
    } else {
        ...
        VDBG(0, "VCL<%d>: app (%s) connecting to VPP api (%s)...", getpid(), app_name, vcl_cfg->vpp_api_filename);

        if (vl_client_connect_to_vlib((char *) vcl_cfg->vpp_api_filename, app_name, vcm->cfg.vpp_api_q_length) < 0) {
