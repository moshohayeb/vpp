...
sock_test_cfg_sync(vcl_test_session_t *socket)
{
    ...
    rx_bytes = sock_test_read(socket->fd, (uint8_t *) socket->rxbuf, sizeof(vcl_test_cfg_t), NULL);
    if (rx_bytes < 0)
        return rx_bytes;

    if (rl_cfg->magic != VCL_TEST_CFG_CTRL_MAGIC) {
        fprintf(stderr,
                "CLIENT (fd %d): ERROR: Bad server reply cfg "
                "-- aborting!\n",
                socket->fd);
        return -1;
    }
    if ((rx_bytes != sizeof(vcl_test_cfg_t)) || !vcl_test_cfg_verify(rl_cfg, &ctrl->cfg)) {
        fprintf(stderr,
                "CLIENT (fd %d): ERROR: Invalid config received "
                "from server!\n",
                socket->fd);
...
echo_test_client()
{
    ...
    while (n) {
...
#ifdef VCL_TEST
        rv = vppcom_select(nfds, (uint64_t *) rfdset, (uint64_t *) wfdset, NULL, 0);
#else
        {
        ...
        for (i = 0; i < ctrl->cfg.num_test_sessions; i++) {
            ...
            if (!((tsock->stats.stop.tv_sec == 0) && (tsock->stats.stop.tv_nsec == 0)))
                continue;
...
#ifndef VCL_TEST
    {
        ...
        if (rv < 0) {
            ...
            fprintf(stderr,
                    "CLIENT: ERROR: connect(fd %d, \"%s\", %lu) "
                    "failed (errno = %d)!\n",
                    fd, SOCK_TEST_AF_UNIX_FILENAME, SUN_LEN(&serveraddr), errno_val);
            goto done;
        }
    ...
    if (ctrl->cfg.verbose) {
        ...
        if (ctrl->cfg.verbose > 1) {
            printf("  ctrl socket info\n" VCL_TEST_SEPARATOR_STRING "          fd:  %d (0x%08x)\n"
                   ...
                   ctrl->fd, (uint32_t) ctrl->fd, ctrl->rxbuf, ctrl->rxbuf_size, ctrl->rxbuf_size, ctrl->txbuf,
                   ctrl->txbuf_size, ctrl->txbuf_size);
        }
    }
}
...
stream_test_client(vcl_test_t test)
{
    ...
    while (n) {
        ...
        for (i = 0; i < ctrl->cfg.num_test_sessions; i++) {
            ...
            if ((test == VCL_TEST_TYPE_BI) && FD_ISSET(tsock->fd, rfdset) &&
                (tsock->stats.rx_bytes < ctrl->cfg.total_bytes)) {
                (void) sock_test_read(tsock->fd, (uint8_t *) tsock->rxbuf, tsock->rxbuf_size, &tsock->stats);
            }

            if (FD_ISSET(tsock->fd, wfdset) && (tsock->stats.tx_bytes < ctrl->cfg.total_bytes)) {
                tx_bytes = sock_test_write(tsock->fd, (uint8_t *) tsock->txbuf, ctrl->cfg.txbuf_size, &tsock->stats,
                                           ctrl->cfg.verbose);
                if (tx_bytes < 0) {
                    fprintf(stderr,
                            "\nCLIENT: ERROR: sock_test_write(%d) "
            ...
            if (((test == VCL_TEST_TYPE_UNI) && (tsock->stats.tx_bytes >= ctrl->cfg.total_bytes)) ||
                ((test == VCL_TEST_TYPE_BI) && (tsock->stats.rx_bytes >= ctrl->cfg.total_bytes))) {
                clock_gettime(CLOCK_REALTIME, &tsock->stats.stop);
                n--;
            }
        }
    ...
    for (i = 0; i < ctrl->cfg.num_test_sessions; i++) {
        ...
        if (ctrl->cfg.verbose) {
            ...
            sprintf(buf, "CLIENT (fd %d) RESULTS", tsock->fd);
            vcl_test_stats_dump(buf, &tsock->stats, test == VCL_TEST_TYPE_BI /* show_rx */, 1 /* show tx */,
                                ctrl->cfg.verbose);
        }
    ...
    vcl_test_stats_dump("CLIENT RESULTS", &ctrl->stats, test == VCL_TEST_TYPE_BI /* show_rx */, 1 /* show tx */,
                        ctrl->cfg.verbose);
    vcl_test_cfg_dump(&ctrl->cfg, 1 /* is_client */);
...
sock_test_connect_test_sockets(uint32_t num_test_sockets)
{
    ...
    if (num_test_sockets < scm->num_test_sockets) {
        ...
        for (i = scm->num_test_sockets; i < num_test_sockets; i++) {
            tsock = &scm->test_socket[i];
#ifdef VCL_TEST
            tsock->fd = vppcom_session_create(ctrl->cfg.transport_udp ? VPPCOM_PROTO_UDP : VPPCOM_PROTO_TCP,
                                              1 /* is_nonblocking */);
            if (tsock->fd < 0) {
                errno     = -tsock->fd;
                tsock->fd = -1;
...
#define INDENT "\n  "
    ...
    printf("CLIENT: Test configuration commands:" INDENT VCL_TEST_TOKEN_HELP
           "\t\t\tDisplay help." INDENT VCL_TEST_TOKEN_EXIT
           "\t\t\tExit test client & server." INDENT VCL_TEST_TOKEN_SHOW_CFG
           "\t\t\tShow the current test cfg." INDENT VCL_TEST_TOKEN_RUN_UNI
           "\t\t\tRun the Uni-directional test." INDENT VCL_TEST_TOKEN_RUN_BI
...
cfg_txbuf_size_set(void)
{
    ...
    if (txbuf_size >= VCL_TEST_CFG_BUF_SIZE_MIN) {
        ...
        fprintf(stderr,
                "CLIENT: ERROR: Invalid txbuf size (%lu) < "
                "minimum buf size (%u)!\n",
...
cfg_num_test_sockets_set(void)
{
    ...
    if ((num_test_sockets > 0) && (num_test_sockets <= VCL_TEST_CFG_MAX_TEST_SESS)) {
        ...
        fprintf(stderr,
                "CLIENT: ERROR: invalid num test sockets: "
                "%u, (%d max)\n",
...
cfg_rxbuf_size_set(void)
{
    ...
    if (rxbuf_size >= VCL_TEST_CFG_BUF_SIZE_MIN) {
        ...
        fprintf(stderr,
                "CLIENT: ERROR: Invalid rxbuf size (%lu) < "
                "minimum buf size (%u)!\n",
...
parse_input()
{
    ...
    else if (!strncmp(VCL_TEST_TOKEN_RUN_UNI, ctrl->txbuf, strlen(VCL_TEST_TOKEN_RUN_UNI)))
        rv = ctrl->cfg.test = VCL_TEST_TYPE_UNI;

    else if (!strncmp(VCL_TEST_TOKEN_RUN_BI, ctrl->txbuf, strlen(VCL_TEST_TOKEN_RUN_BI)))
...
{
    fprintf(stderr, "sock_test_client [OPTIONS] <ipaddr> <port>\n"
                    "  OPTIONS\n"
...
main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "chn:w:XE:I:N:R:T:UBV6D")) != -1)
        ...
        case 's':
            if (sscanf(optarg, "0x%x", &ctrl->cfg.num_test_sessions) != 1)
                if (sscanf(optarg, "%u", &ctrl->cfg.num_test_sessions) != 1) {
                    fprintf(stderr,
                            "CLIENT: ERROR: Invalid value for "
                            "option -%c!\n",
                            c);
            ...
            if (!ctrl->cfg.num_test_sessions || (ctrl->cfg.num_test_sessions > FD_SETSIZE)) {
                fprintf(stderr,
                        "CLIENT: ERROR: Invalid number of "
        ...
        case 'I':
            ...
            if (ctrl->cfg.num_test_sessions > VCL_TEST_CFG_MAX_TEST_SESS) {
                fprintf(stderr,
                        "CLIENT: ERROR: value greater than max "
    ...
    if (ctrl->fd < 0) {
        ...
        ctrl->fd = vppcom_session_create(ctrl->cfg.transport_udp ? VPPCOM_PROTO_UDP : VPPCOM_PROTO_TCP,
                                         0 /* is_nonblocking */);
        if (ctrl->fd < 0) {
            errno    = -ctrl->fd;
            ctrl->fd = -1;
