...
conn_pool_expand(size_t expand_size)
{
    ...
    if (conn_pool) {
        ...
        fprintf(stderr,
                "SERVER: ERROR: Memory allocation "
                "failed (errno = %d)!\n",
                errno_val);
    }
}
...
static void
stream_test_server_start_stop(sock_server_conn_t *conn, vcl_test_cfg_t *rx_cfg)
{
    sock_server_main_t *ssm = &sock_server_main;
    ...
    if (rx_cfg->ctrl_handle == conn->fd) {
        ...
        vcl_test_stats_dump("SERVER RESULTS", &conn->stats, 1 /* show_rx */, (test == VCL_TEST_TYPE_BI) /* show_tx */,
                            conn->cfg.verbose);
        vcl_test_cfg_dump(&conn->cfg, 0 /* is_client */);
        if (conn->cfg.verbose) {
...
stream_test_server(sock_server_conn_t *conn, int rx_bytes)
{
    ...
    if (test == VCL_TEST_TYPE_BI)
        (void) sock_test_write(client_fd, conn->buf, rx_bytes, &conn->stats, conn->cfg.verbose);
...
af_unix_echo(void)
{
...
#if HAVE_ACCEPT4
    af_unix_client_fd = accept4(ssm->af_unix_listen_fd, (struct sockaddr *) NULL, NULL, NULL);
#else
    ...
    if (rv < 0) {
        ...
        fprintf(stderr,
                "SERVER: ERROR: read(af_unix_client_fd %d (0x%x), "
                "\"%s\", nbytes %lu) failed (errno = %d)!\n",
                af_unix_client_fd, af_unix_client_fd, buffer, nbytes, errno_val);
        goto done;
    }
...
main(int argc, char **argv)
{
    ...
    if (rv) {
        ...
        ssm->listen_fd =
            vppcom_session_create(ssm->cfg.transport_udp ? VPPCOM_PROTO_UDP : VPPCOM_PROTO_TCP, 0 /* is_nonblocking */);
    }
#else
    ssm->listen_fd =
        socket(ssm->cfg.address_ip6 ? AF_INET6 : AF_INET, ssm->cfg.transport_udp ? SOCK_DGRAM : SOCK_STREAM, 0);
#if SOCK_SERVER_USE_EPOLL && !defined(VCL_TEST)
    ...
    if (rv < 0) {
        ...
        fprintf(stderr,
                "SERVER: ERROR: bind() fd %d, \"%s\": "
                "failed (errno = %d)!\n",
                ssm->af_unix_listen_fd, SOCK_TEST_AF_UNIX_FILENAME, errno_val);
...
#ifdef VCL_TEST
    rv                      = vppcom_epoll_ctl(ssm->epfd, EPOLL_CTL_ADD, ssm->listen_fd, &ssm->listen_ev);
    if (rv < 0)
        errno = -rv;
#else
    ...
    if (rv < 0) {
        ...
        fprintf(stderr,
                "SERVER: ERROR: mixed epoll_ctl(epfd %d (0x%x), "
                "EPOLL_CTL_ADD, af_unix_listen_fd %d (0x%x), EPOLLIN) failed "
                "(errno = %d)!\n",
                ssm->epfd, ssm->epfd, ssm->af_unix_listen_fd, ssm->af_unix_listen_fd, errno_val);
        close(ssm->af_unix_listen_fd);
        unlink((const char *) SOCK_TEST_AF_UNIX_FILENAME);
        return rv;
    }
    ...
    while (1) {
        ...
        int num_ev;
#ifdef VCL_TEST
        num_ev = vppcom_epoll_wait(ssm->epfd, ssm->wait_events, SOCK_SERVER_MAX_EPOLL_EVENTS, 60.0);
        if (num_ev < 0)
            errno = -num_ev;
#else
        ...
        for (i = 0; i < num_ev; i++) {
            ...
            read_again:
                ...
                if (rx_bytes > 0) {
                    ...
                    if (rx_cfg->magic == VCL_TEST_CFG_CTRL_MAGIC) {
                        ...
                        if (rx_bytes != sizeof(*rx_cfg)) {
                            ...
                            sock_test_write(client_fd, (uint8_t *) &conn->cfg, sizeof(conn->cfg), NULL,
                                            conn->cfg.verbose);
                            continue;
                        }
                    ...
                    else if ((conn->cfg.test == VCL_TEST_TYPE_UNI) || (conn->cfg.test == VCL_TEST_TYPE_BI)) {
                        ...
                        ((char *) conn->buf)[rx_bytes < conn->buf_size ? rx_bytes : conn->buf_size - 1] = 0;
                ...
                if (isascii(conn->buf[0])) {
                    ...
                    tx_bytes = sock_test_write(client_fd, conn->buf, nbytes, &conn->stats, conn->cfg.verbose);
                    if (tx_bytes >= 0)
