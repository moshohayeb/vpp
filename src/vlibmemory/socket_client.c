...
vl_socket_client_read(int wait)
{
    ...
    while (1) {
        ...
        while (vec_len(scm->socket_rx_buffer) < msg_size) {
            n = read(scm->socket_fd, scm->socket_rx_buffer + vec_len(scm->socket_rx_buffer),
        ...
        if (vec_len(scm->socket_rx_buffer) >= data_len + sizeof(*mbp)) {
            ...
            if (vec_len(scm->socket_rx_buffer) == 0 && scm->control_pings_outstanding == 0)
                break;
        }
...
vl_sock_api_recv_fd_msg(int socket_fd, int fds[], int n_fds, u32 wait)
{
    ...
    if (size != 5) {
        return (size == 0) ? clib_error_return(0, "disconnected") :
                             clib_error_return_unix(0, "recvmsg: malformed message (fd %d)", socket_fd);
    }
...
int
vl_socket_client_connect(char *socket_path, char *client_name, u32 socket_buffer_size)
{
    socket_client_main_t *scm = &socket_client_main;
...
vl_socket_client_init_shm(vl_api_shm_elem_config_t *config)
{
    ...
    mp = vl_socket_client_msg_alloc(sizeof(*mp) + vec_len(config) * sizeof(u64));
