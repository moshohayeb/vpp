...
void
vl_socket_process_api_msg(clib_file_t *uf, vl_api_registration_t *rp, i8 *input_v)
{
...
vl_socket_read_ready(clib_file_t *uf)
{
    ...
    if (n <= 0 && errno != EAGAIN) {
        ...
        if (!pool_is_free(socket_main.registration_pool, rp)) {
            ...
            clib_warning("client index %d already free?", rp->vl_api_registration_pool_index);
        }
        return 0;
    }
    ...
    do {
        ...
        if (mbp_set == 0 || (msg_len + sizeof(msgbuf_t)) > vec_len(msg_buffer)) {
        save_and_split:
...
#undef _
    ...
    sock->flags = CLIB_SOCKET_F_IS_SERVER | CLIB_SOCKET_F_SEQPACKET | CLIB_SOCKET_F_ALLOW_GROUP_WRITE;
