                ...
                if (now - rv->gc_mark_timestamp > 10) {
                    if (CLIB_DEBUG > 0) {
                        ...
                        if (msg_id < vec_len(api_main.msg_names))
                            clib_warning("msg id %d name %s", (u32) msg_id, api_main.msg_names[msg_id]);
                    }
...
void
vl_init_shmem(svm_region_t *vlib_rp, vl_api_shm_elem_config_t *config, int is_vlib, int is_private_region)
{
    api_main_t *am            = &api_main;
...
vl_map_shmem(const char *region_name, int is_vlib)
{
    ...
    if (strstr(region_name, vpe_api_region_suffix)) {
        ...
        _vec_len(root_path) = (vec_len(root_path) - strlen(vpe_api_region_suffix));
