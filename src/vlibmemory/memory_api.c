...
vl_api_memclnt_create_internal(char *name, svm_queue_t *q)
{
    ...
    *regpp  = clib_mem_alloc(sizeof(vl_api_registration_t));
    ...
    return vl_msg_api_handle_from_index_and_epoch(regp->vl_api_registration_pool_index,
...
vl_api_memclnt_create_t_handler(vl_api_memclnt_create_t *mp)
{
    ...
    *regpp  = clib_mem_alloc(sizeof(vl_api_registration_t));
    ...
    if (am->serialized_message_table_in_shmem == 0)
        am->serialized_message_table_in_shmem = vl_api_serialize_message_table(am, 0);
    ...
    rp->index         = vl_msg_api_handle_from_index_and_epoch(regp->vl_api_registration_pool_index,
                                                       am->shmem_hdr->application_restarts);
...
vl_api_memclnt_delete_t_handler(vl_api_memclnt_delete_t *mp)
{
    ...
    if (epoch != (am->shmem_hdr->application_restarts & VL_API_EPOCH_MASK)) {
        clib_warning("Stale clnt delete index %d old epoch %d cur epoch %d", client_index, epoch,
...
send_memclnt_keepalive(vl_api_registration_t *regp, f64 now)
{
    ...
    mp->context = mp->client_index = vl_msg_api_handle_from_index_and_epoch(regp->vl_api_registration_pool_index,
                                                                            am->shmem_hdr->application_restarts);
...
static void
vl_mem_send_client_keepalive_w_reg(api_main_t *am, f64 now, vl_api_registration_t **regpp, u32 **dead_indices,
...
static inline int
void_mem_api_handle_msg_i(api_main_t *am, vlib_main_t *vm, vlib_node_runtime_t *node, svm_queue_t *q)
{
    uword mp;
...
{
    api_main_t *am = &api_main;
    return void_mem_api_handle_msg_i(am, vm, node, am->shmem_hdr->vl_input_queue);
}

int
vl_mem_api_handle_msg_private(vlib_main_t *vm, vlib_node_runtime_t *node, u32 reg_index)
{
    api_main_t *am                 = &api_main;
    vl_shmem_hdr_t *save_shmem_hdr = am->shmem_hdr;
...
format_api_message_rings(u8 *s, va_list *args)
{
    ...
    if (main_segment) {
        ...
        s = format(s, "%d application restarts, %d reclaimed msgs, %d garbage collects\n",
                   shmem_hdr->application_restarts, shmem_hdr->restart_reclaims, shmem_hdr->garbage_collects);
    }
    return s;
}

static clib_error_t *
vl_api_ring_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cli_cmd)
{
    int i;
    vl_shmem_hdr_t *shmem_hdr;
    api_main_t *am = &api_main;
