...
const char test_srv_crt_rsa[] = "-----BEGIN CERTIFICATE-----\r\n"
...
const char test_srv_key_rsa[] = "-----BEGIN PRIVATE KEY-----\r\n"
...
vl_api_application_attach_reply_t_handler(vl_api_application_attach_reply_t *mp)
{
    ...
    if (mp->retval) {
        clib_warning("attach failed: %U", format_api_error, clib_net_to_host_u32(mp->retval));
        goto failed;
    }
    ...
    em->our_event_queue = uword_to_pointer(mp->app_event_queue_address, svm_msg_q_t *);
...
connect_to_vpp(char *name)
{
    ...
    if (em->use_sock_api) {
        if (vl_socket_client_connect((char *) em->socket_name, name, 0 /* default rx, tx buffer */)) {
...
vl_api_map_another_segment_t_handler(vl_api_map_another_segment_t *mp)
{
    ...
    clib_warning("Mapped new segment '%s' size %d", mp->segment_name, mp->segment_size);
}
...
test_recv_bytes(session_t *s, u8 *rx_buf, u32 n_read)
{
    ...
    for (i = 0; i < n_read; i++) {
        if (rx_buf[i] != ((s->bytes_received + i) & 0xff)) {
            clib_warning("error at byte %lld, 0x%x not 0x%x", s->bytes_received + i, rx_buf[i],
...
void
client_handle_fifo_event_rx(echo_main_t *em, session_event_t *e, u8 *rx_buf)
{
    session_t *s;
...
send_test_chunk(echo_main_t *em, session_t *s)
{
    ...
    written          = svm_fifo_enqueue_nowait(tx_fifo, bytes_this_chunk, test_data + test_buf_offset);
    ...
    if (written > 0) {
        ...
        if (svm_fifo_set_event(tx_fifo))
            app_send_io_evt_to_vpp(s->vpp_evt_q, tx_fifo, FIFO_EVENT_APP_TX, 0 /* do wait for mutex */);
    }
}
...
session_accepted_handler(session_accepted_msg_t *mp)
{
    ...
    clib_warning("Accepted session from: %s:%d", ip_str, clib_net_to_host_u16(mp->port));
...
session_connected_handler(session_connected_msg_t *mp)
{
    ...
    if (mp->retval) {
        clib_warning("connection failed with code: %U", format_api_error, clib_net_to_host_u32(mp->retval));
        em->state = STATE_FAILED;
        return;
    }
    ...
    rv = pthread_create(&em->client_thread_handles[em->n_clients_connected], NULL /*attr */, client_thread_fn,
    ...
    clib_warning("session %u (0x%llx) connected with local ip %U port %d", session_index, mp->handle,
                 format_ip46_address, mp->lcl_ip, mp->is_ip4, clib_net_to_host_u16(mp->lcl_port));
}
...
session_reset_handler(session_reset_msg_t *mp)
{
    ...
    app_alloc_ctrl_evt_to_vpp(session->vpp_evt_q, app_evt, SESSION_CTRL_EVT_RESET_REPLY);
...
clients_run(echo_main_t *em)
{
    ...
    start_time = clib_time_now(&em->clib_time);
    while (em->n_clients_connected < em->n_clients && (clib_time_now(&em->clib_time) - start_time < timeout) &&
           em->state != STATE_FAILED)
    ...
    fformat(stdout, "%lld bytes (%lld mbytes, %lld gbytes) in %.2f seconds\n", em->tx_total,
            em->tx_total / (1ULL << 20), em->tx_total / (1ULL << 30), deltat);
...
vl_api_bind_uri_reply_t_handler(vl_api_bind_uri_reply_t *mp)
{
    ...
    if (mp->retval) {
        clib_warning("bind failed: %U", format_api_error, clib_net_to_host_u32(mp->retval));
        em->state = STATE_FAILED;
        return;
    }
...
server_handle_fifo_event_rx(echo_main_t *em, session_event_t *e)
{
    ...
    do {
        ...
        if (!em->no_return && n_read > 0) {
            offset = 0;
            do {
                rv = svm_fifo_enqueue_nowait(tx_fifo, n_read, &em->rx_buf[offset]);
                if (rv > 0) {
                    n_read -= rv;
                    offset += rv;
                }
            ...
            if (svm_fifo_set_event(tx_fifo))
                app_send_io_evt_to_vpp(session->vpp_evt_q, tx_fifo, FIFO_EVENT_APP_TX, SVM_Q_WAIT);
        }
...
static void
vl_api_application_tls_cert_add_reply_t_handler(vl_api_application_tls_cert_add_reply_t *mp)
{
    if (mp->retval)
...
static void
vl_api_application_tls_key_add_reply_t_handler(vl_api_application_tls_key_add_reply_t *mp)
{
    if (mp->retval)
