...
{
    session_endpoint_extended_t *sep = va_arg(*args, session_endpoint_extended_t *);
    u32 port;
    char *tmp;

    if (unformat(input, "%s://%U/%d", &tmp, unformat_ip4_address, &sep->ip.ip4, &port)) {
        sep->port   = clib_host_to_net_u16(port);
        sep->is_ip4 = 1;
        return 1;
    } else if (unformat(input, "%s://%U/%d", &tmp, unformat_ip6_address, &sep->ip.ip6, &port)) {
        sep->port   = clib_host_to_net_u16(port);
        sep->is_ip4 = 0;
        return 1;
    }
...
vl_api_application_attach_reply_t_handler(vl_api_application_attach_reply_t *mp)
{
    ...
    utm->our_event_queue = uword_to_pointer(mp->app_event_queue_address, svm_msg_q_t *);
    ...
    while (1) {
        do {
            ...
            if (utm->have_return || do_dequeue)
                actual_transfer = svm_fifo_dequeue_nowait(rx_fifo, vec_len(my_copy_buffer), my_copy_buffer);
            else {
        ...
        if (utm->have_return) {
            buffer_offset = 0;
            while (actual_transfer > 0) {
                rv = svm_fifo_enqueue_nowait(tx_fifo, actual_transfer, my_copy_buffer + buffer_offset);
                if (rv > 0) {
                    actual_transfer -= rv;
                    buffer_offset += rv;
...
session_accepted_handler(session_accepted_msg_t *mp)
{
    ...
    utm->vpp_event_queue = uword_to_pointer(mp->vpp_event_queue_address, svm_msg_q_t *);
    ...
    if (mp->server_event_queue_address) {
        ...
        rv = pthread_create(&utm->cut_through_thread_handle, NULL /*attr */, cut_through_thread_fn, 0);
        if (rv) {
    ...
    app_alloc_ctrl_evt_to_vpp(utm->vpp_event_queue, app_evt, SESSION_CTRL_EVT_ACCEPTED_REPLY);
...
session_disconnected_handler(session_disconnected_msg_t *mp)
{
    ...
    app_alloc_ctrl_evt_to_vpp(session->vpp_evt_q, app_evt, SESSION_CTRL_EVT_DISCONNECTED_REPLY);
...
session_connected_handler(session_connected_msg_t *mp)
{
    ...
    if (mp->client_event_queue_address) {
        ...
        utm->ct_event_queue            = uword_to_pointer(mp->client_event_queue_address, svm_msg_q_t *);
        ...
        unformat_free(input);
        clib_memcpy(&session->transport.rmt_ip, &sep->ip, sizeof(ip46_address_t));
...
send_test_chunk(udp_echo_main_t *utm, app_session_t *s, u32 bytes)
{
    ...
    bytes_this_chunk = clib_min(test_buf_len - test_buf_offset, utm->bytes_to_send);
    ...
    if (s->is_dgram)
        written = app_send_dgram_raw(s->tx_fifo, &s->transport, s->vpp_evt_q, test_data + test_buf_offset,
                                     bytes_this_chunk, et, SVM_Q_WAIT);
    else
        written =
            app_send_stream_raw(s->tx_fifo, s->vpp_evt_q, test_data + test_buf_offset, bytes_this_chunk, et, SVM_Q_WAIT);
...
client_send_data(udp_echo_main_t *utm, u32 session_index)
{
    ...
    vec_validate_aligned(utm->connect_test_data, 1024 * 1024 - 1, CLIB_CACHE_LINE_BYTES);
    ...
    clib_warning("%lld bytes (%lld mbytes, %lld gbytes) in %.2f seconds", utm->total_to_send,
                 utm->total_to_send / (1ULL << 20), utm->total_to_send / (1ULL << 30), delta);
    clib_warning("%.2f bytes/second %s", ((f64) utm->total_to_send) / (delta), transfer_type);
    clib_warning("%.4f gbit/second %s", (((f64) utm->total_to_send * 8.0) / delta / 1e9), transfer_type);
}
...
vl_api_map_another_segment_t_handler(vl_api_map_another_segment_t *mp)
{
    ...
    clib_warning("Mapped new segment '%s' size %d", seg->ssvm.name, seg->ssvm.ssvm_size);
...
static void
vl_api_app_cut_through_registration_add_t_handler(vl_api_app_cut_through_registration_add_t *mp)
{
}
...
server_handle_fifo_event_rx(udp_echo_main_t *utm, u32 session_index)
{
    ...
    do {
        ...
        if (session->is_dgram)
            n_read = app_recv_dgram_raw(rx_fifo, utm->rx_buf, max_transfer, &session->transport, 0, 0);
        else
            n_read = app_recv_stream_raw(rx_fifo, utm->rx_buf, max_transfer, 0, 0);
        ...
        if (utm->have_return && n_read > 0) {
            ...
            do {
                if (session->is_dgram)
                    rv = app_send_dgram_raw(tx_fifo, &session->transport, session->vpp_evt_q, &utm->rx_buf[offset],
                                            n_read, et, SVM_Q_WAIT);
                else
                    rv = app_send_stream_raw(tx_fifo, session->vpp_evt_q, &utm->rx_buf[offset], n_read, et, SVM_Q_WAIT);
                if (rv > 0) {
                    n_read -= rv;
                    offset += rv;
                }
