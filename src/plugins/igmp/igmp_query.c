...
igmp_handle_query(const igmp_query_args_t *args)
{
    ...
    if (igmp_membership_query_v3_is_geeral(args->query)) {
        ...
        if (IGMP_TIMER_ID_INVALID == config->timers[IGMP_CONFIG_TIMER_GENERAL_REPORT]) {
            ...
            IGMP_DBG("...general-query-rx: %U schedule for %f", format_vnet_sw_if_index_name, vnet_get_main(),
        ...
        IGMP_DBG("...group-query-rx: %U for (%U, %U)", format_vnet_sw_if_index_name, vnet_get_main(), args->sw_if_index,
        ...
        if (IGMP_TIMER_ID_INVALID != tid) {
            ...
            group->timers[IGMP_GROUP_TIMER_QUERY_REPLY] =
                igmp_timer_schedule(delay, igmp_group_index(group), igmp_send_group_report_v3, srcs);
        }
    }
}
