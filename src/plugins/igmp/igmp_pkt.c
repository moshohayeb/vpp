...
static vlib_buffer_t *
igmp_pkt_build_ip_header(igmp_pkt_build_t *bk, igmp_msg_type_t msg_type, const igmp_group_t *group)
{
    ip4_header_t *ip4;
    vlib_buffer_t *b;
    u8 *option;
    ...
    ip4_src_address_for_packet(&ip4_main.lookup_main, bk->sw_if_index, &ip4->src_address);
    ...
    case IGMP_MSG_QUERY:
        if (group != NULL)
            clib_memcpy(&ip4->dst_address, &group->key->ip4, sizeof(ip4_address_t));
        else
...
static vlib_buffer_t *
igmp_pkt_build_report_v3(igmp_pkt_build_report_t *br, const igmp_group_t *group)
{
...
igmp_pkt_report_v3_get_size(const igmp_group_t *group)
{
    ...
    return ((hash_elts(group->igmp_src_by_key[IGMP_FILTER_MODE_INCLUDE]) * sizeof(ip4_address_t)) +
            sizeof(igmp_membership_group_v3_t));
}

static igmp_membership_group_v3_t *
igmp_pkt_report_v3_append_group(igmp_pkt_build_report_t *br, const ip46_address_t *grp,
 ...
 *  different sources each time."
  */
static igmp_membership_group_v3_t *
igmp_pkt_report_v3_append_src(igmp_pkt_build_report_t *br, igmp_membership_group_v3_t *igmp_group,
                              const ip46_address_t *grp, igmp_membership_group_v3_type_t type, const ip46_address_t *src)
{
    vlib_buffer_t *b;
...
void
igmp_pkt_report_v3_add_report(igmp_pkt_build_report_t *br, const ip46_address_t *grp, const ip46_address_t *srcs,
                              igmp_membership_group_v3_type_t type)
{
    igmp_membership_group_v3_t *igmp_group;
...
void
igmp_pkt_report_v3_add_group(igmp_pkt_build_report_t *br, const igmp_group_t *group, igmp_membership_group_v3_type_t type)
{
    igmp_membership_group_v3_t *igmp_group;
    vlib_buffer_t *b;
    igmp_src_t *src;
...
void
igmp_pkt_query_v3_add_group(igmp_pkt_build_query_t *bq, const igmp_group_t *group, const ip46_address_t *srcs)
{
    vlib_buffer_t *b;
