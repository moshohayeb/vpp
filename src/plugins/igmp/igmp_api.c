...
vl_api_igmp_listen_t_handler(vl_api_igmp_listen_t *mp)
{
    ...
    rv = igmp_listen(vm, (mp->group.filter ? IGMP_FILTER_MODE_INCLUDE : IGMP_FILTER_MODE_EXCLUDE),
...
static void
send_igmp_details(unix_shared_memory_queue_t *q, igmp_main_t *im, igmp_config_t *config, igmp_group_t *group,
...
static void
igmp_config_dump(igmp_main_t *im, unix_shared_memory_queue_t *q, u32 context, igmp_config_t *config)
{
    igmp_group_t *group;
    igmp_src_t *src;
...
vl_api_igmp_dump_t_handler(vl_api_igmp_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        /* *INDENT-OFF* */
        pool_foreach(config, im->configs, ({ igmp_config_dump(im, q, mp->context, config); }));
        /* *INDENT-ON* */
    } else {
...
static walk_rc_t
igmp_ssm_range_walk_dump(const fib_prefix_t *pfx, igmp_group_prefix_type_t type, void *args)
{
...
void
send_igmp_event(unix_shared_memory_queue_t *q, u32 context, igmp_filter_mode_t filter, u32 sw_if_index,
...
void
igmp_event(igmp_filter_mode_t filter, u32 sw_if_index, const ip46_address_t *saddr, const ip46_address_t *gaddr)
{
    vpe_client_registration_t *api_client;
    unix_shared_memory_queue_t *q;
    igmp_main_t *im;
    ...
    IGMP_DBG("event: (%U, %U) %U %U", format_ip46_address, saddr, IP46_TYPE_ANY, format_ip46_address, saddr,
             IP46_TYPE_ANY, format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index, format_igmp_filter_mode, filter);
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + im->msg_id_base);
...
igmp_plugin_api_hookup(vlib_main_t *vm)
{
    ...
    im->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);

#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + im->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,               \
