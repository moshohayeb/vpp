...
format_igmp_parse_query_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "sw_if_index %u next-input %u", t->sw_if_index, t->next_index);
...
static uword
igmp_parse_query(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, *to_next;
    igmp_parse_query_next_t next_index;
...
static uword
igmp_parse_report(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, *to_next;
