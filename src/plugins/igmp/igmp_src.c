...
igmp_src_alloc(u32 group_index, const igmp_key_t *skey, igmp_mode_t mode)
{
    ...
    if (IGMP_MODE_ROUTER == mode) {
        ...
        src->timers[IGMP_SRC_TIMER_EXP] =
            igmp_timer_schedule(igmp_timer_type_get(IGMP_TIMER_SRC), src - im->srcs, igmp_src_exp, NULL);
...
igmp_src_refresh(igmp_src_t *src)
{
    ...
    src->timers[IGMP_SRC_TIMER_EXP] =
        igmp_timer_schedule(igmp_timer_type_get(IGMP_TIMER_SRC), igmp_src_index(src), igmp_src_exp, NULL);
}
...
igmp_src_blocked(igmp_src_t *src)
{
    ...
    src->timers[IGMP_SRC_TIMER_EXP] =
        igmp_timer_schedule(igmp_timer_type_get(IGMP_TIMER_LEAVE), igmp_src_index(src), igmp_src_exp, NULL);
}
