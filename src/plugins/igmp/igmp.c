...
igmp_send_general_query(u32 obj, void *dat)
{
    ...
    config->timers[IGMP_CONFIG_TIMER_GENERAL_QUERY] =
        igmp_timer_schedule(igmp_timer_type_get(IGMP_TIMER_QUERY), igmp_config_index(config), igmp_send_general_query,
                            NULL);
}

static void
igmp_send_state_change_group_report_v3(u32 sw_if_index, const igmp_group_t *group)
{
...
igmp_resend_state_change_group_report_v3(u32 gi, void *data)
{
    ...
    if (++group->n_reports_sent < config->robustness_var) {
        group->timers[IGMP_GROUP_TIMER_RESEND_REPORT] =
            igmp_timer_schedule(igmp_timer_type_get(IGMP_TIMER_REPORT_INTERVAL), igmp_group_index(group),
                                igmp_resend_state_change_group_report_v3, NULL);
    }
}

int
igmp_listen(vlib_main_t *vm, igmp_filter_mode_t mode, u32 sw_if_index, const ip46_address_t *saddrs,
            const ip46_address_t *gaddr)
{
     ...
     */
    int rv = 0;
    IGMP_DBG("listen: (%U, %U) %U %U", format_igmp_src_addr_list, saddrs, format_igmp_key, gaddr,
             format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index, format_igmp_filter_mode, mode);
    /*
    ...
    if (!group) {
        ...
        IGMP_DBG("... update (%U, %U) %U %U", format_igmp_src_addr_list, saddrs, format_igmp_key, gaddr,
                 format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index, format_igmp_filter_mode, mode);
...
igmp_enable_disable(u32 sw_if_index, u8 enable, igmp_mode_t mode)
{
    ...
    if (!config && enable) {
        ...
        if (IGMP_MODE_ROUTER == mode) {
            config->timers[IGMP_CONFIG_TIMER_GENERAL_QUERY] =
                igmp_timer_schedule(igmp_timer_type_get(IGMP_TIMER_QUERY), igmp_config_index(config),
        ...
        im->igmp_config_by_sw_if_index[config->sw_if_index] = (config - im->configs);
        {
            ...
            if (1 == im->n_configs_per_mfib_index[mfib_index]) {
                ...
                mfib_table_entry_path_update(mfib_index, &mpfx_general_query, MFIB_SOURCE_IGMP, &for_us_path,
                                             MFIB_ITF_FLAG_FORWARD);
                mfib_table_entry_path_update(mfib_index, &mpfx_report, MFIB_SOURCE_IGMP, &for_us_path,
                                             MFIB_ITF_FLAG_FORWARD);
            }
            mfib_table_entry_path_update(mfib_index, &mpfx_general_query, MFIB_SOURCE_IGMP, &via_itf_path,
                                         MFIB_ITF_FLAG_ACCEPT);
            mfib_table_entry_path_update(mfib_index, &mpfx_report, MFIB_SOURCE_IGMP, &via_itf_path, MFIB_ITF_FLAG_ACCEPT);
        }
