...
format_igmp_header(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U%U: code %u, checksum 0x%04x", format_white_space, indent, format_igmp_type, hdr->type, hdr->code,
...
format_igmp_report_v3(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < clib_net_to_host_u16(igmp->n_groups); i++) {
        ...
        s     = format(s, "\n%U%U: %U, sources %u", format_white_space, indent, format_igmp_membership_group_type,
                   group->type, format_ip4_address, &group->group_address, clib_net_to_host_u16(group->n_src_addresses));
        indent += 2;
        ...
        indent -= 2;
        len +=
            sizeof(igmp_membership_group_v3_t) + (sizeof(ip4_address_t) * clib_net_to_host_u16(group->n_src_addresses));
    }
    return s;
}
...
format_igmp_query_v3(u8 *s, va_list *args)
{
    ...
    if ((!ip4_address_compare(&igmp->group_address, &tmp)) && (igmp->n_src_addresses == 0))
        ...
        s = format(s, "%UGroup-and-Source-Specific Query: %U", format_white_space, indent, format_ip4_address,
                   &igmp->group_address);
        indent += 2;
