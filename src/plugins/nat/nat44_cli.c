...
static clib_error_t *
set_workers_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
nat_show_workers_commnad_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    u32 *worker;
...
static clib_error_t *
snat_ipfix_logging_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    u32 domain_id       = 0;
    u32 src_port        = 0;
...
static clib_error_t *
nat44_show_hash_commnad_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    ...
    vlib_cli_output(vm, "%U", format_bihash_8_8, &sm->static_mapping_by_local, verbose);
    vlib_cli_output(vm, "%U", format_bihash_8_8, &sm->static_mapping_by_external, verbose);
    vec_foreach_index(i, sm->per_thread_data)
    {
        ...
        if (sm->endpoint_dependent) {
            vlib_cli_output(vm, "%U", format_bihash_16_8, &tsm->in2out_ed, verbose);
            vlib_cli_output(vm, "%U", format_bihash_16_8, &tsm->out2in_ed, verbose);
        } else {
...
static clib_error_t *
nat44_set_alloc_addr_and_port_alg_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    snat_main_t *sm     = &snat_main;
    clib_error_t *error = 0;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "default"))
            ...
            nat_set_alloc_addr_and_port_range((u16) port_start, (u16) port_end);
        } else {
            error = clib_error_return(0, "unknown input '%U'", format_unformat_error, line_input);
            goto done;
        }
...
static clib_error_t *
nat44_show_alloc_addr_and_port_alg_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    ...
    vlib_cli_output(vm, "NAT address and port: %U", format_nat_addr_and_port_alloc_alg, sm->addr_and_port_alloc_alg);
    switch (sm->addr_and_port_alloc_alg) {
    case NAT_ADDR_AND_PORT_ALLOC_ALG_MAPE:
        vlib_cli_output(vm, "  psid %d psid-offset %d psid-len %d", sm->psid, sm->psid_offset, sm->psid_length);
        break;
    case NAT_ADDR_AND_PORT_ALLOC_ALG_RANGE:
        vlib_cli_output(vm, "  start-port %d end-port %d", sm->start_port, sm->end_port);
        break;
    default:
        break;
    }
...
static clib_error_t *
nat_set_mss_clamping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    snat_main_t *sm     = &snat_main;
    clib_error_t *error = 0;
...
static clib_error_t *
nat_show_mss_clamping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
...
static clib_error_t *
add_address_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    snat_main_t *sm = &snat_main;
    ...
    if (count > 1024)
        nat_log_info("%U - %U, %d addresses...", format_ip4_address, &start_addr, format_ip4_address, &end_addr, count);
...
static clib_error_t *
nat44_show_addresses_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    snat_address_t *ap;
...
static clib_error_t *
snat_feature_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
        ...
        for (i = 0; i < vec_len(inside_sw_if_indices); i++) {
            ...
            if (is_output_feature) {
                if (snat_interface_add_del_output_feature(sw_if_index, 1, is_del)) {
                    error = clib_error_return(0, "%s %U failed", is_del ? "del" : "add", format_vnet_sw_if_index_name,
                                              vnm, sw_if_index);
                    goto done;
                }
            } else {
...
static clib_error_t *
nat44_show_interfaces_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    snat_interface_t *i;
    vnet_main_t *vnm = vnet_get_main();
    ...
    pool_foreach(i, sm->interfaces, ({
                     vlib_cli_output(vm, " %U %s", format_vnet_sw_if_index_name, vnm, i->sw_if_index,
    ...
    pool_foreach(i, sm->output_feature_interfaces, ({
                     vlib_cli_output(vm, " %U output-feature %s", format_vnet_sw_if_index_name, vnm, i->sw_if_index,
                                     (nat_interface_is_inside(i) && nat_interface_is_outside(i)) ?
                                         "in out" :
                                         (nat_interface_is_inside(i) ? "in" : "out"));
                 }));
...
static clib_error_t *
add_static_mapping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    snat_main_t *sm     = &snat_main;
    clib_error_t *error = 0;
    ...
    rv = snat_add_static_mapping(l_addr, e_addr, (u16) l_port, (u16) e_port, vrf_id, addr_only, sw_if_index, proto,
                                 is_add, twice_nat, out2in_only, 0, 0);
    ...
    case VNET_API_ERROR_VALUE_EXIST:
        ...
        goto done;
    case VNET_API_ERROR_FEATURE_DISABLED:
        error = clib_error_return(0, "twice-nat/out2in-only available only for endpoint-dependent mode.");
        goto done;
    default:
        break;
    }
...
static clib_error_t *
add_identity_mapping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    snat_main_t *sm     = &snat_main;
    clib_error_t *error = 0;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U", unformat_ip4_address, &addr))
            ...
            ;
        else if (unformat(line_input, "%U %u", unformat_snat_protocol, &proto, &port))
            addr_only = 0;
        else if (unformat(line_input, "del"))
            is_add = 0;
        else {
    ...
    rv = snat_add_static_mapping(addr, addr, (u16) port, (u16) port, vrf_id, addr_only, sw_if_index, proto, is_add, 0,
                                 0, 0, 1);
...
static clib_error_t *
add_lb_static_mapping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    snat_main_t *sm     = &snat_main;
    clib_error_t *error = 0;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "local %U:%u probability %u", unformat_ip4_address, &l_addr, &l_port, &probability)) {
        ...
        } else if (unformat(line_input, "local %U:%u vrf %u probability %u", unformat_ip4_address, &l_addr, &l_port,
                            &vrf_id, &probability)) {
            memset(&local, 0, sizeof(local));
            local.addr        = l_addr;
            local.port        = (u16) l_port;
            ...
            ;
        else if (unformat(line_input, "protocol %U", unformat_snat_protocol, &proto))
            proto_set = 1;
        else if (unformat(line_input, "twice-nat"))
            twice_nat = TWICE_NAT;
        else if (unformat(line_input, "self-twice-nat"))
    ...
    rv = nat44_add_del_lb_static_mapping(e_addr, (u16) e_port, proto, locals, is_add, twice_nat, out2in_only, 0,
                                         affinity);
    ...
    case VNET_API_ERROR_VALUE_EXIST:
        ...
        goto done;
    case VNET_API_ERROR_FEATURE_DISABLED:
        error = clib_error_return(0, "Available only for endpoint-dependent mode.");
        goto done;
    default:
        break;
    }
...
static clib_error_t *
nat44_show_static_mappings_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    ...
    /* *INDENT-OFF* */
    pool_foreach(m, sm->static_mappings, ({ vlib_cli_output(vm, " %U", format_snat_static_mapping, m); }));
    vec_foreach(rp, sm->to_resolve) vlib_cli_output(vm, " %U", format_snat_static_map_to_resolve, rp);
    /* *INDENT-ON* */
...
static clib_error_t *
snat_add_interface_address_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    unformat_input_t _line_input, *line_input = &_line_input;
    u32 sw_if_index;
    ...
    default:
        error = clib_error_return(0, "snat_add_interface_address returned %d", rv);
        goto done;
    }
...
static clib_error_t *
nat44_show_interface_address_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm  = &snat_main;
    vnet_main_t *vnm = vnet_get_main();
    u32 *sw_if_index;
...
static clib_error_t *
nat44_show_sessions_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    int verbose     = 0;
    snat_main_t *sm = &snat_main;
    snat_main_per_thread_data_t *tsm;
    ...
    vec_foreach_index(i, sm->per_thread_data)
    {
        ...
        pool_foreach(u, tsm->users, ({ vlib_cli_output(vm, "  %U", format_snat_user, tsm, u, verbose); }));
    }
    /* *INDENT-ON* */
...
static clib_error_t *
nat44_del_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    unformat_input_t _line_input, *line_input = &_line_input;
    int is_in = 0, is_ed = 0;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U:%u %U", unformat_ip4_address, &addr, &port, unformat_snat_protocol, &proto))
            ;
    ...
    if (is_ed)
        rv = nat44_del_ed_session(sm, &addr, port, &eh_addr, eh_port, snat_proto_to_ip_proto(proto), vrf_id, is_in);
    else
...
static clib_error_t *
snat_forwarding_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
snat_det_map_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    unformat_input_t _line_input, *line_input = &_line_input;
    ...
    rv = snat_det_add_map(sm, &in_addr, (u8) in_plen, &out_addr, (u8) out_plen, is_add);
...
static clib_error_t *
nat44_det_show_mappings_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    snat_det_map_t *dm;
...
static clib_error_t *
snat_det_forward_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    unformat_input_t _line_input, *line_input = &_line_input;
    ip4_address_t in_addr, out_addr;
    ...
    if (!dm)
        ...
        vlib_cli_output(vm, "%U:<%d-%d>", format_ip4_address, &out_addr, lo_port, lo_port + dm->ports_per_host - 1);
    }
...
static clib_error_t *
snat_det_reverse_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    unformat_input_t _line_input, *line_input = &_line_input;
    ip4_address_t in_addr, out_addr;
...
static clib_error_t *
set_timeout_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    unformat_input_t _line_input, *line_input = &_line_input;
    clib_error_t *error = 0;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        } else if (unformat(line_input, "tcp-established %u", &sm->tcp_established_timeout)) {
            if (nat64_set_tcp_timeouts(sm->tcp_transitory_timeout, sm->tcp_established_timeout)) {
                error = clib_error_return(0, "Invalid TCP established timeouts value");
                goto done;
            }
        } else if (unformat(line_input, "tcp-transitory %u", &sm->tcp_transitory_timeout)) {
            if (nat64_set_tcp_timeouts(sm->tcp_transitory_timeout, sm->tcp_established_timeout)) {
                error = clib_error_return(0, "Invalid TCP transitory timeouts value");
                goto done;
            }
...
static clib_error_t *
nat_show_timeouts_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;

    vlib_cli_output(vm, "udp timeout: %dsec", sm->udp_timeout);
    vlib_cli_output(vm, "tcp-established timeout: %dsec", sm->tcp_established_timeout);
    vlib_cli_output(vm, "tcp-transitory timeout: %dsec", sm->tcp_transitory_timeout);
...
static clib_error_t *
nat44_det_show_sessions_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    snat_det_map_t *dm;
...
static clib_error_t *
snat_det_close_session_out_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    unformat_input_t _line_input, *line_input = &_line_input;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U:%d %U:%d", unformat_ip4_address, &out_addr, &out_port, unformat_ip4_address,
                     &ext_addr, &ext_port))
            ;
        else {
            error = clib_error_return(0, "unknown input '%U'", format_unformat_error, line_input);
...
static clib_error_t *
snat_det_close_session_in_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    unformat_input_t _line_input, *line_input = &_line_input;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U:%d %U:%d", unformat_ip4_address, &in_addr, &in_port, unformat_ip4_address,
                     &ext_addr, &ext_port))
            ;
        else {
            error = clib_error_return(0, "unknown input '%U'", format_unformat_error, line_input);
...
VLIB_CLI_COMMAND(add_static_mapping_command, static) = {
    ...
    .short_help = "nat44 add static mapping tcp|udp|icmp local <addr> [<port>] "
 ...
 *  vpp# nat44 add load-balancing static mapping protocol tcp external 1.2.3.4:80 local 10.100.10.10:8080 probability 80
