...
format_nat_det_out2in_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "NAT_DET_OUT2IN: sw_if_index %d, next index %d, session index %d", t->sw_if_index, t->next_index,
               t->session_index);
    return s;
}
 ...
 */
u32
icmp_match_out2in_det(snat_main_t *sm, vlib_node_runtime_t *node, u32 thread_index, vlib_buffer_t *b0, ip4_header_t *ip0,
                      u8 *p_proto, snat_session_key_t *p_value, u8 *p_dont_translate, void *d, void *e)
{
    ...
    if (PREDICT_FALSE(!dm0)) {
        ...
        nat_log_info("unknown dst address:  %U", format_ip4_address, &ip0->dst_address);
        goto out;
    }

    snat_det_reverse(dm0, &ip0->dst_address, clib_net_to_host_u16(key0.out_port), &new_addr0);
    ...
    if (PREDICT_FALSE(!ses0)) {
        ...
        nat_log_info("no match src %U:%d dst %U:%d for user %U", format_ip4_address, &key0.ext_host_addr,
                     clib_net_to_host_u16(key0.ext_host_port), format_ip4_address, &out_addr,
                     clib_net_to_host_u16(key0.out_port), format_ip4_address, &new_addr0);
        b0->error = node->errors[NAT_DET_OUT2IN_ERROR_NO_TRANSLATION];
        next0     = NAT_DET_OUT2IN_NEXT_DROP;
        goto out;
    }

    if (PREDICT_FALSE(icmp0->type != ICMP4_echo_reply && !icmp_is_error_message(icmp0))) {
...
static uword
snat_det_out2in_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE(proto0 == SNAT_PROTOCOL_ICMP)) {
                ...
                next0 =
                    icmp_out2in(sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, thread_index, &ses0, &dm0);
                goto trace0;
            }
            ...
            snat_det_reverse(dm0, &ip0->dst_address, clib_net_to_host_u16(tcp0->dst), &new_addr0);

            ses0 = snat_det_get_ses_by_out(dm0, &new_addr0, key0.as_u64);
            if (PREDICT_FALSE(!ses0)) {
                nat_log_info("no match src %U:%d dst %U:%d for user %U", format_ip4_address, &ip0->src_address,
                             clib_net_to_host_u16(tcp0->src), format_ip4_address, &ip0->dst_address,
                             clib_net_to_host_u16(tcp0->dst), format_ip4_address, &new_addr0);
                next0     = NAT_DET_OUT2IN_NEXT_DROP;
                b0->error = node->errors[NAT_DET_OUT2IN_ERROR_NO_TRANSLATION];
                goto trace0;
            }
            ...
            sum0          = ip0->checksum;
            sum0          = ip_csum_update(sum0, old_addr0.as_u32, new_addr0.as_u32, ip4_header_t,
        ...
        trace0:
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                nat_det_out2in_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
            ...
            if (PREDICT_FALSE(proto1 == SNAT_PROTOCOL_ICMP)) {
                ...
                next1 =
                    icmp_out2in(sm, b1, ip1, icmp1, sw_if_index1, rx_fib_index1, node, next1, thread_index, &ses1, &dm1);
                goto trace1;
            }
            ...
            if (PREDICT_FALSE(!dm1)) {
                nat_log_info("unknown dst address:  %U", format_ip4_address, &ip1->dst_address);
                next1     = NAT_DET_OUT2IN_NEXT_DROP;
                b1->error = node->errors[NAT_DET_OUT2IN_ERROR_NO_TRANSLATION];
                goto trace1;
            }

            snat_det_reverse(dm1, &ip1->dst_address, clib_net_to_host_u16(tcp1->dst), &new_addr1);

            ses1 = snat_det_get_ses_by_out(dm1, &new_addr1, key1.as_u64);
            if (PREDICT_FALSE(!ses1)) {
                nat_log_info("no match src %U:%d dst %U:%d for user %U", format_ip4_address, &ip1->src_address,
                             clib_net_to_host_u16(tcp1->src), format_ip4_address, &ip1->dst_address,
                             clib_net_to_host_u16(tcp1->dst), format_ip4_address, &new_addr1);
                next1     = NAT_DET_OUT2IN_NEXT_DROP;
                b1->error = node->errors[NAT_DET_OUT2IN_ERROR_NO_TRANSLATION];
                goto trace1;
            }
            ...
            sum1          = ip1->checksum;
            sum1          = ip_csum_update(sum1, old_addr1.as_u32, new_addr1.as_u32, ip4_header_t,
                                  dst_address /* changed member */);
        ...
        trace1:
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
                nat_det_out2in_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
    ...
    vlib_node_increment_counter(vm, snat_det_out2in_node.index, NAT_DET_OUT2IN_ERROR_OUT2IN_PACKETS, pkts_processed);
