 ...
 */
static inline u8 *
snat_template_rewrite(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                      ip4_address_t *src_address, u16 collector_port, nat_event_t event, quota_exceed_event_t quota_event)
{
    snat_ipfix_logging_main_t *silm = &snat_ipfix_logging_main;
    ip4_header_t *ip;
    udp_header_t *udp;
    ...
    vec_validate_aligned(rewrite, sizeof(ip4_ipfix_template_packet_t) + field_count * sizeof(ipfix_field_specifier_t) - 1,
    ...
    if (event == NAT_ADDRESSES_EXHAUTED) {
        ...
        if (quota_event == MAX_ENTRIES_PER_USER) {
            ...
            f->e_id_length = ipfix_e_id_length(0, natQuotaExceededEvent, 4);
            f++;
            f->e_id_length = ipfix_e_id_length(0, maxFragmentsPendingReassembly, 4);
            f++;
            f->e_id_length = ipfix_e_id_length(0, sourceIPv4Address, 4);
            f++;
        } else if (quota_event == MAX_FRAGMENTS_PENDING_REASSEMBLY_IP6) {
...
u8 *
snat_template_rewrite_addr_exhausted(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                                     ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts,
                                     u32 n_elts, u32 *stream_index)
{
    return snat_template_rewrite(frm, fr, collector_address, src_address, collector_port, NAT_ADDRESSES_EXHAUTED, 0);
}

u8 *
snat_template_rewrite_nat44_session(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                                    ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts,
                                    u32 n_elts, u32 *stream_index)
{
    return snat_template_rewrite(frm, fr, collector_address, src_address, collector_port, NAT44_SESSION_CREATE, 0);
}

u8 *
snat_template_rewrite_max_entries_per_usr(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                                          ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts,
                                          u32 n_elts, u32 *stream_index)
{
    return snat_template_rewrite(frm, fr, collector_address, src_address, collector_port, QUOTA_EXCEEDED,
                                 MAX_ENTRIES_PER_USER);
}

u8 *
nat_template_rewrite_max_sessions(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                                  ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts,
                                  u32 n_elts, u32 *stream_index)
{
    return snat_template_rewrite(frm, fr, collector_address, src_address, collector_port, QUOTA_EXCEEDED,
...
u8 *
nat_template_rewrite_max_bibs(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                              ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts, u32 n_elts,
                              u32 *stream_index)
{
    return snat_template_rewrite(frm, fr, collector_address, src_address, collector_port, QUOTA_EXCEEDED,
                                 MAX_BIB_ENTRIES);
...
u8 *
nat_template_rewrite_max_frags_ip4(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                                   ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts,
                                   u32 n_elts, u32 *stream_index)
{
    return snat_template_rewrite(frm, fr, collector_address, src_address, collector_port, QUOTA_EXCEEDED,
...
u8 *
nat_template_rewrite_max_frags_ip6(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                                   ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts,
                                   u32 n_elts, u32 *stream_index)
{
    return snat_template_rewrite(frm, fr, collector_address, src_address, collector_port, QUOTA_EXCEEDED,
...
u8 *
nat_template_rewrite_nat64_bib(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                               ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts, u32 n_elts,
                               u32 *stream_index)
{
    return snat_template_rewrite(frm, fr, collector_address, src_address, collector_port, NAT64_BIB_CREATE, 0);
}

u8 *
nat_template_rewrite_nat64_session(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                                   ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts,
                                   u32 n_elts, u32 *stream_index)
{
    return snat_template_rewrite(frm, fr, collector_address, src_address, collector_port, NAT64_SESSION_CREATE, 0);
}

static inline void
snat_ipfix_header_create(flow_report_main_t *frm, vlib_buffer_t *b0, u32 *offset)
{
    snat_ipfix_logging_main_t *silm = &snat_ipfix_logging_main;
    flow_report_stream_t *stream;
    ip4_ipfix_template_packet_t *tp;
    ...
    h->export_time =
        clib_host_to_net_u32((u32)(((f64) frm->unix_time_0) + (vlib_time_now(frm->vlib_main) - frm->vlib_time_0)));
...
static inline void
snat_ipfix_send(flow_report_main_t *frm, vlib_frame_t *f, vlib_buffer_t *b0, u16 template_id)
{
    ip4_ipfix_template_packet_t *tp;
    ipfix_message_header_t *h = 0;
    ipfix_set_header_t *s     = 0;
...
static void
snat_ipfix_logging_nat44_ses(u8 nat_event, u32 src_ip, u32 nat_src_ip, snat_protocol_t snat_proto, u16 src_port,
...
static void
nat_ipfix_logging_nat64_bibe(u8 nat_event, ip6_address_t *src_ip, u32 nat_src_ip, u8 proto, u16 src_port,
                             u16 nat_src_port, u32 vrf_id, int do_flush)
{
    snat_ipfix_logging_main_t *silm = &snat_ipfix_logging_main;
    flow_report_main_t *frm         = &flow_report_main;
...
static void
nat_ipfix_logging_nat64_ses(u8 nat_event, ip6_address_t *src_ip, u32 nat_src_ip, u8 proto, u16 src_port,
                            u16 nat_src_port, ip6_address_t *dst_ip, u32 nat_dst_ip, u16 dst_port, u16 nat_dst_port,
...
{
    snat_ipfix_logging_nat44_ses(a->nat_event, a->src_ip, a->nat_src_ip, a->snat_proto, a->src_port, a->nat_src_port,
                                 a->vrf_id, 0);
}
 ...
 */
void
snat_ipfix_logging_nat44_ses_create(u32 src_ip, u32 nat_src_ip, snat_protocol_t snat_proto, u16 src_port,
                                    u16 nat_src_port, u32 vrf_id)
{
    ...
    vl_api_rpc_call_main_thread(snat_ipfix_logging_nat44_ses_rpc_cb, (u8 *) &a, sizeof(a));
}
 ...
 */
void
snat_ipfix_logging_nat44_ses_delete(u32 src_ip, u32 nat_src_ip, snat_protocol_t snat_proto, u16 src_port,
                                    u16 nat_src_port, u32 vrf_id)
{
    snat_ipfix_logging_nat44_ses_args_t a;
...
vlib_frame_t *
snat_data_callback_nat44_session(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
...
static void
snat_ipfix_logging_addr_exhausted_rpc_cb(snat_ipfix_logging_addr_exhausted_args_t *a)
{
...
snat_ipfix_logging_addresses_exhausted(u32 pool_id)
{
    ...
    vl_api_rpc_call_main_thread(snat_ipfix_logging_addr_exhausted_rpc_cb, (u8 *) &a, sizeof(a));
}

vlib_frame_t *
snat_data_callback_addr_exhausted(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next,
                                  u32 node_index)
{
...
static void
snat_ipfix_logging_max_entries_per_usr_rpc_cb(snat_ipfix_logging_max_entries_per_user_args_t *a)
{
...
snat_ipfix_logging_max_entries_per_user(u32 limit, u32 src_ip)
{
    ...
    vl_api_rpc_call_main_thread(snat_ipfix_logging_max_entries_per_usr_rpc_cb, (u8 *) &a, sizeof(a));
}

vlib_frame_t *
snat_data_callback_max_entries_per_usr(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next,
                                       u32 node_index)
{
...
nat_ipfix_logging_max_sessions(u32 limit)
{
    ...
    vl_api_rpc_call_main_thread(nat_ipfix_logging_max_ses_rpc_cb, (u8 *) &a, sizeof(a));
}

vlib_frame_t *
nat_data_callback_max_sessions(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
...
nat_ipfix_logging_max_bibs(u32 limit)
{
    ...
    vl_api_rpc_call_main_thread(nat_ipfix_logging_max_bib_rpc_cb, (u8 *) &a, sizeof(a));
}

vlib_frame_t *
nat_data_callback_max_bibs(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
...
nat_ipfix_logging_max_fragments_ip4(u32 limit, ip4_address_t *src)
{
    ...
    vl_api_rpc_call_main_thread(nat_ipfix_logging_max_frag_ip4_rpc_cb, (u8 *) &a, sizeof(a));
}

vlib_frame_t *
nat_data_callback_max_frags_ip4(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
...
nat_ipfix_logging_max_fragments_ip6(u32 limit, ip6_address_t *src)
{
    ...
    vl_api_rpc_call_main_thread(nat_ipfix_logging_max_frag_ip6_rpc_cb, (u8 *) &a, sizeof(a));
}

vlib_frame_t *
nat_data_callback_max_frags_ip6(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
...
nat_ipfix_logging_nat64_bib_rpc_cb(nat_ipfix_logging_nat64_bib_args_t *a)
{
    ...
    nat_ipfix_logging_nat64_bibe(a->nat_event, &src_ip, a->nat_src_ip, a->proto, a->src_port, a->nat_src_port,
                                 a->vrf_id, 0);
}
 ...
 */
void
nat_ipfix_logging_nat64_bib(ip6_address_t *src_ip, ip4_address_t *nat_src_ip, u8 proto, u16 src_port, u16 nat_src_port,
                            u32 vrf_id, u8 is_create)
{
    ...
    vl_api_rpc_call_main_thread(nat_ipfix_logging_nat64_bib_rpc_cb, (u8 *) &a, sizeof(a));
}

vlib_frame_t *
nat_data_callback_nat64_bib(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
...
nat_ipfix_logging_nat64_ses_rpc_cb(nat_ipfix_logging_nat64_ses_args_t *a)
{
    ...
    nat_ipfix_logging_nat64_ses(a->nat_event, &src_ip, a->nat_src_ip, a->proto, a->src_port, a->nat_src_port, &dst_ip,
                                a->nat_dst_ip, a->dst_port, a->nat_dst_port, a->vrf_id, 0);
}
 ...
 */
void
nat_ipfix_logging_nat64_session(ip6_address_t *src_ip, ip4_address_t *nat_src_ip, u8 proto, u16 src_port,
                                u16 nat_src_port, ip6_address_t *dst_ip, ip4_address_t *nat_dst_ip, u16 dst_port,
                                u16 nat_dst_port, u32 vrf_id, u8 is_create)
{
    ...
    vl_api_rpc_call_main_thread(nat_ipfix_logging_nat64_ses_rpc_cb, (u8 *) &a, sizeof(a));
}
