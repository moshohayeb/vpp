...
static inline int
snat_not_translate(snat_main_t *sm, vlib_node_runtime_t *node, u32 sw_if_index0, ip4_header_t *ip0, u32 proto0,
                   u32 rx_fib_index0, u32 thread_index)
{
    ...
    /* has active sessions */
    if (clib_bihash_search_8_8(&sm->per_thread_data[thread_index].out2in, &kv0, &value0)) {
    ...
    return snat_not_translate_fast(sm, node, sw_if_index0, ip0, proto0, rx_fib_index0);
}

static inline int
nat_not_translate_output_feature(snat_main_t *sm, ip4_header_t *ip0, u32 proto0, u16 src_port, u16 dst_port,
...
static u32
slow_path(snat_main_t *sm, vlib_buffer_t *b0, ip4_header_t *ip0, u32 rx_fib_index0, snat_session_key_t *key0,
          snat_session_t **sessionp, vlib_node_runtime_t *node, u32 next0, u32 thread_index, f64 now)
{
    snat_user_t *u;
    snat_session_t *s = 0;
    ...
    if (snat_static_mapping_match(sm, *key0, &key1, 0, 0, 0, 0, 0, &identity_nat)) {
        /* Try to create dynamic translation */
        if (snat_alloc_outside_address_and_port(sm->addresses, rx_fib_index0, thread_index, &key1, sm->port_per_thread,
    ...
    u = nat_user_get_or_create(sm, &ip0->src_address, rx_fib_index0, thread_index);
    if (!u) {
    ...
    *sessionp               = s;
    ...
    if (clib_bihash_add_or_overwrite_stale_8_8(&sm->per_thread_data[thread_index].in2out, &kv0,
    ...
    if (clib_bihash_add_or_overwrite_stale_8_8(&sm->per_thread_data[thread_index].out2in, &kv0,
...
icmp_get_key(ip4_header_t *ip0, snat_session_key_t *p_key0)
{
 ...
 */
u32
icmp_match_in2out_slow(snat_main_t *sm, vlib_node_runtime_t *node, u32 thread_index, vlib_buffer_t *b0, ip4_header_t *ip0,
                       u8 *p_proto, snat_session_key_t *p_value, u8 *p_dont_translate, void *d, void *e)
{
    ...
    if (clib_bihash_search_8_8(&sm->per_thread_data[thread_index].in2out, &kv0, &value0)) {
        if (vnet_buffer(b0)->sw_if_index[VLIB_TX] != ~0) {
            if (PREDICT_FALSE(nat_not_translate_output_feature(sm, ip0, key0.protocol, key0.port, key0.port,
            ...
            if (PREDICT_FALSE(
                    snat_not_translate(sm, node, sw_if_index0, ip0, SNAT_PROTOCOL_ICMP, rx_fib_index0, thread_index))) {
                dont_translate = 1;
                goto out;
            }
        }
        ...
        next0 =
            slow_path(sm, b0, ip0, rx_fib_index0, &key0, &s0, node, next0, thread_index, vlib_time_now(sm->vlib_main));
        ...
        if (PREDICT_FALSE(icmp0->type != ICMP4_echo_request && icmp0->type != ICMP4_echo_reply &&
 ...
 */
u32
icmp_match_in2out_fast(snat_main_t *sm, vlib_node_runtime_t *node, u32 thread_index, vlib_buffer_t *b0, ip4_header_t *ip0,
                       u8 *p_proto, snat_session_key_t *p_value, u8 *p_dont_translate, void *d, void *e)
{
    icmp46_header_t *icmp0;
    u32 sw_if_index0;
    ...
    if (snat_static_mapping_match(sm, key0, &sm0, 0, &is_addr_only, 0, 0, 0, 0)) {
        if (PREDICT_FALSE(snat_not_translate_fast(sm, node, sw_if_index0, ip0, IP_PROTOCOL_ICMP, rx_fib_index0))) {
            dont_translate = 1;
            goto out;
        }
    ...
    if (PREDICT_FALSE(icmp0->type != ICMP4_echo_request && (icmp0->type != ICMP4_echo_reply || !is_addr_only) &&
                      !icmp_is_error_message(icmp0))) {
        b0->error = node->errors[SNAT_IN2OUT_ERROR_BAD_ICMP_TYPE];
        next0     = SNAT_IN2OUT_NEXT_DROP;
        goto out;
...
u32
icmp_in2out(snat_main_t *sm, vlib_buffer_t *b0, ip4_header_t *ip0, icmp46_header_t *icmp0, u32 sw_if_index0,
            u32 rx_fib_index0, vlib_node_runtime_t *node, u32 next0, u32 thread_index, void *d, void *e)
{
    snat_session_key_t sm0;
    u8 protocol;
    icmp_echo_header_t *echo0, *inner_echo0 = 0;
    ...
    next0_tmp = sm->icmp_match_in2out_cb(sm, node, thread_index, b0, ip0, &protocol, &sm0, &dont_translate, d, e);
    if (next0_tmp != ~0)
        next0 = next0_tmp;
    ...
    if (PREDICT_TRUE(!ip4_is_fragment(ip0))) {
        sum0      = ip_incremental_checksum_buffer(sm->vlib_main, b0, (u8 *) icmp0 - (u8 *) vlib_buffer_get_current(b0),
    ...
    if (vnet_buffer(b0)->sw_if_index[VLIB_TX] == ~0) {
        if (sm->deterministic || 0 != snat_icmp_hairpinning(sm, b0, ip0, icmp0, sm->endpoint_dependent))
...
static inline u32
icmp_in2out_slow_path(snat_main_t *sm, vlib_buffer_t *b0, ip4_header_t *ip0, icmp46_header_t *icmp0, u32 sw_if_index0,
                      u32 rx_fib_index0, vlib_node_runtime_t *node, u32 next0, f64 now, u32 thread_index,
                      snat_session_t **p_s0)
{
...
static int
nat_in2out_sm_unknown_proto(snat_main_t *sm, vlib_buffer_t *b, ip4_header_t *ip, u32 rx_fib_index)
{
...
static inline uword
snat_in2out_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_slow_path,
                           int is_output_feature)
{
    ...
    stats_node_index = is_slow_path ? snat_in2out_slowpath_node.index : snat_in2out_node.index;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (is_slow_path) {
                ...
                if (PREDICT_FALSE(proto0 == SNAT_PROTOCOL_ICMP)) {
                    next0 = icmp_in2out_slow_path(sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, now,
                                                  thread_index, &s0);
                    goto trace00;
                }
            } else {
            ...
            if (PREDICT_FALSE(clib_bihash_search_8_8(&sm->per_thread_data[thread_index].in2out, &kv0, &value0) != 0)) {
                if (is_slow_path) {
                    if (is_output_feature) {
                        if (PREDICT_FALSE(nat_not_translate_output_feature(sm, ip0, proto0, udp0->src_port,
                                                                           udp0->dst_port, thread_index, sw_if_index0)))
                            goto trace00;
                    } else {
                        if (PREDICT_FALSE(
                                snat_not_translate(sm, node, sw_if_index0, ip0, proto0, rx_fib_index0, thread_index)))
                            goto trace00;
                    }

                    next0 = slow_path(sm, b0, ip0, rx_fib_index0, &key0, &s0, node, next0, thread_index, now);
                    if (PREDICT_FALSE(next0 == SNAT_IN2OUT_NEXT_DROP))
                        goto trace00;
            ...
            if (PREDICT_TRUE(proto0 == SNAT_PROTOCOL_TCP)) {
                ...
                sum0 = ip_csum_update(sum0, old_addr0, new_addr0, ip4_header_t, dst_address /* changed member */);
                sum0 =
                    ip_csum_update(sum0, old_port0, new_port0, ip4_header_t /* cheat */, length /* changed member */);
        ...
        trace00:
            ...
            rx_fib_index1 = vec_elt(sm->ip4_main->fib_index_by_sw_if_index, sw_if_index1);
            ...
            if (is_slow_path) {
                ...
                if (PREDICT_FALSE(proto1 == SNAT_PROTOCOL_ICMP)) {
                    next1 = icmp_in2out_slow_path(sm, b1, ip1, icmp1, sw_if_index1, rx_fib_index1, node, next1, now,
                                                  thread_index, &s1);
                    goto trace01;
                }
            } else {
            ...
            if (PREDICT_FALSE(clib_bihash_search_8_8(&sm->per_thread_data[thread_index].in2out, &kv1, &value1) != 0)) {
                if (is_slow_path) {
                    if (is_output_feature) {
                        if (PREDICT_FALSE(nat_not_translate_output_feature(sm, ip1, proto1, udp1->src_port,
                                                                           udp1->dst_port, thread_index, sw_if_index1)))
                            goto trace01;
                    } else {
                        if (PREDICT_FALSE(
                                snat_not_translate(sm, node, sw_if_index1, ip1, proto1, rx_fib_index1, thread_index)))
                            goto trace01;
                    }

                    next1 = slow_path(sm, b1, ip1, rx_fib_index1, &key1, &s1, node, next1, thread_index, now);
                ...
                s1 = pool_elt_at_index(sm->per_thread_data[thread_index].sessions, value1.value);
            ...
            if (PREDICT_TRUE(proto1 == SNAT_PROTOCOL_TCP)) {
                ...
                sum1 =
                    ip_csum_update(sum1, old_port1, new_port1, ip4_header_t /* cheat */, length /* changed member */);
    ...
    vlib_node_increment_counter(vm, stats_node_index, SNAT_IN2OUT_ERROR_IN2OUT_PACKETS, pkts_processed);
    return frame->n_vectors;
}

static uword
snat_in2out_fast_path_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
snat_in2out_output_fast_path_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
snat_in2out_slow_path_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
snat_in2out_output_slow_path_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nat44_in2out_reass_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, *to_next;
    snat_in2out_next_t next_index;
    u32 pkts_processed                           = 0;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            /* Hairpinning */
            nat44_reass_hairpinning(sm, b0, ip0, s0->out2in.port, s0->ext_host_port, proto0, 0);
    ...
    vlib_node_increment_counter(vm, nat44_in2out_reass_node.index, SNAT_IN2OUT_ERROR_IN2OUT_PACKETS, pkts_processed);

    nat_send_all_to_node(vm, fragments_to_drop, node, &node->errors[SNAT_IN2OUT_ERROR_DROP_FRAGMENT],
                         SNAT_IN2OUT_NEXT_DROP);
...
static uword
snat_in2out_fast_static_map_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, *to_next;
    snat_in2out_next_t next_index;
    u32 pkts_processed = 0;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(proto0 == SNAT_PROTOCOL_ICMP)) {
                next0 = icmp_in2out(sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, ~0, 0, 0);
                goto trace0;
            }
