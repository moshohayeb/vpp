...
static u32
slow_path(dslite_main_t *dm, dslite_session_key_t *in2out_key, dslite_session_t **sp, u32 next, u8 *error,
          u32 thread_index)
{
    dslite_b4_t *b4;
    ...
    if (b4->nsessions >= 1000) {
        oldest_index =
            clib_dlist_remove_head(dm->per_thread_data[thread_index].list_pool, b4->sessions_per_b4_list_head_index);
        ASSERT(oldest_index != ~0);
        clib_dlist_addtail(dm->per_thread_data[thread_index].list_pool, b4->sessions_per_b4_list_head_index,
                           oldest_index);
        ...
        snat_free_outside_address_and_port(dm->addr_pool, thread_index, &s->out2in);

        if (snat_alloc_outside_address_and_port(dm->addr_pool, 0, thread_index, &out2in_key, dm->port_per_thread,
                                                thread_index))
            ASSERT(0);
    } else {
        if (snat_alloc_outside_address_and_port(dm->addr_pool, 0, thread_index, &out2in_key, dm->port_per_thread,
        ...
        clib_dlist_addtail(dm->per_thread_data[thread_index].list_pool, s->per_b4_list_head_index,
...
static inline u32
dslite_icmp_in2out(dslite_main_t *dm, ip6_header_t *ip6, ip4_header_t *ip4, dslite_session_t **sp, u32 next, u8 *error,
                   u32 thread_index)
{
...
static inline uword
dslite_in2out_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_slow_path)
{
    ...
    node_index = is_slow_path ? dslite_in2out_slowpath_node.index : dslite_in2out_node.index;
    ...
    while (n_left_from > 0) {
        ...
        trace0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ...
                t->session_index  = ~0;
                if (s0)
                    t->session_index = s0 - dm->per_thread_data[thread_index].sessions;
            }
...
static uword
dslite_in2out_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
dslite_in2out_slowpath_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
VLIB_NODE_FUNCTION_MULTIARCH(dslite_in2out_slowpath_node, dslite_in2out_slowpath_node_fn);
