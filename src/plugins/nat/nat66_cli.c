...
static clib_error_t *
nat66_interface_feature_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (vec_len(inside_sw_if_indices)) {
        for (i = 0; i < vec_len(inside_sw_if_indices); i++) {
            ...
            case VNET_API_ERROR_NO_SUCH_ENTRY:
                error = clib_error_return(0, "%U NAT66 feature not enabled.", format_vnet_sw_interface_name, vnm,
                                          vnet_get_sw_interface(vnm, sw_if_index));
                goto done;
            case VNET_API_ERROR_VALUE_EXIST:
                error = clib_error_return(0, "%U NAT66 feature already enabled.", format_vnet_sw_interface_name, vnm,
                                          vnet_get_sw_interface(vnm, sw_if_index));
                goto done;
            case VNET_API_ERROR_INVALID_VALUE:
            case VNET_API_ERROR_INVALID_VALUE_2:
                error = clib_error_return(0, "%U NAT66 feature enable/disable failed.", format_vnet_sw_interface_name,
...
nat66_cli_interface_walk(snat_interface_t *i, void *ctx)
{
    ...
    vlib_cli_output(vm, " %U %s", format_vnet_sw_interface_name, vnm, vnet_get_sw_interface(vnm, i->sw_if_index),
                    nat_interface_is_inside(i) ? "in" : "out");
    return 0;
}

static clib_error_t *
nat66_show_interfaces_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
nat66_add_del_static_mapping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    clib_error_t *error = 0;
    u8 is_add           = 1;
...
nat66_cli_static_mapping_walk(nat66_static_mapping_t *sm, void *ctx)
{
    ...
    vlib_cli_output(vm, " local %U external %U vrf %d", format_ip6_address, &sm->l_addr, format_ip6_address,
                    &sm->e_addr, fib->ft_table_id);
...
static clib_error_t *
nat66_show_static_mappings_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
