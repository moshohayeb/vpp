...
format_nat64_in2out_reass_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
    nat64_in2out_reass_trace_t *t   = va_arg(*args, nat64_in2out_reass_trace_t *);

    s = format(s, "NAT64-in2out-reass: sw_if_index %d, next index %d, status %s", t->sw_if_index, t->next_index,
...
static int
nat64_in2out_tcp_udp_set_cb(ip6_header_t *ip6, ip4_header_t *ip4, void *arg)
{
    nat64_main_t *nm            = &nat64_main;
    ...
    if (ste) {
        ...
        if (!bibe) {
            ...
            if (nat64_alloc_out_addr_and_port(fib_index, ip_proto_to_snat_proto(proto), &out_addr, &out_port,
                                              ctx->thread_index))
                return -1;

            bibe = nat64_db_bib_entry_create(db, &ip6->src_address, &out_addr, sport, out_port, fib_index, proto, 0);
            if (!bibe)
                return -1;
        }

        nat64_extract_ip4(&ip6->dst_address, &daddr.ip4, fib_index);
        ste = nat64_db_st_entry_create(db, bibe, &ip6->dst_address, &daddr.ip4, dport);
        if (!ste)
            return -1;
    }
    ...
    if (icmp->type == ICMP4_echo_request || icmp->type == ICMP4_echo_reply) {
        ...
        if (ste) {
            ...
            if (!bibe) {
                u16 out_id;
                ip4_address_t out_addr;
                if (nat64_alloc_out_addr_and_port(fib_index, SNAT_PROTOCOL_ICMP, &out_addr, &out_id, ctx->thread_index))
                    return -1;

                bibe = nat64_db_bib_entry_create(db, &ip6->src_address, &out_addr, in_id, out_id, fib_index,
                                                 IP_PROTOCOL_ICMP, 0);
                if (!bibe)
                    return -1;
            }
            ...
            nat64_extract_ip4(&ip6->dst_address, &daddr.ip4, fib_index);
            ste = nat64_db_st_entry_create(db, bibe, &ip6->dst_address, &daddr.ip4, 0);
            if (!ste)
                return -1;
        }
...
static int
nat64_in2out_inner_icmp_set_cb(ip6_header_t *ip6, ip4_header_t *ip4, void *arg)
{
    nat64_main_t *nm            = &nat64_main;
    nat64_in2out_set_ctx_t *ctx = arg;
    nat64_db_st_entry_t *ste;
...
unk_proto_st_walk(nat64_db_st_entry_t *ste, void *arg)
{
    ...
    if (ip46_address_is_equal(&ste->in_r_addr, &ctx->dst_addr)) {
        ...
        if (ip46_address_is_equal(&bibe->in_addr, &ctx->src_addr) && bibe->fib_index == ctx->fib_index) {
            memset(&saddr, 0, sizeof(saddr));
            saddr.ip4.as_u32 = bibe->out_addr.as_u32;
            memset(&daddr, 0, sizeof(daddr));
            nat64_extract_ip4(&ctx->dst_addr, &daddr.ip4, ctx->fib_index);

            if (nat64_db_st_entry_find(db, &daddr, &saddr, 0, 0, ctx->proto, ctx->fib_index, 0))
                return -1;
...
static int
nat64_in2out_unk_proto_set_cb(ip6_header_t *ip6, ip4_header_t *ip4, void *arg)
{
    nat64_main_t *nm              = &nat64_main;
    ...
    if (ste) {
        ...
        if (!bibe) {
            ...
            bibe = nat64_db_bib_entry_create(db, &ip6->src_address, &ctx.out_addr, 0, 0, fib_index, proto, 0);
            if (!bibe)
                return -1;
        }
...
static int
nat64_in2out_tcp_udp_hairpinning(vlib_main_t *vm, vlib_buffer_t *b, ip6_header_t *ip6, u32 thread_index)
{
    nat64_main_t *nm = &nat64_main;
    nat64_db_bib_entry_t *bibe;
    nat64_db_st_entry_t *ste;
...
static int
nat64_in2out_icmp_hairpinning(vlib_main_t *vm, vlib_buffer_t *b, ip6_header_t *ip6, u32 thread_index)
{
    nat64_main_t *nm = &nat64_main;
    nat64_db_bib_entry_t *bibe;
    nat64_db_st_entry_t *ste;
...
static int
nat64_in2out_unk_proto_hairpinning(vlib_main_t *vm, vlib_buffer_t *b, ip6_header_t *ip6, u32 thread_index)
{
    nat64_main_t *nm = &nat64_main;
    nat64_db_bib_entry_t *bibe;
    nat64_db_st_entry_t *ste;
...
static inline uword
nat64_in2out_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_slow_path)
{
    ...
    stats_node_index = is_slow_path ? nat64_in2out_slowpath_node.index : nat64_in2out_node.index;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ip6_parse(ip60, b0->current_length, &l4_protocol0, &l4_offset0, &frag_offset0))) {
            ...
            if (proto0 == SNAT_PROTOCOL_ICMP) {
                ...
                if (icmp6_to_icmp(b0, nat64_in2out_icmp_set_cb, &ctx0, nat64_in2out_inner_icmp_set_cb, &ctx0)) {
                    next0     = NAT64_IN2OUT_NEXT_DROP;
                    b0->error = node->errors[NAT64_IN2OUT_ERROR_NO_TRANSLATION];
                    goto trace0;
                }
                ...
                if (is_hairpinning(&ip60->dst_address)) {
                    next0 = NAT64_IN2OUT_NEXT_IP6_LOOKUP;
                    if (nat64_in2out_tcp_udp_hairpinning(vm, b0, ip60, thread_index)) {
                        next0     = NAT64_IN2OUT_NEXT_DROP;
                        b0->error = node->errors[NAT64_IN2OUT_ERROR_NO_TRANSLATION];
                    }
                    goto trace0;
                ...
                if (ip6_to_ip4_tcp_udp(b0, nat64_in2out_tcp_udp_set_cb, &ctx0, 0)) {
                    next0     = NAT64_IN2OUT_NEXT_DROP;
                    b0->error = node->errors[NAT64_IN2OUT_ERROR_NO_TRANSLATION];
                    goto trace0;
                }
    ...
    vlib_node_increment_counter(vm, stats_node_index, NAT64_IN2OUT_ERROR_IN2OUT_PACKETS, pkts_processed);
    return frame->n_vectors;
}

static uword
nat64_in2out_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nat64_in2out_slowpath_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static int
nat64_in2out_frag_hairpinning(vlib_buffer_t *b, ip6_header_t *ip6, nat64_in2out_frag_set_ctx_t *ctx)
{
    nat64_main_t *nm = &nat64_main;
    nat64_db_st_entry_t *ste;
    nat64_db_bib_entry_t *bibe;
...
static uword
nat64_in2out_reass_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, *to_next;
    nat64_in2out_next_t next_index;
    u32 pkts_processed         = 0;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            reass0 = nat_ip6_reass_find_or_create(ip60->src_address, ip60->dst_address, frag0->identification,
                                                  l4_protocol0, 1, &fragments_to_drop);
            ...
            if (PREDICT_TRUE(ip6_frag_hdr_offset(frag0))) {
                ...
                if (PREDICT_FALSE(reass0->sess_index == (u32) ~0)) {
                    if (nat_ip6_reass_add_fragment(reass0, bi0, &fragments_to_drop)) {
                ...
                ste0 = nat64_db_st_entry_find(db, &saddr0, &daddr0, udp0->src_port, udp0->dst_port, l4_protocol0,
                                              fib_index0, 1);
                if (!ste0) {
                    bibe0 = nat64_db_bib_entry_find(db, &saddr0, udp0->src_port, l4_protocol0, fib_index0, 1);
                    if (!bibe0) {
                        u16 out_port0;
                        ip4_address_t out_addr0;
                        if (nat64_alloc_out_addr_and_port(fib_index0, proto0, &out_addr0, &out_port0, thread_index)) {
                            next0     = NAT64_IN2OUT_NEXT_DROP;
                            b0->error = node->errors[NAT64_IN2OUT_ERROR_NO_TRANSLATION];
                            goto trace0;
                        }

                        bibe0 = nat64_db_bib_entry_create(db, &ip60->src_address, &out_addr0, udp0->src_port, out_port0,
                                                          fib_index0, l4_protocol0, 0);
                        if (!bibe0) {
                            next0     = NAT64_IN2OUT_NEXT_DROP;
                            b0->error = node->errors[NAT64_IN2OUT_ERROR_NO_TRANSLATION];
                            goto trace0;
                    ...
                    nat64_extract_ip4(&ip60->dst_address, &daddr0.ip4, fib_index0);
                    ste0 = nat64_db_st_entry_create(db, bibe0, &ip60->dst_address, &daddr0.ip4, udp0->dst_port);
                    if (!ste0) {
                        next0     = NAT64_IN2OUT_NEXT_DROP;
                        b0->error = node->errors[NAT64_IN2OUT_ERROR_NO_TRANSLATION];
                        goto trace0;
        ...
        trace0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                nat64_in2out_reass_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->cached                     = cached0;
    ...
    vlib_node_increment_counter(vm, nat64_in2out_reass_node.index, NAT64_IN2OUT_ERROR_IN2OUT_PACKETS, pkts_processed);

    nat_send_all_to_node(vm, fragments_to_drop, node, &node->errors[NAT64_IN2OUT_ERROR_DROP_FRAGMENT],
                         NAT64_IN2OUT_NEXT_DROP);
...
#define foreach_nat64_in2out_handoff_error _(CONGESTION_DROP, "congestion drop")
...
static inline uword
nat64_in2out_handoff_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    nat64_main_t *nm = &nat64_main;
    ...
    n_enq = vlib_buffer_enqueue_to_thread(vm, fq_index, from, thread_indices, frame->n_vectors, 1);
