...
nat_free_session_data(snat_main_t *sm, snat_session_t *s, u32 thread_index)
{
    ...
    if (is_ed_session(s)) {
        if (is_affinity_sessions(s))
            nat_affinity_unlock(s->ext_host_addr, s->out2in.addr, s->in2out.protocol, s->out2in.port);
    ...
    /* log NAT event */
    snat_ipfix_logging_nat44_ses_delete(s->in2out.addr.as_u32, s->out2in.addr.as_u32, s->in2out.protocol,
    ...
    if (is_twice_nat_session(s)) {
        ...
        snat_free_outside_address_and_port(sm->twice_nat_addresses, thread_index, &key);
    }

    if (snat_is_session_static(s))
        return;

    snat_free_outside_address_and_port(sm->addresses, thread_index, &s->out2in);
}

snat_user_t *
nat_user_get_or_create(snat_main_t *sm, ip4_address_t *addr, u32 fib_index, u32 thread_index)
{
    snat_user_t *u = 0;
...
nat_session_alloc_or_recycle(snat_main_t *sm, snat_user_t *u, u32 thread_index)
{
    ...
    if ((u->nsessions + u->nstaticsessions) >= sm->max_translations_per_user) {
        oldest_per_user_translation_list_index =
            clib_dlist_remove_head(tsm->list_pool, u->sessions_per_user_list_head_index);

        ASSERT(oldest_per_user_translation_list_index != ~0);

        /* Add it back to the end of the LRU list */
        clib_dlist_addtail(tsm->list_pool, u->sessions_per_user_list_head_index, oldest_per_user_translation_list_index);
        /* Get the list element */
        oldest_per_user_translation_list_elt = pool_elt_at_index(tsm->list_pool, oldest_per_user_translation_list_index);
        ...
        clib_dlist_init(tsm->list_pool, per_user_translation_list_elt - tsm->list_pool);
        ...
        clib_dlist_addtail(tsm->list_pool, s->per_user_list_head_index, per_user_translation_list_elt - tsm->list_pool);
    }
...
snat_session_t *
nat_ed_session_alloc(snat_main_t *sm, snat_user_t *u, u32 thread_index, f64 now)
{
    snat_session_t *s;
    snat_main_per_thread_data_t *tsm = &sm->per_thread_data[thread_index];
    ...
    if ((u->nsessions + u->nstaticsessions) >= sm->max_translations_per_user) {
        oldest_index      = clib_dlist_remove_head(tsm->list_pool, u->sessions_per_user_list_head_index);
        oldest_elt        = pool_elt_at_index(tsm->list_pool, oldest_index);
        s                 = pool_elt_at_index(tsm->sessions, oldest_elt->value);
        sess_timeout_time = s->last_heard + (f64) nat44_session_get_timeout(sm, s);
        if (now >= sess_timeout_time) {
            clib_dlist_addtail(tsm->list_pool, u->sessions_per_user_list_head_index, oldest_index);
            nat_free_session_data(sm, s, thread_index);
            if (snat_is_session_static(s))
                u->nstaticsessions--;
            else
            ...
            clib_dlist_addhead(tsm->list_pool, u->sessions_per_user_list_head_index, oldest_index);
            nat_log_warn("max translations per user %U", format_ip4_address, &u->addr);
            snat_ipfix_logging_max_entries_per_user(sm->max_translations_per_user, u->addr.as_u32);
            return 0;
        }
    } else {
        pool_get(tsm->sessions, s);
...
void
snat_add_del_addr_to_fib(ip4_address_t *addr, u8 p_len, u32 sw_if_index, int is_add)
{
    fib_prefix_t prefix = {
        .fp_len   = p_len,
    ...
    if (is_add)
        fib_table_entry_update_one_path(fib_index, &prefix, FIB_SOURCE_PLUGIN_LOW,
...
int
snat_add_address(snat_main_t *sm, ip4_address_t *addr, u32 vrf_id, u8 twice_nat)
{
    snat_address_t *ap;
    snat_interface_t *i;
    ...
    ap->addr = *addr;
    if (vrf_id != ~0)
        ap->fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, vrf_id, FIB_SOURCE_PLUGIN_LOW);
    else
        ap->fib_index = ~0;
#define _(N, i, n, s)                                                                                                  \
...
static void
snat_add_static_mapping_when_resolved(snat_main_t *sm, ip4_address_t l_addr, u16 l_port, u32 sw_if_index, u16 e_port,
                                      u32 vrf_id, snat_protocol_t proto, int addr_only, int is_add, u8 *tag,
                                      int twice_nat, int out2in_only, int identity_nat)
{
...
int
snat_add_static_mapping(ip4_address_t l_addr, ip4_address_t e_addr, u16 l_port, u16 e_port, u32 vrf_id, int addr_only,
                        u32 sw_if_index, snat_protocol_t proto, int is_add, twice_nat_type_t twice_nat, u8 out2in_only,
                        u8 *tag, u8 identity_nat)
{
    ...
    if (sw_if_index != ~0) {
        ...
        for (i = 0; i < vec_len(sm->to_resolve); i++) {
            rp = sm->to_resolve + i;
            if (rp->sw_if_index != sw_if_index || rp->l_addr.as_u32 != l_addr.as_u32 || rp->vrf_id != vrf_id ||
                rp->addr_only != addr_only)
                continue;
        ...
        first_int_addr = ip4_interface_first_address(sm->ip4_main, sw_if_index, 0 /* just want the address */);

        if (is_add) {
            if (rp_match)
                return VNET_API_ERROR_VALUE_EXIST;

            snat_add_static_mapping_when_resolved(sm, l_addr, l_port, sw_if_index, e_port, vrf_id, proto, addr_only,
                                                  is_add, tag, twice_nat, out2in_only, identity_nat);
    ...
    if (is_add) {
        if (m) {
            if (is_identity_static_mapping(m)) {
                ...
                local->vrf_id    = vrf_id;
                local->fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, vrf_id, FIB_SOURCE_PLUGIN_LOW);
        ...
        if (vrf_id != ~0)
            fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, vrf_id, FIB_SOURCE_PLUGIN_LOW);
...
#define _(N, j, n, s)                                                                                                  \
    case SNAT_PROTOCOL_##N:                                                                                            \
        ...
        break;
                        foreach_snat_protocol
#undef _
                            default : nat_log_info("unknown protocol");
            ...
            if (!a && (l_addr.as_u32 != e_addr.as_u32)) {
                if (sw_if_index != ~0) {
                    for (i = 0; i < vec_len(sm->to_resolve); i++) {
                        ...
                        if (rp->addr_only)
                            continue;
                        if (rp->sw_if_index != sw_if_index && rp->l_addr.as_u32 != l_addr.as_u32 &&
                            rp->vrf_id != vrf_id && rp->l_port != l_port && rp->e_port != e_port && rp->proto != proto)
                            continue;
...
#undef _
        ...
        if (!(sm->static_mapping_only) || (sm->static_mapping_only && sm->static_mapping_connection_tracking)) {
            u_key.addr      = m->local_addr;
...
int
nat44_add_del_lb_static_mapping(ip4_address_t e_addr, u16 e_port, snat_protocol_t proto, nat44_lb_addr_port_t *locals,
                                u8 is_add, twice_nat_type_t twice_nat, u8 out2in_only, u8 *tag, u32 affinity)
{
    snat_main_t *sm = &snat_main;
    snat_static_mapping_t *m;
    snat_session_key_t m_key;
...
#undef _
        ...
        for (i = 0; i < vec_len(locals); i++) {
            locals[i].fib_index =
                fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, locals[i].vrf_id, FIB_SOURCE_PLUGIN_LOW);
            ...
            if (sm->num_workers > 1) {
                ...
                bitmap = clib_bitmap_set(bitmap, sm->worker_in2out_cb(&ip, m->fib_index), 1);
            }
        }
...
#undef _
        ...
        vec_foreach(local, m->locals)
        {
            fib_table_unlock(local->fib_index, FIB_PROTOCOL_IP4, FIB_SOURCE_PLUGIN_LOW);
            ...
            if (sm->num_workers > 1) {
                ...
                tsm = vec_elt_at_index(sm->per_thread_data, sm->worker_in2out_cb(&ip, m->fib_index));
            } else
                tsm = vec_elt_at_index(sm->per_thread_data, sm->num_workers);
...
int
snat_del_address(snat_main_t *sm, ip4_address_t addr, u8 delete_sm, u8 twice_nat)
{
    snat_address_t *a = 0;
    snat_session_t *ses;
    ...
    if (delete_sm) {
        ...
        pool_foreach(m, sm->static_mappings, ({
                         if (m->external_addr.as_u32 == addr.as_u32)
                             (void) snat_add_static_mapping(m->local_addr, m->external_addr, m->local_port,
                                                            m->external_port, m->vrf_id, is_addr_only_static_mapping(m),
                                                            ~0, m->proto, 0, m->twice_nat,
                                                            is_out2in_only_static_mapping(m), m->tag,
...
snat_interface_add_del(u32 sw_if_index, u8 is_inside, int is_del)
{
    ...
    if (sm->fq_in2out_index == ~0 && !sm->deterministic && sm->num_workers > 1)
        sm->fq_in2out_index = vlib_frame_queue_main_init(sm->in2out_node_index, NAT_FQ_NELTS);

    if (sm->fq_out2in_index == ~0 && !sm->deterministic && sm->num_workers > 1)
        sm->fq_out2in_index = vlib_frame_queue_main_init(sm->out2in_node_index, NAT_FQ_NELTS);
...
feature_set:
                         ...
                         if (is_del) {
                             if (nat_interface_is_inside(i) && nat_interface_is_outside(i)) {
                                 ...
                                 if (sm->num_workers > 1 && !sm->deterministic) {
                                     ...
                                     feature_name =
                                         !is_inside ? "nat44-in2out-worker-handoff" : "nat44-out2in-worker-handoff";
                                 } else if (sm->deterministic) {
                                 ...
                                 if (!is_inside) {
                                     if (sm->endpoint_dependent)
                                         vnet_feature_enable_disable("ip4-local", "nat44-ed-hairpinning", sw_if_index,
                                                                     1, 0, 0);
                                     else if (!sm->deterministic)
                                         vnet_feature_enable_disable("ip4-local", "nat44-hairpinning", sw_if_index, 1,
                                                                     0, 0);
                                 }
                             } else {
                                 ...
                                 if (is_inside) {
                                     if (sm->endpoint_dependent)
                                         ...
                                         vnet_feature_enable_disable("ip4-local", "nat44-hairpinning", sw_if_index, 0,
                                                                     0, 0);
                                 }
                             }
                         } else {
                             if ((nat_interface_is_inside(i) && is_inside) || (nat_interface_is_outside(i) && !is_inside))
                                 return 0;
                             ...
                             if (sm->num_workers > 1 && !sm->deterministic) {
                                 ...
                                 del_feature_name = !is_inside ? "nat44-det-in2out" : "nat44-det-out2in";
                                 feature_name     = "nat44-det-classify";
                             } else if (sm->endpoint_dependent) {
                                 del_feature_name = !is_inside ? "nat44-ed-in2out" : "nat44-ed-out2in";
                             ...
                             if (!is_inside) {
                                 if (sm->endpoint_dependent)
                                     vnet_feature_enable_disable("ip4-local", "nat44-ed-hairpinning", sw_if_index, 0, 0,
                                                                 0);
                                 else if (!sm->deterministic)
                                     vnet_feature_enable_disable("ip4-local", "nat44-hairpinning", sw_if_index, 0, 0, 0);
                             }
                             goto set_flags;
                         }
    ...
    if (is_inside && !sm->out2in_dpo) {
        if (sm->endpoint_dependent)
            vnet_feature_enable_disable("ip4-local", "nat44-ed-hairpinning", sw_if_index, 1, 0, 0);
        else if (!sm->deterministic)
            vnet_feature_enable_disable("ip4-local", "nat44-hairpinning", sw_if_index, 1, 0, 0);
    }
...
fib:
    /* *INDENT-OFF* */
    vec_foreach(ap, sm->addresses) snat_add_del_addr_to_fib(&ap->addr, 32, sw_if_index, !is_del);
    ...
    pool_foreach(dm, sm->det_maps, ({ snat_add_del_addr_to_fib(&dm->out_addr, dm->out_plen, sw_if_index, !is_del); }));
    /* *INDENT-ON* */
...
int
snat_interface_add_del_output_feature(u32 sw_if_index, u8 is_inside, int is_del)
{
    snat_main_t *sm = &snat_main;
    snat_interface_t *i;
    snat_address_t *ap;
    ...
    if (sm->deterministic || (sm->static_mapping_only && !(sm->static_mapping_connection_tracking)))
        return VNET_API_ERROR_UNSUPPORTED;
    ...
    if (is_inside) {
        if (sm->endpoint_dependent) {
            vnet_feature_enable_disable("ip4-unicast", "nat44-ed-hairpin-dst", sw_if_index, !is_del, 0, 0);
            vnet_feature_enable_disable("ip4-output", "nat44-ed-hairpin-src", sw_if_index, !is_del, 0, 0);
        } else {
            vnet_feature_enable_disable("ip4-unicast", "nat44-hairpin-dst", sw_if_index, !is_del, 0, 0);
            vnet_feature_enable_disable("ip4-output", "nat44-hairpin-src", sw_if_index, !is_del, 0, 0);
        }
        goto fq;
    }

    if (sm->num_workers > 1) {
        vnet_feature_enable_disable("ip4-unicast", "nat44-out2in-worker-handoff", sw_if_index, !is_del, 0, 0);
        vnet_feature_enable_disable("ip4-output", "nat44-in2out-output-worker-handoff", sw_if_index, !is_del, 0, 0);
    } else {
        if (sm->endpoint_dependent) {
            vnet_feature_enable_disable("ip4-unicast", "nat44-ed-out2in", sw_if_index, !is_del, 0, 0);
            vnet_feature_enable_disable("ip4-output", "nat44-ed-in2out-output", sw_if_index, !is_del, 0, 0);
        } else {
            vnet_feature_enable_disable("ip4-unicast", "nat44-out2in", sw_if_index, !is_del, 0, 0);
            vnet_feature_enable_disable("ip4-output", "nat44-in2out-output", sw_if_index, !is_del, 0, 0);
        }
    }

fq:
    if (sm->fq_in2out_output_index == ~0 && sm->num_workers > 1)
        sm->fq_in2out_output_index = vlib_frame_queue_main_init(sm->in2out_output_node_index, 0);
...
static void snat_ip4_add_del_interface_address_cb(ip4_main_t *im, uword opaque, u32 sw_if_index, ip4_address_t *address,
                                                  u32 address_length, u32 if_address_index, u32 is_delete);

static void nat_ip4_add_del_addr_only_sm_cb(ip4_main_t *im, uword opaque, u32 sw_if_index, ip4_address_t *address,
                                            u32 address_length, u32 if_address_index, u32 is_delete);

static int nat_alloc_addr_and_port_default(snat_address_t *addresses, u32 fib_index, u32 thread_index,
                                           snat_session_key_t *k, u16 port_per_thread, u32 snat_thread_index);
...
void
snat_free_outside_address_and_port(snat_address_t *addresses, u32 thread_index, snat_session_key_t *k)
{
    snat_address_t *a;
    u32 address_index;
...
#define _(N, i, n, s)                                                                                                  \
    case SNAT_PROTOCOL_##N:                                                                                            \
        ASSERT(clib_bitmap_get_no_check(a->busy_##n##_port_bitmap, port_host_byte_order) == 1);                        \
...
int
snat_static_mapping_match(snat_main_t *sm, snat_session_key_t match, snat_session_key_t *mapping, u8 by_external,
                          u8 *is_addr_only, twice_nat_type_t *twice_nat, lb_nat_type_t *lb,
                          ip4_address_t *ext_host_addr, u8 *is_identity_nat)
{
    ...
    if (by_external) {
        if (is_lb_static_mapping(m)) {
            ...
            if (m->affinity) {
                if (nat_affinity_find_and_lock(ext_host_addr[0], match.addr, match.protocol, match.port, &backend_index))
                    goto get_local;
        ...
        get_local:
            ...
            if (m->affinity) {
                if (nat_affinity_create_and_lock(ext_host_addr[0], match.addr, match.protocol, match.port, lo,
                                                 m->affinity, m->affinity_per_service_list_head_index))
            ...
            mapping->port = is_addr_only_static_mapping(m) ? match.port : clib_host_to_net_u16(m->local_port);
        }
        mapping->protocol = m->proto;
    } else {
        mapping->addr = m->external_addr;
        /* Address only mapping doesn't change port */
        mapping->port      = is_addr_only_static_mapping(m) ? match.port : clib_host_to_net_u16(m->external_port);
...
{
    snat_main_t *sm = &snat_main;
    return min + random_u32(&sm->random_seed) / (random_u32_max() / (max - min + 1) + 1);
}

int
snat_alloc_outside_address_and_port(snat_address_t *addresses, u32 fib_index, u32 thread_index, snat_session_key_t *k,
                                    u16 port_per_thread, u32 snat_thread_index)
{
    snat_main_t *sm = &snat_main;

    return sm->alloc_addr_and_port(addresses, fib_index, thread_index, k, port_per_thread, snat_thread_index);
}

static int
nat_alloc_addr_and_port_default(snat_address_t *addresses, u32 fib_index, u32 thread_index, snat_session_key_t *k,
                                u16 port_per_thread, u32 snat_thread_index)
{
    int i;
    ...
    case SNAT_PROTOCOL_##N:                                                                                            \
        if (a->busy_##n##_ports_per_thread[thread_index] < port_per_thread) {                                          \
            if (a->fib_index == fib_index) {                                                                           \
                while (1) {                                                                                            \
                    portnum = (port_per_thread * snat_thread_index) + snat_random_port(1, port_per_thread) + 1024;     \
...
static int
nat_alloc_addr_and_port_mape(snat_address_t *addresses, u32 fib_index, u32 thread_index, snat_session_key_t *k,
                             u16 port_per_thread, u32 snat_thread_index)
{
    snat_main_t *sm   = &snat_main;
...
static int
nat_alloc_addr_and_port_range(snat_address_t *addresses, u32 fib_index, u32 thread_index, snat_session_key_t *k,
                              u16 port_per_thread, u32 snat_thread_index)
{
    snat_main_t *sm   = &snat_main;
    snat_address_t *a = addresses;
...
nat44_add_del_address_dpo(ip4_address_t addr, u8 is_add)
{
    ...
    if (is_add) {
        ...
        fib_table_entry_special_dpo_add(0, &pfx, FIB_SOURCE_PLUGIN_HI, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo_v4);
        dpo_reset(&dpo_v4);
    } else {
...
format_static_mapping_kvp(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U static-mapping-index %llu", format_static_mapping_key, &k, v->value);
...
format_ed_session_kvp(u8 *s, va_list *args)
{
    ...
    s = format(s, "local %U:%d remote %U:%d proto %U fib %d session-index %llu", format_ip4_address, &k.l_addr,
               clib_net_to_host_u16(k.l_port), format_ip4_address, &k.r_addr, clib_net_to_host_u16(k.r_port),
...
snat_get_worker_in2out_cb(ip4_header_t *ip0, u32 rx_fib_index0)
{
    ...
    hash              = ip0->src_address.as_u32 + (ip0->src_address.as_u32 >> 8) + (ip0->src_address.as_u32 >> 16) +
           (ip0->src_address.as_u32 >> 24);
...
snat_get_worker_out2in_cb(ip4_header_t *ip0, u32 rx_fib_index0)
{
    ...
    if (PREDICT_FALSE(ip4_is_fragment(ip0))) {
        ...
        if (PREDICT_TRUE(!ip4_is_first_fragment(ip0))) {
            ...
            reass = nat_ip4_reass_find(ip0->src_address, ip0->dst_address, ip0->fragment_id, ip0->protocol);
...
nat44_ed_get_worker_out2in_cb(ip4_header_t *ip, u32 rx_fib_index)
{
    ...
    /* try static mappings with port */
    if (PREDICT_FALSE(pool_elts(sm->static_mappings))) {
        make_sm_kv(&kv, &ip->dst_address, proto, rx_fib_index, clib_net_to_host_u16(port));
        if (!clib_bihash_search_8_8(&sm->static_mapping_by_external, &kv, &value)) {
            m = pool_elt_at_index(sm->static_mappings, value.value);
            if (!is_lb_static_mapping(m))
                return m->workers[0];

            hash = ip->src_address.as_u32 + (ip->src_address.as_u32 >> 8) + (ip->src_address.as_u32 >> 16) +
...
snat_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "translation hash buckets %d", &translation_buckets))
            ;
        else if (unformat(input, "translation hash memory %d", &translation_memory_size))
            ;
    ...
    if (sm->deterministic && sm->endpoint_dependent)
        return clib_error_return(0, "deterministic and endpoint-dependent modes are mutually exclusive");

    if (static_mapping_only && (sm->deterministic || sm->endpoint_dependent))
        return clib_error_return(0, "static mapping only mode available only for simple nat");

    if (sm->out2in_dpo && (sm->deterministic || sm->endpoint_dependent))
        return clib_error_return(0, "out2in dpo mode available only for simple nat");
    ...
    sm->outside_fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, outside_vrf_id, FIB_SOURCE_PLUGIN_HI);
    nm->outside_vrf_id    = outside_ip6_vrf_id;
    nm->outside_fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, outside_ip6_vrf_id, FIB_SOURCE_PLUGIN_HI);
    sm->inside_vrf_id     = inside_vrf_id;
    sm->inside_fib_index    = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, inside_vrf_id, FIB_SOURCE_PLUGIN_HI);
    sm->static_mapping_only = static_mapping_only;
    sm->static_mapping_connection_tracking = static_mapping_connection_tracking;

    nat64_set_hash(nat64_bib_buckets, nat64_bib_memory_size, nat64_st_buckets, nat64_st_memory_size);
    ...
    if (sm->deterministic) {
        ...
        if (!static_mapping_only || (static_mapping_only && static_mapping_connection_tracking)) {
            /* *INDENT-OFF* */
            vec_foreach(tsm, sm->per_thread_data)
            {
                if (sm->endpoint_dependent) {
                    clib_bihash_init_16_8(&tsm->in2out_ed, "in2out-ed", translation_buckets, translation_memory_size);
                    clib_bihash_set_kvp_format_fn_16_8(&tsm->in2out_ed, format_ed_session_kvp);

                    clib_bihash_init_16_8(&tsm->out2in_ed, "out2in-ed", translation_buckets, translation_memory_size);
                    clib_bihash_set_kvp_format_fn_16_8(&tsm->out2in_ed, format_ed_session_kvp);
                } else {
                    clib_bihash_init_8_8(&tsm->in2out, "in2out", translation_buckets, translation_memory_size);
                    clib_bihash_set_kvp_format_fn_8_8(&tsm->in2out, format_session_kvp);

                    clib_bihash_init_8_8(&tsm->out2in, "out2in", translation_buckets, translation_memory_size);
                    clib_bihash_set_kvp_format_fn_8_8(&tsm->out2in, format_session_kvp);
                }

                clib_bihash_init_8_8(&tsm->user_hash, "users", user_buckets, user_memory_size);
        ...
        clib_bihash_init_8_8(&sm->static_mapping_by_local, "static_mapping_by_local", static_mapping_buckets,
                             static_mapping_memory_size);
        clib_bihash_set_kvp_format_fn_8_8(&sm->static_mapping_by_local, format_static_mapping_kvp);

        clib_bihash_init_8_8(&sm->static_mapping_by_external, "static_mapping_by_external", static_mapping_buckets,
                             static_mapping_memory_size);
        clib_bihash_set_kvp_format_fn_8_8(&sm->static_mapping_by_external, format_static_mapping_kvp);
    }
...
static void
nat_ip4_add_del_addr_only_sm_cb(ip4_main_t *im, uword opaque, u32 sw_if_index, ip4_address_t *address,
                                u32 address_length, u32 if_address_index, u32 is_delete)
{
    snat_main_t *sm = &snat_main;
    snat_static_map_resolve_t *rp;
...
match:
    ...
    rv = snat_add_static_mapping(l_addr, address[0], rp->l_port, rp->e_port, rp->vrf_id, rp->addr_only,
                                 ~0 /* sw_if_index */, rp->proto, !is_delete, rp->twice_nat, rp->out2in_only, rp->tag,
                                 rp->identity_nat);
    if (rv)
...
static void
snat_ip4_add_del_interface_address_cb(ip4_main_t *im, uword opaque, u32 sw_if_index, ip4_address_t *address,
                                      u32 address_length, u32 if_address_index, u32 is_delete)
{
    snat_main_t *sm = &snat_main;
    snat_static_map_resolve_t *rp;
    ...
    if (!is_delete) {
        ...
        for (j = 0; j < vec_len(sm->to_resolve); j++) {
            ...
            if (rp->sw_if_index == sw_if_index) {
                ...
                /* Add the static mapping */
                rv = snat_add_static_mapping(l_addr, address[0], rp->l_port, rp->e_port, rp->vrf_id, rp->addr_only,
                                             ~0 /* sw_if_index */, rp->proto, rp->is_add, rp->twice_nat,
...
int
snat_add_interface_address(snat_main_t *sm, u32 sw_if_index, int is_del, u8 twice_nat)
{
    ...
    int i, j;
    u32 *auto_add_sw_if_indices = twice_nat ? sm->auto_add_sw_if_indices_twice_nat : sm->auto_add_sw_if_indices;
...
int
nat44_del_session(snat_main_t *sm, ip4_address_t *addr, u16 port, snat_protocol_t proto, u32 vrf_id, int is_in)
{
    snat_main_per_thread_data_t *tsm;
    clib_bihash_kv_8_8_t kv, value;
    ip4_header_t ip;
    ...
    if (sm->num_workers > 1)
        tsm = vec_elt_at_index(sm->per_thread_data, sm->worker_in2out_cb(&ip, fib_index));
    else
        tsm = vec_elt_at_index(sm->per_thread_data, sm->num_workers);
...
int
nat44_del_ed_session(snat_main_t *sm, ip4_address_t *addr, u16 port, ip4_address_t *eh_addr, u16 eh_port, u8 proto,
