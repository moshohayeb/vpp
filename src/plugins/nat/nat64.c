...
static u8 well_known_prefix[] = {0x00, 0x64, 0xff, 0x9b, 0x00, 0x00, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

/* *INDENT-ON* */

static void
nat64_ip4_add_del_interface_address_cb(ip4_main_t *im, uword opaque, u32 sw_if_index, ip4_address_t *address,
...
nat64_get_worker_out2in(ip4_header_t *ip)
{
    ...
    if (PREDICT_FALSE(ip4_is_fragment(ip))) {
        ...
        if (PREDICT_TRUE(!ip4_is_first_fragment(ip))) {
            ...
            reass = nat_ip4_reass_find(ip->src_address, ip->dst_address, ip->fragment_id, ip->protocol);
...
void
nat64_set_hash(u32 bib_buckets, u32 bib_memory_size, u32 st_buckets, u32 st_memory_size)
{
    nat64_main_t *nm = &nat64_main;
    nat64_db_t *db;
    ...
    vec_foreach(db, nm->db)
    {
        if (nat64_db_init(db, bib_buckets, bib_memory_size, st_buckets, st_memory_size, nat64_free_out_addr_and_port))
...
nat64_add_del_pool_addr(ip4_address_t *addr, u32 vrf_id, u8 is_add)
{
    ...
    if (is_add) {
        ...
        a->fib_index = ~0;
        if (vrf_id != ~0)
            a->fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, vrf_id, FIB_SOURCE_PLUGIN_HI);
#define _(N, id, n, s)                                                                                                 \
...
#undef _
        ...
        if (a->fib_index != ~0)
            fib_table_unlock(a->fib_index, FIB_PROTOCOL_IP6, FIB_SOURCE_PLUGIN_HI);
        /* Delete sessions using address */
        /* *INDENT-OFF* */
        vec_foreach(db, nm->db) nat64_db_free_out_addr(db, &a->addr);
...
nat64_add_del_interface(u32 sw_if_index, u8 is_inside, u8 is_add)
{
    ...
    set_flags:
        ...
        vlib_process_signal_event(nm->sm->vlib_main, nm->nat64_expire_walk_node_index, NAT64_CLEANER_RESCHEDULE, 0);
    ...
    if (!is_inside) {
        /* *INDENT-OFF* */
        vec_foreach(ap, nm->addr_pool) snat_add_del_addr_to_fib(&ap->addr, 32, sw_if_index, is_add);
        /* *INDENT-ON* */
    }
    ...
    if (nm->sm->num_workers > 1) {
        ...
        if (nm->fq_in2out_index == ~0)
            nm->fq_in2out_index = vlib_frame_queue_main_init(nat64_in2out_node.index, 0);
        if (nm->fq_out2in_index == ~0)
            nm->fq_out2in_index = vlib_frame_queue_main_init(nat64_out2in_node.index, 0);
    } else
...
int
nat64_alloc_out_addr_and_port(u32 fib_index, snat_protocol_t proto, ip4_address_t *addr, u16 *port, u32 thread_index)
{
    nat64_main_t *nm = &nat64_main;
    snat_main_t *sm  = nm->sm;
    ...
    rv = sm->alloc_addr_and_port(nm->addr_pool, fib_index, thread_index, &k, sm->port_per_thread, worker_index);
...
static void
nat64_free_out_addr_and_port(struct nat64_db_s *db, ip4_address_t *addr, u16 port, u8 protocol)
{
    nat64_main_t *nm = &nat64_main;
    int i;
    snat_address_t *a;
...
#define _(N, j, n, s)                                                                                                  \
    case SNAT_PROTOCOL_##N:                                                                                            \
        ...
        break;
            foreach_snat_protocol
#undef _
                default : nat_log_notice("unknown protocol");
            return;
        }
        break;
    }
 ...
 */
static uword
nat64_static_bib_worker_fn(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    nat64_main_t *nm = &nat64_main;
    ...
    pool_foreach(static_bib, nm->static_bibs, ({
                     ...
                     if (static_bib->is_add)
                         (void) nat64_db_bib_entry_create(db, &static_bib->in_addr, &static_bib->out_addr,
                                                          static_bib->in_port, static_bib->out_port,
                                                          static_bib->fib_index, static_bib->proto, 1);
                     else {
                         addr.as_u64[0] = static_bib->in_addr.as_u64[0];
                         addr.as_u64[1] = static_bib->in_addr.as_u64[1];
                         bibe           = nat64_db_bib_entry_find(db, &addr, static_bib->in_port, static_bib->proto,
...
int
nat64_add_del_static_bib_entry(ip6_address_t *in_addr, ip4_address_t *out_addr, u16 in_port, u16 out_port, u8 proto,
                               u32 vrf_id, u8 is_add)
{
    nat64_main_t *nm = &nat64_main;
    nat64_db_bib_entry_t *bibe;
    u32 fib_index     = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, vrf_id, FIB_SOURCE_PLUGIN_HI);
...
#define _(N, j, n, s)                                                                                                  \
    case SNAT_PROTOCOL_##N:                                                                                            \
        if (clib_bitmap_get_no_check(a->busy_##n##_port_bitmap, out_port))                                             \
            return VNET_API_ERROR_INVALID_VALUE;                                                                       \
        clib_bitmap_set_no_check(a->busy_##n##_port_bitmap, out_port, 1);                                              \
        if (out_port > 1024) {                                                                                         \
            a->busy_##n##_ports++;                                                                                     \
            a->busy_##n##_ports_per_thread[thread_index]++;                                                            \
        }                                                                                                              \
        break;
                foreach_snat_protocol
#undef _
                    default : memset(&addr, 0, sizeof(addr));
        ...
        if (!nm->sm->num_workers) {
            bibe = nat64_db_bib_entry_create(db, in_addr, out_addr, clib_host_to_net_u16(in_port),
                                             clib_host_to_net_u16(out_port), fib_index, proto, 1);
            if (!bibe)
    ...
    if (nm->sm->num_workers) {
        ...
        vec_foreach(index, to_be_free) pool_put_index(nm->static_bibs, index[0]);
        /* *INDENT-ON* */
        vec_free(to_be_free);
...
void
nat64_tcp_session_set_state(nat64_db_st_entry_t *ste, tcp_header_t *tcp, u8 is_ip6)
{
    switch (ste->tcp_state) {
...
nat64_add_del_prefix(ip6_address_t *prefix, u8 plen, u32 vrf_id, u8 is_add)
{
    ...
    if (is_add) {
        if (!p) {
            ...
            p->fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, vrf_id, FIB_SOURCE_PLUGIN_HI);
            p->vrf_id    = vrf_id;
        }
 ...
 */
static uword
nat64_expire_worker_walk_fn(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    nat64_main_t *nm = &nat64_main;
    u32 thread_index = vm->thread_index;
    nat64_db_t *db   = &nm->db[thread_index];
 ...
 */
static uword
nat64_expire_walk_fn(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    nat64_main_t *nm         = &nat64_main;
    ...
    while (1) {
        ...
        for (i = 0; i < vec_len(worker_vms); i++) {
            ...
            vlib_node_set_interrupt_pending(worker_vm, nat64_expire_worker_walk_node.index);
        }
    }
