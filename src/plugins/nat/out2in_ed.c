...
static inline u32
icmp_out2in_ed_slow_path(snat_main_t *sm, vlib_buffer_t *b0, ip4_header_t *ip0, icmp46_header_t *icmp0,
                         u32 sw_if_index0, u32 rx_fib_index0, vlib_node_runtime_t *node, u32 next0, f64 now,
                         u32 thread_index, snat_session_t **p_s0)
{
    next0 = icmp_out2in(sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, thread_index, p_s0, 0);
...
create_session_for_static_mapping_ed(snat_main_t *sm, vlib_buffer_t *b, snat_session_key_t l_key,
                                     snat_session_key_t e_key, vlib_node_runtime_t *node, u32 thread_index,
                                     twice_nat_type_t twice_nat, lb_nat_type_t lb_nat, f64 now)
{
    ...
    make_ed_kv(&kv, &e_key.addr, &s->ext_host_addr, ip->protocol, e_key.fib_index, e_key.port, s->ext_host_port);
    ...
    if (twice_nat == TWICE_NAT || (twice_nat == TWICE_NAT_SELF && ip->src_address.as_u32 == l_key.addr.as_u32)) {
        eh_key.protocol = e_key.protocol;
        if (snat_alloc_outside_address_and_port(sm->twice_nat_addresses, 0, thread_index, &eh_key, sm->port_per_thread,
        ...
        make_ed_kv(&kv, &l_key.addr, &s->ext_host_nat_addr, ip->protocol, l_key.fib_index, l_key.port,
                   s->ext_host_nat_port);
    } else {
        make_ed_kv(&kv, &l_key.addr, &s->ext_host_addr, ip->protocol, l_key.fib_index, l_key.port, s->ext_host_port);
    }
    kv.value = s - tsm->sessions;
...
static int
next_src_nat(snat_main_t *sm, ip4_header_t *ip, u8 proto, u16 src_port, u16 dst_port, u32 thread_index, u32 rx_fib_index)
{
    clib_bihash_kv_16_8_t kv, value;
    snat_main_per_thread_data_t *tsm = &sm->per_thread_data[thread_index];

    make_ed_kv(&kv, &ip->src_address, &ip->dst_address, proto, rx_fib_index, src_port, dst_port);
...
static void
create_bypass_for_fwd(snat_main_t *sm, ip4_header_t *ip, u32 rx_fib_index, u32 thread_index)
{
...
u32
icmp_match_out2in_ed(snat_main_t *sm, vlib_node_runtime_t *node, u32 thread_index, vlib_buffer_t *b, ip4_header_t *ip,
                     u8 *p_proto, snat_session_key_t *p_value, u8 *p_dont_translate, void *d, void *e)
{
    ...
    if (clib_bihash_search_16_8(&tsm->out2in_ed, &kv, &value)) {
        ...
        if (snat_static_mapping_match(sm, e_key, &l_key, 1, &is_addr_only, 0, 0, 0, &identity_nat)) {
            if (!sm->forwarding_enabled) {
                ...
                dont_translate = 1;
                if (next_src_nat(sm, ip, key.proto, key.l_port, key.r_port, thread_index, rx_fib_index)) {
        ...
        if (PREDICT_FALSE(icmp->type != ICMP4_echo_reply && (icmp->type != ICMP4_echo_request || !is_addr_only))) {
        ...
        s = create_session_for_static_mapping_ed(sm, b, l_key, e_key, node, thread_index, 0, 0,
        ...
        if (PREDICT_FALSE(icmp->type != ICMP4_echo_reply && icmp->type != ICMP4_echo_request &&
...
static snat_session_t *
nat44_ed_out2in_unknown_proto(snat_main_t *sm, vlib_buffer_t *b, ip4_header_t *ip, u32 rx_fib_index, u32 thread_index,
                              f64 now, vlib_main_t *vm, vlib_node_runtime_t *node)
{
    ...
    make_ed_kv(&s_kv, &ip->dst_address, &ip->src_address, ip->protocol, rx_fib_index, 0, 0);
    ...
    if (!clib_bihash_search_16_8(&tsm->out2in_ed, &s_kv, &s_value)) {
        ...
        u = nat_user_get_or_create(sm, &m->local_addr, m->fib_index, thread_index);
        if (!u) {
            nat_log_warn("create NAT user failed");
            return 0;
        }
        ...
        make_ed_kv(&s_kv, &ip->dst_address, &ip->src_address, ip->protocol, m->fib_index, 0, 0);
        s_kv.value = s - tsm->sessions;
...
static inline uword
nat44_ed_out2in_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_slow_path)
{
    ...
    stats_node_index = is_slow_path ? nat44_ed_out2in_slowpath_node.index : nat44_ed_out2in_node.index;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (is_slow_path) {
                if (PREDICT_FALSE(proto0 == ~0)) {
                    s0 = nat44_ed_out2in_unknown_proto(sm, b0, ip0, rx_fib_index0, thread_index, now, vm, node);
                    if (!sm->forwarding_enabled) {
                        if (!s0)
                ...
                if (PREDICT_FALSE(proto0 == SNAT_PROTOCOL_ICMP)) {
                    next0 = icmp_out2in_ed_slow_path(sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, now,
                                                     thread_index, &s0);
                    goto trace00;
                }
            } else {
            ...
            make_ed_kv(&kv0, &ip0->dst_address, &ip0->src_address, ip0->protocol, rx_fib_index0, udp0->dst_port,
                       udp0->src_port);
            ...
            if (clib_bihash_search_16_8(&tsm->out2in_ed, &kv0, &value0)) {
                if (is_slow_path) {
                    ...
                    if (snat_static_mapping_match(sm, e_key0, &l_key0, 1, 0, &twice_nat0, &lb_nat0, &ip0->src_address,
                                                  &identity_nat0)) {
                        /*
                        ...
                        if (!sm->forwarding_enabled) {
                            ...
                            if (next_src_nat(sm, ip0, ip0->protocol, udp0->src_port, udp0->dst_port, thread_index,
                                             rx_fib_index0)) {
                    ...
                    /* Create session initiated by host from external network */
                    s0 = create_session_for_static_mapping_ed(sm, b0, l_key0, e_key0, node, thread_index, twice_nat0,
                                                              lb_nat0, now);
            ...
            if (PREDICT_FALSE(is_twice_nat_session(s0)))
                sum0 = ip_csum_update(sum0, ip0->src_address.as_u32, s0->ext_host_nat_addr.as_u32, ip4_header_t,
                                      src_address);
            ...
            if (PREDICT_TRUE(proto0 == SNAT_PROTOCOL_TCP)) {
                ...
                if (is_twice_nat_session(s0)) {
                    sum0 = ip_csum_update(sum0, ip0->src_address.as_u32, s0->ext_host_nat_addr.as_u32, ip4_header_t,
                                          dst_address);
                    sum0 = ip_csum_update(sum0, tcp0->src_port, s0->ext_host_nat_port, ip4_header_t, length);
        ...
        trace00:
            ...
            if (is_slow_path) {
                if (PREDICT_FALSE(proto1 == ~0)) {
                    s1 = nat44_ed_out2in_unknown_proto(sm, b1, ip1, rx_fib_index1, thread_index, now, vm, node);
                    if (!sm->forwarding_enabled) {
                        if (!s1)
                ...
                if (PREDICT_FALSE(proto1 == SNAT_PROTOCOL_ICMP)) {
                    next1 = icmp_out2in_ed_slow_path(sm, b1, ip1, icmp1, sw_if_index1, rx_fib_index1, node, next1, now,
                                                     thread_index, &s1);
                    goto trace01;
                }
            } else {
            ...
            make_ed_kv(&kv1, &ip1->dst_address, &ip1->src_address, ip1->protocol, rx_fib_index1, udp1->dst_port,
                       udp1->src_port);
            ...
            if (clib_bihash_search_16_8(&tsm->out2in_ed, &kv1, &value1)) {
                if (is_slow_path) {
                    ...
                    if (snat_static_mapping_match(sm, e_key1, &l_key1, 1, 0, &twice_nat1, &lb_nat1, &ip1->src_address,
                                                  &identity_nat1)) {
                        /*
                         * Send DHCP packets to the ipv4 stack, or we won't
                         * be able to use dhcp client on the outside interface
                        ...
                        if (PREDICT_FALSE(proto1 == SNAT_PROTOCOL_UDP &&
                                          (udp1->dst_port == clib_host_to_net_u16(UDP_DST_PORT_dhcp_to_client)))) {
                            vnet_feature_next(&next1, b1);
                            goto trace01;
                        }

                        if (!sm->forwarding_enabled) {
                            b1->error = node->errors[NAT_OUT2IN_ED_ERROR_NO_TRANSLATION];
                            next1     = NAT44_ED_OUT2IN_NEXT_DROP;
                        } else {
                            if (next_src_nat(sm, ip1, ip1->protocol, udp1->src_port, udp1->dst_port, thread_index,
                                             rx_fib_index1)) {
                    ...
                    /* Create session initiated by host from external network */
                    s1 = create_session_for_static_mapping_ed(sm, b1, l_key1, e_key1, node, thread_index, twice_nat1,
                                                              lb_nat1, now);
            ...
            if (PREDICT_FALSE(is_twice_nat_session(s1)))
                sum1 = ip_csum_update(sum1, ip1->src_address.as_u32, s1->ext_host_nat_addr.as_u32, ip4_header_t,
                                      src_address);
            ...
            if (PREDICT_TRUE(proto1 == SNAT_PROTOCOL_TCP)) {
                ...
                if (is_twice_nat_session(s1)) {
                    sum1 = ip_csum_update(sum1, ip1->src_address.as_u32, s1->ext_host_nat_addr.as_u32, ip4_header_t,
                                          dst_address);
                    sum1 = ip_csum_update(sum1, tcp1->src_port, s1->ext_host_nat_port, ip4_header_t, length);
        ...
        trace01:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
                nat44_ed_out2in_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
                t->is_slow_path            = is_slow_path;
    ...
    vlib_node_increment_counter(vm, stats_node_index, NAT_OUT2IN_ED_ERROR_OUT2IN_PACKETS, pkts_processed);
    return frame->n_vectors;
}

static uword
nat44_ed_out2in_fast_path_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nat44_ed_out2in_slow_path_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nat44_ed_out2in_reass_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ip4_is_first_fragment(ip0))) {
                ...
                if (clib_bihash_search_16_8(&per_thread_data->out2in_ed, &kv0, &value0)) {
                    ...
                    e_key0.protocol  = proto0;
                    e_key0.fib_index = rx_fib_index0;
                    if (snat_static_mapping_match(sm, e_key0, &l_key0, 1, 0, &twice_nat0, &lb0, 0, &identity_nat0)) {
                        /*
                         * Send DHCP packets to the ipv4 stack, or we won't
                         * be able to use dhcp client on the outside interface
                         */
    ...
    vlib_node_increment_counter(vm, nat44_ed_out2in_reass_node.index, NAT_OUT2IN_ED_ERROR_OUT2IN_PACKETS, pkts_processed);

    nat_send_all_to_node(vm, fragments_to_drop, node, &node->errors[NAT_OUT2IN_ED_ERROR_DROP_FRAGMENT],
                         NAT44_ED_OUT2IN_NEXT_DROP);
