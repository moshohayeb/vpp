...
int
nat64_db_init(nat64_db_t *db, u32 bib_buckets, u32 bib_memory_size, u32 st_buckets, u32 st_memory_size,
              nat64_db_free_addr_port_function_t free_addr_port_cb)
{
    clib_bihash_init_24_8(&db->bib.in2out, "bib-in2out", bib_buckets, bib_memory_size);

    clib_bihash_init_24_8(&db->bib.out2in, "bib-out2in", bib_buckets, bib_memory_size);

    clib_bihash_init_48_8(&db->st.in2out, "st-in2out", st_buckets, st_memory_size);

    clib_bihash_init_48_8(&db->st.out2in, "st-out2in", st_buckets, st_memory_size);
...
nat64_db_bib_entry_create(nat64_db_t *db, ip6_address_t *in_addr, ip4_address_t *out_addr, u16 in_port, u16 out_port,
                          u32 fib_index, u8 proto, u8 is_static)
{
...
#undef _
    ...
    nat_ipfix_logging_nat64_bib(in_addr, out_addr, proto, in_port, out_port, fib->ft_table_id, 1);
    return bibe;
}
...
#undef _
    ...
    if (bibe->is_static) {
        ...
        vec_foreach(ste_index, ste_to_be_free) nat64_db_st_entry_free(db, pool_elt_at_index(st, ste_index[0]));
    ...
    fib = fib_table_get(bibe->fib_index, FIB_PROTOCOL_IP6);
    nat_ipfix_logging_nat64_bib(&bibe->in_addr, &bibe->out_addr, bibe->proto, bibe->in_port, bibe->out_port,
                                fib->ft_table_id, 0);
...
nat64_db_bib_entry_t *
nat64_db_bib_entry_find(nat64_db_t *db, ip46_address_t *addr, u16 port, u8 proto, u32 fib_index, u8 is_ip6)
{
...
#undef _
    ...
    if (!clib_bihash_search_24_8(is_ip6 ? &db->bib.in2out : &db->bib.out2in, &kv, &value))
...
void
nat64_db_bib_walk(nat64_db_t *db, u8 proto, nat64_db_bib_walk_fn_t fn, void *ctx)
{
...
void
nat64_db_st_walk(nat64_db_t *db, u8 proto, nat64_db_st_walk_fn_t fn, void *ctx)
{
    nat64_db_st_entry_t *st, *ste;
...
nat64_db_st_entry_t *
nat64_db_st_entry_create(nat64_db_t *db, nat64_db_bib_entry_t *bibe, ip6_address_t *in_r_addr,
...
#undef _
    ...
    fib = fib_table_get(bibe->fib_index, FIB_PROTOCOL_IP6);
    nat_ipfix_logging_nat64_session(&bibe->in_addr, &bibe->out_addr, bibe->proto, bibe->in_port, bibe->out_port,
...
nat64_db_st_entry_t *
nat64_db_st_entry_find(nat64_db_t *db, ip46_address_t *l_addr, ip46_address_t *r_addr, u16 l_port, u16 r_port, u8 proto,
...
#undef _
    ...
    if (!clib_bihash_search_48_8(is_ip6 ? &db->st.in2out : &db->st.out2in, &kv, &value))
