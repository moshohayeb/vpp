...
#define foreach_nat44_handoff_error _(CONGESTION_DROP, "congestion drop")
...
format_nat44_handoff_trace(u8 *s, va_list *args)
{
    ...
    s   = format(s, "NAT44_%s_WORKER_HANDOFF: next-worker %d", tag, t->next_worker_index);
...
static uword
snat_in2out_worker_handoff_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
snat_in2out_output_worker_handoff_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
snat_out2in_worker_handoff_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
