...
format_nat64_out2in_reass_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
    nat64_out2in_reass_trace_t *t   = va_arg(*args, nat64_out2in_reass_trace_t *);

    s = format(s, "NAT64-out2in-reass: sw_if_index %d, next index %d, status %s", t->sw_if_index, t->next_index,
...
static int
nat64_out2in_tcp_udp_set_cb(ip4_header_t *ip4, ip6_header_t *ip6, void *arg)
{
    ...
    if (ste) {
        ...
        ste = nat64_db_st_entry_create(db, bibe, &ip6_saddr, &saddr.ip4, sport);
    }
...
static int
nat64_out2in_inner_icmp_set_cb(ip4_header_t *ip4, ip6_header_t *ip6, void *arg)
{
    nat64_main_t *nm            = &nat64_main;
    nat64_out2in_set_ctx_t *ctx = arg;
    nat64_db_bib_entry_t *bibe;
...
static int
nat64_out2in_unk_proto_set_cb(ip4_header_t *ip4, ip6_header_t *ip6, void *arg)
{
    nat64_main_t *nm            = &nat64_main;
    nat64_out2in_set_ctx_t *ctx = arg;
    nat64_db_bib_entry_t *bibe;
...
static uword
nat64_out2in_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    vlib_node_increment_counter(vm, nat64_out2in_node.index, NAT64_OUT2IN_ERROR_OUT2IN_PACKETS, pkts_processed);
...
static uword
nat64_out2in_reass_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, *to_next;
    nat64_out2in_next_t next_index;
    u32 pkts_processed         = 0;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            reass0 = nat_ip4_reass_find_or_create(ip40->src_address, ip40->dst_address, ip40->fragment_id,
                                                  ip40->protocol, 1, &fragments_to_drop);
            ...
            if (PREDICT_FALSE(ip4_is_first_fragment(ip40))) {
                ...
                ste0 = nat64_db_st_entry_find(db, &daddr0, &saddr0, udp0->dst_port, udp0->src_port, ip40->protocol,
                                              fib_index0, 0);
                if (!ste0) {
                    bibe0 = nat64_db_bib_entry_find(db, &daddr0, udp0->dst_port, ip40->protocol, fib_index0, 0);
                    if (!bibe0) {
                        next0     = NAT64_OUT2IN_NEXT_DROP;
                        b0->error = node->errors[NAT64_OUT2IN_ERROR_NO_TRANSLATION];
                        goto trace0;
                    ...
                    nat64_compose_ip6(&ip6_saddr0, &ip40->src_address, bibe0->fib_index);
                    ste0 = nat64_db_st_entry_create(db, bibe0, &ip6_saddr0, &saddr0.ip4, udp0->src_port);
        ...
        trace0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                nat64_out2in_reass_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->cached                     = cached0;
    ...
    vlib_node_increment_counter(vm, nat64_out2in_reass_node.index, NAT64_OUT2IN_ERROR_OUT2IN_PACKETS, pkts_processed);

    nat_send_all_to_node(vm, fragments_to_drop, node, &node->errors[NAT64_OUT2IN_ERROR_DROP_FRAGMENT],
                         NAT64_OUT2IN_NEXT_DROP);
...
VLIB_NODE_FUNCTION_MULTIARCH(nat64_out2in_reass_node, nat64_out2in_reass_node_fn);

#define foreach_nat64_out2in_handoff_error _(CONGESTION_DROP, "congestion drop")
...
static inline uword
nat64_out2in_handoff_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    nat64_main_t *nm = &nat64_main;
    ...
    if (n_enq < frame->n_vectors)
        vlib_node_increment_counter(vm, node->node_index, NAT64_OUT2IN_HANDOFF_ERROR_CONGESTION_DROP,
...
VLIB_NODE_FUNCTION_MULTIARCH(nat64_out2in_handoff_node, nat64_out2in_handoff_node_fn);
/*
