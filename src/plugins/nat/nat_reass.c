...
nat_ip4_reass_get_frags_inline(nat_reass_ip4_t *reass, u32 **bi)
{
    ...
    while ((elt_index = clib_dlist_remove_head(srm->ip4_frags_list_pool, reass->frags_per_reass_list_head_index)) != ~0) {
...
nat_ip6_reass_get_frags_inline(nat_reass_ip6_t *reass, u32 **bi)
{
    ...
    while ((elt_index = clib_dlist_remove_head(srm->ip6_frags_list_pool, reass->frags_per_reass_list_head_index)) != ~0) {
...
int
nat_reass_set(u32 timeout, u16 max_reass, u8 max_frag, u8 drop_frag, u8 is_ip6)
{
    nat_reass_main_t *srm = &nat_reass_main;
    u32 nbuckets;
    ...
    if (is_ip6) {
        if (srm->ip6_max_reass != max_reass) {
            ...
            clib_bihash_init_48_8(&srm->ip6_reass_hash, "nat-ip6-reass", nbuckets, nbuckets * 1024);
        ...
        if (srm->ip4_max_reass != max_reass) {
            ...
            clib_bihash_init_16_8(&srm->ip4_reass_hash, "nat-ip4-reass", nbuckets, nbuckets * 1024);
...
nat_reass_ip4_t *
nat_ip4_reass_find(ip4_address_t src, ip4_address_t dst, u16 frag_id, u8 proto)
{
    nat_reass_main_t *srm  = &nat_reass_main;
...
nat_reass_ip4_t *
nat_ip4_reass_find_or_create(ip4_address_t src, ip4_address_t dst, u16 frag_id, u8 proto, u8 reset_timeout,
                             u32 **bi_to_drop)
{
    nat_reass_main_t *srm  = &nat_reass_main;
    nat_reass_ip4_t *reass = 0;
    ...
    if (reass) {
        if (reset_timeout) {
            ...
            clib_dlist_remove(srm->ip4_reass_lru_list_pool, reass->lru_list_index);
            clib_dlist_addtail(srm->ip4_reass_lru_list_pool, srm->ip4_reass_head_index, reass->lru_list_index);
        }
    ...
    if (srm->ip4_reass_n >= srm->ip4_max_reass) {
        oldest_index = clib_dlist_remove_head(srm->ip4_reass_lru_list_pool, srm->ip4_reass_head_index);
        ASSERT(oldest_index != ~0);
        oldest_elt = pool_elt_at_index(srm->ip4_reass_lru_list_pool, oldest_index);
        reass      = pool_elt_at_index(srm->ip4_reass_pool, oldest_elt->value);
        if (now < reass->last_heard + (f64) srm->ip4_timeout) {
            clib_dlist_addhead(srm->ip4_reass_lru_list_pool, srm->ip4_reass_head_index, oldest_index);
            nat_log_warn("no free resassembly slot");
            reass = 0;
            goto unlock;
        }

        clib_dlist_addtail(srm->ip4_reass_lru_list_pool, srm->ip4_reass_head_index, oldest_index);
        ...
        clib_dlist_addtail(srm->ip4_reass_lru_list_pool, srm->ip4_reass_head_index, elt_index);
        pool_get(srm->ip4_frags_list_pool, per_reass_list_head_elt);
        reass->frags_per_reass_list_head_index = per_reass_list_head_elt - srm->ip4_frags_list_pool;
        clib_dlist_init(srm->ip4_frags_list_pool, reass->frags_per_reass_list_head_index);
        srm->ip4_reass_n++;
    }
...
int
nat_ip4_reass_add_fragment(nat_reass_ip4_t *reass, u32 bi, u32 **bi_to_drop)
{
    nat_reass_main_t *srm = &nat_reass_main;
    dlist_elt_t *elt;
    u32 elt_index;
    ...
    if (reass->frag_n >= srm->ip4_max_frag) {
        nat_ipfix_logging_max_fragments_ip4(srm->ip4_max_frag, &reass->key.src);
    ...
    elt->value = bi;
    clib_dlist_addtail(srm->ip4_frags_list_pool, reass->frags_per_reass_list_head_index, elt_index);
    reass->frag_n++;
...
nat_reass_ip6_t *
nat_ip6_reass_find_or_create(ip6_address_t src, ip6_address_t dst, u32 frag_id, u8 proto, u8 reset_timeout,
                             u32 **bi_to_drop)
{
    nat_reass_main_t *srm  = &nat_reass_main;
    ...
    if (reass) {
        if (reset_timeout) {
            reass->last_heard = now;
            clib_dlist_remove(srm->ip6_reass_lru_list_pool, reass->lru_list_index);
            clib_dlist_addtail(srm->ip6_reass_lru_list_pool, srm->ip6_reass_head_index, reass->lru_list_index);
        }
    ...
    if (srm->ip6_reass_n >= srm->ip6_max_reass) {
        oldest_index = clib_dlist_remove_head(srm->ip6_reass_lru_list_pool, srm->ip6_reass_head_index);
        ASSERT(oldest_index != ~0);
        oldest_elt = pool_elt_at_index(srm->ip4_reass_lru_list_pool, oldest_index);
        reass      = pool_elt_at_index(srm->ip6_reass_pool, oldest_elt->value);
        if (now < reass->last_heard + (f64) srm->ip6_timeout) {
            clib_dlist_addhead(srm->ip6_reass_lru_list_pool, srm->ip6_reass_head_index, oldest_index);
            nat_log_warn("no free resassembly slot");
            reass = 0;
            goto unlock;
        }

        clib_dlist_addtail(srm->ip6_reass_lru_list_pool, srm->ip6_reass_head_index, oldest_index);
        ...
        clib_dlist_addtail(srm->ip6_reass_lru_list_pool, srm->ip6_reass_head_index, elt_index);
        pool_get(srm->ip6_frags_list_pool, per_reass_list_head_elt);
        reass->frags_per_reass_list_head_index = per_reass_list_head_elt - srm->ip6_frags_list_pool;
        clib_dlist_init(srm->ip6_frags_list_pool, reass->frags_per_reass_list_head_index);
        srm->ip6_reass_n++;
    }
...
int
nat_ip6_reass_add_fragment(nat_reass_ip6_t *reass, u32 bi, u32 **bi_to_drop)
{
    nat_reass_main_t *srm = &nat_reass_main;
    dlist_elt_t *elt;
    u32 elt_index;
    ...
    if (reass->frag_n >= srm->ip6_max_frag) {
        nat_ipfix_logging_max_fragments_ip6(srm->ip6_max_frag, &reass->key.src);
        reass->flags |= NAT_REASS_FLAG_MAX_FRAG_DROP;
        nat_ip6_reass_get_frags_inline(reass, bi_to_drop);
        return -1;
    }
    ...
    elt->value = bi;
    clib_dlist_addtail(srm->ip6_frags_list_pool, reass->frags_per_reass_list_head_index, elt_index);
    reass->frag_n++;
...
nat_reass_init(vlib_main_t *vm)
{
    ...
    srm->ip4_reass_head_index = head_index = head - srm->ip4_reass_lru_list_pool;
    ...
    srm->ip6_reass_head_index = head_index = head - srm->ip6_reass_lru_list_pool;
...
static clib_error_t *
nat_reass_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    ...
    rv = nat_reass_set(timeout, (u16) max_reass, (u8) max_frag, drop_frag, is_ip6);
    if (rv) {
...
nat_ip4_reass_walk_cli(nat_reass_ip4_t *reass, void *ctx)
{
    ...
    vlib_cli_output(vm,
                    "  src %U dst %U proto %u id 0x%04x cached %u "
                    "flags %s classify_next %s",
                    format_ip4_address, &reass->key.src, format_ip4_address, &reass->key.dst, reass->key.proto,
                    clib_net_to_host_u16(reass->key.frag_id), reass->frag_n, flags_str, classify_next_str);
...
nat_ip6_reass_walk_cli(nat_reass_ip6_t *reass, void *ctx)
{
    ...
    vlib_cli_output(vm, "  src %U dst %U proto %u id 0x%08x cached %u", format_ip6_address, &reass->key.src,
                    format_ip6_address, &reass->key.dst, reass->key.proto, clib_net_to_host_u32(reass->key.frag_id),
                    reass->frag_n);
...
static clib_error_t *
show_nat_reass_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
