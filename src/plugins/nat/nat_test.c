...
#define foreach_vpe_api_reply_msg                                                                                      \
    _(NAT44_ADD_DEL_ADDRESS_RANGE_REPLY, nat44_add_del_address_range_reply)                                            \
    _(NAT44_INTERFACE_ADD_DEL_FEATURE_REPLY, nat44_interface_add_del_feature_reply)                                    \
    _(NAT44_ADD_DEL_STATIC_MAPPING_REPLY, nat44_add_del_static_mapping_reply)                                          \
    ...
    _(NAT44_ADD_DEL_INTERFACE_ADDR_REPLY, nat44_add_del_interface_addr_reply)                                          \
    _(NAT44_INTERFACE_ADDR_DETAILS, nat44_interface_addr_details)                                                      \
    _(NAT_IPFIX_ENABLE_DISABLE_REPLY, nat_ipfix_enable_disable_reply)                                                  \
    ...
    _(NAT_DET_CLOSE_SESSION_OUT_REPLY, nat_det_close_session_out_reply)                                                \
    _(NAT_DET_CLOSE_SESSION_IN_REPLY, nat_det_close_session_in_reply)                                                  \
    _(NAT_DET_SESSION_DETAILS, nat_det_session_details)

static int
api_nat44_add_del_address_range(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U - %U", unformat_ip4_address, &start_addr, unformat_ip4_address, &end_addr))
            ;
...
static int
api_nat44_interface_add_del_feature(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
...
static int
api_nat44_add_del_static_mapping(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "local_addr %U", unformat_ip4_address, &local_addr))
            local_addr_set = 1;
        else if (unformat(i, "external_addr %U", unformat_ip4_address, &external_addr))
            external_addr_set = 1;
        else if (unformat(i, "local_port %u", &local_port))
            addr_only = 0;
        else if (unformat(i, "external_port %u", &external_port))
            addr_only = 0;
        else if (unformat(i, "external_if %U", unformat_sw_if_index, vam, &sw_if_index))
            sw_if_index_set = 1;
...
static void
vl_api_nat_control_ping_reply_t_handler(vl_api_nat_control_ping_reply_t *mp)
{
...
static void
vl_api_nat44_static_mapping_details_t_handler(vl_api_nat44_static_mapping_details_t *mp)
{
    ...
    if (mp->addr_only && mp->external_sw_if_index != ~0)
        fformat(vam->ofp, "%15U%6s%15d%6s%11d%6d\n", format_ip4_address, &mp->local_ip_address, "",
                ntohl(mp->external_sw_if_index), "", ntohl(mp->vrf_id), mp->protocol);
    else if (mp->addr_only && mp->external_sw_if_index == ~0)
        fformat(vam->ofp, "%15U%6s%15U%6s%11d%6d\n", format_ip4_address, &mp->local_ip_address, "", format_ip4_address,
                &mp->external_ip_address, "", ntohl(mp->vrf_id), mp->protocol);
    else if (!mp->addr_only && mp->external_sw_if_index != ~0)
        fformat(vam->ofp, "%15U%6d%15d%6d%11d%6d\n", format_ip4_address, &mp->local_ip_address, ntohs(mp->local_port),
                ntohl(mp->external_sw_if_index), ntohs(mp->external_port), ntohl(mp->vrf_id), mp->protocol);
    else
        fformat(vam->ofp, "%15U%6d%15U%6d%11d%6d\n", format_ip4_address, &mp->local_ip_address, ntohs(mp->local_port),
                format_ip4_address, &mp->external_ip_address, ntohs(mp->external_port), ntohl(mp->vrf_id), mp->protocol);
}

static int
api_nat44_static_mapping_dump(vat_main_t *vam)
{
    ...
    fformat(vam->ofp, "%15s%6s%15s%6s%11s%6s\n", "address", "port", "address/if_idx", "port", "vrf", "proto");
...
static void
vl_api_nat_show_config_reply_t_handler(vl_api_nat_show_config_reply_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;
    i32 retval           = ntohl(mp->retval);
...
static int
api_nat_show_config(vat_main_t *vam)
{
...
static void
vl_api_nat44_address_details_t_handler(vl_api_nat44_address_details_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;
...
static int
api_nat44_address_dump(vat_main_t *vam)
{
...
static void
vl_api_nat44_interface_details_t_handler(vl_api_nat44_interface_details_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;

    fformat(vam->ofp, "sw_if_index %d %s\n", ntohl(mp->sw_if_index), mp->is_inside ? "in" : "out");
}

static int
api_nat44_interface_dump(vat_main_t *vam)
{
...
static int
api_nat_set_workers(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
...
static void
vl_api_nat_worker_details_t_handler(vl_api_nat_worker_details_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;
...
static int
api_nat_worker_dump(vat_main_t *vam)
{
...
static int
api_nat44_add_del_interface_addr(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
...
static void
vl_api_nat44_interface_addr_details_t_handler(vl_api_nat44_interface_addr_details_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;
...
static int
api_nat44_interface_addr_dump(vat_main_t *vam)
{
...
static int
api_nat_ipfix_enable_disable(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
...
static void
vl_api_nat44_user_session_details_t_handler(vl_api_nat44_user_session_details_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;

    fformat(vam->ofp,
            "%s session %U:%d to %U:%d protocol id %d "
            "total packets %d total bytes %lld\n",
            mp->is_static ? "static" : "dynamic", format_ip4_address, mp->inside_ip_address, ntohs(mp->inside_port),
            format_ip4_address, mp->outside_ip_address, ntohs(mp->outside_port), ntohs(mp->protocol),
            ntohl(mp->total_pkts), clib_net_to_host_u64(mp->total_bytes));
}

static int
api_nat44_user_session_dump(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
    ...
    if (unformat(i, "ip_address %U vrf_id %d", unformat_ip4_address, &addr, &vrf_id))
        ;
    else {
        clib_warning("unknown input '%U'", format_unformat_error, i);
        return -99;
...
static void
vl_api_nat44_user_details_t_handler(vl_api_nat44_user_details_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;

    fformat(vam->ofp,
            "user with ip %U with vrf_id %d "
            "with %d sessions and %d static sessions\n",
            format_ip4_address, mp->ip_address, ntohl(mp->vrf_id), ntohl(mp->nsessions), ntohl(mp->nstaticsessions));
}

static int
api_nat44_user_dump(vat_main_t *vam)
{
...
static int
api_nat_det_add_del_map(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
    ...
    if (unformat(i, "in %U/%d out %U/%d", unformat_ip4_address, &in_addr, &in_plen, unformat_ip4_address, &out_addr,
                 &out_plen))
        ;
    else if (unformat(i, "del"))
        is_add = 0;
...
static void
vl_api_nat_det_forward_reply_t_handler(vl_api_nat_det_forward_reply_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;
    i32 retval           = ntohl(mp->retval);
...
static int
api_nat_det_forward(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
...
static void
vl_api_nat_det_reverse_reply_t_handler(vl_api_nat_det_reverse_reply_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;
    i32 retval           = ntohl(mp->retval);
...
static int
api_nat_det_reverse(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
...
static void
vl_api_nat_det_map_details_t_handler(vl_api_nat_det_map_details_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;

    fformat(vam->ofp,
            "Deterministic S-NAT mapping in %U/%d out %U/%d "
            ...
            ntohs(mp->ports_per_host), ntohl(mp->sharing_ratio), ntohl(mp->ses_num));
}

static int
api_nat_det_map_dump(vat_main_t *vam)
{
...
static int
api_nat_set_timeouts(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
...
static void
vl_api_nat_get_timeouts_reply_t_handler(vl_api_nat_get_timeouts_reply_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;
    i32 retval           = ntohl(mp->retval);
    ...
    if (retval >= 0) {
        ...
        fformat(vam->ofp, "tcp-established timeout: %dsec", ntohl(mp->tcp_established));
        fformat(vam->ofp, "tcp-transitory timeout: %dsec", ntohl(mp->tcp_transitory));
...
static int
api_nat_get_timeouts(vat_main_t *vam)
{
...
static int
api_nat_det_close_session_out(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
    ...
    if (unformat(i, "%U:%d %U:%d", unformat_ip4_address, &out_addr, &out_port, unformat_ip4_address, &ext_addr, &ext_port))
        ;
    else {
        clib_warning("unknown input '%U'", format_unformat_error, i);
        return -99;
...
static int
api_nat_det_close_session_in(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
    ...
    if (unformat(i, "%U:%d %U:%d", unformat_ip4_address, &in_addr, &in_port, unformat_ip4_address, &ext_addr, &ext_port))
        ;
    else {
        clib_warning("unknown input '%U'", format_unformat_error, i);
        return -99;
...
static void
vl_api_nat_det_session_details_t_handler(vl_api_nat_det_session_details_t *mp)
{
    snat_test_main_t *sm = &snat_test_main;
    vat_main_t *vam      = sm->vat_main;

    fformat(vam->ofp,
            "deterministic session, external host address %U, "
            "external host port %d, outer port %d, inside port %d",
            format_ip4_address, mp->ext_addr, mp->ext_port, mp->out_port, mp->in_port);
}

static int
api_nat_det_session_dump(vat_main_t *vam)
{
    unformat_input_t *i = vam->input;
...
#define foreach_vpe_api_msg                                                                                            \
    ...
    _(nat44_interface_add_del_feature, "<intfc> | sw_if_index <id> [in] [out] [del]")                                  \
    ...
    _(nat_worker_dump, "")                                                                                             \
    _(nat44_add_del_interface_addr, "<intfc> | sw_if_index <id> [del]")                                                \
