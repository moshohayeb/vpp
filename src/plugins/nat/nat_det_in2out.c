...
format_nat_det_in2out_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "NAT_DET_IN2OUT: sw_if_index %d, next index %d, session %d", t->sw_if_index, t->next_index,
               t->session_index);
 ...
 */
u32
icmp_match_in2out_det(snat_main_t *sm, vlib_node_runtime_t *node, u32 thread_index, vlib_buffer_t *b0, ip4_header_t *ip0,
                      u8 *p_proto, snat_session_key_t *p_value, u8 *p_dont_translate, void *d, void *e)
{
    ...
    if (PREDICT_FALSE(!dm0)) {
        nat_log_info("no match for internal host %U", format_ip4_address, &in_addr);
    ...
    if (PREDICT_FALSE(!ses0)) {
        ...
        for (i0 = 0; i0 < dm0->ports_per_host; i0++) {
            key0.out_port =
                clib_host_to_net_u16(lo_port0 + ((i0 + clib_net_to_host_u16(echo0->identifier)) % dm0->ports_per_host));
    ...
    if (PREDICT_FALSE(icmp0->type != ICMP4_echo_request && !icmp_is_error_message(icmp0))) {
        b0->error = node->errors[NAT_DET_IN2OUT_ERROR_BAD_ICMP_TYPE];
        next0     = NAT_DET_IN2OUT_NEXT_DROP;
        goto out;
    }
...
static uword
snat_det_in2out_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE(proto0 == SNAT_PROTOCOL_ICMP)) {
                ...
                next0 =
                    icmp_in2out(sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, thread_index, &ses0, &dm0);
                goto trace0;
            }

            dm0 = snat_det_map_by_user(sm, &ip0->src_address);
            if (PREDICT_FALSE(!dm0)) {
                nat_log_info("no match for internal host %U", format_ip4_address, &ip0->src_address);
                next0     = NAT_DET_IN2OUT_NEXT_DROP;
                b0->error = node->errors[NAT_DET_IN2OUT_ERROR_NO_TRANSLATION];
                goto trace0;
            }
            ...
            if (PREDICT_FALSE(!ses0)) {
                for (i0 = 0; i0 < dm0->ports_per_host; i0++) {
                    key0.out_port =
                        clib_host_to_net_u16(lo_port0 + ((i0 + clib_net_to_host_u16(tcp0->src)) % dm0->ports_per_host));
        ...
        trace0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                nat_det_in2out_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
            ...
            if (PREDICT_FALSE(proto1 == SNAT_PROTOCOL_ICMP)) {
                ...
                next1 =
                    icmp_in2out(sm, b1, ip1, icmp1, sw_if_index1, rx_fib_index1, node, next1, thread_index, &ses1, &dm1);
                goto trace1;
            }
            ...
            if (PREDICT_FALSE(!ses1)) {
                for (i1 = 0; i1 < dm1->ports_per_host; i1++) {
                    key1.out_port =
                        clib_host_to_net_u16(lo_port1 + ((i1 + clib_net_to_host_u16(tcp1->src)) % dm1->ports_per_host));
        ...
        trace1:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
                nat_det_in2out_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
    ...
    vlib_node_increment_counter(vm, snat_det_in2out_node.index, NAT_DET_IN2OUT_ERROR_IN2OUT_PACKETS, pkts_processed);
