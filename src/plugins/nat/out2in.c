...
format_snat_out2in_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "NAT44_OUT2IN: sw_if_index %d, next index %d, session index %d", t->sw_if_index, t->next_index,
               t->session_index);
    return s;
}
...
format_snat_out2in_fast_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "NAT44_OUT2IN_FAST: sw_if_index %d, next index %d", t->sw_if_index, t->next_index);
    return s;
}
...
create_session_for_static_mapping(snat_main_t *sm, vlib_buffer_t *b0, snat_session_key_t in2out,
                                  snat_session_key_t out2in, vlib_node_runtime_t *node, u32 thread_index, f64 now)
{
    snat_user_t *u;
    snat_session_t *s;
 ...
 */
u32
icmp_match_out2in_slow(snat_main_t *sm, vlib_node_runtime_t *node, u32 thread_index, vlib_buffer_t *b0, ip4_header_t *ip0,
                       u8 *p_proto, snat_session_key_t *p_value, u8 *p_dont_translate, void *d, void *e)
{
    ...
    if (clib_bihash_search_8_8(&sm->per_thread_data[thread_index].out2in, &kv0, &value0)) {
        ...
        if (snat_static_mapping_match(sm, key0, &sm0, 1, &is_addr_only, 0, 0, 0, &identity_nat)) {
            if (!sm->forwarding_enabled) {
                /* Don't NAT packet aimed at the intfc address */
                if (PREDICT_FALSE(is_interface_addr(sm, node, sw_if_index0, ip0->dst_address.as_u32))) {
                    dont_translate = 1;
                    goto out;
                }
        ...
        if (PREDICT_FALSE(icmp0->type != ICMP4_echo_reply && (icmp0->type != ICMP4_echo_request || !is_addr_only))) {
        ...
        s0 = create_session_for_static_mapping(sm, b0, sm0, key0, node, thread_index, vlib_time_now(sm->vlib_main));
        ...
        if (PREDICT_FALSE(icmp0->type != ICMP4_echo_reply && icmp0->type != ICMP4_echo_request &&
 ...
 */
u32
icmp_match_out2in_fast(snat_main_t *sm, vlib_node_runtime_t *node, u32 thread_index, vlib_buffer_t *b0, ip4_header_t *ip0,
                       u8 *p_proto, snat_session_key_t *p_value, u8 *p_dont_translate, void *d, void *e)
{
    icmp46_header_t *icmp0;
    u32 sw_if_index0;
    ...
    if (snat_static_mapping_match(sm, key0, &sm0, 1, &is_addr_only, 0, 0, 0, 0)) {
        /* Don't NAT packet aimed at the intfc address */
...
u32
icmp_out2in(snat_main_t *sm, vlib_buffer_t *b0, ip4_header_t *ip0, icmp46_header_t *icmp0, u32 sw_if_index0,
            u32 rx_fib_index0, vlib_node_runtime_t *node, u32 next0, u32 thread_index, void *d, void *e)
{
    ...
    next0_tmp = sm->icmp_match_out2in_cb(sm, node, thread_index, b0, ip0, &protocol, &sm0, &dont_translate, d, e);
    if (next0_tmp != ~0)
        next0 = next0_tmp;
...
static inline u32
icmp_out2in_slow_path(snat_main_t *sm, vlib_buffer_t *b0, ip4_header_t *ip0, icmp46_header_t *icmp0, u32 sw_if_index0,
...
static int
nat_out2in_sm_unknown_proto(snat_main_t *sm, vlib_buffer_t *b, ip4_header_t *ip, u32 rx_fib_index)
{
...
static uword
snat_out2in_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE(proto0 == SNAT_PROTOCOL_ICMP)) {
                next0 = icmp_out2in_slow_path(sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, now,
                                              thread_index, &s0);
                goto trace0;
            }
            ...
            if (clib_bihash_search_8_8(&sm->per_thread_data[thread_index].out2in, &kv0, &value0)) {
                /* Try to match static mapping by external address and port,
                   destination address and port in packet */
                if (snat_static_mapping_match(sm, key0, &sm0, 1, 0, 0, 0, 0, &identity_nat0)) {
                    /*
                ...
                /* Create session initiated by host from external network */
                s0 = create_session_for_static_mapping(sm, b0, sm0, key0, node, thread_index, now);
                if (!s0) {
                    next0 = SNAT_OUT2IN_NEXT_DROP;
                    goto trace0;
                }
        ...
        trace0:
            ...
            if (PREDICT_FALSE(proto1 == SNAT_PROTOCOL_ICMP)) {
                next1 = icmp_out2in_slow_path(sm, b1, ip1, icmp1, sw_if_index1, rx_fib_index1, node, next1, now,
                                              thread_index, &s1);
                goto trace1;
            }
            ...
            if (clib_bihash_search_8_8(&sm->per_thread_data[thread_index].out2in, &kv1, &value1)) {
                /* Try to match static mapping by external address and port,
                   destination address and port in packet */
                if (snat_static_mapping_match(sm, key1, &sm1, 1, 0, 0, 0, 0, &identity_nat1)) {
                    /*
                     * Send DHCP packets to the ipv4 stack, or we won't
                     * be able to use dhcp client on the outside interface
                     */
                ...
                /* Create session initiated by host from external network */
                s1 = create_session_for_static_mapping(sm, b1, sm1, key1, node, thread_index, now);
                if (!s1) {
                    next1 = SNAT_OUT2IN_NEXT_DROP;
                    goto trace1;
                }
    ...
    vlib_node_increment_counter(vm, snat_out2in_node.index, SNAT_OUT2IN_ERROR_OUT2IN_PACKETS, pkts_processed);
...
static uword
nat44_out2in_reass_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, *to_next;
    snat_out2in_next_t next_index;
    u32 pkts_processed                           = 0;
    ...
    vlib_node_increment_counter(vm, nat44_out2in_reass_node.index, SNAT_OUT2IN_ERROR_OUT2IN_PACKETS, pkts_processed);

    nat_send_all_to_node(vm, fragments_to_drop, node, &node->errors[SNAT_OUT2IN_ERROR_DROP_FRAGMENT],
                         SNAT_OUT2IN_NEXT_DROP);
...
static uword
snat_out2in_fast_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, *to_next;
    snat_out2in_next_t next_index;
    u32 pkts_processed = 0;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(proto0 == SNAT_PROTOCOL_ICMP)) {
                next0 = icmp_out2in(sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, ~0, 0, 0);
                goto trace00;
            }
    ...
    vlib_node_increment_counter(vm, snat_out2in_fast_node.index, SNAT_OUT2IN_ERROR_OUT2IN_PACKETS, pkts_processed);
    return frame->n_vectors;
}
