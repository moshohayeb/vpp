 ...
 */
int
snat_det_add_map(snat_main_t *sm, ip4_address_t *in_addr, u8 in_plen, ip4_address_t *out_addr, u8 out_plen, int is_add)
{
    ...
    vec_foreach(det_map, sm->det_maps)
    {
        ...
        if (det_map->in_addr.as_u32 == in_cmp.as_u32 && det_map->in_plen == in_plen &&
            det_map->out_addr.as_u32 == out_cmp.as_u32 && det_map->out_plen == out_plen) {
            found = 1;
            break;
        }
    }
    ...
    if (is_add) {
        ...
        vec_validate_init_empty(det_map->sessions, SNAT_DET_SES_PER_USER * (1 << (32 - in_plen)) - 1,
 ...
 */
static uword
snat_det_expire_walk_fn(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
