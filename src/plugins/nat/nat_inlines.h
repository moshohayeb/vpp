...
always_inline u8
is_interface_addr(snat_main_t *sm, vlib_node_runtime_t *node, u32 sw_if_index0, u32 ip4_addr)
{
    snat_runtime_t *rt = (snat_runtime_t *) node->runtime_data;
    ip4_address_t *first_int_addr;

    if (PREDICT_FALSE(rt->cached_sw_if_index != sw_if_index0)) {
        first_int_addr         = ip4_interface_first_address(sm->ip4_main, sw_if_index0, 0 /* just want the address */);
...
always_inline void
nat_send_all_to_node(vlib_main_t *vm, u32 *bi_vector, vlib_node_runtime_t *node, vlib_error_t *error, u32 next)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (error)
                p0->error = *error;
            vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi0, next);
        }
...
always_inline void
nat44_delete_user_with_no_session(snat_main_t *sm, snat_user_t *u, u32 thread_index)
{
...
*/
always_inline int
nat44_set_tcp_session_state_i2o(snat_main_t *sm, snat_session_t *ses, tcp_header_t *tcp, u32 thread_index)
{
    if ((tcp->flags & TCP_FLAG_ACK) && (ses->state & NAT44_SES_I2O_SYN) && (ses->state & NAT44_SES_O2I_SYN))
        ses->state = 0;
...
always_inline int
nat44_set_tcp_session_state_o2i(snat_main_t *sm, snat_session_t *ses, tcp_header_t *tcp, u32 thread_index)
{
    if ((tcp->flags & TCP_FLAG_ACK) && (ses->state & NAT44_SES_I2O_SYN) && (ses->state & NAT44_SES_O2I_SYN))
        ses->state = 0;
    if (tcp->flags & TCP_FLAG_SYN)
...
{
    clib_dlist_remove(sm->per_thread_data[thread_index].list_pool, s->per_user_index);
    clib_dlist_addtail(sm->per_thread_data[thread_index].list_pool, s->per_user_list_head_index, s->per_user_index);
}

always_inline void
make_ed_kv(clib_bihash_kv_16_8_t *kv, ip4_address_t *l_addr, ip4_address_t *r_addr, u8 proto, u32 fib_index, u16 l_port,
           u16 r_port)
{
...
always_inline void
make_sm_kv(clib_bihash_kv_8_8_t *kv, ip4_address_t *addr, u8 proto, u32 fib_index, u16 port)
{
    ...
    for (; opts_len > 0; opts_len -= opt_len, data += opt_len) {
        ...
        if (kind == TCP_OPTION_MSS) {
            ...
            if (clib_net_to_host_u16(mss) > sm->mss_clamping) {
                *sum = ip_csum_update(*sum, mss, sm->mss_value_net, ip4_header_t, length);
 ...
 */
static inline int
snat_not_translate_fast(snat_main_t *sm, vlib_node_runtime_t *node, u32 sw_if_index0, ip4_header_t *ip0, u32 proto0,
                        u32 rx_fib_index0)
{
    if (sm->out2in_dpo)
        return 0;
