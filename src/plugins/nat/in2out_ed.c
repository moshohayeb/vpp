...
nat44_i2o_ed_is_idle_session_cb(clib_bihash_kv_16_8_t *kv, void *arg)
{
    ...
    if (ctx->now >= sess_timeout_time) {
        ...
        if (is_twice_nat_session(s)) {
            for (i = 0; i < vec_len(sm->twice_nat_addresses); i++) {
                ...
                if (a->addr.as_u32 == s->ext_host_nat_addr.as_u32) {
                    snat_free_outside_address_and_port(sm->twice_nat_addresses, ctx->thread_index, &key);
                    break;
                }
            }
        }
        ...
        snat_free_outside_address_and_port(sm->addresses, ctx->thread_index, &s->out2in);
        delete : nat44_delete_session(sm, s, ctx->thread_index);
        return 1;
    }
...
static inline u32
icmp_in2out_ed_slow_path(snat_main_t *sm, vlib_buffer_t *b0, ip4_header_t *ip0, icmp46_header_t *icmp0,
                         u32 sw_if_index0, u32 rx_fib_index0, vlib_node_runtime_t *node, u32 next0, f64 now,
                         u32 thread_index, snat_session_t **p_s0)
{
    next0 = icmp_in2out(sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, thread_index, p_s0, 0);
    snat_session_t *s0 = *p_s0;
    if (PREDICT_TRUE(next0 != NAT_IN2OUT_ED_NEXT_DROP && s0)) {
        /* Accounting */
        nat44_session_update_counters(s0, now, vlib_buffer_length_in_chain(sm->vlib_main, b0));
...
static u32
slow_path_ed(snat_main_t *sm, vlib_buffer_t *b, u32 rx_fib_index, clib_bihash_kv_16_8_t *kv, snat_session_t **sessionp,
             vlib_node_runtime_t *node, u32 next, u32 thread_index, f64 now)
{
    ...
    if (snat_static_mapping_match(sm, key0, &key1, 0, 0, 0, &lb, 0, &identity_nat)) {
        /* Try to create dynamic translation */
        if (snat_alloc_outside_address_and_port(sm->addresses, rx_fib_index, thread_index, &key1, sm->port_per_thread,
    ...
    if (!u) {
        ...
        if (!is_sm)
            snat_free_outside_address_and_port(sm->addresses, thread_index, &key1);
        return NAT_IN2OUT_ED_NEXT_DROP;
    }
    ...
    if (clib_bihash_add_or_overwrite_stale_16_8(&tsm->in2out_ed, kv, nat44_i2o_ed_is_idle_session_cb, &ctx))
        nat_log_notice("in2out-ed key add failed");

    make_ed_kv(kv, &key1.addr, &key->r_addr, key->proto, s->out2in.fib_index, key1.port, key->r_port);
    kv->value = s - tsm->sessions;
    if (clib_bihash_add_or_overwrite_stale_16_8(&tsm->out2in_ed, kv, nat44_o2i_ed_is_idle_session_cb, &ctx))
        nat_log_notice("out2in-ed key add failed");

    *sessionp = s;

    /* log NAT event */
    snat_ipfix_logging_nat44_ses_create(s->in2out.addr.as_u32, s->out2in.addr.as_u32, s->in2out.protocol,
                                        s->in2out.port, s->out2in.port, s->in2out.fib_index);
    return next;
}

static_always_inline int
nat44_ed_not_translate(snat_main_t *sm, vlib_node_runtime_t *node, u32 sw_if_index, ip4_header_t *ip, u32 proto,
                       u32 rx_fib_index, u32 thread_index)
{
    ...
    make_ed_kv(&kv, &ip->dst_address, &ip->src_address, ip->protocol, sm->outside_fib_index, udp->dst_port,
               udp->src_port);
...
static_always_inline int
nat_not_translate_output_feature_fwd(snat_main_t *sm, ip4_header_t *ip, u32 thread_index, f64 now, vlib_main_t *vm,
                                     vlib_buffer_t *b)
{
    ...
    if (ip->protocol == IP_PROTOCOL_ICMP) {
        ...
        make_ed_kv(&kv, &ip->src_address, &ip->dst_address, ip->protocol, 0, udp->src_port, udp->dst_port);
    } else {
        make_ed_kv(&kv, &ip->src_address, &ip->dst_address, ip->protocol, 0, 0, 0);
    }
...
static_always_inline int
nat44_ed_not_translate_output_feature(snat_main_t *sm, ip4_header_t *ip, u8 proto, u16 src_port, u16 dst_port,
                                      u32 thread_index, u32 rx_sw_if_index, u32 tx_sw_if_index)
{
    clib_bihash_kv_16_8_t kv, value;
    snat_main_per_thread_data_t *tsm = &sm->per_thread_data[thread_index];
    snat_interface_t *i;
    ...
    /* src NAT check */
    make_ed_kv(&kv, &ip->src_address, &ip->dst_address, proto, tx_fib_index, src_port, dst_port);
    if (!clib_bihash_search_16_8(&tsm->out2in_ed, &kv, &value))
        return 1;

    /* dst NAT check */
    make_ed_kv(&kv, &ip->dst_address, &ip->src_address, proto, rx_fib_index, dst_port, src_port);
    if (!clib_bihash_search_16_8(&tsm->in2out_ed, &kv, &value)) {
        s = pool_elt_at_index(tsm->sessions, value.value);
        if (is_fwd_bypass_session(s))
            return 0;
...
u32
icmp_match_in2out_ed(snat_main_t *sm, vlib_node_runtime_t *node, u32 thread_index, vlib_buffer_t *b, ip4_header_t *ip,
                     u8 *p_proto, snat_session_key_t *p_value, u8 *p_dont_translate, void *d, void *e)
{
    ...
    if (clib_bihash_search_16_8(&tsm->in2out_ed, &kv, &value)) {
        if (vnet_buffer(b)->sw_if_index[VLIB_TX] != ~0) {
            if (PREDICT_FALSE(nat44_ed_not_translate_output_feature(sm, ip, key.proto, key.l_port, key.r_port,
                                                                    thread_index, sw_if_index,
                                                                    vnet_buffer(b)->sw_if_index[VLIB_TX]))) {
                dont_translate = 1;
                goto out;
            }
        } else {
            if (PREDICT_FALSE(nat44_ed_not_translate(sm, node, sw_if_index, ip, SNAT_PROTOCOL_ICMP, rx_fib_index,
                                                     thread_index))) {
                dont_translate = 1;
                goto out;
            }
        ...
        next = slow_path_ed(sm, b, rx_fib_index, &kv, &s, node, next, thread_index, vlib_time_now(sm->vlib_main));
        ...
        if (PREDICT_FALSE(icmp->type != ICMP4_echo_request && icmp->type != ICMP4_echo_reply &&
...
nat44_ed_in2out_unknown_proto(snat_main_t *sm, vlib_buffer_t *b, ip4_header_t *ip, u32 rx_fib_index, u32 thread_index,
                              f64 now, vlib_main_t *vm, vlib_node_runtime_t *node)
{
    ...
    make_ed_kv(&s_kv, &ip->src_address, &ip->dst_address, ip->protocol, rx_fib_index, 0, 0);
    ...
    if (!clib_bihash_search_16_8(&tsm->in2out_ed, &s_kv, &s_value)) {
        ...
        u = nat_user_get_or_create(sm, &ip->src_address, rx_fib_index, thread_index);
        if (!u) {
            nat_log_warn("create NAT user failed");
            return 0;
        }
        ...
        /* Fallback to 3-tuple key */
            ...
            while (ses_index != ~0) {
                ...
                if (s->ext_host_addr.as_u32 == ip->dst_address.as_u32) {
                    ...
                    make_ed_kv(&s_kv, &s->out2in.addr, &ip->dst_address, ip->protocol, outside_fib_index, 0, 0);
            ...
            for (i = 0; i < vec_len(sm->addresses); i++) {
                make_ed_kv(&s_kv, &sm->addresses[i].addr, &ip->dst_address, ip->protocol, outside_fib_index, 0, 0);
                if (clib_bihash_search_16_8(&tsm->out2in_ed, &s_kv, &s_value)) {
                    new_addr = ip->src_address.as_u32 = sm->addresses[i].addr.as_u32;
                    goto create_ses;
                }
            }
            return 0;
    ...
    create_ses:
        ...
        /* Add to lookup tables */
        make_ed_kv(&s_kv, &s->in2out.addr, &ip->dst_address, ip->protocol, rx_fib_index, 0, 0);
...
static inline uword
nat44_ed_in2out_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_slow_path,
                               int is_output_feature)
{
    ...
    stats_node_index = is_slow_path ? nat44_ed_in2out_slowpath_node.index : nat44_ed_in2out_node.index;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            u32 bi0, bi1;
            vlib_buffer_t *b0, *b1;
            u32 next0, sw_if_index0, rx_fib_index0, iph_offset0 = 0, proto0, new_addr0, old_addr0;
            u32 next1, sw_if_index1, rx_fib_index1, iph_offset1 = 0, proto1, new_addr1, old_addr1;
            ...
            if (is_slow_path) {
                if (PREDICT_FALSE(proto0 == ~0)) {
                    s0 = nat44_ed_in2out_unknown_proto(sm, b0, ip0, rx_fib_index0, thread_index, now, vm, node);
                    if (!s0)
                ...
                if (PREDICT_FALSE(proto0 == SNAT_PROTOCOL_ICMP)) {
                    next0 = icmp_in2out_ed_slow_path(sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, now,
                                                     thread_index, &s0);
                    goto trace00;
                }
            } else {
                ...
                if (is_output_feature) {
                    if (PREDICT_FALSE(nat_not_translate_output_feature_fwd(sm, ip0, thread_index, now, vm, b0)))
                        goto trace00;
                }
            ...
            make_ed_kv(&kv0, &ip0->src_address, &ip0->dst_address, ip0->protocol, rx_fib_index0, udp0->src_port,
                       udp0->dst_port);

            if (clib_bihash_search_16_8(&tsm->in2out_ed, &kv0, &value0)) {
                if (is_slow_path) {
                    if (is_output_feature) {
                        if (PREDICT_FALSE(nat44_ed_not_translate_output_feature(sm, ip0, ip0->protocol, udp0->src_port,
                                                                                udp0->dst_port, thread_index, sw_if_index0,
                                                                                vnet_buffer(b0)->sw_if_index[VLIB_TX])))
                            goto trace00;
                    } else {
                        if (PREDICT_FALSE(nat44_ed_not_translate(sm, node, sw_if_index0, ip0, proto0, rx_fib_index0,
                                                                 thread_index)))
                            goto trace00;
                    }

                    next0 = slow_path_ed(sm, b0, rx_fib_index0, &kv0, &s0, node, next0, thread_index, now);
            ...
            if (PREDICT_FALSE(is_twice_nat_session(s0)))
                sum0 = ip_csum_update(sum0, ip0->dst_address.as_u32, s0->ext_host_addr.as_u32, ip4_header_t, dst_address);
            ...
            if (PREDICT_TRUE(proto0 == SNAT_PROTOCOL_TCP)) {
                ...
                if (PREDICT_FALSE(is_twice_nat_session(s0))) {
                    sum0 = ip_csum_update(sum0, ip0->dst_address.as_u32, s0->ext_host_addr.as_u32, ip4_header_t,
                                          dst_address);
                    sum0 = ip_csum_update(sum0, tcp0->dst_port, s0->ext_host_port, ip4_header_t, length);
            ...
            /* Accounting */
            nat44_session_update_counters(s0, now, vlib_buffer_length_in_chain(vm, b0));
            /* Per-user LRU list maintenance */
            nat44_session_update_lru(sm, s0, thread_index);
        ...
        trace00:
            ...
            rx_fib_index1 = fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP4, sw_if_index1);
            ...
            if (is_slow_path) {
                if (PREDICT_FALSE(proto1 == ~0)) {
                    s1 = nat44_ed_in2out_unknown_proto(sm, b1, ip1, rx_fib_index1, thread_index, now, vm, node);
                    if (!s1)
                ...
                if (PREDICT_FALSE(proto1 == SNAT_PROTOCOL_ICMP)) {
                    next1 = icmp_in2out_ed_slow_path(sm, b1, ip1, icmp1, sw_if_index1, rx_fib_index1, node, next1, now,
                                                     thread_index, &s1);
                    goto trace01;
                }
            } else {
                ...
                if (is_output_feature) {
                    if (PREDICT_FALSE(nat_not_translate_output_feature_fwd(sm, ip1, thread_index, now, vm, b1)))
                        goto trace01;
                }
            ...
            make_ed_kv(&kv1, &ip1->src_address, &ip1->dst_address, ip1->protocol, rx_fib_index1, udp1->src_port,
                       udp1->dst_port);

            if (clib_bihash_search_16_8(&tsm->in2out_ed, &kv1, &value1)) {
                if (is_slow_path) {
                    if (is_output_feature) {
                        if (PREDICT_FALSE(nat44_ed_not_translate_output_feature(sm, ip1, ip1->protocol, udp1->src_port,
                                                                                udp1->dst_port, thread_index, sw_if_index1,
                                                                                vnet_buffer(b1)->sw_if_index[VLIB_TX])))
                            goto trace01;
                    } else {
                        if (PREDICT_FALSE(nat44_ed_not_translate(sm, node, sw_if_index1, ip1, proto1, rx_fib_index1,
                                                                 thread_index)))
                            goto trace01;
                    }

                    next1 = slow_path_ed(sm, b1, rx_fib_index1, &kv1, &s1, node, next1, thread_index, now);
            ...
            sum1                                = ip1->checksum;
            sum1                                = ip_csum_update(sum1, old_addr1, new_addr1, ip4_header_t, src_address);
            if (PREDICT_FALSE(is_twice_nat_session(s1)))
                sum1 = ip_csum_update(sum1, ip1->dst_address.as_u32, s1->ext_host_addr.as_u32, ip4_header_t, dst_address);
            ...
            if (PREDICT_TRUE(proto1 == SNAT_PROTOCOL_TCP)) {
                ...
                sum1 = tcp1->checksum;
                sum1 = ip_csum_update(sum1, old_addr1, new_addr1, ip4_header_t, dst_address);
                sum1 = ip_csum_update(sum1, old_port1, new_port1, ip4_header_t, length);
                if (PREDICT_FALSE(is_twice_nat_session(s1))) {
                    sum1 = ip_csum_update(sum1, ip1->dst_address.as_u32, s1->ext_host_addr.as_u32, ip4_header_t,
                                          dst_address);
                    sum1 = ip_csum_update(sum1, tcp1->dst_port, s1->ext_host_port, ip4_header_t, length);
            ...
            /* Accounting */
            nat44_session_update_counters(s1, now, vlib_buffer_length_in_chain(vm, b1));
            /* Per-user LRU list maintenance */
        ...
        trace01:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
                nat_in2out_ed_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
                t->is_slow_path          = is_slow_path;
    ...
    vlib_node_increment_counter(vm, stats_node_index, NAT_IN2OUT_ED_ERROR_IN2OUT_PACKETS, pkts_processed);
    return frame->n_vectors;
}

static uword
nat44_ed_in2out_fast_path_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nat44_ed_in2out_output_fast_path_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nat44_ed_in2out_slow_path_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nat44_ed_in2out_output_slow_path_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static inline uword
nat44_ed_in2out_reass_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame,
                                     int is_output_feature)
{
    ...
    f64 now                                      = vlib_time_now(vm);
    u32 thread_index                             = vm->thread_index;
    snat_main_per_thread_data_t *per_thread_data = &sm->per_thread_data[thread_index];
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            reass0 = nat_ip4_reass_find_or_create(ip0->src_address, ip0->dst_address, ip0->fragment_id, ip0->protocol,
                                                  1, &fragments_to_drop);
            ...
            if (PREDICT_FALSE(ip4_is_first_fragment(ip0))) {
                ...
                if (clib_bihash_search_16_8(&per_thread_data->in2out_ed, &kv0, &value0)) {
                    ...
                    s0                 = pool_elt_at_index(per_thread_data->sessions, value0.value);
                ...
                s0 = pool_elt_at_index(per_thread_data->sessions, reass0->sess_index);
            }
            ...
            if (PREDICT_FALSE(ip4_is_first_fragment(ip0))) {
                if (PREDICT_TRUE(proto0 == SNAT_PROTOCOL_TCP)) {
                    ...
                    sum0 = ip_csum_update(sum0, old_port0, new_port0, ip4_header_t /* cheat */,
            ...
            /* Hairpinning */
            nat44_reass_hairpinning(sm, b0, ip0, s0->out2in.port, s0->ext_host_port, proto0, 1);
        ...
        trace0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                nat44_reass_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->cached              = cached0;
                t->sw_if_index         = sw_if_index0;
                t->next_index          = next0;
            }
    ...
    vlib_node_increment_counter(vm, nat44_ed_in2out_reass_node.index, NAT_IN2OUT_ED_ERROR_IN2OUT_PACKETS, pkts_processed);

    nat_send_all_to_node(vm, fragments_to_drop, node, &node->errors[NAT_IN2OUT_ED_ERROR_DROP_FRAGMENT],
                         NAT_IN2OUT_ED_NEXT_DROP);
...
static uword
nat44_ed_in2out_reass_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nat44_ed_in2out_reass_output_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
