...
vl_api_nat_control_ping_t_handler(vl_api_nat_control_ping_t *mp)
{
    ...
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_NAT_CONTROL_PING_REPLY, ({ rmp->vpe_pid = ntohl(getpid()); }));
    /* *INDENT-ON* */
}
...
vl_api_nat_show_config_t_handler(vl_api_nat_show_config_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_NAT_SHOW_CONFIG_REPLY, ({
                     ...
                     rmp->static_mapping_connection_tracking = sm->static_mapping_connection_tracking;
...
vl_api_nat_worker_dump_t_handler(vl_api_nat_worker_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    vec_foreach(worker_index, sm->workers) send_nat_worker_details(*worker_index, reg, mp->context);
    /* *INDENT-ON* */
}
...
static void
vl_api_nat_ipfix_enable_disable_t_handler(vl_api_nat_ipfix_enable_disable_t *mp)
{
    snat_main_t *sm = &snat_main;
    vl_api_nat_ipfix_enable_disable_reply_t *rmp;
    int rv = 0;

    rv = snat_ipfix_logging_enable_disable(mp->enable, clib_host_to_net_u32(mp->domain_id),
...
static void *
vl_api_nat_ipfix_enable_disable_t_print(vl_api_nat_ipfix_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
vl_api_nat_set_reass_t_handler(vl_api_nat_set_reass_t *mp)
{
    ...
    rv = nat_reass_set(ntohl(mp->timeout), ntohs(mp->max_reass), mp->max_frag, mp->drop_frag, mp->is_ip6);
...
vl_api_nat_set_reass_t_print(vl_api_nat_set_reass_t *mp, void *handle)
{
    ...
    s = format(s, "timeout %d max_reass %d max_frag %d drop_frag %d is_ip6 %d", clib_host_to_net_u32(mp->timeout),
               clib_host_to_net_u16(mp->max_reass), mp->max_frag, mp->drop_frag, mp->is_ip6);
...
vl_api_nat_set_timeouts_t_handler(vl_api_nat_set_timeouts_t *mp)
{
    ...
    if (rv)
        goto send_reply;
    rv = nat64_set_tcp_timeouts(ntohl(mp->tcp_transitory), ntohl(mp->tcp_established));
...
static void
vl_api_nat_set_addr_and_port_alloc_alg_t_handler(vl_api_nat_set_addr_and_port_alloc_alg_t *mp)
{
    snat_main_t *sm = &snat_main;
    ...
    case NAT_ADDR_AND_PORT_ALLOC_ALG_MAPE:
        nat_set_alloc_addr_and_port_mape(ntohs(mp->psid), mp->psid_offset, mp->psid_length);
        break;
...
static void *
vl_api_nat_set_addr_and_port_alloc_alg_t_print(vl_api_nat_set_addr_and_port_alloc_alg_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: nat_set_addr_and_port_alloc_alg ");
    s = format(s,
               "alg %d psid_offset %d psid_length %d psid %d start_port %d "
               "end_port %d\n",
               ntohl(mp->alg), ntohl(mp->psid_offset), ntohl(mp->psid_length), ntohs(mp->psid), ntohs(mp->start_port),
               ntohs(mp->end_port));
...
static void
vl_api_nat_get_addr_and_port_alloc_alg_t_handler(vl_api_nat_get_addr_and_port_alloc_alg_t *mp)
{
    snat_main_t *sm = &snat_main;
...
static void *
vl_api_nat_get_addr_and_port_alloc_alg_t_print(vl_api_nat_get_addr_and_port_alloc_alg_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_nat_set_mss_clamping_t_print(vl_api_nat_set_mss_clamping_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_nat_get_mss_clamping_t_print(vl_api_nat_get_mss_clamping_t *mp, void *handle)
{
    u8 *s;
...
/*************/
static void
vl_api_nat44_add_del_address_range_t_handler(vl_api_nat44_add_del_address_range_t *mp)
{
    snat_main_t *sm = &snat_main;
    ...
    if (count > 1024)
        nat_log_info("%U - %U, %d addresses...", format_ip4_address, mp->first_ip_address, format_ip4_address,
...
static void *
vl_api_nat44_add_del_address_range_t_print(vl_api_nat44_add_del_address_range_t *mp, void *handle)
{
    u8 *s;
...
static void
send_nat44_address_details(snat_address_t *a, vl_api_registration_t *reg, u32 context, u8 twice_nat)
{
...
vl_api_nat44_address_dump_t_handler(vl_api_nat44_address_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    vec_foreach(a, sm->addresses) send_nat44_address_details(a, reg, mp->context, 0);
    vec_foreach(a, sm->twice_nat_addresses) send_nat44_address_details(a, reg, mp->context, 1);
    /* *INDENT-ON* */
}

static void *
vl_api_nat44_address_dump_t_print(vl_api_nat44_address_dump_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_nat44_interface_add_del_feature_t_handler(vl_api_nat44_interface_add_del_feature_t *mp)
{
    snat_main_t *sm = &snat_main;
...
static void *
vl_api_nat44_interface_add_del_feature_t_print(vl_api_nat44_interface_add_del_feature_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: nat44_interface_add_del_feature ");
    s = format(s, "sw_if_index %d %s %s", clib_host_to_net_u32(mp->sw_if_index), mp->is_inside ? "in" : "out",
               mp->is_add ? "" : "del");
...
static void
send_nat44_interface_details(snat_interface_t *i, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp->is_inside   = (nat_interface_is_inside(i) && nat_interface_is_outside(i)) ? 2 : nat_interface_is_inside(i);
    rmp->context     = context;
...
vl_api_nat44_interface_dump_t_handler(vl_api_nat44_interface_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(i, sm->interfaces, ({ send_nat44_interface_details(i, reg, mp->context); }));
    /* *INDENT-ON* */
}

static void *
vl_api_nat44_interface_dump_t_print(vl_api_nat44_interface_dump_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_nat44_interface_add_del_output_feature_t_handler(vl_api_nat44_interface_add_del_output_feature_t *mp)
{
    snat_main_t *sm = &snat_main;
    ...
    rv = snat_interface_add_del_output_feature(sw_if_index, mp->is_inside, is_del);
...
static void *
vl_api_nat44_interface_add_del_output_feature_t_print(vl_api_nat44_interface_add_del_output_feature_t *mp, void *handle)
{
    u8 *s;
...
static void
send_nat44_interface_output_feature_details(snat_interface_t *i, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id  = ntohs(VL_API_NAT44_INTERFACE_OUTPUT_FEATURE_DETAILS + sm->msg_id_base);
    rmp->sw_if_index = ntohl(i->sw_if_index);
    rmp->context     = context;
...
static void
vl_api_nat44_interface_output_feature_dump_t_handler(vl_api_nat44_interface_output_feature_dump_t *mp)
{
    vl_api_registration_t *reg;
    snat_main_t *sm = &snat_main;
    snat_interface_t *i;
...
static void *
vl_api_nat44_interface_output_feature_dump_t_print(vl_api_nat44_interface_output_feature_dump_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_nat44_add_del_static_mapping_t_handler(vl_api_nat44_add_del_static_mapping_t *mp)
{
    snat_main_t *sm = &snat_main;
    ...
    rv = snat_add_static_mapping(local_addr, external_addr, local_port, external_port, vrf_id, mp->addr_only,
                                 external_sw_if_index, proto, mp->is_add, twice_nat, mp->out2in_only, tag, 0);
...
static void *
vl_api_nat44_add_del_static_mapping_t_print(vl_api_nat44_add_del_static_mapping_t *mp, void *handle)
{
    u8 *s;
    ...
    if (mp->addr_only == 0)
        s = format(s, "local_port %d external_port %d ", clib_net_to_host_u16(mp->local_port),
                   clib_net_to_host_u16(mp->external_port));

    s = format(s, "twice_nat %d out2in_only %d ", mp->twice_nat, mp->out2in_only);

    if (mp->vrf_id != ~0)
        s = format(s, "vrf %d", clib_net_to_host_u32(mp->vrf_id));

    if (mp->external_sw_if_index != ~0)
        s = format(s, "external_sw_if_index %d", clib_net_to_host_u32(mp->external_sw_if_index));
    FINISH;
}

static void
send_nat44_static_mapping_details(snat_static_mapping_t *m, vl_api_registration_t *reg, u32 context)
{
...
static void
send_nat44_static_map_resolve_details(snat_static_map_resolve_t *m, vl_api_registration_t *reg, u32 context)
{
    vl_api_nat44_static_mapping_details_t *rmp;
    snat_main_t *sm = &snat_main;
...
static void
vl_api_nat44_static_mapping_dump_t_handler(vl_api_nat44_static_mapping_dump_t *mp)
{
    vl_api_registration_t *reg;
    snat_main_t *sm = &snat_main;
...
static void *
vl_api_nat44_static_mapping_dump_t_print(vl_api_nat44_static_mapping_dump_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_nat44_add_del_identity_mapping_t_handler(vl_api_nat44_add_del_identity_mapping_t *mp)
{
    snat_main_t *sm = &snat_main;
    ...
    rv = snat_add_static_mapping(addr, addr, port, port, vrf_id, mp->addr_only, sw_if_index, proto, mp->is_add, 0, 0,
                                 tag, 1);
...
static void *
vl_api_nat44_add_del_identity_mapping_t_print(vl_api_nat44_add_del_identity_mapping_t *mp, void *handle)
{
    u8 *s;
    ...
    if (mp->addr_only == 0)
        s = format(s, " protocol %d port %d", mp->protocol, clib_net_to_host_u16(mp->port));
...
static void
send_nat44_identity_mapping_details(snat_static_mapping_t *m, int index, vl_api_registration_t *reg, u32 context)
{
...
static void
send_nat44_identity_map_resolve_details(snat_static_map_resolve_t *m, vl_api_registration_t *reg, u32 context)
{
    vl_api_nat44_identity_mapping_details_t *rmp;
    snat_main_t *sm = &snat_main;
...
static void
vl_api_nat44_identity_mapping_dump_t_handler(vl_api_nat44_identity_mapping_dump_t *mp)
{
    vl_api_registration_t *reg;
    snat_main_t *sm = &snat_main;
    snat_static_mapping_t *m;
...
static void *
vl_api_nat44_identity_mapping_dump_t_print(vl_api_nat44_identity_mapping_dump_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_nat44_add_del_interface_addr_t_handler(vl_api_nat44_add_del_interface_addr_t *mp)
{
    snat_main_t *sm = &snat_main;
...
static void *
vl_api_nat44_add_del_interface_addr_t_print(vl_api_nat44_add_del_interface_addr_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: nat44_add_del_interface_addr ");
    s = format(s, "sw_if_index %d twice_nat %d %s", clib_host_to_net_u32(mp->sw_if_index), mp->twice_nat,
               mp->is_add ? "" : "del");
...
static void
send_nat44_interface_addr_details(u32 sw_if_index, vl_api_registration_t *reg, u32 context, u8 twice_nat)
{
...
static void
vl_api_nat44_interface_addr_dump_t_handler(vl_api_nat44_interface_addr_dump_t *mp)
{
    vl_api_registration_t *reg;
    snat_main_t *sm = &snat_main;
    u32 *i;
    ...
    /* *INDENT-OFF* */
    vec_foreach(i, sm->auto_add_sw_if_indices) send_nat44_interface_addr_details(*i, reg, mp->context, 0);
    vec_foreach(i, sm->auto_add_sw_if_indices_twice_nat) send_nat44_interface_addr_details(*i, reg, mp->context, 1);
    /* *INDENT-ON* */
}

static void *
vl_api_nat44_interface_addr_dump_t_print(vl_api_nat44_interface_addr_dump_t *mp, void *handle)
{
    u8 *s;
...
vl_api_nat44_user_dump_t_handler(vl_api_nat44_user_dump_t *mp)
{
    ...
    {
        pool_foreach(u, tsm->users, ({ send_nat44_user_details(u, reg, mp->context); }));
    }
    /* *INDENT-ON* */
}
...
static void
send_nat44_user_session_details(snat_session_t *s, vl_api_registration_t *reg, u32 context)
{
...
vl_api_nat44_user_session_dump_t_handler(vl_api_nat44_user_session_dump_t *mp)
{
    ...
    key.key               = ukey.as_u64;
    if (sm->num_workers > 1)
        tsm = vec_elt_at_index(sm->per_thread_data, sm->worker_in2out_cb(&ip, ukey.fib_index));
    else
...
static void *
vl_api_nat44_user_session_dump_t_print(vl_api_nat44_user_session_dump_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: nat44_user_session_dump ");
    s = format(s, "ip_address %U vrf_id %d\n", format_ip4_address, mp->ip_address, clib_net_to_host_u32(mp->vrf_id));

    FINISH;
}

static nat44_lb_addr_port_t *
unformat_nat44_lb_addr_port(vl_api_nat44_lb_addr_port_t *addr_port_pairs, u8 addr_port_pair_num)
{
    u8 i;
...
static void
vl_api_nat44_add_del_lb_static_mapping_t_handler(vl_api_nat44_add_del_lb_static_mapping_t *mp)
{
    snat_main_t *sm = &snat_main;
    ...
    rv = nat44_add_del_lb_static_mapping(e_addr, clib_net_to_host_u16(mp->external_port), proto, locals, mp->is_add,
                                         twice_nat, mp->out2in_only, tag, clib_net_to_host_u32(mp->affinity));
...
static void *
vl_api_nat44_add_del_lb_static_mapping_t_print(vl_api_nat44_add_del_lb_static_mapping_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: nat44_add_del_lb_static_mapping ");
    s = format(s, "is_add %d twice_nat %d out2in_only %d ", mp->is_add, mp->twice_nat, mp->out2in_only);

    FINISH;
}

static void
send_nat44_lb_static_mapping_details(snat_static_mapping_t *m, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp) + (vec_len(m->locals) * sizeof(nat44_lb_addr_port_t)));
    memset(rmp, 0, sizeof(*rmp));
...
static void
vl_api_nat44_lb_static_mapping_dump_t_handler(vl_api_nat44_lb_static_mapping_dump_t *mp)
{
    vl_api_registration_t *reg;
    snat_main_t *sm = &snat_main;
    snat_static_mapping_t *m;
...
static void *
vl_api_nat44_lb_static_mapping_dump_t_print(vl_api_nat44_lb_static_mapping_dump_t *mp, void *handle)
{
    u8 *s;
...
vl_api_nat44_del_session_t_handler(vl_api_nat44_del_session_t *mp)
{
    ...
    if (mp->ext_host_valid)
        rv = nat44_del_ed_session(sm, &addr, port, &eh_addr, eh_port, mp->protocol, vrf_id, mp->is_in);
    else
...
static void *
vl_api_nat44_del_session_t_print(vl_api_nat44_del_session_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: nat44_add_del_static_mapping ");
    s = format(s, "addr %U port %d protocol %d vrf_id %d is_in %d", format_ip4_address, mp->address,
               clib_net_to_host_u16(mp->port), mp->protocol, clib_net_to_host_u32(mp->vrf_id), mp->is_in);
    if (mp->ext_host_valid)
        s = format(s, "ext_host_address %U ext_host_port %d", format_ip4_address, mp->ext_host_address,
...
static void
vl_api_nat44_forwarding_enable_disable_t_handler(vl_api_nat44_forwarding_enable_disable_t *mp)
{
    snat_main_t *sm = &snat_main;
...
static void *
vl_api_nat44_forwarding_enable_disable_t_print(vl_api_nat44_forwarding_enable_disable_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_nat44_forwarding_is_enabled_t_handler(vl_api_nat44_forwarding_is_enabled_t *mp)
{
    vl_api_registration_t *reg;
    snat_main_t *sm = &snat_main;
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(VL_API_NAT44_FORWARDING_IS_ENABLED_REPLY + sm->msg_id_base);
...
static void *
vl_api_nat44_forwarding_is_enabled_t_print(vl_api_nat44_forwarding_is_enabled_t *mp, void *handle)
{
    u8 *s;
...
vl_api_nat_det_add_del_map_t_handler(vl_api_nat_det_add_del_map_t *mp)
{
    ...
    rv = snat_det_add_map(sm, &in_addr, mp->in_plen, &out_addr, mp->out_plen, mp->is_add);
...
static void *
vl_api_nat_det_add_del_map_t_print(vl_api_nat_det_add_del_map_t *mp, void *handle)
{
    u8 *s;
...
vl_api_nat_det_reverse_t_print(vl_api_nat_det_reverse_t *mp, void *handle)
{
    ...
    s = format(s, "outside ip address %U outside port %d", format_ip4_address, mp->out_addr, ntohs(mp->out_port));
...
vl_api_nat_det_map_dump_t_handler(vl_api_nat_det_map_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    vec_foreach(m, sm->det_maps) sent_nat_det_map_details(m, reg, mp->context);
    /* *INDENT-ON* */
}
...
static void
vl_api_nat_det_close_session_out_t_handler(vl_api_nat_det_close_session_out_t *mp)
{
    snat_main_t *sm = &snat_main;
...
static void *
vl_api_nat_det_close_session_out_t_print(vl_api_nat_det_close_session_out_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: nat_det_close_session_out ");
    s = format(s,
               "out_addr %U out_port %d "
               "ext_addr %U ext_port %d\n",
               format_ip4_address, mp->out_addr, ntohs(mp->out_port), format_ip4_address, mp->ext_addr,
               ntohs(mp->ext_port));
...
static void
vl_api_nat_det_close_session_in_t_handler(vl_api_nat_det_close_session_in_t *mp)
{
    snat_main_t *sm = &snat_main;
...
static void *
vl_api_nat_det_close_session_in_t_print(vl_api_nat_det_close_session_in_t *mp, void *handle)
{
    u8 *s;
    s = format(0, "SCRIPT: nat_det_close_session_in ");
    s = format(s, "in_addr %U in_port %d ext_addr %U ext_port %d\n", format_ip4_address, mp->in_addr,
               ntohs(mp->in_port), format_ip4_address, mp->ext_addr, ntohs(mp->ext_port));

    FINISH;
}

static void
send_nat_det_session_details(snat_det_session_t *s, vl_api_registration_t *reg, u32 context)
{
...
static void *
vl_api_nat_det_session_dump_t_print(vl_api_nat_det_session_dump_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_nat64_add_del_pool_addr_range_t_handler(vl_api_nat64_add_del_pool_addr_range_t *mp)
{
...
static void *
vl_api_nat64_add_del_pool_addr_range_t_print(vl_api_nat64_add_del_pool_addr_range_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: nat64_add_del_pool_addr_range ");
    s = format(s, "%U - %U vrf_id %u %s\n", format_ip4_address, mp->start_addr, format_ip4_address, mp->end_addr,
...
static void *
vl_api_nat64_pool_addr_dump_t_print(vl_api_nat64_pool_addr_dump_t *mp, void *handle)
{
    u8 *s;
...
vl_api_nat64_add_del_interface_t_handler(vl_api_nat64_add_del_interface_t *mp)
{
    ...
    rv = nat64_add_del_interface(ntohl(mp->sw_if_index), mp->is_inside, mp->is_add);
...
static void *
vl_api_nat64_add_del_interface_t_print(vl_api_nat64_add_del_interface_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_nat64_interface_dump_t_print(vl_api_nat64_interface_dump_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_nat64_add_del_static_bib_t_handler(vl_api_nat64_add_del_static_bib_t *mp)
{
    snat_main_t *sm = &snat_main;
    ...
    rv = nat64_add_del_static_bib_entry(&in_addr, &out_addr, clib_net_to_host_u16(mp->i_port),
                                        clib_net_to_host_u16(mp->o_port), mp->proto, clib_net_to_host_u32(mp->vrf_id),
                                        mp->is_add);
...
static void *
vl_api_nat64_add_del_static_bib_t_print(vl_api_nat64_add_del_static_bib_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: nat64_add_del_static_bib ");
    s = format(s, "protocol %d i_addr %U o_addr %U ", mp->proto, format_ip6_address, mp->i_addr, format_ip4_address,
               mp->o_addr);
...
vl_api_nat64_bib_dump_t_handler(vl_api_nat64_bib_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    vec_foreach(db, nm->db) nat64_db_bib_walk(db, mp->proto, nat64_api_bib_walk, &ctx);
    /* *INDENT-ON* */
}
...
vl_api_nat64_add_del_prefix_t_handler(vl_api_nat64_add_del_prefix_t *mp)
{
    ...
    rv = nat64_add_del_prefix(&prefix, mp->prefix_len, clib_net_to_host_u32(mp->vrf_id), mp->is_add);
    REPLY_MACRO(VL_API_NAT64_ADD_DEL_PREFIX_REPLY);
}

static void *
vl_api_nat64_add_del_prefix_t_print(vl_api_nat64_add_del_prefix_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_nat64_prefix_dump_t_print(vl_api_nat64_prefix_dump_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_nat64_add_del_interface_addr_t_handler(vl_api_nat64_add_del_interface_addr_t *mp)
{
    snat_main_t *sm = &snat_main;
...
static void *
vl_api_nat64_add_del_interface_addr_t_print(vl_api_nat64_add_del_interface_addr_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: nat64_add_del_interface_addr ");
    s = format(s, "sw_if_index %d %s", clib_host_to_net_u32(mp->sw_if_index), mp->is_add ? "" : "del");
...
static void *
vl_api_dslite_set_aftr_addr_t_print(vl_api_dslite_set_aftr_addr_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: dslite_set_aftr_addr ");
    s = format(s, "ip6_addr %U ip4_addr %U\n", format_ip6_address, mp->ip6_addr, format_ip4_address, mp->ip4_addr);
...
static void *
vl_api_dslite_get_aftr_addr_t_print(vl_api_dslite_get_aftr_addr_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_dslite_set_b4_addr_t_print(vl_api_dslite_set_b4_addr_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: dslite_set_b4_addr ");
    s = format(s, "ip6_addr %U ip4_addr %U\n", format_ip6_address, mp->ip6_addr, format_ip6_address, mp->ip4_addr);
...
static void *
vl_api_dslite_get_b4_addr_t_print(vl_api_dslite_get_b4_addr_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_dslite_add_del_pool_addr_range_t_handler(vl_api_dslite_add_del_pool_addr_range_t *mp)
{
...
static void
send_dslite_address_details(snat_address_t *ap, vl_api_registration_t *reg, u32 context)
{
...
static void *
vl_api_dslite_address_dump_t_print(vl_api_dslite_address_dump_t *mp, void *handle)
{
    u8 *s;
...
static void *
vl_api_dslite_add_del_pool_addr_range_t_print(vl_api_dslite_add_del_pool_addr_range_t *mp, void *handle)
{
    u8 *s;
...
vl_api_nat66_add_del_interface_t_handler(vl_api_nat66_add_del_interface_t *mp)
{
    ...
    rv = nat66_interface_add_del(ntohl(mp->sw_if_index), mp->is_inside, mp->is_add);
...
static void *
vl_api_nat66_add_del_interface_t_print(vl_api_nat66_add_del_interface_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_nat66_add_del_static_mapping_t_handler(vl_api_nat66_add_del_static_mapping_t *mp)
{
    snat_main_t *sm = &snat_main;
...
static void *
vl_api_nat66_add_del_static_mapping_t_print(vl_api_nat66_add_del_static_mapping_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: nat66_add_del_static_mapping ");
    s = format(s, "local_ip_address %U external_ip_address %U vrf_id %d %s", format_ip6_address, mp->local_ip_address,
               format_ip6_address, mp->external_ip_address, clib_net_to_host_u32(mp->vrf_id), mp->is_add ? "" : "del");
...
static void *
vl_api_nat66_interface_dump_t_print(vl_api_nat66_interface_dump_t *mp, void *handle)
{
    u8 *s;
...
nat66_api_static_mapping_walk(nat66_static_mapping_t *m, void *arg)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(VL_API_NAT66_STATIC_MAPPING_DETAILS + sm->msg_id_base);
...
static void
vl_api_nat66_static_mapping_dump_t_handler(vl_api_nat66_static_mapping_dump_t *mp)
{
    svm_queue_t *q;
...
static void *
vl_api_nat66_static_mapping_dump_t_print(vl_api_nat66_static_mapping_dump_t *mp, void *handle)
{
    u8 *s;
...
#define foreach_snat_plugin_api_msg                                                                                    \
    ...
    _(NAT44_INTERFACE_ADD_DEL_OUTPUT_FEATURE, nat44_interface_add_del_output_feature)                                  \
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + sm->msg_id_base);
...
{
#define _(n, f) sm->api_main->msg_print_handlers[VL_API_##n + sm->msg_id_base] = (void *) vl_api_##f##_t_print;
    foreach_snat_plugin_api_msg;
#undef _
}
