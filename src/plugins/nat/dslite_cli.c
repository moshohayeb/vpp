...
static clib_error_t *
dslite_add_del_pool_addr_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
dslite_show_pool_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dslite_main_t *dm = &dslite_main;
    snat_address_t *ap;
...
static clib_error_t *
dslite_set_aftr_tunnel_addr_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dslite_main_t *dm = &dslite_main;
    unformat_input_t _line_input, *line_input = &_line_input;
    ...
    if (rv)
        error = clib_error_return(0, "Set DS-Lite AFTR tunnel endpoint address failed.");
...
static clib_error_t *
dslite_show_aftr_ip6_addr_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dslite_main_t *dm = &dslite_main;
...
static clib_error_t *
dslite_set_b4_tunnel_addr_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dslite_main_t *dm = &dslite_main;
    unformat_input_t _line_input, *line_input = &_line_input;
    ip6_address_t ip6_addr;
    ...
    if (rv)
        error = clib_error_return(0, "Set DS-Lite B4 tunnel endpoint address failed.");
...
static clib_error_t *
dslite_show_b4_ip6_addr_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dslite_main_t *dm = &dslite_main;
...
format_dslite_session(u8 *s, va_list *args)
{
    ...
    s = format(s, "%Uin %U:%u out %U:%u protocol %U\n", format_white_space, indent + 2, format_ip4_address,
               &session->in2out.addr, clib_net_to_host_u16(session->in2out.port), format_ip4_address,
               &session->out2in.addr, clib_net_to_host_u16(session->out2in.port), format_snat_protocol,
...
static clib_error_t *
dslite_show_sessions_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dslite_main_t *dm = &dslite_main;
    ...
    {
        pool_foreach(b4, td->b4s, ({ vlib_cli_output(vm, "%U", format_dslite_b4, td, b4); }));
    }
    /* *INDENT-ON* */
