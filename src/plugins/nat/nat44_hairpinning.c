...
int
snat_hairpinning(snat_main_t *sm, vlib_buffer_t *b0, ip4_header_t *ip0, udp_header_t *udp0, tcp_header_t *tcp0,
                 u32 proto0, int is_ed)
{
    ...
    /* or active session */
        ...
        if (is_ed) {
            ...
            make_ed_kv(&ed_kv, &ip0->dst_address, &ip0->src_address, ip0->protocol, sm->outside_fib_index,
                       udp0->dst_port, udp0->src_port);
            rv = clib_bihash_search_16_8(&sm->per_thread_data[ti].out2in_ed, &ed_kv, &ed_value);
            si = ed_value.value;
        } else {
            rv = clib_bihash_search_8_8(&sm->per_thread_data[ti].out2in, &kv0, &value0);
            si = value0.value;
        }
        if (rv)
            return 0;
...
u32
snat_icmp_hairpinning(snat_main_t *sm, vlib_buffer_t *b0, ip4_header_t *ip0, icmp46_header_t *icmp0, int is_ed)
{
    ...
    if (icmp_is_error_message(icmp0)) {
        ...
        if (is_ed) {
            clib_bihash_kv_16_8_t ed_kv, ed_value;
            make_ed_kv(&ed_kv, &ip0->dst_address, &ip0->src_address, inner_ip0->protocol, sm->outside_fib_index,
                       l4_header->src_port, l4_header->dst_port);
            if (clib_bihash_search_16_8(&sm->per_thread_data[ti].out2in_ed, &ed_kv, &ed_value))
                return 1;
            si = ed_value.value;
        } else {
            key0.addr      = ip0->dst_address;
        ...
        if (clib_bihash_search_8_8(&sm->static_mapping_by_external, &kv0, &value0)) {
            if (!is_ed) {
                ...
                if (sm->num_workers > 1)
                    ...
                    ti = sm->num_workers;
                int rv = clib_bihash_search_8_8(&sm->per_thread_data[ti].out2in, &kv0, &value0);
                if (!rv) {
                    si                                    = value0.value;
                    s0                                    = pool_elt_at_index(sm->per_thread_data[ti].sessions, si);
                    new_dst_addr0                         = s0->in2out.addr.as_u32;
...
void
nat_hairpinning_sm_unknown_proto(snat_main_t *sm, vlib_buffer_t *b, ip4_header_t *ip)
{
    clib_bihash_kv_8_8_t kv, value;
...
void
nat44_ed_hairpinning_unknown_proto(snat_main_t *sm, vlib_buffer_t *b, ip4_header_t *ip)
{
    ...
    old_addr = ip->dst_address.as_u32;
    make_ed_kv(&s_kv, &ip->dst_address, &ip->src_address, ip->protocol, sm->outside_fib_index, 0, 0);
...
void
nat44_reass_hairpinning(snat_main_t *sm, vlib_buffer_t *b0, ip4_header_t *ip0, u16 sport, u16 dport, u32 proto0, int is_ed)
{
    snat_session_key_t key0, sm0;
    snat_session_t *s0;
    clib_bihash_kv_8_8_t kv0, value0;
    ...
    if (new_dst_addr0) {
        ...
        old_dst_port0 = dport;
        if (PREDICT_TRUE(new_dst_port0 != old_dst_port0 && ip4_is_first_fragment(ip0))) {
            if (PREDICT_TRUE(proto0 == SNAT_PROTOCOL_TCP)) {
...
static inline uword
nat44_hairpinning_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ed)
{
    ...
    stats_node_index = is_ed ? nat44_ed_hairpinning_node.index : nat44_hairpinning_node.index;
    ...
    vlib_node_increment_counter(vm, stats_node_index, NAT44_HAIRPIN_ERROR_PROCESSED, pkts_processed);
    return frame->n_vectors;
}

static uword
nat44_hairpinning_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nat44_ed_hairpinning_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static inline uword
snat_hairpin_dst_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ed)
{
    u32 n_left_from, *from, *to_next, stats_node_index;
    nat_hairpin_next_t next_index;
    u32 pkts_processed = 0;
    ...
    stats_node_index = is_ed ? nat44_ed_hairpin_dst_node.index : snat_hairpin_dst_node.index;
...
static uword
snat_hairpin_dst_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nat44_ed_hairpin_dst_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static inline uword
snat_hairpin_src_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ed)
{
    u32 n_left_from, *from, *to_next, stats_node_index;
    ...
    stats_node_index = is_ed ? nat44_ed_hairpin_src_node.index : snat_hairpin_src_node.index;
...
static uword
snat_hairpin_src_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nat44_ed_hairpin_src_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
