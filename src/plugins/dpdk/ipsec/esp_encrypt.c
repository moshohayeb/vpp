...
format_esp_encrypt_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "\n%U%U", format_white_space, indent, format_esp_header, t->packet_data + offset);

    return s;
}

static uword
dpdk_esp_encrypt_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (sa_index0 != last_sa_index) {
                ...
                if (PREDICT_FALSE(res_idx == (u16) ~0)) {
                    ...
                    vlib_node_increment_counter(vm, dpdk_esp_encrypt_node.index, ESP_ENCRYPT_ERROR_NOSUP, 1);
                    to_next[0] = bi0;
                    to_next += 1;
                    n_left_to_next -= 1;
                    goto trace;
                ...
                if (PREDICT_FALSE(error || !session)) {
                    ...
                    vlib_node_increment_counter(vm, dpdk_esp_encrypt_node.index, ESP_ENCRYPT_ERROR_SESSION, 1);
                    to_next[0] = bi0;
                    to_next += 1;
                    n_left_to_next -= 1;
                    goto trace;
            ...
            if (PREDICT_FALSE(esp_seq_advance(sa0))) {
                ...
                vlib_node_increment_counter(vm, dpdk_esp_encrypt_node.index, ESP_ENCRYPT_ERROR_SEQ_CYCLED, 1);
                // TODO: rekey SA
                to_next[0] = bi0;
                to_next += 1;
                n_left_to_next -= 1;
            ...
            if (sa0->is_tunnel) {
                ...
                if (!is_ipv6 && !sa0->is_tunnel_ip6) /* ip4inip4 */
                {
                    ...
                    u8 adv     = sizeof(ip4_header_t) + udp_encap_adv + sizeof(esp_header_t) + iv_size;
                     ...
                     */
                    oh0->ip4.checksum_data_64[0] = clib_host_to_net_u64(0x45ULL << 56);
                    /*
                     * oh0->ip4.ttl = 254;
                     * oh0->ip4.protocol = IP_PROTOCOL_IPSEC_ESP;
                     */
                    oh0->ip4.checksum_data_32[2] = clib_host_to_net_u32(0xfe320000);
                ...
                } else if (is_ipv6 && sa0->is_tunnel_ip6) /* ip6inip6 */
                {
                    ...
                    u8 adv = sizeof(ip6_header_t) + sizeof(esp_header_t) + iv_size;
                    vlib_buffer_advance(b0, -adv);
            ...
            } else /* transport mode */
            {
                ...
                } else /* ipv4 */
                {
                    ...
                    if (sa0->udp_encap) {
                        oh0->ip4.protocol = IP_PROTOCOL_UDP;
                        esp0              = (esp_header_t *) (((u8 *) oh0) + ip_size + udp_encap_adv);
                    } else {
                        oh0->ip4.protocol = IP_PROTOCOL_IPSEC_ESP;
                        esp0              = (esp_header_t *) (((u8 *) oh0) + ip_size);
                    }
            ...
            u8 *padding = vlib_buffer_put_uninit(b0, pad_bytes + 2 + trunc_size);
            ...
            if (is_ipv6) {
                ...
                if (sa0->udp_encap && ouh0) {
                    ouh0->udp.length =
                        clib_host_to_net_u16(clib_net_to_host_u16(ouh0->ip4.length) - ip4_header_bytes(&ouh0->ip4));
                }
            }
            ...
            crypto_op_setup(is_aead, mb0, op, session, cipher_off, cipher_len, 0, auth_len, (u8 *) aad, digest,
                            digest_paddr);
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE(dpdk_esp_encrypt_node) = {.function      = dpdk_esp_encrypt_node_fn,
