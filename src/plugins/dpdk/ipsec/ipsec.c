...
cipher_cap_to_alg(const struct rte_cryptodev_capabilities *cap, u8 key_len)
{
    ...
    {
        if ((cap->sym.xform_type == RTE_CRYPTO_SYM_XFORM_CIPHER) && (alg->type == RTE_CRYPTO_SYM_XFORM_CIPHER) &&
            (cap->sym.cipher.algo == alg->alg) && (alg->key_len == key_len))
            return alg;
        if ((cap->sym.xform_type == RTE_CRYPTO_SYM_XFORM_AEAD) && (alg->type == RTE_CRYPTO_SYM_XFORM_AEAD) &&
...
auth_cap_to_alg(const struct rte_cryptodev_capabilities *cap, u8 trunc_size)
{
    ...
    if ((cap->op != RTE_CRYPTO_OP_TYPE_SYMMETRIC) || (cap->sym.xform_type != RTE_CRYPTO_SYM_XFORM_AUTH))
        return NULL;
...
static void
crypto_set_aead_xform(struct rte_crypto_sym_xform *xform, ipsec_sa_t *sa, u8 is_outbound)
{
    dpdk_crypto_main_t *dcm = &dpdk_crypto_main;
    crypto_alg_t *c;
    ...
    xform->aead.iv.offset     = crypto_op_get_priv_offset() + offsetof(dpdk_op_priv_t, cb);
...
static void
crypto_set_cipher_xform(struct rte_crypto_sym_xform *xform, ipsec_sa_t *sa, u8 is_outbound)
{
    dpdk_crypto_main_t *dcm = &dpdk_crypto_main;
    crypto_alg_t *c;
    ...
    xform->cipher.iv.offset  = crypto_op_get_priv_offset() + offsetof(dpdk_op_priv_t, cb);
...
static void
crypto_set_auth_xform(struct rte_crypto_sym_xform *xform, ipsec_sa_t *sa, u8 is_outbound)
{
    dpdk_crypto_main_t *dcm = &dpdk_crypto_main;
    crypto_alg_t *a;
...
clib_error_t *
create_sym_session(struct rte_cryptodev_sym_session **session, u32 sa_idx, crypto_resource_t *res,
                   crypto_worker_main_t *cwm, u8 is_outbound)
{
    ...
    if ((sa->crypto_alg == IPSEC_CRYPTO_ALG_AES_GCM_128) | (sa->crypto_alg == IPSEC_CRYPTO_ALG_AES_GCM_192) |
    ...
    i32 ret = rte_cryptodev_sym_session_init(res->dev_id, session[0], xfs, mp[0]);
    if (ret) {
        data->session_drv_failed[res->drv_id] += 1;
        erorr = clib_error_return(0, "failed to init session for drv %u", res->drv_id);
        goto done;
    }
...
static inline void *
get_session_private_data(const struct rte_cryptodev_sym_session *sess, uint8_t driver_id)
{
...
/* This is from rte_cryptodev_pmd.h */
static inline void
set_session_private_data(struct rte_cryptodev_sym_session *sess, uint8_t driver_id, void *private_data)
{
...
dpdk_ipsec_check_support(ipsec_sa_t *sa)
{
    ...
    if (sa->integ_alg == IPSEC_INTEG_ALG_NONE)
        ...
        case IPSEC_CRYPTO_ALG_AES_GCM_256:
            break;
        default:
            return clib_error_return(0, "unsupported integ-alg %U crypto-alg %U", format_ipsec_integ_alg, sa->integ_alg,
                                     format_ipsec_crypto_alg, sa->crypto_alg);
        }

    /* XXX do we need the NONE check? */
    if (sa->crypto_alg != IPSEC_CRYPTO_ALG_NONE && dcm->cipher_algs[sa->crypto_alg].disabled)
        return clib_error_return(0, "disabled crypto-alg %U", format_ipsec_crypto_alg, sa->crypto_alg);

    /* XXX do we need the NONE check? */
    if (sa->integ_alg != IPSEC_INTEG_ALG_NONE && dcm->auth_algs[sa->integ_alg].disabled)
        return clib_error_return(0, "disabled integ-alg %U", format_ipsec_integ_alg, sa->integ_alg);
    return NULL;
}

static void
crypto_parse_capabilities(crypto_dev_t *dev, const struct rte_cryptodev_capabilities *cap, u32 n_mains)
{
    dpdk_crypto_main_t *dcm = &dpdk_crypto_main;
    crypto_alg_t *alg;
    u8 len, inc;
    ...
    for (; cap->op != RTE_CRYPTO_OP_TYPE_UNDEFINED; cap++) {
        ...
        case RTE_CRYPTO_SYM_XFORM_CIPHER:
            ...
            inc = inc ? inc : 1;
            for (len = cap->sym.cipher.key_size.min; len <= cap->sym.cipher.key_size.max; len += inc) {
        ...
        case RTE_CRYPTO_SYM_XFORM_AUTH:
            ...
            inc = inc ? inc : 1;
            for (len = cap->sym.auth.digest_size.min; len <= cap->sym.auth.digest_size.max; len += inc) {
...
crypto_scan_devs(u32 n_mains)
{
    ...
    vec_validate_init_empty(dcm->dev, rte_cryptodev_count() - 1, (crypto_dev_t) EMPTY_STRUCT);
    ...
    for (i = 0; i < rte_cryptodev_count(); i++) {
        ...
        vec_validate_init_empty_aligned(dcm->resource, res_idx + max_res_idx, (crypto_resource_t) EMPTY_STRUCT,
...
crypto_auto_placement(void)
{
    ...
    vec_foreach(dev, dcm->dev)
    {
        vec_foreach_index(thread_idx, dcm->workers_main)
        {
            ...
            vec_foreach(idx, dev->used_resources) if (dcm->resource[idx[0]].thread_idx == thread_idx) continue;
            ...
            for (i = 0; i < IPSEC_CRYPTO_N_ALG; i++)
                if (dev->cipher_support[i] && cwm->cipher_resource_idx[i] == (u16) ~0) {
...
static void
crypto_op_init(struct rte_mempool *mempool, void *_arg __attribute__((unused)), void *_obj,
...
crypto_create_crypto_op_pool(vlib_main_t *vm, u8 numa)
{
    ...
    error = dpdk_pool_create(vm, pool_name, crypto_op_len(), conf->num_mbufs, pool_priv_size, 512, numa, &mp, &pri);
...
crypto_create_session_h_pool(vlib_main_t *vm, u8 numa)
{
    ...
    error = dpdk_pool_create(vm, pool_name, elt_size, DPDK_CRYPTO_NB_SESS_OBJS, 0, 512, numa, &mp, &pri);
...
crypto_create_session_drv_pool(vlib_main_t *vm, crypto_dev_t *dev)
{
    ...
    vec_validate_aligned(data->session_by_drv_id_and_sa_index, 32, CLIB_CACHE_LINE_BYTES);
...
crypto_disable(void)
{
    ...
    vec_foreach(data, dcm->data)
    {
        ...
        vec_foreach_index(i, data->session_drv) rte_mempool_free(data->session_drv[i]);
...
static uword
dpdk_ipsec_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ipsec_main_t *im        = &ipsec_main;
    dpdk_crypto_main_t *dcm = &dpdk_crypto_main;
    ...
    vec_validate_init_empty_aligned(dcm->workers_main, n_mains - 1, (crypto_worker_main_t) EMPTY_STRUCT,
                                    CLIB_CACHE_LINE_BYTES);

    /* *INDENT-OFF* */
    vec_foreach(cwm, dcm->workers_main)
    {
        vec_validate_init_empty_aligned(cwm->ops, VLIB_FRAME_SIZE - 1, 0, CLIB_CACHE_LINE_BYTES);
        memset(cwm->cipher_resource_idx, ~0, IPSEC_CRYPTO_N_ALG * sizeof(*cwm->cipher_resource_idx));
        memset(cwm->auth_resource_idx, ~0, IPSEC_INTEG_N_ALG * sizeof(*cwm->auth_resource_idx));
    }
    /* *INDENT-ON* */
    ...
    im->esp_encrypt_next_index = vlib_node_add_next(vm, node->index, next_node->index);
    ...
    im->esp_decrypt_next_index = vlib_node_add_next(vm, node->index, next_node->index);
