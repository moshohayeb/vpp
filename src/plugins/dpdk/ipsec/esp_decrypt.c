...
format_esp_decrypt_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "cipher %U auth %U\n", format_ipsec_crypto_alg, t->crypto_alg, format_ipsec_integ_alg, t->integ_alg);
    s = format(s, "%U%U", format_white_space, indent, format_esp_header, t->packet_data);
    return s;
}

static uword
dpdk_esp_decrypt_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    if (ret) {
        vlib_node_increment_counter(vm, dpdk_esp_decrypt_node.index, ESP_DECRYPT_ERROR_DISCARD, 1);
        /* Discard whole frame */
        return n_left_from;
    }
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (sa_index0 != last_sa_index) {
                ...
                if (PREDICT_FALSE(res_idx == (u16) ~0)) {
                    ...
                    vlib_node_increment_counter(vm, dpdk_esp_decrypt_node.index, ESP_DECRYPT_ERROR_NOSUP, 1);
                    to_next[0] = bi0;
                    to_next += 1;
                    n_left_to_next -= 1;
                    goto trace;
                ...
                if (PREDICT_FALSE(error || !session)) {
                    ...
                    vlib_node_increment_counter(vm, dpdk_esp_decrypt_node.index, ESP_DECRYPT_ERROR_SESSION, 1);
                    to_next[0] = bi0;
                    to_next += 1;
                    n_left_to_next -= 1;
                    goto trace;
            ...
            if (sa0->use_anti_replay) {
                ...
                if (PREDICT_FALSE(rv)) {
                    ...
                    vlib_node_increment_counter(vm, dpdk_esp_decrypt_node.index, ESP_DECRYPT_ERROR_REPLAY, 1);
                    to_next[0] = bi0;
                    to_next += 1;
                    n_left_to_next -= 1;
                    goto trace;
            ...
            if (payload_len & (cipher_alg->boundary - 1)) {
                ...
                vlib_node_increment_counter(vm, dpdk_esp_decrypt_node.index, ESP_DECRYPT_ERROR_BAD_LEN, 1);
                res->n_ops -= 1;
                to_next[0] = bi0;
                to_next += 1;
                n_left_to_next -= 1;
            ...
            crypto_op_setup(is_aead, mb0, op, session, cipher_off, cipher_len, 0, auth_len, aad, digest, digest_paddr);
        trace:
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                esp_decrypt_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->crypto_alg          = sa0->crypto_alg;
                tr->integ_alg           = sa0->integ_alg;
                clib_memcpy(tr->packet_data, vlib_buffer_get_current(b0), sizeof(esp_header_t));
            }
        }
...
static uword
dpdk_esp_decrypt_post_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (b0->flags & VNET_BUFFER_F_IS_IP4)
                ih4 = (ip4_header_t *) ((u8 *) esp0 - udp_encap_adv - sizeof(ip4_header_t));
            else
	      ...
	      goto trace;
            ...
            if (sa0->is_tunnel) {
                if (f0->next_header == IP_PROTOCOL_IP_IN_IP)
                    ...
                    vlib_node_increment_counter(vm, dpdk_esp_decrypt_node.index, ESP_DECRYPT_ERROR_DECRYPTION_FAILED, 1);
                    goto trace;
                }
            } else /* transport mode */
            {
