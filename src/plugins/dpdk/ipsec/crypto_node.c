...
dpdk_crypto_dequeue(vlib_main_t *vm, vlib_node_runtime_t *node, crypto_resource_t *res, u8 outbound)
{
    ...
    {
        ops   = cwm->ops;
        n_ops = rte_cryptodev_dequeue_burst(res->dev_id, res->qp_id + outbound, ops, VLIB_FRAME_SIZE);
        ...
        while (n_ops > 0) {
            ...
            while (n_ops > 0 && n_left_to_next > 0) {
                ...
                if (PREDICT_FALSE(op->status != RTE_CRYPTO_OP_STATUS_SUCCESS)) {
                    ...
                    vlib_node_increment_counter(vm, dpdk_crypto_input_node.index, DPDK_CRYPTO_INPUT_ERROR_STATUS, 1);
                }
    ...
    vlib_node_increment_counter(vm, dpdk_crypto_input_node.index, DPDK_CRYPTO_INPUT_ERROR_DQ_COPS, total_n_deq);
    return total_n_deq;
}

static uword
dpdk_crypto_input_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
