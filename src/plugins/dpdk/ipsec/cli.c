...
format_crypto(u8 *s, va_list *args)
{
    ...
    s = format(s, "%-25s%-20s%-10s\n", dev->name, drv->name, rte_cryptodevs[dev->id].data->dev_started ? "up" : "down");
    ...
    if (dev->features) {
        for (mask = 1; mask != 0; mask <<= 1) {
            ...
            if (feat) {
                s   = format(s, "%s%s", pre, rte_cryptodev_get_feature_name(feat));
                pre = ", ";
            }
        }
        s = format(s, "\n");
...
static clib_error_t *
show_dpdk_crypto_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dpdk_crypto_main_t *dcm = &dpdk_crypto_main;
    crypto_dev_t *dev;

    /* *INDENT-OFF* */
    vec_foreach(dev, dcm->dev) vlib_cli_output(vm, "%U", format_crypto, dev);
    /* *INDENT-ON* */
...
format_crypto_worker(u8 *s, va_list *args)
{
    ...
    s = format(s, "Thread %u (%v):\n", thread_idx, vlib_worker_threads[thread_idx].name);
    ...
    vec_foreach(res_idx, cwm->resource_idx)
    {
        ...
        s   = format(s, "%s%-20s dev-id %2u inbound-queue %2u outbound-queue %2u\n", ind,
                   vec_elt_at_index(dcm->dev, res->dev_id)->name, res->dev_id, res->qp_id, res->qp_id + 1);
...
static clib_error_t *
common_crypto_placement_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd, u8 verbose)
{
    dpdk_crypto_main_t *dcm = &dpdk_crypto_main;
...
static clib_error_t *
show_dpdk_crypto_placement_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    return common_crypto_placement_fn(vm, input, cmd, 0);
}

static clib_error_t *
show_dpdk_crypto_placement_v_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
set_dpdk_crypto_placement_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    /* *INDENT-OFF* */
    vec_foreach(idx, dev->used_resources) if (dcm->resource[idx[0]].thread_idx ==
                                              thread_idx) return clib_error_return(0, "thread %u already using device %u",
                                                                                   thread_idx, dev_idx);
    /* *INDENT-ON* */
...
static clib_error_t *
clear_dpdk_crypto_placement_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    dpdk_crypto_main_t *dcm = &dpdk_crypto_main;
    crypto_dev_t *dev;
    ...
    if (free_all) {
        /* *INDENT-OFF* */
        vec_foreach(res_idx, dev->used_resources) dpdk_crypto_clear_resource(res_idx[0]);
        /* *INDENT-ON* */
    ...
    /* *INDENT-OFF* */
    vec_foreach(res_idx, dev->used_resources) if (dcm->resource[res_idx[0]].thread_idx == thread_idx) break;
    /* *INDENT-ON* */

    if (!(res_idx < vec_end(dev->used_resources)))
        return clib_error_return(0, "thread %u is not using device %u", thread_idx, dev_idx);
...
format_dpdk_mempool(u8 *s, va_list *args)
{
    ...
    s = format(s, "%s\n%Uavailable %7d, allocated %7d total %7d\n", mp->name, format_white_space, indent + 2, count,
               mp->size - count, mp->size);
    s = format(s, "%Uphys_addr %p, flags %08x, nb_mem_chunks %u\n", format_white_space, indent + 2, mp->mz->phys_addr,
               mp->flags, mp->nb_mem_chunks);
    s = format(s, "%Uelt_size %4u, header_size %3u, trailer_size %u\n", format_white_space, indent + 2, mp->elt_size,
               mp->header_size, mp->trailer_size);
    s = format(s, "%Uprivate_data_size %3u, total_elt_size %u\n", format_white_space, indent + 2, mp->private_data_size,
               mp->elt_size + mp->header_size + mp->trailer_size);
    return s;
}

static clib_error_t *
show_dpdk_crypto_pools_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dpdk_crypto_main_t *dcm = &dpdk_crypto_main;
    crypto_data_t *data;
    ...
    vec_foreach(data, dcm->data)
    {
        ...
        vec_foreach(mp, data->session_drv) if (mp[0]) vlib_cli_output(vm, "%U\n", format_dpdk_mempool, mp[0]);
    }
    /* *INDENT-ON* */
