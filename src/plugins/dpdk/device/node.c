...
STATIC_ASSERT((PKT_RX_IP_CKSUM_GOOD | PKT_RX_IP_CKSUM_BAD | PKT_RX_FDIR) < 256,
...
static_always_inline uword
dpdk_process_rx_burst(vlib_main_t *vm, dpdk_per_thread_data_t *ptd, uword n_rx_packets, int maybe_multiseg, u8 *or_flagsp)
{
...
static_always_inline void
dpdk_set_next_from_etype(vlib_main_t *vm, vlib_node_runtime_t *node, dpdk_per_thread_data_t *ptd, uword n_rx_packets)
{
    vlib_buffer_t *b[4];
    i16 adv[4];
    u16 etype[4];
...
static_always_inline void
dpdk_process_flow_offload(dpdk_device_t *xd, dpdk_per_thread_data_t *ptd, uword n_rx_packets)
{
    uword n;
...
dpdk_device_input(vlib_main_t *vm, dpdk_main_t *dm, dpdk_device_t *xd, vlib_node_runtime_t *node, u32 thread_index,
                  u16 queue_id)
{
    ...
    while (n_rx_packets < DPDK_RX_BURST_SZ) {
        n = rte_eth_rx_burst(xd->port_id, queue_id, ptd->mbufs + n_rx_packets, DPDK_RX_BURST_SZ - n_rx_packets);
        n_rx_packets += n;
    ...
    /* flow offload - process if rx flow offload enabled and at least one packet
       is marked */
    if (PREDICT_FALSE((xd->flags & DPDK_DEVICE_FLAG_RX_FLOW_OFFLOAD) && (or_flags & (1 << DPDK_RX_F_FDIR))))
    ...
    if (PREDICT_FALSE((n_trace = vlib_get_trace_count(vm, node)))) {
        ...
        while (n_trace && n_left) {
            ...
            clib_memcpy(&t0->data, mb[0]->buf_addr + mb[0]->data_off, sizeof t0->data);
            n_trace--;
            n_left--;
            buffers++;
            mb++;
    ...
    if (PREDICT_FALSE(dm->pcap[VLIB_RX].pcap_enable)) {
        ...
        while (n_left) {
            ...
            if (dm->pcap[VLIB_RX].pcap_sw_if_index == 0 ||
                dm->pcap[VLIB_RX].pcap_sw_if_index == vnet_buffer(b0)->sw_if_index[VLIB_RX]) {
                struct rte_mbuf *mb;
                i16 data_start;
                i32 temp_advance;
...
VLIB_NODE_FN(dpdk_input_node)(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *f)
{
    ...
    foreach_device_and_queue(dq, rt->devices_and_queues)
    {
        ...
        n_rx_packets += dpdk_device_input(vm, dm, xd, node, thread_index, dq->queue_id);
    }
    /* *INDENT-ON* */
    return n_rx_packets;
}
