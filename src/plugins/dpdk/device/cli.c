...
static clib_error_t *
get_hqos(u32 hw_if_index, u32 subport_id, dpdk_device_t **xd, dpdk_device_config_t **devconf)
{
...
pcap_trace_command_internal(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd, int rx_tx)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "on")) {
            ...
            if (dm->pcap[rx_tx].pcap_enable) {
                vlib_cli_output(vm, "captured %d pkts...", dm->pcap[rx_tx].pcap_main.n_packets_captured);
                if (dm->pcap[rx_tx].pcap_main.n_packets_captured) {
                    dm->pcap[rx_tx].pcap_main.n_packets_to_capture = dm->pcap[rx_tx].pcap_main.n_packets_captured;
                    error                                          = pcap_write(&dm->pcap[rx_tx].pcap_main);
                    if (error)
                        clib_error_report(error);
                    else
                        vlib_cli_output(vm, "saved to %s...", dm->pcap[rx_tx].pcap_main.file_name);
                }
            ...
            if (dm->pcap[rx_tx].pcap_enable) {
                vlib_cli_output(vm, "can't change max value while pcap tx capture active...");
                errorFlag = 1;
                break;
            }
            dm->pcap[rx_tx].pcap_main.n_packets_to_capture = max;
        } else if (unformat(line_input, "intfc %U", unformat_vnet_sw_interface, dm->vnet_main,
            ...
            if (dm->pcap[rx_tx].pcap_enable) {
                vlib_cli_output(vm, "can't change file while pcap tx capture active...");
                errorFlag = 1;
                break;
            }
            ...
            if (dm->pcap[rx_tx].pcap_sw_if_index == 0) {
                vlib_cli_output(vm, "max is %d for any interface to file %s",
                                ...
                                dm->pcap[rx_tx].pcap_main.file_name ? (u8 *) dm->pcap[rx_tx].pcap_main.file_name :
                                                                      (u8 *) "/tmp/vpe.pcap");
            } else {
                vlib_cli_output(vm, "max is %d for interface %U to file %s",
                                dm->pcap[rx_tx].pcap_main.n_packets_to_capture ?
                                    dm->pcap[rx_tx].pcap_main.n_packets_to_capture :
                                    PCAP_DEF_PKT_TO_CAPTURE,
                                format_vnet_sw_if_index_name, dm->vnet_main, dm->pcap_sw_if_index,
                                dm->pcap[rx_tx].pcap_main.file_name ? (u8 *) dm->pcap[rx_tx].pcap_main.file_name :
                                                                      (u8 *) "/tmp/vpe.pcap");
            }

            if (dm->pcap[rx_tx].pcap_enable == 0) {
                vlib_cli_output(vm, "pcap %s capture is off...", (rx_tx == VLIB_RX) ? "rx" : "tx");
            } else {
                vlib_cli_output(vm, "pcap %s capture is on: %d of %d pkts...", (rx_tx == VLIB_RX) ? "rx" : "tx",
    ...
    if (errorFlag == 0) {
        ...
        if (enabled) {
            if (dm->pcap[rx_tx].pcap_main.file_name == 0)
                dm->pcap[rx_tx].pcap_main.file_name = (char *) format(0, "/tmp/vpe.pcap%c", 0);
            ...
            vlib_cli_output(vm, "pcap %s capture on...", rx_tx == VLIB_RX ? "rx" : "tx");
        }
...
static clib_error_t *
pcap_rx_trace_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    return pcap_trace_command_internal(vm, input, cmd, VLIB_RX);
}

static clib_error_t *
pcap_tx_trace_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
VLIB_CLI_COMMAND(pcap_tx_trace_command, static) = {
    .path       = "pcap tx trace",
    .short_help = "pcap tx trace [on|off] [max <nn>] [intfc <interface>|any] [file <name>] [status]",
    .function   = pcap_tx_trace_command_fn,
};
VLIB_CLI_COMMAND(pcap_rx_trace_command, static) = {
    .path       = "pcap rx trace",
    .short_help = "pcap rx trace [on|off] [max <nn>] [intfc <interface>|any] [file <name>] [status]",
...
static clib_error_t *
show_dpdk_buffer(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < vec_len(dpdk_main.pktmbuf_pools); i++) {
        ...
        if (rmp) {
            ...
            vlib_cli_output(vm, "name=\"%s\"  available = %7d allocated = %7d total = %7d\n", rmp->name, (u32) count,
                            (u32) free_count, (u32)(count + free_count));
        } else {
...
static clib_error_t *
show_dpdk_physmem(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *err = 0;
    u32 pipe_max_size;
    int fds[2];
...
static clib_error_t *
test_dpdk_buffer(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (n_free) {
        if (vec_len(allocated_buffers) < n_free)
            return clib_error_return(0, "Can't free %d, only %d allocated", n_free, vec_len(allocated_buffers));
    ...
    if (n_alloc) {
        ...
        if (actual_alloc < n_alloc)
            vlib_cli_output(vm, "WARNING: only allocated %d buffers", actual_alloc);
    }
...
static clib_error_t *
set_dpdk_if_desc(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    dpdk_main_t *dm = &dpdk_main;
    vnet_hw_interface_t *hw;
...
static clib_error_t *
show_dpdk_if_hqos_placement(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (cpu = 0; cpu < vec_len(dm->devices_by_hqos_cpu); cpu++) {
        if (cpu >= dm->hqos_cpu_first_index && cpu < (dm->hqos_cpu_first_index + dm->hqos_cpu_count))
            vlib_cli_output(vm, "Thread %u (%s at lcore %u):", cpu, vlib_worker_threads[cpu].name,
...
static clib_error_t *
set_dpdk_if_hqos_placement(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    dpdk_main_t *dm = &dpdk_main;
    dpdk_device_and_queue_t *dq;
    ...
    for (i = 0; i < vec_len(dm->devices_by_hqos_cpu); i++) {
        vec_foreach(dq, dm->devices_by_hqos_cpu[i])
        {
            if (hw_if_index == dm->devices[dq->device].hw_if_index) {
                ...
                vec_del1(dm->devices_by_hqos_cpu[i], dq - dm->devices_by_hqos_cpu[i]);
                vec_add2(dm->devices_by_hqos_cpu[cpu], dq, 1);
                dq->queue_id = 0;
                dq->device   = xd->device_index;

                vec_sort_with_function(dm->devices_by_hqos_cpu[i], dpdk_device_queue_sort);

                vec_sort_with_function(dm->devices_by_hqos_cpu[cpu], dpdk_device_queue_sort);
...
static clib_error_t *
set_dpdk_if_hqos_pipe(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    dpdk_main_t *dm = &dpdk_main;
    vnet_hw_interface_t *hw;
...
static clib_error_t *
set_dpdk_if_hqos_subport(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    dpdk_main_t *dm   = &dpdk_main;
    ...
    u32 tb_rate                                     = (u32) ~0;
    u32 tb_size                                     = (u32) ~0;
    u32 tc_rate[RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE] = {(u32) ~0, (u32) ~0, (u32) ~0, (u32) ~0};
    u32 tc_period                                   = (u32) ~0;
...
static clib_error_t *
set_dpdk_if_hqos_tctbl(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    vlib_thread_main_t *tm = vlib_get_thread_main();
    dpdk_main_t *dm        = &dpdk_main;
...
static clib_error_t *
set_dpdk_if_hqos_pktfield(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    vlib_thread_main_t *tm = vlib_get_thread_main();
    dpdk_main_t *dm        = &dpdk_main;
    ...
    for (i = 0; i < worker_thread_count; i++)
        ...
        case 0:
            ...
            xd->hqos_wt[worker_thread_first + i].hqos_field0_slabshr  = count_trailing_zeros(mask);
            break;
        case 1:
            xd->hqos_wt[worker_thread_first + i].hqos_field1_slabpos  = offset;
            xd->hqos_wt[worker_thread_first + i].hqos_field1_slabmask = mask;
            xd->hqos_wt[worker_thread_first + i].hqos_field1_slabshr  = count_trailing_zeros(mask);
            break;
        case 2:
        default:
            xd->hqos_wt[worker_thread_first + i].hqos_field2_slabpos  = offset;
            xd->hqos_wt[worker_thread_first + i].hqos_field2_slabmask = mask;
            xd->hqos_wt[worker_thread_first + i].hqos_field2_slabshr  = count_trailing_zeros(mask);
        }
...
static clib_error_t *
show_dpdk_if_hqos(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    vlib_thread_main_t *tm = vlib_get_thread_main();
    dpdk_main_t *dm        = &dpdk_main;
    ...
    vlib_cli_output(vm, "   Enqueue burst size = %u packets", ht->hqos_burst_enq);
    vlib_cli_output(vm, "   Dequeue burst size = %u packets", ht->hqos_burst_deq);

    vlib_cli_output(vm, "   Packet field 0: slab position = %4u, slab bitmask = 0x%016llx   (subport)",
                    wk->hqos_field0_slabpos, wk->hqos_field0_slabmask);
    vlib_cli_output(vm, "   Packet field 1: slab position = %4u, slab bitmask = 0x%016llx   (pipe)",
                    wk->hqos_field1_slabpos, wk->hqos_field1_slabmask);
    vlib_cli_output(vm, "   Packet field 2: slab position = %4u, slab bitmask = 0x%016llx   (tc)",
                    wk->hqos_field2_slabpos, wk->hqos_field2_slabmask);
    vlib_cli_output(vm, "   Packet field 2  tc translation table: ([Mapped Value Range]: tc/queue tc/queue ...)");
    vlib_cli_output(vm,
                    "     [ 0 .. 15]: "
                    "%u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u",
                    tctbl[0] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[0] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[1] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[1] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[2] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[2] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[3] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[3] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[4] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[4] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[5] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[5] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[6] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[6] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[7] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[7] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[8] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[8] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[9] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[9] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[10] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[10] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[11] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[11] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[12] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[12] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[13] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[13] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[14] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[14] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[15] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[15] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS);
    vlib_cli_output(vm,
                    "     [16 .. 31]: "
                    "%u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u",
                    tctbl[16] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[16] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[17] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[17] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[18] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[18] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[19] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[19] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[20] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[20] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[21] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[21] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[22] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[22] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[23] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[23] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[24] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[24] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[25] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[25] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[26] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[26] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[27] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[27] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[28] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[28] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[29] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[29] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[30] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[30] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[31] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[31] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS);
    vlib_cli_output(vm,
                    "     [32 .. 47]: "
                    "%u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u",
                    tctbl[32] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[32] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[33] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[33] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[34] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[34] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[35] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[35] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[36] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[36] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[37] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[37] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[38] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[38] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[39] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[39] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[40] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[40] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[41] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[41] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[42] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[42] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[43] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[43] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[44] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[44] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[45] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[45] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[46] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[46] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[47] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[47] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS);
    vlib_cli_output(vm,
                    "     [48 .. 63]: "
                    "%u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u",
                    tctbl[48] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[48] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[49] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[49] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[50] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[50] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[51] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[51] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[52] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[52] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[53] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[53] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[54] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[54] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[55] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[55] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[56] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[56] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[57] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[57] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[58] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[58] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[59] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[59] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[60] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[60] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[61] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[61] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[62] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[62] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS,
                    tctbl[63] / RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS, tctbl[63] % RTE_SCHED_QUEUES_PER_TRAFFIC_CLASS);
    vlib_cli_output(vm, " Port:");
    vlib_cli_output(vm, "   Rate = %u bytes/second", cfg->port.rate);
    vlib_cli_output(vm, "   MTU = %u bytes", cfg->port.mtu);
    vlib_cli_output(vm, "   Frame overhead = %u bytes", cfg->port.frame_overhead);
    vlib_cli_output(vm, "   Number of subports = %u", cfg->port.n_subports_per_port);
    vlib_cli_output(vm, "   Number of pipes per subport = %u", cfg->port.n_pipes_per_subport);
    vlib_cli_output(vm, "   Packet queue size: TC0 = %u, TC1 = %u, TC2 = %u, TC3 = %u packets", cfg->port.qsize[0],
                    cfg->port.qsize[1], cfg->port.qsize[2], cfg->port.qsize[3]);
    vlib_cli_output(vm, "   Number of pipe profiles = %u", cfg->port.n_pipe_profiles);

    for (subport_id = 0; subport_id < vec_len(cfg->subport); subport_id++) {
        vlib_cli_output(vm, " Subport %u:", subport_id);
        vlib_cli_output(vm, "   Rate = %u bytes/second", cfg->subport[subport_id].tb_rate);
        vlib_cli_output(vm, "   Token bucket size = %u bytes", cfg->subport[subport_id].tb_size);
        vlib_cli_output(vm, "   Traffic class rate: TC0 = %u, TC1 = %u, TC2 = %u, TC3 = %u bytes/second",
                        cfg->subport[subport_id].tc_rate[0], cfg->subport[subport_id].tc_rate[1],
                        cfg->subport[subport_id].tc_rate[2], cfg->subport[subport_id].tc_rate[3]);
        vlib_cli_output(vm, "   TC period = %u milliseconds", cfg->subport[subport_id].tc_period);
    }

    for (profile_id = 0; profile_id < vec_len(cfg->pipe); profile_id++) {
        vlib_cli_output(vm, " Pipe profile %u:", profile_id);
        vlib_cli_output(vm, "   Rate = %u bytes/second", cfg->pipe[profile_id].tb_rate);
        vlib_cli_output(vm, "   Token bucket size = %u bytes", cfg->pipe[profile_id].tb_size);
        vlib_cli_output(vm, "   Traffic class rate: TC0 = %u, TC1 = %u, TC2 = %u, TC3 = %u bytes/second",
                        cfg->pipe[profile_id].tc_rate[0], cfg->pipe[profile_id].tc_rate[1],
                        cfg->pipe[profile_id].tc_rate[2], cfg->pipe[profile_id].tc_rate[3]);
        vlib_cli_output(vm, "   TC period = %u milliseconds", cfg->pipe[profile_id].tc_period);
#ifdef RTE_SCHED_SUBPORT_TC_OV
        vlib_cli_output(vm, "   TC3 oversubscription_weight = %u", cfg->pipe[profile_id].tc_ov_weight);
#endif

        for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++) {
            vlib_cli_output(vm, "   TC%u WRR weights: Q0 = %u, Q1 = %u, Q2 = %u, Q3 = %u", i,
                            cfg->pipe[profile_id].wrr_weights[i * 4], cfg->pipe[profile_id].wrr_weights[i * 4 + 1],
                            cfg->pipe[profile_id].wrr_weights[i * 4 + 2], cfg->pipe[profile_id].wrr_weights[i * 4 + 3]);
        }
    }
    ...
    for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++) {
        vlib_cli_output(vm, "   TC%u min: G = %u, Y = %u, R = %u", i, cfg->port.red_params[i][e_RTE_METER_GREEN].min_th,
                        cfg->port.red_params[i][e_RTE_METER_YELLOW].min_th,
                        cfg->port.red_params[i][e_RTE_METER_RED].min_th);

        vlib_cli_output(vm, "   TC%u max: G = %u, Y = %u, R = %u", i, cfg->port.red_params[i][e_RTE_METER_GREEN].max_th,
        ...
        vlib_cli_output(vm, "   TC%u inverted probability: G = %u, Y = %u, R = %u", i,
                        cfg->port.red_params[i][e_RTE_METER_GREEN].maxp_inv,
...
static clib_error_t *
show_dpdk_hqos_queue_stats(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    clib_error_t *error = NULL;
    ...
    if (rte_sched_queue_read_stats(xd->hqos_ht->hqos, qindex, &stats, &qlen) != 0) {
...
#ifdef RTE_SCHED_RED
    vlib_cli_output(vm, "%=24s%=16d", "Packets dropped (RED)", stats.n_pkts_red_dropped);
#endif
...
static clib_error_t *
show_dpdk_version_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
 ...
 * DPDK EAL init args:  -c 1 -n 4 --huge-dir /run/vpp/hugepages --file-prefix vpp -w 0000:00:08.0 -w 0000:00:09.0
...
static clib_error_t *
dpdk_validate_buffers_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    ...
    if (is_test) {
        ...
        if (!n_invalid_bufs)
            vlib_cli_output(vm, "All buffers are valid %d uninitialized", uninitialized);
        else
            vlib_cli_output(vm, "Found %d invalid buffers and %d uninitialized", n_invalid_bufs, uninitialized);
    }
    return 0;
}
