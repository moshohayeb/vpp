...
dpdk_device_lock_init(dpdk_device_t *xd)
{
    ...
    for (q = 0; q < xd->tx_q_used; q++) {
        xd->lockp[q] = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
...
dpdk_ring_alloc(struct rte_mempool *mp)
{
    ...
    r = clib_mem_alloc_aligned(rte_ring_get_memsize(count), CLIB_CACHE_LINE_BYTES);
...
dpdk_lib_init(dpdk_main_t *dm)
{
    ...
    if (dm->conf->enable_tcp_udp_checksum)
        dm->buffer_flags_template &= ~(VNET_BUFFER_F_L4_CHECKSUM_CORRECT | VNET_BUFFER_F_L4_CHECKSUM_COMPUTED);
    ...
    RTE_ETH_FOREACH_DEV(i)
    {
        ...
        if (dev_info.device == 0) {
            clib_warning("DPDK bug: missing device info. Skipping %s device", dev_info.driver_name);
            continue;
        }
        ...
        if (pci_dev) {
            ...
            if (pci_dev && next_pci_dev && pci_addr.as_u32 != last_pci_addr.as_u32 &&
                memcmp(&pci_dev->addr, &next_pci_dev->addr, sizeof(struct rte_pci_addr)) == 0) {
        ...
        clib_memcpy(&xd->tx_conf, &dev_info.default_txconf, sizeof(struct rte_eth_txconf));
        ...
        if (devconf->num_rx_queues > 1 && dev_info.max_rx_queues >= devconf->num_rx_queues) {
            ...
            if (devconf->rss_fn == 0)
                ...
                if (unsupported_bits)
                    dpdk_log_warn("Unsupported RSS hash functions: %U", format_dpdk_rss_hf_name, unsupported_bits);
            }
            xd->port_conf.rx_adv_conf.rss_conf.rss_hf &= dev_info.flow_type_rss_offloads;
        } else
            xd->rx_q_used = 1;
...
#define _(s, f) else if (dev_info.driver_name && !strcmp(dev_info.driver_name, s)) xd->pmd = VNET_DPDK_PMD_##f;
            if (0)
                ;
            foreach_dpdk_pmd
#undef _
            ...
            case VNET_DPDK_PMD_I40E:
                ...
                xd->supported_flow_actions = VNET_FLOW_ACTION_MARK | VNET_FLOW_ACTION_REDIRECT_TO_NODE |
                                             VNET_FLOW_ACTION_BUFFER_ADVANCE | VNET_FLOW_ACTION_COUNT |
                                             VNET_FLOW_ACTION_DROP;

                if (dm->conf->no_tx_checksum_offload == 0) {
#if RTE_VERSION < RTE_VERSION_NUM(18, 8, 0, 0)
                    xd->tx_conf.txq_flags &= ~(ETH_TXQ_FLAGS_NOXSUMUDP | ETH_TXQ_FLAGS_NOXSUMTCP);
#else
        ...
        if (ETHERNET_MAX_PACKET_BYTES > dev_info.max_rx_pktlen) {
             ...
             */
            if (dev_info.max_rx_pktlen >= (ETHERNET_MAX_PACKET_BYTES + sizeof(ethernet_header_t))) {
                ...
                if (dpdk_port_crc_strip_enabled(xd) &&
                    (dev_info.max_rx_pktlen >= (ETHERNET_MAX_PACKET_BYTES + sizeof(ethernet_header_t) + 4))) {
                    max_rx_frame += 4;
                }
            } else {
                max_rx_frame = ETHERNET_MAX_PACKET_BYTES;
                ...
                if (dpdk_port_crc_strip_enabled(xd) && (dev_info.max_rx_pktlen >= (ETHERNET_MAX_PACKET_BYTES + 4))) {
                    max_rx_frame += 4;
                }
            }
        }
        ...
        vnet_hw_interface_set_input_node(dm->vnet_main, xd->hw_if_index, dpdk_input_node.index);
        ...
        if (hi)
            hi->max_packet_bytes = xd->port_conf.rxmode.max_rx_pkt_len - sizeof(ethernet_header_t);
        else
...
dpdk_bind_devices_to_uio(dpdk_config_main_t *conf)
{
    ...
    vec_foreach(addr, addrs)
    {
        ...
        if (d->vendor_id == 0x1af4 &&
            (d->device_id == VIRTIO_PCI_LEGACY_DEVICEID_NET || d->device_id == VIRTIO_PCI_MODERN_DEVICEID_NET))
            ;
        /* vmxnet3 */
...
dpdk_device_config(dpdk_config_main_t *conf, vlib_pci_addr_t pci_addr, unformat_input_t *input, u8 is_default)
{
    ...
    if (is_default) {
        ...
        if (!p) {
            ...
            return clib_error_return(0, "duplicate configuration for PCI address %U", format_vlib_pci_addr, &pci_addr);
    }
...
dpdk_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    huge_dir_path = format(0, "%s/hugepages%c", vlib_unix_get_runtime_dir(), 0);
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "no-hugetlb")) {
            ...
            if (error)
                return error;
        } else if (unformat(input, "dev %U %U", unformat_vlib_pci_addr, &pci_addr, unformat_vlib_cli_sub_input,
                            &sub_input)) {
...
#undef _
    ...
    if (!no_huge && !huge_dir) {
        ...
        vec_foreach_index(x, mem_by_socket) socket_mem =
            format(socket_mem, "%s%u", socket_mem ? "," : "", mem_by_socket[x]);
...
#undef _
    ...
    for (i = 0; i < RTE_MAX_LCORE; i++) {
        error = dpdk_buffer_pool_create(vm, conf->num_mbufs, rte_lcore_to_socket_id(i));
        if (error)
            return error;
    }
...
dpdk_update_link_state(dpdk_device_t *xd, f64 now)
{
    ...
    if ((xd->flags & (DPDK_DEVICE_FLAG_ADMIN_UP | DPDK_DEVICE_FLAG_BOND_SLAVE)) &&
        ((xd->link.link_status != 0) ^ vnet_hw_interface_is_link_up(vnm, xd->hw_if_index))) {
        hw_flags_chg = 1;
...
dpdk_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
            ...
            RTE_ETH_FOREACH_DEV(i)
            {
                ...
                if (xd != NULL && xd->pmd == VNET_DPDK_PMD_BOND) {
                    ...
                    if (nlink > 0) {
                        ...
                        dpdk_log_info("Set MAC for bond port %d BondEthernet%d", i, xd->bond_instance_num);
                        ...
                        bhi = vnet_get_hw_interface(vnm, dm->devices[i].hw_if_index);
                        ...
                        while (nlink >= 1) { /* for all slave links */
                            ...
                            if (nlink) {
                                dpdk_log_info("Add MAC for slave port %d", slave);
                                rv = rte_eth_dev_mac_addr_add(slave, (struct ether_addr *) addr, 0);
                                if (rv)
                                    dpdk_log_warn("Add MAC addr failure rv=%d", rv);
                            }
                            /* Setup slave link state change callback handling */
                            rte_eth_dev_callback_register(slave, RTE_ETH_EVENT_INTR_LSC, dpdk_port_state_callback, NULL);
                            dpdk_device_t *sxd = &dm->devices[slave];
                            sxd->flags |= DPDK_DEVICE_FLAG_BOND_SLAVE;
                            sxd->bond_port = i;
                            /* Set slaves bitmap for bonded interface */
                            bhi->bond_info = clib_bitmap_set(bhi->bond_info, sdev->hw_if_index, 1);
                            ...
                            sei            = pool_elt_at_index(em->interfaces, shi->hw_instance);
    ...
    while (1) {
         ...
         */
        f64 min_wait = dm->link_state_poll_interval < dm->stat_poll_interval ? dm->link_state_poll_interval :
...
dpdk_init(vlib_main_t *vm)
{
    ...
    dm->buffer_flags_template =
        (VLIB_BUFFER_TOTAL_LENGTH_VALID | VLIB_BUFFER_EXT_HDR_VALID | VNET_BUFFER_F_L4_CHECKSUM_COMPUTED |
