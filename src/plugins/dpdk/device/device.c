...
static void
dpdk_tx_trace_buffer(dpdk_main_t *dm, vlib_node_runtime_t *node, dpdk_device_t *xd, u16 queue_id, vlib_buffer_t *buffer)
{
    ...
    clib_memcpy(t0->buffer.pre_data, buffer->data + buffer->current_data, sizeof(t0->buffer.pre_data));
...
tx_burst_vector_internal(vlib_main_t *vm, dpdk_device_t *xd, struct rte_mbuf **mb, u32 n_left)
{
    ...
    do {
        ...
        if (PREDICT_FALSE(xd->flags & DPDK_DEVICE_FLAG_HQOS)) /* HQoS ON */
        {
            ...
            n_sent = rte_ring_sp_enqueue_burst(hqos->swq, (void **) mb, n_left, 0);
        ...
        if (PREDICT_FALSE(n_sent < 0)) {
            ...
            node_index = vec_elt_at_index(im->hw_interfaces, xd->hw_if_index)->tx_node_index;
...
VNET_DEVICE_CLASS_TX_FN(dpdk_device_class)(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *f)
{
    ...
    if (PREDICT_FALSE(dm->pcap[VLIB_TX].pcap_enable)) {
        ...
        while (n_left > 0) {
            ...
            if (dm->pcap[VLIB_TX].pcap_sw_if_index == 0 ||
                dm->pcap[VLIB_TX].pcap_sw_if_index == vnet_buffer(b0)->sw_if_index[VLIB_TX])
    ...
    while (n_left >= 8) {
        ...
        if (PREDICT_FALSE((xd->flags & DPDK_DEVICE_FLAG_TX_OFFLOAD) &&
                          (or_flags & (VNET_BUFFER_F_OFFLOAD_TCP_CKSUM | VNET_BUFFER_F_OFFLOAD_IP_CKSUM |
...
dpdk_clear_hw_interface_counters(u32 instance)
{
    ...
    clib_memcpy(xd->last_cleared_xstats, xd->xstats,
                vec_len(xd->last_cleared_xstats) * sizeof(xd->last_cleared_xstats[0]));
}
...
dpdk_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ...
    xd->per_interface_next_index = vlib_node_add_next(xm->vlib_main, dpdk_input_node.index, node_index);
}


static clib_error_t *
dpdk_subif_add_del_function(vnet_main_t *vnm, u32 hw_if_index, struct vnet_sw_interface_t *st, int is_add)
{
    dpdk_main_t *xm         = &dpdk_main;
    vnet_hw_interface_t *hw = vnet_get_hw_interface(vnm, hw_if_index);
    dpdk_device_t *xd       = vec_elt_at_index(xm->devices, hw->dev_instance);
