...
{
    dpdk_log_err("Interface %U error %d: %s", format_dpdk_device_name, xd->port_id, rv, rte_strerror(rv));
...
dpdk_device_setup(dpdk_device_t *xd)
{
    ...
    if (bitmap) {
        dpdk_log_warn("unsupported tx offloads requested on port %u: %U", xd->port_id, format_dpdk_tx_offload_caps,
                      bitmap);
    ...
    if (bitmap) {
        dpdk_log_warn("unsupported rx offloads requested on port %u: %U", xd->port_id, format_dpdk_rx_offload_caps,
                      bitmap);
        xd->port_conf.rxmode.offloads ^= bitmap;
    }

    rv = rte_eth_dev_configure(xd->port_id, xd->rx_q_used, xd->tx_q_used, &xd->port_conf);
    ...
    for (j = 0; j < xd->tx_q_used; j++) {
        rv = rte_eth_tx_queue_setup(xd->port_id, j, xd->nb_tx_desc, xd->cpu_socket, &xd->tx_conf);

        /* retry with any other CPU socket */
        if (rv < 0)
            rv = rte_eth_tx_queue_setup(xd->port_id, j, xd->nb_tx_desc, SOCKET_ID_ANY, &xd->tx_conf);
        if (rv < 0)
            dpdk_device_error(xd, "rte_eth_tx_queue_setup", rv);
    }

    vec_validate_aligned(xd->buffer_pool_for_queue, xd->rx_q_used - 1, CLIB_CACHE_LINE_BYTES);
    for (j = 0; j < xd->rx_q_used; j++) {
        dpdk_mempool_private_t *privp;
        uword tidx     = vnet_get_device_input_thread_index(dm->vnet_main, xd->hw_if_index, j);
        unsigned lcore = vlib_worker_threads[tidx].cpu_id;
        u16 socket_id  = rte_lcore_to_socket_id(lcore);

        rv = rte_eth_rx_queue_setup(xd->port_id, j, xd->nb_rx_desc, xd->cpu_socket, 0, dm->pktmbuf_pools[socket_id]);

        /* retry with any other CPU socket */
        if (rv < 0)
            rv = rte_eth_rx_queue_setup(xd->port_id, j, xd->nb_rx_desc, SOCKET_ID_ANY, 0, dm->pktmbuf_pools[socket_id]);
...
dpdk_device_start(dpdk_device_t *xd)
{
    ...
    if (xd->default_mac_address)
        rv = rte_eth_dev_default_mac_addr_set(xd->port_id, (struct ether_addr *) xd->default_mac_address);
...
static uword
send_garp_na_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
...
garp_na_proc_callback(uword *dpdk_port)
{
    ...
    ASSERT(vlib_get_thread_index() == 0);
    vlib_process_signal_event(vm, send_garp_na_proc_node.index, SEND_GARP_NA, *dpdk_port);
}

always_inline int
dpdk_port_state_callback_inline(dpdk_portid_t port_id, enum rte_eth_event_type type, void *param)
{
    ...
    if (xd->flags & DPDK_DEVICE_FLAG_BOND_SLAVE) {
        ...
        dpdk_log_info("Port %d state to %s, "
                      ...
                      port_id, (link_up) ? "UP" : "DOWN", bd_port, xd->bond_instance_num, bd_mode);
        if (bd_mode == BONDING_MODE_ACTIVE_BACKUP) {
            vl_api_force_rpc_call_main_thread(garp_na_proc_callback, (u8 *) &bd_port, sizeof(uword));
        }
    ...
    {
        if (link_up)
            dpdk_log_info("Port %d Link Up - speed %u Mbps - %s", port_id, (unsigned) link.link_speed,
...
int
dpdk_port_state_callback(dpdk_portid_t port_id, enum rte_eth_event_type type, void *param,
