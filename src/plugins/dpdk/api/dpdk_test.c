...
#define foreach_vpe_api_reply_msg                                                                                      \
    _(SW_INTERFACE_SET_DPDK_HQOS_PIPE_REPLY, sw_interface_set_dpdk_hqos_pipe_reply)                                    \
    _(SW_INTERFACE_SET_DPDK_HQOS_SUBPORT_REPLY, sw_interface_set_dpdk_hqos_subport_reply)                              \
    _(SW_INTERFACE_SET_DPDK_HQOS_TCTBL_REPLY, sw_interface_set_dpdk_hqos_tctbl_reply)
...
api_sw_interface_set_dpdk_hqos_subport(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "rx sw_if_index %u", &sw_if_index))
            ...
            for (tc_id = 0; tc_id < (sizeof(tc_rate) / sizeof(tc_rate[0])); tc_id++)
...
#define foreach_vpe_api_msg                                                                                            \
    _(sw_interface_set_dpdk_hqos_pipe, "rx sw_if_index <id> subport <subport-id> pipe <pipe-id>\n"                     \
                                       "profile <profile-id>\n")                                                       \
...
static void
dpdk_api_hookup(vat_main_t *vam)
{
