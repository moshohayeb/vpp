...
static void
vl_api_sw_interface_set_dpdk_hqos_pipe_t_handler(vl_api_sw_interface_set_dpdk_hqos_pipe_t *mp)
{
...
static void *
vl_api_sw_interface_set_dpdk_hqos_pipe_t_print(vl_api_sw_interface_set_dpdk_hqos_pipe_t *mp, void *handle)
{
    u8 *s;
    ...
    s = format(s, "subport %u  pipe %u  profile %u ", ntohl(mp->subport), ntohl(mp->pipe), ntohl(mp->profile));

    FINISH;
}

static void
vl_api_sw_interface_set_dpdk_hqos_subport_t_handler(vl_api_sw_interface_set_dpdk_hqos_subport_t *mp)
{
...
static void *
vl_api_sw_interface_set_dpdk_hqos_subport_t_print(vl_api_sw_interface_set_dpdk_hqos_subport_t *mp, void *handle)
{
    u8 *s;
    ...
    s = format(s, "subport %u  rate %u  bkt_size %u  tc0 %u tc1 %u tc2 %u tc3 %u period %u", ntohl(mp->subport),
               ntohl(mp->tb_rate), ntohl(mp->tb_size), ntohl(mp->tc_rate[0]), ntohl(mp->tc_rate[1]),
               ntohl(mp->tc_rate[2]), ntohl(mp->tc_rate[3]), ntohl(mp->tc_period));

    FINISH;
}

static void
vl_api_sw_interface_set_dpdk_hqos_tctbl_t_handler(vl_api_sw_interface_set_dpdk_hqos_tctbl_t *mp)
{
...
static void *
vl_api_sw_interface_set_dpdk_hqos_tctbl_t_print(vl_api_sw_interface_set_dpdk_hqos_tctbl_t *mp, void *handle)
{
    u8 *s;
    ...
    s = format(s, "entry %u  tc %u  queue %u", ntohl(mp->entry), ntohl(mp->tc), ntohl(mp->queue));
...
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + dm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,               \
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + dm->msg_id_base);
...
dpdk_api_init(vlib_main_t *vm)
{
    ...
    dm->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
    vec_free(name);
