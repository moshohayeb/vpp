...
static int
openssl_try_handshake_read(openssl_ctx_t *oc, stream_session_t *tls_session)
{
    u32 deq_max, deq_now;
    svm_fifo_t *f;
    int wrote, rv;
...
static int
openssl_try_handshake_write(openssl_ctx_t *oc, stream_session_t *tls_session)
{
    u32 enq_max, deq_now;
    svm_fifo_t *f;
    int read, rv;
...
openssl_ctx_handshake_rx(tls_ctx_t *ctx, stream_session_t *tls_session)
{
    ...
    while (SSL_in_init(oc->ssl)) {
        ...
        if (SSL_get_async_estatus(oc->ssl, &estatus) && (estatus == ENGINE_STATUS_RETRY)) {
...
openssl_start_listen(tls_ctx_t *lctx)
{
    ...
    if (!app->tls_cert || !app->tls_key) {
        TLS_DBG(1, "tls cert and/or key not configured %d", lctx->parent_app_index);
        return -1;
    }
...
#ifdef HAVE_OPENSSL_ASYNC
static clib_error_t *
tls_openssl_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    openssl_main_t *om = &openssl_main;
