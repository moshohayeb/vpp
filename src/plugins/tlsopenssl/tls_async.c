...
struct engine_polling engine_list[] = {{"qat", qat_polling, qat_pre_init, qat_polling_config},
...
openssl_async_node_enable_disable(u8 is_en)
{
    ...
    /* *INDENT-OFF* */
    foreach_vlib_main(({ vlib_node_set_state(this_vlib_main, tls_async_process_node.index, state); }));
    /* *INDENT-ON* */
}
