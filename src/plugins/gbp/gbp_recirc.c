...
gbp_recirc_add(u32 sw_if_index, epg_id_t epg_id, u8 is_ext)
{
    ...
    if (INDEX_INVALID == gri) {
        ...
        if (gr->gr_is_ext) {
             ...
             * based on its port
             */
            gbp_endpoint_update(gr->gr_sw_if_index, NULL, NULL, gr->gr_epg, &gr->gr_ep);
            vnet_feature_enable_disable("ip4-unicast", "ip4-gbp-src-classify", gr->gr_sw_if_index, 1, 0, 0);
            vnet_feature_enable_disable("ip6-unicast", "ip6-gbp-src-classify", gr->gr_sw_if_index, 1, 0, 0);
        } else {
            /*
             ...
             */
            vnet_feature_enable_disable("ip4-unicast", "ip4-gbp-lpm-classify", gr->gr_sw_if_index, 1, 0, 0);
            vnet_feature_enable_disable("ip6-unicast", "ip6-gbp-lpm-classify", gr->gr_sw_if_index, 1, 0, 0);
        }
...
gbp_recirc_delete(u32 sw_if_index)
{
    ...
    if (INDEX_INVALID != gri) {
        ...
        if (gr->gr_is_ext) {
            ...
            vnet_feature_enable_disable("ip4-unicast", "ip4-gbp-src-classify", gr->gr_sw_if_index, 0, 0, 0);
            vnet_feature_enable_disable("ip6-unicast", "ip6-gbp-src-classify", gr->gr_sw_if_index, 0, 0, 0);
        } else {
            vnet_feature_enable_disable("ip4-unicast", "ip4-gbp-lpm-classify", gr->gr_sw_if_index, 0, 0, 0);
            vnet_feature_enable_disable("ip6-unicast", "ip6-gbp-lpm-classify", gr->gr_sw_if_index, 0, 0, 0);
        }
...
gbp_recirc_show_one(gbp_recirc_t *gr, void *ctx)
{
    ...
    vm = ctx;
    vlib_cli_output(vm, "  %U, epg:%d, ext:%d", format_vnet_sw_if_index_name, vnm, gr->gr_sw_if_index, gr->gr_epg,
                    gr->gr_is_ext);
...
static clib_error_t *
gbp_recirc_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
