 ...
 */
always_inline uword
gbp_classify_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, gbp_src_classify_type_t type,
                    u8 is_l3)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (GBP_SRC_CLASSIFY_NULL == type) {
                ...
                if (is_l3) {
                    ...
                    next0 = vnet_l2_feature_next(b0, gscm->l2_input_feat_next[type], L2INPUT_FEAT_GBP_SRC_CLASSIFY);
                }
            }
...
static uword
gbp_src_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (gbp_classify_inline(vm, node, frame, GBP_SRC_CLASSIFY_PORT, 0));
}

static uword
gbp_null_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (gbp_classify_inline(vm, node, frame, GBP_SRC_CLASSIFY_NULL, 0));
}

static uword
gbp_ip4_src_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (gbp_classify_inline(vm, node, frame, 0, 1));
}

static uword
gbp_ip6_src_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (gbp_classify_inline(vm, node, frame, 0, 1));
}
...
gbp_src_classify_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, gbp_src_classify_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(),
                                em->l2_input_feat_next[GBP_SRC_CLASSIFY_NULL]);
    feat_bitmap_init_next_nodes(vm, gbp_null_classify_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(),
