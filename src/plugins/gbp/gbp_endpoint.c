...
static void
gbp_endpoint_mk_key_mac_itf(const mac_address_t *mac, u32 sw_if_index, clib_bihash_kv_16_8_t *key)
{
...
static void
gbp_endpoint_extract_key_mac_itf(const clib_bihash_kv_16_8_t *key, mac_address_t *mac, u32 *sw_if_index)
{
...
gbp_endpoint_find_mac_itf(const mac_address_t *mac, u32 sw_if_index)
{
    ...
    rv = clib_bihash_search_16_8(&gbp_ep_by_mac_itf_db.gte_table, &key, &value);
...
static void
gbp_endpoint_mk_key_ip_itf(const ip46_address_t *ip, u32 sw_if_index, clib_bihash_kv_24_8_t *key)
{
...
static void
gbp_endpoint_extract_key_ip_itf(const clib_bihash_kv_24_8_t *key, ip46_address_t *ip, u32 *sw_if_index)
{
...
static bool
gbp_endpoint_add_mac_itf(const mac_address_t *mac, u32 sw_if_index, index_t gbpei)
{
...
static bool
gbp_endpoint_add_ip_itf(const ip46_address_t *ip, u32 sw_if_index, index_t gbpei)
{
...
{
    vec_validate_init_empty(gbp_ep_by_itf_db.gte_vec, sw_if_index, INDEX_INVALID);

    if (INDEX_INVALID == gbp_ep_by_itf_db.gte_vec[sw_if_index]) {
        l2input_intf_bitmap_enable(sw_if_index, L2INPUT_FEAT_GBP_SRC_CLASSIFY, 1);
        l2input_intf_bitmap_enable(sw_if_index, L2INPUT_FEAT_GBP_FWD, 1);
...
int
gbp_endpoint_update(u32 sw_if_index, const ip46_address_t *ips, const mac_address_t *mac, epg_id_t epg_id, u32 *handle)
{
        ...
        if (NULL != ips) {
            ...
            vec_foreach_index(ii, ips)
            {
                ...
                if (ip46_address_is_ip4(&ips[ii]))
                    send_ip4_garp_w_addr(vlib_get_main(), &ips[ii].ip4, gepg->gepg_uplink_sw_if_index);
                else
                    send_ip6_na_w_addr(vlib_get_main(), &ips[ii].ip6, gepg->gepg_uplink_sw_if_index);
            }
        }
    } else {
        /*
...
static clib_error_t *
gbp_endpoint_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
format_gbp_endpoint(u8 *s, va_list *args)
{
    ...
    s = format(s, "[@%d] ", gbpei);
    s = format(s, "%U", format_vnet_sw_if_index_name, vnm, gbpe->ge_sw_if_index);
    s = format(s, ", IPs:[");
...
gbp_endpoint_walk_ip_itf(const clib_bihash_kv_24_8_t *kvp, void *arg)
{
    ...
    vlib_cli_output(vm, " {%U, %U} -> %d", format_ip46_address, &ip, IP46_TYPE_ANY, format_vnet_sw_if_index_name,
...
gbp_endpoint_walk_mac_itf(const clib_bihash_kv_16_8_t *kvp, void *arg)
{
    ...
    vlib_cli_output(vm, " {%U, %U} -> %d", format_mac_address_t, &mac, format_vnet_sw_if_index_name, vnet_get_main(),
                    sw_if_index, kvp->value);
}

static clib_error_t *
gbp_endpoint_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (INDEX_INVALID != handle) {
        ...
        vlib_cli_output(vm, "\nDatabases:");
        clib_bihash_foreach_key_value_pair_24_8(&gbp_ep_by_ip_itf_db.gte_table, gbp_endpoint_walk_ip_itf, vm);
        clib_bihash_foreach_key_value_pair_16_8(&gbp_ep_by_mac_itf_db.gte_table, gbp_endpoint_walk_mac_itf, vm);

        vec_foreach_index(sw_if_index, gbp_ep_by_itf_db.gte_vec)
        {
            if (INDEX_INVALID != gbp_ep_by_itf_db.gte_vec[sw_if_index])
                vlib_cli_output(vm, " {%U} -> %d", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index,
...
{
    clib_bihash_init_24_8(&gbp_ep_by_ip_itf_db.gte_table, "GBP Endpoints - IP/Interface", GBP_EP_HASH_NUM_BUCKETS,
                          GBP_EP_HASH_MEMORY_SIZE);

    clib_bihash_init_16_8(&gbp_ep_by_mac_itf_db.gte_table, "GBP Endpoints - MAC/Interface", GBP_EP_HASH_NUM_BUCKETS,
                          GBP_EP_HASH_MEMORY_SIZE);
