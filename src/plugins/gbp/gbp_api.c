...
vl_api_gbp_endpoint_add_t_handler(vl_api_gbp_endpoint_add_t *mp)
{
    ...
    rv = gbp_endpoint_update(sw_if_index, ips, &mac, ntohs(mp->endpoint.epg_id), &handle);
    ...
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_GBP_ENDPOINT_ADD_REPLY + GBP_MSG_BASE, ({ rmp->handle = htonl(handle); }));
    /* *INDENT-ON* */
}
...
gbp_endpoint_send_details(gbp_endpoint_t *gbpe, void *args)
{
    ...
    {
        ip_address_encode(&gbpe->ge_ips[ii], IP46_TYPE_ANY, &mp->endpoint.ips[ii]);
    }
...
static void
vl_api_gbp_endpoint_group_add_del_t_handler(vl_api_gbp_endpoint_group_add_del_t *mp)
{
    ...
    if (mp->is_add) {
        rv = gbp_endpoint_group_add(ntohs(mp->epg.epg_id), ntohl(mp->epg.bd_id), ntohl(mp->epg.ip4_table_id),
                                    ntohl(mp->epg.ip6_table_id), uplink_sw_if_index);
    } else {
...
vl_api_gbp_subnet_add_del_t_handler(vl_api_gbp_subnet_add_del_t *mp)
{
    ...
    rv = gbp_subnet_add_del(ntohl(mp->subnet.table_id), &pfx, ntohl(mp->subnet.sw_if_index), ntohs(mp->subnet.epg_id),
...
static int
gbp_subnet_send_details(u32 table_id, const fib_prefix_t *pfx, u32 sw_if_index, epg_id_t epg, u8 is_internal, void *args)
{
...
vl_api_gbp_recirc_add_del_t_handler(vl_api_gbp_recirc_add_del_t *mp)
{
    ...
    if (mp->is_add)
        gbp_recirc_add(sw_if_index, ntohs(mp->recirc.epg_id), mp->recirc.is_ext);
    else
...
vl_api_gbp_contract_add_del_t_handler(vl_api_gbp_contract_add_del_t *mp)
{
    ...
    if (mp->is_add)
        gbp_contract_update(ntohs(mp->contract.src_epg), ntohs(mp->contract.dst_epg), ntohl(mp->contract.acl_index));
    else
        gbp_contract_delete(ntohs(mp->contract.src_epg), ntohs(mp->contract.dst_epg));
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + GBP_MSG_BASE);
...
{
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers(VL_API_##N + GBP_MSG_BASE, #n, vl_api_##n##_t_handler, vl_noop_handler,                    \
...
gbp_init(vlib_main_t *vm)
{
    ...
    msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
    gbp_api_hookup(vm);
