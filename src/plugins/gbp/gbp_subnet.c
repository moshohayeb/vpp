...
gbp_internal_subnet_add(u32 fib_index, const fib_prefix_t *pfx)
{
    ...
    fib_table_entry_special_dpo_update(fib_index, pfx, FIB_SOURCE_PLUGIN_HI, FIB_ENTRY_FLAG_EXCLUSIVE, &gfd);
...
static int
gbp_external_subnet_add(u32 fib_index, const fib_prefix_t *pfx, u32 sw_if_index, epg_id_t epg)
{
    dpo_id_t gpd = DPO_INVALID;

    gbp_policy_dpo_add_or_lock(fib_proto_to_dpo(pfx->fp_proto), epg, sw_if_index, &gpd);

    fib_table_entry_special_dpo_update(fib_index, pfx, FIB_SOURCE_PLUGIN_HI,
...
int
gbp_subnet_add_del(u32 table_id, const fib_prefix_t *pfx, u32 sw_if_index, epg_id_t epg, u8 is_add, u8 is_internal)
{
    u32 fib_index;
...
gbp_subnet_fib_table_walk(fib_node_index_t fei, void *arg)
{
    ...
    table_id = fib_table_get_table_id(fib_entry_get_fib_index(fei), pfx->fp_proto);
    ...
    if (DPO_LOAD_BALANCE == dpo->dpoi_type) {
        ...
        if (dpo->dpoi_type == gbp_policy_dpo_get_type()) {
            ...
            /* *INDENT-OFF* */
            ctx->cb(table_id, pfx, gpd->gpd_sw_if_index, gpd->gpd_epg,
                    0,   // is_internal
                    ctx->ctx);
            /* *INDENT-ON* */
...
gbp_subnet_walk(gbp_subnet_cb_t cb, void *ctx)
{
    ...
    pool_foreach(fib_table, ip4_main.fibs,
                 ({ fib_table_walk(fib_table->ft_index, FIB_PROTOCOL_IP4, gbp_subnet_fib_table_walk, &wctx); }));
    pool_foreach(fib_table, ip6_main.fibs,
                 ({ fib_table_walk(fib_table->ft_index, FIB_PROTOCOL_IP6, gbp_subnet_fib_table_walk, &wctx); }));
    /* *INDENT-ON* */
}
