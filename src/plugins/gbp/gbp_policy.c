...
static uword
gbp_policy(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (EPG_INVALID != key0.gck_src) {
                if (PREDICT_FALSE(key0.gck_src == key0.gck_dst)) {
                     ...
                     * intra-epg allowed
                     */
                    next0 = vnet_l2_feature_next(b0, gpm->l2_output_feat_next, L2OUTPUT_FEAT_GBP_POLICY);
                } else {
                    ...
                    if (~0 != value0.gc_lc_index) {
                         ...
                         */
                        acl_plugin_fill_5tuple_inline(gm->acl_plugin.p_acl_main, value0.gc_lc_index, b0, is_ip60,
                                                      /* is_input */ 0,
                                                      /* is_l2_path */ 1, &pkt_5tuple0);
                        acl_plugin_match_5tuple_inline(gm->acl_plugin.p_acl_main, value0.gc_lc_index, &pkt_5tuple0,
                                                       is_ip60, &action0, &acl_pos_p0, &acl_match_p0, &rule_match_p0,
                                                       &trace_bitmap0);
            ...
            if (PREDICT_FALSE((b0->flags & VLIB_BUFFER_IS_TRACED))) {
                gbp_policy_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
...
format_gbp_policy_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "src:%d, dst:%d, acl:%d", t->src_epg, t->dst_epg, t->acl_index);
...
gbp_policy_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, gbp_policy_node.index, L2OUTPUT_N_FEAT, l2output_get_feat_names(),
