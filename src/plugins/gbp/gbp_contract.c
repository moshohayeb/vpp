...
gbp_contract_update(epg_id_t src_epg, epg_id_t dst_epg, u32 acl_index)
{
    ...
    if (~0 == gm->gbp_acl_user_id) {
        ...
        gm->gbp_acl_user_id = gm->acl_plugin.register_user_module("GBP ACL", "src-epg", "dst-epg");
    }
    ...
    if (p != NULL) {
        ...
        value.gc_lc_index  = gm->acl_plugin.get_lookup_context_index(gm->gbp_acl_user_id, src_epg, dst_epg);
...
static clib_error_t *
gbp_contract_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
gbp_contract_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    gbp_contract_key_t key;
    gbp_contract_value_t value;
    ...
    /* *INDENT-OFF* */
    hash_foreach(key.as_u32, value.as_u64, gbp_contract_db.gc_hash,
                 { vlib_cli_output(vm, "  {%d,%d} -> %d", key.gck_src, key.gck_dst, value.gc_acl_index); });
    /* *INDENT-ON* */
