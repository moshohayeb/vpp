...
int
gbp_endpoint_group_add(epg_id_t epg_id, u32 bd_id, u32 ip4_table_id, u32 ip6_table_id, u32 uplink_sw_if_index)
{
    ...
    if (NULL == gepg) {
        ...
        FOR_EACH_FIB_IP_PROTOCOL(fproto)
        {
            gepg->gepg_fib_index[fproto] =
                fib_table_find_or_create_and_lock(fproto, gepg->gepg_rd[fproto], FIB_SOURCE_PLUGIN_HI);

            if (~0 == gepg->gepg_fib_index[fproto]) {
                return (VNET_API_ERROR_NO_SUCH_FIB);
            }

            dvr_dpo_add_or_lock(uplink_sw_if_index, fib_proto_to_dpo(fproto), &gepg->gepg_dpo[fproto]);
        }

        /*
         * packets direct from the uplink have had policy applied
         */
        l2input_intf_bitmap_enable(gepg->gepg_uplink_sw_if_index, L2INPUT_FEAT_GBP_NULL_CLASSIFY, 1);

        hash_set(gbp_endpoint_group_db.gepg_hash, gepg->gepg_id, gepg - gbp_endpoint_group_pool);
    }
...
gbp_endpoint_group_delete(epg_id_t epg_id)
{
    ...
    if (NULL != p) {
        ...
        l2input_intf_bitmap_enable(gepg->gepg_uplink_sw_if_index, L2INPUT_FEAT_GBP_NULL_CLASSIFY, 0);

        FOR_EACH_FIB_IP_PROTOCOL(fproto)
        {
            dpo_reset(&gepg->gepg_dpo[fproto]);
            fib_table_unlock(gepg->gepg_fib_index[fproto], fproto, FIB_SOURCE_PLUGIN_HI);
        }
...
static clib_error_t *
gbp_endpoint_group_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "%U", unformat_vnet_sw_interface, vnm, &uplink_sw_if_index))
            ;
    ...
    if (add) {
        ...
        gbp_endpoint_group_add(epg_id, bd_id, rd_id, rd_id, uplink_sw_if_index);
    } else
...
gbp_endpoint_group_show_one(gbp_endpoint_group_t *gepg, void *ctx)
{
    ...
    vm = ctx;
    vlib_cli_output(vm, "  %d, bd:%d, ip4:%d ip6:%d uplink:%U", gepg->gepg_id, gepg->gepg_bd,
                    gepg->gepg_rd[FIB_PROTOCOL_IP4], gepg->gepg_rd[FIB_PROTOCOL_IP6], format_vnet_sw_if_index_name, vnm,
...
static clib_error_t *
gbp_endpoint_group_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
