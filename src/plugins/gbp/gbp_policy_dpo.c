...
void
gbp_policy_dpo_add_or_lock(dpo_proto_t dproto, epg_id_t epg, u32 sw_if_index, dpo_id_t *dpo)
{
    gbp_policy_dpo_t *gpd;
...
format_gbp_policy_dpo(u8 *s, va_list *ap)
{
    ...
    s = format(s, "gbp-policy-dpo: %U, epg:%d out:%U", format_dpo_proto, gpd->gpd_proto, gpd->gpd_epg,
...
{
    gbp_policy_dpo_type = dpo_register_new_type(&gbp_policy_dpo_vft, gbp_policy_dpo_nodes);
...
always_inline uword
gbp_policy_dpo_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u8 is_ip6)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            gpd0                                   = gbp_policy_dpo_get_i(vnet_buffer(b0)->ip.adj_index[VLIB_TX]);
            ...
            if (EPG_INVALID != key0.gck_src) {
                if (PREDICT_FALSE(key0.gck_src == key0.gck_dst)) {
                    ...
                    if (~0 != value0.gc_lc_index) {
                         ...
                         */
                        acl_plugin_fill_5tuple_inline(gm->acl_plugin.p_acl_main, value0.gc_lc_index, b0, is_ip6,
                                                      /* is_input */ 1,
                                                      /* is_l2_path */ 0, &pkt_5tuple0);
                        acl_plugin_match_5tuple_inline(gm->acl_plugin.p_acl_main, value0.gc_lc_index, &pkt_5tuple0,
                                                       is_ip6, &action0, &acl_pos_p0, &acl_match_p0, &rule_match_p0,
                                                       &trace_bitmap0);
...
format_gbp_policy_dpo_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, " src-epg:%d dst-epg:%d acl-index:%d", t->src_epg, t->dst_epg, t->acl_index);

    return s;
}

static uword
ip4_gbp_policy_dpo(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (gbp_policy_dpo_inline(vm, node, from_frame, 0));
}

static uword
ip6_gbp_policy_dpo(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (gbp_policy_dpo_inline(vm, node, from_frame, 1));
}

/* *INDENT-OFF* */
VLIB_REGISTER_NODE(ip4_gbp_policy_dpo_node) = {.function     = ip4_gbp_policy_dpo,
...
VLIB_REGISTER_NODE(ip6_gbp_policy_dpo_node) = {.function     = ip6_gbp_policy_dpo,
 ...
 */
always_inline uword
gbp_lpm_classify_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, fib_protocol_t fproto)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (FIB_PROTOCOL_IP4 == fproto) {
                ...
                lbi0  = ip4_fib_forwarding_lookup(fib_index0, &ip4_0->src_address);
            } else {
                ip6_0 = vlib_buffer_get_current(b0);
                lbi0  = ip6_fib_table_fwding_lookup(&ip6_main, fib_index0, &ip6_0->src_address);
            }
            ...
            if (PREDICT_FALSE((b0->flags & VLIB_BUFFER_IS_TRACED))) {
                gbp_classify_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->src_epg              = src_epg0;
            }
...
static uword
gbp_ip4_lpm_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (gbp_lpm_classify_inline(vm, node, frame, FIB_PROTOCOL_IP4));
}

static uword
gbp_ip6_lpm_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
