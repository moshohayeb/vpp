...
gbp_fwd_dpo_module_init(vlib_main_t *vm)
{
    ...
    gbp_fwd_dpo_type = dpo_register_new_type(&gbp_fwd_dpo_vft, gbp_fwd_dpo_nodes);
...
always_inline uword
gbp_fwd_dpo_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, fib_protocol_t fproto)
{
...
static uword
ip4_gbp_fwd_dpo(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (gbp_fwd_dpo_inline(vm, node, from_frame, FIB_PROTOCOL_IP4));
}

static uword
ip6_gbp_fwd_dpo(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (gbp_fwd_dpo_inline(vm, node, from_frame, FIB_PROTOCOL_IP6));
}

/* *INDENT-OFF* */
VLIB_REGISTER_NODE(ip4_gbp_fwd_dpo_node) = {.function     = ip4_gbp_fwd_dpo,
...
VLIB_REGISTER_NODE(ip6_gbp_fwd_dpo_node) = {.function     = ip6_gbp_fwd_dpo,
