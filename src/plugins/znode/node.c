...
format_znode_trace(u8 *s, va_list *args)
{
    ...
    znode_trace_t *t = va_arg(*args, znode_trace_t *);

    s = format(s, "ZNODE: sw_if_index %d, next index %d\n",
            t->sw_if_index, t->next_index);
    s = format(s, "  new src %U -> new dst %U",
            znode_format_mac_address, t->new_src_mac,
            znode_format_mac_address, t->new_dst_mac);
    return s;
}
...
#define foreach_znode_error \
    _(SWAPPED, "Mac swap packets processed")

typedef enum {
#define _(sym, str) ZNODE_ERROR_ ## sym,
    foreach_znode_error
#undef _
    ZNODE_N_ERROR,
} znode_error_t;
...
VLIB_NODE_FN(znode_node) (vlib_main_t * vm,
...
#undef _
    ...
    vlib_node_increment_counter(vm, znode_node.index,
            ZNODE_ERROR_SWAPPED, pkts_swapped);
    return frame->n_vectors;
}

/* *INDENT-OFF* */
#ifndef CLIB_MARCH_VARIANT
VLIB_REGISTER_NODE (znode_node) = 
{
  .name = "znode",
  ...
  .n_errors = ARRAY_LEN(znode_error_strings),
