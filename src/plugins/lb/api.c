...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + lbm->msg_id_base);
...
static void
vl_api_lb_conf_t_handler(vl_api_lb_conf_t *mp)
{
    ...
    rv = lb_conf((ip4_address_t *) &mp->ip4_src_address, (ip6_address_t *) mp->ip6_src_address,
                 mp->sticky_buckets_per_core, mp->flow_timeout);

    REPLY_MACRO(VL_API_LB_CONF_REPLY);
}

static void *
vl_api_lb_conf_t_print(vl_api_lb_conf_t *mp, void *handle)
{
    u8 *s;
...
static void
vl_api_lb_add_del_vip_t_handler(vl_api_lb_add_del_vip_t *mp)
{
    lb_main_t *lbm = &lb_main;
    vl_api_lb_conf_reply_t *rmp;
    int rv = 0;
    ...
    if ((mp->protocol != IP_PROTOCOL_TCP) && (mp->protocol != IP_PROTOCOL_UDP)) {
        mp->protocol = ~0;
        mp->port     = 0;
    }
    ...
    if (mp->is_del) {
        u32 vip_index;
        if (!(rv = lb_vip_find_index(&(args.prefix), mp->prefix_length, mp->protocol, mp->port, &vip_index)))
        ...
        } else if ((mp->encap == LB_ENCAP_TYPE_NAT4) || (mp->encap == LB_ENCAP_TYPE_NAT6)) {
...
static void *
vl_api_lb_add_del_vip_t_print(vl_api_lb_add_del_vip_t *mp, void *handle)
{
    u8 *s;
    s = format(0, "SCRIPT: lb_add_del_vip ");
    s = format(s, "%U ", format_ip46_prefix, (ip46_address_t *) mp->ip_prefix, mp->prefix_length, IP46_TYPE_ANY);

    s = format(s, "%s ",
               (mp->encap == LB_ENCAP_TYPE_GRE4) ?
                   "gre4" :
                   (mp->encap == LB_ENCAP_TYPE_GRE6) ?
                   "gre6" :
                   (mp->encap == LB_ENCAP_TYPE_NAT4) ? "nat4" : (mp->encap == LB_ENCAP_TYPE_NAT6) ? "nat6" : "l3dsr");

    if (mp->encap == LB_ENCAP_TYPE_L3DSR) {
        s = format(s, "dscp %u ", mp->dscp);
    }

    if ((mp->encap == LB_ENCAP_TYPE_NAT4) || (mp->encap == LB_ENCAP_TYPE_NAT6)) {
...
static void
vl_api_lb_add_del_as_t_handler(vl_api_lb_add_del_as_t *mp)
{
    lb_main_t *lbm = &lb_main;
    vl_api_lb_conf_reply_t *rmp;
    int rv = 0;
    ...
    memcpy(&vip_ip_prefix.ip6, mp->vip_ip_prefix, sizeof(vip_ip_prefix.ip6));

    ip46_address_t as_address;

    memcpy(&as_address.ip6, mp->as_address, sizeof(as_address.ip6));

    if ((rv = lb_vip_find_index(&vip_ip_prefix, mp->vip_prefix_length, mp->protocol, mp->port, &vip_index)))
        goto done;
...
static void *
vl_api_lb_add_del_as_t_print(vl_api_lb_add_del_as_t *mp, void *handle)
{
    u8 *s;
    s = format(0, "SCRIPT: lb_add_del_as ");
    s = format(s, "%U ", format_ip46_prefix, (ip46_address_t *) mp->vip_ip_prefix, mp->vip_prefix_length, IP46_TYPE_ANY);
    s = format(s, "%U ", format_ip46_address, (ip46_address_t *) mp->as_address, IP46_TYPE_ANY);
    s = format(s, "%s ", mp->is_del ? "del" : "add");
    FINISH;
}

static void
vl_api_lb_flush_vip_t_handler(vl_api_lb_flush_vip_t *mp)
{
    lb_main_t *lbm = &lb_main;
    int rv         = 0;
    ...
    rv = lb_vip_find_index(&vip_prefix, vip_plen, mp->protocol, (u16) mp->port, &vip_index);
...
static void *
vl_api_lb_flush_vip_t_print(vl_api_lb_flush_vip_t *mp, void *handle)
{
    u8 *s;
    s = format(0, "SCRIPT: lb_add_del_vip ");
    s = format(s, "%U ", format_ip46_prefix, (ip46_address_t *) mp->ip_prefix, mp->prefix_length, IP46_TYPE_ANY);
...
static clib_error_t *
lb_api_init(vlib_main_t *vm)
{
    lb_main_t *lbm   = &lb_main;
    u8 *name         = format(0, "lb_%08x%c", api_version, 0);
    lbm->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);

#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + lbm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,              \
