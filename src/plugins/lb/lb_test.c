...
api_lb_conf(vat_main_t *vam)
{
    ...
    if (!unformat(i, "%U %U %u %u", unformat_ip4_address, &mps.ip4_src_address, unformat_ip6_address,
                  mps.ip6_src_address, &mps.sticky_buckets_per_core, &mps.flow_timeout)) {
...
api_lb_add_del_vip(vat_main_t *vam)
{
    ...
    if (!unformat(i, "%U", unformat_ip46_prefix, mps.ip_prefix, &mps.prefix_length, IP46_TYPE_ANY)) {
...
api_lb_add_del_as(vat_main_t *vam)
{
    ...
    if (!unformat(i, "%U %U", unformat_ip46_prefix, mps.vip_ip_prefix, &mps.vip_prefix_length, IP46_TYPE_ANY,
...
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + lbtm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,             \
