...
#define lb_get_writer_lock()                                                                                           \
    do {                                                                                                               \
    } while (__sync_lock_test_and_set(lb_main.writer_lock, 1))
...
static char *lb_vip_type_strings[] = {
    [LB_VIP_TYPE_IP6_GRE6] = "ip6-gre6", [LB_VIP_TYPE_IP6_GRE4] = "ip6-gre4",   [LB_VIP_TYPE_IP4_GRE6] = "ip4-gre6",
    [LB_VIP_TYPE_IP4_GRE4] = "ip4-gre4", [LB_VIP_TYPE_IP4_L3DSR] = "ip4-l3dsr", [LB_VIP_TYPE_IP4_NAT4] = "ip4-nat4",
...
format_lb_vip(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U %U new_size:%u #as:%u%s", format_lb_vip_type, vip->type, format_ip46_prefix, &vip->prefix,
               vip->plen, IP46_TYPE_ANY, vip->new_flow_table_mask + 1, pool_elts(vip->as_indexes),
    ...
    if (vip->type == LB_VIP_TYPE_IP4_L3DSR) {
        ...
        s = format(s, " type:%s port:%u target_port:%u",
                   (vip->encap_args.srv_type == LB_SRV_TYPE_CLUSTERIP) ? "clusterip" : "nodeport", ntohs(vip->port),
...
format_lb_vip_detailed(u8 *s, va_list *args)
{
    ...
    s = format(s,
               "%U %U [%lu] %U%s\n"
               "%U  new_size:%u\n",
               format_white_space, indent, format_lb_vip_type, vip->type, vip - lbm->vips, format_ip46_prefix,
               &vip->prefix, (u32) vip->plen, IP46_TYPE_ANY, (vip->flags & LB_VIP_FLAGS_USED) ? "" : " removed",
    ...
    if (vip->type == LB_VIP_TYPE_IP4_L3DSR) {
        ...
        s = format(s, "%U  type:%s port:%u target_port:%u", format_white_space, indent,
                   (vip->encap_args.srv_type == LB_SRV_TYPE_CLUSTERIP) ? "clusterip" : "nodeport", ntohs(vip->port),
                   ntohs(vip->encap_args.target_port));
    }
    ...
    for (i = 0; i < LB_N_VIP_COUNTERS; i++)
        s = format(s, "%U    %s: %Lu\n", format_white_space, indent, lbm->vip_counters[i].name,
    ...
    vec_foreach(nfe, vip->new_flow_table) count[nfe->as_index]++;
    ...
    pool_foreach(as_index, vip->as_indexes, {
        ...
        s  = format(s, "%U    %U %u buckets   %Lu flows  dpo:%u %s\n", format_white_space, indent, format_ip46_address,
                   &as->address, IP46_TYPE_ANY, count[as - lbm->ass], vlib_refcount_get(&lbm->as_refcount, as - lbm->ass),
...
int
lb_conf(ip4_address_t *ip4_address, ip6_address_t *ip6_address, u32 per_cpu_sticky_buckets, u32 flow_timeout)
{
    lb_main_t *lbm = &lb_main;
...
static int
lb_vip_port_find_index(ip46_address_t *prefix, u8 plen, u8 protocol, u16 port, lb_lkp_type_t lkp_type, u32 *vip_index)
{
    lb_main_t *lbm = &lb_main;
    lb_vip_t *vip;
...
static int
lb_vip_port_find_index_with_lock(ip46_address_t *prefix, u8 plen, u8 protocol, u16 port, u32 *vip_index)
{
...
static int
lb_vip_port_find_diff_port(ip46_address_t *prefix, u8 plen, u8 protocol, u16 port, u32 *vip_index)
{
    return lb_vip_port_find_index(prefix, plen, protocol, port, LB_LKP_DIFF_IP_PORT, vip_index);
}

int
lb_vip_find_index(ip46_address_t *prefix, u8 plen, u8 protocol, u16 port, u32 *vip_index)
{
    int ret;
    lb_get_writer_lock();
...
lb_as_find_index_vip(lb_vip_t *vip, ip46_address_t *address, u32 *as_index)
{
    ...
    pool_foreach(asi, vip->as_indexes, {
        as = &lbm->ass[*asi];
        if (as->vip_index == (vip - lbm->vips) && as->address.as_u64[0] == address->as_u64[0] &&
    ...
    while (n--) {
        ...
        if (n) {
            ...
            while (n2--)   // Check for duplicates
                if (addresses[n2].as_u64[0] == addresses[n].as_u64[0] && addresses[n2].as_u64[1] == addresses[n].as_u64[1])
                    goto next;
        }
...
lb_flush_vip_as(u32 vip_index, u32 as_index)
{
    ...
    for (thread_index = 0; thread_index < tm->n_vlib_mains; thread_index++) {
        ...
        if (h != NULL) {
            ...
            {
                if ((vip_index == ~0) || ((b->vip[i] == vip_index) && (as_index == ~0)) ||
...
int
lb_vip_del_ass_withlock(u32 vip_index, ip46_address_t *addresses, u32 n, u8 flush)
{
    lb_main_t *lbm = &lb_main;
    u32 now        = (u32) vlib_time_now(vlib_get_main());
    u32 *ip        = 0;
...
lb_vip_add_adjacency(lb_main_t *lbm, lb_vip_t *vip, u32 *vip_prefix_index)
{
    ...
    fib_table_entry_special_dpo_add(0, &pfx, FIB_SOURCE_PLUGIN_HI, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
    dpo_reset(&dpo);
}
 ...
 */
static int
lb_vip_add_port_filter(lb_main_t *lbm, lb_vip_t *vip, u32 vip_prefix_index, u32 vip_idx)
{
    vip_port_key_t key;
...
lb_as_stack(lb_as_t *as)
{
    ...
    dpo_stack(dpo_type, lb_vip_is_ip4(vip->type) ? DPO_PROTO_IP4 : DPO_PROTO_IP6, &as->dpo,
...
lb_init(vlib_main_t *vm)
{
    ...
    clib_bihash_init_8_8(&lbm->vip_index_per_port, "vip_index_per_port", LB_VIP_PER_PORT_BUCKETS,
