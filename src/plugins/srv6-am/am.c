...
format_srv6_am_localsid(u8 *s, va_list *args)
{
    ...
    return (format(s,
                   ...
                   "\tOutgoing iface: %U\n"
                   "\tIncoming iface: %U",
                   format_ip6_address, &ls_mem->nh_addr.ip6, format_vnet_sw_if_index_name, vnm, ls_mem->sw_if_index_out,
...
unformat_srv6_am_localsid(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "end.am nh %U oif %U iif %U", unformat_ip6_address, &nh_addr.ip6, unformat_vnet_sw_interface,
                 vnm, &sw_if_index_out, unformat_vnet_sw_interface, vnm, &sw_if_index_in)) {
...
srv6_am_init(vlib_main_t *vm)
{
    ...
    rv = sr_localsid_register_function(vm, function_name, keyword_str, def_str, params_str, &sm->srv6_am_dpo_type,
                                       format_srv6_am_localsid, unformat_srv6_am_localsid, srv6_am_localsid_creation_fn,
