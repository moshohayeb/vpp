...
format_ila_entry(u8 *s, va_list *va)
{
    ...
    if (!e) {
        return format(s, "%-15s%=40s%=40s%+16s%+18s%+11s", "Type", "SIR Address", "ILA Address", "Checksum Mode",
                      "Direction", "Next DPO");
    } else if (vnm) {
        if (ip6_address_is_zero(&e->next_hop)) {
            return format(s, "%-15U%=40U%=40U%18U%11U%s", format_ila_type, e->type, format_ip6_address, &e->sir_address,
                          format_ip6_address, &e->ila_address, format_csum_mode, e->csum_mode, format_ila_direction,
                          e->dir, "n/a");
        } else {
            return format(s, "%-15U%=40U%=40U%18U%11U%U", format_ila_type, e->type, format_ip6_address, &e->sir_address,
                          format_ip6_address, &e->ila_address, format_csum_mode, e->csum_mode, format_ila_direction,
                          e->dir, format_dpo_id, &e->ila_dpo, 0);
        }
    }
...
ila_ila2sir(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            vlib_validate_buffer_enqueue_x2(vm, node, next_index, to_next, n_left_to_next, pi0, pi1,
...
ila_add_del_entry(ila_add_del_entry_args_t *args)
{
    ...
    if (args->type == ILA_TYPE_IID || args->type == ILA_TYPE_LUID) {
        ...
        if (args->sir_address.as_u16[1] || args->sir_address.as_u16[2] || args->sir_address.as_u16[3] ||
            args->sir_address.as_u16[4] || args->sir_address.as_u16[5] || (args->sir_address.as_u8[12] & 0xf0)) {
            clib_warning("SIR multicast address must start with fff");
            return -1;
        }
    }
    ...
    if (!args->is_del) {
        ...
        if (!ip6_address_is_zero(&e->next_hop)) {
            ...
            e->next_hop_child_index =
                fib_entry_child_add(e->next_hop_fib_entry_index, ila_fib_node_type, e - ilm->entries);
...
ila_init(vlib_main_t *vm)
{
    ...
    BV(clib_bihash_init)
    (&ilm->id_to_entry_table, "ila id to entry index table", ilm->lookup_table_nbuckets, ilm->lookup_table_size);
...
static clib_error_t *
ila_entry_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
ila_interface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
ila_show_entries_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm = vnet_get_main();
    ila_main_t *ilm  = &ila_main;
    ila_entry_t *e;
    ...
    pool_foreach(e, ilm->entries, ({ vlib_cli_output(vm, "  %U\n", format_ila_entry, vnm, e); }));
