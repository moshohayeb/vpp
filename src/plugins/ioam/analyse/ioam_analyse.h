...
ip6_ioam_analyse_calc_delay(ioam_trace_hdr_t *trace, u16 trace_len, u8 oneway)
{
    ...
    if (oneway && (trace->ioam_trace_type & BIT_TTL_NODEID)) {
        ...
        do {
            ...
            if ((clib_net_to_host_u32(*start_elt) >> 24) <= (clib_net_to_host_u32(*uturn_elt) >> 24))
                done = 1;
...
always_inline void
ip6_ioam_analyse_hbh_trace_loopback(ioam_analyser_data_t *data, ioam_trace_hdr_t *trace, u16 trace_len)
{
    ioam_analyse_trace_data *trace_data;
    ioam_analyse_trace_record *trace_record;
    ioam_path_map_t *path;
...
always_inline int
ip6_ioam_analyse_hbh_trace(ioam_analyser_data_t *data, ioam_trace_hdr_t *trace, u16 pak_len, u16 trace_len)
{
    ioam_analyse_trace_data *trace_data;
    u16 size_of_traceopt_per_node;
    u16 size_of_all_traceopts;
...
always_inline int
ip6_ioam_analyse_hbh_e2e(ioam_analyser_data_t *data, ioam_e2e_packet_t *e2e, u16 len)
{
    while (__sync_lock_test_and_set(data->writer_lock, 1))
        ;
...
print_analyse_flow(u8 *s, ioam_analyser_data_t *record)
{
    ...
    s = format(s,
               ...
               record->seqno_data.rx_packets, record->seqno_data.lost_packets, record->seqno_data.dup_packets,
...
ioam_analyse_init_data(ioam_analyser_data_t *data)
{
    ...
    data->writer_lock    = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
    *(data->writer_lock) = 0;
