...
always_inline u8
ioam_analyse_hbh(u32 flow_id, ip6_hop_by_hop_header_t *hbh0, ip6_hop_by_hop_option_t *opt0,
 ...
 */
static uword
ip6_ioam_analyse_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            data += sizeof(ip4_header_t) + sizeof(udp_header_t) + sizeof(ipfix_message_header_t) +
    ...
    if (PREDICT_FALSE(pkts_failed))
        vlib_node_increment_counter(vm, node->node_index, ANALYSE_ERROR_FAILED, pkts_failed);

    return frame->n_vectors;
}

int
ip6_ioam_analyse_hbh_trace_internal(u32 flow_id, ip6_hop_by_hop_option_t *opt, u16 len)
{
...
int
ip6_ioam_analyse_hbh_e2e_internal(u32 flow_id, ip6_hop_by_hop_option_t *opt, u16 len)
{
    ioam_analyser_data_t *data;
...
int
ip6_ioam_analyse_register_hbh_handler(u8 option, int options(u32 flow_id, ip6_hop_by_hop_option_t *opt, u16 len))
{
    ip6_ioam_analyser_main_t *am = &ioam_analyser_main;
