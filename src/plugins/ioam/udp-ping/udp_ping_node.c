...
format_udp_ping_trace(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               format_ip6_address, &t->src, format_ip6_address, &t->dst, t->src_port, t->dst_port, t->handle,
               t->next_index, t->msg_type);
    return s;
}
...
void
ip46_udp_ping_set_flow(ip46_address_t src, ip46_address_t dst, u16 start_src_port, u16 end_src_port, u16 start_dst_port,
                       u16 end_dst_port, u16 interval, u8 fault_det, u8 is_disable)
{
    u8 found                 = 0;
    ip46_udp_ping_flow *flow = NULL;
    int i;
    ...
    udp_ping_populate_flow(src, dst, start_src_port, end_src_port, start_dst_port, end_dst_port, interval, fault_det,
                           flow);
...
static clib_error_t *
set_udp_ping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    ip46_udp_ping_set_flow(src, dst, start_src_port, end_src_port, start_dst_port, end_dst_port, (u16) interval,
...
VLIB_CLI_COMMAND(set_udp_ping_command, static) = {
    .path       = "set udp-ping",
    .short_help = "set udp-ping src <local IPv6 address>  src-port-range <local port range> \
...
static clib_error_t *
show_udp_ping_summary_cmd_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u8 *s = 0;
    int i, j;
    ...
    for (i = 0; i < vec_len(udp_ping_main.ip46_flow); i++) {
        ...
        s = format(s, "Src: %U, Dst: %U\n", format_ip46_address, &ip46_flow->src, IP46_TYPE_ANY, format_ip46_address,
 ...
 */
void
udp_ping_analyse_hbh(vlib_buffer_t *b0, u32 flow_id, u16 src_port, u16 dst_port, ip6_hop_by_hop_option_t *opt0,
 ...
 */
void
udp_ping_local_analyse(vlib_buffer_t *b0, ip6_header_t *ip0, ip6_hop_by_hop_header_t *hbh0, u16 *next0)
{
    ...
    *next0 = UDP_PING_NEXT_IP6_DROP;
    ...
    if (PREDICT_TRUE(hbh0->protocol == IP_PROTOCOL_UDP)) {
        ...
        if ((udp0->ping_data.probe_marker1 == clib_host_to_net_u32(UDP_PING_PROBE_MARKER1)) &&
            ...
            udp_ping_analyse_hbh(b0, clib_net_to_host_u16(udp0->ping_data.sender_handle),
                                 clib_net_to_host_u16(udp0->udp.dst_port), clib_net_to_host_u16(udp0->udp.src_port),
                                 opt0, limit0, p_len0);
 ...
 */
static uword
udp_ping_local_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
udp_ping_init(vlib_main_t *vm)
{
    ...
    ip6_register_protocol(IP_PROTOCOL_IP6_HOP_BY_HOP_OPTIONS, udp_ping_local.index);
    return 0;
}
