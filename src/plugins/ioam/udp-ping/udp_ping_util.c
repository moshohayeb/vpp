...
u16
udp_ping_fill_udp_data(udp_ping_t *udp_ping, u16 src_port, u16 dst_port, u8 msg_type, u16 ctx)
{
...
udp_ping_create_ip6_pak(u8 *buf, /*u16 len, */
                        ip6_address_t src, ip6_address_t dst, u16 src_port, u16 dst_port, u8 msg_type, u16 ctx)
{
    ip6_header_t *ip0;
...
int
udp_ping_compare_flow(ip46_address_t src, ip46_address_t dst, u16 start_src_port, u16 end_src_port, u16 start_dst_port,
                      u16 end_dst_port, ip46_udp_ping_flow *flow)
{
    if ((0 == ip46_address_cmp(&flow->src, &src)) && (0 == ip46_address_cmp(&flow->dst, &dst)) &&
        (flow->udp_data.start_src_port == start_src_port) && (flow->udp_data.end_src_port == end_src_port) &&
        (flow->udp_data.start_dst_port == start_dst_port) && (flow->udp_data.end_dst_port == end_dst_port)) {
        return 0;
    }
...
void
udp_ping_populate_flow(ip46_address_t src, ip46_address_t dst, u16 start_src_port, u16 end_src_port, u16 start_dst_port,
                       u16 end_dst_port, u16 interval, u8 fault_det, ip46_udp_ping_flow *flow)
{
    flow->src                     = src;
    flow->dst                     = dst;
...
udp_ping_create_rewrite(ip46_udp_ping_flow *flow, u16 ctx)
{
    ...
    for (src_port = flow->udp_data.start_src_port; src_port <= flow->udp_data.end_src_port; src_port++) {
        for (dst_port = flow->udp_data.start_dst_port; dst_port <= flow->udp_data.end_dst_port; dst_port++) {
            ...
            stats->rewrite_len =
                udp_ping_create_ip6_pak(rewrite, flow->src.ip6, flow->dst.ip6, src_port, dst_port, UDP_PING_PROBE, ctx);
    ...
    for (i = 0; i < no_pak; i++) {
        ...
        if (flow->fault_det && (stats->retry > MAX_PING_RETRIES)) {
            ioam_trace_option_t *opt =
                (ioam_trace_option_t *) ip6_hbh_get_option(hbh, HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST);
