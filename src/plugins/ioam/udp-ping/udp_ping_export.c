...
static u8 *
udp_ping_template_rewrite(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                          ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts, u32 n_elts,
                          u32 *stream_index)
{
    return ioam_template_rewrite(frm, fr, collector_address, src_address, collector_port, elts, n_elts, stream_index);
}

static vlib_frame_t *
udp_ping_send_flows(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
    ...
    for (i = 0; i < data_len; i++) {
        ...
        for (src_port = ip46_flow->udp_data.start_src_port; src_port <= ip46_flow->udp_data.end_src_port; src_port++) {
            for (dst_port = ip46_flow->udp_data.start_dst_port; dst_port <= ip46_flow->udp_data.end_dst_port;
                 dst_port++, j++) {
                ...
                next_offset = ioam_analyse_add_ipfix_record(fr, &stats->analyse_data, b0, next_offset,
                                                            &ip46_flow->src.ip6, &ip46_flow->dst.ip6, src_port, dst_port);
    ...
    if (b0) {
        ...
        s->set_id_length =
            ipfix_set_id_length(IOAM_FLOW_TEMPLATE_ID, next_offset - (sizeof(*ip) + sizeof(*udp) + sizeof(*h)));
        h->version_length = version_length(next_offset - (sizeof(*ip) + sizeof(*udp)));
...
static clib_error_t *
set_udp_ping_export_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    // int rv;
    int is_add = 1;
