...
static u8 *
ioam_e2e_trace_handler(u8 *s, ip6_hop_by_hop_option_t *opt)
{
...
ioam_e2e_config_handler(void *data, u8 disable)
{
    ...
    if (!disable) {
        ...
        if (0 == *analyse) {
            if (ip6_hbh_register_option(HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE, ioam_seqno_encap_handler,
...
int
ioam_e2e_rewrite_handler(u8 *rewrite_string, u8 *rewrite_size)
{
...
static clib_error_t *
ioam_show_e2e_cmd_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
ioam_e2e_init(vlib_main_t *vm)
{
    ...
    if (ip6_hbh_add_register_option(HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE, sizeof(ioam_e2e_option_t),
