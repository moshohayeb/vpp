...
static u8 *
format_ioam_pot(u8 *s, va_list *args)
{
    ...
    s = format(s, "random = 0x%Lx, Cumulative = 0x%Lx, Index = 0x%x", random, cumulative,
               pot0 ? pot0->reserved_profile_id : ~0);
    return s;
}
...
int
ip6_hbh_ioam_proof_of_transit_handler(vlib_buffer_t *b, ip6_header_t *ip, ip6_hop_by_hop_option_t *opt0)
{
...
int
ip6_hbh_ioam_proof_of_transit_pop_handler(vlib_buffer_t *b, ip6_header_t *ip, ip6_hop_by_hop_option_t *opt0)
{
    ioam_pot_option_t *pot0;
    u64 random               = 0;
    u64 cumulative           = 0;
...
int
ip6_hop_by_hop_ioam_pot_rewrite_handler(u8 *rewrite_string, u8 *rewrite_size)
{
...
static clib_error_t *
ip6_show_ioam_pot_cmd_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ip6_hop_by_hop_ioam_pot_main_t *hm = &ip6_hop_by_hop_ioam_pot_main;
    u8 *s                              = 0;
    int i                              = 0;
...
ip6_hop_by_hop_ioam_pot_init(vlib_main_t *vm)
{
    ...
    if (ip6_hbh_add_register_option(HBH_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT, sizeof(ioam_pot_option_t),
