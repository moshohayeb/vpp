...
int
pot_profile_create(pot_profile *profile, u64 prime, u64 poly2, u64 lpc, u64 secret_share)
{
...
always_inline u64
pot_update_cumulative_inline(u64 cumulative, u64 random, u64 secret_share, u64 prime, u64 lpc, u64 pre_split,
                             double prime_inv)
{
    u64 share_random   = 0;
...
pot_update_cumulative(pot_profile *profile, u64 cumulative, u64 random)
{
    if (profile && profile->valid != 0) {
        return (pot_update_cumulative_inline(cumulative, random, profile->secret_share, profile->prime, profile->lpc,
                                             profile->poly_pre_eval, profile->primeinv));
    }
    return (0);
}

always_inline u8
pot_validate_inline(u64 secret, u64 prime, double prime_inv, u64 cumulative, u64 random)
{
...
clib_error_t *
clear_pot_profile_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
set_pot_profile_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u64 prime;
    u64 secret_share;
    u64 secret_key;
...
static clib_error_t *
set_pot_profile_activate_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    pot_main_t *sm        = &pot_main;
    u8 *profile_list_name = NULL;
    u32 id                = 0;
    ...
    if (!pot_profile_list_is_enabled(profile_list_name)) {
        result =
            clib_error_return(0, "%s list is not enabled, profile in use %s", profile_list_name, sm->profile_list_name);
...
static clib_error_t *
show_pot_profile_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    pot_main_t *sm = &pot_main;
    pot_profile *p = NULL;
    u16 i;
