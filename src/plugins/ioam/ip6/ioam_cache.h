 ...
 */
always_inline u32
ip6_compute_flow_hash_ext(const ip6_header_t *ip, u8 protocol, u16 src_port, u16 dst_port,
...
static inline void
ioam_e2e_id_rewrite_handler(ioam_e2e_id_option_t *e2e_option, ip6_address_t *address)
{
...
ioam_cache_lookup(ip6_header_t *ip0, u16 src_port, u16 dst_port, u32 seq_no)
{
    ...
    if (clib_bihash_search_8_8(&cm->ioam_rewrite_cache_table, &kv, &value) >= 0) {
        ...
        if (ip6_address_compare(&ip0->src_address, &entry->dst_address) == 0 &&
            ip6_address_compare(&ip0->dst_address, &entry->src_address) == 0 && entry->src_port == dst_port &&
 ...
 */
inline static int
ioam_cache_add(vlib_buffer_t *b0, ip6_header_t *ip0, u16 src_port, u16 dst_port, ip6_hop_by_hop_header_t *hbh0, u32 seq_no)
{
    ioam_cache_main_t *cm     = &ioam_cache_main;
    ioam_cache_entry_t *entry = 0;
...
ioam_cache_table_init(vlib_main_t *vm)
{
    ...
    clib_bihash_init_8_8(&cm->ioam_rewrite_cache_table, "ioam rewrite cache table", cm->lookup_table_nbuckets,
...
ioam_cache_table_destroy(vlib_main_t *vm)
{
    ...
    pool_foreach(entry, cm->ioam_rewrite_pool, ({ ioam_cache_entry_free(entry); }));
...
format_ioam_cache_entry(u8 *s, va_list *args)
{
    ...
    s = format(s, "%d: %U:%d to  %U:%d seq_no %lu\n", (e - cm->ioam_rewrite_pool), format_ip6_address, &e->src_address,
               e->src_port, format_ip6_address, &e->dst_address, e->dst_port, e->seq_no);

    if (rewrite_len) {
        s = format(s, "  %U", format_ip6_hop_by_hop_ext_hdr, (ip6_hop_by_hop_header_t *) e->ioam_rewrite_string,
                   rewrite_len - 1);
    }
    return s;
}
...
ioam_cache_ts_table_init(vlib_main_t *vm)
{
    ...
    for (i = 0; i < no_of_threads; i++) {
        ...
        tw_timer_wheel_init_16t_2w_512sl(&cm->timer_wheels[i], expired_cache_ts_timer_callback,
                                         IOAM_CACHE_TS_TICK
                                         /* timer period 100ms */,
                                         10e4);
...
inline static void
ioam_cache_ts_entry_free(u32 thread_id, ioam_cache_ts_entry_t *entry, u32 node_index)
{
    ioam_cache_main_t *cm = &ioam_cache_main;
...
ioam_cache_ts_table_destroy(vlib_main_t *vm)
{
    ...
    /* free pool and hash table */
    for (i = 0; i < no_of_threads; i++) {
        pool_foreach(entry, cm->ioam_ts_pool[i], ({ ioam_cache_ts_entry_free(i, entry, cm->error_node_index); }));
 ...
 */
inline static int
ioam_cache_ts_add(ip6_header_t *ip0, u16 src_port, u16 dst_port, u32 seq_no, u8 max_responses, u64 now, u32 thread_id,
                  u32 *pool_index)
{
    ioam_cache_main_t *cm        = &ioam_cache_main;
    ioam_cache_ts_entry_t *entry = 0;
...
inline static int
ioam_cache_ts_update(u32 thread_id, i32 pool_index, u32 buffer_index, ip6_hop_by_hop_header_t *hbh)
{
    ioam_cache_main_t *cm        = &ioam_cache_main;
    ioam_cache_ts_entry_t *entry = 0;
    vlib_main_t *vm              = cm->vlib_main;
 ...
 */
inline static int
ioam_cache_ts_lookup(ip6_header_t *ip0, u8 protocol, u16 src_port, u16 dst_port, u32 seq_no,
                     ip6_hop_by_hop_header_t **hbh, u32 *pool_index, u8 *thread_id, u8 response_seen)
{
    ioam_cache_main_t *cm         = &ioam_cache_main;
...
format_ioam_cache_ts_entry(u8 *s, va_list *args)
{
    ...
    if (e->hbh) {
        ...
        s = format(s, "%d: %U:%d to  %U:%d seq_no %u buffer %u %U \n\t\tCreated at %U Received %d\n",
                   (e - cm->ioam_ts_pool[thread_id]), format_ip6_address, &e->src_address, e->src_port,
                   format_ip6_address, &e->dst_address, e->dst_port, e->seq_no, e->buffer_index, format_ip6_address,
                   e2e ? &e2e->id : 0, format_time_interval, "h:m:s:u",
                   (e->created_at - vm->cpu_time_main_loop_start) * ct->seconds_per_clock, e->response_received);
    } else {
        s = format(s, "%d: %U:%d to  %U:%d seq_no %u Buffer %u \n\t\tCreated at %U Received %d\n",
                   (e - cm->ioam_ts_pool[thread_id]), format_ip6_address, &e->src_address, e->src_port,
                   format_ip6_address, &e->dst_address, e->dst_port, e->seq_no, e->buffer_index, format_time_interval,
                   "h:m:s:u", (e->created_at - vm->cpu_time_main_loop_start) * ct->seconds_per_clock,
                   e->response_received);
    }
