...
static uword
ip6_ioam_cache_ts_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (IP_PROTOCOL_TCP == ip6_locate_header(p0, ip0, IP_PROTOCOL_TCP, &tcp_offset0)) {
                ...
                if ((tcp0->flags & TCP_FLAG_SYN) == TCP_FLAG_SYN && (tcp0->flags & TCP_FLAG_ACK) == TCP_FLAG_ACK) {
                    ...
                    if (0 == ioam_cache_ts_lookup(ip0, hbh0->protocol, clib_net_to_host_u16(tcp0->src_port),
                        ...
                        result = -1;
                        if (hbh_cmp)
                            result = ip6_ioam_analyse_compare_path_delay(hbh0, hbh_cmp, cm->criteria_oneway);
                        if (result >= 0) {
                    ...
                    if (0 == ioam_cache_ts_lookup(ip0, hbh0->protocol, clib_net_to_host_u16(tcp0->src_port),
                                                  clib_net_to_host_u16(tcp0->dst_port),
                                                  clib_net_to_host_u32(tcp0->ack_number), &hbh_cmp, &cache_ts_index,
                                                  &cache_thread_id, 1))   // response seen
                    {
                        next0 = IOAM_CACHE_TS_ERROR_NEXT_DROP;
                        if (hbh_cmp)
                            ioam_cache_ts_check_and_send(cache_thread_id, cache_ts_index);
...
static uword
ip6_reset_ts_hbh_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ioam_cache_main_t *cm = &ioam_cache_main;
    u32 n_left_from, *from, *to_next;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if ((tcp0->flags & TCP_FLAG_SYN) == TCP_FLAG_SYN && (tcp0->flags & TCP_FLAG_ACK) == 0) {
                if (no_of_responses > 0) {
                    ...
                    if (0 == ioam_cache_ts_add(ip0, clib_net_to_host_u16(tcp0->src_port),
                                               clib_net_to_host_u16(tcp0->dst_port),
                                               clib_net_to_host_u32(tcp0->seq_number) + 1, no_of_responses, now,
                ...
                ioam_e2e_id_rewrite_handler((ioam_e2e_id_option_t *) ((u8 *) e2e + sizeof(ioam_e2e_cache_option_t)),
                                            &cm->sr_localsid_ts);
        ...
        NEXT00:
            ...
            if ((tcp1->flags & TCP_FLAG_SYN) == TCP_FLAG_SYN && (tcp1->flags & TCP_FLAG_ACK) == 0) {
                if (no_of_responses > 0) {
                    /* Create TS select entry */
                    if (0 == ioam_cache_ts_add(ip1, clib_net_to_host_u16(tcp1->src_port),
                                               clib_net_to_host_u16(tcp1->dst_port),
                                               clib_net_to_host_u32(tcp1->seq_number) + 1, no_of_responses, now,
    ...
    vlib_node_increment_counter(vm, ip6_reset_ts_hbh_node.index, IP6_RESET_TS_HBH_ERROR_PROCESSED, processed);
    vlib_node_increment_counter(vm, ip6_reset_ts_hbh_node.index, IP6_RESET_TS_HBH_ERROR_SAVED, cache_ts_added);
