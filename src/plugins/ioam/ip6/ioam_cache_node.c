...
static uword
ip6_ioam_cache_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (IP_PROTOCOL_TCP == ip6_locate_header(p0, ip0, IP_PROTOCOL_TCP, &tcp_offset0)) {
                ...
                if ((tcp0->flags & TCP_FLAG_SYN) == TCP_FLAG_SYN && (tcp0->flags & TCP_FLAG_ACK) == 0) {
                    ...
                    if (0 == ioam_cache_add(p0, ip0, clib_net_to_host_u16(tcp0->src_port),
...
static uword
ip6_add_from_cache_hbh_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (((tcp0->flags & TCP_FLAG_SYN) == TCP_FLAG_SYN && (tcp0->flags & TCP_FLAG_ACK) == TCP_FLAG_ACK) ||
                ...
                if (0 != (entry = ioam_cache_lookup(ip0, clib_net_to_host_u16(tcp0->src_port),
            ...
            clib_memcpy(srh0->segments, ip0->dst_address.as_u64, sizeof(ip6_address_t));
            /* Rewrite the ip6 dst address with the first hop */
            clib_memcpy(ip0->dst_address.as_u64, entry->next_hop.as_u64, sizeof(ip6_address_t));
            clib_memcpy(&srh0->segments[1], (u8 *) hbh0 + entry->my_address_offset, sizeof(ip6_address_t));
    ...
    vlib_node_increment_counter(vm, ip6_add_from_cache_hbh_node.index, IP6_ADD_FROM_CACHE_HBH_ERROR_PROCESSED, processed);
    return frame->n_vectors;
}
/* *INDENT-OFF* */
...
VLIB_NODE_FUNCTION_MULTIARCH(ip6_add_from_cache_hbh_node, ip6_add_from_cache_hbh_node_fn)
/*
