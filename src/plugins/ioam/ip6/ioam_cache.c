...
int
ioam_cache_ip6_enable_disable(ioam_cache_main_t *em, ip6_address_t *sr_localsid, u8 is_disable)
{
...
/* Action function shared between message handler and debug CLI */
int
ioam_tunnel_select_ip6_enable_disable(ioam_cache_main_t *em, u8 criteria, u8 no_of_responses,
...
/* API message handler */
static void
vl_api_ioam_cache_ip6_enable_disable_t_handler(vl_api_ioam_cache_ip6_enable_disable_t *mp)
{
...
static clib_error_t *
set_ioam_cache_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
/* *INDENT_OFF* */
VLIB_CLI_COMMAND(set_ioam_cache_command,
                 static) = {.path       = "set ioam ip6 cache",
...
static clib_error_t *
set_ioam_tunnel_select_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ioam_cache_main_t *em = &ioam_cache_main;
    u8 is_disable         = 0;
    u8 one_way            = 0;
    ...
    if (is_disable == 0 && !address_set)
        return clib_error_return(0, "Error: SRv6 LocalSID address is mandatory to receive response.");
...
/* *INDENT_OFF* */
VLIB_CLI_COMMAND(set_ioam_cache_ts_command,
                 static) = {.path = "set ioam ip6 sr-tunnel-select",
                            .short_help = "set ioam ip6 sr-tunnel-select [disable] [oneway|rtt] [wait_for_responses <n|default 3>] \
...
ioam_cache_table_print(vlib_main_t *vm, u8 verbose)
{
    ...
    pool_foreach(entry, cm->ioam_rewrite_pool, ({ vlib_cli_output(vm, "%U", format_ioam_cache_entry, entry); }));
...
static clib_error_t *
show_ioam_cache_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u8 verbose = 0;
...
/* *INDENT_OFF* */
VLIB_CLI_COMMAND(show_ioam_cache_command, static) = {.path       = "show ioam ip6 cache",
...
ioam_cache_init(vlib_main_t *vm)
{
    ...
    em->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
