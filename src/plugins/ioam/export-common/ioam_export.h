...
inline static int
ioam_export_init_buffer(ioam_export_main_t *em, vlib_main_t *vm, ioam_export_buffer_t *eb)
{
    vlib_buffer_t *b = 0;
...
ioam_export_thread_buffer_init(ioam_export_main_t *em, vlib_main_t *vm)
{
    ...
    for (i = 0; i < no_of_threads; i++) {
        ...
        em->lockp[i] = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
...
inline static void
ioam_export_header_cleanup(ioam_export_main_t *em, ip4_address_t *collector_address, ip4_address_t *src_address)
{
...
inline static int
ioam_export_header_create(ioam_export_main_t *em, ip4_address_t *collector_address, ip4_address_t *src_address)
{
    ip4_header_t *ip;
    udp_header_t *udp;
    ipfix_message_header_t *h;
...
inline static int
ioam_export_send_buffer(ioam_export_main_t *em, vlib_main_t *vm, ioam_export_buffer_t *eb)
{
    ip4_header_t *ip;
    udp_header_t *udp;
    ipfix_message_header_t *h;
    ...
    /* FIXUP: message header export_time */
    h->export_time =
        clib_host_to_net_u32((u32)(((f64) em->unix_time_0) + (vlib_time_now(em->vlib_main) - em->vlib_time_0)));
    ...
    if (PREDICT_FALSE(eb->records_in_this_buffer != DEFAULT_EXPORT_RECORDS)) {
        s->set_id_length =
            ipfix_set_id_length(em->set_id /* set_id */, b0->current_length - (sizeof(*ip) + sizeof(*udp) + sizeof(*h)));
...
inline static uword
ioam_export_process_common(ioam_export_main_t *em, vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f, u32 index)
{
    f64 now;
    f64 timeout = 30.0;
    uword event_type;
