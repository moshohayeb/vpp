...
static uword
vxlan_gpe_transit_ioam(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE((ip0->ip_version_and_header_length & 0xF0) == 0x40)) {
                ...
                if (PREDICT_FALSE((ip0->protocol == IP_PROTOCOL_UDP) &&
                                  (clib_net_to_host_u16(udp_hdr0->dst_port) == UDP_DST_PORT_VXLAN_GPE))) {
                    ...
                    if (PREDICT_FALSE(gpe_hdr0->protocol == VXLAN_GPE_PROTOCOL_IOAM)) {
                        ...
                        if (t) {
                            ...
                            vxlan_gpe_encap_decap_ioam_v4_one_inline(vm, node, b0, &next0,
                                                                     VXLAN_GPE_TRANSIT_IOAM_NEXT_DROP, 1
                                                                     /* use_adj */
                            );
                            vlib_buffer_advance(b0, -(word)(sizeof(ethernet_header_t)));
                        }
