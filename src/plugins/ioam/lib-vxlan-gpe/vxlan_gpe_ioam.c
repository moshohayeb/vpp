...
int
vxlan_gpe_ioam_set_rewrite(vxlan_gpe_tunnel_t *t, int has_trace_option, int has_pot_option, int has_ppc_option,
                           u8 ipv6_set)
{
    ...
    if (has_trace_option && hm->add_options[VXLAN_GPE_OPTION_TYPE_IOAM_TRACE] != 0) {
        if (0 != hm->add_options[VXLAN_GPE_OPTION_TYPE_IOAM_TRACE](current, &trace_data_size))
            return -1;
        current += trace_data_size;
    }
    if (has_pot_option && hm->add_options[VXLAN_GPE_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT] != 0) {
        pot_data_size = hm->options_size[VXLAN_GPE_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT];
        if (0 == hm->add_options[VXLAN_GPE_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT](current, &pot_data_size))
...
int
vxlan_gpe_ioam_clear_rewrite(vxlan_gpe_tunnel_t *t, int has_trace_option, int has_pot_option, int has_ppc_option,
                             u8 ipv6_set)
{
...
clib_error_t *
vxlan_gpe_ioam_set(vxlan_gpe_tunnel_t *t, int has_trace_option, int has_pot_option, int has_ppc_option, u8 ipv6_set)
{
    int rv;
...
static void
vxlan_gpe_set_clear_output_feature_on_intf(vlib_main_t *vm, u32 sw_if_index0, u8 is_add)
{


    vnet_feature_enable_disable("ip4-output", "vxlan-gpe-transit-ioam", sw_if_index0, is_add,
...
vxlan_gpe_clear_output_feature_on_all_intfs(vlib_main_t *vm)
{
    ...
    pool_foreach(si, im->sw_interfaces, ({ vxlan_gpe_set_clear_output_feature_on_intf(vm, si->sw_if_index, 0); }));
    return;
}
...
int
vxlan_gpe_enable_disable_ioam_for_dest(vlib_main_t *vm, ip46_address_t dst_addr, u32 outer_fib_index, u8 is_ipv4,
                                       u8 is_add)
{
    vxlan_gpe_ioam_main_t *hm = &vxlan_gpe_ioam_main;
    u32 fib_index0            = 0;
    ...
    if (is_ipv4) {
        ...
        if (is_add) {
            ...
            t1->fib_entry_index =
                fib_table_entry_special_add(outer_fib_index, &tun_dst_pfx, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
...
vxlan_gpe_refresh_output_feature_on_all_dest(void)
{
    ...
    pool_foreach(t, hm->dst_tunnels, ({
                     vxlan_gpe_enable_disable_ioam_for_dest(hm->vlib_main, t->dst_addr, t->outer_fib_index,
...
static clib_error_t *
vxlan_gpe_set_ioam_rewrite_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vxlan_gpe_ioam_main_t *hm = &vxlan_gpe_ioam_main;
...
static clib_error_t *
vxlan_gpe_set_ioam_flags_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
int
vxlan_gpe_ioam_disable_for_dest(vlib_main_t *vm, ip46_address_t dst_addr, u32 outer_fib_index, u8 ipv4_set)
{
    vxlan_gpe_ioam_dest_tunnels_t *t;
    vxlan_gpe_ioam_main_t *hm = &vxlan_gpe_ioam_main;
...
static clib_error_t *
vxlan_gpe_set_ioam_transit_rewrite_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vxlan_gpe_ioam_main_t *hm = &vxlan_gpe_ioam_main;
...
clib_error_t *
clear_vxlan_gpe_ioam_rewrite_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
