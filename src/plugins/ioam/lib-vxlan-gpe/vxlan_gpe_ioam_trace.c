...
int
vxlan_gpe_ioam_add_register_option(u8 option, u8 size, int rewrite_options(u8 *rewrite_string, u8 *rewrite_size))
{
...
always_inline void
vxlan_gpe_ioam_trace_stats_increment_counter(u32 counter_index, u64 increment)
{
...
format_ioam_data_list_element(u8 *s, va_list *args)
{
    ...
    if (trace_type & BIT_TTL_NODEID) {
        ...
        s                               = format(s, "ttl 0x%x node id 0x%x ", ttl_node_id_host_byte_order >> 24,
...
int
vxlan_gpe_ioam_trace_data_list_handler(vlib_buffer_t *b, vxlan_gpe_ioam_option_t *opt, u8 is_ipv4, u8 use_adj)
{
    u8 elt_index                         = 0;
...
u8 *
vxlan_gpe_ioam_trace_data_list_trace_handler(u8 *s, vxlan_gpe_ioam_option_t *opt)
{
...
static clib_error_t *
vxlan_gpe_show_ioam_trace_cmd_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vxlan_gpe_ioam_trace_main_t *hm = &vxlan_gpe_ioam_trace_main;
    u8 *s                           = 0;
    int i                           = 0;
...
vxlan_gpe_ioam_trace_init(vlib_main_t *vm)
{
    ...
    if (vxlan_gpe_ioam_register_option(VXLAN_GPE_OPTION_TYPE_IOAM_TRACE, vxlan_gpe_ioam_trace_data_list_handler,
    ...
    if (vxlan_gpe_ioam_add_register_option(VXLAN_GPE_OPTION_TYPE_IOAM_TRACE, sizeof(vxlan_gpe_ioam_trace_option_t),
