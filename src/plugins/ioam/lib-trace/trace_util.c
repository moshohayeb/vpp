...
int
trace_profile_create(trace_profile *profile, u8 trace_type, u8 num_elts, u32 trace_tsp, u32 node_id, u32 app_data)
{
...
show_trace_profile_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    s = format(s, "         Trace timestamp precision : %d (%s)\n", p->trace_tsp,
               (p->trace_tsp == TSP_SECONDS) ?
                   "Seconds" :
                   ((p->trace_tsp == TSP_MILLISECONDS) ?
                        "Milliseconds" :
                        (((p->trace_tsp == TSP_MICROSECONDS) ? "Microseconds" : "Nanoseconds"))));
