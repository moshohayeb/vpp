...
format_stn_rule(u8 *s, va_list *args)
{
    ...
    s = format(s, "%Uiface: %U (%d)\n", format_white_space, indent, format_vnet_sw_if_index_name, vnet_get_main(),
               r->sw_if_index, r->sw_if_index);
    s = format(s, "%Unext_node: %s (%d)", format_white_space, indent, next_node->name, next_node->index);
    return s;
}
...
format_stn_ip46_punt_trace(u8 *s, va_list *args, u8 is_ipv4)
{
    ...
    if (t->kv.value == ~(0L)) {
        ...
        s = format(s, "%Urule:\n%U%U", format_white_space, indent, format_white_space, indent + 2, format_stn_rule,
...
stn_ip46_punt_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ipv4)
{
...
static uword
stn_ip6_punt_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
stn_ip4_punt_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static clib_error_t *
show_stn_rules_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    stn_main_t *stn = &stn_main;
    u8 *s           = 0;
    stn_rule_t *rule;
    pool_foreach(rule, stn->rules, { s = format(s, "- %U\n", format_stn_rule, rule); });
