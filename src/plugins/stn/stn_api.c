 ...
 */
static void *
vl_api_stn_add_del_rule_t_print(vl_api_stn_add_del_rule_t *mp, void *handle)
{
    u8 *s;
...
vl_api_stn_rules_dump_t_handler(vl_api_stn_rules_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(r, stn->rules, ({ send_stn_rules_details(r, reg, mp->context); }));
    /* *INDENT-ON* */
}
...
stn_plugin_api_hookup(vlib_main_t *vm)
{
    stn_main_t *stn = &stn_main;
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + stn->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,              \
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + stn->msg_id_base);
...
{
#define _(n, f) api_main.msg_print_handlers[VL_API_##n + stn->msg_id_base] = (void *) vl_api_##f##_t_print;
    foreach_stn_plugin_api_msg;
#undef _
}
