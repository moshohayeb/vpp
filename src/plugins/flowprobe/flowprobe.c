 ...
 */
static inline u8 *
flowprobe_template_rewrite_inline(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                                  ip4_address_t *src_address, u16 collector_port, flowprobe_variant_t which)
{
    ip4_header_t *ip;
    udp_header_t *udp;
...
static u8 *
flowprobe_template_rewrite_ip6(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                               ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts, u32 n_elts,
                               u32 *stream_index)
{
    return flowprobe_template_rewrite_inline(frm, fr, collector_address, src_address, collector_port, FLOW_VARIANT_IP6);
}

static u8 *
flowprobe_template_rewrite_ip4(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                               ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts, u32 n_elts,
                               u32 *stream_index)
{
    return flowprobe_template_rewrite_inline(frm, fr, collector_address, src_address, collector_port, FLOW_VARIANT_IP4);
}

static u8 *
flowprobe_template_rewrite_l2(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                              ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts, u32 n_elts,
                              u32 *stream_index)
{
    return flowprobe_template_rewrite_inline(frm, fr, collector_address, src_address, collector_port, FLOW_VARIANT_L2);
}

static u8 *
flowprobe_template_rewrite_l2_ip4(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                                  ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts,
                                  u32 n_elts, u32 *stream_index)
{
    return flowprobe_template_rewrite_inline(frm, fr, collector_address, src_address, collector_port,
                                             FLOW_VARIANT_L2_IP4);
}

static u8 *
flowprobe_template_rewrite_l2_ip6(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                                  ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *elts,
                                  u32 n_elts, u32 *stream_index)
{
    return flowprobe_template_rewrite_inline(frm, fr, collector_address, src_address, collector_port,
 ...
 */
vlib_frame_t *
flowprobe_data_callback_ip4(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
...
vlib_frame_t *
flowprobe_data_callback_ip6(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
...
vlib_frame_t *
flowprobe_data_callback_l2(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
...
flowprobe_template_add_del(u32 domain_id, u16 src_port, flowprobe_record_t flags,
                           ...
                           vnet_flow_rewrite_callback_t *rewrite_callback, bool is_add, u16 *template_id)
{
...
static int
validate_feature_on_interface(flowprobe_main_t *fm, u32 sw_if_index, u8 which)
{
...
static int
flowprobe_tx_interface_add_del_feature(flowprobe_main_t *fm, u32 sw_if_index, u8 which, int is_add)
{
    vlib_main_t *vm          = vlib_get_main();
    int rv                   = 0;
    u16 template_id          = 0;
    ...
    if ((is_add && fm->template_per_flow[which] == 1) || (!is_add && fm->template_per_flow[which] == 0)) {
        if (which == FLOW_VARIANT_L2) {
            if (fm->record & FLOW_RECORD_L2) {
                rv = flowprobe_template_add_del(1, UDP_DST_PORT_ipfix, flags, flowprobe_data_callback_l2,
                                                flowprobe_template_rewrite_l2, is_add, &template_id);
            }
            if (fm->record & FLOW_RECORD_L3 || fm->record & FLOW_RECORD_L4) {
                rv = flowprobe_template_add_del(1, UDP_DST_PORT_ipfix, flags, flowprobe_data_callback_l2,
                                                flowprobe_template_rewrite_l2_ip4, is_add, &template_id);
                fm->template_reports[flags | FLOW_RECORD_L2_IP4] = (is_add) ? template_id : 0;
                rv = flowprobe_template_add_del(1, UDP_DST_PORT_ipfix, flags, flowprobe_data_callback_l2,
                                                flowprobe_template_rewrite_l2_ip6, is_add, &template_id);
            ...
            rv = flowprobe_template_add_del(1, UDP_DST_PORT_ipfix, flags, flowprobe_data_callback_ip4,
                                            flowprobe_template_rewrite_ip4, is_add, &template_id);
        else if (which == FLOW_VARIANT_IP6)
            rv = flowprobe_template_add_del(1, UDP_DST_PORT_ipfix, flags, flowprobe_data_callback_ip6,
    ...
    if (which == FLOW_VARIANT_IP4)
        vnet_feature_enable_disable("ip4-output", "flowprobe-ip4", sw_if_index, is_add, 0, 0);
    else if (which == FLOW_VARIANT_IP6)
        vnet_feature_enable_disable("ip6-output", "flowprobe-ip6", sw_if_index, is_add, 0, 0);
    else if (which == FLOW_VARIANT_L2)
        vnet_feature_enable_disable("interface-output", "flowprobe-l2", sw_if_index, is_add, 0, 0);
 ...
 */
void
vl_api_flowprobe_tx_interface_add_del_t_handler(vl_api_flowprobe_tx_interface_add_del_t *mp)
{
    flowprobe_main_t *fm = &flowprobe_main;
    ...
    if (mp->which != FLOW_VARIANT_IP4 && mp->which != FLOW_VARIANT_L2 && mp->which != FLOW_VARIANT_IP6) {
    ...
    rv = flowprobe_tx_interface_add_del_feature(fm, sw_if_index, mp->which, mp->is_add);
 ...
 */
static void *
vl_api_flowprobe_tx_interface_add_del_t_print(vl_api_flowprobe_tx_interface_add_del_t *mp, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: flowprobe_tx_interface_add_del ");
    s = format(s, "sw_if_index %d is_add %d which %d ", clib_host_to_net_u32(mp->sw_if_index), (int) mp->is_add,
               (int) mp->which);
    FINISH;
}
...
static int
flowprobe_params(flowprobe_main_t *fm, u8 record_l2, u8 record_l3, u8 record_l4, u32 active_timer, u32 passive_timer)
{
...
vl_api_flowprobe_params_t_handler(vl_api_flowprobe_params_t *mp)
{
    ...
    rv = flowprobe_params(fm, mp->record_l2, mp->record_l3, mp->record_l4, clib_net_to_host_u32(mp->active_timer),
...
format_flowprobe_entry(u8 *s, va_list *args)
{
    ...
    s = format(s, " %U -> %U", format_ip46_address, &e->key.src_address, IP46_TYPE_ANY, format_ip46_address,
               &e->key.dst_address, IP46_TYPE_ANY);
...
static clib_error_t *
flowprobe_show_table_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cm)
{
    flowprobe_main_t *fm = &flowprobe_main;
    int i;
    flowprobe_entry_t *e;
    ...
    for (i = 0; i < vec_len(fm->pool_per_worker); i++) {
        /* *INDENT-OFF* */
        pool_foreach(e, fm->pool_per_worker[i], ({ vlib_cli_output(vm, "%U", format_flowprobe_entry, e); }));
        /* *INDENT-ON* */
    }
    return 0;
}
...
static clib_error_t *
flowprobe_show_stats_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cm)
{
    flowprobe_main_t *fm = &flowprobe_main;
    int i;
    ...
    for (i = 0; i < vec_len(fm->pool_per_worker); i++)
        vlib_cli_output(vm, "Pool utilisation thread %d is %d%%\n", i,
                        (100 * pool_elts(fm->pool_per_worker[i])) / (0x1 << FLOWPROBE_LOG2_HASHSIZE));
    return 0;
}

static clib_error_t *
flowprobe_tx_interface_add_del_feature_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    flowprobe_main_t *fm = &flowprobe_main;
    u32 sw_if_index      = ~0;
    int is_add           = 1;
    ...
    if (fm->record == 0)
        return clib_error_return(0, "Please specify flowprobe params record first...");
    ...
    if (rv == 1) {
        if (is_add)
            return clib_error_return(0, "Datapath is already enabled for given interface...");
    } else if (rv == 0)
        return clib_error_return(0, "Interface has enable different datapath ...");

    rv = flowprobe_tx_interface_add_del_feature(fm, sw_if_index, which, is_add);
    switch (rv) {
    case 0:
        break;
...
static clib_error_t *
flowprobe_params_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    flowprobe_main_t *fm = &flowprobe_main;
...
VLIB_CLI_COMMAND(flowprobe_enable_disable_command, static) = {
    ...
    .short_help = "flowprobe feature add-del <interface-name> <l2|ip4|ip6> disable",
...
VLIB_CLI_COMMAND(flowprobe_params_command, static) = {
    ...
    .short_help = "flowprobe params record <[l2] [l3] [l4]> [active <timer> passive <timer>]",
...
flowprobe_plugin_api_hookup(vlib_main_t *vm)
{
    flowprobe_main_t *fm = &flowprobe_main;
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + fm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,               \
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + fm->msg_id_base);
...
flowprobe_init(vlib_main_t *vm)
{
    ...
    fm->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
