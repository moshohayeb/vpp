...
static char *flowprobe_variant_strings[] = {
    [FLOW_VARIANT_IP4] = "IP4",       [FLOW_VARIANT_IP6] = "IP6",       [FLOW_VARIANT_L2] = "L2",
    [FLOW_VARIANT_L2_IP4] = "L2-IP4", [FLOW_VARIANT_L2_IP6] = "L2-IP6",
};
...
format_flowprobe_trace(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               flowprobe_variant_strings[t->which], t->rx_sw_if_index, t->tx_sw_if_index, t->timestamp, t->buffer_size);

    if (t->which == FLOW_VARIANT_L2)
        s = format(s, "\n%U -> %U", format_white_space, indent, format_ethernet_address, &t->src_mac,
                   format_ethernet_address, &t->dst_mac);

    if (t->protocol > 0 && (t->which == FLOW_VARIANT_L2_IP4 || t->which == FLOW_VARIANT_IP4 ||
                            t->which == FLOW_VARIANT_L2_IP6 || t->which == FLOW_VARIANT_IP6))
        s = format(s, "\n%U%U: %U -> %U", format_white_space, indent, format_ip_protocol, t->protocol,
                   format_ip46_address, &t->src_address, IP46_TYPE_ANY, format_ip46_address, &t->dst_address,
                   IP46_TYPE_ANY);
    return s;
}
...
flowprobe_get_variant(flowprobe_variant_t which, flowprobe_record_t flags, u16 ethertype)
{
    if (which == FLOW_VARIANT_L2 && (flags & FLOW_RECORD_L3 || flags & FLOW_RECORD_L4))
        return ethertype == ETHERNET_TYPE_IP6 ? FLOW_VARIANT_L2_IP6 :
                                                ethertype == ETHERNET_TYPE_IP4 ? FLOW_VARIANT_L2_IP4 : FLOW_VARIANT_L2;
    return which;
}
...
flowprobe_l3_ip6_add(vlib_buffer_t *to_b, flowprobe_entry_t *e, u16 offset)
{
    ...
    /* ip6 src address */
    clib_memcpy(to_b->data + offset, &e->key.src_address, sizeof(ip6_address_t));
    offset += sizeof(ip6_address_t);

    /* ip6 dst address */
    clib_memcpy(to_b->data + offset, &e->key.dst_address, sizeof(ip6_address_t));
    offset += sizeof(ip6_address_t);
...
flowprobe_l3_ip4_add(vlib_buffer_t *to_b, flowprobe_entry_t *e, u16 offset)
{
    ...
    /* ip4 src address */
    clib_memcpy(to_b->data + offset, &e->key.src_address.ip4, sizeof(ip4_address_t));
    offset += sizeof(ip4_address_t);

    /* ip4 dst address */
    clib_memcpy(to_b->data + offset, &e->key.dst_address.ip4, sizeof(ip4_address_t));
    offset += sizeof(ip4_address_t);
...
flowprobe_entry_t *
flowprobe_lookup(u32 my_cpu_number, flowprobe_key_t *k, u32 *poolindex, bool *collision)
{
    flowprobe_main_t *fm = &flowprobe_main;
    flowprobe_entry_t *e;
    u32 h;
    ...
    *poolindex                            = e - fm->pool_per_worker[my_cpu_number];
    ...
    if (fm->passive_timer > 0) {
        e->passive_timer_handle =
            tw_timer_start_2t_1w_2048sl(fm->timers_per_worker[my_cpu_number], *poolindex, 0, fm->passive_timer);
    }
    return e;
}

static inline void
add_to_flow_record_state(vlib_main_t *vm, vlib_node_runtime_t *node, flowprobe_main_t *fm, vlib_buffer_t *b,
                         timestamp_nsec_t timestamp, u16 length, flowprobe_variant_t which, flowprobe_trace_t *t)
{
    if (fm->disabled)
        return;
...
static void
flowprobe_export_send(vlib_main_t *vm, vlib_buffer_t *b0, flowprobe_variant_t which)
{
    flowprobe_main_t *fm    = &flowprobe_main;
    ...
    /* Nothing to send */
    if (fm->context[which].next_record_offset_per_worker[my_cpu_number] <= flowprobe_get_headersize())
        return;
    ...
    s->set_id_length =
        ipfix_set_id_length(fm->template_reports[flags], b0->current_length - (sizeof(*ip) + sizeof(*udp) + sizeof(*h)));
    ...
    vlib_node_increment_counter(vm, flowprobe_l2_node.index, FLOWPROBE_ERROR_EXPORTED_PACKETS, 1);
...
flowprobe_get_buffer(vlib_main_t *vm, flowprobe_variant_t which)
{
    ...
    if (PREDICT_FALSE(b0 == 0)) {
        if (vlib_buffer_alloc(vm, &bi0, 1) != 1) {
            vlib_node_increment_counter(vm, flowprobe_l2_node.index, FLOWPROBE_ERROR_BUFFER, 1);
            return 0;
        }
...
uword
flowprobe_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, flowprobe_variant_t which)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_TRUE((b0->flags & VNET_BUFFER_F_FLOW_REPORT) == 0))
                add_to_flow_record_state(vm, node, fm, b0, timestamp, len0,
                                         flowprobe_get_variant(which, fm->context[which].flags, ethertype0), 0);
            ...
            if (PREDICT_TRUE((b1->flags & VNET_BUFFER_F_FLOW_REPORT) == 0))
                add_to_flow_record_state(vm, node, fm, b1, timestamp, len1,
                                         flowprobe_get_variant(which, fm->context[which].flags, ethertype1), 0);
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE((b0->flags & VNET_BUFFER_F_FLOW_REPORT) == 0)) {
                ...
                add_to_flow_record_state(vm, node, fm, b0, timestamp, len0,
                                         flowprobe_get_variant(which, fm->context[which].flags, ethertype0), t);
            }
...
static uword
flowprobe_ip4_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return flowprobe_node_fn(vm, node, frame, FLOW_VARIANT_IP4);
}

static uword
flowprobe_ip6_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return flowprobe_node_fn(vm, node, frame, FLOW_VARIANT_IP6);
}

static uword
flowprobe_l2_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
flowprobe_walker_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    flowprobe_main_t *fm    = &flowprobe_main;
    flow_report_main_t *frm = &flow_report_main;
    flowprobe_entry_t *e;
