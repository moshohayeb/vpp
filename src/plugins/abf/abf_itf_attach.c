...
abf_itf_attach_stack(abf_itf_attach_t *aia)
{
    ...
    fib_path_list_contribute_forwarding(ap->ap_pl,
                                        (FIB_PROTOCOL_IP4 == aia->aia_proto ? FIB_FORW_CHAIN_TYPE_UNICAST_IP4 :
                                                                              FIB_FORW_CHAIN_TYPE_UNICAST_IP6),
                                        FIB_PATH_LIST_FWD_FLAG_COLLAPSE, &via_dpo);

    dpo_stack_from_node((FIB_PROTOCOL_IP4 == aia->aia_proto ? abf_ip4_node.index : abf_ip6_node.index), &aia->aia_dpo,
                        &via_dpo);
    dpo_reset(&via_dpo);
}
...
int
abf_itf_attach(fib_protocol_t fproto, u32 policy_id, u32 priority, u32 sw_if_index)
{
    abf_itf_attach_t *aia;
    abf_policy_t *ap;
    u32 api, aiai;
    ...
    if (1 == vec_len(abf_per_itf[fproto][sw_if_index])) {
        ...
        vnet_feature_enable_disable((FIB_PROTOCOL_IP4 == fproto ? "ip4-unicast" : "ip6-unicast"),
                                    (FIB_PROTOCOL_IP4 == fproto ? "abf-input-ip4" : "abf-input-ip6"), sw_if_index, 1,
                                    NULL, 0);
...
abf_itf_detach(fib_protocol_t fproto, u32 policy_id, u32 sw_if_index)
{
    ...
    if (0 == vec_len(abf_per_itf[fproto][sw_if_index])) {
         ...
         */
        vnet_feature_enable_disable((FIB_PROTOCOL_IP4 == fproto ? "ip4-unicast" : "ip6-unicast"),
                                    (FIB_PROTOCOL_IP4 == fproto ? "abf-input-ip4" : "abf-input-ip6"), sw_if_index, 0,
                                    NULL, 0);
...
static clib_error_t *
abf_itf_attach_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
abf_show_attach_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
always_inline uword
abf_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, fib_protocol_t fproto)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            /*
               ...
               . . .
             */
            acl_plugin_fill_5tuple_inline(acl_plugin.p_acl_main, lc_index, b0, (FIB_PROTOCOL_IP6 == fproto), 1, 0,
                                          &fa_5tuple0);

            if (acl_plugin_match_5tuple_inline(acl_plugin.p_acl_main, lc_index, &fa_5tuple0,
                                               (FIB_PROTOCOL_IP6 == fproto), &action, &match_acl_pos, &match_acl_index,
    ...
    vlib_node_increment_counter(vm, (fproto = FIB_PROTOCOL_IP6 ? abf_ip4_node.index : abf_ip6_node.index),
...
static uword
abf_input_ip4(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return abf_input_inline(vm, node, frame, FIB_PROTOCOL_IP4);
}

static uword
abf_input_ip6(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
abf_itf_attach_back_walk_notify(fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
{
    /*
...
{
    abf_itf_attach_fib_node_type = fib_node_register_new_type(&abf_itf_attach_vft);
    clib_error_t *acl_init_res   = acl_plugin_exports_init(&acl_plugin);
    if (acl_init_res)
        return (acl_init_res);

    abf_acl_user_id = acl_plugin.register_user_module("abp plugin", "sw_if_index", NULL);
