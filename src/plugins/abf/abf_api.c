...
vl_api_abf_policy_add_del_t_handler(vl_api_abf_policy_add_del_t *mp)
{
    ...
    if (mp->is_add) {
        abf_policy_update(ntohl(mp->policy.policy_id), ntohl(mp->policy.acl_index), paths);
    } else {
...
vl_api_abf_itf_attach_add_del_t_handler(vl_api_abf_itf_attach_add_del_t *mp)
{
    ...
    if (mp->is_add) {
        abf_itf_attach(fproto, ntohl(mp->attach.policy_id), ntohl(mp->attach.priority), ntohl(mp->attach.sw_if_index));
    } else {
        abf_itf_detach(fproto, ntohl(mp->attach.policy_id), ntohl(mp->attach.sw_if_index));
    }
...
{
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + abf_base_msg_id), #n, vl_api_##n##_t_handler, vl_noop_handler,               \
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(apim, #n "_" #crc, id + abf_base_msg_id);
...
abf_api_init(vlib_main_t *vm)
{
    ...
    abf_base_msg_id = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
