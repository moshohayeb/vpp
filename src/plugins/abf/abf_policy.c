...
void
abf_policy_update(u32 policy_id, u32 acl_index, const fib_route_path_t *rpaths)
{
    abf_policy_t *ap;
    u32 api;
    ...
    if (INDEX_INVALID == api) {
        ...
        if (FIB_NODE_INDEX_INVALID != old_pl) {
            ap->ap_pl = fib_path_list_copy_and_path_add(old_pl, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF),
                                                        rpaths);
...
abf_policy_delete(u32 policy_id, const fib_route_path_t *rpaths)
{
    ...
    if (INDEX_INVALID == api) {
        ...
        ap->ap_pl = fib_path_list_copy_and_path_remove(ap->ap_pl,
...
static clib_error_t *
abf_show_policy_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u32 policy_id;
    abf_policy_t *ap;
    ...
    if (INDEX_INVALID == policy_id) {
        /* *INDENT-OFF* */
        pool_foreach(ap, abf_policy_pool, ({ vlib_cli_output(vm, "%U", format_abf, ap); }));
        /* *INDENT-ON* */
    } else {
