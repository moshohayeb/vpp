...
cdp_enable_disable(cdp_main_t *cm, int enable_disable)
{
    ...
    if (enable_disable)
        vlib_process_signal_event(cm->vlib_main, cdp_process_node.index, CDP_EVENT_ENABLE, 0);
    else
        vlib_process_signal_event(cm->vlib_main, cdp_process_node.index, CDP_EVENT_DISABLE, 0);
...
static clib_error_t *
cdp_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
cdp_plugin_api_hookup(vlib_main_t *vm)
{
    cdp_main_t *cm = &cdp_main;
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + cm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,               \
...
cdp_init(vlib_main_t *vm)
{
    ...
    cm->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
