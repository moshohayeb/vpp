...
end_as_processing(vlib_buffer_t *b0, ip6_header_t *ip0, srv6_as_localsid_t *ls0_mem, u32 *next0)
{
    u16 encap_len;
    ...
    if (PREDICT_FALSE(hdr_type != IP_PROTOCOL_IPV6 && hdr_type != IP_PROTOCOL_IP_IN_IP &&
 ...
 */
static uword
srv6_as_localsid_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            vlib_increment_combined_counter(((next0 == SRV6_AS_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) :
 ...
 */
static uword
srv6_as2_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ls0_mem == NULL || ls0_mem->rewrite == NULL)) {
                ...
                clib_memcpy(((u8 *) en0) - vec_len(ls0_mem->rewrite), ls0_mem->rewrite, vec_len(ls0_mem->rewrite));
            ...
            vlib_increment_combined_counter(((next0 == SRV6_AS_LOCALSID_NEXT_ERROR) ? &(sm->invalid_counters) :
                                                                                      &(sm->valid_counters)),
                                            vm->thread_index, ls0_mem->index, 1, vlib_buffer_length_in_chain(vm, b0));
 ...
 */
static uword
srv6_as4_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ip6_sr_main_t *srm = &sr_main;
    srv6_as_main_t *sm = &srv6_as_main;
    u32 n_left_from, next_index, *from, *to_next;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ls0_mem == NULL || ls0_mem->rewrite == NULL)) {
                ...
                clib_memcpy(((u8 *) ip0_encap) - vec_len(ls0_mem->rewrite), ls0_mem->rewrite, vec_len(ls0_mem->rewrite));
                vlib_buffer_advance(b0, -(word) vec_len(ls0_mem->rewrite));
 ...
 */
static uword
srv6_as6_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ip6_sr_main_t *srm = &sr_main;
    srv6_as_main_t *sm = &srv6_as_main;
    u32 n_left_from, next_index, *from, *to_next;
