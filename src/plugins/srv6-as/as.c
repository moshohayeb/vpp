...
static inline u8 *
prepare_rewrite(ip6_address_t src_addr, ip6_address_t *sid_list, u8 protocol)
{
    u8 *rewrite_str = NULL;
...
srv6_as_localsid_creation_fn(ip6_sr_localsid_t *localsid)
{
    ...
    if (ls_mem->inner_type != AS_TYPE_L2) {
        if (ls_mem->inner_type == AS_TYPE_IP4)
            nh_adj_index =
                adj_nbr_add_or_lock(FIB_PROTOCOL_IP4, VNET_LINK_IP4, &ls_mem->nh_addr, ls_mem->sw_if_index_out);
        else if (ls_mem->inner_type == AS_TYPE_IP6)
            nh_adj_index =
                adj_nbr_add_or_lock(FIB_PROTOCOL_IP6, VNET_LINK_IP6, &ls_mem->nh_addr, ls_mem->sw_if_index_out);
    ...
    if (ls_mem->inner_type == AS_TYPE_L2) {
        ...
        ethernet_set_flags(vnm, ls_mem->sw_if_index_in, ETHERNET_INTERFACE_FLAG_ACCEPT_ALL);

        /* Prepare rewrite string */
        ls_mem->rewrite = prepare_rewrite(ls_mem->src_addr, ls_mem->sid_list, IP_PROTOCOL_IP6_NONXT);
        ...
        if (ls_mem->sw_if_index_in >= vec_len(sm->sw_iface_localsid2)) {
            vec_resize(sm->sw_iface_localsid2,
                       (pool_len(sm->vnet_main->interface_main.sw_interfaces) - vec_len(sm->sw_iface_localsid2)));
        }
        ...
        if (ls_mem->sw_if_index_in >= vec_len(sm->sw_iface_localsid4)) {
            vec_resize(sm->sw_iface_localsid4,
                       (pool_len(sm->vnet_main->interface_main.sw_interfaces) - vec_len(sm->sw_iface_localsid4)));
        }
        ...
        if (ls_mem->sw_if_index_in >= vec_len(sm->sw_iface_localsid6)) {
            vec_resize(sm->sw_iface_localsid6,
                       (pool_len(sm->vnet_main->interface_main.sw_interfaces) - vec_len(sm->sw_iface_localsid6)));
        }
...
#define PARAM_AS_SRC (1 << 3)
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (!(params & PARAM_AS_NH) && unformat(input, "nh %U", unformat_ip6_address, &nh_addr.ip6)) {
            ...
            params |= PARAM_AS_NH;
        } else if (!(params & PARAM_AS_OIF) &&
                   unformat(input, "oif %U", unformat_vnet_sw_interface, vnm, &sw_if_index_out)) {
            params |= PARAM_AS_OIF;
        } else if (!(params & PARAM_AS_IIF) &&
                   unformat(input, "iif %U", unformat_vnet_sw_interface, vnm, &sw_if_index_in)) {
            params |= PARAM_AS_IIF;
...
srv6_as_init(vlib_main_t *vm)
{
    ...
    rv = sr_localsid_register_function(vm, function_name, keyword_str, def_str, params_str, &sm->srv6_as_dpo_type,
                                       format_srv6_as_localsid, unformat_srv6_as_localsid, srv6_as_localsid_creation_fn,
