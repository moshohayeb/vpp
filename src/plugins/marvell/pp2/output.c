...
uword
mrvl_pp2_interface_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (PREDICT_FALSE(pp2_ppio_get_num_outq_done(ppif->ppio, ptd->hif, qid, &n_done))) {
        n_done = 0;
        vlib_error_count(vm, node->node_index, MRVL_PP2_TX_ERROR_PPIO_GET_NUM_OUTQ_DONE, 1);
    }
    ...
    if (PREDICT_FALSE(n_sent != n_desc)) {
        vlib_buffer_free(vm, vlib_frame_args(frame) + n_sent, frame->n_vectors - n_sent);
        vlib_error_count(vm, node->node_index, MRVL_PP2_TX_ERROR_NO_FREE_SLOTS, frame->n_vectors - n_sent);
    }
    ...
    if (n_sent) {
        ...
        if (PREDICT_FALSE(n_copy < n_sent))
            clib_memcpy(outq->buffers, buffers + n_copy, (n_sent - n_copy) * sizeof(u32));
