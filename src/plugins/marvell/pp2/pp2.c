...
mrvl_pp2_main_deinit()
{
    ...
    {
        mrvl_pp2_per_thread_data_t *ptd = vec_elt_at_index(ppm->per_thread_data, i);
        if (ptd->hif)
...
mrvl_pp2_main_init()
{
    ...
    vec_validate_aligned(ppm->per_thread_data, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
...
mrvl_pp2_delete_if(mrvl_pp2_if_t *ppif)
{
    ...
    if (ppif->hw_if_index != ~0) {
        vec_foreach_index(i, ppif->inqs) vnet_hw_interface_unassign_rx_thread(vnm, ppif->hw_if_index, i);
    ...
    vec_foreach(inq, ppif->inqs) if (inq->bpool)
    {
        u32 n_bufs = 0;
        pp2_bpool_get_num_buffs(inq->bpool, &n_bufs);
        while (n_bufs--) {
            struct pp2_buff_inf binf;
            if (pp2_bpool_get_buff(ppm->per_thread_data[0].hif, inq->bpool, &binf) == 0)
...
mrvl_pp2_create_if(mrvl_pp2_create_if_args_t *args)
{
    ...
    if (pp2_netdev_get_ppio_info((char *) args->name, &pp2_id, &port_id)) {
        ...
        args->error = clib_error_return(0, "Invalid interface '%s'", args->name);
        goto error;
    }
    ...
    if (pp2_ppio_get_mac_addr(ppif->ppio, mac_addr)) {
        args->rv    = VNET_API_ERROR_INIT_FAILED;
        args->error = clib_error_return(0, "%s: pp2_ppio_get_mac_addr failed", s);
        goto error;
    }

    args->error = ethernet_register_interface(vnm, mrvl_pp2_device_class.index, ppif->dev_instance, mac_addr,
    ...
    vnet_hw_interface_set_input_node(vnm, ppif->hw_if_index, mrvl_pp2_input_node.index);
    vnet_hw_interface_assign_rx_thread(vnm, ppif->hw_if_index, 0, ~0);
    vnet_hw_interface_set_rx_mode(vnm, ppif->hw_if_index, 0, VNET_HW_INTERFACE_RX_MODE_POLLING);
    vnet_hw_interface_set_flags(vnm, ppif->hw_if_index, VNET_HW_INTERFACE_FLAG_LINK_UP);
    goto done;
...
mrvl_pp2_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (rv)
        return clib_error_return(0, "failed to %s interface", is_up ? "enable" : "disable");
...
static void
mrvl_pp2_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    mrvl_pp2_main_t *ppm    = &mrvl_pp2_main;
    vnet_hw_interface_t *hw = vnet_get_hw_interface(vnm, hw_if_index);
    mrvl_pp2_if_t *ppif     = pool_elt_at_index(ppm->interfaces, hw->dev_instance);
    ...
    ppif->per_interface_next_index = vlib_node_add_next(vlib_get_main(), mrvl_pp2_input_node.index, node_index);
}
