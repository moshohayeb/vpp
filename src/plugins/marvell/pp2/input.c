...
mrvl_pp2_input_trace(vlib_main_t *vm, vlib_node_runtime_t *node, u32 next0, vlib_buffer_t *b0, uword *n_trace,
...
mrvl_pp2_set_buf_data_len_flags(vlib_buffer_t *b, struct pp2_ppio_desc *d, u32 add_flags)
{
    u16 len;
    ...
    if (add_flags & VNET_BUFFER_F_L3_HDR_OFFSET_VALID)
        vnet_buffer(b)->l4_hdr_offset = vnet_buffer(b)->l3_hdr_offset + DM_RXD_GET_IPHDR_LEN(d) * 4;

    return len;
}

static_always_inline u16
mrvl_pp2_next_from_desc(vlib_node_runtime_t *node, struct pp2_ppio_desc *d, vlib_buffer_t *b, u32 *next)
{
    u8 l3_info;
...
mrvl_pp2_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, mrvl_pp2_if_t *ppif, u16 qid)
{
    ...
    mrvl_pp2_per_thread_data_t *ptd = vec_elt_at_index(ppm->per_thread_data, thread_index);
    ...
    if (PREDICT_FALSE(pp2_ppio_recv(ppif->ppio, 0, qid, ptd->descs, &n_desc))) {
        vlib_error_count(vm, node->node_index, MRVL_PP2_INPUT_ERROR_PPIO_RECV, 1);
        n_desc = 0;
    }
    n_rx_packets = n_desc;
    ...
    while (n_desc) {
        ...
        while (n_desc >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_TRUE(ppif->per_interface_next_index == ~0)) {
                ...
                vnet_feature_start_device_input_x2(ppif->sw_if_index, &next0, &next1, b0, b1);
            } else {
            ...
            clib_memcpy(vnet_buffer(b0)->sw_if_index, sw_if_index, sizeof(sw_if_index));
            clib_memcpy(vnet_buffer(b1)->sw_if_index, sw_if_index, sizeof(sw_if_index));
            ...
            if (PREDICT_FALSE(n_trace > 0)) {
                ...
                if (n_trace > 0)
                    mrvl_pp2_input_trace(vm, node, next1, b1, &n_trace, ppif, d + 1);
            }
        ...
        while (n_desc && n_left_to_next) {
            ...
            if (PREDICT_TRUE(ppif->per_interface_next_index == ~0)) {
                n_rx_bytes += mrvl_pp2_next_from_desc(node, d, b0, &next0);
                vnet_feature_start_device_input_x1(ppif->sw_if_index, &next0, b0);
            } else {
                n_rx_bytes += mrvl_pp2_set_buf_data_len_flags(b0, d, 0);
    ...
    if (PREDICT_FALSE(pp2_bpool_get_num_buffs(inq->bpool, &n_bufs))) {
        vlib_error_count(vm, node->node_index, MRVL_PP2_INPUT_ERROR_BPOOL_GET_NUM_BUFFS, 1);
        goto done;
    }
    ...
    while (n_bufs >= MRVL_PP2_BUFF_BATCH_SZ) {
        ...
        if (PREDICT_FALSE(n_alloc == 0)) {
            vlib_error_count(vm, node->node_index, MRVL_PP2_INPUT_ERROR_BUFFER_ALLOC, 1);
            goto done;
        }
        ...
        if (PREDICT_FALSE(pp2_bpool_put_buffs(ptd->hif, ptd->bre, &i))) {
            vlib_error_count(vm, node->node_index, MRVL_PP2_INPUT_ERROR_BPOOL_PUT_BUFFS, 1);
...
uword
mrvl_pp2_input_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_rx                        = 0;
    mrvl_pp2_main_t *ppm            = &mrvl_pp2_main;
    ...
    foreach_device_and_queue(dq, rt->devices_and_queues)
    {
        ...
        if (ppif->flags & MRVL_PP2_IF_F_ADMIN_UP)
            n_rx += mrvl_pp2_device_input_inline(vm, node, frame, ppif, dq->queue_id);
    }
    return n_rx;
}
