...
format_xfilter_trace(u8 *s, va_list *args)
{
    ...
    xfilter_trace_t *t = va_arg(*args, xfilter_trace_t *);
...
#define foreach_xfilter_error _(SWAPPED, "Mac swap packets processed")

typedef enum {
#define _(sym, str) XFILTER_ERROR_ ## sym,
    foreach_xfilter_error
#undef _
    XFILTER_N_ERROR,
} xfilter_error_t;
...
VLIB_NODE_FN(xfilter_node)(vlib_main_t * vm, vlib_node_runtime_t * node, vlib_frame_t * frame)
{
...
/* *INDENT-OFF* */
#ifndef CLIB_MARCH_VARIANT
VLIB_REGISTER_NODE(xfilter_node) = {
    .name         = "xfilter",
    .vector_size  = sizeof(u32),
    .format_trace = format_xfilter_trace,
    .type         = VLIB_NODE_TYPE_INTERNAL,

    .n_errors      = ARRAY_LEN(xfilter_error_strings),
