...
static const u8 icmp4_invmap[] = {[ICMP4_echo_request]         = ICMP4_echo_reply + 1,
...
fa_session_get_timeout_type(acl_main_t *am, fa_session_t *sess)
{
    ...
    u16 masked_flags = sess->tcp_flags_seen.as_u16 & ((TCP_FLAGS_RSTFINACKSYN << 8) + TCP_FLAGS_RSTFINACKSYN);
...
acl_fa_conn_list_add_session(acl_main_t *am, fa_full_session_id_t sess_id, u64 now)
{
    ...
    clib_warning("FA-SESSION-DEBUG: add session id %d on thread %d sw_if_index %d", sess_id.session_index, thread_index,
                 sess->sw_if_index);
#endif
    ...
    if (FA_SESSION_BOGUS_INDEX == pw->fa_conn_list_head[list_id]) {
        ...
        pw->fa_conn_list_head_expiry_time[list_id] = now + fa_session_get_timeout(am, sess);
    }
}

static int
acl_fa_conn_list_delete_session(acl_main_t *am, fa_full_session_id_t sess_id, u64 now)
{
    uword thread_index           = os_get_thread_index();
    acl_fa_per_worker_data_t *pw = &am->per_worker_data[thread_index];
    ...
    if (sess->thread_index != os_get_thread_index()) {
        clib_error("Attempting to delete session belonging to thread %d by thread %d", sess->thread_index, thread_index);
    }
    ...
    if (pw->fa_conn_list_head[sess->link_list_id] == sess_id.session_index) {
        ...
        pw->fa_conn_list_head_expiry_time[sess->link_list_id] = next_expiry_time;
    }
...
acl_fa_restart_timer_for_session(acl_main_t *am, u64 now, fa_full_session_id_t sess_id)
{
    if (acl_fa_conn_list_delete_session(am, sess_id, now)) {
         ...
         */
        aclp_post_session_change_request(am, sess_id.thread_index, sess_id.session_index, ACL_FA_REQ_SESS_RESCHEDULE);
        return 0;
    }
}
...
is_ip6_5tuple(fa_5tuple_t *p5t)
{
    return (p5t->l3_zero_pad[0] | p5t->l3_zero_pad[1] | p5t->l3_zero_pad[2] | p5t->l3_zero_pad[3] |
            p5t->l3_zero_pad[4] | p5t->l3_zero_pad[5]) != 0;
}


always_inline u8
acl_fa_track_session(acl_main_t *am, int is_input, u32 sw_if_index, u64 now, fa_session_t *sess, fa_5tuple_t *pkt_5tuple)
{
...
reverse_l4_u64_slowpath_valid(u64 l4, int is_ip6, u64 *out)
{
    ...
    if (l4i.proto == icmp_protos[is_ip6]) {
        ...
        static const u8 icmp_invmap_size[]    = {sizeof(icmp4_invmap), sizeof(icmp6_invmap)};
        static const u8 icmp_valid_new_size[] = {sizeof(icmp4_valid_new), sizeof(icmp6_valid_new)};
        ...
        int valid_reverse_sess = (type >= 0 && (type <= icmp_valid_new_size[is_ip6]) && (icmp_valid_new[is_ip6][type]) &&
...
always_inline void
reverse_session_add_del_ip6(acl_main_t *am, clib_bihash_kv_40_8_t *pkv, int is_add)
{
...
always_inline void
reverse_session_add_del_ip4(acl_main_t *am, clib_bihash_kv_16_8_t *pkv, int is_add)
{
    clib_bihash_kv_16_8_t kv2;
    kv2.key[0] = ((pkv->key[0] & 0xffffffff) << 32) | ((pkv->key[0] >> 32) & 0xffffffff);
    /* the last u64 needs special treatment (ports, etc.) so we do it last */
    kv2.value = pkv->value;
...
always_inline void
acl_fa_deactivate_session(acl_main_t *am, u32 sw_if_index, fa_full_session_id_t sess_id)
{
    fa_session_t *sess = get_session_ptr(am, sess_id.thread_index, sess_id.session_index);
    ASSERT(sess->thread_index == os_get_thread_index());
    void *oldheap = clib_mem_set_heap(am->acl_mheap);
    if (sess->is_ip6) {
        clib_bihash_add_del_40_8(&am->fa_ip6_sessions_hash, &sess->info.kv_40_8, 0);
        reverse_session_add_del_ip6(am, &sess->info.kv_40_8, 0);
    } else {
        clib_bihash_add_del_16_8(&am->fa_ip4_sessions_hash, &sess->info.kv_16_8, 0);
...
always_inline void
acl_fa_put_session(acl_main_t *am, u32 sw_if_index, fa_full_session_id_t sess_id)
{
    if (sess_id.thread_index != os_get_thread_index()) {
        clib_error("Attempting to delete session belonging to thread %d by thread %d", sess_id.thread_index,
                   os_get_thread_index());
    }
    void *oldheap                = clib_mem_set_heap(am->acl_mheap);
...
always_inline int
acl_fa_two_stage_delete_session(acl_main_t *am, u32 sw_if_index, fa_full_session_id_t sess_id, u64 now)
{
    fa_session_t *sess = get_session_ptr(am, sess_id.thread_index, sess_id.session_index);
    if (sess->deleted) {
...
acl_fa_can_add_session(acl_main_t *am, int is_input, u32 sw_if_index)
{
    ...
    return (curr_sess_count + vec_len(vlib_mains) < am->fa_conn_table_max_entries);
}


always_inline void
acl_fa_try_recycle_session(acl_main_t *am, int is_input, u16 thread_index, u32 sw_if_index, u64 now)
{
    ...
    while ((FA_SESSION_BOGUS_INDEX != sess_id.session_index) && n_recycled < am->fa_max_deleted_sessions_per_interval) {
...
always_inline fa_session_t *
acl_fa_add_session(acl_main_t *am, int is_input, int is_ip6, u32 sw_if_index, u64 now, fa_5tuple_t *p5tuple,
                   u16 current_policy_epoch)
{
    ...
    if (is_ip6) {
        ...
        clib_bihash_add_del_40_8(&am->fa_ip6_sessions_hash, &sess->info.kv_40_8, 1);
    } else {
        reverse_session_add_del_ip4(am, &sess->info.kv_16_8, 1);
        clib_bihash_add_del_16_8(&am->fa_ip4_sessions_hash, &sess->info.kv_16_8, 1);
    }
...
always_inline int
acl_fa_find_session(acl_main_t *am, int is_ip6, u32 sw_if_index0, fa_5tuple_t *p5tuple, u64 *pvalue_sess)
{
    int res = 0;
    if (is_ip6) {
        clib_bihash_kv_40_8_t kv_result;
        res = (clib_bihash_search_inline_2_40_8(&am->fa_ip6_sessions_hash, &p5tuple->kv_40_8, &kv_result) == 0);
        *pvalue_sess = kv_result.value;
    } else {
        clib_bihash_kv_16_8_t kv_result;
        res = (clib_bihash_search_inline_2_16_8(&am->fa_ip4_sessions_hash, &p5tuple->kv_16_8, &kv_result) == 0);
        *pvalue_sess = kv_result.value;
    }
    return res;
}
