...
acl_set_heap(acl_main_t *am)
{
    if (0 == am->acl_mheap) {
        if (0 == am->acl_mheap_size) {
            ...
            u64 per_worker_size    = per_worker_slack + ((u64) am->fa_conn_table_max_entries) * sizeof(fa_session_t);
            ...
            am->acl_mheap_size = per_worker_size_with_slack * tm->n_vlib_mains + bihash_size + main_slack;
        }
        u64 max_possible = ((uword) ~0);
        if (am->acl_mheap_size > max_possible) {
            clib_warning("ACL heap size requested: %lld, max possible %lld", am->acl_mheap_size, max_possible);
        }

        am->acl_mheap = mheap_alloc_with_lock(0 /* use VM */, am->acl_mheap_size, 1 /* locked */);
        if (0 == am->acl_mheap) {
            clib_error("ACL plugin failed to allocate main heap of %U bytes, abort", format_memory_size,
                       am->acl_mheap_size);
        }
    }
...
acl_print_acl_x(acl_vector_print_func_t vpr, vlib_main_t *vm, acl_main_t *am, int acl_index)
{
    ...
    for (j = 0; j < am->acls[acl_index].count; j++) {
        ...
        out0 = format(out0, " src %U/%d", format_ip46_address, &r->src, r->is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4,
                      r->src_prefixlen);
        out0 = format(out0, " dst %U/%d", format_ip46_address, &r->dst, r->is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4,
                      r->dst_prefixlen);
...
increment_policy_epoch(acl_main_t *am, u32 sw_if_index, int is_input)
{
    ...
    u32 **ppolicy_epoch_by_swi =
        is_input ? &am->input_policy_epoch_by_sw_if_index : &am->output_policy_epoch_by_sw_if_index;
...
static int
acl_add_list(u32 count, vl_api_acl_rule_t rules[], u32 *acl_list_index, u8 *tag)
{
    acl_main_t *am = &acl_main;
    acl_list_t *a;
    acl_rule_t *r;
...
u8 ip4_5tuple_mask[] = _("             dmac               smac            etype ") _(ether)
    __ __ __ __ __ __ v __ __ __ __ __ __ v __ __ v _("        v ihl totlen   ") _(0x0000)
        __ __ __ __ _("        ident fl+fo    ") _(0x0004) __ __ __ __ _("       ttl pr checksum ") _(0x0008)
            __ XX __ __ _("        src address    ") _(0x000C) XX XX XX XX _("        dst address    ") _(0x0010)
                XX XX XX XX _("L4 T/U  sport dport    ") _(tcpudp) XX XX XX XX _(padpad) __ __ __ __ _(padpad)
                    __ __ __ __ _(padeth) __ __;

u8 ip6_5tuple_mask[] = _("             dmac               smac            etype ") _(ether)
    __ __ __ __ __ __ v __ __ __ __ __ __ v __ __ v _("        v  tc + flow ") _(0x0000)
        __ __ __ __ _("        plen  nh hl  ") _(0x0004) __ __ XX __ _("        src address  ") _(0x0008)
            XX XX XX XX _(0x000C) XX XX XX XX _(0x0010) XX XX XX XX _(0x0014) XX XX XX XX _("        dst address  ")
                _(0x0018) XX XX XX XX _(0x001C) XX XX XX XX _(0x0020) XX XX XX XX _(0x0024)
                    XX XX XX XX _("L4T/U  sport dport   ") _(tcpudp) XX XX XX XX _(padpad) __ __ __ __ _(padeth) __ __;

u8 dot1q_5tuple_mask[] = _("             dmac               smac          dot1q         etype ") _(ether)
    __ __ __ __ __ __ v __ __ __ __ __ __ v XX XX __ __ v XX XX v _(padpad) __ __ __ __ _(padpad) __ __ __ __ _(padpad)
        __ __ __ __ _(padeth) __ __;

u8 dot1ad_5tuple_mask[] = _("             dmac               smac          dot1ad      dot1q         etype ") _(ether)
    __ __ __ __ __ __ v __ __ __ __ __ __ v XX XX __ __ XX XX __ __ v XX XX v _(padpad) __ __ __ __ _(padpad)
        __ __ __ __ _(padeth) __ __;

u8 ethertype_mask[] = _("             dmac               smac          etype ") _(ether)
    __ __ __ __ __ __ v __ __ __ __ __ __ v XX XX __ __;
...
static int
acl_classify_add_del_table_tiny(vnet_classify_main_t *cm, u8 *mask, u32 mask_len, u32 next_table_index,
                                u32 miss_next_index, u32 *table_index, int is_add)
{
    u32 nbuckets            = 1;
    ...
    int ret       = vnet_classify_add_del_table(cm, skip_mask_ptr, nbuckets, memory_size, skip, match, next_table_index,
                                          miss_next_index, table_index, current_data_flag, current_data_offset, is_add,
                                          1 /* delete_chain */);
    clib_mem_set_heap(oldheap);
    return ret;
}
...
static int
acl_classify_add_del_table_small(vnet_classify_main_t *cm, u8 *mask, u32 mask_len, u32 next_table_index,
                                 u32 miss_next_index, u32 *table_index, int is_add)
{
    u32 nbuckets            = 32;
...
acl_unhook_l2_input_classify(acl_main_t *am, u32 sw_if_index)
{
    ...
    vec_validate_init_empty(am->acl_dot1q_input_classify_table_by_sw_if_index, sw_if_index, ~0);
    vec_validate_init_empty(am->acl_dot1ad_input_classify_table_by_sw_if_index, sw_if_index, ~0);
    vec_validate_init_empty(am->acl_etype_input_classify_table_by_sw_if_index, sw_if_index, ~0);
    ...
    if (am->acl_ip4_input_classify_table_by_sw_if_index[sw_if_index] != ~0) {
        ...
        acl_classify_add_del_table_tiny(cm, ip4_5tuple_mask, sizeof(ip4_5tuple_mask) - 1, ~0,
                                        am->l2_input_classify_next_acl_ip4, &ip4_table_index, 0);
    }
    if (am->acl_ip6_input_classify_table_by_sw_if_index[sw_if_index] != ~0) {
        ...
        acl_classify_add_del_table_tiny(cm, ip6_5tuple_mask, sizeof(ip6_5tuple_mask) - 1, ~0,
                                        am->l2_input_classify_next_acl_ip6, &ip6_table_index, 0);
    }
    if (am->acl_dot1q_input_classify_table_by_sw_if_index[sw_if_index] != ~0) {
        dot1q_table_index = am->acl_dot1q_input_classify_table_by_sw_if_index[sw_if_index];
        am->acl_dot1q_input_classify_table_by_sw_if_index[sw_if_index] = ~0;
        acl_classify_add_del_table_tiny(cm, ip6_5tuple_mask, sizeof(ip6_5tuple_mask) - 1, ~0, ~0, &dot1q_table_index, 0);
    }
    if (am->acl_dot1ad_input_classify_table_by_sw_if_index[sw_if_index] != ~0) {
        dot1ad_table_index = am->acl_dot1ad_input_classify_table_by_sw_if_index[sw_if_index];
        am->acl_dot1ad_input_classify_table_by_sw_if_index[sw_if_index] = ~0;
        acl_classify_add_del_table_tiny(cm, dot1ad_5tuple_mask, sizeof(dot1ad_5tuple_mask) - 1, ~0, ~0,
                                        &dot1ad_table_index, 0);
    }
    if (am->acl_etype_input_classify_table_by_sw_if_index[sw_if_index] != ~0) {
        etype_table_index = am->acl_etype_input_classify_table_by_sw_if_index[sw_if_index];
        am->acl_etype_input_classify_table_by_sw_if_index[sw_if_index] = ~0;
        acl_classify_add_del_table_tiny(cm, ethertype_mask, sizeof(ethertype_mask) - 1, ~0, ~0, &etype_table_index, 0);
    }
    clib_mem_set_heap(oldheap);
    return 0;
}
...
acl_unhook_l2_output_classify(acl_main_t *am, u32 sw_if_index)
{
    ...
    vec_validate_init_empty(am->acl_dot1q_output_classify_table_by_sw_if_index, sw_if_index, ~0);
    vec_validate_init_empty(am->acl_dot1ad_output_classify_table_by_sw_if_index, sw_if_index, ~0);
    vec_validate_init_empty(am->acl_etype_output_classify_table_by_sw_if_index, sw_if_index, ~0);
    ...
    if (am->acl_ip4_output_classify_table_by_sw_if_index[sw_if_index] != ~0) {
        ...
        acl_classify_add_del_table_tiny(cm, ip4_5tuple_mask, sizeof(ip4_5tuple_mask) - 1, ~0,
                                        am->l2_output_classify_next_acl_ip4, &ip4_table_index, 0);
    }
    if (am->acl_ip6_output_classify_table_by_sw_if_index[sw_if_index] != ~0) {
        ...
        acl_classify_add_del_table_tiny(cm, ip6_5tuple_mask, sizeof(ip6_5tuple_mask) - 1, ~0,
                                        am->l2_output_classify_next_acl_ip6, &ip6_table_index, 0);
    }
    if (am->acl_dot1q_output_classify_table_by_sw_if_index[sw_if_index] != ~0) {
        dot1q_table_index = am->acl_dot1q_output_classify_table_by_sw_if_index[sw_if_index];
    ...
    if (am->acl_dot1ad_output_classify_table_by_sw_if_index[sw_if_index] != ~0) {
        dot1ad_table_index = am->acl_dot1ad_output_classify_table_by_sw_if_index[sw_if_index];
    ...
    if (am->acl_etype_output_classify_table_by_sw_if_index[sw_if_index] != ~0) {
        etype_table_index = am->acl_etype_output_classify_table_by_sw_if_index[sw_if_index];
...
acl_add_vlan_session(acl_main_t *am, u32 table_index, u8 is_output, u8 is_dot1ad, u8 is_ip6)
{
    ...
    vnet_classify_add_del_session(cm, table_index, match, next_acl, session_idx, 0, 0, 0, 1);
...
static int
etype_whitelist_add_sessions(acl_main_t *am, u32 sw_if_index, int is_input, u32 etype_table_index)
{
    vnet_classify_main_t *cm = &vnet_classify_main;
    u16 **v   = is_input ? am->input_etype_whitelist_by_sw_if_index : am->output_etype_whitelist_by_sw_if_index;
    ...
    for (i = 0; i < vec_len(whitelist); i++) {
        ...
        rv        = rv || vnet_classify_add_del_session(cm, etype_table_index, match, next, whitelist[i], 0, 0, 0, 1);
    }
...
acl_hook_l2_input_classify(acl_main_t *am, u32 sw_if_index)
{
    ...
    rv = acl_classify_add_del_table_tiny(cm, ip4_5tuple_mask, sizeof(ip4_5tuple_mask) - 1, ~0,
                                         am->l2_input_classify_next_acl_ip4, &ip4_table_index, 1);
    if (rv)
        goto done;

    rv = acl_classify_add_del_table_tiny(cm, ip6_5tuple_mask, sizeof(ip6_5tuple_mask) - 1, ~0,
                                         am->l2_input_classify_next_acl_ip6, &ip6_table_index, 1);
    if (rv) {
        acl_classify_add_del_table_tiny(cm, ip4_5tuple_mask, sizeof(ip4_5tuple_mask) - 1, ~0,
                                        am->l2_input_classify_next_acl_ip4, &ip4_table_index, 0);
        goto done;
    ...
    rv = acl_classify_add_del_table_tiny(cm, dot1ad_5tuple_mask, sizeof(dot1ad_5tuple_mask) - 1, etype_table_index, ~0,
                                         &dot1ad_table_index, 1);
    rv = acl_classify_add_del_table_tiny(cm, dot1q_5tuple_mask, sizeof(dot1q_5tuple_mask) - 1, dot1ad_table_index, ~0,
                                         &dot1q_table_index, 1);
    if (rv) {
        acl_classify_add_del_table_tiny(cm, dot1ad_5tuple_mask, sizeof(dot1ad_5tuple_mask) - 1, ~0, ~0,
                                        &dot1ad_table_index, 0);
    ...
    rv = vnet_l2_input_classify_set_tables(sw_if_index, ip4_table_index, ip6_table_index, dot1q_table_index);
    ...
    if (rv) {
        ...
        acl_classify_add_del_table_tiny(cm, ip6_5tuple_mask, sizeof(ip6_5tuple_mask) - 1, ~0,
                                        am->l2_input_classify_next_acl_ip6, &ip6_table_index, 0);
        acl_classify_add_del_table_tiny(cm, dot1q_5tuple_mask, sizeof(dot1q_5tuple_mask) - 1, ~0, ~0,
                                        &dot1q_table_index, 0);
        acl_classify_add_del_table_tiny(cm, dot1ad_5tuple_mask, sizeof(dot1ad_5tuple_mask) - 1, ~0, ~0,
                                        &dot1ad_table_index, 0);
        goto done;
    ...
    am->acl_dot1q_input_classify_table_by_sw_if_index[sw_if_index]  = dot1q_table_index;
    am->acl_dot1ad_input_classify_table_by_sw_if_index[sw_if_index] = dot1ad_table_index;
    am->acl_dot1ad_input_classify_table_by_sw_if_index[sw_if_index] = dot1ad_table_index;
    am->acl_etype_input_classify_table_by_sw_if_index[sw_if_index]  = etype_table_index;
...
acl_hook_l2_output_classify(acl_main_t *am, u32 sw_if_index)
{
    ...
    rv = acl_classify_add_del_table_tiny(cm, ip4_5tuple_mask, sizeof(ip4_5tuple_mask) - 1, ~0,
                                         am->l2_output_classify_next_acl_ip4, &ip4_table_index, 1);
    if (rv)
        goto done;
    rv = acl_classify_add_del_table_tiny(cm, ip6_5tuple_mask, sizeof(ip6_5tuple_mask) - 1, ~0,
                                         am->l2_output_classify_next_acl_ip6, &ip6_table_index, 1);
    if (rv) {
        acl_classify_add_del_table_tiny(cm, ip4_5tuple_mask, sizeof(ip4_5tuple_mask) - 1, ~0,
                                        am->l2_output_classify_next_acl_ip4, &ip4_table_index, 0);
        goto done;
    ...
    rv = vnet_l2_output_classify_set_tables(sw_if_index, ip4_table_index, ip6_table_index, dot1q_table_index);
    /*
      clib_warning
    ...
    */
    ...
    am->acl_dot1q_output_classify_table_by_sw_if_index[sw_if_index]  = dot1q_table_index;
    am->acl_dot1ad_output_classify_table_by_sw_if_index[sw_if_index] = dot1ad_table_index;
    am->acl_etype_output_classify_table_by_sw_if_index[sw_if_index]  = etype_table_index;
...
static int
acl_interface_in_enable_disable(acl_main_t *am, u32 sw_if_index, int enable_disable)
{
    int rv = 0;

    /* Utterly wrong? */
    if (pool_is_free_index(am->vnet_main->interface_main.sw_interfaces, sw_if_index))
...
static int
acl_interface_out_enable_disable(acl_main_t *am, u32 sw_if_index, int enable_disable)
{
    int rv;
...
static int
acl_interface_inout_enable_disable(acl_main_t *am, u32 sw_if_index, int is_input, int enable_disable)
{
    if (is_input)
                                 ...
                                 int *may_clear_sessions)
{
    ...
    if (am->trace_acl > 255)
        clib_warning("API dbg: acl_interface_set_inout_acl_list: sw_if_index %d is_input %d acl_vec: [%U]", sw_if_index,
                     is_input, format_vec32, vec_acl_list_index, "%d");
    ...
    if (am->trace_acl > 255)
        clib_warning("bitmaps: old seen %U new seen %U changed %U", format_bitmap_hex, old_seen_acl_bitmap,
                     format_bitmap_hex, seen_acl_bitmap, format_bitmap_hex, change_acl_bitmap);
...
static int
acl_set_etype_whitelists(acl_main_t *am, u32 sw_if_index, u16 *vec_in, u16 *vec_out)
{
...
macip_create_classify_tables(acl_main_t *am, u32 macip_acl_index)
{
    ...
    for (i = 0; i < a->count; i++) {
        if (~0 == (match_type_index = macip_find_match_type(mvec, a->rules[i].src_mac_mask, a->rules[i].src_prefixlen,
                                                            a->rules[i].is_ipv6))) {
    ...
    vec_foreach(mt, mvec)
    {
        ...
        if (!is6) {
            ...
            for (tags = 2; tags >= 0; tags--) {
                ...
                if (mt->prefix_len % 8)
                    mask[l3_offset + 14 + (mt->prefix_len / 8)] = 0xff - ((1 << (8 - mt->prefix_len % 8)) - 1);

                mask_len = ((l3_offset + 14 + ((mt->prefix_len + 7) / 8) + (sizeof(u32x4) - 1)) / sizeof(u32x4)) *
                           sizeof(u32x4);
                acl_classify_add_del_table_small(cm, mask, mask_len, last_table, (~0 == last_table) ? 0 : ~0,
                                                 last_tag_table, 1);
                ...
                if (mt->has_egress) {
                    ...
                    acl_classify_add_del_table_small(cm, mask, mask_len, out_last_table,
    ...
    vec_foreach(mt, mvec)
    {
        ...
        if (mt->has_egress) {
            for (tags = 2; tags >= 0; tags--) {
                 ...
                 */
                mask_len =
                    ((l3_dst_offs + ((mt->prefix_len + 7) / 8) + (sizeof(u32x4) - 1)) / sizeof(u32x4)) * sizeof(u32x4);
                acl_classify_add_del_table_small(cm, mask, mask_len, out_last_table, (~0 == out_last_table) ? 0 : ~0,
    ...
    for (i = 0; i < a->count; i++) {
        ...
        match_type_index =
            macip_find_match_type(mvec, a->rules[i].src_mac_mask, a->rules[i].src_prefixlen, a->rules[i].is_ipv6);
        ...
        if (!is6 && (mvec[match_type_index].arp_table_index != ~0)) {
            ...
            for (tags = 2; tags >= 0; tags--) {
                ...
                vnet_classify_add_del_session(cm, tag_table, mask, a->rules[i].is_permit ? ~0 : 0, i, 0, action,
                                              metadata, 1);
            }
        }
...
macip_maybe_apply_unapply_classifier_tables(acl_main_t *am, u32 acl_index, int is_apply)
{
    ...
    for (i = 0; i < vec_len(am->macip_acl_by_sw_if_index); i++)
        if (vec_elt(am->macip_acl_by_sw_if_index, i) == acl_index) {
            rv0 = vnet_set_input_acl_intfc(am->vlib_main, i, a->ip4_table_index, a->ip6_table_index, a->l2_table_index,
                                           is_apply);
            /* return the first unhappy outcome but make try to plough through. */
            rv  = rv || rv0;
            rv0 = vnet_set_output_acl_intfc(am->vlib_main, i, a->out_ip4_table_index, a->out_ip6_table_index,
...
static int
macip_acl_add_list(u32 count, vl_api_macip_acl_rule_t rules[], u32 *acl_list_index, u8 *tag)
{
    acl_main_t *am = &acl_main;
    macip_acl_list_t *a;
    macip_acl_rule_t *r;
...
macip_acl_interface_del_acl(acl_main_t *am, u32 sw_if_index)
{
    ...
    rv = vnet_set_input_acl_intfc(am->vlib_main, sw_if_index, a->ip4_table_index, a->ip6_table_index, a->l2_table_index,
                                  0);
    rv |= vnet_set_output_acl_intfc(am->vlib_main, sw_if_index, a->out_ip4_table_index, a->out_ip6_table_index,
                                    a->out_l2_table_index, 0);
...
static void
send_acl_details(acl_main_t *am, vl_api_registration_t *reg, acl_list_t *acl, u32 context)
{
...
vl_api_acl_dump_t_handler(vl_api_acl_dump_t *mp)
{
    ...
    if (mp->acl_index == ~0) {
        /* *INDENT-OFF* */
        /* Just dump all ACLs */
        pool_foreach(acl, am->acls, ({ send_acl_details(am, reg, acl, mp->context); }));
        /* *INDENT-ON* */
    } else {
...
static void
send_acl_interface_list_details(acl_main_t *am, vl_api_registration_t *reg, u32 sw_if_index, u32 context)
{
...
static void
send_macip_acl_details(acl_main_t *am, vl_api_registration_t *reg, macip_acl_list_t *acl, u32 context)
{
    ...
    if (acl) {
        ...
        for (i = 0; i < acl->count; i++) {
            ...
            if (r->is_ipv6)
                memcpy(rules[i].src_ip_addr, &r->src_ip_addr.ip6, sizeof(r->src_ip_addr.ip6));
            else
                memcpy(rules[i].src_ip_addr, &r->src_ip_addr.ip4, sizeof(r->src_ip_addr.ip4));
...
vl_api_macip_acl_dump_t_handler(vl_api_macip_acl_dump_t *mp)
{
    ...
    if (mp->acl_index == ~0) {
        ...
        pool_foreach(acl, am->macip_acls, ({ send_macip_acl_details(am, reg, acl, mp->context); }));
        /* *INDENT-ON* */
    } else {
...
static void
send_macip_acl_interface_list_details(acl_main_t *am, vl_api_registration_t *reg, u32 sw_if_index, u32 acl_index,
                                      u32 context)
{
...
vl_api_macip_acl_interface_list_dump_t_handler(vl_api_macip_acl_interface_list_dump_t *mp)
{
    ...
    if (sw_if_index == ~0) {
        ...
        {
            if (~0 != am->macip_acl_by_sw_if_index[sw_if_index]) {
                send_macip_acl_interface_list_details(am, reg, sw_if_index, am->macip_acl_by_sw_if_index[sw_if_index],
                                                      mp->context);
            }
        }
    } else {
...
static void
vl_api_acl_interface_set_etype_whitelist_t_handler(vl_api_acl_interface_set_etype_whitelist_t *mp)
{
    acl_main_t *am = &acl_main;
...
static void
send_acl_interface_etype_whitelist_details(acl_main_t *am, vl_api_registration_t *reg, u32 sw_if_index, u32 context)
{
    ...
    if (intf_has_etype_whitelist(am, sw_if_index, 0))
        whitelist_out = vec_elt(am->output_etype_whitelist_by_sw_if_index, sw_if_index);

    if (intf_has_etype_whitelist(am, sw_if_index, 1))
        whitelist_in = vec_elt(am->input_etype_whitelist_by_sw_if_index, sw_if_index);
...
static void
vl_api_acl_interface_etype_whitelist_dump_t_handler(vl_api_acl_interface_list_dump_t *mp)
{
    acl_main_t *am = &acl_main;
    vnet_sw_interface_t *swif;
    vnet_interface_main_t *im = &am->vnet_main->interface_main;
    ...
    if (mp->sw_if_index == ~0) {
        ...
        sw_if_index = ntohl(mp->sw_if_index);
        if (!pool_is_free_index(im->sw_interfaces, sw_if_index))
            send_acl_interface_etype_whitelist_details(am, reg, sw_if_index, mp->context);
    }
}
...
acl_plugin_api_hookup(vlib_main_t *vm)
{
    acl_main_t *am = &acl_main;
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + am->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,               \
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(apim, #n "_" #crc, id + am->msg_id_base);
...
acl_setup_fa_nodes(void)
{
    ...
    am->l2_input_classify_next_acl_ip4 = vlib_node_add_next_with_slot(vm, n->index, n4->index, ~0);
    am->l2_input_classify_next_acl_ip6 = vlib_node_add_next_with_slot(vm, n->index, n6->index, ~0);

    feat_bitmap_init_next_nodes(vm, n4->index, L2INPUT_N_FEAT, l2input_get_feat_names(),
                                am->fa_acl_in_ip4_l2_node_feat_next_node_index);

    feat_bitmap_init_next_nodes(vm, n6->index, L2INPUT_N_FEAT, l2input_get_feat_names(),
    ...
    am->l2_output_classify_next_acl_ip4 = vlib_node_add_next_with_slot(vm, n->index, n4->index, ~0);
    am->l2_output_classify_next_acl_ip6 = vlib_node_add_next_with_slot(vm, n->index, n6->index, ~0);

    feat_bitmap_init_next_nodes(vm, n4->index, L2OUTPUT_N_FEAT, l2output_get_feat_names(),
                                am->fa_acl_out_ip4_l2_node_feat_next_node_index);

    feat_bitmap_init_next_nodes(vm, n6->index, L2OUTPUT_N_FEAT, l2output_get_feat_names(),
...
static clib_error_t *
acl_set_aclplugin_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "session")) {
        if (unformat(input, "table")) {
            ...
            if (unformat(input, "max-entries")) {
                if (!unformat(input, "%u", &val)) {
                    error = clib_error_return(0, "expecting maximum number of entries, got `%U`", format_unformat_error,
                                              input);
                    goto done;
                } else {
            ...
            if (unformat(input, "hash-table-buckets")) {
                if (!unformat(input, "%u", &val)) {
                    error = clib_error_return(0, "expecting maximum number of hash table buckets, got `%U`",
            ...
            if (unformat(input, "hash-table-memory")) {
                if (!unformat(input, "%U", unformat_memory_size, &memory_size)) {
                    error = clib_error_return(0, "expecting maximum amount of hash table memory, got `%U`",
                                              format_unformat_error, input);
                    goto done;
                } else {
            ...
            if (unformat(input, "udp")) {
                if (unformat(input, "idle")) {
                    if (!unformat(input, "%u", &timeout)) {
                        error = clib_error_return(0, "expecting timeout value in seconds, got `%U`",
                                                  format_unformat_error, input);
                        goto done;
                    } else {
...
my_macip_acl_rule_t_pretty_format(u8 *out, va_list *args)
{
    ...
    out = format(out, "%s action %d ip %U/%d mac %U mask %U", a->is_ipv6 ? "ipv6" : "ipv4", a->is_permit,
                 format_ip46_address, &a->src_ip_addr, a->is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, a->src_prefixlen,
...
macip_acl_print(acl_main_t *am, u32 macip_acl_index)
{
    ...
    vlib_cli_output(vm, "  out_ip4_table_index %d, out_ip6_table_index %d, out_l2_table_index %d\n",
                    a->out_ip4_table_index, a->out_ip6_table_index, a->out_l2_table_index);
    for (i = 0; i < vec_len(a->rules); i++)
        vlib_cli_output(vm, "    rule %d: %U\n", i, my_macip_acl_rule_t_pretty_format, vec_elt_at_index(a->rules, i));
}

static clib_error_t *
acl_show_aclplugin_macip_acl_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    acl_main_t *am      = &acl_main;
    int i;
...
static clib_error_t *
acl_show_aclplugin_macip_interface_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    acl_main_t *am      = &acl_main;
    int i;
...
static clib_error_t *
acl_show_aclplugin_acl_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    acl_main_t *am      = &acl_main;
...
static clib_error_t *
acl_show_aclplugin_lookup_context_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
...
static clib_error_t *
acl_show_aclplugin_lookup_user_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
...
acl_plugin_show_interface(acl_main_t *am, u32 sw_if_index, int show_acl, int detail)
{
    ...
    u32 *pj;
    for (swi = 0;
         (swi < vec_len(am->input_acl_vec_by_sw_if_index)) || (swi < vec_len(am->output_acl_vec_by_sw_if_index)); swi++) {
...
static clib_error_t *
acl_show_aclplugin_decode_5tuple_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    u64 five_tuple[6]   = {0, 0, 0, 0, 0, 0};

    if (unformat(input, "%llx %llx %llx %llx %llx %llx", &five_tuple[0], &five_tuple[1], &five_tuple[2], &five_tuple[3],
                 &five_tuple[4], &five_tuple[5]))
...
static clib_error_t *
acl_show_aclplugin_interface_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    acl_main_t *am      = &acl_main;
...
static clib_error_t *
acl_show_aclplugin_memory_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    acl_main_t *am      = &acl_main;
...
static void
acl_plugin_show_sessions(acl_main_t *am, u32 show_session_thread_id, u32 show_session_session_index)
{
    vlib_main_t *vm = am->vlib_main;
    u16 wk;
    vnet_interface_main_t *im = &am->vnet_main->interface_main;
    ...
    for (wk = 0; wk < vec_len(am->per_worker_data); wk++) {
        ...
        if (show_session_thread_id == wk && show_session_session_index < pool_len(pw->fa_sessions_pool)) {
            ...
            u64 *m             = (u64 *) &sess->info;
            vlib_cli_output(vm, "    info: %016llx %016llx %016llx %016llx %016llx %016llx", m[0], m[1], m[2], m[3],
                            m[4], m[5]);
        ...
        pool_foreach(swif, im->sw_interfaces, ({
                         ...
                         u64 n_adds      = sw_if_index < vec_len(pw->fa_session_adds_by_sw_if_index) ?
                                          pw->fa_session_adds_by_sw_if_index[sw_if_index] :
                                          0;
                         u64 n_dels = sw_if_index < vec_len(pw->fa_session_dels_by_sw_if_index) ?
                                          pw->fa_session_dels_by_sw_if_index[sw_if_index] :
                                          0;
                         u64 n_epoch_changes = sw_if_index < vec_len(pw->fa_session_epoch_change_by_sw_if_index) ?
                                                   pw->fa_session_epoch_change_by_sw_if_index[sw_if_index] :
                                                   0;
                         vlib_cli_output(vm, "    sw_if_index %d: add %lu - del %lu = %lu; epoch chg: %lu", sw_if_index,
                                         n_adds, n_dels, n_adds - n_dels, n_epoch_changes);
                     }));
...
static clib_error_t *
acl_show_aclplugin_sessions_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    acl_main_t *am      = &acl_main;
...
static clib_error_t *
acl_show_aclplugin_tables_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
...
static clib_error_t *
acl_clear_aclplugin_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    acl_main_t *am      = &acl_main;
...
acl_plugin_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "connection hash memory %d", &conn_table_hash_memory_size))
        ...
        else if (unformat(input, "hash lookup hash memory %d", &hash_lookup_hash_memory))
...
acl_init(vlib_main_t *vm)
{
    ...
    am->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
