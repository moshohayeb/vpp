...
get_current_policy_epoch(acl_main_t *am, int is_input, u32 sw_if_index0)
{
    ...
    u16 current_policy_epoch = sw_if_index0 < vec_len(*p_epoch_vec) ? vec_elt(*p_epoch_vec, sw_if_index0) :
                                                                      (is_input * FA_POLICY_EPOCH_IS_INPUT);
    return current_policy_epoch;
}

always_inline uword
acl_fa_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip6, int is_input,
               int is_l2_path, u32 *l2_feat_next_node_index, vlib_node_registration_t *acl_fa_node)
{
    u32 n_left, *from;
    ...
    while (n_left > 0) {
        ...
        acl_fill_5tuple(&acl_main, sw_if_index0, b[0], is_ip6, is_input, is_l2_path, &fa_5tuple);

#ifdef FA_NODE_VERBOSE_DEBUG
        clib_warning("ACL_FA_NODE_DBG: packet 5-tuple %016llx %016llx %016llx %016llx %016llx %016llx",
                     fa_5tuple.kv.key[0], fa_5tuple.kv.key[1], fa_5tuple.kv.key[2], fa_5tuple.kv.key[3],
                     fa_5tuple.kv.key[4], fa_5tuple.kv.value);
#endif
        ...
        if (acl_fa_ifc_has_sessions(am, sw_if_index0)) {
            u64 value_sess = ~0ULL;
            if (acl_fa_find_session(am, is_ip6, sw_if_index0, &fa_5tuple, &value_sess) && (value_sess != ~0ULL)) {
                ...
                action               = acl_fa_track_session(am, is_input, sw_if_index0, now, sess, &fa_5tuple);
                ...
                if (PREDICT_FALSE(old_timeout_type != new_timeout_type)) {
                    ...
                    trace_bitmap |= 0x00010000 + ((0xff & old_timeout_type) << 8) + (0xff & new_timeout_type);
                }
                /*
        ...
        if (acl_check_needed) {
            ...
            acl_plugin_match_5tuple_inline(&acl_main, lc_index0, (fa_5tuple_opaque_t *) &fa_5tuple, is_ip6, &action,
                                           &match_acl_pos, &match_acl_in_index, &match_rule_index, &trace_bitmap);
            error0 = action;
            if (1 == action)
                pkts_acl_permit += 1;
            if (2 == action) {
                if (!acl_fa_can_add_session(am, is_input, sw_if_index0))
                    acl_fa_try_recycle_session(am, is_input, thread_index, sw_if_index0, now);

                if (acl_fa_can_add_session(am, is_input, sw_if_index0)) {
                    fa_session_t *sess =
                        acl_fa_add_session(am, is_input, is_ip6, sw_if_index0, now, &fa_5tuple, current_policy_epoch);
                    acl_fa_track_session(am, is_input, sw_if_index0, now, sess, &fa_5tuple);
                    pkts_new_session += 1;
                } else {
                    action = 0;
...
#ifdef FA_NODE_VERBOSE_DEBUG
        clib_warning("ACL_FA_NODE_DBG: sw_if_index %d lc_index %d action %d acl_index %d rule_index %d", sw_if_index0,
                     lc_index0, action, match_acl_in_index, match_rule_index);
#endif
    ...
    vlib_node_increment_counter(vm, acl_fa_node->index, ACL_FA_ERROR_ACL_CHECK, pkts_acl_checked);
    vlib_node_increment_counter(vm, acl_fa_node->index, ACL_FA_ERROR_ACL_PERMIT, pkts_acl_permit);
    vlib_node_increment_counter(vm, acl_fa_node->index, ACL_FA_ERROR_ACL_NEW_SESSION, pkts_new_session);
    vlib_node_increment_counter(vm, acl_fa_node->index, ACL_FA_ERROR_ACL_EXIST_SESSION, pkts_exist_session);
    vlib_node_increment_counter(vm, acl_fa_node->index, ACL_FA_ERROR_ACL_RESTART_SESSION_TIMER,
                                pkts_restart_session_timer);
...
{
    acl_main_t *am = &acl_main;
    return acl_fa_node_fn(vm, node, frame, 1, 1, 1, am->fa_acl_in_ip6_l2_node_feat_next_node_index, &acl_in_l2_ip6_node);
}
...
{
    acl_main_t *am = &acl_main;
    return acl_fa_node_fn(vm, node, frame, 0, 1, 1, am->fa_acl_in_ip4_l2_node_feat_next_node_index, &acl_in_l2_ip4_node);
}
...
format_fa_5tuple(u8 *s, va_list *args)
{
    ...
    if (p5t->pkt.is_ip6)
        return format(s,
                      "lc_index %d (lsb16 of sw_if_index %d) l3 %s%s %U -> %U"
                      " l4 proto %d l4_valid %d port %d -> %d tcp flags (%s) %02x rsvd %x",
                      p5t->pkt.lc_index, p5t->l4.lsb_of_sw_if_index, "ip6",
                      p5t->pkt.is_nonfirst_fragment ? " non-initial fragment" : "", format_ip6_address,
                      &p5t->ip6_addr[0], format_ip6_address, &p5t->ip6_addr[1], p5t->l4.proto, p5t->pkt.l4_valid,
                      p5t->l4.port[0], p5t->l4.port[1], p5t->pkt.tcp_flags_valid ? "valid" : "invalid",
                      p5t->pkt.tcp_flags, p5t->pkt.flags_reserved);
    else
        return format(s,
                      "lc_index %d (lsb16 of sw_if_index %d) l3 %s%s %U -> %U"
                      " l4 proto %d l4_valid %d port %d -> %d tcp flags (%s) %02x rsvd %x",
                      p5t->pkt.lc_index, p5t->l4.lsb_of_sw_if_index, "ip4",
                      p5t->pkt.is_nonfirst_fragment ? " non-initial fragment" : "", format_ip4_address,
                      &p5t->ip4_addr[0], format_ip4_address, &p5t->ip4_addr[1], p5t->l4.proto, p5t->pkt.l4_valid,
                      p5t->l4.port[0], p5t->l4.port[1], p5t->pkt.tcp_flags_valid ? "valid" : "invalid",
                      p5t->pkt.tcp_flags, p5t->pkt.flags_reserved);
}
...
format_acl_plugin_trace(u8 *s, va_list *args)
{
    ...
    s = format(
        ...
        t->lc_index, t->sw_if_index, t->next_index, t->action, t->match_acl_in_index, t->match_rule_index,
        t->trace_bitmap, t->packet_info[0], t->packet_info[1], t->packet_info[2], t->packet_info[3], t->packet_info[4],
        t->packet_info[5]);
