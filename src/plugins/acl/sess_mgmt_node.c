...
format_ip6_session_bihash_kv(u8 *s, va_list *args)
{
    ...
    return (format(s,
                   "l3 %U -> %U"
                   " l4 lsb_of_sw_if_index %d proto %d l4_is_input %d l4_slow_path %d l4_reserved0 %d port %d -> %d | "
                   "sess id %d thread id %d epoch %04x",
                   format_ip6_address, &a5t.ip6_addr[0], format_ip6_address, &a5t.ip6_addr[1],
                   a5t.l4.lsb_of_sw_if_index, a5t.l4.proto, a5t.l4.is_input, a5t.l4.is_slowpath, a5t.l4.reserved0,
                   a5t.l4.port[0], a5t.l4.port[1], sess->session_index, sess->thread_index, sess->intf_policy_epoch));
}
...
format_ip4_session_bihash_kv(u8 *s, va_list *args)
{
    ...
    return (format(s,
                   ...
                   " l4 lsb_of_sw_if_index %d proto %d l4_is_input %d l4_slow_path %d l4_reserved0 %d port %d -> %d | "
                   "sess id %d thread id %d epoch %04x",
                   format_ip4_address, &a5t.ip4_addr[0], format_ip4_address, &a5t.ip4_addr[1],
                   a5t.l4.lsb_of_sw_if_index, a5t.l4.proto, a5t.l4.is_input, a5t.l4.is_slowpath, a5t.l4.reserved0,
                   a5t.l4.port[0], a5t.l4.port[1], sess->session_index, sess->thread_index, sess->intf_policy_epoch));
}
...
acl_fa_verify_init_sessions(acl_main_t *am)
{
    if (!am->fa_sessions_hash_is_initialized) {
        ...
        clib_bihash_init_40_8(&am->fa_ip6_sessions_hash, "ACL plugin FA IPv6 session bihash",
                              am->fa_conn_table_hash_num_buckets, am->fa_conn_table_hash_memory_size);
        clib_bihash_set_kvp_format_fn_40_8(&am->fa_ip6_sessions_hash, format_ip6_session_bihash_kv);

        clib_bihash_init_16_8(&am->fa_ip4_sessions_hash, "ACL plugin FA IPv4 session bihash",
                              am->fa_conn_table_hash_num_buckets, am->fa_conn_table_hash_memory_size);
...
acl_fa_check_idle_sessions(acl_main_t *am, u16 thread_index, u64 now)
{
        ...
        for (tt = 0; tt < ACL_N_TIMEOUTS; tt++) {
            ...
            while (n_expired < am->fa_max_deleted_sessions_per_interval) {
                ...
                if (am->trace_sessions > 3) {
                    elog_acl_maybe_trace_X3(am, "acl_fa_check_idle_sessions: expire session %d in list %d on thread %d",
                                            "i4i4i4", (u32) fsid.session_index, (u32) tt, (u32) thread_index);
                }
    ...
    vec_foreach(psid, pw->expired)
    {
        ...
        if (!pool_is_free_index(pw->fa_sessions_pool, fsid.session_index)) {
            ...
            if (am->trace_sessions > 3) {
                elog_acl_maybe_trace_X2(am, "acl_fa_check_idle_sessions: now %lu sess_timeout_time %lu", "i8i8", now,
                                        sess_timeout_time);
                elog_acl_maybe_trace_X4(am, "acl_fa_check_idle_sessions: session %d sw_if_index %d timeout_passed %d clearing_interface %d",
            ...
            if (timeout_passed || clearing_interface) {
                if (acl_fa_two_stage_delete_session(am, sw_if_index, fsid, now)) {
                    if (am->trace_sessions > 3) {
                        elog_acl_maybe_trace_X2(am, "acl_fa_check_idle_sessions: deleted session %d sw_if_index %d",
                    ...
                    if (am->trace_sessions > 3) {
                        elog_acl_maybe_trace_X2(am, "acl_fa_check_idle_sessions: session %d sw_if_index %d marked as deleted, put to purgatory",
                                                "i4i4", (u32) fsid.session_index, (u32) sess->sw_if_index);
                    }
                }
            } else
            ...
            {
                if (am->trace_sessions > 3) {
                    elog_acl_maybe_trace_X2(am, "acl_fa_check_idle_sessions: restart timer for session %d sw_if_index %d",
                                            "i4i4", (u32) fsid.session_index, (u32) sess->sw_if_index);
                }
    ...
    elog_acl_maybe_trace_X1(am, "acl_fa_check_idle_sessions: done, total sessions expired: %d", "i4",
                            (u32) total_expired);
...
send_one_worker_interrupt(vlib_main_t *vm, acl_main_t *am, int thread_index)
{
    ...
    if (!pw->interrupt_is_pending) {
        ...
        vlib_node_set_interrupt_pending(vlib_mains[thread_index], acl_fa_worker_session_cleaner_process_node.index);
...
void
aclp_post_session_change_request(acl_main_t *am, u32 target_thread, u32 target_session, u32 request_type)
{
...
void
aclp_swap_wip_and_pending_session_change_requests(acl_main_t *am, u32 target_thread)
{
    acl_fa_per_worker_data_t *pw = &am->per_worker_data[target_thread];
    u64 *tmp;
    clib_spinlock_lock_if_init(&pw->pending_session_change_request_lock);
 ...
 */
static uword
acl_fa_worker_conn_cleaner_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    if (pw->clear_in_process) {
        if (0 == pw->swipe_end_time) {
            ...
            if ((pw->pending_clear_sw_if_index_bitmap == 0) || (pw->serviced_sw_if_index_bitmap == 0)) {
                elog_acl_maybe_trace_X1(am, "acl_fa_worker_conn_cleaner: now %lu, someone tried to call clear but one of the bitmaps are empty",
                                        "i8", now);
                ...
                clib_warning("WORKER-CLEAR: (before and) swiping sw-if-index bitmap: %U, my serviced bitmap %U",
                             format_bitmap_hex, pw->pending_clear_sw_if_index_bitmap, format_bitmap_hex,
                             pw->serviced_sw_if_index_bitmap);
#endif
                pw->pending_clear_sw_if_index_bitmap =
                    clib_bitmap_and(pw->pending_clear_sw_if_index_bitmap, pw->serviced_sw_if_index_bitmap);
            }

            if (clib_bitmap_is_zero(pw->pending_clear_sw_if_index_bitmap)) {
                /* if the cross-section is a zero vector, no need to do anything. */
                elog_acl_maybe_trace_X1(am, "acl_fa_worker_conn_cleaner: now %lu, clearing done, nothing to do", "i8",
                                        now);
                pw->clear_in_process = 0;
                pw->swipe_end_time   = 0;
            } else {
#ifdef FA_NODE_VERBOSE_DEBUG
                clib_warning("WORKER-CLEAR: swiping sw-if-index bitmap: %U, my serviced bitmap %U", format_bitmap_hex,
                             pw->pending_clear_sw_if_index_bitmap, format_bitmap_hex, pw->serviced_sw_if_index_bitmap);
#endif
    ...
    elog_acl_maybe_trace_X2(am, "acl_fa_worker_conn_cleaner: checked %d sessions (clear_in_process: %d)", "i4i4",
                            (u32) num_expired, (u32) pw->clear_in_process);
    if (pw->clear_in_process) {
        if (pw->swipe_end_time == 0) {
            ...
            elog_acl_maybe_trace_X1(am, "acl_fa_worker_conn_cleaner: now %lu, more work to do - requesting interrupt",
                                    "i8", now);
        ...
        elog_acl_maybe_trace_X3(am, "acl_fa_worker_conn_cleaner: now %lu, interrupt needed: %u, interrupt unwanted: %u",
                                "i8i4i4", now, ((u32) pw->interrupt_is_needed), ((u32) pw->interrupt_is_unwanted));
    }
...
static uword
acl_fa_session_cleaner_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    acl_main_t *am = &acl_main;
    u64 now;
    ...
    while (1) {
        ...
        for (ti = 0; ti < vec_len(vlib_mains); ti++) {
            ...
            for (tt = 0; tt < vec_len(pw->fa_conn_list_head); tt++) {
                ...
                if ((head_expiry < next_expire) && !pw->interrupt_is_pending) {
                    elog_acl_maybe_trace_X3(am, "acl_fa_session_cleaner_process: now %lu, worker: %u tt: %u", "i8i2i2",
                                            now, ti, tt);
                    elog_acl_maybe_trace_X2(am, "acl_fa_session_cleaner_process: head expiry: %lu, is earlier than curr next expire: %lu",
        ...
        if (!has_pending_conns && (0 == am->fa_total_enabled_count)) {
            ...
            if (timeout <= 0) {
                ...
                elog_acl_maybe_trace_X2(am, "acl_conn_cleaner: now %lu entering wait with timeout %.6f sec", "i8f8",
                                        now, timeout);
        ...
        case ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX: {
            ...
            int clear_all = 0;
            now           = clib_cpu_time_now();
            elog_acl_maybe_trace_X1(am, "acl_fa_session_cleaner_process: now %lu, received ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX",
                                    "i8", now);
            vec_foreach(sw_if_index0, event_data)
            {
                am->fa_cleaner_cnt_delete_by_sw_index++;
                elog_acl_maybe_trace_X1(am, "acl_fa_session_cleaner_process: ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX %u",
                                        "i4", *sw_if_index0);
                if (*sw_if_index0 == ~0) {
                    clear_all = 1;
                } else {
            ...
            acl_log_info("ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX bitmap: %U, clear_all: %u", format_bitmap_hex,
                         clear_sw_if_index_bitmap, clear_all);
            vec_foreach(pw0, am->per_worker_data)
            {
                CLIB_MEMORY_BARRIER();
                while (pw0->clear_in_process) {
                    CLIB_MEMORY_BARRIER();
                    elog_acl_maybe_trace_X1(am, "ACL_FA_NODE_CLEAN: waiting previous cleaning cycle to finish on %u",
                ...
                if (pw0->clear_in_process) {
                    ...
                    acl_log_info("ACL_FA_CLEANER: thread %u, pending clear bitmap: %U", (am->per_worker_data - pw0),
                                 format_bitmap_hex, pw0->pending_clear_sw_if_index_bitmap);
            ...
            vec_foreach(pw0, am->per_worker_data)
            {
                ...
                while (pw0->clear_in_process) {
                    CLIB_MEMORY_BARRIER();
                    elog_acl_maybe_trace_X1(am, "ACL_FA_NODE_CLEAN: waiting for my cleaning cycle to finish on %u",
                                            "i4", (u32)(pw0 - am->per_worker_data));
                    vlib_process_suspend(vm, 0.0001);
                    if (pw0->interrupt_is_needed) {
                        send_one_worker_interrupt(vm, am, (pw0 - am->per_worker_data));
...
#endif
            vlib_node_increment_counter(vm, acl_fa_session_cleaner_process_node.index,
