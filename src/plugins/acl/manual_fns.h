...
vl_api_acl_rule_t_print(vl_api_acl_rule_t *a, void *handle)
{
    ...
    s = format(s, "    sport %d-%d dport %d-%d \\\n", clib_net_to_host_u16(a->srcport_or_icmptype_first),
               clib_net_to_host_u16(a->srcport_or_icmptype_last), clib_net_to_host_u16(a->dstport_or_icmpcode_first),
               clib_net_to_host_u16(a->dstport_or_icmpcode_last));

    s = format(s, "    tcpflags %u mask %u, \\", a->tcp_flags_value, a->tcp_flags_mask);
    PRINT_S;
    return handle;
}
...
vl_api_macip_acl_rule_t_print(vl_api_macip_acl_rule_t *a, void *handle)
{
    ...
    s = format(0, "  %s %s \\\n", a->is_ipv6 ? "ipv6" : "ipv4", a->is_permit ? "permit" : "deny");

    s = format(s, "    src mac %U mask %U \\\n", format_ethernet_address, a->src_mac, format_ethernet_address,
               a->src_mac_mask);

    if (a->is_ipv6)
        s = format(s, "    src ip %U/%d, \\", format_ip6_address, a->src_ip_addr, a->src_ip_prefix_len);
    else
        s = format(s, "    src ip %U/%d, \\", format_ip4_address, a->src_ip_addr, a->src_ip_prefix_len);
...
vl_api_acl_add_replace_t_print(vl_api_acl_add_replace_t *a, void *handle)
{
    ...
    s = format(s, "SCRIPT: acl_add_replace %d count %d ", acl_index, count);
...
vl_api_acl_del_t_print(vl_api_macip_acl_del_t *a, void *handle)
{
    ...
    s = format(0, "SCRIPT: acl_del %d ", clib_host_to_net_u32(a->acl_index));
...
vl_api_acl_details_t_print(vl_api_acl_details_t *a, void *handle)
{
    ...
    s = format(s, "acl_details index %d count %d ", acl_index, count);
...
static inline void *
vl_api_macip_acl_details_t_print(vl_api_macip_acl_details_t *a, void *handle)
{
    u8 *s = 0;
    int i;
    u32 acl_index = clib_net_to_host_u32(a->acl_index);
    ...
    s = format(s, "macip_acl_details index %d count %d ", acl_index, count);
...
vl_api_macip_acl_add_replace_t_print(vl_api_macip_acl_add_replace_t *a, void *handle)
{
    ...
    s = format(s, "SCRIPT: macip_acl_add_replace %d count %d ", acl_index, count);
    if (a->tag[0])
        s = format(s, "tag %s ", a->tag);
...
static inline void *
vl_api_acl_interface_set_acl_list_t_print(vl_api_acl_interface_set_acl_list_t *a, void *handle)
{
    u8 *s;
    int i;

    s = format(0, "SCRIPT: acl_interface_set_acl_list sw_if_index %d count %d\n", clib_net_to_host_u32(a->sw_if_index),
               (u32) a->count);
...
static inline void *
vl_api_acl_interface_set_etype_whitelist_t_print(vl_api_acl_interface_set_etype_whitelist_t *a, void *handle)
{
    u8 *s;
    int i;

    s = format(0, "SCRIPT: acl_interface_set_etype_whitelist sw_if_index %d count %d\n",
...
static inline void *
vl_api_acl_interface_add_del_t_print(vl_api_acl_interface_add_del_t *a, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: acl_interface_add_del sw_if_index %d acl %d ", clib_net_to_host_u32(a->sw_if_index),
               clib_net_to_host_u32(a->acl_index));
    s = format(s, "%s %s", a->is_input ? "input" : "output", a->is_add ? "add" : "del");
...
static inline void *
vl_api_macip_acl_interface_add_del_t_print(vl_api_macip_acl_interface_add_del_t *a, void *handle)
{
    u8 *s;

    s = format(0, "SCRIPT: macip_acl_interface_add_del sw_if_index %d acl_index %d ",
               clib_net_to_host_u32(a->sw_if_index), clib_net_to_host_u32(a->acl_index));
...
vl_api_macip_acl_del_t_print(vl_api_macip_acl_del_t *a, void *handle)
{
    ...
    s = format(0, "SCRIPT: macip_acl_del %d ", clib_host_to_net_u32(a->acl_index));
