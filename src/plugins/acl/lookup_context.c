...
static u32
get_acl_user_id(acl_main_t *am, char *user_module_name, char *val1_label, char *val2_label)
{
...
static u32
acl_plugin_register_user_module(char *user_module_name, char *val1_label, char *val2_label)
{
    acl_main_t *am = &acl_main;
...
static void
acl_plugin_fill_5tuple(u32 lc_index, vlib_buffer_t *b0, int is_ip6, int is_input, int is_l2_path,
...
static int
acl_plugin_match_5tuple(u32 lc_index, fa_5tuple_opaque_t *pkt_5tuple, int is_ip6, u8 *r_action, u32 *r_acl_pos_p,
...
acl_plugin_show_lookup_user(u32 user_index)
{
    ...
    pool_foreach(auser, am->acl_users, ({
                     ...
                     if (user_index == ~0 || (curr_user_index == user_index)) {
                         vlib_cli_output(vm, "index %d:%s:%s:%s", curr_user_index, auser->user_module_name,
                                         auser->val1_label, auser->val2_label);
                     }
                 }));
}
    ...
    pool_foreach(acontext, am->acl_lookup_contexts, ({
                     ...
                     if ((lc_index == ~0) || (curr_lc_index == lc_index)) {
                         if (acl_user_id_valid(am, acontext->context_user_id)) {
                             ...
                             vlib_cli_output(vm, "index %d:%s %s: %d %s: %d, acl_indices: %U", curr_lc_index,
                                             auser->user_module_name, auser->val1_label, acontext->user_val1,
                                             auser->val2_label, acontext->user_val2, format_vec32,
                                             acontext->acl_indices, "%d");
                         } else {
                             vlib_cli_output(vm, "index %d: user_id: %d user_val1: %d user_val2: %d, acl_indices: %U",
                                             curr_lc_index, acontext->context_user_id, acontext->user_val1,
                                             acontext->user_val2, format_vec32, acontext->acl_indices, "%d");
                         }
                     }
                 }));
}
