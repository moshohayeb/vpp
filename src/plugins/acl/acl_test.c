...
static void
vl_api_acl_plugin_get_version_reply_t_handler(vl_api_acl_plugin_get_version_reply_t *mp)
{
    vat_main_t *vam = acl_test_main.vat_main;
...
static void
vl_api_acl_interface_list_details_t_handler(vl_api_acl_interface_list_details_t *mp)
{
    int i;
    vat_main_t *vam = acl_test_main.vat_main;
    u8 *out         = 0;
...
static void
vl_api_acl_interface_etype_whitelist_details_t_handler(vl_api_acl_interface_etype_whitelist_details_t *mp)
{
    int i;
    vat_main_t *vam = acl_test_main.vat_main;
    u8 *out         = 0;
...
vl_api_acl_rule_t_pretty_format(u8 *out, vl_api_acl_rule_t *a)
{
    ...
    out = format(out, "%s action %d src %s/%d dst %s/%d proto %d sport %d-%d dport %d-%d tcpflags %d mask %d",
                 a->is_ipv6 ? "ipv6" : "ipv4", a->is_permit, src, a->src_ip_prefix_len, dst, a->dst_ip_prefix_len,
                 a->proto, a->srcport_or_icmptype_first, a->srcport_or_icmptype_last, a->dstport_or_icmpcode_first,
                 a->dstport_or_icmpcode_last, a->tcp_flags_value, a->tcp_flags_mask);
    return (out);
}
...
vl_api_macip_acl_rule_t_pretty_format(u8 *out, vl_api_macip_acl_rule_t *a)
{
    ...
    out = format(out, "%s action %d ip %s/%d mac %U mask %U", a->is_ipv6 ? "ipv6" : "ipv4", a->is_permit, src,
                 a->src_ip_prefix_len, my_format_mac_address, a->src_mac, my_format_mac_address, a->src_mac_mask);
    return (out);
}
...
static void
vl_api_macip_acl_interface_get_reply_t_handler(vl_api_macip_acl_interface_get_reply_t *mp)
{
    int i;
    vat_main_t *vam = acl_test_main.vat_main;
...
static void
vl_api_acl_plugin_control_ping_reply_t_handler(vl_api_acl_plugin_control_ping_reply_t *mp)
{
...
api_acl_add_replace_from_file(vat_main_t *vam)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (!unformat(input, "@%U/%d\t%U/%d\t%d : %d\t%d : %d\t0x%x/0x%x\t0x%x/0x%x", unformat_ip4_address,
                      &src_v4address, &src_prefix_length, unformat_ip4_address, &dst_v4address, &dst_prefix_length,
...
#define foreach_vpe_api_msg                                                                                            \
    ...
    _(acl_add_replace, "<acl-idx> [<ipv4|ipv6> <permit|permit+reflect|deny|action N> [src IP/plen] [dst IP/plen] "     \
    ...
    _(acl_interface_set_etype_whitelist,                                                                               \
    ...
    _(macip_acl_add, "...")                                                                                            \
    _(macip_acl_add_replace, "<acl-idx> [<ipv4|ipv6> <permit|deny|action N> [count <count>] [src] ip "                 \
                             "<ipaddress/[plen]> mac <mac> mask <mac_mask>, ... , ...")                                \
...
static void
acl_vat_api_hookup(vat_main_t *vam)
{
    acl_test_main_t *sm = &acl_test_main;
