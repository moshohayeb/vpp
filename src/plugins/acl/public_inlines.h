...
always_inline void
acl_fill_5tuple_l3_data(acl_main_t *am, vlib_buffer_t *b0, int is_ip6, int l3_offset, fa_5tuple_t *p5tuple_pkt)
{
    if (is_ip6) {
        clib_memcpy(&p5tuple_pkt->ip6_addr, get_ptr_to_offset(b0, offsetof(ip6_header_t, src_address) + l3_offset),
                    sizeof(p5tuple_pkt->ip6_addr));
    } else {
        memset(p5tuple_pkt->l3_zero_pad, 0, sizeof(p5tuple_pkt->l3_zero_pad));
        clib_memcpy(&p5tuple_pkt->ip4_addr, get_ptr_to_offset(b0, offsetof(ip4_header_t, src_address) + l3_offset),
                                ...
                                int l3_offset, fa_session_l4_key_t *p5tuple_l4, fa_packet_info_t *p5tuple_pkt)
{
    ...
    if (is_ip6) {
        proto     = *(u8 *) get_ptr_to_offset(b0, offsetof(ip6_header_t, protocol) + l3_offset);
        l4_offset = l3_offset + sizeof(ip6_header_t);
#ifdef FA_NODE_VERBOSE_DEBUG
        clib_warning("ACL_FA_NODE_DBG: proto: %d, l4_offset: %d", proto, l4_offset);
#endif
...
#ifdef FA_NODE_VERBOSE_DEBUG
                clib_warning("ACL_FA_NODE_DBG: new proto: %d, new offset: %d", proto, l4_offset);
#endif
                need_skip_eh = clib_bitmap_get(am->fa_ipv6_known_eh_bitmap, proto);
            }
        }
    } else {
        proto     = *(u8 *) get_ptr_to_offset(b0, offsetof(ip4_header_t, protocol) + l3_offset);
        l4_offset = l3_offset + sizeof(ip4_header_t);
        u16 flags_and_fragment_offset;
        clib_memcpy(&flags_and_fragment_offset,
                    get_ptr_to_offset(b0, offsetof(ip4_header_t, flags_and_fragment_offset)) + l3_offset,
    ...
    if (PREDICT_TRUE(offset_within_packet(b0, l4_offset))) {
        ...
        if (icmp_protos_v4v6[is_ip6] == proto) {
            /* type */
            tmp_l4.port[0] = *(u8 *) get_ptr_to_offset(b0, l4_offset + offsetof(icmp46_header_t, type));
            /* code */
            tmp_l4.port[1]     = *(u8 *) get_ptr_to_offset(b0, l4_offset + offsetof(icmp46_header_t, code));
            tmp_l4.is_slowpath = 1;
        } else if ((IP_PROTOCOL_TCP == proto) || (IP_PROTOCOL_UDP == proto)) {
            clib_memcpy(&ports, get_ptr_to_offset(b0, l4_offset + offsetof(tcp_header_t, src_port)), sizeof(ports));
            tmp_l4.port[0] = clib_net_to_host_u16(ports[0]);
            tmp_l4.port[1] = clib_net_to_host_u16(ports[1]);

            tmp_pkt.tcp_flags       = *(u8 *) get_ptr_to_offset(b0, l4_offset + offsetof(tcp_header_t, flags));
...
always_inline void
acl_fill_5tuple(acl_main_t *am, u32 sw_if_index0, vlib_buffer_t *b0, int is_ip6, int is_input, int is_l2_path,
                fa_5tuple_t *p5tuple_pkt)
{
    ...
    acl_fill_5tuple_l4_and_pkt_data(am, sw_if_index0, b0, is_ip6, is_input, l3_offset, &p5tuple_pkt->l4,
                                    &p5tuple_pkt->pkt);
}

always_inline void
acl_plugin_fill_5tuple_inline(void *p_acl_main, u32 lc_index, vlib_buffer_t *b0, int is_ip6, int is_input,
...
always_inline int
fa_acl_match_ip4_addr(ip4_address_t *addr1, ip4_address_t *addr2, int prefixlen)
{
    if (prefixlen == 0) {
...
always_inline int
fa_acl_match_ip6_addr(ip6_address_t *addr1, ip6_address_t *addr2, int prefixlen)
{
    if (prefixlen == 0) {
        /* match any always succeeds */
        return 1;
...
always_inline int
single_acl_match_5tuple(acl_main_t *am, u32 acl_index, fa_5tuple_t *pkt_5tuple, int is_ip6, u8 *r_action,
                        u32 *r_acl_match_p, u32 *r_rule_match_p, u32 *trace_bitmap)
{
    int i;
    acl_list_t *a;
    acl_rule_t *r;
    ...
    for (i = 0; i < a->count; i++) {
        ...
        if (is_ip6) {
            if (!fa_acl_match_ip6_addr(&pkt_5tuple->ip6_addr[1], &r->dst.ip6, r->dst_prefixlen))
                continue;
            if (!fa_acl_match_ip6_addr(&pkt_5tuple->ip6_addr[0], &r->src.ip6, r->src_prefixlen))
                continue;
        } else {
            if (!fa_acl_match_ip4_addr(&pkt_5tuple->ip4_addr[1], &r->dst.ip4, r->dst_prefixlen))
                continue;
            if (!fa_acl_match_ip4_addr(&pkt_5tuple->ip4_addr[0], &r->src.ip4, r->src_prefixlen))
                continue;
        }
...
#ifdef FA_NODE_VERBOSE_DEBUG
            clib_warning("ACL_FA_NODE_DBG acl %d rule %d pkt proto %d match rule %d", acl_index, i,
                         pkt_5tuple->l4.proto, r->proto);
#endif

            if (!fa_acl_match_port(pkt_5tuple->l4.port[0], r->src_port_or_type_first, r->src_port_or_type_last, is_ip6))
                continue;

#ifdef FA_NODE_VERBOSE_DEBUG
            clib_warning("ACL_FA_NODE_DBG acl %d rule %d pkt sport %d match rule [%d..%d]", acl_index, i,
                         pkt_5tuple->l4.port[0], r->src_port_or_type_first, r->src_port_or_type_last);
#endif

            if (!fa_acl_match_port(pkt_5tuple->l4.port[1], r->dst_port_or_code_first, r->dst_port_or_code_last, is_ip6))
                continue;

#ifdef FA_NODE_VERBOSE_DEBUG
            clib_warning("ACL_FA_NODE_DBG acl %d rule %d pkt dport %d match rule [%d..%d]", acl_index, i,
                         pkt_5tuple->l4.port[1], r->dst_port_or_code_first, r->dst_port_or_code_last);
#endif
            if (pkt_5tuple->pkt.tcp_flags_valid && ((pkt_5tuple->pkt.tcp_flags & r->tcp_flags_mask) != r->tcp_flags_value))
                continue;
        }
        /* everything matches! */
#ifdef FA_NODE_VERBOSE_DEBUG
...
always_inline int
acl_plugin_single_acl_match_5tuple(void *p_acl_main, u32 acl_index, fa_5tuple_t *pkt_5tuple, int is_ip6, u8 *r_action,
                                   u32 *r_acl_match_p, u32 *r_rule_match_p, u32 *trace_bitmap)
{
    acl_main_t *am = p_acl_main;
    return single_acl_match_5tuple(am, acl_index, pkt_5tuple, is_ip6, r_action, r_acl_match_p, r_rule_match_p,
                                   trace_bitmap);
}

always_inline int
linear_multi_acl_match_5tuple(void *p_acl_main, u32 lc_index, fa_5tuple_t *pkt_5tuple, int is_ip6, u8 *r_action,
                              u32 *acl_pos_p, u32 *acl_match_p, u32 *rule_match_p, u32 *trace_bitmap)
{
    acl_main_t *am = p_acl_main;
    int i;
    u32 *acl_vector;
...
#endif
        if (single_acl_match_5tuple(am, acl_vector[i], pkt_5tuple, is_ip6, &action, acl_match_p, rule_match_p,
                                    trace_bitmap)) {
            *r_action  = action;
            *acl_pos_p = i;
            return 1;
...
#ifdef FA_NODE_VERBOSE_DEBUG
    clib_warning("PORTMATCH: %d <= %d <= %d && %d <= %d <= %d ?", r->src_port_or_type_first, match->l4.port[0],
                 r->src_port_or_type_last, r->dst_port_or_code_first, match->l4.port[1], r->dst_port_or_code_last);
#endif
...
single_rule_match_5tuple(acl_rule_t *r, int is_ip6, fa_5tuple_t *pkt_5tuple)
{
    ...
    if (r->proto) {
        ...
        if (!fa_acl_match_port(pkt_5tuple->l4.port[0], r->src_port_or_type_first, r->src_port_or_type_last,
                               pkt_5tuple->pkt.is_ip6))
            return 0;


        if (!fa_acl_match_port(pkt_5tuple->l4.port[1], r->dst_port_or_code_first, r->dst_port_or_code_last,
                               pkt_5tuple->pkt.is_ip6))
            return 0;
...
multi_acl_match_get_applied_ace_index(acl_main_t *am, int is_ip6, fa_5tuple_t *match)
{
    ...
    DBG("TRYING TO MATCH: %016llx %016llx %016llx %016llx %016llx %016llx", pmatch[0], pmatch[1], pmatch[2], pmatch[3],
        pmatch[4], pmatch[5]);
...
always_inline int
hash_multi_acl_match_5tuple(void *p_acl_main, u32 lc_index, fa_5tuple_t *pkt_5tuple, int is_ip6, u8 *action,
                            u32 *acl_pos_p, u32 *acl_match_p, u32 *rule_match_p, u32 *trace_bitmap)
{
    acl_main_t *am                               = p_acl_main;
    applied_hash_ace_entry_t **applied_hash_aces = vec_elt_at_index(am->hash_entry_vec_by_lc_index, lc_index);
...
always_inline int
acl_plugin_match_5tuple_inline(void *p_acl_main, u32 lc_index, fa_5tuple_opaque_t *pkt_5tuple, int is_ip6, u8 *r_action,
                               u32 *r_acl_pos_p, u32 *r_acl_match_p, u32 *r_rule_match_p, u32 *trace_bitmap)
{
    acl_main_t *am                    = p_acl_main;
    ...
    if (PREDICT_TRUE(am->use_hash_acl_matching)) {
        if (PREDICT_FALSE(pkt_5tuple_internal->pkt.is_nonfirst_fragment)) {
            ...
            return hash_multi_acl_match_5tuple(p_acl_main, lc_index, pkt_5tuple_internal, is_ip6, r_action, r_acl_pos_p,
                                               r_acl_match_p, r_rule_match_p, trace_bitmap);
        }
    } else {
        return linear_multi_acl_match_5tuple(p_acl_main, lc_index, pkt_5tuple_internal, is_ip6, r_action, r_acl_pos_p,
                                             r_acl_match_p, r_rule_match_p, trace_bitmap);
    }
}
