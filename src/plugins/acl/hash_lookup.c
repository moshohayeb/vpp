...
always_inline applied_hash_ace_entry_t **
get_applied_hash_aces(acl_main_t *am, u32 lc_index)
{
...
{
    DBG("HASH ADD/DEL: %016llx %016llx %016llx %016llx %016llx %016llx %016llx add %d", kv->key[0], kv->key[1],
        kv->key[2], kv->key[3], kv->key[4], kv->key[5], kv->value, is_add);
...
first_mask_contains_second_mask(int is_ip6, fa_5tuple_t *mask1, fa_5tuple_t *mask2)
{
    ...
    if (is_ip6) {
        for (i = 0; i < 2; i++) {
            if ((mask1->ip6_addr[0].as_u64[i] & mask2->ip6_addr[0].as_u64[i]) != mask1->ip6_addr[0].as_u64[i])
                return 0;
            if ((mask1->ip6_addr[1].as_u64[i] & mask2->ip6_addr[1].as_u64[i]) != mask1->ip6_addr[1].as_u64[i])
                return 0;
        }
    } else {
        ...
        if (padcheck != 0)
            return 0;
        if ((mask1->ip4_addr[0].as_u32 & mask2->ip4_addr[0].as_u32) != mask1->ip4_addr[0].as_u32)
            return 0;
        if ((mask1->ip4_addr[1].as_u32 & mask2->ip4_addr[1].as_u32) != mask1->ip4_addr[1].as_u32)
            return 0;
    }
...
relax_ip4_addr(ip4_address_t *ip4_mask, int relax2)
{
    ...
    if (ip4_mask->as_u32 == 0xffffffff)
        ...
        ip4_mask->as_u32 =
            shift_ip4_if(ip4_mask->as_u32, 0xffffff00, shifts[1],
...
static void
fill_applied_hash_ace_kv(acl_main_t *am, applied_hash_ace_entry_t **applied_hash_aces, u32 lc_index, u32 new_index,
...
static void
add_del_hashtable_entry(acl_main_t *am, u32 lc_index, applied_hash_ace_entry_t **applied_hash_aces, u32 index, int is_add)
{
    clib_bihash_kv_48_8_t kv;
...
static void
remake_hash_applied_mask_info_vec(acl_main_t *am, applied_hash_ace_entry_t **applied_hash_aces, u32 lc_index)
{
...
static void
vec_del_collision_rule(collision_match_rule_t **pvec, u32 applied_entry_index)
{
    u32 i       = 0;
    u32 deleted = 0;
...
static void
del_colliding_rule(applied_hash_ace_entry_t **applied_hash_aces, u32 head_index, u32 applied_entry_index)
{
...
static void
add_colliding_rule(acl_main_t *am, applied_hash_ace_entry_t **applied_hash_aces, u32 head_index, u32 applied_entry_index)
{
    applied_hash_ace_entry_t *head_pae = vec_elt_at_index((*applied_hash_aces), head_index);
...
static u32
activate_applied_ace_hash_entry(acl_main_t *am, u32 lc_index, applied_hash_ace_entry_t **applied_hash_aces, u32 new_index)
{
    clib_bihash_kv_48_8_t kv;
    ASSERT(new_index != ~0);
    applied_hash_ace_entry_t *pae = vec_elt_at_index((*applied_hash_aces), new_index);
    ...
    DBG("APPLY ADD KY: %016llx %016llx %016llx %016llx %016llx %016llx", kv.key[0], kv.key[1], kv.key[2], kv.key[3],
        kv.key[4], kv.key[5]);
...
hash_acl_set_heap(acl_main_t *am)
{
    if (0 == am->hash_lookup_mheap) {
        am->hash_lookup_mheap = mheap_alloc_with_lock(0 /* use VM */, am->hash_lookup_mheap_size, 1 /* locked */);
        if (0 == am->hash_lookup_mheap) {
            clib_error("ACL plugin failed to allocate lookup heap of %U bytes", format_memory_size,
...
hash_acl_apply(acl_main_t *am, u32 lc_index, int acl_index, u32 acl_position)
{
    ...
    if (!am->acl_lookup_hash_initialized) {
        BV(clib_bihash_init)
        (&am->acl_lookup_hash, "ACL plugin rule lookup bihash", am->hash_lookup_hash_buckets,
    ...
    if (index2 != ~0) {
        clib_warning("BUG: trying to apply twice acl_index %d on lc_index %d, according to hash h-acl info", acl_index,
                     lc_index);
        goto done;
    }
...
static void
move_applied_ace_hash_entry(acl_main_t *am, u32 lc_index, applied_hash_ace_entry_t **applied_hash_aces, u32 old_index,
                            u32 new_index)
{
    ASSERT(old_index != ~0);
    ASSERT(new_index != ~0);
...
static void
deactivate_applied_ace_hash_entry(acl_main_t *am, u32 lc_index, applied_hash_ace_entry_t **applied_hash_aces,
                                  u32 old_index)
{
    applied_hash_ace_entry_t *pae = vec_elt_at_index((*applied_hash_aces), old_index);
    ...
    if (pae->prev_applied_entry_index != ~0) {
        ...
        if (pae->next_applied_entry_index != ~0) {
            ...
            add_del_hashtable_entry(am, lc_index, applied_hash_aces, pae->next_applied_entry_index, 1);
        } else {
...
hash_acl_unapply(acl_main_t *am, u32 lc_index, int acl_index)
{
    ...
    u32 index = vec_search(pal->applied_acls, acl_index);
    if (index == ~0) {
        clib_warning("BUG: trying to unapply unapplied acl_index %d on lc_index %d, according to lc", acl_index,
                     lc_index);
        return;
    }
    ...
    u32 index2 = vec_search((*hash_acl_applied_lc_index), lc_index);
    if (index2 == ~0) {
        clib_warning("BUG: trying to unapply twice acl_index %d on lc_index %d, according to h-acl info", acl_index,
                     lc_index);
        return;
    }
...
int
hash_acl_exists(acl_main_t *am, int acl_index)
{
...
void
hash_acl_add(acl_main_t *am, int acl_index)
{
    void *oldheap = hash_acl_set_heap(am);
...
void
hash_acl_delete(acl_main_t *am, int acl_index)
{
    void *oldheap = hash_acl_set_heap(am);
...
acl_plugin_show_tables_mask_type(void)
{
    ...
    pool_foreach(mte, am->ace_mask_type_pool, ({
                     vlib_cli_output(vm, "     %3d: %016llx %016llx %016llx %016llx %016llx %016llx  refcount %d",
                                     mte - am->ace_mask_type_pool, mte->mask.kv_40_8.key[0], mte->mask.kv_40_8.key[1],
                                     mte->mask.kv_40_8.key[2], mte->mask.kv_40_8.key[3], mte->mask.kv_40_8.key[4],
                                     mte->mask.kv_40_8.value, mte->refcount);
                 }));
    /* *INDENT-ON* */
}
...
acl_plugin_show_tables_acl_hash_info(u32 acl_index)
{
    ...
    for (i = 0; i < vec_len(am->hash_acl_infos); i++) {
        ...
        for (j = 0; j < vec_len(ha->rules); j++) {
            ...
            m                   = (u64 *) &pa->match;
            vlib_cli_output(vm, "    %4d: %016llx %016llx %016llx %016llx %016llx %016llx base mask index %d acl %d rule %d action %d\n",
                            j, m[0], m[1], m[2], m[3], m[4], m[5], pa->base_mask_type_index, pa->acl_index,
                            pa->ace_index, pa->action);
        }
    }
}
...
{
    vlib_cli_output(vm, "        %4d: acl %d ace %d acl pos %d pae index: %d", j, cr->acl_index, cr->ace_index,
                    cr->acl_position, cr->applied_entry_index);
}

static void
acl_plugin_print_pae(vlib_main_t *vm, int j, applied_hash_ace_entry_t *pae)
{
    vlib_cli_output(vm, "    %4d: acl %d rule %d action %d bitmask-ready rule %d mask type index: %d colliding_rules: %d next %d prev %d tail %d hitcount %lld acl_pos: %d",
                    j, pae->acl_index, pae->ace_index, pae->action, pae->hash_ace_info_index, pae->mask_type_index,
                    vec_len(pae->colliding_rules), pae->next_applied_entry_index, pae->prev_applied_entry_index,
...
{
    vlib_cli_output(vm, "    %4d: mask type index %d first rule index %d num_entries %d max_collisions %d", j,
                    mi->mask_type_index, mi->first_rule_index, mi->num_entries, mi->max_collisions);
}
...
split_partition(acl_main_t *am, u32 first_index, u32 lc_index, int is_ip6)
{
    ...
    for (i = 0; i < collisions; i++) {
        ...
        DBG("TM-collision: base_ace:%d (ace_mask:%d, first_collision_mask:%d)", pae->ace_index, pae->mask_type_index,
        ...
        if (pae->mask_type_index != coll_mask_type_index)
            continue;
        ...
        pae = pae->next_applied_entry_index == ~0 ?
                  0 :
                  vec_elt_at_index((*applied_hash_aces), pae->next_applied_entry_index);
    }
    ...
    if (coll_mask_type_index == new_mask_type_index) {
        // vlib_cli_output(vm, "TM-There are collisions over threshold, but i'm not able to split! %d %d",
        // coll_mask_type_index, new_mask_type_index);
        return;
    }
    ...
    for (r_ace_index = 0; r_ace_index < vec_len((*applied_hash_aces)); r_ace_index++) {
        ...
        DBG("TM-Population-collision: base_ace:%d (ace_mask:%d, first_collision_mask:%d)", pop_pae->ace_index,
            pop_pae->mask_type_index, coll_mask_type_index);

        if (pop_pae->mask_type_index != coll_mask_type_index)
            continue;
        ...
        if (!first_mask_contains_second_mask(is_ip6, min_tuple, pop_mask))
            continue;
