...
CT_ASSERT_EQUAL(fa_ip4_and_ip6_kv_value_match, offsetof(fa_5tuple_t, kv_16_8.value),
...
#define elog_acl_maybe_trace_X2(am, acl_elog_trace_format_label, acl_elog_trace_format_args, acl_elog_val1,            \
                                acl_elog_val2)                                                                         \
    do {                                                                                                               \
        if (am->trace_sessions) {                                                                                      \
...
#define elog_acl_maybe_trace_X3(am, acl_elog_trace_format_label, acl_elog_trace_format_args, acl_elog_val1,            \
    ...
    do {                                                                                                               \
        if (am->trace_sessions) {                                                                                      \
            CLIB_UNUSED(struct {                                                                                       \
                u8 available_space[18 - sizeof(acl_elog_val1) - sizeof(acl_elog_val2) - sizeof(acl_elog_val3)];        \
            } * static_check);                                                                                         \
            u16 thread_index        = os_get_thread_index();                                                           \
            vlib_worker_thread_t *w = vlib_worker_threads + thread_index;                                              \
            ELOG_TYPE_DECLARE(e)    = {                                                                                \
...
/* use like: elog_acl_maybe_trace_X4(am, "foobar: %d some int %d baz: %d bar: %d", "i4i4i4i4", int32_value,
 * int32_value2, int_value, int_value); */
