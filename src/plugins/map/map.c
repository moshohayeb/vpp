...
int
map_create_domain(ip4_address_t *ip4_prefix, u8 ip4_prefix_len, ip6_address_t *ip6_prefix, u8 ip6_prefix_len,
                  ip6_address_t *ip6_src, u8 ip6_src_len, u8 ea_bits_len, u8 psid_offset, u8 psid_length,
                  u32 *map_domain_index, u16 mtu, u8 flags)
{
    ...
    if (flags & MAP_DOMAIN_TRANSLATION) {
        ...
        if (ip6_src_len != 128) {
            clib_warning("MAP-E requires a BR address, not a prefix (ip6_src_len should "
                         "be 128).");
            return -1;
        }
    }
    ...
    *map_domain_index = d - mm->domains;
    ...
    fib_table_entry_special_dpo_add(0, &pfx, FIB_SOURCE_MAP, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo_v4);
    dpo_reset(&dpo_v4);
    ...
    fib_table_entry_special_dpo_add(0, &pfx6, FIB_SOURCE_MAP, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo_v6);
    dpo_reset(&dpo_v6);
...
int
map_add_del_psid(u32 map_domain_index, u16 psid, ip6_address_t *tep, u8 is_add)
{
    map_domain_t *d;
    map_main_t *mm = &map_main;
...
static void
map_fib_resolve(map_main_pre_resolved_t *pr, fib_protocol_t proto, u8 len, const ip46_address_t *addr)
{
    fib_prefix_t pfx = {
        .fp_proto = proto,
        .fp_len   = len,
    ...
    pr->fei     = fib_table_entry_special_add(0,   // default fib
                                          &pfx, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
    pr->sibling = fib_entry_child_add(pr->fei, FIB_NODE_TYPE_MAP_E, proto);
    map_stack(pr);
}

static void
map_fib_unresolve(map_main_pre_resolved_t *pr, fib_protocol_t proto, u8 len, const ip46_address_t *addr)
{
    fib_prefix_t pfx = {
        .fp_proto = proto,
        .fp_len   = len,
...
static clib_error_t *
map_security_check_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
map_security_check_frag_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    map_main_t *mm      = &map_main;
    clib_error_t *error = NULL;
...
static clib_error_t *
map_add_domain_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    ...
    map_create_domain(&ip4_prefix, ip4_prefix_len, &ip6_prefix, ip6_prefix_len, &ip6_src, ip6_src_len, ea_bits_len,
                      psid_offset, psid_length, &map_domain_index, mtu, flags);
...
static clib_error_t *
map_del_domain_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    u32 num_m_args = 0;
...
static clib_error_t *
map_add_rule_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    ip6_address_t tep;
    u32 num_m_args      = 0;
...
static clib_error_t *
map_pre_resolve_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
map_icmp_relay_source_address_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
map_icmp_unreachables_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    map_main_t *mm      = &map_main;
    int num_m_args      = 0;
...
static clib_error_t *
map_fragment_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    map_main_t *mm      = &map_main;
    clib_error_t *error = NULL;
...
static clib_error_t *
map_fragment_df_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    map_main_t *mm      = &map_main;
    clib_error_t *error = NULL;
...
static clib_error_t *
map_traffic_class_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    map_main_t *mm      = &map_main;
    u32 tc              = 0;
...
format_map_domain(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               d - mm->domains, format_ip4_address, &d->ip4_prefix, d->ip4_prefix_len, format_ip6_address, &ip6_prefix,
               d->ip6_prefix_len, format_ip6_address, &d->ip6_src, d->ip6_src_len, d->ea_bits_len, d->psid_offset,
               d->psid_length, d->mtu, map_flags_to_string(d->flags));
...
format_map_ip4_reass(u8 *s, va_list *args)
{
    ...
    s = format(s, "ip4-reass src=%U  dst=%U  protocol=%d  identifier=%d  port=%d  lifetime=%.3lf\n", format_ip4_address,
               &k->src.as_u8, format_ip4_address, &k->dst.as_u8, k->protocol, clib_net_to_host_u16(k->fragment_id),
...
format_map_ip6_reass(u8 *s, va_list *args)
{
    ...
    f64 dt                 = (r->ts + lifetime > now) ? (r->ts + lifetime - now) : -1;
    s = format(s, "ip6-reass src=%U  dst=%U  protocol=%d  identifier=%d  lifetime=%.3lf\n", format_ip6_address,
               &k->src.as_u8, format_ip6_address, &k->dst.as_u8, k->protocol, clib_net_to_host_u32(k->fragment_id), dt);
    return s;
}
...
show_map_stats_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
#if MAP_SKIP_IP6_LOOKUP
    vlib_cli_output(vm, "MAP pre-resolve: IP6 next-hop: %U, IP4 next-hop: %U\n", format_map_pre_resolve,
                    &pre_resolved[FIB_PROTOCOL_IP6], format_map_pre_resolve, &pre_resolved[FIB_PROTOCOL_IP4]);
    ...
    vlib_cli_output(vm, "MAP IPv6 inbound security check: %s, fragmented packet security check: %s",
                    mm->sec_check ? "enabled" : "disabled", mm->sec_check_frag ? "enabled" : "disabled");
    ...
    if (ip4) {
        ...
        if (pool_size != ~0) {
            if (map_ip4_reass_conf_pool_size(pool_size, &reass, &packets)) {
                ...
                vlib_cli_output(vm, "Setting ip4-reass pool-size (destroyed-reassembly=%u , dropped-fragments=%u)",
                                reass, packets);
            }
        }
        if (ht_ratio != (MAP_IP4_REASS_CONF_HT_RATIO_MAX + 1)) {
            if (map_ip4_reass_conf_ht_ratio(ht_ratio, &reass, &packets)) {
                ...
                vlib_cli_output(vm, "Setting ip4-reass ht-log2len (destroyed-reassembly=%u , dropped-fragments=%u)",
                                reass, packets);
            }
        }
        if (lifetime != ~0) {
    ...
    if (ip6) {
        ...
        if (pool_size != ~0) {
            if (map_ip6_reass_conf_pool_size(pool_size, &reass, &packets)) {
                ...
                vlib_cli_output(vm, "Setting ip6-reass pool-size (destroyed-reassembly=%u , dropped-fragments=%u)",
                                reass, packets);
            }
        }
        if (ht_ratio != (MAP_IP4_REASS_CONF_HT_RATIO_MAX + 1)) {
            if (map_ip6_reass_conf_ht_ratio(ht_ratio, &reass, &packets)) {
                ...
                vlib_cli_output(vm, "Setting ip6-reass ht-log2len (destroyed-reassembly=%u , dropped-fragments=%u)",
                                reass, packets);
            }
        }
        if (lifetime != ~0) {
...
map_ip4_reass_lookup(map_ip4_reass_key_t *k, u32 bucket, f64 now)
{
    ...
    while (ri != MAP_REASS_INDEX_NONE) {
        ...
        if (r->key.as_u64[0] == k->as_u64[0] && r->key.as_u64[1] == k->as_u64[1] &&
...
map_ip4_reass_t *
map_ip4_reass_get(u32 src, u32 dst, u16 fragment_id, u8 protocol, u32 **pi_to_drop)
{
    map_ip4_reass_t *r;
    map_main_t *mm        = &map_main;
    map_ip4_reass_key_t k = {.src.data_u32 = src, .dst.data_u32 = dst, .fragment_id = fragment_id, .protocol = protocol};
...
map_ip6_reass_lookup(map_ip6_reass_key_t *k, u32 bucket, f64 now)
{
    ...
    while (ri != MAP_REASS_INDEX_NONE) {
        ...
        if (now < r->ts + (((f64) mm->ip6_reass_conf_lifetime_ms) / 1000) && r->key.as_u64[0] == k->as_u64[0] &&
            r->key.as_u64[1] == k->as_u64[1] && r->key.as_u64[2] == k->as_u64[2] && r->key.as_u64[3] == k->as_u64[3] &&
...
map_ip6_reass_t *
map_ip6_reass_get(ip6_address_t *src, ip6_address_t *dst, u32 fragment_id, u8 protocol, u32 **pi_to_drop)
{
    map_ip6_reass_t *r;
    map_main_t *mm        = &map_main;
    map_ip6_reass_key_t k = {.src = *src, .dst = *dst, .fragment_id = fragment_id, .protocol = protocol};
...
#else
    u64 tmp = k.as_u64[0] ^ k.as_u64[1] ^ k.as_u64[2] ^ k.as_u64[3] ^ k.as_u64[4];
    h       = clib_xxhash(tmp);
#endif
...
int
map_ip6_reass_add_fragment(map_ip6_reass_t *r, u32 pi, u16 data_offset, u16 next_data_offset, u8 *data_start, u16 data_len)
{
...
int
map_ip4_reass_conf_ht_ratio(f32 ht_ratio, u32 *trashed_reass, u32 *dropped_packets)
{
    map_main_t *mm = &map_main;
...
int
map_ip4_reass_conf_pool_size(u16 pool_size, u32 *trashed_reass, u32 *dropped_packets)
{
    map_main_t *mm = &map_main;
...
int
map_ip6_reass_conf_ht_ratio(f32 ht_ratio, u32 *trashed_reass, u32 *dropped_packets)
{
    map_main_t *mm = &map_main;
...
int
map_ip6_reass_conf_pool_size(u16 pool_size, u32 *trashed_reass, u32 *dropped_packets)
{
    map_main_t *mm = &map_main;
