...
ip4_map_fragment(vlib_buffer_t *b, u16 mtu, bool df, u8 *error)
{
    ...
    if (mm->frag_inner) {
        ...
        ip_frag_set_vnet_buffer(b, mtu, IP6_FRAG_NEXT_IP6_LOOKUP, IP_FRAG_FLAG_IP6_HEADER);
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_TRUE(error0 == MAP_ERROR_NONE)) {
                if (PREDICT_FALSE(d0->mtu && (clib_net_to_host_u16(ip6h0->payload_length) + sizeof(*ip6h0) > d0->mtu))) {
                    ...
                    vlib_increment_combined_counter(cm + MAP_DOMAIN_COUNTER_TX, thread_index, map_domain_index0, 1,
            ...
            if (PREDICT_TRUE(error1 == MAP_ERROR_NONE)) {
                if (PREDICT_FALSE(d1->mtu && (clib_net_to_host_u16(ip6h1->payload_length) + sizeof(*ip6h1) > d1->mtu))) {
                    ...
                    vlib_increment_combined_counter(cm + MAP_DOMAIN_COUNTER_TX, thread_index, map_domain_index1, 1,
 ...
 */
static uword
ip4_map_reass(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, next_index, *to_next, n_left_to_next;
