...
int map_create_domain(ip4_address_t *ip4_prefix, u8 ip4_prefix_len, ip6_address_t *ip6_prefix, u8 ip6_prefix_len,
 ...
 */
static_always_inline map_domain_t *
ip6_map_get_domain(u32 mdi, ip4_address_t *addr, u32 *map_domain_index, u8 *error)
{
    map_main_t *mm = &map_main;
    ...
    *map_domain_index = mdi;
    ...
    if (PREDICT_TRUE(dpo->dpoi_type == map_dpo_type || dpo->dpoi_type == map_t_dpo_type)) {
...
#define map_ip4_reass_lock()                                                                                           \
    while (__sync_lock_test_and_set(map_main.ip4_reass_lock, 1)) {                                                     \
    }
#define map_ip4_reass_unlock()                                                                                         \
    do {                                                                                                               \
        CLIB_MEMORY_BARRIER();                                                                                         \
        *map_main.ip4_reass_lock = 0;                                                                                  \
    } while (0)
...
map_ip6_reass_t *map_ip6_reass_get(ip6_address_t *src, ip6_address_t *dst, u32 fragment_id, u8 protocol,
                                   u32 **pi_to_drop);
void map_ip6_reass_free(map_ip6_reass_t *r, u32 **pi_to_drop);

#define map_ip6_reass_lock()                                                                                           \
    while (__sync_lock_test_and_set(map_main.ip6_reass_lock, 1)) {                                                     \
    }
#define map_ip6_reass_unlock()                                                                                         \
    do {                                                                                                               \
        CLIB_MEMORY_BARRIER();                                                                                         \
        *map_main.ip6_reass_lock = 0;                                                                                  \
    } while (0)

int map_ip6_reass_add_fragment(map_ip6_reass_t *r, u32 pi, u16 data_offset, u16 next_data_offset, u8 *data_start,
                               u16 data_len);
...
static_always_inline void
ip4_map_t_embedded_address(map_domain_t *d, ip6_address_t *ip6, const ip4_address_t *ip4)
{
...
static_always_inline void
map_send_all_to_node(vlib_main_t *vm, u32 *pi_vector, vlib_node_runtime_t *node, vlib_error_t *error, u32 next)
{
