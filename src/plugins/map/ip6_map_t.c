...
ip6_map_fragment_cache(ip6_header_t *ip6, ip6_frag_hdr_t *frag, map_domain_t *d, u16 port)
{
    ...
    map_ip4_reass_lock();
    map_ip4_reass_t *r =
        map_ip4_reass_get(map_get_ip4(&ip6->src_address, d->flags), ip6_map_t_embedded_address(d, &ip6->dst_address),
                          frag_id_6to4(frag->identification),
                          (ip6->protocol == IP_PROTOCOL_ICMP6) ? IP_PROTOCOL_ICMP : ip6->protocol, &ignore);
    if (r)
        r->port = port;
...
ip6_to_ip4_set_icmp_cb(ip6_header_t *ip6, ip4_header_t *ip4, void *arg)
{
    ...
    if (mm->is_ce) {
        ...
        if (ip6->dst_address.as_u64[0] != map_get_pfx_net(ctx->d, ip4_dadr, ctx->id) ||
            ip6->dst_address.as_u64[1] != map_get_sfx_net(ctx->d, ip4_dadr, ctx->id))
            return -1;
        ...
        if (ip6->src_address.as_u64[0] != map_get_pfx_net(ctx->d, ip4_sadr, ctx->id) ||
            ip6->src_address.as_u64[1] != map_get_sfx_net(ctx->d, ip4_sadr, ctx->id))
            return -1;
...
ip6_to_ip4_set_inner_icmp_cb(ip6_header_t *ip6, ip4_header_t *ip4, void *arg)
{
    ...
    if (mm->is_ce) {
        ...
        if (ip6->src_address.as_u64[0] != map_get_pfx_net(ctx->d, inner_ip4_sadr, ctx->id) ||
            ip6->src_address.as_u64[1] != map_get_sfx_net(ctx->d, inner_ip4_sadr, ctx->id))
            return -1;
        ...
        if (ip6->dst_address.as_u64[0] != map_get_pfx_net(ctx->d, inner_ip4_dadr, ctx->id) ||
            ip6->dst_address.as_u64[1] != map_get_sfx_net(ctx->d, inner_ip4_dadr, ctx->id))
            return -1;
...
static uword
ip6_map_t_icmp(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            ctx0.id = ip6_get_port(ip60, icmp0->type == ICMP6_echo_request, p0->current_length);
            ctx0.d  = d0;
            if (ctx0.id == 0) {
            ...
            if (icmp6_to_icmp(p0, ip6_to_ip4_set_icmp_cb, &ctx0, ip6_to_ip4_set_inner_icmp_cb, &ctx0)) {
                error0 = MAP_ERROR_ICMP;
                goto err0;
            }
        ...
        err0:
            if (PREDICT_TRUE(error0 == MAP_ERROR_NONE)) {
                vlib_increment_combined_counter(cm + MAP_DOMAIN_COUNTER_RX, thread_index,
...
static uword
ip6_map_t_fragmented(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, next_index, *to_next, n_left_to_next;
    from                            = vlib_frame_vector_args(frame);
    n_left_from                     = frame->n_vectors;
...
static uword
ip6_map_t_tcp_udp(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, next_index, *to_next, n_left_to_next;
...
ip6_map_t_classify(vlib_buffer_t *p0, ip6_header_t *ip60, map_domain_t *d0, i32 *map_port0, u8 *error0,
                   ip6_mapt_next_t *next0, u32 l4_len0, ip6_frag_hdr_t *frag0)
{
    map_main_t *mm = &map_main;
    u32 port_offset;
    ...
    if (PREDICT_FALSE(vnet_buffer(p0)->map_t.v6.frag_offset && ip6_frag_hdr_offset(frag0))) {
        ...
        *map_port0 = (i32) * ((u16 *) u8_ptr_add(ip60, vnet_buffer(p0)->map_t.v6.l4_offset + port_offset));
        ...
        *next0  = IP6_MAPT_NEXT_MAPT_ICMP;
        if (d0->ea_bits_len == 0 && d0->rules) {
            *map_port0 = 0;
                   ...
                   ((icmp46_header_t *) u8_ptr_add(ip60, vnet_buffer(p0)->map_t.v6.l4_offset))->code ==
                       ICMP6_echo_request) {
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (mm->is_ce) {
                ...
                d0     = ip6_map_get_domain(vnet_buffer(p0)->ip.adj_index[VLIB_TX], (ip4_address_t *) &daddr0,
                                        &vnet_buffer(p0)->map_t.map_domain_index, &error0);
                d1     = ip6_map_get_domain(vnet_buffer(p1)->ip.adj_index[VLIB_TX], (ip4_address_t *) &daddr1,
                ...
                vnet_buffer(p0)->map_t.v6.saddr = ip6_map_t_embedded_address(d0, &ip60->src_address);
                vnet_buffer(p1)->map_t.v6.saddr = ip6_map_t_embedded_address(d1, &ip61->src_address);
            } else {
                u32 saddr0, saddr1;
                saddr0 = 0; /* TODO */
                saddr1 = 0; /* TODO */
                ...
                d0     = ip6_map_get_domain(vnet_buffer(p0)->ip.adj_index[VLIB_TX], (ip4_address_t *) &saddr0,
                                        &vnet_buffer(p0)->map_t.map_domain_index, &error0);
                d1     = ip6_map_get_domain(vnet_buffer(p1)->ip.adj_index[VLIB_TX], (ip4_address_t *) &saddr1,
                                        &vnet_buffer(p1)->map_t.map_domain_index, &error1);
            ...
            if (PREDICT_FALSE(ip6_parse(ip60, p0->current_length, &(vnet_buffer(p0)->map_t.v6.l4_protocol),
            ...
            if (PREDICT_FALSE(ip6_parse(ip61, p1->current_length, &(vnet_buffer(p1)->map_t.v6.l4_protocol),
            ...
            l4_len0 =
                (u32) clib_net_to_host_u16(ip60->payload_length) + sizeof(*ip60) - vnet_buffer(p0)->map_t.v6.l4_offset;
            l4_len1 =
                (u32) clib_net_to_host_u16(ip61->payload_length) + sizeof(*ip60) - vnet_buffer(p1)->map_t.v6.l4_offset;
            ...
            ip6_map_t_classify(p0, ip60, d0, &map_port0, &error0, &next0, l4_len0, frag0);
            ip6_map_t_classify(p1, ip61, d1, &map_port1, &error1, &next1, l4_len1, frag1);
            ...
            if (PREDICT_FALSE(
                    (map_port1 != -1) &&
                    (ip61->src_address.as_u64[0] != map_get_pfx_net(d1, vnet_buffer(p1)->map_t.v6.saddr, map_port1) ||
                     ip61->src_address.as_u64[1] != map_get_sfx_net(d1, vnet_buffer(p1)->map_t.v6.saddr, map_port1)))) {
                error1 = MAP_ERROR_SEC_CHECK;
            }

            if (PREDICT_FALSE(
                    vnet_buffer(p0)->map_t.v6.frag_offset &&
                    !ip6_frag_hdr_offset((ip6_frag_hdr_t *) u8_ptr_add(ip60, vnet_buffer(p0)->map_t.v6.frag_offset))) &&
                (map_port0 != -1) && (d0->ea_bits_len != 0 || !d0->rules) && (error0 == MAP_ERROR_NONE)) {
                ip6_map_fragment_cache(ip60, (ip6_frag_hdr_t *) u8_ptr_add(ip60, vnet_buffer(p0)->map_t.v6.frag_offset),
                                       d0, map_port0);
            }

            if (PREDICT_FALSE(
                    vnet_buffer(p1)->map_t.v6.frag_offset &&
                    !ip6_frag_hdr_offset((ip6_frag_hdr_t *) u8_ptr_add(ip61, vnet_buffer(p1)->map_t.v6.frag_offset))) &&
                (map_port1 != -1) && (d1->ea_bits_len != 0 || !d1->rules) && (error1 == MAP_ERROR_NONE)) {
                ip6_map_fragment_cache(ip61, (ip6_frag_hdr_t *) u8_ptr_add(ip61, vnet_buffer(p1)->map_t.v6.frag_offset),
                                       d1, map_port1);
            }
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (mm->is_ce) {
                ...
                d0    = ip6_map_get_domain(vnet_buffer(p0)->ip.adj_index[VLIB_TX], (ip4_address_t *) &daddr,
                                        &vnet_buffer(p0)->map_t.map_domain_index, &error0);
                ...
                d0    = ip6_map_get_domain(vnet_buffer(p0)->ip.adj_index[VLIB_TX], (ip4_address_t *) &saddr,
                                        &vnet_buffer(p0)->map_t.map_domain_index, &error0);
