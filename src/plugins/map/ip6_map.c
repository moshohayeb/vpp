...
format_ip6_map_ip4_reass_trace(u8 *s, va_list *args)
{
    ...
    return format(s, "MAP domain index: %d L4 port: %u Status: %s", t->map_domain_index, t->port,
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_TRUE(ip60->protocol == IP_PROTOCOL_IP_IN_IP && clib_net_to_host_u16(ip60->payload_length) > 20)) {
                d0 = ip6_map_get_domain(vnet_buffer(p0)->ip.adj_index[VLIB_TX],
                                        (ip4_address_t *) &ip40->src_address.as_u32, &map_domain_index0, &error0);
            ...
            if (PREDICT_TRUE(ip61->protocol == IP_PROTOCOL_IP_IN_IP && clib_net_to_host_u16(ip61->payload_length) > 20)) {
                d1 = ip6_map_get_domain(vnet_buffer(p1)->ip.adj_index[VLIB_TX],
                                        (ip4_address_t *) &ip41->src_address.as_u32, &map_domain_index1, &error1);
            ...
            if (d0) {
                ...
                if (PREDICT_TRUE(error0 == MAP_ERROR_NONE && next0 == IP6_MAP_NEXT_IP4_LOOKUP)) {
                    ...
                    vlib_increment_combined_counter(cm + MAP_DOMAIN_COUNTER_RX, thread_index, map_domain_index0, 1,
            ...
            if (d1) {
                ...
                if (PREDICT_TRUE(error1 == MAP_ERROR_NONE && next1 == IP6_MAP_NEXT_IP4_LOOKUP)) {
                    ...
                    vlib_increment_combined_counter(cm + MAP_DOMAIN_COUNTER_RX, thread_index, map_domain_index1, 1,
...
static_always_inline void
ip6_map_ip6_reass_prepare(vlib_main_t *vm, vlib_node_runtime_t *node, map_ip6_reass_t *r, u32 **fragments_ready,
    ...
    for (i = 0; i < MAP_IP6_REASS_MAX_FRAGMENTS_PER_REASSEMBLY; i++) {
        ...
        if (!ip4_is_fragment(ip40)) {
            ...
            ip40->flags_and_fragment_offset =
                clib_host_to_net_u16(ip4_get_fragment_offset(ip40) + ip6_frag_hdr_offset(frag0));
        }
 ...
 */
static uword
ip6_map_ip6_reass(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, next_index, *to_next, n_left_to_next;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            map_ip6_reass_lock();
            map_ip6_reass_t *r = map_ip6_reass_get(&ip60->src_address, &ip60->dst_address, frag0->identification,
                                                   frag0->next_hdr, &fragments_to_drop);
            // FIXME: Use better error codes
            if (PREDICT_FALSE(!r)) {
                // Could not create a caching entry
                error0 = MAP_ERROR_FRAGMENT_MEMORY;
            } else if (PREDICT_FALSE((frag_len <= 20 && (ip6_frag_hdr_more(frag0) || (!offset))))) {
    ...
    map_send_all_to_node(vm, fragments_ready, node, &error_node->errors[MAP_ERROR_NONE], IP6_MAP_IP6_REASS_NEXT_IP6_MAP);
    map_send_all_to_node(vm, fragments_to_drop, node, &error_node->errors[MAP_ERROR_FRAGMENT_DROPPED],
                         IP6_MAP_IP6_REASS_NEXT_DROP);
 ...
 */
static uword
ip6_map_ip4_reass(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, next_index, *to_next, n_left_to_next;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(d0->mtu && (clib_host_to_net_u16(ip40->length) > d0->mtu) && error0 == MAP_ERROR_NONE &&
                              !cached)) {
                vnet_buffer(p0)->ip_frag.flags      = 0;
                vnet_buffer(p0)->ip_frag.next_index = IP4_FRAG_NEXT_IP4_LOOKUP;
                vnet_buffer(p0)->ip_frag.mtu        = d0->mtu;
 ...
 */
static uword
ip6_map_icmp_relay(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, next_index, *to_next, n_left_to_next;
