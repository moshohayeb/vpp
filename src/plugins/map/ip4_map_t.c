...
ip4_map_fragment_cache(ip4_header_t *ip4, u16 port)
{
    ...
    map_ip4_reass_lock();
    map_ip4_reass_t *r =
        map_ip4_reass_get(ip4->src_address.as_u32, ip4->dst_address.as_u32, ip4->fragment_id,
                          (ip4->protocol == IP_PROTOCOL_ICMP) ? IP_PROTOCOL_ICMP6 : ip4->protocol, &ignore);
    if (r)
        r->port = port;
...
ip4_to_ip6_set_icmp_cb(ip4_header_t *ip4, ip6_header_t *ip6, void *arg)
{
    ...
    if (mm->is_ce) {
        ip6->src_address.as_u64[0] = map_get_pfx_net(ctx->d, ip4->src_address.as_u32, ctx->id);
        ip6->src_address.as_u64[1] = map_get_sfx_net(ctx->d, ip4->src_address.as_u32, ctx->id);
        ...
        ip6->dst_address.as_u64[0] = map_get_pfx_net(ctx->d, ip4->dst_address.as_u32, ctx->id);
        ip6->dst_address.as_u64[1] = map_get_sfx_net(ctx->d, ip4->dst_address.as_u32, ctx->id);
    }
...
static int
ip4_to_ip6_set_inner_icmp_cb(ip4_header_t *ip4, ip6_header_t *ip6, void *arg)
{
    icmp_to_icmp6_ctx_t *ctx = arg;
    map_main_t *mm           = &map_main;
...
static uword
ip4_map_t_icmp(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        err0:
            if (PREDICT_TRUE(error0 == MAP_ERROR_NONE)) {
                vlib_increment_combined_counter(cm + MAP_DOMAIN_COUNTER_TX, thread_index,
...
static uword
ip4_map_t_fragmented(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, next_index, *to_next, n_left_to_next;
    from                            = vlib_frame_vector_args(frame);
    n_left_from                     = frame->n_vectors;
...
static uword
ip4_map_t_tcp_udp(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from, next_index, *to_next, n_left_to_next;
    from                            = vlib_frame_vector_args(frame);
    n_left_from                     = frame->n_vectors;
...
ip4_map_t_classify(vlib_buffer_t *p0, map_domain_t *d0, ip4_header_t *ip40, u16 ip4_len0, i32 *dst_port0, u8 *error0,
                   ip4_mapt_next_t *next0)
{
    ...
    if (PREDICT_FALSE(ip4_get_fragment_offset(ip40))) {
        ...
        *dst_port0                             = (i32) * ((u16 *) u8_ptr_add(ip40, sizeof(*ip40) + port_offset));
...
ip4_map_t(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE(p1->current_length < ip4_len1 || ip41->ip_version_and_header_length != 0x45)) {
                error1 = MAP_ERROR_UNKNOWN;
                next1  = IP4_MAPT_NEXT_DROP;
            }

            vnet_buffer(p0)->map_t.map_domain_index = vnet_buffer(p0)->ip.adj_index[VLIB_TX];
            d0                                      = ip4_map_get_domain(vnet_buffer(p0)->map_t.map_domain_index);
            vnet_buffer(p1)->map_t.map_domain_index = vnet_buffer(p1)->ip.adj_index[VLIB_TX];
            ...
            ip4_map_t_classify(p0, d0, ip40, ip4_len0, &map_port0, &error0, &next0);
            ip4_map_t_classify(p1, d1, ip41, ip4_len1, &map_port1, &error1, &next1);
            ...
            if (mm->is_ce) {
                ip4_map_t_embedded_address(d0, &pheader0->daddr, &ip40->dst_address);
                ip4_map_t_embedded_address(d1, &pheader1->daddr, &ip41->dst_address);
                pheader0->saddr.as_u64[0] = map_get_pfx_net(d0, ip40->src_address.as_u32, (u16) map_port0);
                pheader0->saddr.as_u64[1] = map_get_sfx_net(d0, ip40->src_address.as_u32, (u16) map_port0);
                pheader1->saddr.as_u64[0] = map_get_pfx_net(d1, ip41->src_address.as_u32, (u16) map_port1);
                pheader1->saddr.as_u64[1] = map_get_sfx_net(d1, ip41->src_address.as_u32, (u16) map_port1);
            } else {
                ...
                pheader0->daddr.as_u64[0] = map_get_pfx_net(d0, ip40->dst_address.as_u32, (u16) map_port0);
                pheader0->daddr.as_u64[1] = map_get_sfx_net(d0, ip40->dst_address.as_u32, (u16) map_port0);
                pheader1->daddr.as_u64[0] = map_get_pfx_net(d1, ip41->dst_address.as_u32, (u16) map_port1);
                pheader1->daddr.as_u64[1] = map_get_sfx_net(d1, ip41->dst_address.as_u32, (u16) map_port1);
            }

            if (PREDICT_FALSE(ip4_is_first_fragment(ip40) && (map_port0 != -1) &&
                              (d0->ea_bits_len != 0 || !d0->rules) && ip4_map_fragment_cache(ip40, map_port0))) {
                error0 = MAP_ERROR_FRAGMENT_MEMORY;
            }

            if (PREDICT_FALSE(ip4_is_first_fragment(ip41) && (map_port1 != -1) &&
                              (d1->ea_bits_len != 0 || !d1->rules) && ip4_map_fragment_cache(ip41, map_port1))) {
