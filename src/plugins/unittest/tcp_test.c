...
tcp_test_sack_rx(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    hole = scoreboard_first_hole(sb);
    TCP_TEST((pool_elts(sb->holes) == 1), "scoreboard has %d holes", pool_elts(sb->holes));
    ...
    TCP_TEST((sb->snd_una_adv == 900), "snd_una_adv after ack %u", sb->snd_una_adv);
    ...
    TCP_TEST((sb->snd_una_adv == 0), "snd_una_adv after ack %u", sb->snd_una_adv);
    ...
    TCP_TEST((sb->snd_una_adv == 100), "snd_una_adv after ack %u", sb->snd_una_adv);
    TCP_TEST((sb->sacked_bytes == 0), "sacked bytes %d", sb->sacked_bytes);
    TCP_TEST((pool_elts(sb->holes) == 0), "scoreboard has %d elements", pool_elts(sb->holes));
...
static u8
fifo_validate_pattern(vlib_main_t *vm, test_pattern_t *pattern, u32 pattern_length)
{
    ...
    for (i = 0; i < pattern_length - 1; i++) {
        if (tp->offset + tp->len != (tp + 1)->offset) {
            vlib_cli_output(vm, "[%d] missing {%d, %d}", i, (tp->offset + tp->len),
...
fifo_get_validate_pattern(vlib_main_t *vm, test_pattern_t *test_data, u32 test_data_len)
{
...
tcp_test_fifo1(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (verbose)
            vlib_cli_output(vm, "add [%d] [%d, %d]", 2 * i + 1, offset, offset + sizeof(u32));
        if (rv) {
    ...
    TCP_TEST((svm_fifo_number_ooo_segments(f) == 2), "number of ooo segments %u", svm_fifo_number_ooo_segments(f));
    ...
    for (i = 3; i > 1; i--) {
        ...
        rv     = svm_fifo_enqueue_with_offset(f, offset, sizeof(u32), data);
        if (verbose)
            vlib_cli_output(vm, "add [%d] [%d, %d]", 2 * i, offset, offset + sizeof(u32));
        if (rv) {
            clib_warning("enqueue returned %d", rv);
            goto err;
        }
    ...
    if (verbose)
        vlib_cli_output(vm, "fifo before missing link: %U", format_svm_fifo, f, 1);
    TCP_TEST((svm_fifo_number_ooo_segments(f) == 1), "number of ooo segments %u", svm_fifo_number_ooo_segments(f));
    ooo_seg = svm_fifo_first_ooo_segment(f);
    TCP_TEST((ooo_seg->start == 12), "first ooo seg position %u", ooo_seg->start);
    TCP_TEST((ooo_seg->length == 16), "first ooo seg length %u", ooo_seg->length);
    ...
    TCP_TEST((svm_fifo_number_ooo_segments(f) == 0), "number of ooo segments %u", svm_fifo_number_ooo_segments(f));
    ...
    if (verbose)
        vlib_cli_output(vm, "fifo after enqueueing 29: %U", format_svm_fifo, f, 1);
...
tcp_test_fifo2(vlib_main_t *vm)
{
    ...
    for (i = 0; i < test_data_len; i++) {
        tp     = vp + i;
        data64 = tp->offset;
        svm_fifo_enqueue_with_offset(f, tp->offset - f->tail, tp->len, (u8 *) &data64);
    }

    /* Expected result: one big fat chunk at offset 4 */
    TCP_TEST((svm_fifo_number_ooo_segments(f) == 1), "number of ooo segments %u", svm_fifo_number_ooo_segments(f));
    ooo_seg = svm_fifo_first_ooo_segment(f);
    TCP_TEST((ooo_seg->start == 4), "first ooo seg position %u", ooo_seg->start);
    TCP_TEST((ooo_seg->length == 2996), "first ooo seg length %u", ooo_seg->length);
    ...
    for (i = 0; i < test_data_len; i++) {
        tp     = &test_data[i];
        data64 = tp->offset;
        rv     = svm_fifo_enqueue_with_offset(f, tp->offset - f->tail, tp->len, (u8 *) &data64);
        if (rv) {
            clib_warning("enqueue returned %d", rv);
        }
    }
...
tcp_test_fifo3(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    for (i = !randomize; i < vec_len(generate); i++) {
        tp = generate + i;
        svm_fifo_enqueue_with_offset(f, fifo_initial_offset + tp->offset - f->tail, tp->len,
    ...
    if (verbose)
        vlib_cli_output(vm, "fifo before missing link: %U", format_svm_fifo, f, 1 /* verbose */);
    ...
    if (randomize) {
        ...
        TCP_TEST((rv == total_size), "enqueued %u expected %u", rv, total_size);
    }

    TCP_TEST((svm_fifo_has_ooo_data(f) == 0), "number of ooo segments %u", svm_fifo_number_ooo_segments(f));
    ...
    if (compare_data(data_buf, data_pattern, 0, vec_len(data_pattern), &j)) {
        TCP_TEST(0, "[%d] peeked %u expected %u", j, data_buf[j], data_pattern[j]);
    }
    vec_reset_length(data_buf);
    ...
    if (drop) {
        ...
        if (compare_data(data_buf, data_pattern, 0, vec_len(data_pattern), &j)) {
            TCP_TEST(0, "[%d] dequeued %u expected %u", j, data_buf[j], data_pattern[j]);
        }
    }

    TCP_TEST((svm_fifo_max_dequeue(f) == 0), "fifo has %d bytes", svm_fifo_max_dequeue(f));
...
tcp_test_fifo4(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    for (i = test_n_bytes - 1; i > 0; i--) {
        rv = svm_fifo_enqueue_with_offset(f, fifo_initial_offset + i - f->tail, sizeof(u8), &test_data[i]);
        if (verbose)
    ...
    if (rv)
        vlib_cli_output(vm, "[%d] dequeued %u expected %u", j, data_buf[j], test_data[j]);
...
tcp_test_fifo5(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    if (verbose)
        vlib_cli_output(vm, "fifo after [100, 200] and [300, 400] : %U", format_svm_fifo, f, 2 /* verbose */);
    ...
    if (verbose)
        vlib_cli_output(vm, "fifo after [225, 275] : %U", format_svm_fifo, f, 2 /* verbose */);
    TCP_TEST((svm_fifo_number_ooo_segments(f) == 3), "number of ooo segments %u", svm_fifo_number_ooo_segments(f));
    ooo_seg = svm_fifo_first_ooo_segment(f);
    TCP_TEST((ooo_seg->start == fifo_pos(f, 100 + offset)), "first seg start %u expected %u", ooo_seg->start,
             fifo_pos(f, 100 + offset));
    TCP_TEST((ooo_seg->length == 100), "first seg length %u expected %u", ooo_seg->length, 100);
    ooo_seg = ooo_segment_next(f, ooo_seg);
    TCP_TEST((ooo_seg->start == fifo_pos(f, 225 + offset)), "second seg start %u expected %u", ooo_seg->start,
             fifo_pos(f, 225 + offset));
    TCP_TEST((ooo_seg->length == 50), "second seg length %u expected %u", ooo_seg->length, 50);
    ooo_seg = ooo_segment_next(f, ooo_seg);
    TCP_TEST((ooo_seg->start == fifo_pos(f, 300 + offset)), "third seg start %u expected %u", ooo_seg->start,
             fifo_pos(f, 300 + offset));
    TCP_TEST((ooo_seg->length == 100), "third seg length %u expected %u", ooo_seg->length, 100);
    ...
    if (verbose)
        vlib_cli_output(vm, "fifo after [190, 310] : %U", format_svm_fifo, f, 1 /* verbose */);
    TCP_TEST((svm_fifo_number_ooo_segments(f) == 1), "number of ooo segments %u", svm_fifo_number_ooo_segments(f));
    ooo_seg = svm_fifo_first_ooo_segment(f);
    TCP_TEST((ooo_seg->start == fifo_pos(f, offset + 100)), "first seg start %u expected %u", ooo_seg->start,
             fifo_pos(f, offset + 100));
    TCP_TEST((ooo_seg->length == 300), "first seg length %u expected %u", ooo_seg->length, 300);
    ...
    if (verbose)
        vlib_cli_output(vm, "fifo after [0 150] : %U", format_svm_fifo, f, 2 /* verbose */);
    ...
    if (compare_data(data_buf, test_data, 0, 400, &j)) {
        TCP_TEST(0, "[%d] peeked %u expected %u", j, data_buf[j], test_data[j]);
    }
    ...
    TCP_TEST((svm_fifo_number_ooo_segments(f) == 1), "number of ooo segments %u", svm_fifo_number_ooo_segments(f));
    ooo_seg = svm_fifo_first_ooo_segment(f);
    TCP_TEST((ooo_seg->start == 50), "first seg start %u expected %u", ooo_seg->start, 50);
    TCP_TEST((ooo_seg->length == 200), "first seg length %u expected %u", ooo_seg->length, 200);
...
tcp_test_lookup(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    tconn = session_lookup_connection_wt4(0, &tc1->lcl_ip.ip4, &tc1->rmt_ip.ip4, tc1->lcl_port, tc1->rmt_port,
    ...
    tconn = session_lookup_connection_wt4(0, &tc2->lcl_ip.ip4, &tc2->rmt_ip.ip4, tc2->lcl_port, tc2->rmt_port,
...
tcp_test_session(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    if (is_add) {
        ...
        if (stream_session_accept(&tc0->connection, 0 /* listener index */, 0 /* notify */))
