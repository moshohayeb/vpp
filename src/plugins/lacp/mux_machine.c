...
lacp_mux_action_detached(void *p1, void *p2)
{
    ...
    if (sif->selected == LACP_PORT_STANDBY)
        lacp_machine_dispatch(&lacp_mux_machine, vm, sif, LACP_MUX_EVENT_STANDBY, &sif->mux_state);
...
lacp_mux_action_attached(void *p1, void *p2)
{
    ...
    if ((sif->selected == LACP_PORT_UNSELECTED) || (sif->selected == LACP_PORT_STANDBY))
        lacp_machine_dispatch(&lacp_mux_machine, vm, sif, LACP_MUX_EVENT_UNSELECTED, &sif->mux_state);

    if ((sif->selected == LACP_PORT_SELECTED) && (sif->partner.state & LACP_STATE_SYNCHRONIZATION))
        lacp_machine_dispatch(&lacp_mux_machine, vm, sif, LACP_MUX_EVENT_SYNC, &sif->mux_state);
    return 0;
}
...
lacp_mux_action_waiting(void *p1, void *p2)
{
    ...
    if (!lacp_timer_is_running(sif->wait_while_timer))
        lacp_start_wait_while_timer(lm->vlib_main, sif, LACP_AGGREGATE_WAIT_TIME);

    if ((sif->selected == LACP_PORT_SELECTED) && sif->ready)
        lacp_machine_dispatch(&lacp_mux_machine, vm, sif, LACP_MUX_EVENT_READY, &sif->mux_state);
...
lacp_mux_action_collecting_distributing(void *p1, void *p2)
{
    ...
    sif->actor.state |= LACP_STATE_SYNCHRONIZATION | LACP_STATE_COLLECTING | LACP_STATE_DISTRIBUTING;
...
#undef _
    ...
    lacp_event_struct *event_entry = (lacp_event_struct *) &lacp_mux_event_array;
...
void
lacp_mux_debug_func(slave_if_t *sif, int event, int state, lacp_fsm_state_t *transition)
{
    clib_warning("%U-MUX: event %U, old state %U, new state %U", format_vnet_sw_if_index_name, vnet_get_main(),
                 sif->sw_if_index, format_mux_event, event, format_mux_sm_state, state, format_mux_sm_state,
...
{
    lacp_machine_dispatch(&lacp_mux_machine, vm, sif, LACP_MUX_EVENT_BEGIN, &sif->mux_state);
}
