...
lacp_ptx_action_no_periodic(void *p1, void *p2)
{
    ...
    lacp_machine_dispatch(&lacp_ptx_machine, vm, sif, LACP_PTX_EVENT_BEGIN, &sif->ptx_state);
...
lacp_ptx_action_slow_periodic(void *p1, void *p2)
{
    ...
    if (lacp_timer_is_running(sif->periodic_timer) && lacp_timer_is_expired(lm->vlib_main, sif->periodic_timer))
        timer_expired = 1;
    else
        timer_expired = 0;
...
lacp_ptx_action_fast_periodic(void *p1, void *p2)
{
    ...
    if (!(sif->partner.state & LACP_STATE_LACP_TIMEOUT))
        lacp_machine_dispatch(&lacp_ptx_machine, vm, sif, LACP_PTX_EVENT_LONG_TIMEOUT, &sif->ptx_state);
...
#undef _
    ...
    lacp_event_struct *event_entry = (lacp_event_struct *) &lacp_ptx_event_array;
...
void
lacp_ptx_debug_func(slave_if_t *sif, int event, int state, lacp_fsm_state_t *transition)
{
    clib_warning("%U-PTX: event %U, old state %U, new state %U", format_vnet_sw_if_index_name, vnet_get_main(),
                 sif->sw_if_index, format_ptx_event, event, format_ptx_sm_state, state, format_ptx_sm_state,
