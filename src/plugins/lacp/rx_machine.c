...
static lacp_fsm_machine_t lacp_rx_fsm_table[] = {
    {lacp_rx_state_initialize},    {lacp_rx_state_port_disabled}, {lacp_rx_state_expired},
    {lacp_rx_state_lacp_disabled}, {lacp_rx_state_defaulted},     {lacp_rx_state_current},
};
...
lacp_set_port_unselected(vlib_main_t *vm, slave_if_t *sif)
{
    ...
    lacp_machine_dispatch(&lacp_mux_machine, vm, sif, LACP_MUX_EVENT_UNSELECTED, &sif->mux_state);
}

static void
lacp_update_default_selected(vlib_main_t *vm, slave_if_t *sif)
{
    if ((sif->partner_admin.state & LACP_STATE_AGGREGATION) != (sif->partner.state & LACP_STATE_AGGREGATION) ||
        memcmp(&sif->partner, &sif->partner_admin, sizeof(sif->partner) - sizeof(sif->partner.state))) {
...
lacp_update_selected(vlib_main_t *vm, slave_if_t *sif)
{
    ...
    if ((lacpdu->actor.port_info.state & LACP_STATE_AGGREGATION) != (sif->partner.state & LACP_STATE_AGGREGATION) ||
        memcmp(&sif->partner, &lacpdu->actor.port_info, sizeof(sif->partner) - sizeof(sif->partner.state))) {
        lacp_set_port_unselected(vm, sif);
    }
}
...
lacp_update_ntt(vlib_main_t *vm, slave_if_t *sif)
{
    ...
    if ((states & lacpdu->partner.port_info.state) != (states & sif->actor.state) ||
        memcmp(&sif->actor, &lacpdu->partner.port_info, sizeof(sif->actor) - sizeof(sif->actor.state))) {
        sif->ntt = 1;
...
lacp_compare_partner(slave_if_t *sif)
{
    ...
    if ((!memcmp(&sif->partner, &lacpdu->actor.port_info, sizeof(sif->partner) - sizeof(sif->partner.state)) &&
         ((sif->actor.state & LACP_STATE_AGGREGATION) == (lacpdu->partner.port_info.state & LACP_STATE_AGGREGATION))) ||
...
lacp_set_port_moved(vlib_main_t *vm, slave_if_t *sif, u8 val)
{
    ...
    if (sif->port_moved)
        lacp_machine_dispatch(&lacp_rx_machine, vm, sif, LACP_RX_EVENT_PORT_MOVED, &sif->rx_state);
    else if (!sif->port_enabled)
        lacp_machine_dispatch(&lacp_rx_machine, vm, sif, LACP_RX_EVENT_PORT_DISABLED, &sif->rx_state);
}
...
lacp_rx_action_initialize(void *p1, void *p2)
{
    ...
    /* UCT */
    lacp_machine_dispatch(&lacp_rx_machine, vm, sif, LACP_RX_EVENT_BEGIN, &sif->rx_state);
...
lacp_rx_action_port_disabled(void *p1, void *p2)
{
    ...
    if (sif->port_enabled) {
        if (sif->lacp_enabled)
            lacp_machine_dispatch(&lacp_rx_machine, vm, sif, LACP_RX_EVENT_LACP_ENABLED, &sif->rx_state);
        else
            lacp_machine_dispatch(&lacp_rx_machine, vm, sif, LACP_RX_EVENT_LACP_DISABLED, &sif->rx_state);
    }
...
lacp_rx_action_expired(void *p1, void *p2)
{
    ...
    lacp_machine_dispatch(&lacp_ptx_machine, vm, sif, LACP_PTX_EVENT_SHORT_TIMEOUT, &sif->ptx_state);
    if (lacp_timer_is_running(sif->current_while_timer) && lacp_timer_is_expired(lm->vlib_main, sif->current_while_timer))
        timer_expired = 1;
    else
        timer_expired = 0;
    ...
    sif->actor.state |= LACP_STATE_EXPIRED;
    if (timer_expired)
        lacp_machine_dispatch(&lacp_rx_machine, vm, sif, LACP_RX_EVENT_TIMER_EXPIRED, &sif->rx_state);
    if (sif->last_rx_pkt && vec_len(sif->last_rx_pkt))
        lacp_machine_dispatch(&lacp_rx_machine, vm, sif, LACP_RX_EVENT_PDU_RECEIVED, &sif->rx_state);
...
#undef _
    ...
    lacp_event_struct *event_entry = (lacp_event_struct *) &lacp_rx_event_array;
...
void
lacp_rx_debug_func(slave_if_t *sif, int event, int state, lacp_fsm_state_t *transition)
{
    clib_warning("%U-RX: event %U, old state %U, new state %U", format_vnet_sw_if_index_name, vnet_get_main(),
                 sif->sw_if_index, format_rx_event, event, format_rx_sm_state, state, format_rx_sm_state,
