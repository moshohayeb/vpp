...
lacp_send_ethernet_lacp_pdu(slave_if_t *sif)
{
     ...
     */
    h0 = vlib_packet_template_get_packet(vm, &lm->packet_templates[sif->packet_template_index], &bi0);
...
static void
lacp_interface_enable_disable(vlib_main_t *vm, bond_if_t *bif, slave_if_t *sif, u8 enable)
{
    lacp_main_t *lm = &lacp_main;
    uword port_number;
    ...
    if (enable) {
        ...
        lm->lacp_int--;
        if (lm->lacp_int == 0) {
            vlib_process_signal_event(vm, lm->lacp_process_node_index, LACP_PROCESS_EVENT_STOP, 0);
        }
    }
}
...
lacp_periodic_init(vlib_main_t *vm)
{
    ...
    vlib_packet_template_init(vm, &lm->packet_templates[LACP_PACKET_TEMPLATE_ETHERNET],
                              /* data */ &h, sizeof(h),
                              /* alloc chunk size */ 8, "lacp-ethernet");
    ...
    vlib_packet_template_init(vm, &lm->marker_packet_templates[MARKER_PACKET_TEMPLATE_ETHERNET],
                              /* data */ &m, sizeof(m),
                              /* alloc chunk size */ 8, "marker-ethernet");
...
int
lacp_machine_dispatch(lacp_machine_t *machine, vlib_main_t *vm, slave_if_t *sif, int event, int *state)
{
...
void
lacp_init_neighbor(slave_if_t *sif, u8 *hw_address, u16 port_number, u32 group)
{
    lacp_stop_timer(&sif->wait_while_timer);
...
lacp_sw_interface_up_down(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    ...
    if (sif) {
        ...
        if (sif->port_enabled == 0) {
            if (sif->lacp_enabled) {
                lacp_init_neighbor(sif, sif->actor_admin.system, ntohs(sif->actor_admin.port_number),
