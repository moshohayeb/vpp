...
static void
lacp_send_sw_interface_details(vl_api_registration_t *reg, lacp_interface_details_t *lacp_if, u32 context)
{
    ...
    clib_memcpy(mp->interface_name, lacp_if->interface_name,
                MIN(ARRAY_LEN(mp->interface_name) - 1, strlen((const char *) lacp_if->interface_name)));
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + lm->msg_id_base);
...
lacp_plugin_api_hookup(vlib_main_t *vm)
{
    ...
    lm->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
