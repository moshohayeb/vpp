...
send_ethernet_marker_response_pdu(slave_if_t *sif)
{
     ...
     */
    h0 = vlib_packet_template_get_packet(vm, &lm->marker_packet_templates[sif->packet_template_index], &bi0);
...
handle_marker_protocol(vlib_main_t *vm, slave_if_t *sif)
{
     ...
     * But we may check the tlv lengths.
     */
    if ((marker->marker_info.tlv_length != sizeof(marker_information_t)) || (marker->terminator.tlv_length != 0))
        return (LACP_ERROR_BAD_TLV);
...
lacp_input(vlib_main_t *vm, vlib_buffer_t *b0, u32 bi0)
{
    ...
    sif = bond_get_slave_by_sw_if_index(vnet_buffer(b0)->sw_if_index[VLIB_RX]);
    ...
    if (marker->subtype == MARKER_SUBTYPE) {
        ...
        vec_validate(sif->last_marker_pkt, vlib_buffer_length_in_chain(vm, b0) - 1);
    ...
    last_packet_signature = hash_memory(sif->last_rx_pkt, vec_len(sif->last_rx_pkt), 0xd00b);

    bif = bond_get_master_by_dev_instance(sif->bif_dev_instance);
    if (sif->last_packet_signature_valid && (sif->last_packet_signature == last_packet_signature) &&
