...
int
nsim_enable_disable(nsim_main_t *nsm, u32 sw_if_index0, u32 sw_if_index1, int enable_disable)
{
    ...
    /* Utterly wrong? */
    if (pool_is_free_index(nsm->vnet_main->interface_main.sw_interfaces, sw_if_index0))
        return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    if (pool_is_free_index(nsm->vnet_main->interface_main.sw_interfaces, sw_if_index1))
        return VNET_API_ERROR_INVALID_SW_IF_INDEX;
...
nsim_configure(nsim_main_t *nsm, f64 bandwidth, f64 delay, f64 packet_size)
{
    ...
    for (i = num_workers ? 1 : 0; i < num_workers + 1; i++) {
        ...
        nsm->mmap_size = sizeof(nsim_wheel_t) + wheel_slots_per_worker * sizeof(nsim_wheel_entry_t);
 ...
 */
static clib_error_t *
nsim_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "disable"))
            enable_disable = 0;
        else if (unformat(input, "%U", unformat_vnet_sw_interface, nsm->vnet_main, &tmp)) {
            if (sw_if_index0 == ~0)
                sw_if_index0 = tmp;
            else
                sw_if_index1 = tmp;
...
VLIB_CLI_COMMAND(nsim_enable_disable_command, static) = {
    ...
    .short_help = "nsim enable-disable <interface-name-1> <interface-name-2> [disable]",
...
/* API message handler */
static void
vl_api_nsim_enable_disable_t_handler(vl_api_nsim_enable_disable_t *mp)
{
    vl_api_nsim_enable_disable_reply_t *rmp;
    nsim_main_t *nsm = &nsim_main;
    int rv;

    rv = nsim_enable_disable(nsm, ntohl(mp->sw_if_index0), ntohl(mp->sw_if_index1), (int) (mp->enable_disable));
...
nsim_plugin_api_hookup(vlib_main_t *vm)
{
    nsim_main_t *nsm = &nsim_main;
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + nsm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,              \
...
nsim_init(vlib_main_t *vm)
{
    ...
    nsm->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
...
set_nsim_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "Configured link delay %.2f ms, %.2f ms round-trip", nsm->delay * 1e3, 2.0 * nsm->delay * 1e3);

    if (num_workers)
        vlib_cli_output(vm, "Sim uses %llu bytes per thread, %llu bytes total", nsm->mmap_size,
                        nsm->mmap_size * num_workers);
    else
...
static clib_error_t *
show_nsim_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    nsim_main_t *nsm = &nsim_main;
    u32 num_workers  = vlib_num_workers();
    int verbose      = 0;
    ...
    vlib_cli_output(vm, "Network simulator cross-connects %U and %U", format_vnet_sw_if_index_name, nsm->vnet_main,
                    nsm->sw_if_index0, format_vnet_sw_if_index_name, nsm->vnet_main, nsm->sw_if_index1);

    vlib_cli_output(vm, "...inserting link delay of %.2f ms, %.2f ms round-trip", nsm->delay * 1e3,
                    2.0 * nsm->delay * 1e3);

    if (verbose) {

        vlib_cli_output(vm, "  Configured bandwidth: %.2f gbit/sec", nsm->bandwidth / 1e9);
