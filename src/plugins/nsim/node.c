...
nsim_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_trace)
{
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_TRUE(wp->cursize < wp->wheel_size)) {
            ...
            ep->tx_time = expires;
            ep->tx_sw_if_index =
                (vnet_buffer(b[0])->sw_if_index[VLIB_RX] == nsm->sw_if_index0) ? nsm->sw_if_index1 : nsm->sw_if_index0;
            ep->current_length = vlib_buffer_length_in_chain(vm, b[0]);
            ASSERT(ep->current_length <= WHEEL_ENTRY_DATA_SIZE);
            clib_memcpy(ep->data, vlib_buffer_get_current(b[0]), ep->current_length);
