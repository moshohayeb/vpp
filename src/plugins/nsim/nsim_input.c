...
format_nsim_tx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "NSIM: tx at %.6f sw_if_index %d", t->expired, t->tx_sw_if_index);
    return s;
}
    ...
    while (wp->cursize) {
        ...
        while (n_left_to_next > 0 && ep->tx_time <= now) {
            ...
            if (PREDICT_FALSE(_vec_len(my_buffer_cache) == 0)) {
                u32 n                     = vlib_buffer_alloc(vm, my_buffer_cache, VLIB_FRAME_SIZE);
                ...
                if (n == 0) {
                    ...
                    /* Count the drops */
                    vlib_node_increment_counter(vm, node->node_index, NSIM_TX_ERROR_DROPPED, drops);
            ...
            vnet_buffer(b0)->sw_if_index[VLIB_RX] =
                (ep->tx_sw_if_index == nsm->sw_if_index0) ? nsm->sw_if_index1 : nsm->sw_if_index0;
            next0 = (ep->tx_sw_if_index == nsm->sw_if_index0) ? nsm->output_next_index0 : nsm->output_next_index1;
...
VLIB_NODE_FN(nsim_input_node)(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
