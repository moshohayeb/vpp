...
#define foreach_standard_reply_retval_handler _(memif_delete_reply)
...
api_memif_socket_filename_add_del(vat_main_t *vam)
{
    ...
    strncpy((char *) mp->socket_filename, (char *) socket_filename, sizeof(mp->socket_filename) - 1);
...
vl_api_memif_details_t_handler(vl_api_memif_details_t *mp)
{
    ...
    fformat(vam->ofp,
            ...
            "   state %s link %s\n",
            mp->if_name, ntohl(mp->sw_if_index), format_ethernet_address, mp->hw_addr, clib_net_to_host_u32(mp->id),
            clib_net_to_host_u32(mp->socket_id), mp->role ? "slave" : "master", ntohl(mp->ring_size),
            ntohs(mp->buffer_size), mp->admin_up_down ? "up" : "down", mp->link_up_down ? "up" : "down");
}
...
static void
vl_api_memif_socket_filename_details_t_handler(vl_api_memif_socket_filename_details_t *mp)
{
    vat_main_t *vam = memif_test_main.vat_main;
