...
memif_interface_tx_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, memif_if_t *mif,
                          memif_ring_type_t type, memif_queue_t *mq, memif_per_thread_data_t *ptd)
{
    memif_ring_t *ring;
...
retry:
    ...
    while (n_left && free_slots) {
        ...
        CLIB_PREFETCH(&ring->desc[(slot + 8) & mask], CLIB_CACHE_LINE_BYTES, LOAD);
...
no_free_slots:
    ...
    while (n_copy_op >= 8) {
        ...
        clib_memcpy(co[0].data, b0->data + co[0].buffer_offset, co[0].data_len);
        clib_memcpy(co[1].data, b1->data + co[1].buffer_offset, co[1].data_len);
        clib_memcpy(co[2].data, b2->data + co[2].buffer_offset, co[2].data_len);
        clib_memcpy(co[3].data, b3->data + co[3].buffer_offset, co[3].data_len);
    ...
    if (n_left) {
        vlib_error_count(vm, node->node_index, MEMIF_TX_ERROR_NO_FREE_SLOTS, n_left);
    }
...
static_always_inline uword
memif_interface_tx_zc_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, memif_if_t *mif,
...
retry:
    ...
    while (n_left && free_slots) {
        ...
        CLIB_PREFETCH(&ring->desc[(slot + 8) & mask], CLIB_CACHE_LINE_BYTES, STORE);
...
VNET_DEVICE_CLASS_TX_FN(memif_device_class)(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (mif->flags & MEMIF_IF_FLAG_ZERO_COPY)
        ...
        return memif_interface_tx_inline(vm, node, frame, mif, MEMIF_RING_S2M, mq, ptd);
    else
        return memif_interface_tx_inline(vm, node, frame, mif, MEMIF_RING_M2S, mq, ptd);
}

static void
memif_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
...
static clib_error_t *
memif_interface_rx_mode_change(vnet_main_t *vnm, u32 hw_if_index, u32 qid, vnet_hw_interface_rx_mode mode)
{
    memif_main_t *mm        = &memif_main;
    vnet_hw_interface_t *hw = vnet_get_hw_interface(vnm, hw_if_index);
...
static clib_error_t *
memif_subif_add_del_function(vnet_main_t *vnm, u32 hw_if_index, struct vnet_sw_interface_t *st, int is_add)
{
    /* Nothing for now */
    return 0;
}
