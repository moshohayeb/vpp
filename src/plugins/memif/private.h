...
#define memif_log_debug(dev, f, ...)                                                                                   \
    do {                                                                                                               \
        ...
        if (_dev)                                                                                                      \
            vlib_log(VLIB_LOG_LEVEL_DEBUG, memif_main.log_class, "%U: " f, format_vnet_hw_if_index_name,               \
                     vnet_get_main(), _dev->hw_if_index, ##__VA_ARGS__);                                               \
        else                                                                                                           \
...
#define memif_log_warn(dev, f, ...)                                                                                    \
    do {                                                                                                               \
        memif_if_t *_dev = (memif_if_t *) dev;                                                                         \
        if (_dev)                                                                                                      \
            vlib_log(VLIB_LOG_LEVEL_WARNING, memif_main.log_class, "%U: " f, format_vnet_hw_if_index_name,             \
                     vnet_get_main(), _dev->hw_if_index, ##__VA_ARGS__);                                               \
        else                                                                                                           \
...
#define memif_log_err(dev, f, ...)                                                                                     \
    do {                                                                                                               \
        memif_if_t *_dev = (memif_if_t *) dev;                                                                         \
        if (_dev)                                                                                                      \
            vlib_log(VLIB_LOG_LEVEL_ERR, memif_main.log_class, "%U: " f, format_vnet_hw_if_index_name,                 \
                     vnet_get_main(), _dev->hw_if_index, ##__VA_ARGS__);                                               \
        else                                                                                                           \
...
#define memif_file_add(a, b)                                                                                           \
    do {                                                                                                               \
        *a = clib_file_add(&file_main, b);                                                                             \
        memif_log_warn(0, "clib_file_add fd %d private_data %u idx %u", (b)->file_descriptor, (b)->private_data, *a);  \
    } while (0)
