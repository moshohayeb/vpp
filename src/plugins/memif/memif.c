...
memif_disconnect(memif_if_t *mif, clib_error_t *err)
{
    ...
    vec_foreach_index(i, mif->rx_queues)
    {
        ...
        if (mq->ring) {
            ...
            if (rv)
                memif_log_warn(mif, "Unable to unassign interface %d, queue %d: rc=%d", mif->hw_if_index, i, rv);
            mq->ring = 0;
        }
    }
...
memif_connect(memif_if_t *mif)
{
    ...
    vec_foreach_index(i, mif->rx_queues)
    {
        ...
        rv = vnet_hw_interface_set_rx_mode(vnm, mif->hw_if_index, i, VNET_HW_INTERFACE_RX_MODE_DEFAULT);
        if (rv)
            memif_log_err(mif,
...
memif_init_regions_and_queues(memif_if_t *mif)
{
    ...
    buffer_offset = (mif->run.num_s2m_rings + mif->run.num_m2s_rings) *
                    (sizeof(memif_ring_t) + sizeof(memif_desc_t) * (1 << mif->run.log2_ring_size));

    r->region_size = buffer_offset;

    if ((mif->flags & MEMIF_IF_FLAG_ZERO_COPY) == 0)
        r->region_size +=
            mif->run.buffer_size * (1 << mif->run.log2_ring_size) * (mif->run.num_s2m_rings + mif->run.num_m2s_rings);
    ...
    vec_foreach_index(i, mif->tx_queues)
    {
        ...
        if (mif->flags & MEMIF_IF_FLAG_ZERO_COPY)
            vec_validate_aligned(mq->buffers, 1 << mq->log2_ring_size, CLIB_CACHE_LINE_BYTES);
    }
    /* *INDENT-ON* */
...
memif_socket_filename_add_del(u8 is_add, u32 sock_id, u8 *sock_filename)
{
    ...
    if (sock_filename[0] != '/') {
        ...
        sock_filename = format(0, "%s/%s%c", vlib_unix_get_runtime_dir(), sock_filename, 0);
    } else {
        ...
        if ((dir == NULL) || (faccessat(/* ignored */ -1, dir, F_OK | R_OK | W_OK, AT_EACCESS) < 0)) {
            vec_free(dir);
            return VNET_API_ERROR_INVALID_ARGUMENT;
        }
    }
...
memif_delete_if(vlib_main_t *vm, memif_if_t *mif)
{
    ...
    if (--(msf->ref_cnt) == 0) {
        if (msf->is_listener) {
            int i;
            /* *INDENT-OFF* */
            vec_foreach_index(i, msf->pending_clients) memif_socket_close(msf->pending_clients + i);
            /* *INDENT-ON* */
    ...
    if (pool_elts(mm->interfaces) == 0)
        vlib_process_signal_event(vm, memif_process_node.index, MEMIF_PROCESS_EVENT_STOP, 0);
...
memif_create_if(vlib_main_t *vm, memif_create_if_args_t *args)
{
    ...
    if (msf->ref_cnt == 0) {
        ...
        if (args->is_master && (stat((char *) msf->filename, &file_stat) == 0)) {
            if (S_ISSOCK(file_stat.st_mode)) {
                ...
                error = clib_error_return(0, "File exists for %s", msf->filename);
    ...
    if (mif->mode == MEMIF_INTERFACE_MODE_ETHERNET) {
        ...
        error = ethernet_register_interface(vnm, memif_device_class.index, mif->dev_instance, args->hw_addr,
                                            &mif->hw_if_index, memif_eth_flag_change);
    } else if (mif->mode == MEMIF_INTERFACE_MODE_IP) {
        mif->hw_if_index = vnet_register_interface(vnm, memif_device_class.index, mif->dev_instance,
    ...
    if (msf->is_listener && msf->ref_cnt == 0) {
        ...
        s->flags  = CLIB_SOCKET_F_IS_SERVER | CLIB_SOCKET_F_ALLOW_GROUP_WRITE | CLIB_SOCKET_F_SEQPACKET |
    ...
    vnet_hw_interface_set_input_node(vnm, mif->hw_if_index, memif_input_node.index);
