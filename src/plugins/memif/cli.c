...
static clib_error_t *
memif_socket_filename_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
memif_socket_filename_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    int r;
    u32 socket_id;
...
static clib_error_t *
memif_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    int r;
    ...
    if (r <= VNET_API_ERROR_SYSCALL_ERROR_1 && r >= VNET_API_ERROR_SYSCALL_ERROR_10)
...
static clib_error_t *
memif_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    u32 sw_if_index = ~0;
...
format_memif_queue(u8 *s, va_list *args)
{
    ...
    s = format(s, "%Uregion %u offset %u ring-size %u int-fd %d\n", format_white_space, indent + 4, mq->region,
               mq->offset, (1 << mq->log2_ring_size), mq->int_fd);

    if (mq->ring)
        s = format(s, "%Uhead %u tail %u flags 0x%04x interrupts %u\n", format_white_space, indent + 4, mq->ring->head,
                   mq->ring->tail, mq->ring->flags, mq->int_count);
...
format_memif_descriptor(u8 *s, va_list *args)
{
    ...
    if (ring) {
        ...
        s = format(s, "%Uid    flags   len         address       offset    user address\n", format_white_space, indent);
        s = format(s, "%U===== ===== ======== ================== ====== ==================\n", format_white_space,
                   indent);
        for (slot = 0; slot < ring_size; slot++) {
            s = format(s, "%U%-5d %-5d %-7d  0x%016lx %-6d 0x%016lx\n", format_white_space, indent, slot,
                       ring->desc[slot].flags, ring->desc[slot].length, mif->regions[ring->desc[slot].region].shm,
...
static clib_error_t *
memif_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    memif_main_t *mm = &memif_main;
    memif_if_t *mif;
    vnet_main_t *vnm = vnet_get_main();
    ...
    if (vec_len(hw_if_indices) == 0) {
        /* *INDENT-OFF* */
        pool_foreach(mif, mm->interfaces, vec_add1(hw_if_indices, mif->hw_if_index););
        /* *INDENT-ON* */
    }
    ...
    for (hw_if_index = 0; hw_if_index < vec_len(hw_if_indices); hw_if_index++) {
        ...
        vlib_cli_output(vm,
                        ...
                        mif->run.num_s2m_rings, mif->run.num_m2s_rings, mif->run.buffer_size, vec_len(mif->regions));
