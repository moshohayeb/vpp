...
static clib_error_t *
memif_msg_receive_init(memif_if_t **mifp, memif_msg_t *msg, clib_socket_t *sock, uword socket_file_index)
{
...
memif_msg_receive(memif_if_t **mifp, clib_socket_t *sock, clib_file_t *uf)
{
    ...
    case MEMIF_MSG_TYPE_HELLO:
        ...
        /* *INDENT-OFF* */
        vec_foreach_index(i, mif->regions) memif_msg_enq_add_region(mif, i);
        vec_foreach_index(i, mif->tx_queues) memif_msg_enq_add_ring(mif, i, MEMIF_RING_S2M);
        vec_foreach_index(i, mif->rx_queues) memif_msg_enq_add_ring(mif, i, MEMIF_RING_M2S);
        /* *INDENT-ON* */
    ...
    if (clib_fifo_elts(mif->msg_queue))
        clib_file_set_data_available_to_write(&file_main, mif->sock->private_data, 1);
    return 0;
...
memif_master_conn_fd_read_ready(clib_file_t *uf)
{
    ...
    if (p) {
        ...
        int i;
        vec_foreach_index(i, msf->pending_clients) if (msf->pending_clients[i]->fd == uf->file_descriptor)
        {
