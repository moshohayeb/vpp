...
memif_trace_buffer(vlib_main_t *vm, vlib_node_runtime_t *node, memif_if_t *mif, vlib_buffer_t *b, u32 next, u16 qid,
                   uword *n_tracep)
{
...
static_always_inline void
memif_add_copy_op(memif_per_thread_data_t *ptd, void *data, u32 len, u16 buffer_offset, u16 buffer_vec_index)
{
    memif_copy_op_t *co;
...
static_always_inline void
memif_add_to_chain(vlib_main_t *vm, vlib_buffer_t *b, u32 *buffers, u32 buffer_size)
{
    vlib_buffer_t *seg = b;
...
memif_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, memif_if_t *mif,
                          memif_ring_type_t type, u16 qid, memif_interface_mode_t mode)
{
    ...
    next_slot:
        CLIB_PREFETCH(&ring->desc[(cur_slot + 8) & mask], CLIB_CACHE_LINE_BYTES, LOAD);
        s0           = cur_slot & mask;
        d0           = &ring->desc[s0];
    ...
    while (n_left >= 8) {
        ...
        clib_memcpy(b0->data + co[0].buffer_offset, co[0].data, co[0].data_len);
        clib_memcpy(b1->data + co[1].buffer_offset, co[1].data, co[1].data_len);
        clib_memcpy(b2->data + co[2].buffer_offset, co[2].data, co[2].data_len);
        clib_memcpy(b3->data + co[3].buffer_offset, co[3].data, co[3].data_len);
    ...
    vlib_buffer_enqueue_to_next(vm, node, to_next_buffers, nexts, n_rx_packets);

    vlib_increment_combined_counter(vnm->interface_main.combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX,
                                    thread_index, mif->hw_if_index, n_rx_packets, n_rx_bytes);
...
static_always_inline uword
memif_device_input_zc_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, memif_if_t *mif, u16 qid,
                             memif_interface_mode_t mode)
{
    ...
    next_index =
        (mode == MEMIF_INTERFACE_MODE_IP) ? VNET_DEVICE_INPUT_NEXT_IP6_INPUT : VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT;
        ...
        while (n_from >= 8 && n_left_to_next >= 4) {
            ...
            if (mode == MEMIF_INTERFACE_MODE_IP) {
                ...
                if (PREDICT_FALSE(mif->per_interface_next_index != ~0)) {
                    ...
                    vnet_feature_start_device_input_x1(mif->sw_if_index, &next1, b1);
                    vnet_feature_start_device_input_x1(mif->sw_if_index, &next2, b2);
                    vnet_feature_start_device_input_x1(mif->sw_if_index, &next3, b3);
                }
            }
...
refill:
    ...
    n_alloc = vlib_buffer_alloc_to_ring(vm, mq->buffers, head & mask, ring_size, n_slots);
    ...
    while (n_alloc >= 32) {
        ...
        d0->offset = (void *) b0->data - mif->regions[d0->region].shm + start_offset;
        d1->offset = (void *) b1->data - mif->regions[d1->region].shm + start_offset;
        d2->offset = (void *) b2->data - mif->regions[d2->region].shm + start_offset;
        d3->offset = (void *) b3->data - mif->regions[d3->region].shm + start_offset;
        ...
        if ((mif->flags & MEMIF_IF_FLAG_ADMIN_UP) && (mif->flags & MEMIF_IF_FLAG_CONNECTED)) {
            if (mif->flags & MEMIF_IF_FLAG_ZERO_COPY) {
                if (mif->mode == MEMIF_INTERFACE_MODE_IP)
                    n_rx += memif_device_input_zc_inline(vm, node, frame, mif, dq->queue_id, mode_ip);
                else
                    n_rx += memif_device_input_zc_inline(vm, node, frame, mif, dq->queue_id, mode_eth);
            } else if (mif->flags & MEMIF_IF_FLAG_IS_SLAVE) {
                if (mif->mode == MEMIF_INTERFACE_MODE_IP)
                    n_rx += memif_device_input_inline(vm, node, frame, mif, MEMIF_RING_M2S, dq->queue_id, mode_ip);
                else
                    n_rx += memif_device_input_inline(vm, node, frame, mif, MEMIF_RING_M2S, dq->queue_id, mode_eth);
            } else {
                if (mif->mode == MEMIF_INTERFACE_MODE_IP)
                    n_rx += memif_device_input_inline(vm, node, frame, mif, MEMIF_RING_S2M, dq->queue_id, mode_ip);
                else
                    n_rx += memif_device_input_inline(vm, node, frame, mif, MEMIF_RING_S2M, dq->queue_id, mode_eth);
            }
        }
    }
