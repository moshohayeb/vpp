...
format_gtpu_rx_trace(u8 *s, va_list *args)
{
    ...
    if (t->tunnel_index != ~0) {
        s = format(s, "GTPU decap from gtpu_tunnel%d teid %d next %d error %d", t->tunnel_index, t->teid, t->next_index,
                   t->error);
    } else {
        s = format(s, "GTPU decap error - tunnel for teid %d does not exist", t->teid);
    }
    return s;
}
...
always_inline uword
gtpu_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u32 is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        trace0:
            ...
            if (is_ip4) {
                ...
                /* Validate GTPU tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip6_address_is_equal(&ip6_1->dst_address, &t1->src.ip6)))
                    goto next1; /* valid packet */
    ...
    vlib_node_increment_counter(vm, is_ip4 ? gtpu4_input_node.index : gtpu6_input_node.index, GTPU_ERROR_DECAPSULATED,
                                pkts_decapsulated);
...
static uword
gtpu4_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return gtpu_input(vm, node, from_frame, /* is_ip4 */ 1);
}

static uword
gtpu6_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword
ip_gtpu_bypass_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 is_ip4)
{
    gtpu_main_t *gtm = &gtpu_main;
...
static uword
ip4_gtpu_bypass(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ip6_gtpu_bypass(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
