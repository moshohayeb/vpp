...
format_gtpu_tunnel(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d] src %U dst %U teid %d fib-idx %d sw-if-idx %d ", t - ngm->tunnels, format_ip46_address, &t->src,
               IP46_TYPE_ANY, format_ip46_address, &t->dst, IP46_TYPE_ANY, t->teid, t->encap_fib_index, t->sw_if_index);
...
gtpu_tunnel_restack_dpo(gtpu_tunnel_t *t)
{
    ...
    fib_forward_chain_type_t forw_type =
        ip46_address_is_ip4(&t->dst) ? FIB_FORW_CHAIN_TYPE_UNICAST_IP4 : FIB_FORW_CHAIN_TYPE_UNICAST_IP6;
...
{
    uword *vtep =
        ip46_address_is_ip4(ip) ? hash_get(gtpu_main.vtep4, ip->ip4.as_u32) : hash_get_mem(gtpu_main.vtep6, &ip->ip6);
    if (vtep)
        return ++(*vtep);
    ip46_address_is_ip4(ip) ? hash_set(gtpu_main.vtep4, ip->ip4.as_u32, 1) :
...
vtep_addr_unref(ip46_address_t *ip)
{
    ...
    if (--(*vtep) != 0)
        return *vtep;
    ip46_address_is_ip4(ip) ? hash_unset(gtpu_main.vtep4, ip->ip4.as_u32) :
...
int
vnet_gtpu_add_del_tunnel(vnet_gtpu_add_del_tunnel_args_t *a, u32 *sw_if_indexp)
{
    gtpu_main_t *gtm = &gtpu_main;
    gtpu_tunnel_t *t = 0;
...
#undef _
        ...
        if (vec_len(gtm->free_gtpu_tunnel_hw_if_indices) > 0) {
            ...
            hw_if_index = vnet_register_interface(vnm, gtpu_device_class.index, t - gtm->tunnels, gtpu_hw_class.index,
                                                  t - gtm->tunnels);
            hi          = vnet_get_hw_interface(vnm, hw_if_index);
        }
        ...
        if (!ip46_address_is_multicast(&t->dst)) {
            ...
            t->sibling_index = fib_entry_child_add(t->fib_entry_index, gtm->fib_node_type, t - gtm->tunnels);
            ...
            if (vtep_addr_ref(&t->dst) == 1) {
                ...
                mfei                 = mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_GTPU, &path,
...
static clib_error_t *
gtpu_add_del_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del")) {
            ...
            dst_set  = 1;
            ipv6_set = 1;
        } else if (unformat(line_input, "group %U %U", unformat_ip4_address, &dst.ip4, unformat_vnet_sw_interface,
                            vnet_get_main(), &mcast_sw_if_index)) {
            grp_set = dst_set = 1;
            ipv4_set          = 1;
        } else if (unformat(line_input, "group %U %U", unformat_ip6_address, &dst.ip6, unformat_vnet_sw_interface,
                            vnet_get_main(), &mcast_sw_if_index)) {
            grp_set = dst_set = 1;
            ipv6_set          = 1;
...
VLIB_CLI_COMMAND(create_gtpu_tunnel_command, static) = {
    ...
    .short_help = "create gtpu tunnel src <local-vtep-addr>"
...
static clib_error_t *
show_gtpu_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    gtpu_main_t *gtm = &gtpu_main;
    gtpu_tunnel_t *t;
    ...
    pool_foreach(t, gtm->tunnels, ({ vlib_cli_output(vm, "%U", format_gtpu_tunnel, t); }));
...
static clib_error_t *
set_ip_gtpu_bypass(u32 is_ip6, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
set_ip4_gtpu_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
set_ip6_gtpu_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
gtpu_init(vlib_main_t *vm)
{
    ...
    udp_register_dst_port(vm, UDP_DST_PORT_GTPU, gtpu4_input_node.index, /* is_ip4 */ 1);
    udp_register_dst_port(vm, UDP_DST_PORT_GTPU6, gtpu6_input_node.index, /* is_ip4 */ 0);
