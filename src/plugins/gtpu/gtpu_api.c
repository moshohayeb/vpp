...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + gtm->msg_id_base);
...
static void
vl_api_sw_interface_set_gtpu_bypass_t_handler(vl_api_sw_interface_set_gtpu_bypass_t *mp)
{
...
static void
send_gtpu_tunnel_details(gtpu_tunnel_t *t, vl_api_registration_t *reg, u32 context)
{
...
vl_api_gtpu_tunnel_dump_t_handler(vl_api_gtpu_tunnel_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        /* *INDENT-OFF* */
        pool_foreach(t, gtm->tunnels, ({ send_gtpu_tunnel_details(t, reg, mp->context); }));
        /* *INDENT-ON* */
    } else {
...
gtpu_api_hookup(vlib_main_t *vm)
{
    ...
    gtm->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
