...
static void
vl_api_gtpu_add_del_tunnel_reply_t_handler(vl_api_gtpu_add_del_tunnel_reply_t *mp)
{
...
#define foreach_standard_reply_retval_handler _(sw_interface_set_gtpu_bypass_reply)
...
vl_api_gtpu_tunnel_details_t_handler(vl_api_gtpu_tunnel_details_t *mp)
{
    ...
    print(vam->ofp, "%11d%24U%24U%14d%18d%13d%19d", ntohl(mp->sw_if_index), format_ip46_address, &src, IP46_TYPE_ANY,
          ...
          ntohl(mp->teid), ntohl(mp->mcast_sw_if_index));
}
...
api_gtpu_tunnel_dump(vat_main_t *vam)
{
    ...
    if (!vam->json_output) {
        print(vam->ofp, "%11s%24s%24s%14s%18s%13s%19s", "sw_if_index", "src_address", "dst_address", "encap_vrf_id",
              "decap_next_index", "teid", "mcast_sw_if_index");
    }
...
#define foreach_vpe_api_msg                                                                                            \
    _(sw_interface_set_gtpu_bypass, "<intfc> | sw_if_index <id> [ip4 | ip6] [enable | disable]")                       \
    _(gtpu_add_del_tunnel, "src <ip-addr> { dst <ip-addr> | group <mcast-ip-addr>\n"                                   \
...
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + gtm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,              \
