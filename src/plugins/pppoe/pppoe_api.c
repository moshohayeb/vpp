...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + pem->msg_id_base);
...
static void
vl_api_pppoe_add_del_session_t_handler(vl_api_pppoe_add_del_session_t *mp)
{
...
static void
send_pppoe_session_details(pppoe_session_t *t, vl_api_registration_t *reg, u32 context)
{
...
vl_api_pppoe_session_dump_t_handler(vl_api_pppoe_session_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        /* *INDENT-OFF* */
        pool_foreach(t, pem->sessions, ({ send_pppoe_session_details(t, reg, mp->context); }));
        /* *INDENT-ON* */
    } else {
...
pppoe_api_hookup(vlib_main_t *vm)
{
    ...
    pem->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
