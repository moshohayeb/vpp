...
format_pppoe_session(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d] sw-if-index %d client-ip %U session-id %d ", t - pem->sessions, t->sw_if_index,
               format_ip46_address, &t->client_ip, IP46_TYPE_ANY, t->session_id);

    s = format(s, "encap-if-index %d decap-fib-index %d\n", t->encap_if_index, t->decap_fib_index);

    s = format(s, "    local-mac %U  client-mac %U", format_ethernet_address, t->local_mac, format_ethernet_address,
               t->client_mac);
...
static u8 *
pppoe_build_rewrite(vnet_main_t *vnm, u32 sw_if_index, vnet_link_t link_type, const void *dst_address)
{
 ...
 */
static void
pppoe_fixup(vlib_main_t *vm, ip_adjacency_t *adj, vlib_buffer_t *b0, const void *data)
{
    ...
    pppoe0->length = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b0) - sizeof(pppoe_header_t) +
...
pppoe_update_adj(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
    ...
    case IP_LOOKUP_NEXT_BCAST:
        adj_nbr_midchain_update_rewrite(ai, pppoe_fixup, t, ADJ_FLAG_NONE,
    ...
    case IP_LOOKUP_NEXT_MCAST:
         ...
         */
        adj_mcast_midchain_update_rewrite(ai, pppoe_fixup, t, ADJ_FLAG_NONE,
                                          pppoe_build_rewrite(vnm, sw_if_index, adj->ia_link, NULL), 0, 0);
        break;
...
int
vnet_pppoe_add_del_session(vnet_pppoe_add_del_session_args_t *a, u32 *sw_if_indexp)
{
    pppoe_main_t *pem  = &pppoe_main;
    pppoe_session_t *t = 0;
    ...
    pppoe_lookup_1(&pem->session_table, &cached_key, &cached_result, a->client_mac, clib_host_to_net_u16(a->session_id),
                   &key, &bucket, &result);
...
#undef _
        ...
        if (vec_len(pem->free_pppoe_session_hw_if_indices) > 0) {
            ...
            hw_if_index = vnet_register_interface(vnm, pppoe_device_class.index, t - pem->sessions,
        ...
        fib_table_entry_path_add(a->decap_fib_index, &pfx, FIB_SOURCE_PLUGIN_HI, FIB_ENTRY_FLAG_NONE,
        ...
        fib_table_entry_path_remove(a->decap_fib_index, &pfx, FIB_SOURCE_PLUGIN_HI, fib_proto_to_dpo(pfx.fp_proto),
...
static clib_error_t *
pppoe_add_del_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
VLIB_CLI_COMMAND(create_pppoe_session_command, static) = {
    ...
    .short_help = "create pppoe session client-ip <client-ip> session-id <nn>"
...
/* *INDENT-OFF* */
static clib_error_t *
show_pppoe_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    pppoe_main_t *pem = &pppoe_main;
    pppoe_session_t *t;
    ...
    pool_foreach(t, pem->sessions, ({ vlib_cli_output(vm, "%U", format_pppoe_session, t); }));
...
pppoe_show_walk_cb(BVT(clib_bihash_kv) * kvp, void *arg)
{
    ...
    vlib_cli_output(ctx->vm, "%=19U%=12d%=13d%=14d", format_ethernet_address, key.fields.mac,
...
static clib_error_t *
show_pppoe_fib_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    pppoe_main_t *pem         = &pppoe_main;
