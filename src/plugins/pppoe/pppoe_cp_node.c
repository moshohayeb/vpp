...
static u8 *
format_pppoe_cp_trace(u8 *s, va_list *args)
{
    ...
    if (t->sw_if_index != pem->cp_if_index) {
        s = format(s,
                   ...
                   t->sw_if_index, t->next_index, t->error, t->pppoe_code, t->ppp_proto);
    } else {
        s = format(s,
                   "PPPoE dispatch from cp_if_index %d next %d error %d \n"
                   "  pppoe_code 0x%x  ppp_proto 0x%x",
                   t->cp_if_index, t->next_index, t->error, t->pppoe_code, t->ppp_proto);
    }
    return s;
}

static uword
pppoe_cp_dispatch(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (rx_sw_if_index0 == pem->cp_if_index) {
                pppoe_lookup_1(&pem->link_table, &cached_key, &cached_result, h0->dst_address, 0, &key0, &bucket0,
                               &result0);
                ...
                pppoe_lookup_1(&pem->link_table, &cached_key, &cached_result, h0->src_address, 0, &key0, &bucket0,
                               &result0);
                tx_sw_if_index0 = result0.fields.sw_if_index;
