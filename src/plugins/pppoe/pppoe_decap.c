...
static u8 *
format_pppoe_rx_trace(u8 *s, va_list *args)
{
    ...
    if (t->session_index != ~0) {
        s = format(s, "PPPoE decap from pppoe_session%d session_id %d next %d error %d", t->session_index,
                   t->session_id, t->next_index, t->error);
    } else {
...
static uword
pppoe_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            pppoe_lookup_1(&pem->session_table, &cached_key, &cached_result, h0->src_address, pppoe0->session_id, &key0,
                           &bucket0, &result0);
        ...
        trace0:
            ...
            pppoe_lookup_1(&pem->session_table, &cached_key, &cached_result, h1->src_address, pppoe1->session_id, &key1,
                           &bucket1, &result1);
