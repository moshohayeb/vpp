...
static void
vl_api_pppoe_add_del_session_reply_t_handler(vl_api_pppoe_add_del_session_reply_t *mp)
{
...
static void
vl_api_pppoe_session_details_t_handler(vl_api_pppoe_session_details_t *mp)
{
    vat_main_t *vam          = &vat_main;
    ip46_address_t client_ip = to_ip46(mp->is_ipv6, mp->client_ip);

    print(vam->ofp, "%11d%14d%24U%14d%14d%30U%30U", ntohl(mp->sw_if_index), ntohl(mp->session_id), format_ip46_address,
          &client_ip, IP46_TYPE_ANY, ntohl(mp->encap_if_index), ntohl(mp->decap_vrf_id), format_ethernet_address,
          mp->local_mac, format_ethernet_address, mp->client_mac);
}
...
api_pppoe_session_dump(vat_main_t *vam)
{
    ...
    if (!vam->json_output) {
        print(vam->ofp, "%11s%24s%14s%14s%14s", "sw_if_index", "client_ip", "session_id", "encap_if_index",
              "decap_fib_index", "local-mac", "client-mac");
    }
...
#define foreach_vpe_api_msg                                                                                            \
    _(pppoe_add_del_session, " client-addr <client-addr> session-id <nn>"                                              \
...
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + pem->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,              \
