...
feature_init(mactime_main_t *mm)
{
    if (mm->feature_initialized == 0) {
        ...
        clib_bihash_init_8_8(&mm->lookup_table, "mactime lookup table", mm->lookup_table_num_buckets,
                             mm->lookup_table_memory_size);
        clib_timebase_init(&mm->timebase, mm->timezone_offset, CLIB_TIMEBASE_DAYLIGHT_USA);
 ...
 */
int
mactime_enable_disable(mactime_main_t *mm, u32 sw_if_index, int enable_disable)
{
...
static clib_error_t *
mactime_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
mactime_plugin_api_hookup(vlib_main_t *vm)
{
    mactime_main_t *mm = &mactime_main;
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + mm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,               \
...
mactime_init(vlib_main_t *vm)
{
    ...
    mm->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
...
static clib_error_t *
show_mactime_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    mactime_main_t *mm = &mactime_main;
    mactime_device_t *dp;
    u8 *macstring = 0;
    ...
    /* *INDENT-OFF* */
    pool_foreach(dp, mm->devices, ({ vec_add1(pool_indices, dp - mm->devices); }));
    /* *INDENT-ON* */

    vlib_cli_output(vm, "%-15s %18s %14s %10s %10s %10s", "Device Name", "Addresses", "Status", "AllowPkt", "AllowByte",
                    "DropPkt");
    ...
    for (i = 0; i < vec_len(pool_indices); i++) {
        ...
        for (j = 0; j < vec_len(dp->ranges); j++) {
            ...
            if (verbose > 1)
                vlib_cli_output(vm, "  Range %d: %U - %U", j, format_clib_timebase_time, start0,
            ...
            if (now >= start0 && now <= end0) {
                ...
                if (verbose) {
                    ...
                    vlib_cli_output(vm, "     %U - %U", format_clib_timebase_time, start0, format_clib_timebase_time,
                                    end0);
                }
                goto print;
            }
    ...
    print:
        ...
        vlib_cli_output(vm, "%-15s %18s %14s %10lld %10lld %10lld", dp->device_name, macstring, status_string,
        ...
        for (j = 0; j < vec_len(mm->arp_cache_copy); j++) {
            ...
            if (!memcmp(dp->mac_address, n->ethernet_address, sizeof(n->ethernet_address))) {
                vlib_cli_output(vm, "%17s%U", " ", format_ip4_address, &n->ip4_address);
            }
        }
    }
    vec_free(macstring);
...
static clib_error_t *
clear_mactime_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    mactime_main_t *mm = &mactime_main;
