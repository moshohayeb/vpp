...
static uword
mactime_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_tx)
{
    ...
    while (n_left_from > 0) {
        ...
        trace0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ...
                if (dp) {
                    clib_memcpy(t->device_name, dp->device_name, ARRAY_LEN(t->device_name));
    ...
    vlib_node_increment_counter(vm, node->node_index, MACTIME_ERROR_DROP, packets_dropped);
    vlib_node_increment_counter(vm, node->node_index, MACTIME_ERROR_OK, packets_ok);
    return frame->n_vectors;
}

static uword
mactime_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
mactime_tx_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
