...
tls_init_ctr_seed_drbgs(void)
{
    ...
    if ((rv = mbedtls_ctr_drbg_seed(&tm->ctr_drbgs[thread_index], mbedtls_entropy_func, &tm->entropy_pools[thread_index],
                                    (const unsigned char *) pers, vec_len(pers))) != 0) {
        vec_free(pers);
...
mbedtls_ctx_init_client(tls_ctx_t *ctx)
{
    ...
    if ((rv = mbedtls_ssl_config_defaults(&mc->conf, MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM,
...
mbedtls_ctx_init_server(tls_ctx_t *ctx)
{
    ...
    if (!app->tls_cert || !app->tls_key) {
        TLS_DBG(1, " failed\n  ! tls cert and/or key not configured %d", ctx->parent_app_index);
        return -1;
    }

    rv = mbedtls_x509_crt_parse(&mc->srvcert, (const unsigned char *) app->tls_cert, vec_len(app->tls_cert));
    if (rv != 0) {
        TLS_DBG(1, " failed\n  !  mbedtls_x509_crt_parse returned %d", rv);
        goto exit;
    }

    rv = mbedtls_pk_parse_key(&mc->pkey, (const unsigned char *) app->tls_key, vec_len(app->tls_key), NULL, 0);
    if (rv != 0) {
     ...
     */
    if ((rv = mbedtls_ssl_config_defaults(&mc->conf, MBEDTLS_SSL_IS_SERVER, MBEDTLS_SSL_TRANSPORT_STREAM,
                                          MBEDTLS_SSL_PRESET_DEFAULT)) != 0) {
...
mbedtls_ctx_write(tls_ctx_t *ctx, stream_session_t *app_session)
{
    ...
    svm_fifo_peek(app_session->server_tx_fifo, 0, deq_now, mm->tx_bufs[thread_index]);
...
mbedtls_ctx_read(tls_ctx_t *ctx, stream_session_t *tls_session)
{
    ...
    enq = svm_fifo_enqueue_nowait(app_session->server_rx_fifo, read, mm->rx_bufs[thread_index]);
    ASSERT(enq == read);
