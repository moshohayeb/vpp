...
static clib_error_t *
l2_emulation_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    if (is_add) {
        vec_validate_init_empty(l2_emulations, sw_if_index, ezero);
    }
...
static clib_error_t *
l2_emulation_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
VLIB_CLI_COMMAND(l2_emulation_cli_node, static) = {
    ...
    .short_help = "set interface l2 l2-emulation <interface-name> [disable|enable]\n",
    .function   = l2_emulation_cli,
};
/* *INDENT-ON* */

static clib_error_t *
l2_emulation_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm = vnet_get_main();
    l2_emulation_t *l23e;
    u32 sw_if_index;
...
static uword
l2_emulation_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                l2_emulation_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->extracted            = (next0 != ~0);
            }
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
                l2_emulation_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
                t->extracted            = (next1 != ~0);
            }

            /* Determine the next node and remove ourself from bitmap */
            if (PREDICT_TRUE(next0 == ~0))
                next0 = vnet_l2_feature_next(b0, em->l2_input_feat_next, L2INPUT_FEAT_L2_EMULATION);

            /* Determine the next node and remove ourself from bitmap */
            if (PREDICT_TRUE(next1 == ~0))
                next1 = vnet_l2_feature_next(b1, em->l2_input_feat_next, L2INPUT_FEAT_L2_EMULATION);
    ...
    vlib_node_increment_counter(vm, node->node_index, L2_EMULATION_ERROR_IP4, ip4_hits);
    vlib_node_increment_counter(vm, node->node_index, L2_EMULATION_ERROR_IP6, ip6_hits);
...
l2_emulation_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2_emulation_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(),
