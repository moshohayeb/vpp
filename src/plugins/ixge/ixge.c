...
u32
ixge_read_write_phy_reg(ixge_device_t *xd, u32 dev_type, u32 reg_index, u32 v, u32 is_read)
{
    ixge_regs_t *r     = xd->regs;
...
format_ixge_rx_from_hw_descriptor(u8 *s, va_list *va)
{
    ...
    s = format(s, "%s-owned", (s2 & IXGE_RX_DESCRIPTOR_STATUS2_IS_OWNED_BY_SOFTWARE) ? "sw" : "hw");
    s = format(s, ", length this descriptor %d, l3 offset %d", d->n_packet_bytes_this_descriptor,
    ...
    if (s2 & IXGE_RX_DESCRIPTOR_STATUS2_IS_VLAN)
        s = format(s, "vlan header 0x%x\n%U", d->vlan_tag, format_white_space, indent);

    if ((is_ip4 = (s0 & IXGE_RX_DESCRIPTOR_STATUS0_IS_IP4))) {
        s = format(s, "ip4%s", (s0 & IXGE_RX_DESCRIPTOR_STATUS0_IS_IP4_EXT) ? " options" : "");
        if (s2 & IXGE_RX_DESCRIPTOR_STATUS2_IS_IP4_CHECKSUMMED)
            s = format(s, " checksum %s", (s2 & IXGE_RX_DESCRIPTOR_STATUS2_IP4_CHECKSUM_ERROR) ? "bad" : "ok");
    }
    if ((is_ip6 = (s0 & IXGE_RX_DESCRIPTOR_STATUS0_IS_IP6)))
        s = format(s, "ip6%s", (s0 & IXGE_RX_DESCRIPTOR_STATUS0_IS_IP6_EXT) ? " extended" : "");
    is_tcp = is_udp = 0;
    ...
    if (s2 & IXGE_RX_DESCRIPTOR_STATUS2_IS_TCP_CHECKSUMMED)
        s = format(s, ", tcp checksum %s", (s2 & IXGE_RX_DESCRIPTOR_STATUS2_TCP_CHECKSUM_ERROR) ? "bad" : "ok");
    if (s2 & IXGE_RX_DESCRIPTOR_STATUS2_IS_UDP_CHECKSUMMED)
        s = format(s, ", udp checksum %s", (s2 & IXGE_RX_DESCRIPTOR_STATUS2_UDP_CHECKSUM_ERROR) ? "bad" : "ok");
...
format_ixge_tx_descriptor(u8 *s, va_list *va)
{
    ...
    s = format(s, "buffer 0x%Lx, %d packet bytes, %d bytes this buffer", d->buffer_address, s1 >> 14,
               d->n_bytes_this_buffer);
    ...
    s = format(s, "%s%s%s%s%s%s%s%s", (s0 & (1 << 8)) ? "eop, " : "", (s0 & (1 << 9)) ? "insert-fcs, " : "",
               (s0 & (1 << 10)) ? "reserved26, " : "", (s0 & (1 << 11)) ? "report-status, " : "",
               (s0 & (1 << 12)) ? "reserved28, " : "", (s0 & (1 << 13)) ? "is-advanced, " : "",
               (s0 & (1 << 14)) ? "vlan-enable, " : "", (s0 & (1 << 15)) ? "tx-segmentation, " : "");
...
format_ixge_rx_dma_trace(u8 *s, va_list *va)
{
...

    {
        ...
        s                       = format(s, "%U rx queue %d", format_vnet_sw_interface_name, vnm, sw, t->queue_index);
    }

    s = format(s, "\n%Ubefore: %U", format_white_space, indent, format_ixge_rx_from_hw_descriptor, &t->before);
    s = format(s, "\n%Uafter : head/tail address 0x%Lx/0x%Lx", format_white_space, indent,
               t->after.rx_to_hw.head_address, t->after.rx_to_hw.tail_address);

    s = format(s, "\n%Ubuffer 0x%x: %U", format_white_space, indent, t->buffer_index, format_vnet_buffer, &t->buffer);
...
always_inline void
ixge_rx_next_and_error_from_status_x1(ixge_device_t *xd, u32 s00, u32 s02, u8 *next0, u8 *error0, u32 *flags0)
{
    ...
    e0 = (is0_ip4 && (s02 & IXGE_RX_DESCRIPTOR_STATUS2_IP4_CHECKSUM_ERROR) ? IXGE_ERROR_ip4_checksum_error : e0);

    is0_ip6 = s00 & IXGE_RX_DESCRIPTOR_STATUS0_IS_IP6;
    n0      = is0_ip6 ? IXGE_RX_NEXT_IP6_INPUT : n0;

    n0 = (xd->per_interface_next_index != ~0) ? xd->per_interface_next_index : n0;

    /* Check for error. */
    n0 = e0 != IXGE_ERROR_none ? IXGE_RX_NEXT_DROP : n0;

    f0 = ((s02 & (IXGE_RX_DESCRIPTOR_STATUS2_IS_TCP_CHECKSUMMED | IXGE_RX_DESCRIPTOR_STATUS2_IS_UDP_CHECKSUMMED)) ?
              VNET_BUFFER_F_L4_CHECKSUM_COMPUTED :
              0);

    f0 |= ((s02 & (IXGE_RX_DESCRIPTOR_STATUS2_TCP_CHECKSUM_ERROR | IXGE_RX_DESCRIPTOR_STATUS2_UDP_CHECKSUM_ERROR)) ?
               0 :
...
always_inline void
ixge_rx_next_and_error_from_status_x2(ixge_device_t *xd, u32 s00, u32 s02, u32 s10, u32 s12, u8 *next0, u8 *error0,
                                      u32 *flags0, u8 *next1, u8 *error1, u32 *flags1)
{
    u8 is0_ip4, is0_ip6, n0, e0;
    ...
    e0 = (is0_ip4 && (s02 & IXGE_RX_DESCRIPTOR_STATUS2_IP4_CHECKSUM_ERROR) ? IXGE_ERROR_ip4_checksum_error : e0);
    e1 = (is1_ip4 && (s12 & IXGE_RX_DESCRIPTOR_STATUS2_IP4_CHECKSUM_ERROR) ? IXGE_ERROR_ip4_checksum_error : e1);
    ...
    n0 = (xd->per_interface_next_index != ~0) ? xd->per_interface_next_index : n0;
    n1 = (xd->per_interface_next_index != ~0) ? xd->per_interface_next_index : n1;
    ...
    *next1 = n1;
    ...
    f0 = ((s02 & (IXGE_RX_DESCRIPTOR_STATUS2_IS_TCP_CHECKSUMMED | IXGE_RX_DESCRIPTOR_STATUS2_IS_UDP_CHECKSUMMED)) ?
              VNET_BUFFER_F_L4_CHECKSUM_COMPUTED :
              0);
    f1 = ((s12 & (IXGE_RX_DESCRIPTOR_STATUS2_IS_TCP_CHECKSUMMED | IXGE_RX_DESCRIPTOR_STATUS2_IS_UDP_CHECKSUMMED)) ?
              VNET_BUFFER_F_L4_CHECKSUM_COMPUTED :
              0);
    ...
    f0 |= ((s02 & (IXGE_RX_DESCRIPTOR_STATUS2_TCP_CHECKSUM_ERROR | IXGE_RX_DESCRIPTOR_STATUS2_UDP_CHECKSUM_ERROR)) ?
               0 :
               VNET_BUFFER_F_L4_CHECKSUM_CORRECT);
    f1 |= ((s12 & (IXGE_RX_DESCRIPTOR_STATUS2_TCP_CHECKSUM_ERROR | IXGE_RX_DESCRIPTOR_STATUS2_UDP_CHECKSUM_ERROR)) ?
               0 :
               VNET_BUFFER_F_L4_CHECKSUM_CORRECT);
...
static void
ixge_rx_trace(ixge_main_t *xm, ixge_device_t *xd, ixge_dma_queue_t *dq, ixge_descriptor_t *before_descriptors,
              u32 *before_buffers, ixge_descriptor_t *after_descriptors, uword n_descriptors)
{
    vlib_main_t *vm           = xm->vlib_main;
    ...
    while (n_left >= 2) {
        ...
        ixge_rx_next_and_error_from_status_x2(xd, bd[0].status[0], bd[0].status[2], bd[1].status[0], bd[1].status[2],
    ...
    while (n_left >= 1) {
        ...
        ixge_rx_next_and_error_from_status_x1(xd, bd[0].status[0], bd[0].status[2], &next0, &error0, &flags0);
...
format_ixge_tx_dma_trace(u8 *s, va_list *va)
{
    ...
    {
        vnet_sw_interface_t *sw = vnet_get_sw_interface(vnm, xd->vlib_sw_if_index);
        s                       = format(s, "%U tx queue %d", format_vnet_sw_interface_name, vnm, sw, t->queue_index);
    }

    s = format(s, "\n%Udescriptor: %U", format_white_space, indent, format_ixge_tx_descriptor, &t->descriptor);
...
static void
ixge_tx_trace(ixge_main_t *xm, ixge_device_t *xd, ixge_dma_queue_t *dq, ixge_tx_state_t *tx_state,
              ixge_tx_descriptor_t *descriptors, u32 *buffers, uword n_descriptors)
{
    vlib_main_t *vm           = xm->vlib_main;
...
ixge_tx_descriptor_matches_template(ixge_main_t *xm, ixge_tx_descriptor_t *d)
{
    ...
    cmp = ((d->status0 & xm->tx_descriptor_template_mask.status0) ^ xm->tx_descriptor_template.status0);
    if (cmp)
        return 0;
    cmp = ((d->status1 & xm->tx_descriptor_template_mask.status1) ^ xm->tx_descriptor_template.status1);
    if (cmp)
        return 0;
...
static uword
ixge_tx_no_wrap(ixge_main_t *xm, ixge_device_t *xd, ixge_dma_queue_t *dq, u32 *buffers, u32 start_descriptor_index,
                u32 n_descriptors, ixge_tx_state_t *tx_state)
{
    ...
    if (tx_state->node->flags & VLIB_NODE_FLAG_TRACE) {
        ...
        ixge_tx_trace(xm, xd, dq, tx_state, &dq->descriptors[start_descriptor_index].tx, to_tx, n_descriptors);
    }
...
static uword
ixge_rx_queue_no_wrap(ixge_main_t *xm, ixge_device_t *xd, ixge_dma_queue_t *dq, u32 start_descriptor_index,
                      u32 n_descriptors)
{
    vlib_main_t *vm           = xm->vlib_main;
    vlib_node_runtime_t *node = dq->rx.node;
...

    {
        ...
        if (l < n_descriptors_left) {
            ...
            n_allocated                     = vlib_buffer_alloc(vm, xm->rx_buffers_to_add + l, n_to_alloc);
            ...
            if (PREDICT_FALSE(l + n_allocated <= n_descriptors_left)) {
                if (n_allocated == 0)
                    vlib_error_count(vm, ixge_input_node.index, IXGE_ERROR_rx_alloc_no_physmem, 1);
                else
                    vlib_error_count(vm, ixge_input_node.index, IXGE_ERROR_rx_alloc_fail, 1);
    ...
    while (n_descriptors_left > 0) {
	  ...
	  ASSERT (VLIB_BUFFER_KNOWN_ALLOCATED == vlib_buffer_is_known (fi1));
            ...
            ixge_rx_next_and_error_from_status_x2(xd, s00, s20, s01, s21, &next0, &error0, &flags0, &next1, &error1,
                                                  &flags1);
            ...
            d0.rx_to_hw.tail_address = vlib_get_buffer_data_physical_address(vm, fi0);
            d1.rx_to_hw.tail_address = vlib_get_buffer_data_physical_address(vm, fi1);
            ...
            l3_offset0 =
                (is_sop && (next0 != IXGE_RX_NEXT_ETHERNET_INPUT)) ? IXGE_RX_DESCRIPTOR_STATUS0_L3_OFFSET(s00) : 0;
            l3_offset1 =
                (is_eop0 && (next1 != IXGE_RX_NEXT_ETHERNET_INPUT)) ? IXGE_RX_DESCRIPTOR_STATUS0_L3_OFFSET(s01) : 0;
            ...
            if (0) /* "Dave" version */
            {
                ...
                if (is_eop0) {
                    ...
                    vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi_sop0, next0);
                }
                if (is_eop1) {
                    to_next[0] = bi_sop1;
                    to_next++;
                    ...
                    vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi_sop1, next1);
                }
                is_sop = is_eop1;
                bi_sop = bi_sop1;
            }
    ...
    found_hw_owned_descriptor_x2:
	  ...
	  ASSERT (VLIB_BUFFER_KNOWN_ALLOCATED == vlib_buffer_is_known (fi0));
            ...
            is_eop0 = (s20 & IXGE_RX_DESCRIPTOR_STATUS2_IS_END_OF_PACKET) != 0;
            ixge_rx_next_and_error_from_status_x1(xd, s00, s20, &next0, &error0, &flags0);
            ...
            if (0) /* "Dave" version */
            {
                if (is_eop0) {
                    ...
                    vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi_sop, next0);
                }
            }
            if (1) /* "Eliot" version */
            {
...
found_hw_owned_descriptor_x1:
...

    {
        ...
        if (n_trace > 0 && n_done > 0) {
            ...
            ixge_rx_trace(xm, xd, dq, d_trace_save, d_trace_buffers, &dq->descriptors[start_descriptor_index], n);
...
static uword
ixge_rx_queue(ixge_main_t *xm, ixge_device_t *xd, vlib_node_runtime_t *node, u32 queue_index)
{
...
ixge_interrupt(ixge_main_t *xm, ixge_device_t *xd, u32 i)
{
    ...
    if (i != 20) {
        ...
        vlib_process_signal_event(vm, ixge_process_node.index, EVENT_SET_FLAGS, ((is_up << 31) | xd->vlib_hw_if_index));
    }
}
...
ixge_tx_queue(ixge_main_t *xm, ixge_device_t *xd, u32 queue_index)
{
     ...
     */
    while ((dq->head_index == dq->tx.head_index_write_back[0]) && dq->tx.n_buffers_on_ring &&
           (dq->head_index != dq->tail_index)) {
        hwbp_race++;
    ...
    last_to_clean = (last_to_clean < 0) ? last_to_clean + dq->n_descriptors : last_to_clean;

    first_to_clean = (last_to_clean) - (n_clean - 1);
    first_to_clean = (first_to_clean < 0) ? first_to_clean + dq->n_descriptors : first_to_clean;
...
static uword
ixge_device_input(ixge_main_t *xm, ixge_device_t *xd, vlib_node_runtime_t *node)
{
    ixge_regs_t *r = xd->regs;
    u32 i, s;
...
format_ixge_device_name(u8 *s, va_list *args)
{
    ...
    return format(s, "TenGigabitEthernet%x/%x/%x/%x", addr->domain, addr->bus, addr->slot, addr->function);
}
...
format_ixge_link_status(u8 *s, va_list *args)
{
...

    {
        ...
        s = format(s, ", mode %s, speed %s", modes[(v >> 26) & 3], speeds[(v >> 28) & 3]);
    }
...
format_ixge_device(u8 *s, va_list *args)
{
    ...
    s = format(s, "Intel 8259X: id %U\n%Ulink %U", format_ixge_device_id, xd->device_id, format_white_space, indent + 2,
...

    {
        ...
        if (d)
            s = format(s, "\n%UPCIe %U", format_white_space, indent + 2, format_vlib_pci_link_speed, d);
    }
    ...
    /* FIXME */
    {
        ...
        s = format(s, "\n%U%d unprocessed, %d total buffers on rx queue 0 ring", format_white_space, indent + 2, nitems,
                   dq->n_descriptors);

        s = format(s, "\n%U%d buffers in driver rx cache", format_white_space, indent + 2,
                   vec_len(xm->rx_buffers_to_add));

        s = format(s, "\n%U%d buffers on tx queue 0 ring", format_white_space, indent + 2,
...
#undef _64
        ...
        for (i = 0; i < ARRAY_LEN(names); i++) {
            ...
            if (v != 0)
                s = format(s, "\n%U%-40U%16Ld", format_white_space, indent + 2, format_c_identifier, names[i], v);
        }
    }
 ...
 */
static void
ixge_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ixge_main_t *xm         = &ixge_main;
    ...
    xd->per_interface_next_index = vlib_node_add_next(xm->vlib_main, ixge_input_node.index, node_index);
}
...
#define IXGE_N_BYTES_IN_RX_BUFFER                                                                                      \
    (2048)   // DAW-HACK: Set Rx buffer size so all packets < ETH_MTU_SIZE fit in the buffer (i.e. sop & eop for all descriptors).
...
ixge_dma_init(ixge_device_t *xd, vlib_rx_or_tx_t rt, u32 queue_index)
{
    ...
    dq->descriptors =
        vlib_physmem_alloc_aligned(vm, xm->physmem_region, &error, dq->n_descriptors * sizeof(dq->descriptors[0]),
                                   128 /* per chip spec */);
    if (error)
        return error;
    ...
    if (rt == VLIB_RX) {
        ...
        n_alloc = vlib_buffer_alloc(vm, dq->descriptor_buffer_indices, vec_len(dq->descriptor_buffer_indices));
        ASSERT(n_alloc == vec_len(dq->descriptor_buffer_indices));
        for (i = 0; i < n_alloc; i++) {
            dq->descriptors[i].rx_to_hw.tail_address =
                vlib_get_buffer_data_physical_address(vm, dq->descriptor_buffer_indices[i]);
        }
    } else {
        u32 i;

        dq->tx.head_index_write_back = vlib_physmem_alloc(vm, xm->physmem_region, &error, CLIB_CACHE_LINE_BYTES);
...

    {
        ...
        a                         = vlib_physmem_virtual_to_physical(vm, xm->physmem_region, dq->descriptors);
        ...
        if (rt == VLIB_RX) {
            ...
            dr->rx_split_control = (/* buffer size */ ((xm->n_bytes_in_rx_buffer / 1024) << 0) |
            ...
            a = vlib_physmem_virtual_to_physical(vm, xm->physmem_region, dq->tx.head_index_write_back);
...
ixge_device_init(ixge_main_t *xm)
{
    ...
    vec_foreach(xd, xm->devices)
    {
        ...
        /* Register ethernet interface. */
        {
            ...
            error = ethernet_register_interface(vnm, ixge_device_class.index, xd->device_index,
                                                /* ethernet address */ addr8, &xd->vlib_hw_if_index, ixge_flag_change);
            if (error)
        ...
        r->interrupt.queue_mapping[0] = ((/* valid bit */ (1 << 7) | ixge_rx_queue_to_interrupt(0)) << 0);

        r->interrupt.queue_mapping[0] |= ((/* valid bit */ (1 << 7) | ixge_tx_queue_to_interrupt(0)) << 8);
        ...
        case ~0:
            ...
            if (vlib_time_now(vm) > link_debounce_deadline) {
                vec_foreach(xd, xm->devices)
                {
                    ...
                    vnet_hw_interface_set_flags(vnm, xd->vlib_hw_if_index, is_up ? VNET_HW_INTERFACE_FLAG_LINK_UP : 0);
                }
                link_debounce_deadline = 1e70;
                timeout                = 30.0;
            }
...
ixge_init(vlib_main_t *vm)
{
    ...
    xm->tx_descriptor_template.status0 = (IXGE_TX_DESCRIPTOR_STATUS0_ADVANCED | IXGE_TX_DESCRIPTOR_STATUS0_IS_ADVANCED |
    ...
    xm->tx_descriptor_template_mask.status0 &=
        ~(IXGE_TX_DESCRIPTOR_STATUS0_IS_END_OF_PACKET | IXGE_TX_DESCRIPTOR_STATUS0_REPORT_STATUS);
...
ixge_pci_init(vlib_main_t *vm, vlib_pci_dev_handle_t h)
{
    ...
    if (xm->physmem_region_allocated == 0) {
        error =
            vlib_physmem_region_alloc(vm, "ixge decriptors", 2 << 20, 0, VLIB_PHYSMEM_F_INIT_MHEAP, &xm->physmem_region);
    ...
    /* Chip found so enable node. */
    {
        vlib_node_set_state(vm, ixge_input_node.index,
                            (IXGE_ALWAYS_POLL ? VLIB_NODE_STATE_POLLING : VLIB_NODE_STATE_INTERRUPT));
