...
avf_rxq_refill(vlib_main_t *vm, vlib_node_runtime_t *node, avf_rxq_t *rxq, int use_iova)
{
    ...
    n_alloc = vlib_buffer_alloc_to_ring(vm, rxq->bufs, slot, rxq->size, n_refill);
    ...
    while (n_alloc >= 4) {
        ...
        if (use_iova) {
            ...
            d[0]->qword[0] = vlib_get_buffer_data_physical_address(vm, rxq->bufs[s0]);
            d[1]->qword[0] = vlib_get_buffer_data_physical_address(vm, rxq->bufs[s1]);
            d[2]->qword[0] = vlib_get_buffer_data_physical_address(vm, rxq->bufs[s2]);
            d[3]->qword[0] = vlib_get_buffer_data_physical_address(vm, rxq->bufs[s3]);
        }
...
static_always_inline void
avf_check_for_error(vlib_node_runtime_t *node, avf_rx_vector_entry_t *rxve, vlib_buffer_t *b, u16 *next)
{
...
avf_find_next(avf_rx_vector_entry_t *rxve, vlib_buffer_t *b, int maybe_tagged)
{
    avf_main_t *am       = &avf_main;
...
avf_process_rx_burst(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *bt, avf_rx_vector_entry_t *rxve,
                     vlib_buffer_t **b, u16 *next, u32 n_rxv, u8 maybe_error, int known_next)
{
    uword n_rx_bytes = 0;
    ...
    while (n_rxv >= 4) {
        ...
        if (!known_next) {
            ...
            if (ethernet_frame_is_any_tagged_x4(e0->type, e1->type, e2->type, e3->type)) {
            ...
            vnet_buffer(b[0])->feature_arc_index = vnet_buffer(bt)->feature_arc_index;
            vnet_buffer(b[1])->feature_arc_index = vnet_buffer(bt)->feature_arc_index;
            vnet_buffer(b[2])->feature_arc_index = vnet_buffer(bt)->feature_arc_index;
            vnet_buffer(b[3])->feature_arc_index = vnet_buffer(bt)->feature_arc_index;
        }

        clib_memcpy(vnet_buffer(b[0])->sw_if_index, vnet_buffer(bt)->sw_if_index, 2 * sizeof(u32));
        clib_memcpy(vnet_buffer(b[1])->sw_if_index, vnet_buffer(bt)->sw_if_index, 2 * sizeof(u32));
        clib_memcpy(vnet_buffer(b[2])->sw_if_index, vnet_buffer(bt)->sw_if_index, 2 * sizeof(u32));
        clib_memcpy(vnet_buffer(b[3])->sw_if_index, vnet_buffer(bt)->sw_if_index, 2 * sizeof(u32));
...
static_always_inline uword
avf_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, avf_device_t *ad, u16 qid)
{
    avf_main_t *am              = &avf_main;
    ...
    while (n_rxv < AVF_RX_VECTOR_SZ) {
        if (rxq->next + 11 < rxq->size) {
            int stride = 8;
            CLIB_PREFETCH((void *) (rxq->descs + (rxq->next + stride)), CLIB_CACHE_LINE_BYTES, LOAD);
            CLIB_PREFETCH((void *) (rxq->descs + (rxq->next + stride + 1)), CLIB_CACHE_LINE_BYTES, LOAD);
            CLIB_PREFETCH((void *) (rxq->descs + (rxq->next + stride + 2)), CLIB_CACHE_LINE_BYTES, LOAD);
            CLIB_PREFETCH((void *) (rxq->descs + (rxq->next + stride + 3)), CLIB_CACHE_LINE_BYTES, LOAD);
        }
        ...
        /* *INDENT-OFF* */
        q1x4 = (u64x4){d[0].qword[1], d[1].qword[1], d[2].qword[1], d[3].qword[1]};
        /* *INDENT-ON* */
    ...
    one_by_one:
#endif
        CLIB_PREFETCH((void *) (rxq->descs + ((rxq->next + 8) & mask)), CLIB_CACHE_LINE_BYTES, LOAD);
    ...
    if (known_next) {
        ...
        n_rx_bytes = avf_process_rx_burst(vm, node, bt, ptd->rx_vector, bufs, nexts, n_rxv, maybe_error, 1);
        vnet_buffer(bt)->feature_arc_index = 0;
        bt->current_config_index           = 0;
    } else
        n_rx_bytes = avf_process_rx_burst(vm, node, bt, ptd->rx_vector, bufs, nexts, n_rxv, maybe_error, 0);
    ...
    vlib_increment_combined_counter(vnm->interface_main.combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX, thr_idx,
