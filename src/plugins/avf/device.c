...
clib_error_t *
avf_aq_desc_enq(vlib_main_t *vm, avf_device_t *ad, avf_aq_desc_t *dt, void *data, int len)
{
    ...
    if (len) {
        ...
        d->addr_hi = (u32)(pa >> 32);
        d->addr_lo = (u32) pa;
        clib_memcpy(ad->atq_bufs + ad->atq_next_slot * AVF_MBOX_BUF_SZ, data, len);
...
clib_error_t *
avf_cmd_rx_ctl_reg_write(vlib_main_t *vm, avf_device_t *ad, u32 reg, u32 val)
{
    clib_error_t *err;
...
avf_rxq_init(vlib_main_t *vm, avf_device_t *ad, u16 qid, u16 rxq_size)
{
    ...
    rxq->size  = rxq_size;
    rxq->next  = 0;
    rxq->descs = vlib_physmem_alloc_aligned(vm, am->physmem_region, &error, rxq->size * sizeof(avf_rx_desc_t),
...
avf_txq_init(vlib_main_t *vm, avf_device_t *ad, u16 qid, u16 txq_size)
{
    ...
    txq->size  = txq_size;
    txq->next  = 0;
    txq->descs = vlib_physmem_alloc_aligned(vm, am->physmem_region, &error, txq->size * sizeof(avf_tx_desc_t),
                                            2 * CLIB_CACHE_LINE_BYTES);
...
{
    avf_main_t *am = &avf_main;
    return (ad->flags & AVF_DEVICE_F_IOVA) ? pointer_to_uword(p) :
...
clib_error_t *
avf_send_to_pf(vlib_main_t *vm, avf_device_t *ad, virtchnl_ops_t op, void *in, int in_len, void *out, int out_len)
{
    clib_error_t *err;
...
retry:
    ...
    if (d->v_opcode == VIRTCHNL_OP_EVENT) {
        ...
        if ((d->datalen != sizeof(virtchnl_pf_event_t)) || ((d->flags & AVF_AQ_F_BUF) == 0))
...
clib_error_t *
avf_op_version(vlib_main_t *vm, avf_device_t *ad, virtchnl_version_info_t *ver)
{
    clib_error_t *err             = 0;
    ...
    err = avf_send_to_pf(vm, ad, VIRTCHNL_OP_VERSION, &myver, sizeof(virtchnl_version_info_t), ver,
...
clib_error_t *
avf_op_get_vf_resources(vlib_main_t *vm, avf_device_t *ad, virtchnl_vf_resource_t *res)
{
    u32 bitmap = (VIRTCHNL_VF_OFFLOAD_L2 | VIRTCHNL_VF_OFFLOAD_RSS_PF | VIRTCHNL_VF_OFFLOAD_WB_ON_ITR |
                  VIRTCHNL_VF_OFFLOAD_VLAN | VIRTCHNL_VF_OFFLOAD_RX_POLLING);

    return avf_send_to_pf(vm, ad, VIRTCHNL_OP_GET_VF_RESOURCES, &bitmap, sizeof(u32), res,
...
{
    int count   = 1;
    int msg_len = sizeof(virtchnl_irq_map_info_t) + count * sizeof(virtchnl_vector_map_t);
    u8 msg[msg_len];
...
avf_op_add_eth_addr(vlib_main_t *vm, avf_device_t *ad, u8 count, u8 *macs)
{
    ...
    return avf_send_to_pf(vm, ad, VIRTCHNL_OP_ADD_ETH_ADDR, msg, msg_len, 0, 0);
}
...
clib_error_t *
avf_op_get_stats(vlib_main_t *vm, avf_device_t *ad, virtchnl_eth_stats_t *es)
{
    virtchnl_queue_select_t qs = {0};
    qs.vsi_id                  = ad->vsi_id;
    return avf_send_to_pf(vm, ad, VIRTCHNL_OP_GET_STATS, &qs, sizeof(virtchnl_queue_select_t), es,
...
avf_request_queues(vlib_main_t *vm, avf_device_t *ad, u16 num_queue_pairs)
{
    ...
    error = avf_send_to_pf(vm, ad, VIRTCHNL_OP_REQUEST_QUEUES, &res_req, sizeof(virtchnl_vf_res_request_t), &res_req,
...
clib_error_t *
avf_device_init(vlib_main_t *vm, avf_main_t *am, avf_device_t *ad, avf_create_if_args_t *args)
{
    ...
    if (args->rxq_num == 0) {
        ...
        vlib_log_warn(am->log_class,
                      "Requested more rx queues than"
    ...
    if ((ad->feature_bitmap & VIRTCHNL_VF_OFFLOAD_RSS_PF) && (error = avf_op_config_rss_lut(vm, ad)))
        return error;

    if ((ad->feature_bitmap & VIRTCHNL_VF_OFFLOAD_RSS_PF) && (error = avf_op_config_rss_key(vm, ad)))
        return error;
...
avf_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        /* *INDENT-OFF* */
        pool_foreach(ad, am->devices, { avf_process_one_device(vm, ad, irq); });
        /* *INDENT-ON* */
...
avf_irq_0_handler(vlib_pci_dev_handle_t h, u16 line)
{
    ...
    if (icr0 & (1 << 30))
        vlib_process_signal_event(vm, avf_process_node.index, AVF_PROCESS_EVENT_AQ_INT, 0);
}
...
avf_create_if(vlib_main_t *vm, avf_create_if_args_t *args)
{
    ...
    if (am->physmem_region_alloc == 0) {
        ...
        error     = vlib_physmem_region_alloc(vm, "avf descriptors", 4 << 20, 0, flags, &am->physmem_region);
        if (error)
            goto error;
        am->physmem_region_alloc = 1;
    }
    ad->atq = vlib_physmem_alloc_aligned(vm, am->physmem_region, &error, sizeof(avf_aq_desc_t) * AVF_MBOX_LEN, 64);
    if (error)
        goto error;

    ad->arq = vlib_physmem_alloc_aligned(vm, am->physmem_region, &error, sizeof(avf_aq_desc_t) * AVF_MBOX_LEN, 64);
    if (error)
        goto error;

    ad->atq_bufs = vlib_physmem_alloc_aligned(vm, am->physmem_region, &error, AVF_MBOX_BUF_SZ * AVF_MBOX_LEN, 64);
    if (error)
        goto error;

    ad->arq_bufs = vlib_physmem_alloc_aligned(vm, am->physmem_region, &error, AVF_MBOX_BUF_SZ * AVF_MBOX_LEN, 64);
    if (error)
        goto error;
    ...
    /* create interface */
    error = ethernet_register_interface(vnm, avf_device_class.index, ad->dev_instance, ad->hwaddr, &ad->hw_if_index,
                                        avf_flag_change);
    ...
    vnet_hw_interface_set_input_node(vnm, ad->hw_if_index, avf_input_node.index);

    for (i = 0; i < ad->n_rx_queues; i++)
        vnet_hw_interface_assign_rx_thread(vnm, ad->hw_if_index, i, ~0);

    if (pool_elts(am->devices) == 1)
        vlib_process_signal_event(vm, avf_process_node.index, AVF_PROCESS_EVENT_START, 0);
...
static clib_error_t *
avf_interface_rx_mode_change(vnet_main_t *vnm, u32 hw_if_index, u32 qid, vnet_hw_interface_rx_mode mode)
{
    avf_main_t *am          = &avf_main;
...
static void
avf_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    avf_main_t *am          = &avf_main;
    vnet_hw_interface_t *hw = vnet_get_hw_interface(vnm, hw_if_index);
    avf_device_t *ad        = pool_elt_at_index(am->devices, hw->dev_instance);
