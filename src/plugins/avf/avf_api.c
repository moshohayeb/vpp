...
vl_api_avf_create_t_handler(vl_api_avf_create_t *mp)
{
    ...
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_AVF_CREATE_REPLY + am->msg_id_base, ({ rmp->sw_if_index = ntohl(args.sw_if_index); }));
    /* *INDENT-ON* */
}
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + avm->msg_id_base);
...
avf_plugin_api_hookup(vlib_main_t *vm)
{
    ...
    avm->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);

#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + avm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,              \
