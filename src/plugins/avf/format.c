...
format_avf_device(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               format_white_space, indent, ad->num_queue_pairs, ad->max_vectors, ad->max_mtu, ad->rss_key_size,
               ad->rss_lut_size);
...
#define _(c)                                                                                                           \
    if (ad->eth_stats.c)                                                                                               \
        a = format(a, "\n%U%-20U %u", format_white_space, indent + 2, format_c_identifier, #c, ad->eth_stats.c);
...
format_avf_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "avf: %v (%d) next-node %U", hi->name, t->hw_if_index, format_vlib_next_node_name, vm, node->index,
               t->next_index);
    s = format(s, "\n%Ustatus 0x%x error 0x%x ptype 0x%x length %u", format_white_space, indent + 2, rxve->status,
               rxve->error, rxve->ptype, rxve->length);
