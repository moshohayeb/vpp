...
end_ad_processing(vlib_buffer_t *b0, ip6_header_t *ip0, ip6_sr_header_t *sr0, ip6_sr_localsid_t *ls0, u32 *next0)
{
...
srv6_ad_localsid_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            vlib_increment_combined_counter(((next0 == SRV6_AD_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) :
 ...
 */
static uword
srv6_ad2_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ls0_mem == NULL || ls0_mem->rewrite == NULL)) {
                ...
                clib_memcpy(((u8 *) en0) - ls0_mem->rw_len, ls0_mem->rewrite, ls0_mem->rw_len);
            ...
            vlib_increment_combined_counter(((next0 == SRV6_AD_LOCALSID_NEXT_ERROR) ? &(sm->invalid_counters) :
                                                                                      &(sm->valid_counters)),
 ...
 */
static uword
srv6_ad4_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ip6_sr_main_t *srm = &sr_main;
    srv6_ad_main_t *sm = &srv6_ad_main;
    u32 n_left_from, next_index, *from, *to_next;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ls0_mem == NULL || ls0_mem->rewrite == NULL)) {
                ...
                clib_memcpy(((u8 *) ip0_encap) - ls0_mem->rw_len, ls0_mem->rewrite, ls0_mem->rw_len);
                vlib_buffer_advance(b0, -(word) ls0_mem->rw_len);
 ...
 */
static uword
srv6_ad6_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ip6_sr_main_t *srm = &sr_main;
    srv6_ad_main_t *sm = &srv6_ad_main;
    u32 n_left_from, next_index, *from, *to_next;
