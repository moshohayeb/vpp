...
static void
nsh_md2_ioam_set_clear_output_feature_on_intf(vlib_main_t *vm, u32 sw_if_index0, u8 is_add)
{


    vnet_feature_enable_disable("ip4-output", "nsh-md2-ioam-encap-transit", sw_if_index0, is_add,
...
nsh_md2_ioam_clear_output_feature_on_all_intfs(vlib_main_t *vm)
{
    ...
    pool_foreach(si, im->sw_interfaces, ({ nsh_md2_ioam_set_clear_output_feature_on_intf(vm, si->sw_if_index, 0); }));
    return;
}
...
int
nsh_md2_ioam_enable_disable_for_dest(vlib_main_t *vm, ip46_address_t dst_addr, u32 outer_fib_index, u8 is_ipv4, u8 is_add)
{
...
#define TRANSIT_UNIT_TEST_HACK 1
  ...
  for (i = 0; i < lb_m->lb_n_buckets; i++)
    ...
    vec_foreach(sw_if_index0, intf_list) if (is_add)
    {
        vnet_feature_enable_disable("ip4-output", "nsh-md2-ioam-encap-transit", *sw_if_index0, is_add, 0,
                                    /* void *feature_config */
                                    0 /* u32 n_feature_config_bytes */
        );
...
nsh_md2_ioam_refresh_output_feature_on_all_dest(void)
{
    ...
    pool_foreach(t, hm->dst_tunnels, ({
                     nsh_md2_ioam_enable_disable_for_dest(gm->vlib_main, t->dst_addr, t->outer_fib_index,
...
clib_error_t *
nsh_md2_ioam_enable_disable(int has_trace_option, int has_pot_option, int has_ppc_option)
{
    nsh_md2_ioam_main_t *hm = &nsh_md2_ioam_main;
...
int
nsh_md2_ioam_disable_for_dest(vlib_main_t *vm, ip46_address_t dst_addr, u32 outer_fib_index, u8 ipv4_set)
{
    nsh_md2_ioam_dest_tunnels_t *t;
    nsh_md2_ioam_main_t *hm = &nsh_md2_ioam_main;
    nsh_main_t *gm          = &nsh_main;
...
static clib_error_t *
nsh_md2_ioam_set_transit_rewrite_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    nsh_main_t *gm = &nsh_main;
    ...
    if (dst_addr_set == 0)
        return clib_error_return(0, "LISP-GPE Tunnel destination address not specified");
