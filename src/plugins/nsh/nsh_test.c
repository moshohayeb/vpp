...
static int
api_nsh_add_del_entry(vat_main_t *vam)
{
...
static void
vl_api_nsh_entry_details_t_handler(vl_api_nsh_entry_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    fformat(vam->ofp, "%11d%11d%11d%11d%14d%14d%14d%14d%14d\n", mp->ver_o_c, mp->length, mp->md_type, mp->next_protocol,
            ntohl(mp->nsp_nsi), ntohl(mp->c1), ntohl(mp->c2), ntohl(mp->c3), ntohl(mp->c4));
}

static int
api_nsh_entry_dump(vat_main_t *vam)
{
    nsh_test_main_t *sm = &nsh_test_main;
...
static int
api_nsh_add_del_map(vat_main_t *vam)
{
    nsh_test_main_t *sm          = &nsh_test_main;
    unformat_input_t *line_input = vam->input;
    f64 timeout;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            ...
            next_node =
                NSH_NODE_NEXT_DROP;   // Once moved to NSHSFC see nsh.h:foreach_nsh_input_next to handle this case
        else
...
static void
vl_api_nsh_map_details_t_handler(vl_api_nsh_map_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    fformat(vam->ofp, "%14d%14d%14d%14d\n", ntohl(mp->nsp_nsi), ntohl(mp->mapped_nsp_nsi), ntohl(mp->sw_if_index),
            ntohl(mp->next_node));
}

static int
api_nsh_map_dump(vat_main_t *vam)
{
    nsh_test_main_t *sm = &nsh_test_main;
...
#define foreach_vpe_api_msg                                                                                            \
    ...
    _(nsh_entry_dump, "")                                                                                              \
    _(nsh_add_del_map, "nsp <nn> nsi <nn> [del] mapped-nsp <nn> mapped-nsi <nn> [encap-gre-intf <nn> | "               \
                       "encap-vxlan-gpe-intf <nn> | encap-none]")                                                      \
    _(nsh_map_dump, "")
