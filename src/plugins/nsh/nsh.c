...
int
nsh_md2_register_option(u16 class, u8 type, u8 option_size, int add_options(u8 *opt, u8 *opt_size),
                        ...
                        int pop_options(vlib_buffer_t *b, nsh_tlv_header_t *opt), u8 *trace(u8 *s, nsh_tlv_header_t *opt))
{
...
int
nsh_md2_unregister_option(u16 class, u8 type, int options(vlib_buffer_t *b, nsh_tlv_header_t *opt),
...
format_nsh_header(u8 *s, va_list *args)
{
    ...
    s = format(s, "len %d (%d bytes) md_type %d next_protocol %d\n", (nsh_base->length & NSH_LEN_MASK),
               (nsh_base->length & NSH_LEN_MASK) * 4, nsh_base->md_type, nsh_base->next_protocol);
    ...
    if (nsh_base->md_type == 1) {
        s = format(s, "  c1 %d c2 %d c3 %d c4 %d\n", clib_net_to_host_u32(nsh_md1->c1), clib_net_to_host_u32(nsh_md1->c2),
                   clib_net_to_host_u32(nsh_md1->c3), clib_net_to_host_u32(nsh_md1->c4));
...
format_nsh_map(u8 *s, va_list *args)
{
    ...
    s = format(s, "nsh entry nsp: %d nsi: %d ", (map->nsp_nsi >> NSH_NSP_SHIFT) & NSH_NSP_MASK,
               map->nsp_nsi & NSH_NSI_MASK);
    s = format(s, "maps to nsp: %d nsi: %d ", (map->mapped_nsp_nsi >> NSH_NSP_SHIFT) & NSH_NSP_MASK,
...
static clib_error_t *
nsh_add_del_map_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (next_node == ~0)
        return clib_error_return(0, "must specific action: [encap-gre-intf <nn> | encap-vxlan-gpe-intf <nn> | "
                                    "encap-lisp-gpe-intf <nn> | encap-none <tx_sw_if_index> <rx_sw_if_index>]");
...
VLIB_CLI_COMMAND(create_nsh_map_command, static) = {
    .path       = "create nsh map",
    .short_help = "create nsh map nsp <nn> nsi <nn> [del] mapped-nsp <nn> mapped-nsi <nn> nsh_action [swap|push|pop] "
 ...
 */
static clib_error_t *
show_nsh_map_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    nsh_main_t *nm = &nsh_main;
    nsh_map_t *map;
    ...
    pool_foreach(map, nm->nsh_mappings, ({ vlib_cli_output(vm, "%U", format_nsh_map, map); }));
...
static clib_error_t *
nsh_add_del_entry_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    u8 is_add        = 1;
    u8 ver_o_c       = 0;
...
VLIB_CLI_COMMAND(create_nsh_entry_command, static) = {
    ...
    .short_help = "create nsh entry {nsp <nn> nsi <nn>} [ttl <nn>] [md-type <nn>]"
...
/** API message handler */
static void
vl_api_nsh_add_del_entry_t_handler(vl_api_nsh_add_del_entry_t *mp)
{
...
static void
send_nsh_entry_details(nsh_entry_t *t, unix_shared_memory_queue_t *q, u32 context)
{
...
vl_api_nsh_entry_dump_t_handler(vl_api_nsh_entry_dump_t *mp)
{
    ...
    if (~0 == entry_index) {
        pool_foreach(t, nm->nsh_entries, ({ send_nsh_entry_details(t, q, mp->context); }));
    } else {
...
static void
send_nsh_map_details(nsh_map_t *t, unix_shared_memory_queue_t *q, u32 context)
{
...
vl_api_nsh_map_dump_t_handler(vl_api_nsh_map_dump_t *mp)
{
    ...
    if (~0 == map_index) {
        pool_foreach(t, nm->nsh_mappings, ({ send_nsh_map_details(t, q, mp->context); }));
    } else {
...
static clib_error_t *
show_nsh_entry_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    nsh_main_t *nm = &nsh_main;
...
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + nm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,               \
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + nm->msg_id_base);
    foreach_vl_msg_name_crc_nsh;
#undef _
}

always_inline void
nsh_md2_encap(vlib_buffer_t *b, nsh_base_header_t *hdr, nsh_entry_t *nsh_entry)
{
    nsh_main_t *nm = &nsh_main;
    nsh_base_header_t *nsh_base;
...
always_inline void
nsh_md2_swap(vlib_buffer_t *b, nsh_base_header_t *hdr, u32 header_len, nsh_entry_t *nsh_entry, u32 *next,
             u32 drop_node_val)
{
    nsh_main_t *nm = &nsh_main;
    nsh_base_header_t *nsh_base;
...
always_inline void
nsh_md2_decap(vlib_buffer_t *b, nsh_base_header_t *hdr, u32 *header_len, u32 *next, u32 drop_node_val)
{
    nsh_main_t *nm = &nsh_main;
    nsh_md2_data_t *opt0;
    nsh_md2_data_t *limit0;
...
static uword
nsh_input_map(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u32 node_type)
{
 ...
 */
static uword
nsh_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
 ...
 */
static uword
nsh_proxy(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
 ...
 */
static uword
nsh_classifier(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
 ...
 */
static uword
nsh_aware_vnf_proxy(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
nsh_init(vlib_main_t *vm)
{
    ...
    /* Set up the API */
    nm->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);
