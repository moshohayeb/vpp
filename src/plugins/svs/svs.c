...
svs_route_add_i(u32 fib_index, const fib_prefix_t *pfx, u32 src_fib_index)
{
    ...
    lookup_dpo_add_or_lock_w_fib_index(src_fib_index, fib_proto_to_dpo(pfx->fp_proto), LOOKUP_UNICAST,
                                       LOOKUP_INPUT_SRC_ADDR, LOOKUP_TABLE_FROM_CONFIG, &dpo);

    fib_table_entry_special_dpo_add(fib_index, pfx, FIB_SOURCE_PLUGIN_LOW, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
...
svs_enable(fib_protocol_t fproto, u32 table_id, u32 sw_if_index)
{
    ...
    vnet_feature_enable_disable((FIB_PROTOCOL_IP4 == fproto ? "ip4-unicast" : "ip6-unicast"),
                                (FIB_PROTOCOL_IP4 == fproto ? "svs-ip4" : "svs-ip6"), sw_if_index, 1, NULL, 0);
...
static void
svs_ip6_table_bind(ip6_main_t *im, uword opaque, u32 sw_if_index, u32 new_fib_index, u32 old_fib_index)
{
    svs_table_bind(FIB_PROTOCOL_IP6, sw_if_index, new_fib_index);
}

static void
svs_ip4_table_bind(ip4_main_t *im, uword opaque, u32 sw_if_index, u32 new_fib_index, u32 old_fib_index)
{
...
svs_disable(fib_protocol_t fproto, u32 table_id, u32 sw_if_index)
{
    ...
    vnet_feature_enable_disable((FIB_PROTOCOL_IP4 == fproto ? "ip4-unicast" : "ip6-unicast"),
                                (FIB_PROTOCOL_IP4 == fproto ? "svs-ip4" : "svs-ip6"), sw_if_index, 0, NULL, 0);
...
svs_walk(svs_walk_fn_t fn, void *ctx)
{
    ...
    FOR_EACH_FIB_IP_PROTOCOL(fproto)
    {
        vec_foreach_index(ii, svs_itf_db[fproto])
        {
            ...
            if (~0 != fib_index) {
                if (WALK_CONTINUE != fn(fproto, fib_table_get_table_id(fib_index, fproto), ii, ctx))
                    return;
            }
        }
    }
...
always_inline uword
svs_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, fib_protocol_t fproto)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (FIB_PROTOCOL_IP4 == fproto) {
                ...
                ip0  = vlib_buffer_get_current(b0);
                lbi0 = ip6_fib_table_fwding_lookup(&ip6_main, svs_itf_db[fproto][sw_if_index0], &ip0->src_address);
            }
...
static clib_error_t *
svs_table_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    fib_protocol_t fproto;
    u32 table_id;
    u8 add;
...
static clib_error_t *
svs_enable_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
svs_route_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rv != 0)
        return clib_error_return(0, "failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
...
static clib_error_t *
svs_show_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    fib_protocol_t fproto;
    u32 ii;
    ...
    FOR_EACH_FIB_IP_PROTOCOL(fproto)
    {
        ...
        vec_foreach_index(ii, svs_itf_db[fproto])
        {
            if (~0 != svs_itf_db[fproto][ii])
                vlib_cli_output(vm, "  %U -> %d", format_vnet_sw_if_index_name, vnet_get_main(), ii,
                                svs_itf_db[fproto][ii]);
        }
    }
    return (NULL);
}
