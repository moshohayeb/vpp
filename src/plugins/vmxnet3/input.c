...
vmxnet3_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vmxnet3_device_t *vd, u16 qid)
{
    ...
    while (PREDICT_TRUE(n_rx_packets < VLIB_FRAME_SIZE) && (comp_ring->gen == (rx_comp->flags & VMXNET3_RXCF_GEN))) {
        ...
        if (got_packet) {
            ...
            if (PREDICT_FALSE(vnet_device_input_have_features(vd->sw_if_index))) {
                vnet_feature_start_device_input_x1(vd->sw_if_index, &next_index, hb);
                known_next = 1;
            }
    ...
    if (PREDICT_TRUE(n_rx_packets)) {
        ...
        vlib_increment_combined_counter(vnm->interface_main.combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX,
                                        thread_index, vd->hw_if_index, n_rx_packets, n_rx_bytes);
    }
