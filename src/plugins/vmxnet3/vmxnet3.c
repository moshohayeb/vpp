...
static clib_error_t *
vmxnet3_interface_rx_mode_change(vnet_main_t *vnm, u32 hw_if_index, u32 qid, vnet_hw_interface_rx_mode mode)
{
    vmxnet3_main_t *vmxm    = &vmxnet3_main;
...
static void
vmxnet3_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    vmxnet3_main_t *vmxm    = &vmxnet3_main;
    vnet_hw_interface_t *hw = vnet_get_hw_interface(vnm, hw_if_index);
    vmxnet3_device_t *vd    = pool_elt_at_index(vmxm->devices, hw->dev_instance);
...
vmxnet3_provision_driver_shared(vlib_main_t *vm, vmxnet3_device_t *vd)
{
    ...
    vd->dma = vlib_physmem_alloc_aligned(vm, vmxm->physmem_region, &error, sizeof(*vd->dma), 512);
    if (error)
        return error;
    ...
    for (rid = 0; rid < VMXNET3_RX_RING_SIZE; rid++) {
        q->rx.cfg.desc_address[rid] = vmxnet3_dma_addr(vm, vd, rxq->rx_desc[rid]);
    ...
    shared->rx_filter.mode             = VMXNET3_RXMODE_UCAST | VMXNET3_RXMODE_BCAST | VMXNET3_RXMODE_ALL_MULTI;
...
vmxnet3_rxq_init(vlib_main_t *vm, vmxnet3_device_t *vd, u16 qid, u16 qsz)
{
    ...
    for (rid = 0; rid < VMXNET3_RX_RING_SIZE; rid++) {
        rxq->rx_desc[rid] =
            vlib_physmem_alloc_aligned(vm, vmxm->physmem_region, &error, qsz * sizeof(*rxq->rx_desc[rid]), 512);
        if (error)
            return error;
        memset(rxq->rx_desc[rid], 0, qsz * sizeof(*rxq->rx_desc[rid]));
    }
    rxq->rx_comp = vlib_physmem_alloc_aligned(vm, vmxm->physmem_region, &error, qsz * sizeof(*rxq->rx_comp), 512);
    if (error)
        return error;
...
vmxnet3_txq_init(vlib_main_t *vm, vmxnet3_device_t *vd, u16 qid, u16 qsz)
{
    ...
    txq->size    = qsz;
    txq->tx_desc = vlib_physmem_alloc_aligned(vm, vmxm->physmem_region, &error, qsz * sizeof(*txq->tx_desc), 512);
    if (error)
        return error;
    memset(txq->tx_desc, 0, qsz * sizeof(*txq->tx_desc));
    txq->tx_comp = vlib_physmem_alloc_aligned(vm, vmxm->physmem_region, &error, qsz * sizeof(*txq->tx_comp), 512);
    if (error)
        return error;
...
static clib_error_t *
vmxnet3_device_init(vlib_main_t *vm, vmxnet3_device_t *vd, vmxnet3_create_if_args_t *args)
{
    ...
    if (vmxm->physmem_region_alloc == 0) {
        ...
        error     = vlib_physmem_region_alloc(vm, "vmxnet3 descriptors", 4 << 20, 0, flags, &vmxm->physmem_region);
        if (error)
            return error;
...
vmxnet3_create_if(vlib_main_t *vm, vmxnet3_create_if_args_t *args)
{
    ...
    if (!vmxnet3_queue_size_valid(args->rxq_size) || !vmxnet3_queue_size_valid(args->txq_size)) {
        ...
        args->error = clib_error_return(error, "queue size must be <= 4096, >= 64, "
                                               "and multiples of 64");
        return;
    }
    ...
    if ((error = vlib_pci_register_msix_handler(h, 0, 1, &vmxnet3_irq_0_handler)))
        goto error;

    if ((error = vlib_pci_register_msix_handler(h, 1, 1, &vmxnet3_irq_1_handler)))
        goto error;
    ...
    /* create interface */
    error = ethernet_register_interface(vnm, vmxnet3_device_class.index, vd->dev_instance, vd->mac_addr,
    ...
    vnet_hw_interface_set_input_node(vnm, vd->hw_if_index, vmxnet3_input_node.index);
