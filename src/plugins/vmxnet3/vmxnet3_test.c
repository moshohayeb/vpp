...
#define foreach_standard_reply_retval_handler _(vmxnet3_delete_reply)
...
vl_api_vmxnet3_details_t_handler(vl_api_vmxnet3_details_t *mp)
{
    ...
    fformat(vam->ofp,
            ...
            "   state %s\n",
            mp->if_name, ntohl(mp->sw_if_index), format_ethernet_address, mp->hw_addr, mp->version, format_pci_addr,
            &pci_addr, ntohs(mp->rx_next), ntohs(mp->rx_qid), ntohs(mp->rx_qsize), ntohs(mp->rx_fill[0]),
            ntohs(mp->rx_consume[0]), ntohs(mp->rx_produce[0]), ntohs(mp->rx_qsize), ntohs(mp->rx_fill[1]),
            ntohs(mp->rx_consume[1]), ntohs(mp->rx_produce[1]), ntohs(mp->tx_next), ntohs(mp->tx_qid),
            ntohs(mp->tx_qsize), ntohs(mp->tx_consume), ntohs(mp->tx_produce), mp->admin_up_down ? "up" : "down");
}
...
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + vxm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,              \
