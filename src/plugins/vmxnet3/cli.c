...
static clib_error_t *
vmxnet3_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
vmxnet3_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    u32 sw_if_index = ~0;
...
static clib_error_t *
vmxnet3_test_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    u32 sw_if_index = ~0;
    vnet_hw_interface_t *hw;
...
static void
show_vmxnet3(vlib_main_t *vm, u32 *hw_if_indices, u8 show_descr, u8 show_one_table, u32 which, u8 show_one_slot, u32 slot)
{
    u32 i, desc_idx;
    vmxnet3_device_t *vd;
    ...
    for (i = 0; i < vec_len(hw_if_indices); i++) {
        ...
        vec_foreach_index(qid, vd->rxqs)
        {
            ...
            for (rid = 0; rid < VMXNET3_RX_RING_SIZE; rid++) {
                ...
                vlib_cli_output(vm,
                                ...
                                "consume %u produce %u",
                                rid, rxq->size, ring->fill, ring->consume, ring->produce);
                if (show_descr) {
            ...
            if (show_descr) {
                ...
                for (desc_idx = 0; desc_idx < rxq->size; desc_idx++) {
                    ...
                    vlib_cli_output(vm, "  %5u  0x%08x  %10u  %10u  0x%08x", desc_idx, rx_comp->index, rx_comp->rss,
                ...
                if (which == VMXNET3_SHOW_RX_COMP) {
                    ...
                    if (show_one_slot) {
                        ...
                        vlib_cli_output(vm, "  %5u  0x%08x  %10u  %10u  0x%08x", slot, rx_comp->index, rx_comp->rss,
                                        rx_comp->len, rx_comp->flags);
                    } else
                        for (desc_idx = 0; desc_idx < rxq->size; desc_idx++) {
                            rx_comp = &rxq->rx_comp[desc_idx];
                            vlib_cli_output(vm, "  %5u  0x%08x  %10u  %10u  0x%08x", desc_idx, rx_comp->index,
                                            rx_comp->rss, rx_comp->len, rx_comp->flags);
                        }
                }
            }
        }
        ...
        vec_foreach_index(qid, vd->rxqs)
        {
            ...
            vlib_cli_output(vm, "    size %u consume %u produce %u", txq->size, txq->tx_ring.consume,
                            txq->tx_ring.produce);
            if (show_descr) {
                ...
                for (desc_idx = 0; desc_idx < txq->size; desc_idx++) {
                    ...
                    vlib_cli_output(vm, "  %5u  0x%016llx  0x%08x  0x%08x", desc_idx, txd->address, txd->flags[0],
                                    txd->flags[1]);
                }
                ...
                if (which == VMXNET3_SHOW_TX_DESC) {
                    ...
                    if (show_one_slot) {
                        ...
                        vlib_cli_output(vm, "  %5u  0x%016llx  0x%08x  0x%08x", slot, txd->address, txd->flags[0],
                                        txd->flags[1]);
                    } else
                        for (desc_idx = 0; desc_idx < txq->size; desc_idx++) {
                            txd = &txq->tx_desc[desc_idx];
                            vlib_cli_output(vm, "  %5u  0x%016llx  0x%08x  0x%08x", desc_idx, txd->address,
                                            txd->flags[0], txd->flags[1]);
                        }
...
static clib_error_t *
show_vmxnet3_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vmxnet3_main_t *vmxm = &vmxnet3_main;
    vnet_main_t *vnm     = &vnet_main;
    vmxnet3_device_t *vd;
    ...
    if (vec_len(hw_if_indices) == 0) {
        pool_foreach(vd, vmxm->devices, vec_add1(hw_if_indices, vd->hw_if_index););
    }
