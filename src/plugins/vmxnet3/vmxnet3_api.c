...
vl_api_vmxnet3_create_t_handler(vl_api_vmxnet3_create_t *mp)
{
    ...
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_VMXNET3_CREATE_REPLY + vmxm->msg_id_base, ({ rmp->sw_if_index = ntohl(args.sw_if_index); }));
    /* *INDENT-ON* */
}
...
static void
send_vmxnet3_details(vl_api_registration_t *reg, vmxnet3_device_t *vd, u16 rx_qid, vmxnet3_rxq_t *rxq, u16 tx_qid,
                     vmxnet3_txq_t *txq, vnet_sw_interface_t *swif, u8 *interface_name, u32 context)
{
...
vl_api_vmxnet3_dump_t_handler(vl_api_vmxnet3_dump_t *mp)
{
    ...
    pool_foreach(vd, vmxm->devices, ({
                     ...
                     send_vmxnet3_details(reg, vd, qid, rxq, qid, txq, swif, if_name, mp->context);
                     _vec_len(if_name) = 0;
                 }));
    /* *INDENT-ON* */
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + vmxm->msg_id_base);
...
vmxnet3_plugin_api_hookup(vlib_main_t *vm)
{
    ...
    vmxm->msg_id_base = vl_msg_api_get_msg_ids((char *) name, VL_MSG_FIRST_AVAILABLE);

#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers((VL_API_##N + vmxm->msg_id_base), #n, vl_api_##n##_t_handler, vl_noop_handler,             \
