 ...
 */
u8 *
vlib_node_serialize(vlib_main_t *vm, vlib_node_t ***node_dups, u8 *vector, int include_nexts, int include_stats)
{
    ...
    for (j = 0; j < vec_len(node_dups); j++) {
        ...
        for (i = 0; i < vec_len(nodes); i++) {
            ...
            if (n->type == VLIB_NODE_TYPE_PROCESS) {
                ...
                switch (p->flags &
                        (VLIB_PROCESS_IS_SUSPENDED_WAITING_FOR_CLOCK | VLIB_PROCESS_IS_SUSPENDED_WAITING_FOR_EVENT)) {
                default:
...
static clib_error_t *
test_node_serialize_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < vec_len(nodes_by_thread); i++) {
        ...
        for (j = 0; j < vec_len(nodes); j++) {
            ...
            vlib_cli_output(vm,
                            ...
                            " %lld vectors %lld",
                            node->stats_total.clocks, node->stats_total.calls, node->stats_total.suspends,
