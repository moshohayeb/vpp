...
int
vl_msg_api_trace_configure(api_main_t *am, vl_api_trace_which_t which, u32 nitems)
{
    vl_api_trace_t *tp;
    int was_on = 0;
...
always_inline void
msg_handler_internal(api_main_t *am, void *the_msg, int trace_it, int do_it, int free_it)
{
...
void
vl_msg_api_handler_with_vm_node(api_main_t *am, void *the_msg, vlib_main_t *vm, vlib_node_runtime_t *node)
{
    u16 id = ntohs(*((u16 *) the_msg));
...
vl_msg_api_handler(void *the_msg)
{
    ...
    msg_handler_internal(am, the_msg, (am->rx_trace && am->rx_trace->enabled) /* trace_it */, 1 /* do_it */,
                         1 /* free_it */);
}
...
{
    api_main_t *am = &api_main;
    msg_handler_internal(am, the_msg, 0 /* trace_it */, 1 /* do_it */, 0 /* free_it */);
}
...
vl_msg_api_trace_only(void *the_msg)
{
    ...
    msg_handler_internal(am, the_msg, (am->rx_trace && am->rx_trace->enabled) /* trace_it */, 0 /* do_it */,
                         0 /* free_it */);
}
...
vl_msg_api_register_pd_handler(void *fp, u16 msg_id_host_byte_order)
{
    ...
    if (msg_id_host_byte_order > 10000)
        clib_warning("msg_id_host_byte_order endian issue? %d arg vs %d", msg_id_host_byte_order,
...
vl_msg_api_pd_handler(void *mp, int rv)
{
    ...
    if (msg_id >= vec_len(am->pd_msg_handlers) || am->pd_msg_handlers[msg_id] == 0)
        return rv;
...
void
vl_msg_api_add_version(api_main_t *am, const char *string, u32 major, u32 minor, u32 patch)
{
