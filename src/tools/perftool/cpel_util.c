...
write_string_table(FILE *ofp)
{
    ...
    if (fwrite(the_strtab, 1, vec_len(the_strtab), ofp) != vec_len(the_strtab))
        return (0);
...
write_event_defs(FILE *ofp)
{
    ...
    sh.data_length  = ntohl(vec_len(the_event_definitions) * sizeof(the_event_definitions[0]) +
    ...
    for (i = 0; i < vec_len(the_event_definitions); i++) {
        ...
        if (fwrite(this_event_definition, sizeof(the_event_definitions[0]), 1, ofp) != 1)
            return (0);
    }
    return (1);
}
...
ntohll(u64 x)
{
    if (clib_arch_is_little_endian)
        x = ((((x >> 0) & 0xff) << 56) | (((x >> 8) & 0xff) << 48) | (((x >> 16) & 0xff) << 40) |
...
write_track_defs(FILE *ofp)
{
    ...
    sh.data_length =
        ntohl(vec_len(the_tracks) * sizeof(this_track_definition[0]) + sizeof(track_definition_section_header_t));
    ...
    for (i = 0; i < vec_len(the_tracks); i++) {
        ...
        if (fwrite(this_track_definition, sizeof(this_track_definition[0]), 1, ofp) != 1)
            return (0);
    }
    return (1);
}
