...
actor_t *
find_or_create_actor(ulong key)
{
    ulong bucket;
    actor_t *ap;
    u8 *mem;
...
void
record_instance(ulong key, ulonglong time)
{
    actor_t *ap;
    instance_t *ip;
...
int
actor_compare(const void *arg1, const void *arg2)
{
    double e10k1, e10k2;
...
report_actors(void)
{
    ...
    double e10k;
    extern void linreg(double *x, double *y, int nitems, double *a, double *b, double *minp, double *maxp,
                       double *meanp, double *r);
    ...
    if (summary_stats)
        printf("NInst       Offset       Slope    T(Ninst)         Min         Max         Avg   %%InstTime           "
               "R    Key");
    else
    ...
    for (i = 0; i < nactors; i++) {
        ...
        if (ap->ninst) {
            if (summary_stats) {
                ...
                printf("%6ld %11.2f %11.2f %11.2f %11.2f %11.2f %11.2f %11.2f %11.2f 0x%08lx ", ap->ninst, ap->a, ap->b,
                       e10k, ap->min, ap->max, ap->mean, pcttot, ap->r, ap->key);
            } else
...
int
trackdef_pass(cpel_section_header_t *sh, int verbose, FILE *ofp)
{
    int i, nevents;
...
int
event_pass(cpel_section_header_t *sh, int verbose, FILE *ofp)
{
    ...
    again:
        ...
        case 0: /* not in state */
            ...
            if (vec_len(tp->start_datum) >= MAXSTACK) {
                ...
                fprintf(stderr, "Stack overflow... This occurs when "
            ...
            printf("timestamp %llu, now %llu, thread on cpu %llu\n", (unsigned long long) tp->thread_timestamp,
                   (unsigned long long) now, (unsigned long long) tp->time_thread_on_cpu);
#endif
...
#ifdef HAVING_TROUBLE
                    printf("Factor out %lld from key 0x%08x\n", (end_time - tp->start_time[sp]),
                           tp->start_datum[ancestor]);
#endif
...
int
process_section(cpel_section_header_t *sh, int verbose, FILE *ofp, pass_t pass)
{
    u32 type;
...
int
process_file(u8 *cpel, int verbose)
{
