...
point_selector_init(void)
{
    ...
    gtk_signal_connect(GTK_OBJECT(s_nonebutton), "clicked", GTK_SIGNAL_FUNC(button_click_callback),
    ...
    s_ps_vsadj = gtk_adjustment_new(0.0 /* initial value */, 0.0 /* minimum value */, 2000.0 /* maximum value */,
                                    0.1 /* step increment */, 10.0 /* page increment */, 10.0 /* page size */);

    s_ps_vscroll = gtk_vscrollbar_new(GTK_ADJUSTMENT(s_ps_vsadj));
    gtk_signal_connect(GTK_OBJECT(s_ps_vsadj), "value-changed", GTK_SIGNAL_FUNC(scroll_callback),
...
create_point_selector(void)
{
    ...
    for (i = 0; i < g_neventdefs; i++) {
        ep = &g_eventdefs[i];
        sprintf(tmpbuf, "[%lu] %s", ep->event, ep->name ? ep->name : "(none)");
        ...
        gtk_signal_connect(GTK_OBJECT(wp), "toggled", GTK_SIGNAL_FUNC(point_select_callback),
...
void
add_event_from_clib_file(unsigned int event, char *name, unsigned int vec_index)
{
    event_def_t *ep;
