...
#define zi_x_hot 22
#define zi_y_hot 14
static unsigned char zi_bits[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00,
...
static unsigned char zi_bkgd[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00,
                                  0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00,
                                  0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x82,
                                  0x00, 0x00, 0x00, 0x84, 0x00, 0x00, 0x00, 0x88, 0x00, 0x00, 0x00, 0x90, 0x00,
                                  0x00, 0x00, 0xa0, 0x00, 0x00, 0x00, 0xc0, 0x00, 0x00, 0xfc, 0xff, 0x00, 0x00,
...
next_snapshot(void)
{
    ...
    if (summary_mode != next->summary_mode) {
        view1_button_click_callback(NULL,
                                    (gpointer)(unsigned long long) (summary_mode ? NOSUMMARY_BUTTON : SUMMARY_BUTTON));
    }
...
write_snapshot(void)
{
    ...
    for (snap = s_snapshots; !error && snap != NULL; snap = snap->next) {
        if (fwrite(&snap->geometry, sizeof(snap->geometry), 1, file) != 1 ||
            fwrite(&snap->show_event, sizeof(snap->show_event), 1, file) != 1 ||
...
button_press_event(GtkWidget *widget, GdkEventButton *event)
{
    ...
    case GDK_MOTION_NOTIFY:
        ...
        if (press3_valid) {
            ...
            time_per_pixel =
                ((double) (s_v1->maxvistime - s_v1->minvistime)) / ((double) (s_v1->total_width - s_v1->pid_ax_width));
...
{
    gdk_draw_pixmap(widget->window, widget->style->fg_gc[GTK_WIDGET_STATE(widget)], pm, event->area.x, event->area.y,
...
event_search(void)
{
    modal_dialog("Event Search: Please Enter Event Code", "Invalid: Please Reenter Event Code", NULL,
                 event_search_callback);
}
...
{
    modal_dialog("Print Screen (PostScript format) to file:", "Invalid file: Print Screen to file:", "g2.ps",
                 print_screen_callback);
}
...
view1_init(void)
{
    ...
    gtk_widget_set_events(da,
                          GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_EXPOSURE_MASK | GDK_BUTTON_MOTION_MASK);
    ...
    s_view1_vsadj = gtk_adjustment_new(0.0 /* initial value */, 0.0 /* minimum value */, 2000.0 /* maximum value */,
                                       0.1 /* step increment */, 10.0 /* page increment */, 10.0 /* page size */);

    s_view1_vscroll = gtk_vscrollbar_new(GTK_ADJUSTMENT(s_view1_vsadj));

    gtk_signal_connect(GTK_OBJECT(s_view1_vsadj), "value-changed", GTK_SIGNAL_FUNC(view1_vscroll),
    ...
    gtk_signal_connect(GTK_OBJECT(s_view1_topbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) TOP_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_bottombutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
    ...
    gtk_signal_connect(GTK_OBJECT(s_view1_more_traces_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) MORE_TRACES_BUTTON);
    gtk_signal_connect(GTK_OBJECT(s_view1_less_traces_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
    ...
    gtk_signal_connect(GTK_OBJECT(s_view1_startbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) START_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_zoominbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) ZOOMIN_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_searchbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) SEARCH_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_srchagainbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) SEARCH_AGAIN_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_zoomoutbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) ZOOMOUT_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_endbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) END_BUTTON);

    s_view1_hsadj = gtk_adjustment_new(0.0 /* initial value */, 0.0 /* minimum value */, 2000.0 /* maximum value */,
                                       0.1 /* step increment */, 10.0 /* page increment */, 10.0 /* page size */);

    s_view1_hscroll = gtk_hscrollbar_new(GTK_ADJUSTMENT(s_view1_hsadj));

    gtk_signal_connect(GTK_OBJECT(s_view1_hsadj), "value-changed", GTK_SIGNAL_FUNC(view1_hscroll),
    ...
    gtk_signal_connect(GTK_OBJECT(s_view1_snapbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) SNAP_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_nextbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) NEXT_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_delbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) DEL_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_chase_event_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) CHASE_EVENT_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_chase_datum_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) CHASE_DATUM_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_chase_track_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) CHASE_TRACK_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_unchasebutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) UNCHASE_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_forward_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) FORWARD_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_backward_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) BACKWARD_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_summary_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) SUMMARY_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_nosummary_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) NOSUMMARY_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_time_slew_left_button), "clicked",
                       GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) SLEW_LEFT_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_time_slew_right_button), "clicked",
                       GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) SLEW_RIGHT_BUTTON);
 ...
 ****************************************************************************/
GdkRectangle *
tbox_print(char *s, int x, int y, enum view1_tbox_fn function, GdkRectangle *rp)
{
...
tbox(char *s, int x, int y, enum view1_tbox_fn function)
{
    ...
    case TBOX_DRAW_BOXED:
        ...
        gdk_draw_rectangle(pm, da->style->black_gc, FALSE, x, y - (ascent + descent + 3), width + 2,
                           ascent + descent + 3);
...
display_pid_axis(v1_geometry_t *vp)
{
    ...
    for (i = 0; i < vp->npids; i++) {
        ...
        if (vp->strip_height > 4) {
            line(vp->pid_ax_width - label_tick, y + 4 * s_print_offset, vp->total_width, y + 4 * s_print_offset,
