...
static void
get_dialog_value(GtkWidget *dialog, gpointer user_data)
{
...
void
modal_dialog(char *label_text, char *retry_text, char *default_value, boolean (*cb)(char *))
{
...
static void
get_eventdef_name(GtkFileSelection *sel, gpointer user_data)
{
...
static void
read_eventdef_callback(GtkToggleButton *item, gpointer data)
{
    ...
    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(s_filesel)->ok_button), "clicked",
                       GTK_SIGNAL_FUNC(get_eventdef_name), NULL);

    gtk_signal_connect_object(GTK_OBJECT(GTK_FILE_SELECTION(s_filesel)->ok_button), "clicked",
                              GTK_SIGNAL_FUNC(gtk_widget_destroy), (gpointer) s_filesel);

    gtk_signal_connect_object(GTK_OBJECT(GTK_FILE_SELECTION(s_filesel)->cancel_button), "clicked",
                              GTK_SIGNAL_FUNC(gtk_widget_destroy), (gpointer) s_filesel);
...
static void
get_events_name(GtkFileSelection *sel, gpointer user_data)
{
...
static void
read_events_callback(GtkToggleButton *item, gpointer data)
{
    char tmpbuf[32];

    s_eventsel = gtk_file_selection_new("Read Events From...");

    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(s_eventsel)->ok_button), "clicked",
                       GTK_SIGNAL_FUNC(get_events_name), NULL);

    gtk_signal_connect_object(GTK_OBJECT(GTK_FILE_SELECTION(s_eventsel)->ok_button), "clicked",
                              GTK_SIGNAL_FUNC(gtk_widget_destroy), (gpointer) s_eventsel);

    gtk_signal_connect_object(GTK_OBJECT(GTK_FILE_SELECTION(s_eventsel)->cancel_button), "clicked",
                              GTK_SIGNAL_FUNC(gtk_widget_destroy), (gpointer) s_eventsel);
    ...
    if (ticks_per_ns_set)
        ...
        modal_dialog("Please enter clock ticks per nanosecond", "Invalid: Please enter clock ticks per nanosecond",
...
static void
read_eventsclock_callback(GtkToggleButton *item, gpointer data)
{
...
void
infobox_size_request(GtkWidget *widget, GtkRequisition *req, gpointer user_data)
{
...
void
infobox(char *label_text, char *text)
{
    GtkWidget *dialog, *label, *ok_button, *entry;
    GtkWidget *box;
...
static void
help_general_callback(GtkToggleButton *item, gpointer data)
{
...
static void
help_about_callback(GtkToggleButton *item, gpointer data)
{
    char tmpbuf[1024];
...
static void
get_cpel_name(GtkFileSelection *sel, gpointer user_data)
{
    char *filename = (char *) gtk_file_selection_get_filename(GTK_FILE_SELECTION(s_filesel));
...
static void
get_clib_name(GtkFileSelection *sel, gpointer user_data)
{
    char *filename = (char *) gtk_file_selection_get_filename(GTK_FILE_SELECTION(s_filesel));
...
static void
read_cpel_callback(GtkToggleButton *item, gpointer data)
{
    ...
    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(s_filesel)->ok_button), "clicked", GTK_SIGNAL_FUNC(get_cpel_name),
                       NULL);
...
static void
read_clib_callback(GtkToggleButton *item, gpointer data)
{
    ...
    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(s_filesel)->ok_button), "clicked", GTK_SIGNAL_FUNC(get_clib_name),
                       NULL);
