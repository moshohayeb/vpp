...
barrier_trace_sync(f64 t_entry, f64 t_open, f64 t_closed)
{
    ...
    ed->caller   = elog_global_id_for_msg_name(vlib_worker_threads[0].barrier_caller);
...
vlib_thread_init(vlib_main_t *vm)
{
    ...
    tm->cpu_core_bitmap   = clib_sysfs_list_to_bitmap("/sys/devices/system/cpu/online");
    tm->cpu_socket_bitmap = clib_sysfs_list_to_bitmap("/sys/devices/system/node/online");
    ...
    vec_validate_aligned(vlib_worker_threads, vec_len(vlib_thread_stacks) - 1, CLIB_CACHE_LINE_BYTES);
...
vlib_worker_thread_bootstrap_fn(void *arg)
{
    ...
    rv = (void *) clib_calljmp((uword(*)(uword)) w->thread_function, (uword) arg,
...
vlib_get_thread_core_socket(vlib_worker_thread_t *w, unsigned cpu_id)
{
    ...
    vec_reset_length(p);
    p = format(p, "%s%u/topology/physical_package_id%c", sys_cpu_path, cpu_id, 0);
...
start_workers(vlib_main_t *vm)
{
    ...
    vm->elog_main.lock    = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
    ...
    if (n_vlib_mains > 1) {
        ...
        vec_validate_aligned(vlib_mains, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
        ...
        *vlib_worker_threads->node_reforks_required     = 0;
        ...
        for (i = 0; i < vec_len(tm->registrations); i++) {
            ...
            for (k = 0; k < tr->count; k++) {
...
#else
                    w->thread_mheap = create_mspace(tr->mheap_size, 0 /* unlocked */);
#endif
                } else
                ...
                vm_clone->heap_aligned_base     = (void *) (((uword) w->thread_mheap) & ~(VLIB_FRAME_ALIGN - 1));
                vm_clone->init_functions_called = hash_create(0, /* value bytes */ 0);
                ...
                nm_clone->nodes_by_type[VLIB_NODE_TYPE_INTERNAL] =
                    vec_dup_aligned(nm->nodes_by_type[VLIB_NODE_TYPE_INTERNAL], CLIB_CACHE_LINE_BYTES);
                ...
                nm_clone->nodes_by_type[VLIB_NODE_TYPE_INPUT] =
                    vec_dup_aligned(nm->nodes_by_type[VLIB_NODE_TYPE_INPUT], CLIB_CACHE_LINE_BYTES);
                ...
                vm_clone->error_main.counters =
                    vec_dup_aligned(vlib_mains[0]->error_main.counters, CLIB_CACHE_LINE_BYTES);
                vm_clone->error_main.counters_last_clear =
                    vec_dup_aligned(vlib_mains[0]->error_main.counters_last_clear, CLIB_CACHE_LINE_BYTES);
                ...
                pool_foreach(fl_orig, orig_freelist_pool, ({
                                 pool_get_aligned(vm_clone->buffer_free_list_pool, fl_clone, CLIB_CACHE_LINE_BYTES);
                                 ASSERT(fl_orig - orig_freelist_pool == fl_clone - vm_clone->buffer_free_list_pool);
        ...
        for (i = 0; i < vec_len(tm->registrations); i++) {
            ...
            for (j = 0; j < tr->count; j++) {
                ...
                if (tr->mheap_size) {
                    ...
                    w->thread_mheap = mheap_alloc(0 /* use VM */, tr->mheap_size);
#else
                    w->thread_mheap = create_mspace(tr->mheap_size, 0 /* locked */);
#endif
                } else
                    w->thread_mheap = main_heap;
                w->thread_stack        = vlib_thread_stack_init(w - vlib_worker_threads);
...
worker_thread_node_runtime_update_internal(void)
{
    ...
    vlib_main_t *vm_clone;
    vlib_node_runtime_t *rt;
    never_inline void vlib_node_runtime_sync_stats(vlib_main_t * vm, vlib_node_runtime_t * r, uword n_calls,
...
vlib_worker_thread_node_refork(void)
{
    ...
    for (j = 0; j < vec_len(nm->nodes); j++) {
        ...
        if (j >= vec_len(old_nodes_clone)) {
            ...
            memset(&new_n_clone->stats_total, 0, sizeof(new_n_clone->stats_total));
            memset(&new_n_clone->stats_last_clear, 0, sizeof(new_n_clone->stats_last_clear));
        } else {
            /* Copy stats if the old data is valid */
            clib_memcpy(&new_n_clone->stats_total, &old_n_clone->stats_total, sizeof(new_n_clone->stats_total));
            clib_memcpy(&new_n_clone->stats_last_clear, &old_n_clone->stats_last_clear,
    ...
    for (j = 0; j < vec_len(old_rt); j++) {
        ...
        clib_memcpy(rt->runtime_data, old_rt[j].runtime_data, VLIB_NODE_RUNTIME_DATA_SIZE);
    }
...
cpu_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "use-pthreads"))
            ...
            if (tr->use_pthreads)
                return clib_error_return(0, "corelist cannot be set for '%s' threads", name);
            ...
            tr = (vlib_thread_registration_t *) p[0];
            if (tr->fixed_count)
                return clib_error_return(0, "number of %s threads not configurable", tr->name);
            tr->count = count;
        } else
            break;
    }
    ...
    if (tm->sched_priority != ~0) {
        if (tm->sched_policy == SCHED_FIFO || tm->sched_policy == SCHED_RR) {
            ...
            return clib_error_return(0, "scheduling priority (%d) is not allowed for `normal` scheduling policy",
                                     tm->sched_priority);
        }
    }
    tr = tm->next;
...
vlib_worker_thread_barrier_release(vlib_main_t *vm)
{
    ...
    if (vm->need_vlib_worker_thread_node_runtime_update) {
        ...
        refork_needed = 1;
        clib_smp_atomic_add(vlib_worker_threads->node_reforks_required, (vec_len(vlib_mains) - 1));
        now           = vlib_time_now(vm);
...
vlib_worker_thread_fn(void *arg)
{
    ...
    e = vlib_call_init_exit_functions(vm, vm->worker_init_function_registrations, 1 /* call_once */);
    if (e)
        clib_error_report(e);
...
vlib_frame_queue_main_init(u32 node_index, u32 frame_queue_nelts)
{
    ...
    for (i = 0; i < tm->n_vlib_mains; i++) {
        ...
        vec_validate(ptd->handoff_queue_elt_by_thread_index, tm->n_vlib_mains - 1);
        vec_validate_init_empty(ptd->congested_handoff_queue_by_thread_index, tm->n_vlib_mains - 1,
