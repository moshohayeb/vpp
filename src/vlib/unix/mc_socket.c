...
always_inline void
msg_handler(mc_main_t *mcm, u32 buffer_index, u32 handler_frees_buffer, void *_h)
{
...
static uword
append_buffer_index_to_iovec(vlib_main_t *vm, u32 buffer_index, struct iovec **iovs_return)
{
    struct iovec *i;
    vlib_buffer_t *b;
    u32 bi = buffer_index;
...
static clib_error_t *
sendmsg_helper(mc_socket_main_t *msm, int socket, struct sockaddr_in *tx_addr, u32 buffer_index)
{
...
static clib_error_t *
recvmsg_helper(mc_socket_main_t *msm, int socket, struct sockaddr_in *rx_addr, u32 *buffer_index, u32 drop_message)
{
    vlib_main_t *vm = msm->mc_main.vlib_main;
    vlib_buffer_t *b;
...
ack_socket_read_ready(clib_file_t *uf)
{
    ...
    error = recvmsg_helper(msm, msm->ack_socket, /* rx_addr */ 0, &bi,
                           /* drop_message */ 0);
    if (!error)
        msg_handler(mcm, bi, /* handler_frees_buffer */ 0, mc_msg_user_ack_handler);
    return error;
}

static void
catchup_cleanup(mc_socket_main_t *msm, mc_socket_catchup_t *c, clib_file_main_t *um, clib_file_t *uf)
{
...
catchup_socket_write_ready(clib_file_t *uf, int is_server)
{
    ...
    while (1) {
        ...
        n_this_write = clib_min(vec_len(c->output_vector) - c->output_vector_n_written,
...
static clib_error_t *
setup_mutlicast_socket(mc_socket_main_t *msm, mc_multicast_socket_t *ms, char *type, uword udp_port)
{
    int one = 1;
    ...
    if (bind(ms->socket, (struct sockaddr *) &ms->tx_addr, sizeof(ms->tx_addr)) < 0)
...
socket_setup(mc_socket_main_t *msm)
{
    ...
    if (!msm->base_multicast_udp_port_host_byte_order)
        msm->base_multicast_udp_port_host_byte_order =
            0xffff - ((MC_N_TRANSPORT_TYPE + 2 /* ack socket, catchup socket */) - 1);
    ...
    error =
        setup_mutlicast_socket(msm, &msm->multicast_sockets[MC_TRANSPORT_USER_REQUEST_FROM_RELAY], "from relay", port++);
    if (error)
        return error;
...
static uword
catchup_request_fun(void *transport_main, u32 stream_index, mc_peer_id_t catchup_peer_id)
{
...
clib_error_t *
mc_socket_main_init(mc_socket_main_t *msm, char **intfc_probe_list, int n_intfcs_to_probe)
{
    clib_error_t *error;
    mc_main_t *mcm;
    u32 mtu;
