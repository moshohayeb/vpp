...
load_one_plugin(plugin_main_t *pm, plugin_info_t *pi, int from_early_init)
{
    ...
    if ((strlen(version_required) > 0) && (strncmp(vlib_plugin_app_version, version_required, strlen(version_required)))) {
        clib_warning("Plugin %s version mismatch: %s != %s", pi->name, vlib_plugin_app_version, reg->version_required);
    ...
    if (reg->early_init) {
        ...
        if (h) {
            ...
            clib_warning("Plugin %s: early init function %s set but not found", (char *) pi->name, reg->early_init);
    }
...
static clib_error_t *
vlib_plugins_show_cmd_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    plugin_main_t *pm = &vlib_plugin_main;
    u8 *s             = 0;
    u8 *key           = 0;
