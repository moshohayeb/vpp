...
syslog:
    if (sc->syslog_level != VLIB_LOG_LEVEL_DISABLED && level <= sc->syslog_level) {
        u8 *tmp = format(NULL, "%U", format_vlib_log_class, class);
        if (use_formatted_log_entry) {
            syslog(vlib_log_level_to_syslog_priority(level), "%.*s: %.*s", (int) vec_len(tmp), tmp,
                   (int) (vec_len(s) - (vec_c_string_is_terminated(s) ? 1 : 0)), s);
        } else {
            tmp = format(tmp, ": ");
            va_start(va, fmt);
            ...
            syslog(vlib_log_level_to_syslog_priority(level), "%.*s",
                   (int) (vec_len(tmp) - (vec_c_string_is_terminated(tmp) ? 1 : 0)), tmp);
        }
        vec_free(tmp);
    }
}
...
vlib_log_init(vlib_main_t *vm)
{
    ...
    u8 *tmp =
        format(NULL, "%U %-10U %-10U ", format_time_float, 0, (f64) 0, format_white_space, 255, format_white_space, 255);
...
static clib_error_t *
show_log(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (count--) {
        ...
        vlib_cli_output(vm, "%U %-10U %-10U %v", format_time_float, 0, e->timestamp + time_offset,
                        format_vlib_log_level, e->level, format_vlib_log_class, e->class, e->string);
        i = (i + 1) % lm->size;
    }
...
static clib_error_t *
show_log_config(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    vlib_log_main_t *lm = &log_main;
    ...
    vec_foreach(c, lm->classes)
    {
        ...
        {
            vlib_cli_output(vm, "  %-20v %-14U %-14U %d", sc->name ? sc->name : defstr, format_vlib_log_level,
                            sc->level, format_vlib_log_level, sc->syslog_level, sc->rate_limit);
        }
    }
    vec_free(defstr);
...
static clib_error_t *
clear_log(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    vlib_log_main_t *lm = &log_main;
    vlib_log_entry_t *e;
...
static clib_error_t *
set_log_class(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
set_log_unth_time(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    clib_error_t *rv = NULL;
    int unthrottle_time;
...
static clib_error_t *
set_log_size(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    clib_error_t *rv = NULL;
    int size;
...
static clib_error_t *
test_log_class_subclass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    /* Get a line of input. */
    if (!unformat_user(input, unformat_line_input, line_input))
    ...
    if (unformat(line_input, "%U", unformat_vlib_log_level, &level)) {
        if (unformat(line_input, "%U", unformat_vlib_log_class, &class)) {
            if (unformat(line_input, "%U", unformat_vlib_log_subclass, class, &subclass)) {
                ...
                return clib_error_return(0, "unknown log subclass near beginning of `%U'", format_unformat_error,
                                         line_input);
            }
        } else {
