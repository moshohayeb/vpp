    ...
    while (1) {
        ...
        case VLIB_LEX_KEYWORD_CHECK: {
            ...
            if (p) {
                rv->token = p[0];
                if (LEX_DEBUG > 0)
                    clib_warning("keyword '%s' token %s", lm->token_buffer, lm->lex_token_names[rv->token]);
            } else {
                /* it's a WORD */
                rv->token            = VLIB_LEX_word;
                rv->value.as_pointer = vec_dup(lm->token_buffer);
                if (LEX_DEBUG > 0)
                    clib_warning("%s, value '%s'", lm->lex_token_names[VLIB_LEX_word], rv->value.as_pointer);
            }
        ...
        case VLIB_LEX_RETURN:
            ...
            lm->current_token_value = ~0;
            if (LEX_DEBUG > 0) {
                clib_warning("table %s char '%c'(0x%02x) next table %s return %s", t->name, c, c,
                             lm->lex_tables[e->next_table_index].name, lm->lex_token_names[e->token]);
                if (rv->token == VLIB_LEX_number)
                    clib_warning("  numeric value 0x%x (%d)", rv->value, rv->value);
            }
            return;
        }
    }
...
u16
vlib_lex_find_or_add_keyword(vlib_lex_main_t *lm, char *keyword, char *token_name)
{
...
void
vlib_lex_set_action_range(u32 table_index, u8 lo, u8 hi, u16 action, u16 token, u32 next_table_index)
{
    int i;
