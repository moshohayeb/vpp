...
linux_vfio_group_get_device_fd(vlib_pci_addr_t *addr, int *fdp)
{
    ...
    s      = format(s, "/sys/bus/pci/devices/%U/iommu_group", format_vlib_pci_addr, addr);
    ...
    s   = format(s, "/sys/bus/pci/devices/%U/iommu_group/name", format_vlib_pci_addr, addr);
