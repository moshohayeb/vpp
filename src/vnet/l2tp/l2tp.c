...
format_l2t_trace(u8 *s, va_list *args)
{
    ...
    if (t->is_user_to_network)
        s = format(s, "L2T: %U (client) -> %U (our) session %d", format_ip6_address, &t->client_address,
                   format_ip6_address, &t->our_address, t->session_index);
    else
        s = format(s, "L2T: %U (our) -> %U (client) session %d)", format_ip6_address, &t->our_address,
...
format_l2t_session(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d] %U (our) %U (client) %U (sw_if_index %d)\n", session - lm->sessions, format_ip6_address,
               &session->our_address, format_ip6_address, &session->client_address, format_vnet_sw_interface_name,
               lm->vnet_main, vnet_get_sw_interface(lm->vnet_main, session->sw_if_index), session->sw_if_index);

    s = format(s, "   local cookies %016llx %016llx remote cookie %016llx\n",
               clib_net_to_host_u64(session->local_cookie[0]), clib_net_to_host_u64(session->local_cookie[1]),
...
static clib_error_t *
show_l2tp_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (verbose) {
        ...
        /* *INDENT-OFF* */
        pool_foreach(session, lm->sessions, ({ vlib_cli_output(vm, "%U", format_l2t_session, session); }));
        /* *INDENT-ON* */
    }
...
static clib_error_t *
test_counters_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    l2t_session_t *session;
    l2t_main_t *lm = &l2t_main;
    u32 session_index;
    ...
    pool_foreach(session, lm->sessions, ({
                     ...
                     vlib_increment_combined_counter(&lm->counter_main, thread_index, counter_index, 1 /*pkt*/,
                                                     1111 /*bytes*/);
                     vlib_increment_combined_counter(&lm->counter_main, thread_index, counter_index + 1, 1 /*pkt*/,
                                                     2222 /*bytes*/);
                     nincr++;
                 }));
    /* *INDENT-ON* */
...
static clib_error_t *
clear_counters_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    l2t_session_t *session;
    l2t_main_t *lm = &l2t_main;
    u32 session_index;
...
int
create_l2tpv3_ipv6_tunnel(l2t_main_t *lm, ip6_address_t *client_address, ip6_address_t *our_address,
                          u32 local_session_id, u32 remote_session_id, u64 local_cookie, u64 remote_cookie,
                          int l2_sublayer_present, u32 encap_fib_index, u32 *sw_if_index)
{
    l2t_session_t *s = 0;
    ...
    s->l2tp_hdr_size =
        l2_sublayer_present ? sizeof(l2tpv3_header_t) : sizeof(l2tpv3_header_t) - sizeof(l2tp_hdr.l2_specific_sublayer);
    s->admin_up        = 0;
    ...
    if (vec_len(lm->free_l2tpv3_tunnel_hw_if_indices) > 0) {
        ...
        hw_if_index = vnet_register_interface(vnm, l2tpv3_device_class.index, s - lm->sessions, l2tpv3_hw_class.index,
                                              s - lm->sessions);
        hi          = vnet_get_hw_interface(vnm, hw_if_index);
...
static clib_error_t *
create_l2tpv3_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    rv = create_l2tpv3_ipv6_tunnel(lm, &client_address, &our_address, local_session_id, remote_session_id, local_cookie,
                                   remote_cookie, l2_sublayer_present, encap_fib_index, &sw_if_index);
    switch (rv) {
    case 0:
...
VLIB_CLI_COMMAND(create_l2tpv3_tunnel_command, static) = {
    ...
    .short_help = "create l2tpv3 tunnel client <ip6> our <ip6> local-cookie <hex> remote-cookie <hex> local-session "
                  "<dec> remote-session <dec>",
    .function = create_l2tpv3_tunnel_command_fn,
};
/* *INDENT-ON* */

int
l2tpv3_set_tunnel_cookies(l2t_main_t *lm, u32 sw_if_index, u64 new_local_cookie, u64 new_remote_cookie)
{
    l2t_session_t *s;
    vnet_hw_interface_t *hi;
...
static clib_error_t *
set_l2tp_tunnel_cookie_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    l2t_main_t *lm   = &l2t_main;
    vnet_main_t *vnm = vnet_get_main();
    u32 sw_if_index  = ~0;
...
int
l2tpv3_interface_enable_disable(vnet_main_t *vnm, u32 sw_if_index, int enable_disable)
{
...
static clib_error_t *
set_ip6_l2tpv3(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u32 sw_if_index = ~0;
    int is_add      = 1;
    int rv;
...
l2tp_init(vlib_main_t *vm)
{
    ...
    lm->session_by_src_address =
        hash_create_mem(0, sizeof(ip6_address_t) /* key bytes */, sizeof(u32) /* value bytes */);
    lm->session_by_dst_address =
        hash_create_mem(0, sizeof(ip6_address_t) /* key bytes */, sizeof(u32) /* value bytes */);
