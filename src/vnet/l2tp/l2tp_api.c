...
static void
send_sw_if_l2tpv3_tunnel_details(vpe_api_main_t *am, vl_api_registration_t *reg, l2t_session_t *s, l2t_main_t *lm,
                                 u32 context)
{
...
static void
vl_api_sw_if_l2tpv3_tunnel_dump_t_handler(vl_api_sw_if_l2tpv3_tunnel_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(session, lm->sessions, ({ send_sw_if_l2tpv3_tunnel_details(am, reg, session, lm, mp->context); }));
    /* *INDENT-ON* */
}
...
vl_api_l2tpv3_create_tunnel_t_handler(vl_api_l2tpv3_create_tunnel_t *mp)
{
    ...
    rv = create_l2tpv3_ipv6_tunnel(lm, (ip6_address_t *) mp->client_address, (ip6_address_t *) mp->our_address,
                                   ntohl(mp->local_session_id), ntohl(mp->remote_session_id),
                                   clib_net_to_host_u64(mp->local_cookie), clib_net_to_host_u64(mp->remote_cookie),
                                   mp->l2_sublayer_present, encap_fib_index, &sw_if_index);
...
static void
vl_api_l2tpv3_set_tunnel_cookies_t_handler(vl_api_l2tpv3_set_tunnel_cookies_t *mp)
{
    ...
    rv = l2tpv3_set_tunnel_cookies(lm, ntohl(mp->sw_if_index), clib_net_to_host_u64(mp->new_local_cookie),
...
static void
vl_api_l2tpv3_interface_enable_disable_t_handler(vl_api_l2tpv3_interface_enable_disable_t *mp)
{
    int rv;
