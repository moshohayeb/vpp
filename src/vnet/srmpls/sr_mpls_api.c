...
vl_api_sr_mpls_policy_add_t_handler(vl_api_sr_mpls_policy_add_t *mp)
{
    ...
    int rv = 0;
    rv     = sr_mpls_policy_add(ntohl(mp->bsid), segments, mp->type, ntohl(mp->weight));
    vec_free(segments);
...
vl_api_sr_mpls_policy_mod_t_handler(vl_api_sr_mpls_policy_mod_t *mp)
{
    ...
    int rv = 0;
    rv     = sr_mpls_policy_mod(ntohl(mp->bsid), mp->operation, segments, ntohl(mp->sl_index), ntohl(mp->weight));
    vec_free(segments);
...
static void
vl_api_sr_mpls_steering_add_del_t_handler(vl_api_sr_mpls_steering_add_del_t *mp)
{
    ...
    int rv = 0;
    if (mp->is_del)
        rv = sr_mpls_steering_policy_del(&prefix, ntohl(mp->mask_width), mp->traffic_type, ntohl(mp->table_id),
                                         ntohl(mp->color));
    else
        rv = sr_mpls_steering_policy_add(ntohl(mp->bsid), ntohl(mp->table_id), &prefix, ntohl(mp->mask_width),
                                         mp->traffic_type, (ip46_address_t *) &mp->next_hop, mp->nh_type,
...
static void
vl_api_sr_mpls_policy_assign_endpoint_color_t_handler(vl_api_sr_mpls_policy_assign_endpoint_color_t *mp)
{
    ...
    rv = sr_mpls_policy_assign_endpoint_color(ntohl(mp->bsid), &endpoint, mp->endpoint_type, ntohl(mp->color));
...
#undef _
     ...
     */
    vl_msg_api_set_handlers(VL_API_SR_MPLS_POLICY_ADD, "sr_mpls_policy_add", vl_api_sr_mpls_policy_add_t_handler,
                            vl_noop_handler, vl_api_sr_mpls_policy_add_t_endian, vl_api_sr_mpls_policy_add_t_print, 256,
                            1);
     ...
     * to capture a typical segment list
     */
    vl_msg_api_set_handlers(VL_API_SR_MPLS_POLICY_MOD, "sr_mpls_policy_mod", vl_api_sr_mpls_policy_mod_t_handler,
                            vl_noop_handler, vl_api_sr_mpls_policy_mod_t_endian, vl_api_sr_mpls_policy_mod_t_print, 256,
                            1);
