...
create_sl(mpls_sr_policy_t *sr_policy, mpls_label_t *sl, u32 weight)
{
    ...
    FOR_EACH_MPLS_EOS_BIT(eos)
    {
        ...
        fib_table_entry_path_add2(0, &pfx, FIB_SOURCE_SR,
                                  (sr_policy->type == SR_POLICY_TYPE_DEFAULT ? FIB_ENTRY_FLAG_NONE :
 ...
 */
int
sr_mpls_policy_add(mpls_label_t bsid, mpls_label_t *segments, u8 behavior, u32 weight)
{
    mpls_sr_main_t *sm          = &sr_mpls_main;
    ...
    if (1 == pool_elts(sm->sr_policies))
        fib_table_find_or_create_and_lock(FIB_PROTOCOL_MPLS, MPLS_FIB_DEFAULT_TABLE_ID, FIB_SOURCE_SR);
    sr_policy->bsid          = bsid;
...
sr_mpls_policy_del(mpls_label_t bsid)
{
    ...
    if (0 == pool_elts(sm->sr_policies))
        fib_table_unlock(MPLS_FIB_DEFAULT_TABLE_ID, FIB_PROTOCOL_MPLS, FIB_SOURCE_SR);
 ...
 */
int
sr_mpls_policy_mod(mpls_label_t bsid, u8 operation, mpls_label_t *segments, u32 sl_index, u32 weight)
{
    mpls_sr_main_t *sm          = &sr_mpls_main;
    mpls_sr_policy_t *sr_policy = 0;
    mpls_sr_sl_t *segment_list;
 ...
 */
static clib_error_t *
sr_mpls_policy_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    int rv      = -1;
 ...
 */
static clib_error_t *
show_sr_mpls_policies_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    mpls_sr_main_t *sm              = &sr_mpls_main;
    ...
    vec_foreach_index(i, vec_policies)
    {
        ...
        vlib_cli_output(vm, "[%u].-\tBSID: %U", (u32)(sr_policy - sm->sr_policies), format_mpls_unicast_label,
                        sr_policy->bsid);
 ...
 */
int
sr_mpls_policy_assign_endpoint_color(mpls_label_t bsid, ip46_address_t *endpoint, u8 endpoint_type, u32 color)
{
    mpls_sr_main_t *sm          = &sr_mpls_main;
    mpls_sr_policy_t *sr_policy = 0;
    ...
    if (sr_policy->endpoint_type) {
        ...
        {
            pfx.fp_eos = eos;
            fib_table_entry_path_remove(sm->fib_table_EC, &pfx, FIB_SOURCE_SR, DPO_PROTO_MPLS, NULL, ~0, 0, 1,
    ...
    if (endpoint_type) {
        ...
        if (sm->fib_table_EC == (u32) ~0) {
            sm->fib_table_EC = fib_table_create_and_lock(FIB_PROTOCOL_MPLS, FIB_SOURCE_SR,
 ...
 */
static clib_error_t *
cli_sr_mpls_policy_ec_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
