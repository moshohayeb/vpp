...
compute_sr_te_automated_steering_fib_entry(mpls_sr_steering_policy_t *steer_pl)
{
    ...
    if (sm->fib_table_EC == (u32) ~0) {
        sm->fib_table_EC =
            fib_table_create_and_lock(FIB_PROTOCOL_MPLS, FIB_SOURCE_SR, "SR-MPLS Traffic Engineering (NextHop,Color)");
    ...
    if (steer_pl->nh_type == SR_STEER_IPV6) {
        ...
        path.frp_fib_index =
            fib_table_find(FIB_PROTOCOL_IP6,
                           (steer_pl->classify.fib_table != (u32) ~0 ? steer_pl->classify.fib_table : 0));
    ...
    vec_add1(paths, path);
    if (steer_pl->classify.traffic_type == SR_STEER_IPV6)
        fib_table_entry_update(fib_table_find(FIB_PROTOCOL_IP6,
                                              (steer_pl->classify.fib_table != (u32) ~0 ? steer_pl->classify.fib_table :
                                                                                          0)),
                               &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT, paths);
    else if (steer_pl->classify.traffic_type == SR_STEER_IPV4)
        fib_table_entry_update(fib_table_find(FIB_PROTOCOL_IP4,
                                              (steer_pl->classify.fib_table != (u32) ~0 ? steer_pl->classify.fib_table :
                                                                                          0)),
                               &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT, paths);
 ...
 */
int
sr_mpls_steering_policy_add(mpls_label_t bsid, u32 table_id, ip46_address_t *prefix, u32 mask_width, u8 traffic_type,
                            ip46_address_t *next_hop, u8 nh_type, u32 color, char co_bits, mpls_label_t vpn_label)
{
    mpls_sr_main_t *sm = &sr_mpls_main;
    ...
    if (bsid != (u32) ~0) {
        ...
        if (traffic_type == SR_STEER_IPV6) {
            ...
            vec_add1(paths, path);
            fib_table_entry_path_add2(fib_table_find(FIB_PROTOCOL_IP6, (table_id != (u32) ~0 ? table_id : 0)), &pfx,
                                      FIB_SOURCE_SR, FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT, paths);
            vec_free(paths);
            ...
            path.frp_preference = 0;
            vec_add1(paths, path);
            fib_table_entry_path_add2(fib_table_find(FIB_PROTOCOL_IP4, (table_id != (u32) ~0 ? table_id : 0)), &pfx,
                                      FIB_SOURCE_SR, FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT, paths);
            vec_free(paths);
        }
    }
 ...
 */
int
sr_mpls_steering_policy_del(ip46_address_t *prefix, u32 mask_width, u8 traffic_type, u32 table_id, u32 color)
{
    mpls_sr_main_t *sm = &sr_mpls_main;
    sr_mpls_steering_key_t key;
    mpls_sr_steering_policy_t *steer_pl;
    ...
    if (steer_pl->bsid == (u32) ~0) {
        ...
        if (vec_len(steer_pl->color)) {
            ...
            if (steer_pl->classify.traffic_type == SR_STEER_IPV6) {
                ...
                pfx.fp_len      = steer_pl->classify.mask_width;
                pfx.fp_addr.ip6 = steer_pl->classify.prefix.ip6;
                fib_table_entry_delete(fib_table_find(FIB_PROTOCOL_IP6, steer_pl->classify.fib_table), &pfx,
                                       FIB_SOURCE_SR);
            } else if (steer_pl->classify.traffic_type == SR_STEER_IPV4) {
                pfx.fp_proto    = FIB_PROTOCOL_IP4;
                pfx.fp_len      = steer_pl->classify.mask_width;
                pfx.fp_addr.ip4 = steer_pl->classify.prefix.ip4;
                fib_table_entry_delete(fib_table_find(FIB_PROTOCOL_IP4, steer_pl->classify.fib_table), &pfx,
                                       FIB_SOURCE_SR);
            }
            /* Remove all the locks for this ones... */
            internal_label_unlock_co(steer_pl->next_hop, color, steer_pl->co_bits);
...
static clib_error_t *
sr_mpls_steer_policy_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    int is_del = 0;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del"))
            ...
            sr_policy_set = 1;
                 ...
                 unformat(input, "via next-hop %U color %d co %d", unformat_ip4_address, &nh.ip4, &color, &co_bits)) {
            sr_policy_set = 1;
            nh_type       = SR_STEER_IPV4;
        } else if (!sr_policy_set &&
                   unformat(input, "via next-hop %U color %d co %d", unformat_ip6_address, &nh.ip6, &color, &co_bits)) {
            sr_policy_set = 1;
    ...
    if (is_del)
        ...
        rv = sr_mpls_steering_policy_add(bsid, fib_table, &prefix, dst_mask_width, traffic_type, &nh, nh_type, color,
                                         co_bits, vpn_label);
...
VLIB_CLI_COMMAND(sr_mpls_steer_policy_command, static) = {
    ...
    .short_help = "sr mpls steer (del) l3 <ip_addr/mask> "
                  "via [sr policy bsid <mpls_label> || next-hop <ip46_addr> color <u32> co <0|1|2|3> ](fib-table "
                  "<fib_table_index>)(vpn-label 500)",
    .long_help = "\tSteer L3 traffic through an existing SR policy.\n"
                 "\tExamples:\n"
...
static clib_error_t *
show_sr_mpls_steering_policies_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    mpls_sr_main_t *sm                         = &sr_mpls_main;
    ...
    for (i = 0; i < vec_len(steer_policies); i++) {
        ...
        if (steer_pl->classify.traffic_type == SR_STEER_IPV4) {
            vlib_cli_output(vm, "Prefix: %U/%d via:", format_ip4_address, &steer_pl->classify.prefix.ip4,
                            steer_pl->classify.mask_width);
        } else if (steer_pl->classify.traffic_type == SR_STEER_IPV6) {
            vlib_cli_output(vm, "Prefix: %U/%d via:", format_ip6_address, &steer_pl->classify.prefix.ip6,
                            steer_pl->classify.mask_width);
        }
