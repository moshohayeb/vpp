...
vnet_app_namespace_add_del(vnet_app_namespace_add_del_args_t *a)
{
    ...
    if (a->is_add) {
        if (a->sw_if_index != APP_NAMESPACE_INVALID_INDEX && !vnet_get_sw_interface_safe(vnet_get_main(), a->sw_if_index))
            return clib_error_return_code(0, VNET_API_ERROR_INVALID_SW_IF_INDEX, 0, "sw_if_index %u doesn't exist",
                                          a->sw_if_index);
        ...
        if (a->sw_if_index == APP_NAMESPACE_INVALID_INDEX && a->ip4_fib_id == APP_NAMESPACE_INVALID_INDEX)
        ...
        return clib_error_return_code(0, VNET_API_ERROR_UNIMPLEMENTED, 0, "namespace deletion not supported");
    }
    return 0;
}
...
format_app_namespace(u8 *s, va_list *args)
{
    ...
    s = format(s, "%-10u%-20lu%-20u%-50v", app_namespace_index(app_ns), app_ns->ns_secret, app_ns->sw_if_index,
               app_ns->ns_id);
    return s;
}

static clib_error_t *
show_app_ns_fn(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    app_namespace_t *app_ns;
    session_table_t *st;
...
do_ns_list:
    vlib_cli_output(vm, "%-10s%-20s%-20s%-50s", "Index", "Secret", "sw_if_index", "Name");

    /* *INDENT-OFF* */
    pool_foreach(app_ns, app_namespace_pool, ({ vlib_cli_output(vm, "%U", format_app_namespace, app_ns); }));
    /* *INDENT-ON* */
