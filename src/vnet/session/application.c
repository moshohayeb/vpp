...
static void
application_local_listener_session_endpoint(local_session_t *ll, session_endpoint_t *sep)
{
    sep->transport_proto = session_type_transport_proto(ll->listener_session_type);
    sep->port            = ll->port;
...
application_table_add(application_t *app)
{
    if (app->api_client_index != APP_INVALID_INDEX)
        hash_set(app_main.app_by_api_client_index, app->api_client_index, app->app_index);
    else if (app->name)
...
application_alloc_and_init(app_init_args_t *a)
{
    ...
    if (!application_has_global_scope(app) && !application_has_local_scope(app))
    ...
    APP_DBG("New app name: %v api index: %u index %u", app->name, app->api_client_index, app->app_index);
...
application_free(application_t *app)
{
     ...
     */
    APP_DBG("Delete app name %v api index: %d index: %d", app->name, app->api_client_index, app->app_index);
...
application_listener_select_worker(stream_session_t *ls, u8 is_local)
{
    ...
    wrk_index = clib_bitmap_next_set(app_listener->workers, app_listener->accept_rotor + 1);
    if (wrk_index == ~0)
...
app_worker_alloc_and_init(application_t *app, app_worker_t **wrk)
{
    ...
    if ((rv = segment_manager_init(sm, app->sm_properties.segment_size, app->sm_properties.prealloc_fifos))) {
        app_worker_free(app_wrk);
        return rv;
    }
...
app_worker_start_listen(app_worker_t *app_wrk, stream_session_t *ls)
{
    ...
    if (!ls->server_rx_fifo && session_transport_service_type(ls) == TRANSPORT_SERVICE_CL) {
 ...
 */
int
application_start_listen(application_t *app, session_endpoint_extended_t *sep_ext, session_handle_t *res)
{
    app_listener_t *app_listener;
     ...
     */
    sst           = session_type_from_proto_and_ip(sep_ext->transport_proto, sep_ext->is_ip4);
 ...
 */
int
application_stop_listen(u32 app_index, u32 app_wrk_index, session_handle_t handle)
{
    app_listener_t *app_listener;
...
int
app_worker_open_session(app_worker_t *app, session_endpoint_t *sep, u32 api_context)
{
    int rv;
...
segment_manager_t *
app_worker_get_listen_segment_manager(app_worker_t *app, stream_session_t *listener)
{
    uword *smp;
...
vnet_app_worker_add_del(vnet_app_worker_add_del_args_t *a)
{
    ...
    if (!app)
        return clib_error_return_code(0, VNET_API_ERROR_INVALID_VALUE, 0, "App %u does not exist", a->app_index);
    ...
    if (a->is_add) {
        ...
        if (!wrk_map)
            return clib_error_return_code(0, VNET_API_ERROR_INVALID_VALUE, 0, "App %u does not have worker %u",
                                          app->app_index, a->wrk_index);
        app_wrk = app_worker_get(wrk_map->wrk_index);
        app_worker_map_free(app, wrk_map);
        if (!app_wrk)
            return clib_error_return_code(0, VNET_API_ERROR_INVALID_VALUE, 0, "No worker %u", a->wrk_index);
        app_worker_free(app_wrk);
    }
    return 0;
}
...
segment_manager_t *
application_get_local_segment_manager_w_session(app_worker_t *app, local_session_t *ls)
{
    stream_session_t *listener;
...
stream_session_t *
app_worker_first_listener(app_worker_t *app, u8 fib_proto, u8 transport_proto)
{
    stream_session_t *listener;
    u64 handle;
    u32 sm_index;
    ...
    hash_foreach(handle, sm_index, app->listeners_table, ({
                     listener = listen_session_get_from_handle(handle);
                     if (listener->session_type == sst && listener->enqueue_epoch != SESSION_PROXY_LISTENER_INDEX)
                         return listener;
                 }));
    /* *INDENT-ON* */
...
stream_session_t *
application_proxy_listener(app_worker_t *app, u8 fib_proto, u8 transport_proto)
{
    stream_session_t *listener;
    u64 handle;
    u32 sm_index;
...
static clib_error_t *
application_start_stop_proxy_fib_proto(application_t *app, u8 fib_proto, u8 transport_proto, u8 is_start)
{
...
static void
application_start_stop_proxy_local_scope(application_t *app, u8 transport_proto, u8 is_start)
{
...
void
application_start_stop_proxy(application_t *app, transport_proto_t transport_proto, u8 is_start)
{
    if (application_has_local_scope(app))
...
app_send_io_evt_rx(app_worker_t *app_wrk, stream_session_t *s, u8 lock)
{
    ...
    if (PREDICT_FALSE(s->session_state != SESSION_STATE_READY && s->session_state != SESSION_STATE_LISTENING)) {
    ...
    if (svm_fifo_has_event(s->server_rx_fifo) || svm_fifo_is_empty(s->server_rx_fifo))
        return 0;
 ...
 */
int
app_worker_lock_and_send_event(app_worker_t *app, stream_session_t *s, u8 evt_type)
{
...
void
application_local_listen_session_free(application_t *app, local_session_t *ll)
{
...
int
application_start_local_listen(application_t *app, session_endpoint_extended_t *sep_ext, session_handle_t *handle)
{
    app_listener_t *app_listener;
    session_endpoint_t *sep;
    app_worker_t *app_wrk;
    ...
    ll->listener_session_type    = session_type_from_proto_and_ip(sep_ext->transport_proto, sep_ext->is_ip4);
 ...
 */
int
application_stop_local_listen(u32 app_index, u32 wrk_map_index, session_handle_t lh)
{
    session_endpoint_t sep = SESSION_ENDPOINT_NULL;
    ...
    if (!clib_bitmap_get(app_listener->workers, wrk_map_index)) {
        clib_warning("app wrk %u not listening on handle %lu", wrk_map_index, lh);
        return -1;
    }
...
int
application_local_session_connect(app_worker_t *client_wrk, app_worker_t *server_wrk, local_session_t *ll, u32 opaque)
{
    ...
    rv = segment_manager_try_alloc_fifos(seg, props->rx_fifo_size, props->tx_fifo_size, &ls->server_rx_fifo,
                                         &ls->server_tx_fifo);
    if (rv) {
    ...
    if ((rv = server->cb_fns.add_segment_callback(server->api_client_index, &seg->ssvm))) {
...
static void
application_client_local_connect_key_parse(uword key, u32 *app_wrk_index, u32 *session_index)
{
...
application_local_session_connect_notify(local_session_t *ls)
{
    ...
    if ((rv = client->cb_fns.add_segment_callback(client->api_client_index, &seg->ssvm))) {
        clib_warning("failed to notify client %u of new segment", ls->client_wrk_index);
        segment_manager_segment_reader_unlock(sm);
    ...
    client->cb_fns.session_connected_callback(client_wrk->wrk_index, ls->client_opaque, (stream_session_t *) ls, is_fail);
...
int
application_local_session_cleanup(app_worker_t *client_wrk, app_worker_t *server_wrk, local_session_t *ls)
{
    svm_fifo_segment_private_t *seg;
    stream_session_t *listener;
    segment_manager_t *sm;
...
application_local_session_disconnect(u32 app_index, local_session_t *ls)
{
    ...
    if (!is_server && !is_client) {
        clib_warning("app %u is neither client nor server for session 0x%lx", app_index,
    ...
    if (app_index == ls->client_wrk_index) {
        ...
        if (!client_wrk) {
            ...
            application_t *client = application_get(client_wrk->app_index);
            client->cb_fns.session_connected_callback(client_wrk->wrk_index, ls->client_opaque, (stream_session_t *) ls,
                                                      1 /* is_fail */);
...
app_worker_local_sessions_free(app_worker_t *app_wrk)
{
    ...
    if (app_wrk->local_sessions) {
        /* *INDENT-OFF* */
        pool_foreach(ls, app_wrk->local_sessions, ({ application_local_session_disconnect(app_wrk->wrk_index, ls); }));
        /* *INDENT-ON* */
    }
    ...
    /* *INDENT-OFF* */
    hash_foreach(handle, index, app_wrk->local_connects, ({ vec_add1(handles, handle); }));
    /* *INDENT-ON* */

    for (i = 0; i < vec_len(handles); i++) {
        application_client_local_connect_key_parse(handles[i], &server_wrk_index, &session_index);
        server_wrk = app_worker_get_if_valid(server_wrk_index);
        if (server_wrk) {
...
vnet_app_add_tls_cert(vnet_app_add_tls_cert_args_t *a)
{
    ...
    app = application_get(a->app_index);
    if (!app)
        return clib_error_return_code(0, VNET_API_ERROR_APPLICATION_NOT_ATTACHED, 0, "app %u doesn't exist",
                                      a->app_index);
...
format_app_worker_listener(u8 *s, va_list *args)
{
    ...
    if (!app_wrk) {
        if (verbose)
            s = format(s, "%-40s%-25s%=10s%-15s%-15s%-10s", "Connection", "App", "Wrk", "API Client", "ListenerID",
                       "SegManager");
        else
    ...
    if (verbose) {
        ...
        s = format(s, "%-40s%-25s%=10s%-15u%-15u%-10u", str, app_name, buf, app->api_client_index, handle, sm_index);
    } else
...
application_format_listeners(application_t *app, int verbose)
{
    ...
    if (!app) {
        vlib_cli_output(vm, "%U", format_app_worker_listener, 0 /* header */, 0, 0, verbose);
        return;
    }
    ...
    pool_foreach(wrk_map, app->worker_maps, ({
                     ...
                     if (hash_elts(app_wrk->listeners_table) == 0)
                         continue;
                     hash_foreach(handle, sm_index, app_wrk->listeners_table, ({
                                      vlib_cli_output(vm, "%U", format_app_worker_listener, app_wrk, handle, sm_index,
                                                      verbose);
                                  }));
                 }));
    /* *INDENT-ON* */
...
app_worker_format_connects(app_worker_t *app_wrk, int verbose)
{
    ...
    if (!app_wrk) {
        if (verbose)
            vlib_cli_output(vm, "%-40s%-20s%-15s%-10s", "Connection", "App", "API Client", "SegManager");
        else
...
app_worker_format_local_sessions(app_worker_t *app_wrk, int verbose)
{
    ...
    /* Header */
    if (app_wrk == 0) {
        vlib_cli_output(vm, "%-40s%-15s%-20s", "Connection", "ServerApp", "ClientApp");
        return;
    }

    if (!pool_elts(app_wrk->local_sessions) && !pool_elts(app_wrk->local_connects))
        return;

    /* *INDENT-OFF* */
    pool_foreach(ls, app_wrk->local_sessions, ({
                     tp   = session_type_transport_proto(ls->listener_session_type);
                     conn = format(0, "[L][%U] *:%u", format_transport_proto_short, tp, ls->port);
                     vlib_cli_output(vm, "%-40v%-15u%-20u", conn, ls->app_wrk_index, ls->client_wrk_index);
...
app_worker_format_local_connects(app_worker_t *app, int verbose)
{
    ...
    if (app == 0) {
        if (verbose)
            vlib_cli_output(vm, "%-40s%-15s%-20s%-10s", "Connection", "App", "Peer App", "SegManager");
        else
            vlib_cli_output(vm, "%-40s%-15s%-20s", "Connection", "App", "Peer App");
        return;
    }
    ...
    hash_foreach(client_key, value, app->local_connects, ({
                     application_client_local_connect_key_parse(client_key, &app_wrk_index, &session_index);
                     server_wrk = app_worker_get(app_wrk_index);
                     ls         = application_get_local_session(server_wrk, session_index);
                     vlib_cli_output(vm, "%-40s%-15s%-20s", "TODO", ls->app_wrk_index, ls->client_wrk_index);
                 }));
    /* *INDENT-ON* */
}
...
format_application(u8 *s, va_list *args)
{
    ...
    if (app == 0) {
        if (verbose)
            s = format(s, "%-10s%-20s%-15s%-15s%-15s%-15s%-15s", "Index", "Name", "API Client", "Namespace",
                       "Add seg size", "Rx-f size", "Tx-f size");
        else
            s = format(s, "%-10s%-20s%-15s%-40s", "Index", "Name", "API Client", "Namespace");
        return s;
    }
    ...
    props       = application_segment_manager_properties(app);
    if (verbose)
        s = format(s, "%-10u%-20s%-15d%-15u%-15U%-15U%-15U", app->app_index, app_name, app->api_client_index,
                   app->ns_index, format_memory_size, props->add_segment_size, format_memory_size, props->rx_fifo_size,
                   format_memory_size, props->tx_fifo_size);
    else
        s = format(s, "%-10u%-20s%-15d%-40s", app->app_index, app_name, app->api_client_index, app_ns_name);
    return s;
}
...
application_format_all_listeners(vlib_main_t *vm, int do_local, int verbose)
{
    ...
    if (do_local) {
        ...
        /* *INDENT-OFF* */
        pool_foreach(app, app_main.app_pool, ({ application_format_local_sessions(app, verbose); }));
        /* *INDENT-ON* */
    } else {
        application_format_listeners(0, verbose);

        /* *INDENT-OFF* */
        pool_foreach(app, app_main.app_pool, ({ application_format_listeners(app, verbose); }));
        /* *INDENT-ON* */
    }
}
...
application_format_all_clients(vlib_main_t *vm, int do_local, int verbose)
{
    ...
    if (do_local) {
        ...
        /* *INDENT-OFF* */
        pool_foreach(app, app_main.app_pool, ({ application_format_local_connects(app, verbose); }));
        /* *INDENT-ON* */
    } else {
        application_format_connects(0, verbose);

        /* *INDENT-OFF* */
        pool_foreach(app, app_main.app_pool, ({ application_format_connects(app, verbose); }));
        /* *INDENT-ON* */
    }
}

static clib_error_t *
show_app_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!do_server && !do_client) {
        ...
        /* *INDENT-OFF* */
        pool_foreach(app, app_main.app_pool, ({ vlib_cli_output(vm, "%U", format_application, app, verbose); }));
        /* *INDENT-ON* */
    }
