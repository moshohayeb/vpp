...
always_inline void
make_v4_ss_kv(session_kv4_t *kv, ip4_address_t *lcl, ip4_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto)
{
...
always_inline void
make_v4_listener_kv(session_kv4_t *kv, ip4_address_t *lcl, u16 lcl_port, u8 proto)
{
...
always_inline void
make_v6_ss_kv(session_kv6_t *kv, ip6_address_t *lcl, ip6_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto)
{
...
always_inline void
make_v6_listener_kv(session_kv6_t *kv, ip6_address_t *lcl, u16 lcl_port, u8 proto)
{
    kv->key[0] = lcl->as_u64[0];
    kv->key[1] = lcl->as_u64[1];
    kv->key[2] = 0;
...
int
session_lookup_add_session_endpoint(u32 table_index, session_endpoint_t *sep, u64 value)
{
    session_table_t *st;
    session_kv4_t kv4;
    session_kv6_t kv6;
...
{
    if (action_index == SESSION_RULES_TABLE_ACTION_ALLOW || action_index == SESSION_RULES_TABLE_INVALID_INDEX)
        return 0;
    return 1;
}
...
session_lookup_app_listen_session(u32 app_index, u8 fib_proto, u8 transport_proto)
{
    application_t *app;
...
static stream_session_t *
session_lookup_action_to_session(u32 action_index, u8 fib_proto, u8 transport_proto)
{
    u32 app_index;
...
/** UNUSED */
stream_session_t *
session_lookup_rules_table_session4(session_table_t *st, u8 proto, ip4_address_t *lcl, u16 lcl_port, ip4_address_t *rmt,
                                    u16 rmt_port)
{
    session_rules_table_t *srt = &st->session_rules[proto];
    u32 action_index, app_index;
    action_index = session_rules_table_lookup4(srt, lcl, rmt, lcl_port, rmt_port);
    app_index    = session_lookup_action_to_handle(action_index);
    /* Nothing sophisticated for now, action index is app index */
...
/** UNUSED */
stream_session_t *
session_lookup_rules_table_session6(session_table_t *st, u8 proto, ip6_address_t *lcl, u16 lcl_port, ip6_address_t *rmt,
                                    u16 rmt_port)
{
    session_rules_table_t *srt = &st->session_rules[proto];
    u32 action_index, app_index;
    action_index = session_rules_table_lookup6(srt, lcl, rmt, lcl_port, rmt_port);
    app_index    = session_lookup_action_to_handle(action_index);
 ...
 */
u64
session_lookup_endpoint_listener(u32 table_index, session_endpoint_t *sep, u8 use_rules)
{
    ...
    if (sep->is_ip4) {
        ...
        if (use_rules) {
            ...
            srt = &st->session_rules[sep->transport_proto];
            ai  = session_rules_table_lookup6(srt, &lcl6, &sep->ip.ip6, 0, sep->port);
            if (session_lookup_action_index_is_valid(ai))
                return session_lookup_action_to_handle(ai);
        }
    }
...
session_lookup_listener4_i(session_table_t *st, ip4_address_t *lcl, u16 lcl_port, u8 proto, u8 use_wildcard)
{
    session_kv4_t kv4;
    int rv;
...
stream_session_t *
session_lookup_listener4(u32 fib_index, ip4_address_t *lcl, u16 lcl_port, u8 proto)
{
    session_table_t *st;
...
static stream_session_t *
session_lookup_listener6_i(session_table_t *st, ip6_address_t *lcl, u16 lcl_port, u8 proto, u8 ip_wildcard)
{
    session_kv6_t kv6;
    int rv;
...
stream_session_t *
session_lookup_listener6(u32 fib_index, ip6_address_t *lcl, u16 lcl_port, u8 proto)
{
    session_table_t *st;
 ...
 */
transport_connection_t *
session_lookup_connection_wt4(u32 fib_index, ip4_address_t *lcl, ip4_address_t *rmt, u16 lcl_port, u16 rmt_port,
                              u8 proto, u32 thread_index, u8 *is_filtered)
{
    ...
    if (rv == 0) {
        ...
        return tp_vfts[proto].get_connection(s->connection_index, thread_index);
    }
     ...
     */
    action_index = session_rules_table_lookup4(&st->session_rules[proto], lcl, rmt, lcl_port, rmt_port);
 ...
 */
transport_connection_t *
session_lookup_connection4(u32 fib_index, ip4_address_t *lcl, ip4_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto)
{
    session_table_t *st;
    session_kv4_t kv4;
    stream_session_t *s;
    ...
    if (rv == 0) {
        ...
        return tp_vfts[proto].get_connection(s->connection_index, s->thread_index);
    }
 ...
 */
stream_session_t *
session_lookup_safe4(u32 fib_index, ip4_address_t *lcl, ip4_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto)
{
    session_table_t *st;
    session_kv4_t kv4;
    stream_session_t *s;
 ...
 */
transport_connection_t *
session_lookup_connection_wt6(u32 fib_index, ip6_address_t *lcl, ip6_address_t *rmt, u16 lcl_port, u16 rmt_port,
                              u8 proto, u32 thread_index, u8 *is_filtered)
{
    session_table_t *st;
    stream_session_t *s;
    ...
    /* Check the session rules table */
    action_index = session_rules_table_lookup6(&st->session_rules[proto], lcl, rmt, lcl_port, rmt_port);
    if (session_lookup_action_index_is_valid(action_index)) {
        if ((*is_filtered = (action_index == SESSION_RULES_TABLE_ACTION_DROP)))
            return 0;
 ...
 */
transport_connection_t *
session_lookup_connection6(u32 fib_index, ip6_address_t *lcl, ip6_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto)
{
    session_table_t *st;
    stream_session_t *s;
    session_kv6_t kv6;
 ...
 */
stream_session_t *
session_lookup_safe6(u32 fib_index, ip6_address_t *lcl, ip6_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto)
{
    session_table_t *st;
    session_kv6_t kv6;
    stream_session_t *s;
...
vnet_session_rule_add_del(session_rule_add_del_args_t *args)
{
    ...
    if (!app_ns)
        return clib_error_return_code(0, VNET_API_ERROR_APP_INVALID_NS, 0, "invalid app ns");
    if (args->scope > 3)
        return clib_error_return_code(0, VNET_API_ERROR_INVALID_VALUE, 0, "invalid scope");
    if (args->transport_proto != TRANSPORT_PROTO_TCP && args->transport_proto != TRANSPORT_PROTO_UDP)
        return clib_error_return_code(0, VNET_API_ERROR_INVALID_VALUE, 0, "invalid transport proto");
...
format_ip4_session_lookup_kvp(u8 *s, va_list *args)
{
    ...
    if (!is_local) {
        ...
        str = format(0, "[%U] %U:%d->%U:%d", format_transport_proto_short, key->proto, format_ip4_address, &key->src,
                     clib_net_to_host_u16(key->src_port), format_ip4_address, &key->dst,
        ...
        app_name = application_name_from_index(app_wrk->app_index);
        str      = format(0, "[%U] %U:%d", format_transport_proto_short, key->proto, format_ip4_address, &key->src,
...
void
session_lookup_show_table_entries(vlib_main_t *vm, session_table_t *table, u8 type, u8 is_local)
{
...
static clib_error_t *
session_rule_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del"))
            ...
            ;
        else if (unformat(input, "%U/%d %d %U/%d %d", unformat_ip4_address, &lcl_ip.ip4, &lcl_plen, &lcl_port,
                          unformat_ip4_address, &rmt_ip.ip4, &rmt_plen, &rmt_port)) {
            is_ip4   = 1;
            conn_set = 1;
        } else if (unformat(input, "%U/%d %d %U/%d %d", unformat_ip6_address, &lcl_ip.ip6, &lcl_plen, &lcl_port,
                            unformat_ip6_address, &rmt_ip.ip6, &rmt_plen, &rmt_port)) {
            is_ip4   = 0;
            conn_set = 1;
...
void
session_lookup_dump_rules_table(u32 fib_index, u8 fib_proto, u8 transport_proto)
{
...
void
session_lookup_dump_local_rules_table(u32 table_index, u8 fib_proto, u8 transport_proto)
{
    vlib_main_t *vm = vlib_get_main();
    session_rules_table_t *srt;
    session_table_t *st;
...
static clib_error_t *
show_session_rules_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (show_one) {
        srt = &st->session_rules[transport_proto];
        session_rules_table_show_rule(vm, srt, &lcl_ip, lcl_port, &rmt_ip, rmt_port, is_ip4);
        return 0;
    }
