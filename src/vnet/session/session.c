...
static inline int
session_send_evt_to_thread(void *data, void *args, u32 thread_index, session_evt_type_t evt_type)
{
    session_event_t *evt;
    svm_msg_q_msg_t msg;
    svm_msg_q_t *mq;
...
int
session_send_io_evt_to_thread_custom(void *data, u32 thread_index, session_evt_type_t evt_type)
{
    return session_send_evt_to_thread(data, 0, thread_index, evt_type);
}

int
session_send_ctrl_evt_to_thread(stream_session_t *s, session_evt_type_t evt_type)
{
    /* only event supported for now is disconnect */
    ASSERT(evt_type == FIFO_EVENT_DISCONNECT);
    return session_send_evt_to_thread(s, 0, s->thread_index, FIFO_EVENT_DISCONNECT);
}
...
session_alloc(u32 thread_index)
{
    ...
    stream_session_t *s;
    u8 will_expand = 0;
    pool_get_aligned_will_expand(smm->sessions[thread_index], will_expand, CLIB_CACHE_LINE_BYTES);
    /* If we have peekers, let them finish */
    if (PREDICT_FALSE(will_expand && vlib_num_workers())) {
        clib_rwlock_writer_lock(&smm->peekers_rw_locks[thread_index]);
        pool_get_aligned(session_manager_main.sessions[thread_index], s, CLIB_CACHE_LINE_BYTES);
...
{
    segment_manager_dealloc_fifos(s->svm_segment_index, s->server_rx_fifo, s->server_tx_fifo);
    session_free(s);
}
...
session_alloc_fifos(segment_manager_t *sm, stream_session_t *s)
{
    ...
    if ((rv = segment_manager_alloc_session_fifos(sm, &server_rx_fifo, &server_tx_fifo, &fifo_segment_index)))
        return rv;
...
static int
session_alloc_and_init(segment_manager_t *sm, transport_connection_t *tc, u8 alloc_fifos, stream_session_t **ret_s)
{
    stream_session_t *s;
    int rv;
 ...
 */
always_inline void
session_enqueue_discard_chain_bytes(vlib_main_t *vm, vlib_buffer_t *b, vlib_buffer_t **chain_b, u32 n_bytes_to_drop)
{
 ...
 */
always_inline int
session_enqueue_chain_tail(stream_session_t *s, vlib_buffer_t *b, u32 offset, u8 is_in_order)
{
    ...
    do {
        ...
        if (is_in_order) {
            ...
            rv = svm_fifo_enqueue_with_offset(s->server_rx_fifo, offset, len, data);
            if (rv) {
...
session_enqueue_stream_connection(transport_connection_t *tc, vlib_buffer_t *b, u32 offset, u8 queue_event, u8 is_in_order)
{
    ...
    if (is_in_order) {
        enqueued = svm_fifo_enqueue_nowait(s->server_rx_fifo, b->current_length, vlib_buffer_get_current(b));
        ...
        rv = svm_fifo_enqueue_with_offset(s->server_rx_fifo, offset, b->current_length, vlib_buffer_get_current(b));
    ...
    if (queue_event) {
        ...
        if (s->enqueue_epoch != enqueue_epoch) {
            ...
            vec_add1(smm->session_to_enqueue[tc->proto][thread_index], s->session_index);
        }
    }
...
int
session_enqueue_dgram_connection(stream_session_t *s, session_dgram_hdr_t *hdr, vlib_buffer_t *b, u8 proto, u8 queue_event)
{
    int enqueued = 0, rv, in_order_off;

    ASSERT(svm_fifo_max_enqueue(s->server_rx_fifo) >= b->current_length + sizeof(*hdr));

    svm_fifo_enqueue_nowait(s->server_rx_fifo, sizeof(session_dgram_hdr_t), (u8 *) hdr);
    enqueued = svm_fifo_enqueue_nowait(s->server_rx_fifo, b->current_length, vlib_buffer_get_current(b));
    if (PREDICT_FALSE((b->flags & VLIB_BUFFER_NEXT_PRESENT) && enqueued >= 0)) {
        in_order_off = enqueued > b->current_length ? enqueued : 0;
        rv           = session_enqueue_chain_tail(s, b, in_order_off, 1);
    ...
    if (queue_event) {
        ...
        if (s->enqueue_epoch != enqueue_epoch) {
            s->enqueue_epoch = enqueue_epoch;
            vec_add1(smm->session_to_enqueue[proto][thread_index], s->session_index);
        }
    }
    return enqueued;
}
...
int
stream_session_peek_bytes(transport_connection_t *tc, u8 *buffer, u32 offset, u32 max_bytes)
{
    stream_session_t *s = session_get(tc->s_index, tc->thread_index);
 ...
 */
void
stream_session_init_fifos_pointers(transport_connection_t *tc, u32 rx_pointer, u32 tx_pointer)
{
    stream_session_t *s;
    s = session_get(tc->s_index, tc->thread_index);
...
session_stream_connect_notify(transport_connection_t *tc, u8 is_fail)
{
     ...
     */
    if (app->cb_fns.session_connected_callback(app_wrk->wrk_index, opaque, new_s, is_fail)) {
 ...
 */
int
session_dgram_connect_notify(transport_connection_t *tc, u32 old_thread_index, stream_session_t **new_session)
{
 ...
 */
int
stream_session_accept(transport_connection_t *tc, u32 listener_index, u8 notify)
{
...
stream_session_disconnect_transport(stream_session_t *s)
{
    ...
    s->session_state = SESSION_STATE_CLOSED;
    tp_vfts[session_get_transport_proto(s)].close(s->connection_index, s->thread_index);
}
...
stream_session_cleanup(stream_session_t *s)
{
    ...
    session_lookup_del_session(s);
    tp_vfts[session_get_transport_proto(s)].cleanup(s->connection_index, s->thread_index);
...
session_vpp_event_queues_allocate(session_manager_main_t *smm)
{
    ...
    for (i = 0; i < vec_len(smm->vpp_event_queues); i++) {
        ...
        svm_msg_q_ring_cfg_t rc[SESSION_MQ_N_RINGS] = {{evt_q_length, evt_size, 0}, {notif_q_size, 256, 0}};
        cfg->consumer_pid                           = 0;
        cfg->n_rings                                = 2;
 ...
 */
void
session_register_transport(transport_proto_t transport_proto, const transport_proto_vft_t *vft, u8 is_ip4, u32 output_node)
{
    session_manager_main_t *smm = &session_manager_main;
    ...
    /* *INDENT-OFF* */
    if (output_node != ~0) {
        foreach_vlib_main(({ next_index = vlib_node_add_next(this_vlib_main, session_queue_node.index, output_node); }));
    }
    /* *INDENT-ON* */
...
session_get_transport(stream_session_t *s)
{
    ...
    if (s->session_state != SESSION_STATE_LISTENING) {
        tp = session_get_transport_proto(s);
        return tp_vfts[tp].get_connection(s->connection_index, s->thread_index);
    }
    return 0;
}
...
int
listen_session_get_local_session_endpoint(stream_session_t *listener, session_endpoint_t *sep)
{
...
session_flush_frames_main_thread(vlib_main_t *vm)
{
    ...
    vlib_process_signal_event_mt(vm, session_queue_process_node.index, SESSION_Q_PROCESS_FLUSH_FRAMES, 0);
}
    ...
    if (smm->preallocated_sessions) {
        if (num_threads == 1) {
            ...
            for (j = 1; j < num_threads; j++) {
                pool_init_fixed(smm->sessions[j], preallocated_sessions_per_worker);
            }
        }
    }
...
session_node_enable_disable(u8 is_en)
{
    ...
    foreach_vlib_main(({
        if (have_workers && ii == 0) {
            vlib_node_set_state(this_vlib_main, session_queue_process_node.index, state);
            if (is_en) {
                vlib_node_t *n = vlib_get_node(this_vlib_main, session_queue_process_node.index);
                vlib_start_process(this_vlib_main, n->runtime_index);
            } else {
                vlib_process_signal_event_mt(this_vlib_main, session_queue_process_node.index, SESSION_Q_PROCESS_STOP, 0);
            }
...
session_config_fn(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "event-queue-length %d", &nitems)) {
            ...
            if (tmp >= 0x100000000)
                return clib_error_return(0, "memory size %llx (%lld) too large", tmp, tmp);
