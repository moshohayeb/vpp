...
u32
transport_endpoint_lookup(transport_endpoint_table_t *ht, u8 proto, ip46_address_t *ip, u16 port)
{
...
void
transport_endpoint_table_add(transport_endpoint_table_t *ht, u8 proto, transport_endpoint_t *te, u32 value)
{
    clib_bihash_kv_24_8_t kv;
...
void
transport_endpoint_table_del(transport_endpoint_table_t *ht, u8 proto, transport_endpoint_t *te)
{
    clib_bihash_kv_24_8_t kv;
 ...
 */
void
transport_register_protocol(transport_proto_t transport_proto, const transport_proto_vft_t *vft,
...
transport_endpoint_cleanup(u8 proto, ip46_address_t *lcl_ip, u16 port)
{
    ...
    tepi = transport_endpoint_lookup(&local_endpoints_table, proto, lcl_ip, clib_net_to_host_u16(port));
...
transport_alloc_local_port(u8 proto, ip46_address_t *ip)
{
    ...
    for (tries = 0; tries < limit; tries++) {
        ...
        if (tei == ENDPOINT_INVALID_INDEX) {
            ...
            tep->port = port;
            transport_endpoint_table_add(&local_endpoints_table, proto, tep, tep - local_endpoints);
            clib_spinlock_unlock_if_init(&local_endpoints_lock);
...
int
transport_alloc_local_endpoint(u8 proto, transport_endpoint_t *rmt, ip46_address_t *lcl_addr, u16 *lcl_port)
{
    fib_prefix_t prefix;
    ...
    if (sw_if_index == ENDPOINT_INVALID_INDEX) {
        clib_warning("no resolving interface for %U", format_ip46_address, &rmt->ip, (rmt->is_ip4 == 0) + 1);
        return -1;
    }
    ...
    if (rmt->is_ip4) {
        ...
        if (!ip4) {
            clib_warning("no routable ip4 address on %U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
            return -1;
        }
        ...
        if (ip6 == 0) {
            clib_warning("no routable ip6 addresses on %U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
            return -1;
        }
...
transport_init(void)
{
    ...
    clib_bihash_init_24_8(&local_endpoints_table, "local endpoints table", smm->local_endpoints_table_buckets,
