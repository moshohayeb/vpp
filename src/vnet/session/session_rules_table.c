...
void
session_rules_table_add_tag(session_rules_table_t *srt, u8 *tag, u32 rule_index, u8 is_ip4)
{
    uword *rip;
    session_rule_tag_t *rt;
    u32 rti_key;
...
format_session_rule4(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d] rule: %U/%d %d %U/%d %d action: %d tag: %v", ri, format_ip4_address, &match->lcl_ip,
               ip4_mask_to_preflen(&mask->lcl_ip), clib_net_to_host_u16(match->lcl_port), format_ip4_address,
               &match->rmt_ip, ip4_mask_to_preflen(&mask->rmt_ip), clib_net_to_host_u16(match->rmt_port),
...
format_session_rule6(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d] rule: %U/%d %d %U/%d %d action: %d tag: %v", ri, format_ip6_address, &match->lcl_ip,
               ip6_mask_to_preflen(&mask->lcl_ip), clib_net_to_host_u16(match->lcl_port), format_ip6_address,
               &match->rmt_ip, ip6_mask_to_preflen(&mask->rmt_ip), clib_net_to_host_u16(match->rmt_port),
               sr->action_index, tag ? tag : null_tag);
    if (vec_len(sr->next_indices)) {
        s = format(s, "\n    children: ");
        for (i = 0; i < vec_len(sr->next_indices); i++)
...
void
session_rules_table_init_rule_16(mma_rule_16_t *rule, fib_prefix_t *lcl, u16 lcl_port, fib_prefix_t *rmt, u16 rmt_port)
{
...
void
session_rules_table_init_rule_40(mma_rule_40_t *rule, fib_prefix_t *lcl, u16 lcl_port, fib_prefix_t *rmt, u16 rmt_port)
{
...
mma_rule_16_t *
session_rules_table_alloc_rule_16(mma_rules_table_16_t *srt, fib_prefix_t *lcl, u16 lcl_port, fib_prefix_t *rmt,
                                  u16 rmt_port)
{
...
mma_rule_40_t *
session_rules_table_alloc_rule_40(mma_rules_table_40_t *srt, fib_prefix_t *lcl, u16 lcl_port, fib_prefix_t *rmt,
                                  u16 rmt_port)
{
    mma_rule_40_t *rule;
...
u32
session_rules_table_lookup_rule4(session_rules_table_t *srt, ip4_address_t *lcl_ip, ip4_address_t *rmt_ip, u16 lcl_port,
                                 u16 rmt_port)
{
...
u32
session_rules_table_lookup_rule6(session_rules_table_t *srt, ip6_address_t *lcl_ip, ip6_address_t *rmt_ip, u16 lcl_port,
                                 u16 rmt_port)
{
...
u32
session_rules_table_lookup6(session_rules_table_t *srt, ip6_address_t *lcl_ip, ip6_address_t *rmt_ip, u16 lcl_port,
                            u16 rmt_port)
{
    mma_rules_table_40_t *srt6    = &srt->session_rules_tables_40;
    session_mask_or_match_6_t key = {
 ...
 */
clib_error_t *
session_rules_table_add_del(session_rules_table_t *srt, session_rule_table_add_del_args_t *args)
{
    ...
    if (args->is_add && ri_from_tag != SESSION_RULES_TABLE_INVALID_INDEX)
        return clib_error_return_code(0, VNET_API_ERROR_INVALID_VALUE, 0, "tag exists");
    ...
    if (fib_proto == FIB_PROTOCOL_IP4) {
        ...
        if (args->is_add) {
            ...
            if (!rv) {
                ...
                ri = session_rules_table_lookup_rule4(srt, &args->lcl.fp_addr.ip4, &args->rmt.fp_addr.ip4,
        ...
        if (args->is_add) {
            ...
            if (!rv) {
                ...
                ri = session_rules_table_lookup_rule6(srt, &args->lcl.fp_addr.ip6, &args->rmt.fp_addr.ip6,
                                                      args->lcl_port, args->rmt_port);
                if (ri != SESSION_RULES_TABLE_INVALID_INDEX) {
        ...
        return clib_error_return_code(0, VNET_API_ERROR_INVALID_VALUE_2, 0, "invalid fib proto");
    return 0;
}
...
void
session_rules_table_show_rule(vlib_main_t *vm, session_rules_table_t *srt, ip46_address_t *lcl_ip, u16 lcl_port,
...
void
session_rules_table_cli_dump(vlib_main_t *vm, session_rules_table_t *srt, u8 fib_proto)
{
    if (fib_proto == FIB_PROTOCOL_IP4) {
        mma_rules_table_16_t *srt4;
        mma_rule_16_t *sr4;
        ...
        /* *INDENT-OFF* */
        pool_foreach(sr4, srt4->rules, ({ vlib_cli_output(vm, "%U", format_session_rule4, srt, sr4); }));
        /* *INDENT-ON* */
        ...
        /* *INDENT-OFF* */
        pool_foreach(sr6, srt6->rules, ({ vlib_cli_output(vm, "%U", format_session_rule6, srt, sr6); }));
        /* *INDENT-ON* */
    }
}
