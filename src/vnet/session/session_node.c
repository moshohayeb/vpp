...
session_mq_accepted_reply_handler(void *data)
{
    ...
    if (session_handle_is_local(mp->handle)) {
        ...
        if (app_wrk->app_index != mp->context) {
            clib_warning("server %u doesn't own local handle 0x%lx", mp->context, mp->handle);
            return;
        }
...
session_mq_reset_reply_handler(void *data)
{
    ...
    if (!app_wrk || app_wrk->app_index != app->app_index) {
        clib_warning("App % does not own handle 0x%lx!", app->app_index, mp->handle);
        return;
    }
...
format_session_queue_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "SESSION_QUEUE: session index %d, server thread index %d", t->session_index, t->server_thread_index);
    return s;
}
...
static void
session_tx_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, u32 next_index, u32 *to_next, u16 n_segs,
...
always_inline void
session_tx_fifo_chain_tail(vlib_main_t *vm, session_tx_context_t *ctx, vlib_buffer_t *b, u16 *n_bufs, u8 peek_data)
{
    ...
    chain_b       = b;
    left_from_seg = clib_min(ctx->snd_mss - b->current_length, ctx->left_to_snd);
    ...
    for (j = 1; j < ctx->n_bufs_per_seg; j++) {
        ...
        *n_bufs -= 1;
        ...
        if (peek_data) {
            n_bytes_read = svm_fifo_peek(ctx->s->server_tx_fifo, ctx->tx_offset, len_to_deq, data);
            ...
            if (ctx->transport_vft->tx_type == TRANSPORT_TX_DGRAM) {
                ...
                n_bytes_read = svm_fifo_peek(f, hdr->data_offset, deq_now, data);
                ...
                n_bytes_read = svm_fifo_dequeue_nowait(ctx->s->server_tx_fifo, len_to_deq, data);
        }
...
always_inline int
session_output_try_get_buffers(vlib_main_t *vm, session_manager_main_t *smm, u32 thread_index, u16 *n_bufs, u32 wanted)
{
    u32 n_alloc;
    vec_validate_aligned(smm->tx_buffers[thread_index], wanted - 1, CLIB_CACHE_LINE_BYTES);
    n_alloc = vlib_buffer_alloc(vm, &smm->tx_buffers[thread_index][*n_bufs], wanted - *n_bufs);
    *n_bufs += n_alloc;
...
always_inline void
session_tx_fill_buffer(vlib_main_t *vm, session_tx_context_t *ctx, vlib_buffer_t *b, u16 *n_bufs, u8 peek_data)
{
    u32 len_to_deq;
    u8 *data0;
    int n_bytes_read;
    ...
    if (peek_data) {
        n_bytes_read = svm_fifo_peek(ctx->s->server_tx_fifo, ctx->tx_offset, len_to_deq, data0);
        ASSERT(n_bytes_read > 0);
        ...
        if (ctx->transport_vft->tx_type == TRANSPORT_TX_DGRAM) {
            ...
            n_bytes_read = svm_fifo_dequeue_nowait(ctx->s->server_tx_fifo, len_to_deq, data0);
            ASSERT(n_bytes_read > 0);
        }
    }
...
always_inline void
session_tx_set_dequeue_params(vlib_main_t *vm, session_tx_context_t *ctx, u32 max_segs, u8 peek_data)
{
    ...
    if (peek_data) {
        ...
        if (ctx->transport_vft->tx_type == TRANSPORT_TX_DGRAM) {
            ...
            svm_fifo_peek(ctx->s->server_tx_fifo, 0, sizeof(ctx->hdr), (u8 *) &ctx->hdr);
    ...
    if (ctx->max_dequeue < ctx->snd_space) {
        ...
        ctx->max_len_to_snd =
            (ctx->max_dequeue > ctx->snd_mss) ? ctx->max_dequeue - ctx->max_dequeue % ctx->snd_mss : ctx->max_dequeue;
        /* TODO Nagle ? */
    } else {
    ...
    n_bytes_per_buf = vlib_buffer_free_list_buffer_size(vm, VLIB_BUFFER_DEFAULT_FREE_LIST_INDEX);
    ...
    ctx->deq_per_first_buf = clib_min(ctx->snd_mss, n_bytes_per_buf - MAX_HDRS_LEN);
}

always_inline int
session_tx_fifo_read_and_snd_i(vlib_main_t *vm, vlib_node_runtime_t *node, session_event_t *e, stream_session_t *s,
                               int *n_tx_packets, u8 peek_data)
{
    ...
    session_tx_set_dequeue_params(vm, ctx, VLIB_FRAME_SIZE - *n_tx_packets, peek_data);
    ...
    if (n_bufs < n_bufs_needed) {
        session_output_try_get_buffers(vm, smm, thread_index, &n_bufs, ctx->n_bufs_per_seg * VLIB_FRAME_SIZE);
    ...
    if (PREDICT_FALSE(n_trace > 0))
        session_tx_trace_frame(vm, node, next_index, to_next, ctx->n_segs_per_evt, s, n_trace);
    ...
    *n_tx_packets += ctx->n_segs_per_evt;
    ...
    if (!peek_data && ctx->transport_vft->tx_type == TRANSPORT_TX_DGRAM) {
        ...
        if (ctx->max_len_to_snd < ctx->max_dequeue)
            svm_fifo_overwrite_head(s->server_tx_fifo, (u8 *) &ctx->hdr, sizeof(session_dgram_pre_hdr_t));
...
int
session_tx_fifo_peek_and_snd(vlib_main_t *vm, vlib_node_runtime_t *node, session_event_t *e, stream_session_t *s,
                             int *n_tx_pkts)
{
...
int
session_tx_fifo_dequeue_and_snd(vlib_main_t *vm, vlib_node_runtime_t *node, session_event_t *e, stream_session_t *s,
                                int *n_tx_pkts)
{
...
int
session_tx_fifo_dequeue_internal(vlib_main_t *vm, vlib_node_runtime_t *node, session_event_t *e, stream_session_t *s,
                                 int *n_tx_pkts)
{
    application_t *app;
...
session_queue_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (!n_to_dequeue && !vec_len(pending_events) && !vec_len(smm->pending_disconnects[thread_index]))
        return 0;
...
skip_dequeue:
    ...
    for (i = 0; i < n_events; i++) {
        ...
        case FIFO_EVENT_APP_TX:
             ...
             * different nodes */
            rv = (smm->session_tx_fns[s->session_type])(vm, node, e, s, &n_tx_packets);
            if (PREDICT_TRUE(rv == SESSION_TX_OK)) {
                ...
                vlib_node_increment_counter(vm, node->node_index, SESSION_QUEUE_ERROR_NO_BUFFER, 1);
                continue;
            }
            break;
