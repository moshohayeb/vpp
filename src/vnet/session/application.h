...
typedef struct _stream_session_cb_vft {
    ...
    int (*add_segment_callback)(u32 api_client_index, const ssvm_private_t *ssvm_seg);
    /** Notify server of new segment */
    int (*del_segment_callback)(u32 api_client_index, const ssvm_private_t *ssvm_seg);

    /** Notify server of newly accepted session */
    int (*session_accept_callback)(stream_session_t *new_session);

    /** Connection request callback */
    int (*session_connected_callback)(u32 app_wrk_index, u32 opaque, stream_session_t *s, u8 code);
...
always_inline void
local_session_parse_handle(session_handle_t handle, u32 *app_or_wrk_index, u32 *session_index)
{
    u32 bottom;
...
{
    return ((u64) SESSION_LOCAL_HANDLE_PREFIX << 32) | (u64) local_session_id(ls);
}
