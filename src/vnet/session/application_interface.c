...
session_endpoint_in_ns(session_endpoint_t *sep)
{
    ...
    if (!is_lep && sep->sw_if_index != ENDPOINT_INVALID_INDEX &&
        ...
        clib_warning("sw_if_index %u not configured with ip %U", sep->sw_if_index, format_ip46_address, &sep->ip,
                     sep->is_ip4);
        return 0;
    }
...
static void
session_endpoint_update_for_app(session_endpoint_extended_t *sep, application_t *app)
{
    ...
    if (app_ns) {
        ...
        sep->fib_index   = sep->is_ip4 ? app_ns->ip4_fib_index : app_ns->ip6_fib_index;
    }
}
...
vnet_bind_inline(vnet_bind_args_t *a)
{
     ...
     */
    if (application_has_local_scope(app) && session_endpoint_is_local(&a->sep)) {
        if ((rv = application_start_local_listen(app, &a->sep_ext, &a->handle)))
            return rv;
        ll_handle = a->handle;
    }
    ...
    if (rv && ll_handle != SESSION_INVALID_HANDLE) {
        application_stop_local_listen(a->app_index, a->wrk_map_index, ll_handle);
        return rv;
    }
...
vnet_unbind_inline(vnet_unbind_args_t *a)
{
    ...
    if (application_has_local_scope(app)) {
        if ((rv = application_stop_local_listen(a->app_index, a->wrk_map_index, a->handle)))
            return rv;
    }
     ...
     */
    if (application_has_global_scope(app))
        return application_stop_listen(a->app_index, a->wrk_map_index, a->handle);
    return 0;
}
...
application_connect(vnet_connect_args_t *a)
{
    ...
    if (application_has_local_scope(client)) {
        ...
        if (server_index != a->app_index) {
            ...
            server_wrk = application_listener_select_worker(listener, 1 /* is_local */);
            return application_local_session_connect(client_wrk, server_wrk, ll, a->api_context);
        }
    }
...
global_scope:
    ...
    if (listener) {
        server_wrk = application_listener_select_worker(listener, 0 /* is_local */);
...
unformat_vnet_uri(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%U://%U/%d", unformat_transport_proto, &transport_proto, unformat_ip4_address, &sep->ip.ip4,
                 &port)) {
        sep->transport_proto = transport_proto;
        sep->port            = clib_host_to_net_u16(port);
        sep->is_ip4          = 1;
        return 1;
    } else if (unformat(input, "%U://[%s]%U/%d", unformat_transport_proto, &transport_proto, &sep->hostname,
                        unformat_ip4_address, &sep->ip.ip4, &port)) {
        sep->transport_proto = transport_proto;
        sep->port            = clib_host_to_net_u16(port);
        sep->is_ip4          = 1;
        return 1;
    } else if (unformat(input, "%U://%U/%d", unformat_transport_proto, &transport_proto, unformat_ip6_address,
                        &sep->ip.ip6, &port)) {
        sep->transport_proto = transport_proto;
        sep->port            = clib_host_to_net_u16(port);
        sep->is_ip4          = 0;
        return 1;
                        ...
                        unformat_ip6_address, &sep->ip.ip6, &port)) {
        sep->transport_proto = transport_proto;
        sep->port            = clib_host_to_net_u16(port);
        sep->is_ip4          = 0;
        return 1;
...
vnet_application_attach(vnet_app_attach_args_t *a)
{
    ...
    if (a->api_client_index != APP_INVALID_INDEX)
        ...
        return clib_error_return_code(0, VNET_API_ERROR_INVALID_VALUE, 0, "api index or name must be provided");

    if (app)
        return clib_error_return_code(0, VNET_API_ERROR_APP_ALREADY_ATTACHED, 0, "app already attached");
...
vnet_unbind_uri(vnet_unbind_args_t *a)
{
    ...
    table_index = session_lookup_get_index_for_fib(fib_ip_proto(!sep.is_ip4), 0);
