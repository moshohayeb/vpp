...
always_inline u32
segment_manager_segment_index(segment_manager_t *sm, svm_fifo_segment_private_t *seg)
{
...
segment_manager_init(segment_manager_t *sm, u32 first_seg_size, u32 prealloc_fifo_pairs)
{
    ...
    if (prealloc_fifo_pairs) {
        ...
        for (i = 0; i < approx_segment_count + 1; i++) {
            ...
            svm_fifo_segment_preallocate_fifo_pairs(segment, props->rx_fifo_size, props->tx_fifo_size,
                                                    &prealloc_fifo_pairs);
...
segment_manager_del(segment_manager_t *sm)
{
    ...
    ASSERT(!segment_manager_has_fifos(sm) && segment_manager_app_detached(sm));
    ...
    /* *INDENT-OFF* */
    pool_foreach(fifo_segment, sm->segments, ({ segment_manager_del_segment(sm, fifo_segment); }));
    /* *INDENT-ON* */
...
int
segment_manager_try_alloc_fifos(svm_fifo_segment_private_t *fifo_segment, u32 rx_fifo_size, u32 tx_fifo_size,
                                svm_fifo_t **rx_fifo, svm_fifo_t **tx_fifo)
{
    rx_fifo_size = clib_max(rx_fifo_size, default_fifo_size);
    *rx_fifo     = svm_fifo_segment_alloc_fifo(fifo_segment, rx_fifo_size, FIFO_SEGMENT_RX_FREELIST);

    tx_fifo_size = clib_max(tx_fifo_size, default_fifo_size);
    *tx_fifo     = svm_fifo_segment_alloc_fifo(fifo_segment, tx_fifo_size, FIFO_SEGMENT_TX_FREELIST);
...
int
segment_manager_alloc_session_fifos(segment_manager_t *sm, svm_fifo_t **rx_fifo, svm_fifo_t **tx_fifo,
                                    u32 *fifo_segment_index)
{
    ...
    segment_manager_foreach_segment_w_lock(fifo_segment, sm, ({
                                               alloc_fail =
                                                   segment_manager_try_alloc_fifos(fifo_segment, props->rx_fifo_size,
...
alloc_check:
    ...
    alloc_success:
        ...
        *fifo_segment_index         = segment_manager_segment_index(sm, fifo_segment);
        ...
        if (added_a_segment)
            rv = app_worker_add_segment_notify(sm->app_wrk_index, &fifo_segment->ssvm);
    ...
    if (props->add_segment) {
        ...
        alloc_fail =
            segment_manager_try_alloc_fifos(fifo_segment, props->rx_fifo_size, props->tx_fifo_size, rx_fifo, tx_fifo);
        added_a_segment = 1;
        goto alloc_check;
    } else {
...
void
segment_manager_dealloc_fifos(u32 segment_index, svm_fifo_t *rx_fifo, svm_fifo_t *tx_fifo)
{
    svm_fifo_segment_private_t *fifo_segment;
    segment_manager_t *sm;
 ...
 */
svm_msg_q_t *
segment_manager_alloc_queue(svm_fifo_segment_private_t *segment, segment_manager_properties_t *props)
{
    ...
    /* *INDENT-OFF* */
    svm_msg_q_ring_cfg_t rc[SESSION_MQ_N_RINGS] = {{props->evt_q_size, fifo_evt_size, 0},
...
static clib_error_t *
segment_manager_show_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    segment_manager_main_t *smm = &segment_manager_main;
    svm_fifo_segment_private_t *seg;
    segment_manager_t *sm;
    ...
    if (verbose && pool_elts(smm->segment_managers)) {
        vlib_cli_output(vm, "%-10s%=15s%=12s", "Index", "App Index", "Segments");

        /* *INDENT-OFF* */
        pool_foreach(sm, smm->segment_managers, ({
                         vlib_cli_output(vm, "%-10d%=15d%=12d", segment_manager_index(sm), sm->app_wrk_index,
                                         pool_elts(sm->segments));
                     }));
        /* *INDENT-ON* */
    }
    if (show_segments) {
        vlib_cli_output(vm, "%-15s%15s%15s%15s%15s%15s", "Name", "Type", "HeapSize (M)", "ActiveFifos", "FreeFifos",
                        "Address");
        ...
        pool_foreach(sm, smm->segment_managers, ({
                         segment_manager_foreach_segment_w_lock(seg, sm, ({
                                                                    ...
                                                                    free_fifos =
                                                                        svm_fifo_segment_num_free_fifos(seg,
                                                                                                        ~0 /* size */);
                                                                    vlib_cli_output(vm, "%-15v%15U%15llu%15u%15u%15llx",
                                                                                    ssvm_name(&seg->ssvm),
                                                                                    format_svm_fifo_segment_type, seg,
                                                                                    size >> 20ULL, active_fifos,
                                                                                    free_fifos, address);
                                                                    if (verbose)
                                                                        vlib_cli_output(vm, "%U", format_svm_fifo_segment,
                                                                                        seg, verbose);
                                                                }));
                     }));
        /* *INDENT-ON* */
