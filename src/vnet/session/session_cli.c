...
format_stream_session_fifos(u8 *s, va_list *args)
{
    ...
    s = format(s, " Rx fifo: %U", format_svm_fifo, ss->server_rx_fifo, verbose);
    ...
    s = format(s, " Tx fifo: %U", format_svm_fifo, ss->server_tx_fifo, verbose);
...
format_stream_session(u8 *s, va_list *args)
{
    ...
    if (verbose == 1 && ss->session_state >= SESSION_STATE_ACCEPTING)
        str = format(0, "%-10u%-10u%-10lld", svm_fifo_max_dequeue(ss->server_rx_fifo),
                     svm_fifo_max_enqueue(ss->server_tx_fifo), session_get_index(ss));

    if (ss->session_state >= SESSION_STATE_ACCEPTING) {
        s = format(s, "%U", tp_vft->format_connection, ss->connection_index, ss->thread_index, verbose);
        if (verbose == 1)
            s = format(s, "%v", str);
        if (verbose > 1)
            s = format(s, "%U", format_stream_session_fifos, ss, verbose);
    } else if (ss->session_state == SESSION_STATE_LISTENING) {
        s = format(s, "%-40U%v", tp_vft->format_listener, ss->connection_index, str);
        if (verbose > 1)
            s = format(s, "\n%U", format_stream_session_fifos, ss, verbose);
    } else if (ss->session_state == SESSION_STATE_CONNECTING) {
        s = format(s, "%-40U%v", tp_vft->format_half_open, ss->connection_index, str);
    } else {
...
unformat_stream_session_id(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%U:%d->%U:%d", unformat_ip4_address, &lcl->ip4, lcl_port, unformat_ip4_address, &rmt->ip4,
                 rmt_port)) {
        *is_ip4      = 1;
        tuple_is_set = 1;
    } else if (unformat(input, "%U:%d->%U:%d", unformat_ip6_address, &lcl->ip6, lcl_port, unformat_ip6_address,
                        &rmt->ip6, rmt_port)) {
        *is_ip4      = 0;
        tuple_is_set = 1;
    }
...
unformat_stream_session(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%U", unformat_stream_session_id, &proto, &lcl, &rmt, &lcl_port, &rmt_port, &is_ip4))
        return 0;

    if (is_ip4)
        s = session_lookup_safe4(fib_index, &lcl.ip4, &rmt.ip4, clib_host_to_net_u16(lcl_port),
                                 clib_host_to_net_u16(rmt_port), proto);
    else
        s = session_lookup_safe6(fib_index, &lcl.ip6, &rmt.ip6, clib_host_to_net_u16(lcl_port),
                                 clib_host_to_net_u16(rmt_port), proto);
    if (s) {
        *result = s;
...
unformat_transport_connection(unformat_input_t *input, va_list *args)
{
    ...
    if (proto == (u8) ~0)
        return 0;
    if (is_ip4)
        tc = session_lookup_connection4(fib_index, &lcl.ip4, &rmt.ip4, clib_host_to_net_u16(lcl_port),
                                        clib_host_to_net_u16(rmt_port), proto);
    else
        tc = session_lookup_connection6(fib_index, &lcl.ip6, &rmt.ip6, clib_host_to_net_u16(lcl_port),
                                        clib_host_to_net_u16(rmt_port), proto);
...
static clib_error_t *
show_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (do_listeners) {
        ...
        pool_foreach(s, smm->sessions[0], ({
                         if (s->session_state != SESSION_STATE_LISTENING || s->session_type != sst)
                             continue;
                         app_name = application_name_from_index(s->app_wrk_index);
                         vlib_cli_output(vm, "%U%-25v%-10u", format_stream_session, s, 1, app_name, s->session_index);
                         vec_free(app_name);
                     }));
        /* *INDENT-ON* */
        return 0;
    ...
    for (i = 0; i < vec_len(smm->sessions); i++) {
        ...
        if (pool_elts(pool)) {
            ...
            vlib_cli_output(vm, "Thread %d: %d active sessions", i, pool_elts(pool));
            if (verbose) {
                if (once_per_pool && verbose == 1) {
                    str = format(str, "%-50s%-15s%-10s%-10s%-10s", "Connection", "State", "Rx-f", "Tx-f", "S-idx");
...
static clib_error_t *
clear_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    session_manager_main_t *smm = &session_manager_main;
    ...
    if (session_index != ~0) {
        ...
        if (!session)
            return clib_error_return(0, "no session %d on thread %d", session_index, thread_index);
    ...
    if (clear_all) {
        ...
        {
            pool_foreach(session, *pool, ({ clear_session(session); }));
        };
        /* *INDENT-ON* */
    }
...
static clib_error_t *
show_session_fifo_trace_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    str = is_rx ? svm_fifo_dump_trace(str, s->server_rx_fifo) : svm_fifo_dump_trace(str, s->server_tx_fifo);
...
static clib_error_t *
session_replay_fifo_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    stream_session_t *s = 0;
    u8 is_rx = 0, *str = 0;
    ...
    str = is_rx ? svm_fifo_replay(str, s->server_rx_fifo, 0, 1) : svm_fifo_replay(str, s->server_tx_fifo, 0, 1);
...
static clib_error_t *
session_enable_disable_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
