...
send_add_segment_callback(u32 api_client_index, const ssvm_private_t *sp)
{
    ...
    strncpy((char *) mp->segment_name, (char *) sp->name, sizeof(mp->segment_name) - 1);
...
send_del_segment_callback(u32 api_client_index, const ssvm_private_t *fs)
{
    ...
    strncpy((char *) mp->segment_name, (char *) fs->name, sizeof(mp->segment_name) - 1);
...
static int
send_app_cut_through_registration_add(u32 api_client_index, u32 wrk_map_index, u64 mq_addr, u64 peer_mq_addr)
{
    ...
    mp = vl_mem_api_alloc_as_if_client_w_reg(reg, sizeof(*mp));
    memset(mp, 0, sizeof(*mp));
    mp->_vl_msg_id = clib_host_to_net_u16(VL_API_APP_CUT_THROUGH_REGISTRATION_ADD);
...
send_session_accept_callback(stream_session_t *s)
{
    ...
    if (session_has_transport(s)) {
        ...
        if (application_is_proxy(server)) {
            listener = app_worker_first_listener(server_wrk, session_get_fib_proto(s), session_get_transport_proto(s));
            if (listener)
                mp->listener_handle = listen_session_get_handle(listener);
        }
...
int
send_session_connected_callback(u32 app_wrk_index, u32 api_context, stream_session_t *s, u8 is_fail)
{
    ...
    mp->retval = is_fail ? clib_host_to_net_u32(VNET_API_ERROR_SESSION_CONNECT) : 0;
    vl_msg_api_send_shmem(reg->vl_input_queue, (u8 *) &mp);
    return 0;
}
...
mq_try_lock_and_alloc_msg(svm_msg_q_t *app_mq, svm_msg_q_msg_t *msg)
{
    ...
    u8 try
        = 0;
    while (try < 100) {
        rv = svm_msg_q_lock_and_alloc_msg_w_ring(app_mq, SESSION_MQ_CTRL_EVT_RING, SVM_Q_NOWAIT, msg);
        if (!rv)
            return 0;
        try
            ++;
...
mq_send_session_accepted_cb(stream_session_t *s)
{
    ...
    if (session_has_transport(s)) {
        ...
        send_app_cut_through_registration_add(app->api_client_index, app_wrk->wrk_map_index, ls->server_evt_q,
                                              ls->client_evt_q);
...
void
mq_send_local_session_disconnected_cb(u32 app_wrk_index, local_session_t *ls)
{
...
static int
mq_send_session_connected_cb(u32 app_wrk_index, u32 api_context, stream_session_t *s, u8 is_fail)
{
    svm_msg_q_msg_t _msg, *msg = &_msg;
    ...
    app_mq  = app_wrk->event_queue;
    if (!app_mq) {
        clib_warning("app %u with api index: %u not attached", app->app_index, app->api_client_index);
        return -1;
    }
    ...
    if (session_has_transport(s)) {
        ...
        send_app_cut_through_registration_add(app->api_client_index, app_wrk->wrk_map_index, ls->client_evt_q,
                                              ls->server_evt_q);
...
vl_api_accept_session_reply_t_handler(vl_api_accept_session_reply_t *mp)
{
    ...
    if (session_handle_is_local(mp->handle)) {
        ...
        if (!ls || ls->app_wrk_index != mp->context) {
            clib_warning("server %u doesn't own local handle %llu", mp->context, mp->handle);
            return;
        }
    ...
    REPLY_MACRO2(VL_API_APP_WORKER_ADD_DEL_REPLY, ({
                     ...
                     if (!rv && mp->is_add) {
                         if (vec_len(args.segment->name)) {
                             memcpy(rmp->segment_name, args.segment->name, vec_len(args.segment->name));
...
static void
send_session_rule_details4(mma_rule_16_t *rule, u8 is_local, u8 transport_proto, u32 appns_index, u8 *tag,
                           vl_api_registration_t *reg, u32 context)
{
    ...
    rmp->scope           = is_local ? SESSION_RULE_SCOPE_LOCAL : SESSION_RULE_SCOPE_GLOBAL;
...
static void
send_session_rule_details6(mma_rule_40_t *rule, u8 is_local, u8 transport_proto, u32 appns_index, u8 *tag,
                           vl_api_registration_t *reg, u32 context)
{
    vl_api_session_rules_details_t *rmp = 0;
...
static void
send_session_rules_table_details(session_rules_table_t *srt, u8 fib_proto, u8 tp, u8 is_local, u32 appns_index,
                                 vl_api_registration_t *reg, u32 context)
{
...
vl_api_session_rules_dump_t_handler(vl_api_one_map_server_dump_t *mp)
{
    ...
    session_table_foreach(st, ({
                              for (tp = 0; tp < TRANSPORT_N_PROTO; tp++) {
                                  send_session_rules_table_details(&st->session_rules[tp], st->active_fib_proto, tp,
                                                                   st->is_local, st->appns_index, reg, mp->context);
                              }
                          }));
    /* *INDENT-ON* */
}
...
static void
vl_api_application_tls_cert_add_t_handler(vl_api_application_tls_cert_add_t *mp)
{
    vl_api_app_namespace_add_del_reply_t *rmp;
