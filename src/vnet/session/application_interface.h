 ...
 */
static inline int
app_send_io_evt_to_vpp(svm_msg_q_t *mq, svm_fifo_t *f, u8 evt_type, u8 noblock)
{
    session_event_t *evt;
    svm_msg_q_msg_t msg;
...
always_inline int
app_send_dgram_raw(svm_fifo_t *f, app_session_transport_t *at, svm_msg_q_t *vpp_evt_q, u8 *data, u32 len, u8 evt_type,
                   u8 noblock)
{
...
{
    return app_send_dgram_raw(s->tx_fifo, &s->transport, s->vpp_evt_q, data, len, FIFO_EVENT_APP_TX, noblock);
}

always_inline int
app_send_stream_raw(svm_fifo_t *f, svm_msg_q_t *vpp_evt_q, u8 *data, u32 len, u8 evt_type, u8 noblock)
{
    int rv;
...
{
    return app_send_stream_raw(s->tx_fifo, s->vpp_evt_q, data, len, FIFO_EVENT_APP_TX, noblock);
}
...
always_inline int
app_recv_dgram_raw(svm_fifo_t *f, u8 *buf, u32 len, app_session_transport_t *at, u8 clear_evt, u8 peek)
{
