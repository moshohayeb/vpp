...
typedef int(session_fifo_rx_fn)(vlib_main_t *vm, vlib_node_runtime_t *node, session_event_t *e0, stream_session_t *s0,
                                int *n_tx_pkts);
...
always_inline void
session_parse_handle(session_handle_t handle, u32 *index, u32 *thread_index)
{
...
session_get_from_handle_safe(u64 handle)
{
    ...
    if (thread_index == vlib_get_thread_index()) {
        return pool_elt_at_index(smm->sessions[thread_index], session_index_from_handle(handle));
    } else {
...
int session_enqueue_stream_connection(transport_connection_t *tc, vlib_buffer_t *b, u32 offset, u8 queue_event,
                                      u8 is_in_order);
int session_enqueue_dgram_connection(stream_session_t *s, session_dgram_hdr_t *hdr, vlib_buffer_t *b, u8 proto,
                                     u8 queue_event);
...
void session_register_transport(transport_proto_t transport_proto, const transport_proto_vft_t *vft, u8 is_ip4,
                                u32 output_node);
