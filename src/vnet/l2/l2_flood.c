...
format_l2flood_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "l2-flood: sw_if_index %d dst %U src %U bd_index %d", t->sw_if_index, format_ethernet_address, t->dst,
 ...
 */
static uword
l2flood_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (0 == n_clones) {
                 ...
                 */
                n_cloned =
                    vlib_buffer_clone(vm, bi0, msm->clones[thread_index], n_clones,
                                      (vnet_buffer(b0)->l2.l2_len + sizeof(udp_header_t) + 2 * sizeof(ip6_header_t)));
