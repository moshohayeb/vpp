...
static inline uword
l2_in_out_acl_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_output)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            next0 = vnet_l2_feature_next(b0, msm->feat_next_node_index[is_output],
                ...
                if (e0) {
                    ...
                    while (1) {
                        if (PREDICT_TRUE(t0->next_table_index != ~0))
                            ...
                            if (is_output)
                                error0 =
                                    (next0 == ACL_NEXT_INDEX_DENY) ? L2_OUTACL_ERROR_TABLE_MISS : L2_OUTACL_ERROR_NONE;
                            else
                        ...
                        if (e0) {
                            ...
                            if (is_output)
                                error0 = (next0 == ACL_NEXT_INDEX_DENY) ? L2_OUTACL_ERROR_SESSION_DENY :
                                                                          L2_OUTACL_ERROR_NONE;
                            else
                                error0 =
                                    (next0 == ACL_NEXT_INDEX_DENY) ? L2_INACL_ERROR_SESSION_DENY : L2_INACL_ERROR_NONE;
                            b0->error = node->errors[error0];
                            break;
                        }
                    }
    ...
    vlib_node_increment_counter(vm, node->node_index, is_output ? L2_OUTACL_ERROR_CHAIN_HIT : L2_INACL_ERROR_CHAIN_HIT,
                                chain_hits);
    return frame->n_vectors;
}

static uword
l2_inacl_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return l2_in_out_acl_node_fn(vm, node, frame, IN_OUT_ACL_INPUT_TABLE_GROUP);
}

static uword
l2_outacl_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
l2_in_out_acl_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2_inacl_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(),
                                mp->feat_next_node_index[IN_OUT_ACL_INPUT_TABLE_GROUP]);
    feat_bitmap_init_next_nodes(vm, l2_outacl_node.index, L2OUTPUT_N_FEAT, l2output_get_feat_names(),
