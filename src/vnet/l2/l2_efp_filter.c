...
format_l2_efp_filter_trace(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               t->sw_if_index, format_ethernet_address, t->dst, format_ethernet_address, t->src, t->raw[0], t->raw[1],
               t->raw[2], t->raw[3], t->raw[4], t->raw[5], t->raw[6], t->raw[7], t->raw[8], t->raw[9], t->raw[10],
               t->raw[11]);
    return s;
}
...
extract_keys(vnet_main_t *vnet_main, u32 sw_if_index0, vlib_buffer_t *b0, u32 *port_sw_if_index0, u16 *first_ethertype0,
...
static uword
l2_efp_filter_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 6 && n_left_to_next >= 2) {
            ...
            extract_keys(msm->vnet_main, sw_if_index0, b0, &port_sw_if_index0, &first_ethertype0, &outer_id0,
                         &inner_id0, &match_flags0);

            extract_keys(msm->vnet_main, sw_if_index1, b1, &port_sw_if_index1, &first_ethertype1, &outer_id1,
                         &inner_id1, &match_flags1);

            eth_vlan_table_lookups(&ethernet_main, msm->vnet_main, port_sw_if_index0, first_ethertype0, outer_id0,
                                   inner_id0, &hi0, &main_intf0, &vlan_intf0, &qinq_intf0);

            eth_vlan_table_lookups(&ethernet_main, msm->vnet_main, port_sw_if_index1, first_ethertype1, outer_id1,
                                   inner_id1, &hi1, &main_intf1, &vlan_intf1, &qinq_intf1);

            matched0 = eth_identify_subint(hi0, b0, match_flags0, main_intf0, vlan_intf0, qinq_intf0,
                                           &subint_sw_if_index0, &error0, &is_l20);

            matched1 = eth_identify_subint(hi1, b1, match_flags1, main_intf1, vlan_intf1, qinq_intf1,
...
l2_efp_filter_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2_efp_filter_node.index, L2OUTPUT_N_FEAT, l2output_get_feat_names(),
                                mp->l2_out_feat_next);
 ...
 */
static clib_error_t *
int_l2_efp_filter(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
