...
format_l2_output_classify_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "l2-classify: sw_if_index %d, table %d, offset %x, next %d", t->sw_if_index, t->table_index,
               t->session_offset, t->next_index);
    return s;
}
...
static uword
l2_output_classify_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from >= 4) {
        ...
        vnet_buffer(b0)->l2_classify.table_index = table_index0 =
            rt->l2cm->classify_table_index_by_sw_if_index[type_index0][sw_if_index0];
        ...
        vnet_buffer(b1)->l2_classify.table_index = table_index1 =
            rt->l2cm->classify_table_index_by_sw_if_index[type_index1][sw_if_index1];
...
l2_output_classify_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2_output_classify_node.index, L2OUTPUT_N_FEAT, l2output_get_feat_names(),
                                cm->l2_out_feat_next);
    rt->l2cm = cm;
...
int
vnet_l2_output_classify_set_tables(u32 sw_if_index, u32 ip4_table_index, u32 ip6_table_index, u32 other_table_index)
{
    l2_output_classify_main_t *cm = &l2_output_classify_main;
    vnet_classify_main_t *vcm     = cm->vnet_classify_main;
    ...
    if (ip4_table_index != ~0 && pool_is_free_index(vcm->tables, ip4_table_index))
        return VNET_API_ERROR_NO_SUCH_TABLE;

    if (ip6_table_index != ~0 && pool_is_free_index(vcm->tables, ip6_table_index))
        return VNET_API_ERROR_NO_SUCH_TABLE2;

    if (other_table_index != ~0 && pool_is_free_index(vcm->tables, other_table_index))
...
static clib_error_t *
int_l2_output_classify_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
VLIB_CLI_COMMAND(int_l2_output_classify_cli, static) = {
    ...
    .short_help = "set interface l2 output classify intfc <<interface-name>> [ip4-table <n>]\n"
