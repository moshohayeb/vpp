...
format_l2fwd_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "l2-fwd:   sw_if_index %d dst %U src %U bd_index %d", t->sw_if_index, format_ethernet_address, t->dst,
...
l2fwd_process(vlib_main_t *vm, vlib_node_runtime_t *node, l2fwd_main_t *msm, vlib_error_main_t *em, vlib_buffer_t *b0,
    ...
    if (PREDICT_FALSE(try_flood)) {
        ...
        *next0                                = L2FWD_NEXT_L2_OUTPUT;
        ...
        if (!l2fib_entry_result_is_set_AGE_NOT(result0)) {
            ...
            l2fib_seq_num_valid = expected_sn.as_u16 == result0->fields.sn.as_u16;
        }
...
l2fwd_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int do_trace)
{
    u32 n_left, *from;
...
#endif
        /* *INDENT-OFF* */
        l2fib_lookup_4(msm->mac_table, &cached_key, &cached_result, h0->dst_address, h1->dst_address, h2->dst_address,
                       h3->dst_address, vnet_buffer(b[0])->l2.bd_index, vnet_buffer(b[1])->l2.bd_index,
                       vnet_buffer(b[2])->l2.bd_index, vnet_buffer(b[3])->l2.bd_index, &key0, /* not used */
                       &key1,                                                                 /* not used */
                       &key2,                                                                 /* not used */
                       &key3,                                                                 /* not used */
                       &bucket0,                                                              /* not used */
...
#endif
        l2fib_lookup_1(msm->mac_table, &cached_key, &cached_result, h0->dst_address, vnet_buffer(b[0])->l2.bd_index,
                       &key0,    /* not used */
                       &bucket0, /* not used */
                       &result0);
        l2fwd_process(vm, node, msm, em, b[0], sw_if_index0, &result0, next);
...
void
l2fwd_register_input_type(vlib_main_t *vm, ethernet_type_t type, u32 node_index)
{
    l2fwd_main_t *mp = &l2fwd_main;
    u32 next_index;
