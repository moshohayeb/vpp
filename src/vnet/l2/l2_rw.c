...
format_l2_rw_entry(u8 *s, va_list *args)
{
    ...
    s                = format(s, "%d -  mask:%U value:%U\n", e - rw->entries, format_hex_bytes, e->mask,
               e->rewrite_n_vectors * sizeof(u32x4), format_hex_bytes, e->value, e->rewrite_n_vectors * sizeof(u32x4));
...
format_l2_rw_trace(u8 *s, va_list *args)
{
    ...
    return format(s, "l2-rw: sw_if_index %d, table %d, entry %d", t->sw_if_index, t->classify_table_index,
...
static uword
l2_rw_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    l2_rw_main_t *rw = &l2_rw_main;
...
static clib_error_t *
l2_rw_entry_cli_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u32 index = ~0;
    u8 *mask  = 0;
    u8 *value = 0;
...
static clib_error_t *
l2_rw_interface_cli_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (sw_if_index == ~0)
        return clib_error_return(0, "You must specify an interface 'iface <interface>'", format_unformat_error, input);
    int ret;
...
static clib_error_t *
l2_rw_show_interfaces_cli_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    l2_rw_main_t *rw = &l2_rw_main;
...
static clib_error_t *
l2_rw_show_entries_cli_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    l2_rw_main_t *rw = &l2_rw_main;
    l2_rw_entry_t *e;
...
static clib_error_t *
l2_rw_set_cli_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u32 bridge_domain;
    u8 disable = 0;
