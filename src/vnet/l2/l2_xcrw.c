...
static uword
l2_xcrw_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
create_xcrw_interface(vlib_main_t *vm)
{
    ...
    (void) ethernet_register_interface(vnm, xcrw_device_class.index, instance++, address, &hw_if_index,
                                       /* flag change */ 0);
...
int
vnet_configure_l2_xcrw(vlib_main_t *vm, vnet_main_t *vnm, u32 l2_sw_if_index, u32 tx_fib_index, u8 *rewrite,
                       u32 next_node_index, int is_add)
{
    l2_xcrw_main_t *xcm = &l2_xcrw_main;
...
static clib_error_t *
set_l2_xcrw_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
format_l2xcrw(u8 *s, va_list *args)
{
    ...
    s = format(s, "%-25U %U ", format_vnet_sw_if_index_name, vnm, t->l2_sw_if_index, format_vnet_sw_if_index_name, vnm,
...
static clib_error_t *
show_l2xcrw_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm    = vnet_get_main();
    l2_xcrw_main_t *xcm = &l2_xcrw_main;
    l2_xcrw_tunnel_t *t;
    ...
    /* *INDENT-OFF* */
    pool_foreach(t, xcm->tunnels, ({ vlib_cli_output(vm, "%U", format_l2xcrw, vnm, t); }));
    /* *INDENT-ON* */
