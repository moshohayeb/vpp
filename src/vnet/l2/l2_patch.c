...
static uword
l2_patch_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            vnet_buffer(b0)->sw_if_index[VLIB_TX] = l2pm->tx_sw_if_index_by_rx_sw_if_index[sw_if_index0];
            vnet_buffer(b1)->sw_if_index[VLIB_TX] = l2pm->tx_sw_if_index_by_rx_sw_if_index[sw_if_index1];

            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
                if (b0->flags & VLIB_BUFFER_IS_TRACED) {
                    l2_patch_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                    t->rx_sw_if_index   = sw_if_index0;
                    t->tx_sw_if_index   = l2pm->tx_sw_if_index_by_rx_sw_if_index[sw_if_index0];
                }
                if (b1->flags & VLIB_BUFFER_IS_TRACED) {
                    l2_patch_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
                    t->rx_sw_if_index   = sw_if_index1;
                    t->tx_sw_if_index   = l2pm->tx_sw_if_index_by_rx_sw_if_index[sw_if_index1];
                }
            }
...
vnet_l2_patch_add_del(u32 rx_sw_if_index, u32 tx_sw_if_index, int is_add)
{
    ...
    if (is_add) {
        ...
        vnet_feature_enable_disable("device-input", "l2-patch", rxhi->hw_if_index, 1, 0, 0);
    } else {
        ethernet_set_flags(l2pm->vnet_main, rxhi->hw_if_index, 0 /* disable promiscuous mode */);

        vnet_feature_enable_disable("device-input", "l2-patch", rxhi->hw_if_index, 0, 0, 0);
...
static clib_error_t *
test_patch_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    l2_patch_main_t *l2pm = &l2_patch_main;
...
static clib_error_t *
show_l2patch(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    l2_patch_main_t *l2pm = &l2_patch_main;
    u32 rx_sw_if_index;
    u32 no_entries = 1;
    ...
    for (rx_sw_if_index = 0; rx_sw_if_index < vec_len(l2pm->tx_sw_if_index_by_rx_sw_if_index); rx_sw_if_index++) {
        ...
        if (tx_sw_if_index != ~0) {
            no_entries = 0;
            vlib_cli_output(vm, "%26U -> %U", format_vnet_sw_if_index_name, l2pm->vnet_main, rx_sw_if_index,
                            format_vnet_sw_if_index_name, l2pm->vnet_main, tx_sw_if_index);
        }
    }
