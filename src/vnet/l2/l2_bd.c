...
update_flood_count(l2_bridge_domain_t *bd_config)
{
    bd_config->flood_count =
        (vec_len(bd_config->members) - (bd_config->tun_master_count ? bd_config->tun_normal_count : 0));
...
bd_set_mac_age(vlib_main_t *vm, u32 bd_index, u8 age)
{
    ...
    vec_foreach(bd_config, l2input_main.bd_configs) enable |= bd_config->bd_id != ~0 && bd_config->mac_age != 0;

    vlib_process_signal_event(vm, l2fib_mac_age_scanner_process_node.index,
                              enable ? L2_MAC_AGE_PROCESS_EVENT_START : L2_MAC_AGE_PROCESS_EVENT_STOP, 0);
}
...
bd_learn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (bd_id == 0)
        return clib_error_return(0, "No operations on the default bridge domain are supported");
 ...
 */
u32
bd_add_del_ip_mac(u32 bd_index, u8 *ip_addr, u8 *mac_addr, u8 is_ip6, u8 is_add)
{
    ...
    if (is_ip6) {
        ...
        if (is_add) {
            if (old_mac == 0) { /* new entry - allocate and create ip6 address key */
                ip6_addr_key = clib_mem_alloc(sizeof(ip6_address_t));
                clib_memcpy(ip6_addr_key, ip_addr, sizeof(ip6_address_t));
            } else if (*old_mac == new_mac) { /* same mac entry already exist for ip6 address */
                return 0;
            } else { /* update mac for ip6 address */
...
*/
static clib_error_t *
bd_arp_entry(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    bd_main_t *bdm      = &bd_main;
    clib_error_t *error = 0;
    u32 bd_index, bd_id;
    ...
    if (bd_add_del_ip_mac(bd_index, ip_addr, mac_addr, is_ip6, is_add)) {
        error = clib_error_return(0, "MAC %s for IP %U and MAC %U failed", is_add ? "add" : "del",
                                  is_ip6 ? format_ip4_address : format_ip6_address, ip_addr, format_ethernet_address,
                                  mac_addr);
    }
...
bd_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (bd_index = start; bd_index < end; bd_index++) {
        ...
        if (bd_is_valid(bd_config)) {
            if (!printed) {
                printed = 1;
                vlib_cli_output(vm, "%=8s %=7s %=4s %=9s %=9s %=9s %=11s %=9s %=9s %=11s", "BD-ID", "Index", "BSN",
                                "Age(min)", "Learning", "U-Forwrd", "UU-Flood", "Flooding", "ARP-Term", "BVI-Intf");
            }
            ...
            if (bd_config->mac_age)
                ...
                as = format(as, "off");
            vlib_cli_output(vm, "%=8d %=7d %=4d %=9v %=9s %=9s %=11U %=9s %=9s %=11U", bd_config->bd_id, bd_index,
                            bd_config->seq_num, as, bd_config->feature_bitmap & L2INPUT_FEAT_LEARN ? "on" : "off",
                            bd_config->feature_bitmap & L2INPUT_FEAT_FWD ? "on" : "off", format_uu_cfg, bd_config,
            ...
            if (detail || intf) {
                ...
                vec_foreach_index(i, bd_config->members)
                {
                    ...
                    if (i == 0) {
                        vlib_cli_output(vm, "\n%=30s%=7s%=5s%=5s%=5s%=9s%=30s", "Interface", "If-idx", "ISN", "SHG",
                                        "BVI", "TxFlood", "VLAN-Tag-Rewrite");
                    }
                    l2vtr_get(vm, vnm, member->sw_if_index, &vtr_opr, &dot1q, &tag1, &tag2);
                    vlib_cli_output(vm, "%=30U%=7d%=5d%=5d%=5s%=9s%=30U", format_vnet_sw_if_index_name, vnm,
                                    member->sw_if_index, member->sw_if_index, swif_seq_num, member->shg,
                                    member->flags & L2_FLOOD_MEMBER_BVI ? "*" : "-",
                                    i < bd_config->flood_count ? "*" : "-", format_vtr, vtr_opr, dot1q, tag1, tag2);
                }
                if (~0 != bd_config->uu_fwd_sw_if_index)
                    vlib_cli_output(vm, "%=30U%=7d%=5d%=5d%=5s%=9s%=30s", format_vnet_sw_if_index_name, vnm,
                                    bd_config->uu_fwd_sw_if_index, bd_config->uu_fwd_sw_if_index, 0, 0, "uu", "-",
                                    "None");
            }
            ...
            if ((detail || arp) && (bd_config->feature_bitmap & L2INPUT_FEAT_ARP_TERM)) {
                ...
                hash_foreach(ip4_addr, mac_addr, bd_config->mac_by_ip4, ({
                                 vlib_cli_output(vm, "%=40U => %=20U", format_ip4_address, &ip4_addr,
                                                 format_ethernet_address, &mac_addr);
                             }));

                hash_foreach_mem(ip6_addr, mac_addr, bd_config->mac_by_ip6, ({
                                     vlib_cli_output(vm, "%=40U => %=20U", format_ip6_address, ip6_addr,
                                                     format_ethernet_address, &mac_addr);
                                 }));
                /* *INDENT-ON* */
            }
...
static clib_error_t *
bd_add_del_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
