...
format_l2input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "l2-input: sw_if_index %d dst %U src %U", t->sw_if_index, format_ethernet_address, t->dst,
...
#define get_u16(addr) (*((u16 *) (addr)))
        ...
        feat_mask &= ~(L2INPUT_FEAT_FWD | L2INPUT_FEAT_UU_FLOOD | L2INPUT_FEAT_GBP_FWD);
...
l2input_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int do_trace)
{
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4) {
            ...
            if (do_trace) {
                ...
                if (b1->flags & VLIB_BUFFER_IS_TRACED) {
                    ...
                    l2input_trace_t *t    = vlib_add_trace(vm, node, b1, sizeof(*t));
                ...
                if (b2->flags & VLIB_BUFFER_IS_TRACED) {
                    ...
                    l2input_trace_t *t    = vlib_add_trace(vm, node, b2, sizeof(*t));
                ...
                if (b3->flags & VLIB_BUFFER_IS_TRACED) {
                    ...
                    l2input_trace_t *t    = vlib_add_trace(vm, node, b3, sizeof(*t));
...
static uword
l2input_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
l2input_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2input_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(),
...
void
l2input_interface_mac_change(u32 sw_if_index, const u8 *old_address, const u8 *new_address)
{
    ...
    if (intf_config->bridge && intf_config->bvi) {
        ...
        l2fib_add_entry(new_address, intf_config->bd_index, sw_if_index,
                        L2FIB_ENTRY_RESULT_FLAG_BVI | L2FIB_ENTRY_RESULT_FLAG_STATIC);
    }
}
...
u32 set_int_l2_mode(vlib_main_t *vm, vnet_main_t *vnet_main, /*           */
                    ...
                    u32 xc_sw_if_index)                      /* peer interface for xconnect       */
{
    ...
    if (config->bridge) {
        ...
        if ((config->feature_bitmap & L2INPUT_FEAT_LEARN) || (bd_config->feature_bitmap & L2INPUT_FEAT_LEARN))
    ...
    if (mode == MODE_L3) {
        ...
        if (mode == MODE_L2_BRIDGE) {
            ...
            *l2fib_valid_swif_seq_num(sw_if_index) += 1;
             ...
             */
            config->feature_bitmap |= (L2INPUT_FEAT_FWD | L2INPUT_FEAT_UU_FLOOD | L2INPUT_FEAT_UU_FWD |
 ...
 */
static clib_error_t *
int_l2_bridge(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm = vnet_get_main();
 ...
 */
static clib_error_t *
show_int_mode(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm    = vnet_get_main();
    clib_error_t *error = 0;
    char *mode;
    ...
    vec_foreach(si, sis)
    {
        ...
        if (config->bridge) {
            ...
            args = format(0, "bd_id %d%s%d", bd_id, config->bvi ? " bvi shg " : " shg ", config->shg);
        } else if (config->xconnect) {
            mode = "l2 xconnect";
