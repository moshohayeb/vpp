...
static void
send_l2_xconnect_details(vl_api_registration_t *reg, u32 context, u32 rx_sw_if_index, u32 tx_sw_if_index)
{
...
vl_api_l2_xconnect_dump_t_handler(vl_api_l2_xconnect_dump_t *mp)
{
    ...
    pool_foreach(swif, im->sw_interfaces, ({
                     ...
                     if (config->xconnect)
                         send_l2_xconnect_details(reg, mp->context, swif->sw_if_index, config->output_sw_if_index);
                 }));
    /* *INDENT-ON* */
}
...
static void
send_l2fib_table_entry(vpe_api_main_t *am, vl_api_registration_t *reg, l2fib_entry_key_t *l2fe_key,
...
static void
vl_api_bridge_domain_set_mac_age_t_handler(vl_api_bridge_domain_set_mac_age_t *mp)
{
    vlib_main_t *vm = vlib_get_main();
    bd_main_t *bdm  = &bd_main;
...
static void
send_bridge_domain_details(l2input_main_t *l2im, vl_api_registration_t *reg, l2_bridge_domain_t *bd_config,
...
vl_api_bridge_domain_dump_t_handler(vl_api_bridge_domain_dump_t *mp)
{
    ...
    for (; bd_index < end; bd_index++) {
        ...
        if (bd_config && (bd_config->bd_id > 0))
            send_bridge_domain_details(l2im, reg, bd_config, vec_len(bd_config->members), mp->context);
    }
}
...
vl_api_l2_interface_vlan_tag_rewrite_t_handler(vl_api_l2_interface_vlan_tag_rewrite_t *mp)
{
    ...
    rv = l2vtr_configure(vm, vnm, ntohl(mp->sw_if_index), vtr_op, ntohl(mp->push_dot1q), ntohl(mp->tag1),
                         ntohl(mp->tag2));
...
static void
vl_api_l2_interface_pbb_tag_rewrite_t_handler(vl_api_l2_interface_pbb_tag_rewrite_t *mp)
{
    ...
    rv = l2pbb_configure(vm, vnm, ntohl(mp->sw_if_index), vtr_op, mp->b_dmac, mp->b_smac, ntohs(mp->b_vlanid),
...
static void
vl_api_sw_interface_set_l2_xconnect_t_handler(vl_api_sw_interface_set_l2_xconnect_t *mp)
{
...
static void
vl_api_sw_interface_set_l2_bridge_t_handler(vl_api_sw_interface_set_l2_bridge_t *mp)
{
    bd_main_t *bdm = &bd_main;
...
static void
send_bd_ip_mac_entry(vpe_api_main_t *am, vl_api_registration_t *reg, u32 bd_id, u8 is_ipv6, u8 *ip_address,
                     u8 *mac_address, u32 context)
{
    ...
    for (bd_index = start; bd_index < end; bd_index++) {
        ...
        if (bd_is_valid(bd_config)) {
            ...
            hash_foreach(ip4_addr.as_u32, mac_addr, bd_config->mac_by_ip4, ({
                             send_bd_ip_mac_entry(am, reg, bd_id, 0, (u8 *) &(ip4_addr.as_u8), (u8 *) &mac_addr,
                                                  mp->context);
                         }));

            hash_foreach_mem(ip6_addr, mac_addr, bd_config->mac_by_ip6, ({
                                 send_bd_ip_mac_entry(am, reg, bd_id, 1, (u8 *) &(ip6_addr->as_u8), (u8 *) &mac_addr,
                                                      mp->context);
                             }));
            /* *INDENT-ON* */
        }
...
vl_api_bd_ip_mac_add_del_t_handler(vl_api_bd_ip_mac_add_del_t *mp)
{
    ...
    bd_index = p[0];
    if (bd_add_del_ip_mac(bd_index, mp->ip_address, mp->mac_address, mp->is_ipv6, mp->is_add))
