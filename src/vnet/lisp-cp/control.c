...
ip_interface_get_first_interface_address(ip_lookup_main_t *lm, u32 sw_if_index, u8 loop)
{
...
void *
ip_interface_get_first_address(ip_lookup_main_t *lm, u32 sw_if_index, u8 version)
{
...
int
ip_interface_get_first_ip_address(lisp_cp_main_t *lcm, u32 sw_if_index, u8 version, ip_address_t *result)
{
    ip_lookup_main_t *lm;
    void *addr;
 ...
 */
void
ip_prefix_to_fib_prefix(const ip_prefix_t *ip_prefix, fib_prefix_t *fib_prefix)
{
 ...
 */
int
ip_fib_get_first_egress_ip_for_dst(lisp_cp_main_t *lcm, ip_address_t *dst, ip_address_t *result)
{
    u32 si;
    ip_lookup_main_t *lm;
    void *addr = 0;
...
static int
dp_add_del_iface(lisp_cp_main_t *lcm, u32 vni, u8 is_l2, u8 is_add, u8 with_default_route)
{
    uword *dp_table;
 ...
 */
static u32
get_locator_pairs(lisp_cp_main_t *lcm, mapping_t *lcl_map, mapping_t *rmt_map, locator_pair_t **locator_pairs)
{
        ...
        if (rmt != 0) {
            ...
            for (i = 0; i < vec_len(lcl_ls->locator_indices); i++) {
                ...
                if (sl->sw_if_index == esi) {
                    ...
                    if (0 == ip_interface_get_first_ip_address(lcm, sl->sw_if_index,
...
static void
gid_address_sd_to_flat(gid_address_t *dst, gid_address_t *src, fid_address_t *fid)
{
 ...
 */
int
vnet_lisp_map_cache_add_del(vnet_lisp_add_del_mapping_args_t *a, u32 *map_index_result)
{
    lisp_cp_main_t *lcm = vnet_lisp_cp_get_main();
 ...
 */
int
vnet_lisp_add_del_local_mapping(vnet_lisp_add_del_mapping_args_t *a, u32 *map_index_result)
{
    uword *dp_table = 0;
    u32 vni;
    u8 type;
...
static u8
compare_locators(lisp_cp_main_t *lcm, u32 *old_ls_indexes, locator_t *new_locators)
{
    u32 i, old_li;
 ...
 */
static void
remove_overlapping_sub_prefixes(lisp_cp_main_t *lcm, gid_address_t *eid, u8 is_negative)
{
    gid_address_t *e;
    ...
    if (GID_ADDR_SRC_DST != gid_address_type(eid) || (FID_ADDR_IP_PREF != gid_address_sd_dst_type(eid)))
        return;
 ...
 */
int
vnet_lisp_add_mapping(vnet_lisp_add_del_mapping_args_t *a, locator_t *rlocs, u32 *res_map_index, u8 *is_updated)
{
...
vnet_lisp_clear_all_remote_adjacencies(void)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach_index(mi, lcm->mapping_pool, ({ vec_add1(map_indices, mi); }));
    /* *INDENT-ON* */
...
static inline int
is_locator_in_locator_set(lisp_cp_main_t *lcm, locator_set_t *ls, locator_t *loc)
{
    locator_t *itloc;
    u32 *locit;
...
static void
update_adjacencies_by_map_index(lisp_cp_main_t *lcm, u32 mapping_index, u8 remove_only)
{
    fwd_entry_t *fwd;
    mapping_t *map;
...
static void
update_fwd_entries_by_locator_set(lisp_cp_main_t *lcm, u32 ls_index, u8 remove_only)
{
    u32 i, *map_indexp;
    u32 **eid_indexes;
...
static inline void
remove_locator_from_locator_set(locator_set_t *ls, u32 *locit, u32 ls_index, u32 loc_id)
{
    lisp_cp_main_t *lcm = vnet_lisp_cp_get_main();
    u32 **ls_indexes    = NULL;
...
int
vnet_lisp_add_del_locator(vnet_lisp_add_del_locator_set_args_t *a, locator_set_t *ls, u32 *ls_result)
{
    lisp_cp_main_t *lcm = vnet_lisp_cp_get_main();
    ...
    if (a->is_add) {
        ...
        vec_foreach(itloc, a->locators)
        {
            ...
            vec_foreach(locit, ls->locator_indices)
            {
                ...
                if (0 == loc->local && !gid_address_cmp(&loc->address, &itloc->address)) {
                    removed = 1;
                    remove_locator_from_locator_set(ls, locit, ls_index, loc_id);
                }
...
int
vnet_lisp_add_del_locator_set(vnet_lisp_add_del_locator_set_args_t *a, u32 *ls_result)
{
    lisp_cp_main_t *lcm = vnet_lisp_cp_get_main();
    locator_set_t *ls;
    uword _p = (u32) ~0, *p = &_p;
            ...
            if (a->local) {
                ...
                if (!lcm->locator_set_index_by_name)
                    lcm->locator_set_index_by_name = hash_create_vec(
                        /* size */
                        0, sizeof(ls->name[0]), sizeof(uword));
...
lisp_cp_enable_l2_l3_ifaces(lisp_cp_main_t *lcm, u8 with_default_route)
{
    ...
    hash_foreach(vni, dp_table, lcm->table_id_by_vni,
                 ({ dp_add_del_iface(lcm, vni, /* is_l2 */ 0, /* is_add */ 1, with_default_route); }));
    hash_foreach(vni, dp_table, lcm->bd_id_by_vni,
                 ({ dp_add_del_iface(lcm, vni, /* is_l2 */ 1, 1, with_default_route); }));
    /* *INDENT-ON* */
}
...
lisp_cp_disable_l2_l3_ifaces(lisp_cp_main_t *lcm)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(rmts, lcm->lcl_to_rmt_adjacencies, { vec_free(rmts[0]); });
    /* *INDENT-ON* */
...
format_lisp_cp_lookup_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "LISP-CP-LOOKUP: map-resolver: %U destination eid %U", format_ip_address, &t->map_resolver_ip,
               format_gid_address, &t->dst_eid);
    return s;
}

int
get_mr_and_local_iface_ip(lisp_cp_main_t *lcm, ip_address_t *mr_ip, ip_address_t *sloc)
{
    lisp_msmr_t *mrit;
    ip_address_t *a;
...
build_map_request(lisp_cp_main_t *lcm, gid_address_t *deid, ip_address_t *sloc, ip_address_t *rloc,
...
static vlib_buffer_t *
build_encapsulated_map_request(lisp_cp_main_t *lcm, gid_address_t *seid, gid_address_t *deid, locator_set_t *loc_set,
                               ip_address_t *mr_ip, ip_address_t *sloc, u8 is_smr_invoked, u64 *nonce_res, u32 *bi_res)
{
    vlib_buffer_t *b;
    u32 bi;
    gid_address_t *rlocs = 0;
...
static void
add_locators(lisp_cp_main_t *lcm, mapping_t *m, u32 locator_set_index, ip_address_t *probed_loc)
{
    u32 *li;
    locator_t *loc, new;
    ip_interface_address_t *ia = 0;
...
static int
update_map_register_auth_data(map_register_hdr_t *map_reg_hdr, lisp_key_type_t key_id, u8 *key, u16 auth_data_len,
                              u32 msg_len)
{
    ...
    unsigned char *result =
        HMAC(get_encrypt_fcn(key_id), key, vec_len(key), (unsigned char *) map_reg_hdr, msg_len, NULL, NULL);
...
static vlib_buffer_t *
build_map_register(lisp_cp_main_t *lcm, ip_address_t *sloc, ip_address_t *ms_ip, u64 *nonce_res, u8 want_map_notif,
                   mapping_t *records, lisp_key_type_t key_id, u8 *key, u32 *bi_res)
{
    void *map_reg_hdr;
    vlib_buffer_t *b;
...
#define _(name)                                                                                                        \
    ...
    {                                                                                                                  \
        ...
        while (lcm->do_map_##name##_election |                                                                         \
               (0 == ip_fib_get_first_egress_ip_for_dst(lcm, &lcm->active_map_##name, ip))) {                          \
...
static uword
lisp_cp_output(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return 0;
}
...
static int
send_rloc_probe(lisp_cp_main_t *lcm, gid_address_t *deid, u32 local_locator_set_index, ip_address_t *sloc,
                ip_address_t *rloc)
{
    locator_set_t *ls;
    u32 bi;
    ...
    next_index = (ip_addr_version(rloc) == IP4) ? ip4_lookup_node.index : ip6_lookup_node.index;
...
send_rloc_probes(lisp_cp_main_t *lcm)
{
    ...
    vlib_node_increment_counter(vlib_get_main(), lisp_cp_output_node.index, LISP_CP_OUTPUT_ERROR_RLOC_PROBES_SENT,
                                rloc_probes_sent);
    return 0;
}
...
send_map_register(lisp_cp_main_t *lcm, u8 want_map_notif)
{
    ...
    vec_foreach(r, records)
    {
        ...
        next_index = (ip_addr_version(&lcm->active_map_server) == IP4) ? ip4_lookup_node.index : ip6_lookup_node.index;
    ...
    vlib_node_increment_counter(vlib_get_main(), lisp_cp_output_node.index, LISP_CP_OUTPUT_ERROR_MAP_REGISTERS_SENT,
                                map_registers_sent);
...
static int
_send_encapsulated_map_request(lisp_cp_main_t *lcm, gid_address_t *seid, gid_address_t *deid, u8 is_smr_invoked,
                               u8 is_resend)
{
    ...
    b = build_encapsulated_map_request(lcm, seid, deid, loc_set, &lcm->active_map_resolver, &sloc, is_smr_invoked,
                                       &nonce, &bi);
    ...
    vnet_buffer(b)->sw_if_index[VLIB_TX] = 0;
    next_index = (ip_addr_version(&lcm->active_map_resolver) == IP4) ? ip4_lookup_node.index : ip6_lookup_node.index;
...
static u32
lisp_get_vni_from_buffer_ip(lisp_cp_main_t *lcm, vlib_buffer_t *b, u8 version)
{
    uword *vnip;
    u32 vni = ~0, table_id = ~0;

    table_id = fib_table_get_table_id_for_sw_if_index((version == IP4 ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6),
...
void
get_src_and_dst_eids_from_buffer(lisp_cp_main_t *lcm, vlib_buffer_t *b, gid_address_t *src, gid_address_t *dst, u16 type)
{
    ...
    if (LISP_AFI_IP == type || LISP_AFI_IP6 == type) {
        ...
        if (clib_net_to_host_u16(eh->type) == ETHERNET_TYPE_ARP) {
            ...
            if (clib_net_to_host_u16(ah->opcode) != ETHERNET_ARP_OPCODE_request) {
                memset(&gid_address_arp_ndp_ip(dst), 0, sizeof(ip_address_t));
                ...
                if (IP_PROTOCOL_ICMP6 == ip->protocol) {
                    ...
                    if (ndh->icmp.type == ICMP6_neighbor_solicitation) {
                        ...
                        if ((opt->header.type != ICMP6_NEIGHBOR_DISCOVERY_OPTION_source_link_layer_address) ||
                            ...
                            memset(&gid_address_arp_ndp_ip(dst), 0, sizeof(ip_address_t));
                            ip_addr_version(&gid_address_arp_ndp_ip(dst)) = IP6;
                            gid_address_arp_ndp_bd(dst)                   = ~0;
                            gid_address_type(src)                         = GID_ADDR_NO_ADDRESS;
                            return;
                        }
                        ...
                        ip_address_set(&gid_address_arp_ndp_ip(dst), &ndh->target_address, IP6);
                        return;
                    }
                }
            }
...
static uword
lisp_cp_lookup_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int overlay)
{
    icmp6_neighbor_discovery_ethernet_link_layer_address_option_t *opt;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (gid_address_type(&dst) == GID_ADDR_ARP) {
                ...
                clib_memcpy(arp0->ip4_over_ethernet[0].ethernet, (u8 *) &mac0, 6);
...
static uword
lisp_cp_lookup_ip4(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (lisp_cp_lookup_inline(vm, node, from_frame, LISP_AFI_IP));
}

static uword
lisp_cp_lookup_ip6(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (lisp_cp_lookup_inline(vm, node, from_frame, LISP_AFI_IP6));
}

static uword
lisp_cp_lookup_l2(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (lisp_cp_lookup_inline(vm, node, from_frame, LISP_AFI_MAC));
}

static uword
lisp_cp_lookup_nsh(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static void
mapping_start_expiration_timer(lisp_cp_main_t *lcm, u32 mi, f64 expiration_time)
{
    mapping_t *m;
...
process_map_reply(map_records_arg_t *a)
{
    ...
    vec_foreach(m, a->mappings)
    {
        ...
        if ((u32) ~0 != m->ttl)
            mapping_start_expiration_timer(lcm, dst_map_index, (m->ttl == 0) ? 0 : MAPPING_TIMEOUT);
    }
...
static int
is_auth_data_valid(map_notify_hdr_t *h, u32 msg_len, lisp_key_type_t key_id, u8 *key)
{
    u8 *auth_data = 0;
    u16 auth_data_len;
    int result;
    ...
    unsigned char *code = HMAC(get_encrypt_fcn(key_id), key, vec_len(key), (unsigned char *) h, msg_len, NULL, NULL);
 ...
 */
static int
map_record_integrity_check(lisp_cp_main_t *lcm, mapping_t *maps, u32 key_id, u8 **key_out)
{
...
static vlib_buffer_t *
build_map_reply(lisp_cp_main_t *lcm, ip_address_t *sloc, ip_address_t *dst, u64 nonce, u8 probe_bit, mapping_t *records,
                u16 dst_port, u32 *bi_res)
{
    vlib_buffer_t *b;
    u32 bi;
    vlib_main_t *vm = lcm->vlib_main;
...
static int
send_map_reply(lisp_cp_main_t *lcm, u32 mi, ip_address_t *dst, u8 probe_bit, u64 nonce, u16 dst_port,
...
void
process_map_request(vlib_main_t *vm, vlib_node_runtime_t *node, lisp_cp_main_t *lcm, vlib_buffer_t *b)
{
    u8 *ip_hdr            = 0;
...
static uword
lisp_cp_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static int
lisp_stats_api_fill(lisp_cp_main_t *lcm, lisp_gpe_main_t *lgm, lisp_api_stats_t *stat, lisp_stats_key_t *key,
                    u32 stats_index)
{
    vlib_counter_t v;
...
static int
queue_map_request(gid_address_t *seid, gid_address_t *deid, u8 smr_invoked, u8 is_resend)
{
...
remove_dead_pending_map_requests(lisp_cp_main_t *lcm)
{
    ...
    vec_foreach(pmr_index, to_be_removed) pool_put_index(lcm->pending_map_requests_pool, pmr_index[0]);
...
update_map_register(lisp_cp_main_t *lcm, f64 dt)
{
    ...
    if (del_all) {
        ...
        vec_foreach(pmr_index, to_be_removed) pool_put_index(lcm->pending_map_registers_pool, pmr_index[0]);
    }
...
static uword
send_map_resolver_service(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    u32 *expired = 0;
    f64 period   = 2.0;
    pending_map_request_t *pmr;
...
vnet_lisp_set_transport_protocol(u8 protocol)
{
    ...
    if (protocol < LISP_TRANSPORT_PROTOCOL_UDP || protocol > LISP_TRANSPORT_PROTOCOL_API)
        return VNET_API_ERROR_INVALID_ARGUMENT;
