...
static void
gid_dict_foreach_ip4_subprefix(gid_dictionary_t *db, u32 vni, ip_prefix_t *src, ip_prefix_t *dst,
...
static void
gid_dict_foreach_ip6_subprefix(gid_dictionary_t *db, u32 vni, ip_prefix_t *src, ip_prefix_t *dst,
                               foreach_subprefix_match_cb_t cb, void *arg)
{
    u32 sfi;
...
void
gid_dict_foreach_subprefix(gid_dictionary_t *db, gid_address_t *eid, foreach_subprefix_match_cb_t cb, void *arg)
{
    ip_prefix_t *ippref = &gid_address_sd_dst_ippref(eid);

    if (IP4 == ip_prefix_version(ippref))
        gid_dict_foreach_ip4_subprefix(db, gid_address_vni(eid), &gid_address_sd_src_ippref(eid),
                                       &gid_address_sd_dst_ippref(eid), cb, arg);
    else
        gid_dict_foreach_ip6_subprefix(db, gid_address_vni(eid), &gid_address_sd_src_ippref(eid),
                                       &gid_address_sd_dst_ippref(eid), cb, arg);
}

void
gid_dict_foreach_l2_arp_ndp_entry(gid_dictionary_t *db, void (*cb)(BVT(clib_bihash_kv) * kvp, void *arg), void *ht)
{
...
static void
make_mac_sd_key(BVT(clib_bihash_kv) * kv, u32 vni, u8 src_mac[6], u8 dst_mac[6])
{
    kv->key[0] = (u64) vni;
...
static u32
ip_sd_lookup(gid_dictionary_t *db, u32 vni, ip_prefix_t *dst, ip_prefix_t *src)
{
    u32 sfi;
    gid_ip4_table_t *sfib4;
    gid_ip6_table_t *sfib6;
...
gid_dictionary_lookup(gid_dictionary_t *db, gid_address_t *key)
{
    ...
    case GID_ADDR_SRC_DST:
        ...
        case FID_ADDR_IP_PREF:
            return ip_sd_lookup(db, gid_address_vni(key), &gid_address_sd_dst_ippref(key),
                                &gid_address_sd_src_ippref(key));
            break;
        case FID_ADDR_MAC:
            return mac_sd_lookup(&db->sd_mac_table, gid_address_vni(key), gid_address_sd_dst_mac(key),
...
u32
gid_dictionary_sd_lookup(gid_dictionary_t *db, gid_address_t *dst, gid_address_t *src)
{
    ...
    case GID_ADDR_SRC_DST:
        ...
        case FID_ADDR_IP_PREF:
            return ip_sd_lookup(db, gid_address_vni(dst), &gid_address_sd_dst_ippref(dst),
                                &gid_address_sd_src_ippref(dst));
            break;
        case FID_ADDR_MAC:
            return mac_sd_lookup(&db->sd_mac_table, gid_address_vni(dst), gid_address_sd_dst_mac(dst),
...
static u32
add_del_ip4_key(gid_ip4_table_t *db, u32 vni, ip_prefix_t *pref, u32 val, u8 is_add)
{
    BVT(clib_bihash_kv) kv, value;
    u32 old_val = ~0;
    ip4_address_t key;
    ...
    if (is_add) {
        db->ip4_non_empty_dst_address_length_bitmap =
            clib_bitmap_set(db->ip4_non_empty_dst_address_length_bitmap, 32 - plen, 1);
        ...
        if (db->ip4_prefix_len_refcount[plen] == 0) {
            db->ip4_non_empty_dst_address_length_bitmap =
                clib_bitmap_set(db->ip4_non_empty_dst_address_length_bitmap, 32 - plen, 0);
            ip4_compute_prefix_lengths_in_search_order(db);
        }
    }
...
ip4_lookup_init(gid_ip4_table_t *db)
{
    ...
    BV(clib_bihash_init)
    (&db->ip4_lookup_table, "ip4 lookup table", db->ip4_lookup_table_nbuckets, db->ip4_lookup_table_size);
}

static u32
add_del_sd_ip4_key(gid_dictionary_t *db, u32 vni, ip_prefix_t *dst_pref, ip_prefix_t *src_pref, u32 val, u8 is_add)
{
    u32 sfi, old_val = ~0;
...
static u32
add_del_ip6_key(gid_ip6_table_t *db, u32 vni, ip_prefix_t *pref, u32 val, u8 is_add)
{
    BVT(clib_bihash_kv) kv, value;
    u32 old_val = ~0;
    ip6_address_t key;
    ...
    if (is_add) {
        db->ip6_non_empty_dst_address_length_bitmap =
            clib_bitmap_set(db->ip6_non_empty_dst_address_length_bitmap, 128 - plen, 1);
        ...
        if (db->ip6_prefix_len_refcount[plen] == 0) {
            db->ip6_non_empty_dst_address_length_bitmap =
                clib_bitmap_set(db->ip6_non_empty_dst_address_length_bitmap, 128 - plen, 0);
            ip6_compute_prefix_lengths_in_search_order(db);
        }
    }
...
static u32
add_del_mac(gid_mac_table_t *db, u32 vni, u8 *dst_mac, u8 *src_mac, u32 val, u8 is_add)
{
    BVT(clib_bihash_kv) kv, value;
    u32 old_val = ~0;
...
ip6_lookup_init(gid_ip6_table_t *db)
{
    ...
    for (i = 0; i < ARRAY_LEN(db->ip6_fib_masks); i++) {
        ...
        if (i1)
            db->ip6_fib_masks[i].as_u32[i0] = clib_host_to_net_u32(pow2_mask(i1) << (32 - i1));
    }
    ...
    BV(clib_bihash_init)
    (&db->ip6_lookup_table, "ip6 lookup table", db->ip6_lookup_table_nbuckets, db->ip6_lookup_table_size);
}

static u32
add_del_sd_ip6_key(gid_dictionary_t *db, u32 vni, ip_prefix_t *dst_pref, ip_prefix_t *src_pref, u32 val, u8 is_add)
{
    u32 sfi, old_val = ~0;
...
static u32
add_del_ip(gid_dictionary_t *db, u32 vni, ip_prefix_t *dst_key, ip_prefix_t *src_key, u32 value, u8 is_add)
{
...
static u32
add_del_sd(gid_dictionary_t *db, u32 vni, source_dest_t *key, u32 value, u8 is_add)
{
...
static u64
add_del_arp_ndp(gid_l2_arp_ndp_table_t *db, u32 bd, ip_address_t *key, u64 value, u8 is_add)
{
...
static u32
add_del_nsh(gid_nsh_table_t *db, u32 vni, u32 spi, u8 si, u32 value, u8 is_add)
{
    BVT(clib_bihash_kv) kv, result;
    u32 old_val = ~0;
...
u32
gid_dictionary_add_del(gid_dictionary_t *db, gid_address_t *key, u64 value, u8 is_add)
{
    switch (gid_address_type(key)) {
    case GID_ADDR_IP_PREFIX:
    ...
    case GID_ADDR_ARP:
    case GID_ADDR_NDP:
        return add_del_arp_ndp(&db->arp_ndp_table, gid_address_arp_ndp_bd(key), &gid_address_arp_ndp_ip(key), value,
                               is_add);
    case GID_ADDR_NSH:
        return add_del_nsh(&db->nsh_table, gid_address_vni(key), gid_address_nsh_spi(key), gid_address_nsh_si(key),
                           value, is_add);
...
mac_lookup_init(gid_mac_table_t *db)
{
    ...
    BV(clib_bihash_init)
    (&db->mac_lookup_table, "mac lookup table", db->mac_lookup_table_nbuckets, db->mac_lookup_table_size);
}
...
arp_ndp_lookup_init(gid_l2_arp_ndp_table_t *db)
{
    if (db->arp_ndp_lookup_table_nbuckets == 0)
        db->arp_ndp_lookup_table_nbuckets = ARP_NDP_LOOKUP_DEFAULT_HASH_NUM_BUCKETS;
    ...
    BV(clib_bihash_init)
    (&db->arp_ndp_lookup_table, "arp ndp lookup table", db->arp_ndp_lookup_table_nbuckets, db->arp_ndp_lookup_table_size);
}
...
nsh_lookup_init(gid_nsh_table_t *db)
{
    ...
    BV(clib_bihash_init)
    (&db->nsh_lookup_table, "nsh lookup table", db->nsh_lookup_table_nbuckets, db->nsh_lookup_table_size);
}
