...
size_to_write_fct size_to_write_fcts[GID_ADDR_TYPES] = {ip_prefix_size_to_write, lcaf_size_to_write, mac_size_to_write,
                                                        sd_size_to_write,        nsh_size_to_write,  0 /* arp */,
                                                        no_addr_size_to_write};

serdes_fct write_fcts[GID_ADDR_TYPES] = {ip_prefix_write, lcaf_write,  mac_write,    sd_write,
                                         nsh_write,       0 /* arp */, no_addr_write};

cast_fct cast_fcts[GID_ADDR_TYPES] = {ip_prefix_cast, lcaf_cast,   mac_cast,    sd_cast,
                                      nsh_cast,       0 /* arp */, no_addr_cast};

addr_len_fct addr_len_fcts[GID_ADDR_TYPES] = {ip_prefix_length, lcaf_length, mac_length,    sd_length,
                                              nsh_length,       0 /* arp */, no_addr_length};

copy_fct copy_fcts[GID_ADDR_TYPES] = {ip_prefix_copy, lcaf_copy,   mac_copy,    sd_copy,
                                      nsh_copy,       0 /* arp */, no_addr_copy};
...
{
    ip_prefix_t *a = va_arg(*args, ip_prefix_t *);
    if (unformat(input, "%U/%d", unformat_ip_address, &ip_prefix_addr(a), &ip_prefix_len(a))) {
...
format_gid_address(u8 *s, va_list *args)
{
    ...
    case GID_ADDR_SRC_DST:
        return format(s, "[%d] %U|%U", gid_address_vni(a), format_fid_address, &gid_address_sd_src(a),
...
void
ip_address_to_46(const ip_address_t *addr, ip46_address_t *a, fib_protocol_t *proto)
{
    *proto = (IP4 == ip_addr_version(addr) ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6);
    switch (*proto) {
...
{
    source_dest_t *sd = a;
    return sizeof(u16) + sizeof(lcaf_hdr_t) + sizeof(lcaf_src_dst_hdr_t) + fid_addr_size_to_write(&sd_src(sd)) +
