...
vl_api_bfd_udp_add_t_handler(vl_api_bfd_udp_add_t *mp)
{
    ...
    rv = bfd_udp_add_session(BFD_UDP_API_PARAM_FROM_MP(mp), clib_net_to_host_u32(mp->desired_min_tx),
                             clib_net_to_host_u32(mp->required_min_rx), mp->detect_mult, mp->is_authenticated,
                             clib_net_to_host_u32(mp->conf_key_id), mp->bfd_key_id);
...
vl_api_bfd_udp_mod_t_handler(vl_api_bfd_udp_mod_t *mp)
{
    ...
    rv = bfd_udp_mod_session(BFD_UDP_API_PARAM_FROM_MP(mp), clib_net_to_host_u32(mp->desired_min_tx),
...
void
send_bfd_udp_session_details(vl_api_registration_t *reg, u32 context, bfd_session_t *bs)
{
    ...
    if (mp->is_ipv6) {
        clib_memcpy(mp->local_addr, &key->local_addr, sizeof(key->local_addr));
        clib_memcpy(mp->peer_addr, &key->peer_addr, sizeof(key->peer_addr));
    } else {
        clib_memcpy(mp->local_addr, key->local_addr.ip4.data, sizeof(key->local_addr.ip4.data));
        clib_memcpy(mp->peer_addr, key->peer_addr.ip4.data, sizeof(key->peer_addr.ip4.data));
    }
...
static void
vl_api_bfd_udp_session_set_flags_t_handler(vl_api_bfd_udp_session_set_flags_t *mp)
{
    ...
    rv = bfd_udp_session_set_flags(BFD_UDP_API_PARAM_FROM_MP(mp), mp->admin_up_down);
...
vl_api_bfd_udp_auth_activate_t_handler(vl_api_bfd_udp_auth_activate_t *mp)
{
    ...
    rv = bfd_udp_auth_activate(BFD_UDP_API_PARAM_FROM_MP(mp), clib_net_to_host_u32(mp->conf_key_id), mp->bfd_key_id,
                               mp->is_delayed);
...
static void
vl_api_bfd_udp_auth_deactivate_t_handler(vl_api_bfd_udp_auth_deactivate_t *mp)
{
...
static void
vl_api_bfd_udp_set_echo_source_t_handler(vl_api_bfd_udp_set_echo_source_t *mp)
{
...
static void
vl_api_bfd_udp_del_echo_source_t_handler(vl_api_bfd_udp_del_echo_source_t *mp)
{
...
static void
vl_api_bfd_udp_get_echo_source_t_handler(vl_api_bfd_udp_get_echo_source_t *mp)
{
