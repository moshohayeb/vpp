...
static void
bfd_set_state(bfd_main_t *bm, bfd_session_t *bs, bfd_state_e new_state, int handling_wakeup)
{
...
{
    bs->transmit_interval_clocks = clib_max(bs->effective_desired_min_tx_clocks, bs->remote_min_rx_clocks);
    BFD_DBG("Recalculated transmit interval " BFD_CLK_FMT, BFD_CLK_PRN(bs->transmit_interval_clocks));
}

static void
bfd_recalc_echo_tx_interval(bfd_main_t *bm, bfd_session_t *bs)
{
    bs->echo_transmit_interval_clocks = clib_max(bs->effective_desired_min_tx_clocks, bs->remote_min_echo_rx_clocks);
    BFD_DBG("Recalculated echo transmit interval " BFD_CLK_FMT, BFD_CLK_PRN(bs->echo_transmit_interval_clocks));
}
...
bfd_calc_next_tx(bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
    if (bs->local_detect_mult > 1) {
        ...
        bs->tx_timeout_clocks =
            bs->last_tx_clocks + (1 - .25 * (random_f64(&bm->random_seed))) * bs->transmit_interval_clocks;
        if (bs->tx_timeout_clocks < now) {
            ...
            BFD_DBG("Missed %lu transmit events (now is %lu, calc "
                    "tx_timeout is %lu)",
                    (now - bs->tx_timeout_clocks) / bs->transmit_interval_clocks, now, bs->tx_timeout_clocks);
            bs->tx_timeout_clocks = now;
        }
    } else {
        /* special case - 75-90% of transmit interval */
        bs->tx_timeout_clocks =
            bs->last_tx_clocks + (.9 - .15 * (random_f64(&bm->random_seed))) * bs->transmit_interval_clocks;
        if (bs->tx_timeout_clocks < now) {
            /*
             * the timeout is in the past, which means that either remote
             * demand mode was set or performance/clock issues ...
    ...
    if (bs->tx_timeout_clocks) {
        BFD_DBG("Next transmit in %lu clocks/%.02fs@%lu", bs->tx_timeout_clocks - now,
                (bs->tx_timeout_clocks - now) / bm->cpu_cps, bs->tx_timeout_clocks);
    }
}
...
{
    bs->echo_tx_timeout_clocks = bs->echo_last_tx_clocks + bs->echo_transmit_interval_clocks;
    if (bs->echo_tx_timeout_clocks < now) {
        ...
        BFD_DBG("Missed %lu echo transmit events (now is %lu, calc tx_timeout "
                "is %lu)",
                (now - bs->echo_tx_timeout_clocks) / bs->echo_transmit_interval_clocks, now, bs->echo_tx_timeout_clocks);
        bs->echo_tx_timeout_clocks = now;
    }
    BFD_DBG("Next echo transmit in %lu clocks/%.02fs@%lu", bs->echo_tx_timeout_clocks - now,
            (bs->echo_tx_timeout_clocks - now) / bm->cpu_cps, bs->echo_tx_timeout_clocks);
}
...
bfd_recalc_detection_time(bfd_main_t *bm, bfd_session_t *bs)
{
    if (bs->local_state == BFD_STATE_init || bs->local_state == BFD_STATE_up) {
        bs->detection_time_clocks =
            bs->remote_detect_mult * clib_max(bs->effective_required_min_rx_clocks, bs->remote_desired_min_tx_clocks);
        BFD_DBG("Recalculated detection time %lu clocks/%.2fs", bs->detection_time_clocks,
...
bfd_set_timer(bfd_main_t *bm, bfd_session_t *bs, u64 now, int handling_wakeup)
{
    ...
    BFD_DBG("bs_idx=%u, tx_timeout=%lu, echo_tx_timeout=%lu, rx_timeout=%lu, "
            ...
            next == tx_timeout ? "tx" : (next == bs->echo_tx_timeout_clocks ? "echo tx" : "rx"));
    /* sometimes the wheel expires an event a bit sooner than requested, account
       for that here */
    if (next &&
        (now + bm->wheel_inaccuracy > bs->wheel_time_clocks || next < bs->wheel_time_clocks || !bs->wheel_time_clocks)) {
        int send_signal       = 0;
        bs->wheel_time_clocks = next;
        BFD_DBG("timing_wheel_insert(%p, %lu (%ld clocks/%.2fs in the "
                "future), %u);",
                &bm->wheel, bs->wheel_time_clocks, (i64) bs->wheel_time_clocks - clib_cpu_time_now(),
                (i64)(bs->wheel_time_clocks - clib_cpu_time_now()) / bm->cpu_cps, bs->bs_idx);
        bfd_lock(bm);
        ...
        if (!handling_wakeup) {
            ...
            send_signal = (bs->wheel_time_clocks < bm->bfd_process_next_wakeup_clocks) &&
                          /*
...
static void
bfd_set_effective_desired_min_tx(bfd_main_t *bm, bfd_session_t *bs, u64 now, u64 desired_min_tx_clocks)
{
    bs->effective_desired_min_tx_clocks = desired_min_tx_clocks;
    BFD_DBG("Set effective desired min tx to " BFD_CLK_FMT, BFD_CLK_PRN(bs->effective_desired_min_tx_clocks));
...
static void
bfd_set_effective_required_min_rx(bfd_main_t *bm, bfd_session_t *bs, u64 required_min_rx_clocks)
{
    bs->effective_required_min_rx_clocks = required_min_rx_clocks;
    BFD_DBG("Set effective required min rx to " BFD_CLK_FMT, BFD_CLK_PRN(bs->effective_required_min_rx_clocks));
    bfd_recalc_detection_time(bm, bs);
}

static void
bfd_set_remote_required_min_rx(bfd_main_t *bm, bfd_session_t *bs, u64 now, u32 remote_required_min_rx_usec)
{
    if (bs->remote_min_rx_usec != remote_required_min_rx_usec) {
        bs->remote_min_rx_usec   = remote_required_min_rx_usec;
        bs->remote_min_rx_clocks = bfd_usec_to_clocks(bm, remote_required_min_rx_usec);
        BFD_DBG("Set remote min rx to " BFD_CLK_FMT, BFD_CLK_PRN(bs->remote_min_rx_clocks));
        bfd_recalc_detection_time(bm, bs);
        bfd_recalc_tx_interval(bm, bs);
    }
}
...
static void
bfd_set_remote_required_min_echo_rx(bfd_main_t *bm, bfd_session_t *bs, u64 now, u32 remote_required_min_echo_rx_usec)
{
    if (bs->remote_min_echo_rx_usec != remote_required_min_echo_rx_usec) {
        ...
        bs->remote_min_echo_rx_clocks = bfd_usec_to_clocks(bm, bs->remote_min_echo_rx_usec);
        BFD_DBG("Set remote min echo rx to " BFD_CLK_FMT, BFD_CLK_PRN(bs->remote_min_echo_rx_clocks));
        bfd_recalc_echo_tx_interval(bm, bs);
    }
}

static void
bfd_notify_listeners(bfd_main_t *bm, bfd_listen_event_e event, const bfd_session_t *bs)
{
    bfd_notify_fn_t *fn;
...
bfd_session_start(bfd_main_t *bm, bfd_session_t *bs)
{
    ...
    bfd_set_effective_required_min_rx(bm, bs, bs->config_required_min_rx_clocks);
    bfd_recalc_tx_interval(bm, bs);
...
bfd_input_format_trace(u8 *s, va_list *args)
{
    ...
    if (t->len > STRUCT_SIZE_OF(bfd_pkt_t, head)) {
        s = format(s,
                   ...
                   bfd_pkt_get_version(pkt), bfd_pkt_get_diag_code(pkt), bfd_diag_code_string(bfd_pkt_get_diag_code(pkt)),
                   bfd_pkt_get_state(pkt), bfd_state_string(bfd_pkt_get_state(pkt)), bfd_pkt_get_poll(pkt),
                   bfd_pkt_get_final(pkt), bfd_pkt_get_control_plane_independent(pkt), bfd_pkt_get_auth_present(pkt),
                   bfd_pkt_get_demand(pkt), bfd_pkt_get_multipoint(pkt), pkt->head.detect_mult, pkt->head.length);
        if (t->len >= sizeof(bfd_pkt_t) && pkt->head.length >= sizeof(bfd_pkt_t)) {
        ...
        if (t->len >= sizeof(bfd_pkt_with_common_auth_t) && pkt->head.length >= sizeof(bfd_pkt_with_common_auth_t) &&
            ...
            s                                           = format(s, "\n    auth len: %u\n", common->len);
            s = format(s, "    auth type: %u:%s\n", common->type, bfd_auth_type_str(common->type));
            if (t->len >= sizeof(bfd_pkt_with_sha1_auth_t) && pkt->head.length >= sizeof(bfd_pkt_with_sha1_auth_t) &&
...
bfd_on_state_change(bfd_main_t *bm, bfd_session_t *bs, u64 now, int handling_wakeup)
{
    ...
    case BFD_STATE_admin_down:
        ...
        bfd_set_effective_desired_min_tx(bm, bs, now,
                                         clib_max(bs->config_desired_min_tx_clocks, bm->default_desired_min_tx_clocks));
        bfd_set_effective_required_min_rx(bm, bs, bs->config_required_min_rx_clocks);
    ...
    case BFD_STATE_init:
        bs->echo = 0;
        bfd_set_effective_desired_min_tx(bm, bs, now, bs->config_desired_min_tx_clocks);
        bfd_set_timer(bm, bs, now, handling_wakeup);
        break;
    case BFD_STATE_up:
        bfd_set_effective_desired_min_tx(bm, bs, now, bs->config_desired_min_tx_clocks);
...
static void
bfd_on_config_change(vlib_main_t *vm, vlib_node_runtime_t *rt, bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
    /*
     * if remote demand mode is set and we need to do a poll, set the next
     * timeout so that the session wakes up immediately
     */
    if (bs->remote_demand && BFD_POLL_NEEDED == bs->poll_state && bs->poll_state_start_or_timeout_clocks < now) {
        bs->tx_timeout_clocks = now;
    }
    bfd_recalc_detection_time(bm, bs);
    bfd_set_timer(bm, bs, now, 0);
...
bfd_init_control_frame(bfd_main_t *bm, bfd_session_t *bs, vlib_buffer_t *b)
{
    ...
    if (bs->echo) {
        pkt->req_min_rx = clib_host_to_net_u32(bfd_clocks_to_usec(bm, bs->effective_required_min_rx_clocks));
    } else {
...
bfd_send_echo(vlib_main_t *vm, vlib_node_runtime_t *rt, bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
    ...
    if (now + bm->wheel_inaccuracy >= bs->echo_tx_timeout_clocks) {
        ...
        pkt->expire_time_clocks = now + bs->echo_transmit_interval_clocks * bs->local_detect_mult;
        pkt->checksum           = bfd_calc_echo_checksum(bs->local_discr, pkt->expire_time_clocks, bs->echo_secret);
        ...
        BFD_DBG("No need to send echo packet now, now is %lu, tx_timeout is %lu", now, bs->echo_tx_timeout_clocks);
    }
}

static void
bfd_send_periodic(vlib_main_t *vm, vlib_node_runtime_t *rt, bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
    ...
    if (BFD_POLL_NOT_NEEDED == bs->poll_state && bs->remote_demand && BFD_STATE_up == bs->local_state &&
    ...
    if (now + bm->wheel_inaccuracy >= bs->tx_timeout_clocks) {
        ...
        case BFD_POLL_NEEDED:
            if (now < bs->poll_state_start_or_timeout_clocks) {
                BFD_DBG("Cannot start a poll sequence yet, need to wait "
                        "for " BFD_CLK_FMT,
                        BFD_CLK_PRN(bs->poll_state_start_or_timeout_clocks - now));
                break;
            }
        ...
        BFD_DBG("No need to send control frame now, now is %lu, tx_timeout is %lu", now, bs->tx_timeout_clocks);
    }
}

void
bfd_init_final_control_frame(vlib_main_t *vm, vlib_buffer_t *b, bfd_main_t *bm, bfd_session_t *bs, int is_local)
{
...
static void
bfd_check_rx_timeout(bfd_main_t *bm, bfd_session_t *bs, u64 now, int handling_wakeup)
{
    /*
     * sometimes the wheel expires an event a bit sooner than requested, account
     * for that here
     */
    if (bs->last_rx_clocks + bs->detection_time_clocks <= now + bm->wheel_inaccuracy) {
    ...
    } else if (bs->echo && bs->echo_last_rx_clocks + bs->echo_transmit_interval_clocks * bs->local_detect_mult <=
...
void
bfd_on_timeout(vlib_main_t *vm, vlib_node_runtime_t *rt, bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
    ...
    case BFD_STATE_up:
        ...
        if (BFD_POLL_NOT_NEEDED == bs->poll_state && !bs->echo && bfd_is_echo_possible(bs)) {
            ...
            bfd_set_effective_required_min_rx(bm, bs,
                                              clib_max(bm->min_required_min_rx_while_echo_clocks,
...
bfd_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        BFD_DBG("timing_wheel_next_expiring_elt_time(%p) returns %lu", &bm->wheel, next_expire);
        bm->bfd_process_next_wakeup_clocks = (i64) next_expire >= 0 ? next_expire : ~0;
        bfd_unlock(bm);
        ...
        case BFD_EVENT_RESCHEDULE:
            bfd_lock(bm);
            bm->bfd_process_wakeup_event_delay_clocks = now - bm->bfd_process_wakeup_event_start_clocks;
            bm->bfd_process_wakeup_events_in_flight--;
            bfd_unlock(bm);
        ...
        BFD_DBG("timing_wheel_advance (%p, %lu, %p, 0);", &bm->wheel, now, expired);
        bfd_lock(bm);
...
bfd_main_init(vlib_main_t *vm)
{
    ...
    bm->default_desired_min_tx_clocks         = bfd_usec_to_clocks(bm, BFD_DEFAULT_DESIRED_MIN_TX_USEC);
    bm->min_required_min_rx_while_echo_clocks = bfd_usec_to_clocks(bm, BFD_REQUIRED_MIN_RX_USEC_WHILE_ECHO);
...
{
    BFD_DBG("Switching authentication key from %U to %U for bs_idx=%u", format_bfd_auth_key, bs->auth.curr_key,
            format_bfd_auth_key, bs->auth.next_key, bs->bs_idx);
...
static int
bfd_verify_pkt_auth_seq_num(bfd_session_t *bs, u32 received_seq_num, int is_meticulous)
{
    /*
     * RFC 5880 6.8.1:
     *
    ...
    if (bs->auth.remote_seq_number_known) {
        ...
        if (bs->auth.remote_seq_number > max_u32 - 3 * bs->local_detect_mult) {
            ...
            if (received_seq_num > x && received_seq_num < bs->auth.remote_seq_number + is_meticulous) {
                BFD_ERR("Recvd sequence number=%u out of ranges <0, %u>, <%u, %u>", received_seq_num, x,
...
static int
bfd_verify_pkt_auth_key_sha1(const bfd_pkt_t *pkt, u32 pkt_size, bfd_session_t *bs, u8 bfd_key_id,
                             bfd_auth_key_t *auth_key)
{
    ...
    if (with_common->common_auth.type != auth_key->auth_type) {
        BFD_ERR("BFD auth type mismatch, packet auth=%d:%s doesn't match "
                "in-use auth=%d:%s",
                with_common->common_auth.type, bfd_auth_type_str(with_common->common_auth.type), auth_key->auth_type,
    ...
    if (pkt_size < sizeof(*with_sha1) || with_sha1->sha1_auth.type_len.len < sizeof(with_sha1->sha1_auth)) {
        BFD_ERR("BFD size mismatch, payload size=%u, expected=%u, auth_len=%u, "
                "expected=%u",
                pkt_size, sizeof(*with_sha1), with_sha1->sha1_auth.type_len.len, sizeof(with_sha1->sha1_auth));
        return 0;
    }
    if (with_sha1->sha1_auth.key_id != bfd_key_id) {
        BFD_ERR("BFD key ID mismatch, packet key ID=%u doesn't match key ID=%u%s", with_sha1->sha1_auth.key_id,
                bfd_key_id, bs->auth.is_delayed ? " (but a delayed auth change is scheduled)" : "");
        return 0;
    }
    SHA_CTX ctx;
    if (!SHA1_Init(&ctx)) {
    ...
    if (!SHA1_Update(&ctx, with_sha1, sizeof(*with_sha1) - sizeof(with_sha1->sha1_auth.hash))) {
    ...
    BFD_ERR("SHA1 hash: %U doesn't match the expected value: %U", format_hex_bytes, with_sha1->sha1_auth.hash,
            SHA_DIGEST_LENGTH, format_hex_bytes, result, SHA_DIGEST_LENGTH);
    return 0;
}

static int
bfd_verify_pkt_auth_key(const bfd_pkt_t *pkt, u32 pkt_size, bfd_session_t *bs, u8 bfd_key_id, bfd_auth_key_t *auth_key)
{
    bfd_main_t *bm = &bfd_main;
    switch (auth_key->auth_type) {
    case BFD_AUTH_TYPE_reserved:
        vlib_log_err(bm->log_class, "internal error, unexpected auth_type=%d:%s", auth_key->auth_type,
                     bfd_auth_type_str(auth_key->auth_type));
        return 0;
    case BFD_AUTH_TYPE_simple_password:
        vlib_log_err(bm->log_class, "internal error, not implemented, unexpected auth_type=%d:%s", auth_key->auth_type,
                     bfd_auth_type_str(auth_key->auth_type));
        return 0;
    case BFD_AUTH_TYPE_keyed_md5:
        /* fallthrough */
    ...
    case BFD_AUTH_TYPE_meticulous_keyed_sha1:
        ...
        do {
            ...
            return bfd_verify_pkt_auth_seq_num(bs, seq_num, bfd_auth_type_is_meticulous(auth_key->auth_type)) &&
                   bfd_verify_pkt_auth_key_sha1(pkt, pkt_size, bs, bfd_key_id, auth_key);
        } while (0);
#else
        vlib_log_err(bm->log_class, "internal error, attempt to use SHA1 without SSL support");
        return 0;
#endif
    }
    return 0;
    ...
    if (bfd_pkt_get_auth_present(pkt)) {
        ...
        if (!bs->auth.curr_key) {
            ...
            if (bs->auth.is_delayed && bs->auth.next_key) {
                ...
                if (bfd_verify_pkt_auth_key(pkt, pkt_size, bs, bs->auth.next_bfd_key_id, bs->auth.next_key)) {
            ...
            if (bfd_verify_pkt_auth_key(pkt, pkt_size, bs, bs->auth.curr_bfd_key_id, bs->auth.curr_key)) {
...
bfd_consume_pkt(bfd_main_t *bm, const bfd_pkt_t *pkt, u32 bs_idx)
{
    ...
    bs->remote_desired_min_tx_clocks = bfd_usec_to_clocks(bm, clib_net_to_host_u32(pkt->des_min_tx));
    bs->remote_detect_mult           = pkt->head.detect_mult;
    bfd_set_remote_required_min_rx(bm, bs, now, clib_net_to_host_u32(pkt->req_min_rx));
    bfd_set_remote_required_min_echo_rx(bm, bs, now, clib_net_to_host_u32(pkt->req_min_echo_rx));
    if (bfd_pkt_get_final(pkt)) {
        if (BFD_POLL_IN_PROGRESS == bs->poll_state) {
            ...
            if (BFD_STATE_up == bs->local_state) {
                bfd_set_effective_required_min_rx(bm, bs,
                                                  clib_max(bs->echo * bm->min_required_min_rx_while_echo_clocks,
                                                           bs->config_required_min_rx_clocks));
            }
            ...
            BFD_DBG("Next poll sequence can commence in " BFD_CLK_FMT,
                    BFD_CLK_PRN(now - bs->poll_state_start_or_timeout_clocks));
            bs->poll_state_start_or_timeout_clocks = now + (now - bs->poll_state_start_or_timeout_clocks);
...
bfd_consume_echo_pkt(bfd_main_t *bm, vlib_buffer_t *b)
{
    ...
    u64 checksum = bfd_calc_echo_checksum(bs->local_discr, pkt->expire_time_clocks, bs->echo_secret);
    ...
    if (pkt->expire_time_clocks < now) {
        BFD_DBG("Stale packet received, expire time %lu < now %lu", pkt->expire_time_clocks, now);
    } else {
        bs->echo_last_rx_clocks = now;
    }
    return 1;
...
format_bfd_session(u8 *s, va_list *args)
{
    ...
    s                       = format(s,
               ...
               "%U      curr-key=%U\n"
               "%U      next-key=%U",
               bs->bs_idx, bfd_state_string(bs->local_state), bfd_state_string(bs->remote_state), format_white_space,
               indent, bs->local_discr, bs->remote_discr, format_white_space, indent,
               bfd_diag_code_string(bs->local_diag), (bs->echo ? "yes" : "no"), format_white_space, indent,
               bs->config_desired_min_tx_usec, bs->config_required_min_rx_usec, format_white_space, indent, 1,
               bs->local_detect_mult, format_white_space, indent, bs->remote_min_rx_usec, bs->remote_min_echo_rx_usec,
               format_white_space, indent, (bs->remote_demand ? "yes" : "no"), bfd_poll_state_string(bs->poll_state),
               format_white_space, indent, bs->auth.local_seq_number, bs->auth.remote_seq_number, format_white_space,
               indent, (bs->auth.is_delayed ? "yes" : "no"), format_white_space, indent, format_bfd_auth_key,
               bs->auth.curr_key, format_white_space, indent, format_bfd_auth_key, bs->auth.next_key);
    return s;
}
...
{
    const bfd_session_t *bs = va_arg(*args, bfd_session_t *);
    s = format(s, "bs_idx=%u local-state=%s remote-state=%s", bs->bs_idx, bfd_state_string(bs->local_state),
...
bfd_auth_deactivate(bfd_session_t *bs, u8 is_delayed)
{
    ...
    return 0;
#else
    vlib_log_err(bm->log_class, "SSL missing, cannot deactivate BFD authentication");
    return VNET_API_ERROR_BFD_NOTSUPP;
#endif
}

vnet_api_error_t
bfd_session_set_params(bfd_main_t *bm, bfd_session_t *bs, u32 desired_min_tx_usec, u32 required_min_rx_usec,
                       u8 detect_mult)
{
    if (bs->local_detect_mult != detect_mult || bs->config_desired_min_tx_usec != desired_min_tx_usec ||
        ...
        bs->config_desired_min_tx_clocks  = bfd_usec_to_clocks(bm, desired_min_tx_usec);
        bs->config_required_min_rx_usec   = required_min_rx_usec;
        bs->config_required_min_rx_clocks = bfd_usec_to_clocks(bm, required_min_rx_usec);
...
bfd_auth_set_key(u32 conf_key_id, u8 auth_type, u8 key_len, const u8 *key_data)
{
    ...
    if (!key_len || key_len > bfd_max_key_len_for_auth_type(auth_type)) {
        vlib_log_err(bm->log_class,
                     ...
                     auth_type, bfd_auth_type_str(auth_type), key_len, (u32) bfd_max_key_len_for_auth_type(auth_type));
    ...
    return 0;
#else
    vlib_log_err(bm->log_class, "SSL missing, cannot manipulate authentication keys");
    return VNET_API_ERROR_BFD_NOTSUPP;
#endif
}
