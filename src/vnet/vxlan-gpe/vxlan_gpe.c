...
format_vxlan_gpe_tunnel(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d] lcl %U rmt %U vni %d fib-idx %d sw-if-idx %d ", t - ngm->tunnels, format_ip46_address, &t->local,
               IP46_TYPE_ANY, format_ip46_address, &t->remote, IP46_TYPE_ANY, t->vni, t->encap_fib_index, t->sw_if_index);
...
vxlan_gpe_tunnel_restack_dpo(vxlan_gpe_tunnel_t *t)
{
    ...
    fib_forward_chain_type_t forw_type =
        ip46_address_is_ip4(&t->remote) ? FIB_FORW_CHAIN_TYPE_UNICAST_IP4 : FIB_FORW_CHAIN_TYPE_UNICAST_IP6;
 ...
 */
int
vxlan4_gpe_rewrite(vxlan_gpe_tunnel_t *t, u32 extension_size, u8 protocol_override, uword encap_next_node)
{
    u8 *rw = 0;
    ip4_header_t *ip0;
 ...
 */
int
vxlan6_gpe_rewrite(vxlan_gpe_tunnel_t *t, u32 extension_size, u8 protocol_override, uword encap_next_node)
{
    u8 *rw = 0;
    ip6_header_t *ip0;
...
{
    uword *vtep = ip46_address_is_ip4(ip) ? hash_get(vxlan_gpe_main.vtep4, ip->ip4.as_u32) :
                                            hash_get_mem(vxlan_gpe_main.vtep6, &ip->ip6);
    if (vtep)
        return ++(*vtep);
    ip46_address_is_ip4(ip) ? hash_set(vxlan_gpe_main.vtep4, ip->ip4.as_u32, 1) :
...
vtep_addr_unref(ip46_address_t *ip)
{
    ...
    if (--(*vtep) != 0)
        return *vtep;
    ip46_address_is_ip4(ip) ? hash_unset(vxlan_gpe_main.vtep4, ip->ip4.as_u32) :
...
static inline void
mcast_shared_add(ip46_address_t *remote, fib_node_index_t mfei, adj_index_t ai)
{
 ...
 */
int
vnet_vxlan_gpe_add_del_tunnel(vnet_vxlan_gpe_add_del_tunnel_args_t *a, u32 *sw_if_indexp)
{
    vxlan_gpe_main_t *ngm = &vxlan_gpe_main;
...
#undef _
        ...
        if (vec_len(ngm->free_vxlan_gpe_tunnel_hw_if_indices) > 0) {
            ...
            hw_if_index = vnet_register_interface(vnm, vxlan_gpe_device_class.index, t - ngm->tunnels,
        ...
        if (!ip46_address_is_multicast(&t->remote)) {
            ...
            t->fib_entry_index =
                fib_table_entry_special_add(t->encap_fib_index, &tun_remote_pfx, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
            t->sibling_index = fib_entry_child_add(t->fib_entry_index, FIB_NODE_TYPE_VXLAN_GPE_TUNNEL, t - ngm->tunnels);
            ...
            if (vtep_addr_ref(&t->remote) == 1) {
                 ...
                 */
                mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_VXLAN_GPE, &path,
                                             MFIB_ITF_FLAG_FORWARD);

                path.frp_sw_if_index = a->mcast_sw_if_index;
                path.frp_flags       = FIB_ROUTE_PATH_FLAG_NONE;
                mfei = mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_VXLAN_GPE, &path,
...
static clib_error_t *
vxlan_gpe_add_del_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            ...
            remote_set = 1;
            ipv6_set   = 1;
        } else if (unformat(line_input, "group %U %U", unformat_ip4_address, &remote.ip4, unformat_vnet_sw_interface,
                            vnet_get_main(), &mcast_sw_if_index)) {
            grp_set = remote_set = 1;
            ipv4_set             = 1;
        } else if (unformat(line_input, "group %U %U", unformat_ip6_address, &remote.ip6, unformat_vnet_sw_interface,
                            vnet_get_main(), &mcast_sw_if_index)) {
            grp_set = remote_set = 1;
            ipv6_set             = 1;
...
VLIB_CLI_COMMAND(create_vxlan_gpe_tunnel_command, static) = {
    ...
    .short_help = "create vxlan-gpe tunnel local <local-addr> "
 ...
 */
static clib_error_t *
show_vxlan_gpe_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vxlan_gpe_main_t *ngm = &vxlan_gpe_main;
    vxlan_gpe_tunnel_t *t;
    ...
    /* *INDENT-OFF* */
    pool_foreach(t, ngm->tunnels, ({ vlib_cli_output(vm, "%U", format_vxlan_gpe_tunnel, t); }));
    /* *INDENT-ON* */
...
static clib_error_t *
set_ip_vxlan_gpe_bypass(u32 is_ip6, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
set_ip4_vxlan_gpe_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
set_ip6_vxlan_gpe_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
vxlan_gpe_init(vlib_main_t *vm)
{
    ...
    udp_register_dst_port(vm, UDP_DST_PORT_VXLAN_GPE, vxlan4_gpe_input_node.index, 1 /* is_ip4 */);
    udp_register_dst_port(vm, UDP_DST_PORT_VXLAN6_GPE, vxlan6_gpe_input_node.index, 0 /* is_ip4 */);
