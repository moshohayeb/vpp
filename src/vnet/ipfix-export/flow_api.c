...
vl_api_ipfix_exporter_dump_t_handler(vl_api_ipfix_exporter_dump_t *mp)
{
    ...
    memcpy(rmp->collector_address, frm->ipfix_collector.data, sizeof(frm->ipfix_collector.data));
    rmp->collector_port = htons(frm->collector_port);
    memcpy(rmp->src_address, frm->src_address.data, sizeof(frm->src_address.data));
    if (frm->fib_index == ~0)
        vrf_id = ~0;
    else
...
static void
vl_api_set_ipfix_classify_stream_t_handler(vl_api_set_ipfix_classify_stream_t *mp)
{
    ...
    if (fcm->src_port != 0 && (fcm->domain_id != domain_id || fcm->src_port != (u16) src_port)) {
        int rv = vnet_stream_change(frm, fcm->domain_id, fcm->src_port, domain_id, (u16) src_port);
        ASSERT(rv == 0);
    }
...
static void
vl_api_ipfix_classify_stream_dump_t_handler(vl_api_ipfix_classify_stream_dump_t *mp)
{
    flow_report_classify_main_t *fcm = &flow_report_classify_main;
    vl_api_registration_t *reg;
...
static void
vl_api_ipfix_classify_table_add_del_t_handler(vl_api_ipfix_classify_table_add_del_t *mp)
{
...
static void
send_ipfix_classify_table_details(u32 table_index, vl_api_registration_t *reg, u32 context)
{
    flow_report_classify_main_t *fcm = &flow_report_classify_main;
...
static void
vl_api_ipfix_classify_table_dump_t_handler(vl_api_ipfix_classify_table_dump_t *mp)
{
    flow_report_classify_main_t *fcm = &flow_report_classify_main;
    vl_api_registration_t *reg;
    u32 i;
