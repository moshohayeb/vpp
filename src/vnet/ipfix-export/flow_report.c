...
stream_index_valid(u32 index)
{
    ...
    return index < vec_len(frm->streams) && frm->streams[index].domain_id != ~0;
}
...
int
send_template_packet(flow_report_main_t *frm, flow_report_t *fr, u32 *buffer_indexp)
{
    u32 bi0;
    vlib_buffer_t *b0;
    ...
    if (fr->update_rewrite) {
        fr->rewrite = fr->rewrite_callback(frm, fr, &frm->ipfix_collector, &frm->src_address, frm->collector_port,
...
u8 *
vnet_flow_rewrite_generic_callback(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
                                   ip4_address_t *src_address, u16 collector_port, ipfix_report_element_t *report_elts,
                                   u32 n_elts, u32 *stream_indexp)
{
    ...
    *stream_indexp = fr->stream_index;
    ...
    vec_validate_aligned(rewrite, sizeof(ip4_ipfix_template_packet_t) + n_elts * sizeof(ipfix_field_specifier_t) - 1,
...
static uword
flow_report_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    flow_report_main_t *frm = &flow_report_main;
    flow_report_t *fr;
...
int
vnet_flow_report_add_del(flow_report_main_t *frm, vnet_flow_report_add_del_args_t *a, u16 *template_id)
{
    int i;
    int found_index = ~0;
    flow_report_t *fr;
...
vnet_stream_reset(flow_report_main_t *frm, u32 stream_index)
{
    ...
    vec_foreach(fr, frm->reports) if (frm->reports->stream_index == stream_index)
    {
        fr->update_rewrite     = 1;
        fr->last_template_sent = 0;
    }
...
int
vnet_stream_change(flow_report_main_t *frm, u32 old_domain_id, u16 old_src_port, u32 new_domain_id, u16 new_src_port)
{
...
static clib_error_t *
set_ipfix_exporter_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    flow_report_main_t *frm = &flow_report_main;
    ...
    if (frm->ipfix_collector.as_u32 != collector.as_u32 || frm->src_address.as_u32 != src.as_u32 ||
    ...
    if (collector.as_u32)
        vlib_cli_output(vm,
                        ...
                        "udp checksum %s",
                        format_ip4_address, &frm->ipfix_collector, format_ip4_address, &frm->src_address, fib_index,
                        path_mtu, template_interval, udp_checksum ? "enabled" : "disabled");
    else
...
static clib_error_t *
ipfix_flush_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
