...
u8 *
ipfix_classify_template_rewrite(flow_report_main_t *frm, flow_report_t *fr, ip4_address_t *collector_address,
...
vlib_frame_t *
ipfix_classify_send_flows(flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
    flow_report_classify_main_t *fcm = &flow_report_classify_main;
    vnet_classify_main_t *vcm        = &vnet_classify_main;
    u32 flow_table_index             = fr->opaque.as_uword;
...
#define _(field, mask, item, length)                                                                                   \
    if (clib_bitmap_get(fr->fields_to_send, field_index)) {                                                            \
        clib_memcpy(b0->data + next_offset, &field, length);                                                           \
        next_offset += length;                                                                                         \
    }                                                                                                                  \
    field_index++;
                foreach_ipfix_field;
#undef _
                ...
                if (next_offset + next_record_size > frm->path_mtu) {
                    s->set_id_length =
                        ipfix_set_id_length(fr->template_id, next_offset - (sizeof(*ip) + sizeof(*udp) + sizeof(*h)));
...
static clib_error_t *
ipfix_classify_table_add_del_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    flow_report_classify_main_t *fcm = &flow_report_classify_main;
...
static clib_error_t *
set_ipfix_classify_stream_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    flow_report_classify_main_t *fcm = &flow_report_classify_main;
    flow_report_main_t *frm          = &flow_report_main;
    u32 domain_id                    = 1;
