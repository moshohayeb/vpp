 ...
 */
adj_index_t
adj_glean_add_or_lock(fib_protocol_t proto, vnet_link_t linkt, u32 sw_if_index, const ip46_address_t *nh_addr)
{
    ip_adjacency_t *adj;
    ...
    if (ADJ_INDEX_INVALID == adj_gleans[proto][sw_if_index]) {
        ...
        adj->rewrite_header.max_l3_packet_bytes =
            vnet_sw_interface_get_mtu(vnet_get_main(), sw_if_index, vnet_link_to_mtu(linkt));
        adj_lock(adj_get_index(adj));

        vnet_update_adjacency_for_sw_interface(vnet_get_main(), sw_if_index, adj_get_index(adj));
    } else {
...
adj_glean_update_rewrite(adj_index_t adj_index)
{
    ...
    vnet_rewrite_for_sw_interface(vnet_get_main(), adj_fib_proto_2_nd(adj->ia_nh_proto),
                                  adj->rewrite_header.sw_if_index, adj_get_glean_node(adj->ia_nh_proto)->index,
                                  VNET_REWRITE_FOR_SW_INTERFACE_ADDRESS_BROADCAST, &adj->rewrite_header,
                                  sizeof(adj->rewrite_data));
}

adj_index_t
adj_glean_get(fib_protocol_t proto, u32 sw_if_index)
{
...
void
adj_glean_remove(fib_protocol_t proto, u32 sw_if_index)
{
...
static clib_error_t *
adj_glean_interface_state_change(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    /*
    ...
    for (proto = FIB_PROTOCOL_IP4; proto <= FIB_PROTOCOL_IP6; proto++) {
        if (sw_if_index >= vec_len(adj_gleans[proto]) || ADJ_INDEX_INVALID == adj_gleans[proto][sw_if_index])
            continue;
 ...
 */
static clib_error_t *
adj_glean_hw_interface_state_change(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    /*
    ...
    vnet_hw_interface_walk_sw(vnm, hw_if_index, adj_nbr_hw_sw_interface_state_change, (void *) sw_flags);
...
static clib_error_t *
adj_glean_interface_delete(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    /*
     * for each glean on the interface trigger a walk back to the children
     */
...
format_adj_glean(u8 *s, va_list *ap)
{
    ...
    s = format(s, "%U-glean: %U", format_fib_protocol, adj->ia_nh_proto, format_vnet_rewrite, &adj->rewrite_header,
