...
adj_alloc(fib_protocol_t proto)
{
    ...
    vlib_validate_combined_counter(&adjacency_counters, adj_get_index(adj));
    vlib_zero_combined_counter(&adjacency_counters, adj_get_index(adj));
    fib_node_init(&adj->ia_node, FIB_NODE_TYPE_ADJ);
    ...
    memset(&adj->sub_type.midchain.next_dpo, 0, sizeof(adj->sub_type.midchain.next_dpo));
...
adj_last_lock_gone(ip_adjacency_t *adj)
{
    ...
    case IP_LOOKUP_NEXT_BCAST:
         ...
         */
        adj_nbr_remove(adj_get_index(adj), adj->ia_nh_proto, adj->ia_link, &adj->sub_type.nbr.next_hop,
    ...
    case IP_LOOKUP_NEXT_MCAST:
    case IP_LOOKUP_NEXT_MCAST_MIDCHAIN:
        adj_mcast_remove(adj->ia_nh_proto, adj->rewrite_header.sw_if_index);
        break;
...
u32
adj_child_add(adj_index_t adj_index, fib_node_type_t child_type, fib_node_index_t child_index)
{
    ...
    return (fib_node_child_add(FIB_NODE_TYPE_ADJ, adj_index, child_type, child_index));
}

void
adj_child_remove(adj_index_t adj_index, u32 sibling_index)
{
    if (adj_index_is_special(adj_index)) {
        return;
    }

    fib_node_child_remove(FIB_NODE_TYPE_ADJ, adj_index, sibling_index);
}
...
adj_feature_update_walk_cb(adj_index_t ai, void *arg)
{
     ...
     * adjacencies
     */
    if (((ctx->arc == ip6_main.lookup_main.output_feature_arc_index) && (VNET_LINK_IP6 == adj->ia_link)) ||
        ((ctx->arc == ip4_main.lookup_main.output_feature_arc_index) && (VNET_LINK_IP4 == adj->ia_link)) ||
        ((ctx->arc == mpls_main.output_feature_arc_index) && (VNET_LINK_MPLS == adj->ia_link))) {
        if (ctx->enable)
...
void
adj_feature_update(u32 sw_if_index, u8 arc_index, u8 is_enable)
{
    /*
...
static adj_walk_rc_t
adj_mtu_update_walk_cb(adj_index_t ai, void *arg)
{
    ip_adjacency_t *adj;

    adj = adj_get(ai);

    vnet_rewrite_update_mtu(vnet_get_main(), adj->ia_link, &adj->rewrite_header);
 ...
 */
void
adj_walk(u32 sw_if_index, adj_walk_cb_t cb, void *ctx)
{
    /*
...
adj_back_walk_notify(fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
{
    /*
...
static clib_error_t *
adj_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (summary) {
        ...
        vlib_cli_output(vm, "Per-adjacency counters: %s", (adj_are_counters_enabled() ? "enabled" : "disabled"));
    } else {
        if (ADJ_INDEX_INVALID != ai) {
            ...
            pool_foreach_index(ai, adj_pool, ({
                                   if (~0 != sw_if_index && sw_if_index != adj_get_sw_if_index(ai)) {
                                   } else {
                                       vlib_cli_output(vm, "[@%d] %U", ai, format_ip_adjacency, ai,
 ...
 */
static clib_error_t *
adj_cli_counters_set(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
