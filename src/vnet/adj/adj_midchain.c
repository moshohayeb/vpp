...
always_inline uword
adj_midchain_tx_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int interface_count)
{
        ...
        while (n_left_from >= 8 && n_left_to_next > 4) {
            ...
            if (interface_count) {
                vlib_increment_combined_counter(im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX, thread_index,
                                                adj0->rewrite_header.sw_if_index, 1, vlib_buffer_length_in_chain(vm, b0));
                vlib_increment_combined_counter(im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX, thread_index,
                                                adj1->rewrite_header.sw_if_index, 1, vlib_buffer_length_in_chain(vm, b1));
                vlib_increment_combined_counter(im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX, thread_index,
                                                adj2->rewrite_header.sw_if_index, 1, vlib_buffer_length_in_chain(vm, b2));
                vlib_increment_combined_counter(im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX, thread_index,
                                                adj3->rewrite_header.sw_if_index, 1, vlib_buffer_length_in_chain(vm, b3));
            }

            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                adj_midchain_tx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->ai                      = adj_index0;
            }
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                adj_midchain_tx_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
                tr->ai                      = adj_index1;
            }
            if (PREDICT_FALSE(b2->flags & VLIB_BUFFER_IS_TRACED)) {
                adj_midchain_tx_trace_t *tr = vlib_add_trace(vm, node, b2, sizeof(*tr));
                tr->ai                      = adj_index2;
            }
            if (PREDICT_FALSE(b3->flags & VLIB_BUFFER_IS_TRACED)) {
                adj_midchain_tx_trace_t *tr = vlib_add_trace(vm, node, b3, sizeof(*tr));
                tr->ai                      = adj_index3;
            }
...
format_adj_midchain_tx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "adj-midchain:[%d]:%U", tr->ai, format_ip_adjacency, tr->ai, FORMAT_IP_ADJACENCY_NONE);

    return (s);
}

static uword
adj_midchain_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
adj_midchain_tx_no_count(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
{
    return ((adj->ia_flags & ADJ_FLAG_MIDCHAIN_NO_COUNT) ? adj_midchain_tx_no_count_node.index :
 ...
 */
void
adj_midchain_setup(adj_index_t adj_index, adj_midchain_fixup_t fixup, const void *data, adj_flags_t flags)
{
    ...
    vnet_feature_enable_disable_with_index(arc_index, feature_index, adj->rewrite_header.sw_if_index, 1 /* enable */, 0,
                                           0);
     ...
     */
    dpo_stack_from_node(tx_node, &adj->sub_type.midchain.next_dpo, drop_dpo_get(vnet_link_to_dpo_proto(adj->ia_link)));
}
 ...
 */
void
adj_nbr_midchain_update_rewrite(adj_index_t adj_index, adj_midchain_fixup_t fixup, const void *fixup_data,
                                adj_flags_t flags, u8 *rewrite)
{
     ...
     */
    ASSERT((adj->lookup_next_index == IP_LOOKUP_NEXT_ARP) || (adj->lookup_next_index == IP_LOOKUP_NEXT_GLEAN) ||
     ...
     */
    adj_nbr_update_rewrite_internal(adj, IP_LOOKUP_NEXT_MIDCHAIN, adj_get_midchain_node(adj->ia_link),
                                    adj_nbr_midchain_get_tx_node(adj), rewrite);
}
...
adj_nbr_midchain_unstack(adj_index_t adj_index)
{
     ...
     * stack on the drop
     */
    dpo_stack(DPO_ADJACENCY_MIDCHAIN, vnet_link_to_dpo_proto(adj->ia_link), &adj->sub_type.midchain.next_dpo,
 ...
 */
void
adj_nbr_midchain_stack(adj_index_t adj_index, const dpo_id_t *next)
{
    ip_adjacency_t *adj;
    ...
    dpo_stack_from_node(adj_nbr_midchain_get_tx_node(adj), &adj->sub_type.midchain.next_dpo, next);
}
...
format_adj_midchain(u8 *s, va_list *ap)
{
    ...
    s = format(s, " %U", format_vnet_rewrite, &adj->rewrite_header, sizeof(adj->rewrite_data), indent);
    s = format(s, "\n%Ustacked-on:\n%U%U", format_white_space, indent, format_white_space, indent + 2, format_dpo_id,
               &adj->sub_type.midchain.next_dpo, indent + 2);
...
const static char *const *const midchain_nodes[DPO_PROTO_NUM] = {
    [DPO_PROTO_IP4] = midchain_ip4_nodes,   [DPO_PROTO_IP6] = midchain_ip6_nodes,
    [DPO_PROTO_MPLS] = midchain_mpls_nodes, [DPO_PROTO_ETHERNET] = midchain_ethernet_nodes,
