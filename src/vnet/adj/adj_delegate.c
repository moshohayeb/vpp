...
adj_delegate_find_i(const ip_adjacency_t *adj, adj_delegate_type_t type, u32 *index)
{
...
adj_delegate_t *
adj_delegate_get(const ip_adjacency_t *adj, adj_delegate_type_t type)
{
    return (adj_delegate_find_i(adj, type, NULL));
}

void
adj_delegate_remove(adj_index_t ai, adj_delegate_type_t type)
{
    ip_adjacency_t *adj;
    adj_delegate_t *aed;
    u32 index = ~0;
...
static int
adj_delegate_cmp_for_sort(void *v1, void *v2)
{
...
static void
adj_delegate_init(ip_adjacency_t *adj, adj_delegate_type_t adt, index_t adi)

{
    ...
    vec_sort_with_function(adj->ia_delegates, adj_delegate_cmp_for_sort);
}

int
adj_delegate_add(ip_adjacency_t *adj, adj_delegate_type_t adt, index_t adi)
{
    adj_delegate_t *delegate;
 ...
 */
void
adj_delegate_register_type(adj_delegate_type_t type, const adj_delegate_vft_t *vft)
{
    /*
