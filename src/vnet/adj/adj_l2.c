...
always_inline uword
adj_l2_rewrite_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_midchain)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(p0->flags & VLIB_BUFFER_IS_TRACED)) {
                adj_l2_trace_t *tr = vlib_add_trace(vm, node, p0, sizeof(*tr));
...
static uword
adj_l2_rewrite(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return adj_l2_rewrite_inline(vm, node, frame, 0);
}

static uword
adj_l2_midchain(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
