...
#define ADJ_NBR_ITF_OK(_proto, _itf)                                                                                   \
    (((_itf) < vec_len(adj_nbr_tables[_proto])) && (NULL != adj_nbr_tables[_proto][sw_if_index]))

static void
adj_nbr_insert(fib_protocol_t nh_proto, vnet_link_t link_type, const ip46_address_t *nh_addr, u32 sw_if_index,
               adj_index_t adj_index)
{
    ...
    if (NULL == adj_nbr_tables[nh_proto][sw_if_index]) {
        adj_nbr_tables[nh_proto][sw_if_index] = clib_mem_alloc_aligned(sizeof(BVT(clib_bihash)), CLIB_CACHE_LINE_BYTES);
        memset(adj_nbr_tables[nh_proto][sw_if_index], 0, sizeof(BVT(clib_bihash)));

        BV(clib_bihash_init)
        (adj_nbr_tables[nh_proto][sw_if_index], "Adjacency Neighbour table", ADJ_NBR_DEFAULT_HASH_NUM_BUCKETS,
...
void
adj_nbr_remove(adj_index_t ai, fib_protocol_t nh_proto, vnet_link_t link_type, const ip46_address_t *nh_addr,
               u32 sw_if_index)
{
    BVT(clib_bihash_kv) kv;
...
adj_index_t
adj_nbr_find(fib_protocol_t nh_proto, vnet_link_t link_type, const ip46_address_t *nh_addr, u32 sw_if_index)
{
    BVT(clib_bihash_kv) kv;
    ...
    if (BV(clib_bihash_search)(adj_nbr_tables[nh_proto][sw_if_index], &kv, &kv) < 0) {
        return (ADJ_INDEX_INVALID);
    } else {
        return (kv.value);
    }
...
adj_nbr_alloc(fib_protocol_t nh_proto, vnet_link_t link_type, const ip46_address_t *nh_addr, u32 sw_if_index)
{
    ip_adjacency_t *adj;

    adj = adj_alloc(nh_proto);

    adj_nbr_insert(nh_proto, link_type, nh_addr, sw_if_index, adj_get_index(adj));
 ...
 */
adj_index_t
adj_nbr_add_or_lock(fib_protocol_t nh_proto, vnet_link_t link_type, const ip46_address_t *nh_addr, u32 sw_if_index)
{
    adj_index_t adj_index;
    ip_adjacency_t *adj;
    ...
    if (ADJ_INDEX_INVALID == adj_index) {
        ...
        vnet_rewrite_init(vnm, sw_if_index, link_type, adj_get_nd_node(nh_proto),
                          vnet_tx_node_index_for_sw_interface(vnm, sw_if_index), &adj->rewrite_header);
...
adj_index_t
adj_nbr_add_or_lock_w_rewrite(fib_protocol_t nh_proto, vnet_link_t link_type, const ip46_address_t *nh_addr,
                              u32 sw_if_index, u8 *rewrite)
{
    adj_index_t adj_index;
    ip_adjacency_t *adj;
    ...
    adj_nbr_update_rewrite(adj_get_index(adj), ADJ_NBR_REWRITE_FLAG_COMPLETE, rewrite);
 ...
 */
void
adj_nbr_update_rewrite(adj_index_t adj_index, adj_nbr_rewrite_flag_t flags, u8 *rewrite)
{
    ip_adjacency_t *adj;
    ...
    if (flags & ADJ_NBR_REWRITE_FLAG_COMPLETE) {
         ...
         */
        adj_nbr_update_rewrite_internal(adj, IP_LOOKUP_NEXT_REWRITE, adj_get_rewrite_node(adj->ia_link),
                                        vnet_tx_node_index_for_sw_interface(vnet_get_main(),
                                                                            adj->rewrite_header.sw_if_index),
                                        rewrite);
    } else {
        adj_nbr_update_rewrite_internal(adj, IP_LOOKUP_NEXT_ARP, adj_get_nd_node(adj->ia_nh_proto),
                                        vnet_tx_node_index_for_sw_interface(vnet_get_main(),
                                                                            adj->rewrite_header.sw_if_index),
                                        rewrite);
    }
 ...
 */
void
adj_nbr_update_rewrite_internal(ip_adjacency_t *adj, ip_lookup_next_t adj_next_index, u32 this_node, u32 next_node,
                                u8 *rewrite)
{
    ...
    if (VNET_LINK_MPLS == adj->ia_link) {
         ...
         */
        walk_ai = adj_nbr_find(adj->ia_nh_proto, fib_proto_to_link(adj->ia_nh_proto), &adj->sub_type.nbr.next_hop,
                               adj->rewrite_header.sw_if_index);
    }
     ...
     *  2 - adj type change
     */
    if (do_walk && old_next != adj_next_index && ADJ_INDEX_INVALID != walk_ai) {
        /*
    ...
    if (NULL != rewrite) {
         ...
         */
        vnet_rewrite_set_data_internal(&adj->rewrite_header, sizeof(adj->rewrite_data), rewrite, vec_len(rewrite));
        vec_free(rewrite);
    } else {
        vnet_rewrite_clear_data_internal(&adj->rewrite_header, sizeof(adj->rewrite_data));
    }
    adj->rewrite_header.next_index = vlib_node_add_next(vlib_get_main(), this_node, next_node);
    ...
    if (do_walk && (old_next != adj->lookup_next_index) && (ADJ_INDEX_INVALID != walk_ai)) {
        /*
...
static void
adj_db_count(BVT(clib_bihash_kv) * kvp, void *arg)
{
    ...
    for (proto = FIB_PROTOCOL_IP4; proto <= FIB_PROTOCOL_IP6; proto++) {
        vec_foreach_index(sw_if_index, adj_nbr_tables[proto])
        {
            if (NULL != adj_nbr_tables[proto][sw_if_index]) {
                BV(clib_bihash_foreach_key_value_pair)(adj_nbr_tables[proto][sw_if_index], adj_db_count, &ctx);
            }
        }
    }
    return (ctx.count);
...
static void
adj_nbr_walk_cb(BVT(clib_bihash_kv) * kvp, void *arg)
{
...
void
adj_nbr_walk(u32 sw_if_index, fib_protocol_t adj_nh_proto, adj_walk_cb_t cb, void *ctx)
{
    ...
    BV(clib_bihash_foreach_key_value_pair)(adj_nbr_tables[adj_nh_proto][sw_if_index], adj_nbr_walk_cb, &awc);
}
 ...
 */
void
adj_nbr_walk_nh4(u32 sw_if_index, const ip4_address_t *addr, adj_walk_cb_t cb, void *ctx)
{
 ...
 */
void
adj_nbr_walk_nh6(u32 sw_if_index, const ip6_address_t *addr, adj_walk_cb_t cb, void *ctx)
{
 ...
 */
void
adj_nbr_walk_nh(u32 sw_if_index, fib_protocol_t adj_nh_proto, const ip46_address_t *nh, adj_walk_cb_t cb, void *ctx)
{
    if (!ADJ_NBR_ITF_OK(adj_nh_proto, sw_if_index))
        return;
...
adj_nbr_interface_state_change_one(adj_index_t ai, void *arg)
{
    /*
    ...
    fib_node_back_walk_ctx_t bw_ctx           = {
        .fnbw_reason = ((ctx->flags & ADJ_NBR_INTERFACE_UP) ? FIB_NODE_BW_REASON_FLAG_INTERFACE_UP :
         ...
         */
        .fnbw_flags = (!(ctx->flags & ADJ_NBR_INTERFACE_UP) ? FIB_NODE_BW_FLAG_FORCE_SYNC : FIB_NODE_BW_FLAG_NONE),
    };
    ip_adjacency_t *adj;
 ...
 */
static clib_error_t *
adj_nbr_sw_interface_state_change(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    fib_protocol_t proto;
    ...
    for (proto = FIB_PROTOCOL_IP4; proto <= FIB_PROTOCOL_IP6; proto++) {
        adj_nbr_interface_state_change_ctx_t ctx = {
            .flags = ((flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) ? ADJ_NBR_INTERFACE_UP : 0),
        };

        adj_nbr_walk(sw_if_index, proto, adj_nbr_interface_state_change_one, &ctx);
    }
 ...
 */
static walk_rc_t
adj_nbr_hw_sw_interface_state_change(vnet_main_t *vnm, u32 sw_if_index, void *arg)
{
    adj_nbr_interface_state_change_ctx_t *ctx = arg;
    fib_protocol_t proto;
 ...
 */
static clib_error_t *
adj_nbr_hw_interface_state_change(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    /*
     * walk SW interface on the HW
     */
    adj_nbr_interface_state_change_ctx_t ctx = {
        .flags = ((flags & VNET_HW_INTERFACE_FLAG_LINK_UP) ? ADJ_NBR_INTERFACE_UP : 0),
    };

    vnet_hw_interface_walk_sw(vnm, hw_if_index, adj_nbr_hw_sw_interface_state_change, &ctx);
...
static adj_walk_rc_t
adj_nbr_interface_delete_one(adj_index_t ai, void *arg)
{
    /*
     * Back walk the graph to inform the forwarding entries
 ...
 */
static clib_error_t *
adj_nbr_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    fib_protocol_t proto;
    ...
    for (proto = FIB_PROTOCOL_IP4; proto <= FIB_PROTOCOL_IP6; proto++) {
        adj_nbr_walk(sw_if_index, proto, adj_nbr_interface_delete_one, NULL);
    }
...
static adj_walk_rc_t
adj_nbr_show_one(adj_index_t ai, void *arg)
{
    vlib_cli_output(arg, "[@%d]  %U", ai, format_ip_adjacency, ai, FORMAT_IP_ADJACENCY_NONE);

    return (ADJ_WALK_RC_CONTINUE);
}

static clib_error_t *
adj_nbr_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (ADJ_INDEX_INVALID != ai) {
        vlib_cli_output(vm, "[@%d] %U", ai, format_ip_adjacency, ai, FORMAT_IP_ADJACENCY_DETAIL);
    } else if (~0 != sw_if_index) {
        fib_protocol_t proto;

        for (proto = FIB_PROTOCOL_IP4; proto <= FIB_PROTOCOL_IP6; proto++) {
            adj_nbr_walk(sw_if_index, proto, adj_nbr_show_one, vm);
        }
    } else {
        fib_protocol_t proto;
...
format_adj_nbr_incomplete(u8 *s, va_list *ap)
{
    ...
    s = format(s, ": via %U", format_ip46_address, &adj->sub_type.nbr.next_hop, adj_proto_to_46(adj->ia_nh_proto));
    s = format(s, " %U", format_vnet_sw_if_index_name, vnm, adj->rewrite_header.sw_if_index);
...
adj_mem_show(void)
{
    fib_show_memory_usage("Adjacency", pool_elts(adj_pool), pool_len(adj_pool), sizeof(ip_adjacency_t));
}
