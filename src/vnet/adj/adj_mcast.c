 ...
 */
adj_index_t
adj_mcast_add_or_lock(fib_protocol_t proto, vnet_link_t link_type, u32 sw_if_index)
{
    ip_adjacency_t *adj;
    ...
    if (ADJ_INDEX_INVALID == adj_mcasts[proto][sw_if_index]) {
        ...
        vnet_rewrite_init(vnm, sw_if_index, link_type, adj_get_mcast_node(proto),
         ...
         */
        vnet_update_adjacency_for_sw_interface(vnm, sw_if_index, adj_get_index(adj));
    } else {
 ...
 */
void
adj_mcast_update_rewrite(adj_index_t adj_index, u8 *rewrite, u8 offset)
{
    ip_adjacency_t *adj;
     ...
     */
    adj_nbr_update_rewrite_internal(adj, IP_LOOKUP_NEXT_MCAST, adj_get_mcast_node(adj->ia_nh_proto),
                                    vnet_tx_node_index_for_sw_interface(vnet_get_main(), adj->rewrite_header.sw_if_index),
                                    rewrite);
    /*
 ...
 */
void
adj_mcast_midchain_update_rewrite(adj_index_t adj_index, adj_midchain_fixup_t fixup, const void *fixup_data,
                                  adj_flags_t flags, u8 *rewrite, u8 offset, u32 mask)
{
     ...
     * from the rewrite node to the interface's TX node
     */
    adj_nbr_update_rewrite_internal(adj, IP_LOOKUP_NEXT_MCAST_MIDCHAIN, adj_get_mcast_node(adj->ia_nh_proto),
                                    vnet_tx_node_index_for_sw_interface(vnet_get_main(), adj->rewrite_header.sw_if_index),
                                    rewrite);
...
void
adj_mcast_remove(fib_protocol_t proto, u32 sw_if_index)
{
...
static clib_error_t *
adj_mcast_interface_state_change(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    /*
    ...
    for (proto = FIB_PROTOCOL_IP4; proto <= FIB_PROTOCOL_IP6; proto++) {
        if (sw_if_index >= vec_len(adj_mcasts[proto]) || ADJ_INDEX_INVALID == adj_mcasts[proto][sw_if_index])
            continue;

        adj = adj_get(adj_mcasts[proto][sw_if_index]);

        fib_node_back_walk_ctx_t bw_ctx = {
            .fnbw_reason = (flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP ? FIB_NODE_BW_REASON_FLAG_INTERFACE_UP :
 ...
 */
static walk_rc_t
adj_mcast_hw_sw_interface_state_change(vnet_main_t *vnm, u32 sw_if_index, void *arg)
{
 ...
 */
static clib_error_t *
adj_mcast_hw_interface_state_change(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    /*
    ...
    sw_flags = ((flags & VNET_HW_INTERFACE_FLAG_LINK_UP) ? VNET_SW_INTERFACE_FLAG_ADMIN_UP : 0);

    vnet_hw_interface_walk_sw(vnm, hw_if_index, adj_mcast_hw_sw_interface_state_change, (void *) sw_flags);
...
static clib_error_t *
adj_mcast_interface_delete(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    /*
     * for each mcast on the interface trigger a walk back to the children
     */
 ...
 */
void
adj_mcast_walk(u32 sw_if_index, fib_protocol_t proto, adj_walk_cb_t cb, void *ctx)
{
...
format_adj_mcast(u8 *s, va_list *ap)
{
    ...
    s = format(s, "%U-mcast: ", format_fib_protocol, adj->ia_nh_proto);
...
format_adj_mcast_midchain(u8 *s, va_list *ap)
{
    ...
    s = format(s, "%U-mcast-midchain: ", format_fib_protocol, adj->ia_nh_proto);
    s = format(s, "%U", format_vnet_rewrite, &adj->rewrite_header, sizeof(adj->rewrite_data), 0);
...
adj_mcast_db_size(void)
{
    ...
    n_adjs = 0;
    for (proto = FIB_PROTOCOL_IP4; proto <= FIB_PROTOCOL_IP6; proto++) {
        for (sw_if_index = 0; sw_if_index < vec_len(adj_mcasts[proto]); sw_if_index++) {
            if (ADJ_INDEX_INVALID != adj_mcasts[proto][sw_if_index]) {
                n_adjs++;
            }
        }
...
adj_mcast_module_init(void)
{
    ...
    dpo_register(DPO_ADJACENCY_MCAST_MIDCHAIN, &adj_mcast_midchain_dpo_vft, adj_mcast_midchain_nodes);
}
