...
int
span_add_delete_entry(vlib_main_t *vm, u32 src_sw_if_index, u32 dst_sw_if_index, u8 state, span_feat_t sf)
{
    ...
    case SPAN_FEAT_DEVICE:
        if (enable_rx || disable_rx)
            vnet_feature_enable_disable("device-input", "span-input", src_sw_if_index, rx, 0, 0);
        if (enable_tx || disable_tx)
            vnet_feature_enable_disable("interface-output", "span-output", src_sw_if_index, tx, 0, 0);
        break;
    case SPAN_FEAT_L2:
        if (enable_rx || disable_rx)
...
static clib_error_t *
set_interface_span_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    span_main_t *sm     = &span_main;
...
static clib_error_t *
show_interfaces_span_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    span_main_t *sm = &span_main;
    span_interface_t *si;
    ...
    vec_foreach(si, sm->interfaces)
    {
        ...
        if (drxm->num_mirror_ports || dtxm->num_mirror_ports || lrxm->num_mirror_ports || ltxm->num_mirror_ports) {
            ...
            if (header) {
                vlib_cli_output(vm, "%-20s %-20s  %6s   %6s", "Source", "Destination", "Device", "L2");
                header = 0;
            }
            ...
            clib_bitmap_foreach(i, b, ({
                                    ...
                                    vlib_cli_output(vm, "%-20v %-20U (%6s) (%6s)", s, format_vnet_sw_if_index_name, vnm,
                                                    i, states[device], states[l2]);
                                    vec_reset_length(s);
                                }));
            clib_bitmap_free(b);
            clib_bitmap_free(l);
