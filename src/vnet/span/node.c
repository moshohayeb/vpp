...
format_span_trace(u8 *s, va_list *args)
{
    ...
    s                = format(s, "SPAN: mirrored %U -> %U", format_vnet_sw_if_index_name, vnm, t->src_sw_if_index,
...
span_mirror(vlib_main_t *vm, vlib_node_runtime_t *node, u32 sw_if_index0, vlib_buffer_t *b0,
            vlib_frame_t **mirror_frames, vlib_rx_or_tx_t rxtx, span_feat_t sf)
{
    vlib_buffer_t *c0;
...
span_node_inline_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vlib_rx_or_tx_t rxtx, span_feat_t sf)
{
    span_main_t *sm  = &span_main;
    vnet_main_t *vnm = &vnet_main;
    ...
    vlib_node_increment_counter(vm, span_node.index, SPAN_ERROR_HITS, n_span_packets);

    return frame->n_vectors;
}

static uword
span_device_input_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return span_node_inline_fn(vm, node, frame, VLIB_RX, SPAN_FEAT_DEVICE);
}

static uword
span_device_output_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return span_node_inline_fn(vm, node, frame, VLIB_TX, SPAN_FEAT_DEVICE);
}

static uword
span_l2_input_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return span_node_inline_fn(vm, node, frame, VLIB_RX, SPAN_FEAT_L2);
}

static uword
span_l2_output_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return span_node_inline_fn(vm, node, frame, VLIB_TX, SPAN_FEAT_L2);
}

#define span_node_defs                                                                                                 \
    .vector_size = sizeof(u32), .format_trace = format_span_trace, .type = VLIB_NODE_TYPE_INTERNAL,                    \
    .n_errors = ARRAY_LEN(span_error_strings), .error_strings = span_error_strings, .n_next_nodes = 0,                 \
...
span_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, span_l2_input_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(),
                                sm->l2_input_next);

    feat_bitmap_init_next_nodes(vm, span_l2_output_node.index, L2OUTPUT_N_FEAT, l2output_get_feat_names(),
                                sm->l2_output_next);
    return 0;
}
