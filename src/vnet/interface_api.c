...
static void
vl_api_sw_interface_set_ip_directed_broadcast_t_handler(vl_api_sw_interface_set_ip_directed_broadcast_t *mp)
{
...
static void
send_sw_interface_details(vpe_api_main_t *am, vl_api_registration_t *rp, vnet_sw_interface_t *swif, u8 *interface_name,
                          u32 context)
{
    ...
    mp->link_duplex        = ((hi->flags & VNET_HW_INTERFACE_FLAG_DUPLEX_MASK) >> VNET_HW_INTERFACE_FLAG_DUPLEX_SHIFT);
    mp->link_speed         = ((hi->flags & VNET_HW_INTERFACE_FLAG_SPEED_MASK) >> VNET_HW_INTERFACE_FLAG_SPEED_SHIFT);
    ...
    if (swif->sup_sw_if_index == swif->sw_if_index && hi->hw_class_index == ethernet_hw_interface_class.index) {
        ...
        if (l2vtr_get(am->vlib_main, am->vnet_main, swif->sw_if_index, &vtr_op, &vtr_push_dot1q, &vtr_tag1, &vtr_tag2) !=
            0) {
    ...
    if (!l2pbb_get(am->vlib_main, am->vnet_main, swif->sw_if_index, &vtr_op, &outer_tag, &eth_hdr, &b_vlanid, &i_sid)) {
        mp->sub_dot1ah = 1;
        clib_memcpy(mp->b_dmac, eth_hdr.dst_address, sizeof(eth_hdr.dst_address));
        clib_memcpy(mp->b_smac, eth_hdr.src_address, sizeof(eth_hdr.src_address));
...
static void
vl_api_sw_interface_add_del_address_t_handler(vl_api_sw_interface_add_del_address_t *mp)
{
    ...
    if (mp->del_all)
        ...
        error =
            ip6_add_del_interface_address(vm, ntohl(mp->sw_if_index), (void *) mp->address, mp->address_length, is_del);
    else
        error =
            ip4_add_del_interface_address(vm, ntohl(mp->sw_if_index), (void *) mp->address, mp->address_length, is_del);
...
ip_table_bind(fib_protocol_t fproto, u32 sw_if_index, u32 table_id, u8 is_api)
{
    ...
    if (FIB_PROTOCOL_IP6 == fproto) {
         ...
         */
        /* *INDENT-OFF* */
        foreach_ip_interface_address(&ip6_main.lookup_main, ia, sw_if_index, 1 /* honor unnumbered */,
        ...
        if (0 == table_id) {
            ...
            if (0 != ip6_main.fib_index_by_sw_if_index[sw_if_index])
                fib_table_unlock(ip6_main.fib_index_by_sw_if_index[sw_if_index], FIB_PROTOCOL_IP6, src);
            if (0 != ip6_main.mfib_index_by_sw_if_index[sw_if_index])
                mfib_table_unlock(ip6_main.mfib_index_by_sw_if_index[sw_if_index], FIB_PROTOCOL_IP6, msrc);
         ...
         */
        /* *INDENT-OFF* */
        foreach_ip_interface_address(&ip4_main.lookup_main, ia, sw_if_index, 1 /* honor unnumbered */,
                                     ({ return (VNET_API_ERROR_ADDRESS_FOUND_FOR_INTERFACE); }));
        /* *INDENT-ON* */
        ...
        if (0 == table_id) {
            ...
            if (0 != ip4_main.fib_index_by_sw_if_index[sw_if_index])
                fib_table_unlock(ip4_main.fib_index_by_sw_if_index[sw_if_index], FIB_PROTOCOL_IP4, src);
            if (0 != ip4_main.mfib_index_by_sw_if_index[sw_if_index])
                mfib_table_unlock(ip4_main.mfib_index_by_sw_if_index[sw_if_index], FIB_PROTOCOL_IP4, msrc);

        } else {
            /* we need to lock the table now it's inuse */
            fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, table_id, src);

            mfib_index = mfib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, table_id, msrc);
        }
...
static void
send_sw_interface_get_table_reply(vl_api_registration_t *reg, u32 context, int retval, u32 vrf_id)
{
...
static void
vl_api_sw_interface_set_unnumbered_t_handler(vl_api_sw_interface_set_unnumbered_t *mp)
{
    ...
    vnet_sw_interface_update_unnumbered(unnumbered_sw_if_index, sw_if_index, mp->is_add);
done:
    REPLY_MACRO(VL_API_SW_INTERFACE_SET_UNNUMBERED_REPLY);
}

static void
vl_api_sw_interface_clear_stats_t_handler(vl_api_sw_interface_clear_stats_t *mp)
{
...
static void
send_sw_interface_event(vpe_api_main_t *am, vpe_client_registration_t *reg, vl_api_registration_t *vl_reg,
                        vnet_sw_interface_t *swif)
{
    vl_api_sw_interface_event_t *mp;
    vnet_main_t *vnm = am->vnet_main;

    vnet_hw_interface_t *hi = vnet_get_sup_hw_interface(vnm, swif->sw_if_index);
    mp                      = vl_msg_api_alloc(sizeof(*mp));
    memset(mp, 0, sizeof(*mp));
...
link_state_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        vlib_process_get_events_with_type(vm, &event_data, API_LINK_STATE_EVENT);
        vlib_process_get_events_with_type(vm, &event_data, API_ADMIN_UP_DOWN_EVENT);
        ...
        for (i = 0; i < vec_len(event_data); i++) {
            ...
            pool_foreach(reg, vam->interface_events_registrations, ({
                             ...
                             if (vl_reg) {
                                 ...
                                 if (!pool_is_free_index(vnm->interface_main.sw_interfaces, event_data[i])) {
...
link_up_down_function(vnet_main_t *vm, u32 hw_if_index, u32 flags)
{
    ...
    if (vam->link_state_process_up)
        vlib_process_signal_event(vam->vlib_main, link_state_process_node.index, API_LINK_STATE_EVENT, hi->sw_if_index);
    return 0;
}
...
admin_up_down_function(vnet_main_t *vm, u32 sw_if_index, u32 flags)
{
     ...
     */
    if (vam->link_state_process_up)
        vlib_process_signal_event(vam->vlib_main, link_state_process_node.index, API_ADMIN_UP_DOWN_EVENT, sw_if_index);
    return 0;
}

static void
vl_api_sw_interface_tag_add_del_t_handler(vl_api_sw_interface_tag_add_del_t *mp)
{
    vnet_main_t *vnm = vnet_get_main();
...
static void
vl_api_sw_interface_set_mac_address_t_handler(vl_api_sw_interface_set_mac_address_t *mp)
{
    ...
    error = vnet_hw_interface_change_mac_address(vnm, si->hw_if_index, mp->mac_address);
    if (error) {
...
static void
vl_api_sw_interface_get_mac_address_t_handler(vl_api_sw_interface_get_mac_address_t *mp)
{
...
static void
vl_api_sw_interface_set_rx_mode_t_handler(vl_api_sw_interface_set_rx_mode_t *mp)
{
    ...
    error = set_hw_interface_change_rx_mode(vnm, si->hw_if_index, mp->queue_id_valid, ntohl(mp->queue_id), mp->mode);
    if (error) {
        rv = VNET_API_ERROR_UNIMPLEMENTED;
        clib_error_report(error);
        goto out;
...
static void
send_interface_rx_placement_details(vpe_api_main_t *am, vl_api_registration_t *rp, u32 sw_if_index, u32 worker_id,
...
static void
vl_api_sw_interface_rx_placement_dump_t_handler(vl_api_sw_interface_rx_placement_dump_t *mp)
{
    vnet_main_t *vnm   = vnet_get_main();
    vpe_api_main_t *am = &vpe_api_main;
    u32 sw_if_index    = ntohl(mp->sw_if_index);
    ...
    if (sw_if_index == ~0) {
        ...
        vlib_node_t *pn = vlib_get_node_by_name(am->vlib_main, (u8 *) "device-input");
        uword si;
        int index = 0;
        ...
        foreach_vlib_main(({
            clib_bitmap_foreach(si, pn->sibling_bitmap, ({
                                    ...
                                    {
                                        vnet_hw_interface_t *hw = vnet_get_hw_interface(vnm, dq->hw_if_index);
        ...
        for (i = 0; i < vec_len(hw->dq_runtime_index_by_queue); i++) {
            send_interface_rx_placement_details(am, reg, hw->sw_if_index, hw->input_node_thread_index_by_queue[i], i,
                                                hw->rx_mode_by_queue[i], mp->context);
        }
    }
...
static void
vl_api_sw_interface_set_rx_placement_t_handler(vl_api_sw_interface_set_rx_placement_t *mp)
{
...
out:
    ...
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_CREATE_SUBIF_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
    /* *INDENT-ON* */
}
...
static void
vl_api_create_loopback_instance_t_handler(vl_api_create_loopback_instance_t *mp)
{
    ...
    rv = vnet_create_loopback_interface(&sw_if_index, mp->mac_address, is_specified, user_instance);
...
static void
vl_api_collect_detailed_interface_stats_t_handler(vl_api_collect_detailed_interface_stats_t *mp)
{
