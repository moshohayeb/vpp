...
static void
send_client_message_start_stop(u32 sw_if_index, u32 server_index, u8 msg_type, address_info_t *address_list, u8 start)
{
...
dhcp6_reply_event_handler(vl_api_dhcp6_reply_event_t *mp)
{
    ...
    if (inner_status_code == DHCPV6_STATUS_NOADDRS_AVAIL) {
        ...
        if (n_addresses > 0) {
            clib_warning("Invalid Reply message arrived: It contains NoAddrsAvail "
...
static uword
dhcp6_client_cp_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    dhcp6_client_cp_main_t *rm = &dhcp6_client_cp_main;
...
static clib_error_t *
dhcp6_addresses_show_command_function(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    pool_foreach(address_info, dm->address_pool, ({
                     vlib_cli_output(vm,
                                     ...
                                     "(%f remaining)",
                                     format_ip6_address, &address_info->address, address_info->preferred_lt,
                                     address_info->valid_lt, address_info->due_time - current_time);
                 }));
    /* *INDENT-ON* */
...
static clib_error_t *
dhcp6_clients_show_command_function(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dhcp6_client_cp_main_t *rm = &dhcp6_client_cp_main;
    clib_error_t *error        = 0;
    client_state_t *cs;
        ...
        if (cs->enabled) {
            ...
            if (cs->T1)
                vlib_cli_output(vm,
                                ...
                                i, cs->T1, buf1, cs->T2, buf2, cs->server_index, rebinding);
            else
...
static clib_error_t *
dhcp6_client_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dhcp6_client_cp_main_t *rm = &dhcp6_client_cp_main;
    vnet_main_t *vnm           = rm->vnet_main;
    clib_error_t *error        = 0;
...
static void
vl_api_dhcp6_client_enable_disable_t_handler(vl_api_dhcp6_client_enable_disable_t *mp)
{
