...
active_prefix_index_by_prefix_group_index_get(u32 prefix_group_index)
{
    ...
    if (prefix_group_index >= vec_len(apm->active_prefix_index_by_prefix_group_index))
        return ~0;
...
active_prefix_index_by_prefix_group_index_set(u32 prefix_group_index, u32 prefix_index)
{
    ip6_address_with_prefix_main_t *apm = &ip6_address_with_prefix_main;
    ...
    if (prefix_index == ~0 && prefix_group_index >= vec_len(apm->active_prefix_index_by_prefix_group_index))
        return;
...
static void
send_client_message_start_stop(u32 sw_if_index, u32 server_index, u8 msg_type, prefix_info_t *prefix_list, u8 start)
{
    dhcp6_pd_client_cp_main_t *rm                = &dhcp6_pd_client_cp_main;
...
dhcp6_pd_reply_event_handler(vl_api_dhcp6_pd_reply_event_t *mp)
{
    ...
    if (mp->msg_type == DHCPV6_MSG_ADVERTISE && client_state->server_index == ~0) {
    ...
    if (inner_status_code == DHCPV6_STATUS_NOPREFIX_AVAIL) {
        ...
        if (n_prefixes > 0) {
            clib_warning("Invalid Reply message arrived: It contains NoPrefixAvail "
...
static uword
dhcp6_pd_client_cp_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    dhcp6_pd_client_cp_main_t *rm = &dhcp6_pd_client_cp_main;
    ip6_prefix_main_t *pm         = &ip6_prefix_main;
...
static u32
cp_ip6_construct_address(ip6_address_info_t *address_info, u32 prefix_index, ip6_address_t *r_addr)
{
    ip6_prefix_main_t *pm = &ip6_prefix_main;
    ...
    if (is_add && !address_info->configured_in_data_plane) {
        if (prefix_index != ~0) {
            if (cp_ip6_construct_address(address_info, prefix_index, &addr) != 0)
                return;
            error = ip6_add_del_interface_address(vm, address_info->sw_if_index, &addr, address_info->prefix_length,
                                                  0 /* add */);
            if (error)
            ...
            if (address_info->prefix_group_index == ~0) {
                error = ip6_add_del_interface_address(vm, address_info->sw_if_index, &address_info->address,
        ...
        if (prefix_index == ~0) {
            if (address_info->prefix_group_index == ~0) {
                ...
                if (error)
                    clib_warning("Failed deleting IPv6 address: %U", format_clib_error, error);
...
static u32
cp_ip6_address_find_new_active_prefix(u32 prefix_group_index, u32 ignore_prefix_index)
{
    ip6_prefix_main_t *pm = &ip6_prefix_main;
    prefix_info_t *prefix_info;
...
static int
cp_ip6_address_add_del(u32 sw_if_index, const u8 *prefix_group, ip6_address_t address, u8 prefix_length, u8 is_add)
{
...
static void
vl_api_ip6_add_del_address_using_prefix_t_handler(vl_api_ip6_add_del_address_using_prefix_t *mp)
{
    ...
    rv = cp_ip6_address_add_del(sw_if_index, mp->prefix_group, address, prefix_length, mp->is_add);
...
static clib_error_t *
cp_ip6_address_add_del_command_function(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm    = vnet_get_main();
    clib_error_t *error = 0;
    u32 sw_if_index     = ~0;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "%U", unformat_vnet_sw_interface, vnm, &sw_if_index))
            ...
            ;
        else if (unformat(input, "%U/%d", unformat_ip6_address, &address, &prefix_length))
            address_set = 1;
        else if (unformat(input, "del"))
            add = 0;
        else {
            error = clib_error_return(0, "unexpected input `%U'", format_unformat_error, input);
            goto done;
        }
    }
    ...
    if (sw_if_index == ~0)
        ...
        if (cp_ip6_address_add_del(sw_if_index, prefix_group, address, prefix_length, add) != 0)
...
static clib_error_t *
cp_ip6_addresses_show_command_function(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ip6_address_with_prefix_main_t *apm = &ip6_address_with_prefix_main;
    ip6_prefix_main_t *pm               = &ip6_prefix_main;
    ip6_address_info_t *address_info;
    ...
    for (i = 0; i < vec_len(apm->addresses); i++) {
        ...
        vlib_cli_output(vm, "sw_if_index: %u, prefix_group: %s, address: %U/%d", address_info->sw_if_index,
                        prefix_group, format_ip6_address, &address_info->address, address_info->prefix_length);
    }
...
static clib_error_t *
cp_ip6_prefixes_show_command_function(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ip6_prefix_main_t *pm = &ip6_prefix_main;
    clib_error_t *error   = 0;
    prefix_info_t *prefix_info;
...
static clib_error_t *
ip6_pd_clients_show_command_function(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dhcp6_pd_client_cp_main_t *rm = &dhcp6_pd_client_cp_main;
    ip6_prefix_main_t *pm         = &ip6_prefix_main;
    clib_error_t *error           = 0;
    ...
    for (i = 0; i < vec_len(rm->client_state_by_sw_if_index); i++) {
        ...
        if (cs->enabled) {
            if (cs->T1_due_time != DBL_MAX && cs->T1_due_time > current_time) {
                sprintf(buf1, "%u remaining", (u32) round(cs->T1_due_time - current_time));
            } else
                sprintf(buf1, "timeout");
            if (cs->T2_due_time != DBL_MAX && cs->T2_due_time > current_time)
                sprintf(buf2, "%u remaining", (u32) round(cs->T2_due_time - current_time));
            else
            ...
            if (cs->T1)
                vlib_cli_output(vm,
                                ...
                                "T2: %u (%s), server index: %d%s",
                                i, prefix_group, cs->T1, buf1, cs->T2, buf2, cs->server_index, rebinding);
            else
...
static int
dhcp6_pd_client_enable_disable(u32 sw_if_index, const u8 *prefix_group, u8 enable)
{
    dhcp6_pd_client_cp_main_t *rm = &dhcp6_pd_client_cp_main;
    ip6_prefix_main_t *pm         = &ip6_prefix_main;
    ...
    if (enable) {
        if (strnlen((const char *) prefix_group, 64) == 64 || prefix_group[0] == '\0')
            return VNET_API_ERROR_INVALID_VALUE;
        prefix_group_index = prefix_group_find_or_create(prefix_group, !old_enabled);
        if (old_enabled && prefix_group_index != client_state->prefix_group_index)
            return VNET_API_ERROR_INVALID_VALUE;
    }
    ...
    if (!old_enabled && enable) {
        ...
        pool_foreach(prefix_info, pm->prefix_pool, ({
                         if (is_dhcpv6_pd_prefix(prefix_info) && prefix_info->opaque_data == sw_if_index) {
                             ...
                             send_client_message_start_stop(sw_if_index, client_state->server_index, DHCPV6_MSG_RELEASE,
                                                            prefix_list, 1);
                             u32 prefix_index = prefix_info - pm->prefix_pool;
                             notify_prefix_add_del(prefix_index, 0);
                             set_is_dhcpv6_pd_prefix(prefix_info, 0);
...
static clib_error_t *
dhcp6_pd_client_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dhcp6_pd_client_cp_main_t *rm = &dhcp6_pd_client_cp_main;
    vnet_main_t *vnm              = rm->vnet_main;
    clib_error_t *error           = 0;
...
static void
vl_api_dhcp6_pd_client_enable_disable_t_handler(vl_api_dhcp6_pd_client_enable_disable_t *mp)
{
