...
create_buffer_for_client_message(vlib_main_t *vm, u32 sw_if_index, dhcp6_ia_na_client_state_t *client_state, u32 type)
{
    ...
    if (type == DHCPV6_MSG_SOLICIT || type == DHCPV6_MSG_REQUEST || type == DHCPV6_MSG_RENEW ||
        ...
        ia_hdr->opt.length =
            clib_host_to_net_u16(sizeof(*ia_hdr) + n_addresses * sizeof(*opt_addr) - sizeof(ia_hdr->opt));
...
static inline u8
check_send_client_message(vlib_main_t *vm, dhcp6_ia_na_client_state_t *client_state, f64 current_time, f64 *due_time)
{
    vlib_buffer_t *p0;
    vlib_frame_t *f;
    u32 *to_next;
        ...
        do {
            ...
            for (i = 0; i < vec_len(cm->client_state_by_sw_if_index); i++) {
                ...
                if (!client_state->entry_valid)
                    continue;
                if (check_send_client_message(vm, client_state, current_time, &dt) && (dt < due_time))
                    due_time = dt;
            }
            current_time = vlib_time_now(vm);
...
void
dhcp6_send_client_message(vlib_main_t *vm, u32 sw_if_index, u8 stop, dhcp6_send_client_message_params_t *params)
{
    dhcp6_ia_na_client_main_t *cm            = &dhcp6_ia_na_client_main;
...
void
vl_api_dhcp6_send_client_message_t_handler(vl_api_dhcp6_send_client_message_t *mp)
{
...
clib_error_t *
call_dhcp6_reply_event_callbacks(void *data, _vnet_dhcp6_reply_event_function_list_elt_t *elt)
{
...
static uword
dhcp6_reply_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        if (event_type == DHCP6_DP_REPLY_REPORT) {
            ...
            for (i = 0; i < vec_len(events); i++) {
                u32 event_size = sizeof(vl_api_dhcp6_reply_event_t) +
                                 vec_len(events[i].addresses) * sizeof(vl_api_dhcp6_address_info_t);
...
void
vl_api_want_dhcp6_reply_events_t_handler(vl_api_want_dhcp6_reply_events_t *mp)
{
