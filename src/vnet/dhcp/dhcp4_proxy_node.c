...
#define VPP_DHCP_OPTION82_SIZE                                                                                         \
    (VPP_DHCP_OPTION82_SUB1_SIZE + VPP_DHCP_OPTION82_SUB5_SIZE + VPP_DHCP_OPTION82_VSS_SIZE + 3)
...
static uword
dhcp_proxy_to_server_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            fib_index = im->fib_index_by_sw_if_index[vnet_buffer(b0)->sw_if_index[VLIB_RX]];
        ...
        do_trace:
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ...
                tr->sw_if_index          = sw_if_index;
                if (next0 == DHCP_PROXY_TO_SERVER_INPUT_NEXT_LOOKUP)
                    tr->trace_ip4_address.as_u32 = proxy->dhcp_servers[0].dhcp_server.ip4.as_u32;
            }
    ...
    vlib_node_increment_counter(vm, dhcp_proxy_to_server_node.index, DHCP_PROXY_ERROR_NO_INTERFACE_ADDRESS,
                                pkts_no_interface_address);
...
static uword
dhcp_proxy_to_client_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
        ...
        if (1 /* dpm->insert_option_82 */) {
            ...
            while (o->option != 0xFF /* end of options */ &&
                ...
                if (o->option == 82) {
                    ...
                    while (sub->option != 0xFF /* end of options */ && (u8 *) sub < (u8 *) (o + o->length)) {
                        /* If this is one of ours, it will have
                           total length 12, circuit-id suboption type,
                           and the sw_if_index */
                        if (sub->option == 1 && sub->length == 4) {
                            sw_if_index =
                                ((sub->data[0] << 24) | (sub->data[1] << 16) | (sub->data[2] << 8) | (sub->data[3]));
                            ...
                            vss_ctrl = 1;
                        sub = (dhcp_option_t *) (((uword) sub) + (sub->length + 2));
                    }
    ...
    server_found:
        ...
        sum0 =
            ip_csum_update(sum0, old0, new0, ip4_header_t /* structure */, dst_address /* offset of changed member */);
        ip0->checksum = ip_csum_fold(sum0);
        ...
        ip0->src_address.as_u32 = new0;
        sum0 =
            ip_csum_update(sum0, old0, new0, ip4_header_t /* structure */, src_address /* offset of changed member */);
        ip0->checksum = ip_csum_fold(sum0);
        ...
        mac0->type =
            (si0->type == VNET_SW_INTERFACE_TYPE_SUB) ? clib_net_to_host_u16(0x8100) : clib_net_to_host_u16(0x0800);
...
int
dhcp4_proxy_set_server(ip46_address_t *addr, ip46_address_t *src_addr, u32 rx_table_id, u32 server_table_id, int is_del)
{
    u32 rx_fib_index = 0;
    int rc           = 0;
...
static clib_error_t *
dhcp4_proxy_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
format_dhcp4_proxy_server(u8 *s, va_list *args)
{
    ...
    vec_foreach(server, proxy->dhcp_servers)
    {
        ...
        s = format(s, "%u,%U  ", server_fib->table_id, format_ip46_address, &server->dhcp_server, IP46_TYPE_ANY);
    }
    return s;
}
...
static clib_error_t *
dhcp4_proxy_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
dhcp_option_82_vss_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
dhcp_vss_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
dhcp_option_82_address_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm = vnet_get_main();
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "%U", unformat_vnet_sw_interface, vnm, &sw_if_index0)) {
            ...
            if (ia0) {
                ...
                vlib_cli_output(vm, "%=20U%=20U", format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip4_address,
                                ia0);
            } else
                vlib_cli_output(vm, "%=34s %=20U", "No IPv4 address configured on", format_vnet_sw_if_index_name, vnm,
                                sw_if_index);
        } else
            break;
    }
