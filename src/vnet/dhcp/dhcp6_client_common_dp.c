...
dhcpv6_client_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            u32 dhcpv6_ip6_palyoad_offset = (u8 *) dhcpv60 - ((u8 *) ip0 + sizeof(*ip0));
            options_length                = ntohs(ip0->payload_length) - dhcpv6_ip6_palyoad_offset - sizeof(*dhcpv60);
            ...
            case DHCPV6_MSG_REPLY:
                ...
                while (options_length > 0) {
                    if (options_length < ntohs(option->length) + sizeof(*option)) {
                        clib_warning("remaining payload length < option length (%d < %d)", options_length,
                    ...
                    if (oo == DHCPV6_OPTION_IA_NA || oo == DHCPV6_OPTION_IA_PD) {
                        ...
                        u16 inner_options_length = ntohs(option->length) - (sizeof(*ia_header) - sizeof(dhcpv6_option_t));
                        while (inner_options_length > 0) {
                            ...
                            if (discard_option)
                                ...
                                address_info->preferred_time       = ntohl(iaaddr->preferred);
                                address_info->valid_time           = ntohl(iaaddr->valid);
                                ...
                                prefix_info->preferred_time      = ntohl(iaprefix->preferred);
                                prefix_info->valid_time          = ntohl(iaprefix->valid);
                                ...
                                report.inner_status_code = ntohs(sc->status_code);
                            }
                            inner_options_length -= sizeof(*inner_option) + ntohs(inner_option->length);
                            inner_option =
                                (void *) ((u8 *) inner_option + sizeof(*inner_option) + ntohs(inner_option->length));
                        }
                        ...
                        if (client_id_present) {
                            ...
                            if (len != CLIENT_DUID_LENGTH ||
                                0 != memcmp(option->data, client_duid.bin_string, CLIENT_DUID_LENGTH)) {
                    ...
                    option = (void *) ((u8 *) option + sizeof(*option) + ntohs(option->length));
                }
...
void
vl_api_dhcp6_clients_enable_disable_t_handler(vl_api_dhcp6_clients_enable_disable_t *mp)
{
