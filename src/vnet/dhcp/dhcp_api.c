...
vl_api_dhcp_proxy_set_vss_t_handler(vl_api_dhcp_proxy_set_vss_t *mp)
{
    ...
    rv = dhcp_proxy_set_vss((mp->is_ipv6 ? FIB_PROTOCOL_IP6 : FIB_PROTOCOL_IP4), ntohl(mp->tbl_id), mp->vss_type,
                            vpn_ascii_id, ntohl(mp->oui), ntohl(mp->vpn_index), mp->is_add == 0);
...
static void
vl_api_dhcp_proxy_config_t_handler(vl_api_dhcp_proxy_config_t *mp)
{
    vl_api_dhcp_proxy_set_vss_reply_t *rmp;
    ...
    if (mp->is_ipv6) {
        ...
        rv = dhcp6_proxy_set_server(&server, &src, (u32) ntohl(mp->rx_vrf_id), (u32) ntohl(mp->server_vrf_id),
                                    (int) (mp->is_add == 0));
    } else {
        ...
        rv = dhcp4_proxy_set_server(&server, &src, (u32) ntohl(mp->rx_vrf_id), (u32) ntohl(mp->server_vrf_id),
                                    (int) (mp->is_add == 0));
    }
...
void
dhcp_send_details(fib_protocol_t proto, void *opaque, u32 context, dhcp_proxy_t *proxy)
{
    ...
    if (vss) {
        ...
        if (vss->vss_type == VSS_TYPE_ASCII) {
            ...
            u32 fib_id = ((u32) vss->vpn_id[3] << 24) + ((u32) vss->vpn_id[4] << 16) + ((u32) vss->vpn_id[5] << 8) +
                         ((u32) vss->vpn_id[6]);
...
static void
dhcp_client_lease_encode(vl_api_dhcp_lease_t *lease, const dhcp_client_t *client)
{
    size_t len;
...
static void
dhcp_client_data_encode(vl_api_dhcp_client_t *vclient, const dhcp_client_t *client)
{
    size_t len;
...
static void
vl_api_dhcp_client_config_t_handler(vl_api_dhcp_client_config_t *mp)
{
    ...
    rv = dhcp_client_config(mp->is_add, mp->client_index, vm, sw_if_index, mp->client.hostname, mp->client.id,
