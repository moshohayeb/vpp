...
dhcp_client_acquire_address(dhcp_client_main_t *dcm, dhcp_client_t *c)
{
     ...
     */
    ip4_add_del_interface_address(dcm->vlib_main, c->sw_if_index, (void *) &c->leased_address, c->subnet_mask_width,
                                  0 /*is_del */);
}
...
set_l2_rewrite(dhcp_client_main_t *dcm, dhcp_client_t *c)
{
    ...
    c->l2_rewrite =
        vnet_build_rewrite_for_sw_interface(dcm->vnet_main, c->sw_if_index, VNET_LINK_IP4, 0 /* broadcast */);
}
...
dhcp_client_addr_callback(dhcp_client_t *c)
{
    ...
    if (c->router_address.as_u32) {
        ...
        /* *INDENT-OFF* */
        fib_table_entry_path_add(fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP4, c->sw_if_index), &all_0s,
                                 FIB_SOURCE_DHCP, FIB_ENTRY_FLAG_NONE, DPO_PROTO_IP4, &nh, c->sw_if_index, ~0, 1,
                                 NULL,   // no label stack
 ...
 */
int
dhcp_client_for_us(u32 bi, vlib_buffer_t *b, ip4_header_t *ip, udp_header_t *udp, dhcp_header_t *dhcp)
{
    dhcp_client_main_t *dcm = &dhcp_client_main;
    ...
    case DHCP_REQUEST:
        if (dhcp_message_type == DHCP_PACKET_NAK) {
            ...
            if (c->state == DHCP_BOUND) {
                ip4_add_del_interface_address(dcm->vlib_main, c->sw_if_index, (void *) &c->leased_address,
                                              c->subnet_mask_width, 1 /*is_del */);
                vnet_feature_enable_disable("ip4-unicast", "ip4-dhcp-client-detect", c->sw_if_index, 1 /* enable */, 0,
                                            0);
        ...
        if (dhcp_message_type != DHCP_PACKET_ACK && dhcp_message_type != DHCP_PACKET_OFFER) {
            vlib_node_increment_counter(vm, dhcp_client_process_node.index, DHCP_STAT_NON_OFFER_DISCOVER, 1);
            clib_warning("sw_if_index %d state %U message type %d", c->sw_if_index, format_dhcp_client_state, c->state,
                         dhcp_message_type);
            c->next_transmit = now + 5.0;
            break;
        }
        /* OK, we own the address (etc), add to the routing table(s) */
        vl_api_rpc_call_main_thread(dhcp_client_addr_callback, (u8 *) c, sizeof(*c));
...
static void
send_dhcp_pkt(dhcp_client_main_t *dcm, dhcp_client_t *c, dhcp_packet_type_t type, int is_broadcast)
{
    vlib_main_t *vm             = dcm->vlib_main;
    ...
    if (is_broadcast) {
        ...
        ip = (void *) (((u8 *) vlib_buffer_get_current(b)) + vec_len(c->l2_rewrite));
    } else {
    ...
    if (vec_len(c->client_identifier)) {
        ...
        clib_memcpy(o->data, c->client_identifier, vec_len(c->client_identifier));
        o = (dhcp_option_t *) (((uword) o) + (o->length + 2));
    }
...
dhcp_bound_state(dhcp_client_main_t *dcm, dhcp_client_t *c, f64 now)
{
    ...
    if (now > c->lease_expires) {
        ...
        if (c->router_address.as_u32) {
            ...
            fib_table_entry_path_remove(fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP4, c->sw_if_index), &all_0s,
                                        FIB_SOURCE_DHCP, DPO_PROTO_IP4, &nh, c->sw_if_index, ~0, 1,
                                        FIB_ROUTE_PATH_FLAG_NONE);
        }
...
static uword
dhcp_client_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    f64 timeout = 100.0;
    f64 now;
    uword event_type;
    ...
    while (1) {
        ...
        case ~0:
            /* *INDENT-OFF* */
            pool_foreach(c, dcm->clients, ({ timeout = dhcp_client_sm(now, timeout, (uword)(c - dcm->clients)); }));
            /* *INDENT-ON* */
...
format_dhcp_client(u8 *s, va_list *va)
{
    ...
    s = format(s, "[%d] %U state %U ", c - dcm->clients, format_vnet_sw_if_index_name, dcm->vnet_main, c->sw_if_index,
               format_dhcp_client_state, c->state);

    if (c->leased_address.as_u32)
        s = format(s, "addr %U/%d gw %U\n", format_ip4_address, &c->leased_address, c->subnet_mask_width,
                   format_ip4_address, &c->router_address);
    else
        s = format(s, "no address\n");

    if (verbose) {
        s = format(s, "retry count %d, next xmt %.2f", c->retry_count, c->next_transmit);
    }
    return s;
}

static clib_error_t *
show_dhcp_client_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dhcp_client_main_t *dcm = &dhcp_client_main;
    dhcp_client_t *c;
    int verbose     = 0;
    ...
    /* *INDENT-OFF* */
    pool_foreach(c, dcm->clients, ({ vlib_cli_output(vm, "%U", format_dhcp_client, dcm, c, verbose); }));
    /* *INDENT-ON* */
...
int
dhcp_client_config(u32 is_add, u32 client_index, vlib_main_t *vm, u32 sw_if_index, u8 *hostname, u8 *client_id,
                   dhcp_event_cb_t event_callback, u8 set_broadcast_flag, u32 pid)
{
    ...
    case VNET_API_ERROR_INVALID_VALUE:
        ...
        if (is_add)
            clib_warning("dhcp client already enabled on intf_idx %d", sw_if_index);
        else
            clib_warning("dhcp client not enabled on on intf_idx %d", sw_if_index);
        break;
...
static clib_error_t *
dhcp_client_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    case VNET_API_ERROR_INVALID_VALUE:
        ...
        vec_free(a->option_55_data);
        if (is_add)
            return clib_error_return(0, "dhcp client already enabled on %U", format_vnet_sw_if_index_name,
                                     dcm->vnet_main, sw_if_index);
        else
            return clib_error_return(0, "dhcp client not enabled on %U", format_vnet_sw_if_index_name, dcm->vnet_main,
                                     sw_if_index);
        break;
