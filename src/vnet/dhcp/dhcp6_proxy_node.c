...
static uword
dhcpv6_proxy_to_server_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (h0->msg_type == DHCPV6_MSG_RELAY_FORW &&
                ((ip0->src_address.as_u8[0] & 0xe0) == 0x20 || (ip0->src_address.as_u8[0] & 0xfe) == 0xfc)) {
        ...
        link_address_set:
            ...
            if ((b0->current_length + sizeof(*id1) + sizeof(*vss1) + sizeof(*cmac)) > fl->n_data_bytes) {
            ...
            u1->length = clib_host_to_net_u16(clib_net_to_host_u16(fwd_opt->length) + sizeof(*r1) + sizeof(*fwd_opt) +
            ...
            src = (proxy->dhcp_src_address.ip6.as_u64[0] || proxy->dhcp_src_address.ip6.as_u64[1]) ?
            ...
            if (is_solicit && vec_len(proxy->dhcp_servers) > 1) {
                ...
                for (ii = 1; ii < vec_len(proxy->dhcp_servers); ii++) {
                    ...
                    src = ((server->dhcp_server.ip6.as_u64[0] || server->dhcp_server.ip6.as_u64[1]) ?
    ...
    vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_RELAY_TO_CLIENT,
                                pkts_to_client);
    vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_RELAY_TO_SERVER,
                                pkts_to_server);
    vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_NO_INTERFACE_ADDRESS,
                                pkts_no_interface_address);
    vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_WRONG_MESSAGE_TYPE,
                                pkts_wrong_msg_type);
    vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_NO_SRC_ADDRESS,
                                pkts_no_src_address);
...
static uword
dhcpv6_proxy_to_client_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    server_found:
        ...
        mac0->type =
            (si0->type == VNET_SW_INTERFACE_TYPE_SUB) ? clib_net_to_host_u16(0x8100) : clib_net_to_host_u16(0x86dd);
...
int
dhcp6_proxy_set_server(ip46_address_t *addr, ip46_address_t *src_addr, u32 rx_table_id, u32 server_table_id, int is_del)
{
    ...
    if (is_del) {
        ...
        if (dhcp_proxy_server_add(FIB_PROTOCOL_IP6, addr, src_addr, rx_fib_index, server_table_id)) {
            mfib_table_entry_path_update(rx_fib_index, &all_dhcp_servers, MFIB_SOURCE_DHCP, &path_for_us,
             ...
             */
            mfib_table_entry_update(rx_fib_index, &all_dhcp_servers, MFIB_SOURCE_DHCP, MFIB_RPF_ID_NONE,
...
static clib_error_t *
dhcpv6_proxy_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
dhcpv6_proxy_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
dhcpv6_vss_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
dhcpv6_vss_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
dhcpv6_link_address_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm = vnet_get_main();
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "%U", unformat_vnet_sw_interface, vnm, &sw_if_index0)) {
            ...
            if (ia0) {
                ...
                vlib_cli_output(vm, "%=20U%=48U", format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip6_address,
                                ia0);
            } else
                vlib_cli_output(vm, "%=34s%=20U", "No IPv6 address configured on", format_vnet_sw_if_index_name, vnm,
                                sw_if_index);
        } else
            break;
    }
