...
ipv6_get_key(ip6_header_t *ip)
{
    ...
    hash_key = ip->src_address.as_u64[0] ^ rotate_left(ip->src_address.as_u64[1], 13) ^
               rotate_left(ip->dst_address.as_u64[0], 26) ^ rotate_left(ip->dst_address.as_u64[1], 39) ^ ip->protocol;
...
eth_get_sym_key(ethernet_header_t *h0)
{
    ...
    if (PREDICT_TRUE(h0->type) == clib_host_to_net_u16(ETHERNET_TYPE_IP4)) {
        ...
        hash_key         = (u64)(ip->src_address.as_u64[0] ^ ip->src_address.as_u64[1] ^ ip->dst_address.as_u64[0] ^
