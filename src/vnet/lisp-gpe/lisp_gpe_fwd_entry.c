...
ip_dst_fib_add_route(u32 dst_fib_index, const ip_prefix_t *dst_prefix)
{
    ...
    if (dst_fei == FIB_NODE_INDEX_INVALID || NULL == fib_entry_get_source_data(dst_fei, FIB_SOURCE_LISP)) {
        ...
        src_fib_index = fib_table_create_and_lock(dst_fib_prefix.fp_proto, FIB_SOURCE_LISP, "LISP-src for [%d,%U]",
        ...
        fib_table_entry_special_dpo_add(src_fib_index, &prefix, FIB_SOURCE_LISP, FIB_ENTRY_FLAG_EXCLUSIVE,
        ...
        lookup_dpo_add_or_lock_w_fib_index(src_fib_index,
                                           (ip_prefix_version(dst_prefix) == IP6 ? DPO_PROTO_IP6 : DPO_PROTO_IP4),
         ...
         */
        dst_fei = fib_table_entry_special_dpo_add(dst_fib_index, &dst_fib_prefix, FIB_SOURCE_LISP,
 ...
 */
static void
ip_src_dst_fib_del_route(u32 src_fib_index, const ip_prefix_t *src_prefix, u32 dst_fib_index,
                         const ip_prefix_t *dst_prefix)
{
    ...
    if (fib_table_lookup_exact_match(src_fib_index, &default_pref) != FIB_NODE_INDEX_INVALID)
        have_default = 1;
    ...
    if (n_entries == 0 || (have_default && n_entries == 1)) {
         ...
         */
        if (have_default)
            fib_table_entry_special_remove(src_fib_index, &default_pref, FIB_SOURCE_LISP);
...
ip_src_fib_add_route_w_dpo(u32 src_fib_index, const ip_prefix_t *src_prefix, const dpo_id_t *src_dpo)
{
    ...
    if (FIB_NODE_INDEX_INVALID == src_fei || !fib_entry_is_sourced(src_fei, FIB_SOURCE_LISP)) {
        fei = fib_table_entry_special_dpo_add(src_fib_index, &src_fib_prefix, FIB_SOURCE_LISP, FIB_ENTRY_FLAG_EXCLUSIVE,
                                              src_dpo);
    }
    return fei;
}
 ...
 */
static fib_node_index_t
ip_src_fib_add_route(u32 src_fib_index, const ip_prefix_t *src_prefix, const lisp_fwd_path_t *paths)
{
    fib_prefix_t src_fib_prefix;
    ...
    fib_node_index_t fib_entry_index =
        fib_table_entry_update(src_fib_index, &src_fib_prefix, FIB_SOURCE_LISP, FIB_ENTRY_FLAG_NONE, rpaths);
    vec_free(rpaths);
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE == lfe->type) {
        ...
        case LISP_FORWARD_NATIVE:
            ...
            if (vec_len(lgm->native_fwd_rpath[ip_version])) {
                ...
                fi = fib_table_entry_update(lfe->eid_fib_index, &fib_prefix, FIB_SOURCE_LISP, FIB_ENTRY_FLAG_NONE,
...
find_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a, lisp_gpe_fwd_entry_key_t *key)
{
    uword *p;
...
static void
lisp_gpe_fwd_entry_mk_paths(lisp_gpe_fwd_entry_t *lfe, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
...
void
vnet_lisp_gpe_add_fwd_counters(vnet_lisp_gpe_add_del_fwd_entry_args_t *a, u32 fwd_entry_index)
{
    const lisp_gpe_adjacency_t *ladj;
    lisp_fwd_path_t *path;
    lisp_gpe_main_t *lgm = vnet_lisp_gpe_get_main();
    ...
    vec_foreach(path, lfe->paths)
    {
        ...
        hash_set_mem(lgm->lisp_stats_index_by_key, key_copy, dummy_elt - lgm->dummy_stats_pool);

        vlib_validate_combined_counter(&lgm->counters, dummy_elt - lgm->dummy_stats_pool);
        vlib_zero_combined_counter(&lgm->counters, dummy_elt - lgm->dummy_stats_pool);
    }
}
 ...
 */
static int
add_ip_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
    ...
    fproto = (IP4 == ip_prefix_version(&fid_addr_ippref(&lfe->key->rmt)) ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6);
 ...
 */
static int
del_ip_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
    lisp_gpe_fwd_entry_key_t key;
    lisp_gpe_fwd_entry_t *lfe;
...
static void
make_mac_fib_key(BVT(clib_bihash_kv) * kv, u16 bd_index, u8 src_mac[6], u8 dst_mac[6])
{
 ...
 */
index_t
lisp_l2_fib_lookup(lisp_gpe_main_t *lgm, u16 bd_index, u8 src_mac[6], u8 dst_mac[6])
{
    int rv;
 ...
 */
static u32
lisp_l2_fib_add_del_entry(u16 bd_index, u8 src_mac[6], u8 dst_mac[6], const dpo_id_t *dpo, u8 is_add)
{
...
l2_fib_init(lisp_gpe_main_t *lgm)
{
    ...
    BV(clib_bihash_init)
    (&lgm->l2_fib, "l2 fib", 1 << max_log2(L2_FIB_DEFAULT_HASH_NUM_BUCKETS), L2_FIB_DEFAULT_HASH_MEMORY_SIZE);
 ...
 */
static int
del_l2_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
    lisp_gpe_fwd_entry_key_t key;
    lisp_gpe_fwd_entry_t *lfe;
...
lisp_gpe_l2_update_fwding(lisp_gpe_fwd_entry_t *lfe)
{
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE != lfe->type) {
        fib_path_list_contribute_forwarding(lfe->l2.path_list_index, FIB_FORW_CHAIN_TYPE_ETHERNET,
 ...
 */
static int
add_l2_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
    lisp_gpe_fwd_entry_key_t key;
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE != lfe->type) {
         ...
         */
        lfe->l2.child_index = fib_path_list_child_add(lfe->l2.path_list_index, FIB_NODE_TYPE_LISP_GPE_FWD_ENTRY,
...
{
    BV(clib_bihash_init)
    (&lgm->nsh_fib, "nsh fib", 1 << max_log2(NSH_FIB_DEFAULT_HASH_NUM_BUCKETS), NSH_FIB_DEFAULT_HASH_MEMORY_SIZE);
 ...
 */
static int
del_nsh_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
    lisp_gpe_fwd_entry_key_t key;
    lisp_gpe_fwd_entry_t *lfe;
...
lisp_gpe_nsh_update_fwding(lisp_gpe_fwd_entry_t *lfe)
{
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE != lfe->type) {
        fib_path_list_contribute_forwarding(lfe->nsh.path_list_index, FIB_FORW_CHAIN_TYPE_NSH,
 ...
 */
static int
add_nsh_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
    lisp_gpe_fwd_entry_key_t key;
    lisp_gpe_fwd_entry_t *lfe;
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE != lfe->type) {
         ...
         * its forwarding state changes. this includes an implicit lock.
         */
        lfe->nsh.child_index = fib_path_list_child_add(lfe->nsh.path_list_index, FIB_NODE_TYPE_LISP_GPE_FWD_ENTRY,
                                                       lfe - lgm->lisp_fwd_entry_pool);
    } else {
        lfe->action = a->action;
    }
 ...
 */
int
vnet_lisp_gpe_add_del_fwd_entry(vnet_lisp_gpe_add_del_fwd_entry_args_t *a, u32 *hw_if_indexp)
{
    lisp_gpe_main_t *lgm = &lisp_gpe_main;
    u8 type;
...
void
vnet_lisp_gpe_del_fwd_counters(vnet_lisp_gpe_add_del_fwd_entry_args_t *a, u32 fwd_entry_index)
{
    lisp_gpe_main_t *lgm = &lisp_gpe_main;
    lisp_gpe_fwd_entry_key_t fe_key;
    lisp_gpe_fwd_entry_t *lfe;
...
format_lisp_gpe_fwd_entry(u8 *s, va_list *ap)
{
    ...
    s = format(s, "VNI:%d VRF:%d EID: %U -> %U  [index:%d]", lfe->key->vni, lfe->eid_table_id, format_fid_address,
               &lfe->key->lcl, format_fid_address, &lfe->key->rmt, lfe - lgm->lisp_fwd_entry_pool);
...
static clib_error_t *
lisp_gpe_fwd_entry_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    lisp_gpe_main_t *lgm = &lisp_gpe_main;
    lisp_gpe_fwd_entry_t *lfe;
    index_t index;
    ...
    pool_foreach(lfe, lgm->lisp_fwd_entry_pool, ({
                     if ((vni == ~0) || (lfe->key->vni == vni))
                         vlib_cli_output(vm, "%U", format_lisp_gpe_fwd_entry, lfe, LISP_GPE_FWD_ENTRY_FORMAT_NONE);
                 }));
    /* *INDENT-ON* */
...
vnet_lisp_gpe_get_fwd_entry_vnis(void)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(lfe, lgm->lisp_fwd_entry_pool, ({ hash_set(vnis, lfe->key->vni, 0); }));
    /* *INDENT-ON* */
...
int
vnet_lisp_gpe_get_fwd_stats(vnet_lisp_gpe_add_del_fwd_entry_args_t *a, vlib_counter_t *c)
{
    lisp_gpe_main_t *lgm = vnet_lisp_gpe_get_main();
    lisp_gpe_fwd_entry_t *lfe;
