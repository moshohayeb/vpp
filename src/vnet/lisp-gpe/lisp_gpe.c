...
static clib_error_t *
lisp_gpe_add_del_fwd_entry_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            ...
            is_negative = 1;
        } else if (unformat(line_input, "loc-pair %U %U w %d", unformat_ip_address, &lloc, unformat_ip_address, &rloc,
                            &w)) {
...
static clib_error_t *
gpe_set_encap_mode_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
gpe_show_encap_mode_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
lisp_gpe_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    u8 is_en = 1;
...
static clib_error_t *
lisp_show_iface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    lisp_gpe_main_t *lgm = &lisp_gpe_main;
    hash_pair_t *p;
...
static clib_error_t *
gpe_show_native_fwd_rpath_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    lisp_gpe_main_t *lgm = &lisp_gpe_main;
    ...
    if (vec_len(lgm->native_fwd_rpath[IP4])) {
        ...
        {
            vlib_cli_output(vm, "nh: %U fib_index %u sw_if_index %u", format_ip46_address, &rpath->frp_addr,
                            IP46_TYPE_IP4, rpath->frp_fib_index, rpath->frp_sw_if_index);
        }
    }
    if (vec_len(lgm->native_fwd_rpath[IP6])) {
        ...
        {
            vlib_cli_output(vm, "nh: %U fib_index %u sw_if_index %u", format_ip46_address, &rpath->frp_addr,
                            IP46_TYPE_IP6, rpath->frp_fib_index, rpath->frp_sw_if_index);
        }
    }
    return 0;
}
...
gpe_update_native_fwd_path(u8 ip_version)
{
    ...
    vec_foreach(lfei, lgm->native_fwd_lfes[ip_version])
    {
        ...
        fib_table_entry_update(lfe->eid_fib_index, &fib_prefix, FIB_SOURCE_LISP, FIB_ENTRY_FLAG_NONE,
 ...
 */
static clib_error_t *
gpe_native_forward_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
lisp_gpe_test_send_nsh_packet(u8 *file_name)
{
    ...
    vlib_node_t *n = vlib_get_node_by_name(lgm->vlib_main, (u8 *) "interface-tx");
...
static clib_error_t *
lisp_test_nsh_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    u8 *file_name       = 0;
