 ...
 */
static uword
lisp_gpe_interface_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
#undef _
    ...
    s = format(s, "\n  ver_res %d res %d next_protocol %d iid %d(%x)", h->ver_res, h->res, h->next_protocol,
               clib_net_to_host_u32(h->iid << 8), clib_net_to_host_u32(h->iid << 8));
    return s;
}
 ...
 */
static uword
l2_lisp_gpe_interface_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    u32 n_left_from, next_index, *from, *to_next;
    lisp_gpe_main_t *lgm             = &lisp_gpe_main;
 ...
 */
static uword
nsh_lisp_gpe_interface_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    u32 n_left_from, next_index, *from, *to_next;
    lisp_gpe_main_t *lgm = &lisp_gpe_main;
...
lisp_gpe_create_iface(lisp_gpe_main_t *lgm, u32 vni, u32 dp_table, vnet_device_class_t *dev_class, tunnel_lookup_t *tuns)
{
    u32 flen;
    u32 hw_if_index = ~0;
    u8 *new_name;
...
static void
lisp_gpe_remove_iface(lisp_gpe_main_t *lgm, u32 hi_index, u32 dp_table, tunnel_lookup_t *tuns)
{
    vnet_main_t *vnm = lgm->vnet_main;
    vnet_hw_interface_t *hi;
    uword *vnip;
...
lisp_gpe_tenant_add_default_routes(u32 table_id)
{
    ...
    FOR_EACH_FIB_IP_PROTOCOL(proto)
    {
        ...
        fib_table_entry_special_dpo_add(fib_index, &prefix, FIB_SOURCE_LISP, FIB_ENTRY_FLAG_EXCLUSIVE,
 ...
 */
u32
lisp_gpe_add_l3_iface(lisp_gpe_main_t *lgm, u32 vni, u32 table_id, u8 with_default_routes)
{
    vnet_main_t *vnm           = lgm->vnet_main;
...
static clib_error_t *
lisp_gpe_add_del_iface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
