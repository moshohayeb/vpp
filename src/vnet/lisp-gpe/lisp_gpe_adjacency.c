...
lisp_gpe_adj_stack_one(lisp_gpe_adjacency_t *ladj, adj_index_t ai)
{
    ...
    fib_entry_contribute_forwarding(lgt->fib_entry_index, lisp_gpe_adj_get_fib_chain_type(ladj), &tmp);
...
static void
lisp_gpe_increment_stats_counters(lisp_cp_main_t *lcm, ip_adjacency_t *adj, vlib_buffer_t *b)
{
...
static void
lisp_gpe_fixup(vlib_main_t *vm, ip_adjacency_t *adj, vlib_buffer_t *b, const void *data)
{
...
lisp_gpe_update_adjacency(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
    ...
    adj_nbr_midchain_update_rewrite(ai, lisp_gpe_fixup, NULL,
                                    (VNET_LINK_ETHERNET == linkt ? ADJ_FLAG_MIDCHAIN_NO_COUNT : ADJ_FLAG_NONE),
                                    lisp_gpe_tunnel_build_rewrite(lgt, ladj,
                                                                  lisp_gpe_adj_proto_from_vnet_link_type(linkt)));

    lisp_gpe_adj_stack_one(ladj, ai);
}

u8 *
lisp_gpe_build_rewrite(vnet_main_t *vnm, u32 sw_if_index, vnet_link_t link_type, const void *dst_address)
{
    ASSERT(0);
    return (NULL);
}
...
index_t
lisp_gpe_adjacency_find_or_create_and_lock(const locator_pair_t *pair, u32 overlay_table_id, u32 vni)
{
...
static clib_error_t *
lisp_gpe_adjacency_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    lisp_gpe_adjacency_t *ladj;
    index_t index;
    ...
    if (unformat(input, "%d", &index)) {
        ...
        pool_foreach(ladj, lisp_adj_pool, ({
                         vlib_cli_output(vm, "[%d] %U\n", ladj - lisp_adj_pool, format_lisp_gpe_adjacency, ladj,
...
{
    BV(clib_bihash_init)
    (&lisp_adj_db, "Adjacency Neighbour table", LISP_ADJ_NBR_DEFAULT_HASH_NUM_BUCKETS,
