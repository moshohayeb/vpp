 ...
 */
u32
lisp_gpe_tenant_l3_iface_add_or_lock(u32 vni, u32 table_id, u8 with_default_route)
{
    lisp_gpe_tenant_t *lt;
 ...
 */
static clib_error_t *
lisp_gpe_tenant_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    lisp_gpe_tenant_t *lt;

    /* *INDENT-OFF* */
    pool_foreach(lt, lisp_gpe_tenant_pool, ({ vlib_cli_output(vm, "%U", format_lisp_gpe_tenant, lt); }));
    /* *INDENT-ON* */
