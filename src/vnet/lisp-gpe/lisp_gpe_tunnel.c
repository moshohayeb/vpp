 ...
 */
u8 *
lisp_gpe_tunnel_build_rewrite(const lisp_gpe_tunnel_t *lgt, const lisp_gpe_adjacency_t *ladj,
...
index_t
lisp_gpe_tunnel_find_or_create_and_lock(const locator_pair_t *pair, u32 rloc_fib_index)
{
 ...
 */
static clib_error_t *
show_lisp_gpe_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    lisp_gpe_tunnel_t *lgt;
    index_t index;
    ...
    if (unformat(input, "%d", &index)) {
        ...
        /* *INDENT-OFF* */
        pool_foreach(lgt, lisp_gpe_tunnel_pool, ({ vlib_cli_output(vm, "%U", format_lisp_gpe_tunnel, lgt); }));
        /* *INDENT-ON* */
    }
