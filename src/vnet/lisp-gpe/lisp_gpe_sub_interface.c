...
index_t
lisp_gpe_sub_interface_find_or_create_and_lock(const ip_address_t *lrloc, u32 overlay_table_id, u32 vni)
{
    ...
    if (INDEX_INVALID == l3si) {
        ...
        if (NULL != vnet_create_sw_interface(vnet_get_main(), &sub_itf_template, &sub_sw_if_index))
            return (INDEX_INVALID);
...
format_lisp_gpe_sub_interface(u8 *s, va_list *ap)
{
    ...
    s = format(s, "%-16U", format_vnet_sw_interface_name, vnm, vnet_get_sw_interface(vnm, l3s->sw_if_index));
...
static clib_error_t *
lisp_gpe_sub_interface_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    lisp_gpe_sub_interface_t *l3s;
    ...
    /* *INDENT-OFF* */
    pool_foreach(l3s, lisp_gpe_sub_interface_pool, ({ vlib_cli_output(vm, "%U", format_lisp_gpe_sub_interface, l3s); }));
    /* *INDENT-ON* */
