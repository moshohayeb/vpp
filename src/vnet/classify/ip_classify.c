...
static inline uword
ip_classify_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
    ...
    while (n_left_from > 2) {
        ...
        h0  = (void *) vlib_buffer_get_current(b0) - ethernet_buffer_header_size(b0);

        bi1 = from[1];
        b1  = vlib_get_buffer(vm, bi1);
        h1  = (void *) vlib_buffer_get_current(b1) - ethernet_buffer_header_size(b1);
...
static uword
ip4_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ip6_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
