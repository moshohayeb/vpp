...
rogue(vnet_classify_table_t *t)
{
    ...
    if (active_elements != t->active_elements)
        clib_warning("found %u expected %u elts", active_elements, t->active_elements);
}
#else
void
mv(vnet_classify_table_t *t)
...
vnet_classify_new_table(vnet_classify_main_t *cm, u8 *mask, u32 nbuckets, u32 memory_size, u32 skip_n_vectors,
                        u32 match_n_vectors)
{
    ...
    t->writer_lock    = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
    t->writer_lock[0] = 0;
...
void
vnet_classify_delete_table_index(vnet_classify_main_t *cm, u32 table_index, int del_chain)
{
    vnet_classify_table_t *t;
...
vnet_classify_entry_alloc(vnet_classify_table_t *t, u32 log2_pages)
{
    ...
    required_length =
        (sizeof(vnet_classify_entry_t) + (t->match_n_vectors * sizeof(u32x4))) * t->entries_per_page * (1 << log2_pages);
...
static void
vnet_classify_entry_free(vnet_classify_table_t *t, vnet_classify_entry_t *v, u32 log2_pages)
{
    ASSERT(t->writer_lock[0]);
...
static inline void
make_working_copy(vnet_classify_table_t *t, vnet_classify_bucket_t *b)
{
    ...
    required_length     = (sizeof(vnet_classify_entry_t) + (t->match_n_vectors * sizeof(u32x4))) * t->entries_per_page *
                      (1 << b->log2_pages);
...
static vnet_classify_entry_t *
split_and_rehash(vnet_classify_table_t *t, vnet_classify_entry_t *old_values, u32 old_log2_pages, u32 new_log2_pages)
{
        ...
        if (vnet_classify_entry_is_busy(v)) {
            ...
            for (j = 0; j < t->entries_per_page; j++) {
                ...
                if (vnet_classify_entry_is_free(new_v)) {
                    clib_memcpy(new_v, v, sizeof(vnet_classify_entry_t) + (t->match_n_vectors * sizeof(u32x4)));
...
static vnet_classify_entry_t *
split_and_rehash_linear(vnet_classify_table_t *t, vnet_classify_entry_t *old_values, u32 old_log2_pages,
                        u32 new_log2_pages)
{
    vnet_classify_entry_t *new_values, *v, *new_v;
...
int
vnet_classify_add_del(vnet_classify_table_t *t, vnet_classify_entry_t *add_v, int is_add)
{
    u32 bucket_index;
    ...
    if (b->offset == 0) {
        ...
        clib_memcpy(v, add_v, sizeof(vnet_classify_entry_t) + t->match_n_vectors * sizeof(u32x4));
    ...
    if (is_add) {
        ...
        for (i = 0; i < limit; i++) {
            ...
            if (!memcmp(v->key, add_v->key, t->match_n_vectors * sizeof(u32x4))) {
                ...
                memset(v, 0xff, sizeof(vnet_classify_entry_t) + t->match_n_vectors * sizeof(u32x4));
                v->flags |= VNET_CLASSIFY_ENTRY_FREE;
    ...
    for (i = 0; i < limit; i++) {
        ...
        if (vnet_classify_entry_is_free(new_v)) {
            clib_memcpy(new_v, add_v, sizeof(vnet_classify_entry_t) + t->match_n_vectors * sizeof(u32x4));
            new_v->flags &= ~(VNET_CLASSIFY_ENTRY_FREE);
...
vnet_classify_find_entry(vnet_classify_table_t *t, u8 *h, u64 hash, f64 now)
{
...
format_classify_entry(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%u]: next_index %d advance %d opaque %d action %d metadata %d\n", vnet_classify_get_offset(t, e),
               e->next_index, e->advance, e->opaque_index, e->action, e->metadata);
    ...
    for (i = 0; i < t->nbuckets; i++) {
        ...
        if (verbose) {
            s = format(s, "[%d]: heap offset %d, elts %d, %s\n", i, b->offset,
                       (1 << b->log2_pages) * t->entries_per_page, b->linear_search ? "LINEAR" : "normal");
        }
...
int
vnet_classify_add_del_table(vnet_classify_main_t *cm, u8 *mask, u32 nbuckets, u32 memory_size, u32 skip, u32 match,
...
static clib_error_t *
classify_table_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u32 nbuckets         = 2;
    u32 skip             = ~0;
    u32 match            = ~0;
    ...
    rv = vnet_classify_add_del_table(cm, mask, nbuckets, memory_size, skip, match, next_table_index, miss_next_index,
                                     &table_index, current_data_flag, current_data_offset, is_add, del_chain);
    switch (rv) {
    case 0:
        break;
...
VLIB_CLI_COMMAND(classify_table, static) = {
    .path       = "classify table",
    .short_help = "classify table [miss-next|l2-miss_next|acl-miss-next <next_index>]"
...
format_vnet_classify_table(u8 *s, va_list *args)
{
    ...
    if (index == ~0) {
        s = format(s, "%10s%10s%10s%10s", "TableIdx", "Sessions", "NextTbl", "NextNode", verbose ? "Details" : "");
        return s;
    }
    ...
    s = format(s, "\n  nbuckets %d, skip %d match %d flag %d offset %d", t->nbuckets, t->skip_n_vectors,
               t->match_n_vectors, t->current_data_flag, t->current_data_offset);
...
static clib_error_t *
show_classify_tables_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_classify_main_t *cm = &vnet_classify_main;
    vnet_classify_table_t *t;
    u32 match_index = ~0;
...
int
vnet_classify_add_del_session(vnet_classify_main_t *cm, u32 table_index, u8 *match, u32 hit_next_index,
                              u32 opaque_index, i32 advance, u8 action, u32 metadata, int is_add)
{
    ...
    clib_memcpy(&e->key, match + t->skip_n_vectors * sizeof(u32x4), t->match_n_vectors * sizeof(u32x4));
...
static clib_error_t *
classify_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_classify_main_t *cm = &vnet_classify_main;
    int is_add               = 1;
    u32 table_index          = ~0;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del"))
            ...
            for (i = 0; i < vec_len(cm->unformat_opaque_index_fns); i++) {
                if (unformat(input, "%U", cm->unformat_opaque_index_fns[i], &opaque_index))
                    goto found_opaque;
            }
            break;
        }
    ...
    rv = vnet_classify_add_del_session(cm, table_index, match, hit_next_index, opaque_index, advance, action, metadata,
                                       is_add);
...
VLIB_CLI_COMMAND(classify_session_command, static) = {
    ...
    .short_help = "classify session [hit-next|l2-hit-next|"
...
test_classify_churn(test_classify_main_t *tm)
{
    ...
    tm->table = vnet_classify_new_table(tm->classify_main, (u8 *) mask, tm->buckets, tm->memory_size, 0 /* skip */,
                                        3 /* vectors to match */);
    ...
    for (i = 0; i < tm->sessions / 2; i++) {
        ...
        rv = vnet_classify_add_del_session(tm->classify_main, tm->table_index, (u8 *) data, IP_LOOKUP_NEXT_DROP,
                                           i /* opaque_index */, 0 /* advance */, 0 /* action */, 0 /* metadata */,
                                           1 /* is_add */);
    ...
    for (i = 0; i < tm->iterations; i++) {
        ...
        if (tm->verbose)
            vlib_cli_output(vm, "%s: %U", is_add ? "add" : "del", format_ip4_address, &ep->addr.as_u32);

        rv = vnet_classify_add_del_session(tm->classify_main, tm->table_index, (u8 *) data, IP_LOOKUP_NEXT_DROP,
                                           i /* opaque_index */, 0 /* advance */, 0 /* action */, 0 /* metadata */,
                                           is_add);
        if (rv != 0)
            vlib_cli_output(vm, "%s[%d]: %U returned %d", is_add ? "add" : "del", index, format_ip4_address,
                            &ep->addr.as_u32, rv);
        else
            ep->in_table = is_add;
    }
    ...
    for (i = 0; i < tm->sessions; i++) {
        ...
        rv = vnet_classify_add_del_session(tm->classify_main, tm->table_index, key_minus_skip, IP_LOOKUP_NEXT_DROP,
                                           i /* opaque_index */, 0 /* advance */, 0, 0, 0 /* is_add */);
...
static clib_error_t *
test_classify_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
VLIB_CLI_COMMAND(test_classify_command, static) = {
    .path       = "test classify",
    .short_help = "test classify [src <ip>] [sessions <nn>] [buckets <nn>] [seed <nnn>]\n"
                  "              [memory-size <nn>[M|G]]\n"
                  "              [churn-test]",
