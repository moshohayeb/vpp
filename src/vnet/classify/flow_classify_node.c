...
format_flow_classify_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "FLOW_CLASSIFY: sw_if_index %d next %d table %d offset %d", t->sw_if_index, t->next_index,
...
static inline uword
flow_classify_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, flow_classify_table_id_t tid)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            vnet_get_config_data(fcm->vnet_config_main[tid], &b0->current_config_index, &next0,
...
static uword
ip4_flow_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ip6_flow_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
