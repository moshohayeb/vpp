...
#undef _
    ...
    rv = vnet_classify_add_del_table(cm, mp->mask, nbuckets, memory_size, skip_n_vectors, match_n_vectors,
...
static void
vl_api_classify_add_del_session_t_handler(vl_api_classify_add_del_session_t *mp)
{
    vnet_classify_main_t *cm = &vnet_classify_main;
    ...
    rv = vnet_classify_add_del_session(cm, table_index, mp->match, hit_next_index, opaque_index, advance, action,
                                       metadata, mp->is_add);
...
static void
vl_api_policer_classify_set_interface_t_handler(vl_api_policer_classify_set_interface_t *mp)
{
...
static void
send_policer_classify_details(u32 sw_if_index, u32 table_index, vl_api_registration_t *reg, u32 context)
{
...
vl_api_classify_table_ids_t_handler(vl_api_classify_table_ids_t *mp)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(t, cm->tables, ({ vec_add1(table_ids, ntohl(t - cm->tables)); }));
    /* *INDENT-ON* */
...
static void
vl_api_classify_table_by_interface_t_handler(vl_api_classify_table_by_interface_t *mp)
{
...
static void
send_classify_session_details(vl_api_registration_t *reg, u32 table_id, u32 match_length, vnet_classify_entry_t *e,
                              u32 context)
{
                         ...
                         for (i = 0; i < t->nbuckets; i++) {
                             ...
                             for (j = 0; j < (1 << b->log2_pages); j++) {
                                 for (k = 0; k < t->entries_per_page; k++) {
                                     ...
                                     send_classify_session_details(reg, table_id, t->match_n_vectors * sizeof(u32x4), v,
                                                                   mp->context);
                                 }
                             }
                         }
...
static void
vl_api_flow_classify_set_interface_t_handler(vl_api_flow_classify_set_interface_t *mp)
{
    vlib_main_t *vm = vlib_get_main();
...
static void
send_flow_classify_details(u32 sw_if_index, u32 table_index, vl_api_registration_t *reg, u32 context)
{
...
static void
vl_api_classify_set_interface_ip_table_t_handler(vl_api_classify_set_interface_ip_table_t *mp)
{
    vlib_main_t *vm = vlib_get_main();
...
static void
vl_api_classify_set_interface_l2_tables_t_handler(vl_api_classify_set_interface_l2_tables_t *mp)
{
...
static void
vl_api_output_acl_set_interface_t_handler(vl_api_output_acl_set_interface_t *mp)
{
    vlib_main_t *vm = vlib_get_main();
