 ...
 */
int
sr_cli_localsid(char is_del, ip6_address_t *localsid_addr, char end_psp, u8 behavior, u32 sw_if_index, u32 vlan_index,
                u32 fib_table, ip46_address_t *nh_addr, void *ls_plugin_mem)
{
    ...
    mhash_set(&sm->sr_localsids_index_hash, localsid_addr, ls - sm->localsids, NULL);
 ...
 */
static clib_error_t *
sr_cli_localsid_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del"))
            ...
            if (unformat(input, "end.x %U %U", unformat_vnet_sw_interface, vnm, &sw_if_index, unformat_ip6_address,
                         &next_hop.ip6))
                behavior = SR_BEHAVIOR_X;
            else if (unformat(input, "end.t %u", &sw_if_index))
                behavior = SR_BEHAVIOR_T;
            else if (unformat(input, "end.dx6 %U %U", unformat_vnet_sw_interface, vnm, &sw_if_index,
                              unformat_ip6_address, &next_hop.ip6))
                behavior = SR_BEHAVIOR_DX6;
            else if (unformat(input, "end.dx4 %U %U", unformat_vnet_sw_interface, vnm, &sw_if_index,
                ...
                /* *INDENT-OFF* */
                pool_foreach(plugin, sm->plugin_functions, { vec_add1(vec_plugins, plugin); });
                /* *INDENT-ON* */
    ...
    rv = sr_cli_localsid(is_del, &resulting_address, end_psp, behavior, sw_if_index, vlan_index, fib_index, &next_hop,
                         ls_plugin_mem);
...
VLIB_CLI_COMMAND(sr_localsid_command, static) = {
    ...
    .long_help = "Create SR LocalSID and binds it to a particular behavior\n"
                 "Arguments:\n"
 ...
 */
static clib_error_t *
show_sr_localsid_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm                  = vnet_get_main();
    ip6_sr_main_t *sm                 = &sr_main;
    ...
    for (i = 0; i < vec_len(localsid_list); i++) {
        ...
        case SR_BEHAVIOR_X:
            vlib_cli_output(vm,
                            ...
                            "\n\tIface:  \t%U\n\tNext hop: \t%U",
                            format_ip6_address, &ls->localsid, format_vnet_sw_if_index_name, vnm, ls->sw_if_index,
        ...
        default:
            if (ls->behavior >= SR_BEHAVIOR_LAST) {
                ...
                vlib_cli_output(vm,
                                "\tAddress: \t%U\n"
                                "\tBehavior: \t%s (%s)\n\t%U",
                                format_ip6_address, &ls->localsid, plugin->keyword_str, plugin->def_str,
 ...
 */
static clib_error_t *
clear_sr_localsid_counters_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ip6_sr_main_t *sm = &sr_main;
...
end_srh_processing(vlib_node_runtime_t *node, vlib_buffer_t *b0, ip6_header_t *ip0, ip6_sr_header_t *sr0,
                   ip6_sr_localsid_t *ls0, u32 *next0, u8 psp, ip6_ext_header_t *prev0)
{
 ...
 */
static_always_inline void
end_decaps_srh_processing(vlib_node_runtime_t *node, vlib_buffer_t *b0, ip6_header_t *ip0, ip6_sr_header_t *sr0,
 ...
 */
static uword
sr_localsid_d_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4) {
            ...
            vlib_increment_combined_counter(((next0 == SR_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) :
                                                                                 &(sm->sr_ls_valid_counters)),
                                            thread_index, ls0 - sm->localsids, 1, vlib_buffer_length_in_chain(vm, b0));

            vlib_increment_combined_counter(((next1 == SR_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) :
                                                                                 &(sm->sr_ls_valid_counters)),
                                            thread_index, ls1 - sm->localsids, 1, vlib_buffer_length_in_chain(vm, b1));

            vlib_increment_combined_counter(((next2 == SR_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) :
                                                                                 &(sm->sr_ls_valid_counters)),
                                            thread_index, ls2 - sm->localsids, 1, vlib_buffer_length_in_chain(vm, b2));

            vlib_increment_combined_counter(((next3 == SR_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) :
                                                                                 &(sm->sr_ls_valid_counters)),
                                            thread_index, ls3 - sm->localsids, 1, vlib_buffer_length_in_chain(vm, b3));
 ...
 */
static uword
sr_localsid_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    u32 n_left_from, next_index, *from, *to_next;
    ip6_sr_main_t *sm = &sr_main;
    from              = vlib_frame_vector_args(from_frame);
 ...
 */
int
sr_localsid_register_function(vlib_main_t *vm, u8 *fn_name, u8 *keyword_str, u8 *def_str, u8 *params_str,
 ...
 */
static clib_error_t *
show_sr_localsid_behaviors_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ip6_sr_main_t *sm = &sr_main;
    sr_localsid_fn_registration_t *plugin;
    ...
    /* *INDENT-OFF* */
    pool_foreach(plugin, sm->plugin_functions, ({ vec_add1(plugins_vec, plugin); }));
    /* *INDENT-ON* */
...
sr_localsids_init(vlib_main_t *vm)
{
    ...
    ip6_sr_main_t *sm = &sr_main;
    mhash_init(&sm->sr_localsids_index_hash, sizeof(uword), sizeof(ip6_address_t));
