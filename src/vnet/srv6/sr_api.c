...
static void
vl_api_sr_localsid_add_del_t_handler(vl_api_sr_localsid_add_del_t *mp)
{
     ...
     */
    if (mp->behavior == SR_BEHAVIOR_X || mp->behavior == SR_BEHAVIOR_DX6 || mp->behavior == SR_BEHAVIOR_DX4 ||
    ...
    if ((mp->nh_addr4[0] | mp->nh_addr4[1] | mp->nh_addr4[2] | mp->nh_addr4[3]) != 0)
        memcpy(&prefix.ip4, mp->nh_addr4, sizeof(prefix.ip4));
    else
        memcpy(&prefix.ip6, mp->nh_addr6, sizeof(prefix.ip6));

    rv = sr_cli_localsid(mp->is_del, (ip6_address_t *) &mp->localsid, mp->end_psp, mp->behavior, ntohl(mp->sw_if_index),
                         ntohl(mp->vlan_index), ntohl(mp->fib_table), &prefix, NULL);
...
vl_api_sr_policy_add_t_handler(vl_api_sr_policy_add_t *mp)
{
     ...
     */
    int rv = 0;
    rv     = sr_policy_add((ip6_address_t *) &mp->bsid_addr, segments, ntohl(mp->sids.weight), mp->type,
                       ntohl(mp->fib_table), mp->is_encap);
    vec_free(segments);
...
vl_api_sr_policy_mod_t_handler(vl_api_sr_policy_mod_t *mp)
{
     ...
     */
    rv = sr_policy_mod((ip6_address_t *) &mp->bsid_addr, ntohl(mp->sr_policy_index), ntohl(mp->fib_table),
                       mp->operation, segments, ntohl(mp->sl_index), ntohl(mp->sids.weight));
    vec_free(segments);
...
vl_api_sr_policy_del_t_handler(vl_api_sr_policy_del_t *mp)
{
     ...
     */
    rv = sr_policy_del((ip6_address_t *) &mp->bsid_addr, ntohl(mp->sr_policy_index));
...
static void
vl_api_sr_steering_add_del_t_handler(vl_api_sr_steering_add_del_t *mp)
{
    ...
    rv = sr_steering_policy(mp->is_del, (ip6_address_t *) &mp->bsid_addr, ntohl(mp->sr_policy_index),
                            ntohl(mp->table_id), (ip46_address_t *) &mp->prefix_addr, ntohl(mp->mask_width),
...
static void
send_sr_localsid_details(ip6_sr_localsid_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    if (ip46_address_is_ip4(&t->next_hop))
        clib_memcpy(rmp->xconnect_nh_addr4, &t->next_hop.ip4, sizeof(ip4_address_t));
    else
        clib_memcpy(rmp->xconnect_nh_addr6, &t->next_hop.ip6, sizeof(ip6_address_t));
...
static void
vl_api_sr_localsids_dump_t_handler(vl_api_sr_localsids_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(t, sm->localsids, ({ send_sr_localsid_details(t, reg, mp->context); }));
    /* *INDENT-ON* */
}

static void
send_sr_policies_details(ip6_sr_policy_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    memset(rmp, 0, (sizeof(*rmp) + vec_len(t->segments_lists) * sizeof(vl_api_srv6_sid_list_t)));
...
vl_api_sr_policies_dump_t_handler(vl_api_sr_policies_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(t, sm->sr_policies, ({ send_sr_policies_details(t, reg, mp->context); }));
    /* *INDENT-ON* */
}

static void
send_sr_steering_pol_details(ip6_sr_steering_policy_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    if (ip46_address_is_ip4(&t->classify.l3.prefix))
        clib_memcpy(rmp->prefix_addr, &t->classify.l3.prefix.ip4, sizeof(ip4_address_t));
    else
        clib_memcpy(rmp->prefix_addr, &t->classify.l3.prefix.ip6, sizeof(ip6_address_t));
...
static void
vl_api_sr_steering_pol_dump_t_handler(vl_api_sr_policies_dump_t *mp)
{
    vl_api_registration_t *reg;
    ip6_sr_main_t *sm = &sr_main;
    ...
    /* *INDENT-OFF* */
    pool_foreach(t, sm->steer_policies, ({ send_sr_steering_pol_details(t, reg, mp->context); }));
    /* *INDENT-ON* */
}
