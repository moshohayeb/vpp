 ...
 */
int
sr_steering_policy(int is_del, ip6_address_t *bsid, u32 sr_policy_index, u32 table_id, ip46_address_t *prefix,
                   u32 mask_width, u32 sw_if_index, u8 traffic_type)
{
    ...
    if (p) {
        ...
        if (is_del) {
            if (steer_pl->classify.traffic_type == SR_STEER_IPV6) {
                ...
                fib_table_entry_delete(fib_table_find(FIB_PROTOCOL_IP6, steer_pl->classify.l3.fib_table), &pfx,
                                       FIB_SOURCE_SR);
                ...
                fib_table_entry_delete(fib_table_find(FIB_PROTOCOL_IP4, steer_pl->classify.l3.fib_table), &pfx,
                                       FIB_SOURCE_SR);
            } else if (steer_pl->classify.traffic_type == SR_STEER_L2) {
                /* Remove HW redirection */
                vnet_feature_enable_disable("device-input", "sr-policy-rewrite-encaps-l2", sw_if_index, 0, 0, 0);
    ...
    if (traffic_type == SR_STEER_L2) {
        ...
        if (vnet_feature_enable_disable("device-input", "sr-pl-rewrite-encaps-l2", sw_if_index, 1, 0, 0))
            goto cleanup_error_redirection;
...
update_fib:
    ...
    if (traffic_type == SR_STEER_IPV6) {
        ...
        fib_table_entry_path_add(fib_table_find(FIB_PROTOCOL_IP6, (table_id != (u32) ~0 ? table_id : 0)), &pfx,
                                 FIB_SOURCE_SR, FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT, DPO_PROTO_IP6,
        ...
        fib_table_entry_path_add(fib_table_find(FIB_PROTOCOL_IP4, (table_id != (u32) ~0 ? table_id : 0)), &pfx,
                                 FIB_SOURCE_SR, FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT, DPO_PROTO_IP6,
        ...
        if (sw_if_index < vec_len(sm->sw_iface_sr_policies))
            ...
            vec_resize(sm->sw_iface_sr_policies,
                       (pool_len(sm->vnet_main->interface_main.sw_interfaces) - vec_len(sm->sw_iface_sr_policies)));
            sm->sw_iface_sr_policies[sw_if_index] = steer_pl->sr_policy;
        }
    }
...
static clib_error_t *
sr_steer_policy_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm = vnet_get_main();
    ...
    rv = sr_steering_policy(is_del, (sr_policy_index == ~(u32) 0 ? &bsid : NULL), sr_policy_index, fib_table, &prefix,
                            dst_mask_width, sw_if_index, traffic_type);
...
static clib_error_t *
show_sr_steering_policies_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ip6_sr_main_t *sm                         = &sr_main;
    ...
    for (i = 0; i < vec_len(steer_policies); i++) {
        ...
        if (steer_pl->classify.traffic_type == SR_STEER_L2) {
            vlib_cli_output(vm, "L2 %U\t%U", format_vnet_sw_if_index_name, vnm, steer_pl->classify.l2.sw_if_index,
                            format_ip6_address, &pl->bsid);
        } else if (steer_pl->classify.traffic_type == SR_STEER_IPV4) {
            vlib_cli_output(vm, "L3 %U/%d\t%U", format_ip4_address, &steer_pl->classify.l3.prefix.ip4,
                            steer_pl->classify.l3.mask_width, format_ip6_address, &pl->bsid);
        } else if (steer_pl->classify.traffic_type == SR_STEER_IPV6) {
            vlib_cli_output(vm, "L3 %U/%d\t%U", format_ip6_address, &steer_pl->classify.l3.prefix.ip6,
                            steer_pl->classify.l3.mask_width, format_ip6_address, &pl->bsid);
        }
    }
    return 0;
