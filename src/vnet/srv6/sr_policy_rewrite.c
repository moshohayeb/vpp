...
static clib_error_t *
set_sr_src_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
update_lb(ip6_sr_policy_t *sr_policy)
{
    ...
    if (!dpo_id_is_valid(&sr_policy->bsid_dpo)) {
        ...
        fib_table_entry_special_dpo_update(fib_table_find(FIB_PROTOCOL_IP6, sr_policy->fib_table), &pfx, FIB_SOURCE_SR,
                                           FIB_ENTRY_FLAG_EXCLUSIVE, &sr_policy->bsid_dpo);

        fib_table_entry_special_dpo_update(sm->fib_table_ip6, &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_EXCLUSIVE,
                                           &sr_policy->ip6_dpo);

        if (sr_policy->is_encap) {
            dpo_set(&sr_policy->ip4_dpo, DPO_LOAD_BALANCE, DPO_PROTO_IP4, load_balance_create(0, DPO_PROTO_IP4, fhc));

            fib_table_entry_special_dpo_update(sm->fib_table_ip4, &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_EXCLUSIVE,
                                               &sr_policy->ip4_dpo);
        }
    }
    ...
    if (sr_policy->is_encap)
        load_balance_multipath_update(&sr_policy->ip4_dpo, ip4_path_vector, LOAD_BALANCE_FLAG_NONE);
 ...
 */
int
sr_policy_add(ip6_address_t *bsid, ip6_address_t *segments, u32 weight, u8 behavior, u32 fib_table, u8 is_encap)
{
    ip6_sr_main_t *sm          = &sr_main;
    ...
    if (sm->fib_table_ip6 == (u32) ~0) {
        sm->fib_table_ip6 =
            fib_table_create_and_lock(FIB_PROTOCOL_IP6, FIB_SOURCE_SR, "SRv6 steering of IP6 prefixes through BSIDs");
        sm->fib_table_ip4 =
            fib_table_create_and_lock(FIB_PROTOCOL_IP6, FIB_SOURCE_SR, "SRv6 steering of IP4 prefixes through BSIDs");
    }
 ...
 */
int
sr_policy_mod(ip6_address_t *bsid, u32 index, u32 fib_table, u8 operation, ip6_address_t *segments, u32 sl_index,
              u32 weight)
{
    ip6_sr_main_t *sm          = &sr_main;
    ip6_sr_policy_t *sr_policy = 0;
    ...
    } else if (operation == 2) /* Delete SR List from an existing SR policy */
    {
        ...
        vec_foreach(sl_index_iterate, sr_policy->segments_lists) if (*sl_index_iterate == sl_index) break;
 ...
 */
static clib_error_t *
sr_policy_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    int rv      = -1;
    ...
    if (is_add) {
        ...
        rv = sr_policy_add(&bsid, segments, weight, (is_spray ? SR_POLICY_TYPE_SPRAY : SR_POLICY_TYPE_DEFAULT),
                           fib_table, is_encap);
        vec_free(segments);
    } else if (is_del)
        ...
        rv = sr_policy_mod((sr_policy_index != (u32) ~0 ? NULL : &bsid), sr_policy_index, fib_table, operation,
                           segments, sl_index, weight);
        vec_free(segments);
    }
    ...
    case 1:
        return 0;
    case -12:
        return clib_error_return(0, "There is already a FIB entry for the BindingSID address.\n"
                                    "The SR policy could not be created.");
    case -13:
        return clib_error_return(0, "The specified FIB table does not exist.");
    case -21:
        return clib_error_return(0, "The selected SR policy only contains ONE segment list. "
                                    "Please remove the SR policy instead");
    case -22:
        return clib_error_return(0, "Could not delete the segment list. "
                                    "It is not associated with that SR policy.");
    case -32:
        return clib_error_return(0, "Could not modify the segment list. "
...
VLIB_CLI_COMMAND(sr_policy_command, static) = {
    ...
    .long_help = "Manipulation of SR policies.\n"
 ...
 */
static clib_error_t *
show_sr_policies_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ip6_sr_main_t *sm = &sr_main;
    u32 *sl_index;
...
encaps_processing_v6(vlib_node_runtime_t *node, vlib_buffer_t *b0, ip6_header_t *ip0, ip6_header_t *ip0_encap)
{
    u32 new_l0;
 ...
 */
static uword
sr_policy_rewrite_encaps(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ip6_sr_main_t *sm = &sr_main;
    ...
    /* Update counters */
    vlib_node_increment_counter(vm, sr_policy_rewrite_encaps_node.index, SR_POLICY_REWRITE_ERROR_COUNTER_TOTAL,
                                encap_pkts);
 ...
 */
static_always_inline void
encaps_processing_v4(vlib_node_runtime_t *node, vlib_buffer_t *b0, ip6_header_t *ip0, ip4_header_t *ip0_encap)
{
    u32 new_l0;
    ip6_sr_header_t *sr0;
 ...
 */
static uword
sr_policy_rewrite_encaps_v4(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ip6_sr_main_t *sm = &sr_main;
    u32 n_left_from, next_index, *from, *to_next;
 ...
 */
static uword
sr_policy_rewrite_encaps_l2(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ip6_sr_main_t *sm = &sr_main;
    u32 n_left_from, next_index, *from, *to_next;
 ...
 */
static uword
sr_policy_rewrite_insert(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ip6_sr_main_t *sm = &sr_main;
    u32 n_left_from, next_index, *from, *to_next;
    ...
    /* Update counters */
    vlib_node_increment_counter(vm, sr_policy_rewrite_insert_node.index, SR_POLICY_REWRITE_ERROR_COUNTER_TOTAL,
                                insert_pkts);
 ...
 */
static uword
sr_policy_rewrite_b_insert(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ip6_sr_main_t *sm = &sr_main;
    u32 n_left_from, next_index, *from, *to_next;
 ...
 */
static_always_inline void
end_bsid_encaps_srh_processing(vlib_node_runtime_t *node, vlib_buffer_t *b0, ip6_header_t *ip0, ip6_sr_header_t *sr0,
                               u32 *next0)
{
 ...
 */
static uword
sr_policy_rewrite_b_encaps(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ip6_sr_main_t *sm = &sr_main;
    u32 n_left_from, next_index, *from, *to_next;
