...
format_ah_encrypt_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "ah: spi %u seq %u integrity %U", t->spi, t->seq, format_ipsec_integ_alg, t->integ_alg);
    return s;
}

static uword
ah_encrypt_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(esp_seq_advance(sa0))) {
                ...
                vlib_node_increment_counter(vm, ah_encrypt_node.index, AH_ENCRYPT_ERROR_SEQ_CYCLED, 1);
            ...
            if (PREDICT_FALSE(!sa0->is_tunnel)) {
                ethernet_header_t *ieh0 =
                    (ethernet_header_t *) ((u8 *) vlib_buffer_get_current(i_b0) - sizeof(ethernet_header_t));
            ...
            if (PREDICT_FALSE(is_ipv6)) { /* is ipv6 */
                ih6_0       = (ip6_and_ah_header_t *) ih0;
                ip_hdr_size = sizeof(ip6_header_t);
                oh6_0       = vlib_buffer_get_current(i_b0);

                hop_limit                               = ih6_0->ip6.hop_limit;
                ip_version_traffic_class_and_flow_label = ih6_0->ip6.ip_version_traffic_class_and_flow_label;
                if (PREDICT_TRUE(sa0->is_tunnel)) {
            ...
            unsigned size = hmac_calc(sa0->integ_alg, sa0->integ_key, sa0->integ_key_len, vlib_buffer_get_current(i_b0),
                                      i_b0->current_length, sig, sa0->use_esn, sa0->seq_hi);

            memcpy(digest, sig, size);
            if (PREDICT_FALSE(is_ipv6)) {
                oh6_0->ip6.hop_limit                               = hop_limit;
                oh6_0->ip6.ip_version_traffic_class_and_flow_label = ip_version_traffic_class_and_flow_label;
            } else {
                oh0->ip4.ttl      = ttl;
                oh0->ip4.tos      = tos;
        ...
        trace:
            if (PREDICT_FALSE(i_b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ...
                ah_encrypt_trace_t *tr = vlib_add_trace(vm, node, i_b0, sizeof(*tr));
                tr->spi                = sa0->spi;
                tr->seq                = sa0->seq - 1;
    ...
    vlib_node_increment_counter(vm, ah_encrypt_node.index, AH_ENCRYPT_ERROR_RX_PKTS, from_frame->n_vectors);
