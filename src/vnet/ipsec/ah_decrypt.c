...
static uword
ah_decrypt_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if ((ih4->ip_version_and_header_length & 0xF0) == 0x40) {
                ...
                vlib_node_increment_counter(vm, ah_decrypt_node.index, AH_DECRYPT_ERROR_NOT_IP, 1);
                goto trace;
            }
            ...
            if (sa0->use_anti_replay) {
                ...
                if (PREDICT_FALSE(rv)) {
                    ...
                    vlib_node_increment_counter(vm, ah_decrypt_node.index, AH_DECRYPT_ERROR_REPLAY, 1);
                    to_next[0] = i_bi0;
                    to_next += 1;
                    goto trace;
                }
            ...
            if (PREDICT_TRUE(sa0->integ_alg != IPSEC_INTEG_ALG_NONE)) {
                ...
                if ((ih4->ip_version_and_header_length & 0xF0) == 0x40) {
                    ...
                    icv_padding_len                = ah_calc_icv_padding_len(icv_size, 0 /* is_ipv6 */);
                } else {
                    ip_version_traffic_class_and_flow_label      = ih6->ip_version_traffic_class_and_flow_label;
                    hop_limit                                    = ih6->hop_limit;
                    ih6->ip_version_traffic_class_and_flow_label = 0x60;
                    ih6->hop_limit                               = 0;
                    nexthdr                                      = ah0->nexthdr;
                    icv_padding_len                              = ah_calc_icv_padding_len(icv_size, 1 /* is_ipv6 */);
                }
                hmac_calc(sa0->integ_alg, sa0->integ_key, sa0->integ_key_len, (u8 *) ih4, i_b0->current_length, sig,
                          sa0->use_esn, sa0->seq_hi);

                if (PREDICT_FALSE(memcmp(digest, sig, icv_size))) {
                    vlib_node_increment_counter(vm, ah_decrypt_node.index, AH_DECRYPT_ERROR_INTEG_ERROR, 1);
                    to_next[0] = i_bi0;
                    to_next += 1;
                    goto trace;
                }
            ...
            vlib_buffer_advance(i_b0, ip_hdr_size + sizeof(ah_header_t) + icv_size + icv_padding_len);
            ...
            if (PREDICT_TRUE(tunnel_mode)) {
                if (PREDICT_TRUE(ah0->nexthdr == IP_PROTOCOL_IP_IN_IP))
                    ...
                    clib_warning("next header: 0x%x", ah0->nexthdr);
                    vlib_node_increment_counter(vm, ah_decrypt_node.index, AH_DECRYPT_ERROR_DECRYPTION_FAILED, 1);
                    goto trace;
                }
            }
            /* transport mode */
                ...
                if (PREDICT_FALSE(transport_ip6)) {
                    ...
                    oh6->ip_version_traffic_class_and_flow_label = ip_version_traffic_class_and_flow_label;
                    oh6->payload_length =
                        clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, i_b0) - sizeof(ip6_header_t));
                } else {
        ...
        trace:
            if (PREDICT_FALSE(i_b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ...
                ah_decrypt_trace_t *tr = vlib_add_trace(vm, node, i_b0, sizeof(*tr));
    ...
    vlib_node_increment_counter(vm, ah_decrypt_node.index, AH_DECRYPT_ERROR_RX_PKTS, from_frame->n_vectors);
