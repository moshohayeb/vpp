...
always_inline ipsec_policy_t *
ipsec_input_ip6_protect_policy_match(ipsec_spd_t *spd, ip6_address_t *sa, ip6_address_t *da, u32 spi)
{
    ipsec_main_t *im = &ipsec_main;
    ipsec_policy_t *p;
    ipsec_sa_t *s;
...
static uword
ipsec_input_ip4_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(ip0->protocol == IP_PROTOCOL_IPSEC_ESP || ip0->protocol == IP_PROTOCOL_UDP)) {
#if 0
	      clib_warning
		("packet received from %U to %U spi %u size %u spd_id %u",
		 ...
		 clib_net_to_host_u16 (ip0->length), spd0->id);
                ...
                p0 = ipsec_input_protect_policy_match(spd0, clib_net_to_host_u32(ip0->src_address.as_u32),
            ...
            trace0:
                if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                    ...
                    if (ip0->protocol == IP_PROTOCOL_IPSEC_ESP || ip0->protocol == IP_PROTOCOL_UDP) {
                        if (p0)
                            tr->sa_id = p0->sa_id;
    ...
    vlib_node_increment_counter(vm, ipsec_input_ip4_node.index, IPSEC_INPUT_ERROR_RX_PKTS, from_frame->n_vectors);
...
static uword
ipsec_input_ip6_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    u32 n_left_from, *from, next_index, *to_next;
    ipsec_main_t *im = &ipsec_main;
    ...
    while (n_left_from > 0) {
		...
		("packet received from %U to %U spi %u size %u spd_id %u",
		 ...
		 spd0->id);
#endif
                p0 = ipsec_input_ip6_protect_policy_match(spd0, &ip0->src_address, &ip0->dst_address,
                                                          clib_net_to_host_u32(esp0->spi));
    ...
    vlib_node_increment_counter(vm, ipsec_input_ip6_node.index, IPSEC_INPUT_ERROR_RX_PKTS, from_frame->n_vectors);
