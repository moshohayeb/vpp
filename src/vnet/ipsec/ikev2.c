...
ikev2_select_proposal(ikev2_sa_proposal_t *proposals, ikev2_protocol_id_t prot_id)
{
    ...
    if (prot_id == IKEV2_PROTOCOL_IKE) {
        mandatory_bitmap = (1 << IKEV2_TRANSFORM_TYPE_ENCR) | (1 << IKEV2_TRANSFORM_TYPE_PRF) |
...
ikev2_child_sa_t *
ikev2_sa_get_child(ikev2_sa_t *sa, u32 spi, ikev2_protocol_id_t prot_id, int by_initiator)
{
    ikev2_child_sa_t *c;
...
ikev2_generate_sa_init_data(ikev2_sa_t *sa)
{
    ...
    if (!t) {
        clib_warning("unknown dh data group %u (data len %u)", sa->dh_group, vec_len(sa->i_dh_data));
...
ikev2_process_sa_init_req(vlib_main_t *vm, ikev2_sa_t *sa, ike_header_t *ike)
{
    ...
    clib_warning("ispi %lx rspi %lx nextpayload %x version %x "
                 ...
                 clib_net_to_host_u64(ike->ispi), clib_net_to_host_u64(ike->rspi), payload, ike->version, ike->exchange,
...
ikev2_decrypt_sk_payload(ikev2_sa_t *sa, ike_header_t *ike, u8 *payload)
{
    ...
    hmac = ikev2_calc_integr(tr_integ, sa->is_initiator ? sa->sk_ar : sa->sk_ai, (u8 *) ike, len - tr_integ->key_trunc);
...
ikev2_initial_contact_cleanup(ikev2_sa_t *sa)
{
    ...
    pool_foreach(tmp, km->per_thread_data[thread_index].sas, ({
                     if (tmp->i_id.type != sa->i_id.type || vec_len(tmp->i_id.data) != vec_len(sa->i_id.data) ||
    ...
    for (i = 0; i < vec_len(delete); i++) {
        tmp = pool_elt_at_index(km->per_thread_data[thread_index].sas, delete[i]);
        vec_foreach(c, tmp->childs) ikev2_delete_tunnel_interface(km->vnet_main, tmp, c);
        ikev2_delete_sa(tmp);
    }
...
ikev2_process_auth_req(vlib_main_t *vm, ikev2_sa_t *sa, ike_header_t *ike)
{
    ...
    while (p < vec_len(plaintext) && payload != IKEV2_PAYLOAD_NONE) {
        ...
        } else if (payload == IKEV2_PAYLOAD_IDI) /* 35 */
        {
            ...
            clib_warning("received payload IDi, len %u id_type %u", plen - sizeof(*id), id->id_type);
        } else if (payload == IKEV2_PAYLOAD_IDR) /* 36 */
        {
            ...
            clib_warning("received payload IDr len %u id_type %u", plen - sizeof(*id), id->id_type);
        } else if (payload == IKEV2_PAYLOAD_AUTH) /* 39 */
        {
            ...
            clib_warning("received payload AUTH, len %u auth_type %u", plen - sizeof(*a), a->auth_method);
        ...
        } else if (payload == IKEV2_PAYLOAD_TSR) /* 45 */
        {
            ...
            clib_warning("unknown payload %u flags %x length %u data %u", payload, ikep->flags, plen - 4,
...
{
    if (ts1->ts_type == ts2->ts_type && ts1->protocol_id == ts2->protocol_id && ts1->start_port == ts2->start_port &&
        ts1->end_port == ts2->end_port && ts1->start_addr.as_u32 == ts2->start_addr.as_u32 &&
...
ikev2_sa_match_ts(ikev2_sa_t *sa)
{
    ...
    pool_foreach(p, km->profiles, ({
                     ...
                     /* check id */
                     if (p->rem_id.type != id->type || vec_len(p->rem_id.data) != vec_len(id->data) ||
...
ikev2_sa_auth(ikev2_sa_t *sa)
{
    ...
    pool_foreach(p, km->profiles, ({
                     /* check id */
                     if (p->rem_id.type != sa_id->type || vec_len(p->rem_id.data) != vec_len(sa_id->data) ||
                         memcmp(p->rem_id.data, sa_id->data, vec_len(p->rem_id.data)))
                         continue;

                     if (sa_auth->method == IKEV2_AUTH_METHOD_SHARED_KEY_MIC) {
                         if (!p->auth.data || p->auth.method != IKEV2_AUTH_METHOD_SHARED_KEY_MIC)
                             continue;
...
ikev2_create_tunnel_interface(vnet_main_t *vnm, ikev2_sa_t *sa, ikev2_child_sa_t *child)
{
    ...
    if (sa->profile && sa->profile->lifetime) {
        ...
        if (sa->profile->lifetime_jitter) {
            ...
            child->time_to_expiration += 1 + (rnd % sa->profile->lifetime_jitter);
        }
    }
...
ikev2_generate_message(ikev2_sa_t *sa, ike_header_t *ike, void *user)
{
    ...
    if (ike->exchange == IKEV2_EXCHANGE_SA_INIT) {
        ...
        integ = ikev2_calc_integr(tr_integ, sa->is_initiator ? sa->sk_ai : sa->sk_ar, (u8 *) ike,
                                  tlen - tr_integ->key_trunc);

        clib_memcpy(ike->payload + tlen - tr_integ->key_trunc - sizeof(*ike), integ, tr_integ->key_trunc);
...
static int
ikev2_retransmit_sa_init(ike_header_t *ike, ip4_address_t iaddr, ip4_address_t raddr)
{
    ikev2_main_t *km = &ikev2_main;
    ikev2_sa_t *sa;
    u32 thread_index = vlib_get_thread_index();
    ...
    pool_foreach(sa, km->per_thread_data[thread_index].sas, ({
                     if (sa->ispi == clib_net_to_host_u64(ike->ispi) && sa->iaddr.as_u32 == iaddr.as_u32 &&
                             ...
                             if (payload == IKEV2_PAYLOAD_NONCE) {
                                 if (!memcmp(sa->i_nonce, ikep->payload, plen - sizeof(*ikep))) {
                                     ...
                                     if (sa->state == IKEV2_STATE_SA_INIT) {
                                         ...
                                         clib_warning("IKE_SA_INIT retransmit from %U to %U", format_ip4_address,
                                                      &raddr, format_ip4_address, &iaddr);
                                         return 1;
                                     }
                                     /* else ignore req */
                                     else {
                                         clib_warning("IKE_SA_INIT ignore from %U to %U", format_ip4_address, &raddr,
...
ikev2_retransmit_resp(ikev2_sa_t *sa, ike_header_t *ike)
{
    ...
    /* retransmitted req */
        ...
        ike->msgid       = tmp->msgid;
        ike->length      = tmp->length;
        clib_memcpy(ike->payload, tmp->payload, clib_net_to_host_u32(tmp->length) - sizeof(*ike));
        clib_warning("IKE msgid %u retransmit from %U to %U", msg_id, format_ip4_address, &sa->raddr,
                     format_ip4_address, &sa->iaddr);
        return 1;
    }
    /* old req ignore */
    else {
        clib_warning("IKE msgid %u req ignore from %U to %U", msg_id, format_ip4_address, &sa->raddr,
                     format_ip4_address, &sa->iaddr);
        return -1;
    }
}
...
static uword
ikev2_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (ike0->version != IKE_VERSION_2) {
                vlib_node_increment_counter(vm, ikev2_node.index, IKEV2_ERROR_NOT_IKEV2, 1);
                goto dispatch0;
            }
            ...
            if (ike0->exchange == IKEV2_EXCHANGE_SA_INIT) {
                ...
                if (ike0->flags & IKEV2_HDR_FLAG_INITIATOR) {
                    if (ike0->rspi == 0) {
                        ...
                        if (r == 1) {
                            vlib_node_increment_counter(vm, ikev2_node.index, IKEV2_ERROR_IKE_SA_INIT_RETRANSMIT, 1);
                            len = clib_net_to_host_u32(ike0->length);
                            goto dispatch0;
                        } else if (r == -1) {
                            vlib_node_increment_counter(vm, ikev2_node.index, IKEV2_ERROR_IKE_SA_INIT_IGNORE, 1);
                            goto dispatch0;
                        }
                        ...
                        if (sa0->state == IKEV2_STATE_SA_INIT) {
                            ...
                            hash_set(km->per_thread_data[thread_index].sa_by_rspi, sa0->rspi,
                ...
                uword *p;
                p = hash_get(km->per_thread_data[thread_index].sa_by_rspi, clib_net_to_host_u64(ike0->rspi));
                if (p) {
                    sa0 = pool_elt_at_index(km->per_thread_data[thread_index].sas, p[0]);

                    r = ikev2_retransmit_resp(sa0, ike0);
                    if (r == 1) {
                        vlib_node_increment_counter(vm, ikev2_node.index, IKEV2_ERROR_IKE_REQ_RETRANSMIT, 1);
                        len = clib_net_to_host_u32(ike0->length);
                        goto dispatch0;
                    } else if (r == -1) {
                        vlib_node_increment_counter(vm, ikev2_node.index, IKEV2_ERROR_IKE_REQ_IGNORE, 1);
                        goto dispatch0;
                    }
                    ...
                    if (sa0->state == IKEV2_STATE_AUTHENTICATED) {
                        ...
                        if (sa0->state != IKEV2_STATE_TS_UNACCEPTABLE)
                            ikev2_create_tunnel_interface(km->vnet_main, sa0, &sa0->childs[0]);
                    }
                ...
                if (p) {
                    ...
                    if (sa0->rekey) {
                        if (sa0->rekey[0].protocol_id != IKEV2_PROTOCOL_IKE) {
                            ...
                            ikev2_create_tunnel_interface(km->vnet_main, sa0, child);
                        }
                        if (sa0->is_initiator) {
                ...
                clib_warning("IKEv2 exchange %u packet received from %U to %U", ike0->exchange, format_ip4_address,
                             ip40->src_address.as_u8, format_ip4_address, ip40->dst_address.as_u8);
            }
        ...
        dispatch0:
            ...
            if (sa0 && (sa0->state == IKEV2_STATE_DELETED || sa0->state == IKEV2_STATE_NOTIFY_AND_DELETE)) {
                ...
                vec_foreach(c, sa0->childs) ikev2_delete_tunnel_interface(km->vnet_main, sa0, c);
    ...
    vlib_node_increment_counter(vm, ikev2_node.index, IKEV2_ERROR_PROCESSED, frame->n_vectors);
...
static clib_error_t *
ikev2_set_initiator_proposals(vlib_main_t *vm, ikev2_sa_t *sa, ikev2_transforms_set *ts,
                              ikev2_sa_proposal_t **proposals, int is_ike)
{
    ...
    vec_foreach(td, km->supported_transforms)
    {
        if (td->type == IKEV2_TRANSFORM_TYPE_ENCR && td->encr_type == ts->crypto_alg &&
    ...
    if (error) {
        clib_warning("Didn't find any supported algorithm for IKEV2_TRANSFORM_TYPE_INTEG");
        r = clib_error_return(0, "Unsupported algorithm");
        return r;
    }
...
static void
ikev2_send_ike(vlib_main_t *vm, ip4_address_t *src, ip4_address_t *dst, u32 bi0, u32 len)
{
    ip4_header_t *ip40;
    udp_header_t *udp0;
    vlib_buffer_t *b0;
...
clib_error_t *
ikev2_set_profile_auth(vlib_main_t *vm, u8 *name, u8 auth_method, u8 *auth_data, u8 data_hex_format)
{
    ikev2_profile_t *p;
    clib_error_t *r;
...
clib_error_t *
ikev2_set_profile_id(vlib_main_t *vm, u8 *name, u8 id_type, u8 *data, int is_local)
{
    ikev2_profile_t *p;
    clib_error_t *r;
...
clib_error_t *
ikev2_set_profile_ts(vlib_main_t *vm, u8 *name, u8 protocol_id, u16 start_port, u16 end_port, ip4_address_t start_addr,
...
clib_error_t *
ikev2_set_profile_responder(vlib_main_t *vm, u8 *name, u32 sw_if_index, ip4_address_t ip4)
{
    ikev2_profile_t *p;
    clib_error_t *r;
...
clib_error_t *
ikev2_set_profile_esp_transforms(vlib_main_t *vm, u8 *name, ikev2_transform_encr_type_t crypto_alg,
                                 ikev2_transform_integ_type_t integ_alg, ikev2_transform_dh_type_t dh_type,
                                 u32 crypto_key_size)
{
    ikev2_profile_t *p;
...
clib_error_t *
ikev2_set_profile_sa_lifetime(vlib_main_t *vm, u8 *name, u64 lifetime, u32 jitter, u32 handover, u64 maxdata)
{
    ikev2_profile_t *p;
    clib_error_t *r;
...
ikev2_initiate_sa_init(vlib_main_t *vm, u8 *name)
{
    ...
    /* Create the Initiator Request */
    {
        ...
        /* Get own iface IP */
        if_add_index0                  = lm->if_address_pool_index_by_sw_if_index[p->responder.sw_if_index];
        ip_interface_address_t *if_add = pool_elt_at_index(lm->if_address_pool, if_add_index0);
        ...
        clib_memcpy(&nat_detection_source[8 + 8 + 4], &tmpport, sizeof(tmpport));
        u32 tmpip = clib_host_to_net_u32(if_ip->as_u32);
        clib_memcpy(&nat_detection_source[8 + 8], &tmpip, sizeof(tmpip));
        SHA1(nat_detection_source, sizeof(nat_detection_source), nat_detection_sha1);
...
static void
ikev2_delete_child_sa_internal(vlib_main_t *vm, ikev2_sa_t *sa, ikev2_child_sa_t *csa)
{
...
static void
ikev2_rekey_child_sa_internal(vlib_main_t *vm, ikev2_sa_t *sa, ikev2_child_sa_t *csa)
{
...
ikev2_init(vlib_main_t *vm)
{
    ...
    for (thread_id = 0; thread_id < tm->n_vlib_mains - 1; thread_id++) {
        km->per_thread_data[thread_id].sa_by_rspi = hash_create(0, sizeof(uword));
    }
...
ikev2_mngr_process_child_sa(ikev2_sa_t *sa, ikev2_child_sa_t *csa)
{
    ...
    if (sa->is_initiator && sa->profile && csa->time_to_expiration && now > csa->time_to_expiration) {
        if (!csa->is_expired || csa->rekey_retries > 0) {
            ...
            if (csa->rekey_retries == 0) {
                ...
                csa->rekey_retries--;
                clib_warning("Rekeying Child SA 0x%x, retries left %d", csa->i_proposals->spi, csa->rekey_retries);
                if (csa->rekey_retries == 0) {
                    csa->rekey_retries = -1;
                }
            }
...
ikev2_mngr_process_ipsec_sa(ipsec_sa_t *ipsec_sa)
{
    ...
    if (fchild && fsa && fsa->profile && fsa->profile->lifetime_maxdata) {
        if (!fchild->is_expired && ipsec_sa->total_data_size > fsa->profile->lifetime_maxdata) {
...
static uword
ikev2_mngr_process_fn(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ikev2_main_t *km = &ikev2_main;
    ...
    while (1) {
        ...
        ipsec_sa_t *sa;
        /* *INDENT-OFF* */
        pool_foreach(sa, im->sad, ({ ikev2_mngr_process_ipsec_sa(sa); }));
        /* *INDENT-ON* */
