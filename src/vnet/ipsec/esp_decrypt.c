...
format_esp_decrypt_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "esp: crypto %U integrity %U", format_ipsec_crypto_alg, t->crypto_alg, format_ipsec_integ_alg,
               t->integ_alg);
    return s;
}

always_inline void
esp_decrypt_cbc(ipsec_crypto_alg_t alg, u8 *in, u8 *out, size_t in_len, u8 *key, u8 *iv)
{
    ...
    if (PREDICT_FALSE(alg != em->per_thread_data[thread_index].last_decrypt_alg)) {
...
static uword
esp_decrypt_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    if (PREDICT_FALSE(vec_len(empty_buffers) < n_left_from)) {
        vlib_node_increment_counter(vm, esp_decrypt_node.index, ESP_DECRYPT_ERROR_NO_BUFFER, n_left_from);
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (sa0->use_anti_replay) {
                ...
                if (PREDICT_FALSE(rv)) {
                    ...
                    vlib_node_increment_counter(vm, esp_decrypt_node.index, ESP_DECRYPT_ERROR_REPLAY, 1);
                    o_bi0      = i_bi0;
                    to_next[0] = o_bi0;
                    to_next += 1;
                    goto trace;
            ...
            if (PREDICT_TRUE(sa0->integ_alg != IPSEC_INTEG_ALG_NONE)) {
                ...
                hmac_calc(sa0->integ_alg, sa0->integ_key, sa0->integ_key_len, (u8 *) esp0, i_b0->current_length, sig,
            ...
            if ((sa0->crypto_alg >= IPSEC_CRYPTO_ALG_AES_CBC_128 && sa0->crypto_alg <= IPSEC_CRYPTO_ALG_AES_CBC_256) ||
                (sa0->crypto_alg >= IPSEC_CRYPTO_ALG_DES_CBC && sa0->crypto_alg <= IPSEC_CRYPTO_ALG_3DES_CBC)) {
                ...
                esp_decrypt_cbc(sa0->crypto_alg, esp0->data + IV_SIZE, (u8 *) vlib_buffer_get_current(o_b0) + ip_hdr_size,
                                BLOCK_SIZE * blocks, sa0->crypto_key, esp0->data);
                ...
                if (PREDICT_TRUE(tunnel_mode)) {
                    if (PREDICT_TRUE(f0->next_header == IP_PROTOCOL_IP_IN_IP)) {
                        ...
                        vlib_node_increment_counter(vm, esp_decrypt_node.index, ESP_DECRYPT_ERROR_DECRYPTION_FAILED, 1);
                        o_b0 = 0;
                        goto trace;
                    }
                }
                /* transport mode */
                    ...
                    if (PREDICT_FALSE(transport_ip6)) {
                        next0                                        = ESP_DECRYPT_NEXT_IP6_INPUT;
                        oh6->ip_version_traffic_class_and_flow_label = ih6->ip_version_traffic_class_and_flow_label;
                        ...
                        oh4->length                       = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, o_b0));
    ...
    vlib_node_increment_counter(vm, esp_decrypt_node.index, ESP_DECRYPT_ERROR_RX_PKTS, from_frame->n_vectors);
