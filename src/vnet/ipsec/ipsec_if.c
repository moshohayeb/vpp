...
static uword
ipsec_if_tx_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ipsec_main_t *im           = &ipsec_main;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(sw_if_index0 == last_sw_if_index)) {
                ...
                vlib_increment_combined_counter(vim->combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX, thread_index,
                                                sw_if_index0, n_packets, n_bytes);
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ipsec_if_tx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
...
{
    vl_api_rpc_call_main_thread(ipsec_add_del_tunnel_if_rpc_callback, (u8 *) args, sizeof(*args));
    return 0;
}

int
ipsec_add_del_tunnel_if_internal(vnet_main_t *vnm, ipsec_add_del_tunnel_args_t *args, u32 *sw_if_index)
{
    ipsec_tunnel_if_t *t;
    ipsec_main_t *im        = &ipsec_main;
    ...
    if (args->is_add) {
        ...
        hash_set(im->ipsec_if_real_dev_by_show_dev, t->show_instance, dev_instance);
        ...
        hw_if_index = vnet_register_interface(vnm, ipsec_device_class.index, t - im->tunnel_interfaces,
                                              ipsec_hw_class.index, t - im->tunnel_interfaces);

        hi = vnet_get_hw_interface(vnm, hw_if_index);

        slot = vlib_node_add_next_with_slot(vnm->vlib_main, hi->tx_node_index, im->esp_encrypt_node_index,
        ...
        vnet_feature_enable_disable("interface-output", "ipsec-if-output", hi->sw_if_index, 1, 0, 0);
        ...
        vnet_feature_enable_disable("interface-output", "ipsec-if-output", hi->sw_if_index, 0, 0, 0);
...
int
ipsec_add_del_ipsec_gre_tunnel(vnet_main_t *vnm, ipsec_add_del_ipsec_gre_tunnel_args_t *args)
{
...
int
ipsec_set_interface_key(vnet_main_t *vnm, u32 hw_if_index, ipsec_if_set_key_type_t type, u8 alg, u8 *key)
{
    ipsec_main_t *im = &ipsec_main;
    vnet_hw_interface_t *hi;
    ipsec_tunnel_if_t *t;
...
int
ipsec_set_interface_sa(vnet_main_t *vnm, u32 hw_if_index, u32 sa_id, u8 is_outbound)
{
    ipsec_main_t *im = &ipsec_main;
    vnet_hw_interface_t *hi;
    ipsec_tunnel_if_t *t;
    ...
    if (!is_outbound) {
        ...
        key = (u64) old_sa->tunnel_src_addr.ip4.as_u32 << 32 | (u64) old_sa->spi;
        p   = hash_get(im->ipsec_if_pool_index_by_key, key);
        if (p)
            hash_unset(im->ipsec_if_pool_index_by_key, key);
