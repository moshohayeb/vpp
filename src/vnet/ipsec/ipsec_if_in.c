...
static uword
ipsec_if_input_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            u64 key = (u64) ip0->src_address.as_u32 << 32 | (u64) clib_net_to_host_u32(esp0->spi);
            ...
            if (p) {
                ...
                if (t->hw_if_index != ~0) {
                    ...
                    if (PREDICT_FALSE(!(hi->flags & VNET_HW_INTERFACE_FLAG_LINK_UP))) {
                    ...
                    if (PREDICT_TRUE(sw_if_index0 == last_sw_if_index)) {
                        ...
                        n_bytes -= n_packets * (sizeof(ip4_header_t) + sizeof(esp_header_t) + sizeof(esp_footer_t) +
                                                16 /* aes-cbc IV */ + icv_len);

                        if (last_t) {
                            vlib_increment_combined_counter(rx_counter, thread_index, sw_if_index0, n_packets, n_bytes);
                        }
    ...
    if (last_t) {
        ...
        n_bytes -= n_packets *
                   (sizeof(ip4_header_t) + sizeof(esp_header_t) + sizeof(esp_footer_t) + 16 /* aes-cbc IV */ + icv_len);
        vlib_increment_combined_counter(rx_counter, thread_index, last_sw_if_index, n_packets, n_bytes);
    }

    vlib_node_increment_counter(vm, ipsec_if_input_node.index, IPSEC_IF_INPUT_ERROR_RX,
                                from_frame->n_vectors - n_disabled);

    vlib_node_increment_counter(vm, ipsec_if_input_node.index, IPSEC_IF_INPUT_ERROR_DISABLED, n_disabled);
