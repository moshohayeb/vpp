...
{
    if ((memcmp(a->as_u64, la->as_u64, 2 * sizeof(u64)) >= 0) && (memcmp(a->as_u64, ua->as_u64, 2 * sizeof(u64)) <= 0))
        return 1;
    return 0;
}

always_inline ipsec_policy_t *
ipsec_output_ip6_policy_match(ipsec_spd_t *spd, ip6_address_t *la, ip6_address_t *ra, u16 lp, u16 rp, u8 pr)
{
    ipsec_policy_t *p;
    u32 *i;
...
static inline uword
ipsec_output_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ipv6)
{
    ...
    while (n_left_from > 0) {
	  ...
	  clib_warning
	    ("packet received from %U port %u to %U port %u spd_id %u",
	     ...
	     spd0->id);
            ...
            p0 = ipsec_output_ip6_policy_match(spd0, &ip6_0->src_address, &ip6_0->dst_address,
	  ...
	  clib_warning ("sw_if_index0 %u spd_index0 %u spd_id %u",
            ...
            p0 = ipsec_output_policy_match(spd0, ip0->protocol, clib_net_to_host_u32(ip0->src_address.as_u32),
                                           clib_net_to_host_u32(ip0->dst_address.as_u32),
                                           clib_net_to_host_u16(udp0->src_port), clib_net_to_host_u16(udp0->dst_port));
        }
        tcp0 = (void *) udp0;
        ...
        if (PREDICT_TRUE(p0 != NULL)) {
            if (p0->policy == IPSEC_POLICY_ACTION_PROTECT) {
                ...
                if (is_ipv6) {
                    ...
                    if (PREDICT_FALSE(b0->flags & VNET_BUFFER_F_OFFLOAD_TCP_CKSUM)) {
                        tcp0->checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b0, ip6_0, &bogus);
                        b0->flags &= ~VNET_BUFFER_F_OFFLOAD_TCP_CKSUM;
                    }
                    if (PREDICT_FALSE(b0->flags & VNET_BUFFER_F_OFFLOAD_UDP_CKSUM)) {
...
static uword
ipsec_output_ip4_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ipsec_output_ip6_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ipsec_output_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
