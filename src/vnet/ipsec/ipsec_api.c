...
static void
vl_api_ipsec_interface_add_del_spd_t_handler(vl_api_ipsec_interface_add_del_spd_t *mp)
{
    vlib_main_t *vm __attribute__((unused)) = vlib_get_main();
...
static void
vl_api_ipsec_spd_add_del_entry_t_handler(vl_api_ipsec_spd_add_del_entry_t *mp)
{
    vlib_main_t *vm __attribute__((unused)) = vlib_get_main();
...
static void
vl_api_ipsec_sad_add_del_entry_t_handler(vl_api_ipsec_sad_add_del_entry_t *mp)
{
    vlib_main_t *vm __attribute__((unused)) = vlib_get_main();
    ...
    if (mp->crypto_algorithm >= IPSEC_CRYPTO_N_ALG) {
        clib_warning("unsupported crypto-alg: '%U'", format_ipsec_crypto_alg, mp->crypto_algorithm);
        rv = VNET_API_ERROR_UNIMPLEMENTED;
        goto out;
    }
    ...
    if (mp->integrity_algorithm >= IPSEC_INTEG_N_ALG) {
        clib_warning("unsupported integ-alg: '%U'", format_ipsec_integ_alg, mp->integrity_algorithm);
        rv = VNET_API_ERROR_UNIMPLEMENTED;
        goto out;
    }
...
static void
send_ipsec_spds_details(ipsec_spd_t *spd, vl_api_registration_t *reg, u32 context)
{
...
vl_api_ipsec_spds_dump_t_handler(vl_api_ipsec_spds_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(spd, im->spds, ({ send_ipsec_spds_details(spd, reg, mp->context); }));
    /* *INDENT-ON* */
#else
    clib_warning("unimplemented");
#endif
...
static void
vl_api_ipsec_tunnel_if_add_del_t_handler(vl_api_ipsec_tunnel_if_add_del_t *mp)
{
    ...
    memcpy(&tun.local_crypto_key, &mp->local_crypto_key, mp->local_crypto_key_len);
    memcpy(&tun.remote_crypto_key, &mp->remote_crypto_key, mp->remote_crypto_key_len);
    memcpy(&tun.local_integ_key, &mp->local_integ_key, mp->local_integ_key_len);
    memcpy(&tun.remote_integ_key, &mp->remote_integ_key, mp->remote_integ_key_len);
    ...
    REPLY_MACRO2(VL_API_IPSEC_TUNNEL_IF_ADD_DEL_REPLY, ({ rmp->sw_if_index = htonl(sw_if_index); }));
}

static void
send_ipsec_sa_details(ipsec_sa_t *sa, vl_api_registration_t *reg, u32 context, u32 sw_if_index)
{
...
vl_api_ipsec_sa_dump_t_handler(vl_api_ipsec_sa_dump_t *mp)
{
    ...
    vec_validate_init_empty(sa_index_to_tun_if_index, vec_len(im->sad) - 1, ~0);
    ...
    pool_foreach(sa, im->sad, ({
                     if (mp->sa_id == ~(0) || ntohl(mp->sa_id) == sa->id)
                         send_ipsec_sa_details(sa, reg, mp->context, sa_index_to_tun_if_index[sa - im->sad]);
                 }));
    /* *INDENT-ON* */
...
static void
vl_api_ipsec_tunnel_if_set_key_t_handler(vl_api_ipsec_tunnel_if_set_key_t *mp)
{
    ...
    case IPSEC_IF_SET_KEY_TYPE_REMOTE_CRYPTO:
        if (mp->alg < IPSEC_CRYPTO_ALG_AES_CBC_128 || mp->alg >= IPSEC_CRYPTO_N_ALG) {
            rv = VNET_API_ERROR_UNIMPLEMENTED;
            goto out;
        }
        break;
    ...
    rv = ipsec_set_interface_key(vnm, sw->hw_if_index, mp->key_type, mp->alg, key);
    vec_free(key);
#else
    clib_warning("unimplemented");
#endif
...
vl_api_ipsec_tunnel_if_set_sa_t_handler(vl_api_ipsec_tunnel_if_set_sa_t *mp)
{
    ...
    rv = ipsec_set_interface_sa(vnm, sw->hw_if_index, ntohl(mp->sa_id), mp->is_outbound);
#else
    clib_warning("unimplemented");
#endif
...
vl_api_ikev2_profile_set_ts_t_handler(vl_api_ikev2_profile_set_ts_t *mp)
{
    ...
    clib_error_t *error;
    u8 *tmp = format(0, "%s", mp->name);
    error   = ikev2_set_profile_ts(vm, tmp, mp->proto, mp->start_port, mp->end_port, (ip4_address_t) mp->start_addr,
...
static void
vl_api_ikev2_set_ike_transforms_t_handler(vl_api_ikev2_set_ike_transforms_t *mp)
{
    ...
    error = ikev2_set_profile_ike_transforms(vm, tmp, mp->crypto_alg, mp->integ_alg, mp->dh_group, mp->crypto_key_size);
    vec_free(tmp);
    if (error)
        rv = VNET_API_ERROR_UNSPECIFIED;
#else
...
static void
vl_api_ikev2_set_esp_transforms_t_handler(vl_api_ikev2_set_esp_transforms_t *mp)
{
    ...
    error = ikev2_set_profile_esp_transforms(vm, tmp, mp->crypto_alg, mp->integ_alg, mp->dh_group, mp->crypto_key_size);
    vec_free(tmp);
    if (error)
        rv = VNET_API_ERROR_UNSPECIFIED;
#else
...
vl_api_ikev2_set_sa_lifetime_t_handler(vl_api_ikev2_set_sa_lifetime_t *mp)
{
    ...
    error = ikev2_set_profile_sa_lifetime(vm, tmp, mp->lifetime, mp->lifetime_jitter, mp->handover, mp->lifetime_maxdata);
    vec_free(tmp);
    if (error)
        rv = VNET_API_ERROR_UNSPECIFIED;
#else
...
static void
vl_api_ikev2_initiate_del_ike_sa_t_handler(vl_api_ikev2_initiate_del_ike_sa_t *mp)
{
...
static void
vl_api_ikev2_initiate_del_child_sa_t_handler(vl_api_ikev2_initiate_del_child_sa_t *mp)
{
...
static void
vl_api_ikev2_initiate_rekey_child_sa_t_handler(vl_api_ikev2_initiate_rekey_child_sa_t *mp)
{
