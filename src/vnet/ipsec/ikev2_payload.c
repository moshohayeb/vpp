...
void
ikev2_payload_add_notify_2(ikev2_payload_chain_t *c, u16 msg_type, u8 *data, ikev2_notify_t *notify)
{
...
ikev2_payload_add_sa(ikev2_payload_chain_t *c, ikev2_sa_proposal_t *proposals)
{
    ...
    vec_foreach(p, proposals)
    {
        ...
        DBG_PLD("proposal num %u protocol_id %u last_or_more %u spi_size %u%s%U", prop->proposal_num, prop->protocol_id,
                prop->last_or_more, prop->spi_size, prop->spi_size ? " spi_data " : "", format_hex_bytes, prop->spi,
                prop->spi_size);
        ...
        vec_foreach(t, p->transforms)
        {
            ...
            DBG_PLD("transform type %U transform_id %u last_or_more %u attr_size %u%s%U", format_ikev2_transform_type,
                    tr->transform_type, t->transform_id, tr->last_or_more, vec_len(t->attrs),
                    vec_len(t->attrs) ? " attrs " : "", format_hex_bytes, tr->attributes, vec_len(t->attrs));
        }
...
ikev2_parse_sa_payload(ike_payload_header_t *ikep)
{
    ...
    do {
        ...
        DBG_PLD("proposal num %u len %u last_or_more %u id %u "
                ...
                sap->proposal_num, clib_net_to_host_u16(sap->proposal_len), sap->last_or_more, sap->protocol_id,
                sap->spi_size, sap->num_transforms);
        ...
        for (i = 0; i < sap->num_transforms; i++) {
            ...
            DBG_PLD("transform num %u len %u last_or_more %u type %U id %u%s%U", i, tlen, tr->last_or_more,
                    format_ikev2_sa_transform, transform, clib_net_to_host_u16(tr->transform_id),
                    tlen > sizeof(*tr) ? " attrs " : "", format_hex_bytes, tr->attributes, tlen - sizeof(*tr));
...
ikev2_parse_notify_payload(ike_payload_header_t *ikep)
{
    ...
    DBG_PLD("msg_type %U len %u%s%U", format_ikev2_notify_msg_type, clib_net_to_host_u16(n->msg_type), plen,
            plen > sizeof(*n) ? " data " : "", format_hex_bytes, n->payload, plen - sizeof(*n));
...
ikev2_parse_vendor_payload(ike_payload_header_t *ikep)
{
    ...
    DBG_PLD("len %u data %s:%U", plen, is_string ? "string" : "hex", is_string ? format_ascii_bytes : format_hex_bytes,
...
ikev2_parse_delete_payload(ike_payload_header_t *ikep)
{
    ...
    DBG_PLD("protocol_id %u spi_size %u num_of_spi %u len %u%s%U", d->protocol_id, d->spi_size, num_of_spi, plen,
            plen > sizeof(d) ? " data " : "", format_hex_bytes, d->spi, plen - sizeof(*d));
