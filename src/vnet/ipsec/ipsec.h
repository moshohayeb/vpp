...
typedef enum { IPSEC_PROTOCOL_AH = 0, IPSEC_PROTOCOL_ESP = 1 } ipsec_protocol_t;
...
ipsec_alloc_empty_buffers(vlib_main_t *vm, ipsec_main_t *im)
{
    ...
    if (PREDICT_FALSE(l < VLIB_FRAME_SIZE)) {
        ...
        n_alloc = vlib_buffer_alloc(vm, im->empty_buffers[thread_index] + l, 2 * VLIB_FRAME_SIZE - l);
