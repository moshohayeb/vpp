...
static uword
unformat_ethernet_arp_opcode_host_byte_order(unformat_input_t *input, va_list *args)
{
...
static uword
unformat_ethernet_arp_opcode_net_byte_order(unformat_input_t *input, va_list *args)
{
    int *result = va_arg(*args, int *);
...
format_ethernet_arp_header(u8 *s, va_list *va)
{
    ...
    s = format(s, "%U, type %U/%U, address size %d/%d", format_ethernet_arp_opcode, clib_net_to_host_u16(a->opcode),
               format_ethernet_arp_hardware_type, l2_type, format_ethernet_type, l3_type, a->n_l2_address_bytes,
    ...
    if (l2_type == ETHERNET_ARP_HARDWARE_TYPE_ethernet && l3_type == ETHERNET_TYPE_IP4) {
        s = format(s, "\n%U%U/%U -> %U/%U", format_white_space, indent, format_ethernet_address,
                   a->ip4_over_ethernet[0].ethernet, format_ip4_address, &a->ip4_over_ethernet[0].ip4,
                   format_ethernet_address, a->ip4_over_ethernet[1].ethernet, format_ip4_address,
                   &a->ip4_over_ethernet[1].ip4);
    } else {
        uword n2 = a->n_l2_address_bytes;
        uword n3 = a->n_l3_address_bytes;
        s = format(s, "\n%U%U/%U -> %U/%U", format_white_space, indent, format_hex_bytes, a->data + 0 * n2 + 0 * n3, n2,
                   format_hex_bytes, a->data + 1 * n2 + 0 * n3, n3, format_hex_bytes, a->data + 1 * n2 + 1 * n3, n2,
...
format_ethernet_arp_ip4_entry(u8 *s, va_list *va)
{
    ...
    if (!e)
        return format(s, "%=12s%=16s%=6s%=20s%=24s", "Time", "IP4", "Flags", "Ethernet", "Interface");
    ...
    s = format(s, "%=12U%=16U%=6s%=20U%U", format_vlib_time, vnm->vlib_main, e->time_last_updated, format_ip4_address,
               &e->ip4_address, flags ? (char *) flags : "", format_ethernet_address, e->ethernet_address,
...
format_arp_term_input_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "%U", t->packet_data[0] == 0 ? format_ethernet_arp_header : format_ip6_header, t->packet_data,
...
arp_nbr_probe(ip_adjacency_t *adj)
{
    ...
    src = ip4_interface_address_matching_destination(im, &adj->sub_type.nbr.next_hop.ip4,
                                                     adj->rewrite_header.sw_if_index, &ia);
    if (!src) {
        return;
    }
...
arp_mk_complete(adj_index_t ai, ethernet_arp_ip4_entry_t *e)
{
    adj_nbr_update_rewrite(ai, ADJ_NBR_REWRITE_FLAG_COMPLETE,
                           ethernet_build_rewrite(vnet_get_main(), e->sw_if_index, adj_get_link_type(ai),
                                                  e->ethernet_address));
}
...
arp_mk_incomplete(adj_index_t ai)
{
    ...
    adj_nbr_update_rewrite(ai, ADJ_NBR_REWRITE_FLAG_INCOMPLETE,
                           ethernet_build_rewrite(vnet_get_main(), adj->rewrite_header.sw_if_index, VNET_LINK_ARP,
...
arp_update_adjacency(vnet_main_t *vnm, u32 sw_if_index, u32 ai)
{
    ...
    case IP_LOOKUP_NEXT_BCAST:
        adj_nbr_update_rewrite(ai, ADJ_NBR_REWRITE_FLAG_COMPLETE,
                               ethernet_build_rewrite(vnm, sw_if_index, VNET_LINK_IP4,
                                                      VNET_REWRITE_FOR_SW_INTERFACE_ADDRESS_BROADCAST));
        break;
    case IP_LOOKUP_NEXT_MCAST: {
        ...
        rewrite = ethernet_build_rewrite(vnm, sw_if_index, adj->ia_link, ethernet_ip4_mcast_dst_addr());
...
arp_adj_fib_add(ethernet_arp_ip4_entry_t *e, u32 fib_index)
{
    ...
    e->fib_entry_index = fib_table_entry_path_add(fib_index, &pfx, FIB_SOURCE_ADJ, FIB_ENTRY_FLAG_ATTACHED, DPO_PROTO_IP4,
                                                  &pfx.fp_addr, e->sw_if_index, ~0, 1, NULL, FIB_ROUTE_PATH_FLAG_NONE);
...
arp_adj_fib_remove(ethernet_arp_ip4_entry_t *e, u32 fib_index)
{
    if (FIB_NODE_INDEX_INVALID != e->fib_entry_index) {
        ...
        fib_table_entry_path_remove(fib_index, &pfx, FIB_SOURCE_ADJ, DPO_PROTO_IP4, &pfx.fp_addr, e->sw_if_index, ~0, 1,
...
force_reuse_arp_entry(void)
{
    ...
    hash_unset(am->ethernet_arp_by_sw_if_index[e->sw_if_index].arp_entries, e->ip4_address.as_u32);
    arp_adj_fib_remove(e, ip4_fib_table_get_index_for_sw_if_index(e->sw_if_index));
    adj_nbr_walk_nh4(e->sw_if_index, &e->ip4_address, arp_mk_incomplete_walk, e);
    return e;
}

static int
vnet_arp_set_ip4_over_ethernet_internal(vnet_main_t *vnm, vnet_arp_set_ip4_over_ethernet_rpc_args_t *args)
{
    ...
    if (NULL != arp_int->arp_entries) {
        ...
        if (p) {
            ...
            if (!is_static && (e->flags & ETHERNET_ARP_IP4_ENTRY_FLAG_STATIC)) {
                ...
                if (0 == memcmp(e->ethernet_address, a->ethernet, sizeof(e->ethernet_address)))
    ...
    if (make_new_arp_cache_entry) {
        if (am->limit_arp_cache_size && pool_elts(am->ip4_entry_pool) >= am->limit_arp_cache_size) {
        ...
        if (!is_no_fib_entry) {
            arp_adj_fib_add(e, ip4_fib_table_get_index_for_sw_if_index(e->sw_if_index));
        } else {
...
void
vnet_register_ip4_arp_resolution_event(vnet_main_t *vnm, void *address_arg, uword node_index, uword type_opaque,
                                       uword data)
{
    ethernet_arp_main_t *am = &ethernet_arp_main;
    ...
    hash_set(am->pending_resolutions_by_address, address->as_u32, pr - am->pending_resolutions);
}

int
vnet_add_del_ip4_arp_change_event(vnet_main_t *vnm, void *data_callback, u32 pid, void *address_arg, uword node_index,
...
static int
arp_unnumbered(vlib_buffer_t *p0, u32 input_sw_if_index, u32 conn_sw_if_index)
{
    vnet_main_t *vnm           = vnet_get_main();
...
static u32
arp_learn(vnet_main_t *vnm, ethernet_arp_main_t *am, u32 sw_if_index, const ethernet_arp_ip4_over_ethernet_address_t *addr)
{
...
arp_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            error0 = (arp0->l2_type != clib_net_to_host_u16(ETHERNET_ARP_HARDWARE_TYPE_ethernet) ?
                          ...
                          error0);
            error0 =
                (arp0->l3_type != clib_net_to_host_u16(ETHERNET_TYPE_IP4) ? ETHERNET_ARP_ERROR_l3_type_not_ip4 : error0);
            error0 = (0 == arp0->ip4_over_ethernet[0].ip4.as_u32 ? ETHERNET_ARP_ERROR_l3_dst_address_unset : error0);

            sw_if_index0 = vnet_buffer(p0)->sw_if_index[VLIB_RX];

            /* not playing the ARP game if the interface is not IPv4 enabled */
            error0 = (im4->ip_enabled_by_sw_if_index[sw_if_index0] == 0 ? ETHERNET_ARP_ERROR_interface_not_ip_enabled :
                                                                          error0);
            ...
            if (fib_entry_is_sourced(dst_fei, FIB_SOURCE_ADJ)) {
                ...
                if (arp0->ip4_over_ethernet[0].ip4.as_u32 == arp0->ip4_over_ethernet[1].ip4.as_u32)
                    error0 = arp_learn(vnm, am, sw_if_index0, &arp0->ip4_over_ethernet[0]);
                goto drop2;
            ...
            is_vrrp_reply0 = ((arp0->opcode == clib_host_to_net_u16(ETHERNET_ARP_OPCODE_reply)) &&
                              (!memcmp(arp0->ip4_over_ethernet[0].ethernet, vrrp_prefix, sizeof(vrrp_prefix))));

            /* Trash ARP packets whose ARP-level source addresses do not
               match their L2-frame-level source addresses, unless it's
               a reply from a VRRP virtual router */
            if (memcmp(eth_rx->src_address, arp0->ip4_over_ethernet[0].ethernet, sizeof(eth_rx->src_address)) &&
                !is_vrrp_reply0) {
        ...
        send_reply:
            ...
            if (!error0)
                error0 = arp_learn(vnm, am, sw_if_index0, &arp0->ip4_over_ethernet[1]);
        ...
        drop1:
            ...
            if (is_request0) {
                ...
                fib_index0 = vec_elt(im4->fib_index_by_sw_if_index, sw_if_index0);
    ...
    vlib_error_count(vm, node->node_index, ETHERNET_ARP_ERROR_replies_sent, n_replies_sent - n_proxy_arp_replies_sent);

    vlib_error_count(vm, node->node_index, ETHERNET_ARP_ERROR_proxy_arp_replies_sent, n_proxy_arp_replies_sent);
...
static clib_error_t *
show_ip4_arp(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm        = vnet_get_main();
    ethernet_arp_main_t *am = &ethernet_arp_main;
    ...
    if (vec_len(am->proxy_arps)) {
        ...
        vec_foreach(pa, am->proxy_arps)
        {
            vlib_cli_output(vm, "Fib_index %d   %U - %U ", pa->fib_index, format_ip4_address, &pa->lo_addr,
...
unformat_pg_arp_header(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%U: %U/%U -> %U/%U", unformat_pg_edit, unformat_ethernet_arp_opcode_net_byte_order,
                  &p->opcode, unformat_pg_edit, unformat_ethernet_address, &p->ip4_over_ethernet[0].ethernet,
                  unformat_pg_edit, unformat_ip4_address, &p->ip4_over_ethernet[0].ip4, unformat_pg_edit,
                  unformat_ethernet_address, &p->ip4_over_ethernet[1].ethernet, unformat_pg_edit, unformat_ip4_address,
 ...
 */
int
vnet_arp_unset_ip4_over_ethernet(vnet_main_t *vnm, u32 sw_if_index, const ethernet_arp_ip4_over_ethernet_address_t *a)
{
    ...
    vl_api_rpc_call_main_thread(set_ip4_over_ethernet_rpc_callback, (u8 *) &args, sizeof(args));
    return 0;
}
 ...
 */
static int
vnet_arp_wc_publish(u32 sw_if_index, const ethernet_arp_ip4_over_ethernet_address_t *a)
{
    vnet_arp_set_ip4_over_ethernet_rpc_args_t args = {.flags       = ETHERNET_ARP_ARGS_WC_PUB,
...
static void
vnet_arp_wc_publish_internal(vnet_main_t *vnm, vnet_arp_set_ip4_over_ethernet_rpc_args_t *args)
{
    vlib_main_t *vm         = vlib_get_main();
    ethernet_arp_main_t *am = &ethernet_arp_main;
    ...
    if (ni == (uword) ~0)
        return;
    wc_arp_report_t *r = vlib_process_signal_event_data(vm, ni, et, 1, sizeof *r);
...
static int
vnet_arp_flush_ip4_over_ethernet_internal(vnet_main_t *vnm, vnet_arp_set_ip4_over_ethernet_rpc_args_t *args)
{
    ethernet_arp_main_t *am = &ethernet_arp_main;
    ethernet_arp_ip4_entry_t *e;
 ...
 */
static void
arp_add_del_interface_address(ip4_main_t *im, uword opaque, u32 sw_if_index, ip4_address_t *address, u32 address_length,
                              u32 if_address_index, u32 is_del)
{
    ...
    if (is_del) {
        ...
        hash_foreach_pair(pair, eai->arp_entries, ({
                              ...
                              if (ip4_destination_matches_route(im, &e->ip4_address, address, address_length)) {
...
static void
arp_table_bind(ip4_main_t *im, uword opaque, u32 sw_if_index, u32 new_fib_index, u32 old_fib_index)
{
    ethernet_arp_main_t *am = &ethernet_arp_main;
    ethernet_arp_interface_t *eai;
    ethernet_arp_ip4_entry_t *e;
...
#define _(a, b) vnet_pcap_drop_trace_filter_add_del(rt->errors[ETHERNET_ARP_ERROR_##a], 1 /* is_add */);
        foreach_ethernet_arp_error
#undef _
    }
...
static inline int
vnet_arp_unset_ip4_over_ethernet_internal(vnet_main_t *vnm, vnet_arp_set_ip4_over_ethernet_rpc_args_t *args)
{
    ethernet_arp_main_t *am = &ethernet_arp_main;
    ethernet_arp_ip4_entry_t *e;
    ethernet_arp_interface_t *eai;
...
static int
vnet_arp_populate_ip4_over_ethernet_internal(vnet_main_t *vnm, vnet_arp_set_ip4_over_ethernet_rpc_args_t *args)
{
    ethernet_arp_main_t *am = &ethernet_arp_main;
    ethernet_arp_ip4_entry_t *e;
    ethernet_arp_interface_t *eai;
    ...
    if (NULL != e) {
        adj_nbr_walk_nh4(e->sw_if_index, &e->ip4_address, arp_mk_complete_walk, e);
    }
    return (0);
}

static void
set_ip4_over_ethernet_rpc_callback(vnet_arp_set_ip4_over_ethernet_rpc_args_t *a)
{
 ...
 */
static clib_error_t *
ethernet_arp_sw_interface_up_down(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    ethernet_arp_main_t *am = &ethernet_arp_main;
    ethernet_arp_ip4_entry_t *e;
    u32 i, *to_update = 0;
    ...
    pool_foreach(e, am->ip4_entry_pool, ({
                     if (e->sw_if_index == sw_if_index)
                         vec_add1(to_update, e - am->ip4_entry_pool);
                 }));
    /* *INDENT-ON* */
...
int
vnet_arp_set_ip4_over_ethernet(vnet_main_t *vnm, u32 sw_if_index, const ethernet_arp_ip4_over_ethernet_address_t *a,
...
int
vnet_proxy_arp_add_del(ip4_address_t *lo_addr, ip4_address_t *hi_addr, u32 fib_index, int is_del)
{
    ethernet_arp_main_t *am = &ethernet_arp_main;
    ethernet_proxy_arp_t *pa;
...
static clib_error_t *
ip_arp_add_del_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm = vnet_get_main();
    u32 sw_if_index;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "%U %U %U", unformat_vnet_sw_interface, vnm, &sw_if_index, unformat_ip4_address, &addr.ip4,
        ...
        else if (unformat(input, "proxy %U - %U", unformat_ip4_address, &lo_addr.ip4, unformat_ip4_address, &hi_addr.ip4))
            is_proxy = 1;
        else
            break;
    }
    ...
    if (is_proxy) {
        (void) vnet_proxy_arp_add_del(&lo_addr.ip4, &hi_addr.ip4, fib_index, is_del);
        return 0;
    }
    ...
    if (addr_valid) {
        ...
        for (i = 0; i < count; i++) {
            if (is_del == 0) {
                ...
                vnet_register_ip4_arp_resolution_event(vnm, &addr.ip4, vlib_current_process(vm), 1 /* type */,
                                                       0 /* data */);

                vnet_arp_set_ip4_over_ethernet(vnm, sw_if_index, &addr, is_static, is_no_fib_entry);
...
VLIB_CLI_COMMAND(ip_arp_add_del_command, static) = {
    .path       = "set ip arp",
    .short_help = "set ip arp [del] <intfc> <ip-address> <mac-address> [static] [no-fib-entry] [count <count>] [fib-id "
                  "<fib-id>] [proxy <lo-addr> - <hi-addr>]",
    .function = ip_arp_add_del_command_fn,
};
/* *INDENT-ON* */

static clib_error_t *
set_int_proxy_arp_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm = vnet_get_main();
    u32 sw_if_index;
    vnet_sw_interface_t *si;
...
static uword
arp_term_l2bd(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(memcmp(eth0->src_address, arp0->ip4_over_ethernet[0].ethernet, sizeof(eth0->src_address)) ||
                              ethernet_address_cast(arp0->ip4_over_ethernet[0].ethernet))) {
                /* VRRP virtual MAC may be different to SMAC in ARP reply */
                if (memcmp(arp0->ip4_over_ethernet[0].ethernet, vrrp_prefix, sizeof(vrrp_prefix))) {
                    error0 = ETHERNET_ARP_ERROR_l2_address_mismatch;
                    goto drop;
                }
            }
        ...
        check_ip6_nd:
            ...
            if (PREDICT_FALSE(ethertype0 == ETHERNET_TYPE_IP6 && iph0->protocol == IP_PROTOCOL_ICMP6 &&
                ...
                sw_if_index0 = vnet_buffer(p0)->sw_if_index[VLIB_RX];
                if (vnet_ip6_nd_term(vm, node, p0, eth0, iph0, sw_if_index0, vnet_buffer(p0)->l2.bd_index))
        ...
        drop:
            if (0 == arp0->ip4_over_ethernet[0].ip4.as_u32 ||
                (arp0->ip4_over_ethernet[0].ip4.as_u32 == arp0->ip4_over_ethernet[1].ip4.as_u32)) {
                error0 = ETHERNET_ARP_ERROR_gratuitous_arp;
            }
...
arp_term_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, arp_term_l2bd_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(),
...
ethernet_arp_change_mac(u32 sw_if_index)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(e, am->ip4_entry_pool, ({ change_arp_mac(sw_if_index, e); }));
    /* *INDENT-ON* */
...
void
send_ip4_garp_w_addr(vlib_main_t *vm, const ip4_address_t *ip4_addr, u32 sw_if_index)
{
    ip4_main_t *i4m  = &ip4_main;
    vnet_main_t *vnm = vnet_get_main();
    ...
    if (ip4_addr) {
        clib_warning("Sending GARP for IP4 address %U on sw_if_idex %d", format_ip4_address, ip4_addr, sw_if_index);
        ...
        clib_memcpy(h->ip4_over_ethernet[0].ethernet, hi->hw_address, sizeof(h->ip4_over_ethernet[0].ethernet));
        clib_memcpy(h->ip4_over_ethernet[1].ethernet, hi->hw_address, sizeof(h->ip4_over_ethernet[1].ethernet));
        ...
        rewrite =
            ethernet_build_rewrite(vnm, sw_if_index, VNET_LINK_ARP, VNET_REWRITE_FOR_SW_INTERFACE_ADDRESS_BROADCAST);
