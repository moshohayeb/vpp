...
int
p2p_ethernet_add_del(vlib_main_t *vm, u32 parent_if_index, u8 *client_mac, u32 p2pe_subif_id, int is_add,
                     u32 *p2pe_if_index)
{
    ...
    if (is_add) {
        if (p2pe_sw_if_index == ~0) {
            ...
            *kp = sup_and_sub_key;
            ...
            set_int_l2_mode(vm, vnm, MODE_L3, p2pe_subif_id, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0);
            return 0;
        }
        return VNET_API_ERROR_SUBIF_ALREADY_EXISTS;
    } else {
...
static clib_error_t *
vnet_p2p_ethernet_add_del(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm = vnet_get_main();
