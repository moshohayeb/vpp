...
no_args:
    /* *INDENT-OFF* */
    pool_foreach(f, fm->global_flow_pool, { vlib_cli_output(vm, "%U\n", format_flow, f); });
    /* *INDENT-ON* */
...
test_flow(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "add"))
            ...
            action = FLOW_DISABLE;
        else if (unformat(line_input, "src-ip %U", unformat_ip4_address_and_mask, &flow.ip4_n_tuple.src_addr))
            ;
        else if (unformat(line_input, "dst-ip %U", unformat_ip4_address_and_mask, &flow.ip4_n_tuple.dst_addr))
            ;
        else if (unformat(line_input, "src-port %U", unformat_ip_port_and_mask, &flow.ip4_n_tuple.src_port))
            ;
        else if (unformat(line_input, "dst-port %U", unformat_ip_port_and_mask, &flow.ip4_n_tuple.dst_port))
            ;
