...
{
    if ((vec_len(mpls_tunnel_db) <= sw_if_index) || (~0 == mpls_tunnel_db[sw_if_index]))
        return (NULL);
 ...
 */
static u8 *
mpls_tunnel_build_rewrite(vnet_main_t *vnm, u32 sw_if_index, vnet_link_t link_type, const void *dst_address)
{
...
mpls_tunnel_collect_forwarding(fib_node_index_t pl_index, fib_node_index_t path_index, void *arg)
{
     ...
     * info for this path.
     */
    ctx->next_hops = fib_path_ext_stack(path_ext, ctx->fct, ctx->fct, ctx->next_hops);

    return (FIB_PATH_LIST_WALK_CONTINUE);
}

static void
mpls_tunnel_mk_lb(mpls_tunnel_t *mt, vnet_link_t linkt, fib_forward_chain_type_t fct, dpo_id_t *dpo_lb)
{
...
mpls_tunnel_restack(mpls_tunnel_t *mt)
{
    ...
    if (mt->mt_flags & MPLS_TUNNEL_FLAG_L2) {
        ...
        dpo_stack_from_node(hi->tx_node_index, &mt->mt_l2_lb, &dpo);
        dpo_reset(&dpo);
    } else {
 ...
 */
static void
mpls_tunnel_fixup(vlib_main_t *vm, ip_adjacency_t *adj, vlib_buffer_t *b0, const void *data)
{
    /*
...
mpls_tunnel_update_adj(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
    ...
    case IP_LOOKUP_NEXT_MCAST:
         ...
         */
        adj_mcast_midchain_update_rewrite(ai, mpls_tunnel_fixup, NULL, ADJ_FLAG_NONE, mpls_tunnel_build_rewrite_i(), 0,
                                          0);
        break;
 ...
 */
static uword
mpls_tunnel_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 next_index;
...
mpls_tunnel_walk(mpls_tunnel_walk_cb_t cb, void *ctx)
{
    ...
    pool_foreach_index(mti, mpls_tunnel_pool, ({ cb(mti, ctx); }));
}
...
u32
vnet_mpls_tunnel_create(u8 l2_only, u8 is_multicast)
{
    vnet_hw_interface_t *hi;
    mpls_tunnel_t *mt;
    vnet_main_t *vnm;
     ...
     */
    mt->mt_hw_if_index =
        vnet_register_interface(vnm, mpls_tunnel_class.index, mti, mpls_tunnel_hw_interface_class.index, mti);
...
vnet_mpls_tunnel_path_add(u32 sw_if_index, fib_route_path_t *rpaths)
{
    ...
    fib_path_ext_list_insert(&mt->mt_path_exts, mt->mt_path_list, FIB_PATH_EXT_MPLS, rpaths);
    mpls_tunnel_restack(mt);
}
...
static clib_error_t *
vnet_create_mpls_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
VLIB_CLI_COMMAND(create_mpls_tunnel_command, static) = {
    .path       = "mpls tunnel",
    .short_help = "mpls tunnel [multicast] [l2-only] via [next-hop-address] [next-hop-interface] [next-hop-table "
                  "<value>] [weight <value>] [preference <value>] [udp-encap-id <value>] [ip4-lookup-in-table <value>] "
                  "[ip6-lookup-in-table <value>] [mpls-lookup-in-table <value>] [resolve-via-host] "
                  "[resolve-via-connected] [rx-ip4 <interface>] [out-labels <value value value>]",
...
format_mpls_tunnel(u8 *s, va_list *args)
{
    ...
    s = format(s, "mpls_tunnel%d: sw_if_index:%d hw_if_index:%d", mt - mpls_tunnel_pool, mt->mt_sw_if_index,
               mt->mt_hw_if_index);
...
static clib_error_t *
show_mpls_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    mpls_tunnel_t *mt;
    u32 mti = ~0;
