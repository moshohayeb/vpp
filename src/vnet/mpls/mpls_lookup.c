...
static inline uword
mpls_lookup(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    vlib_node_increment_counter(vm, mpls_lookup_node.index, MPLS_ERROR_PKTS_DECAP, from_frame->n_vectors);
...
format_mpls_load_balance_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "MPLS: next [%d], LB index %d hash %d", t->next_index, t->lb_index, t->hash);
    return s;
}

static uword
mpls_load_balance(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
mpls_lookup_init(vlib_main_t *vm)
{
    ...
    mpls_lookup_to_replicate_edge = vlib_node_add_named_next(vm, mpls_lookup_node.index, "mpls-replicate");
