...
#undef _
    ...
    s = format(s, "MPLS: next %s[%d]  label %d ttl %d exp %d", next_name, t->next_index, vnet_mpls_uc_get_label(label),
...
static inline uword
mpls_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword
mpls_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
