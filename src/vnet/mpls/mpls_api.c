...
static int
mpls_ip_bind_unbind_handler(vnet_main_t *vnm, vl_api_mpls_ip_bind_unbind_t *mp)
{
    ...
    ip_fib_index = fib_table_find((mp->mb_is_ip4 ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6), ntohl(mp->mb_ip_table_id));
    if (~0 == ip_fib_index)
        return VNET_API_ERROR_NO_SUCH_FIB;
    ...
    if (mp->mb_is_ip4) {
        ...
        clib_memcpy(&pfx.fp_addr.ip4, mp->mb_address, sizeof(pfx.fp_addr.ip4));
    } else {
        pfx.fp_proto = FIB_PROTOCOL_IP6;
        clib_memcpy(&pfx.fp_addr.ip6, mp->mb_address, sizeof(pfx.fp_addr.ip6));
    }
...
static int
mpls_route_add_del_t_handler(vnet_main_t *vnm, vl_api_mpls_route_add_del_t *mp, u32 *stats_index)
{
    ...
    rv = add_del_route_check(FIB_PROTOCOL_MPLS, mp->mr_table_id, mp->mr_next_hop_sw_if_index, pfx.fp_payload_proto,
                             mp->mr_next_hop_table_id, mp->mr_is_rpf_id, &fib_index, &next_hop_fib_index);
    ...
    if (n_labels == 0)
        ...
        for (ii = 0; ii < n_labels; ii++) {
            label_stack[ii].fml_value = ntohl(mp->mr_next_hop_out_label_stack[ii].label);
            ...
            label_stack[ii].fml_mode =
                (mp->mr_next_hop_out_label_stack[ii].is_uniform ? FIB_MPLS_LSP_MODE_UNIFORM : FIB_MPLS_LSP_MODE_PIPE);
        }
    }
    ...
    rv = add_del_route_t_handler(mp->mr_is_multipath, mp->mr_is_add,
                                 ...
                                 0,   // mp->is_prohibit,
                                 0,   // mp->is_local,
                                 mp->mr_is_multicast, mp->mr_is_classify, mp->mr_classify_table_index,
                                 mp->mr_is_resolve_host, mp->mr_is_resolve_attached, mp->mr_is_interface_rx,
                                 mp->mr_is_rpf_id,
                                 0,                                                 // l2_bridged
                                 0,                                                 // is source_lookup
                                 0,                                                 // is_udp_encap
                                 fib_index, &pfx, mp->mr_next_hop_proto, &nh, ~0,   // next_hop_id
                                 ntohl(mp->mr_next_hop_sw_if_index), next_hop_fib_index, mp->mr_next_hop_weight,
                                 mp->mr_next_hop_preference, ntohl(mp->mr_next_hop_via_label), label_stack);
    /* *INDENT-ON* */
...
mpls_table_create(u32 table_id, u8 is_api, const u8 *name)
{
    ...
    if (~0 == fib_index) {
        fib_table_find_or_create_and_lock_w_name(FIB_PROTOCOL_MPLS, table_id,
                                                 (is_api ? FIB_SOURCE_API : FIB_SOURCE_CLI), name);
    }
}
...
vl_api_mpls_tunnel_add_del_t_handler(vl_api_mpls_tunnel_add_del_t *mp)
{
    ...
    if (mp->mt_next_hop_proto_is_ip4) {
        ...
        clib_memcpy(&rpath.frp_addr.ip4, mp->mt_next_hop, sizeof(rpath.frp_addr.ip4));
    } else {
        rpath.frp_proto = DPO_PROTO_IP6;
        clib_memcpy(&rpath.frp_addr.ip6, mp->mt_next_hop, sizeof(rpath.frp_addr.ip6));
    }
    ...
    if (rpath.frp_sw_if_index == ~0) { /* recursive path, set fib index */
        rpath.frp_fib_index = fib_table_find(dpo_proto_to_fib(rpath.frp_proto), ntohl(mp->mt_next_hop_table_id));
    ...
    if (mp->mt_is_add) {
        for (ii = 0; ii < mp->mt_next_hop_n_out_labels; ii++) {
            fib_mpls_label_t fml = {
                ...
                .fml_mode  = (mp->mt_next_hop_out_label_stack[ii].is_uniform ? FIB_MPLS_LSP_MODE_UNIFORM :
    ...
    if (mp->mt_is_add) {
        if (~0 == tunnel_sw_if_index)
            tunnel_sw_if_index = vnet_mpls_tunnel_create(mp->mt_l2_only, mp->mt_is_multicast);
...
static void
vl_api_sw_interface_set_mpls_enable_t_handler(vl_api_sw_interface_set_mpls_enable_t *mp)
{
    ...
    rv = mpls_sw_interface_enable_disable(&mpls_main, ntohl(mp->sw_if_index), mp->enable, 1);
...
static void
send_mpls_fib_details(vpe_api_main_t *am, vl_api_registration_t *reg, const fib_table_t *table, const fib_prefix_t *pfx,
...
vl_api_mpls_fib_dump_t_handler(vl_api_mpls_fib_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(mpls_fib, mm->mpls_fibs, ({ mpls_fib_table_walk(mpls_fib, vl_api_mpls_fib_dump_table_walk, &ctx); }));
    /* *INDENT-ON* */
