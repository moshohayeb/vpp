...
format_mpls_header(u8 *s, va_list *args)
{
    ...
    return (format(s, "[%U:%d:%d:%U]", format_mpls_unicast_label, vnet_mpls_uc_get_label(hdr.label_exp_s_ttl),
                   vnet_mpls_uc_get_ttl(hdr.label_exp_s_ttl), vnet_mpls_uc_get_exp(hdr.label_exp_s_ttl),
                   format_mpls_eos_bit, vnet_mpls_uc_get_s(hdr.label_exp_s_ttl)));
}
...
format_mpls_unicast_header_host_byte_order(u8 *s, va_list *args)
{
    ...
    s = format(s, "label %d exp %d, s %d, ttl %d", vnet_mpls_uc_get_label(label), vnet_mpls_uc_get_exp(label),
...
static clib_error_t *
vnet_mpls_local_label(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (is_ip) {
        ...
        if (is_del) {
            ...
            if (FIB_NODE_INDEX_INVALID == lfe) {
                error = clib_error_return(0, "Failed to create %U-%U in MPLS table-id %d", format_mpls_unicast_label,
                                          local_label, format_mpls_eos_bit, eos, table_id);
                goto done;
            }
        }
    }
...
VLIB_CLI_COMMAND(mpls_local_label_command, static) = {
    ...
    .short_help = "mpls local-label [add|del] <label-value> [eos|non-eos] via [next-hop-address] [next-hop-interface] "
                  "[next-hop-table <value>] [weight <value>] [preference <value>] [udp-encap-id <value>] "
                  "[ip4-lookup-in-table <value>] [ip6-lookup-in-table <value>] [mpls-lookup-in-table <value>] "
                  "[resolve-via-host] [resolve-via-attached] [rx-ip4 <interface>] [out-labels <value value value>]",
};

clib_error_t *
vnet_mpls_table_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmdo)
{
    unformat_input_t _line_input, *line_input = &_line_input;
