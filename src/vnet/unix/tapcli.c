...
tapcli_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    for (i = 0; i < n_packets; i++) {
        ...
        p = hash_get(tm->tapcli_interface_index_by_sw_if_index, tx_sw_if_index);
        if (p == 0) {
        ...
        if (writev(ti->unix_fd, tm->threads[thread_index].iovecs, vec_len(tm->threads[thread_index].iovecs)) < l)
 ...
 */
static uword
tapcli_rx_iface(vlib_main_t *vm, vlib_node_runtime_t *node, tapcli_interface_t *ti)
{
    tapcli_main_t *tm       = &tapcli_main;
    ...
    while (n_left_to_next) {   // Fill at most one vector
        ...
        if (PREDICT_FALSE(vec_len(tm->threads[thread_index].rx_buffers) < tm->mtu_buffers)) {
            uword len = vec_len(tm->threads[thread_index].rx_buffers);
            _vec_len(tm->threads[thread_index].rx_buffers) +=
                vlib_buffer_alloc_from_free_list(vm, &tm->threads[thread_index].rx_buffers[len], VLIB_FRAME_SIZE - len,
                                                 VLIB_BUFFER_DEFAULT_FREE_LIST_INDEX);
            if (PREDICT_FALSE(vec_len(tm->threads[thread_index].rx_buffers) < tm->mtu_buffers)) {
                vlib_node_increment_counter(vm, tapcli_rx_node.index, TAPCLI_ERROR_BUFFER_ALLOC,
                                            tm->mtu_buffers - vec_len(tm->threads[thread_index].rx_buffers));
                break;
            }
        }
        ...
        n_bytes_left      = readv(ti->unix_fd, tm->threads[thread_index].iovecs, tm->mtu_buffers);
        n_bytes_in_packet = n_bytes_left;
        if (n_bytes_left <= 0) {
            if (errno != EAGAIN) {
                vlib_node_increment_counter(vm, tapcli_rx_node.index, TAPCLI_ERROR_READ, 1);
            }
            break;
        }

        bi_first = tm->threads[thread_index].rx_buffers[i_rx];
        b = b_first = vlib_get_buffer(vm, tm->threads[thread_index].rx_buffers[i_rx]);
        prev        = NULL;
        ...
        next_index     = (ti->per_interface_next_index != ~0) ? ti->per_interface_next_index : next_index;
        ...
        vlib_validate_buffer_enqueue_x1(vm, node, next, to_next, n_left_to_next, bi_first, next_index);
        ...
        if (PREDICT_TRUE(!admin_down)) {
            vlib_increment_combined_counter(vnet_main.interface_main.combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX,
                                            thread_index, ti->sw_if_index, 1, n_bytes_in_packet);

            if (PREDICT_FALSE(n_trace > 0)) {
                vlib_trace_buffer(vm, node, next_index, b_first, /* follow_chain */ 1);
                n_trace--;
                set_trace             = 1;
...
tapcli_rx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    /* *INDENT-OFF* */
    clib_bitmap_foreach(i, tm->pending_read_bitmap, ({ vec_add1(ready_interface_indices, i); }));
    /* *INDENT-ON* */
    ...
    for (i = 0; i < vec_len(ready_interface_indices); i++) {
        tm->pending_read_bitmap = clib_bitmap_set(tm->pending_read_bitmap, ready_interface_indices[i], 0);
...
tapcli_read_ready(clib_file_t *uf)
{
    ...
    if (p)
        tm->pending_read_bitmap = clib_bitmap_set(tm->pending_read_bitmap, p[0], 1);
    else
...
tap_name_renumber(vnet_hw_interface_t *hi, u32 new_dev_instance)
{
    ...
    vec_validate_init_empty(tm->show_dev_instance_by_real_dev_instance, hi->dev_instance, ~0);

    tm->show_dev_instance_by_real_dev_instance[hi->dev_instance] = new_dev_instance;
 ...
 */
static void
tapcli_nopunt_frame(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 *buffers    = vlib_frame_args(frame);
    uword n_packets = frame->n_vectors;
 ...
 */
static void
tapcli_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    tapcli_main_t *tm = &tapcli_main;
    tapcli_interface_t *ti;
    ...
    ti->per_interface_next_index = vlib_node_add_next(tm->vlib_main, tapcli_rx_node.index, node_index);
}
...
tapcli_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    u32 hw_flags;
    u32 speed_duplex = VNET_HW_INTERFACE_FLAG_FULL_DUPLEX | VNET_HW_INTERFACE_FLAG_SPEED_1G;
...
vnet_tap_connect(vlib_main_t *vm, vnet_tap_connect_args_t *ap)
{
    ...
    if (ap->ip6_address_set) {
        ...
        strncpy(ifr2.ifr_name, (char *) ap->intfc_name, sizeof(ifr2.ifr_name) - 1);
    ...
    error = ethernet_register_interface(tm->vnet_main, tapcli_dev_class.index,
                                        ti - tm->tapcli_interfaces /* device instance */, hwaddr /* ethernet address */,
    ...
    hash_set(tm->tapcli_interface_index_by_sw_if_index, ti->sw_if_index, ti - tm->tapcli_interfaces);

    hash_set(tm->tapcli_interface_index_by_unix_fd, ti->unix_fd, ti - tm->tapcli_interfaces);
 ...
 */
static clib_error_t *
tap_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    tapcli_main_t *tm = &tapcli_main;
    u32 sw_if_index   = ~0;
    ...
    if (unformat(input, "%U", unformat_vnet_sw_interface, tm->vnet_main, &sw_if_index))
        ;
    else
        return clib_error_return(0, "unknown input `%U'", format_unformat_error, input);
 ...
 */
static clib_error_t *
tap_modify_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u8 *intfc_name;
    tapcli_main_t *tm   = &tapcli_main;
    u32 sw_if_index     = ~0;
    ...
    if (!rc) {
        vlib_cli_output(vm, "Modified %U for Linux tap '%s'", format_vnet_sw_if_index_name, tm->vnet_main,
 ...
 */
static clib_error_t *
tap_connect_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u8 *intfc_name = 0;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "hwaddr %U", unformat_ethernet_address, &hwaddr))
            hwaddr_arg = hwaddr;
        ...
        else if (unformat(line_input, "address %U/%d", unformat_ip4_address, &ip4_address, &ip4_mask_width))
            ip4_address_set = 1;

        else if (unformat(line_input, "address %U/%d", unformat_ip6_address, &ip6_address, &ip6_mask_width))
            ip6_address_set = 1;
...
VLIB_CLI_COMMAND(tap_connect_command, static) = {
    .path       = "tap connect",
    .short_help = "tap connect <intfc-name> [address <ip-addr>/mw] [hwaddr <addr>]",
