...
*/
always_inline uword
udp46_punt_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip4)
{
...
*/
static uword
udp4_punt(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
*/
static uword
udp6_punt(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
{
    punt_main_t *pm  = &punt_main;
    punt_client_t *v = is_ip4 ? pm->clients_by_dst_port4 : pm->clients_by_dst_port6;
...
static void
punt_socket_register(bool is_ip4, u8 protocol, u16 port, char *client_pathname)
{
    punt_main_t *pm = &punt_main;
    punt_client_t c, *n;
    punt_client_t *v = is_ip4 ? pm->clients_by_dst_port4 : pm->clients_by_dst_port6;
...
always_inline uword
udp46_punt_socket_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_ip4)
{
    ...
    u32 node_index = is_ip4 ? udp4_punt_socket_node.index : udp6_punt_socket_node.index;
...
static uword
udp4_punt_socket(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return udp46_punt_socket_inline(vm, node, from_frame, true /* is_ip4 */);
}

static uword
udp6_punt_socket(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword
punt_socket_rx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    punt_main_t *pm = &punt_main;
    u32 total_count = 0;
    int i;
...
clib_error_t *
vnet_punt_socket_add(vlib_main_t *vm, u32 header_version, bool is_ip4, u8 protocol, u16 port, char *client_pathname)
{
    punt_main_t *pm = &punt_main;
    ...
    if (protocol != IP_PROTOCOL_UDP)
        return clib_error_return(0, "only UDP protocol (%d) is supported, got %d", IP_PROTOCOL_UDP, protocol);
 ...
 */
clib_error_t *
vnet_punt_add_del(vlib_main_t *vm, u8 ipv, u8 protocol, u16 port, bool is_add)
{

    /* For now we only support TCP, UDP and SCTP punt */
    if (protocol != IP_PROTOCOL_UDP && protocol != IP_PROTOCOL_TCP && protocol != IP_PROTOCOL_SCTP)
        return clib_error_return(0, "only UDP (%d), TCP (%d) and SCTP (%d) protocols are supported, got %d",
                                 IP_PROTOCOL_UDP, IP_PROTOCOL_TCP, IP_PROTOCOL_SCTP, protocol);
    ...
    if (port == (u16) ~0) {
        ...
        if (protocol == IP_PROTOCOL_TCP || protocol == IP_PROTOCOL_SCTP)
            return clib_error_return(0, "punt TCP/SCTP ports is not supported yet");
...
static clib_error_t *
punt_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u32 port;
    bool is_add         = true;
    u32 protocol        = ~0;
...
punt_init(vlib_main_t *vm)
{
    ...
    pm->clients_by_dst_port6 = sparse_vec_new(sizeof(pm->clients_by_dst_port6[0]), BITS(((udp_header_t *) 0)->dst_port));
    pm->clients_by_dst_port4 = sparse_vec_new(sizeof(pm->clients_by_dst_port4[0]), BITS(((udp_header_t *) 0)->dst_port));
