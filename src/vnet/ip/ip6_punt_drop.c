...
static uword
ip6_punt_policer(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (
        ip_punt_policer(vm, node, frame, vnet_feat_arc_ip6_punt.feature_arc_index, ip6_punt_policer_cfg.policer_index));
}
...
VNET_FEATURE_INIT(ip6_punt_policer_node, static) = {.arc_name    = "ip6-punt",
...
static uword
ip6_not_enabled(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (node->flags & VLIB_NODE_FLAG_TRACE)
        ip6_forward_next_trace(vm, node, frame, VLIB_TX);
...
static uword
ip6_punt_redirect(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (ip_punt_redirect(vm, node, frame, vnet_feat_arc_ip6_punt.feature_arc_index, &ip6_punt_redirect_cfg));
}
...
VNET_FEATURE_INIT(ip6_punt_redirect_node, static) = {.arc_name    = "ip6-punt",
                                                     .node_name   = "ip6-punt-redirect",
                                                     .runs_before = VNET_FEATURES("error-punt")};
/* *INDENT-ON* */

void
ip6_punt_redirect_add(u32 rx_sw_if_index, u32 tx_sw_if_index, ip46_address_t *nh)
{
    ...
    ip_punt_redirect_add(&ip6_punt_redirect_cfg, rx_sw_if_index, &rx, FIB_PROTOCOL_IP6, VNET_LINK_IP6);
...
static clib_error_t *
ip6_punt_redirect_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    clib_error_t *error = 0;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            ...
            ;
        else if (unformat(line_input, "via %U %U", unformat_ip6_address, &nh.ip6, unformat_vnet_sw_interface, vnm,
                          &tx_sw_if_index))
            ;
...
static clib_error_t *
ip6_punt_redirect_show_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
