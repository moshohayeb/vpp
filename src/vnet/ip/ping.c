...
format_icmp_echo_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "ICMP echo id %d seq %d%s", clib_net_to_host_u16(t->id), clib_net_to_host_u16(t->seq),
               t->bound ? "" : " (unknown)");
...
signal_ip46_icmp_reply_event(u8 event_type, vlib_buffer_t *b0)
{
    ...
    uword *p = hash_get(pm->ping_run_by_icmp_id, clib_net_to_host_u16(net_icmp_id));
    if (!p)
        return 0;
    ...
    clib_memcpy(vnet_buffer(vlib_get_buffer(vm, bi0_copy))->unused, &nowts, sizeof(nowts));
 ...
 */
static uword
ip6_icmp_echo_reply_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        next0 = signal_ip46_icmp_reply_event(PING_RESPONSE_IP6, b0) ? ICMP6_ECHO_REPLY_NEXT_DROP :
 ...
 */
static uword
ip4_icmp_echo_reply_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, *from;
    ...
    while (n_left_from > 0) {
        ...
        next0 = signal_ip46_icmp_reply_event(PING_RESPONSE_IP4, b0) ? ICMP4_ECHO_REPLY_NEXT_DROP :
...
static u16
init_icmp46_echo_request(icmp46_echo_request_t *icmp46_echo, u16 seq_host, u16 id_host, u16 data_len)
{
    int i;
...
send_ip6_ping(vlib_main_t *vm, ip6_main_t *im, u32 table_id, ip6_address_t *pa6, u32 sw_if_index, u16 seq_host,
              u16 id_host, u16 data_len, u32 burst, u8 verbose)
{
    ...
    if (~0 == sw_if_index) {
         ...
         * source interface.
         */
        vnet_buffer(p0)->sw_if_index[VLIB_TX] = ip6_fib_table_get_index_for_sw_if_index(sw_if_index);
    }
    ...
    h0->ip6.ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(0x6 << 28);
    ...
    if (!ip6_src_address_for_packet(&im->lookup_main, sw_if_index, &h0->ip6.dst_address, &h0->ip6.src_address)) {
        vlib_buffer_free(vm, &bi0, 1);
    ...
    data_len                = init_icmp46_echo_request(&h0->icmp_echo, seq_host, id_host, data_len);
    h0->icmp_echo.time_sent = vlib_time_now(vm);

    /* Fix up the lengths */
    h0->ip6.payload_length = clib_host_to_net_u16(data_len + sizeof(icmp46_header_t));

    p0->current_length =
        clib_net_to_host_u16(h0->ip6.payload_length) + STRUCT_OFFSET_OF(icmp6_echo_request_header_t, icmp);

    /* Calculate the ICMP checksum */
    h0->icmp.checksum = 0;
    h0->icmp.checksum = ip6_tcp_udp_icmp_compute_checksum(vm, 0, &h0->ip6, &bogus_length);
...
static send_ip46_ping_result_t
send_ip4_ping(vlib_main_t *vm, ip4_main_t *im, u32 table_id, ip4_address_t *pa4, u32 sw_if_index, u16 seq_host,
              u16 id_host, u16 data_len, u32 burst, u8 verbose)
{
    ...
    if (~0 == sw_if_index) {
         ...
         * source interface.
         */
        vnet_buffer(p0)->sw_if_index[VLIB_TX] = ip4_fib_table_get_index_for_sw_if_index(sw_if_index);
    }
    ...
    if (PREDICT_TRUE(if_add_index0 != ~0)) {
        ...
        if (verbose) {
            vlib_cli_output(vm, "Source address: %U", format_ip4_address, &h0->ip4.src_address);
        }
    }
    ...
    /* Fix up the lengths */
    h0->ip4.length = clib_host_to_net_u16(data_len + sizeof(icmp46_header_t) + sizeof(ip4_header_t));
    ...
    h0->icmp.checksum = ~ip_csum_fold(ip_incremental_checksum(0, &(h0->icmp), p0->current_length - sizeof(ip4_header_t)));
...
print_ip6_icmp_reply(vlib_main_t *vm, u32 bi0)
{
    ...
    vlib_cli_output(vm, "%d bytes from %U: icmp_seq=%d ttl=%d time=%.4f ms",
                    clib_host_to_net_u16(h0->ip6.payload_length), format_ip6_address, &h0->ip6.src_address,
                    clib_host_to_net_u16(h0->icmp_echo.seq), h0->ip6.hop_limit, rtt * 1000.0);
}
...
print_ip4_icmp_reply(vlib_main_t *vm, u32 bi0)
{
    ...
    clib_memcpy(&rtt, vnet_buffer(b0)->unused, sizeof(rtt));
    rtt -= h0->icmp_echo.time_sent;
    u32 rcvd_icmp_len = clib_host_to_net_u16(h0->ip4.length) - (4 * (0xF & h0->ip4.ip_version_and_header_length));

    vlib_cli_output(vm, "%d bytes from %U: icmp_seq=%d ttl=%d time=%.4f ms", rcvd_icmp_len, format_ip4_address,
                    &h0->ip4.src_address, clib_host_to_net_u16(h0->icmp_echo.seq), h0->ip4.ttl, rtt * 1000.0);
}
...
static void
run_ping_ip46_address(vlib_main_t *vm, u32 table_id, ip4_address_t *pa4, ip6_address_t *pa6, u32 sw_if_index,
                      f64 ping_interval, u32 ping_repeat, u32 data_len, u32 ping_burst, u32 verbose)
{
    int i;
    ping_main_t *pm    = &ping_main;
    ...
    for (i = 1; i <= ping_repeat; i++) {
        ...
        pr->curr_seq = i;
        if (pa6 && (SEND_PING_OK == send_ip6_ping(vm, ping_main.ip6_main, table_id, pa6, sw_if_index, i, icmp_id,
                                                  data_len, ping_burst, verbose))) {
            n_requests += ping_burst;
        }
        if (pa4 && (SEND_PING_OK == send_ip4_ping(vm, ping_main.ip4_main, table_id, pa4, sw_if_index, i, icmp_id,
                                                  data_len, ping_burst, verbose))) {
            n_requests += ping_burst;
        }
...
static clib_error_t *
ping_ip_address(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ip4_address_t a4;
    ip6_address_t a6;
    ...
    if (unformat(input, "%U", unformat_ip4_address, &a4)) {
        ...
        error =
            clib_error_return(0, "expecting IP4/IP6 address `%U'. Usage: ping <addr> [source <intf>] [size <datasz>] [repeat <count>] [verbose]",
    ...
    while (!unformat_eof(input, NULL)) {
        if (unformat(input, "source")) {
            ...
            if (data_len > PING_MAXIMUM_DATA_SIZE) {
                error = clib_error_return(0, "%d is bigger than maximum allowed payload size %d", data_len,
            ...
            if (!unformat(input, "%f", &ping_interval)) {
                error = clib_error_return(0, "expecting interval (floating point number) got `%U'",
                                          format_unformat_error, input);
                goto done;
            }
    ...
    if (ping_burst < 1 || ping_burst > VLIB_FRAME_SIZE)
        return clib_error_return(0, "burst size must be between 1 and %u", VLIB_FRAME_SIZE);

    run_ping_ip46_address(vm, table_id, ping_ip4 ? &a4 : NULL, ping_ip6 ? &a6 : NULL, sw_if_index, ping_interval,
