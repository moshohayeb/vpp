...
format_ip_in_out_acl_trace(u8 *s, u32 is_output, va_list *args)
{
    ...
    s = format(s, "%s: sw_if_index %d, next_index %d, table %d, offset %d", is_output ? "OUTACL" : "INACL",
...
static inline uword
ip_in_out_acl_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4, int is_output)
{
    ...
    while (n_left_from > 2) {
        ...
        if (is_output) {
            ...
            vnet_buffer(b0)->l2_classify.pad.l2_len = vnet_buffer(b0)->ip.save_rewrite_length;
        ...
        if (is_output) {
            /* Save the rewrite length, since we are using the l2_classify struct */
            vnet_buffer(b1)->l2_classify.pad.l2_len = vnet_buffer(b1)->ip.save_rewrite_length;
            /* advance the match pointer so the matching happens on IP header */
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            e0           = 0;
            t0           = 0;
            vnet_get_config_data(am->vnet_config_main[is_output][tid], &b0->current_config_index, &next0,
                ...
                if (e0) {
                    ...
                    if (is_ip4)
                        error0 = (next0 == ACL_NEXT_INDEX_DENY) ?
                                     (is_output ? IP4_ERROR_OUTACL_SESSION_DENY : IP4_ERROR_INACL_SESSION_DENY) :
                                     IP4_ERROR_NONE;
                    else
                        error0 = (next0 == ACL_NEXT_INDEX_DENY) ?
                                     (is_output ? IP6_ERROR_OUTACL_SESSION_DENY : IP6_ERROR_INACL_SESSION_DENY) :
                                     IP6_ERROR_NONE;
                    ...
                    while (1) {
                        if (PREDICT_TRUE(t0->next_table_index != ~0))
                            ...
                            if (is_ip4)
                                error0 = (next0 == ACL_NEXT_INDEX_DENY) ?
                                             (is_output ? IP4_ERROR_OUTACL_TABLE_MISS : IP4_ERROR_INACL_TABLE_MISS) :
                                             IP4_ERROR_NONE;
                            else
                                error0 = (next0 == ACL_NEXT_INDEX_DENY) ?
                                             (is_output ? IP6_ERROR_OUTACL_TABLE_MISS : IP6_ERROR_INACL_TABLE_MISS) :
                                             IP6_ERROR_NONE;
                            b0->error = error_node->errors[error0];
                            break;
                        }
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ip_in_out_acl_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
    ...
    vlib_node_increment_counter(vm, node->node_index, is_output ? IP_OUTACL_ERROR_CHAIN_HIT : IP_INACL_ERROR_CHAIN_HIT,
                                chain_hits);
...
{
    return ip_in_out_acl_inline(vm, node, frame, 1 /* is_ip4 */, 0 /* is_output */);
}

static uword
ip4_outacl(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip_in_out_acl_inline(vm, node, frame, 1 /* is_ip4 */, 1 /* is_output */);
}
...
{
    return ip_in_out_acl_inline(vm, node, frame, 0 /* is_ip4 */, 0 /* is_output */);
}

static uword
ip6_outacl(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip_in_out_acl_inline(vm, node, frame, 0 /* is_ip4 */, 1 /* is_output */);
}
