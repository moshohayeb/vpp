...
format_ip4_source_and_port_range_check_trace(u8 *s, va_list *va)
{
    ...
    if (t->bypass)
        ...
        s = format(s, "fib %d src ip %U %s dst port %d: %s", t->fib_index, format_ip4_address, &t->src_addr,
                   t->is_tcp ? "TCP" : "UDP", (u32) t->port, (t->pass == 1) ? "PASS" : "FAIL");
    return s;
}
...
static inline u32
check_adj_port_range_x1(const protocol_port_range_dpo_t *ppr_dpo, u16 dst_port, u32 next)
{
    u16x8vec_t key;
    u16x8vec_t diff1;
    u16x8vec_t diff2;
    ...
    for (i = 0; i < ppr_dpo->n_used_blocks; i++) {
        diff1.as_u16x8           = u16x8_sub_saturate(ppr_dpo->blocks[i].low.as_u16x8, key.as_u16x8);
        diff2.as_u16x8           = u16x8_sub_saturate(ppr_dpo->blocks[i].hi.as_u16x8, key.as_u16x8);
...
always_inline uword
ip4_source_and_port_range_check_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_tx)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (is_tx) {
                if (ip0->protocol == IP_PROTOCOL_UDP)
                    fib_index0 = c0->fib_index[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_UDP_IN];
                if (ip0->protocol == IP_PROTOCOL_TCP)
                    fib_index0 = c0->fib_index[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_TCP_IN];
            } else {
                if (ip0->protocol == IP_PROTOCOL_UDP)
    ...
    if (is_tx)
        vlib_node_increment_counter(vm, ip4_source_port_and_range_check_tx.index,
                                    IP4_SOURCE_AND_PORT_RANGE_CHECK_ERROR_CHECK_OK, good_packets);
    else
...
static uword
ip4_source_and_port_range_check_rx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip4_source_and_port_range_check_inline(vm, node, frame, 0 /* !is_tx */);
}

static uword
ip4_source_and_port_range_check_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
int
set_ip_source_and_port_range_check(vlib_main_t *vm, u32 *fib_index, u32 sw_if_index, u32 is_add)
{
    ...
    if ((fib_index[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_TCP_OUT] != ~0) ||
        ...
        vnet_feature_enable_disable("ip4-unicast", "ip4-source-and-port-range-check-rx", sw_if_index, is_add, &config,
                                    sizeof(config));
    }
    ...
    if ((fib_index[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_TCP_IN] != ~0) ||
        ...
        vnet_feature_enable_disable("ip4-output", "ip4-source-and-port-range-check-tx", sw_if_index, is_add, &config,
                                    sizeof(config));
    }
    return rv;
}
...
static clib_error_t *
set_ip_source_and_port_range_check_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < IP_SOURCE_AND_PORT_RANGE_CHECK_N_PROTOCOLS; i++) {
        ...
        if (vrf_id[i] == 0)
            return clib_error_return(
                0, "TCP, UDP VRF ID should not be 0 (default). Should be distinct VRF for this purpose. ");
...
VLIB_CLI_COMMAND(set_interface_ip_source_and_port_range_check_command, static) = {
    ...
    .short_help = "set interface ip source-and-port-range-check <interface> [tcp-out-vrf <table-id>] [udp-out-vrf "
                  "<table-id>] [tcp-in-vrf <table-id>] [udp-in-vrf <table-id>] [del]",
};
/* *INDENT-ON* */
        ...
        for (j = 0; j < 8; j++) {
            if (ppr_dpo->blocks[i].low.as_u16[j]) {
                ...
                if (ppr_dpo->blocks[i].hi.as_u16[j] > (ppr_dpo->blocks[i].low.as_u16[j] + 1))
                    s = format(s, "%d-%d", (u32) ppr_dpo->blocks[i].low.as_u16[j],
...
static int
add_port_range_adjacency(u32 fib_index, ip4_address_t *address, u32 length, u16 *low_ports, u16 *high_ports)
{
    protocol_port_range_dpo_t *ppr_dpo;
...
static int
remove_port_range_adjacency(u32 fib_index, ip4_address_t *address, u32 length, u16 *low_ports, u16 *high_ports)
{
    protocol_port_range_dpo_t *ppr_dpo;
    fib_node_index_t fei;
    int i, j, k;
...
int
ip6_source_and_port_range_check_add_del(ip6_address_t *address, u32 length, u32 vrf_id, u16 *low_ports, u16 *high_ports,
                                        int is_add)
{
    u32 fib_index;
...
int
ip4_source_and_port_range_check_add_del(ip4_address_t *address, u32 length, u32 vrf_id, u16 *low_ports, u16 *high_ports,
                                        int is_add)
{
    u32 fib_index;
...
static clib_error_t *
ip_source_and_port_range_check_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u16 *low_ports  = 0;
    u16 *high_ports = 0;
    u16 this_low;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "%U/%d", unformat_ip4_address, &ip4_addr, &length))
            ...
            if (tmp > tmp2)
                return clib_error_return(0, "ports %d and %d out of order", tmp, tmp2);
            if (tmp == 0 || tmp > 65535)
...
static clib_error_t *
show_source_and_port_range_check_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    protocol_port_range_dpo_t *ppr_dpo;
    u32 fib_index;
    u8 addr_set = 0;
