...
static int
add_slaac_address(vlib_main_t *vm, u32 sw_if_index, u8 address_length, ip6_address_t *address, f64 due_time)
{
    rd_cp_main_t *rm = &rd_cp_main;
...
static void
add_default_route(vlib_main_t *vm, u32 sw_if_index, ip6_address_t *next_hop_address, f64 due_time)
{
    rd_cp_main_t *rm = &rd_cp_main;
...

    {
        ...
        fib_table_entry_update_one_path(fib_index, &pfx, FIB_SOURCE_API, FIB_ENTRY_FLAG_NONE, DPO_PROTO_IP6, &nh,
...
remove_slaac_address(vlib_main_t *vm, slaac_address_t *slaac_address)
{
    ...
    rv = ip6_add_del_interface_address(vm, slaac_address->sw_if_index, &slaac_address->address,
...
remove_default_route(vlib_main_t *vm, default_route_t *default_route)
{
...

    {
        ...
        fib_table_entry_path_remove(fib_index, &pfx, FIB_SOURCE_API, DPO_PROTO_IP6, &nh, default_route->sw_if_index, 0,
    ...
    default_route_pool_foreach_out:
        ...
        if (!route_already_present) {
            if (router_lifetime_in_sec != 0)
                add_default_route(vm, sw_if_index, (void *) r->router_address, current_time + router_lifetime_in_sec);
        } else {
            if (router_lifetime_in_sec != 0)
...
set_address_autoconfig(u32 sw_if_index, u8 enable, u8 install_default_routes)
{
    ...
    if (if_config->enabled && !enable) {
        /* *INDENT-OFF* */
        pool_foreach(slaac_address, rm->slaac_address_pool, ({ remove_slaac_address(vm, slaac_address); }));
        /* *INDENT-ON* */
    }
    if (if_config->install_default_routes && !install_default_routes) {
        /* *INDENT-OFF* */
        pool_foreach(default_route, rm->default_route_pool, ({ remove_default_route(vm, default_route); }));
        /* *INDENT-ON* */
    }
...
static clib_error_t *
ip6_nd_address_autoconfig(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    rd_cp_main_t *rm    = &rd_cp_main;
    vnet_main_t *vnm    = rm->vnet_main;
    clib_error_t *error = 0;
...
static void
vl_api_ip6_nd_address_autoconfig_t_handler(vl_api_ip6_nd_address_autoconfig_t *mp)
{
    ...
    rv = set_address_autoconfig(sw_if_index, mp->enable, mp->install_default_routes);
