...
/* *INDENT-OFF* */
static u8 icmp_to_icmp6_updater_pointer_table[] = {0,  1,  4, 4, ~0, ~0, ~0, ~0, 7,  6,
                                                   ~0, ~0, 8, 8, 8,  8,  24, 24, 24, 24};
/* *INDENT-ON* */
 ...
 */
always_inline int
icmp_to_icmp6(vlib_buffer_t *p, ip4_to_ip6_set_fn_t fn, void *ctx, ip4_to_ip6_set_fn_t inner_fn, void *inner_ctx)
{
    ...
    if (inner_ip4) {
        ...
        if (PREDICT_FALSE(ip4_is_fragment(inner_ip4))) {
            ...
            inner_frag_more =
                !!(inner_ip4->flags_and_fragment_offset & clib_net_to_host_u16(IP4_HEADER_FLAG_MORE_FRAGMENTS));
        } else {
        ...
        if (PREDICT_TRUE(inner_ip4->protocol == IP_PROTOCOL_TCP)) {
            ...
            *inner_L4_checksum = ip_csum_fold(ip_csum_sub_even(*inner_L4_checksum, *((u64 *) (&inner_ip4->src_address))));
            ...
            if (*inner_L4_checksum)
                *inner_L4_checksum =
                    ip_csum_fold(ip_csum_sub_even(*inner_L4_checksum, *((u64 *) (&inner_ip4->src_address))));
        ...
        if (PREDICT_FALSE(inner_frag != NULL)) {
            ...
            inner_ip6->payload_length =
                clib_host_to_net_u16(clib_net_to_host_u16(inner_ip6->payload_length) + sizeof(*inner_frag));
        }
...
ip4_to_ip6_fragmented(vlib_buffer_t *p, ip4_to_ip6_set_fn_t fn, void *ctx)
{
    ...
    frag->fragment_offset_and_more =
        ip6_frag_hdr_offset_and_more(ip4_get_fragment_offset(ip4), clib_net_to_host_u16(ip4->flags_and_fragment_offset) &
