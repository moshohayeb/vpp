...
ip6_ll_fib_create(u32 sw_if_index)
{
    ...
    desc = format(NULL, "IP6-link-local:%U", format_vnet_sw_interface_name, vnm, vnet_get_sw_interface(vnm, sw_if_index));
    ...
    fib_table_entry_update_one_path(ip6_ll_table.ilt_fibs[sw_if_index], &pfx, FIB_SOURCE_SPECIAL,
                                    (FIB_ENTRY_FLAG_ATTACHED | FIB_ENTRY_FLAG_NO_ATTACHED_EXPORT), DPO_PROTO_IP6, NULL,
...
fib_node_index_t
ip6_ll_table_entry_update(const ip6_ll_prefix_t *ilp, fib_route_path_flags_t flags)
{
    ...
    ip6_ll_entry_index =
        fib_table_entry_update(ip6_ll_fib_get(ilp->ilp_sw_if_index), &fp, FIB_SOURCE_IP6_ND,
                               (flags & FIB_ROUTE_PATH_LOCAL ? FIB_ENTRY_FLAG_LOCAL : FIB_ENTRY_FLAG_NONE), rpaths);
    vec_free(rpaths);
...
{
    vlib_cli_output(vm, "%U", format_fib_entry, ip6_ll_table_lookup(ilp),
...
static clib_error_t *
ip6_ll_show_fib(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vec_foreach_index(sw_if_index, ip6_ll_table.ilt_fibs)
    {
        ...
        s = format(s, "%U, fib_index:%d, locks:[", format_fib_table_name, fib_index, FIB_PROTOCOL_IP6, fib_index);
