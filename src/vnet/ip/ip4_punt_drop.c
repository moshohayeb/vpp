...
static uword
ip4_punt_policer(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (
        ip_punt_policer(vm, node, frame, vnet_feat_arc_ip4_punt.feature_arc_index, ip4_punt_policer_cfg.policer_index));
}
...
format_ip_punt_redirect_trace(u8 *s, va_list *args)
{
    ...
    if (NULL != si)
        s = format(s, "via %U on %U using adj:%d", format_ip46_address, &t->redirect.nh, IP46_TYPE_ANY,
                   format_vnet_sw_interface_name, vnm, si, t->redirect.adj_index);
    else
        s = format(s, "via %U on %d using adj:%d", format_ip46_address, &t->redirect.nh, IP46_TYPE_ANY,
...
static uword
ip4_punt_redirect(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (ip_punt_redirect(vm, node, frame, vnet_feat_arc_ip4_punt.feature_arc_index, &ip4_punt_redirect_cfg));
}
...
static uword
ip4_not_enabled(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (node->flags & VLIB_NODE_FLAG_TRACE)
        ip4_forward_next_trace(vm, node, frame, VLIB_TX);
...
void
ip_punt_redirect_add(ip_punt_redirect_t *cfg, u32 rx_sw_if_index, ip_punt_redirect_rx_t *redirect,
                     fib_protocol_t fproto, vnet_link_t linkt)
{
    ...
    if (~0 == rx_sw_if_index) {
        ...
        vec_validate_init_empty(cfg->redirect_by_rx_sw_if_index, rx_sw_if_index, uninit_rx_redirect);
        cfg->redirect_by_rx_sw_if_index[rx_sw_if_index] = *redirect;
        new                                             = &cfg->redirect_by_rx_sw_if_index[rx_sw_if_index];
    }

    new->adj_index = adj_nbr_add_or_lock(fproto, linkt, &redirect->nh, redirect->tx_sw_if_index);
}
...
void
ip4_punt_redirect_add(u32 rx_sw_if_index, u32 tx_sw_if_index, ip46_address_t *nh)
{
    ...
    ip_punt_redirect_add(&ip4_punt_redirect_cfg, rx_sw_if_index, &rx, FIB_PROTOCOL_IP4, VNET_LINK_IP4);
...
static clib_error_t *
ip4_punt_redirect_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    clib_error_t *error = 0;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            ...
            ;
        else if (unformat(line_input, "via %U %U", unformat_ip4_address, &nh.ip4, unformat_vnet_sw_interface, vnm,
                          &tx_sw_if_index))
            ;
        else if (unformat(line_input, "via %U", unformat_vnet_sw_interface, vnm, &tx_sw_if_index))
    ...
    vec_foreach_index(rx_sw_if_index, cfg->redirect_by_rx_sw_if_index)
    {
        ...
        if (~0 != rx->tx_sw_if_index) {
            s = format(s, " rx %U redirect via %U %U\n", format_vnet_sw_interface_name, vnm,
                       vnet_get_sw_interface(vnm, rx_sw_if_index), format_ip46_address, &rx->nh, IP46_TYPE_ANY,
                       format_vnet_sw_interface_name, vnm, vnet_get_sw_interface(vnm, rx->tx_sw_if_index));
        }
    }
    if (~0 != cfg->any_rx_sw_if_index.tx_sw_if_index) {
        s = format(s, " rx all redirect via %U %U\n", format_ip46_address, &cfg->any_rx_sw_if_index.nh, IP46_TYPE_ANY,
                   format_vnet_sw_interface_name, vnm,
                   vnet_get_sw_interface(vnm, cfg->any_rx_sw_if_index.tx_sw_if_index));
    }
...
static clib_error_t *
ip4_punt_redirect_show_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
