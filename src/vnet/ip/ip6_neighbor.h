...
extern int ip6_neighbor_ra_config(vlib_main_t *vm, u32 sw_if_index, u8 suppress, u8 managed, u8 other, u8 ll_option,
...
extern int ip6_neighbor_ra_prefix(vlib_main_t *vm, u32 sw_if_index, ip6_address_t *prefix_addr, u8 prefix_len,
                                  u8 use_default, u32 val_lifetime, u32 pref_lifetime, u8 no_advertise, u8 off_link,
                                  u8 no_autoconfig, u8 no_onlink, u8 is_no);

extern clib_error_t *ip6_set_neighbor_limit(u32 neighbor_limit);

extern void vnet_register_ip6_neighbor_resolution_event(vnet_main_t *vnm, void *address_arg, uword node_index,
                                                        uword type_opaque, uword data);

extern int vnet_set_ip6_ethernet_neighbor(vlib_main_t *vm, u32 sw_if_index, const ip6_address_t *a,
                                          const u8 *link_layer_address, uword n_bytes_link_layer_address, int is_static,
                                          int is_no_fib_entry);

extern int vnet_unset_ip6_ethernet_neighbor(vlib_main_t *vm, u32 sw_if_index, const ip6_address_t *a);

extern int ip6_neighbor_proxy_add_del(u32 sw_if_index, ip6_address_t *addr, u8 is_add);
...
void icmp6_send_router_solicitation(vlib_main_t *vm, u32 sw_if_index, u8 stop,
...
#define _VNET_IP6_NEIGHBOR_FUNCTION_DECL(f, tag)                                                                       \
                                                                                                                       \
    static void __vnet_ip6_neighbor_function_init_##tag##_##f(void) __attribute__((__constructor__));                  \
                                                                                                                       \
    ...
    {                                                                                                                  \
        ...
        init_function.fp                         = (void *) &f;                                                        \
                                                                                                                       ...
                                                                                                                       \
    static void __vnet_ip6_neighbor_function_deinit_##tag##_##f(void) __attribute__((__destructor__));                 \
                                                                                                                       \
    ...
    {                                                                                                                  \
        ...
        if (nm->tag##_functions->fp == (void *) &f) {                                                                  \
            nm->tag##_functions = nm->tag##_functions->next_ip6_neighbor_function;                                     \
            return;                                                                                                    \
        }                                                                                                              \
        next = nm->tag##_functions;                                                                                    \
        while (next->next_ip6_neighbor_function) {                                                                     \
            if (next->next_ip6_neighbor_function->fp == (void *) &f) {                                                 \
                next->next_ip6_neighbor_function = next->next_ip6_neighbor_function->next_ip6_neighbor_function;       \
                return;                                                                                                \
            }                                                                                                          \
