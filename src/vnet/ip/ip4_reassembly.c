...
static void
ip4_reass_trace_details(vlib_main_t *vm, u32 bi, ip4_reass_range_trace_t *trace)
{
...
format_ip4_reass_trace(u8 *s, va_list *args)
{
    ...
    switch (t->action) {
    case RANGE_SHRINK:
        s = format(s, "\n%Ushrink %U by %u", format_white_space, indent, format_ip4_reass_range_trace, &t->trace_range,
                   t->size_diff);
        break;
    case RANGE_DISCARD:
        s = format(s, "\n%Udiscard %U", format_white_space, indent, format_ip4_reass_range_trace, &t->trace_range);
        break;
    case RANGE_NEW:
        s = format(s, "\n%Unew %U", format_white_space, indent, format_ip4_reass_range_trace, &t->trace_range);
        break;
    case RANGE_OVERLAP:
        s = format(s, "\n%Uoverlapping/ignored %U", format_white_space, indent, format_ip4_reass_range_trace,
                   &t->trace_range);
        break;
    case FINALIZE:
...
static void
ip4_reass_add_trace(vlib_main_t *vm, vlib_node_runtime_t *node, ip4_reass_main_t *rm, ip4_reass_t *reass, u32 bi,
...
always_inline void
ip4_reass_free(ip4_reass_main_t *rm, ip4_reass_per_thread_t *rt, ip4_reass_t *reass)
{
...
always_inline void
ip4_reass_on_timeout(vlib_main_t *vm, ip4_reass_main_t *rm, ip4_reass_t *reass, u32 **vec_drop_timeout)
{
...
ip4_reass_t *
ip4_reass_find_or_create(vlib_main_t *vm, ip4_reass_main_t *rm, ip4_reass_per_thread_t *rt, ip4_reass_key_t *k,
...
always_inline void
ip4_reass_finalize(vlib_main_t *vm, vlib_node_runtime_t *node, ip4_reass_main_t *rm, ip4_reass_per_thread_t *rt,
                   ip4_reass_t *reass, u32 *bi0, u32 *next0, u32 *error0, u32 **vec_drop_compress,
                   u32 **vec_drop_overlap, bool is_feature)
{
    ...
    do {
        ...
        u32 trim_front     = ip4_header_bytes(ip) + ip4_reass_buffer_get_data_offset(tmp);
...
always_inline void
ip4_reass_insert_range_in_chain(vlib_main_t *vm, ip4_reass_main_t *rm, ip4_reass_per_thread_t *rt, ip4_reass_t *reass,
...
always_inline void
ip4_reass_remove_range_from_chain(vlib_main_t *vm, vlib_node_runtime_t *node, ip4_reass_main_t *rm,
                                  u32 **vec_drop_overlap, ip4_reass_t *reass, u32 prev_range_bi, u32 discard_bi)
{
    ...
    while (1) {
        ...
        if (PREDICT_FALSE(discard_b->flags & VLIB_BUFFER_IS_TRACED)) {
            ip4_reass_add_trace(vm, node, rm, reass, discard_bi, RANGE_DISCARD, 0);
        }
...
always_inline void
ip4_reass_update(vlib_main_t *vm, vlib_node_runtime_t *node, ip4_reass_main_t *rm, ip4_reass_per_thread_t *rt,
                 ip4_reass_t *reass, u32 *bi0, u32 *next0, u32 *error0, u32 **vec_drop_overlap, u32 **vec_drop_compress,
                 bool is_feature)
{
    int consumed      = 0;
    ...
    u32 fragment_first = fvnb->ip.reass.fragment_first = ip4_get_fragment_offset_bytes(fip);
    u32 fragment_length                                = clib_net_to_host_u16(fip->length) - ip4_header_bytes(fip);
    ...
    if (~0 == reass->first_bi) {
        ...
        ip4_reass_insert_range_in_chain(vm, rm, rt, reass, prev_range_bi, *bi0);
    ...
    while (~0 != candidate_range_bi) {
        ...
        if (fragment_last < candidate_vnb->ip.reass.range_first) {
            ...
            if (fragment_first >= candidate_vnb->ip.reass.range_first &&
                ...
                if (PREDICT_FALSE(fb->flags & VLIB_BUFFER_IS_TRACED)) {
                    ip4_reass_add_trace(vm, node, rm, reass, *bi0, RANGE_OVERLAP, 0);
                }
                break;
            }
            ...
            if (fragment_first < candidate_vnb->ip.reass.range_first) {
                ...
                if (overlap < ip4_reass_buffer_get_data_len(candidate_b)) {
                    ...
                    if (PREDICT_FALSE(fb->flags & VLIB_BUFFER_IS_TRACED)) {
                        ip4_reass_add_trace(vm, node, rm, reass, candidate_range_bi, RANGE_SHRINK, overlap);
                    }
                    ip4_reass_insert_range_in_chain(vm, rm, rt, reass, prev_range_bi, *bi0);
                    consumed = 1;
                } else {
                ...
                if (overlap < ip4_reass_buffer_get_data_len(candidate_b)) {
                    ...
                    if (~0 != candidate_vnb->ip.reass.next_range_bi) {
                        ...
                        ip4_reass_insert_range_in_chain(vm, rm, rt, reass, candidate_range_bi, *bi0);
                        consumed = 1;
                    }
                } else {
                    discard_candidate = 1;
            ...
            if (discard_candidate) {
                ...
                ip4_reass_remove_range_from_chain(vm, node, rm, vec_drop_overlap, reass, prev_range_bi,
                                                  candidate_range_bi);
                if (~0 != next_range_bi) {
    ...
    if (~0 != reass->last_packet_octet && reass->data_len == reass->last_packet_octet + 1) {
        ip4_reass_finalize(vm, node, rm, rt, reass, bi0, next0, error0, vec_drop_compress, vec_drop_overlap, is_feature);
    } else {
        if (consumed) {
            *bi0 = ~0;
        } else {
...
always_inline uword
ip4_reassembly_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_feature)
{
    ...
    while (n_left_from > 0 || vec_len(vec_drop_timeout) > 0 || vec_len(vec_drop_overlap) > 0 ||
        ...
        while (vec_len(vec_drop_timeout) > 0 && n_left_to_next > 0) {
            ...
            to_next += 1;
            n_left_to_next -= 1;
            vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi, IP4_REASSEMBLY_NEXT_DROP);
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (!ip4_get_fragment_more(ip0) && !ip4_get_fragment_offset(ip0)) {
                ...
                ip4_reass_key_t k;
                k.as_u64[0] = (u64) vnet_buffer(b0)->sw_if_index[VLIB_RX] << 32 | (u64) ip0->src_address.as_u32;
                k.as_u64[1] =
                    (u64) ip0->dst_address.as_u32 << 32 | (u64) ip0->fragment_id << 16 | (u64) ip0->protocol << 8;

                ip4_reass_t *reass = ip4_reass_find_or_create(vm, rm, rt, &k, &vec_drop_timeout);

                if (reass) {
                    ip4_reass_update(vm, node, rm, rt, reass, &bi0, &next0, &error0, &vec_drop_overlap,
...
static uword
ip4_reassembly(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ip4_reassembly_feature(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static void
ip4_reass_set_params(u32 timeout_ms, u32 max_reassemblies, u32 expire_walk_interval_ms)
{
...
vnet_api_error_t
ip4_reass_set(u32 timeout_ms, u32 max_reassemblies, u32 expire_walk_interval_ms)
{
    u32 old_nbuckets = ip4_reass_get_nbuckets();
    ip4_reass_set_params(timeout_ms, max_reassemblies, expire_walk_interval_ms);
    vlib_process_signal_event(ip4_reass_main.vlib_main, ip4_reass_main.ip4_reass_expire_node_idx,
    ...
    if (ip4_reass_main.max_reass_n > 0 && new_nbuckets > old_nbuckets) {
        ...
        clib_bihash_init_16_8(&new_hash, "ip4-reass", new_nbuckets, new_nbuckets * 1024);
        clib_bihash_foreach_key_value_pair_16_8(&ip4_reass_main.hash, ip4_rehash_cb, &ctx);
        if (ctx.failure) {
            clib_bihash_free_16_8(&new_hash);
            return -1;
        } else {
            clib_bihash_free_16_8(&ip4_reass_main.hash);
            clib_memcpy(&ip4_reass_main.hash, &new_hash, sizeof(ip4_reass_main.hash));
        }
    }
    return 0;
}
...
vnet_api_error_t
ip4_reass_get(u32 *timeout_ms, u32 *max_reassemblies, u32 *expire_walk_interval_ms)
{
...
ip4_reass_init_function(vlib_main_t *vm)
{
    ...
    ip4_reass_set_params(IP4_REASS_TIMEOUT_DEFAULT_MS, IP4_REASS_MAX_REASSEMBLIES_DEFAULT,
...
static uword
ip4_reass_walk_expired(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *f)
{
    ip4_reass_main_t *rm = &ip4_reass_main;
...
format_ip4_reass_key(u8 *s, va_list *args)
{
    ...
    s = format(s, "xx_id: %u, src: %U, dst: %U, frag_id: %u, proto: %u", key->xx_id, format_ip4_address, &key->src,
               format_ip4_address, &key->dst, clib_net_to_host_u16(key->frag_id), key->proto);
    return s;
}
...
format_ip4_reass(u8 *s, va_list *args)
{
    ...
    s           = format(s,
               ...
               reass->id, format_ip4_reass_key, &reass->key, reass->first_bi, reass->data_len, reass->last_packet_octet,
               reass->trace_op_counter);
    u32 bi      = reass->first_bi;
    u32 counter = 0;
    while (~0 != bi) {
        ...
        s                         = format(s,
                   ...
                   ip4_reass_buffer_get_data_offset_no_check(b), ip4_reass_buffer_get_data_len_no_check(b),
...
static clib_error_t *
show_ip4_reass(vlib_main_t *vm, unformat_input_t *input, CLIB_UNUSED(vlib_cli_command_t *lmd))
{
    ip4_reass_main_t *rm = &ip4_reass_main;
    ...
    for (thread_index = 0; thread_index < nthreads; ++thread_index) {
        ...
        if (details) {
            /* *INDENT-OFF* */
            pool_foreach(reass, rt->pool, { vlib_cli_output(vm, "%U", format_ip4_reass, vm, reass); });
            /* *INDENT-ON* */
        }
    ...
    vlib_cli_output(vm, "Maximum configured concurrent IP4 reassemblies per worker-thread: %lu\n",
...
{
    return vnet_feature_enable_disable("ip4-unicast", "ip4-reassembly-feature", sw_if_index, enable_disable, 0, 0);
}
