...
vl_api_punt_t_handler(vl_api_punt_t *mp)
{
    ...
    error = vnet_punt_add_del(vm, mp->ipv, mp->l4_protocol, ntohs(mp->l4_port), mp->is_add);
    if (error) {
        rv = -1;
...
vl_api_punt_socket_register_t_handler(vl_api_punt_socket_register_t *mp)
{
    ...
    error = vnet_punt_socket_add(vm, ntohl(mp->header_version), mp->is_ip4, mp->l4_protocol, ntohs(mp->l4_port),
                                 (char *) mp->pathname);
    if (error) {
        rv = -1;
        clib_error_report(error);
...
vl_api_punt_socket_deregister_t_handler(vl_api_punt_socket_deregister_t *mp)
{
    ...
    error = vnet_punt_socket_del(vm, mp->is_ip4, mp->l4_protocol, ntohs(mp->l4_port));
    if (error) {
        rv = -1;
        clib_error_report(error);
    }
