...
static ip46_type_t
ip_address_union_decode(const vl_api_address_union_t *in, vl_api_address_family_t af, ip46_address_t *out)
{
    ip46_type_t type;
...
static void
ip_address_union_encode(const ip46_address_t *in, vl_api_address_family_t af, vl_api_address_union_t *out)
{
...
void
ip_address_encode(const ip46_address_t *in, ip46_type_t type, vl_api_address_t *out)
{
    switch (type) {
    case IP46_TYPE_IP4:
...
{
    out->fp_proto = (ADDRESS_IP6 == clib_net_to_host_u32(in->af) ? FIB_PROTOCOL_IP6 : FIB_PROTOCOL_IP4);
...
{
    const vl_api_address_union_t *addr = va_arg(*args, vl_api_address_union_t *);
...
format_vl_api_prefix(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U/%d", format_vl_api_address, &pfx->address, pfx->address_length);
