...
format_ip_frag_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "IPv%s mtu: %u fragments: %u", t->ipv6 ? "6" : "4", t->mtu, t->n_fragments);
    return s;
}
 ...
 */
void
ip4_frag_do_fragment(vlib_main_t *vm, u32 from_bi, u32 **buffer, ip_frag_error_t *error)
{
    ...
    max = (clib_min(mtu, VLIB_BUFFER_DATA_SIZE) - sizeof(ip4_header_t)) & ~0x7;
    ...
    while (rem) {
        ...
        if (vnet_buffer(org_from_b)->ip_frag.flags & IP_FRAG_FLAG_IP4_HEADER) {
            ...
            ip4_header_t *encap_header4 = (ip4_header_t *) vlib_buffer_get_current(to_b);
            ...
            ip6_header_t *encap_header6   = (ip6_header_t *) vlib_buffer_get_current(to_b);
            encap_header6->payload_length = clib_host_to_net_u16(to_b->current_length - sizeof(*encap_header6));
        }
        rem -= len;
        fo += len;
    }
...
static inline uword
frag_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 node_index, bool is_ip6)
{
 ...
 */
void
ip6_frag_do_fragment(vlib_main_t *vm, u32 from_bi, u32 **buffer, ip_frag_error_t *error)
{
    vlib_buffer_t *from_b;
    ip6_header_t *ip6;
    ...
    while (rem) {
        ...
        len = (rem > (mtu - sizeof(ip6_header_t) - sizeof(ip6_frag_hdr_t)) ? max : rem);
        if (len != rem) /* Last fragment does not need to divisible by 8 */
            len &= ~0x7;
        if ((to_b = frag_buffer_alloc(org_from_b, &to_bi)) == 0) {
            *error = IP_FRAG_ERROR_MEMORY;
        ...
        to_b->current_length                  = len + sizeof(ip6_header_t) + sizeof(ip6_frag_hdr_t);
...
VLIB_REGISTER_NODE(ip4_frag_node) = {
    ...
    .next_nodes   = {[IP4_FRAG_NEXT_IP4_REWRITE] = "ip4-rewrite",
...
VLIB_REGISTER_NODE(ip6_frag_node) = {
    ...
    .next_nodes   = {[IP6_FRAG_NEXT_IP6_REWRITE] = "ip6-rewrite",
