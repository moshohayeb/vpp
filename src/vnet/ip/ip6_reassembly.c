...
static void
ip6_reass_trace_details(vlib_main_t *vm, u32 bi, ip6_reass_range_trace_t *trace)
{
...
format_ip6_reass_range_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "range: [%u, %u], off %d, len %u, bi %u", trace->range_first, trace->range_last, trace->data_offset,
               trace->data_len, trace->range_bi);
    return s;
}
...
format_ip6_reass_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "first bi: %u, data len: %u, ip/fragment[%u, %u]", t->trace_range.first_bi, t->total_data_len,
               t->fragment_first, t->fragment_last);
    switch (t->action) {
    case RANGE_NEW:
        s = format(s, "\n%Unew %U", format_white_space, indent, format_ip6_reass_range_trace, &t->trace_range);
        break;
    case RANGE_OVERLAP:
        s = format(s, "\n%Uoverlap %U", format_white_space, indent, format_ip6_reass_range_trace, &t->trace_range);
        break;
    case ICMP_ERROR_FL_TOO_BIG:
        s = format(s, "\n%Uicmp-error - frag_len > 65535 %U", format_white_space, indent, format_ip6_reass_range_trace,
                   &t->trace_range);
        break;
    case ICMP_ERROR_FL_NOT_MULT_8:
        s = format(s, "\n%Uicmp-error - frag_len mod 8 != 0 %U", format_white_space, indent,
...
static void
ip6_reass_add_trace(vlib_main_t *vm, vlib_node_runtime_t *node, ip6_reass_main_t *rm, ip6_reass_t *reass, u32 bi,
                    ip6_reass_trace_operation_e action, u32 size_diff)
{
    vlib_buffer_t *b          = vlib_get_buffer(vm, bi);
    vnet_buffer_opaque_t *vnb = vnet_buffer(b);
...
always_inline void
ip6_reass_free(ip6_reass_main_t *rm, ip6_reass_per_thread_t *rt, ip6_reass_t *reass)
{
...
always_inline void
ip6_reass_drop_all(vlib_main_t *vm, ip6_reass_main_t *rm, ip6_reass_t *reass, u32 **vec_drop_bi)
{
...
always_inline void
ip6_reass_on_timeout(vlib_main_t *vm, vlib_node_runtime_t *node, ip6_reass_main_t *rm, ip6_reass_t *reass, u32 *icmp_bi,
                     u32 **vec_timeout)
{
    if (~0 == reass->first_bi) {
        return;
    ...
    if (0 == vnet_buffer(b)->ip.reass.fragment_first) {
        *icmp_bi = reass->first_bi;
        if (PREDICT_FALSE(b->flags & VLIB_BUFFER_IS_TRACED)) {
            ip6_reass_add_trace(vm, node, rm, reass, reass->first_bi, ICMP_ERROR_RT_EXCEEDED, 0);
        }
...
ip6_reass_find_or_create(vlib_main_t *vm, vlib_node_runtime_t *node, ip6_reass_main_t *rm, ip6_reass_per_thread_t *rt,
                         ip6_reass_key_t *k, u32 *icmp_bi, u32 **vec_timeout)
{
    ...
    if (rt->reass_n >= rm->max_reass_n) {
        ...
        reass->id = ((u64) os_get_thread_index() * 1000000000) + rt->id_counter;
        ++rt->id_counter;
        reass->first_bi          = ~0;
...
always_inline void
ip6_reass_finalize(vlib_main_t *vm, vlib_node_runtime_t *node, ip6_reass_main_t *rm, ip6_reass_per_thread_t *rt,
                   ip6_reass_t *reass, u32 *bi0, u32 *next0, u32 *error0, u32 **vec_drop_compress, bool is_feature)
{
    ...
    *error0 = IP6_ERROR_NONE;
    ...
    do {
        ...
        u32 trim_front =
            vnet_buffer(tmp)->ip.reass.ip6_frag_hdr_offset + sizeof(*frag_hdr) + ip6_reass_buffer_get_data_offset(tmp);
        ...
        if (tmp_bi == reass->first_bi) {
            ...
            trim_end   = vlib_buffer_length_in_chain(vm, tmp) - data_len -
                       (vnet_buffer(tmp)->ip.reass.ip6_frag_hdr_offset + sizeof(*frag_hdr));
    ...
    ip6_ext_header_find_t(ip, prev_hdr, frag_hdr, IP_PROTOCOL_IPV6_FRAGMENTATION);
    if (prev_hdr) {
    ...
    memmove(frag_hdr, (u8 *) frag_hdr + sizeof(*frag_hdr),
            first_b->current_length - ip6_frag_hdr_offset - sizeof(ip6_frag_hdr_t));
...
always_inline void
ip6_reass_insert_range_in_chain(vlib_main_t *vm, ip6_reass_main_t *rm, ip6_reass_per_thread_t *rt, ip6_reass_t *reass,
...
always_inline void
ip6_reass_update(vlib_main_t *vm, vlib_node_runtime_t *node, ip6_reass_main_t *rm, ip6_reass_per_thread_t *rt,
                 ip6_reass_t *reass, u32 *bi0, u32 *next0, u32 *error0, ip6_frag_hdr_t *frag_hdr,
                 u32 **vec_drop_overlap, u32 **vec_drop_compress, bool is_feature)
{
    int consumed                       = 0;
    ...
    ASSERT(fvnb->ip.reass.ip6_frag_hdr_offset > 0 && fvnb->ip.reass.ip6_frag_hdr_offset < fb->current_length);
    ...
    if (~0 == reass->first_bi) {
        ...
        ip6_reass_insert_range_in_chain(vm, rm, rt, reass, prev_range_bi, *bi0);
    ...
    while (~0 != candidate_range_bi) {
        ...
        if (fragment_last < candidate_vnb->ip.reass.range_first) {
            ...
            ip6_reass_free(rm, rt, reass);
            if (PREDICT_FALSE(fb->flags & VLIB_BUFFER_IS_TRACED)) {
                ip6_reass_add_trace(vm, node, rm, reass, *bi0, RANGE_OVERLAP, 0);
            }
    ...
    if (~0 != reass->last_packet_octet && reass->data_len == reass->last_packet_octet + 1) {
        ip6_reass_finalize(vm, node, rm, rt, reass, bi0, next0, error0, vec_drop_compress, is_feature);
    } else {
        if (consumed) {
            *bi0 = ~0;
        } else {
...
always_inline bool
ip6_reass_verify_upper_layer_present(vlib_node_runtime_t *node, vlib_buffer_t *b, ip6_frag_hdr_t *frag_hdr)
{
...
always_inline bool
ip6_reass_verify_fragment_multiple_8(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b,
                                     ip6_frag_hdr_t *frag_hdr)
{
    vnet_buffer_opaque_t *vnb = vnet_buffer(b);
    ...
    if (more_fragments && 0 != fragment_length % 8) {
        icmp6_error_set_vnet_buffer(b, ICMP6_parameter_problem, ICMP6_parameter_problem_erroneous_header_field,
...
always_inline bool
ip6_reass_verify_packet_size_lt_64k(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b, ip6_frag_hdr_t *frag_hdr)
{
    vnet_buffer_opaque_t *vnb = vnet_buffer(b);
...
always_inline uword
ip6_reassembly_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_feature)
{
    ...
    while (n_left_from > 0 || vec_len(vec_timeout) > 0 || vec_len(vec_drop_overlap) > 0 ||
        ...
        while (vec_len(vec_timeout) > 0 && n_left_to_next > 0) {
            ...
            to_next += 1;
            n_left_to_next -= 1;
            vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi, IP6_REASSEMBLY_NEXT_DROP);
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (ip6_ext_hdr(ip0->protocol)) {
                ip6_ext_header_find_t(ip0, prev_hdr, frag_hdr, IP_PROTOCOL_IPV6_FRAGMENTATION);
            }
            if (!frag_hdr) {
            ...
            if (!ip6_reass_verify_fragment_multiple_8(vm, node, b0, frag_hdr) ||
                !ip6_reass_verify_packet_size_lt_64k(vm, node, b0, frag_hdr)) {
                next0 = IP6_REASSEMBLY_NEXT_ICMP_ERROR;
                goto skip_reass;
            }
            vnet_buffer(b0)->ip.reass.ip6_frag_hdr_offset = (u8 *) frag_hdr - (u8 *) ip0;
            ...
            k.as_u64[4]        = (u64) vnet_buffer(b0)->sw_if_index[VLIB_RX] << 32 | frag_hdr->identification;
            k.as_u64[5]        = ip0->protocol;
            ip6_reass_t *reass = ip6_reass_find_or_create(vm, node, rm, rt, &k, &icmp_bi, &vec_timeout);

            if (reass) {
                ip6_reass_update(vm, node, rm, rt, reass, &bi0, &next0, &error0, frag_hdr, &vec_drop_overlap,
...
static uword
ip6_reassembly(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ip6_reassembly_feature(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static void
ip6_reass_set_params(u32 timeout_ms, u32 max_reassemblies, u32 expire_walk_interval_ms)
{
...
vnet_api_error_t
ip6_reass_set(u32 timeout_ms, u32 max_reassemblies, u32 expire_walk_interval_ms)
{
    u32 old_nbuckets = ip6_reass_get_nbuckets();
    ip6_reass_set_params(timeout_ms, max_reassemblies, expire_walk_interval_ms);
    vlib_process_signal_event(ip6_reass_main.vlib_main, ip6_reass_main.ip6_reass_expire_node_idx,
    ...
    if (ip6_reass_main.max_reass_n > 0 && new_nbuckets > old_nbuckets) {
        ...
        clib_bihash_init_48_8(&new_hash, "ip6-reass", new_nbuckets, new_nbuckets * 1024);
        clib_bihash_foreach_key_value_pair_48_8(&ip6_reass_main.hash, ip6_rehash_cb, &ctx);
        if (ctx.failure) {
            clib_bihash_free_48_8(&new_hash);
            return -1;
        } else {
            clib_bihash_free_48_8(&ip6_reass_main.hash);
            clib_memcpy(&ip6_reass_main.hash, &new_hash, sizeof(ip6_reass_main.hash));
        }
    }
    return 0;
}
...
vnet_api_error_t
ip6_reass_get(u32 *timeout_ms, u32 *max_reassemblies, u32 *expire_walk_interval_ms)
{
...
ip6_reass_init_function(vlib_main_t *vm)
{
    ...
    ip6_reass_set_params(IP6_REASS_TIMEOUT_DEFAULT_MS, IP6_REASS_MAX_REASSEMBLIES_DEFAULT,
    ...
    if ((error = vlib_call_init_function(vm, ip_main_init)))
        return error;
    ip6_register_protocol(IP_PROTOCOL_IPV6_FRAGMENTATION, ip6_reass_node.index);
...
static uword
ip6_reass_walk_expired(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *f)
{
    ip6_reass_main_t *rm = &ip6_reass_main;
...
format_ip6_reass_key(u8 *s, va_list *args)
{
    ...
    s = format(s, "xx_id: %u, src: %U, dst: %U, frag_id: %u, proto: %u", key->xx_id, format_ip6_address, &key->src,
               format_ip6_address, &key->dst, clib_net_to_host_u16(key->frag_id), key->proto);
    return s;
}
...
format_ip6_reass(u8 *s, va_list *args)
{
    ...
    s           = format(s,
               ...
               reass->id, format_ip6_reass_key, &reass->key, reass->first_bi, reass->data_len, reass->last_packet_octet,
               reass->trace_op_counter);
    u32 bi      = reass->first_bi;
    u32 counter = 0;
    while (~0 != bi) {
        ...
        s                         = format(s,
                   ...
                   "fragment[%u, %u]\n",
                   counter, vnb->ip.reass.range_first, vnb->ip.reass.range_last, bi,
                   ip6_reass_buffer_get_data_offset_no_check(b), ip6_reass_buffer_get_data_len_no_check(b),
...
static clib_error_t *
show_ip6_reass(vlib_main_t *vm, unformat_input_t *input, CLIB_UNUSED(vlib_cli_command_t *lmd))
{
    ip6_reass_main_t *rm = &ip6_reass_main;
    ...
    for (thread_index = 0; thread_index < nthreads; ++thread_index) {
        ...
        if (details) {
            /* *INDENT-OFF* */
            pool_foreach(reass, rt->pool, { vlib_cli_output(vm, "%U", format_ip6_reass, vm, reass); });
            /* *INDENT-ON* */
        }
    ...
    vlib_cli_output(vm,
                    "Maximum configured concurrent IP6 reassemblies per "
                    "worker-thread: %lu\n",
...
{
    return vnet_feature_enable_disable("ip6-unicast", "ip6-reassembly-feature", sw_if_index, enable_disable, 0, 0);
}
