...
static void
send_ip_neighbor_details(u32 sw_if_index, u8 is_ipv6, u8 is_static, u8 *mac_address, u8 *ip_address,
...
vl_api_ip_neighbor_dump_t_handler(vl_api_ip_neighbor_dump_t *mp)
{
    ...
    if (mp->is_ipv6) {
        ...
        vec_foreach(n, ns)
        {
            send_ip_neighbor_details(n->key.sw_if_index, mp->is_ipv6, ((n->flags & IP6_NEIGHBOR_FLAG_STATIC) ? 1 : 0),
                                     (u8 *) n->link_layer_address, (u8 *) &(n->key.ip6_address.as_u8), reg, mp->context);
        }
        /* *INDENT-ON* */
        vec_free(ns);
    } else {
        ...
        vec_foreach(n, ns)
        {
            send_ip_neighbor_details(n->sw_if_index, mp->is_ipv6,
                                     ...
                                     (u8 *) n->ethernet_address, (u8 *) &(n->ip4_address.as_u8), reg, mp->context);
        }
        /* *INDENT-ON* */
        vec_free(ns);
    }
...
static void
send_ip_fib_details(vpe_api_main_t *am, vl_api_registration_t *reg, const fib_table_t *table, const fib_prefix_t *pfx,
                    fib_route_path_encode_t *api_rpaths, u32 context)
{
    ...
    memcpy(mp->table_name, table->ft_desc, clib_min(vec_len(table->ft_desc), sizeof(mp->table_name)));
    mp->address_length = pfx->fp_len;
    memcpy(mp->address, &pfx->fp_addr.ip4, sizeof(pfx->fp_addr.ip4));
    mp->stats_index = htonl(fib_table_entry_get_stats_index(table->ft_index, pfx));
...
vl_api_ip_fib_dump_t_handler(vl_api_ip_fib_dump_t *mp)
{
    ...
    pool_foreach(fib_table, im->fibs,
                 ({ fib_table_walk(fib_table->ft_index, FIB_PROTOCOL_IP4, vl_api_ip_fib_dump_walk, &ctx); }));
    /* *INDENT-ON* */
...
static void
send_ip6_fib_details(vpe_api_main_t *am, vl_api_registration_t *reg, const fib_table_t *table, const fib_prefix_t *pfx,
                     fib_route_path_encode_t *api_rpaths, u32 context)
{
...
static void
api_ip6_fib_table_get_all(vl_api_registration_t *reg, vl_api_ip6_fib_dump_t *mp, fib_table_t *fib_table)
{
    vpe_api_main_t *am = &vpe_api_main;
    ...
    ((BVT(clib_bihash) *) &im6->ip6_table[IP6_FIB_TABLE_NON_FWDING].ip6_hash, api_ip6_fib_table_put_entries, &ctx);
...
static void
send_ip_mfib_details(vl_api_registration_t *reg, u32 context, u32 table_id, fib_node_index_t mfei)
{
    ...
    mp->address_length = pfx.fp_len;
    memcpy(mp->grp_address, &pfx.fp_grp_addr.ip4, sizeof(pfx.fp_grp_addr.ip4));
    memcpy(mp->src_address, &pfx.fp_src_addr.ip4, sizeof(pfx.fp_src_addr.ip4));
...
vl_api_ip_mfib_dump_t_handler(vl_api_ip_mfib_dump_t *mp)
{
    ...
    pool_foreach(mfib_table, im->mfibs, ({
                     ip4_mfib_table_walk(&mfib_table->v4, vl_api_ip_mfib_table_dump_walk, &ctx);

                     vec_sort_with_function(ctx.entries, mfib_entry_cmp_for_sort);

                     vec_foreach(mfeip, ctx.entries)
                     {
                         send_ip_mfib_details(reg, mp->context, mfib_table->mft_table_id, *mfeip);
                     }
...
static void
send_ip6_mfib_details(vpe_api_main_t *am, vl_api_registration_t *reg, u32 table_id, mfib_prefix_t *pfx,
                      fib_route_path_encode_t *api_rpaths, u32 context)
{
    ...
    mp->table_id       = htonl(table_id);
    mp->address_length = pfx->fp_len;
    memcpy(mp->grp_address, &pfx->fp_grp_addr.ip6, sizeof(pfx->fp_grp_addr.ip6));
    memcpy(mp->src_address, &pfx->fp_src_addr.ip6, sizeof(pfx->fp_src_addr.ip6));
...
vl_api_ip6_mfib_dump_t_handler(vl_api_ip6_mfib_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(mfib_table, im->mfibs, ({
                     ip6_mfib_table_walk(&mfib_table->v6, vl_api_ip6_mfib_table_dump_walk, &ctx);
                     ...
                     vec_foreach(mfeip, ctx.entries)
                     {
                         ...
                         send_ip6_mfib_details(am, reg, mfib_table->mft_table_id, &pfx, api_rpaths, mp->context);
                     }
...
static void
vl_api_ip_punt_redirect_t_handler(vl_api_ip_punt_redirect_t *mp, vlib_main_t *vm)
{
    ...
    if (mp->is_add) {
        ...
        if (mp->is_ip6) {
            ...
            ip6_punt_redirect_add(ntohl(mp->rx_sw_if_index), ntohl(mp->tx_sw_if_index), &nh);
        } else {
            memcpy(&nh.ip4, mp->nh, sizeof(nh.ip4));

            ip4_punt_redirect_add(ntohl(mp->rx_sw_if_index), ntohl(mp->tx_sw_if_index), &nh);
        }
    } else {
        if (mp->is_ip6) {
...
vl_api_ip_neighbor_add_del_t_handler(vl_api_ip_neighbor_add_del_t *mp, vlib_main_t *vm)
{
    ...
    if (mp->is_add)
        rv = ip_neighbor_add(&ip, mp->is_ipv6, mp->mac_address, ntohl(mp->sw_if_index), flags, &stats_index);
    else
...
int
add_del_route_t_handler(u8 is_multipath, u8 is_add, u8 is_drop, u8 is_unreach, u8 is_prohibit, u8 is_local,
                        u8 is_multicast, u8 is_classify, u32 classify_table_index, u8 is_resolve_host,
                        u8 is_resolve_attached, u8 is_interface_rx, u8 is_rpf_id, u8 is_dvr, u8 is_source_lookup,
                        u8 is_udp_encap, u32 fib_index, const fib_prefix_t *prefix, dpo_proto_t next_hop_proto,
                        const ip46_address_t *next_hop, u32 next_hop_id, u32 next_hop_sw_if_index,
                        u8 next_hop_fib_index, u16 next_hop_weight, u16 next_hop_preference,
                        mpls_label_t next_hop_via_label, fib_mpls_label_t *next_hop_out_label_stack)
{
    ...
    if (path.frp_sw_if_index == ~0 && ip46_address_is_zero(&path.frp_addr) && path.frp_fib_index != ~0) {
    ...
    if (is_drop || is_local || is_classify || is_unreach || is_prohibit) {
        ...
        if (is_add) {
            ...
            if (is_drop)
                ...
                ip_null_dpo_add_and_lock(dproto, IP_NULL_ACTION_SEND_ICMP_UNREACH, &dpo);
            else if (is_prohibit)
                ip_null_dpo_add_and_lock(dproto, IP_NULL_ACTION_SEND_ICMP_PROHIBIT, &dpo);
            else if (is_classify) {
                if (pool_is_free_index(cm->tables, ntohl(classify_table_index))) {
                    stats_dsunlock();
                    return VNET_API_ERROR_NO_SUCH_TABLE;
                }

                dpo_set(&dpo, DPO_CLASSIFY, dproto, classify_dpo_create(dproto, ntohl(classify_table_index)));
            } else {
                stats_dsunlock();
                return VNET_API_ERROR_NO_SUCH_TABLE;
            }
        ...
        if (is_add)
            fib_table_entry_path_add2(fib_index, prefix, FIB_SOURCE_API, entry_flags, paths);
        else
            fib_table_entry_path_remove2(fib_index, prefix, FIB_SOURCE_API, paths);

        vec_free(paths);
    } else {
        if (is_add) {
            vec_add1(paths, path);
            fib_table_entry_update(fib_index, prefix, FIB_SOURCE_API, entry_flags, paths);
            vec_free(paths);
        } else {
...
int
add_del_route_check(fib_protocol_t table_proto, u32 table_id, u32 next_hop_sw_if_index, dpo_proto_t next_hop_table_proto,
                    u32 next_hop_table_id, u8 is_rpf_id, u32 *fib_index, u32 *next_hop_fib_index)
{
    ...
    *fib_index = fib_table_find(table_proto, ntohl(table_id));
    ...
    if (!is_rpf_id && ~0 != ntohl(next_hop_sw_if_index)) {
        if (pool_is_free_index(vnm->interface_main.sw_interfaces, ntohl(next_hop_sw_if_index))) {
        ...
        if (is_rpf_id)
            *next_hop_fib_index = mfib_table_find(fib_nh_proto, ntohl(next_hop_table_id));
        else
            *next_hop_fib_index = fib_table_find(fib_nh_proto, ntohl(next_hop_table_id));
...
static int
ip4_add_del_route_t_handler(vl_api_ip_add_del_route_t *mp, u32 *stats_index)
{
    ...
    rv = add_del_route_check(FIB_PROTOCOL_IP4, mp->table_id, mp->next_hop_sw_if_index, DPO_PROTO_IP4,
                             mp->next_hop_table_id, 0, &fib_index, &next_hop_fib_index);
    ...
    if (n_labels == 0)
        ...
        for (ii = 0; ii < n_labels; ii++) {
            label_stack[ii].fml_value = ntohl(mp->next_hop_out_label_stack[ii].label);
            ...
            label_stack[ii].fml_mode =
                (mp->next_hop_out_label_stack[ii].is_uniform ? FIB_MPLS_LSP_MODE_UNIFORM : FIB_MPLS_LSP_MODE_PIPE);
        }
    }

    rv = add_del_route_t_handler(mp->is_multipath, mp->is_add, mp->is_drop, mp->is_unreach, mp->is_prohibit,
                                 mp->is_local, 0, mp->is_classify, mp->classify_table_index, mp->is_resolve_host,
                                 mp->is_resolve_attached, 0, 0, mp->is_dvr, mp->is_source_lookup, mp->is_udp_encap,
                                 fib_index, &pfx, DPO_PROTO_IP4, &nh, ntohl(mp->next_hop_id),
                                 ntohl(mp->next_hop_sw_if_index), next_hop_fib_index, mp->next_hop_weight,
                                 mp->next_hop_preference, ntohl(mp->next_hop_via_label), label_stack);
...
static int
ip6_add_del_route_t_handler(vl_api_ip_add_del_route_t *mp, u32 *stats_index)
{
    fib_mpls_label_t *label_stack = NULL;
    u32 fib_index, next_hop_fib_index;
    int rv, ii, n_labels;
    ...
    rv = add_del_route_check(FIB_PROTOCOL_IP6, mp->table_id, mp->next_hop_sw_if_index, DPO_PROTO_IP6,
                             mp->next_hop_table_id, 0, &fib_index, &next_hop_fib_index);
    ...
    rv = add_del_route_t_handler(mp->is_multipath, mp->is_add, mp->is_drop, mp->is_unreach, mp->is_prohibit,
                                 mp->is_local, 0, mp->is_classify, mp->classify_table_index, mp->is_resolve_host,
                                 mp->is_resolve_attached, 0, 0, mp->is_dvr, mp->is_source_lookup, mp->is_udp_encap,
                                 fib_index, &pfx, DPO_PROTO_IP6, &nh, ntohl(mp->next_hop_id),
                                 ntohl(mp->next_hop_sw_if_index), next_hop_fib_index, mp->next_hop_weight,
                                 mp->next_hop_preference, ntohl(mp->next_hop_via_label), label_stack);
...
vl_api_ip_add_del_route_t_handler(vl_api_ip_add_del_route_t *mp)
{
    ...
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_IP_ADD_DEL_ROUTE_REPLY, ({ rmp->stats_index = htonl(stats_index); }))
    /* *INDENT-ON* */
}

void
ip_table_create(fib_protocol_t fproto, u32 table_id, u8 is_api, const u8 *name)
{
    u32 fib_index, mfib_index;
    ...
    if (0 != table_id) {
        ...
        if (~0 == mfib_index) {
            mfib_table_find_or_create_and_lock_w_name(fproto, table_id, (is_api ? MFIB_SOURCE_API : MFIB_SOURCE_CLI),
                                                      name);
        }
    }
}
...
static int
add_del_mroute_check(fib_protocol_t table_proto, u32 table_id, u32 next_hop_sw_if_index, u8 is_local, u32 *fib_index)
{
    vnet_main_t *vnm = vnet_get_main();
...
mroute_add_del_handler(u8 is_add, u8 is_local, u32 fib_index, const mfib_prefix_t *prefix, dpo_proto_t nh_proto,
                       u32 entry_flags, fib_rpf_id_t rpf_id, u32 next_hop_sw_if_index, ip46_address_t *nh,
                       u32 itf_flags, u32 bier_imp)
{
    ...
    if (is_add) {
        mfib_entry_index = mfib_table_entry_path_update(fib_index, prefix, MFIB_SOURCE_API, &path, itf_flags);
    } else {
        mfib_table_entry_path_remove(fib_index, prefix, MFIB_SOURCE_API, &path);
    }
...
static int
api_mroute_add_del_t_handler(vl_api_ip_mroute_add_del_t *mp, u32 *stats_index)
{
    ...
    rv       = add_del_mroute_check(fproto, mp->table_id, mp->next_hop_sw_if_index, mp->is_local, &fib_index);
    ...
    if (FIB_PROTOCOL_IP4 == fproto) {
        clib_memcpy(&pfx.fp_grp_addr.ip4, mp->grp_address, sizeof(pfx.fp_grp_addr.ip4));
        clib_memcpy(&pfx.fp_src_addr.ip4, mp->src_address, sizeof(pfx.fp_src_addr.ip4));
        memset(&nh.ip6, 0, sizeof(nh.ip6));
        clib_memcpy(&nh.ip4, mp->nh_address, sizeof(nh.ip4));
        if (!ip46_address_is_zero(&pfx.fp_src_addr))
            pfx.fp_len = 64;
    } else {
        clib_memcpy(&pfx.fp_grp_addr.ip6, mp->grp_address, sizeof(pfx.fp_grp_addr.ip6));
        clib_memcpy(&pfx.fp_src_addr.ip6, mp->src_address, sizeof(pfx.fp_src_addr.ip6));
        clib_memcpy(&nh.ip6, mp->nh_address, sizeof(nh.ip6));
        if (!ip46_address_is_zero(&pfx.fp_src_addr))
            pfx.fp_len = 256;
    }

    mfib_entry_index = mroute_add_del_handler(mp->is_add, mp->is_local, fib_index, &pfx, nh_proto,
                                              ntohl(mp->entry_flags), ntohl(mp->rpf_id), ntohl(mp->next_hop_sw_if_index),
                                              &nh, ntohl(mp->itf_flags), ntohl(mp->bier_imp));
...
static void
send_ip_details(vpe_api_main_t *am, vl_api_registration_t *reg, u32 sw_if_index, u8 is_ipv6, u32 context)
{
...
static void
send_ip_address_details(vpe_api_main_t *am, vl_api_registration_t *reg, u8 *ip, u16 prefix_length, u32 sw_if_index,
...
vl_api_ip_address_dump_t_handler(vl_api_ip_address_dump_t *mp)
{
    ...
    if (mp->is_ipv6) {
        ...
        foreach_ip_interface_address(lm6, ia, sw_if_index, 0, ({
                                         ...
                                         send_ip_address_details(am, reg, (u8 *) r6, prefix_length, sw_if_index, 1,
                                                                 mp->context);
                                     }));
        /* *INDENT-ON* */
    } else {
        ...
        foreach_ip_interface_address(lm4, ia, sw_if_index, 0, ({
                                         ...
                                         u16 prefix_length = ia->address_length;
                                         send_ip_address_details(am, reg, (u8 *) r4, prefix_length, sw_if_index, 0,
                                                                 mp->context);
                                     }));
        /* *INDENT-ON* */
    }
...
static void
send_ip_unnumbered_details(vpe_api_main_t *am, vl_api_registration_t *reg, u32 sw_if_index, u32 ip_sw_if_index,
                           u32 context)
{
...
vl_api_ip_unnumbered_dump_t_handler(vl_api_ip_unnumbered_dump_t *mp)
{
    ...
    if (~0 != sw_if_index) {
        ...
        pool_foreach(si, im->sw_interfaces, ({
                         if ((si->flags & VNET_SW_INTERFACE_FLAG_UNNUMBERED)) {
                             send_ip_unnumbered_details(am, reg, si->sw_if_index, si->unnumbered_sw_if_index,
                                                        mp->context);
                         }
                     }));
        /* *INDENT-ON* */
...
static void
vl_api_sw_interface_ip6nd_ra_config_t_handler(vl_api_sw_interface_ip6nd_ra_config_t *mp)
{
    ...
    rv = ip6_neighbor_ra_config(vm, ntohl(mp->sw_if_index), suppress, managed, other, ll_option, send_unicast, cease,
                                default_router, ntohl(mp->lifetime), ntohl(mp->initial_count),
                                ntohl(mp->initial_interval), ntohl(mp->max_interval), ntohl(mp->min_interval), is_no);
...
static void
vl_api_sw_interface_ip6nd_ra_prefix_t_handler(vl_api_sw_interface_ip6nd_ra_prefix_t *mp)
{
    vlib_main_t *vm = vlib_get_main();
    ...
    rv = ip6_neighbor_ra_prefix(vm, ntohl(mp->sw_if_index), (ip6_address_t *) mp->address, mp->address_length,
                                use_default, ntohl(mp->val_lifetime), ntohl(mp->pref_lifetime), no_advertise, off_link,
...
static void
send_ip6nd_proxy_details(vl_api_registration_t *reg, u32 context, const ip46_address_t *addr, u32 sw_if_index)
{
...
vl_api_ip6nd_proxy_dump_t_handler(vl_api_ip6nd_proxy_dump_t *mp)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(fib_table, im6->fibs,
                 ({ fib_table_walk(fib_table->ft_index, FIB_PROTOCOL_IP6, api_ip6nd_proxy_fib_table_walk, &ctx); }));
    /* *INDENT-ON* */
...
vl_api_ip6nd_proxy_add_del_t_handler(vl_api_ip6nd_proxy_add_del_t *mp)
{
    ...
    rv = ip6_neighbor_proxy_add_del(ntohl(mp->sw_if_index), (ip6_address_t *) mp->address, mp->is_del);
...
static void
vl_api_ip6nd_send_router_solicitation_t_handler(vl_api_ip6nd_send_router_solicitation_t *mp)
{
...
static void
vl_api_sw_interface_ip6_enable_disable_t_handler(vl_api_sw_interface_ip6_enable_disable_t *mp)
{
    vlib_main_t *vm = vlib_get_main();
...
static void
vl_api_sw_interface_ip6_set_link_local_address_t_handler(vl_api_sw_interface_ip6_set_link_local_address_t *mp)
{
    vlib_main_t *vm = vlib_get_main();
    ...
    error = set_ip6_link_local_address(vm, ntohl(mp->sw_if_index), (ip6_address_t *) mp->address);
    if (error) {
        clib_error_report(error);
        rv = VNET_API_ERROR_UNSPECIFIED;
    } else {
...
void
vl_mfib_signal_send_one(vl_api_registration_t *reg, u32 context, const mfib_signal_t *mfs)
{
...
static void
vl_api_ip_container_proxy_add_del_t_handler(vl_api_ip_container_proxy_add_del_t *mp)
{
...
static void
vl_api_ip_source_and_port_range_check_add_del_t_handler(vl_api_ip_source_and_port_range_check_add_del_t *mp)
{
    ...
    if (is_ipv6) {
        ...
        rv = ip6_source_and_port_range_check_add_del(&ip6_addr, mask_length, vrf_id, low_ports, high_ports, is_add);
    } else {
        clib_memcpy(ip4_addr.data, mp->address, sizeof(ip4_addr));
        rv = ip4_source_and_port_range_check_add_del(&ip4_addr, mask_length, vrf_id, low_ports, high_ports, is_add);
    }
...
handle_ip4_arp_event(u32 pool_index)
{
    ...
    if (!reg) {
        (void) vnet_add_del_ip4_arp_change_event(vnm, arp_change_delete_callback, event->pid, &event->address,
    ...
    if (vl_api_can_send_msg(reg)) {
        ...
        if (vlib_time_now(vm) > last_time + 10.0) {
            clib_warning("arp event for %U to pid %d: queue stuffed!", format_ip4_address, &event->address, event->pid);
...
handle_ip6_nd_event(u32 pool_index)
{
    ...
    reg = vl_api_client_index_to_registration(event->client_index);
    if (!reg) {
        (void) vnet_add_del_ip6_nd_change_event(vnm, nd_change_delete_callback, event->pid, &event->address,
    ...
    if (vl_api_can_send_msg(reg)) {
         ...
         */
        if (vlib_time_now(vm) > last_time + 10.0) {
            clib_warning("ip6 nd event for %U to pid %d: queue stuffed!", format_ip6_address, &event->address,
                         event->pid);
            last_time = vlib_time_now(vm);
        }
    }
...
static uword
resolver_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
...
static int
nd_change_data_callback(u32 pool_index, u8 *new_mac, u32 sw_if_index, ip6_address_t *address)
{
    vpe_api_main_t *am = &vpe_api_main;
    vl_api_ip6_nd_event_t *event;
    ...
    if (eth_mac_equal(event->new_mac, new_mac) && sw_if_index == ntohl(event->sw_if_index)) {
        return 1;
    }
    ...
    while (1) {
        ...
        if (event_type == WC_ARP_REPORT) {
            ...
            for (i = 0; i < vec_len(arp_events); i++) {
                /* discard dup event */
                if (arp_prev.ip4 == arp_events[i].ip4 && eth_mac_equal((u8 *) arp_prev.mac, arp_events[i].mac) &&
                    arp_prev.sw_if_index == arp_events[i].sw_if_index && (now - last_arp) < 10.0) {
                    continue;
                }
            ...
            for (i = 0; i < vec_len(nd_events); i++) {
                /* discard dup event */
                if (ip6_address_is_equal((ip6_address_t *) &nd_prev.ip6, &nd_events[i].ip6) &&
                    eth_mac_equal((u8 *) nd_prev.mac, nd_events[i].mac) &&
                    nd_prev.sw_if_index == nd_events[i].sw_if_index && (now - last_nd) < 10.0) {
                    continue;
                }
                nd_prev = nd_events[i];
                last_nd = now;
            ...
            for (i = 0; i < vec_len(ra_events); i++) {
                ...
                call_ip6_neighbor_callbacks(&ra_events[i], npm->ra_report_functions);
                ...
                pool_foreach(reg, vpe_api_main.ip6_ra_events_registrations, ({
                                 ...
                                 if (vl_reg && vl_api_can_send_msg(vl_reg)) {
                                     u32 event_size =
                                         ...
                                         vec_len(ra_events[i].prefixes) * sizeof(vl_api_ip6_ra_prefix_info_t);
                                     vl_api_ip6_ra_event_t *event = vl_msg_api_alloc(event_size);
                                     ...
                                     event->router_lifetime_in_sec =
                                         clib_host_to_net_u16(ra_events[i].router_lifetime_in_sec);
                                     event->neighbor_reachable_time_in_msec =
                                         clib_host_to_net_u32(ra_events[i].neighbor_reachable_time_in_msec);
                                     event->time_in_msec_between_retransmitted_neighbor_solicitations =
                                         clib_host_to_net_u32(
                                             ra_events[i].time_in_msec_between_retransmitted_neighbor_solicitations);

                                     event->n_prefixes = clib_host_to_net_u32(vec_len(ra_events[i].prefixes));
                                     vl_api_ip6_ra_prefix_info_t *prefix = (typeof(prefix)) event->prefixes;
                                     u32 j;
                                     for (j = 0; j < vec_len(ra_events[i].prefixes); j++) {
                                         ra_report_prefix_info_t *info = &ra_events[i].prefixes[j];
                                         memcpy(prefix->dst_address, info->dst_address.as_u8, 16);
...
static int
arp_change_data_callback(u32 pool_index, u8 *new_mac, u32 sw_if_index, u32 address)
{
    vpe_api_main_t *am = &vpe_api_main;
    vl_api_ip4_arp_event_t *event;
...
vl_api_want_ip4_arp_events_t_handler(vl_api_want_ip4_arp_events_t *mp)
{
    ...
    if (mp->address == 0) {
        uword *p = hash_get(am->wc_ip4_arp_events_registration_hash, mp->client_index);
        ...
        if (p) {
            if (mp->enable_disable) {
                ...
                hash_unset(am->wc_ip4_arp_events_registration_hash, mp->client_index);
        ...
        hash_set(am->wc_ip4_arp_events_registration_hash, rp->client_index, rp - am->wc_ip4_arp_events_registrations);
    ...
    if (mp->enable_disable) {
        ...
        rv = vnet_add_del_ip4_arp_change_event(vnm, arp_change_data_callback, mp->pid,
                                               &mp->address /* addr, in net byte order */, ip_resolver_process_node.index,
        ...
        rv = vnet_add_del_ip4_arp_change_event(vnm, arp_change_delete_callback, mp->pid,
                                               &mp->address /* addr, in net byte order */, ip_resolver_process_node.index,
...
want_ip4_arp_events_reaper(u32 client_index)
{
    ...
    vec_foreach(event_id, to_delete)
    {
        ...
        vnet_add_del_ip4_arp_change_event(vnm, arp_change_delete_callback, event->pid, &event->address,
                                          ip_resolver_process_node.index, IP4_ARP_EVENT, ~0 /* pool index, notused */,
                                          0 /* is_add */);
    }
    vec_free(to_delete);
...
vl_api_want_ip6_nd_events_t_handler(vl_api_want_ip6_nd_events_t *mp)
{
    ...
    if (ip6_address_is_zero((ip6_address_t *) mp->address)) {
        uword *p = hash_get(am->wc_ip6_nd_events_registration_hash, mp->client_index);
        vpe_client_registration_t *rp;
        if (p) {
            if (mp->enable_disable) {
                clib_warning("pid %d: already enabled...", mp->pid);
                ...
                hash_unset(am->wc_ip6_nd_events_registration_hash, mp->client_index);
        ...
        rp->client_index = mp->client_index;
        rp->client_pid   = mp->pid;
        hash_set(am->wc_ip6_nd_events_registration_hash, rp->client_index, rp - am->wc_ip6_nd_events_registrations);
    ...
    if (mp->enable_disable) {
        ...
        rv = vnet_add_del_ip6_nd_change_event(vnm, nd_change_data_callback, mp->pid,
                                              mp->address /* addr, in net byte order */, ip_resolver_process_node.index,
        ...
        rv = vnet_add_del_ip6_nd_change_event(vnm, nd_change_delete_callback, mp->pid,
                                              mp->address /* addr, in net byte order */, ip_resolver_process_node.index,
...
want_ip6_nd_events_reaper(u32 client_index)
{
    ...
    vec_foreach(event_id, to_delete)
    {
        ...
        vnet_add_del_ip6_nd_change_event(vnm, nd_change_delete_callback, event->pid, &event->address,
                                         ip_resolver_process_node.index, IP6_ND_EVENT, ~0 /* pool index, notused */,
                                         0 /* is_add */);
    }
    vec_free(to_delete);
...
vl_api_proxy_arp_add_del_t_handler(vl_api_proxy_arp_add_del_t *mp)
{
    ...
    rv = vnet_proxy_arp_add_del((ip4_address_t *) mp->proxy.low_address, (ip4_address_t *) mp->proxy.hi_address,
...
static walk_rc_t
send_proxy_arp_details(const ip4_address_t *lo_addr, const ip4_address_t *hi_addr, u32 fib_index, void *data)
{
    ...
    clib_memcpy(mp->proxy.low_address, lo_addr, sizeof(mp->proxy.low_address));
...
static walk_rc_t
send_proxy_arp_intfc_details(vnet_main_t *vnm, vnet_sw_interface_t *si, void *data)
{
...
vl_api_proxy_arp_intfc_dump_t_handler(vl_api_proxy_arp_intfc_dump_t *mp)
{
    ...
    vnet_sw_interface_walk(vnet_get_main(), send_proxy_arp_intfc_details, &wctx);
}

static void
vl_api_proxy_arp_intfc_enable_disable_t_handler(vl_api_proxy_arp_intfc_enable_disable_t *mp)
{
    int rv           = 0;
    vnet_main_t *vnm = vnet_get_main();
...
vl_api_ip_probe_neighbor_t_handler(vl_api_ip_probe_neighbor_t *mp)
{
    ...
    if (mp->is_ipv6)
        error = ip6_probe_neighbor(vm, (ip6_address_t *) mp->dst_address, sw_if_index, 0);
    else
        error = ip4_probe_neighbor(vm, (ip4_address_t *) mp->dst_address, sw_if_index, 0);
...
static void
vl_api_ip_scan_neighbor_enable_disable_t_handler(vl_api_ip_scan_neighbor_enable_disable_t *mp)
{
    int rv = 0;
    ...
    pool_foreach(fib_table, im4->fibs, ({
                     ...
                     pool_foreach(si, im->sw_interfaces, ({
                                      ...
                                      if (sw_if_index < vec_len(im4->fib_index_by_sw_if_index) &&
                                          (im4->fib_index_by_sw_if_index[si->sw_if_index] == fib->index))
...
ip6_reset_fib_t_handler(vl_api_reset_fib_t *mp)
{
    ...
    pool_foreach(fib_table, im6->fibs, ({
                     ...
                     /* Shut down interfaces in this FIB / clean out intfc routes */
                     pool_foreach(si, im->sw_interfaces, ({
                                      if (im6->fib_index_by_sw_if_index[si->sw_if_index] == fib->index)
                                          vec_add1(sw_if_indices_to_shut, si->sw_if_index);
                                  }));
...
vl_api_ip_reassembly_set_t_handler(vl_api_ip_reassembly_set_t *mp)
{
    ...
    int rv = 0;
    if (mp->is_ip6) {
        rv = ip6_reass_set(clib_net_to_host_u32(mp->timeout_ms), clib_net_to_host_u32(mp->max_reassemblies),
                           clib_net_to_host_u32(mp->expire_walk_interval_ms));
    } else {
        rv = ip4_reass_set(clib_net_to_host_u32(mp->timeout_ms), clib_net_to_host_u32(mp->max_reassemblies),
                           clib_net_to_host_u32(mp->expire_walk_interval_ms));
    }
...
vl_api_ip_reassembly_get_t_handler(vl_api_ip_reassembly_get_t *mp)
{
    ...
    if (mp->is_ip6) {
        rmp->is_ip6 = 1;
        ip6_reass_get(&rmp->timeout_ms, &rmp->max_reassemblies, &rmp->expire_walk_interval_ms);
    } else {
        rmp->is_ip6 = 0;
        ip4_reass_get(&rmp->timeout_ms, &rmp->max_reassemblies, &rmp->expire_walk_interval_ms);
    }
    rmp->timeout_ms              = clib_host_to_net_u32(rmp->timeout_ms);
    rmp->max_reassemblies        = clib_host_to_net_u32(rmp->max_reassemblies);
    rmp->expire_walk_interval_ms = clib_host_to_net_u32(rmp->expire_walk_interval_ms);
    vl_msg_api_send_shmem(q, (u8 *) &rmp);
}

void
vl_api_ip_reassembly_enable_disable_t_handler(vl_api_ip_reassembly_enable_disable_t *mp)
{
    vl_api_ip_reassembly_enable_disable_reply_t *rmp;
    int rv = 0;
    rv     = ip4_reass_enable_disable(clib_net_to_host_u32(mp->sw_if_index), mp->enable_ip4);
    if (0 == rv) {
        rv = ip6_reass_enable_disable(clib_net_to_host_u32(mp->sw_if_index), mp->enable_ip6);
    }
...
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers(VL_API_##N, #n, vl_api_##n##_t_handler, vl_noop_handler, vl_api_##n##_t_endian,            \
