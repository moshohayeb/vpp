...
wc_nd_signal_report(wc_nd_report_t *r)
{
    ...
    if (ni == (uword) ~0)
        return;
    wc_nd_report_t *q = vlib_process_signal_event_data(vm, ni, et, 1, sizeof *q);
...
format_ip6_neighbor_ip6_entry(u8 *s, va_list *va)
{
    ...
    if (!n)
        return format(s, "%=12s%=45s%=6s%=20s%=40s", "Time", "Address", "Flags", "Link layer", "Interface");
    ...
    s  = format(s, "%=12U%=45U%=6s%=20U%=40U", format_vlib_time, vm, n->time_last_updated, format_ip6_address,
               &n->key.ip6_address, flags ? (char *) flags : "", format_ethernet_address, n->link_layer_address,
...
ip6_neighbor_adj_fib_remove(ip6_neighbor_t *n, u32 fib_index)
{
    if (FIB_NODE_INDEX_INVALID != n->fib_entry_index) {
        if (ip6_address_is_link_local_unicast(&n->key.ip6_address)) {
            ...
            fib_table_entry_path_remove(fib_index, &pfx, FIB_SOURCE_ADJ, DPO_PROTO_IP6, &pfx.fp_addr,
...
static void
set_unset_ip6_neighbor_rpc(vlib_main_t *vm, u32 sw_if_index, const ip6_address_t *a, const u8 *link_layer_address,
                           int is_add, int is_static, int is_no_fib_entry)
{
    ...
    vl_api_rpc_call_main_thread(ip6_neighbor_set_unset_rpc_callback, (u8 *) &args, sizeof(args));
}
...
ip6_nbr_probe(ip_adjacency_t *adj)
{
    ...
    src = ip6_interface_address_matching_destination(im, dst, adj->rewrite_header.sw_if_index, &ia);
    if (!src) {
        return;
    }
    ...
    vnet_buffer(b)->sw_if_index[VLIB_RX] = vnet_buffer(b)->sw_if_index[VLIB_TX] = adj->rewrite_header.sw_if_index;
...
ip6_nd_mk_complete(adj_index_t ai, ip6_neighbor_t *nbr)
{
    adj_nbr_update_rewrite(ai, ADJ_NBR_REWRITE_FLAG_COMPLETE,
                           ethernet_build_rewrite(vnet_get_main(), nbr->key.sw_if_index, adj_get_link_type(ai),
...
ip6_nd_mk_incomplete(adj_index_t ai)
{
    ...
    adj_nbr_update_rewrite(ai, ADJ_NBR_REWRITE_FLAG_INCOMPLETE,
                           ethernet_build_rewrite(vnet_get_main(), adj->rewrite_header.sw_if_index, adj_get_link_type(ai),
...
static clib_error_t *
ip6_neighbor_sw_interface_up_down(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    ip6_neighbor_t *n;
    u32 i, *to_delete = 0;
    ...
    if (flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) {
        for (i = 0; i < vec_len(to_delete); i++) {
            ...
            adj_nbr_walk_nh6(n->key.sw_if_index, &n->key.ip6_address, ip6_nd_mk_complete_walk, n);
        }
    } else {
        for (i = 0; i < vec_len(to_delete); i++) {
            n = pool_elt_at_index(nm->neighbor_pool, to_delete[i]);
            adj_nbr_walk_nh6(n->key.sw_if_index, &n->key.ip6_address, ip6_nd_mk_incomplete_walk, NULL);
            if (n->flags & IP6_NEIGHBOR_FLAG_STATIC)
                continue;
            ip6_neighbor_adj_fib_remove(n, ip6_fib_table_get_index_for_sw_if_index(n->key.sw_if_index));
...
ip6_ethernet_update_adjacency(vnet_main_t *vnm, u32 sw_if_index, u32 ai)
{
    ...
    case IP_LOOKUP_NEXT_ARP:
        if (NULL != nbr) {
            adj_nbr_walk_nh6(sw_if_index, &nbr->key.ip6_address, ip6_nd_mk_complete_walk, nbr);
        } else {
            /*
             ...
             */
            adj_nbr_update_rewrite(ai, ADJ_NBR_REWRITE_FLAG_INCOMPLETE,
                                   ethernet_build_rewrite(vnm, sw_if_index, VNET_LINK_IP6,
                                                          VNET_REWRITE_FOR_SW_INTERFACE_ADDRESS_BROADCAST));
    ...
    case IP_LOOKUP_NEXT_MCAST: {
        ...
        rewrite = ethernet_build_rewrite(vnm, sw_if_index, adj->ia_link, ethernet_ip6_mcast_dst_addr());
...
ip6_neighbor_adj_fib_add(ip6_neighbor_t *n, u32 fib_index)
{
    if (ip6_address_is_link_local_unicast(&n->key.ip6_address)) {
        ...
        n->fib_entry_index = ip6_ll_table_entry_update(&pfx, FIB_ROUTE_PATH_FLAG_NONE);
    } else {
        fib_prefix_t pfx = {
            .fp_len      = 128,
            .fp_proto    = FIB_PROTOCOL_IP6,
        ...
        n->fib_entry_index =
            fib_table_entry_path_add(fib_index, &pfx, FIB_SOURCE_ADJ, FIB_ENTRY_FLAG_ATTACHED, DPO_PROTO_IP6,
                                     &pfx.fp_addr, n->key.sw_if_index, ~0, 1, NULL, FIB_ROUTE_PATH_FLAG_NONE);
    }
}
...
int
vnet_set_ip6_ethernet_neighbor(vlib_main_t *vm, u32 sw_if_index, const ip6_address_t *a, const u8 *link_layer_address,
                               uword n_bytes_link_layer_address, int is_static, int is_no_fib_entry)
{
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    ip6_neighbor_key_t k;
    ip6_neighbor_t *n           = 0;
    ...
    if (vlib_get_thread_index()) {
        set_unset_ip6_neighbor_rpc(vm, sw_if_index, a, link_layer_address, 1 /* set new neighbor */, is_static,
                                   is_no_fib_entry);
        return 0;
    }
    ...
    if (p) {
        ...
        if (!is_static && (n->flags & IP6_NEIGHBOR_FLAG_STATIC)) {
            ...
            if (0 == memcmp(n->link_layer_address, link_layer_address, n_bytes_link_layer_address))
    ...
    if (make_new_nd_cache_entry) {
        if (nm->limit_neighbor_cache_size && pool_elts(nm->neighbor_pool) >= nm->limit_neighbor_cache_size) {
         ...
         */
        if (!is_no_fib_entry) {
            ip6_neighbor_adj_fib_add(n, ip6_fib_table_get_index_for_sw_if_index(n->key.sw_if_index));
        } else {
    ...
    adj_nbr_walk_nh6(sw_if_index, &n->key.ip6_address, ip6_nd_mk_complete_walk, n);
...
check_customers:
    ...
    if (p) {
        ...
        while (next_index != (u32) ~0) {
            ...
            vlib_process_signal_event(vm, pr->node_index, pr->type_opaque, pr->data);
    ...
    if (p) {
        ...
        while (next_index != (u32) ~0) {
            ...
            if (fp)
                rv = (*fp)(mc->data, (u8 *) link_layer_address, sw_if_index, &ip6a_zero);
            /*
...
int
vnet_unset_ip6_ethernet_neighbor(vlib_main_t *vm, u32 sw_if_index, const ip6_address_t *a)
{
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    ip6_neighbor_key_t k;
    ip6_neighbor_t *n;
    ...
    if (vlib_get_thread_index()) {
        set_unset_ip6_neighbor_rpc(vm, sw_if_index, a, NULL, 0 /* unset */, 0, 0);
        return 0;
    }
    ...
    adj_nbr_walk_nh6(sw_if_index, &n->key.ip6_address, ip6_nd_mk_incomplete_walk, NULL);
    ip6_neighbor_adj_fib_remove(n, ip6_fib_table_get_index_for_sw_if_index(sw_if_index));
...
static void
ip6_neighbor_set_unset_rpc_callback(ip6_neighbor_set_unset_rpc_args_t *a)
{
    vlib_main_t *vm = vlib_get_main();
    if (a->is_add)
        vnet_set_ip6_ethernet_neighbor(vm, a->sw_if_index, &a->addr, a->link_layer_address, 6, a->is_static,
                                       a->is_no_fib_entry);
    else
...
ip6_neighbor_sort(void *a1, void *a2)
{
    ...
    int cmp;
    cmp = vnet_sw_interface_compare(vnm, n1->key.sw_if_index, n2->key.sw_if_index);
    if (!cmp)
...
static clib_error_t *
show_ip6_neighbors(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm = vnet_get_main();
    ip6_neighbor_t *n, *ns;
...
static clib_error_t *
set_ip6_neighbor(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm = vnet_get_main();
    ip6_address_t addr;
    u8 mac_address[6];
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "%U %U %U", unformat_vnet_sw_interface, vnm, &sw_if_index, unformat_ip6_address, &addr,
    ...
    if (!is_del)
        vnet_set_ip6_ethernet_neighbor(vm, sw_if_index, &addr, mac_address, sizeof(mac_address), is_static,
                                       is_no_fib_entry);
    else
...
icmp6_neighbor_solicitation_or_advertisement(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame,
                                             uword is_solicitation)
{
    ...
    option_type           = (is_solicitation ? ICMP6_NEIGHBOR_DISCOVERY_OPTION_source_link_layer_address :
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (!ip6_sadd_unspecified && !ip6_sadd_link_local) {
                ...
                if (ADJ_INDEX_INVALID != src_adj_index0) {
                    ...
                    error0 = ((adj0->rewrite_header.sw_if_index != sw_if_index0 || !is_rewrite0) ?
            ...
            if (PREDICT_TRUE(error0 == ICMP6_ERROR_NONE && o0 != 0 && !ip6_sadd_unspecified)) {
                vnet_set_ip6_ethernet_neighbor(vm, sw_if_index0,
                                               ...
                                               o0->ethernet_address, sizeof(o0->ethernet_address), 0, 0);
            }
            ...
            if (is_solicitation)
                next0 = (error0 != ICMP6_ERROR_NONE ? ICMP6_NEIGHBOR_SOLICITATION_NEXT_DROP :
                                                      ICMP6_NEIGHBOR_SOLICITATION_NEXT_REPLY);
            else {
                next0  = 0;
            ...
            if (is_solicitation && error0 == ICMP6_ERROR_NONE) {
                ...
                if (!ip6_sadd_unspecified)
                    ...
                    ip6_set_reserved_multicast_address(&ip0->dst_address, IP6_MULTICAST_SCOPE_link_local,
                ...
                h0->advertisement_flags = clib_host_to_net_u32(ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_SOLICITED |
                                                               ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_OVERRIDE);

                h0->icmp.checksum = 0;
                h0->icmp.checksum = ip6_tcp_udp_icmp_compute_checksum(vm, p0, ip0, &bogus_length);
                ASSERT(bogus_length == 0);
...
clib_error_t *
call_ip6_neighbor_callbacks(void *data, _vnet_ip6_neighbor_function_list_elt_t *elt)
{
    clib_error_t *error = 0;
...
static_always_inline uword
icmp6_router_solicitation(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    vnet_main_t *vnm        = vnet_get_main();
    ip6_main_t *im          = &ip6_main;
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(error0 == ICMP6_ERROR_NONE && o0 != 0 && !is_unspecified && !is_link_local)) {
                vnet_set_ip6_ethernet_neighbor(vm, sw_if_index0, &ip0->src_address, o0->ethernet_address,
                                               sizeof(o0->ethernet_address), 0, 0);
            }
                    ...
                    if (error0 == ICMP6_ERROR_NONE) {
                        ...
                        if (is_solicitation) {
                            if (0 != radv_info->last_radv_time &&
                                (now - radv_info->last_radv_time) < MIN_DELAY_BETWEEN_RAS)
                                is_dropped = 1;
                            else
                        ...
                        rh.time_in_msec_between_retransmitted_neighbor_solicitations = clib_host_to_net_u32(
                        ...
                        rh.flags =
                            (radv_info->adv_managed_flag) ? ICMP6_ROUTER_DISCOVERY_FLAG_ADDRESS_CONFIG_VIA_DHCP : 0;
                        ...
                        vlib_buffer_add_data(vm, vlib_buffer_get_free_list_index(p0), bi0, (void *) &rh,
                        ...
                        if (radv_info->adv_link_layer_address) {
                            ...
                            vlib_buffer_add_data(vm, vlib_buffer_get_free_list_index(p0), bi0, (void *) &h,
                        ...
                        pool_foreach(pr_info, radv_info->adv_prefixes_pool, ({
                                         if (pr_info->enabled && (!pr_info->decrement_lifetime_flag ||
                                             ...
                                             h.flags = (pr_info->adv_on_link_flag) ?
                                                           ICMP6_NEIGHBOR_DISCOVERY_PREFIX_INFORMATION_FLAG_ON_LINK :
                                                           0;
                                             h.flags |= (pr_info->adv_autonomous_flag) ?
                                             ...
                                             if (radv_info->cease_radv && pr_info->deprecated_prefix_flag) {
                                                 ...
                                                 if (pr_info->decrement_lifetime_flag) {
                                                     pr_info->adv_valid_lifetime_in_secs =
                                                         ((pr_info->valid_lifetime_expires > now)) ?
                                                             (pr_info->valid_lifetime_expires - now) :
                                                             0;

                                                     pr_info->adv_pref_lifetime_in_secs =
                                                         ((pr_info->pref_lifetime_expires > now)) ?
                                                             (pr_info->pref_lifetime_expires - now) :
                                                             0;
                                                 }
                                                 ...
                                                 h.preferred_time =
                                                     clib_host_to_net_u32(pr_info->adv_pref_lifetime_in_secs);
                                             }
                                             h.unused = 0;
                                             ...
                                             vlib_buffer_add_data(vm, vlib_buffer_get_free_list_index(p0), bi0,
                                                                  (void *) &h,
                                                                  sizeof(
...
static_always_inline uword
icmp6_router_advertisement(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    vnet_main_t *vnm        = vnet_get_main();
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    uword n_packets         = frame->n_vectors;
                ...
                if (error0 == ICMP6_ERROR_NONE) {
                    ...
                    if (error0 == ICMP6_ERROR_NONE) {
                        ...
                        if ((h0->current_hop_limit && radv_info->curr_hop_limit) &&
                            ...
                            ip6_neighbor_syslog(vm, LOG_WARNING, "our AdvCurHopLimit on %U doesn't agree with %U",
                                                format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip6_address,
                                                &ip0->src_address);
                        }

                        if ((h0->flags & ICMP6_ROUTER_DISCOVERY_FLAG_ADDRESS_CONFIG_VIA_DHCP) !=
                            radv_info->adv_managed_flag) {
                            ip6_neighbor_syslog(vm, LOG_WARNING, "our AdvManagedFlag on %U doesn't agree with %U",
                                                format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip6_address,
                                                &ip0->src_address);
                        }

                        if ((h0->flags & ICMP6_ROUTER_DISCOVERY_FLAG_OTHER_CONFIG_VIA_DHCP) != radv_info->adv_other_flag) {
                            ip6_neighbor_syslog(vm, LOG_WARNING, "our AdvOtherConfigFlag on %U doesn't agree with %U",
                                                format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip6_address,
                                                &ip0->src_address);
                        }
                        ...
                        if ((h0->time_in_msec_between_retransmitted_neighbor_solicitations &&
                            ...
                            (h0->time_in_msec_between_retransmitted_neighbor_solicitations !=
                             clib_host_to_net_u32(
                                 radv_info->adv_time_in_msec_between_retransmitted_neighbor_solicitations))) {
                            ip6_neighbor_syslog(vm, LOG_WARNING, "our AdvRetransTimer on %U doesn't agree with %U",
                                                format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip6_address,
                                                &ip0->src_address);
                        }
                        ...
                        if ((h0->neighbor_reachable_time_in_msec && radv_info->adv_neighbor_reachable_time_in_msec) &&
                            (h0->neighbor_reachable_time_in_msec !=
                             clib_host_to_net_u32(radv_info->adv_neighbor_reachable_time_in_msec))) {
                            ip6_neighbor_syslog(vm, LOG_WARNING, "our AdvReachableTime on %U doesn't agree with %U",
                                                format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip6_address,
                                                &ip0->src_address);
                        }
                        ...
                        while (options_len0 > 0) {
                            icmp6_neighbor_discovery_option_header_t *o0 =
                                (icmp6_neighbor_discovery_option_header_t *) opt_hdr;
                            int opt_len                                        = o0->n_data_u64s << 3;
                            icmp6_neighbor_discovery_option_type_t option_type = o0->type;

                            if (options_len0 < 2) {
                                ip6_neighbor_syslog(vm, LOG_ERR, "malformed RA packet on %U from %U",
                                                    format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip6_address,
                                                    &ip0->src_address);
                                break;
                            }
                            ...
                            if (opt_len == 0) {
                                ip6_neighbor_syslog(vm, LOG_ERR, " zero length option in RA on %U from %U",
                                                    format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip6_address,
                                                    &ip0->src_address);
                                break;
                            } else if (opt_len > options_len0) {
                                ip6_neighbor_syslog(vm,
                                                    LOG_ERR, "option length in RA packet  greater than total length on %U from %U",
                                                    format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip6_address,
                                                    &ip0->src_address);
                                break;
                            }
                            ...
                            case ICMP6_NEIGHBOR_DISCOVERY_OPTION_mtu: {
                                ...
                                if ((h->mtu && radv_info->adv_link_mtu) &&
                                    (h->mtu != clib_host_to_net_u32(radv_info->adv_link_mtu))) {
                                    ip6_neighbor_syslog(vm, LOG_WARNING, "our AdvLinkMTU on %U doesn't agree with %U",
                            ...
                            case ICMP6_NEIGHBOR_DISCOVERY_OPTION_prefix_information: {
                                ...
                                prefix->dst_address_length = h->dst_address_length;
                                ...
                                pool_foreach(pr_info, radv_info->adv_prefixes_pool, ({
                                                 ...
                                                 if (pr_info->enabled && (pr_info->prefix_len == h->dst_address_length) &&
                                                     ip6_address_is_equal_masked(&pr_info->prefix, &h->dst_address,
                                                                                 &mask)) {
                                                     /* found it */
                                                     if (!pr_info->decrement_lifetime_flag &&
                                                         valid != pr_info->adv_valid_lifetime_in_secs) {
                                                         ip6_neighbor_syslog(vm,
                                                                             LOG_WARNING, "our ADV validlifetime on  %U for %U does not  agree with %U",
                                                                             format_vnet_sw_if_index_name, vnm,
                                                                             sw_if_index0, format_ip6_address,
                                                                             &pr_info->prefix, format_ip6_address,
                                                                             &h->dst_address);
                                                     }
                                                     if (!pr_info->decrement_lifetime_flag &&
                                                         preferred != pr_info->adv_pref_lifetime_in_secs) {
                                                         ip6_neighbor_syslog(vm,
                                                                             LOG_WARNING, "our ADV preferredlifetime on  %U for %U does not  agree with %U",
                                                                             format_vnet_sw_if_index_name, vnm,
                                                                             sw_if_index0, format_ip6_address,
                                                                             &pr_info->prefix, format_ip6_address,
                                                                             &h->dst_address);
...
static inline bool
check_send_rs(vlib_main_t *vm, ip6_radv_t *radv_info, f64 current_time, f64 *due_time)
{
    vlib_buffer_t *p0;
    vlib_frame_t *f;
    u32 *to_next;
    ...
    if (params->mrc != 0 && --radv_info->n_left == 0)
        ...
        if (radv_info->sleep_interval > params->mrt)
            radv_info->sleep_interval = (1 + random_f64_from_to(-0.1, 0.1)) * params->mrt;
    ...
    while (true) {
        ...
        do {
            ...
            pool_foreach(radv_info, nm->if_radv_pool, ({
                             if (check_send_rs(vm, radv_info, current_time, &dt) && (dt < due_time))
                                 due_time = dt;
                         }));
            /* *INDENT-ON* */
            current_time = vlib_time_now(vm);
...
void
icmp6_send_router_solicitation(vlib_main_t *vm, u32 sw_if_index, u8 stop, icmp6_send_router_solicitation_params_t *params)
{
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    u32 rai;
    ip6_radv_t *ra = 0;
 ...
 */
static void
ip6_neighbor_add_mld_grp(ip6_radv_t *a, ip6_multicast_address_scope_t scope, ip6_multicast_link_local_group_id_t group)
{
    ip6_address_t addr;
 ...
 */
u32
ip6_neighbor_sw_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    ip6_radv_t *a           = 0;
    u32 ri                  = ~0;
    ...
    if (ri != ~0) {
        ...
        if (!is_add) {
            ...
            /* *INDENT-OFF* */
            pool_flush(p, a->adv_prefixes_pool, ({ mhash_unset(&a->address_to_prefix_index, &p->prefix, 0); }));
            pool_flush(m, a->mldp_group_pool, ({ mhash_unset(&a->address_to_mldp_index, &m->mcast_address, 0); }));
            /* *INDENT-ON* */
        ...
        if (is_add) {
            ...
            a->adv_link_mtu = vnet_sw_interface_get_mtu(vnm, sw_if_index, VNET_MTU_IP6);
            ...
            a->mcast_adj_index = adj_mcast_add_or_lock(FIB_PROTOCOL_IP6, VNET_LINK_IP6, sw_if_index);

            a->keep_sending_rs = 0;

            /* add multicast groups we will always be reporting  */
            ip6_neighbor_add_mld_grp(a, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_all_hosts);
            ip6_neighbor_add_mld_grp(a, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_all_routers);
            ip6_neighbor_add_mld_grp(a, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_mldv2_routers);
        }
    }
    return ri;
}
...
ip6_neighbor_send_mldpv2_report(u32 sw_if_index)
{
    ...
    n_allocated = vlib_buffer_alloc_from_free_list(vm, &bo0, n_to_alloc, VLIB_BUFFER_DEFAULT_FREE_LIST_INDEX);
    ...
    pool_foreach(m, radv_info->mldp_group_pool, ({
                     ...
                     vlib_buffer_add_data(vm, vlib_buffer_get_free_list_index(b0), bo0, (void *) &rr,
...
static uword
ip6_neighbor_process_timer_event(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    vnet_main_t *vnm        = vnet_get_main();
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    ip6_radv_t *radv_info;
    ...
    pool_foreach(radv_info, nm->if_radv_pool, ({
                     ...
                     if (radv_info->send_radv && (now >= radv_info->next_multicast_time)) {
                         ...
                         n_allocated =
                             vlib_buffer_alloc_from_free_list(vm, &bo0, n_to_alloc, VLIB_BUFFER_DEFAULT_FREE_LIST_INDEX);
                         ...
                         h0->ip.payload_length =
                             clib_host_to_net_u16(sizeof(icmp6_router_solicitation_header_t) -
...
static uword
ip6_icmp_neighbor_discovery_event_process(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    uword event_type;
...
static uword
icmp6_neighbor_solicitation(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
icmp6_neighbor_advertisement(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return icmp6_neighbor_solicitation_or_advertisement(vm, node, frame,
                                                        /* is_solicitation */
                                                        0);
...
static uword
ip6_discover_neighbor_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_glean)
{
    vnet_main_t *vnm     = vnet_get_main();
    ip6_main_t *im       = &ip6_main;
    ...
    if (time_now - im->nd_throttle_last_seed_change_time[thread_index] > 1e-3) {
        ...
        memset(im->nd_throttle_bitmaps[thread_index], 0, ND_THROTTLE_BITS / BITS(u8));
    ...
    while (n_left_from > 0) {
        vlib_get_next_frame(vm, node, IP6_DISCOVER_NEIGHBOR_NEXT_DROP, to_next_drop, n_left_to_next_drop);
        ...
        while (n_left_from > 0 && n_left_to_next_drop > 0) {
...

            {
                 ...
                 * adjacency.
                 */
                if (!ip6_src_address_for_packet(lm, sw_if_index0, &ip0->dst_address, &h0->ip.src_address)) {
                ...
                vnet_buffer(b0)->sw_if_index[VLIB_TX] = vnet_buffer(p0)->sw_if_index[VLIB_TX];
...
static uword
ip6_discover_neighbor(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static char *ip6_discover_neighbor_error_strings[] = {
    ...
    [IP6_DISCOVER_NEIGHBOR_ERROR_NO_SOURCE_ADDRESS] = "no source address for ND solicitation",
};
...
int
ip6_neighbor_ra_config(vlib_main_t *vm, u32 sw_if_index, u8 suppress, u8 managed, u8 other, u8 ll_option,
                       u8 send_unicast, u8 cease, u8 use_lifetime, u32 lifetime, u32 initial_count,
                       u32 initial_interval, u32 max_interval, u32 min_interval, u8 is_no)
{
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    int error;
    u32 ri;
    ...
    if (!error) {
        ...
        max_interval =
            (max_interval != 0) ? ((is_no) ? DEF_MAX_RADV_INTERVAL : max_interval) : radv_info->max_radv_interval;
        min_interval =
            (min_interval != 0) ? ((is_no) ? DEF_MIN_RADV_INTERVAL : min_interval) : radv_info->min_radv_interval;
        lifetime =
            (use_lifetime != 0) ? ((is_no) ? DEF_DEF_RTR_LIFETIME : lifetime) : radv_info->adv_router_lifetime_in_sec;
        ...
        if ((initial_count > MAX_INITIAL_RTR_ADVERTISEMENTS) || (initial_interval > MAX_INITIAL_RTR_ADVERT_INTERVAL))
            return VNET_API_ERROR_INVALID_VALUE;
        ...
        radv_info->initial_adverts_count = (initial_count != 0) ?
                                               ((is_no) ? MAX_INITIAL_RTR_ADVERTISEMENTS : initial_count) :
                                               radv_info->initial_adverts_count;
        radv_info->initial_adverts_interval = (initial_interval != 0) ?
...
int
ip6_neighbor_ra_prefix(vlib_main_t *vm, u32 sw_if_index, ip6_address_t *prefix_addr, u8 prefix_len, u8 use_default,
                       u32 val_lifetime, u32 pref_lifetime, u8 no_advertise, u8 off_link, u8 no_autoconfig,
...
ip6_neighbor_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    ...
    if (add_radv_info) {
        ip6_neighbor_ra_config(vm, sw_if_index, suppress, managed, other, suppress_ll_option, send_unicast, cease,
                               use_lifetime, ra_lifetime, ra_initial_count, ra_initial_interval, ra_max_interval,
                               ra_min_interval, is_no);
    } else {
        ...
        ip6_neighbor_ra_prefix(vm, sw_if_index, &ip6_addr, addr_len, use_prefix_default_values, valid_lifetime_in_secs,
                               pref_lifetime_in_secs, no_advertise, off_link, no_autoconfig, no_onlink, is_no);
    }
...
ip6_print_addrs(vlib_main_t *vm, u32 *addrs)
{
    ...
    for (i = 0; i < vec_len(addrs); i++) {
        ...
        vlib_cli_output(vm, "\t\t%U/%d", format_ip6_address, address, a->address_length);
    }
}

static clib_error_t *
show_ip6_interface_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm        = vnet_get_main();
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    clib_error_t *error     = 0;
    ...
    if (unformat_user(input, unformat_vnet_sw_interface, vnm, &sw_if_index)) {
        ...
        if (ri != ~0) {
            ...
            vlib_cli_output(vm, "\tLink-local address(es):\n");
            vlib_cli_output(vm, "\t\t%U\n", format_ip6_address, &radv_info->link_local_address);
            ...
            error = clib_error_return(0, "IPv6 not enabled on interface", format_unformat_error, input);
        }
    }
    return error;
}
...
disable_ip6_interface(vlib_main_t *vm, u32 sw_if_index)
{
    ...
    if (ri != ~0) {
        ...
        if (radv_info->ref_count == 0) {
            ...
            ip6_neighbor_sw_interface_add_del(vnet_get_main(), sw_if_index, 0);
        }
    }
    return error;
}
        ...
        if (sw_if0->type == VNET_SW_INTERFACE_TYPE_HARDWARE) {
            ...
            if (eth_if0) {
                ...
                if (ri != ~0) {
                    ...
                    ip6_link_local_address_from_ethernet_mac_address(&link_local_address, eth_if0->address);

                    sw_if0 = vnet_get_sw_interface(vnm, sw_if_index);
                    if (sw_if0->type == VNET_SW_INTERFACE_TYPE_SUB || sw_if0->type == VNET_SW_INTERFACE_TYPE_PIPE ||
                        sw_if0->type == VNET_SW_INTERFACE_TYPE_P2P) {
                        /* make up  an interface id */
                        link_local_address.as_u64[1] = random_u64(&radv_info->randomizer);
...
static clib_error_t *
enable_ip6_interface_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm    = vnet_get_main();
    clib_error_t *error = 0;
    u32 sw_if_index;
...
static clib_error_t *
disable_ip6_interface_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm    = vnet_get_main();
    clib_error_t *error = 0;
    u32 sw_if_index;
 ...
 *
 * '<em><b>ip6 nd <interface> [no] [ra-managed-config-flag] | [ra-other-config-flag] | [ra-suppress] |
[ra-suppress-link-layer] | [ra-send-unicast] | [ra-lifetime <lifetime>] | [ra-initial <cnt> <interval>] | [ra-interval
<max-interval> [<min-interval>]] | [ra-cease]</b></em>'
 *
 * Where:
 *
 ...
 *
 * '<em><b>ip6 nd <interface> [no] prefix <ip6-address>/<width> [<valid-lifetime> <pref-lifetime> | infinite]
[no-advertise] [off-link] [no-autoconfig] [no-onlink]</b></em>'
 *
 * Where:
 *
...
clib_error_t *
set_ip6_link_local_address(vlib_main_t *vm, u32 sw_if_index, ip6_address_t *address)
{
    clib_error_t *error     = 0;
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    u32 ri;
    ...
    if (!ip6_address_is_link_local_unicast(address)) {
        ...
        return (error = clib_error_return(0, "address not link-local", format_unformat_error));
    }
    ...
    if (ri != ~0) {
        ...
        /* delete the old one */
        error = ip6_add_del_interface_address(vm, sw_if_index, &radv_info->link_local_address, 128, 1 /* is_del */);

        if (!error) {
            /* add the new one */
            error = ip6_add_del_interface_address(vm, sw_if_index, address, 128, 0 /* is_del */);
        ...
        error          = clib_error_return(0, "ip6 not enabled for interface", format_unformat_error);
    }
    return error;
}

clib_error_t *
set_ip6_link_local_address_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm    = vnet_get_main();
    clib_error_t *error = 0;
    u32 sw_if_index;
 ...
 */
static void
ip6_neighbor_add_del_interface_address(ip6_main_t *im, uword opaque, u32 sw_if_index, ip6_address_t *address,
...
static void
ip6_neighbor_table_bind(ip6_main_t *im, uword opaque, u32 sw_if_index, u32 new_fib_index, u32 old_fib_index)
{
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    ip6_neighbor_t *n       = NULL;
    u32 i, *to_re_add = 0;
...
void
vnet_register_ip6_neighbor_resolution_event(vnet_main_t *vnm, void *address_arg, uword node_index, uword type_opaque,
                                            uword data)
{
    ip6_neighbor_main_t *nm = &ip6_neighbor_main;
    ...
    mhash_set(&nm->pending_resolutions_by_address, address, pr - nm->pending_resolutions, 0 /* old value */);
}

int
vnet_add_del_ip6_nd_change_event(vnet_main_t *vnm, void *data_callback, u32 pid, void *address_arg, uword node_index,
                                 uword type_opaque, uword data, int is_add)
{
    ...
    while (p && *p != ~0) {
        ...
        if (mc->node_index == node_index && mc->type_opaque == type_opaque && mc->pid == pid)
            break;
        p = &mc->next_index;
    }
...
int
vnet_ip6_nd_term(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *p0, ethernet_header_t *eth,
                 ip6_header_t *ip, u32 sw_if_index, u16 bd_index)
{
    ...
    if (ndh->icmp.type != ICMP6_neighbor_solicitation && ndh->icmp.type != ICMP6_neighbor_advertisement)
        return 0;
    ...
    if (PREDICT_FALSE(nm->wc_ip6_nd_publisher_node != (uword) ~0 && !ip6_address_is_link_local_unicast(&ip->src_address))) {
    ...
    if (ndh->icmp.type == ICMP6_neighbor_solicitation) {
        ...
        opt = (void *) (ndh + 1);
        if ((opt->header.type != ICMP6_NEIGHBOR_DISCOVERY_OPTION_source_link_layer_address) ||
        ...
        if (macp) { /* found ip-mac entry, generate eighbor advertisement response */
            ...
            ndh->advertisement_flags = clib_host_to_net_u32(ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_SOLICITED |
                                                            ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_OVERRIDE);
            ndh->icmp.checksum       = 0;
...
ip6_neighbor_proxy_add_del(u32 sw_if_index, ip6_address_t *addr, u8 is_del)
{
    ...
    if (is_del) {
        fib_table_entry_path_remove(fib_index, &pfx, FIB_SOURCE_IP6_ND_PROXY, DPO_PROTO_IP6, &nh, sw_if_index, ~0, 1,
                                    FIB_ROUTE_PATH_FLAG_NONE);
        /* flush the ND cache of this address if it's there */
        vnet_unset_ip6_ethernet_neighbor(vlib_get_main(), sw_if_index, addr);
    } else {
        fib_table_entry_path_add(fib_index, &pfx, FIB_SOURCE_IP6_ND_PROXY, FIB_ENTRY_FLAG_NONE, DPO_PROTO_IP6, &nh,
                                 sw_if_index, ~0, 1, NULL, FIB_ROUTE_PATH_FLAG_NONE);
    }
    return (0);
}

static clib_error_t *
set_ip6_nd_proxy_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm    = vnet_get_main();
    clib_error_t *error = 0;
    ip6_address_t addr;
...
void
send_ip6_na_w_addr(vlib_main_t *vm, const ip6_address_t *ip6_addr, u32 sw_if_index)
{
    ip6_main_t *i6m  = &ip6_main;
    vnet_main_t *vnm = vnet_get_main();
    ...
    if (ip6_addr) {
        clib_warning("Sending unsolicitated NA IP6 address %U on sw_if_idex %d", format_ip6_address, ip6_addr,
                     sw_if_index);
        ...
        u32 bi = 0;
        icmp6_neighbor_solicitation_header_t *h =
            vlib_packet_template_get_packet(vm, &i6m->discover_neighbor_packet_template, &bi);
        if (!h)
            return;

        ip6_set_reserved_multicast_address(&h->ip.dst_address, IP6_MULTICAST_SCOPE_link_local,
                                           IP6_MULTICAST_GROUP_ID_all_hosts);
