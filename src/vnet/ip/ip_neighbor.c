...
int
ip_neighbor_add(const ip46_address_t *ip, u8 is_ip6, const u8 *mac, u32 sw_if_index, ip_neighbor_flags_t flags,
                u32 *stats_index)
{
     ...
     */
    if (is_ip6) {
        rv     = vnet_set_ip6_ethernet_neighbor(vlib_get_main(), sw_if_index, &ip->ip6, mac, 6,
                                            (flags & IP_NEIGHBOR_FLAG_STATIC), (flags & IP_NEIGHBOR_FLAG_NO_ADJ_FIB));
        ...
        rv     = vnet_arp_set_ip4_over_ethernet(vnet_get_main(), sw_if_index, &a, (flags & IP_NEIGHBOR_FLAG_STATIC),
...
ip_neighbor_del(const ip46_address_t *ip, u8 is_ip6, u32 sw_if_index)
{
    ...
    if (is_ip6) {
        rv = vnet_unset_ip6_ethernet_neighbor(vlib_get_main(), sw_if_index, &ip->ip6);
    } else {
        ethernet_arp_ip4_over_ethernet_address_t a = {
            .ip4 = ip->ip4,
        };

        rv = vnet_arp_unset_ip4_over_ethernet(vnet_get_main(), sw_if_index, &a);
    }
...
ip_neighbor_scan_enable_disable(ip_neighbor_scan_arg_t *arg)
{
    ...
    if (arg->mode) {
        cfg->scan_interval   = arg->scan_interval ? arg->scan_interval * 60.0 : IP_NEIGHBOR_DEF_SCAN_INTERVAL;
        cfg->max_proc_time   = arg->max_proc_time ? arg->max_proc_time * 1e-6 : IP_NEIGHBOR_DEF_MAX_PROC_TIME;
        cfg->scan_int_delay  = arg->scan_int_delay ? arg->scan_int_delay * 1e-3 : IP_NEIGHBOR_DEF_SCAN_INT_DELAY;
        cfg->stale_threshold = arg->stale_threshold ? arg->stale_threshold * 60.0 : cfg->scan_interval * 4;
        cfg->max_update      = arg->max_update ? cfg->max_update : IP_NEIGHBOR_DEF_MAX_UPDATE;
    } else
        cfg->scan_interval = IP_NEIGHBOR_DEF_SCAN_INTERVAL;
}

static_always_inline u32
ip_neighbor_scan(vlib_main_t *vm, f64 start_time, u32 start_idx, u8 is_ip6, u8 delete_stale, u8 *update_count)
{
    ...
    while (curr_idx != ~0) {
        ...
        if ((update_count[0] >= cfg->max_update) ||
            (((loop_count % 100) == 0) && ((vlib_time_now(vm) - start_time) > cfg->max_proc_time)))
            break;
        ...
        if (delete_stale && (delta >= cfg->stale_threshold)) {
            ...
            if (!is_ip6) {
                ...
                vnet_unset_ip6_ethernet_neighbor(vm, n6->key.sw_if_index, &n6->key.ip6_address);
            }
            ...
            if (!is_ip6)
                ...
                ip6_probe_neighbor(vm, &n6->key.ip6_address, n6->key.sw_if_index, 1);
        }
...
static uword
neighbor_scan_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ip_neighbor_scan_config_t *cfg = &ip_neighbor_scan_conf;
    ...
    while (1) {
        ...
        if (ip4_nidx != ~0) /* scan ip4 neighbors */
            ip4_nidx = ip_neighbor_scan(vm, start, ip4_nidx, /* ip4 */ 0, purge4, &update);

        if (ip6_nidx != ~0) /* scan ip6 neighbors */
            ip6_nidx = ip_neighbor_scan(vm, start, ip6_nidx, /* ip6 */ 1, purge6, &update);

        if ((ip4_nidx == ~0) && (ip6_nidx == ~0)) { /* scan complete */
...
static clib_error_t *
ip_neighbor_scan_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
VLIB_CLI_COMMAND(ip_scan_neighbor_command, static) = {
    ...
    .short_help = "ip scan-neighbor [ip4|ip6|both|disable] [interval <n-min>] [max-time <n-usec>] [max-update <n>] "
                  "[delay <n-msec>] [stale <n-min>]",
    .is_mp_safe = 1,
};
/* *INDENT-ON* */
...
static clib_error_t *
show_ip_neighbor_scan(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ip_neighbor_scan_config_t *cfg = &ip_neighbor_scan_conf;

    if (cfg->mode == 0)
        vlib_cli_output(vm, "IP neighbor scan disabled - current time is %.4f sec", vlib_time_now(vm));
    else
        vlib_cli_output(vm,
                        "IP neighbor scan enabled for %U neighbors - "
                        ...
                        format_ip_scan_mode, cfg->mode, vlib_time_now(vm), cfg->scan_interval / 60.0,
                        cfg->stale_threshold / 60.0, cfg->max_proc_time / 1e-6, cfg->max_update,
                        cfg->scan_int_delay / 1e-3);
    return 0;
}
