...
/* *INDENT-OFF* */
static u8 icmp6_to_icmp_updater_pointer_table[] = {0,  1,  ~0, ~0, 2,  2,  9,  8,  12, 12, 12, 12, 12, 12,
...
ip6_parse(const ip6_header_t *ip6, u32 buff_len, u8 *l4_protocol, u16 *l4_offset, u16 *frag_hdr_offset)
{
    ...
    return (buff_len < (*l4_offset + 4)) || (clib_net_to_host_u16(ip6->payload_length) < (*l4_offset + 4 - sizeof(*ip6)));
}
...
ip6_get_port(ip6_header_t *ip6, u8 sender, u16 buffer_len)
{
    ...
    if (l4_protocol == IP_PROTOCOL_TCP || l4_protocol == IP_PROTOCOL_UDP) {
        return (sender) ? ((udp_header_t *) (l4))->src_port : ((udp_header_t *) (l4))->dst_port;
 ...
 */
always_inline int
icmp6_to_icmp(vlib_buffer_t *p, ip6_to_ip4_set_fn_t fn, void *ctx, ip6_to_ip4_set_fn_t inner_fn, void *inner_ctx)
{
    ...
    if (inner_ip6) {
        ...
        if (ip6_parse(inner_ip6, ip6_pay_len - 8, &inner_protocol, &inner_l4_offset, &inner_frag_offset))
            return -1;
    ...
    ip4->length =
        u16_net_add(ip6->payload_length, (inner_ip6 == NULL) ? sizeof(*ip4) : (2 * sizeof(*ip4) - sizeof(*ip6)));
...
ip6_to_ip4_fragmented(vlib_buffer_t *p, ip6_to_ip4_set_fn_t fn, void *ctx)
{
    ...
    if (ip6_parse(ip6, p->current_length, &l4_protocol, &l4_offset, &frag_offset))
        return -1;
 ...
 */
always_inline int
ip6_to_ip4_tcp_udp(vlib_buffer_t *p, ip6_to_ip4_set_fn_t fn, void *ctx, u8 udp_checksum)
{
    ip6_header_t *ip6;
    u16 *checksum;
    ip_csum_t csum = 0;
