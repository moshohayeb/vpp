...
format_gre_tunnel(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d] instance %d src %U dst %U fib-idx %d sw-if-idx %d ", t->dev_instance, t->user_instance,
               format_ip46_address, &t->tunnel_src, IP46_TYPE_ANY, format_ip46_address, &t->tunnel_dst.fp_addr,
               IP46_TYPE_ANY, t->outer_fib_index, t->sw_if_index);
...
static gre_tunnel_t *
gre_tunnel_db_find(const vnet_gre_add_del_tunnel_args_t *a, u32 outer_fib_index, gre_tunnel_key_t *key)
{
    gre_main_t *gm = &gre_main;
    uword *p;

    if (!a->is_ipv6) {
        gre_mk_key4(a->src.ip4, a->dst.ip4, outer_fib_index, a->tunnel_type, a->session_id, &key->gtk_v4);
        p = hash_get_mem(gm->tunnel_by_key4, &key->gtk_v4);
    } else {
        gre_mk_key6(&a->src.ip6, &a->dst.ip6, outer_fib_index, a->tunnel_type, a->session_id, &key->gtk_v6);
...
gre_tunnel_from_fib_node(fib_node_t *node)
{
    ...
    return ((gre_tunnel_t *) (((char *) node) - STRUCT_OFFSET_OF(gre_tunnel_t, node)));
}
...
gre_tunnel_stack(adj_index_t ai)
{
    ...
    if ((vec_len(gm->tunnel_index_by_sw_if_index) <= sw_if_index) || (~0 == gm->tunnel_index_by_sw_if_index[sw_if_index]))
        return;
    ...
    if (DPO_LOAD_BALANCE == tmp.dpoi_type) {
        ...
        if (fib_fwd == FIB_FORW_CHAIN_TYPE_UNICAST_IP4)
            hash = ip4_compute_flow_hash((ip4_header_t *) adj_get_rewrite(ai), lb->lb_hash_config);
        else
            hash = ip6_compute_flow_hash((ip6_header_t *) adj_get_rewrite(ai), lb->lb_hash_config);
...
static int
vnet_gre_tunnel_add(vnet_gre_add_del_tunnel_args_t *a, u32 outer_fib_index, u32 *sw_if_indexp)
{
    gre_main_t *gm   = &gre_main;
    ...
    if (t->type == GRE_TUNNEL_TYPE_L3)
        ...
        error         = ethernet_register_interface(vnm, gre_device_class.index, t_idx, address, &hw_if_index, 0);
        if (error) {
    ...
    t->fib_entry_index = fib_table_entry_special_add(outer_fib_index, &t->tunnel_dst, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
    t->sibling_index   = fib_entry_child_add(t->fib_entry_index, FIB_NODE_TYPE_GRE_TUNNEL, t_idx);
...
static int
vnet_gre_tunnel_delete(vnet_gre_add_del_tunnel_args_t *a, u32 outer_fib_index, u32 *sw_if_indexp)
{
    gre_main_t *gm   = &gre_main;
    vnet_main_t *vnm = gm->vnet_main;
    gre_tunnel_t *t;
...
int
vnet_gre_add_del_tunnel(vnet_gre_add_del_tunnel_args_t *a, u32 *sw_if_indexp)
{
    u32 outer_fib_index;
...
gre_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (NULL == gm->tunnel_index_by_sw_if_index || hi->sw_if_index >= vec_len(gm->tunnel_index_by_sw_if_index))
        return (NULL);
...
static clib_error_t *
create_gre_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
show_gre_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    gre_main_t *gm = &gre_main;
    gre_tunnel_t *t;
    u32 ti = ~0;
    ...
    if (~0 == ti) {
        /* *INDENT-OFF* */
        pool_foreach(t, gm->tunnels, ({ vlib_cli_output(vm, "%U", format_gre_tunnel, t); }));
        /* *INDENT-ON* */
    } else {
        t = pool_elt_at_index(gm->tunnels, ti);
