...
format_gre_tx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "GRE: tunnel %d len %d src %U dst %U", t->tunnel_id, t->length, format_ip46_address, &t->src,
...
format_gre_header_with_length(u8 *s, va_list *args)
{
    ...
    if (max_header_bytes != 0 && header_bytes < max_header_bytes) {
        ...
        if (node->format_buffer)
            s = format(s, "\n%U%U", format_white_space, indent, node->format_buffer, (void *) (h + 1),
...
static u8 *
gre_build_rewrite(vnet_main_t *vnm, u32 sw_if_index, vnet_link_t link_type, const void *dst_address)
{
    gre_main_t *gm = &gre_main;
...
static void
gre4_fixup(vlib_main_t *vm, ip_adjacency_t *adj, vlib_buffer_t *b0, const void *data)
{
    ip4_header_t *ip0;
...
static void
gre6_fixup(vlib_main_t *vm, ip_adjacency_t *adj, vlib_buffer_t *b0, const void *data)
{
    ip6_header_t *ip0;
...
gre_update_adj(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
    ...
    adj_nbr_midchain_update_rewrite(ai, !is_ipv6 ? gre4_fixup : gre6_fixup, NULL,
 ...
 */
static uword
gre_interface_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    gre_main_t *gm   = &gre_main;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE(gt0->type == GRE_TUNNEL_TYPE_ERSPAN)) {
                ...
                h0->t2_u64      = hdr;
                h0->t2.cos_en_t_session |= clib_host_to_net_u16(gt0->session_id);
            }
            if (PREDICT_FALSE(gt1->type == GRE_TUNNEL_TYPE_ERSPAN)) {
                ...
                h1->t2_u64      = hdr;
                h1->t2.cos_en_t_session |= clib_host_to_net_u16(gt1->session_id);
            }

            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                gre_tx_trace_t *tr0 = vlib_add_trace(vm, node, b0, sizeof(*tr0));
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                gre_tx_trace_t *tr1 = vlib_add_trace(vm, node, b1, sizeof(*tr1));
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                gre_tx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
