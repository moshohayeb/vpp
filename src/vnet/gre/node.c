...
format_gre_rx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "GRE: tunnel %d len %d src %U dst %U", t->tunnel_id, clib_net_to_host_u16(t->length),
               format_ip46_address, &t->src, IP46_TYPE_ANY, format_ip46_address, &t->dst, IP46_TYPE_ANY);
    return s;
}
...
always_inline uword
gre_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u8 is_ipv6)
{
    ...
    if (!is_ipv6)
        memset(&cached_tunnel_key.gtk_v4, 0xff, sizeof(cached_tunnel_key.gtk_v4));
    else
        memset(&cached_tunnel_key.gtk_v6, 0xff, sizeof(cached_tunnel_key.gtk_v6));
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            sparse_vec_index2(gm->next_by_protocol, protocol0, protocol1, &i0, &i1);
            ...
            b0->error = verr0 ? node->errors[GRE_ERROR_UNSUPPORTED_VERSION] : b0->error;
            next0     = verr0 ? GRE_INPUT_NEXT_DROP : next0;
            b1->error = verr1 ? node->errors[GRE_ERROR_UNSUPPORTED_VERSION] : b1->error;
            ...
            if (PREDICT_TRUE(next0 > GRE_INPUT_NEXT_DROP)) {
                if (is_ipv6) {
                    gre_mk_key6(&ip6_0->dst_address, &ip6_0->src_address, vnet_buffer(b0)->ip.fib_index, ttype0, 0,
                                &key0.gtk_v6);
                } else {
                    gre_mk_key4(ip4_0->dst_address, ip4_0->src_address, vnet_buffer(b0)->ip.fib_index, ttype0, 0,
                                &key0.gtk_v4);
                }

                if ((!is_ipv6 && !gre_match_key4(&cached_tunnel_key.gtk_v4, &key0.gtk_v4)) ||
                    (is_ipv6 && !gre_match_key6(&cached_tunnel_key.gtk_v6, &key0.gtk_v6))) {
                    gre_tunnel_t *t;
                    uword *p;
            ...
            vlib_increment_combined_counter(im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX, thread_index,
        ...
        drop0:
            if (PREDICT_TRUE(next1 > GRE_INPUT_NEXT_DROP)) {
                if (is_ipv6) {
                    gre_mk_key6(&ip6_1->dst_address, &ip6_1->src_address, vnet_buffer(b1)->ip.fib_index, ttype1, 0,
                                &key1.gtk_v6);
                } else {
                    gre_mk_key4(ip4_1->dst_address, ip4_1->src_address, vnet_buffer(b1)->ip.fib_index, ttype1, 0,
                                &key1.gtk_v4);
                }

                if ((!is_ipv6 && !gre_match_key4(&cached_tunnel_key.gtk_v4, &key1.gtk_v4)) ||
                    (is_ipv6 && !gre_match_key6(&cached_tunnel_key.gtk_v6, &key1.gtk_v6))) {
                    gre_tunnel_t *t;
                    uword *p;
        ...
        drop1:
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                gre_rx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                gre_rx_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
                tr->tunnel_id      = tunnel_sw_if_index;
                if (!is_ipv6) {
    ...
    vlib_node_increment_counter(vm, !is_ipv6 ? gre4_input_node.index : gre6_input_node.index, GRE_ERROR_PKTS_DECAP,
                                from_frame->n_vectors);
    return from_frame->n_vectors;
}

static uword
gre4_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return gre_input(vm, node, from_frame, /* is_ip6 */ 0);
}

static uword
gre6_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
void
gre_register_input_protocol(vlib_main_t *vm, gre_protocol_t protocol, u32 node_index, gre_tunnel_type_t tunnel_type)
{
