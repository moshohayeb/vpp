...
format_ipip_tx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "IPIP: tunnel %d len %d src %U dst %U", t->tunnel_id, t->length, format_ip46_address, &t->src,
               IP46_TYPE_ANY, format_ip46_address, &t->dst, IP46_TYPE_ANY);
    return s;
}

static u8 *
ipip_build_rewrite(vnet_main_t *vnm, u32 sw_if_index, vnet_link_t link_type, const void *dst_address)
{
    ip4_header_t *ip4;
    ip6_header_t *ip6;
    u8 *rewrite      = NULL;
...
static void
ipip4_fixup(vlib_main_t *vm, ip_adjacency_t *adj, vlib_buffer_t *b, const void *data)
{
    ip4_header_t *ip4;
    ...
    case VNET_LINK_IP6:
        ...
        if (t->tc_tos == 0xFF)
            ip4->tos = ip6_traffic_class_network_order((const ip6_header_t *) (ip4 + 1));
        break;
...
static void
ipip6_fixup(vlib_main_t *vm, ip_adjacency_t *adj, vlib_buffer_t *b, const void *data)
{
    ip6_header_t *ip6;
    const ipip_tunnel_t *t = data;
    ...
    case VNET_LINK_IP6:
        ...
        if (t->tc_tos == 0xFF)
            ip6_set_traffic_class_network_order(ip6, ip6_traffic_class_network_order((const ip6_header_t *) (ip6 + 1)));
        break;

    case VNET_LINK_IP4:
        ip6->protocol = IP_PROTOCOL_IP_IN_IP;
        if (t->tc_tos == 0xFF)
            ip6_set_traffic_class_network_order(ip6, ((ip4_header_t *) (ip6 + 1))->tos);
        break;
...
ipip_tunnel_stack(adj_index_t ai)
{
    ...
    fib_forward_chain_type_t fib_fwd =
        t->transport == IPIP_TRANSPORT_IP6 ? FIB_FORW_CHAIN_TYPE_UNICAST_IP6 : FIB_FORW_CHAIN_TYPE_UNICAST_IP4;
...
ipip_tunnel_from_fib_node(fib_node_t *node)
{
    ...
    return ((ipip_tunnel_t *) (((char *) node) - offsetof(ipip_tunnel_t, p2p.node)));
}
...
ipip_fib_add(ipip_tunnel_t *t)
{
    ...
    t->p2p.fib_entry_index = fib_table_entry_special_add(t->fib_index, &dst, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
    t->p2p.sibling_index   = fib_entry_child_add(t->p2p.fib_entry_index, gm->fib_node_type, t->dev_instance);
}
...
int
ipip_add_tunnel(ipip_transport_t transport, u32 instance, ip46_address_t *src, ip46_address_t *dst, u32 fib_index,
                u8 tc_tos, u32 *sw_if_indexp)
{
    ipip_main_t *gm  = &ipip_main;
    ...
    ipip_tunnel_key_t key = {.transport = transport, .fib_index = fib_index, .src = *src, .dst = *dst};
    ...
    hw_if_index = vnet_register_interface(vnm, ipip_device_class.index, t_idx, ipip_hw_interface_class.index, t_idx);
