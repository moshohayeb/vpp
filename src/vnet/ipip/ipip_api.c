...
vl_api_ipip_add_tunnel_t_handler(vl_api_ipip_add_tunnel_t *mp)
{
    ...
    fib_index = fib_table_find(fib_ip_proto(mp->is_ipv6), ntohl(mp->table_id));

    if (~0 == fib_index) {
        rv = VNET_API_ERROR_NO_SUCH_FIB;
    } else {
        rv = ipip_add_tunnel((mp->is_ipv6 ? IPIP_TRANSPORT_IP6 : IPIP_TRANSPORT_IP4), ntohl(mp->instance), &src, &dst,
...
static void
send_ipip_tunnel_details(ipip_tunnel_t *t, vl_api_registration_t *reg, u32 context)
{
...
vl_api_ipip_tunnel_dump_t_handler(vl_api_ipip_tunnel_dump_t *mp)
{
    ...
    if (sw_if_index == ~0) {
        /* *INDENT-OFF* */
        pool_foreach(t, gm->tunnels, ({ send_ipip_tunnel_details(t, reg, mp->context); }));
        /* *INDENT-ON* */
    } else {
...
vl_api_ipip_6rd_add_tunnel_t_handler(vl_api_ipip_6rd_add_tunnel_t *mp)
{
...

    {
        ...
        rv = sixrd_add_tunnel((ip6_address_t *) &mp->ip6_prefix, mp->ip6_prefix_len, (ip4_address_t *) &mp->ip4_prefix,
                              mp->ip4_prefix_len, (ip4_address_t *) &mp->ip4_src, mp->security_check, ip4_fib_index,
