...
static void
sixrd_fixup(vlib_main_t *vm, ip_adjacency_t *adj, vlib_buffer_t *b0, const void *data)
{
...
static void
ip6ip_fixup(vlib_main_t *vm, ip_adjacency_t *adj, vlib_buffer_t *b0, const void *data)
{
    const ipip_tunnel_t *t  = data;
    ip4_header_t *ip4       = vlib_buffer_get_current(b0);
    ip4->length             = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b0));
...
static u8 *
sixrd_build_rewrite(vnet_main_t *vnm, u32 sw_if_index, vnet_link_t link_type, const void *dst_address)
{
    u8 *rewrite = NULL;
    ipip_tunnel_t *t;
...
ip6ip_tunnel_stack(adj_index_t ai, u32 fib_entry_index)
{
    ...
    if (vnet_hw_interface_get_flags(vnet_get_main(), t->hw_if_index) & VNET_HW_INTERFACE_FLAG_LINK_UP) {
        adj_nbr_midchain_stack(ai, fib_entry_contribute_ip_forwarding(fib_entry_index));
    } else {
    ...
    if (IP_LOOKUP_NEXT_BCAST == adj->lookup_next_index) {
        ...
        if (sixrd_ad == NULL) {
            ...
            sixrd_ad->sixrd_fib_entry_index =
                fib_table_entry_special_add(t->fib_index, &pfx, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
            sixrd_ad->sixrd_sibling = fib_entry_child_add(sixrd_ad->sixrd_fib_entry_index, sixrd_fib_node_type,
...
int
sixrd_add_tunnel(ip6_address_t *ip6_prefix, u8 ip6_prefix_len, ip4_address_t *ip4_prefix, u8 ip4_prefix_len,
                 ip4_address_t *ip4_src, bool security_check, u32 ip4_fib_index, u32 ip6_fib_index, u32 *sw_if_index)
{
    ...
    ipip_tunnel_key_t key = {.transport = IPIP_TRANSPORT_IP4, .fib_index = ip4_fib_index, .src = src, .dst = dst};
    ...
    /* Create interface */
    u32 hw_if_index =
        vnet_register_interface(vnet_get_main(), sixrd_device_class.index, t_idx, sixrd_hw_interface_class.index, t_idx);
    ...
    fib_table_entry_update_one_path(ip6_fib_index, &pfx6, FIB_SOURCE_6RD, FIB_ENTRY_FLAG_ATTACHED, DPO_PROTO_IP6,
                                    &ADJ_BCAST_ADDR, t->sw_if_index, ~0, 1, NULL, FIB_ROUTE_PATH_FLAG_NONE);
...
sixrd_del_tunnel(u32 sw_if_index)
{
    ...
    fib_table_entry_path_remove(t->sixrd.ip6_fib_index, &pfx6, FIB_SOURCE_6RD, DPO_PROTO_IP6, &ADJ_BCAST_ADDR,
...
sixrd_adj_delegate_adj_deleted(adj_delegate_t *aed)
{
    ...
    fib_entry_child_remove(sixrd_ad->sixrd_fib_entry_index, sixrd_ad->sixrd_sibling);
    fib_table_entry_delete_index(sixrd_ad->sixrd_fib_entry_index, FIB_SOURCE_RR);
...
sixrd_fib_node_back_walk_notify(fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
{
    sixrd_adj_delegate_t *sixrd_ad;
