...
format_ipip_rx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "IPIP: tunnel %d len %d src %U dst %U", t->tunnel_id, clib_net_to_host_u16(t->length),
...
ipip_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, bool is_ipv6)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            ip46_address_t src0 = ip46_address_initializer, dst0 = ip46_address_initializer;
    ...
    vlib_node_increment_counter(vm, !is_ipv6 ? ipip4_input_node.index : ipip6_input_node.index, IPIP_ERROR_DECAP_PKTS,
                                from_frame->n_vectors);
    return from_frame->n_vectors;
}

static uword
ipip4_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return ipip_input(vm, node, from_frame, /* is_ip6 */ false);
}

static uword
ipip6_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
