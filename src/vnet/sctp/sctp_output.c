...
sctp_flush_frame_to_output(vlib_main_t *vm, u8 thread_index, u8 is_ip4)
{
    if (sctp_main.tx_frames[!is_ip4][thread_index]) {
        ...
        vlib_put_frame_to_node(vm, next_index, sctp_main.tx_frames[!is_ip4][thread_index]);
...
sctp_flush_frame_to_ip_lookup(vlib_main_t *vm, u8 thread_index, u8 is_ip4)
{
    if (sctp_main.ip_lookup_tx_frames[!is_ip4][thread_index]) {
        ...
        vlib_put_frame_to_node(vm, next_index, sctp_main.ip_lookup_tx_frames[!is_ip4][thread_index]);
...
u32
ip4_sctp_compute_checksum(vlib_main_t *vm, vlib_buffer_t *p0, ip4_header_t *ip0)
{
    ip_csum_t checksum;
    ...
    checksum                       = clib_host_to_net_u32(payload_length_host_byte_order + (ip0->protocol << 16));

    if (BITS(uword) == 32) {
        checksum = ip_csum_with_carry(checksum, clib_mem_unaligned(&ip0->src_address, u32));
        checksum = ip_csum_with_carry(checksum, clib_mem_unaligned(&ip0->dst_address, u32));
    } else
        checksum = ip_csum_with_carry(checksum, clib_mem_unaligned(&ip0->src_address, u64));
    ...
    while (1) {
        checksum = ip_incremental_checksum(checksum, data_this_buffer, n_this_buffer);
...
u32
ip6_sctp_compute_checksum(vlib_main_t *vm, vlib_buffer_t *p0, ip6_header_t *ip0, int *bogus_lengthp)
{
    ip_csum_t checksum;
    ...
    *bogus_lengthp = 0;
    ...
    for (i = 0; i < ARRAY_LEN(ip0->src_address.as_uword); i++) {
        checksum = ip_csum_with_carry(checksum, clib_mem_unaligned(&ip0->src_address.as_uword[i], uword));
        checksum = ip_csum_with_carry(checksum, clib_mem_unaligned(&ip0->dst_address.as_uword[i], uword));
    }
...
void
sctp_push_ip_hdr(sctp_main_t *tm, sctp_sub_connection_t *sctp_sub_conn, vlib_buffer_t *b)
{
    sctp_header_t *th = vlib_buffer_get_current(b);
    vlib_main_t *vm   = vlib_get_main();
    if (sctp_sub_conn->c_is_ip4) {
        ip4_header_t *ih;
        ih = vlib_buffer_push_ip4(vm, b, &sctp_sub_conn->c_lcl_ip4, &sctp_sub_conn->c_rmt_ip4, IP_PROTOCOL_SCTP, 1);
        th->checksum = ip4_sctp_compute_checksum(vm, b, ih);
    } else {
        ip6_header_t *ih;
        int bogus = ~0;

        ih = vlib_buffer_push_ip6(vm, b, &sctp_sub_conn->c_lcl_ip6, &sctp_sub_conn->c_rmt_ip6, IP_PROTOCOL_SCTP);
...
sctp_alloc_tx_buffers(sctp_main_t *tm, u8 thread_index, u32 n_free_buffers)
{
    ...
    vec_validate(tm->tx_buffers[thread_index], current_length + n_free_buffers - 1);
    n_allocated = vlib_buffer_alloc(vm, &tm->tx_buffers[thread_index][current_length], n_free_buffers);
...
always_inline void
sctp_enqueue_to_output_i(vlib_main_t *vm, vlib_buffer_t *b, u32 bi, u8 is_ip4, u8 flush)
{
...
always_inline void
sctp_enqueue_to_output_now(vlib_main_t *vm, vlib_buffer_t *b, u32 bi, u8 is_ip4)
{
    sctp_enqueue_to_output_i(vm, b, bi, is_ip4, 1);
}

always_inline void
sctp_enqueue_to_ip_lookup_i(vlib_main_t *vm, vlib_buffer_t *b, u32 bi, u8 is_ip4, u32 fib_index, u8 flush)
{
    sctp_main_t *tm  = vnet_get_sctp_main();
    u32 thread_index = vlib_get_thread_index();
    u32 *to_next, next_index;
...
always_inline void
sctp_enqueue_to_ip_lookup(vlib_main_t *vm, vlib_buffer_t *b, u32 bi, u8 is_ip4, u32 fib_index)
{
 ...
 */
void
sctp_prepare_init_chunk(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b)
{
    ...
    init_chunk->sctp_hdr.src_port =
        sub_conn->c_lcl_port; /* No need of host_to_net conversion, already in net-byte order */
    init_chunk->sctp_hdr.dst_port =
        sub_conn->c_rmt_port; /* No need of host_to_net conversion, already in net-byte order */
    ...
    init_chunk->inboud_streams_count   = clib_host_to_net_u16(INBOUND_STREAMS_COUNT);
    init_chunk->outbound_streams_count = clib_host_to_net_u16(OUTBOUND_STREAMS_COUNT);

    init_chunk->initial_tsn = clib_host_to_net_u32(sctp_conn->local_initial_tsn);
    SCTP_CONN_TRACKING_DBG("sctp_conn->local_initial_tsn = %u", sctp_conn->local_initial_tsn);
    ...
    SCTP_DBG_STATE_MACHINE("CONN_INDEX = %u, CURR_CONN_STATE = %u (%s), "
                           ...
                           sub_conn->connection.c_index, sctp_conn->state, sctp_state_to_string(sctp_conn->state),
                           sctp_chunk_to_string(INIT), init_chunk->sctp_hdr.src_port, init_chunk->sctp_hdr.dst_port);
}

void
sctp_compute_mac(sctp_connection_t *sctp_conn, sctp_state_cookie_param_t *state_cookie)
{
...
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
    ctx = HMAC_CTX_new();
    HMAC_Init_ex(ctx, &state_cookie->creation_time, sizeof(state_cookie->creation_time), md, NULL);
...
void
sctp_prepare_cookie_ack_chunk(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b)
{
    vlib_main_t *vm = vlib_get_main();
    ...
    sctp_cookie_ack_chunk_t *cookie_ack_chunk = vlib_buffer_push_uninit(b, alloc_bytes);

    cookie_ack_chunk->sctp_hdr.checksum         = 0;
    cookie_ack_chunk->sctp_hdr.src_port         = sctp_conn->sub_conn[idx].connection.lcl_port;
    cookie_ack_chunk->sctp_hdr.dst_port         = sctp_conn->sub_conn[idx].connection.rmt_port;
    ...
    vnet_buffer(b)->sctp.connection_index = sctp_conn->sub_conn[idx].connection.c_index;
    vnet_buffer(b)->sctp.subconn_idx      = idx;
}

void
sctp_prepare_cookie_echo_chunk(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b, u8 reuse_buffer)
{
    vlib_main_t *vm = vlib_get_main();
    ...
    u16 chunk_len = alloc_bytes - sizeof(sctp_header_t);
    alloc_bytes += vnet_sctp_calculate_padding(alloc_bytes);
    sctp_cookie_echo_chunk_t *cookie_echo_chunk  = vlib_buffer_push_uninit(b, alloc_bytes);
    cookie_echo_chunk->sctp_hdr.checksum         = 0;
    cookie_echo_chunk->sctp_hdr.src_port         = sctp_conn->sub_conn[idx].connection.lcl_port;
    cookie_echo_chunk->sctp_hdr.dst_port         = sctp_conn->sub_conn[idx].connection.rmt_port;
    ...
    clib_memcpy(&(cookie_echo_chunk->cookie), &sctp_conn->cookie_param, sizeof(sctp_state_cookie_param_t));
...
sctp_send_cookie_echo(sctp_connection_t *sctp_conn)
{
    ...
    if (PREDICT_FALSE(sctp_conn->init_retransmit_err > SCTP_MAX_INIT_RETRANS)) {
        ...
        session_stream_connect_notify(&sctp_conn->sub_conn[SCTP_PRIMARY_PATH_IDX].connection, 1);
    ...
    sctp_timer_set(sctp_conn, idx, SCTP_TIMER_T1_INIT, sctp_conn->sub_conn[idx].RTO);
 ...
 */
void
sctp_prepare_operation_error(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b, u8 err_cause)
{
    vlib_main_t *vm = vlib_get_main();
    ...
    u16 alloc_bytes = sizeof(sctp_operation_error_t) + sizeof(sctp_err_cause_param_t);
    ...
    sctp_operation_error_t *err_chunk = vlib_buffer_push_uninit(b, alloc_bytes);
    ...
    err_chunk->err_causes[0].param_hdr.length = clib_host_to_net_u16(sizeof(err_chunk->err_causes[0].param_hdr.type) +
 ...
 */
void
sctp_prepare_abort_for_collision(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b, ip4_address_t *ip4_addr,
                                 ip6_address_t *ip6_addr)
{
    ...
    abort_chunk->sctp_hdr.src_port = sctp_conn->sub_conn[idx].connection.lcl_port;
    abort_chunk->sctp_hdr.dst_port = sctp_conn->sub_conn[idx].connection.rmt_port;
    /* As per RFC4960 Section 5.2.2: copy the INITIATE_TAG into the VERIFICATION_TAG of the ABORT chunk */
 ...
 */
void
sctp_prepare_initack_chunk_for_collision(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b,
                                         ip4_address_t *ip4_addr, ip6_address_t *ip6_addr)
{
    vlib_main_t *vm                   = vlib_get_main();
    sctp_ipv4_addr_param_t *ip4_param = 0;
    sctp_ipv6_addr_param_t *ip6_param = 0;
    ...
    /* The minimum size of the message is given by the sctp_init_ack_chunk_t */
    u16 alloc_bytes = sizeof(sctp_init_ack_chunk_t) + sizeof(sctp_state_cookie_param_t);
    ...
    sctp_init_ack_chunk_t *init_ack_chunk = vlib_buffer_push_uninit(b, alloc_bytes);
    ...
    state_cookie_param->param_hdr.type   = clib_host_to_net_u16(SCTP_STATE_COOKIE_TYPE);
    state_cookie_param->param_hdr.length = clib_host_to_net_u16(sizeof(sctp_state_cookie_param_t));
    state_cookie_param->creation_time    = clib_host_to_net_u64(sctp_time_now());
    state_cookie_param->cookie_lifespan  = clib_host_to_net_u32(SCTP_VALID_COOKIE_LIFE);
    ...
    if (PREDICT_TRUE(ip4_addr != NULL)) {
        sctp_ipv4_addr_param_t *ipv4_addr = (sctp_ipv4_addr_param_t *) init_ack_chunk + pointer_offset;

        ipv4_addr->param_hdr.type   = clib_host_to_net_u16(SCTP_IPV4_ADDRESS_TYPE);
        ipv4_addr->param_hdr.length = clib_host_to_net_u16(SCTP_IPV4_ADDRESS_TYPE_LENGTH);
        ipv4_addr->address.as_u32   = ip4_addr->as_u32;

        pointer_offset += SCTP_IPV4_ADDRESS_TYPE_LENGTH;
    }
    if (PREDICT_TRUE(ip6_addr != NULL)) {
        sctp_ipv6_addr_param_t *ipv6_addr = (sctp_ipv6_addr_param_t *) init_ack_chunk + pointer_offset;

        ipv6_addr->param_hdr.type    = clib_host_to_net_u16(SCTP_IPV6_ADDRESS_TYPE);
        ipv6_addr->param_hdr.length  = clib_host_to_net_u16(SCTP_IPV6_ADDRESS_TYPE_LENGTH);
    ...
    if (sctp_conn->sub_conn[idx].connection.is_ip4) {
        ...
        ip4_param->address.as_u32 = sctp_conn->sub_conn[idx].connection.lcl_ip.ip4.as_u32;

        pointer_offset += sizeof(sctp_ipv4_addr_param_t);
    } else {
        ip6_param                    = (sctp_ipv6_addr_param_t *) init_ack_chunk + pointer_offset;
        ip6_param->address.as_u64[0] = sctp_conn->sub_conn[idx].connection.lcl_ip.ip6.as_u64[0];
        ip6_param->address.as_u64[1] = sctp_conn->sub_conn[idx].connection.lcl_ip.ip6.as_u64[1];
    ...
    init_ack_chunk->sctp_hdr.src_port = sctp_conn->sub_conn[idx].connection.lcl_port;
    init_ack_chunk->sctp_hdr.dst_port = sctp_conn->sub_conn[idx].connection.rmt_port;
    ...
    init_ack_chunk->initial_tsn               = clib_host_to_net_u32(sctp_conn->local_initial_tsn);
    SCTP_CONN_TRACKING_DBG("init_ack_chunk->initial_tsn = %u", init_ack_chunk->initial_tsn);
    ...
    init_ack_chunk->a_rwnd                 = clib_host_to_net_u32(sctp_conn->sub_conn[idx].cwnd);
    init_ack_chunk->inboud_streams_count   = clib_host_to_net_u16(INBOUND_STREAMS_COUNT);
    init_ack_chunk->outbound_streams_count = clib_host_to_net_u16(OUTBOUND_STREAMS_COUNT);
 ...
 */
void
sctp_prepare_initack_chunk(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b, ip4_address_t *ip4_addr, u8 add_ip4,
                           ip6_address_t *ip6_addr, u8 add_ip6)
{
    vlib_main_t *vm                   = vlib_get_main();
    sctp_ipv4_addr_param_t *ip4_param = 0;
    sctp_ipv6_addr_param_t *ip6_param = 0;
    ...
    init_ack_chunk->initiate_tag = clib_host_to_net_u32(random_u32(&random_seed));
 ...
 */
void
sctp_prepare_shutdown_chunk(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b)
{
    ...
    sctp_shutdown_association_chunk_t *shutdown_chunk = vlib_buffer_push_uninit(b, alloc_bytes);

    shutdown_chunk->sctp_hdr.checksum = 0;
    /* No need of host_to_net conversion, already in net-byte order */
    shutdown_chunk->sctp_hdr.src_port         = sctp_conn->sub_conn[idx].connection.lcl_port;
    shutdown_chunk->sctp_hdr.dst_port         = sctp_conn->sub_conn[idx].connection.rmt_port;
...
sctp_send_shutdown(sctp_connection_t *sctp_conn)
{
    ...
    sctp_enqueue_to_output_now(vm, b, bi, sctp_conn->sub_conn[idx].connection.is_ip4);
}
 ...
 */
void
sctp_prepare_shutdown_ack_chunk(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b)
{
    u16 alloc_bytes = sizeof(sctp_shutdown_association_chunk_t);
    alloc_bytes += vnet_sctp_calculate_padding(alloc_bytes);
    ...
    sctp_shutdown_ack_chunk_t *shutdown_ack_chunk = vlib_buffer_push_uninit(b, alloc_bytes);

    shutdown_ack_chunk->sctp_hdr.checksum = 0;
    /* No need of host_to_net conversion, already in net-byte order */
    shutdown_ack_chunk->sctp_hdr.src_port         = sctp_conn->sub_conn[idx].connection.lcl_port;
    shutdown_ack_chunk->sctp_hdr.dst_port         = sctp_conn->sub_conn[idx].connection.rmt_port;
 ...
 */
void
sctp_send_shutdown_ack(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b)
{
    vlib_main_t *vm = vlib_get_main();
 ...
 */
void
sctp_prepare_sack_chunk(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b)
{
    vlib_main_t *vm = vlib_get_main();
 ...
 */
void
sctp_prepare_heartbeat_ack_chunk(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b)
{
    vlib_main_t *vm = vlib_get_main();
    ...
    hb_ack->hb_info.param_hdr.length  = clib_host_to_net_u16(sizeof(hb_ack->hb_info.hb_info));
 ...
 */
void
sctp_prepare_heartbeat_chunk(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b)
{
    ...
    hb_req->hb_info.param_hdr.length  = clib_host_to_net_u16(sizeof(hb_req->hb_info.hb_info));
...
sctp_send_heartbeat(sctp_connection_t *sctp_conn)
{
    ...
    for (i = 0; i < MAX_SCTP_CONNECTIONS; i++) {
        ...
        if (now > (sctp_conn->sub_conn[i].last_seen + SCTP_HB_INTERVAL)) {
            ...
            sctp_enqueue_to_output_now(vm, b, bi, sctp_conn->sub_conn[i].connection.is_ip4);
 ...
 */
void
sctp_prepare_shutdown_complete_chunk(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b)
{
    u16 alloc_bytes = sizeof(sctp_shutdown_association_chunk_t);
    alloc_bytes += vnet_sctp_calculate_padding(alloc_bytes);
    ...
    sctp_shutdown_complete_chunk_t *shutdown_complete = vlib_buffer_push_uninit(b, alloc_bytes);

    shutdown_complete->sctp_hdr.checksum = 0;
    /* No need of host_to_net conversion, already in net-byte order */
    shutdown_complete->sctp_hdr.src_port         = sctp_conn->sub_conn[idx].connection.lcl_port;
    shutdown_complete->sctp_hdr.dst_port         = sctp_conn->sub_conn[idx].connection.rmt_port;
...
void
sctp_send_shutdown_complete(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b0)
{
    vlib_main_t *vm = vlib_get_main();
...
sctp_send_init(sctp_connection_t *sctp_conn)
{
    ...
    sctp_enqueue_to_ip_lookup(vm, b, bi, sctp_conn->sub_conn[idx].c_is_ip4, sctp_conn->sub_conn[idx].c_fib_index);
 ...
 */
static void
sctp_push_hdr_i(sctp_connection_t *sctp_conn, vlib_buffer_t *b, sctp_state_t next_state)
{
    u16 data_len = b->current_length + b->total_length_not_including_first_buffer;
    ASSERT(!b->total_length_not_including_first_buffer || (b->flags & VLIB_BUFFER_NEXT_PRESENT));
    ...
    sctp_payload_data_chunk_t *data_chunk = vlib_buffer_push_uninit(b, bytes_to_add);

    u8 idx = sctp_data_subconn_select(sctp_conn);
    SCTP_DBG_OUTPUT("SCTP_CONN = %p, IDX = %u, S_INDEX = %u, C_INDEX = %u, sctp_conn->[...].LCL_PORT = %u, "
                    "sctp_conn->[...].RMT_PORT = %u",
                    sctp_conn, idx, sctp_conn->sub_conn[idx].connection.s_index,
                    sctp_conn->sub_conn[idx].connection.c_index, sctp_conn->sub_conn[idx].connection.lcl_port,
                    sctp_conn->sub_conn[idx].connection.rmt_port);
    data_chunk->sctp_hdr.checksum         = 0;
    data_chunk->sctp_hdr.src_port         = sctp_conn->sub_conn[idx].connection.lcl_port;
    data_chunk->sctp_hdr.dst_port         = sctp_conn->sub_conn[idx].connection.rmt_port;
    ...
    SCTP_ADV_DBG_OUTPUT("POINTER_WITH_DATA = %p, DATA_OFFSET = %u", b->data, b->current_data);
    ...
    if (sctp_conn->sub_conn[idx].partially_acked_bytes >= sctp_conn->sub_conn[idx].cwnd &&
        inflight >= sctp_conn->sub_conn[idx].cwnd) {
        sctp_conn->sub_conn[idx].cwnd += sctp_conn->sub_conn[idx].PMTU;
        sctp_conn->sub_conn[idx].partially_acked_bytes -= sctp_conn->sub_conn[idx].cwnd;
    }
...
sctp_push_header(transport_connection_t *trans_conn, vlib_buffer_t *b)
{
    ...
    SCTP_DBG_OUTPUT("TRANS_CONN = %p, SCTP_CONN = %p, "
                    ...
                    trans_conn, sctp_conn, trans_conn->s_index, trans_conn->c_index, trans_conn->lcl_port,
                    trans_conn->rmt_port);
...
u32
sctp_prepare_data_retransmit(sctp_connection_t *sctp_conn, u8 idx, u32 offset, u32 max_deq_bytes, vlib_buffer_t **b)
{
    sctp_main_t *tm = vnet_get_sctp_main();
    vlib_main_t *vm = vlib_get_main();
    int n_bytes     = 0;
     ...
     */
    available_bytes = session_tx_fifo_max_dequeue(&sctp_conn->sub_conn[idx].connection);
    ...
    *b   = vlib_get_buffer(vm, bi);
    ...
    if (PREDICT_TRUE(seg_size <= tm->bytes_per_buffer)) {
        n_bytes = stream_session_peek_bytes(&sctp_conn->sub_conn[idx].connection, data, offset, max_deq_bytes);
...
sctp_data_retransmit(sctp_connection_t *sctp_conn)
{
    ...
    n_bytes = sctp_prepare_data_retransmit(sctp_conn, idx, 0, sctp_conn->sub_conn[idx].cwnd, &b);
    if (n_bytes > 0)
...
always_inline u8
sctp_validate_output_state_machine(sctp_connection_t *sctp_conn, u8 chunk_type)
{
    u8 result = 0;
    ...
    case SCTP_STATE_ESTABLISHED:
        if (chunk_type != DATA && chunk_type != HEARTBEAT && chunk_type != HEARTBEAT_ACK && chunk_type != SACK &&
...
always_inline uword
sctp46_output_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            sctp_tx_trace_t *t0;
            sctp_header_t *th0 = 0;
            u32 error0 = SCTP_ERROR_PKTS_SENT, next0 = SCTP_OUTPUT_NEXT_IP_LOOKUP;
            ...
            if (is_ip4) {
                ip4_header_t *iph4 =
                    vlib_buffer_push_ip4(vm, b0, &sctp_conn->sub_conn[idx].connection.lcl_ip.ip4,
                                         &sctp_conn->sub_conn[idx].connection.rmt_ip.ip4, IP_PROTOCOL_SCTP, 1);
                ...
                ip6_header_t *iph6 =
                    vlib_buffer_push_ip6(vm, b0, &sctp_conn->sub_conn[idx].connection.lcl_ip.ip6,
                                         &sctp_conn->sub_conn[idx].connection.rmt_ip.ip6, IP_PROTOCOL_SCTP);
            ...
            if (chunk_type >= UNKNOWN) {
                clib_warning("Trying to send an unrecognized chunk... something is really bad.");
...
#if SCTP_DEBUG_STATE_MACHINE
            u8 is_valid = (sctp_conn->sub_conn[idx].connection.lcl_port == sctp_hdr->src_port ||
                           sctp_conn->sub_conn[idx].connection.lcl_port == sctp_hdr->dst_port) &&
                          (sctp_conn->sub_conn[idx].connection.rmt_port == sctp_hdr->dst_port ||
                           sctp_conn->sub_conn[idx].connection.rmt_port == sctp_hdr->src_port);
            ...
            if (!is_valid) {
                SCTP_DBG_STATE_MACHINE("BUFFER IS INCORRECT: conn_index = %u, "
                                       ...
                                       sctp_conn->sub_conn[idx].connection.c_index, packet_length, chunk_type,
                                       sctp_chunk_to_string(chunk_type), sctp_conn->sub_conn[idx].connection.lcl_port,
                                       sctp_hdr->src_port, sctp_conn->sub_conn[idx].connection.rmt_port,
                                       sctp_hdr->dst_port);
...
#endif
            SCTP_DBG_STATE_MACHINE("SESSION_INDEX = %u, CONN_INDEX = %u, CURR_CONN_STATE = %u (%s), "
                                   "CHUNK_TYPE = %s, "
                                   "SRC_PORT = %u, DST_PORT = %u",
                                   sctp_conn->sub_conn[idx].connection.s_index,
                                   sctp_conn->sub_conn[idx].connection.c_index, sctp_conn->state,
                                   sctp_state_to_string(sctp_conn->state), sctp_chunk_to_string(chunk_type),
                                   full_hdr->hdr.src_port, full_hdr->hdr.dst_port);
            ...
            if (sctp_validate_output_state_machine(sctp_conn, chunk_type) != 0) {
                SCTP_DBG_STATE_MACHINE("Sending the wrong chunk (%s) based on state-machine status (%s)",
                                       sctp_chunk_to_string(chunk_type), sctp_state_to_string(sctp_conn->state));
            ...
            if (!sctp_is_retransmitting(sctp_conn, idx)) {
                /* Measure RTT with this */
                if (chunk_type == DATA && sctp_conn->sub_conn[idx].RTO_pending == 0) {
            ...
            case SHUTDOWN: {
                ...
                sctp_timer_set(sctp_conn, idx, SCTP_TIMER_T2_SHUTDOWN, sctp_conn->sub_conn[idx].RTO);
            ...
            vnet_buffer(b0)->sw_if_index[VLIB_TX] = sctp_conn->sub_conn[idx].c_fib_index;

            b0->flags |= VNET_BUFFER_F_LOCALLY_ORIGINATED;

            SCTP_DBG_STATE_MACHINE("SESSION_INDEX = %u, CONNECTION_INDEX = %u, "
                                   "NEW_STATE = %s, "
                                   "CHUNK_SENT = %s",
                                   sctp_conn->sub_conn[idx].connection.s_index,
                                   sctp_conn->sub_conn[idx].connection.c_index, sctp_state_to_string(sctp_conn->state),
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ...
                clib_memcpy(&t0->sctp_connection, sctp_conn, sizeof(t0->sctp_connection));
            }
...
static uword
sctp4_output(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return sctp46_output_inline(vm, node, from_frame, 1 /* is_ip4 */);
}

static uword
sctp6_output(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
