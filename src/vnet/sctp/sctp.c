...
sctp_connection_bind(u32 session_index, transport_endpoint_t *tep)
{
    ...
    listener->sub_conn[SCTP_PRIMARY_PATH_IDX].subconn_idx         = SCTP_PRIMARY_PATH_IDX;
    listener->sub_conn[SCTP_PRIMARY_PATH_IDX].c_c_index           = listener - tm->listener_pool;
    ...
    if (ip_is_zero(&tep->ip, 1) && tep->sw_if_index != ENDPOINT_INVALID_INDEX) {
        if ((iface_ip = ip_interface_get_first_ip(tep->sw_if_index, tep->is_ip4)))
            ip_set(&tep->ip, iface_ip, tep->is_ip4);
    }
    ip_copy(&listener->sub_conn[SCTP_PRIMARY_PATH_IDX].connection.lcl_ip, &tep->ip, tep->is_ip4);

    u32 mtu = tep->is_ip4 ? vnet_sw_interface_get_mtu(vnet_get_main(), tep->sw_if_index, VNET_MTU_IP4) :
                            vnet_sw_interface_get_mtu(vnet_get_main(), tep->sw_if_index, VNET_MTU_IP6);
    listener->sub_conn[SCTP_PRIMARY_PATH_IDX].PMTU                 = mtu;
    listener->sub_conn[SCTP_PRIMARY_PATH_IDX].connection.is_ip4    = tep->is_ip4;
    listener->sub_conn[SCTP_PRIMARY_PATH_IDX].connection.proto     = TRANSPORT_PROTO_SCTP;
    listener->sub_conn[SCTP_PRIMARY_PATH_IDX].c_s_index            = session_index;
    listener->sub_conn[SCTP_PRIMARY_PATH_IDX].connection.fib_index = tep->fib_index;
...
static int
sctp_alloc_custom_local_endpoint(sctp_main_t *tm, ip46_address_t *lcl_addr, u16 *lcl_port, u8 is_ip4)
{
    int index, port;
    if (is_ip4) {
        ...
        clib_memcpy(&lcl_addr->ip6, &tm->ip6_src_addresses[index], sizeof(ip6_address_t));
    }
...
format_sctp_connection_id(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < MAX_SCTP_CONNECTIONS; i++) {
        if (sctp_conn->sub_conn[i].connection.is_ip4) {
            s = format(s, "%U[#%d][%s] %U:%d->%U:%d", s, sctp_conn->sub_conn[i].connection.thread_index, "T",
                       format_ip4_address, &sctp_conn->sub_conn[i].connection.lcl_ip.ip4,
                       clib_net_to_host_u16(sctp_conn->sub_conn[i].connection.lcl_port), format_ip4_address,
                       &sctp_conn->sub_conn[i].connection.rmt_ip.ip4,
                       clib_net_to_host_u16(sctp_conn->sub_conn[i].connection.rmt_port));
        } else {
            s = format(s, "%U[#%d][%s] %U:%d->%U:%d", s, sctp_conn->sub_conn[i].connection.thread_index, "T",
                       format_ip6_address, &sctp_conn->sub_conn[i].connection.lcl_ip.ip6,
                       clib_net_to_host_u16(sctp_conn->sub_conn[i].connection.lcl_port), format_ip6_address,
...
sctp_sub_connection_add(u8 thread_index)
{
    ...
    sctp_conn->sub_conn[subconn_idx].connection.c_index = sctp_conn->sub_conn[SCTP_PRIMARY_PATH_IDX].connection.c_index;
...
u8
sctp_sub_connection_add_ip4(vlib_main_t *vm, ip4_address_t *lcl_addr, ip4_address_t *rmt_addr)
{
    ...
    clib_memcpy(&sctp_conn->sub_conn[subconn_idx].connection.lcl_ip, &lcl_addr, sizeof(lcl_addr));

    clib_memcpy(&sctp_conn->sub_conn[subconn_idx].connection.rmt_ip, &rmt_addr, sizeof(rmt_addr));
...
u8
sctp_sub_connection_del_ip4(ip4_address_t *lcl_addr, ip4_address_t *rmt_addr)
{
    ...
    for (i = 0; i < MAX_SCTP_CONNECTIONS; i++) {
        ...
        sctp_sub_connection_t *sub_conn = &sctp_main->connections[thread_idx]->sub_conn[i];
        ip46_address_t *lcl_ip          = &sctp_main->connections[thread_idx]->sub_conn[i].connection.lcl_ip;
        ip46_address_t *rmt_ip          = &sctp_main->connections[thread_idx]->sub_conn[i].connection.rmt_ip;

        if (!sub_conn->connection.is_ip4)
            continue;
        if (lcl_ip->ip4.as_u32 == lcl_addr->as_u32 && rmt_ip->ip4.as_u32 == rmt_addr->as_u32) {
...
u8
sctp_sub_connection_add_ip6(vlib_main_t *vm, ip6_address_t *lcl_addr, ip6_address_t *rmt_addr)
{
    sctp_connection_t *sctp_conn = sctp_sub_connection_add(vm->thread_index);
...
u8
sctp_sub_connection_del_ip6(ip6_address_t *lcl_addr, ip6_address_t *rmt_addr)
{
    sctp_main_t *sctp_main = vnet_get_sctp_main();
    ...
    for (i = 0; i < MAX_SCTP_CONNECTIONS; i++) {
        ...
        if (!sub_conn->connection.is_ip4)
            continue;
        if ((lcl_ip->ip6.as_u64[0] == lcl_addr->as_u64[0] && lcl_ip->ip6.as_u64[1] == lcl_addr->as_u64[1]) &&
            (rmt_ip->ip6.as_u64[0] == rmt_addr->as_u64[0] && rmt_ip->ip6.as_u64[1] == rmt_addr->as_u64[1])) {
            sub_conn->state                        = SCTP_SUBCONN_STATE_DOWN;
            sctp_conn->forming_association_changed = 1;
            break;
        }
...
sctp_configure(sctp_user_configuration_t config)
{
    ...
    sctp_main->connections[thread_idx]->conn_config.never_delay_sack = config.never_delay_sack;
    sctp_main->connections[thread_idx]->conn_config.never_bundle     = config.never_bundle;
...
sctp_connection_new(u8 thread_index)
{
    ...
    sctp_conn->sub_conn[SCTP_PRIMARY_PATH_IDX].subconn_idx    = SCTP_PRIMARY_PATH_IDX;
    sctp_conn->sub_conn[SCTP_PRIMARY_PATH_IDX].c_c_index      = sctp_conn - sctp_main->connections[thread_index];
...
sctp_half_open_connection_new(u8 thread_index)
{
    ...
    memset(sctp_conn, 0, sizeof(*sctp_conn));
    sctp_conn->sub_conn[SCTP_PRIMARY_PATH_IDX].c_c_index   = sctp_conn - tm->half_open_connections;
    sctp_conn->sub_conn[SCTP_PRIMARY_PATH_IDX].subconn_idx = SCTP_PRIMARY_PATH_IDX;
    return sctp_conn;
}
...
sctp_connection_open(transport_endpoint_t *rmt)
{
     ...
     */
    if ((rmt->is_ip4 && vec_len(tm->ip4_src_addresses)) || (!rmt->is_ip4 && vec_len(tm->ip6_src_addresses)))
        rv = sctp_alloc_custom_local_endpoint(tm, &lcl_addr, &lcl_port, rmt->is_ip4);
    else
        rv = transport_alloc_local_endpoint(TRANSPORT_PROTO_SCTP, rmt, &lcl_addr, &lcl_port);
    ...
    u32 mtu   = rmt->is_ip4 ? vnet_sw_interface_get_mtu(vnet_get_main(), rmt->sw_if_index, VNET_MTU_IP4) :
                            vnet_sw_interface_get_mtu(vnet_get_main(), rmt->sw_if_index, VNET_MTU_IP6);
...
sctp_connection_cleanup(sctp_connection_t *sctp_conn)
{
    ...
    for (i = 0; i < MAX_SCTP_CONNECTIONS; i++)
        transport_endpoint_cleanup(TRANSPORT_PROTO_SCTP, &sctp_conn->sub_conn[i].connection.lcl_ip,
                                   sctp_conn->sub_conn[i].connection.lcl_port);

    int thread_index = sctp_conn->sub_conn[SCTP_PRIMARY_PATH_IDX].connection.thread_index;
...
sctp_check_outstanding_data_chunks(sctp_connection_t *sctp_conn)
{
    ...
    for (i = 0; i < MAX_SCTP_CONNECTIONS; i++) {
        ...
        if (sctp_conn->sub_conn[i].is_retransmitting == 1 || sctp_conn->sub_conn[i].enqueue_state != SCTP_ERROR_ENQUEUED) {
            SCTP_DBG_OUTPUT("Connection %u has still DATA to be enqueued inboud / outboud",
...
{
    SCTP_DBG("Closing connection %u...", sctp_conn->sub_conn[SCTP_PRIMARY_PATH_IDX].connection.c_index);
...
sctp_session_close(u32 conn_index, u32 thread_index)
{
    ...
    sctp_connection_t *sctp_conn = sctp_connection_get(conn_index, thread_index);
    if (sctp_conn != NULL)
...
sctp_snd_space(sctp_connection_t *sctp_conn)
{
    ...
    u32 available_wnd = clib_min(sctp_conn->peer_rwnd, sctp_conn->sub_conn[idx].cwnd);
    ...
    return available_wnd - flight_size - sizeof(sctp_payload_data_chunk_t) - sizeof(sctp_full_hdr_t);
}
...
sctp_expired_timers_cb(u32 conn_index, u32 timer_id)
{
    ...
    if (sctp_conn->sub_conn[conn_index].unacknowledged_hb > SCTP_PATH_MAX_RETRANS) {
        ...
        if (total_subs_down == MAX_SCTP_CONNECTIONS) {
            ...
            stream_session_delete_notify(&sctp_conn->sub_conn[SCTP_PRIMARY_PATH_IDX].connection);
...
sctp_initialize_timer_wheels(sctp_main_t *tm)
{
    ...
    foreach_vlib_main(({
        ...
        tw_timer_wheel_init_16t_2w_512sl(tw, sctp_expired_timers_dispatch, 100e-3 /* timer period 100ms */, ~0);
...
sctp_main_enable(vlib_main_t *vm)
{
    ...
    if (num_threads == 1) {
        ...
        thread                              = 1;
        preallocated_connections_per_thread = tm->preallocated_connections / (num_threads - 1);
    }
    for (; thread < num_threads; thread++) {
        if (preallocated_connections_per_thread)
            pool_init_fixed(tm->connections[thread], preallocated_connections_per_thread);
    }
    ...
    tm->tstamp_ticks_per_clock = vm->clib_time.seconds_per_clock / SCTP_TSTAMP_RESOLUTION;
    ...
    tm->bytes_per_buffer = vlib_buffer_free_list_buffer_size(vm, VLIB_BUFFER_DEFAULT_FREE_LIST_INDEX);
...
sctp_update_time(f64 now, u8 thread_index)
{
    ...
    tw_timer_expire_timers_16t_2w_512sl(&sctp_main.timer_wheels[thread_index], now);
...
sctp_init(vlib_main_t *vm)
{
    ...
    transport_register_protocol(TRANSPORT_PROTO_SCTP, &sctp_proto, FIB_PROTOCOL_IP4, sctp4_output_node.index);
    transport_register_protocol(TRANSPORT_PROTO_SCTP, &sctp_proto, FIB_PROTOCOL_IP6, sctp6_output_node.index);
