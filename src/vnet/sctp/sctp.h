...
void sctp_prepare_initack_chunk(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b, ip4_address_t *ip4_addr,
                                u8 add_ip4, ip6_address_t *ip6_addr, u8 add_ip6);
void sctp_prepare_initack_chunk_for_collision(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b,
                                              ip4_address_t *ip4_addr, ip6_address_t *ip6_addr);
void sctp_prepare_abort_for_collision(sctp_connection_t *sctp_conn, u8 idx, vlib_buffer_t *b, ip4_address_t *ip4_addr,
...
#define SCTP_DATA_IDLE_INTERVAL                                                                                        \
    15 * SHZ /* 15 seconds; the time-interval after which the connetion is considered IDLE */
...
sctp_buffer_hdr(vlib_buffer_t *b)
{
    ...
    return (sctp_header_t *) (b->data + b->current_data + vnet_buffer(b)->sctp.hdr_offset);
}
...
sctp_half_open_connection_del(sctp_connection_t *tc)
{
    ...
    clib_spinlock_lock_if_init(&sctp_main->half_open_lock);
    pool_put_index(sctp_main->half_open_connections, tc->sub_conn[SCTP_PRIMARY_PATH_IDX].c_c_index);
    if (CLIB_DEBUG)
...
{
    sctp_main.time_now[thread_index] = clib_cpu_time_now() * sctp_main.tstamp_ticks_per_clock;
    return sctp_main.time_now[thread_index];
}

always_inline void
sctp_timer_set(sctp_connection_t *tc, u8 conn_idx, u8 timer_id, u32 interval)
{
    ASSERT(tc->sub_conn[conn_idx].connection.thread_index == vlib_get_thread_index());
    ASSERT(tc->sub_conn[conn_idx].timers[timer_id] == SCTP_TIMER_HANDLE_INVALID);

    sctp_sub_connection_t *sub = &tc->sub_conn[conn_idx];
    sub->timers[timer_id] =
        tw_timer_start_16t_2w_512sl(&sctp_main.timer_wheels[sub->c_thread_index], sub->c_c_index, timer_id, interval);
}
...
sctp_timer_reset(sctp_connection_t *tc, u8 conn_idx, u8 timer_id)
{
    ...
    tw_timer_stop_16t_2w_512sl(&sctp_main.timer_wheels[sub->c_thread_index], sub->timers[timer_id]);
...
sctp_half_open_connection_cleanup(sctp_connection_t *tc)
{
    ...
    if (tc->sub_conn[SCTP_PRIMARY_PATH_IDX].c_thread_index != vlib_get_thread_index())
        return 1;
...
sctp_get_connection_from_transport(transport_connection_t *tconn)
{
    ...
    if (sub->subconn_idx > 0)
        return (sctp_connection_t *) sub - (sizeof(sctp_sub_connection_t) * (sub->subconn_idx - 1));
...
sctp_calculate_rto(sctp_connection_t *sctp_conn, u8 conn_idx)
{
    ...
    if (sctp_conn->sub_conn[conn_idx].RTO == SCTP_RTO_MIN &&
        sctp_conn->sub_conn[conn_idx].SRTT == 0)   // C2: First RTT calculation
    {
    ...
    } else   // C3: RTT already exists; let's recalculate
    {
        ...
        sctp_conn->sub_conn[conn_idx].SRTT =
            (1 - SCTP_RTO_ALPHA) * sctp_conn->sub_conn[conn_idx].SRTT + SCTP_RTO_ALPHA * R;
    }

    RTO = sctp_conn->sub_conn[conn_idx].SRTT + 4 * sctp_conn->sub_conn[conn_idx].RTTVAR;
...
always_inline void
sctp_timer_update(sctp_connection_t *tc, u8 conn_idx, u8 timer_id, u32 interval)
{
    ASSERT(tc->sub_conn[conn_idx].connection.thread_index == vlib_get_thread_index());
    sctp_sub_connection_t *sub = &tc->sub_conn[conn_idx];
...
{
    if (PREDICT_FALSE(pool_is_free_index(sctp_main.connections[thread_index], conn_index)))
        return 0;
...
sctp_sub_conn_id_via_ip6h(sctp_connection_t *sctp_conn, ip6_header_t *ip6h)
{
    ...
    for (i = 0; i < MAX_SCTP_CONNECTIONS; i++) {
        if (sctp_conn->sub_conn[i].connection.lcl_ip.ip6.as_u64[0] == ip6h->dst_address.as_u64[0] &&
            sctp_conn->sub_conn[i].connection.lcl_ip.ip6.as_u64[1] == ip6h->dst_address.as_u64[1] &&
            sctp_conn->sub_conn[i].connection.rmt_ip.ip6.as_u64[0] == ip6h->src_address.as_u64[0] &&
            sctp_conn->sub_conn[i].connection.rmt_ip.ip6.as_u64[1] == ip6h->src_address.as_u64[1])
            return i;
    }
    clib_warning("Did not find a sub-connection; defaulting to %u", SCTP_PRIMARY_PATH_IDX);
...
sctp_sub_conn_id_via_ip4h(sctp_connection_t *sctp_conn, ip4_header_t *ip4h)
{
    ...
    for (i = 0; i < MAX_SCTP_CONNECTIONS; i++) {
        if (sctp_conn->sub_conn[i].connection.lcl_ip.ip4.as_u32 == ip4h->dst_address.as_u32 &&
            sctp_conn->sub_conn[i].connection.rmt_ip.ip4.as_u32 == ip4h->src_address.as_u32)
            return i;
    }
    clib_warning("Did not find a sub-connection; defaulting to %u", SCTP_PRIMARY_PATH_IDX);
    return SCTP_PRIMARY_PATH_IDX;
 ...
 */
always_inline void *
vlib_buffer_push_sctp_net_order(vlib_buffer_t *b, u16 sp, u16 dp, u8 sctp_hdr_opts_len)
{
 ...
 */
always_inline void *
vlib_buffer_push_sctp(vlib_buffer_t *b, u16 sp_net, u16 dp_net, u8 sctp_hdr_opts_len)
{
    return vlib_buffer_push_sctp_net_order(b, sp_net, dp_net, sctp_hdr_opts_len);
}
...
update_smallest_pmtu_idx(sctp_connection_t *sctp_conn)
{
    ...
    for (i = 1; i < MAX_SCTP_CONNECTIONS; i++) {
        if (sctp_conn->sub_conn[i].state != SCTP_SUBCONN_STATE_DOWN) {
            if (sctp_conn->sub_conn[i].PMTU < sctp_conn->sub_conn[smallest_pmtu_index].PMTU)
...
sctp_init_cwnd(sctp_connection_t *sctp_conn)
{
    ...
    for (i = 0; i < MAX_SCTP_CONNECTIONS; i++) {
        ...
        sctp_conn->sub_conn[i].cwnd =
            clib_min(4 * sctp_conn->sub_conn[i].PMTU, clib_max(2 * sctp_conn->sub_conn[i].PMTU, 4380));
...
update_cwnd(sctp_connection_t *sctp_conn)
{
    ...
    for (i = 0; i < MAX_SCTP_CONNECTIONS; i++) {
        ...
        if (sctp_conn->sub_conn[i].last_data_ts > sctp_time_now() + SCTP_DATA_IDLE_INTERVAL) {
            sctp_conn->sub_conn[i].cwnd = clib_max(sctp_conn->sub_conn[i].cwnd / 2, 4 * sctp_conn->sub_conn[i].PMTU);
            continue;
        }
        ...
        if (sctp_conn->sub_conn[i].cwnd <= sctp_conn->sub_conn[i].ssthresh) {
            ...
            sctp_conn->sub_conn[i].cwnd = clib_min(sctp_conn->sub_conn[i].PMTU, 1);
        }

        /* Section 6.1; point (D) */
        if ((inflight + SCTP_RTO_BURST * sctp_conn->sub_conn[i].PMTU) < sctp_conn->sub_conn[i].cwnd)
            sctp_conn->sub_conn[i].cwnd = inflight + SCTP_RTO_BURST * sctp_conn->sub_conn[i].PMTU;
    }
}
