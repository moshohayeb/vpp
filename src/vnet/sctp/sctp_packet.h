...
always_inline void
vnet_sctp_set_cookie_preservative(sctp_cookie_preservative_param_t *c, u32 life_span_inc)
{
...
vnet_sctp_set_supported_address_types(sctp_supported_addr_types_param_t *s)
{
    ...
    s->param_hdr.length =
        4 /* base = type + length */ + MAX_SUPPORTED_ADDRESS_TYPES * 4; /* each address type is 4 bytes */
...
always_inline void
vnet_sctp_set_cumulative_tsn_ack(sctp_selective_ack_chunk_t *s, u32 cumulative_tsn_ack)
{
...
always_inline void
vnet_sctp_set_gap_ack_blocks_count(sctp_selective_ack_chunk_t *s, u16 gap_ack_blocks_count)
{
    vnet_sctp_set_chunk_type(&s->chunk_hdr, SACK);
    s->gap_ack_blocks_count = clib_host_to_net_u16(gap_ack_blocks_count);

    if (s->gap_ack_block_start == NULL)
        s->gap_ack_block_start = clib_mem_alloc(sizeof(u16) * gap_ack_blocks_count);
    if (s->gap_ack_block_end == NULL)
        s->gap_ack_block_end = clib_mem_alloc(sizeof(u16) * gap_ack_blocks_count);
}
...
always_inline void
vnet_sctp_set_duplicate_tsn_count(sctp_selective_ack_chunk_t *s, u16 duplicate_tsn_count)
{
    vnet_sctp_set_chunk_type(&s->chunk_hdr, SACK);
...
always_inline void
vnet_sctp_set_heartbeat_info(sctp_hb_info_param_t *h, u64 hb_info, u16 hb_info_length)
{
...
always_inline void
vnet_sctp_set_hb_request_info(sctp_hb_req_chunk_t *h, sctp_hb_info_param_t *hb_info)
{
...
always_inline void
vnet_sctp_set_hb_ack_info(sctp_hb_ack_chunk_t *h, sctp_hb_info_param_t *hb_info)
{
...
always_inline void
vnet_sctp_set_tsn_last_received_chunk(sctp_shutdown_association_chunk_t *s, u32 tsn_last_chunk)
{
