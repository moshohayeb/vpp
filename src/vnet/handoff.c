...
format_worker_handoff_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "worker-handoff: sw_if_index %d, next_worker %d, buffer 0x%x", t->sw_if_index, t->next_worker_index,
               t->buffer_index);
    return s;
}
...
static uword
worker_handoff_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        hash_key = hm->hash_fn((ethernet_header_t *) vlib_buffer_get_current(b[0]));
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b[0]->flags & VLIB_BUFFER_IS_TRACED))) {
            worker_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    n_enq = vlib_buffer_enqueue_to_thread(vm, hm->frame_queue_index, from, thread_indices, frame->n_vectors, 1);

    if (n_enq < frame->n_vectors)
        vlib_node_increment_counter(vm, node->node_index, WORKER_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
...
int
interface_handoff_enable_disable(vlib_main_t *vm, u32 sw_if_index, uword *bitmap, int enable_disable)
{
    handoff_main_t *hm = &handoff_main;
    ...
    if (enable_disable) {
        ...
        /* *INDENT-OFF* */
        clib_bitmap_foreach(i, bitmap, ({ vec_add1(d->workers, i); }));
        /* *INDENT-ON* */
    }
...
static clib_error_t *
set_interface_handoff_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    handoff_main_t *hm = &handoff_main;
    u32 sw_if_index    = ~0;
...
VLIB_CLI_COMMAND(set_interface_handoff_command, static) = {
    ...
    .short_help = "set interface handoff <interface-name> workers <workers-list> [symmetrical|asymmetrical]",
