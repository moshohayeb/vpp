...
static u8 *
format_ip6_cop_whitelist_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
    ip6_cop_whitelist_trace_t *t    = va_arg(*args, ip6_cop_whitelist_trace_t *);

    s = format(s, "IP6_COP_WHITELIST: sw_if_index %d, next index %d", t->sw_if_index, t->next_index);
    return s;
}
...
static uword
ip6_cop_whitelist_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            c0 = vnet_get_config_data(&ccm0->config_main, &vnet_buffer(b0)->cop.current_config_index, &next0,
                                      sizeof(c0[0]));

            lb_index0 = ip6_fib_table_fwding_lookup(im6, c0->fib_index, &ip0->src_address);
            ...
            c1 = vnet_get_config_data(&ccm1->config_main, &vnet_buffer(b1)->cop.current_config_index, &next1,
                                      sizeof(c1[0]));
            ...
            vlib_increment_combined_counter(vcm, thread_index, lb_index0, 1,
                                            vlib_buffer_length_in_chain(vm, b0) + sizeof(ethernet_header_t));

            vlib_increment_combined_counter(vcm, thread_index, lb_index1, 1,
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ip6_cop_whitelist_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->sw_if_index               = sw_if_index0;
                t->next_index                = next0;
            }

            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
                ip6_cop_whitelist_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
