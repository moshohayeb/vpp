...
cop_sw_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    ...
    for (address_family = VNET_COP_IP4; address_family < VNET_N_COPS; address_family++) {
        ...
        if (!(ccm->config_main.node_index_by_feature_index)) {
            ...
            case VNET_COP_IP4: {
                ...
                vnet_config_init(vm, &ccm->config_main, start_nodes, ARRAY_LEN(start_nodes), feature_nodes,
        ...
        vec_validate_init_empty(ccm->config_index_by_sw_if_index, sw_if_index, ~0);
        ...
        if (is_add)
            ci = vnet_config_add_feature(vm, &ccm->config_main, ci, default_next, data, sizeof(*data));
        else
            ci = vnet_config_del_feature(vm, &ccm->config_main, ci, default_next, data, sizeof(*data));
...
static clib_error_t *
cop_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    cop_main_t *cm     = &cop_main;
    u32 sw_if_index    = ~0;
    ...
    default:
        return clib_error_return(0, "cop_interface_enable_disable returned %d", rv);
    }
    return 0;
}
...
VLIB_CLI_COMMAND(cop_interface_command, static) = {
    .path       = "cop interface",
    .short_help = "cop interface <interface-name> [disable]",
...
cop_whitelist_enable_disable(cop_whitelist_enable_disable_args_t *a)
{
    ...
    for (address_family = VNET_COP_IP4; address_family < VNET_N_COPS; address_family++) {
        ...
        if (is_add)
            ci = vnet_config_add_feature(vm, &ccm->config_main, ci, next_to_add_del, data, sizeof(*data));
        else
            ci = vnet_config_del_feature(vm, &ccm->config_main, ci, next_to_add_del, data, sizeof(*data));
...
static clib_error_t *
cop_whitelist_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    cop_main_t *cm  = &cop_main;
    u32 sw_if_index = ~0;
    u8 ip4          = 0;
    ...
    default:
        return clib_error_return(0, "cop_whitelist_enable_disable returned %d", rv);
    }
...
VLIB_CLI_COMMAND(cop_whitelist_command, static) = {
    .path       = "cop whitelist",
    .short_help = "cop whitelist <interface-name> [ip4][ip6][default][fib-id <NN>][disable]",
