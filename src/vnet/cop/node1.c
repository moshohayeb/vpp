...
static u8 *
format_cop_input_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
    cop_input_trace_t *t            = va_arg(*args, cop_input_trace_t *);

    s = format(s, "COP_INPUT: sw_if_index %d, next index %d", t->sw_if_index, t->next_index);
    return s;
}
...
static uword
cop_input_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            vnet_buffer(b0)->cop.current_config_index = ccm0->config_index_by_sw_if_index[sw_if_index0];

            vnet_buffer(b1)->cop.current_config_index = ccm1->config_index_by_sw_if_index[sw_if_index1];

            vlib_buffer_advance(b0, advance0);
            vlib_buffer_advance(b1, advance1);

            vnet_get_config_data(&ccm0->config_main, &vnet_buffer(b0)->cop.current_config_index, &next0,
                                 0 /* bytes of config data */);

            vnet_get_config_data(&ccm1->config_main, &vnet_buffer(b1)->cop.current_config_index, &next1,
                                 0 /* bytes of config data */);

            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                cop_input_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->sw_if_index       = sw_if_index0;
                t->next_index        = next0;
            }

            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
                cop_input_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
    ...
    vlib_node_increment_counter(vm, cop_input_node.index, COP_INPUT_ERROR_PROCESSED, frame->n_vectors);
