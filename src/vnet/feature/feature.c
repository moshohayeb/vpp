...
vnet_feature_init(vlib_main_t *vm)
{
    ...
    while (areg) {
        ...
        if ((error = vnet_feature_arc_init(vm, vcm, areg->start_nodes, areg->n_start_nodes,
                                           fm->next_feature_by_arc[arc_index], &fm->feature_nodes[arc_index]))) {
...
int
vnet_feature_enable_disable_with_index(u8 arc_index, u32 feature_index, u32 sw_if_index, int enable_disable,
                                       void *feature_config, u32 n_feature_config_bytes)
{
    ...
    ci = (enable_disable ? vnet_config_add_feature : vnet_config_del_feature)(vlib_get_main(), &cm->config_main, ci,
                                                                              feature_index, feature_config,
                            ...
                            void *feature_config, u32 n_feature_config_bytes)
{
    ...
    return vnet_feature_enable_disable_with_index(arc_index, feature_index, sw_if_index, enable_disable, feature_config,
                                                  n_feature_config_bytes);
}
...
vnet_interface_features_show(vlib_main_t *vm, u32 sw_if_index, int verbose)
{
    ...
    while (areg) {
        ...
        ASSERT(current_config_index < vec_len(vcm->config_pool_index_by_user_index));
...
static clib_error_t *
set_interface_features_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U %v", unformat_vnet_sw_interface, vnm, &sw_if_index, &feature_name))
            ;
    ...
    if (!error)
        vnet_feature_enable_disable((const char *) arc_name, (const char *) feature_name, sw_if_index, enable, 0, 0);
