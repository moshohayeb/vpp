...
#define foreach_feature_api_msg _(FEATURE_ENABLE_DISABLE, feature_enable_disable)
...
vl_api_feature_enable_disable_t_handler(vl_api_feature_enable_disable_t *mp)
{
    ...
    if (reg == 0)
        ...
        if (!error)
            vnet_feature_enable_disable((const char *) arc_name, (const char *) feature_name, sw_if_index, mp->enable,
                                        0, 0);
        else {
