...
#define SSE2_QOS_POL_AVG_RATE_MAX (SSE2_QOS_POL_AVG_RATE_MANT_MAX << SSE2_QOS_POL_RATE_EXP_MAX)
...
#define SSE2_QOS_POL_PEAK_RATE_MAX (SSE2_QOS_POL_PEAK_RATE_MANT_MAX << SSE2_QOS_POL_RATE_EXP_MAX)
...
#define SSE2_QOS_POL_COMM_BKT_LIMIT_MAX                                                                                \
    ((u64) SSE2_QOS_POL_COMM_BKT_LIMIT_MANT_MAX << (u64) SSE2_QOS_POL_COMM_BKT_LIMIT_EXP_MAX)
...
#define SSE2_QOS_POL_EXT_BKT_LIMIT_MAX                                                                                 \
    ((u64) SSE2_QOS_POL_EXTD_BKT_LIMIT_MANT_MAX << (u64) SSE2_QOS_POL_EXTD_BKT_LIMIT_EXP_MAX)
...
static int
sse2_qos_pol_round(u64 numerator, u64 denominator, u64 *rounded_value, sse2_qos_round_type_en round_type)
{
    int rc = 0;
...
sse2_pol_validate_cfg_params(sse2_qos_pol_cfg_params_st *cfg)
{
    ...
    if ((cfg->rfc == SSE2_QOS_POLICER_TYPE_2R3C_RFC_2698) && (cfg->rb.kbps.eir_kbps < cfg->rb.kbps.cir_kbps)) {
        SSE2_QOS_DEBUG_ERROR("CIR (%u kbps) is greater than PIR (%u kbps)", cfg->rb.kbps.cir_kbps, cfg->rb.kbps.eir_kbps);
        SSE2_QOS_TR_ERR(SSE2_QOS_TP_ERR_39, cfg->rb.kbps.cir_kbps, cfg->rb.kbps.eir_kbps);
        return (EINVAL);
    }
    ...
    rc    = sse2_qos_pol_round(numer, denom, &rnd_value, (sse2_qos_round_type_en) cfg->rnd_type);
    if (rc != 0) {
    ...
    if (cir_hw > SSE2_QOS_POL_AVG_RATE_MAX) {
        SSE2_QOS_DEBUG_ERROR("hw cir (%u bytes/tick) is greater than the "
                             ...
                             cir_hw, SSE2_QOS_POL_AVG_RATE_MAX);
    ...
    if (eir_hw > SSE2_QOS_POL_PEAK_RATE_MAX) {
        ...
        SSE2_QOS_TR_ERR(SSE2_QOS_TP_ERR_85, eir_hw, SSE2_QOS_POL_PEAK_RATE_MAX);
        return (EINVAL);
    }
    /*
    ...
    if ((cfg->rb.kbps.eir_kbps == 0) && (cfg->rfc > SSE2_QOS_POLICER_TYPE_1R3C_RFC_2697)) {
    ...
    if (cfg->rb.kbps.eir_kbps && (cfg->rfc < SSE2_QOS_POLICER_TYPE_2R3C_RFC_2698)) {
        SSE2_QOS_DEBUG_ERROR("EIR: %u kbps for a 1-rate policer (rfc: %u)", cfg->rb.kbps.eir_kbps, cfg->rfc);
        SSE2_QOS_TR_ERR(SSE2_QOS_TP_ERR_23, cfg->rb.kbps.eir_kbps, cfg->rfc);
        return (EINVAL);
    }
...
static void
sse2_qos_convert_value_to_exp_mant_fmt(u64 value, u16 max_exp_value, u16 max_mant_value, sse2_qos_round_type_en type,
                                       u8 *exp, u32 *mant)
{
    u64 rnd_value;
    u64 temp_mant;
    ...
    while (temp_exp <= max_exp_value) {
        ...
        temp_exp++;
        rnd_value = 0;
        (void) sse2_qos_pol_round((u64) value, (u64)(1 << temp_exp), &rnd_value, type);
        temp_mant = rnd_value;
    }
...
static int
sse2_pol_convert_cfg_rates_to_hw(sse2_qos_pol_cfg_params_st *cfg, sse2_qos_pol_hw_params_st *hw)
{
    int rc = 0;
    ...
    rc    = sse2_qos_pol_round(numer, denom, &rnd_value, (sse2_qos_round_type_en) cfg->rnd_type);
    if (rc != 0) {
        SSE2_QOS_DEBUG_ERROR("Rounding error, rate: %d kbps, rounding_type: %d", cfg->rb.kbps.cir_kbps, cfg->rnd_type);
        // Error is traced
        return (rc);
    }
    cir_hw = (u32) rnd_value;
    ...
    rc    = sse2_qos_pol_round(numer, denom, &rnd_value, (sse2_qos_round_type_en) cfg->rnd_type);
    if (rc != 0) {
        SSE2_QOS_DEBUG_ERROR("Rounding error, rate: %d kbps, rounding_type: %d", eir_kbps, cfg->rnd_type);
        // Error is traced
        return (rc);
    }
    eir_hw = (u32) rnd_value;
    ...
    SSE2_QOS_DEBUG_INFO("cir_hw: %u bytes/tick, eir_hw: %u bytes/tick", cir_hw, eir_hw);
    ...
    if ((cir_hw == 0) && (eir_hw == 0)) {
        ...
        sse2_qos_convert_value_to_exp_mant_fmt(hi_rate, (u16) SSE2_QOS_POL_RATE_EXP_MAX,
                                               (u16) SSE2_QOS_POL_AVG_RATE_MANT_MAX,
                                               (sse2_qos_round_type_en) cfg->rnd_type, &exp, &hi_mant);
    }

    denom = (1ULL << exp);
    if (hi_rate == eir_hw) {
        hw->peak_rate_man = (u16) hi_mant;
        rc                = sse2_qos_pol_round((u64) cir_hw, denom, &rnd_value, (sse2_qos_round_type_en) cfg->rnd_type);
        hw->avg_rate_man  = (u16) rnd_value;
    } else {
        hw->avg_rate_man  = (u16) hi_mant;
        rc                = sse2_qos_pol_round((u64) eir_hw, denom, &rnd_value, (sse2_qos_round_type_en) cfg->rnd_type);
...
static void
sse2_pol_rnd_burst_byte_fmt(u64 cfg_burst, u16 max_exp_value, u16 max_mant_value, u32 max_bkt_value, u32 rate_hw,
                            u8 *exp, u32 *mant, u32 *bkt_value)
{
    ...
    if (cfg_burst > bkt_max) {
        SSE2_QOS_DEBUG_ERROR("burst 0x%llx bytes is greater than the max. "
                             ...
                             "max",
                             cfg_burst, bkt_max);
        SSE2_QOS_TR_INFO(SSE2_QOS_TP_INFO_38, (uint) cfg_burst, (uint) bkt_max);
        cfg_burst = bkt_max;
    }
    ...
    if (cfg_burst < SSE2_QOS_POL_MIN_BURST_BYTE) {
        ...
        SSE2_QOS_TR_INFO(SSE2_QOS_TP_INFO_39, (uint) cfg_burst, SSE2_QOS_POL_MIN_BURST_BYTE);
        cfg_burst = SSE2_QOS_POL_MIN_BURST_BYTE;
    }

    sse2_qos_convert_value_to_exp_mant_fmt(cfg_burst, max_exp_value, max_mant_value, SSE2_QOS_ROUND_TO_DOWN, exp, mant);
...
static int
sse2_pol_convert_cfg_burst_to_hw(sse2_qos_pol_cfg_params_st *cfg, sse2_qos_pol_hw_params_st *hw)
{
    u8 temp_exp;
    ...
    sse2_pol_rnd_burst_byte_fmt(cfg->rb.kbps.cb_bytes, (u16) SSE2_QOS_POL_COMM_BKT_LIMIT_EXP_MAX,
                                (u16) SSE2_QOS_POL_COMM_BKT_LIMIT_MANT_MAX, (u32) SSE2_QOS_POL_COMM_BKT_MAX, rate_hw,
                                &temp_exp, &temp_mant, &bkt_value);
    SSE2_QOS_DEBUG_INFO("Committed burst, burst_limit: 0x%llx mant : %u, "
                        "exp: %u, rnded: 0x%llx cb:%u bytes",
                        cfg->rb.kbps.cb_bytes, temp_mant, temp_exp, ((u64) temp_mant << (u64) temp_exp), bkt_value);
    ...
    if (cfg->rfc == SSE2_QOS_POLICER_TYPE_1R2C) {
        ...
        SSE2_QOS_TR_INFO(SSE2_QOS_TP_INFO_20, (uint) cfg->rb.kbps.eb_bytes, hw->extd_bkt_limit_man,
    ...
    sse2_pol_rnd_burst_byte_fmt(eb_bytes, (u16) SSE2_QOS_POL_EXTD_BKT_LIMIT_EXP_MAX,
                                (u16) SSE2_QOS_POL_EXTD_BKT_LIMIT_MANT_MAX, (u32) SSE2_QOS_POL_EXTD_BKT_MAX, rate_hw,
                                &temp_exp, &temp_mant, &bkt_value);

    SSE2_QOS_DEBUG_INFO("Excess burst, burst_limit: 0x%llx mant: %u, "
                        "exp: %u, rnded: 0x%llx eb:%u bytes",
                        cfg->rb.kbps.eb_bytes, temp_mant, temp_exp, ((u64) temp_mant << (u64) temp_exp), bkt_value);
 ...
 */
static int
sse2_pol_convert_cfg_to_hw_params(sse2_qos_pol_cfg_params_st *cfg, sse2_qos_pol_hw_params_st *hw)
{
    int rc = 0;
    ...
    if ((cfg->rfc == SSE2_QOS_POLICER_TYPE_1R2C) || (cfg->rfc == SSE2_QOS_POLICER_TYPE_2R3C_RFC_4115)) {
        hw->rfc = IPE_RFC_RFC4115;
    } else if (cfg->rfc == SSE2_QOS_POLICER_TYPE_1R3C_RFC_2697) {
    ...
    if (rc != 0) {
        SSE2_QOS_DEBUG_ERROR("Unable to convert config rates to hw. Error: %d", rc);
        // Error is traced
        return (rc);
    }
    ...
    if (rc != 0) {
        SSE2_QOS_DEBUG_ERROR("Unable to convert config burst to hw. Error: %d", rc);
        // Error is traced
        return (rc);
    }
...
sse2_qos_convert_pps_to_kbps(u32 rate_pps)
{
    ...
    numer = (u64)((u64) rate_pps * (u64) SSE2_QOS_POLICER_FIXED_PKT_SIZE * 8LL);
    (void) sse2_qos_pol_round(numer, 1000LL, &rnd_value, SSE2_QOS_ROUND_TO_CLOSEST);
...
sse2_qos_convert_burst_ms_to_bytes(u32 burst_ms, u32 rate_kbps)
{
    ...
    (void) sse2_qos_pol_round(numer, 8LL, &rnd_value, SSE2_QOS_ROUND_TO_CLOSEST);
 ...
 */
int
sse2_pol_compute_hw_params(sse2_qos_pol_cfg_params_st *cfg, sse2_qos_pol_hw_params_st *hw)
{
    int rc = 0;
    ...
    if (rc != 0) {
        SSE2_QOS_DEBUG_ERROR("Config parameter validation failed. Error: %d", rc);
        // Error is traced
        return (-1);
    }
...
// #define PRINT_X86_POLICE_PARAMS
#ifdef PRINT_X86_POLICE_PARAMS
    {
        ...
        // rates.
        effective_BPS = (((u64)(*cir_bytes_per_period * (u64) period)) >> *scale);
        ...
        printf("hz=%llu, cir_rate=%llu, limit=%u => "
               ...
               // input values
               (unsigned long long) hz, (unsigned long long) cir_rate, orig_current_limit,
               // computed values
               (u32)(period),                     // periods per second
               (u32)(1000.0 * 1000.0 / period),   // in usec
               *scale, *cir_bytes_per_period, *current_limit,
               // accuracy
               (unsigned long long) effective_BPS, (double) cir_rate / (double) effective_BPS);
    }
#endif
 ...
 */
int
x86_pol_compute_hw_params(sse2_qos_pol_cfg_params_st *cfg, policer_read_response_type_st *hw)
{
    ...
    if ((cfg->rfc == SSE2_QOS_POLICER_TYPE_1R2C) || (cfg->rfc == SSE2_QOS_POLICER_TYPE_1R3C_RFC_2697)) {
        ...
        if (compute_policer_params(hz, (u64) cfg->rb.kbps.cir_kbps * BYTES_PER_KBIT, 0, &hw->current_limit,
                                   &hw->extended_limit, &hw->cir_tokens_per_period, &hw->pir_tokens_per_period,
                                   &hw->scale)) {
            SSE2_QOS_DEBUG_ERROR("Policer parameter computation failed.");
            return (-1);
        }

    } else if ((cfg->rfc == SSE2_QOS_POLICER_TYPE_2R3C_RFC_2698) || (cfg->rfc == SSE2_QOS_POLICER_TYPE_2R3C_RFC_4115)) {
        // Two-rate policer
        ...
        if (compute_policer_params(hz, (u64) cfg->rb.kbps.cir_kbps * BYTES_PER_KBIT,
                                   (u64) cfg->rb.kbps.eir_kbps * BYTES_PER_KBIT, &hw->current_limit, &hw->extended_limit,
                                   &hw->cir_tokens_per_period, &hw->pir_tokens_per_period, &hw->scale)) {
            SSE2_QOS_DEBUG_ERROR("Policer parameter computation failed.");
            return (-1);
        }
 ...
 */
int
sse2_pol_logical_2_physical(sse2_qos_pol_cfg_params_st *cfg, policer_read_response_type_st *phys)
{
    int rc;
    ...
    case SSE2_QOS_RATE_PPS:
        kbps_cfg.rb.kbps.cir_kbps = sse2_qos_convert_pps_to_kbps(cfg->rb.pps.cir_pps);
        kbps_cfg.rb.kbps.eir_kbps = sse2_qos_convert_pps_to_kbps(cfg->rb.pps.eir_pps);
        kbps_cfg.rb.kbps.cb_bytes =
            sse2_qos_convert_burst_ms_to_bytes((u32) cfg->rb.pps.cb_ms, kbps_cfg.rb.kbps.cir_kbps);
        kbps_cfg.rb.kbps.eb_bytes =
            sse2_qos_convert_burst_ms_to_bytes((u32) cfg->rb.pps.eb_ms, kbps_cfg.rb.kbps.eir_kbps);
        break;
    default:
...
static void
sse2_qos_convert_pol_bucket_to_hw_fmt(policer_read_response_type_st *bkt, sse2_qos_pol_hw_params_st *hw_fmt)
{
 ...
 */
static int
sse2_pol_convert_hw_to_cfg_params(sse2_qos_pol_hw_params_st *hw, sse2_qos_pol_cfg_params_st *cfg)
{
    u64 temp_rate;
    ...
    temp_rate             = (((u64) hw->avg_rate_man << hw->rate_exp) * 8LL * SSE2_QOS_POL_TICKS_PER_SEC) / 1000;
    cfg->rb.kbps.cir_kbps = (u32) temp_rate;

    temp_rate             = (((u64) hw->peak_rate_man << hw->rate_exp) * 8LL * SSE2_QOS_POL_TICKS_PER_SEC) / 1000;
    ...
    SSE2_QOS_DEBUG_INFO("configured params, cir: %u kbps, eir: %u kbps, cb "
                        ...
                        cfg->rb.kbps.cir_kbps, cfg->rb.kbps.eir_kbps, cfg->rb.kbps.cb_bytes, cfg->rb.kbps.eb_bytes);
    SSE2_QOS_TR_INFO(SSE2_QOS_TP_INFO_22, cfg->rb.kbps.cir_kbps, cfg->rb.kbps.eir_kbps, (uint) cfg->rb.kbps.cb_bytes,
...
sse2_qos_convert_kbps_to_pps(u32 rate_kbps)
{
    ...
    (void) sse2_qos_pol_round(numer, denom, &rnd_value, SSE2_QOS_ROUND_TO_CLOSEST);
 ...
 */
int
sse2_pol_physical_2_logical(policer_read_response_type_st *phys, sse2_qos_pol_cfg_params_st *cfg)
{
    int rc;
    ...
    case SSE2_QOS_RATE_KBPS:
        ...
        break;
    case SSE2_QOS_RATE_PPS:
        cfg->rb.pps.cir_pps = sse2_qos_convert_kbps_to_pps(kbps_cfg.rb.kbps.cir_kbps);
        cfg->rb.pps.eir_pps = sse2_qos_convert_kbps_to_pps(kbps_cfg.rb.kbps.eir_kbps);
        cfg->rb.pps.cb_ms   = sse2_qos_convert_burst_bytes_to_ms(kbps_cfg.rb.kbps.cb_bytes, kbps_cfg.rb.kbps.cir_kbps);
        cfg->rb.pps.eb_ms   = sse2_qos_convert_burst_bytes_to_ms(kbps_cfg.rb.kbps.eb_bytes, kbps_cfg.rb.kbps.eir_kbps);
        break;
    default:
        SSE2_QOS_DEBUG_ERROR("Illegal rate type");
        return (-1);
