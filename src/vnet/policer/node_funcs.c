...
format_policer_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "VNET_POLICER: sw_if_index %d policer_index %d next %d", t->sw_if_index, t->policer_index,
               t->next_index);
    return s;
}
...
static inline uword
vnet_policer_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vnet_policer_index_t which)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (which == VNET_POLICER_INDEX_BY_EITHER) {
                pi0 = vnet_buffer(b0)->policer.index;
                pi0 = (pi0 != ~0) ? pi0 : pm->policer_index_by_sw_if_index[sw_if_index0];
                pi1 = vnet_buffer(b1)->policer.index;
                pi1 = (pi1 != ~0) ? pi1 : pm->policer_index_by_sw_if_index[sw_if_index1];
            }

            act0 = vnet_policer_police(vm, b0, pi0, time_in_policer_periods, POLICE_CONFORM /* no chaining */);

            act1 = vnet_policer_police(vm, b1, pi1, time_in_policer_periods, POLICE_CONFORM /* no chaining */);
...
uword
vnet_policer_by_sw_if_index(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return vnet_policer_inline(vm, node, frame, VNET_POLICER_INDEX_BY_SW_IF_INDEX);
}

uword
vnet_policer_by_opaque(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return vnet_policer_inline(vm, node, frame, VNET_POLICER_INDEX_BY_OPAQUE);
}

uword
vnet_policer_by_either(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
test_policer_add_del(u32 rx_sw_if_index, u8 *config_name, int is_add)
{
    ...
    if (is_add) {
        ...
        vnet_hw_interface_rx_redirect_to_node(pm->vnet_main, rxhi->hw_if_index, policer_by_sw_if_index_node.index);
        ...
        pm->policer_index_by_sw_if_index[rx_sw_if_index] = policer - pm->policers;
    } else {
        u32 pi;
        vnet_hw_interface_rx_redirect_to_node(pm->vnet_main, rxhi->hw_if_index, ~0 /* disable */);
...
static clib_error_t *
test_policer_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_policer_main_t *pm = &vnet_policer_main;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "intfc %U", unformat_vnet_sw_interface, pm->vnet_main, &rx_sw_if_index))
            rx_set = 1;
    ...
    default:
        error = clib_error_return(0, "WARNING: vnet_vnet_policer_add_del returned %d", rv);
        goto done;
    }
...
static inline uword
policer_classify_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, policer_classify_table_id_t tid)
{
    u32 n_left_from, *from, *to_next;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(table_index0 != ~0)) {
                ...
                if (e0) {
                    act0 = vnet_policer_police(vm, b0, e0->next_index, time_in_policer_periods, e0->opaque_index);
    ...
    vlib_node_increment_counter(vm, node->node_index, POLICER_CLASSIFY_ERROR_DROP, drop);

    return frame->n_vectors;
}

static uword
ip4_policer_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ip6_policer_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
l2_policer_classify(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
policer_classify_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2_policer_classify_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(),
