...
clib_error_t *
policer_add_del(vlib_main_t *vm, u8 *name, sse2_qos_pol_cfg_params_st *cfg, u32 *policer_index, u8 is_add)
{
...
{
    policer_read_response_type_st *i = va_arg(*va, policer_read_response_type_st *);
    ...
    s = format(s, "cur lim %u, cur bkt %u, ext lim %u, ext bkt %u\n", i->current_limit, i->current_bucket,
               i->extended_limit, i->extended_bucket);
...
format_policer_config(u8 *s, va_list *va)
{
    ...
    s = format(s, "conform action %U, exceed action %U, violate action %U\n", format_policer_action_type,
               &c->conform_action, format_policer_action_type, &c->exceed_action, format_policer_action_type,
               &c->violate_action);
    return s;
}
...
static clib_error_t *
configure_policer_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
show_policer_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    hash_foreach_pair(p, pm->policer_config_by_name, ({
                          ...
                          if (match_name == 0 || !strcmp((char *) name, (char *) match_name)) {
                              ...
                              vlib_cli_output(vm, "Name \"%s\" %U ", name, format_policer_config, config);
...
show_policer_pools_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "pool sizes: configs=%d templates=%d policers=%d", pool_elts(pm->configs),
                    pool_elts(pm->policer_templates), pool_elts(pm->policers));
    return 0;
}
/* *INDENT-OFF* */
