...
tcp_options_write(u8 *data, tcp_options_t *opts)
{
    ...
    if (tcp_opts_sack(opts)) {
        int i;
        u32 n_sack_blocks = clib_min(vec_len(opts->sacks), TCP_OPTS_MAX_SACK_BLOCKS);
...
tcp_make_established_options(tcp_connection_t *tc, tcp_options_t *opts)
{
    ...
    if (tcp_opts_sack_permitted(&tc->rcv_opts)) {
        if (vec_len(tc->snd_sacks)) {
            ...
            opts->n_sack_blocks = clib_min(vec_len(tc->snd_sacks), TCP_OPTS_MAX_SACK_BLOCKS);
...
always_inline int
tcp_make_options(tcp_connection_t *tc, tcp_options_t *opts, tcp_state_t state)
{
    switch (state) {
...
static int
tcp_alloc_tx_buffers(tcp_main_t *tm, u8 thread_index, u16 *n_bufs, u32 wanted)
{
    ...
    n_alloc = vlib_buffer_alloc(vm, &ctx->tx_buffers[*n_bufs], wanted - *n_bufs);
    *n_bufs += n_alloc;
...
tcp_make_syn(tcp_connection_t *tc, vlib_buffer_t *b)
{
    ...
    th = vlib_buffer_push_tcp(b, tc->c_lcl_port, tc->c_rmt_port, tc->iss, tc->rcv_nxt, tcp_hdr_opts_len, TCP_FLAG_SYN,
                              initial_wnd);
    vnet_buffer(b)->tcp.connection_index = tc->c_c_index;
    tcp_options_write((u8 *) (th + 1), &snd_opts);
}
...
tcp_make_synack(tcp_connection_t *tc, vlib_buffer_t *b)
{
    ...
    th = vlib_buffer_push_tcp(b, tc->c_lcl_port, tc->c_rmt_port, tc->iss, tc->rcv_nxt, tcp_hdr_opts_len,
...
always_inline void
tcp_enqueue_to_ip_lookup_i(vlib_main_t *vm, vlib_buffer_t *b, u32 bi, u8 is_ip4, u32 fib_index, u8 flush)
{
...
static void
tcp_enqueue_to_ip_lookup_now(vlib_main_t *vm, vlib_buffer_t *b, u32 bi, u8 is_ip4, u32 fib_index)
{
    tcp_enqueue_to_ip_lookup_i(vm, b, bi, is_ip4, fib_index, 1);
}

static void
tcp_enqueue_to_ip_lookup(vlib_main_t *vm, vlib_buffer_t *b, u32 bi, u8 is_ip4, u32 fib_index)
{
...
always_inline void
tcp_enqueue_to_output_i(vlib_main_t *vm, vlib_buffer_t *b, u32 bi, u8 is_ip4, u8 flush)
{
    tcp_main_t *tm   = vnet_get_tcp_main();
    u32 thread_index = vlib_get_thread_index();
    u32 *to_next, next_index;
...
static void
tcp_enqueue_to_output_now(vlib_main_t *vm, vlib_buffer_t *b, u32 bi, u8 is_ip4)
{
    tcp_enqueue_to_output_i(vm, b, bi, is_ip4, 1);
}

static int
tcp_make_reset_in_place(vlib_main_t *vm, vlib_buffer_t *b0, tcp_state_t state, u8 thread_index, u8 is_ip4)
{
    ip4_header_t *ih4;
    ip6_header_t *ih6;
    tcp_header_t *th0;
    ...
    th0 = vlib_buffer_push_tcp_net_order(b0, dst_port, src_port, seq, ack, sizeof(tcp_header_t), flags, 0);

    if (is_ip4) {
        ih4           = vlib_buffer_push_ip4(vm, b0, &dst_ip40, &src_ip40, IP_PROTOCOL_TCP, 1);
        th0->checksum = ip4_tcp_udp_compute_checksum(vm, b0, ih4);
    } else {
        int bogus     = ~0;
        ih6           = vlib_buffer_push_ip6(vm, b0, &dst_ip60, &src_ip60, IP_PROTOCOL_TCP);
...
tcp_send_reset_w_pkt(tcp_connection_t *tc, vlib_buffer_t *pkt, u8 is_ip4)
{
    ...
    if (is_ip4) {
        ...
        ih4          = vlib_buffer_push_ip4(vm, b, &pkt_ih4->dst_address, &pkt_ih4->src_address, IP_PROTOCOL_TCP, 1);
        th->checksum = ip4_tcp_udp_compute_checksum(vm, b, ih4);
    } else {
        int bogus = ~0;
        ASSERT((pkt_ih6->ip_version_traffic_class_and_flow_label & 0xF0) == 0x60);
        ih6          = vlib_buffer_push_ip6(vm, b, &pkt_ih6->dst_address, &pkt_ih6->src_address, IP_PROTOCOL_TCP);
...
tcp_send_reset(tcp_connection_t *tc)
{
    ...
    flags            = TCP_FLAG_RST;
    th = vlib_buffer_push_tcp(b, tc->c_lcl_port, tc->c_rmt_port, tc->snd_nxt, tc->rcv_nxt, tcp_hdr_opts_len, flags,
                              advertise_wnd);
    ...
    if (tc->c_is_ip4) {
        ip4_header_t *ih4;
        ih4          = vlib_buffer_push_ip4(vm, b, &tc->c_lcl_ip.ip4, &tc->c_rmt_ip.ip4, IP_PROTOCOL_TCP, 0);
        th->checksum = ip4_tcp_udp_compute_checksum(vm, b, ih4);
    } else {
        int bogus = ~0;
        ip6_header_t *ih6;
        ih6          = vlib_buffer_push_ip6(vm, b, &tc->c_lcl_ip.ip6, &tc->c_rmt_ip.ip6, IP_PROTOCOL_TCP);
        th->checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b, ih6, &bogus);
        ASSERT(!bogus);
    }
...
tcp_push_ip_hdr(tcp_main_t *tm, tcp_connection_t *tc, vlib_buffer_t *b)
{
    ...
    if (tc->c_is_ip4) {
        ip4_header_t *ih;
        ih           = vlib_buffer_push_ip4(vm, b, &tc->c_lcl_ip4, &tc->c_rmt_ip4, IP_PROTOCOL_TCP, 1);
        th->checksum = ip4_tcp_udp_compute_checksum(vm, b, ih);
    } else {
        ip6_header_t *ih;
        int bogus = ~0;

        ih           = vlib_buffer_push_ip6(vm, b, &tc->c_lcl_ip6, &tc->c_rmt_ip6, IP_PROTOCOL_TCP);
...
tcp_flush_frame_to_output(vlib_main_t *vm, u8 thread_index, u8 is_ip4)
{
    if (tcp_main.wrk_ctx[thread_index].tx_frames[!is_ip4]) {
        u32 next_index;
        next_index = is_ip4 ? tcp4_output_node.index : tcp6_output_node.index;
        vlib_put_frame_to_node(vm, next_index, tcp_main.wrk_ctx[thread_index].tx_frames[!is_ip4]);
...
tcp_flush_frame_to_ip_lookup(vlib_main_t *vm, u8 thread_index, u8 is_ip4)
{
    if (tcp_main.wrk_ctx[thread_index].ip_lookup_tx_frames[!is_ip4]) {
        u32 next_index;
        next_index = is_ip4 ? ip4_lookup_node.index : ip6_lookup_node.index;
        vlib_put_frame_to_node(vm, next_index, tcp_main.wrk_ctx[thread_index].ip_lookup_tx_frames[!is_ip4]);
 ...
 */
always_inline void
tcp_push_hdr_i(tcp_connection_t *tc, vlib_buffer_t *b, tcp_state_t next_state, u8 compute_opts, u8 maybe_burst)
{
    ...
    if (maybe_burst) {
        clib_memcpy((u8 *) (th + 1), tm->wrk_ctx[tc->c_thread_index].cached_opts, tc->snd_opts_len);
    } else {
...
tcp_push_header(tcp_connection_t *tc, vlib_buffer_t *b)
{
    ...
    ASSERT(seq_leq(tc->snd_una_max, tc->snd_una + tc->snd_wnd + tcp_fastrecovery_sent_1_smss(tc) * tc->snd_mss));
 ...
 */
static u32
tcp_prepare_retransmit_segment(tcp_connection_t *tc, u32 offset, u32 max_deq_bytes, vlib_buffer_t **b)
{
    tcp_main_t *tm  = vnet_get_tcp_main();
    vlib_main_t *vm = vlib_get_main();
    int n_bytes     = 0;
    ...
    if (PREDICT_TRUE(seg_size <= tm->bytes_per_buffer)) {
        ...
        *b      = vlib_get_buffer(vm, bi);
        ...
        n_bytes = stream_session_peek_bytes(&tc->connection, data, offset, max_deq_bytes);
    ...
    /* Split mss into multiple buffers */
        ...
        if (n_bufs_per_seg > available_bufs) {
            tcp_alloc_tx_buffers(tm, thread_index, &available_bufs, VLIB_FRAME_SIZE);
        ...
        *b      = vlib_get_buffer(vm, bi);
        data    = tcp_init_buffer(vm, *b);
        n_bytes = stream_session_peek_bytes(&tc->connection, data, offset, tm->bytes_per_buffer - MAX_HDRS_LEN);
        b[0]->current_length = n_bytes;
        ...
        for (i = 1; i < n_bufs_per_seg; i++) {
            ...
            n_peeked              = stream_session_peek_bytes(&tc->connection, data, offset + n_bytes, len_to_deq);
    ...
    ASSERT(n_bytes > 0);
    ASSERT(((*b)->current_data + (*b)->current_length) <= tm->bytes_per_buffer);
...
tcp_timer_retransmit_handler_i(u32 index, u8 is_syn)
{
    ...
    if (tc->state >= TCP_STATE_ESTABLISHED) {
        ...
        if ((tc->rto_boff == 0 && tc->snd_una == tc->snd_una_max) ||
            (tc->rto_boff > 0 && seq_geq(tc->snd_una, tc->snd_congestion) && !tcp_flight_size(tc))) {
...
tcp_timer_persist_handler(u32 index)
{
    ...
    if (tc->state == TCP_STATE_CLOSED || tc->state > TCP_STATE_ESTABLISHED || tc->snd_wnd > tc->snd_mss ||
        tcp_in_recovery(tc))
        return;
    ...
    n_bytes           = stream_session_peek_bytes(&tc->connection, data, offset, max_snd_bytes);
    b->current_length = n_bytes;
    ASSERT(n_bytes != 0 &&
           (tcp_timer_is_active(tc, TCP_TIMER_RETRANSMIT) || tc->snd_nxt == tc->snd_una_max || tc->rto_boff > 1));
...
tcp_fast_retransmit_sack(tcp_connection_t *tc)
{
    ...
    while (hole && snd_space > 0 && n_segs++ < VLIB_FRAME_SIZE) {
        hole = scoreboard_next_rxt_hole(sb, hole, tcp_fastrecovery_sent_1_smss(tc), &can_rescue, &snd_limited);
        if (!hole) {
            if (!can_rescue || !(seq_lt(sb->rescue_rxt, tc->snd_una) || seq_gt(sb->rescue_rxt, tc->snd_congestion)))
                break;
...
static void
tcp_output_handle_link_local(tcp_connection_t *tc0, vlib_buffer_t *b0, u16 *next0, u32 *error0)
{
    ip_adjacency_t *adj;
    adj_index_t ai;
...
static void
tcp46_output_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, u32 *to_next, u32 n_bufs)
{
...
always_inline void
tcp_output_push_ip(vlib_main_t *vm, vlib_buffer_t *b0, tcp_connection_t *tc0, u8 is_ip4)
{
    tcp_header_t *th0 = 0;
    ...
    if (is_ip4) {
        vlib_buffer_push_ip4(vm, b0, &tc0->c_lcl_ip4, &tc0->c_rmt_ip4, IP_PROTOCOL_TCP, 1);
        b0->flags |= VNET_BUFFER_F_OFFLOAD_TCP_CKSUM;
        vnet_buffer(b0)->l4_hdr_offset = (u8 *) th0 - b0->data;
        th0->checksum                  = 0;
    } else {
        ip6_header_t *ih0;
        ih0 = vlib_buffer_push_ip6(vm, b0, &tc0->c_lcl_ip6, &tc0->c_rmt_ip6, IP_PROTOCOL_TCP);
        b0->flags |= VNET_BUFFER_F_OFFLOAD_TCP_CKSUM;
...
always_inline void
tcp_output_handle_packet(tcp_connection_t *tc0, vlib_buffer_t *b0, u32 *error0, u16 *next0, u8 is_ip4)
{
...
always_inline uword
tcp46_output_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
...
static uword
tcp4_output(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_output_inline(vm, node, from_frame, 1 /* is_ip4 */);
}

static uword
tcp6_output(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword
tcp46_send_reset_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u8 is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (tcp_make_reset_in_place(vm, b0, vnet_buffer(b0)->tcp.flags, my_thread_index, is_ip4)) {
...
static uword
tcp4_send_reset(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_send_reset_inline(vm, node, from_frame, 1);
}

static uword
tcp6_send_reset(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
