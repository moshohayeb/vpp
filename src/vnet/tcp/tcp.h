...
sack_scoreboard_hole_t *scoreboard_next_rxt_hole(sack_scoreboard_t *sb, sack_scoreboard_hole_t *start,
                                                 u8 have_sent_1_smss, u8 *can_rescue, u8 *snd_limited);
...
{
    if (PREDICT_FALSE(pool_is_free_index(tcp_main.connections[thread_index], conn_index)))
        return 0;
...
tcp_flight_size(const tcp_connection_t *tc)
{
    ...
    flight_size = (int) (tc->snd_una_max - tc->snd_una) - tcp_bytes_out(tc) + tc->snd_rxt_bytes;

    if (flight_size < 0) {
        if (0)
            clib_warning("Negative: %u %u %u dupacks %u sacked bytes %u flags %d", tc->snd_una_max - tc->snd_una,
                         tcp_bytes_out(tc), tc->snd_rxt_bytes, tc->rcv_dupacks, tc->sack_sb.sacked_bytes,
                         tc->rcv_opts.flags);
        return 0;
    }
...
{
    tcp_main.wrk_ctx[thread_index].time_now = clib_cpu_time_now() * tcp_main.tstamp_ticks_per_clock;
...
tcp_timer_set(tcp_connection_t *tc, u8 timer_id, u32 interval)
{
    ...
    tc->timers[timer_id] = tw_timer_start_16t_2w_512sl(&tcp_main.wrk_ctx[tc->c_thread_index].timer_wheel, tc->c_c_index,
                                                       timer_id, interval);
}
...
tcp_timer_reset(tcp_connection_t *tc, u8 timer_id)
{
    ...
    tw_timer_stop_16t_2w_512sl(&tcp_main.wrk_ctx[tc->c_thread_index].timer_wheel, tc->timers[timer_id]);
...
tcp_timer_update(tcp_connection_t *tc, u8 timer_id, u32 interval)
{
    ASSERT(tc->c_thread_index == vlib_get_thread_index());
    if (tc->timers[timer_id] != TCP_TIMER_HANDLE_INVALID)
        tw_timer_update_16t_2w_512sl(&tcp_main.wrk_ctx[tc->c_thread_index].timer_wheel, tc->timers[timer_id], interval);
    else
        tc->timers[timer_id] = tw_timer_start_16t_2w_512sl(&tcp_main.wrk_ctx[tc->c_thread_index].timer_wheel,
...
tcp_retransmit_timer_set(tcp_connection_t *tc)
{
    ...
    tcp_timer_set(tc, TCP_TIMER_RETRANSMIT, clib_max(tc->rto * TCP_TO_TIMER_TICK, 1));
}
...
{
    tcp_timer_update(tc, TCP_TIMER_RETRANSMIT, clib_max(tc->rto * TCP_TO_TIMER_TICK, 1));
}
...
tcp_persist_timer_set(tcp_connection_t *tc)
{
    ...
    tcp_timer_set(tc, TCP_TIMER_PERSIST, clib_max(tc->rto * TCP_TO_TIMER_TICK, TCP_TIMER_PERSIST_MIN));
}

always_inline void
tcp_persist_timer_update(tcp_connection_t *tc)
{
    tcp_timer_update(tc, TCP_TIMER_PERSIST, clib_max(tc->rto * TCP_TO_TIMER_TICK, TCP_TIMER_PERSIST_MIN));
}
...
#define tcp_validate_txf_size(_tc, _a)                                                                                 \
    ASSERT(_tc->state != TCP_STATE_ESTABLISHED || session_tx_fifo_max_dequeue(&_tc->connection) >= _a)
 ...
 */
always_inline void *
vlib_buffer_push_tcp_net_order(vlib_buffer_t *b, u16 sp, u16 dp, u32 seq, u32 ack, u8 tcp_hdr_opts_len, u8 flags, u16 wnd)
{
    tcp_header_t *th;
 ...
 */
always_inline void *
vlib_buffer_push_tcp(vlib_buffer_t *b, u16 sp_net, u16 dp_net, u32 seq, u32 ack, u8 tcp_hdr_opts_len, u8 flags, u16 wnd)
{
    return vlib_buffer_push_tcp_net_order(b, sp_net, dp_net, clib_host_to_net_u32(seq), clib_host_to_net_u32(ack),
                                          tcp_hdr_opts_len, flags, clib_host_to_net_u16(wnd));
}
