...
#define TCP_EVT_INIT_HANDLER(_tc, _is_l, ...)                                                                            \
    {                                                                                                                    \
        ...
        if (_tc->c_is_ip4) {                                                                                             \
            _tc->c_elog_track.name = (char *) format(0, _fmt, _tc->c_thread_index, _tc->c_lcl_ip.ip4.as_u8[3],           \
                                                     clib_net_to_host_u16(_tc->c_lcl_port), _tc->c_rmt_ip.ip4.as_u8[3],  \
                                                     clib_net_to_host_u16(_tc->c_rmt_port), 0);                          \
        } else                                                                                                           \
            _tc->c_elog_track.name = (char *) format(0, _fmt, _tc->c_thread_index, _tc->c_lcl_ip.ip6.as_u8[15],          \
                                                     clib_net_to_host_u16(_tc->c_lcl_port), _tc->c_rmt_ip.ip6.as_u8[15], \
                                                     clib_net_to_host_u16(_tc->c_rmt_port), 0);                          \
...
#define TCP_EVT_TIMER_POP_HANDLER(_tc_index, _timer_id, ...)                                                           \
    {                                                                                                                  \
        ...
        if (_timer_id == TCP_TIMER_RETRANSMIT_SYN || _timer_id == TCP_TIMER_ESTABLISH) {                               \
    ...
    {                                                                                                                  \
        ...
        if (PREDICT_FALSE(tcp_main.buffer_fail_fraction != 0.0)) {                                                     \
            if (PREDICT_TRUE(buffer_fail_counters[thread_index] > 0)) {                                                \
                if ((1.0 / (f32)(buffer_fail_counters[thread_index])) < tcp_main.buffer_fail_fraction) {               \
