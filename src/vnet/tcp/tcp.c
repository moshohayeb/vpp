...
tcp_connection_cleanup(tcp_connection_t *tc)
{
    ...
    transport_endpoint_cleanup(TRANSPORT_PROTO_TCP, &tc->c_lcl_ip, tc->c_lcl_port);
...
tcp_connection_close(tcp_connection_t *tc)
{
    ...
    if (!tcp_timer_is_active(tc, TCP_TIMER_WAITCLOSE) && tc->state == TCP_STATE_CLOSED)
...
void
tcp_cc_algo_register(tcp_cc_algorithm_type_e type, const tcp_cc_algorithm_t *vft)
{
    tcp_main_t *tm = vnet_get_tcp_main();
...
static int
tcp_alloc_custom_local_endpoint(tcp_main_t *tm, ip46_address_t *lcl_addr, u16 *lcl_port, u8 is_ip4)
{
    int index, port;
    if (is_ip4) {
...
tcp_connection_open(transport_endpoint_t *rmt)
{
    ...
    if ((rmt->is_ip4 && vec_len(tm->ip4_src_addresses)) || (!rmt->is_ip4 && vec_len(tm->ip6_src_addresses)))
        rv = tcp_alloc_custom_local_endpoint(tm, &lcl_addr, &lcl_port, rmt->is_ip4);
    else
...
format_tcp_vars(u8 *s, va_list *args)
{
    ...
    s = format(s, " snd_una %u snd_nxt %u snd_una_max %u", tc->snd_una - tc->iss, tc->snd_nxt - tc->iss,
    ...
    s = format(s, " flight size %u out space %u cc space %u rcv_wnd_av %u\n", tcp_flight_size(tc),
               tcp_available_output_snd_space(tc), tcp_available_cc_snd_space(tc), tcp_rcv_wnd_available(tc));
    s = format(s, " cong %U ", format_tcp_congestion_status, tc);
    s = format(s, "cwnd %u ssthresh %u rtx_bytes %u bytes_acked %u\n", tc->cwnd, tc->ssthresh, tc->snd_rxt_bytes,
               tc->bytes_acked);
    s = format(s, " prev_ssthresh %u snd_congestion %u dupack %u", tc->prev_ssthresh, tc->snd_congestion - tc->iss,
               tc->rcv_dupacks);
    s = format(s, " limited_transmit %u\n", tc->limited_transmit - tc->iss);
    s = format(s, " tsecr %u tsecr_last_ack %u\n", tc->rcv_opts.tsecr, tc->tsecr_last_ack);
    s = format(s, " rto %u rto_boff %u srtt %u rttvar %u rtt_ts %u ", tc->rto, tc->rto_boff, tc->srtt, tc->rttvar,
               tc->rtt_ts);
    s = format(s, "rtt_seq %u\n", tc->rtt_seq);
    s = format(s, " tsval_recent %u tsval_recent_age %u\n", tc->tsval_recent, tcp_time_now() - tc->tsval_recent_age);
...
format_tcp_connection_id(u8 *s, va_list *args)
{
    ...
    if (!tc)
        return s;
    if (tc->c_is_ip4) {
        s = format(s, "[#%d][%s] %U:%d->%U:%d", tc->c_thread_index, "T", format_ip4_address, &tc->c_lcl_ip4,
                   clib_net_to_host_u16(tc->c_lcl_port), format_ip4_address, &tc->c_rmt_ip4,
                   clib_net_to_host_u16(tc->c_rmt_port));
    } else {
        s = format(s, "[#%d][%s] %U:%d->%U:%d", tc->c_thread_index, "T", format_ip6_address, &tc->c_lcl_ip6,
                   clib_net_to_host_u16(tc->c_lcl_port), format_ip6_address, &tc->c_rmt_ip6,
                   clib_net_to_host_u16(tc->c_rmt_port));
    }
...
tcp_snd_space_inline(tcp_connection_t *tc)
{
    ...
    if (PREDICT_TRUE(tcp_in_cong_recovery(tc) == 0)) {
        ...
        if (PREDICT_FALSE(tc->rcv_dupacks != 0 && (tcp_opts_sack_permitted(tc) && tc->sack_sb.last_sacked_bytes == 0))) {
    ...
    if (tcp_in_recovery(tc)) {
        ...
        snd_space   = tcp_available_snd_wnd(tc) - tc->snd_rxt_bytes - (tc->snd_una_max - tc->snd_congestion);
...
{
    tcp_connection_t *tc = (tcp_connection_t *) trans_conn;
    return clib_min(tcp_snd_space_inline(tc), tc->snd_wnd - (tc->snd_nxt - tc->snd_una));
}
...
tcp_update_time(f64 now, u8 thread_index)
{
    ...
    tw_timer_expire_timers_16t_2w_512sl(&tcp_main.wrk_ctx[thread_index].timer_wheel, now);
...
static timer_expiration_handler *timer_expiration_handlers[TCP_N_TIMERS] =
    {tcp_timer_retransmit_handler, tcp_timer_delack_handler,         tcp_timer_persist_handler,  tcp_timer_keep_handler,
     tcp_timer_waitclose_handler,  tcp_timer_retransmit_syn_handler, tcp_timer_establish_handler};
/* *INDENT-ON* */
...
tcp_initialize_timer_wheels(tcp_main_t *tm)
{
    ...
    foreach_vlib_main(({
        ...
        tw_timer_wheel_init_16t_2w_512sl(tw, tcp_expired_timers_dispatch, 100e-3 /* timer period 100ms */, ~0);
...
tcp_main_enable(vlib_main_t *vm)
{
    ...
    tm->tstamp_ticks_per_clock = vm->clib_time.seconds_per_clock / TCP_TSTAMP_RESOLUTION;
...
tcp_config_fn(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "preallocated-connections %d", &tm->preallocated_connections))
            ;
        else if (unformat(input, "preallocated-half-open-connections %d", &tm->preallocated_half_open_connections))
            ;
        else if (unformat(input, "buffer-fail-fraction %f", &tm->buffer_fail_fraction))
            ;
        else if (unformat(input, "max-rx-fifo %U", unformat_memory_size, &tm->max_rx_fifo))
            ;
        else
...
int
tcp_configure_v4_source_address_range(vlib_main_t *vm, ip4_address_t *start, ip4_address_t *end, u32 table_id)
{
    tcp_main_t *tm   = vnet_get_tcp_main();
    ...
    do {
        ...
        receive_dpo_add_or_lock(DPO_PROTO_IP4, ~0 /* sw_if_index */, NULL, &dpo);
        prefix.fp_len             = 32;
        prefix.fp_proto           = FIB_PROTOCOL_IP4;
...
int
tcp_configure_v6_source_address_range(vlib_main_t *vm, ip6_address_t *start, ip6_address_t *end, u32 table_id)
{
    tcp_main_t *tm = vnet_get_tcp_main();
    fib_prefix_t prefix;
    u32 fib_index = 0;
    ...
    while (1) {
        ...
        receive_dpo_add_or_lock(DPO_PROTO_IP6, ~0 /* sw_if_index */, NULL, &dpo);
...
static clib_error_t *
tcp_src_address(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    ...
    if (v4set) {
        ...
        case VNET_API_ERROR_INVALID_ARGUMENT:
            return clib_error_return(0, "Invalid address range %U - %U", format_ip4_address, &v4start,
...
static clib_error_t *
tcp_show_scoreboard_trace_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
...
tcp_scoreboard_replay(u8 *s, tcp_connection_t *tc, u8 verbose)
{
    ...
    while (left < trace_len) {
        ...
        while (trace[left].group == group) {
            if (trace[left].ack != 0) {
                if (verbose)
                    s = format(s, "Adding ack %u, snd_una_max %u, segs: ", trace[left].ack, trace[left].snd_una_max);
                dummy_tc->snd_una_max = trace[left].snd_una_max;
                next_ack              = trace[left].ack;
                has_new_ack           = 1;
            } else {
                if (verbose)
                    s = format(s, "[%u, %u], ", trace[left].start, trace[left].end);
...
static clib_error_t *
tcp_scoreboard_trace_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    transport_connection_t *tconn = 0;
    tcp_connection_t *tc          = 0;
    u8 *str                       = 0;
...
static clib_error_t *
show_tcp_punt_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    tcp_main_t *tm = vnet_get_tcp_main();
