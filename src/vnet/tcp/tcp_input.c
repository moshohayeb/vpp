...
#define tcp_next_output(is_ip4) (is_ip4 ? TCP_NEXT_TCP4_OUTPUT : TCP_NEXT_TCP6_OUTPUT)

#define tcp_next_drop(is_ip4) (is_ip4 ? TCP_NEXT_DROP4 : TCP_NEXT_DROP6)
...
tcp_options_parse(tcp_header_t *th, tcp_options_t *to)
{
    ...
    to->flags &= (TCP_OPTS_FLAG_SACK_PERMITTED | TCP_OPTS_FLAG_WSCALE | TCP_OPTS_FLAG_SACK);
...
{
    return tcp_opts_tstamp(&tc->rcv_opts) && tc->tsval_recent && timestamp_lt(tc->rcv_opts.tsval, tc->tsval_recent);
}
 ...
 */
static int
tcp_segment_validate(vlib_main_t *vm, tcp_connection_t *tc0, vlib_buffer_t *b0, tcp_header_t *th0, u32 *next0, u32 *error0)
{
    ...
    if (PREDICT_FALSE(tcp_segment_check_paws(tc0))) {
        *error0 = TCP_ERROR_PAWS;
        ...
        TCP_EVT_DBG(TCP_EVT_PAWS_FAIL, tc0, vnet_buffer(b0)->tcp.seq_number, vnet_buffer(b0)->tcp.seq_end);

        /* If it just so happens that a segment updates tsval_recent for a
         * segment over 24 days old, invalidate tsval_recent. */
        if (timestamp_lt(tc0->tsval_recent_age + TCP_PAWS_IDLE, tcp_time_now())) {
    ...
    if (!tcp_segment_in_rcv_wnd(tc0, vnet_buffer(b0)->tcp.seq_number, vnet_buffer(b0)->tcp.seq_end)) {
        *error0 = TCP_ERROR_RCV_WND;
        ...
        if (!(tc0->rcv_wnd == 0 && tc0->rcv_nxt == vnet_buffer(b0)->tcp.seq_number)) {
...
tcp_dequeue_acked(tcp_connection_t *tc, u32 ack)
{
    ...
    stream_session_dequeue_drop(&tc->connection, tc->bytes_acked + tc->sack_sb.snd_una_adv);
 ...
 */
static u8
tcp_ack_is_dupack(tcp_connection_t *tc, vlib_buffer_t *b, u32 prev_snd_wnd, u32 prev_snd_una)
{
    return ((vnet_buffer(b)->tcp.ack_number == prev_snd_una) && seq_gt(tc->snd_una_max, tc->snd_una) &&
 ...
 */
static u8
tcp_ack_is_cc_event(tcp_connection_t *tc, vlib_buffer_t *b, u32 prev_snd_wnd, u32 prev_snd_una, u8 *is_dack)
{
...
scoreboard_insert_hole(sack_scoreboard_t *sb, u32 prev_index, u32 start, u32 end)
{
...
scoreboard_update_bytes(tcp_connection_t *tc, sack_scoreboard_t *sb)
{
    ...
    while ((right = left) && bytes < (TCP_DUPACK_THRESHOLD - 1) * tc->snd_mss &&
 ...
 */
sack_scoreboard_hole_t *
scoreboard_next_rxt_hole(sack_scoreboard_t *sb, sack_scoreboard_hole_t *start, u8 have_sent_1_smss, u8 *can_rescue,
                         u8 *snd_limited)
{
...
tcp_scoreboard_is_sane_post_recovery(tcp_connection_t *tc)
{
    ...
    return (!hole || (seq_geq(hole->start, tc->snd_una) && seq_lt(hole->end, tc->snd_una_max)));
}
...
tcp_rcv_sacks(tcp_connection_t *tc, u32 ack)
{
    ...
    if (!tcp_opts_sack(&tc->rcv_opts) && sb->head == TCP_INVALID_SACK_HOLE_INDEX)
        return;
    ...
    while (blk < vec_end(tc->rcv_opts.sacks)) {
        if (seq_lt(blk->start, blk->end) && seq_gt(blk->start, tc->snd_una) && seq_gt(blk->start, ack) &&
    ...
    if (sb->head == TCP_INVALID_SACK_HOLE_INDEX) {
        ...
        last_hole       = scoreboard_insert_hole(sb, TCP_INVALID_SACK_HOLE_INDEX, tc->snd_una, tc->snd_una_max);
        ...
        if (seq_gt(tc->snd_una_max, last_hole->end)) {
            ...
            /* New hole after high sacked block */
                ...
                scoreboard_insert_hole(sb, sb->tail, sb->high_sacked, tc->snd_una_max);
            }
        }
    ...
    if (pool_elts(sb->holes) == 1) {
        hole = scoreboard_first_hole(sb);
        if (hole->start == ack + sb->snd_una_adv && hole->end == tc->snd_una_max)
            scoreboard_remove_hole(sb, hole);
    }

    scoreboard_update_bytes(tc, sb);
    sb->last_sacked_bytes = sb->sacked_bytes - (old_sacked_bytes - sb->last_bytes_delivered);
    ASSERT(sb->last_sacked_bytes <= sb->sacked_bytes || tcp_in_recovery(tc));
    ASSERT(sb->sacked_bytes == 0 || sb->sacked_bytes < tc->snd_una_max - seq_max(tc->snd_una, ack));
    ASSERT(sb->last_sacked_bytes + sb->lost_bytes <= tc->snd_una_max - seq_max(tc->snd_una, ack));
...
tcp_update_snd_wnd(tcp_connection_t *tc, u32 seq, u32 ack, u32 snd_wnd)
{
    ...
    if (seq_lt(tc->snd_wl1, seq) || (tc->snd_wl1 == seq && seq_leq(tc->snd_wl2, ack))) {
        ...
        if (tc->snd_wnd < tc->snd_mss) {
            ...
            if (!tcp_timer_is_active(tc, TCP_TIMER_PERSIST) && !tcp_timer_is_active(tc, TCP_TIMER_RETRANSMIT))
...
tcp_cc_update(tcp_connection_t *tc, vlib_buffer_t *b)
{
    ...
    if (PREDICT_FALSE(seq_leq(tc->snd_congestion, tc->snd_una - tc->bytes_acked) &&
...
{
    return (tc->rcv_dupacks == TCP_DUPACK_THRESHOLD || tcp_should_fastrecover_sack(tc));
}
...
tcp_cc_handle_event(tcp_connection_t *tc, u32 is_dack)
{
     ...
     */
        ...
        ASSERT(tc->snd_una != tc->snd_una_max || tc->sack_sb.last_sacked_bytes);
        ...
        if (tc->rcv_dupacks > TCP_DUPACK_THRESHOLD && !tc->bytes_acked) {
            ...
            if (seq_leq(tc->snd_una, tc->snd_congestion) &&
                ((!(tc->cwnd > tc->snd_mss && tc->bytes_acked <= 4 * tc->snd_mss)) ||
            ...
            return;
        } else if (!tc->bytes_acked || (tc->bytes_acked && !tcp_in_cong_recovery(tc))) {
            tc->cc_algo->rcv_cong_ack(tc, TCP_CC_DUPACK);
            return;
        } else
            goto partial_ack;
...
partial_ack:
    ...
    ASSERT(tc->bytes_acked + tc->sack_sb.snd_una_adv >= tc->sack_sb.last_bytes_delivered ||
    ...
    if (seq_lt(tc->snd_una, tc->sack_sb.high_rxt)) {
        ...
        rxt_delivered = tc->bytes_acked + tc->sack_sb.snd_una_adv - tc->sack_sb.last_bytes_delivered;
 ...
 */
static int
tcp_rcv_ack(tcp_connection_t *tc, vlib_buffer_t *b, tcp_header_t *th, u32 *next, u32 *error)
{
    ...
    if (PREDICT_FALSE(seq_gt(vnet_buffer(b)->tcp.ack_number, tc->snd_nxt))) {
        ...
        if (tcp_in_recovery(tc) && seq_leq(vnet_buffer(b)->tcp.ack_number, tc->snd_congestion)) {
        ...
        if (seq_gt(vnet_buffer(b)->tcp.ack_number, tc->snd_una_max)) {
            ...
            *error = TCP_ERROR_ACK_INVALID;
            TCP_EVT_DBG(TCP_EVT_ACK_RCV_ERR, tc, 0, vnet_buffer(b)->tcp.ack_number);
            return -1;
        }

        TCP_EVT_DBG(TCP_EVT_ACK_RCV_ERR, tc, 2, vnet_buffer(b)->tcp.ack_number);
    ...
    if (PREDICT_FALSE(seq_lt(vnet_buffer(b)->tcp.ack_number, tc->snd_una))) {
        *error = TCP_ERROR_ACK_OLD;
        TCP_EVT_DBG(TCP_EVT_ACK_RCV_ERR, tc, 1, vnet_buffer(b)->tcp.ack_number);
...
process_ack:
    ...
    tcp_update_snd_wnd(tc, vnet_buffer(b)->tcp.seq_number, vnet_buffer(b)->tcp.ack_number,
...
tcp_update_sack_list(tcp_connection_t *tc, u32 start, u32 end)
{
    ...
    for (i = 0; i < vec_len(tc->snd_sacks); i++) {
        ...
        if (block &&
            (seq_geq(tc->snd_sacks[i].end, new_list[0].start) && seq_leq(tc->snd_sacks[i].start, new_list[0].end))) {
...
tcp_session_enqueue_ooo(tcp_connection_t *tc, vlib_buffer_t *b, u16 data_len)
{
    ...
    rv = session_enqueue_stream_connection(&tc->connection, b, vnet_buffer(b)->tcp.seq_number - tc->rcv_nxt,
                                           0 /* queue event */, 0);
...
format_tcp_rx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U\n%U%U", format_tcp_header, &t->tcp_header, 128, format_white_space, indent, format_tcp_connection,
               &t->tcp_connection, 1);
...
format_tcp_rx_trace_short(u8 *s, va_list *args)
{
    ...
    s = format(s, "%d -> %d (%U)", clib_net_to_host_u16(t->tcp_header.dst_port),
               clib_net_to_host_u16(t->tcp_header.src_port), format_tcp_state, t->tcp_connection.state);

    return s;
}

static void
tcp_set_rx_trace_data(tcp_rx_trace_t *t0, tcp_connection_t *tc0, tcp_header_t *th0, vlib_buffer_t *b0, u8 is_ip4)
{
    if (tc0) {
...
static void
tcp_established_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4)
{
    u32 *from, n_left;
...
always_inline void
tcp_node_inc_counter_i(vlib_main_t *vm, u32 tcp4_node, u32 tcp6_node, u8 is_ip4, u32 evt, u32 val)
{
    if (is_ip4)
...
#define tcp_maybe_inc_counter(node_id, err, count)                                                                     \
    {                                                                                                                  \
        if (next0 != tcp_next_drop(is_ip4))                                                                            \
            tcp_node_inc_counter_i(vm, tcp4_##node_id##_node.index, tcp6_##node_id##_node.index, is_ip4, err, 1);      \
    }
#define tcp_inc_counter(node_id, err, count)                                                                           \
    tcp_node_inc_counter_i(vm, tcp4_##node_id##_node.index, tcp6_##node_id##_node.index, is_ip4, err, count)
...
always_inline uword
tcp46_established_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            b0  = vlib_get_buffer(vm, bi0);
            tc0 = tcp_connection_get(vnet_buffer(b0)->tcp.connection_index, thread_index);
            ...
            vnet_buffer(b0)->tcp.seq_end =
                vnet_buffer(b0)->tcp.seq_number + tcp_is_syn(th0) + is_fin + vnet_buffer(b0)->tcp.data_len;

            /* TODO header prediction fast path */

            /* 1-4: check SEQ, RST, SYN */
            if (PREDICT_FALSE(tcp_segment_validate(vm, tc0, b0, th0, &next0, &error0))) {
    ...
    errors                                  = session_manager_flush_enqueue_events(TRANSPORT_PROTO_TCP, thread_index);
...
static uword
tcp4_established(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_established_inline(vm, node, from_frame, 1 /* is_ip4 */);
}

static uword
tcp6_established(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
tcp_lookup_is_valid(tcp_connection_t *tc, tcp_header_t *hdr)
{
    ...
    u8 is_valid = (tc->c_lcl_port == hdr->dst_port && (tc->state == TCP_STATE_LISTEN || tc->c_rmt_port == hdr->src_port));
...
tcp_lookup_connection(u32 fib_index, vlib_buffer_t *b, u8 thread_index, u8 is_ip4)
{
    tcp_header_t *tcp;
    ...
    if (is_ip4) {
        ...
        tconn = session_lookup_connection_wt4(fib_index, &ip4->dst_address, &ip4->src_address, tcp->dst_port,
        ...
        tconn = session_lookup_connection_wt6(fib_index, &ip6->dst_address, &ip6->src_address, tcp->dst_port,
                                              tcp->src_port, TRANSPORT_PROTO_TCP, thread_index, &is_filtered);
        tc    = tcp_get_connection_from_transport(tconn);
        ASSERT(tcp_lookup_is_valid(tc, tcp));
    }
...
always_inline uword
tcp46_syn_sent_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            b0  = vlib_get_buffer(vm, bi0);
            tc0 = tcp_half_open_connection_get(vnet_buffer(b0)->tcp.connection_index);
            if (PREDICT_FALSE(tc0 == 0)) {
                error0 = TCP_ERROR_INVALID_CONNECTION;
                goto drop;
            }
            ...
            if (PREDICT_FALSE(tc0->flags & TCP_CONN_HALF_OPEN_DONE)) {
                ...
                ASSERT(tcp_lookup_connection(tc0->c_fib_index, b0, my_thread_index, is_ip4));
                goto drop;
            }
            ...
            if (PREDICT_FALSE(tcp0->dst_port != tc0->c_lcl_port || tcp0->src_port != tc0->c_rmt_port))
                goto drop;

            if (PREDICT_FALSE(!tcp_ack(tcp0) && !tcp_rst(tcp0) && !tcp_syn(tcp0)))
                goto drop;

            /* SYNs, FINs and data consume sequence numbers */
            vnet_buffer(b0)->tcp.seq_end = seq0 + tcp_is_syn(tcp0) + tcp_is_fin(tcp0) + vnet_buffer(b0)->tcp.data_len;
            ...
            new_tc0->snd_wnd = clib_net_to_host_u16(tcp0->window) << new_tc0->snd_wscale;
    ...
    errors = session_manager_flush_enqueue_events(TRANSPORT_PROTO_TCP, my_thread_index);
    tcp_inc_counter(syn_sent, TCP_ERROR_EVENT_FIFO_FULL, errors);
    return from_frame->n_vectors;
}

static uword
tcp4_syn_sent(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_syn_sent_inline(vm, node, from_frame, 1 /* is_ip4 */);
}

static uword
tcp6_syn_sent_rcv(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
 ...
 */
always_inline uword
tcp46_rcv_process_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            b0  = vlib_get_buffer(vm, bi0);
            tc0 = tcp_connection_get(vnet_buffer(b0)->tcp.connection_index, my_thread_index);
            if (PREDICT_FALSE(tc0 == 0)) {
                error0 = TCP_ERROR_INVALID_CONNECTION;
                goto drop;
            }
            ...
            /* SYNs, FINs and data consume sequence numbers */
            vnet_buffer(b0)->tcp.seq_end =
                vnet_buffer(b0)->tcp.seq_number + tcp_is_syn(tcp0) + is_fin0 + vnet_buffer(b0)->tcp.data_len;

            if (CLIB_DEBUG) {
                tcp_connection_t *tmp;
                tmp = tcp_lookup_connection(tc0->c_fib_index, b0, my_thread_index, is_ip4);
            ...
            /* 1-4: check SEQ, RST, SYN */
            if (PREDICT_FALSE(tcp_segment_validate(vm, tc0, b0, tcp0, &next0, &error0))) {
            ...
            case TCP_STATE_SYN_RCVD:
                ...
                tc0->snd_wnd = clib_net_to_host_u16(tcp0->window) << tc0->rcv_opts.wscale;
...
static uword
tcp4_rcv_process(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_rcv_process_inline(vm, node, from_frame, 1 /* is_ip4 */);
}

static uword
tcp6_rcv_process(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
 ...
 */
always_inline uword
tcp46_listen_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            child0 = tcp_lookup_connection(lc0->c_fib_index, b0, my_thread_index, is_ip4);
            ...
            if (is_ip4) {
                ...
                clib_memcpy(&child0->c_lcl_ip6, &ip60->dst_address, sizeof(ip6_address_t));
                clib_memcpy(&child0->c_rmt_ip6, &ip60->src_address, sizeof(ip6_address_t));
            }
            ...
            child0->snd_wnd = clib_net_to_host_u16(th0->window) << child0->snd_wscale;
            ...
            if (stream_session_accept(&child0->connection, lc0->c_s_index, 0 /* notify */)) {
...
static uword
tcp4_listen(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_listen_inline(vm, node, from_frame, 1 /* is_ip4 */);
}

static uword
tcp6_listen(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static void
tcp_input_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t **bs, u32 n_bufs, u8 is_ip4)
{
    tcp_connection_t *tc;
    tcp_header_t *tcp;
    tcp_rx_trace_t *t;
...
tcp_input_lookup_buffer(vlib_buffer_t *b, u8 thread_index, u32 *error, u8 is_ip4)
{
    ...
    if (is_ip4) {
        ...
        tc = session_lookup_connection_wt4(fib_index, &ip4->dst_address, &ip4->src_address, tcp->dst_port,
                                           tcp->src_port, TRANSPORT_PROTO_TCP, thread_index, &is_filtered);
    } else {
        ...
        tc = session_lookup_connection_wt6(fib_index, &ip6->dst_address, &ip6->src_address, tcp->dst_port,
                                           tcp->src_port, TRANSPORT_PROTO_TCP, thread_index, &is_filtered);
    }
...
static inline void
tcp_input_dispatch_buffer(tcp_main_t *tm, tcp_connection_t *tc, vlib_buffer_t *b, u16 *next, u32 *error)
{
    tcp_header_t *tcp;
    u8 flags;
    ...
    *error = tm->dispatch_table[tc->state][flags].error;
    ...
    if (PREDICT_FALSE(*error == TCP_ERROR_DISPATCH || *next == TCP_INPUT_NEXT_RESET)) {
        /* Overload tcp flags to store state */
        tcp_state_t state         = tc->state;
        vnet_buffer(b)->tcp.flags = tc->state;

        if (*error == TCP_ERROR_DISPATCH)
            clib_warning("disp error state %U flags %U", format_tcp_state, state, format_tcp_flags, (int) flags);
    }
}

always_inline uword
tcp46_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
...
static uword
tcp4_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_input_inline(vm, node, from_frame, 1 /* is_ip4 */);
}

static uword
tcp6_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
#define _(t, f, n, e)                                                                                                  \
    ...
    _(LISTEN, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RESET, TCP_ERROR_NONE);
    ...
    _(CLOSED, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_DROP, TCP_ERROR_CONNECTION_CLOSED);
#undef _
}
