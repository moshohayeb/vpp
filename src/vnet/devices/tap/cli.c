...
static clib_error_t *
tap_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat_user(input, unformat_line_input, line_input)) {
        while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
            if (unformat(line_input, "id %u", &args.id))
                ...
                ;
            else if (unformat(line_input, "host-mac-addr %U", unformat_ethernet_address, args.host_mac_addr))
                ;
            else if (unformat(line_input, "host-bridge %s", &args.host_bridge))
                ;
            else if (unformat(line_input, "host-ip4-addr %U/%d", unformat_ip4_address, &args.host_ip4_addr,
            ...
            else if (unformat(line_input, "host-ip6-addr %U/%d", unformat_ip6_address, &args.host_ip6_addr,
                ...
                ;
            else if (unformat(line_input, "hw-addr %U", unformat_ethernet_address, args.mac_addr))
                args.mac_addr_set = 1;
            else {
...
static clib_error_t *
tap_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
#undef _
    ...
    if (vec_len(hw_if_indices) == 0) {
        /* *INDENT-OFF* */
        pool_foreach(vif, mm->interfaces, vec_add1(hw_if_indices, vif->hw_if_index););
        /* *INDENT-ON* */
    }
    ...
    for (hw_if_index = 0; hw_if_index < vec_len(hw_if_indices); hw_if_index++) {
        ...
        vlib_cli_output(vm, "interface %U", format_vnet_sw_if_index_name, vnm, vif->sw_if_index);
        if (vif->host_if_name)
        ...
        while (flag_entry->str) {
            if (vif->flags & (1ULL << flag_entry->bit))
                vlib_cli_output(vm, "    %s (%d)", flag_entry->str, flag_entry->bit);
            flag_entry++;
        }
        ...
        vec_foreach_index(i, vif->vrings)
        {
            ...
            vlib_cli_output(vm, "    qsz %d, last_used_idx %d, desc_next %d, desc_in_use %d", vring->size,
                            vring->last_used_idx, vring->desc_next, vring->desc_in_use);
            vlib_cli_output(vm, "    avail.flags 0x%x avail.idx %d used.flags 0x%x used.idx %d", vring->avail->flags,
                            vring->avail->idx, vring->used->flags, vring->used->idx);
