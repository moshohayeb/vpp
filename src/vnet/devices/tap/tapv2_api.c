...
static void
tap_send_sw_interface_event_deleted(vpe_api_main_t *am, vl_api_registration_t *reg, u32 sw_if_index)
{
...
static void
tap_send_sw_interface_details(vpe_api_main_t *am, vl_api_registration_t *reg, tap_interface_details_t *tap_if, u32 context)
{
    ...
    clib_memcpy(mp->dev_name, tap_if->dev_name,
                MIN(ARRAY_LEN(mp->dev_name) - 1, strlen((const char *) tap_if->dev_name)));
    ...
    clib_memcpy(mp->host_if_name, tap_if->host_if_name,
                MIN(ARRAY_LEN(mp->host_if_name) - 1, strlen((const char *) tap_if->host_if_name)));
...
static void
vl_api_sw_interface_tap_v2_dump_t_handler(vl_api_sw_interface_tap_v2_dump_t *mp)
{
    int rv;
