...
#define AF_PACKET_TX_FRAME_NR (AF_PACKET_TX_BLOCK_NR * AF_PACKET_TX_FRAMES_PER_BLOCK)
#define AF_PACKET_TX_BLOCK_SIZE (AF_PACKET_TX_FRAME_SIZE * AF_PACKET_TX_FRAMES_PER_BLOCK)

#define AF_PACKET_RX_FRAMES_PER_BLOCK 1024
#define AF_PACKET_RX_FRAME_SIZE (2048 * 5)
#define AF_PACKET_RX_BLOCK_NR 1
#define AF_PACKET_RX_FRAME_NR (AF_PACKET_RX_BLOCK_NR * AF_PACKET_RX_FRAMES_PER_BLOCK)
#define AF_PACKET_RX_BLOCK_SIZE (AF_PACKET_RX_FRAME_SIZE * AF_PACKET_RX_FRAMES_PER_BLOCK)
...
af_packet_eth_flag_change(vnet_main_t *vnm, vnet_hw_interface_t *hi, u32 flags)
{
    ...
    if (ETHERNET_INTERFACE_FLAG_MTU == (flags & ETHERNET_INTERFACE_FLAG_MTU)) {
        ...
        if (error) {
            vlib_log_err(apm->log_class, "sysfs write failed to change MTU: %U", format_clib_error, error);
...
static int
create_packet_v2_sock(int host_if_index, tpacket_req_t *rx_req, tpacket_req_t *tx_req, int *fd, u8 **ring)
{
    af_packet_main_t *apm = &af_packet_main;
    int ret, err;
    ...
    u32 ring_sz      = rx_req->tp_block_size * rx_req->tp_block_nr + tx_req->tp_block_size * tx_req->tp_block_nr;
    ...
    if ((err = bind(*fd, (struct sockaddr *) &sll, sizeof(sll))) < 0) {
        vlib_log_debug(apm->log_class, "Failed to bind rx packet socket (error %d)", err);
        ret = VNET_API_ERROR_SYSCALL_ERROR_1;
        goto error;
    }

    if ((err = setsockopt(*fd, SOL_PACKET, PACKET_VERSION, &ver, sizeof(ver))) < 0) {
        vlib_log_debug(apm->log_class, "Failed to set rx packet interface version");
        ret = VNET_API_ERROR_SYSCALL_ERROR_1;
        goto error;
    }
    ...
    if ((err = setsockopt(*fd, SOL_PACKET, PACKET_LOSS, &opt, sizeof(opt))) < 0) {
        vlib_log_debug(apm->log_class, "Failed to set packet tx ring error handling option");
        ret = VNET_API_ERROR_SYSCALL_ERROR_1;
        goto error;
    }
...
af_packet_create_if(vlib_main_t *vm, u8 *host_if_name, u8 *hw_addr_set, u32 *sw_if_index)
{
    ...
    clib_memcpy(ifr.ifr_name, (const char *) host_if_name, vec_len(host_if_name));
...

    {
        ...
        template.description     = format(0, "%U", format_af_packet_device_name, if_index);
    ...
    error = ethernet_register_interface(vnm, af_packet_device_class.index, if_index, hw_addr, &apif->hw_if_index,
                                        af_packet_eth_flag_change);

    if (error) {
        memset(apif, 0, sizeof(*apif));
        pool_put(apm->interfaces, apif);
        vlib_log_err(apm->log_class, "Unable to register interface: %U", format_clib_error, error);
        clib_error_free(error);
        ret = VNET_API_ERROR_SYSCALL_ERROR_1;
        goto error;
    }
    ...
    vnet_hw_interface_set_input_node(vnm, apif->hw_if_index, af_packet_input_node.index);
    ...
    vnet_hw_interface_set_rx_mode(vnm, apif->hw_if_index, 0, VNET_HW_INTERFACE_RX_MODE_INTERRUPT);
...
af_packet_delete_if(vlib_main_t *vm, u8 *host_if_name)
{
    ...
    if (munmap(apif->rx_ring, ring_sz))
        vlib_log_warn(apm->log_class, "Host interface %s could not free rx/tx ring", host_if_name);
    apif->rx_ring = NULL;
    apif->tx_ring = NULL;
    apif->fd      = -1;
