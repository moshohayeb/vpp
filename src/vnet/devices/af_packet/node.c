...
format_af_packet_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "af_packet: hw_if_index %d next-index %d", t->hw_if_index, t->next_index);
...
#endif
               ,
               format_white_space, indent + 2, format_white_space, indent + 4, t->tph.tp_status, t->tph.tp_len,
               t->tph.tp_snaplen, t->tph.tp_mac, t->tph.tp_net, format_white_space, indent + 4, t->tph.tp_sec,
...
mark_tcp_udp_cksum_calc(vlib_buffer_t *b)
{
    ...
    if (clib_net_to_host_u16(eth->type) == ETHERNET_TYPE_IP4) {
        ip4_header_t *ip4 = (vlib_buffer_get_current(b) + sizeof(ethernet_header_t));
        b->flags |= VNET_BUFFER_F_IS_IP4;
        if (ip4->protocol == IP_PROTOCOL_TCP) {
            b->flags |= VNET_BUFFER_F_OFFLOAD_TCP_CKSUM;
            ((tcp_header_t *) (vlib_buffer_get_current(b) + sizeof(ethernet_header_t) + ip4_header_bytes(ip4)))->checksum =
                0;
        } else if (ip4->protocol == IP_PROTOCOL_UDP) {
            b->flags |= VNET_BUFFER_F_OFFLOAD_UDP_CKSUM;
            ((udp_header_t *) (vlib_buffer_get_current(b) + sizeof(ethernet_header_t) + ip4_header_bytes(ip4)))->checksum =
                0;
        }
        vnet_buffer(b)->l3_hdr_offset = sizeof(ethernet_header_t);
        vnet_buffer(b)->l4_hdr_offset = sizeof(ethernet_header_t) + ip4_header_bytes(ip4);
    } else if (clib_net_to_host_u16(eth->type) == ETHERNET_TYPE_IP6) {
        ip6_header_t *ip6 = (vlib_buffer_get_current(b) + sizeof(ethernet_header_t));
        ...
        if (ip6->protocol == IP_PROTOCOL_TCP) {
            b->flags |= VNET_BUFFER_F_OFFLOAD_TCP_CKSUM;
            ((tcp_header_t *) (vlib_buffer_get_current(b) + sizeof(ethernet_header_t) + ip6_hdr_len))->checksum = 0;
        } else if (ip6->protocol == IP_PROTOCOL_UDP) {
            b->flags |= VNET_BUFFER_F_OFFLOAD_UDP_CKSUM;
            ((udp_header_t *) (vlib_buffer_get_current(b) + sizeof(ethernet_header_t) + ip6_hdr_len))->checksum = 0;
        }
        vnet_buffer(b)->l3_hdr_offset = sizeof(ethernet_header_t);
        vnet_buffer(b)->l4_hdr_offset = sizeof(ethernet_header_t) + ip6_hdr_len;
    }
}

always_inline uword
af_packet_device_input_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, af_packet_if_t *apif)
{
    ...
    u32 n_buffer_bytes = vlib_buffer_free_list_buffer_size(vm, VLIB_BUFFER_DEFAULT_FREE_LIST_INDEX);
            ...
            while (data_len) {
                ...
                if (PREDICT_FALSE(tph->tp_status & TP_STATUS_VLAN_VALID)) {
                    if (PREDICT_TRUE(offset == 0)) {
                        clib_memcpy(vlib_buffer_get_current(b0), (u8 *) tph + tph->tp_mac, sizeof(ethernet_header_t));
                ...
                clib_memcpy(((u8 *) vlib_buffer_get_current(b0)) + bytes_copied + vlan_len,
                            (u8 *) tph + tph->tp_mac + offset + bytes_copied, (bytes_to_copy - bytes_copied));
...
static uword
af_packet_input_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_rx_packets                = 0;
    af_packet_main_t *apm           = &af_packet_main;
