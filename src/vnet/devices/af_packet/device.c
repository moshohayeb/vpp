...
static uword
af_packet_interface_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    af_packet_main_t *apm               = &af_packet_main;
    ...
    while (n_left > 0) {
        ...
        do {
            ...
            clib_memcpy((u8 *) tph + TPACKET_ALIGN(sizeof(struct tpacket2_hdr)) + offset, vlib_buffer_get_current(b0),
                        len);
            offset += len;
    ...
    if (PREDICT_TRUE(n_sent)) {
        ...
        if (PREDICT_FALSE(sendto(apif->fd, NULL, 0, MSG_DONTWAIT, NULL, 0) == -1)) {
            ...
            vlib_error_count(vm, node->node_index,
                             unix_error_is_fatal(errno) ? AF_PACKET_TX_ERROR_TXRING_FATAL :
    ...
    if (PREDICT_FALSE(frame_not_ready))
        vlib_error_count(vm, node->node_index, AF_PACKET_TX_ERROR_FRAME_NOT_READY, frame_not_ready);

    if (PREDICT_FALSE(frame_not_ready + n_sent == frame_num))
        vlib_error_count(vm, node->node_index, AF_PACKET_TX_ERROR_TXRING_OVERRUN, n_left);
...
af_packet_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if ((rv = ioctl(fd, SIOCGIFNAME, &ifr)) < 0) {
        vlib_log_warn(apm->log_class, "af_packet_%s ioctl could not retrieve eth name", apif->host_if_name);
        goto error;
    }
    ...
    if ((rv = ioctl(fd, SIOCGIFFLAGS, &ifr)) < 0) {
        vlib_log_warn(apm->log_class, "af_packet_%s error: %d", apif->is_admin_up ? "up" : "down", rv);
        goto error;
    }
...
static clib_error_t *
af_packet_subif_add_del_function(vnet_main_t *vnm, u32 hw_if_index, struct vnet_sw_interface_t *st, int is_add)
{
    /* Nothing for now */
    return 0;
}
...
static clib_error_t *
af_packet_set_mac_address_function(struct vnet_hw_interface_t *hi, const u8 *old_address, const u8 *address)
{
    af_packet_main_t *apm = &af_packet_main;
    ...
    ifr.ifr_ifindex = apif->host_if_index;
    if ((rv = ioctl(fd, SIOCGIFNAME, &ifr)) < 0) {
        vlib_log_warn(apm->log_class, "af_packet_%s ioctl could not retrieve eth name, error: %d", apif->host_if_name,
                      rv);
        goto error;
    }
...
VLIB_DEVICE_TX_FUNCTION_MULTIARCH(af_packet_device_class, af_packet_interface_tx)
/* *INDENT-ON* */
