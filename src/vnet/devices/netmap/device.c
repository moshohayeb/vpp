...
format_netmap_device(u8 *s, va_list *args)
{
    ...
    if (verbose) {
        s = format(s,
                   ...
                   format_white_space, indent + 2, nif->req->nr_version, nif->req->nr_flags, format_white_space,
                   indent + 2, nif->mem_region, nif->req->nr_memsize, nif->req->nr_offset, format_white_space, indent + 2,
                   nif->req->nr_tx_slots, nif->req->nr_rx_slots, nif->req->nr_tx_rings, nif->req->nr_rx_rings);
    }
    return s;
}
...
static uword
netmap_interface_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    netmap_main_t *nm                   = &netmap_main;
    ...
    while (n_left && cur_ring <= nif->last_tx_ring) {
        ...
        while (n_left && n_free_slots) {
            ...
            do {
                ...
                /* memcpy */
                clib_memcpy((u8 *) NETMAP_BUF(ring, slot->buf_idx) + offset, vlib_buffer_get_current(b0), len);
                offset += len;
    ...
    if (n_left)
        vlib_error_count(vm, node->node_index,
                         (n_left == frame->n_vectors ? NETMAP_TX_ERROR_PENDING_MSGS : NETMAP_TX_ERROR_NO_FREE_SLOTS),
                         n_left);
...
static void
netmap_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    netmap_main_t *apm      = &netmap_main;
...
static clib_error_t *
netmap_subif_add_del_function(vnet_main_t *vnm, u32 hw_if_index, struct vnet_sw_interface_t *st, int is_add)
{
    /* Nothing for now */
    return 0;
}
