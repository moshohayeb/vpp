...
format_netmap_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "\n%Uslot: flags 0x%x len %u buf_idx %u", format_white_space, indent + 2, t->slot.flags, t->slot.len,
               t->slot.buf_idx);
    return s;
}
...
always_inline uword
netmap_device_input_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, netmap_if_t *nif)
{
        ...
        while (r) {
            ...
            while (r && n_left_to_next) {
                ...
                while (data_len && n_free_bufs) {
                    ...
                    b0->current_data  = 0;
                    clib_memcpy(vlib_buffer_get_current(b0), (u8 *) NETMAP_BUF(ring, slot->buf_idx) + offset,
                                bytes_to_copy);
                ...
                vnet_feature_start_device_input_x1(nif->sw_if_index, &next0, first_b0);
...
static uword
netmap_input_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    int i;
    u32 n_rx_packets  = 0;
    u32 thread_index  = vm->thread_index;
