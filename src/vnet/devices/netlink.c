...
static void
vnet_netlink_msg_init(vnet_netlink_msg_t *m, u16 type, u16 flags, void *msg_data, int msg_len)
{
    struct nlmsghdr *nh;
    u8 *p;
...
static void
vnet_netlink_msg_add_rtattr(vnet_netlink_msg_t *m, u16 rta_type, void *rta_data, int rta_data_len)
{
    struct rtattr *rta;
    u8 *p;
...
vnet_netlink_add_ip4_addr(int ifindex, void *addr, int pfx_len)
{
    ...
    vnet_netlink_msg_init(&m, RTM_NEWADDR, NLM_F_REQUEST | NLM_F_CREATE | NLM_F_EXCL, &ifa, sizeof(struct ifaddrmsg));
...
vnet_netlink_add_ip4_route(void *dst, u8 dst_len, void *gw)
{
    ...
    vnet_netlink_msg_init(&m, RTM_NEWROUTE, NLM_F_REQUEST | NLM_F_CREATE | NLM_F_EXCL, &rtm, sizeof(struct rtmsg));
