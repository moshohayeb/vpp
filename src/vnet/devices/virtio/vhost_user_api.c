 ...
 */
static void
send_sw_interface_event_deleted(vpe_api_main_t *am, vl_api_registration_t *reg, u32 sw_if_index)
{
...
vl_api_create_vhost_user_if_t_handler(vl_api_create_vhost_user_if_t *mp)
{
    ...
    rv = vhost_user_create_if(vnm, vm, (char *) mp->sock_filename, mp->is_server, &sw_if_index, features, mp->renumber,
                              ntohl(mp->custom_dev_instance), (mp->use_custom_mac) ? mp->mac_address : NULL);
    ...
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_CREATE_VHOST_USER_IF_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
    /* *INDENT-ON* */
}
...
vl_api_modify_vhost_user_if_t_handler(vl_api_modify_vhost_user_if_t *mp)
{
    ...
    rv = vhost_user_modify_if(vnm, vm, (char *) mp->sock_filename, mp->is_server, sw_if_index, (u64) ~0, mp->renumber,
...
static void
send_sw_interface_vhost_user_details(vpe_api_main_t *am, vl_api_registration_t *reg, vhost_user_intf_details_t *vui,
                                     u32 context)
{
...
static void
vl_api_sw_interface_vhost_user_dump_t_handler(vl_api_sw_interface_vhost_user_dump_t *mp)
{
    int rv                            = 0;
