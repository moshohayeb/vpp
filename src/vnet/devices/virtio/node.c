...
format_virtio_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               format_white_space, indent + 2, t->hdr.flags, t->hdr.gso_type, t->hdr.hdr_len, t->hdr.gso_size,
...
virtio_refill_vring(vlib_main_t *vm, virtio_vring_t *vring)
{
    ...
    n_slots = vlib_buffer_alloc_to_ring(vm, vring->buffers, next, vring->size, n_slots);
...
virtio_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, virtio_if_t *vif, u16 qid)
{
    ...
    vlib_increment_combined_counter(vnm->interface_main.combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX,
                                    thread_index, vif->hw_if_index, n_rx_packets, n_rx_bytes);
...
static uword
virtio_input_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_rx                        = 0;
    ...
    foreach_device_and_queue(dq, rt->devices_and_queues)
    {
        ...
        if (mif->flags & VIRTIO_IF_FLAG_ADMIN_UP) {
            n_rx += virtio_device_input_inline(vm, node, frame, mif, dq->queue_id);
        }
    }
