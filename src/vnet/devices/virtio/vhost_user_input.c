...
vhost_user_rx_trace(vhost_trace_t *t, vhost_user_intf_t *vui, u16 qid, vlib_buffer_t *b, vhost_user_vring_t *txvq)
{
...
vhost_user_input_copy(vhost_user_intf_t *vui, vhost_copy_t *cpy, u16 copy_len, u32 *map_hint)
{
    ...
    if (PREDICT_TRUE(copy_len >= 4)) {
        ...
        while (PREDICT_TRUE(copy_len >= 4)) {
            ...
            if (PREDICT_FALSE(!(src2 = map_guest_mem(vui, cpy[2].src, map_hint))))
                return 1;
            if (PREDICT_FALSE(!(src3 = map_guest_mem(vui, cpy[3].src, map_hint))))
                return 1;
 ...
 */
static_always_inline u32
vhost_user_rx_discard_packet(vlib_main_t *vm, vhost_user_intf_t *vui, vhost_user_vring_t *txvq, u32 discard_max)
{
    /*
    ...
    while (discarded_packets != discard_max) {
        ...
        u16 desc_chain_head = txvq->avail->ring[txvq->last_avail_idx & txvq->qsz_mask];
        txvq->last_avail_idx++;
        txvq->used->ring[txvq->last_used_idx & txvq->qsz_mask].id  = desc_chain_head;
        txvq->used->ring[txvq->last_used_idx & txvq->qsz_mask].len = 0;
        vhost_user_log_dirty_ring(vui, txvq, ring[txvq->last_used_idx & txvq->qsz_mask]);
        txvq->last_used_idx++;
        discarded_packets++;
    }
...
vhost_user_input_rewind_buffers(vlib_main_t *vm, vhost_cpu_t *cpu, vlib_buffer_t *b_head)
{
...
vhost_user_if_input(vlib_main_t *vm, vhost_user_main_t *vum, vhost_user_intf_t *vui, u16 qid, vlib_node_runtime_t *node,
                    vnet_hw_interface_rx_mode mode)
{
     ...
     */
    if (PREDICT_FALSE(vum->cpus[thread_index].rx_buffers_len < n_left + 1 || vum->cpus[thread_index].rx_buffers_len < 40)) {
        u32 curr_len = vum->cpus[thread_index].rx_buffers_len;
        vum->cpus[thread_index].rx_buffers_len +=
            vlib_buffer_alloc_from_free_list(vm, vum->cpus[thread_index].rx_buffers + curr_len,
                                             VHOST_USER_RX_BUFFERS_N - curr_len, VLIB_BUFFER_DEFAULT_FREE_LIST_INDEX);

        if (PREDICT_FALSE(vum->cpus[thread_index].rx_buffers_len < VHOST_USER_RX_BUFFER_STARVATION)) {
            ...
            u32 flush = (n_left + 1 > vum->cpus[thread_index].rx_buffers_len) ?
                            n_left + 1 - vum->cpus[thread_index].rx_buffers_len :
                            1;
            ...
            vlib_increment_simple_counter(vnet_main.interface_main.sw_if_counters + VNET_INTERFACE_COUNTER_DROP,
                                          vlib_get_thread_index(), vui->sw_if_index, flush);
    ...
    while (n_left > 0) {
        ...
        while (n_left > 0 && n_left_to_next > 0) {
            ...
            vum->cpus[thread_index].rx_buffers_len--;
            bi_current = (vum->cpus[thread_index].rx_buffers)[vum->cpus[thread_index].rx_buffers_len];
            ...
            vlib_prefetch_buffer_with_index(vm,
                                            (vum->cpus[thread_index].rx_buffers)[vum->cpus[thread_index].rx_buffers_len - 1],
                                            LOAD);
            ...
            if (PREDICT_TRUE(vui->is_any_layout) || (!(desc_table[desc_current].flags & VIRTQ_DESC_F_NEXT))) {
            ...
            while (1) {
                ...
                if (desc_data_offset == desc_table[desc_current].len) {
                    if (PREDICT_FALSE(desc_table[desc_current].flags & VIRTQ_DESC_F_NEXT)) {
                ...
                if (PREDICT_FALSE(b_current->current_length == VLIB_BUFFER_DATA_SIZE)) {
                    if (PREDICT_FALSE(vum->cpus[thread_index].rx_buffers_len == 0)) {
                        /* Cancel speculation */
                        to_next--;
                        n_left_to_next++;
                         ...
                         * but valid.
                         */
                        vhost_user_input_rewind_buffers(vm, &vum->cpus[thread_index], b_head);
                        n_left = 0;
                        goto stop;
                    }
                    ...
                    /* Get next output */
                    vum->cpus[thread_index].rx_buffers_len--;
                    u32 bi_next = (vum->cpus[thread_index].rx_buffers)[vum->cpus[thread_index].rx_buffers_len];
        ...
        out:
...

            {
                ...
                vnet_feature_start_device_input_x1(vui->sw_if_index, &next0, b_head);

                u32 bi = to_next[-1];   // Cannot use to_next[-1] in the macro
                vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi, next0);
            }
            ...
            if (PREDICT_FALSE(copy_len >= VHOST_USER_RX_COPY_THRESHOLD)) {
                if (PREDICT_FALSE(vhost_user_input_copy(vui, vum->cpus[thread_index].copy, copy_len, &map_hint))) {
                    vlib_error_count(vm, node->node_index, VHOST_USER_INPUT_FUNC_ERROR_MMAP_FAIL, 1);
                }
                copy_len = 0;
...
VLIB_NODE_FN(vhost_user_input_node)(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    {
        if (clib_smp_swap(&dq->interrupt_pending, 0) || (node->state == VLIB_NODE_STATE_POLLING)) {
            vui          = pool_elt_at_index(vum->vhost_user_interfaces, dq->dev_instance);
            n_rx_packets = vhost_user_if_input(vm, vum, vui, dq->queue_id, node, dq->mode);
        }
    }
