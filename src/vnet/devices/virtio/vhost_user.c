...
vhost_user_tx_thread_placement(vhost_user_intf_t *vui)
{
    ...
    while (1) {
        ...
        if (thread_index == 0) {
            ...
            for (thread_index = 0; thread_index < vlib_get_thread_main()->n_vlib_mains; thread_index++) {
...
vhost_user_rx_thread_placement()
{
    ...
    pool_foreach(vui, vum->vhost_user_interfaces, {
        ...
        {
            rv = vnet_hw_interface_unassign_rx_thread(vnm, vui->hw_if_index, *queue);
            if (rv)
                vu_log_warn(vui,
                            "unable to unassign interface %d, "
                            "queue %d: rc=%d",
                            vui->hw_if_index, *queue, rv);
        }
    ...
    /* *INDENT-OFF* */
    pool_foreach(vui, vum->vhost_user_interfaces, {
        vnet_hw_interface_set_input_node(vnm, vui->hw_if_index, vhost_user_input_node.index);
        vec_foreach(queue, vui->rx_queues)
        {
            vnet_hw_interface_assign_rx_thread(vnm, vui->hw_if_index, *queue, ~0);
            txvq = &vui->vrings[VHOST_VRING_IDX_TX(*queue)];
            rv   = vnet_hw_interface_set_rx_mode(vnm, vui->hw_if_index, *queue, txvq->mode);
            if (rv)
                vu_log_warn(vui,
...
vhost_user_update_iface_state(vhost_user_intf_t *vui)
{
    ...
    if (is_ready != vui->is_ready) {
        ...
        if (vui->admin_up)
            vnet_hw_interface_set_flags(vnet_get_main(), vui->hw_if_index, is_ready ? VNET_HW_INTERFACE_FLAG_LINK_UP : 0);
...
vhost_user_kickfd_read_ready(clib_file_t *uf)
{
    __attribute__((unused)) int n;
    u8 buff[8];
    vhost_user_intf_t *vui = pool_elt_at_index(vhost_user_main.vhost_user_interfaces, uf->private_data >> 8);
    u32 qid                = uf->private_data & 0xff;

    n = read(uf->file_descriptor, ((char *) &buff), 8);
    vu_log_debug(vui, "if %d KICK queue %d", uf->private_data >> 8, qid);
    if (!vui->vrings[qid].started || (vhost_user_intf_ready(vui) != vui->is_ready)) {
...
vhost_user_socket_read(clib_file_t *uf)
{
    ...
    case VHOST_USER_GET_FEATURES:
        msg.flags |= 4;
        msg.u64 = (1ULL << FEAT_VIRTIO_NET_F_MRG_RXBUF) | (1ULL << FEAT_VIRTIO_NET_F_CTRL_VQ) |
                  (1ULL << FEAT_VIRTIO_F_ANY_LAYOUT) | (1ULL << FEAT_VIRTIO_F_INDIRECT_DESC) |
                  (1ULL << FEAT_VHOST_F_LOG_ALL) | (1ULL << FEAT_VIRTIO_NET_F_GUEST_ANNOUNCE) |
                  (1ULL << FEAT_VIRTIO_NET_F_MQ) | (1ULL << FEAT_VHOST_USER_F_PROTOCOL_FEATURES) |
    ...
    case VHOST_USER_SET_FEATURES:
        ...
        if (vui->features & ((1 << FEAT_VIRTIO_NET_F_MRG_RXBUF) | (1ULL << FEAT_VIRTIO_F_VERSION_1)))
    ...
    case VHOST_USER_SET_MEM_TABLE:
        ...
        for (i = 0; i < msg.memory.nregions; i++) {
            clib_memcpy(&(vui->regions[i]), &msg.memory.regions[i], sizeof(vhost_user_memory_region_t));
            ...
            vui->region_mmap_addr[i]     = mmap(0, map_sz, PROT_READ | PROT_WRITE, MAP_SHARED, fds[i], 0);
            ...
            vu_log_debug(vui,
                         ...
                         i, map_sz, fds[i], vui->region_mmap_addr[i], page_sz);
    ...
    case VHOST_USER_SET_VRING_NUM:
        vu_log_debug(vui, "if %d msg VHOST_USER_SET_VRING_NUM idx %d num %d", vui->hw_if_index, msg.state.index,
                     msg.state.num);
    ...
    case VHOST_USER_SET_VRING_ADDR:
        ...
        vui->vrings[msg.state.index].desc  = (vring_desc_t *) map_user_mem(vui, msg.addr.desc_user_addr);
        vui->vrings[msg.state.index].used  = (vring_used_t *) map_user_mem(vui, msg.addr.used_user_addr);
        vui->vrings[msg.state.index].avail = (vring_avail_t *) map_user_mem(vui, msg.addr.avail_user_addr);

        if ((vui->vrings[msg.state.index].desc == NULL) || (vui->vrings[msg.state.index].used == NULL) ||
    ...
    case VHOST_USER_SET_VRING_BASE:
        vu_log_debug(vui, "if %d msg VHOST_USER_SET_VRING_BASE idx %d num %d", vui->hw_if_index, msg.state.index,
                     msg.state.num);
    ...
    case VHOST_USER_GET_VRING_BASE:
        ...
        vu_log_debug(vui, "if %d msg VHOST_USER_GET_VRING_BASE idx %d num %d", vui->hw_if_index, msg.state.index,
                     msg.state.num);
        break;
    ...
    case VHOST_USER_SET_LOG_BASE: {
        ...
        vui->log_base_addr = mmap(0, map_sz, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    case VHOST_USER_SET_VRING_ENABLE:
        vu_log_debug(vui, "if %d VHOST_USER_SET_VRING_ENABLE: %s queue %d", vui->hw_if_index,
                     msg.state.num ? "enable" : "disable", msg.state.index);
        if (msg.state.index >= VHOST_VRING_MAX_N) {
            vu_log_debug(vui,
...
vhost_user_socksvr_accept_ready(clib_file_t *uf)
{
    ...
    if (vui->clib_file_index != ~0) {
        vu_log_debug(vui, "Close client socket for vhost interface %d, fd %d", vui->sw_if_index,
                     UNIX_GET_FD(vui->clib_file_index));
        clib_file_del(&file_main, file_main.file_pool + vui->clib_file_index);
    }
...
static uword
vhost_user_send_interrupt_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    vhost_user_intf_t *vui;
    ...
    while (1) {
        ...
        if (vlib_process_suspend_time_is_zero(timeout) || (timeout > vum->coalesce_time))
        ...
        case ~0:
            ...
            pool_foreach(vui, vum->vhost_user_interfaces, {
                ...
                vec_foreach(queue, vui->rx_queues)
                {
                    vhost_user_vring_t *rxvq = &vui->vrings[VHOST_VRING_IDX_RX(*queue)];
                    vhost_user_vring_t *txvq = &vui->vrings[VHOST_VRING_IDX_TX(*queue)];
...
vhost_user_delete_if(vnet_main_t *vnm, vlib_main_t *vm, u32 sw_if_index)
{
    ...
    if (!(hwif = vnet_get_sup_hw_interface(vnm, sw_if_index)) || hwif->dev_class_index != vhost_user_device_class.index)
    ...
    vec_foreach(queue, vui->rx_queues)
    {
        ...
        txvq = &vui->vrings[VHOST_VRING_IDX_TX(*queue)];
        if ((vum->ifq_count > 0) && ((txvq->mode == VNET_HW_INTERFACE_RX_MODE_INTERRUPT) ||
                                     (txvq->mode == VNET_HW_INTERFACE_RX_MODE_ADAPTIVE))) {
            vum->ifq_count--;
            // Stop the timer if there is no more interrupt interface/queue
            if ((vum->ifq_count == 0) && (vum->coalesce_time > 0.0) && (vum->coalesce_frames > 0)) {
...
vhost_user_exit(vlib_main_t *vm)
{
    ...
    vlib_worker_thread_barrier_sync(vlib_get_main());
    /* *INDENT-OFF* */
    pool_foreach(vui, vum->vhost_user_interfaces, { vhost_user_delete_if(vnm, vm, vui->sw_if_index); });
    /* *INDENT-ON* */
    vlib_worker_thread_barrier_release(vlib_get_main());
    return 0;
}
 ...
 */
static void
vhost_user_create_ethernet(vnet_main_t *vnm, vlib_main_t *vm, vhost_user_intf_t *vui, u8 *hwaddress)
{
    vhost_user_main_t *vum = &vhost_user_main;
    u8 hwaddr[6];
    clib_error_t *error;
    ...
    error = ethernet_register_interface(vnm, vhost_user_device_class.index,
 ...
 */
static void
vhost_user_vui_init(vnet_main_t *vnm, vhost_user_intf_t *vui, int server_sock_fd, const char *sock_filename,
                    u64 feature_mask, u32 *sw_if_index)
{
    ...
    for (q = 0; q < VHOST_VRING_MAX_N; q++) {
        vui->vring_locks[q] = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
...
int
vhost_user_create_if(vnet_main_t *vnm, vlib_main_t *vm, const char *sock_filename, u8 is_server, u32 *sw_if_index,
                     u64 feature_mask, u8 renumber, u32 custom_dev_instance, u8 *hwaddr)
{
    ...
    if (is_server) {
        if ((rv = vhost_user_init_server_sock(sock_filename, &server_sock_fd)) != 0) {
            return rv;
        }
    }
    ...
    vhost_user_vui_init(vnm, vui, server_sock_fd, sock_filename, feature_mask, &sw_if_idx);
...
int
vhost_user_modify_if(vnet_main_t *vnm, vlib_main_t *vm, const char *sock_filename, u8 is_server, u32 sw_if_index,
...
clib_error_t *
vhost_user_connect_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    int rv;
    if ((rv = vhost_user_create_if(vnm, vm, (char *) sock_filename, is_server, &sw_if_index, feature_mask, renumber,
                                   custom_dev_instance, hw))) {
...
clib_error_t *
vhost_user_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    u32 sw_if_index     = ~0;
    vnet_main_t *vnm    = vnet_get_main();
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "sw_if_index %d", &sw_if_index))
            ...
            vnet_hw_interface_t *hwif = vnet_get_sup_hw_interface(vnm, sw_if_index);
...
int
vhost_user_dump_ifs(vnet_main_t *vnm, vlib_main_t *vm, vhost_user_intf_details_t **out_vuids)
{
    int rv                 = 0;
    vhost_user_main_t *vum = &vhost_user_main;
    vhost_user_intf_t *vui;
    ...
    pool_foreach(vui, vum->vhost_user_interfaces, vec_add1(hw_if_indices, vui->hw_if_index););
    ...
    for (i = 0; i < vec_len(hw_if_indices); i++) {
        ...
        strncpy((char *) vuid->sock_filename, (char *) vui->sock_filename, sizeof(vuid->sock_filename));
        vuid->sock_filename[ARRAY_LEN(vuid->sock_filename) - 1] = '\0';
        s                                                       = format(s, "%v%c", hi->name, 0);

        strncpy((char *) vuid->if_name, (char *) s, ARRAY_LEN(vuid->if_name) - 1);
        _vec_len(s) = 0;
    }
...
clib_error_t *
show_vhost_user_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
#undef _
    ...
    for (i = 0; i < vec_len(hw_if_indices); i++) {
        hi  = vnet_get_hw_interface(vnm, hw_if_indices[i]);
        vui = pool_elt_at_index(vum->vhost_user_interfaces, hi->dev_instance);
        vlib_cli_output(vm, "Interface: %U (ifindex %d)", format_vnet_hw_if_index_name, vnm, hw_if_indices[i],
                        hw_if_indices[i]);
        ...
        vlib_cli_output(vm, " socket filename %s type %s errno \"%s\"\n\n", vui->sock_filename,
                        (vui->unix_server_index != ~0) ? "server" : "client", strerror(vui->sock_errno));
        ...
        vec_foreach(queue, vui->rx_queues)
        {
            ...
            thread_index = vnet_get_device_input_thread_index(vnm, vui->hw_if_index, *queue);
        ...
        {
            vlib_cli_output(vm, "   thread %d on vring %d\n", ci, VHOST_VRING_IDX_RX(vui->per_cpu_tx_qid[ci]));
        }
        ...
        if (vui->nregions) {
            vlib_cli_output(vm, " region fd    guest_phys_addr    memory_size        userspace_addr     mmap_offset    "
                                "    mmap_addr\n");
            vlib_cli_output(vm, " ====== ===== ================== ================== ================== "
                                "================== ==================\n");
        }
        for (j = 0; j < vui->nregions; j++) {
            vlib_cli_output(vm, "  %d     %-5d 0x%016lx 0x%016lx 0x%016lx 0x%016lx 0x%016lx\n", j, vui->region_mmap_fd[j],
                            vui->regions[j].guest_phys_addr, vui->regions[j].memory_size, vui->regions[j].userspace_addr,
                            vui->regions[j].mmap_offset, pointer_to_uword(vui->region_mmap_addr[j]));
        }
        for (q = 0; q < VHOST_VRING_MAX_N; q++) {
            ...
            vlib_cli_output(vm, "  qsz %d last_avail_idx %d last_used_idx %d\n", vui->vrings[q].qsz_mask + 1,
                            vui->vrings[q].last_avail_idx, vui->vrings[q].last_used_idx);

            if (vui->vrings[q].avail && vui->vrings[q].used)
                vlib_cli_output(vm, "  avail.flags %x avail.idx %d used.flags %x used.idx %d\n",
                                vui->vrings[q].avail->flags, vui->vrings[q].avail->idx, vui->vrings[q].used->flags,
            ...
            if (show_descr) {
                ...
                for (j = 0; j < vui->vrings[q].qsz_mask + 1; j++) {
                    ...
                    vlib_cli_output(vm, "  %-5d 0x%016lx %-5d 0x%04x %-5d 0x%016lx\n", j, vui->vrings[q].desc[j].addr,
                                    vui->vrings[q].desc[j].len, vui->vrings[q].desc[j].flags, vui->vrings[q].desc[j].next,
                                    pointer_to_uword(map_guest_mem(vui, vui->vrings[q].desc[j].addr, &mem_hint)));
                }
            }
        }
        vlib_cli_output(vm, "\n");
...
vhost_user_unmap_all(void)
{
    ...
    if (vum->dont_dump_vhost_user_memory) {
        pool_foreach(vui, vum->vhost_user_interfaces, unmap_all_mem_regions(vui););
    }
}
