...
static_always_inline void
vhost_user_tx_trace(vhost_trace_t *t, vhost_user_intf_t *vui, u16 qid, vlib_buffer_t *b, vhost_user_vring_t *rxvq)
{
    vhost_user_main_t *vum = &vhost_user_main;
...
vhost_user_tx_copy(vhost_user_intf_t *vui, vhost_copy_t *cpy, u16 copy_len, u32 *map_hint)
{
    ...
    if (PREDICT_TRUE(copy_len >= 4)) {
        ...
        while (PREDICT_TRUE(copy_len >= 4)) {
            ...
            if (PREDICT_FALSE(!(dst2 = map_guest_mem(vui, cpy[2].dst, map_hint))))
                return 1;
            if (PREDICT_FALSE(!(dst3 = map_guest_mem(vui, cpy[3].dst, map_hint))))
                return 1;
...
VNET_DEVICE_CLASS_TX_FN(vhost_user_device_class)(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    qid  = VHOST_VRING_IDX_RX(*vec_elt_at_index(vui->per_cpu_tx_qid, thread_index));
    rxvq = &vui->vrings[qid];
    ...
    while (n_left > 0) {
        ...
        if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
            vum->cpus[thread_index].current_trace =
                vlib_add_trace(vm, node, b0, sizeof(*vum->cpus[thread_index].current_trace));
            vhost_user_tx_trace(vum->cpus[thread_index].current_trace, vui, qid / 2, b0, rxvq);
        }
...

        {
            ...
            virtio_net_hdr_mrg_rxbuf_t *hdr = &vum->cpus[thread_index].tx_headers[tx_headers_len];
            tx_headers_len++;
            hdr->hdr.flags    = 0;
            hdr->hdr.gso_type = 0;
        ...
        while (1) {
            if (buffer_len == 0) {   // Get new output
                ...
                {
                    virtio_net_hdr_mrg_rxbuf_t *hdr = &vum->cpus[thread_index].tx_headers[tx_headers_len - 1];
        ...
        if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
            vum->cpus[thread_index].current_trace->hdr = vum->cpus[thread_index].tx_headers[tx_headers_len - 1];
        }
        ...
        if (PREDICT_FALSE(copy_len >= VHOST_USER_TX_COPY_THRESHOLD)) {
            if (PREDICT_FALSE(vhost_user_tx_copy(vui, vum->cpus[thread_index].copy, copy_len, &map_hint))) {
...
vhost_user_interface_rx_mode_change(vnet_main_t *vnm, u32 hw_if_index, u32 qid, vnet_hw_interface_rx_mode mode)
{
    ...
    if ((mode == VNET_HW_INTERFACE_RX_MODE_INTERRUPT) || (mode == VNET_HW_INTERFACE_RX_MODE_ADAPTIVE)) {
        ...
        if (((txvq->mode == VNET_HW_INTERFACE_RX_MODE_INTERRUPT) || (txvq->mode == VNET_HW_INTERFACE_RX_MODE_ADAPTIVE)) &&
            vum->ifq_count) {
            vum->ifq_count--;
    ...
    else if ((mode == VNET_HW_INTERFACE_RX_MODE_ADAPTIVE) || (mode == VNET_HW_INTERFACE_RX_MODE_INTERRUPT))
        txvq->used->flags = 0;
    else {
...
vhost_user_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (link_old != link_new)
        vnet_hw_interface_set_flags(vnm, vui->hw_if_index, link_new ? VNET_HW_INTERFACE_FLAG_LINK_UP : 0);
