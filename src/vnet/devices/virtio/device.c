...
add_buffer_to_slot(vlib_main_t *vm, virtio_vring_t *vring, u32 bi, u16 avail, u16 next, u16 mask)
{
    u16 n_added      = 0;
...
virtio_interface_tx_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, virtio_if_t *vif)
{
    u8 qid                = 0;
    ...
    if (n_left) {
        vlib_error_count(vm, node->node_index, TAP_TX_ERROR_NO_FREE_SLOTS, n_left);
...
static uword
virtio_interface_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static void
virtio_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
...
static clib_error_t *
virtio_interface_rx_mode_change(vnet_main_t *vnm, u32 hw_if_index, u32 qid, vnet_hw_interface_rx_mode mode)
{
    virtio_main_t *mm       = &virtio_main;
    vnet_hw_interface_t *hw = vnet_get_hw_interface(vnm, hw_if_index);
...
static clib_error_t *
virtio_subif_add_del_function(vnet_main_t *vnm, u32 hw_if_index, struct vnet_sw_interface_t *st, int is_add)
{
    /* Nothing for now */
    return 0;
}
...
VLIB_DEVICE_TX_FUNCTION_MULTIARCH(virtio_device_class, virtio_interface_tx)
/* *INDENT-ON* */
