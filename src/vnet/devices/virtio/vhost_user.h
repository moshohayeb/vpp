...
#define vu_log_debug(dev, f, ...)                                                                                      \
    {                                                                                                                  \
        vlib_log(VLIB_LOG_LEVEL_DEBUG, vhost_user_main.log_default, "%U: " f, format_vnet_hw_if_index_name,            \
                 vnet_get_main(), dev->hw_if_index, ##__VA_ARGS__);                                                    \
    };

#define vu_log_warn(dev, f, ...)                                                                                       \
    {                                                                                                                  \
        vlib_log(VLIB_LOG_LEVEL_WARNING, vhost_user_main.log_default, "%U: " f, format_vnet_hw_if_index_name,          \
                 vnet_get_main(), dev->hw_if_index, ##__VA_ARGS__);                                                    \
    };
#define vu_log_err(dev, f, ...)                                                                                        \
    {                                                                                                                  \
        vlib_log(VLIB_LOG_LEVEL_ERR, vhost_user_main.log_default, "%U: " f, format_vnet_hw_if_index_name,              \
                 vnet_get_main(), dev->hw_if_index, ##__VA_ARGS__);                                                    \
    };
...
int vhost_user_create_if(vnet_main_t *vnm, vlib_main_t *vm, const char *sock_filename, u8 is_server, u32 *sw_if_index,
                         u64 feature_mask, u8 renumber, u32 custom_dev_instance, u8 *hwaddr);
int vhost_user_modify_if(vnet_main_t *vnm, vlib_main_t *vm, const char *sock_filename, u8 is_server, u32 sw_if_index,
