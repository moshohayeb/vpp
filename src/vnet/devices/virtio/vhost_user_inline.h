...
static_always_inline void
vhost_user_log_dirty_pages_2(vhost_user_intf_t *vui, u64 addr, u64 len, u8 is_host_address)
{
    if (PREDICT_TRUE(vui->log_base_addr == 0 || !(vui->features & (1 << FEAT_VHOST_F_LOG_ALL)))) {
        return;
    }
    if (is_host_address) {
...
format_vhost_trace(u8 *s, va_list *va)
{
    ...
    vhost_user_intf_t *vui          = pool_elt_at_index(vum->vhost_user_interfaces, t->device_index);
...
#undef _
    ...
    if (vui->virtio_net_hdr_sz == 12)
        s = format(s, "%U   num_buff %u", format_white_space, indent, t->hdr.num_buffers);
