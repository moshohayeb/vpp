...
#define VNET_DEVICE_INPUT_NEXT_NODES                                                                                   \
    {                                                                                                                  \
        ...
        [VNET_DEVICE_INPUT_NEXT_IP4_INPUT] = "ip4-input", [VNET_DEVICE_INPUT_NEXT_IP6_INPUT] = "ip6-input",            \
...
#define foreach_device_and_queue(var, vec)                                                                             \
    for (var = (vec); var < vec_end(vec); var++)                                                                       \
        if ((var->mode == VNET_HW_INTERFACE_RX_MODE_POLLING) || clib_smp_swap(&((var)->interrupt_pending), 0))
