...
static uword
device_input_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return 0;
}
...
device_input_next_node_advance[((VNET_DEVICE_INPUT_N_NEXT_NODES / CLIB_CACHE_LINE_BYTES) + 1) * CLIB_CACHE_LINE_BYTES] = {
...
const u32 __attribute__((aligned(CLIB_CACHE_LINE_BYTES)))
device_input_next_node_flags[((VNET_DEVICE_INPUT_N_NEXT_NODES / CLIB_CACHE_LINE_BYTES) + 1) * CLIB_CACHE_LINE_BYTES] = {
...
void
vnet_hw_interface_assign_rx_thread(vnet_main_t *vnm, u32 hw_if_index, u16 queue_id, uword thread_index)
{
    ...
    if (thread_index != 0 &&
        (thread_index < vdm->first_worker_thread_index || thread_index > vdm->last_worker_thread_index)) {
...
int
vnet_hw_interface_unassign_rx_thread(vnet_main_t *vnm, u32 hw_if_index, u16 queue_id)
{
    vlib_main_t *vm, *vm0;
    vnet_hw_interface_t *hw = vnet_get_hw_interface(vnm, hw_if_index);
    vnet_device_input_runtime_t *rt;
    ...
    vec_foreach(dq, rt->devices_and_queues) if (dq->hw_if_index == hw_if_index && dq->queue_id == queue_id)
    {
        mode = dq->mode;
        goto delete;
    }
...
int
vnet_hw_interface_set_rx_mode(vnet_main_t *vnm, u32 hw_if_index, u16 queue_id, vnet_hw_interface_rx_mode mode)
{
    vlib_main_t *vm;
    uword thread_index;
    vnet_device_and_queue_t *dq;
    ...
    if (mode != VNET_HW_INTERFACE_RX_MODE_POLLING && (hw->flags & VNET_HW_INTERFACE_FLAG_SUPPORTS_INT_MODE) == 0)
        return VNET_API_ERROR_UNSUPPORTED;

    if ((vec_len(hw->input_node_thread_index_by_queue) < queue_id + 1) || (vec_len(hw->rx_mode_by_queue) < queue_id + 1))
    ...
    if (rt->enabled_node_state != enabled_node_state) {
        ...
        if (vlib_node_get_state(vm, hw->input_node_index) != VLIB_NODE_STATE_DISABLED)
...
int
vnet_hw_interface_get_rx_mode(vnet_main_t *vnm, u32 hw_if_index, u16 queue_id, vnet_hw_interface_rx_mode *mode)
{
    vlib_main_t *vm;
    uword thread_index;
    vnet_device_and_queue_t *dq;
