...
{
    if (svm_fifo_set_event(s->server_tx_fifo))
        session_send_io_evt_to_thread_custom(s, s->thread_index, FIFO_EVENT_BUILTIN_TX);
    return 0;
}
...
tls_notify_app_accept(tls_ctx_t *ctx)
{
    ...
    session_lookup_add_connection(&ctx->connection, session_handle(app_session));
...
tls_notify_app_connected(tls_ctx_t *ctx, u8 is_failed)
{
    ...
    if (cb_fn(ctx->parent_app_index, ctx->parent_app_api_context, app_session, 0 /* not failed */)) {
...
failed:
    tls_disconnect(ctx->tls_ctx_handle, vlib_get_thread_index());
    return cb_fn(ctx->parent_app_index, ctx->parent_app_api_context, 0, 1 /* failed */);
}
...
int
tls_session_connected_callback(u32 tls_app_index, u32 ho_ctx_index, stream_session_t *tls_session, u8 is_fail)
{
    ...
    TLS_DBG(1, "TCP connect for %u returned %u. New connection [%u]%x", ho_ctx_index, is_fail, vlib_get_thread_index(),
            (ctx) ? ctx_handle : ~0);
...
tls_stop_listen(u32 lctx_index)
{
    ...
    vnet_unbind_args_t a = {
        .handle = lctx->tls_session_handle, .app_index = tls_main.app_index, .wrk_map_index = 0 /* default wrk */
    };
    if (vnet_unbind(&a))
...
format_tls_ctx(u8 *s, va_list *args)
{
    ...
    s = format(s, "[#%d][TLS] app %u child %u", child_ti, ctx->parent_app_index, child_si);
    return s;
}
...
format_tls_listener(u8 *s, va_list *args)
{
    ...
    listen_session_parse_handle(ctx->tls_session_handle, &listener_index, &thread_index);
    return format(s, "[TLS] listener app %u child %u", ctx->parent_app_index, listener_index);
}
