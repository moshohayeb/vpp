...
static void
fib_entry_src_copy_default(const fib_entry_src_t *orig_src, const fib_entry_t *fib_entry, fib_entry_src_t *copy_src)
{
    clib_memcpy(&copy_src->u, &orig_src->u, sizeof(copy_src->u));
}

void
fib_entry_src_register(fib_source_t source, const fib_entry_src_vft_t *vft)
{
...
static int
fib_entry_src_cmp_for_sort(void *v1, void *v2)
{
...
static void
fib_entry_src_action_init(fib_entry_t *fib_entry, fib_source_t source, fib_entry_flag_t flags)
{
    ...
    vec_sort_with_function(fib_entry->fe_srcs, fib_entry_src_cmp_for_sort);
}

static fib_entry_src_t *
fib_entry_src_find_i(const fib_entry_t *fib_entry, fib_source_t source, u32 *index)
...
static fib_entry_src_t *
fib_entry_src_find(const fib_entry_t *fib_entry, fib_source_t source)
...
int
fib_entry_is_sourced(fib_node_index_t fib_entry_index, fib_source_t source)
{
...
static fib_entry_src_t *
fib_entry_src_find_or_create(fib_entry_t *fib_entry, fib_source_t source, fib_entry_flag_t flags)
{
    fib_entry_src_t *esrc;
...
static void
fib_entry_src_action_deinit(fib_entry_t *fib_entry, fib_source_t source)
...
fib_entry_src_action_cover_change(fib_entry_t *fib_entry, fib_entry_src_t *esrc)
{
    FIB_ENTRY_SRC_VFT_INVOKE_AND_RETURN(esrc, fesv_cover_change, (esrc, fib_entry));
...
fib_entry_src_cover_res_t
fib_entry_src_action_cover_update(fib_entry_t *fib_entry, fib_entry_src_t *esrc)
{
    FIB_ENTRY_SRC_VFT_INVOKE_AND_RETURN(esrc, fesv_cover_update, (esrc, fib_entry));
...
fib_entry_calc_lb_flags(fib_entry_src_collect_forwarding_ctx_t *ctx)
{
     ...
     */
    if (ctx->n_recursive_constrained > 1 && fib_path_list_is_popular(ctx->esrc->fes_pl)) {
...
{
    return ((MPLS_LABEL_IS_REAL(label) || MPLS_LABEL_POP == label || MPLS_IETF_IPV4_EXPLICIT_NULL_LABEL == label ||
             MPLS_IETF_IPV6_EXPLICIT_NULL_LABEL == label || MPLS_IETF_IMPLICIT_NULL_LABEL == label));
}
...
fib_entry_chain_type_fixup(const fib_entry_t *entry, fib_forward_chain_type_t fct)
{
    /*
    ...
    if (FIB_FORW_CHAIN_TYPE_MPLS_EOS == dfct) {
         ...
         */
        return (fib_forw_chain_type_from_dpo_proto(entry->fe_prefix.fp_payload_proto));
    }
    /*
...
static void
fib_entry_src_get_path_forwarding(fib_node_index_t path_index, fib_entry_src_collect_forwarding_ctx_t *ctx)
{
    ...
    case FIB_FORW_CHAIN_TYPE_MPLS_NON_EOS:
        if (fib_path_is_exclusive(path_index) || fib_path_is_deag(path_index)) {
            vec_add2(ctx->next_hops, nh, 1);

            nh->path_index  = path_index;
            nh->path_weight = fib_path_get_weight(path_index);
            fib_path_contribute_forwarding(path_index, FIB_FORW_CHAIN_TYPE_MPLS_NON_EOS, &nh->path_dpo);
        }
        break;
    case FIB_FORW_CHAIN_TYPE_MPLS_EOS: {
        ...
        nh->path_index  = path_index;
        nh->path_weight = fib_path_get_weight(path_index);
        fib_path_contribute_forwarding(path_index, fib_entry_chain_type_fixup(ctx->fib_entry, ctx->fct), &nh->path_dpo);
        fib_path_stack_mpls_disp(path_index, fib_prefix_get_payload_proto(&ctx->fib_entry->fe_prefix),
                                 FIB_MPLS_LSP_MODE_PIPE, &nh->path_dpo);
...
fib_entry_src_collect_forwarding(fib_node_index_t pl_index, fib_node_index_t path_index, void *arg)
{
     ...
     */
    path_ext = fib_path_ext_list_find_by_path_index(&ctx->esrc->fes_path_exts, path_index);
    ...
    if (NULL != path_ext) {
        ...
        case FIB_PATH_EXT_MPLS:
            if (fib_entry_src_valid_out_label(path_ext->fpe_label_stack[0].fml_value)) {
                 ...
                 * info for this path.
                 */
                ctx->next_hops = fib_path_ext_stack(path_ext, ctx->fct,
                                                    fib_entry_chain_type_fixup(ctx->fib_entry, ctx->fct), ctx->next_hops);
            } else {
    ...
    if (n_nhs < vec_len(ctx->next_hops)) {
        ...
        if (NULL != vft->fesv_contribute_interpose) {
            ...
            interposer = vft->fesv_contribute_interpose(ctx->esrc, ctx->fib_entry);

            if (NULL != interposer) {
                dpo_id_t clone = DPO_INVALID;

                dpo_mk_interpose(interposer, &ctx->next_hops[n_nhs].path_dpo, &clone);
...
void
fib_entry_src_mk_lb(fib_entry_t *fib_entry, const fib_entry_src_t *esrc, fib_forward_chain_type_t fct, dpo_id_t *dpo_lb)
{
    ...
    fib_path_list_walk(esrc->fes_pl, fib_entry_src_collect_forwarding, &ctx);
    ...
    if (esrc->fes_entry_flags & FIB_ENTRY_FLAG_EXCLUSIVE) {
         ...
         * then we can use it.
         */
        if ((1 == vec_len(ctx.next_hops)) && (DPO_LOAD_BALANCE == ctx.next_hops[0].path_dpo.dpoi_type)) {
    ...
    if (!dpo_id_is_valid(dpo_lb)) {
        ...
        if (esrc->fes_entry_flags & FIB_ENTRY_FLAG_MULTICAST) {
            dpo_set(dpo_lb, DPO_REPLICATE, lb_proto, MPLS_IS_REPLICATE | replicate_create(0, lb_proto));
        } else {
            ...
            if (fib_entry->fe_prefix.fp_proto != flow_hash_proto) {
                ...
                fhc = fib_table_get_flow_hash_config(fib_entry->fe_fib_index, flow_hash_proto);
            }
    ...
    if (esrc->fes_entry_flags & FIB_ENTRY_FLAG_MULTICAST) {
        ...
        load_balance_multipath_update(dpo_lb, ctx.next_hops, fib_entry_calc_lb_flags(&ctx));
        ...
        if ((fib_entry_is_sourced(fib_entry_get_index(fib_entry), FIB_SOURCE_URPF_EXEMPT) ||
        ...
        load_balance_set_fib_entry_flags(dpo_lb->dpoi_index, fib_entry_get_flags_i(fib_entry));
    }
}

void
fib_entry_src_action_install(fib_entry_t *fib_entry, fib_source_t source)
{
    /*
     ...
     */
    if (insert) {
        fib_table_fwding_dpo_update(fib_entry->fe_fib_index, &fib_entry->fe_prefix, &fib_entry->fe_lb);
    }
    ...
    FOR_EACH_DELEGATE_CHAIN(fib_entry, fdt, fed, {
        fib_entry_src_mk_lb(fib_entry, esrc, fib_entry_delegate_type_to_chain_type(fdt), &fed->fd_dpo);
    });
}
...
fib_entry_src_action_uninstall(fib_entry_t *fib_entry)
{
     ...
     */
    FIB_ENTRY_DBG(fib_entry, "uninstall: %d", fib_entry->fe_adj_index);

    if (dpo_id_is_valid(&fib_entry->fe_lb)) {
        fib_table_fwding_dpo_remove(fib_entry->fe_fib_index, &fib_entry->fe_prefix, &fib_entry->fe_lb);
 ...
 */
static fib_entry_t *
fib_entry_src_action_copy(fib_entry_t *fib_entry, const fib_entry_src_t *orig_src)
{
    fib_entry_src_t *esrc;

    esrc = fib_entry_src_find_or_create(fib_entry, orig_src->fes_src, orig_src->fes_entry_flags);

    FIB_ENTRY_SRC_VFT_INVOKE(esrc, fesv_copy, (orig_src, fib_entry, esrc));
    ...
    esrc->fes_flags &= ~(FIB_ENTRY_SRC_FLAG_ACTIVE | FIB_ENTRY_SRC_FLAG_CONTRIBUTING);
 ...
 */
static fib_entry_src_t *
fib_entry_src_action_update_from_cover(fib_entry_t *fib_entry, const fib_entry_src_t *orig_src)
{
    fib_entry_src_t *esrc;
...
fib_entry_src_covered_inherit_add_i(fib_entry_t *fib_entry, const fib_entry_src_t *cover_src)
{
    fib_entry_src_t *esrc;
    ...
    if (NULL != esrc) {
        ...
        best_source = fib_entry_get_best_source(fib_entry_get_index(fib_entry));
...
static fib_table_walk_rc_t
fib_entry_src_covered_inherit_walk_add(fib_node_index_t fei, void *ctx)
{
    return (fib_entry_src_covered_inherit_add_i(fib_entry_get(fei), ctx));
}

static fib_table_walk_rc_t
fib_entry_src_covered_inherit_walk_remove(fib_node_index_t fei, void *ctx)
{
...
void
fib_entry_src_inherit(const fib_entry_t *cover, fib_entry_t *covered)
{
...
static void
fib_entry_src_covered_inherit_add(fib_entry_t *fib_entry, fib_source_t source)

{
    ...
    if ((esrc->fes_entry_flags & FIB_ENTRY_FLAG_COVERED_INHERIT) || (esrc->fes_flags & FIB_ENTRY_SRC_FLAG_INHERITED)) {
        fib_table_sub_tree_walk(fib_entry->fe_fib_index, fib_entry->fe_prefix.fp_proto, &fib_entry->fe_prefix,
                                fib_entry_src_covered_inherit_walk_add, esrc);
    }
}

static void
fib_entry_src_covered_inherit_remove(fib_entry_t *fib_entry, fib_entry_src_t *esrc)

{
    ...
    if (esrc->fes_entry_flags & FIB_ENTRY_FLAG_COVERED_INHERIT) {
        fib_table_sub_tree_walk(fib_entry->fe_fib_index, fib_entry->fe_prefix.fp_proto, &fib_entry->fe_prefix,
                                fib_entry_src_covered_inherit_walk_remove, esrc);
    }
}

void
fib_entry_src_action_activate(fib_entry_t *fib_entry, fib_source_t source)

{
    ...
    fib_entry->fe_sibling =
        fib_path_list_child_add(fib_entry->fe_parent, FIB_NODE_TYPE_ENTRY, fib_entry_get_index(fib_entry));

    fib_entry_recursive_loop_detect_i(fib_entry->fe_parent);

    FIB_ENTRY_DBG(fib_entry, "activate: %d", fib_entry->fe_parent);
...
void
fib_entry_src_action_deactivate(fib_entry_t *fib_entry, fib_source_t source)

{
    ...
    FIB_ENTRY_SRC_VFT_INVOKE(esrc, fesv_deactivate, (esrc, fib_entry));
...
static void
fib_entry_src_action_fwd_update(const fib_entry_t *fib_entry, fib_source_t source)
{
    fib_entry_src_t *esrc;

    vec_foreach(esrc, fib_entry->fe_srcs)
    {
        FIB_ENTRY_SRC_VFT_INVOKE(esrc, fesv_fwd_update, (esrc, fib_entry, source));
    }
}

void
fib_entry_src_action_reactivate(fib_entry_t *fib_entry, fib_source_t source)
{
    fib_node_index_t path_list_index;
    const fib_entry_src_vft_t *vft;
    fib_entry_src_t *esrc;
    ...
    FIB_ENTRY_DBG(fib_entry, "reactivate: %d to %d", fib_entry->fe_parent, esrc->fes_pl);
...
void
fib_entry_src_action_installed(const fib_entry_t *fib_entry, fib_source_t source)
{
    fib_entry_src_t *esrc;

    esrc = fib_entry_src_find(fib_entry, source);

    FIB_ENTRY_SRC_VFT_INVOKE(esrc, fesv_installed, (esrc, fib_entry));
 ...
 */
fib_entry_t *
fib_entry_src_action_add(fib_entry_t *fib_entry, fib_source_t source, fib_entry_flag_t flags, const dpo_id_t *dpo)
{
    ...
    if (flags != esrc->fes_entry_flags) {
        FIB_ENTRY_SRC_VFT_INVOKE(esrc, fesv_flags_change, (esrc, fib_entry, flags));
    }
    ...
    FIB_ENTRY_SRC_VFT_INVOKE(esrc, fesv_add, (esrc, fib_entry, flags, fib_entry_get_dpo_proto(fib_entry), dpo));
 ...
 */
fib_entry_t *
fib_entry_src_action_update(fib_entry_t *fib_entry, fib_source_t source, fib_entry_flag_t flags, const dpo_id_t *dpo)
{
...
fib_entry_src_action_remove_or_update_inherit(fib_entry_t *fib_entry, fib_source_t source)
{
    fib_entry_src_t *esrc;
    ...
    if ((esrc->fes_entry_flags & FIB_ENTRY_FLAG_COVERED_INHERIT) && (esrc->fes_flags & FIB_ENTRY_SRC_FLAG_INHERITED)) {
         ...
         */
        coveri = fib_table_get_less_specific(fib_entry->fe_fib_index, &fib_entry->fe_prefix);
...
fib_entry_src_flag_t
fib_entry_src_action_remove(fib_entry_t *fib_entry, fib_source_t source)
 ...
 */
static inline int
fib_route_attached_cross_table(const fib_entry_t *fib_entry, const fib_route_path_t *rpath)
{
    /*
     * - All zeros next-hop
     * - a valid interface
     ...
     */
    if (ip46_address_is_zero(&rpath->frp_addr) && (~0 != rpath->frp_sw_if_index) &&
        !(rpath->frp_flags & FIB_ROUTE_PATH_DVR) &&
        (fib_entry->fe_fib_index !=
         fib_table_get_index_for_sw_if_index(fib_entry_get_proto(fib_entry), rpath->frp_sw_if_index))) {
        return (!0);
    }
    return (0);
}
...
static void
fib_entry_flags_update(const fib_entry_t *fib_entry, const fib_route_path_t *rpath, fib_path_list_flags_t *pl_flags,
                       fib_entry_src_t *esrc)
{
    if ((esrc->fes_src == FIB_SOURCE_API) || (esrc->fes_src == FIB_SOURCE_CLI)) {
    ...
    if (fib_route_attached_cross_table(fib_entry, rpath) && !(esrc->fes_entry_flags & FIB_ENTRY_FLAG_NO_ATTACHED_EXPORT)) {
 ...
 */
fib_entry_t *
fib_entry_src_action_path_add(fib_entry_t *fib_entry, fib_source_t source, fib_entry_flag_t flags,
                              const fib_route_path_t *rpath)
{
    ...
    esrc = fib_entry_src_find(fib_entry, source);
    if (NULL == esrc) {
        fib_entry = fib_entry_src_action_add(fib_entry, source, flags, drop_dpo_get(fib_entry_get_dpo_proto(fib_entry)));
        esrc      = fib_entry_src_find(fib_entry, source);
    }
    ...
    FIB_ENTRY_SRC_VFT_INVOKE(esrc, fesv_path_add, (esrc, fib_entry, pl_flags, rpath));
    fib_entry = fib_entry_get(fib_entry_index);
 ...
 */
fib_entry_t *
fib_entry_src_action_path_swap(fib_entry_t *fib_entry, fib_source_t source, fib_entry_flag_t flags,
                               const fib_route_path_t *rpaths)
{
    ...
    FIB_ENTRY_SRC_VFT_INVOKE(esrc, fesv_path_swap, (esrc, fib_entry, pl_flags, rpaths));
...
fib_entry_src_flag_t
fib_entry_src_action_path_remove(fib_entry_t *fib_entry, fib_source_t source, const fib_route_path_t *rpath)
{
    fib_path_list_flags_t pl_flags;
    fib_node_index_t old_path_list;
    fib_entry_src_t *esrc;
    ...
    FIB_ENTRY_SRC_VFT_INVOKE(esrc, fesv_path_remove, (esrc, pl_flags, rpath));
...
u8 *
fib_entry_src_format(fib_entry_t *fib_entry, fib_source_t source, u8 *s)
{
    fib_entry_src_t *esrc;
...
adj_index_t
fib_entry_get_adj_for_source(fib_node_index_t fib_entry_index, fib_source_t source)
{
    fib_entry_t *fib_entry;
    fib_entry_src_t *esrc;
    ...
    if (NULL != esrc) {
        if (FIB_NODE_INDEX_INVALID != esrc->fes_pl) {
            return (fib_path_list_get_adj(esrc->fes_pl, fib_entry_get_default_chain_type(fib_entry)));
        }
    }
    return (ADJ_INDEX_INVALID);
}
...
const int
fib_entry_get_dpo_for_source(fib_node_index_t fib_entry_index, fib_source_t source, dpo_id_t *dpo)
{
    fib_entry_t *fib_entry;
    fib_entry_src_t *esrc;
    ...
    if (NULL != esrc) {
        if (FIB_NODE_INDEX_INVALID != esrc->fes_pl) {
            fib_path_list_contribute_forwarding(esrc->fes_pl, fib_entry_get_default_chain_type(fib_entry),
...
u32
fib_entry_get_resolving_interface_for_source(fib_node_index_t entry_index, fib_source_t source)
{
    fib_entry_t *fib_entry;
    fib_entry_src_t *esrc;
...
fib_entry_flag_t
fib_entry_get_flags_for_source(fib_node_index_t entry_index, fib_source_t source)
{
    fib_entry_t *fib_entry;
    fib_entry_src_t *esrc;
...
void
fib_entry_set_source_data(fib_node_index_t fib_entry_index, fib_source_t source, const void *data)
{
    fib_entry_t *fib_entry;
    fib_entry_src_t *esrc;
    ...
    if (NULL != esrc) {
        FIB_ENTRY_SRC_VFT_INVOKE(esrc, fesv_set_data, (esrc, fib_entry, data));
    }
}

const void *
fib_entry_get_source_data(fib_node_index_t fib_entry_index, fib_source_t source)
{
    fib_entry_t *fib_entry;
    fib_entry_src_t *esrc;
    ...
    if (NULL != esrc) {
        FIB_ENTRY_SRC_VFT_INVOKE_AND_RETURN(esrc, fesv_get_data, (esrc, fib_entry));
    }
    return (NULL);
}
