...
static void
fib_entry_src_lisp_path_swap(fib_entry_src_t *src, const fib_entry_t *entry, fib_path_list_flags_t pl_flags,
                             const fib_route_path_t *paths)
{
    src->fes_pl = fib_path_list_create((FIB_PATH_LIST_FLAG_SHARED | pl_flags), paths);
}

static void
fib_entry_src_lisp_path_add(fib_entry_src_t *src, const fib_entry_t *entry, fib_path_list_flags_t pl_flags,
                            const fib_route_path_t *paths)
{
    if (FIB_NODE_INDEX_INVALID == src->fes_pl) {
        src->fes_pl = fib_path_list_create((FIB_PATH_LIST_FLAG_SHARED | pl_flags), paths);
    } else {
        src->fes_pl = fib_path_list_copy_and_path_add(src->fes_pl, (FIB_PATH_LIST_FLAG_SHARED | pl_flags), paths);
    }
}

static void
fib_entry_src_lisp_path_remove(fib_entry_src_t *src, fib_path_list_flags_t pl_flags, const fib_route_path_t *paths)
{
    if (FIB_NODE_INDEX_INVALID != src->fes_pl) {
        src->fes_pl = fib_path_list_copy_and_path_remove(src->fes_pl, (FIB_PATH_LIST_FLAG_SHARED | pl_flags), paths);
    }
}

static void
fib_entry_src_lisp_add(fib_entry_src_t *src, const fib_entry_t *entry, fib_entry_flag_t flags, dpo_proto_t proto,
                       const dpo_id_t *dpo)
{
...
static void
fib_entry_src_lisp_set_data(fib_entry_src_t *src, const fib_entry_t *entry, const void *data)
{
    src->u.lisp.fesl_fib_index = *(u32 *) data;
}

static const void *
fib_entry_src_lisp_get_data(fib_entry_src_t *src, const fib_entry_t *entry)
{
