...
format_fib_path_ext(u8 *s, va_list *args)
{
    ...
    case FIB_PATH_EXT_MPLS: {
        ...
        for (ii = 0; ii < vec_len(path_ext->fpe_path.frp_label_stack); ii++) {
            s = format(s, "[%U]", format_fib_mpls_label, &path_ext->fpe_path.frp_label_stack[ii]);
        }
        s = format(s, "]");
        break;
    }
...
int
fib_path_ext_cmp(fib_path_ext_t *path_ext, const fib_route_path_t *rpath)
{
    return (fib_route_path_cmp(&path_ext->fpe_path, rpath));
}

static fib_path_list_walk_rc_t
fib_path_ext_match(fib_node_index_t pl_index, fib_node_index_t path_index, void *ctx)
{
    fib_path_ext_t *path_ext = ctx;

    if (!fib_path_cmp_w_route_path(path_index, &path_ext->fpe_path)) {
...
void
fib_path_ext_resolve(fib_path_ext_t *path_ext, fib_node_index_t path_list_index)
{
    /*
     * Find the path on the path list that this is an extension for
     */
    path_ext->fpe_path_index = FIB_NODE_INDEX_INVALID;
    fib_path_list_walk(path_list_index, fib_path_ext_match, path_ext);
}

static void
fib_path_ext_init(fib_path_ext_t *path_ext, fib_node_index_t path_list_index, fib_path_ext_type_t ext_type,
...
load_balance_path_t *
fib_path_ext_stack(fib_path_ext_t *path_ext, fib_forward_chain_type_t child_fct, fib_forward_chain_type_t imp_null_fct,
                   load_balance_path_t *nhs)
{
     ...
     */
    fib_path_contribute_forwarding(path_ext->fpe_path_index, parent_fct, &via_dpo);
    ...
    if (dpo_is_drop(&via_dpo) || load_balance_is_drop(&via_dpo)) {
        ...
        if (!fib_path_ext_is_imp_null(path_ext)) {
            ...
            eos         = (child_fct == FIB_FORW_CHAIN_TYPE_MPLS_NON_EOS ? MPLS_NON_EOS : MPLS_EOS);
            chain_proto = fib_forw_chain_type_to_dpo_proto(child_fct);

            dpo_copy(&parent, &nh->path_dpo);
            mpls_label_dpo_create(path_ext->fpe_label_stack, eos, chain_proto,
                                  fib_path_ext_mpls_flags_to_mpls_label(path_ext->fpe_mpls_flags), &parent,
                                  &nh->path_dpo);
             ...
             */
            fib_path_stack_mpls_disp(nh->path_index, fib_forw_chain_type_to_dpo_proto(parent_fct),
                                     path_ext->fpe_label_stack[0].fml_mode, &nh->path_dpo);
        }
    }
    dpo_reset(&via_dpo);
...
fib_path_ext_t *
fib_path_ext_list_find(const fib_path_ext_list_t *list, fib_path_ext_type_t ext_type, const fib_route_path_t *rpath)
{
    fib_path_ext_t *path_ext;

    vec_foreach(path_ext, list->fpel_exts)
    {
        if ((path_ext->fpe_type == ext_type) && !fib_path_ext_cmp(path_ext, rpath)) {
            return (path_ext);
        }
    }
    return (NULL);
...
fib_path_ext_t *
fib_path_ext_list_find_by_path_index(const fib_path_ext_list_t *list, fib_node_index_t path_index)
{
    fib_path_ext_t *path_ext;
...
fib_path_ext_t *
fib_path_ext_list_push_back(fib_path_ext_list_t *list, fib_node_index_t path_list_index, fib_path_ext_type_t ext_type,
                            const fib_route_path_t *rpath)
{
    fib_path_ext_t *path_ext;
 ...
 */
fib_path_ext_t *
fib_path_ext_list_insert(fib_path_ext_list_t *list, fib_node_index_t path_list_index, fib_path_ext_type_t ext_type,
                         const fib_route_path_t *rpath)
{
    ...
    if (0 == fib_path_ext_list_length(list)) {
        return (fib_path_ext_list_push_back(list, path_list_index, ext_type, rpath));
    }
...
void
fib_path_ext_list_resolve(fib_path_ext_list_t *list, fib_node_index_t path_list_index)
{
    fib_path_ext_t *path_ext;
...
void
fib_path_ext_list_remove(fib_path_ext_list_t *list, fib_path_ext_type_t ext_type, const fib_route_path_t *rpath)
{
    fib_path_ext_t *path_ext;
