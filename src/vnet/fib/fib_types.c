...
format_fib_mpls_label(u8 *s, va_list *ap)
{
    ...
    s = format(s, "%U %U ttl:%d exp:%d", format_mpls_unicast_label, label->fml_value, format_fib_mpls_lsp_mode,
               label->fml_mode, label->fml_ttl, label->fml_exp);

    return (s);
}

void
fib_prefix_from_ip46_addr(const ip46_address_t *addr, fib_prefix_t *pfx)
{
    ASSERT(!ip46_address_is_zero(addr));

    pfx->fp_proto = ((ip46_address_is_ip4(addr) ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6));
    pfx->fp_len   = ((ip46_address_is_ip4(addr) ? 32 : 128));
    pfx->fp_addr  = *addr;
}

void
fib_prefix_from_mpls_label(mpls_label_t label, mpls_eos_bit_t eos, fib_prefix_t *pfx)
{
...
int
fib_prefix_cmp(const fib_prefix_t *p1, const fib_prefix_t *p2)
{
    int res;
...
int
fib_prefix_is_cover(const fib_prefix_t *p1, const fib_prefix_t *p2)
{
    switch (p1->fp_proto) {
    case FIB_PROTOCOL_IP4:
        return (ip4_destination_matches_route(&ip4_main, &p1->fp_addr.ip4, &p2->fp_addr.ip4, p1->fp_len));
    case FIB_PROTOCOL_IP6:
        return (ip6_destination_matches_route(&ip6_main, &p1->fp_addr.ip6, &p2->fp_addr.ip6, p1->fp_len));
    case FIB_PROTOCOL_MPLS:
        break;
    }
    return (0);
...
format_fib_prefix(u8 *s, va_list *args)
{
    ...
    case FIB_PROTOCOL_MPLS:
        s = format(s, "%U:%U", format_mpls_unicast_label, fp->fp_label, format_mpls_eos_bit, fp->fp_eos);
        break;
    }
...
int
fib_route_path_cmp(const fib_route_path_t *rpath1, const fib_route_path_t *rpath2)
{
    int res;

    res = ip46_address_cmp(&rpath1->frp_addr, &rpath2->frp_addr);
...
unformat_fib_route_path(unformat_input_t *input, va_list *args)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "%U %U", unformat_ip4_address, &rpath->frp_addr.ip4, unformat_vnet_sw_interface, vnm,
                     &rpath->frp_sw_if_index)) {
            rpath->frp_proto = DPO_PROTO_IP4;
        } else if (unformat(input, "%U %U", unformat_ip6_address, &rpath->frp_addr.ip6, unformat_vnet_sw_interface, vnm,
                            &rpath->frp_sw_if_index)) {
        ...
        } else if (unformat(input, "%U next-hop-table %d", unformat_ip4_address, &rpath->frp_addr.ip4,
                            &rpath->frp_fib_index)) {
            rpath->frp_sw_if_index = ~0;
            rpath->frp_proto       = DPO_PROTO_IP4;
        ...
        } else if (unformat(input, "%U next-hop-table %d", unformat_ip6_address, &rpath->frp_addr.ip6,
                            &rpath->frp_fib_index)) {
            rpath->frp_sw_if_index = ~0;
            rpath->frp_proto       = DPO_PROTO_IP6;
            ...
            if (~0 == fi)
                return 0;
            rpath->frp_fib_index = fi;
        } else if (unformat(input, "%U", unformat_ip4_address, &rpath->frp_addr.ip4)) {
            /*
            ...
            rpath->frp_sw_if_index = ~0;
            rpath->frp_proto       = DPO_PROTO_IP4;
        } else if (unformat(input, "%U", unformat_ip6_address, &rpath->frp_addr.ip6)) {
            rpath->frp_fib_index   = 0;
            rpath->frp_sw_if_index = ~0;
            rpath->frp_proto       = DPO_PROTO_IP6;
        ...
        } else if (unformat(input, "ip4-lookup-in-table %d", &rpath->frp_fib_index)) {
            rpath->frp_proto = DPO_PROTO_IP4;
            *payload_proto   = DPO_PROTO_IP4;
            ...
            if (~0 == fi)
                return 0;
            rpath->frp_fib_index = fi;
        } else if (unformat(input, "ip6-lookup-in-table %d", &rpath->frp_fib_index)) {
            rpath->frp_proto = DPO_PROTO_IP6;
            *payload_proto   = DPO_PROTO_IP6;
            ...
            if (~0 == fi)
                return 0;
            rpath->frp_fib_index = fi;
        } else if (unformat(input, "mpls-lookup-in-table %d", &rpath->frp_fib_index)) {
        ...
        } else if (unformat(input, "l2-input-on %U", unformat_vnet_sw_interface, vnm, &rpath->frp_sw_if_index)) {
            rpath->frp_proto = DPO_PROTO_ETHERNET;
            *payload_proto   = DPO_PROTO_ETHERNET;
            rpath->frp_flags |= FIB_ROUTE_PATH_INTF_RX;
        } else if (unformat(input, "via-label %U", unformat_mpls_unicast_label, &rpath->frp_local_label)) {
            rpath->frp_eos         = MPLS_NON_EOS;
            rpath->frp_proto       = DPO_PROTO_MPLS;
            rpath->frp_sw_if_index = ~0;
        } else if (unformat(input, "rx-ip4 %U", unformat_vnet_sw_interface, vnm, &rpath->frp_sw_if_index)) {
            rpath->frp_proto = DPO_PROTO_IP4;
            rpath->frp_flags = FIB_ROUTE_PATH_INTF_RX;
        } else if (unformat(input, "out-labels")) {
            while (unformat(input, "%U", unformat_mpls_unicast_label, &out_label)) {
                fib_mpls_label_t fml = {
                    .fml_value = out_label,
                };
                vec_add1(rpath->frp_label_stack, fml);
            }
        } else if (unformat(input, "%U", unformat_vnet_sw_interface, vnm, &rpath->frp_sw_if_index)) {
            rpath->frp_proto = *payload_proto;
        } else {
            return (0);
        }
