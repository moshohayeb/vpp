 ...
 */
void
fib_entry_src_rr_resolve_via_connected(fib_entry_src_t *src, const fib_entry_t *fib_entry, const fib_entry_t *cover)
{
    const fib_route_path_t path = {
        .frp_proto       = fib_proto_to_dpo(fib_entry->fe_prefix.fp_proto),
        .frp_addr        = fib_entry->fe_prefix.fp_addr,
        .frp_sw_if_index = fib_entry_get_resolving_interface(fib_entry_get_index(cover)),
        .frp_fib_index   = ~0,
        .frp_weight      = 1,
    };
    ...
    src->fes_entry_flags |= (fib_entry_get_flags(fib_entry_get_index(cover)) & FIB_ENTRY_FLAGS_RR_INHERITED);
 ...
 */
void
fib_entry_src_rr_use_covers_pl(fib_entry_src_t *src, const fib_entry_t *fib_entry, const fib_entry_t *cover)
{
    ...
    if (fib_path_list_recursive_loop_detect(cover->fe_parent, &entries)) {
        src->fes_pl = fib_path_list_create_special(proto, FIB_PATH_LIST_FLAG_DROP, drop_dpo_get(proto));
    } else {
 ...
 */
static int
fib_entry_src_rr_activate(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
    fib_entry_t *cover;
    ...
    src->u.rr.fesr_cover = fib_table_get_less_specific(fib_entry->fe_fib_index, &fib_entry->fe_prefix);

    ASSERT(FIB_NODE_INDEX_INVALID != src->u.rr.fesr_cover);

    cover = fib_entry_get(src->u.rr.fesr_cover);

    src->u.rr.fesr_sibling = fib_entry_cover_track(cover, fib_entry_get_index(fib_entry));
 ...
 */
static void
fib_entry_src_rr_deactivate(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
    fib_entry_t *cover;
...
fib_entry_src_rr_cover_change(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
     ...
     */
    if (src->u.rr.fesr_cover != fib_table_get_less_specific(fib_entry->fe_fib_index, &fib_entry->fe_prefix)) {
 ...
 */
fib_entry_src_cover_res_t
fib_entry_src_rr_cover_update(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
    fib_entry_src_cover_res_t res = {
        .install   = !0,
        .bw_reason = FIB_NODE_BW_REASON_FLAG_NONE,
...
static u8 *
fib_entry_src_rr_format(fib_entry_src_t *src, u8 *s)
{
