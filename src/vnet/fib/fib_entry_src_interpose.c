...
fib_entry_src_rr_get_next_best(const fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
 ...
 */
static int
fib_entry_src_interpose_activate(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
    ...
    if (NULL != best_src) {
        ...
        if (FIB_PROTOCOL_MPLS == fib_entry->fe_prefix.fp_proto) {
            src->fes_pl = fib_path_list_create_special(DPO_PROTO_MPLS, FIB_PATH_LIST_FLAG_DROP, NULL);
        } else {
            src->u.interpose.fesi_cover = fib_table_get_less_specific(fib_entry->fe_fib_index, &fib_entry->fe_prefix);

            ASSERT(FIB_NODE_INDEX_INVALID != src->u.interpose.fesi_cover);

            cover = fib_entry_get(src->u.interpose.fesi_cover);

            src->u.interpose.fesi_sibling = fib_entry_cover_track(cover, fib_entry_get_index(fib_entry));
 ...
 */
static void
fib_entry_src_interpose_deactivate(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
    fib_entry_t *cover;
    ...
    if (FIB_NODE_INDEX_INVALID != src->u.interpose.fesi_cover) {
        ...
        if (best_src) {
             ...
             * can provide forwarding
             */
            FIB_ENTRY_SRC_VFT_INVOKE(best_src, fesv_deactivate, (best_src, fib_entry));
        }
    }
...
static int
fib_entry_src_interpose_reactivate(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
...
fib_entry_src_interpose_cover_change(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
     ...
     */
    if (src->u.interpose.fesi_cover != fib_table_get_less_specific(fib_entry->fe_fib_index, &fib_entry->fe_prefix)) {
        fib_entry_src_interpose_deactivate(src, fib_entry);
...
static void
fib_entry_src_interpose_add(fib_entry_src_t *src, const fib_entry_t *entry, fib_entry_flag_t flags, dpo_proto_t proto,
                            const dpo_id_t *dpo)
{
...
static void
fib_entry_src_interpose_set_data(fib_entry_src_t *src, const fib_entry_t *fib_entry, const void *data)
{
 ...
 */
const dpo_id_t *
fib_entry_src_interpose_contribute(const fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
    return (&src->u.interpose.fesi_dpo);
}

static void
fib_entry_src_interpose_copy(const fib_entry_src_t *orig_src, const fib_entry_t *fib_entry, fib_entry_src_t *copy_src)
{
    ...
    if (FIB_NODE_INDEX_INVALID != copy_src->u.interpose.fesi_cover) {
        ...
        copy_src->u.interpose.fesi_sibling = fib_entry_cover_track(cover, fib_entry_get_index(fib_entry));
    }

    dpo_copy(&copy_src->u.interpose.fesi_dpo, &orig_src->u.interpose.fesi_dpo);
}

static void
fib_entry_src_interpose_flag_change(fib_entry_src_t *src, const fib_entry_t *fib_entry, fib_entry_flag_t new_flags)
{
