...
#define FIB_SOURCES                                                                                                    \
    {                                                                                                                  \
        [FIB_SOURCE_SPECIAL] = "special", [FIB_SOURCE_INTERFACE] = "interface", [FIB_SOURCE_PROXY] = "proxy",          \
        [FIB_SOURCE_BIER] = "BIER", [FIB_SOURCE_6RD] = "6RD", [FIB_SOURCE_API] = "API", [FIB_SOURCE_CLI] = "CLI",      \
        [FIB_SOURCE_ADJ] = "adjacency", [FIB_SOURCE_MAP] = "MAP", [FIB_SOURCE_SR] = "SR", [FIB_SOURCE_LISP] = "LISP",  \
        [FIB_SOURCE_CLASSIFY] = "classify", [FIB_SOURCE_DHCP] = "DHCP", [FIB_SOURCE_IP6_ND_PROXY] = "IPv6-proxy-nd",   \
        [FIB_SOURCE_IP6_ND] = "IPv6-nd", [FIB_SOURCE_RR] = "recursive-resolution",                                     \
        [FIB_SOURCE_AE] = "attached_export", [FIB_SOURCE_MPLS] = "mpls", [FIB_SOURCE_URPF_EXEMPT] = "urpf-exempt",     \
        [FIB_SOURCE_DEFAULT_ROUTE] = "default-route", [FIB_SOURCE_PLUGIN_HI] = "plugin-hi",                            \
        [FIB_SOURCE_PLUGIN_LOW] = "plugin-low", [FIB_SOURCE_INTERPOSE] = "interpose",                                  \
    }
...
#define FIB_ENTRY_ATTRIBUTES                                                                                           \
    {                                                                                                                  \
        [FIB_ENTRY_ATTRIBUTE_CONNECTED] = "connected", [FIB_ENTRY_ATTRIBUTE_ATTACHED] = "attached",                    \
        [FIB_ENTRY_ATTRIBUTE_IMPORT] = "import", [FIB_ENTRY_ATTRIBUTE_DROP] = "drop",                                  \
        [FIB_ENTRY_ATTRIBUTE_EXCLUSIVE] = "exclusive", [FIB_ENTRY_ATTRIBUTE_LOCAL] = "local",                          \
        [FIB_ENTRY_ATTRIBUTE_URPF_EXEMPT] = "uRPF-exempt", [FIB_ENTRY_ATTRIBUTE_MULTICAST] = "multicast",              \
        [FIB_ENTRY_ATTRIBUTE_NO_ATTACHED_EXPORT] = "no-attached-export",                                               \
        [FIB_ENTRY_ATTRIBUTE_COVERED_INHERIT] = "covered-inherit", [FIB_ENTRY_ATTRIBUTE_INTERPOSE] = "interpose",      \
    }

#define FOR_EACH_FIB_ATTRIBUTE(_item)                                                                                  \
    for (_item = FIB_ENTRY_ATTRIBUTE_FIRST; _item <= FIB_ENTRY_ATTRIBUTE_LAST; _item++)
...
#define FIB_ENTRY_SRC_ATTRIBUTES                                                                                       \
    {                                                                                                                  \
        [FIB_ENTRY_SRC_ATTRIBUTE_ADDED] = "added", [FIB_ENTRY_SRC_ATTRIBUTE_CONTRIBUTING] = "contributing",            \
        [FIB_ENTRY_SRC_ATTRIBUTE_ACTIVE] = "active", [FIB_ENTRY_SRC_ATTRIBUTE_INHERITED] = "inherited",                \
    }

#define FOR_EACH_FIB_SRC_ATTRIBUTE(_item)                                                                              \
    for (_item = FIB_ENTRY_SRC_ATTRIBUTE_FIRST; _item <= FIB_ENTRY_SRC_ATTRIBUTE_LAST; _item++)
...
extern fib_node_index_t fib_entry_create_special(u32 fib_index, const fib_prefix_t *prefix, fib_source_t source,
                                                 fib_entry_flag_t flags, const dpo_id_t *dpo);

extern fib_node_index_t fib_entry_create(u32 fib_index, const fib_prefix_t *prefix, fib_source_t source,
                                         fib_entry_flag_t flags, const fib_route_path_t *paths);
extern void fib_entry_update(fib_node_index_t fib_entry_index, fib_source_t source, fib_entry_flag_t flags,
                             const fib_route_path_t *paths);

extern void fib_entry_path_add(fib_node_index_t fib_entry_index, fib_source_t source, fib_entry_flag_t flags,
                               const fib_route_path_t *rpath);
extern void fib_entry_special_add(fib_node_index_t fib_entry_index, fib_source_t source, fib_entry_flag_t flags,
                                  const dpo_id_t *dpo);
extern void fib_entry_special_update(fib_node_index_t fib_entry_index, fib_source_t source, fib_entry_flag_t flags,
                                     const dpo_id_t *dpo);
extern fib_entry_src_flag_t fib_entry_special_remove(fib_node_index_t fib_entry_index, fib_source_t source);

extern fib_entry_src_flag_t fib_entry_path_remove(fib_node_index_t fib_entry_index, fib_source_t source,
                                                  const fib_route_path_t *rpath);

extern void fib_entry_inherit(fib_node_index_t cover, fib_node_index_t covered);

extern fib_entry_src_flag_t fib_entry_delete(fib_node_index_t fib_entry_index, fib_source_t source);

extern void fib_entry_recalculate_forwarding(fib_node_index_t fib_entry_index);
extern void fib_entry_contribute_urpf(fib_node_index_t path_index, index_t urpf);
extern void fib_entry_contribute_forwarding(fib_node_index_t fib_entry_index, fib_forward_chain_type_t type,
                                            dpo_id_t *dpo);
extern const dpo_id_t *fib_entry_contribute_ip_forwarding(fib_node_index_t fib_entry_index);
extern adj_index_t fib_entry_get_adj_for_source(fib_node_index_t fib_entry_index, fib_source_t source);
extern const int fib_entry_get_dpo_for_source(fib_node_index_t fib_entry_index, fib_source_t source, dpo_id_t *dpo);
...
extern int fib_entry_recursive_loop_detect(fib_node_index_t entry_index, fib_node_index_t **entry_indicies);

extern void fib_entry_lock(fib_node_index_t fib_entry_index);
extern void fib_entry_unlock(fib_node_index_t fib_entry_index);

extern u32 fib_entry_child_add(fib_node_index_t fib_entry_index, fib_node_type_t type, fib_node_index_t child_index);
extern void fib_entry_child_remove(fib_node_index_t fib_entry_index, u32 sibling_index);
extern u32 fib_entry_get_resolving_interface(fib_node_index_t fib_entry_index);
extern u32 fib_entry_get_resolving_interface_for_source(fib_node_index_t fib_entry_index, fib_source_t source);

extern void fib_entry_encode(fib_node_index_t fib_entry_index, fib_route_path_encode_t **api_rpaths);
...
extern void fib_entry_set_source_data(fib_node_index_t fib_entry_index, fib_source_t source, const void *data);
extern const void *fib_entry_get_source_data(fib_node_index_t fib_entry_index, fib_source_t source);

extern fib_entry_flag_t fib_entry_get_flags(fib_node_index_t fib_entry_index);
extern fib_entry_flag_t fib_entry_get_flags_for_source(fib_node_index_t fib_entry_index, fib_source_t source);
extern fib_source_t fib_entry_get_best_source(fib_node_index_t fib_entry_index);
extern int fib_entry_is_sourced(fib_node_index_t fib_entry_index, fib_source_t source);
...
extern void fib_entry_set_flow_hash_config(fib_node_index_t fib_entry_index, flow_hash_config_t hash_config);
