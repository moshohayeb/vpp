...
static void
fib_entry_src_interface_add(fib_entry_src_t *src, const fib_entry_t *entry, fib_entry_flag_t flags, dpo_proto_t proto,
                            const dpo_id_t *dpo)
{
...
static void
fib_entry_src_interface_path_swap(fib_entry_src_t *src, const fib_entry_t *entry, fib_path_list_flags_t pl_flags,
                                  const fib_route_path_t *paths)
{
    ...
    if (!(FIB_ENTRY_FLAG_LOCAL & src->fes_entry_flags)) {
        adj = adj_get(fib_path_list_get_adj(src->fes_pl, fib_entry_get_default_chain_type(entry)));
 ...
 */
static int
fib_entry_src_interface_activate(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
    fib_entry_t *cover;
    ...
    if (FIB_ENTRY_FLAG_LOCAL & src->fes_entry_flags) {
         ...
         * also exported
         */
        src->u.interface.fesi_cover = fib_table_get_less_specific(fib_entry->fe_fib_index, &fib_entry->fe_prefix);

        ASSERT(FIB_NODE_INDEX_INVALID != src->u.interface.fesi_cover);

        cover = fib_entry_get(src->u.interface.fesi_cover);

        src->u.interface.fesi_sibling = fib_entry_cover_track(cover, fib_entry_get_index(fib_entry));
    }
 ...
 */
static void
fib_entry_src_interface_deactivate(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
    fib_entry_t *cover;
...
fib_entry_src_interface_cover_change(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
     ...
     */
    if (src->u.interface.fesi_cover != fib_table_get_less_specific(fib_entry->fe_fib_index, &fib_entry->fe_prefix)) {
...
static void
fib_entry_src_interface_installed(fib_entry_src_t *src, const fib_entry_t *fib_entry)
{
    /*
...
static u8 *
fib_entry_src_interface_format(fib_entry_src_t *src, u8 *s)
{
