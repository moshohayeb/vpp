 ...
 */
void
fib_node_register_type(fib_node_type_t type, const fib_node_vft_t *vft)
{
    /*
...
u32
fib_node_child_add(fib_node_type_t parent_type, fib_node_index_t parent_index, fib_node_type_t type,
                   fib_node_index_t index)
{
    ...
    return (fib_node_list_push_front(parent->fn_children, 0, type, index));
}

void
fib_node_child_remove(fib_node_type_t parent_type, fib_node_index_t parent_index, fib_node_index_t sibling_index)
{
    fib_node_t *parent;
...
u32
fib_node_get_n_children(fib_node_type_t parent_type, fib_node_index_t parent_index)
{
    fib_node_t *parent;
...
fib_node_back_walk_one(fib_node_ptr_t *ptr, fib_node_back_walk_ctx_t *ctx)
{
    fib_node_t *node;
...
static int
fib_node_ptr_format_one_child(fib_node_ptr_t *ptr, void *arg)
{
    u8 **s = (u8 **) arg;
...
u8 *
fib_node_children_format(fib_node_list_t list, u8 *s)
{
...
void
fib_node_init(fib_node_t *node, fib_node_type_t type)
{
    /**
...
void
fib_show_memory_usage(const char *name, u32 in_use_elts, u32 allocd_elts, size_t size_elt)
{
    vlib_cli_output(vlib_get_main(), "%=30s %=5d %=8d/%=9d   %d/%d ", name, size_elt, in_use_elts, allocd_elts,
                    in_use_elts * size_elt, allocd_elts * size_elt);
}

static clib_error_t *
fib_memory_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    fib_node_vft_t *vft;
    ...
    vlib_cli_output(vm, "%=30s %=5s %=8s/%=9s   totals", "Name", "Size", "in-use", "allocated");
