...
format_fib_urpf_list(u8 *s, va_list *args)
{
    ...
    if (INDEX_INVALID != ui) {
        ...
        s = format(s, "uPRF-list:%d len:%d itfs:[", ui, vec_len(urpf->furpf_itfs));
 ...
 */
void
fib_urpf_list_append(index_t ui, u32 sw_if_index)
{
    fib_urpf_list_t *urpf;
 ...
 */
void
fib_urpf_list_combine(index_t ui1, index_t ui2)
{
 ...
 */
static int
fib_urpf_itf_cmp_for_sort(void *v1, void *v2)
{
    ...
    if (vec_len(urpf->furpf_itfs) > 1) {
        ...
        while (j < vec_len(urpf->furpf_itfs)) {
            if (urpf->furpf_itfs[i] == urpf->furpf_itfs[j]) {
                 ...
                 */
                while (urpf->furpf_itfs[i] == urpf->furpf_itfs[j] && j < vec_len(urpf->furpf_itfs)) {
                    j++;
                }
...
{
    fib_show_memory_usage("uRPF-list", pool_elts(fib_urpf_list_pool), pool_len(fib_urpf_list_pool),
                          sizeof(fib_urpf_list_t));
}

static clib_error_t *
show_fib_urpf_list_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    index_t ui;
    ...
    if (unformat(input, "%d", &ui)) {
        ...
        if (!pool_is_free_index(fib_urpf_list_pool, ui)) {
            vlib_cli_output(vm, "%d@%U", ui, format_fib_urpf_list, ui);
        } else {
        ...
        pool_foreach_index(ui, fib_urpf_list_pool, ({ vlib_cli_output(vm, "%d@%U", ui, format_fib_urpf_list, ui); }));
    }
