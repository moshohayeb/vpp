...
#define FIB_WALK_PRIORITIES                                                                                            \
    {                                                                                                                  \
        [FIB_WALK_PRIORITY_HIGH] = "high", [FIB_WALK_PRIORITY_LOW] = "low",                                            \
    }

#define FOR_EACH_FIB_WALK_PRIORITY(_prio)                                                                              \
    for ((_prio) = FIB_WALK_PRIORITY_HIGH; (_prio) < FIB_WALK_PRIORITY_NUM; (_prio)++)

extern void fib_walk_module_init(void);

extern void fib_walk_async(fib_node_type_t parent_type, fib_node_index_t parent_index, fib_walk_priority_t prio,
                           fib_node_back_walk_ctx_t *ctx);

extern void fib_walk_sync(fib_node_type_t parent_type, fib_node_index_t parent_index, fib_node_back_walk_ctx_t *ctx);
