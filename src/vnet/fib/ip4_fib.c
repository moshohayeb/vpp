...
static u32
ip4_create_fib_with_table_id(u32 table_id, fib_source_t src)
{
...
ip4_fib_table_destroy(u32 fib_index)
{
    ...
    for (ii = ARRAY_LEN(ip4_specials) - 1; ii >= 0; ii--) {
        ...
        fib_table_entry_special_remove(fib_table->ft_index, &prefix, ip4_specials[ii].ift_source);
    }
...
u32
ip4_fib_table_find_or_create_and_lock(u32 table_id, fib_source_t src)
{
    u32 index;
 ...
 */
fib_node_index_t
ip4_fib_table_lookup_exact_match(const ip4_fib_t *fib, const ip4_address_t *addr, u32 len)
{
    uword *hash, *result;
    u32 key;
 ...
 */
index_t
ip4_fib_table_lookup_lb(ip4_fib_t *fib, const ip4_address_t *addr)
{
 ...
 */
fib_node_index_t
ip4_fib_table_lookup(const ip4_fib_t *fib, const ip4_address_t *addr, u32 len)
{
    uword *hash, *result;
    i32 mask_len;
    u32 key;
...
void
ip4_fib_table_entry_insert(ip4_fib_t *fib, const ip4_address_t *addr, u32 len, fib_node_index_t fib_entry_index)
{
    uword *hash, *result;
    u32 key;
...
void
ip4_fib_table_entry_remove(ip4_fib_t *fib, const ip4_address_t *addr, u32 len)
{
    uword *hash, *result;
    u32 key;
...
void
ip4_fib_table_fwding_dpo_update(ip4_fib_t *fib, const ip4_address_t *addr, u32 len, const dpo_id_t *dpo)
{
    ip4_fib_mtrie_route_add(&fib->mtrie, addr, len, dpo->dpoi_index);
}

void
ip4_fib_table_fwding_dpo_remove(ip4_fib_t *fib, const ip4_address_t *addr, u32 len, const dpo_id_t *dpo, u32 cover_index)
{
    ...
    ip4_fib_mtrie_route_del(&fib->mtrie, addr, len, dpo->dpoi_index, cover_prefix->fp_len, cover_dpo->dpoi_index);
}

void
ip4_fib_table_walk(ip4_fib_t *fib, fib_table_walk_fn_t fn, void *ctx)
{
    fib_prefix_t root = {
        .fp_proto = FIB_PROTOCOL_IP4,
...
void
ip4_fib_table_sub_tree_walk(ip4_fib_t *fib, const fib_prefix_t *root, fib_table_walk_fn_t fn, void *ctx)
{
     ...
     */
    for (i = root->fp_len; i < ARRAY_LEN(fib->fib_entry_by_dst_address); i++) {
        ...
        if (NULL != hash) {
            ...
            hash_foreach_pair(p, hash, ({
                                  key.as_u32 = p->key;
                                  if (ip4_destination_matches_route(&ip4_main, &key, &root->fp_addr.ip4, root->fp_len)) {
                                      ...
                                      {
                                          if (ip4_destination_matches_route(&ip4_main, &key, &sub_tree->fp_addr.ip4,
                                                                            sub_tree->fp_len)) {
                                              skip = 1;
                                              break;
                                          }
...
static void
ip4_fib_table_show_all(ip4_fib_t *fib, vlib_main_t *vm)
{
...
static void
ip4_fib_table_show_one(ip4_fib_t *fib, vlib_main_t *vm, ip4_address_t *address, u32 mask_len, int detail)
{
    vlib_cli_output(vm, "%U", format_fib_entry, ip4_fib_table_lookup(fib, address, mask_len),
...
{
#if USE_DLMALLOC == 0
    s = format(s, "%=30s %=6d %=8ld\n", "IPv4 unicast", pool_elts(ip4_main.fibs), mheap_bytes(ip4_main.mtrie_mheap));
#else
    s = format(s, "%=30s %=6d %=8ld\n", "IPv4 unicast", pool_elts(ip4_main.fibs), mspace_footprint(ip4_main.mtrie_mheap));
#endif
...
ip4_show_fib(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "%U/%d", unformat_ip4_address, &matching_address, &matching_mask))
            matching = 1;
    ...
    pool_foreach(fib_table, im4->fibs, ({
                     ...
                     if (memory) {
                         ...
                         if (verbose)
                             vlib_cli_output(vm, "%U mtrie:%d hash:%d", format_fib_table_name, fib->index,
                                             FIB_PROTOCOL_IP4, mtrie_size, hash_size);
                     ...
                     s = format(s, "%U, fib_index:%d, flow hash:[%U] locks:[", format_fib_table_name, fib->index,
                                FIB_PROTOCOL_IP4, fib->index, format_ip_flow_hash_config, fib_table->ft_flow_hash_config);
                     ...
                     if (!matching) {
                         ...
                         ip4_fib_table_show_one(fib, vm, &matching_address, matching_mask, detail);
                     }
                 }));

    if (memory)
        vlib_cli_output(vm, "totals: mtrie:%ld hash:%ld all:%ld", total_mtrie_memory, total_hash_memory,
