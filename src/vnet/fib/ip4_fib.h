...
extern fib_node_index_t ip4_fib_table_lookup(const ip4_fib_t *fib, const ip4_address_t *addr, u32 len);
extern fib_node_index_t ip4_fib_table_lookup_exact_match(const ip4_fib_t *fib, const ip4_address_t *addr, u32 len);

extern void ip4_fib_table_entry_remove(ip4_fib_t *fib, const ip4_address_t *addr, u32 len);

extern void ip4_fib_table_entry_insert(ip4_fib_t *fib, const ip4_address_t *addr, u32 len,
                                       fib_node_index_t fib_entry_index);
extern void ip4_fib_table_destroy(u32 fib_index);

extern void ip4_fib_table_fwding_dpo_update(ip4_fib_t *fib, const ip4_address_t *addr, u32 len, const dpo_id_t *dpo);

extern void ip4_fib_table_fwding_dpo_remove(ip4_fib_t *fib, const ip4_address_t *addr, u32 len, const dpo_id_t *dpo,
                                            fib_node_index_t cover_index);
extern u32 ip4_fib_table_lookup_lb(ip4_fib_t *fib, const ip4_address_t *dst);
 ...
 */
extern void ip4_fib_table_walk(ip4_fib_t *fib, fib_table_walk_fn_t fn, void *ctx);
 ...
 * table and store elements in a vector, then delete the elements
 */
extern void ip4_fib_table_sub_tree_walk(ip4_fib_t *fib, const fib_prefix_t *root, fib_table_walk_fn_t fn, void *ctx);
...
{
    return (ip4_fib_table_lookup_lb(ip4_fib_get(vec_elt(im->fib_index_by_sw_if_index, sw_if_index)), dst));
}
 ...
 *
 */
extern u32 ip4_fib_table_find_or_create_and_lock(u32 table_id, fib_source_t src);
extern u32 ip4_fib_table_create_and_lock(fib_source_t src);

extern u8 *format_ip4_fib_table_memory(u8 *s, va_list *args);

static inline u32
ip4_fib_index_from_table_id(u32 table_id)
{
...
ip4_fib_forwarding_lookup(u32 fib_index, const ip4_address_t *addr)
{
