...
fib_entry_ae_add_or_lock(fib_node_index_t connected)
{
    ...
    fed   = fib_entry_delegate_get(entry, FIB_ENTRY_DELEGATE_ATTACHED_EXPORT);

    if (NULL == fed) {
        fed = fib_entry_delegate_find_or_add(entry, FIB_ENTRY_DELEGATE_ATTACHED_EXPORT);
...
static void
fib_entry_import_remove(fib_ae_import_t *import, fib_node_index_t entry_index)
{
    u32 index;
    ...
    if (index < vec_len(import->faei_importeds)) {
         ...
         */
        fib_table_entry_special_remove(import->faei_import_fib, fib_entry_get_prefix(entry_index), FIB_SOURCE_AE);
...
static void
fib_entry_import_add(fib_ae_import_t *import, fib_node_index_t entry_index)
{
    ...
    if (0 != fib_prefix_cmp(prefix, &import->faei_prefix)) {
        ...
        if (dpo_id_is_valid(dpo) && !dpo_is_drop(dpo)) {
            fib_table_entry_special_dpo_add(import->faei_import_fib, prefix, FIB_SOURCE_AE,
                                            (fib_entry_get_flags(entry_index) | FIB_ENTRY_FLAG_EXCLUSIVE),
 ...
 */
static int
fib_entry_covered_walk_import(fib_entry_t *cover, fib_node_index_t covered, void *ctx)
{
 ...
 */
static void
fib_ae_export_import_add(fib_ae_export_t *export, fib_ae_import_t *import)
{
    fib_entry_t *entry;
    ...
    fib_entry_cover_walk(entry, fib_entry_covered_walk_import, import);
}

void
fib_attached_export_import(fib_entry_t *fib_entry, fib_node_index_t export_fib)
{
    fib_entry_delegate_t *fed;
     ...
     */
    import->faei_export_entry = fib_table_lookup_exact_match(import->faei_export_fib, &import->faei_prefix);
    ...
    if (FIB_NODE_INDEX_INVALID == import->faei_export_entry) {
         ...
         */
        import->faei_export_entry = fib_table_lookup(import->faei_export_fib, &import->faei_prefix);
         ...
         */
        if (FIB_ENTRY_FLAG_ATTACHED & fib_entry_get_flags_i(fib_entry_get(import->faei_export_entry))) {
            fib_ae_export_t *export;
    ...
    import->faei_export_sibling = fib_entry_cover_track(fib_entry_get(import->faei_export_entry), fei);

    fed           = fib_entry_delegate_find_or_add(fib_entry, FIB_ENTRY_DELEGATE_ATTACHED_IMPORT);
...
fib_attached_export_purge(fib_entry_t *fib_entry)
{
    ...
    fed = fib_entry_delegate_get(fib_entry, FIB_ENTRY_DELEGATE_ATTACHED_IMPORT);
    ...
    if (NULL != fed) {
        ...
        {
            fib_table_entry_delete(import->faei_import_fib, fib_entry_get_prefix(*import_index), FIB_SOURCE_AE);
        ...
        if (FIB_NODE_INDEX_INVALID != import->faei_exporter) {
            ...
            fed = fib_entry_delegate_get(export_entry, FIB_ENTRY_DELEGATE_ATTACHED_EXPORT);
            ASSERT(NULL != fed);

            export = pool_elt_at_index(fib_ae_export_pool, fed->fd_index);

            u32 index = vec_search(export->faee_importers, (import - fib_ae_import_pool));
            ...
            if (0 == --export->faee_locks) {
                ...
                fib_entry_delegate_remove(export_entry, FIB_ENTRY_DELEGATE_ATTACHED_EXPORT);
            }
        }
        ...
        fib_entry_delegate_remove(fib_entry, FIB_ENTRY_DELEGATE_ATTACHED_IMPORT);
    }
}

void
fib_attached_export_covered_added(fib_entry_t *cover, fib_node_index_t covered)
{
    fib_entry_delegate_t *fed;

    fed = fib_entry_delegate_get(cover, FIB_ENTRY_DELEGATE_ATTACHED_EXPORT);
...
void
fib_attached_export_covered_removed(fib_entry_t *cover, fib_node_index_t covered)
{
    fib_entry_delegate_t *fed;
...
u8 *
fib_ae_import_format(fib_node_index_t impi, u8 *s)
{
...
u8 *
fib_ae_export_format(fib_node_index_t expi, u8 *s)
{
    fib_node_index_t *index;
    fib_ae_export_t *export;
