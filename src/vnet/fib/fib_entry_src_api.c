...
static void
fib_entry_src_api_path_swap(fib_entry_src_t *src, const fib_entry_t *entry, fib_path_list_flags_t pl_flags,
                            const fib_route_path_t *rpaths)
{
    ...
    vec_foreach(rpath, rpaths)
    {
        if (NULL != rpath->frp_label_stack) {
            fib_path_ext_list_push_back(&src->fes_path_exts, src->fes_pl, FIB_PATH_EXT_MPLS, rpath);
        }
    }
}

static void
fib_entry_src_api_path_add(fib_entry_src_t *src, const fib_entry_t *entry, fib_path_list_flags_t pl_flags,
                           const fib_route_path_t *rpaths)
{
    const fib_route_path_t *rpath;
    ...
    if (FIB_NODE_INDEX_INVALID == src->fes_pl) {
        ...
        src->fes_pl = fib_path_list_copy_and_path_add(src->fes_pl, (FIB_PATH_LIST_FLAG_SHARED | pl_flags), rpaths);
    }
    ...
    {
        if (NULL != rpath->frp_label_stack) {
            fib_path_ext_list_insert(&src->fes_path_exts, src->fes_pl, FIB_PATH_EXT_MPLS, rpath);
        }
    }
}

static void
fib_entry_src_api_path_remove(fib_entry_src_t *src, fib_path_list_flags_t pl_flags, const fib_route_path_t *rpaths)
{
    const fib_route_path_t *rpath;

    if (FIB_NODE_INDEX_INVALID != src->fes_pl) {
        src->fes_pl = fib_path_list_copy_and_path_remove(src->fes_pl, (FIB_PATH_LIST_FLAG_SHARED | pl_flags), rpaths);
        /*
...
static void
fib_entry_src_api_add(fib_entry_src_t *src, const fib_entry_t *entry, fib_entry_flag_t flags, dpo_proto_t proto,
                      const dpo_id_t *dpo)
{
