...
static void
vl_api_sw_interface_set_vxlan_bypass_t_handler(vl_api_sw_interface_set_vxlan_bypass_t *mp)
{
...
vl_api_vxlan_add_del_tunnel_t_handler(vl_api_vxlan_add_del_tunnel_t *mp)
{
    ...
    fib_index = fib_table_find(fib_ip_proto(mp->is_ipv6), ntohl(mp->encap_vrf_id));
    if (fib_index == ~0) {
        rv = VNET_API_ERROR_NO_SUCH_FIB;
        goto out;
    }
...
static void
send_vxlan_tunnel_details(vxlan_tunnel_t *t, vl_api_registration_t *reg, u32 context)
{
...
static void
vl_api_vxlan_tunnel_dump_t_handler(vl_api_vxlan_tunnel_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        /* *INDENT-OFF* */
        pool_foreach(t, vxm->tunnels, ({ send_vxlan_tunnel_details(t, reg, mp->context); }));
        /* *INDENT-ON* */
    } else {
