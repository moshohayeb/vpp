...
format_vxlan_tunnel(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d] instance %d src %U dst %U vni %d fib-idx %d sw-if-idx %d ", t->dev_instance, t->user_instance,
               format_ip46_address, &t->src, IP46_TYPE_ANY, format_ip46_address, &t->dst, IP46_TYPE_ANY, t->vni,
    ...
    if (t->flow_index != ~0)
        s = format(s, "flow-index %d [%U]", t->flow_index, format_flow_enabled_hw, t->flow_index);
...
{
    uword *vtep =
        ip46_address_is_ip4(ip) ? hash_get(vxlan_main.vtep4, ip->ip4.as_u32) : hash_get_mem(vxlan_main.vtep6, &ip->ip6);
    if (vtep)
        return ++(*vtep);
    ip46_address_is_ip4(ip) ? hash_set(vxlan_main.vtep4, ip->ip4.as_u32, 1) :
...
vtep_addr_unref(ip46_address_t *ip)
{
    ...
    if (--(*vtep) != 0)
        return *vtep;
    ip46_address_is_ip4(ip) ? hash_unset(vxlan_main.vtep4, ip->ip4.as_u32) :
...
int
vnet_vxlan_add_del_tunnel(vnet_vxlan_add_del_tunnel_args_t *a, u32 *sw_if_indexp)
{
    vxlan_main_t *vxm = &vxlan_main;
    ...
    if (!is_ip6) {
        ...
        key4.key[0] = ip46_address_is_multicast(&a->dst) ? a->dst.ip4.as_u32 :
...
#undef _
        ...
        t->hw_if_index =
            vnet_register_interface(vnm, vxlan_device_class.index, dev_instance, vxlan_hw_class.index, dev_instance);
        ...
        if (!ip46_address_is_multicast(&t->dst)) {
            ...
            t->sibling_index = fib_entry_child_add(t->fib_entry_index, FIB_NODE_TYPE_VXLAN_TUNNEL, dev_instance);
            ...
            if (vtep_addr_ref(&t->dst) == 1) {
                ...
                mfei                 = mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_VXLAN, &path,
...
static clib_error_t *
vxlan_add_del_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
show_vxlan_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vxlan_main_t *vxm = &vxlan_main;
    vxlan_tunnel_t *t;
    int raw = 0;
    ...
    /* *INDENT-OFF* */
    pool_foreach(t, vxm->tunnels, ({ vlib_cli_output(vm, "%U", format_vxlan_tunnel, t); }));
    /* *INDENT-ON* */
...
static clib_error_t *
set_ip_vxlan_bypass(u32 is_ip6, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
set_ip4_vxlan_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
set_ip6_vxlan_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
vxlan_offload_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    ...
    ;
    if (t_index == ~0)
        return clib_error_return(0, "%U is not a vxlan tunnel", format_vnet_sw_if_index_name, vnm, rx_sw_if_index);
    ...
    if (vnet_vxlan_add_del_rx_flow(hw_if_index, t_index, is_add))
        return clib_error_return(0, "error %s flow", is_add ? "enabling" : "disabling");
...
vxlan_init(vlib_main_t *vm)
{
    ...
    udp_register_dst_port(vm, UDP_DST_PORT_vxlan, vxlan4_input_node.index, /* is_ip4 */ 1);
    udp_register_dst_port(vm, UDP_DST_PORT_vxlan6, vxlan6_input_node.index, /* is_ip4 */ 0);
