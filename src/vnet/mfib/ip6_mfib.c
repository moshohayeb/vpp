...
#define FOR_EACH_IP6_SPECIAL(_pfx, _body)                                                                              \
    {                                                                                                                  \
        ...
        u8 _ii;                                                                                                        \
        for (_ii = 0; _ii < ARRAY_LEN(ip6_mfib_specials); _ii++) {                                                     \
            _spec = &ip6_mfib_specials[_ii];                                                                           \
            if (IP6_MFIB_SPECIAL_TYPE_SOLICITED == _spec->ims_type) {                                                  \
                ip6_set_solicited_node_multicast_address(&(_pfx)->fp_grp_addr.ip6, 0);                                 \
            } else {                                                                                                   \
                ip6_set_reserved_multicast_address(&(_pfx)->fp_grp_addr.ip6, _spec->ims_scope, _spec->ims_byte);       \
            }                                                                                                          \
...
static u32
ip6_create_mfib_with_table_id(u32 table_id, mfib_source_t src)
{
    ...
    mfib_table->mft_index = mfib_table->v6.index = (mfib_table - ip6_main.mfibs);

    hash_set(ip6_main.mfib_index_by_table_id, table_id, mfib_table->mft_index);

    mfib_table->mft_table_id = mfib_table->v6.table_id = table_id;

    mfib_table_lock(mfib_table->mft_index, FIB_PROTOCOL_IP6, src);

    mfib_table->v6.rhead = clib_mem_alloc_aligned(sizeof(*mfib_table->v6.rhead), CLIB_CACHE_LINE_BYTES);
    rn_inithead0(mfib_table->v6.rhead, 8);

    /*
     * add the special entries into the new FIB
     */
    mfib_table_entry_update(mfib_table->mft_index, &all_zeros, MFIB_SOURCE_DEFAULT_ROUTE, MFIB_RPF_ID_NONE,
    ...
    FOR_EACH_IP6_SPECIAL(&pfx, ({
        mfib_table_entry_path_update(mfib_table->mft_index, &pfx, MFIB_SOURCE_SPECIAL, &path_for_us,
...
ip6_mfib_table_destroy(ip6_mfib_t *mfib)
{
     ...
     */
    FOR_EACH_IP6_SPECIAL(&pfx, {
        mfib_table_entry_path_remove(mfib_table->mft_index, &pfx, MFIB_SOURCE_SPECIAL, &path_for_us);
    });
...
ip6_mfib_interface_enable_disable(u32 sw_if_index, int is_enable)
{
    ...
    if (is_enable) {
        FOR_EACH_IP6_SPECIAL(&pfx, {
            mfib_table_entry_path_update(mfib_index, &pfx, MFIB_SOURCE_SPECIAL, &path, MFIB_ITF_FLAG_ACCEPT);
        });
    } else {
        FOR_EACH_IP6_SPECIAL(&pfx, { mfib_table_entry_path_remove(mfib_index, &pfx, MFIB_SOURCE_SPECIAL, &path); });
    }
}

u32
ip6_mfib_table_find_or_create_and_lock(u32 table_id, mfib_source_t src)
{
    u32 index;
 ...
 */
fib_node_index_t
ip6_mfib_table_lookup_exact_match(const ip6_mfib_t *mfib, const ip6_address_t *grp, const ip6_address_t *src, u32 len)
{
    ...
    i6mn = (ip6_mfib_node_t *) rn_lookup(key.key, key.mask, (struct radix_node_head *) mfib->rhead);
 ...
 */
fib_node_index_t
ip6_mfib_table_lookup(const ip6_mfib_t *mfib, const ip6_address_t *src, const ip6_address_t *grp, u32 len)
{
    ip6_mfib_node_t *i6mn;
    ip6_mfib_key_t key;
    ...
    i6mn = (ip6_mfib_node_t *) rn_search_m(key.key, mfib->rhead->rnh_treetop, key.mask);
 ...
 */
fib_node_index_t
ip6_mfib_table_lookup2(const ip6_mfib_t *mfib, const ip6_address_t *src, const ip6_address_t *grp)
{
    ip6_mfib_node_t *i6mn;
    ip6_mfib_key_t key;
...
void
ip6_mfib_table_entry_insert(ip6_mfib_t *mfib, const ip6_address_t *grp, const ip6_address_t *src, u32 len,
                            fib_node_index_t mfib_entry_index)
{
    ...
    if (NULL == rn_addroute(i6mn->i6mn_key.key, i6mn->i6mn_key.mask, mfib->rhead, i6mn->i6mn_nodes)) {
        ASSERT(0);
    }
}

void
ip6_mfib_table_entry_remove(ip6_mfib_t *mfib, const ip6_address_t *grp, const ip6_address_t *src, u32 len)
{
    ip6_mfib_node_t *i6mn;
    ip6_mfib_key_t key;
...
{
    s = format(s, "%=30s %=6d %=8s\n", "IPv6 multicast", pool_elts(ip6_main.mfibs), "???");

    return (s);
}

static void
ip6_mfib_table_show_one(ip6_mfib_t *mfib, vlib_main_t *vm, ip6_address_t *src, ip6_address_t *grp, u32 mask_len)
{
    vlib_cli_output(vm, "%U", format_mfib_entry, ip6_mfib_table_lookup(mfib, src, grp, mask_len),
...
static void
ip6_mfib_table_show_all(ip6_mfib_t *mfib, vlib_main_t *vm)
{
    fib_node_index_t *mfib_entry_index;
    ...
    ip6_mfib_table_walk(mfib, ip6_mfib_table_collect_entries, &ctx);
...
static int
ip6_mfib_table_radix_walk(struct radix_node *rn, void *arg)
{
...
void
ip6_mfib_table_walk(ip6_mfib_t *mfib, mfib_table_walk_fn_t fn, void *ctx)
{
    ...
    rn_walktree(mfib->rhead, ip6_mfib_table_radix_walk, &rn_ctx);
}

static clib_error_t *
ip6_show_mfib(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "%U %U", unformat_ip6_address, &src, unformat_ip6_address, &grp)) {
            matching = 1;
            mask     = 256;
    ...
    pool_foreach(mfib_table, im6->mfibs, ({
                     ...
                     vlib_cli_output(vm, "%U, fib_index %d", format_mfib_table_name, mfib->index, FIB_PROTOCOL_IP6,
                                     mfib->index);
