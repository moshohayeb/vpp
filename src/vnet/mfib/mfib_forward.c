...
void
mfib_forward_lookup_trace(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 *from, n_left;
    ...
    while (n_left >= 4) {
        ...
        if (b0->flags & VLIB_BUFFER_IS_TRACED) {
            ...
            t0->fib_index   = vec_elt(im->mfib_index_by_sw_if_index, vnet_buffer(b1)->sw_if_index[VLIB_RX]);
        }
        if (b1->flags & VLIB_BUFFER_IS_TRACED) {
            t1              = vlib_add_trace(vm, node, b1, sizeof(t1[0]));
            t1->entry_index = vnet_buffer(b1)->ip.adj_index[VLIB_TX];
            t1->fib_index   = vec_elt(im->mfib_index_by_sw_if_index, vnet_buffer(b1)->sw_if_index[VLIB_RX]);
        }
        from += 2;
        n_left -= 2;
    }
    ...
    while (n_left >= 1) {
        ...
        if (b0->flags & VLIB_BUFFER_IS_TRACED) {
            t0              = vlib_add_trace(vm, node, b0, sizeof(t0[0]));
            t0->entry_index = vnet_buffer(b0)->ip.adj_index[VLIB_TX];
            t0->fib_index   = vec_elt(im->mfib_index_by_sw_if_index, vnet_buffer(b0)->sw_if_index[VLIB_RX]);
        }
        from += 1;
        n_left -= 1;
    }
...
static uword
mfib_forward_lookup(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_v4)
{
    ...
    while (n_left_from > 0) {
        vlib_get_next_frame(vm, node, MFIB_FORWARD_LOOKUP_NEXT_RPF, to_next, n_left_to_next);
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (is_v4) {
                ...
                fib_index0 = vec_elt(ip4_main.mfib_index_by_sw_if_index, vnet_buffer(p0)->sw_if_index[VLIB_RX]);
                ip0        = vlib_buffer_get_current(p0);
                mfei0      = ip4_mfib_table_lookup(ip4_mfib_get(fib_index0), &ip0->src_address, &ip0->dst_address, 64);
            } else {
                ip6_header_t *ip0;

                fib_index0 = vec_elt(ip6_main.mfib_index_by_sw_if_index, vnet_buffer(p0)->sw_if_index[VLIB_RX]);
                ip0        = vlib_buffer_get_current(p0);
        ...
        vlib_put_next_frame(vm, node, MFIB_FORWARD_LOOKUP_NEXT_RPF, n_left_to_next);
    }
...
static uword
ip4_mfib_forward_lookup(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ip6_mfib_forward_lookup(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
mfib_forward_connected_check(vlib_buffer_t *b0, u32 sw_if_index, int is_v4)
{
    ...
    if (is_v4) {
        ...
        lbi0 = ip4_fib_forwarding_lookup(ip4_fib_table_get_index_for_sw_if_index(sw_if_index), &ip0->src_address);
...
static void
mfib_forward_itf_signal(vlib_main_t *vm, const mfib_entry_t *mfe, mfib_itf_t *mfi, vlib_buffer_t *b0)
{
    mfib_itf_flags_t old_flags;

    old_flags = __sync_fetch_and_or(&mfi->mfi_flags, MFIB_ITF_FLAG_SIGNAL_PRESENT);
...
always_inline uword
mfib_forward_rpf(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_v4)
{
    u32 n_left_from, n_left_to_next, *from, *to_next;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            mfi0   = mfib_entry_get_itf(mfe0, vnet_buffer(b0)->sw_if_index[VLIB_RX]);
            ...
            if (MFIB_RPF_ID_NONE != mfe0->mfe_rpf_id) {
                iflags0 = (mfe0->mfe_rpf_id == vnet_buffer(b0)->ip.rpf_id ? MFIB_ITF_FLAG_ACCEPT : MFIB_ITF_FLAG_NONE);
            } else {
            ...
            if (PREDICT_FALSE(eflags0 & MFIB_ENTRY_FLAG_CONNECTED)) {
                 ...
                 * matches a connected.
                 */
                if (mfib_forward_connected_check(b0, vnet_buffer(b0)->sw_if_index[VLIB_RX], is_v4)) {
                    mfib_forward_itf_signal(vm, mfe0, mfi0, b0);
                }
            }
            if (PREDICT_FALSE((eflags0 & MFIB_ENTRY_FLAG_SIGNAL) ^ (iflags0 & MFIB_ITF_FLAG_NEGATE_SIGNAL))) {
                /*
            ...
            if (PREDICT_TRUE((iflags0 & MFIB_ITF_FLAG_ACCEPT) || (eflags0 & MFIB_ENTRY_FLAG_ACCEPT_ALL_ITF))) {
                /*
                 * This interface is accepting packets for the matching entry
                 */
                next0 = mfe0->mfe_rep.dpoi_next_node;

                vnet_buffer(b0)->ip.adj_index[VLIB_TX] = mfe0->mfe_rep.dpoi_index;
            } else {
...
static uword
ip4_mfib_forward_rpf(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ip6_mfib_forward_rpf(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mfib_forward_rpf(vm, node, frame, 1));
}
...
VLIB_NODE_FUNCTION_MULTIARCH(ip6_mfib_forward_rpf_node, ip6_mfib_forward_rpf)
