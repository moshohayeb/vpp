...
#DEFIne MFIB_ENTRY_DBG(_e, _fmt, _args...){                                                                            \
    u8 *__tmp = NULL; __tmp = format(__tmp, "e:[%d:%U", mfib_entry_get_index(_e), format_ip46_address,                 \
                                     &_e->mfe_prefix.fp_grp_addr, IP46_TYPE_ANY);                                      \
    __tmp                   = format(__tmp, "/%d,", _e->mfe_prefix.fp_len);                                            \
    __tmp = format(__tmp, "%U]", mfib_entry_get_index(_e), format_ip46_address, &_e->mfe_prefix.fp_src_addr,           \
                   IP46_TYPE_ANY);                                                                                     \
    __tmp = format(__tmp, _fmt, ##_args); clib_warning("%s", __tmp); vec_free(__tmp); }
#else
#define MFIB_ENTRY_DBG(_e, _fmt, _args...)
#endif
...
format_mfib_entry_path_ext(u8 *s, va_list *args)
{
    ...
    return (format(s, "path:%d flags:%U", path_ext->mfpe_path, format_mfib_itf_flags, path_ext->mfpe_flags));
}
...
format_mfib_entry(u8 *s, va_list *args)
{
    ...
    s = format(s, "\n  Interfaces:");
    hash_foreach(sw_if_index, mfi, mfib_entry->mfe_itfs, ({ s = format(s, "\n  %U", format_mfib_itf, mfi); }));
    if (MFIB_RPF_ID_NONE != mfib_entry->mfe_rpf_id) {
        s = format(s, "\n  RPF-ID:%d", mfib_entry->mfe_rpf_id);
    }
    s = format(s, "\n  %U-chain\n  %U", format_fib_forw_chain_type, mfib_entry_get_default_chain_type(mfib_entry),
               format_dpo_id, &mfib_entry->mfe_rep, 2);
    s = format(s, "\n");
...
mfib_entry_src_find(const mfib_entry_t *mfib_entry, mfib_source_t source, u32 *index)
...
mfib_entry_src_flush(mfib_entry_src_t *msrc)
{
    ...
    hash_foreach(sw_if_index, mfii, msrc->mfes_itfs, ({ mfib_itf_delete(mfib_itf_get(mfii)); }));
...
u32
mfib_entry_child_add(fib_node_index_t mfib_entry_index, fib_node_type_t child_type, fib_node_index_t child_index)
{
    return (fib_node_child_add(FIB_NODE_TYPE_MFIB_ENTRY, mfib_entry_index, child_type, child_index));
};

void
mfib_entry_child_remove(fib_node_index_t mfib_entry_index, u32 sibling_index)
{
    fib_node_child_remove(FIB_NODE_TYPE_MFIB_ENTRY, mfib_entry_index, sibling_index);
}

static mfib_entry_t *
mfib_entry_alloc(u32 fib_index, const mfib_prefix_t *prefix, fib_node_index_t *mfib_entry_index)
{
    mfib_entry_t *mfib_entry;
...
mfib_path_ext_add(mfib_entry_src_t *msrc, fib_node_index_t path_index, mfib_itf_flags_t mfi_flags)
{
    mfib_path_ext_t *path_ext;
...
mfib_entry_src_collect_forwarding(fib_node_index_t pl_index, fib_node_index_t path_index, void *arg)
{
...
mfib_entry_stack(mfib_entry_t *mfib_entry, mfib_entry_src_t *msrc)
{
    ...
    if (NULL != msrc && FIB_NODE_INDEX_INVALID != msrc->mfes_pl) {
        mfib_entry_collect_forwarding_ctx_t ctx = {
            .next_hops = NULL,
        ...
        fib_path_list_walk(msrc->mfes_pl, mfib_entry_src_collect_forwarding, &ctx);
        ...
        if (!(MFIB_ENTRY_FLAG_EXCLUSIVE & mfib_entry->mfe_flags)) {
            if (NULL == ctx.next_hops) {
                 ...
                 * from those choices.
                 */
                if (!dpo_id_is_valid(&mfib_entry->mfe_rep) || dpo_is_drop(&mfib_entry->mfe_rep)) {
                    dpo_id_t tmp_dpo = DPO_INVALID;

                    dpo_set(&tmp_dpo, DPO_REPLICATE, dp, replicate_create(0, dp));

                    dpo_stack(DPO_MFIB_ENTRY, dp, &mfib_entry->mfe_rep, &tmp_dpo);
            ...
            dpo_stack(DPO_MFIB_ENTRY, dp, &mfib_entry->mfe_rep, &ctx.next_hops[0].path_dpo);
        ...
        mfib_entry->mfe_sibling =
            fib_path_list_child_add(mfib_entry->mfe_pl, FIB_NODE_TYPE_MFIB_ENTRY, mfib_entry_get_index(mfib_entry));
    } else {
        dpo_stack(DPO_MFIB_ENTRY, dp, &mfib_entry->mfe_rep, drop_dpo_get(dp));
    }
}
...
mfib_entry_src_path_add(mfib_entry_src_t *msrc, const fib_route_path_t *rpath)
{
...
static fib_node_index_t
mfib_entry_src_path_remove(mfib_entry_src_t *msrc, const fib_route_path_t *rpath)
{
    fib_node_index_t path_index;
    fib_route_path_t *rpaths;
...
fib_node_index_t
mfib_entry_create(u32 fib_index, mfib_source_t source, const mfib_prefix_t *prefix, fib_rpf_id_t rpf_id,
...
{
    return ((MFIB_ENTRY_FLAG_NONE == msrc->mfes_flags && 0 == fib_path_list_get_n_paths(msrc->mfes_pl)));
}

int
mfib_entry_update(fib_node_index_t mfib_entry_index, mfib_source_t source, mfib_entry_flags_t entry_flags,
                  fib_rpf_id_t rpf_id, index_t repi)
{
    ...
    if (INDEX_INVALID != repi) {
        ...
        msrc->mfes_pl = fib_path_list_create_special(dp, FIB_PATH_LIST_FLAG_EXCLUSIVE, &dpo);
...
void
mfib_entry_path_update(fib_node_index_t mfib_entry_index, mfib_source_t source, const fib_route_path_t *rpath,
                       mfib_itf_flags_t itf_flags)
{
    ...
    if (~0 != rpath[0].frp_sw_if_index) {
        ...
        if (old != itf_flags) {
             ...
             */
            mfib_itf = mfib_entry_itf_find(msrc->mfes_itfs, rpath[0].frp_sw_if_index);

            if (NULL == mfib_itf) {
                mfib_entry_itf_add(msrc, rpath[0].frp_sw_if_index, mfib_itf_create(path_index, itf_flags));
            } else {
                if (mfib_itf_update(mfib_itf, path_index, itf_flags)) {
                    /*
 ...
 */
int
mfib_entry_path_remove(fib_node_index_t mfib_entry_index, mfib_source_t source, const fib_route_path_t *rpath)
{
    fib_node_index_t path_index;
    mfib_entry_t *mfib_entry;
    mfib_entry_src_t *msrc;
...
static int
mfib_entry_cmp(fib_node_index_t mfib_entry_index1, fib_node_index_t mfib_entry_index2)
{
    ...
    case FIB_PROTOCOL_IP4:
        cmp = fib_ip4_address_compare(&mfib_entry1->mfe_prefix.fp_grp_addr.ip4, &mfib_entry2->mfe_prefix.fp_grp_addr.ip4);
        ...
        break;
    case FIB_PROTOCOL_IP6:
        cmp = fib_ip6_address_compare(&mfib_entry1->mfe_prefix.fp_grp_addr.ip6, &mfib_entry2->mfe_prefix.fp_grp_addr.ip6);
...
{
    fib_show_memory_usage("multicast-Entry", pool_elts(mfib_entry_pool), pool_len(mfib_entry_pool), sizeof(mfib_entry_t));
}
...
void
mfib_entry_encode(fib_node_index_t mfib_entry_index, fib_route_path_encode_t **api_rpaths)
{
    mfib_entry_t *mfib_entry;
    mfib_entry_src_t *bsrc;
    ...
    if (FIB_NODE_INDEX_INVALID != bsrc->mfes_pl) {
        fib_path_list_walk(bsrc->mfes_pl, fib_path_encode, api_rpaths);
    }
}


void
mfib_entry_get_prefix(fib_node_index_t mfib_entry_index, mfib_prefix_t *pfx)
{
    mfib_entry_t *mfib_entry;
...
void
mfib_entry_contribute_forwarding(fib_node_index_t mfib_entry_index, fib_forward_chain_type_t type, dpo_id_t *dpo)
{
    /*
...
static clib_error_t *
show_mfib_entry_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "%d", &fei)) {
        ...
        if (!pool_is_free_index(mfib_entry_pool, fei)) {
            vlib_cli_output(vm, "%d@%U", fei, format_mfib_entry, fei, MFIB_ENTRY_FORMAT_DETAIL2);
        } else {
        ...
        pool_foreach_index(fei, mfib_entry_pool,
                           ({ vlib_cli_output(vm, "%d@%U", fei, format_mfib_entry, fei, MFIB_ENTRY_FORMAT_BRIEF); }));
    }
