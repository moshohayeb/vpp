...
clib_error_t *
mfib_show_route_flags(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    mfib_entry_attribute_t attr;

    FOR_EACH_MFIB_ATTRIBUTE(attr)
    {
        vlib_cli_output(vm, "%s = %s", mfib_flag_names[attr], mfib_flag_names_long[attr]);
    }
...
clib_error_t *
mfib_show_itf_flags(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    mfib_itf_attribute_t attr;

    FOR_EACH_MFIB_ITF_ATTRIBUTE(attr)
    {
        vlib_cli_output(vm, "%s = %s", mfib_itf_flag_names[attr], mfib_itf_flag_long_names[attr]);
    }
