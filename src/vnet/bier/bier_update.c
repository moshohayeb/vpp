...
clib_error_t *
vnet_bier_table_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
clib_error_t *
vnet_bier_route_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
VLIB_CLI_COMMAND(bier_route_command) = {
    .path = "bier route",
    .short_help =
        "bier route [add|del] sd <sud-domain> set <set> bsl <bit-string-length> bp <bit-position> via "
...
static clib_error_t *
show_bier_fib_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
