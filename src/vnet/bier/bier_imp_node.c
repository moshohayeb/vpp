...
always_inline uword
bier_imp_dpo_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, fib_protocol_t fproto,
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(BIER_RX_ITF == vnet_buffer(b0)->ip.adj_index[VLIB_RX])) {
                ...
                hdr0->bh_first_word |= clib_host_to_net_u32((vnet_buffer(b0)->ip.flow_hash & BIER_HDR_ENTROPY_FIELD_MASK)
...
static uword
bier_imp_ip4(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (bier_imp_dpo_inline(vm, node, frame, FIB_PROTOCOL_IP4, BIER_HDR_PROTO_IPV4));
}

VLIB_REGISTER_NODE(bier_imp_ip4_node) = {.function    = bier_imp_ip4,
                                         .name        = "bier-imp-ip4",
...
static uword
bier_imp_ip6(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (bier_imp_dpo_inline(vm, node, frame, FIB_PROTOCOL_IP6, BIER_HDR_PROTO_IPV6));
}

VLIB_REGISTER_NODE(bier_imp_ip6_node) = {.function    = bier_imp_ip6,
                                         .name        = "bier-imp-ip6",
                                         .vector_size = sizeof(u32),
