...
bier_disp_entry_path_list_walk(fib_node_index_t pl_index, fib_node_index_t path_index, void *arg)
{
...
static void
bier_disp_entry_restack(bier_disp_entry_t *bde, bier_hdr_proto_id_t pproto)
{
    ...
    if (FIB_NODE_INDEX_INVALID == pli) {
        ...
        fib_path_list_contribute_forwarding(pli, fib_forw_chain_type_from_dpo_proto(bier_hdr_proto_to_dpo(pproto)),
...
void
bier_disp_entry_path_add(index_t bdei, bier_hdr_proto_id_t pproto, const fib_route_path_t *rpaths)
{
...
int
bier_disp_entry_path_remove(index_t bdei, bier_hdr_proto_id_t pproto, const fib_route_path_t *rpaths)
{
    fib_node_index_t *pli, old_pli;
    bier_disp_entry_t *bde;
     ...
     */
    if (FIB_NODE_INDEX_INVALID != *pli) {
        *pli = fib_path_list_copy_and_path_remove(old_pli, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF),
                                                  rpaths);
    ...
    FOR_EACH_BIER_HDR_PROTO(pproto)
    {
        if (INDEX_INVALID != bde->bde_pl[pproto]) {
            ...
            if (flags & BIER_SHOW_DETAIL) {
                ...
                s = format(s, "\n%U%U", format_white_space, indent + 6, format_dpo_id, &bde->bde_fwd[pproto].bde_dpo,
                           indent + 2);
            }
        }
    }
...
void
bier_disp_entry_contribute_forwarding(index_t bdei, dpo_id_t *dpo)
{
...
{
    fib_show_memory_usage("BIER dispositon", pool_elts(bier_disp_entry_pool), pool_len(bier_disp_entry_pool),
...
static clib_error_t *
show_bier_disp_entry(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    index_t bdei;
