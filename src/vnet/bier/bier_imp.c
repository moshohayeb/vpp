...
index_t
bier_imp_add_or_lock(const bier_table_id_t *bti, bier_bp_t sender, const bier_bit_string_t *bs)
{
    bier_imp_t *bi = NULL;
     ...
     */
    bier_hdr_init(&bi->bi_hdr, BIER_HDR_VERSION_1,
...
format_bier_imp(u8 *s, va_list *args)
{
    ...
    s = format(s, "bier-imp:[%d]: tbl:[%U] hdr:[%U]", bier_imp_get_index(bi), format_bier_table_id, &bi->bi_tbl,
...
void
bier_imp_contribute_forwarding(index_t bii, dpo_proto_t proto, dpo_id_t *dpo)
{
...
static clib_error_t *
show_bier_imp(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    bier_imp_t *bi;
    index_t bii;
