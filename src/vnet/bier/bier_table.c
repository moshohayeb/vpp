...
bier_table_mk_key(const bier_table_id_t *id)
{
     ...
     * for later
     */
    u32 key = ((id->bti_sub_domain << 24) | (id->bti_set << 16) | (id->bti_ecmp << 8) | (id->bti_hdr_len << 4) |
               (id->bti_type));
...
static void
bier_table_init(bier_table_t *bt, const bier_table_id_t *id, mpls_label_t ll)
{
    u32 num_entries;
...
bier_table_mk_bift(bier_table_t *bt)
{
    ...
    bier_bift_table_entry_add(bier_bift_id_encode(bt->bt_id.bti_set, bt->bt_id.bti_sub_domain, bt->bt_id.bti_hdr_len),
                              &dpo);
...
bier_table_mk_lfib(bier_table_t *bt)
{
    ...
    if (MPLS_LABEL_INVALID != bt->bt_ll) {
        ...
        bt->bt_lfei =
            fib_table_entry_special_dpo_add(mpls_fib_index, &pfx, FIB_SOURCE_BIER, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
        dpo_reset(&dpo);
    }
}
...
static index_t
bier_table_create(const bier_table_id_t *btid, mpls_label_t local_label)
{
    /*
     * add a new table
     */
...
index_t
bier_table_add_or_lock(const bier_table_id_t *btid, mpls_label_t local_label)
{
    bier_table_t *bt;
    index_t bti;
...
static void
bier_table_insert(bier_table_t *bt, bier_bp_t bp, index_t bei)
{
    bt->bt_entries[BIER_BP_TO_INDEX(bp)] = bei;
}

static void
bier_table_remove(bier_table_t *bt, bier_bp_t bp)
{
    bt->bt_entries[BIER_BP_TO_INDEX(bp)] = INDEX_INVALID;
}

void
bier_table_route_path_update_i(const bier_table_id_t *btid, bier_bp_t bp, fib_route_path_t *brps, u8 is_replace)
{
...
void
bier_table_route_path_update(const bier_table_id_t *btid, bier_bp_t bp, fib_route_path_t *brps)
{
    bier_table_route_path_update_i(btid, bp, brps, 1);
}
void
bier_table_route_path_add(const bier_table_id_t *btid, bier_bp_t bp, fib_route_path_t *brps)
{
    bier_table_route_path_update_i(btid, bp, brps, 0);
}

void
bier_table_route_delete(const bier_table_id_t *btid, bier_bp_t bp)
{
    bier_table_t *bt;
    index_t bei;
...
void
bier_table_route_path_remove(const bier_table_id_t *btid, bier_bp_t bp, fib_route_path_t *brps)
{
...
void
bier_table_contribute_forwarding(index_t bti, dpo_id_t *dpo)
{
    bier_table_t *bt;
...
bier_table_ecmp_walk_path_list(fib_node_index_t pl_index, fib_node_index_t path_index, void *arg)
{
...
void
bier_table_ecmp_walk(index_t bti, bier_table_ecmp_walk_fn_t fn, void *ctx)
{
...
void
bier_table_ecmp_set_fmask(index_t bti, bier_bp_t bp, index_t bfmi)
{
    bier_table_t *bt;
...
format_bier_table(u8 *s, va_list *ap)
{
    ...
    s = format(s, "[@%d] bier-table:[%U local-label:%U", bti, format_bier_table_id, &bt->bt_id,
...
void
bier_table_show_all(vlib_main_t *vm, bier_show_flags_t flags)
{
    if (!pool_elts(bier_table_pool)) {
        ...
        pool_foreach_index(ii, bier_table_pool, ({ vlib_cli_output(vm, "%U", format_bier_table, ii, flags); }));
    }
}

void
bier_tables_walk(bier_tables_walk_fn_t fn, void *ctx)
{
    ASSERT(0);
}
...
void
bier_table_walk(const bier_table_id_t *bti, bier_table_walk_fn_t fn, void *ctx)
{
    bier_table_t *bt;
    bier_entry_t *be;
    index_t *bei;
