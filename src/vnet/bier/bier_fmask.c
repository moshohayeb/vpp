...
bier_fmask_bits_init(bier_fmask_bits_t *bits, bier_hdr_len_id_t hlid)
{
    ...
    memset(bits->bfmb_refs, 0, (sizeof(bits->bfmb_refs[0]) * bier_hdr_len_id_to_num_bits(hlid)));
    ...
    bits->bfmb_input_reset_string.bbs_buckets =
        clib_mem_alloc_aligned(sizeof(bits->bfmb_input_reset_string.bbs_buckets[0]) * bier_hdr_len_id_to_num_buckets(hlid),
                               CLIB_CACHE_LINE_BYTES);
    memset(bits->bfmb_input_reset_string.bbs_buckets, 0,
...
u32
bier_fmask_child_add(fib_node_index_t bfmi, fib_node_type_t child_type, fib_node_index_t child_index)
{
...
static void
bier_fmask_init(bier_fmask_t *bfm, const bier_fmask_id_t *fmid, const fib_route_path_t *rpath)
{
...
index_t
bier_fmask_create_and_lock(const bier_fmask_id_t *fmid, const fib_route_path_t *rpath)
{
    bier_fmask_t *bfm;
    index_t bfmi;
...
format_bier_fmask(u8 *s, va_list *ap)
{
    ...
    s = format(s, "fmask: nh:%U bs:%U locks:%d ", format_ip46_address, &bfm->bfm_id->bfmi_nh, IP46_TYPE_ANY,
               format_bier_bit_string, &bfm->bfm_bits.bfmb_input_reset_string, bfm->bfm_node.fn_locks);
    s = format(s, "flags:");
    ...
    if (bfm->bfm_flags & BIER_FMASK_FLAG_MPLS) {
        s = format(s, "  output-label:%U", format_mpls_unicast_label,
                   vnet_mpls_uc_get_label(clib_net_to_host_u32(bfm->bfm_label)));
    } else {
        s = format(s, "  output-bfit:[%U]", format_bier_bift_id,
                   vnet_mpls_uc_get_label(clib_net_to_host_u32(bfm->bfm_label)));
    }
...
void
bier_fmask_encode(index_t bfmi, bier_table_id_t *btid, fib_route_path_encode_t *rpath)
{
    bier_fmask_t *bfm;
    ...
    *btid = *bier_table_get_id(bfm->bfm_id->bfmi_bti);
    ...
    case BIER_NH_UDP:
        ...
        break;
    case BIER_NH_IP:
        memcpy(&rpath->rpath.frp_addr, &bfm->bfm_id->bfmi_nh, sizeof(rpath->rpath.frp_addr));
        break;
    }
}
...
static clib_error_t *
bier_fmask_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    bier_fmask_t *bfm;
    index_t bfmi;
    ...
    if (INDEX_INVALID == bfmi) {
        pool_foreach(bfm, bier_fmask_pool, ({
                         vlib_cli_output(vm, "[@%d] %U", bier_fmask_get_index(bfm), format_bier_fmask,
                                         bier_fmask_get_index(bfm), 0);
                     }));
    } else {
