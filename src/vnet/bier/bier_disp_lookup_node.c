...
always_inline uword
bier_disp_lookup_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword
bier_disp_lookup(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (bier_disp_lookup_inline(vm, node, frame));
}

VLIB_REGISTER_NODE(bier_disp_lookup_node) = {.function    = bier_disp_lookup,
