...
bier_bift_table_entry_add(bier_bift_id_t id, const dpo_id_t *dpo)
{
    if (NULL == bier_bift_table) {
        ...
        for (ii = 0; ii < BIER_BIFT_N_ENTRIES; ii++) {
            dpo_stack_from_node(bier_bift_input_node.index, &bier_bift_table->bblt_dpos[ii],
...
static uword
bier_bift_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
clib_error_t *
show_bier_bift_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
