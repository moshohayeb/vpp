...
static void
send_bier_table_details(vl_api_registration_t *reg, u32 context, const bier_table_t *bt)
{
...
static void
send_bier_route_details(const bier_table_t *bt, const bier_entry_t *be, void *args)
{
...
static void
send_bier_imp_details(vl_api_registration_t *reg, u32 context, const bier_imp_t *bi)
{
...
vl_api_bier_imp_dump_t_handler(vl_api_bier_imp_dump_t *mp)
{
    ...
    pool_foreach(bi, bier_imp_pool, ({ send_bier_imp_details(reg, mp->context, bi); }));
}
...
static void
send_bier_disp_table_details(vl_api_registration_t *reg, u32 context, const bier_disp_table_t *bdt)
{
...
vl_api_bier_disp_entry_add_del_t_handler(vl_api_bier_disp_entry_add_del_t *mp)
{
    ...
    vec_foreach_index(ii, brps)
    {
        ...
        if (0 == mp->bde_paths[ii].afi) {
            clib_memcpy(&brp->frp_addr.ip4, mp->bde_paths[ii].next_hop, sizeof(brp->frp_addr.ip4));
        } else {
            clib_memcpy(&brp->frp_addr.ip6, mp->bde_paths[ii].next_hop, sizeof(brp->frp_addr.ip6));
        }
...
static void
send_bier_disp_entry_details(const bier_disp_table_t *bdt, const bier_disp_entry_t *bde, u16 bp, void *args)
{
    fib_route_path_encode_t *api_rpaths = NULL, *api_rpath;
