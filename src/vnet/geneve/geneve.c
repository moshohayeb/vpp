...
static bool
geneve_decap_next_is_valid(geneve_main_t *vxm, u32 is_ip6, u32 decap_next_index)
{
...
{
    uword *vtep = ip46_address_is_ip4(ip) ? hash_get(geneve_main.vtep4, ip->ip4.as_u32) :
                                            hash_get_mem(geneve_main.vtep6, &ip->ip6);
    if (vtep)
        return ++(*vtep);
    ip46_address_is_ip4(ip) ? hash_set(geneve_main.vtep4, ip->ip4.as_u32, 1) :
...
vtep_addr_unref(ip46_address_t *ip)
{
    ...
    if (--(*vtep) != 0)
        return *vtep;
    ip46_address_is_ip4(ip) ? hash_unset(geneve_main.vtep4, ip->ip4.as_u32) :
...
int
vnet_geneve_add_del_tunnel(vnet_geneve_add_del_tunnel_args_t *a, u32 *sw_if_indexp)
{
    geneve_main_t *vxm = &geneve_main;
    geneve_tunnel_t *t = 0;
    ...
    if (!is_ip6) {
        ...
        key4.vni    = clib_host_to_net_u32((a->vni << GENEVE_VNI_SHIFT) & GENEVE_VNI_MASK);
        p           = hash_get(vxm->geneve4_tunnel_by_key, key4.as_u64);
    } else {
        key6.remote = a->remote.ip6;
        key6.vni    = clib_host_to_net_u32((a->vni << GENEVE_VNI_SHIFT) & GENEVE_VNI_MASK);
...
#undef _
        ...
        if (vec_len(vxm->free_geneve_tunnel_hw_if_indices) > 0) {
            ...
            hw_if_index = vxm->free_geneve_tunnel_hw_if_indices[vec_len(vxm->free_geneve_tunnel_hw_if_indices) - 1];
            ...
            hw_if_index = vnet_register_interface(vnm, geneve_device_class.index, t - vxm->tunnels,
        ...
        if (!ip46_address_is_multicast(&t->remote)) {
            ...
            t->sibling_index = fib_entry_child_add(t->fib_entry_index, FIB_NODE_TYPE_GENEVE_TUNNEL, t - vxm->tunnels);
            ...
            if (vtep_addr_ref(&t->remote) == 1) {
                ...
                mfei = mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_GENEVE, &path,
...
static clib_error_t *
geneve_add_del_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
VLIB_CLI_COMMAND(create_geneve_tunnel_command, static) = {
    ...
    .short_help = "create geneve tunnel local <local-vtep-addr>"
...
static clib_error_t *
show_geneve_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    geneve_main_t *vxm = &geneve_main;
    geneve_tunnel_t *t;
    ...
    pool_foreach(t, vxm->tunnels, ({ vlib_cli_output(vm, "%U", format_geneve_tunnel, t); }));
...
static clib_error_t *
set_ip_geneve_bypass(u32 is_ip6, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
set_ip4_geneve_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
set_ip6_geneve_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
geneve_init(vlib_main_t *vm)
{
    ...
    udp_register_dst_port(vm, UDP_DST_PORT_geneve, geneve4_input_node.index, /* is_ip4 */ 1);
    udp_register_dst_port(vm, UDP_DST_PORT_geneve6, geneve6_input_node.index, /* is_ip4 */ 0);
