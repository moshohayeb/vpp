...
always_inline uword
geneve_encap_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u32 is_ip4)
{
...
#undef _
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                geneve_encap_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->tunnel_index         = t0 - vxm->tunnels;
                tr->vni                  = t0->vni;
            }

            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                geneve_encap_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
...
static uword
geneve4_encap(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return geneve_encap_inline(vm, node, from_frame, /* is_ip4 */ 1);
}

static uword
geneve6_encap(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
