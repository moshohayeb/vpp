...
format_geneve_rx_trace(u8 *s, va_list *args)
{
    ...
    if (t->tunnel_index != ~0) {
        s = format(s, "GENEVE decap from geneve_tunnel%d vni %d next %d error %d", t->tunnel_index, t->vni_rsvd,
...
always_inline uword
geneve_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u32 is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            /* pop (ip, udp, geneve) */
            if (is_ip4) {
                vlib_buffer_advance(b0, sizeof(*ip4_0) + sizeof(udp_header_t) + GENEVE_BASE_HEADER_LENGTH +
                                            vnet_get_geneve_options_len(geneve0));
                vlib_buffer_advance(b1, sizeof(*ip4_1) + sizeof(udp_header_t) + GENEVE_BASE_HEADER_LENGTH +
                                            vnet_get_geneve_options_len(geneve1));
            } else {
                vlib_buffer_advance(b0, sizeof(*ip6_0) + sizeof(udp_header_t) + GENEVE_BASE_HEADER_LENGTH +
                                            vnet_get_geneve_options_len(geneve0));
                vlib_buffer_advance(b0, sizeof(*ip6_0) + sizeof(udp_header_t) + GENEVE_BASE_HEADER_LENGTH +
                                            vnet_get_geneve_options_len(geneve1));
            }
            ...
            if (is_ip4) {
                ...
                if (PREDICT_TRUE(ip4_0->dst_address.as_u32 == t0->local.ip4.as_u32))
            ...
            } else /* !is_ip4 */
            {
                ...
                /* Validate GENEVE tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip6_address_is_equal(&ip6_0->dst_address, &t0->local.ip6)))
                    goto next0; /* valid packet */
        ...
        trace0:
            ...
            if (is_ip4) {
                ...
                /* Validate GENEVE tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip4_1->dst_address.as_u32 == t1->local.ip4.as_u32))
            ...
            } else /* !is_ip4 */
            {
                ...
                /* Validate GENEVE tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip6_address_is_equal(&ip6_1->dst_address, &t1->local.ip6)))
                    goto next1; /* valid packet */
    ...
    vlib_node_increment_counter(vm, is_ip4 ? geneve4_input_node.index : geneve6_input_node.index,
...
static uword
geneve4_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return geneve_input(vm, node, from_frame, /* is_ip4 */ 1);
}

static uword
geneve6_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword
ip_geneve_bypass_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 is_ip4)
{
    geneve_main_t *vxm = &geneve_main;
...
static uword
ip4_geneve_bypass(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
clib_error_t *
ip4_geneve_bypass_init(vlib_main_t *vm)
{
    return 0;
}
...
static uword
ip6_geneve_bypass(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
