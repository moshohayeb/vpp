...
format_vxlan_gbp_encap_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "VXLAN_GBP encap to vxlan_gbp_tunnel%d vni %d sclass %d", t->tunnel_index, t->vni, t->sclass);
    return s;
}

always_inline uword
vxlan_gbp_encap_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u8 is_ip4, u8 csum_offload)
{
    ...
    u8 const underlay_hdr_len = is_ip4 ? sizeof(ip4_vxlan_gbp_header_t) : sizeof(ip6_vxlan_gbp_header_t);
    ...
    u32 const csum_flags      = is_ip4 ?
                               VNET_BUFFER_F_OFFLOAD_IP_CKSUM | VNET_BUFFER_F_IS_IP4 | VNET_BUFFER_F_OFFLOAD_UDP_CKSUM :
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            vxlan_gbp0->sclass  = clib_host_to_net_u16(vnet_buffer2(b0)->gbp.src_epg);
            vxlan_gbp1->sclass  = clib_host_to_net_u16(vnet_buffer2(b1)->gbp.src_epg);
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                vxlan_gbp_encap_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                vxlan_gbp_encap_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
    ...
    vlib_node_increment_counter(vm, node->node_index, VXLAN_GBP_ENCAP_ERROR_ENCAPSULATED, pkts_encapsulated);

    return from_frame->n_vectors;
}

static uword
vxlan4_gbp_encap(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword
vxlan6_gbp_encap(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
