...
static void
vl_api_sw_interface_set_vxlan_gbp_bypass_t_handler(vl_api_sw_interface_set_vxlan_gbp_bypass_t *mp)
{
...
static void
vl_api_vxlan_gbp_tunnel_add_del_t_handler(vl_api_vxlan_gbp_tunnel_add_del_t *mp)
{
    ...
    fib_index = fib_table_find(fib_proto_from_ip46(itype), ntohl(mp->tunnel.encap_table_id));
    if (fib_index == ~0) {
...
out:
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_VXLAN_GBP_TUNNEL_ADD_DEL_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
    /* *INDENT-ON* */
}

static void
send_vxlan_gbp_tunnel_details(vxlan_gbp_tunnel_t *t, vl_api_registration_t *reg, u32 context)
{
    vl_api_vxlan_gbp_tunnel_details_t *rmp;
    ip46_type_t itype = (ip46_address_is_ip4(&t->dst) ? IP46_TYPE_IP4 : IP46_TYPE_IP6);
    ...
    rmp->tunnel.encap_table_id = fib_table_get_table_id(t->encap_fib_index, fib_proto_from_ip46(itype));
...
vl_api_vxlan_gbp_tunnel_dump_t_handler(vl_api_vxlan_gbp_tunnel_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        /* *INDENT-OFF* */
        pool_foreach(t, vxm->tunnels, ({ send_vxlan_gbp_tunnel_details(t, reg, mp->context); }));
        /* *INDENT-ON* */
    } else {
