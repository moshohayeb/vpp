...
format_vxlan_gbp_tunnel(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               " sw-if-idx %d ",
               t->dev_instance, t->user_instance, format_ip46_address, &t->src, IP46_TYPE_ANY, format_ip46_address,
               &t->dst, IP46_TYPE_ANY, t->vni, t->encap_fib_index, t->sw_if_index);
...
static clib_error_t *
vxlan_gbp_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
...
vxlan_gbp_tunnel_restack_dpo(vxlan_gbp_tunnel_t *t)
{
    ...
    u32 encap_index = is_ip4 ? vxlan4_gbp_encap_node.index : vxlan6_gbp_encap_node.index;
...
vxlan_gbp_tunnel_from_fib_node(fib_node_t *node)
{
    ...
    return ((vxlan_gbp_tunnel_t *) (((char *) node) - STRUCT_OFFSET_OF(vxlan_gbp_tunnel_t, node)));
}
...
static bool
vxlan_gbp_decap_next_is_valid(vxlan_gbp_main_t *vxm, u32 is_ip6, u32 decap_next_index)
{
    vlib_main_t *vm        = vxm->vlib_main;
    u32 input_idx          = (!is_ip6) ? vxlan4_gbp_input_node.index : vxlan6_gbp_input_node.index;
...
{
    uword *vtep = ip46_address_is_ip4(ip) ? hash_get(vxlan_gbp_main.vtep4, ip->ip4.as_u32) :
                                            hash_get_mem(vxlan_gbp_main.vtep6, &ip->ip6);
    if (vtep)
        return ++(*vtep);
    ip46_address_is_ip4(ip) ? hash_set(vxlan_gbp_main.vtep4, ip->ip4.as_u32, 1) :
...
vtep_addr_unref(ip46_address_t *ip)
{
    ...
    if (--(*vtep) != 0)
        return *vtep;
    ip46_address_is_ip4(ip) ? hash_unset(vxlan_gbp_main.vtep4, ip->ip4.as_u32) :
...
int
vnet_vxlan_gbp_tunnel_add_del(vnet_vxlan_gbp_tunnel_add_del_args_t *a, u32 *sw_if_indexp)
{
    vxlan_gbp_main_t *vxm = &vxlan_gbp_main;
    ...
    if (!is_ip6) {
        ...
        key4.key[1] = (((u64) a->encap_fib_index) << 32) | clib_host_to_net_u32(a->vni << 8);
        not_found   = clib_bihash_search_inline_16_8(&vxm->vxlan4_gbp_tunnel_by_key, &key4);
        p           = &key4.value;
    } else {
        key6.key[0] = a->dst.ip6.as_u64[0];
        key6.key[1] = a->dst.ip6.as_u64[1];
        key6.key[2] = (((u64) a->encap_fib_index) << 32) | clib_host_to_net_u32(a->vni << 8);
        not_found   = clib_bihash_search_inline_24_8(&vxm->vxlan6_gbp_tunnel_by_key, &key6);
        p           = &key6.value;
    }
...
#undef _
        ...
        if (is_ip6) {
            ...
            add_failed = clib_bihash_add_del_24_8(&vxm->vxlan6_gbp_tunnel_by_key, &key6, 1 /*add */);
        } else {
            key4.value = (u64) dev_instance;
            add_failed = clib_bihash_add_del_16_8(&vxm->vxlan4_gbp_tunnel_by_key, &key4, 1 /*add */);
        }
        ...
        t->hw_if_index          = vnet_register_interface(vnm, vxlan_gbp_device_class.index, dev_instance,
                                                 vxlan_gbp_hw_class.index, dev_instance);
        vnet_hw_interface_t *hi = vnet_get_hw_interface(vnm, t->hw_if_index);

        /* Set vxlan_gbp tunnel output node */
        u32 encap_index = !is_ip6 ? vxlan4_gbp_encap_node.index : vxlan6_gbp_encap_node.index;
        ...
        if (!ip46_address_is_multicast(&t->dst)) {
            ...
            vtep_addr_ref(&t->src);
            t->fib_entry_index =
                fib_table_entry_special_add(t->encap_fib_index, &tun_dst_pfx, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
            t->sibling_index = fib_entry_child_add(t->fib_entry_index, FIB_NODE_TYPE_VXLAN_GBP_TUNNEL, dev_instance);
            ...
            if (vtep_addr_ref(&t->dst) == 1) {
                 ...
                 */
                mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_VXLAN_GBP, &path,
                                             MFIB_ITF_FLAG_FORWARD);

                path.frp_sw_if_index = a->mcast_sw_if_index;
                path.frp_flags       = FIB_ROUTE_PATH_FLAG_NONE;
                mfei = mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_VXLAN_GBP, &path,
        ...
        if (!is_ip6)
            clib_bihash_add_del_16_8(&vxm->vxlan4_gbp_tunnel_by_key, &key4, 0 /*del */);
        else
            clib_bihash_add_del_24_8(&vxm->vxlan6_gbp_tunnel_by_key, &key6, 0 /*del */);
...
get_decap_next_for_node(u32 node_index, u32 ipv4_set)
{
    vxlan_gbp_main_t *vxm = &vxlan_gbp_main;
    vlib_main_t *vm       = vxm->vlib_main;
    uword input_node      = (ipv4_set) ? vxlan4_gbp_input_node.index : vxlan6_gbp_input_node.index;
...
static clib_error_t *
vxlan_gbp_tunnel_add_del_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        } else if (unformat(line_input, "group %U %U", unformat_ip46_address, &dst, IP46_TYPE_ANY,
                            unformat_vnet_sw_interface, vnet_get_main(), &mcast_sw_if_index)) {
            grp_set = dst_set = 1;
            ip46_address_is_ip4(&dst) ? (ipv4_set = 1) : (ipv6_set = 1);
    ...
    vnet_vxlan_gbp_tunnel_add_del_args_t a = {.is_add   = is_add,
                                              .is_ip6   = ipv6_set,
                                              .instance = instance,
#define _(x) .x = x,
                                              foreach_copy_field
#undef _
    ...
    default:
        return clib_error_return(0, "vnet_vxlan_gbp_tunnel_add_del returned %d", rv);
    }
...
VLIB_CLI_COMMAND(create_vxlan_gbp_tunnel_command, static) = {
    ...
    .short_help = "create vxlan-gbp tunnel src <local-vtep-addr>"
...
static clib_error_t *
show_vxlan_gbp_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vxlan_gbp_main_t *vxm = &vxlan_gbp_main;
    vxlan_gbp_tunnel_t *t;
    int raw = 0;
    ...
    /* *INDENT-OFF* */
    pool_foreach(t, vxm->tunnels, ({ vlib_cli_output(vm, "%U", format_vxlan_gbp_tunnel, t); }));
    /* *INDENT-ON* */
...
{
    if (is_ip6)
        vnet_feature_enable_disable("ip6-unicast", "ip6-vxlan-gbp-bypass", sw_if_index, is_enable, 0, 0);
    else
        vnet_feature_enable_disable("ip4-unicast", "ip4-vxlan-gbp-bypass", sw_if_index, is_enable, 0, 0);
}


static clib_error_t *
set_ip_vxlan_gbp_bypass(u32 is_ip6, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
set_ip4_vxlan_gbp_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
set_ip6_vxlan_gbp_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
vxlan_gbp_init(vlib_main_t *vm)
{
    ...
    clib_bihash_init_16_8(&vxm->vxlan4_gbp_tunnel_by_key, "vxlan4-gbp", VXLAN_GBP_HASH_NUM_BUCKETS,
                          VXLAN_GBP_HASH_MEMORY_SIZE);
    clib_bihash_init_24_8(&vxm->vxlan6_gbp_tunnel_by_key, "vxlan6-gbp", VXLAN_GBP_HASH_NUM_BUCKETS,
                          VXLAN_GBP_HASH_MEMORY_SIZE);
    ...
    udp_register_dst_port(vm, UDP_DST_PORT_vxlan_gbp, vxlan4_gbp_input_node.index, /* is_ip4 */ 1);
    udp_register_dst_port(vm, UDP_DST_PORT_vxlan6_gbp, vxlan6_gbp_input_node.index, /* is_ip4 */ 0);
