...
format_vxlan_gbp_rx_trace(u8 *s, va_list *args)
{
    ...
    if (t->tunnel_index == ~0)
        return format(s, "VXLAN_GBP decap error - tunnel for vni %d does not exist", t->vni);
    return format(s,
...
vxlan4_gbp_find_tunnel(vxlan_gbp_main_t *vxm, last_tunnel_cache4 *cache, u32 fib_index, ip4_header_t *ip4_0,
                       vxlan_gbp_header_t *vxlan_gbp0, vxlan_gbp_tunnel_t **stats_t0)
{
    ...
    if (PREDICT_FALSE(key4.key[1] != cache->key[1] || ip4_0->src_address.as_u32 != (u32) cache->key[0])) {
        key4.key[0] = ip4_0->src_address.as_u32;
        int rv      = clib_bihash_search_inline_16_8(&vxm->vxlan4_gbp_tunnel_by_key, &key4);
...
always_inline vxlan_gbp_tunnel_t *
vxlan6_gbp_find_tunnel(vxlan_gbp_main_t *vxm, last_tunnel_cache6 *cache, u32 fib_index, ip6_header_t *ip6_0,
                       vxlan_gbp_header_t *vxlan_gbp0, vxlan_gbp_tunnel_t **stats_t0)
{
    /* Make sure VXLAN_GBP tunnel exist according to packet SIP and VNI */
    vxlan6_gbp_tunnel_key_t key6 = {.key = {
    ...
    if (PREDICT_FALSE(clib_bihash_key_compare_24_8(key6.key, cache->key) == 0)) {
        int rv = clib_bihash_search_inline_24_8(&vxm->vxlan6_gbp_tunnel_by_key, &key6);
        if (PREDICT_FALSE(rv != 0))
            return 0;
...
always_inline uword
vxlan_gbp_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u32 is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (is_ip4) {
                t0 = vxlan4_gbp_find_tunnel(vxm, &last4, fi0, ip4_0, vxlan_gbp0, &stats_t0);
                t1 = vxlan4_gbp_find_tunnel(vxm, &last4, fi1, ip4_1, vxlan_gbp1, &stats_t1);
            } else {
                t0 = vxlan6_gbp_find_tunnel(vxm, &last6, fi0, ip6_0, vxlan_gbp0, &stats_t0);
                t1 = vxlan6_gbp_find_tunnel(vxm, &last6, fi1, ip6_1, vxlan_gbp1, &stats_t1);
            }
            ...
            if (PREDICT_FALSE(t0 == 0 || flags0 != (VXLAN_GBP_FLAGS_I | VXLAN_GBP_FLAGS_G))) {
                next0 = VXLAN_GBP_INPUT_NEXT_DROP;

                if (t0 != 0 && flags0 != (VXLAN_GBP_FLAGS_I | VXLAN_GBP_FLAGS_G)) {
                    error0 = VXLAN_GBP_ERROR_BAD_FLAGS;
                    vlib_increment_combined_counter(drop_counter, thread_index, stats_t0->sw_if_index, 1, len0);
                } else
                ...
                vlib_increment_combined_counter(rx_counter, thread_index, stats_t0->sw_if_index, 1, len0);
                pkts_decapsulated++;
            }

            /* Validate VXLAN_GBP tunnel encap-fib index against packet */
            if (PREDICT_FALSE(t1 == 0 || flags1 != (VXLAN_GBP_FLAGS_I | VXLAN_GBP_FLAGS_G))) {
                next1 = VXLAN_GBP_INPUT_NEXT_DROP;

                if (t1 != 0 && flags1 != (VXLAN_GBP_FLAGS_I | VXLAN_GBP_FLAGS_G)) {
                    error1 = VXLAN_GBP_ERROR_BAD_FLAGS;
                    vlib_increment_combined_counter(drop_counter, thread_index, stats_t1->sw_if_index, 1, len1);
                } else
                ...
                vlib_increment_combined_counter(rx_counter, thread_index, stats_t1->sw_if_index, 1, len1);
            }

            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                vxlan_gbp_rx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->next_index           = next0;
                tr->error                = error0;
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                vxlan_gbp_rx_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
                tr->next_index           = next1;
                tr->error                = error1;
    ...
    u32 node_idx = is_ip4 ? vxlan4_gbp_input_node.index : vxlan6_gbp_input_node.index;
    vlib_node_increment_counter(vm, node_idx, VXLAN_GBP_ERROR_DECAPSULATED, pkts_decapsulated);

    return from_frame->n_vectors;
}

static uword
vxlan4_gbp_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return vxlan_gbp_input(vm, node, from_frame, /* is_ip4 */ 1);
}

static uword
vxlan6_gbp_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword
ip_vxlan_gbp_bypass_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 is_ip4)
{
    vxlan_gbp_main_t *vxm = &vxlan_gbp_main;
    ...
    while (n_left_from > 0) {
        ...
        exit0:
            ...
            next1     = error1 ? IP_VXLAN_GBP_BYPASS_NEXT_DROP : IP_VXLAN_GBP_BYPASS_NEXT_VXLAN_GBP;
...
static uword
ip4_vxlan_gbp_bypass(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ip6_vxlan_gbp_bypass(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
