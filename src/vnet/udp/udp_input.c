...
always_inline uword
udp46_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (is_ip4) {
                ...
                s0       = session_lookup_safe4(fib_index0, &ip40->dst_address, &ip40->src_address, udp0->dst_port,
                ...
                s0       = session_lookup_safe6(fib_index0, &ip60->dst_address, &ip60->src_address, udp0->dst_port,
                                          udp0->src_port, TRANSPORT_PROTO_UDP);
            ...
            if (s0->session_state == SESSION_STATE_OPENED) {
                ...
                if (uc0->is_connected) {
                     ...
                     */
                    new_uc0 = udp_connection_clone_safe(s0->connection_index, s0->thread_index);
                    ...
                    session_dgram_connect_notify(&new_uc0->connection, s0->thread_index, &s0);
                ...
                if (uc0->is_connected) {
                    ...
                    if (stream_session_accept(&child0->connection, tc0->s_index, 1)) {
            ...
            if (!uc0->is_connected) {
                if (svm_fifo_max_enqueue(s0->server_rx_fifo) < b0->current_length + sizeof(session_dgram_hdr_t)) {
                ...
                wrote0 = session_enqueue_dgram_connection(s0, &hdr0, b0, TRANSPORT_PROTO_UDP, 1 /* queue evt */);
                ...
                if (svm_fifo_max_enqueue(s0->server_rx_fifo) < b0->current_length) {
                    error0 = UDP_ERROR_FIFO_FULL;
                    goto trace0;
                }
                wrote0 = session_enqueue_stream_connection(tc0, b0, 0, 1 /* queue evt */, 1 /* in order */);
                ASSERT(wrote0 > 0);
            }
        ...
        trace0:
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                udp_input_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
