...
udp_connection_alloc(u32 thread_index)
{
    ...
    udp_connection_t *uc;
    u32 will_expand = 0;
    pool_get_aligned_will_expand(um->connections[thread_index], will_expand, CLIB_CACHE_LINE_BYTES);

    if (PREDICT_FALSE(will_expand)) {
        clib_spinlock_lock_if_init(&udp_main.peekers_write_locks[thread_index]);
        pool_get_aligned(udp_main.connections[thread_index], uc, CLIB_CACHE_LINE_BYTES);
        clib_spinlock_unlock_if_init(&udp_main.peekers_write_locks[thread_index]);
    } else {
        pool_get_aligned(um->connections[thread_index], uc, CLIB_CACHE_LINE_BYTES);
    }
...
udp_session_bind(u32 session_index, transport_endpoint_t *lcl)
{
    ...
    udp_register_dst_port(vm, clib_net_to_host_u16(lcl->port), node_index, 1 /* is_ipv4 */);
...
udp_session_unbind(u32 listener_index)
{
    ...
    udp_unregister_dst_port(vm, clib_net_to_host_u16(listener->c_lcl_port), listener->c_is_ip4);
    return 0;
}
...
udp_push_header(transport_connection_t *tc, vlib_buffer_t *b)
{
    ...
    if (tc->is_ip4)
        vlib_buffer_push_ip4(vm, b, &uc->c_lcl_ip4, &uc->c_rmt_ip4, IP_PROTOCOL_UDP, 1);
    else {
        ip6_header_t *ih;
        ih                            = vlib_buffer_push_ip6(vm, b, &uc->c_lcl_ip6, &uc->c_rmt_ip6, IP_PROTOCOL_UDP);
...
udp_session_close(u32 connection_index, u32 thread_index)
{
    ...
    uc = udp_connection_get(connection_index, thread_index);
    if (uc) {
        udp_unregister_dst_port(vm, clib_net_to_host_u16(uc->c_lcl_port), uc->c_is_ip4);
...
format_udp_connection_id(u8 *s, va_list *args)
{
    ...
    if (!uc)
        return s;
    if (uc->c_is_ip4)
        s = format(s, "[#%d][%s] %U:%d->%U:%d", uc->c_thread_index, "U", format_ip4_address, &uc->c_lcl_ip4,
                   clib_net_to_host_u16(uc->c_lcl_port), format_ip4_address, &uc->c_rmt_ip4,
                   clib_net_to_host_u16(uc->c_rmt_port));
    else
        s = format(s, "[#%d][%s] %U:%d->%U:%d", uc->c_thread_index, "U", format_ip6_address, &uc->c_lcl_ip6,
                   clib_net_to_host_u16(uc->c_lcl_port), format_ip6_address, &uc->c_rmt_ip6,
                   clib_net_to_host_u16(uc->c_rmt_port));
    return s;
}
...
udp_open_connection(transport_endpoint_t *rmt)
{
    ...
    if (transport_alloc_local_endpoint(TRANSPORT_PROTO_UDP, rmt, &lcl_addr, &lcl_port))
        return -1;
...
udp_init(vlib_main_t *vm)
{
    ...
    transport_register_protocol(TRANSPORT_PROTO_UDP, &udp_proto, FIB_PROTOCOL_IP4, ip4_lookup_node.index);
    transport_register_protocol(TRANSPORT_PROTO_UDP, &udp_proto, FIB_PROTOCOL_IP6, ip6_lookup_node.index);
    transport_register_protocol(TRANSPORT_PROTO_UDPC, &udpc_proto, FIB_PROTOCOL_IP4, ip4_lookup_node.index);
    transport_register_protocol(TRANSPORT_PROTO_UDPC, &udpc_proto, FIB_PROTOCOL_IP6, ip6_lookup_node.index);
