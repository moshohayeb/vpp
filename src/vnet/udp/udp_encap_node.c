...
format_udp4_encap_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U\n  %U", format_ip4_header, &t->ip, sizeof(t->ip), format_udp_header, &t->udp, sizeof(t->udp));
    return (s);
}
...
format_udp6_encap_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U\n  %U", format_ip6_header, &t->ip, sizeof(t->ip), format_udp_header, &t->udp, sizeof(t->udp));
    return (s);
}

always_inline uword
udp_encap_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_encap_v6)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (is_encap_v6) {
                ...
                ip_udp_encap_two(vm, b0, b1, (u8 *) &ue0->ue_hdrs, (u8 *) &ue1->ue_hdrs, n_bytes, 0);
                ...
                ip_udp_encap_two(vm, b0, b1, (u8 *) &ue0->ue_hdrs, (u8 *) &ue1->ue_hdrs, n_bytes, 1);
...
static uword
udp4_encap(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return udp_encap_inline(vm, node, frame, 0);
}

static uword
udp6_encap(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
