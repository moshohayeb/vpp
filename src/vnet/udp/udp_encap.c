...
{
    dpo_stack(udp_encap_dpo_types[ue->ue_ip_proto], fib_proto_to_dpo(ue->ue_ip_proto), &ue->ue_dpo,
              fib_entry_contribute_ip_forwarding(ue->ue_fib_entry_index));
}

index_t
udp_encap_add_and_lock(fib_protocol_t proto, index_t fib_index, const ip46_address_t *src_ip,
                       const ip46_address_t *dst_ip, u16 src_port, u16 dst_port, udp_encap_fixup_flags_t flags)
{
    udp_encap_t *ue;
    u8 pfx_len = 0;
    index_t uei;
    ...
    ue->ue_fib_entry_index = fib_table_entry_special_add(fib_index, &dst_pfx, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
    ue->ue_fib_sibling     = fib_entry_child_add(ue->ue_fib_entry_index, FIB_NODE_TYPE_UDP_ENCAP, uei);
...
void
udp_encap_contribute_forwarding(index_t uei, dpo_proto_t proto, dpo_id_t *dpo)
{
...
format_udp_encap_i(u8 *s, va_list *args)
{
    ...
    if (FIB_PROTOCOL_IP4 == ue->ue_ip_proto) {
        s = format(s, "ip:[src:%U, dst:%U] udp:[src:%d, dst:%d]", format_ip4_address,
                   &ue->ue_hdrs.ip4.ue_ip4.src_address, format_ip4_address, &ue->ue_hdrs.ip4.ue_ip4.dst_address,
                   clib_net_to_host_u16(ue->ue_hdrs.ip4.ue_udp.src_port),
                   clib_net_to_host_u16(ue->ue_hdrs.ip4.ue_udp.dst_port));
    } else {
        s = format(s, "ip:[src:%U, dst:%U] udp:[src:%d dst:%d]", format_ip6_address,
                   &ue->ue_hdrs.ip6.ue_ip6.src_address, format_ip6_address, &ue->ue_hdrs.ip6.ue_ip6.dst_address,
...
clib_error_t *
udp_encap_cli(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
...
clib_error_t *
udp_encap_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    index_t uei;
    ...
    if (INDEX_INVALID == uei) {
        /* *INDENT-OFF* */
        pool_foreach_index(uei, udp_encap_pool, ({ vlib_cli_output(vm, "%U", format_udp_encap, uei, 0); }));
        /* *INDENT-ON* */
    } else {
...
VLIB_CLI_COMMAND(udp_encap_add_command, static) = {
    .path       = "udp encap",
    .short_help = "udp encap [add|del] <id ID> <src-ip> <dst-ip> [<src-port>] <dst-port>  [src-port-is-entropy] "
                  "[table-id <table>]",
