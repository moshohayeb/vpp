...
format_udp_rx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "UDP: src-port %d dst-port %d%s", clib_net_to_host_u16(t->src_port),
...
always_inline uword
udp46_local_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            sparse_vec_index2(is_ip4 ? um->next_by_dst_port4 : um->next_by_dst_port6, dst_port0, dst_port1, &i0, &i1);
            next0 = (error0 == 0) ? vec_elt(is_ip4 ? um->next_by_dst_port4 : um->next_by_dst_port6, i0) : next0;
            next1 = (error1 == 0) ? vec_elt(is_ip4 ? um->next_by_dst_port4 : um->next_by_dst_port6, i1) : next1;
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ...
                if (b0->error != node->errors[UDP_ERROR_LENGTH_ERROR]) {
                    ...
                    tr->bound    = (next0 != UDP_LOCAL_NEXT_ICMP4_ERROR && next0 != UDP_LOCAL_NEXT_ICMP6_ERROR);
                }
            }
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                ...
                if (b1->error != node->errors[UDP_ERROR_LENGTH_ERROR]) {
                    ...
                    tr->bound    = (next1 != UDP_LOCAL_NEXT_ICMP4_ERROR && next1 != UDP_LOCAL_NEXT_ICMP6_ERROR);
                }
            }
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(clib_net_to_host_u16(h0->length) <= vlib_buffer_length_in_chain(vm, b0))) {
                i0    = sparse_vec_index(is_ip4 ? um->next_by_dst_port4 : um->next_by_dst_port6, h0->dst_port);
                next0 = vec_elt(is_ip4 ? um->next_by_dst_port4 : um->next_by_dst_port6, i0);
...
static uword
udp4_local(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return udp46_local_inline(vm, node, from_frame, 1 /* is_ip4 */);
}

static uword
udp6_local(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static void
add_dst_port(udp_main_t *um, udp_dst_port_t dst_port, char *dst_port_name, u8 is_ip4)
{
