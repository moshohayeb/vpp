...
static void
send_udp_encap_details(const udp_encap_t *ue, vl_api_registration_t *reg, u32 context)
{
...
vl_api_udp_encap_dump_t_handler(vl_api_udp_encap_dump_t *mp, vlib_main_t *vm)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(ue, udp_encap_pool, ({ send_udp_encap_details(ue, reg, mp->context); }));
    /* *INDENT-ON* */
}
...
vl_api_udp_encap_add_t_handler(vl_api_udp_encap_add_t *mp, vlib_main_t *vm)
{
    ...
    uei = udp_encap_add_and_lock(fproto, fib_index, &src_ip, &dst_ip, ntohs(mp->udp_encap.src_port),
