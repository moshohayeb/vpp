...
static inline uword
qos_mark_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, qos_source_t output_source, int is_ip6)
{
...
static inline uword
ip4_qos_mark(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_mark_inline(vm, node, frame, QOS_SOURCE_IP, 0));
}

static inline uword
ip6_qos_mark(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_mark_inline(vm, node, frame, QOS_SOURCE_IP, 1));
}

static inline uword
mpls_qos_mark(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_mark_inline(vm, node, frame, QOS_SOURCE_MPLS, 0));
}

static inline uword
vlan_mpls_qos_mark(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_mark_inline(vm, node, frame, QOS_SOURCE_VLAN, 0));
}

static inline uword
vlan_ip4_qos_mark(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_mark_inline(vm, node, frame, QOS_SOURCE_VLAN, 0));
}

static inline uword
vlan_ip6_qos_mark(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_mark_inline(vm, node, frame, QOS_SOURCE_VLAN, 0));
}
...
int
qos_mark_enable(u32 sw_if_index, qos_source_t output_source, qos_egress_map_id_t mid)
{
    index_t qemi;
...
static clib_error_t *
qos_mark_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
