...
static void
qos_record_feature_config(u32 sw_if_index, qos_source_t input_source, u8 enable)
{
    switch (input_source) {
    case QOS_SOURCE_IP:
        vnet_feature_enable_disable("ip6-unicast", "ip6-qos-record", sw_if_index, enable, NULL, 0);
        vnet_feature_enable_disable("ip6-multicast", "ip6-qos-record", sw_if_index, enable, NULL, 0);
        vnet_feature_enable_disable("ip4-unicast", "ip4-qos-record", sw_if_index, enable, NULL, 0);
        vnet_feature_enable_disable("ip4-multicast", "ip4-qos-record", sw_if_index, enable, NULL, 0);
    ...
    case QOS_SOURCE_VLAN:
        vnet_feature_enable_disable("ip6-unicast", "vlan-ip6-qos-record", sw_if_index, enable, NULL, 0);
        vnet_feature_enable_disable("ip6-multicast", "vlan-ip6-qos-record", sw_if_index, enable, NULL, 0);
        vnet_feature_enable_disable("ip4-unicast", "vlan-ip4-qos-record", sw_if_index, enable, NULL, 0);
        vnet_feature_enable_disable("ip4-multicast", "vlan-ip4-qos-record", sw_if_index, enable, NULL, 0);
...
static inline uword
qos_record_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, dpo_proto_t dproto, int is_l2)
{
    ...
    while (n_left_from > 0) {
        ...
        non_ip:
            if (is_l2) {
                vlib_buffer_advance(b0, -l2_len);
                next0 = vnet_l2_feature_next(b0, l2_qos_input_next[QOS_SOURCE_IP], L2INPUT_FEAT_L2_IP_QOS_RECORD);
            } else
...
static inline uword
ip4_qos_record(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_record_inline(vm, node, frame, DPO_PROTO_IP4, 0));
}

static inline uword
ip6_qos_record(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_record_inline(vm, node, frame, DPO_PROTO_IP6, 0));
}

static inline uword
mpls_qos_record(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_record_inline(vm, node, frame, DPO_PROTO_MPLS, 0));
}

static inline uword
vlan_ip4_qos_record(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_record_inline(vm, node, frame, DPO_PROTO_ETHERNET, 0));
}

static inline uword
vlan_ip6_qos_record(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_record_inline(vm, node, frame, DPO_PROTO_ETHERNET, 0));
}

static inline uword
vlan_mpls_qos_record(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_record_inline(vm, node, frame, DPO_PROTO_ETHERNET, 0));
}

static inline uword
l2_ip_qos_record(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
l2_ip_qos_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2_ip_qos_record_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(),
...
static clib_error_t *
qos_record_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
