...
bond_disable_collecting_distributing(vlib_main_t *vm, slave_if_t *sif)
{
    ...
    vec_foreach_index(i, bif->active_slaves)
    {
        ...
        if (p == sif->sw_if_index) {
            ...
            if ((vec_len(bif->active_slaves) >= 1) && (bif->mode == BOND_MODE_ACTIVE_BACKUP) && switching_active)
...
bond_enable_collecting_distributing(vlib_main_t *vm, slave_if_t *sif)
{
    ...
    if (!hash_get(bif->active_slave_by_sw_if_index, sif->sw_if_index)) {
        hash_set(bif->active_slave_by_sw_if_index, sif->sw_if_index, sif->sw_if_index);
        vec_add1(bif->active_slaves, sif->sw_if_index);

        /* First slave becomes active? */
        if ((vec_len(bif->active_slaves) == 1) && (bif->mode == BOND_MODE_ACTIVE_BACKUP))
            vlib_process_signal_event(bm->vlib_main, bond_process_node.index, BOND_SEND_GARP_NA, bif->hw_if_index);
    }
    clib_spinlock_unlock_if_init(&bif->lockp);
}
...
bond_dump_ifs(bond_interface_details_t **out_bondifs)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(bif, bm->interfaces, vec_add2(r_bondifs, bondif, 1); memset(bondif, 0, sizeof(*bondif));
                 bondif->sw_if_index = bif->sw_if_index; hi = vnet_get_hw_interface(vnm, bif->hw_if_index);
                 clib_memcpy(bondif->interface_name, hi->name,
                             MIN(ARRAY_LEN(bondif->interface_name) - 1, strlen((const char *) hi->name)));
                 bondif->mode = bif->mode; bondif->lb = bif->lb; bondif->active_slaves = vec_len(bif->active_slaves);
...
int
bond_dump_slave_ifs(slave_interface_details_t **out_slaveifs, u32 bond_sw_if_index)
{
    vnet_main_t *vnm = vnet_get_main();
    bond_if_t *bif;
    vnet_hw_interface_t *hi;
    ...
    vec_foreach(sw_if_index, bif->slaves)
    {
        ...
        if (sif) {
            ...
            clib_memcpy(slaveif->interface_name, hi->name,
                        MIN(ARRAY_LEN(slaveif->interface_name) - 1, strlen((const char *) hi->name)));
...
bond_delete_neighbor(vlib_main_t *vm, bond_if_t *bif, slave_if_t *sif)
{
    ...
    bif->port_number_bitmap = clib_bitmap_set(bif->port_number_bitmap, ntohs(sif->actor_admin.port_number) - 1, 0);
    ...
    vnet_feature_enable_disable("device-input", "bond-input", sif_hw->hw_if_index, 0, 0, 0);

    /* Put back the old mac */
    vnet_hw_interface_change_mac_address(vnm, sif_hw->hw_if_index, sif->persistent_hw_address);
...
bond_create_if(vlib_main_t *vm, bond_create_if_args_t *args)
{
    ...
    args->error =
        ethernet_register_interface(vnm, bond_dev_class.index, bif - bm->interfaces /* device instance */,
                                    bif->hw_address /* ethernet address */, &bif->hw_if_index, 0 /* flag change */);
...
static clib_error_t *
bond_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
bond_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    u32 sw_if_index  = ~0;
    vnet_main_t *vnm = vnet_get_main();
    ...
    if (sw_if_index == ~0)
        return clib_error_return(0, "please specify interface name or sw_if_index");
...
bond_enslave(vlib_main_t *vm, bond_enslave_args_t *args)
{
    ...
    if (sif_hw->dev_class_index == bond_dev_class.index) {
        args->rv    = VNET_API_ERROR_INVALID_INTERFACE;
        args->error = clib_error_return(0, "bond interface cannot be enslaved");
        return;
    }
    ...
    vec_validate_aligned(bm->slave_by_sw_if_index, sif->sw_if_index, CLIB_CACHE_LINE_BYTES);
    /*
     ...
     */
    bm->slave_by_sw_if_index[sif->sw_if_index] = (uword)(((sif - bm->neighbors) << 1) | 1);
    ...
    if (bif->use_custom_mac) {
        vnet_hw_interface_change_mac_address(vnm, sif_hw->hw_if_index, bif->hw_address);
    } else {
        // bond interface gets the mac address from the first slave
        if (vec_len(bif->slaves) == 1) {
            memcpy(bif->hw_address, sif_hw->hw_address, 6);
            vnet_hw_interface_change_mac_address(vnm, bif_hw->hw_if_index, sif_hw->hw_address);
        } else {
    ...
    vec_foreach_index(thread_index, bm->per_thread_data)
    {
        ...
        vec_validate_aligned(ptd->per_port_queue, sif->sw_if_index, CLIB_CACHE_LINE_BYTES);
    ...
    args->rv = vnet_feature_enable_disable("device-input", "bond-input", sif_hw->hw_if_index, 1, 0, 0);
...
static clib_error_t *
enslave_interface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    args.group = ~0;
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U %U", unformat_vnet_sw_interface, vnm, &args.group, unformat_vnet_sw_interface, vnm,
                     &args.slave))
            ;
...
static clib_error_t *
detach_interface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    args.slave = ~0;
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U", unformat_vnet_sw_interface, vnm, &args.slave))
            ;
        else {
            args.error = clib_error_return(0, "unknown input `%U'", format_unformat_error, input);
            break;
...
show_bond(vlib_main_t *vm)
{
    ...
    vlib_cli_output(vm, "%-16s %-12s %-13s %-13s %-14s %s", "interface name", "sw_if_index", "mode", "load balance",
                    "active slaves", "slaves");

    /* *INDENT-OFF* */
    pool_foreach(bif, bm->interfaces, ({
                     vlib_cli_output(vm, "%-16U %-12d %-13U %-13U %-14u %u", format_bond_interface_name,
                                     bif->dev_instance, bif->sw_if_index, format_bond_mode, bif->mode,
                                     format_bond_load_balance, bif->lb, vec_len(bif->active_slaves),
                                     vec_len(bif->slaves));
                 }));
    /* *INDENT-ON* */
}
...
show_bond_details(vlib_main_t *vm)
{
    ...
    pool_foreach(bif, bm->interfaces, ({
                     ...
                     if (bif->mode == BOND_MODE_ROUND_ROBIN)
                         vlib_cli_output(vm, "  last xmit slave index: %u", bif->lb_rr_last_index);
                     vlib_cli_output(vm, "  number of active slaves: %d", vec_len(bif->active_slaves));
...
static clib_error_t *
show_bond_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u8 details = 0;
