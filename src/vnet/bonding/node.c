...
format_bond_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "src %U, dst %U, %U -> %U", format_ethernet_address, t->ethernet.src_address, format_ethernet_address,
               t->ethernet.dst_address, format_vnet_sw_if_index_name, vnet_get_main(), t->sw_if_index,
               format_vnet_sw_if_index_name, vnet_get_main(), t->bond_sw_if_index);
...
packet_is_cdp(ethernet_header_t *eth)
{
    ...
    return ((eth->type == htons(ETHERNET_TYPE_CDP)) ||
            ((llc->src_sap == 0xAA) && (llc->control == 0x03) && (snap->protocol == htons(0x2000)) &&
             (snap->oui[0] == 0) && (snap->oui[1] == 0) && (snap->oui[2] == 0x0C)));
}

static inline u32
bond_sw_if_idx_rewrite(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b, u32 bond_sw_if_index)
{
    ...
    if (!ethernet_frame_is_tagged(ntohs(ethertype))) {
        ...
        if (PREDICT_TRUE((ethertype != htons(ETHERNET_TYPE_SLOW_PROTOCOLS)) &&
                         (ethertype != htons(ETHERNET_TYPE_CDP)) && (ethertype != htons(ETHERNET_TYPE_802_1_LLDP)))) {
            /* Change the physical interface to bond interface */
            vnet_buffer(b)->sw_if_index[VLIB_RX] = bond_sw_if_index;
            return 1;
        }
...
static inline void
bond_update_next(vlib_main_t *vm, vlib_node_runtime_t *node, u32 *last_slave_sw_if_index, u32 slave_sw_if_index,
                 u32 packet_count, u32 *bond_sw_if_index, vlib_buffer_t *b, u32 *next_index, vlib_error_t *error)
{
    ...
    if (packet_count)
        vlib_increment_simple_counter(vnet_main.interface_main.sw_if_counters + VNET_INTERFACE_COUNTER_RX, thread_index,
...
VLIB_NODE_FN(bond_input_node)(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left >= 4) {
        ...
        if (PREDICT_TRUE(n_left >= 16)) {
            CLIB_PREFETCH(vlib_buffer_get_current(b[8]), CLIB_CACHE_LINE_BYTES, LOAD);
            CLIB_PREFETCH(vlib_buffer_get_current(b[9]), CLIB_CACHE_LINE_BYTES, LOAD);
            CLIB_PREFETCH(vlib_buffer_get_current(b[10]), CLIB_CACHE_LINE_BYTES, LOAD);
            CLIB_PREFETCH(vlib_buffer_get_current(b[11]), CLIB_CACHE_LINE_BYTES, LOAD);
        ...
        if (PREDICT_TRUE(x == 0)) {
            ...
            bond_update_next(vm, node, &last_slave_sw_if_index, sw_if_index[0], cnt, &bond_sw_if_index, b[0],
                             &next_index, &error);
            next[0] = next_index;
            if (next_index == BOND_INPUT_NEXT_DROP)
                b[0]->error = error;
            ...
            bond_update_next(vm, node, &last_slave_sw_if_index, sw_if_index[1], cnt, &bond_sw_if_index, b[1],
                             &next_index, &error);
            next[1] = next_index;
            if (next_index == BOND_INPUT_NEXT_DROP)
                b[1]->error = error;
            ...
            bond_update_next(vm, node, &last_slave_sw_if_index, sw_if_index[2], cnt, &bond_sw_if_index, b[2],
                             &next_index, &error);
            next[2] = next_index;
            if (next_index == BOND_INPUT_NEXT_DROP)
                b[2]->error = error;
            ...
            bond_update_next(vm, node, &last_slave_sw_if_index, sw_if_index[3], cnt, &bond_sw_if_index, b[3],
                             &next_index, &error);
            next[3] = next_index;
            if (next_index == BOND_INPUT_NEXT_DROP)
                b[3]->error = error;
    ...
    while (n_left) {
        sw_if_index[0] = vnet_buffer(b[0])->sw_if_index[VLIB_RX];
        bond_update_next(vm, node, &last_slave_sw_if_index, sw_if_index[0], cnt, &bond_sw_if_index, b[0], &next_index,
                         &error);
        next[0] = next_index;
        if (next_index == BOND_INPUT_NEXT_DROP)
            b[0]->error = error;
    ...
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
        ...
        while (n_left) {
            if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
                ...
                clib_memcpy(&t0->ethernet, vlib_buffer_get_current(b[0]), sizeof(ethernet_header_t));
