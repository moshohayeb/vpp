...
format_bond_tx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "src %U, dst %U, %s -> %s", format_ethernet_address, t->ethernet.src_address, format_ethernet_address,
...
bond_set_l2_mode_function(vnet_main_t *vnm, struct vnet_hw_interface_t *bif_hw, i32 l2_if_adjust)
{
    bond_if_t *bif;
    u32 *sw_if_index;
    ...
    if ((bif_hw->l2_if_count == 1) && (l2_if_adjust == 1)) {
        ...
        vec_foreach(sw_if_index, bif->slaves)
        {
            ...
            ethernet_set_flags(vnm, sif_hw->hw_if_index, ETHERNET_INTERFACE_FLAG_ACCEPT_ALL);
...
static __clib_unused clib_error_t *
bond_subif_add_del_function(vnet_main_t *vnm, u32 hw_if_index, struct vnet_sw_interface_t *st, int is_add)
{
    /* Nothing for now */
    return 0;
}
...
bond_load_balance_broadcast(vlib_main_t *vm, vlib_node_runtime_t *node, bond_if_t *bif, vlib_buffer_t *b0,
                            uword slave_count)
{
    bond_main_t *bm = &bond_main;
    vlib_buffer_t *c0;
    ...
    for (port = 1; port < slave_count; port++) {
        ...
        if (PREDICT_TRUE(c0 != 0)) {
            ...
            ptd->per_port_queue[sw_if_index].buffers[ptd->per_port_queue[sw_if_index].n_buffers] =
...
static_always_inline u32
bond_load_balance_l2(vlib_main_t *vm, vlib_node_runtime_t *node, bond_if_t *bif, vlib_buffer_t *b0, uword slave_count)
{
...
static_always_inline u32
bond_load_balance_l23(vlib_main_t *vm, vlib_node_runtime_t *node, bond_if_t *bif, vlib_buffer_t *b0, uword slave_count)
{
    ethernet_header_t *eth = (ethernet_header_t *) vlib_buffer_get_current(b0);
    u8 ip_version;
    ip4_header_t *ip4;
    ...
    if (ip_version == 0x4) {
        ...
        c = lb_hash_hash_2_tuples(clib_mem_unaligned(&ip4->address_pair, u64), a);
        if (BOND_MODULO_SHORTCUT(slave_count))
            return (c & (slave_count - 1));
        else
            return c % slave_count;
        ...
        a = clib_mem_unaligned(mac1, u32) ^ clib_mem_unaligned(mac2, u32) ^ clib_mem_unaligned(mac3, u32);
        c = lb_hash_hash(clib_mem_unaligned(&ip6->src_address.as_uword[0], uword),
...
static_always_inline u32
bond_load_balance_l34(vlib_main_t *vm, vlib_node_runtime_t *node, bond_if_t *bif, vlib_buffer_t *b0, uword slave_count)
{
    ethernet_header_t *eth = (ethernet_header_t *) vlib_buffer_get_current(b0);
    u8 ip_version;
    uword is_tcp_udp;
...
static_always_inline u32
bond_load_balance_round_robin(vlib_main_t *vm, vlib_node_runtime_t *node, bond_if_t *bif, vlib_buffer_t *b0,
                              uword slave_count)
{
...
static_always_inline u32
bond_load_balance_active_backup(vlib_main_t *vm, vlib_node_runtime_t *node, bond_if_t *bif, vlib_buffer_t *b0,
                                uword slave_count)
{
...
bond_tx_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bond_if_t *bif, uword slave_count,
               u32 lb_alg)
{
    bond_main_t *bm  = &bond_main;
    vnet_main_t *vnm = vnet_get_main();
    ...
    while (n_left >= 4) {
        ...
        if (PREDICT_TRUE(slave_count > 1)) {
            if (lb_alg == BOND_LB_L2) {
                ...
                port0 = bond_load_balance_l34(vm, node, bif, b[0], slave_count);
                port1 = bond_load_balance_l34(vm, node, bif, b[1], slave_count);
                port2 = bond_load_balance_l34(vm, node, bif, b[2], slave_count);
                port3 = bond_load_balance_l34(vm, node, bif, b[3], slave_count);
            } else if (lb_alg == BOND_LB_L23) {
                port0 = bond_load_balance_l23(vm, node, bif, b[0], slave_count);
                port1 = bond_load_balance_l23(vm, node, bif, b[1], slave_count);
                port2 = bond_load_balance_l23(vm, node, bif, b[2], slave_count);
                port3 = bond_load_balance_l23(vm, node, bif, b[3], slave_count);
            } else if (lb_alg == BOND_LB_RR) {
                port0 = bond_load_balance_round_robin(vm, node, bif, b[0], slave_count);
                port1 = bond_load_balance_round_robin(vm, node, bif, b[1], slave_count);
                port2 = bond_load_balance_round_robin(vm, node, bif, b[2], slave_count);
                port3 = bond_load_balance_round_robin(vm, node, bif, b[3], slave_count);
            } else if (lb_alg == BOND_LB_BC) {
                port0 = bond_load_balance_broadcast(vm, node, bif, b[0], slave_count);
                port1 = bond_load_balance_broadcast(vm, node, bif, b[1], slave_count);
                port2 = bond_load_balance_broadcast(vm, node, bif, b[2], slave_count);
                port3 = bond_load_balance_broadcast(vm, node, bif, b[3], slave_count);
            } else if (lb_alg == BOND_LB_AB) {
                port0 = bond_load_balance_active_backup(vm, node, bif, b[0], slave_count);
                port1 = bond_load_balance_active_backup(vm, node, bif, b[1], slave_count);
                port2 = bond_load_balance_active_backup(vm, node, bif, b[2], slave_count);
                port3 = bond_load_balance_active_backup(vm, node, bif, b[3], slave_count);
            } else {
                ASSERT(0);
            }
        }
        ...
        if (PREDICT_FALSE(n_trace > 0)) {
            ...
            if (PREDICT_TRUE(n_trace > 0)) {
                vlib_trace_buffer(vm, node, next1, b[1], 0 /* follow_chain */);
                vlib_set_trace_count(vm, node, --n_trace);
                ...
                if (PREDICT_TRUE(n_trace > 0)) {
                    vlib_trace_buffer(vm, node, next2, b[2], 0 /* follow_chain */);
                    vlib_set_trace_count(vm, node, --n_trace);
                    ...
                    if (PREDICT_TRUE(n_trace > 0)) {
                        vlib_trace_buffer(vm, node, next3, b[3], 0 /* follow_chain */);
                        vlib_set_trace_count(vm, node, --n_trace);
                        t0                   = vlib_add_trace(vm, node, b[3], sizeof(*t0));
                        eth                  = (ethernet_header_t *) vlib_buffer_get_current(b[3]);
                        t0->ethernet         = *eth;
                        t0->sw_if_index      = vnet_buffer(b[3])->sw_if_index[VLIB_TX];
        ...
        ptd->per_port_queue[sif_if_index0].buffers[ptd->per_port_queue[sif_if_index0].n_buffers] =
            vlib_get_buffer_index(vm, b[0]);
        ptd->per_port_queue[sif_if_index0].n_buffers++;

        ptd->per_port_queue[sif_if_index1].buffers[ptd->per_port_queue[sif_if_index1].n_buffers] =
            vlib_get_buffer_index(vm, b[1]);
        ptd->per_port_queue[sif_if_index1].n_buffers++;

        ptd->per_port_queue[sif_if_index2].buffers[ptd->per_port_queue[sif_if_index2].n_buffers] =
            vlib_get_buffer_index(vm, b[2]);
        ptd->per_port_queue[sif_if_index2].n_buffers++;

        ptd->per_port_queue[sif_if_index3].buffers[ptd->per_port_queue[sif_if_index3].n_buffers] =
    ...
    for (port0 = 0; port0 < slave_count; port0++) {
        ...
        if (PREDICT_TRUE(ptd->per_port_queue[sw_if_index].n_buffers)) {
            ...
            clib_memcpy(to_next, ptd->per_port_queue[sw_if_index].buffers, f->n_vectors << 2);
            vnet_put_frame_to_sw_interface(vnm, sw_if_index, f);
            ptd->per_port_queue[sw_if_index].n_buffers = 0;
        }
    }

    vlib_increment_simple_counter(vnet_main.interface_main.sw_if_counters + VNET_INTERFACE_COUNTER_TX, thread_index,
