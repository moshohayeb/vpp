...
void
ip_null_dpo_add_and_lock(dpo_proto_t proto, ip_null_dpo_action_t action, dpo_id_t *dpo)
{
    int i;
...
always_inline uword
ip_null_dpo_switch(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4)
{
            ...
            if (PREDICT_FALSE(!drop0)) {
                if (is_ip4) {
                    ...
                    if (IP_NULL_ACTION_SEND_ICMP_UNREACH == ind0->ind_action) {
                        ...
                        next0 = IP_NULL_NEXT_ICMP;
                        icmp4_error_set_vnet_buffer(p0, ICMP4_destination_unreachable,
                                                    ICMP4_destination_unreachable_host_administratively_prohibited, 0);
                    }
                } else {
                    if (IP_NULL_ACTION_SEND_ICMP_UNREACH == ind0->ind_action) {
                        next0 = IP_NULL_NEXT_ICMP;
...
static uword
ip4_null_dpo_switch(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ip6_null_dpo_switch(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
