...
#define REP_DBG(_rep, _fmt, _args...)                                                                                  \
    {                                                                                                                  \
        u8 *_tmp = NULL;                                                                                               \
        clib_warning("rep:[%s]:" _fmt, replicate_format(replicate_get_index((_rep)), 0, _tmp), ##_args);               \
        vec_free(_tmp);                                                                                                \
    }
#else
...
static u8 *
replicate_format(index_t repi, replicate_format_flags_t flags, u32 indent, u8 *s)
{
    vlib_counter_t to;
    replicate_t *rep;
    dpo_id_t *buckets;
...
static inline void
replicate_set_bucket_i(replicate_t *rep, u32 bucket, dpo_id_t *buckets, const dpo_id_t *next)
{
...
replicate_multipath_next_hop_fixup(load_balance_path_t *nhs, dpo_proto_t drop_proto)
{
    if (0 == vec_len(nhs)) {
 ...
 */
static void
replicate_fill_buckets(replicate_t *rep, load_balance_path_t *nhs, dpo_id_t *buckets, u32 n_buckets)
{
    load_balance_path_t *nh;
    u16 bucket;
...
void
replicate_multipath_update(const dpo_id_t *dpo, load_balance_path_t *next_hops)
{
...
static clib_error_t *
replicate_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static uword
replicate_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
mpls_replicate(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (replicate_inline(vm, node, frame));
}
