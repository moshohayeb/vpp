...
void
mpls_label_dpo_create(fib_mpls_label_t *label_stack, mpls_eos_bit_t eos, dpo_proto_t payload_proto,
                      mpls_label_dpo_flags_t flags, const dpo_id_t *parent, dpo_id_t *dpo)
{
    mpls_label_dpo_t *mld;
    dpo_type_t dtype;
    u32 ii;

    if ((DPO_PROTO_IP4 != payload_proto) && (DPO_PROTO_IP6 != payload_proto)) {
        /*
    ...
    if (MPLS_LABEL_DPO_MAX_N_LABELS < vec_len(label_stack)) {
        ...
        dpo_stack(dtype, mld->mld_payload_proto, &mld->mld_dpo, drop_dpo_get(DPO_PROTO_MPLS));
    } else {
        ...
        for (ii = 0; ii < mld->mld_n_labels - 1; ii++) {
            ...
            vnet_mpls_uc_set_exp(&mld->mld_hdr[ii].label_exp_s_ttl, label_stack[ii].fml_exp);
            ...
            if (0 != label_stack[ii].fml_ttl) {
                ...
                vnet_mpls_uc_set_ttl(&mld->mld_hdr[ii].label_exp_s_ttl, MPLS_LABEL_DEFAULT_TTL);
            }
            mld->mld_hdr[ii].label_exp_s_ttl = clib_host_to_net_u32(mld->mld_hdr[ii].label_exp_s_ttl);
        }
         ...
         */
        dpo_stack(dtype, mld->mld_payload_proto, &mld->mld_dpo, parent);
    }

    dpo_set(dpo, dtype, mld->mld_payload_proto, mpls_label_dpo_get_index(mld));
}
...
format_mpls_label_dpo(u8 *s, va_list *args)
{
    ...
    s   = format(s, "mpls-label[%U@%d]:", format_mpls_label_dpo_flags, (int) mld->mld_flags, index);

    for (ii = 0; ii < mld->mld_n_labels; ii++) {
        hdr.label_exp_s_ttl = clib_net_to_host_u32(mld->mld_hdr[ii].label_exp_s_ttl);
...
mpls_label_paint(vlib_buffer_t *b0, mpls_label_dpo_t *mld0)
{
 ...
 */
always_inline mpls_unicast_header_t *
mpls_label_paint_w_ttl(vlib_buffer_t *b0, mpls_label_dpo_t *mld0, u8 ttl0)
{
    mpls_unicast_header_t *hdr0;
 ...
 */
always_inline mpls_unicast_header_t *
mpls_label_paint_w_ttl_exp(vlib_buffer_t *b0, mpls_label_dpo_t *mld0, u8 ttl0, u8 exp0)
{
    mpls_unicast_header_t *hdr0;
 ...
 */
always_inline mpls_unicast_header_t *
mpls_label_paint_w_ttl_mpls_exp(vlib_buffer_t *b0, mpls_label_dpo_t *mld0, u8 ttl0, u8 exp0)
{
    mpls_unicast_header_t *hdr0;
...
always_inline uword
mpls_label_imposition_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame,
                             const dpo_proto_t dproto, const mpls_label_dpo_flags_t flags)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4) {
            ...
            if (DPO_PROTO_MPLS != dproto) {
                ...
                if (DPO_PROTO_IP4 == dproto) {
                    ...
                    if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE) {
                        ...
                        ttl3 = ip3->hop_limit;
                        /* by default copy the 3 most significant bits */
                        exp0 = ip_dscp_to_mpls_exp(ip6_traffic_class_network_order(ip0));
                        exp1 = ip_dscp_to_mpls_exp(ip6_traffic_class_network_order(ip1));
                        exp2 = ip_dscp_to_mpls_exp(ip6_traffic_class_network_order(ip2));
                        exp3 = ip_dscp_to_mpls_exp(ip6_traffic_class_network_order(ip3));
                    }
                } else {
                    /*
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_label_imposition_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->hdr                           = *hdr0;
                if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE) {
                    tr->ttl = ttl0;
                    tr->exp = exp0;
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_label_imposition_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
                tr->hdr                           = *hdr1;
                if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE) {
                    tr->ttl = ttl1;
                    tr->exp = exp1;
            ...
            if (PREDICT_FALSE(b2->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_label_imposition_trace_t *tr = vlib_add_trace(vm, node, b2, sizeof(*tr));
                tr->hdr                           = *hdr2;
                if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE) {
                    tr->ttl = ttl2;
                    tr->exp = exp2;
            ...
            if (PREDICT_FALSE(b3->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_label_imposition_trace_t *tr = vlib_add_trace(vm, node, b3, sizeof(*tr));
                tr->hdr                           = *hdr3;
                if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE) {
                    tr->ttl = ttl3;
                    tr->exp = exp3;
            ...
            vlib_validate_buffer_enqueue_x4(vm, node, next_index, to_next, n_left_to_next, bi0, bi1, bi2, bi3, next0,
                                            next1, next2, next3);
        }
...
format_mpls_label_imposition_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "%Umpls-header:%U", format_white_space, indent, format_mpls_header, hdr);
    return (s);
}

static uword
mpls_mpls_label_imposition_pipe(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_MPLS, MPLS_LABEL_DPO_FLAG_NONE));
}

VLIB_REGISTER_NODE(mpls_mpls_label_imposition_pipe_node) = {.function    = mpls_mpls_label_imposition_pipe,
...
static uword
ip4_mpls_label_imposition_pipe(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_IP4, MPLS_LABEL_DPO_FLAG_NONE));
}

VLIB_REGISTER_NODE(ip4_mpls_label_imposition_pipe_node) = {.function    = ip4_mpls_label_imposition_pipe,
...
static uword
ip6_mpls_label_imposition_pipe(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_IP6, MPLS_LABEL_DPO_FLAG_NONE));
}

VLIB_REGISTER_NODE(ip6_mpls_label_imposition_pipe_node) = {.function    = ip6_mpls_label_imposition_pipe,
...
static uword
ethernet_mpls_label_imposition_pipe(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_ETHERNET, MPLS_LABEL_DPO_FLAG_NONE));
}

VLIB_REGISTER_NODE(ethernet_mpls_label_imposition_pipe_node) = {.function    = ethernet_mpls_label_imposition_pipe,
...
static uword
mpls_mpls_label_imposition_uniform(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_MPLS, MPLS_LABEL_DPO_FLAG_UNIFORM_MODE));
}

VLIB_REGISTER_NODE(mpls_mpls_label_imposition_uniform_node) = {.function    = mpls_mpls_label_imposition_uniform,
...
static uword
ip4_mpls_label_imposition_uniform(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_IP4, MPLS_LABEL_DPO_FLAG_UNIFORM_MODE));
}

VLIB_REGISTER_NODE(ip4_mpls_label_imposition_uniform_node) = {.function    = ip4_mpls_label_imposition_uniform,
...
static uword
ip6_mpls_label_imposition_uniform(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_IP6, MPLS_LABEL_DPO_FLAG_UNIFORM_MODE));
}

VLIB_REGISTER_NODE(ip6_mpls_label_imposition_uniform_node) = {.function    = ip6_mpls_label_imposition_uniform,
...
static uword
ethernet_mpls_label_imposition_uniform(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_ETHERNET, MPLS_LABEL_DPO_FLAG_UNIFORM_MODE));
}

VLIB_REGISTER_NODE(ethernet_mpls_label_imposition_uniform_node) = {.function = ethernet_mpls_label_imposition_uniform,
...
static uword
ip4_mpls_label_imposition_pipe_no_ip_ttl_decr(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_IP4, MPLS_LABEL_DPO_FLAG_NO_IP_TTL_DECR));
}

VLIB_REGISTER_NODE(
    ip4_mpls_label_imposition_pipe_no_ip_ttl_decr_node) = {.function = ip4_mpls_label_imposition_pipe_no_ip_ttl_decr,
...
static uword
ip6_mpls_label_imposition_pipe_no_ip_ttl_decr(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_IP6, MPLS_LABEL_DPO_FLAG_NO_IP_TTL_DECR));
}

VLIB_REGISTER_NODE(
    ip6_mpls_label_imposition_pipe_no_ip_ttl_decr_node) = {.function = ip6_mpls_label_imposition_pipe_no_ip_ttl_decr,
...
static uword
ip4_mpls_label_imposition_uniform_no_ip_ttl_decr(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_IP4,
                                         (MPLS_LABEL_DPO_FLAG_UNIFORM_MODE | MPLS_LABEL_DPO_FLAG_NO_IP_TTL_DECR)));
}
...
static uword
ip6_mpls_label_imposition_uniform_no_ip_ttl_decr(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (mpls_label_imposition_inline(vm, node, frame, DPO_PROTO_IP6,
                                         (MPLS_LABEL_DPO_FLAG_UNIFORM_MODE | MPLS_LABEL_DPO_FLAG_NO_IP_TTL_DECR)));
}
...
{
    fib_show_memory_usage("MPLS label", pool_elts(mpls_label_dpo_pool), pool_len(mpls_label_dpo_pool),
 ...
 */
static void
mpls_label_interpose(const dpo_id_t *original, const dpo_id_t *parent, dpo_id_t *clone)
{
    ...
    clib_memcpy(&mld_clone->mld_hdr, &mld->mld_hdr, sizeof(mld_clone->mld_hdr));
    ...
    dpo_stack(mpls_label_dpo_types[MPLS_LABEL_DPO_FLAG_NONE], mld_clone->mld_payload_proto, &mld_clone->mld_dpo, parent);

    dpo_set(clone, mpls_label_dpo_types[MPLS_LABEL_DPO_FLAG_NONE], mld_clone->mld_payload_proto,
...
{
    mpls_label_dpo_types[MPLS_LABEL_DPO_FLAG_NONE] = dpo_register_new_type(&mld_vft, mpls_label_imp_pipe_nodes);
    mpls_label_dpo_types[MPLS_LABEL_DPO_FLAG_NO_IP_TTL_DECR] =
        dpo_register_new_type(&mld_vft, mpls_label_imp_pipe_no_ip_tll_decr_nodes);
    mpls_label_dpo_types[MPLS_LABEL_DPO_FLAG_UNIFORM_MODE] =
        dpo_register_new_type(&mld_vft, mpls_label_imp_uniform_nodes);
    mpls_label_dpo_types[MPLS_LABEL_DPO_FLAG_UNIFORM_MODE | MPLS_LABEL_DPO_FLAG_NO_IP_TTL_DECR] =
        dpo_register_new_type(&mld_vft, mpls_label_imp_uniform_no_ip_tll_decr_nodes);
}
