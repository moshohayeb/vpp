...
static void
lookup_dpo_add_or_lock_i(fib_node_index_t fib_index, dpo_proto_t proto, lookup_cast_t cast, lookup_input_t input,
                         lookup_table_t table_config, dpo_id_t *dpo)
{
    lookup_dpo_t *lkd;
    dpo_type_t type;
...
void
lookup_dpo_add_or_lock_w_fib_index(fib_node_index_t fib_index, dpo_proto_t proto, lookup_cast_t cast,
                                   lookup_input_t input, lookup_table_t table_config, dpo_id_t *dpo)
{
...
void
lookup_dpo_add_or_lock_w_table_id(u32 table_id, dpo_proto_t proto, lookup_cast_t cast, lookup_input_t input,
                                  lookup_table_t table_config, dpo_id_t *dpo)
{
...
format_lookup_dpo(u8 *s, va_list *args)
{
    ...
    if (LOOKUP_TABLE_FROM_INPUT_INTERFACE == lkd->lkd_table) {
        s = format(s, "%s,%s lookup in interface's %U table", lookup_input_names[lkd->lkd_input],
                   lookup_cast_names[lkd->lkd_cast], format_dpo_proto, lkd->lkd_proto);
    } else {
        if (LOOKUP_UNICAST == lkd->lkd_cast) {
            s = format(s, "%s,%s lookup in %U", lookup_input_names[lkd->lkd_input], lookup_cast_names[lkd->lkd_cast],
                       format_fib_table_name, lkd->lkd_fib_index, dpo_proto_to_fib(lkd->lkd_proto));
        } else {
            s = format(s, "%s,%s lookup in %U", lookup_input_names[lkd->lkd_input], lookup_cast_names[lkd->lkd_cast],
                       format_mfib_table_name, lkd->lkd_fib_index, dpo_proto_to_fib(lkd->lkd_proto));
        }
    }
    return (s);
}
...
always_inline void
ip4_src_fib_lookup_one(u32 src_fib_index0, const ip4_address_t *addr0, u32 *src_adj_index0)
{
...
always_inline void
ip4_src_fib_lookup_two(u32 src_fib_index0, u32 src_fib_index1, const ip4_address_t *addr0, const ip4_address_t *addr1,
                       u32 *src_adj_index0, u32 *src_adj_index1)
{
...
always_inline uword
lookup_dpo_ip4_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int input_src_addr,
                      int table_from_interface)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next > 2) {
            ...
            /* do lookup */
            ip4_src_fib_lookup_two(fib_index0, fib_index1, input_addr0, input_addr1, &lbi0, &lbi1);
...
format_lookup_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U fib-index:%d addr:%U load-balance:%d", format_white_space, indent, t->fib_index,
               format_ip46_address, &t->addr, IP46_TYPE_ANY, t->lbi);
    return s;
}

static uword
lookup_ip4_dst(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword
lookup_ip4_dst_itf(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword
lookup_ip4_src(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword
lookup_dpo_ip6_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int input_src_addr,
                      int table_from_interface)
{
    vlib_combined_counter_main_t *cm = &load_balance_main.lbm_to_counters;
    u32 n_left_from, next_index, *from, *to_next;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next > 2) {
            ...
            /* do src lookup */
            lbi0 = ip6_fib_table_fwding_lookup(&ip6_main, fib_index0, input_addr0);
            lbi1 = ip6_fib_table_fwding_lookup(&ip6_main, fib_index1, input_addr1);
            lb0  = load_balance_get(lbi0);
            lb1  = load_balance_get(lbi1);
...
static uword
lookup_ip6_dst(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword
lookup_ip6_dst_itf(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword
lookup_ip6_src(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword
lookup_dpo_mpls_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int table_from_interface)
{
    u32 n_left_from, next_index, *from, *to_next;
    u32 thread_index                 = vlib_get_thread_index();
    vlib_combined_counter_main_t *cm = &load_balance_main.lbm_to_counters;
...
format_lookup_mpls_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U fib-index:%d hdr:%U load-balance:%d", format_white_space, indent, t->fib_index,
               format_mpls_header, hdr, t->lbi);
    return s;
}

static uword
lookup_mpls_dst(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword
lookup_mpls_dst_itf(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword
lookup_dpo_ip_dst_mcast_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_v4)
{
    u32 n_left_from, next_index, *from, *to_next;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (is_v4) {
                ...
                ip0   = vlib_buffer_get_current(b0);
                mfei0 = ip6_mfib_table_lookup2(ip6_mfib_get(fib_index0), &ip0->src_address, &ip0->dst_address);
                if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                    lookup_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                    tr->fib_index      = fib_index0;
                    tr->lbi            = mfei0;
...
static uword
lookup_ip4_dst_mcast(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword
lookup_ip6_dst_mcast(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static clib_error_t *
lookup_dpo_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (INDEX_INVALID != lkdi) {
        ...
        pool_foreach(lkd, lookup_dpo_pool, ({
                         vlib_cli_output(vm, "[@%d] %U", lookup_dpo_get_index(lkd), format_lookup_dpo,
