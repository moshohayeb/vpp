...
#define LB_DBG(_lb, _fmt, _args...)                                                                                    \
    {                                                                                                                  \
        u8 *_tmp = NULL;                                                                                               \
        clib_warning("lb:[%s]:" _fmt, load_balance_format(load_balance_get_index((_lb)), 0, _tmp), ##_args);           \
        vec_free(_tmp);                                                                                                \
    }
#else
 ...
 */
load_balance_main_t load_balance_main = {.lbm_to_counters =
                                             {
                                                 .name              = "route-to",
...
load_balance_alloc_i(void)
{
    ...
    vlib_validate_combined_counter(&(load_balance_main.lbm_to_counters), load_balance_get_index(lb));
    vlib_validate_combined_counter(&(load_balance_main.lbm_via_counters), load_balance_get_index(lb));
    vlib_zero_combined_counter(&(load_balance_main.lbm_to_counters), load_balance_get_index(lb));
    vlib_zero_combined_counter(&(load_balance_main.lbm_via_counters), load_balance_get_index(lb));

    return (lb);
}

static u8 *
load_balance_format(index_t lbi, load_balance_format_flags_t flags, u32 indent, u8 *s)
{
    ...
    if (INDEX_INVALID != lb->lb_map) {
        s = format(s, "\n%U%U", format_white_space, indent + 4, format_load_balance_map, lb->lb_map, indent + 4);
    }
    for (i = 0; i < lb->lb_n_buckets; i++) {
        s = format(s, "\n%U[%d] %U", format_white_space, indent + 2, i, format_dpo_id, &buckets[i], indent + 6);
    }
    return (s);
}
...
static load_balance_t *
load_balance_create_i(u32 num_buckets, dpo_proto_t lb_proto, flow_hash_config_t fhc)
{
    load_balance_t *lb;
    ...
    if (!LB_HAS_INLINE_BUCKETS(lb)) {
        vec_validate_aligned(lb->lb_buckets, lb->lb_n_buckets - 1, CLIB_CACHE_LINE_BYTES);
    }
...
index_t
load_balance_create(u32 n_buckets, dpo_proto_t lb_proto, flow_hash_config_t fhc)
{
    return (load_balance_get_index(load_balance_create_i(n_buckets, lb_proto, fhc)));
}

static inline void
load_balance_set_bucket_i(load_balance_t *lb, u32 bucket, dpo_id_t *buckets, const dpo_id_t *next)
{
...
static int
next_hop_sort_by_weight(const load_balance_path_t *n1, const load_balance_path_t *n2)
{
...
u32
ip_multipath_normalize_next_hops(const load_balance_path_t *raw_next_hops, load_balance_path_t **normalized_next_hops,
                                 u32 *sum_weight_in, f64 multipath_next_hop_error_tolerance)
{
...
load_balance_multipath_next_hop_fixup(const load_balance_path_t *nhs, dpo_proto_t drop_proto)
{
    if (0 == vec_len(nhs)) {
 ...
 */
static void
load_balance_fill_buckets(load_balance_t *lb, load_balance_path_t *nhs, dpo_id_t *buckets, u32 n_buckets)
{
...
void
load_balance_multipath_update(const dpo_id_t *dpo, const load_balance_path_t *raw_nhs, load_balance_flags_t flags)
{
    ...
    n_buckets = ip_multipath_normalize_next_hops((NULL == fixed_nhs ? raw_nhs : fixed_nhs), &nhs, &sum_of_weights,
    ...
    if (0 == lb->lb_n_buckets) {
        ...
        load_balance_fill_buckets(lb, nhs, load_balance_get_buckets(lb), n_buckets);
        lb->lb_map = lbmi;
    } else {
        /*
        ...
        if (n_buckets == lb->lb_n_buckets) {
             ...
             * update it last.
             */
            if (n_buckets > LB_NUM_INLINE_BUCKETS && lb->lb_n_buckets <= LB_NUM_INLINE_BUCKETS) {
                /*
                ...
                vec_validate_aligned(lb->lb_buckets, n_buckets - 1, CLIB_CACHE_LINE_BYTES);
            ...
            if (n_buckets <= LB_NUM_INLINE_BUCKETS && lb->lb_n_buckets > LB_NUM_INLINE_BUCKETS) {
                 ...
                 */
                load_balance_fill_buckets(lb, nhs, lb->lb_buckets_inline, n_buckets);
                CLIB_MEMORY_BARRIER();
                load_balance_set_n_buckets(lb, n_buckets);
                CLIB_MEMORY_BARRIER();
                ...
                load_balance_fill_buckets(lb, nhs, buckets, n_buckets);
...
{
    fib_show_memory_usage("load-balance", pool_elts(load_balance_pool), pool_len(load_balance_pool),
...
const static char *const load_balance_nsh_nodes[]  = {"nsh-load-balance", NULL};
const static char *const load_balance_bier_nodes[] = {
    "bier-load-balance",
    NULL,
};
const static char *const *const load_balance_nodes[DPO_PROTO_NUM] = {
    [DPO_PROTO_IP4] = load_balance_ip4_nodes,   [DPO_PROTO_IP6] = load_balance_ip6_nodes,
    [DPO_PROTO_MPLS] = load_balance_mpls_nodes, [DPO_PROTO_ETHERNET] = load_balance_l2_nodes,
    [DPO_PROTO_NSH] = load_balance_nsh_nodes,   [DPO_PROTO_BIER] = load_balance_bier_nodes,
};
...
static clib_error_t *
load_balance_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (INDEX_INVALID != lbi) {
        vlib_cli_output(vm, "%U", format_load_balance, lbi, LOAD_BALANCE_FORMAT_DETAIL);
    } else {
        load_balance_t *lb;

        pool_foreach(lb, load_balance_pool, ({
                         vlib_cli_output(vm, "%U", format_load_balance, load_balance_get_index(lb),
...
always_inline uword
load_balance_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_l2)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            dpo0 = load_balance_get_bucket_i(lb0, vnet_buffer(b0)->ip.flow_hash & (lb0->lb_n_buckets_minus_1));

            next0                                  = dpo0->dpoi_next_node;
            vnet_buffer(b0)->ip.adj_index[VLIB_TX] = dpo0->dpoi_index;

            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                load_balance_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->lb_index             = lbi0;
            }
...
static uword
l2_load_balance(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
nsh_load_balance(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32 n_left_from, next_index, *from, *to_next;
...
static uword
bier_load_balance(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
