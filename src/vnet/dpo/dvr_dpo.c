...
void
dvr_dpo_add_or_lock(u32 sw_if_index, dpo_proto_t dproto, dpo_id_t *dpo)
{
    dvr_dpo_t *dd;
...
static clib_error_t *
dvr_dpo_interface_state_change(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    /*
     */
    return (NULL);
 ...
 */
static clib_error_t *
dvr_dpo_hw_interface_state_change(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    return (NULL);
}
...
static clib_error_t *
dvr_dpo_interface_delete(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    return (NULL);
}
...
format_dvr_dpo(u8 *s, va_list *ap)
{
    ...
    return (format(s, "dvr-%U-dpo", format_vnet_sw_interface_name, vnm, vnet_get_sw_interface(vnm, dd->dd_sw_if_index)));
}
...
always_inline uword
dvr_dpo_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u8 is_ip6)
{
...
static uword
ip4_dvr_dpo(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (dvr_dpo_inline(vm, node, from_frame, 0));
}

static uword
ip6_dvr_dpo(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword
dvr_reinject_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    u32 n_left_from, next_index, *from, *to_next;
...
static uword
ip4_dvr_reinject(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (dvr_reinject_inline(vm, node, from_frame));
}

static uword
ip6_dvr_reinject(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (dvr_reinject_inline(vm, node, from_frame));
}
...
VNET_FEATURE_INIT(ip4_dvr_reinject_feat_node, static) = {
    ...
    .runs_after = VNET_FEATURES("nat44-in2out-output", "acl-plugin-out-ip4-fa"),
};
