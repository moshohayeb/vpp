...
dns_enable_disable(dns_main_t *dm, int is_enable)
{
    ...
    if (is_enable) {
        if (vec_len(dm->ip4_name_servers) == 0 && (vec_len(dm->ip6_name_servers) == 0))
            return VNET_API_ERROR_NO_NAME_SERVERS;

        if (dm->cache_entry_by_name == 0) {
            if (n_vlib_mains > 1)
                dm->cache_lock = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
...
static void
vl_api_dns_enable_disable_t_handler(vl_api_dns_enable_disable_t *mp)
{
...
static int
dns6_name_server_add_del(dns_main_t *dm, u8 *server_address_as_u8, int is_add)
{
    int i;
    ip6_address_t *ap;
...
static int
dns4_name_server_add_del(dns_main_t *dm, u8 *server_address_as_u8, int is_add)
{
    int i;
    ip4_address_t *ap;
...
static void
vl_api_dns_name_server_add_del_t_handler(vl_api_dns_name_server_add_del_t *mp)
{
    dns_main_t *dm = &dns_main;
...
void
vnet_dns_send_dns4_request(dns_main_t *dm, dns_cache_entry_t *ep, ip4_address_t *server)
{
    ...
    if (sw_if_index == ~0) {
        clib_warning("route to %U exists, fei %d, get_resolving_interface returned"
                     " ~0",
...
void
vnet_dns_send_dns6_request(dns_main_t *dm, dns_cache_entry_t *ep, ip6_address_t *server)
{
    vlib_main_t *vm = dm->vlib_main;
    f64 now         = vlib_time_now(vm);
    u32 bi;
    ...
    if (ep->retry_count++ < DNS_RETRIES_PER_SERVER) {
        if (ep->server_af == 1 /* ip6 */) {
            if (vec_len(dm->ip6_name_servers)) {
                vnet_dns_send_dns6_request(dm, ep, dm->ip6_name_servers + ep->server_rotor);
                goto out;
            } else
                ep->server_af = 0;
        }
        if (vec_len(dm->ip4_name_servers)) {
            vnet_dns_send_dns4_request(dm, ep, dm->ip4_name_servers + ep->server_rotor);
            goto out;
        }
...
out:
    ...
    vlib_process_signal_event_mt(dm->vlib_main, dns_resolver_node.index, DNS_RESOLVER_EVENT_PENDING, 0);
}
...
int
vnet_dns_resolve_name(dns_main_t *dm, u8 *name, dns_pending_request_t *t, dns_cache_entry_t **retp)
{
    dns_cache_entry_t *ep;
    int rv;
    f64 now;
...
int
vnet_dns_response_to_reply(u8 *response, vl_api_dns_resolve_name_reply_t *rmp, u32 *min_ttlp)
{
    dns_header_t *h;
    dns_query_t *qp;
    dns_rr_t *rr;
...
int
vnet_dns_response_to_name(u8 *response, vl_api_dns_resolve_ip_reply_t *rmp, u32 *min_ttlp)
{
    dns_header_t *h;
    dns_query_t *qp;
    dns_rr_t *rr;
...
format_dns_reply_data(u8 *s, va_list *args)
{
    ...
    case DNS_TYPE_A:
        if (verbose > 1) {
            ...
            if (*print_ip4)
                s = format(s, "%U [%u] ", format_ip4_address, rr->rdata, clib_net_to_host_u32(rr->ttl));
            if (verbose == 0)
                *print_ip4 = 0;
        }
        pos += sizeof(*rr) + 4;
    ...
    case DNS_TYPE_AAAA:
        if (verbose > 1) {
            ...
            if (*print_ip6)
                s = format(s, "%U [%u] ", format_ip6_address, rr->rdata, clib_net_to_host_u32(rr->ttl));
            if (verbose == 0)
                *print_ip6 = 0;
        }
        pos += sizeof(*rr) + 16;
...
format_dns_reply(u8 *s, va_list *args)
{
    ...
    if (verbose > 1) {
        ...
        s = format(s, "  %s %s %s %s\n", (flags & DNS_RA) ? "recur" : "no-recur",
                   (flags & DNS_RD) ? "recur-des" : "no-recur-des", (flags & DNS_TC) ? "trunc" : "no-trunc",
                   (flags & DNS_AA) ? "auth" : "non-auth");
        s = format(s,
                   "  %d queries, %d answers, %d name-servers,"
                   " %d add'l recs\n",
                   clib_net_to_host_u16(h->qdcount), clib_net_to_host_u16(h->anscount),
                   clib_net_to_host_u16(h->nscount), clib_net_to_host_u16(h->arcount));
    }
...
static clib_error_t *
show_dns_cache_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dns_main_t *dm = &dns_main;
    int verbose    = 0;
    u8 *name       = 0;
...
static clib_error_t *
dns_cache_add_del_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dns_main_t *dm = &dns_main;
    u8 *dns_reply_data;
    u8 *name;
...
static u8 dns_reply_data_initializer[] = {
    ...
    0x0b, 0x77, 0x65, 0x61, 0x74, 0x68, 0x65, 0x72, 0x6c, 0x69, 0x6e, 0x6b, 0x03, 0x63, 0x6f, 0x6d,
    0x00, 0x00, 0xff, 0x00, 0x01, 0xc0, 0x0c, 0x00, 0x05, 0x00, 0x01, 0x00, 0x00, 0x0c, 0x9e, 0x00,
    0x1f, 0x0e, 0x64, 0x33, 0x6b, 0x72, 0x30, 0x67, 0x75, 0x62, 0x61, 0x31, 0x64, 0x76, 0x77, 0x66,
    0x0a, 0x63, 0x6c, 0x6f, 0x75, 0x64, 0x66, 0x72, 0x6f, 0x6e, 0x74, 0x03, 0x6e, 0x65, 0x74, 0x00,
};

#endif

static clib_error_t *
test_dns_fmt_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u8 *dns_reply_data = 0;
    int verbose        = 0;
    int rv;
    ...
    case 0:
        if (rmp->ip4_set)
            vlib_cli_output(vm, "ip4 address: %U", format_ip4_address, (ip4_address_t *) rmp->ip4_address);
        if (rmp->ip6_set)
            vlib_cli_output(vm, "ip6 address: %U", format_ip6_address, (ip6_address_t *) rmp->ip6_address);
        break;
    }
...
static clib_error_t *
test_dns_unfmt_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u8 *dns_reply_data = 0;
    int verbose        = 0;
    int reply_set      = 0;
...
static clib_error_t *
test_dns_expire_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    dns_main_t *dm = &dns_main;
    u8 *name;
    uword *p;
...
void
vnet_send_dns6_reply(dns_main_t *dm, dns_pending_request_t *pr, dns_cache_entry_t *ep, vlib_buffer_t *b0)
{
...
void
vnet_send_dns4_reply(dns_main_t *dm, dns_pending_request_t *pr, dns_cache_entry_t *ep, vlib_buffer_t *b0)
{
    vlib_main_t *vm = dm->vlib_main;
    u32 bi          = 0;
    fib_prefix_t prefix;
    ...
    if (sw_if_index == ~0) {
        clib_warning("route to %U exists, fei %d, get_resolving_interface returned"
                     " ~0",
                     fei, format_ip4_address, &prefix.fp_addr);
        return;
    }
