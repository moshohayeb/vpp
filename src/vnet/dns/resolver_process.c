...
extern int vnet_dns_response_to_reply(u8 *response, vl_api_dns_resolve_name_reply_t *rmp, u32 *min_ttlp);
extern int vnet_dns_response_to_name(u8 *response, vl_api_dns_resolve_ip_reply_t *rmp, u32 *min_ttlp);
...
resolve_event(dns_main_t *dm, f64 now, u8 *reply)
{
    ...
    if (rv < 0) {
        ...
        if (ep->server_af /* ip6 */) {
            if (0)
                clib_warning("Server %U failed to resolve '%s'", format_ip6_address,
            ...
            if (ep->server_rotor >= vec_len(dm->ip6_name_servers))
                ep->server_rotor = 0;
            if (0)
                clib_warning("Try server %U", format_ip6_address, dm->ip6_name_servers + ep->server_rotor);
            vnet_dns_send_dns6_request(dm, ep, dm->ip6_name_servers + ep->server_rotor);
        } else {
            if (0)
                clib_warning("Server %U failed to resolve '%s'", format_ip4_address,
            ...
            if (ep->server_rotor >= vec_len(dm->ip4_name_servers))
                ep->server_rotor = 0;
            if (0)
                clib_warning("Try server %U", format_ip4_address, dm->ip4_name_servers + ep->server_rotor);
...
reply:
    ...
    for (i = 0; i < vec_len(ep->pending_requests); i++) {
        ...
        case DNS_API_PENDING_IP_TO_NAME: {
            ...
            rmp             = vl_msg_api_alloc(sizeof(*rmp));
            rmp->_vl_msg_id = clib_host_to_net_u16(VL_API_DNS_RESOLVE_IP_REPLY);
            rmp->context    = pr->client_context;
            min_ttl         = ~0;
...
found:
    ...
    case DNS_RCODE_REFUSED:
        if (ep->server_af == 0)
            clib_warning("name server %U backfire", format_ip4_address, dm->ip4_name_servers + ep->server_rotor);
        else
            clib_warning("name server %U backfire", format_ip6_address, dm->ip6_name_servers + ep->server_rotor);
        /* FALLTHROUGH */
...
static uword
dns_resolver_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
