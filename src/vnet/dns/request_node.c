...
static uword
dns46_request_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip6)
{
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  vlib_validate_buffer_enqueue_x2 (vm, node, next_index,
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            t0->name         = name0;
            if (is_ip6)
                clib_memcpy(t0->dst_address, ip60->src_address.as_u8, sizeof(ip6_address_t));
            else
                clib_memcpy(t0->dst_address, ip40->src_address.as_u8, sizeof(ip4_address_t));
...
static uword
dns4_request_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
dns6_request_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
