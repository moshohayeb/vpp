...
static const char *html_header_template = "<html><head><title>%v</title></head>"
                                          "<link rel=\"icon\" href=\"data:,\">"
                                          "<body><pre>";

static const char *html_footer = "</pre></body></html>\r\n";

static const char *html_header_static = "<html><head><title>static reply</title></head>"
                                        "<link rel=\"icon\" href=\"data:,\">"
...
send_data(stream_session_t *s, u8 *data)
{
    ...
    while (bytes_to_send > 0) {
        ...
        actual_transfer = svm_fifo_enqueue_nowait(s->server_tx_fifo, bytes_to_send, data + offset);
...
session_rx_request(stream_session_t *s)
{
    ...
    actual_transfer = svm_fifo_dequeue_nowait(rx_fifo, max_dequeue, hsm->rx_buf[s->thread_index]);
...
http_server_session_accept_callback(stream_session_t *s)
{
    ...
    bsm->vpp_queue[s->thread_index] = session_manager_get_vpp_event_queue(s->thread_index);
...
static int
http_server_session_connected_callback(u32 app_index, u32 api_context, stream_session_t *s, u8 is_fail)
{
    clib_warning("called...");
    return -1;
}
...
static session_cb_vft_t http_server_session_cb_vft = {.session_accept_callback = http_server_session_accept_callback,
...
create_api_loopback(vlib_main_t *vm)
{
    ...
    hsm->my_client_index = vl_api_memclnt_create_internal("http_server", hsm->vl_input_queue);
    return 0;
}
...
static clib_error_t *
http_server_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    http_server_main_t *hsm = &http_server_main;
    int rv, is_static = 0;
    u64 seg_size;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "static"))
            ...
            ;
        else if (unformat(input, "private-segment-size %U", unformat_memory_size, &seg_size)) {
            if (seg_size >= 0x100000000ULL) {
                vlib_cli_output(vm, "private segment size %llu, too large", seg_size);
                return 0;
            }
    ...
    if (is_static) {
        http_server_session_cb_vft.builtin_app_rx_callback = http_server_rx_callback_static;
