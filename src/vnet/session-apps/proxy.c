...
delete_proxy_session(stream_session_t *s, int is_active_open)
{
    ...
    if (is_active_open) {
        ...
        if (p == 0) {
            clib_warning("proxy session for %s handle %lld (%llx) AWOL", is_active_open ? "active open" : "server",
                         handle, handle);
        ...
        if (p == 0) {
            ...
            if (ps->vpp_active_open_handle != ~0)
                active_open_session = session_get_from_handle(ps->vpp_active_open_handle);
            else
    ...
    if (active_open_session) {
        ...
        hash_unset(pm->proxy_session_by_active_open_handle, session_handle(active_open_session));
    ...
    if (server_session) {
        ...
        hash_unset(pm->proxy_session_by_server_handle, session_handle(server_session));
        vnet_disconnect_session(a);
    }
}
...
static int
proxy_connected_callback(u32 app_index, u32 api_context, stream_session_t *s, u8 is_fail)
{
    clib_warning("called...");
    return -1;
}
...
proxy_rx_callback(stream_session_t *s)
{
    ...
    if (PREDICT_TRUE(p != 0)) {
        ...
        if (svm_fifo_set_event(active_open_tx_fifo)) {
            ...
            if (session_send_io_evt_to_thread_custom(active_open_tx_fifo, ao_thread_index, FIFO_EVENT_APP_TX))
        ...
        hash_set(pm->proxy_session_by_server_handle, ps->vpp_server_handle, proxy_index);
...
static session_cb_vft_t proxy_session_cb_vft = {.session_accept_callback     = proxy_accept_callback,
...
static int
active_open_connected_callback(u32 app_index, u32 opaque, stream_session_t *s, u8 is_fail)
{
    proxy_main_t *pm = &proxy_main;
    proxy_session_t *ps;
    ...
    hash_set(pm->proxy_session_by_active_open_handle, ps->vpp_active_open_handle, opaque);
...
active_open_rx_callback(stream_session_t *s)
{
    ...
    if (svm_fifo_set_event(proxy_tx_fifo)) {
        ...
        return session_send_io_evt_to_thread_custom(proxy_tx_fifo, thread_index, FIFO_EVENT_APP_TX);
    }
...
/* *INDENT-OFF* */
static session_cb_vft_t active_open_clients = {.session_reset_callback      = active_open_reset_callback,
...
create_api_loopbacks(vlib_main_t *vm)
{
    ...
    pm->server_client_index      = vl_api_memclnt_create_internal("proxy_server", pm->vl_input_queue);
    pm->active_open_client_index = vl_api_memclnt_create_internal("proxy_active_open", pm->vl_input_queue);
}
...
proxy_server_create(vlib_main_t *vm)
{
    ...
    for (i = 0; i < num_threads; i++) {
        pm->active_open_event_queue[i] = session_manager_get_vpp_event_queue(i);
...
static clib_error_t *
proxy_server_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    proxy_main_t *pm         = &proxy_main;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "fifo-size %d", &pm->fifo_size))
            ...
            ;
        else if (unformat(input, "private-segment-count %d", &pm->private_segment_count))
            ;
