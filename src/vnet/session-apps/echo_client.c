...
signal_evt_to_cli(int code)
{
    if (vlib_get_thread_index() != 0)
        vl_api_rpc_call_main_thread(signal_evt_to_cli_i, (u8 *) &code, sizeof(code));
    else
...
send_data_chunk(echo_client_main_t *ecm, eclient_session_t *s)
{
    ...
    bytes_this_chunk = clib_min(test_buf_len - test_buf_offset, s->bytes_to_send);
    ...
    if (!ecm->is_dgram) {
        if (ecm->no_copy) {
            ...
            session_send_io_evt_to_thread_custom(f, s->thread_index, FIFO_EVENT_APP_TX);
        } else
            rv = app_send_stream(&s->data, test_data + test_buf_offset, bytes_this_chunk, 0);
    } else {
        if (ecm->no_copy) {
            ...
            rv = app_send_dgram(&s->data, test_data + test_buf_offset, bytes_this_chunk, 0);
    }
...
receive_data_chunk(echo_client_main_t *ecm, eclient_session_t *s)
{
    ...
    if (ecm->test_bytes) {
        if (!ecm->is_dgram)
            n_read = app_recv_stream(&s->data, ecm->rx_buf[thread_index], vec_len(ecm->rx_buf[thread_index]));
        else
            n_read = app_recv_dgram(&s->data, ecm->rx_buf[thread_index], vec_len(ecm->rx_buf[thread_index]));
    } else {
    ...
    if (n_read > 0) {
        ...
        if (ecm->test_bytes) {
            for (i = 0; i < n_read; i++) {
                if (ecm->rx_buf[thread_index][i] != ((s->bytes_received + i) & 0xff)) {
                    clib_warning("read %d error at byte %lld, 0x%x not 0x%x", n_read, s->bytes_received + i,
                                 ecm->rx_buf[thread_index][i], ((s->bytes_received + i) & 0xff));
                    ecm->test_failed = 1;
                }
            }
        }
...
static uword
echo_client_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    echo_client_main_t *ecm = &echo_client_main;
    ...
    connections_this_batch = ecm->connections_this_batch_by_thread[my_thread_index];

    if ((ecm->run_test != ECHO_CLIENTS_RUNNING) ||
        ((vec_len(connection_indices) == 0) && vec_len(connections_this_batch) == 0))
        return 0;

    /* Grab another pile of connections */
    if (PREDICT_FALSE(vec_len(connections_this_batch) == 0)) {
        nconnections_this_batch = clib_min(ecm->connections_per_batch, vec_len(connection_indices));

        ASSERT(nconnections_this_batch > 0);
        vec_validate(connections_this_batch, nconnections_this_batch - 1);
        clib_memcpy(connections_this_batch, connection_indices + vec_len(connection_indices) - nconnections_this_batch,
    ...
    ecm->connections_this_batch_by_thread[my_thread_index] = connections_this_batch;
    return 0;
}
...
create_api_loopback(echo_client_main_t *ecm)
{
    ...
    ecm->my_client_index = vl_api_memclnt_create_internal("echo_client", ecm->vl_input_queue);
    return 0;
}
...
static int
echo_clients_session_connected_callback(u32 app_index, u32 api_context, stream_session_t *s, u8 is_fail)
{
    echo_client_main_t *ecm = &echo_client_main;
    ...
    if (!ecm->vpp_event_queue[thread_index])
        ecm->vpp_event_queue[thread_index] = session_manager_get_vpp_event_queue(thread_index);
    ...
    if (ecm->is_dgram) {
        ...
        clib_memcpy(&session->data.transport, tc, sizeof(session->data.transport));
...
echo_clients_rx_callback(stream_session_t *s)
{
    ...
    sp = pool_elt_at_index(ecm->sessions, s->server_rx_fifo->client_session_index);
...
/* *INDENT-OFF* */
static session_cb_vft_t echo_clients = {.session_reset_callback      = echo_clients_session_reset_callback,
...
echo_clients_start_tx_pthread(echo_client_main_t *ecm)
{
    if (ecm->client_thread_handle == 0) {
        int rv = pthread_create(&ecm->client_thread_handle, NULL /*attr */, echo_client_thread_fn, 0);
        if (rv) {
...
static clib_error_t *
echo_clients_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    echo_client_main_t *ecm         = &echo_client_main;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "uri %s", &ecm->connect_uri))
            ...
            ecm->fifo_size <<= 10;
        else if (unformat(input, "private-segment-count %d", &ecm->private_segment_count))
            ;
        else if (unformat(input, "private-segment-size %U", unformat_memory_size, &tmp)) {
            if (tmp >= 0x100000000ULL)
                return clib_error_return(0, "private segment size %lld (%llu) too large", tmp, tmp);
        ...
        else if (unformat(input, "client-batch %d", &ecm->connections_per_batch))
            ;
    ...
    for (i = 0; i < thread_main->n_vlib_mains; i++)
        vlib_node_set_state(vlib_mains[i], echo_clients_node.index, VLIB_NODE_STATE_POLLING);
    ...
    switch (event_type) {
    case ~0:
        ec_cli_output("Timeout with only %d sessions active...", ecm->ready_connections);
    ...
    case 1:
        ...
        if (delta != 0.0)
            ec_cli_output("%d three-way handshakes in %.2f seconds %.2f/s", n_clients, delta, ((f64) n_clients) / delta);
    ...
    default:
        ...
        error = clib_error_return(0, "failed: unexpected event(1): %d", event_type);
        goto cleanup;
    }
    ...
    switch (event_type) {
    case ~0:
        ec_cli_output("Timeout with %d sessions still active...", ecm->ready_connections);
    ...
    if (delta != 0.0) {
        ...
        ec_cli_output("%lld bytes (%lld mbytes, %lld gbytes) in %.2f seconds", total_bytes, total_bytes / (1ULL << 20),
                      total_bytes / (1ULL << 30), delta);
        ec_cli_output("%.2f bytes/second %s", ((f64) total_bytes) / (delta), transfer_type);
        ec_cli_output("%.4f gbit/second %s", (((f64) total_bytes * 8.0) / delta / 1e9), transfer_type);
    } else {
