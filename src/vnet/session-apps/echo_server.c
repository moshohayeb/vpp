...
echo_server_session_accept_callback(stream_session_t *s)
{
    ...
    esm->vpp_queue[s->thread_index] = session_manager_get_vpp_event_queue(s->thread_index);
...
int
echo_server_session_connected_callback(u32 app_index, u32 api_context, stream_session_t *s, u8 is_fail)
{
    clib_warning("called...");
    return -1;
}
...
test_bytes(echo_server_main_t *esm, int actual_transfer)
{
    ...
    for (i = 0; i < actual_transfer; i++) {
        if (esm->rx_buf[my_thread_id][i] != ((esm->byte_index + i) & 0xff)) {
            clib_warning("at %lld expected %d got %d", esm->byte_index + i, (esm->byte_index + i) & 0xff,
...
echo_server_rx_callback(stream_session_t *s)
{
    ...
    if (!esm->is_dgram) {
        actual_transfer = app_recv_stream_raw(rx_fifo, esm->rx_buf[thread_index], max_transfer,
                                              0 /* don't clear event */, 0 /* peek */);
    } else {
        actual_transfer = app_recv_dgram_raw(rx_fifo, esm->rx_buf[thread_index], max_transfer, &at,
                                             0 /* don't clear event */, 0 /* peek */);
    }
    ...
    if (!esm->is_dgram) {
        n_written = app_send_stream_raw(tx_fifo, esm->vpp_queue[thread_index], esm->rx_buf[thread_index],
                                        actual_transfer, FIFO_EVENT_APP_TX, 0);
    } else {
        n_written = app_send_dgram_raw(tx_fifo, &at, esm->vpp_queue[s->thread_index], esm->rx_buf[thread_index],
                                       actual_transfer, FIFO_EVENT_APP_TX, 0);
    }
...
static session_cb_vft_t echo_server_session_cb_vft = {.session_accept_callback = echo_server_session_accept_callback,
                                                      .session_disconnect_callback =
                                                          echo_server_session_disconnect_callback,
...
create_api_loopback(vlib_main_t *vm)
{
    ...
    esm->my_client_index = vl_api_memclnt_create_internal("echo_server", esm->vl_input_queue);
    return 0;
}
...
echo_server_attach(u8 *appns_id, u64 appns_flags, u64 appns_secret)
{
    ...
    if (esm->no_echo)
        echo_server_session_cb_vft.builtin_app_rx_callback = echo_server_builtin_server_rx_callback_no_echo;
    else
        echo_server_session_cb_vft.builtin_app_rx_callback = echo_server_rx_callback;
...
echo_server_create(vlib_main_t *vm, u8 *appns_id, u64 appns_flags, u64 appns_secret)
{
    ...
    for (i = 0; i < vec_len(esm->rx_retries); i++)
        vec_validate(esm->rx_retries[i], pool_elts(session_manager_main.sessions[i]));
