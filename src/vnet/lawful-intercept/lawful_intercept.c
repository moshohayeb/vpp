...
static clib_error_t *
set_li_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (is_add == 1) {
        for (i = 0; i < vec_len(lm->collectors); i++) {
            if (lm->collectors[i].as_u32 == collector.as_u32) {
                if (lm->ports[i] == udp_port)
                    ...
                    return clib_error_return(0, "collector %U already configured with port %d", &collector,
                                             (int) (lm->ports[i]));
            }
        }
        ...
        for (i = 0; i < vec_len(lm->collectors); i++) {
            if ((lm->collectors[i].as_u32 == collector.as_u32) && lm->ports[i] == udp_port) {
...
VLIB_CLI_COMMAND(set_li_command, static) = {
    .path       = "set li",
    .short_help = "set li src <ip4-address> collector <ip4-address> udp-port <nnnn>",
