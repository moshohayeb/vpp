...
format_ipsec_gre_tx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "GRE: tunnel %d len %d src %U dst %U sa-id %d", t->tunnel_id, clib_net_to_host_u16(t->length),
               format_ip4_address, &t->src.as_u8, format_ip4_address, &t->dst.as_u8, t->sa_id);
    return s;
}
 ...
 */
static uword
ipsec_gre_interface_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            ip1->length             = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b1));
            ...
            vnet_buffer(b0)->sw_if_index[VLIB_RX] = vnet_buffer(b0)->sw_if_index[VLIB_TX];
            vnet_buffer(b1)->sw_if_index[VLIB_RX] = vnet_buffer(b1)->sw_if_index[VLIB_TX];
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ipsec_gre_tx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                ipsec_gre_tx_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
                tr->tunnel_id            = t - igm->tunnels;
    ...
    vlib_node_increment_counter(vm, ipsec_gre_input_node.index, IPSEC_GRE_ERROR_PKTS_ENCAP, frame->n_vectors);

    return frame->n_vectors;
}

static clib_error_t *
ipsec_gre_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
