...
u8 *
format_ipsec_gre_rx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "GRE: tunnel %d len %d src %U dst %U", t->tunnel_id, clib_net_to_host_u16(t->length),
               format_ip4_address, &t->src.as_u8, format_ip4_address, &t->dst.as_u8);
    return s;
}
 ...
 */
static uword
ipsec_gre_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            b0->error = verr0 ? node->errors[IPSEC_GRE_ERROR_UNSUPPORTED_VERSION] : b0->error;
            next0     = verr0 ? IPSEC_GRE_INPUT_NEXT_DROP : next0;
            b1->error = verr1 ? node->errors[IPSEC_GRE_ERROR_UNSUPPORTED_VERSION] : b1->error;
            ...
            if (PREDICT_TRUE(next0 == IPSEC_GRE_INPUT_NEXT_L2_INPUT)) {
                ...
                if (cached_tunnel_key != key) {
                    ...
                    hi                        = vnet_get_hw_interface(igm->vnet_main, t->hw_if_index);
        ...
        drop1:
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ipsec_gre_rx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->tunnel_id            = ~0;
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                ipsec_gre_rx_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
                tr->tunnel_id            = ~0;
    ...
    vlib_node_increment_counter(vm, ipsec_gre_input_node.index, IPSEC_GRE_ERROR_PKTS_DECAP, from_frame->n_vectors);
...
static clib_error_t *
ipsec_gre_input_init(vlib_main_t *vm)
{
    {
        clib_error_t *error;
