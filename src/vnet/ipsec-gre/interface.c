...
format_ipsec_gre_tunnel(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d] %U (src) %U (dst) local-sa %d remote-sa %d", t - gm->tunnels, format_ip4_address,
               &t->tunnel_src, format_ip4_address, &t->tunnel_dst, t->local_sa_id, t->remote_sa_id);
    return s;
}

static clib_error_t *
show_ipsec_gre_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    /* *INDENT-OFF* */
    pool_foreach(t, igm->tunnels, ({ vlib_cli_output(vm, "%U", format_ipsec_gre_tunnel, t); }));
    /* *INDENT-ON* */
 ...
 */
int
vnet_ipsec_gre_add_del_tunnel(vnet_ipsec_gre_add_del_tunnel_args_t *a, u32 *sw_if_indexp)
{
    ipsec_gre_main_t *igm = &ipsec_gre_main;
    ...
    if (a->is_add) {
        ...
        if (vec_len(igm->free_ipsec_gre_tunnel_hw_if_indices) > 0) {
            ...
            hw_if_index = igm->free_ipsec_gre_tunnel_hw_if_indices[vec_len(igm->free_ipsec_gre_tunnel_hw_if_indices) - 1];
            ...
            hw_if_index = vnet_register_interface(vnm, ipsec_gre_device_class.index, t - igm->tunnels,
        ...
        vec_validate_init_empty(igm->tunnel_index_by_sw_if_index, sw_if_index, ~0);
        igm->tunnel_index_by_sw_if_index[sw_if_index] = t - igm->tunnels;

        vec_validate(im->fib_index_by_sw_if_index, sw_if_index);

        hi->min_packet_bytes =
            64 + sizeof(gre_header_t) + sizeof(ip4_header_t) + sizeof(esp_header_t) + sizeof(esp_footer_t);
        ...
        slot = vlib_node_add_named_next_with_slot(vnm->vlib_main, hi->tx_node_index, "esp-encrypt",
        ...
        set_int_l2_mode(igm->vlib_main, vnm, MODE_L3, sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0);
...
static clib_error_t *
create_ipsec_gre_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    case VNET_API_ERROR_INVALID_VALUE:
        ...
        goto done;
    default:
        error = clib_error_return(0, "vnet_ipsec_gre_add_del_tunnel returned %d", rv);
        goto done;
    }
