...
static void
vl_api_ipsec_gre_add_del_tunnel_t_handler(vl_api_ipsec_gre_add_del_tunnel_t *mp)
{
...
out:
    /* *INDENT-OFF* */
    REPLY_MACRO2(VL_API_GRE_ADD_DEL_TUNNEL_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
    /* *INDENT-ON* */
}

static void
send_ipsec_gre_tunnel_details(ipsec_gre_tunnel_t *t, vl_api_registration_t *reg, u32 context)
{
...
static void
vl_api_ipsec_gre_tunnel_dump_t_handler(vl_api_ipsec_gre_tunnel_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        /* *INDENT-OFF* */
        pool_foreach(t, igm->tunnels, ({ send_ipsec_gre_tunnel_details(t, reg, mp->context); }));
        /* *INDENT-ON* */
    } else {
