...
vat_socket_connect(vat_main_t *vam)
{
    ...
    return vl_socket_client_connect((char *) vam->socket_name, "vpp_api_test", 0 /* default socket rx, tx buffer */);
}
...
unformat_ethernet_address(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%_%x:%x:%x:%x:%x:%x%_", &a[0], &a[1], &a[2], &a[3], &a[4], &a[5]))
        return 0;
...
{
    sse2_qos_pol_action_params_st *a = va_arg(*va, sse2_qos_pol_action_params_st *);
...
static void
vl_api_create_loopback_reply_t_handler_json(vl_api_create_loopback_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_create_loopback_instance_reply_t_handler(vl_api_create_loopback_instance_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
...
static void
vl_api_create_loopback_instance_reply_t_handler_json(vl_api_create_loopback_instance_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_af_packet_create_reply_t_handler_json(vl_api_af_packet_create_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_create_vlan_subif_reply_t_handler_json(vl_api_create_vlan_subif_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_interface_name_renumber_reply_t_handler_json(vl_api_interface_name_renumber_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
vl_api_sw_interface_details_t_handler_json(vl_api_sw_interface_details_t *mp)
{
    ...
    if (mp->sub_dot1ah) {
        vat_json_object_add_string_copy(node, "pbb_vtr_dmac", format(0, "%U", format_ethernet_address, &mp->b_dmac));
        vat_json_object_add_string_copy(node, "pbb_vtr_smac", format(0, "%U", format_ethernet_address, &mp->b_smac));
...
vl_api_sw_interface_event_t_handler(vl_api_sw_interface_event_t *mp)
{
    ...
    if (vam->interface_event_display)
        errmsg("interface flags: sw_if_index %d %s %s", ntohl(mp->sw_if_index),
               mp->admin_up_down ? "admin-up" : "admin-down", mp->link_up_down ? "link-up" : "link-down");
}
#endif
...
static void
vl_api_classify_add_del_table_reply_t_handler(vl_api_classify_add_del_table_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
        ...
        vam->retval = retval;
        if (retval == 0 && ((mp->new_table_index != 0xFFFFFFFF) || (mp->skip_n_vectors != 0xFFFFFFFF) ||
             ...
             */
            errmsg("new index %d, skip_n_vectors %d, match_n_vectors %d", ntohl(mp->new_table_index),
...
static void
vl_api_classify_add_del_table_reply_t_handler_json(vl_api_classify_add_del_table_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
vl_api_show_threads_reply_t_handler(vl_api_show_threads_reply_t *mp)
{
    ...
    for (i = 0; i < count; i++)
        print(vam->ofp, "\n%-2d %-11s %-11s %-5d %-6d %-4d %-6d", ntohl(mp->thread_data[i].id), mp->thread_data[i].name,
              mp->thread_data[i].type, ntohl(mp->thread_data[i].pid), ntohl(mp->thread_data[i].cpu_id),
              ntohl(mp->thread_data[i].core), ntohl(mp->thread_data[i].cpu_socket));
...
vl_api_ip4_arp_event_t_handler(vl_api_ip4_arp_event_t *mp)
{
    ...
    errmsg("arp %s event: pid %d address %U new mac %U sw_if_index %d\n",
           mp->mac_ip ? "mac/ip binding" : "address resolution", ntohl(mp->pid), format_ip4_address, &mp->address,
...
vl_api_ip6_nd_event_t_handler(vl_api_ip6_nd_event_t *mp)
{
    ...
    errmsg("ip6 nd %s event: pid %d address %U new mac %U sw_if_index %d\n",
           mp->mac_ip ? "mac/ip binding" : "address resolution", ntohl(mp->pid), format_ip6_address, mp->address,
           format_ethernet_address, mp->new_mac, sw_if_index);
}
...
vl_api_bridge_domain_details_t_handler(vl_api_bridge_domain_details_t *mp)
{
    ...
    print(vam->ofp, "%3d %3d %3d %3d %3d %6d %3d", ntohl(mp->bd_id), mp->learn, mp->forward, mp->flood,
          ntohl(mp->bvi_sw_if_index), ntohl(mp->uu_fwd_sw_if_index), n_sw_ifs);
...
static void
vl_api_bridge_domain_details_t_handler_json(vl_api_bridge_domain_details_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_bridge_domain_set_mac_age_reply_t_handler_json(vl_api_bridge_domain_set_mac_age_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_tap_connect_reply_t_handler_json(vl_api_tap_connect_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_tap_modify_reply_t_handler_json(vl_api_tap_modify_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_tap_delete_reply_t_handler_json(vl_api_tap_delete_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_bond_enslave_reply_t_handler_json(vl_api_bond_enslave_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_bond_detach_slave_reply_t_handler_json(vl_api_bond_detach_slave_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_sw_interface_bond_details_t_handler(vl_api_sw_interface_bond_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp, "%-16s %-12d %-12U %-13U %-14u %-14u", mp->interface_name, ntohl(mp->sw_if_index), format_bond_mode,
          mp->mode, format_bond_load_balance, mp->lb, ntohl(mp->active_slaves), ntohl(mp->slaves));
}

static void
vl_api_sw_interface_bond_details_t_handler_json(vl_api_sw_interface_bond_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
...
api_sw_interface_bond_dump(vat_main_t *vam)
{
    ...
    print(vam->ofp, "\n%-16s %-12s %-12s %-13s %-14s %-14s", "interface name", "sw_if_index", "mode", "load balance",
...
static void
vl_api_sw_interface_slave_details_t_handler(vl_api_sw_interface_slave_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp, "%-25s %-12d %-12d %d", mp->interface_name, ntohl(mp->sw_if_index), mp->is_passive,
          mp->is_long_timeout);
}

static void
vl_api_sw_interface_slave_details_t_handler_json(vl_api_sw_interface_slave_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
...
static void
vl_api_mpls_tunnel_add_del_reply_t_handler(vl_api_mpls_tunnel_add_del_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
...
static void
vl_api_mpls_tunnel_add_del_reply_t_handler_json(vl_api_mpls_tunnel_add_del_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    ...
    vat_json_init_object(&node);
    vat_json_object_add_int(&node, "retval", ntohl(mp->retval));
    vat_json_object_add_uint(&node, "tunnel_sw_if_index", ntohl(mp->sw_if_index));
...
static void
vl_api_l2tpv3_create_tunnel_reply_t_handler(vl_api_l2tpv3_create_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
...
static void
vl_api_l2tpv3_create_tunnel_reply_t_handler_json(vl_api_l2tpv3_create_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_gpe_add_del_fwd_entry_reply_t_handler(vl_api_gpe_add_del_fwd_entry_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
...
static void
vl_api_gpe_add_del_fwd_entry_reply_t_handler_json(vl_api_gpe_add_del_fwd_entry_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    ...
    vat_json_init_object(&node);
    vat_json_object_add_int(&node, "retval", ntohl(mp->retval));
    vat_json_object_add_uint(&node, "fwd_entry_index", clib_net_to_host_u32(mp->fwd_entry_index));
...
static void
vl_api_one_get_transport_protocol_reply_t_handler(vl_api_one_get_transport_protocol_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
...
static void
vl_api_one_get_transport_protocol_reply_t_handler_json(vl_api_one_get_transport_protocol_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    u8 *s;
...
static void
vl_api_one_add_del_locator_set_reply_t_handler(vl_api_one_add_del_locator_set_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
...
static void
vl_api_one_add_del_locator_set_reply_t_handler_json(vl_api_one_add_del_locator_set_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_vxlan_add_del_tunnel_reply_t_handler(vl_api_vxlan_add_del_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
...
static void
vl_api_vxlan_add_del_tunnel_reply_t_handler_json(vl_api_vxlan_add_del_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_vxlan_offload_rx_reply_t_handler_json(vl_api_vxlan_offload_rx_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_geneve_add_del_tunnel_reply_t_handler(vl_api_geneve_add_del_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
...
static void
vl_api_geneve_add_del_tunnel_reply_t_handler_json(vl_api_geneve_add_del_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_vxlan_gpe_add_del_tunnel_reply_t_handler(vl_api_vxlan_gpe_add_del_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
...
static void
vl_api_vxlan_gpe_add_del_tunnel_reply_t_handler_json(vl_api_vxlan_gpe_add_del_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_gre_add_del_tunnel_reply_t_handler(vl_api_gre_add_del_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
...
static void
vl_api_gre_add_del_tunnel_reply_t_handler_json(vl_api_gre_add_del_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_create_vhost_user_if_reply_t_handler(vl_api_create_vhost_user_if_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
...
static void
vl_api_create_vhost_user_if_reply_t_handler_json(vl_api_create_vhost_user_if_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
vl_api_dns_resolve_name_reply_t_handler(vl_api_dns_resolve_name_reply_t *mp)
{
    ...
    if (vam->async_mode) {
        ...
        if (retval == 0) {
            if (mp->ip4_set)
                clib_warning("ip4 address %U", format_ip4_address, (ip4_address_t *) mp->ip4_address);
            if (mp->ip6_set)
                clib_warning("ip6 address %U", format_ip6_address, (ip6_address_t *) mp->ip6_address);
        } else
...
static void
vl_api_dns_resolve_name_reply_t_handler_json(vl_api_dns_resolve_name_reply_t *mp)
{
...
static void
vl_api_dns_resolve_ip_reply_t_handler_json(vl_api_dns_resolve_ip_reply_t *mp)
{
    clib_warning("not implemented");
}
...
vl_api_dhcp_compl_event_t_handler(vl_api_dhcp_compl_event_t *mp)
{
    errmsg("DHCP compl event: pid %d %s hostname %s host_addr %U "
           ...
           ntohl(mp->pid), mp->lease.is_ipv6 ? "ipv6" : "ipv4", mp->lease.hostname, format_ip4_address,
           &mp->lease.host_address, format_ip4_address, &mp->lease.router_address, format_ethernet_address,
           mp->lease.host_mac);
}

static void
vl_api_dhcp_compl_event_t_handler_json(vl_api_dhcp_compl_event_t *mp)
{
    /* JSON output not supported */
}

static void
set_simple_interface_counter(u8 vnet_counter_type, u32 sw_if_index, u32 counter)
{
    vat_main_t *vam            = &vat_main;
    static u64 default_counter = 0;

    vec_validate_init_empty(vam->simple_interface_counters, vnet_counter_type, NULL);
    vec_validate_init_empty(vam->simple_interface_counters[vnet_counter_type], sw_if_index, default_counter);
    vam->simple_interface_counters[vnet_counter_type][sw_if_index] = counter;
}

static void
set_combined_interface_counter(u8 vnet_counter_type, u32 sw_if_index, interface_counter_t counter)
{
    vat_main_t *vam                            = &vat_main;
    static interface_counter_t default_counter = {
        0,
    };

    vec_validate_init_empty(vam->combined_interface_counters, vnet_counter_type, NULL);
    vec_validate_init_empty(vam->combined_interface_counters[vnet_counter_type], sw_if_index, default_counter);
    vam->combined_interface_counters[vnet_counter_type][sw_if_index] = counter;
}

static void
vl_api_vnet_interface_simple_counters_t_handler(vl_api_vnet_interface_simple_counters_t *mp)
{
    /* not supported */
}

static void
vl_api_vnet_interface_combined_counters_t_handler(vl_api_vnet_interface_combined_counters_t *mp)
{
    /* not supported */
}

static void
vl_api_vnet_interface_simple_counters_t_handler_json(vl_api_vnet_interface_simple_counters_t *mp)
{
    u64 *v_packets;
    u64 packets;
    u32 count;
    ...
    for (i = 0; i < count; i++) {
        ...
        set_simple_interface_counter(mp->vnet_counter_type, first_sw_if_index + i, packets);
        v_packets++;
    }
}

static void
vl_api_vnet_interface_combined_counters_t_handler_json(vl_api_vnet_interface_combined_counters_t *mp)
{
    ...
    for (i = 0; i < count; i++) {
        counter.packets = clib_net_to_host_u64(clib_mem_unaligned(&v->packets, u64));
        counter.bytes   = clib_net_to_host_u64(clib_mem_unaligned(&v->bytes, u64));
        set_combined_interface_counter(mp->vnet_counter_type, first_sw_if_index + i, counter);
        v++;
    }
}
...
static void
vl_api_vnet_ip4_fib_counters_t_handler_json(vl_api_vnet_ip4_fib_counters_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_vnet_ip4_nbr_counters_t_handler_json(vl_api_vnet_ip4_nbr_counters_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_vnet_ip6_fib_counters_t_handler_json(vl_api_vnet_ip6_fib_counters_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_vnet_ip6_nbr_counters_t_handler_json(vl_api_vnet_ip6_nbr_counters_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_get_first_msg_id_reply_t_handler_json(vl_api_get_first_msg_id_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
vl_api_one_locator_details_t_handler(vl_api_one_locator_details_t *mp)
{
    ...
    if (mp->local) {
        ...
        s = format(s, "%=16U%=16d%=16d", mp->is_ipv6 ? format_ip6_address : format_ip4_address, mp->ip_address,
...
vl_api_one_locator_details_t_handler_json(vl_api_one_locator_details_t *mp)
{
    ...
    if (mp->local)
        vat_json_object_add_uint(node, "sw_if_index", clib_net_to_host_u32(mp->sw_if_index));
    else {
        if (mp->is_ipv6) {
...
static void
vl_api_one_locator_set_details_t_handler_json(vl_api_one_locator_set_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    ...
    vat_json_object_add_uint(node, "ls_index", clib_net_to_host_u32(mp->ls_index));
    vec_free(ls_name);
}
...
vl_api_one_eid_table_details_t_handler(vl_api_one_eid_table_details_t *mp)
{
    ...
    eid = format(0, "%U", format_lisp_eid_vat, mp->eid_type, mp->eid, mp->eid_prefix_len, mp->seid, mp->seid_prefix_len,
                 mp->is_src_dst);
    vec_add1(eid, 0);

    print(vam->ofp, "[%d] %-35s%-20s%-30s%-20d%-20d%-10d%-20s", clib_net_to_host_u32(mp->vni), eid,
          mp->is_local ? "local" : "remote", s, clib_net_to_host_u32(mp->ttl), mp->authoritative,
...
static void
vl_api_one_eid_table_details_t_handler_json(vl_api_one_eid_table_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = 0;
    u8 *eid               = 0;
    ...
    if (~0 == mp->locator_set_index)
        ...
        vat_json_object_add_uint(node, "locator_set_index", clib_net_to_host_u32(mp->locator_set_index));
    ...
    if (mp->eid_type == 3) {
        ...
        vat_json_object_add_uint(nsh_json, "spi", clib_net_to_host_u32(nsh->spi));
        vat_json_object_add_uint(nsh_json, "si", nsh->si);
    } else {
        eid = format(0, "%U", format_lisp_eid_vat, mp->eid_type, mp->eid, mp->eid_prefix_len, mp->seid,
                     mp->seid_prefix_len, mp->is_src_dst);
        vec_add1(eid, 0);
...
vl_api_one_stats_details_t_handler(vl_api_one_stats_details_t *mp)
{
    ...
    deid = format(0, "%U", format_lisp_eid_vat, mp->eid_type, mp->deid, mp->deid_pref_len, 0, 0, 0);

    seid = format(0, "%U", format_lisp_eid_vat, mp->eid_type, mp->seid, mp->seid_pref_len, 0, 0, 0);
    ...
    print(vam->ofp, "([%d] %s %s) (%U %U) %u %u", clib_net_to_host_u32(mp->vni), seid, deid, format_ip_address_fcn,
          mp->lloc, format_ip_address_fcn, mp->rloc, clib_net_to_host_u32(mp->pkt_count),
...
vl_api_one_stats_details_t_handler_json(vl_api_one_stats_details_t *mp)
{
    ...
    vat_json_object_add_uint(node, "pkt_count", clib_net_to_host_u32(mp->pkt_count));
...
static void
vl_api_one_eid_table_map_details_t_handler(vl_api_one_eid_table_map_details_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_one_eid_table_map_details_t_handler_json(vl_api_one_eid_table_map_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
    ...
    node = vat_json_array_add(&vam->json_tree);
    vat_json_init_object(node);
    vat_json_object_add_uint(node, "dp_table", clib_net_to_host_u32(mp->dp_table));
    vat_json_object_add_uint(node, "vni", clib_net_to_host_u32(mp->vni));
}

static void
vl_api_one_eid_table_vni_details_t_handler(vl_api_one_eid_table_vni_details_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_one_eid_table_vni_details_t_handler_json(vl_api_one_eid_table_vni_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
...
static void
vl_api_show_one_map_register_fallback_threshold_reply_t_handler(vl_api_show_one_map_register_fallback_threshold_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_show_one_map_register_state_reply_t_handler(vl_api_show_one_map_register_state_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    int retval      = clib_net_to_host_u32(mp->retval);
...
static void
vl_api_show_one_map_register_state_reply_t_handler_json(vl_api_show_one_map_register_state_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t _node, *node = &_node;
    int retval = clib_net_to_host_u32(mp->retval);
...
static void
vl_api_show_one_rloc_probe_state_reply_t_handler(vl_api_show_one_rloc_probe_state_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    int retval      = clib_net_to_host_u32(mp->retval);
...
static void
vl_api_show_one_rloc_probe_state_reply_t_handler_json(vl_api_show_one_rloc_probe_state_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t _node, *node = &_node;
    int retval = clib_net_to_host_u32(mp->retval);
...
static void
vl_api_show_one_stats_enable_disable_reply_t_handler(vl_api_show_one_stats_enable_disable_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    int retval      = clib_net_to_host_u32(mp->retval);
...
static void
vl_api_show_one_stats_enable_disable_reply_t_handler_json(vl_api_show_one_stats_enable_disable_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t _node, *node = &_node;
    int retval = clib_net_to_host_u32(mp->retval);
...
static void
gpe_fwd_entries_get_reply_t_net_to_host(vl_api_gpe_fwd_entries_get_reply_t *mp)
{
    u32 i;
...
static void
vl_api_gpe_get_encap_mode_reply_t_handler(vl_api_gpe_get_encap_mode_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_gpe_get_encap_mode_reply_t_handler_json(vl_api_gpe_get_encap_mode_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_gpe_fwd_entry_path_details_t_handler(vl_api_gpe_fwd_entry_path_details_t *mp)
{
    vat_main_t *vam                               = &vat_main;
    u8 *(*format_ip_address_fcn)(u8 *, va_list *) = 0;
    ...
    print(vam->ofp, "w:%d %30U %30U", mp->rmt_loc.weight, format_ip_address_fcn, &mp->lcl_loc, format_ip_address_fcn,
          &mp->rmt_loc);
}
...
static void
vl_api_gpe_fwd_entry_path_details_t_handler_json(vl_api_gpe_fwd_entry_path_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
...
static void
vl_api_gpe_fwd_entries_get_reply_t_handler(vl_api_gpe_fwd_entries_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32 i;
    int retval = clib_net_to_host_u32(mp->retval);
    ...
    for (i = 0; i < mp->count; i++) {
        e = &mp->entries[i];
        print(vam->ofp, "%10d %10d %U %40U", e->fwd_entry_index, e->dp_table, format_lisp_flat_eid, e->eid_type,
              e->leid, e->leid_prefix_len, format_lisp_flat_eid, e->eid_type, e->reid, e->reid_prefix_len);
    }
...
static void
vl_api_gpe_fwd_entries_get_reply_t_handler_json(vl_api_gpe_fwd_entries_get_reply_t *mp)
{
    u8 *s              = 0;
    vat_main_t *vam    = &vat_main;
    ...
    for (i = 0; i < mp->count; i++) {
        ...
        s = format(0, "%U", format_lisp_flat_eid, fwd->eid_type, fwd->leid, fwd->leid_prefix_len);
        vec_add1(s, 0);
        vat_json_object_add_string_copy(e, "leid", s);
        vec_free(s);

        s = format(0, "%U", format_lisp_flat_eid, fwd->eid_type, fwd->reid, fwd->reid_prefix_len);
        vec_add1(s, 0);
...
static void
vl_api_gpe_native_fwd_rpaths_get_reply_t_handler(vl_api_gpe_native_fwd_rpaths_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32 i, n;
    int retval = clib_net_to_host_u32(mp->retval);
    ...
    for (i = 0; i < n; i++) {
        r = &mp->entries[i];
        print(vam->ofp, "fib_index: %d sw_if_index %d nh %U", clib_net_to_host_u32(r->fib_index),
              clib_net_to_host_u32(r->nh_sw_if_index), r->is_ip4 ? format_ip4_address : format_ip6_address, r->nh_addr);
    }
...
static void
vl_api_gpe_native_fwd_rpaths_get_reply_t_handler_json(vl_api_gpe_native_fwd_rpaths_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    ...
    for (i = 0; i < n; i++) {
        ...
        vat_json_init_object(e);
        r = &mp->entries[i];
        s = format(0, "%U", r->is_ip4 ? format_ip4_address : format_ip6_address, r->nh_addr);
        vec_add1(s, 0);
        vat_json_object_add_string_copy(e, "ip4", s);
        vec_free(s);

        vat_json_object_add_uint(e, "fib_index", clib_net_to_host_u32(r->fib_index));
        vat_json_object_add_uint(e, "nh_sw_if_index", clib_net_to_host_u32(r->nh_sw_if_index));
    }
...
static void
vl_api_gpe_fwd_entry_vnis_get_reply_t_handler(vl_api_gpe_fwd_entry_vnis_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32 i, n;
    int retval = clib_net_to_host_u32(mp->retval);
...
static void
vl_api_gpe_fwd_entry_vnis_get_reply_t_handler_json(vl_api_gpe_fwd_entry_vnis_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_one_ndp_entries_get_reply_t_handler(vl_api_one_ndp_entries_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32 i, n;
    int retval = clib_net_to_host_u32(mp->retval);
...
static void
vl_api_one_ndp_entries_get_reply_t_handler_json(vl_api_one_ndp_entries_get_reply_t *mp)
{
    u8 *s              = 0;
    vat_main_t *vam    = &vat_main;
    vat_json_node_t *e = 0, root;
...
static void
vl_api_one_l2_arp_entries_get_reply_t_handler(vl_api_one_l2_arp_entries_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32 i, n;
    int retval = clib_net_to_host_u32(mp->retval);
...
static void
vl_api_one_l2_arp_entries_get_reply_t_handler_json(vl_api_one_l2_arp_entries_get_reply_t *mp)
{
    u8 *s              = 0;
    vat_main_t *vam    = &vat_main;
    vat_json_node_t *e = 0, root;
...
static void
vl_api_one_ndp_bd_get_reply_t_handler_json(vl_api_one_ndp_bd_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t root;
    u32 i, n;
    ...
    for (i = 0; i < n; i++) {
        vat_json_array_add_uint(&root, clib_net_to_host_u32(mp->bridge_domains[i]));
    }
...
static void
vl_api_one_l2_arp_bd_get_reply_t_handler_json(vl_api_one_l2_arp_bd_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t root;
    u32 i, n;
...
static void
vl_api_one_adjacencies_get_reply_t_handler(vl_api_one_adjacencies_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32 i, n;
    int retval = clib_net_to_host_u32(mp->retval);
    ...
    for (i = 0; i < n; i++) {
        a = &mp->adjacencies[i];
        print(vam->ofp, "%U %40U", format_lisp_flat_eid, a->eid_type, a->leid, a->leid_prefix_len, format_lisp_flat_eid,
              a->eid_type, a->reid, a->reid_prefix_len);
    }
...
static void
vl_api_one_adjacencies_get_reply_t_handler_json(vl_api_one_adjacencies_get_reply_t *mp)
{
    u8 *s              = 0;
    vat_main_t *vam    = &vat_main;
    vat_json_node_t *e = 0, root;
    ...
    for (i = 0; i < n; i++) {
        ...
        vat_json_init_object(e);
        s = format(0, "%U", format_lisp_flat_eid, a->eid_type, a->leid, a->leid_prefix_len);
        vec_add1(s, 0);
        vat_json_object_add_string_copy(e, "leid", s);
        vec_free(s);

        s = format(0, "%U", format_lisp_flat_eid, a->eid_type, a->reid, a->reid_prefix_len);
        vec_add1(s, 0);
        vat_json_object_add_string_copy(e, "reid", s);
        vec_free(s);
    }
...
vl_api_one_map_server_details_t_handler(vl_api_one_map_server_details_t *mp)
{
    ...
    print(vam->ofp, "%=20U", mp->is_ipv6 ? format_ip6_address : format_ip4_address, mp->ip_address);
}

static void
vl_api_one_map_server_details_t_handler_json(vl_api_one_map_server_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
    struct in6_addr ip6;
...
static void
vl_api_one_map_resolver_details_t_handler(vl_api_one_map_resolver_details_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_one_map_resolver_details_t_handler_json(vl_api_one_map_resolver_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
    struct in6_addr ip6;
...
vl_api_show_one_status_reply_t_handler(vl_api_show_one_status_reply_t *mp)
{
    ...
    if (0 <= retval) {
        print(vam->ofp, "feature: %s\ngpe: %s", mp->feature_status ? "enabled" : "disabled",
...
static void
vl_api_show_one_status_reply_t_handler_json(vl_api_show_one_status_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    u8 *gpe_status     = NULL;
    ...
    feature_status = format(0, "%s", mp->feature_status ? "enabled" : "disabled");
...
static void
vl_api_one_get_map_request_itr_rlocs_reply_t_handler(vl_api_one_get_map_request_itr_rlocs_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
...
static void
vl_api_one_get_map_request_itr_rlocs_reply_t_handler_json(vl_api_one_get_map_request_itr_rlocs_reply_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
...
static void
vl_api_show_one_map_request_mode_reply_t_handler(vl_api_show_one_map_request_mode_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
...
static void
vl_api_show_one_map_request_mode_reply_t_handler_json(vl_api_show_one_map_request_mode_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    u8 *s = 0;
...
static void
vl_api_one_show_xtr_mode_reply_t_handler_json(vl_api_one_show_xtr_mode_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    u8 *status = 0;
...
static void
vl_api_one_show_pitr_mode_reply_t_handler(vl_api_one_show_pitr_mode_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
...
static void
vl_api_one_show_pitr_mode_reply_t_handler_json(vl_api_one_show_pitr_mode_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    u8 *status = 0;
...
static void
vl_api_one_show_petr_mode_reply_t_handler(vl_api_one_show_petr_mode_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
...
static void
vl_api_one_show_petr_mode_reply_t_handler_json(vl_api_one_show_petr_mode_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    u8 *status = 0;
...
vl_api_show_one_use_petr_reply_t_handler(vl_api_show_one_use_petr_reply_t *mp)
{
    ...
    if (0 <= retval) {
        ...
        if (mp->status) {
            print(vam->ofp, "Proxy-ETR address; %U", mp->is_ip4 ? format_ip4_address : format_ip6_address, mp->address);
        }
    }
...
static void
vl_api_show_one_use_petr_reply_t_handler_json(vl_api_show_one_use_petr_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    u8 *status = 0;
...
static void
vl_api_show_one_nsh_mapping_reply_t_handler(vl_api_show_one_nsh_mapping_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
...
static void
vl_api_show_one_nsh_mapping_reply_t_handler_json(vl_api_show_one_nsh_mapping_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    u8 *status = 0;
...
static void
vl_api_show_one_map_register_ttl_reply_t_handler(vl_api_show_one_map_register_ttl_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
...
static void
vl_api_show_one_map_register_ttl_reply_t_handler_json(vl_api_show_one_map_register_ttl_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
#define _(v, f, str)                                                                                                   \
    case VNET_DSCP_##f:                                                                                                \
        ...
        foreach_vnet_dscp
#undef _
            default : return format(s, "ILLEGAL");
    }
    s = format(s, "%s", t);
    return s;
}
...
vl_api_policer_details_t_handler(vl_api_policer_details_t *mp)
{
    ...
    print(vam->ofp,
          ...
          mp->name, format_policer_type, mp->type, ntohl(mp->cir), ntohl(mp->eir), clib_net_to_host_u64(mp->cb),
          clib_net_to_host_u64(mp->eb), format_policer_rate_type, mp->rate_type, format_policer_round_type,
          mp->round_type, mp->single_rate ? "single" : "dual", mp->color_aware ? "is" : "not",
          ntohl(mp->cir_tokens_per_period), ntohl(mp->pir_tokens_per_period), ntohl(mp->scale),
          ntohl(mp->current_limit), ntohl(mp->current_bucket), ntohl(mp->extended_limit), ntohl(mp->extended_bucket),
          clib_net_to_host_u64(mp->last_update_time), format_policer_action_type, mp->conform_action_type,
          conform_dscp_str, format_policer_action_type, mp->exceed_action_type, exceed_dscp_str,
          format_policer_action_type, mp->violate_action_type, violate_dscp_str);
...
vl_api_policer_details_t_handler_json(vl_api_policer_details_t *mp)
{
    ...
    conform_action_str = format(0, "%U", format_policer_action_type, mp->conform_action_type);
    exceed_action_str  = format(0, "%U", format_policer_action_type, mp->exceed_action_type);
    violate_action_str = format(0, "%U", format_policer_action_type, mp->violate_action_type);
    ...
    vat_json_object_add_uint(node, "cir_tokens_per_period", ntohl(mp->cir_tokens_per_period));
    ...
    vat_json_object_add_uint(node, "extended_bucket", ntohl(mp->extended_bucket));
    vat_json_object_add_uint(node, "last_update_time", ntohl(mp->last_update_time));
...
static void
vl_api_classify_table_ids_reply_t_handler(vl_api_classify_table_ids_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_classify_table_ids_reply_t_handler_json(vl_api_classify_table_ids_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    int i, count = ntohl(mp->count);
...
static void
vl_api_classify_table_by_interface_reply_t_handler(vl_api_classify_table_by_interface_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32 table_id;
...
static void
vl_api_classify_table_by_interface_reply_t_handler_json(vl_api_classify_table_by_interface_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_policer_add_del_reply_t_handler_json(vl_api_policer_add_del_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_classify_table_info_reply_t_handler(vl_api_classify_table_info_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (retval == 0) {
        print(vam->ofp, "classify table info :");
        print(vam->ofp, "sessions: %d nexttbl: %d nextnode: %d", ntohl(mp->active_sessions),
              ntohl(mp->next_table_index), ntohl(mp->miss_next_index));
        print(vam->ofp, "nbuckets: %d skip: %d match: %d", ntohl(mp->nbuckets), ntohl(mp->skip_n_vectors),
              ntohl(mp->match_n_vectors));
        print(vam->ofp, "mask: %U", format_hex_bytes, mp->mask, ntohl(mp->mask_length));
    }
    vam->retval       = retval;
    vam->result_ready = 1;
}
...
static void
vl_api_classify_table_info_reply_t_handler_json(vl_api_classify_table_info_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_classify_session_details_t_handler(vl_api_classify_session_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp, "next_index: %d advance: %d opaque: %d ", ntohl(mp->hit_next_index), ntohl(mp->advance),
          ntohl(mp->opaque_index));
    print(vam->ofp, "mask: %U", format_hex_bytes, mp->match, ntohl(mp->match_length));
}

static void
vl_api_classify_session_details_t_handler_json(vl_api_classify_session_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
...
static void
vl_api_pg_create_interface_reply_t_handler(vl_api_pg_create_interface_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void
vl_api_pg_create_interface_reply_t_handler_json(vl_api_pg_create_interface_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_policer_classify_details_t_handler(vl_api_policer_classify_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp, "%10d%20d", ntohl(mp->sw_if_index), ntohl(mp->table_index));
}

static void
vl_api_policer_classify_details_t_handler_json(vl_api_policer_classify_details_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t *node;
...
static void
vl_api_ipsec_gre_add_del_tunnel_reply_t_handler(vl_api_ipsec_gre_add_del_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    i32 retval      = ntohl(mp->retval);
    if (vam->async_mode) {
...
static void
vl_api_ipsec_gre_add_del_tunnel_reply_t_handler_json(vl_api_ipsec_gre_add_del_tunnel_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
static void
vl_api_flow_classify_details_t_handler_json(vl_api_flow_classify_details_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t *node;
...
#define foreach_vpe_api_reply_msg                                                                                      \
    ...
    _(CLASSIFY_SET_INTERFACE_IP_TABLE_REPLY, classify_set_interface_ip_table_reply)                                    \
    ...
    _(L2TPV3_INTERFACE_ENABLE_DISABLE_REPLY, l2tpv3_interface_enable_disable_reply)                                    \
    ...
    _(ONE_MAP_REGISTER_ENABLE_DISABLE_REPLY, one_map_register_enable_disable_reply)                                    \
    ...
    _(ONE_MAP_REGISTER_FALLBACK_THRESHOLD_REPLY, one_map_register_fallback_threshold_reply)                            \
    _(ONE_RLOC_PROBE_ENABLE_DISABLE_REPLY, one_rloc_probe_enable_disable_reply)                                        \
    ...
    _(SHOW_ONE_STATS_ENABLE_DISABLE_REPLY, show_one_stats_enable_disable_reply)                                        \
    ...
    _(ONE_ENABLE_DISABLE_PITR_MODE_REPLY, one_enable_disable_pitr_mode_reply)                                          \
    _(ONE_ENABLE_DISABLE_PETR_MODE_REPLY, one_enable_disable_petr_mode_reply)                                          \
    ...
    _(GPE_ADD_DEL_NATIVE_FWD_RPATH_REPLY, gpe_add_del_native_fwd_rpath_reply)                                          \
    _(GPE_FWD_ENTRY_PATH_DETAILS, gpe_fwd_entry_path_details)                                                          \
    _(SHOW_ONE_STATUS_REPLY, show_one_status_reply)                                                                    \
    _(ONE_ADD_DEL_MAP_REQUEST_ITR_RLOCS_REPLY, one_add_del_map_request_itr_rlocs_reply)                                \
    _(ONE_GET_MAP_REQUEST_ITR_RLOCS_REPLY, one_get_map_request_itr_rlocs_reply)                                        \
    ...
    _(SHOW_ONE_MAP_REGISTER_STATE_REPLY, show_one_map_register_state_reply)                                            \
    _(SHOW_ONE_MAP_REGISTER_TTL_REPLY, show_one_map_register_ttl_reply)                                                \
    _(SHOW_ONE_MAP_REGISTER_FALLBACK_THRESHOLD_REPLY, show_one_map_register_fallback_threshold_reply)                  \
    ...
    _(IP_SOURCE_AND_PORT_RANGE_CHECK_ADD_DEL_REPLY, ip_source_and_port_range_check_add_del_reply)                      \
    _(IP_SOURCE_AND_PORT_RANGE_CHECK_INTERFACE_ADD_DEL_REPLY, ip_source_and_port_range_check_interface_add_del_reply)  \
...
dump_sub_interface_table(vat_main_t *vam)
{
    ...
    print(vam->ofp, "%-30s%-12s%-11s%-7s%-5s%-9s%-9s%-6s%-8s%-10s%-10s", "Interface", "sw_if_index", "sub id", "dot1ad",
          "tags", "outer id", "inner id", "exact", "default", "outer any", "inner any");

    vec_foreach(sub, vam->sw_if_subif_table)
    {
        print(vam->ofp, "%-30s%-12d%-11d%-7s%-5d%-9d%-9d%-6d%-8d%-10d%-10d", sub->interface_name, sub->sw_if_index,
              sub->sub_id, sub->sub_dot1ad ? "dot1ad" : "dot1q", sub->sub_number_of_tags, sub->sub_outer_vlan_id,
              sub->sub_inner_vlan_id, sub->sub_exact_match, sub->sub_default, sub->sub_outer_vlan_id_any,
              sub->sub_inner_vlan_id_any);
        if (sub->vtr_op != L2_VTR_DISABLED) {
            print(vam->ofp,
                  "  vlan-tag-rewrite - op: %-14s [ dot1q: %d "
                  "tag1: %d tag2: %d ]",
                  str_vtr_op(sub->vtr_op), sub->vtr_push_dot1q, sub->vtr_tag1, sub->vtr_tag2);
        }
    }
...
dump_ip_table(vat_main_t *vam, int is_ipv6)
{
    ...
    vec_foreach(det, vam->ip_details_by_sw_if_index[is_ipv6])
    {
        ...
        {
            print(vam->ofp, "            %-30U%-13d", is_ipv6 ? format_ip6_address : format_ip4_address, address->ip,
...
dump_stats_table(vat_main_t *vam)
{
    ...
    for (i = 0; i < vec_len(vam->simple_interface_counters); i++) {
        ...
        vat_json_object_add_string_copy(msg, "vnet_counter_type", (u8 *) counter_type_to_str(i, 0));
    ...
    for (i = 0; i < vec_len(vam->combined_interface_counters); i++) {
        msg = vat_json_array_add(msg_array);
        vat_json_init_object(msg);
        vat_json_object_add_string_copy(msg, "vnet_counter_type", (u8 *) counter_type_to_str(i, 1));
...
static void
vl_api_sw_interface_rx_placement_details_t_handler(vl_api_sw_interface_rx_placement_details_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32 worker_id   = ntohl(mp->worker_id);

    print(vam->ofp, "\n%-11d %-11s %-6d %-5d %-9s", ntohl(mp->sw_if_index), (worker_id == 0) ? "main" : "worker",
          worker_id, ntohl(mp->queue_id), (mp->mode == 1) ? "polling" : ((mp->mode == 2) ? "interrupt" : "adaptive"));
}

static void
vl_api_sw_interface_rx_placement_details_t_handler_json(vl_api_sw_interface_rx_placement_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
...
static void
vl_api_sw_interface_get_table_reply_t_handler_json(vl_api_sw_interface_get_table_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "rx_sw_if_index %d", &rx_sw_if_index))
            ...
            if (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
                if (unformat(i, "%U", api_unformat_sw_if_index, vam, &rx_sw_if_index))
                    rx_sw_if_index_set = 1;
            } else
                break;
        } else if (unformat(i, "tx")) {
            if (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
                if (unformat(i, "%U", api_unformat_sw_if_index, vam, &tx_sw_if_index))
                    tx_sw_if_index_set = 1;
            } else
                break;
        } else if (unformat(i, "enable"))
...
api_sw_interface_set_l2_bridge(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "sw_if_index %d", &rx_sw_if_index))
            ...
            bd_id_set = 1;
        else if (unformat(i, "%U", api_unformat_sw_if_index, vam, &rx_sw_if_index))
            rx_sw_if_index_set = 1;
...
vl_api_bd_ip_mac_details_t_handler(vl_api_bd_ip_mac_details_t *mp)
{
    ...
    print(vam->ofp, "\n%-5d %-7s %-20U %-30s", ntohl(mp->bd_id), mp->is_ipv6 ? "ip6" : "ip4", format_ethernet_address,
          mp->mac_address, ip);
...
vl_api_bd_ip_mac_details_t_handler_json(vl_api_bd_ip_mac_details_t *mp)
{
    ...
    vat_json_object_add_string_copy(node, "mac_address", format(0, "%U", format_ethernet_address, &mp->mac_address));
    u8 *ip = 0;
...
api_tap_connect(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "mac %U", unformat_ethernet_address, mac_address)) {
            ...
            ;
        else if (unformat(i, "address %U/%d", unformat_ip4_address, &ip4_address, &ip4_mask_width))
            ip4_address_set = 1;
        else if (unformat(i, "address %U/%d", unformat_ip6_address, &ip6_address, &ip6_mask_width))
            ip6_address_set = 1;
        else
            break;
    }
...
api_tap_create_v2(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "hw-addr %U", unformat_ethernet_address, mac_address)) {
            ...
            ;
        else if (unformat(i, "host-mac-addr %U", unformat_ethernet_address, host_mac_addr))
            host_mac_addr_set = 1;
        else if (unformat(i, "host-bridge %s", &host_bridge))
            ;
        else if (unformat(i, "host-ip4-addr %U/%d", unformat_ip4_address, &host_ip4_addr, &host_ip4_prefix_len))
            ;
        else if (unformat(i, "host-ip6-addr %U/%d", unformat_ip6_address, &host_ip6_addr, &host_ip6_prefix_len))
            ;
        else if (unformat(i, "host-ip4-gw %U", unformat_ip4_address, &host_ip4_gw))
            host_ip4_gw_set = 1;
        else if (unformat(i, "host-ip6-gw %U", unformat_ip6_address, &host_ip6_gw))
            host_ip6_gw_set = 1;
...
api_bond_create(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "mode %U", unformat_bond_mode, &mode))
            ...
            ;
        else if (unformat(i, "hw-addr %U", unformat_ethernet_address, mac_address))
            custom_mac = 1;
        else
            break;
    }
...
api_ip_add_del_route(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (is_ipv6 == 0 && unformat(i, "via %U", unformat_ip4_address, &v4_next_hop_address)) {
            next_hop_set = 1;
        } else if (is_ipv6 == 1 && unformat(i, "via %U", unformat_ip6_address, &v6_next_hop_address)) {
            next_hop_set = 1;
    ...
    if (!next_hop_set && !is_drop && !is_local && !is_classify && !is_unreach && !is_prohibit &&
    ...
    for (j = 0; j < count; j++) {
        /* Construct the API message */
        M2(IP_ADD_DEL_ROUTE, mp, sizeof(vl_api_fib_mpls_label_t) * vec_len(next_hop_out_label_stack));
        ...
        if (0 != mp->next_hop_n_out_labels) {
            memcpy(mp->next_hop_out_label_stack, next_hop_out_label_stack,
                   (vec_len(next_hop_out_label_stack) * sizeof(vl_api_fib_mpls_label_t)));
        ...
        if (is_ipv6) {
            ...
            if (next_hop_set)
                clib_memcpy(mp->next_hop_address, &v6_next_hop_address, sizeof(v6_next_hop_address));
            increment_v6_address(&v6_dst_address);
        } else {
            clib_memcpy(mp->dst_address, &v4_dst_address, sizeof(v4_dst_address));
            if (next_hop_set)
                clib_memcpy(mp->next_hop_address, &v4_next_hop_address, sizeof(v4_next_hop_address));
            if (random_add_del)
...
api_ip_mroute_add_del(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "sw_if_index %d", &sw_if_index))
            ;
        else if (unformat(i, "%U %U", unformat_ip4_address, &v4_src_address, unformat_ip4_address, &v4_grp_address)) {
            grp_address_length = 64;
            address_set        = 1;
            is_ipv6            = 0;
        } else if (unformat(i, "%U %U", unformat_ip6_address, &v6_src_address, unformat_ip6_address, &v6_grp_address)) {
...
api_mpls_route_add_del(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U", api_unformat_sw_if_index, vam, &sw_if_index))
            ...
            is_eos = 0;
        else if (unformat(i, "via %U", unformat_ip4_address, &v4_next_hop_address)) {
            next_hop_set   = 1;
            next_hop_proto = DPO_PROTO_IP4;
        } else if (unformat(i, "via %U", unformat_ip6_address, &v6_next_hop_address)) {
            next_hop_set   = 1;
            ...
            next_hop_set   = 1;
            next_hop_proto = DPO_PROTO_IP6;
        } else if (unformat(i, "via l2-input-on %U", api_unformat_sw_if_index, vam, &sw_if_index)) {
            next_hop_set    = 1;
    ...
    for (j = 0; j < count; j++) {
        /* Construct the API message */
        M2(MPLS_ROUTE_ADD_DEL, mp, sizeof(vl_api_fib_mpls_label_t) * vec_len(next_hop_out_label_stack));
        ...
        if (0 != mp->mr_next_hop_n_out_labels) {
            memcpy(mp->mr_next_hop_out_label_stack, next_hop_out_label_stack,
                   vec_len(next_hop_out_label_stack) * sizeof(vl_api_fib_mpls_label_t));
            vec_free(next_hop_out_label_stack);
        }
        ...
        if (next_hop_set) {
            if (DPO_PROTO_IP4 == next_hop_proto) {
                clib_memcpy(mp->mr_next_hop, &v4_next_hop_address, sizeof(v4_next_hop_address));
            } else if (DPO_PROTO_IP6 == next_hop_proto)

            {
                clib_memcpy(mp->mr_next_hop, &v6_next_hop_address, sizeof(v6_next_hop_address));
            }
        }
        local_label++;
...
api_mpls_ip_bind_unbind(vat_main_t *vam)
{
    ...
    /* Parse args required to build the message */
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U/%d", unformat_ip4_address, &v4_address, &address_length)) {
            is_ip4      = 1;
            address_set = 1;
        } else if (unformat(i, "%U/%d", unformat_ip6_address, &v6_address, &address_length)) {
            is_ip4      = 0;
            address_set = 1;
        } else if (unformat(i, "%d", &local_label))
            ;
...
api_bier_route_add_del(vat_main_t *vam)
{
    ...
    if (next_hop_proto_is_ip4) {
        clib_memcpy(mp->br_paths[0].next_hop, &v4_next_hop_address, sizeof(v4_next_hop_address));
    } else {
        clib_memcpy(mp->br_paths[0].next_hop, &v6_next_hop_address, sizeof(v6_next_hop_address));
    }
...
api_proxy_arp_add_del(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "vrf %d", &vrf_id))
            ;
        else if (unformat(i, "%U - %U", unformat_ip4_address, &lo, unformat_ip4_address, &hi))
            range_set = 1;
        else if (unformat(i, "del"))
            is_add = 0;
        else {
...
api_mpls_tunnel_add_del(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "add"))
            is_add = 1;
        else if (unformat(i, "del %U", api_unformat_sw_if_index, vam, &sw_if_index))
            is_add = 0;
            ...
            ;
        else if (unformat(i, "%U", api_unformat_sw_if_index, vam, &next_hop_sw_if_index))
            ;
    ...
    M2(MPLS_TUNNEL_ADD_DEL, mp, sizeof(vl_api_fib_mpls_label_t) * vec_len(next_hop_out_label_stack));
    ...
    if (0 != mp->mt_next_hop_n_out_labels) {
        clib_memcpy(mp->mt_next_hop_out_label_stack, next_hop_out_label_stack,
                    (vec_len(next_hop_out_label_stack) * sizeof(vl_api_fib_mpls_label_t)));
        vec_free(next_hop_out_label_stack);
    }

    if (next_hop_proto_is_ip4) {
        clib_memcpy(mp->mt_next_hop, &v4_next_hop_address, sizeof(v4_next_hop_address));
    } else {
        clib_memcpy(mp->mt_next_hop, &v6_next_hop_address, sizeof(v6_next_hop_address));
    }
...
vl_api_dhcp_proxy_details_t_handler(vl_api_dhcp_proxy_details_t *mp)
{
    ...
    if (mp->is_ipv6)
        print(vam->ofp,
              ...
              ntohl(mp->rx_vrf_id), format_ip6_address, mp->dhcp_src_address, mp->vss_type, mp->vss_vpn_ascii_id,
              ntohl(mp->vss_oui), ntohl(mp->vss_fib_id));
    else
        print(vam->ofp,
              "RX Table-ID %d, Source Address %U, VSS Type %d, "
              "VSS ASCII VPN-ID '%s', VSS RFC2685 VPN-ID (oui:id) %d:%d",
              ntohl(mp->rx_vrf_id), format_ip4_address, mp->dhcp_src_address, mp->vss_type, mp->vss_vpn_ascii_id,
              ntohl(mp->vss_oui), ntohl(mp->vss_fib_id));
    ...
    for (i = 0; i < count; i++) {
        ...
        if (mp->is_ipv6)
            print(vam->ofp, " Server Table-ID %d, Server Address %U", ntohl(s->server_vrf_id), format_ip6_address,
                  s->dhcp_server);
        else
            print(vam->ofp, " Server Table-ID %d, Server Address %U", ntohl(s->server_vrf_id), format_ip4_address,
                  s->dhcp_server);
    }
}

static void
vl_api_dhcp_proxy_details_t_handler_json(vl_api_dhcp_proxy_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
    u32 i, count = mp->count;
    ...
    vat_json_object_add_bytes(node, "vss-type", &mp->vss_type, sizeof(mp->vss_type));
    vat_json_object_add_string_copy(node, "vss-vpn-ascii-id", mp->vss_vpn_ascii_id);
    ...
    for (i = 0; i < count; i++) {
        ...
        vat_json_object_add_uint(node, "server-table-id", ntohl(s->server_vrf_id));
...
vl_api_ip6nd_proxy_details_t_handler(vl_api_ip6nd_proxy_details_t *mp)
{
    ...
    print(vam->ofp, "host %U sw_if_index %d", format_ip6_address, mp->address, ntohl(mp->sw_if_index));
}

static void
vl_api_ip6nd_proxy_details_t_handler_json(vl_api_ip6nd_proxy_details_t *mp)
{
    vat_main_t *vam = &vat_main;
    struct in6_addr ip6;
    vat_json_node_t *node = NULL;
...
#define _(a)                                                                                                           \
    if (a)                                                                                                             \
        memset(&tcp->a, 0xff, sizeof(tcp->a));
    foreach_tcp_proto_field;
#undef _
...
#define _(a)                                                                                                           \
    if (a)                                                                                                             \
        memset(&udp->a, 0xff, sizeof(udp->a));
    foreach_udp_proto_field;
#undef _
...
unformat_l2_mask(unformat_input_t *input, va_list *args)
{
    ...
    if ((src + dst + proto + tag1 + tag2 + dot1q + dot1ad + ignore_tag1 + ignore_tag2 + cos1 + cos2) == 0)
        return 0;
...
api_classify_add_del_table(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "del"))
            ...
            ;
        else if (unformat(i, "mask %U", unformat_classify_mask, &mask, &skip, &match))
            ;
        else if (unformat(i, "next-table %d", &next_table_index))
            ;
        else if (unformat(i, "miss-next %U", api_unformat_ip_next_index, &miss_next_index))
            ;
        else if (unformat(i, "l2-miss-next %U", unformat_l2_next_index, &miss_next_index))
            ;
        else if (unformat(i, "acl-miss-next %U", api_unformat_acl_next_index, &miss_next_index))
            ;
...
unformat_ip4_match(unformat_input_t *input, va_list *args)
{
    ...
    if (version + hdr_length + src + dst + proto + tos + length + fragment_id + ttl + checksum == 0)
        return 0;
...
unformat_ip6_match(unformat_input_t *input, va_list *args)
{
    ...
    if (version + traffic_class + flow_label + src + dst + proto + payload_length + hop_limit == 0)
        return 0;
...
unformat_l2_match(unformat_input_t *input, va_list *args)
{
    ...
    if ((src + dst + proto + tag1 + tag2 + ignore_tag1 + ignore_tag2 + cos1 + cos2) == 0)
        return 0;
...
api_classify_add_del_session(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "del"))
            is_add = 0;
        else if (unformat(i, "hit-next %U", api_unformat_ip_next_index, &hit_next_index))
            ;
        else if (unformat(i, "l2-hit-next %U", unformat_l2_next_index, &hit_next_index))
            ;
        else if (unformat(i, "acl-hit-next %U", api_unformat_acl_next_index, &hit_next_index))
            ;
            ...
            ;
        else if (unformat(i, "match %U", api_unformat_classify_match, &match, skip_n_vectors, match_n_vectors))
            ;
...
api_set_ipfix_exporter(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "collector_address %U", unformat_ip4_address, &collector_address))
            collector_address_set = 1;
        else if (unformat(i, "collector_port %d", &collector_port))
            ;
        else if (unformat(i, "src_address %U", unformat_ip4_address, &src_address))
            src_address_set = 1;
    ...
    memcpy(mp->collector_address, collector_address.data, sizeof(collector_address.data));
...
api_l2tpv3_create_tunnel(vat_main_t *vam)
{
    ...
    clib_memcpy(mp->client_address, client_address.as_u8, sizeof(mp->client_address));
...
static void
vl_api_sw_if_l2tpv3_tunnel_details_t_handler(vl_api_sw_if_l2tpv3_tunnel_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp, "* %U (our) %U (client) (sw_if_index %d)", format_ip6_address, mp->our_address, format_ip6_address,
          mp->client_address, clib_net_to_host_u32(mp->sw_if_index));

    print(vam->ofp, "   local cookies %016llx %016llx remote cookie %016llx", clib_net_to_host_u64(mp->local_cookie[0]),
          clib_net_to_host_u64(mp->local_cookie[1]), clib_net_to_host_u64(mp->remote_cookie));

    print(vam->ofp, "   local session-id %d remote session-id %d", clib_net_to_host_u32(mp->local_session_id),
...
static void
vl_api_sw_if_l2tpv3_tunnel_details_t_handler_json(vl_api_sw_if_l2tpv3_tunnel_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
    ...
    vat_json_object_add_string_copy(node, "l2_sublayer", mp->l2_sublayer_present ? (u8 *) "present" : (u8 *) "absent");
}
...
static void
vl_api_sw_interface_tap_details_t_handler(vl_api_sw_interface_tap_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp, "%-16s %d", mp->dev_name, clib_net_to_host_u32(mp->sw_if_index));
}

static void
vl_api_sw_interface_tap_details_t_handler_json(vl_api_sw_interface_tap_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
...
static void
vl_api_sw_interface_tap_v2_details_t_handler(vl_api_sw_interface_tap_v2_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    u8 *ip4 = format(0, "%U/%d", format_ip4_address, mp->host_ip4_addr, mp->host_ip4_prefix_len);
    u8 *ip6 = format(0, "%U/%d", format_ip6_address, mp->host_ip6_addr, mp->host_ip6_prefix_len);

    print(vam->ofp, "\n%-16s %-12d %-5d %-12d %-12d %-14U %-30s %-20s %-20s %-30s", mp->dev_name,
          ntohl(mp->sw_if_index), ntohl(mp->id), ntohs(mp->rx_ring_sz), ntohs(mp->tx_ring_sz), format_ethernet_address,
...
static void
vl_api_sw_interface_tap_v2_details_t_handler_json(vl_api_sw_interface_tap_v2_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
    ...
    vat_json_object_add_string_copy(node, "host_mac_addr", format(0, "%U", format_ethernet_address, &mp->host_mac_addr));
    ...
    vat_json_object_add_string_copy(node, "host_ip4_addr",
                                    format(0, "%U/%d", format_ip4_address, mp->host_ip4_addr, mp->host_ip4_prefix_len));
    vat_json_object_add_string_copy(node, "host_ip6_addr",
                                    format(0, "%U/%d", format_ip6_address, mp->host_ip6_addr, mp->host_ip6_prefix_len));
}
...
api_sw_interface_tap_v2_dump(vat_main_t *vam)
{
    ...
    print(vam->ofp, "\n%-16s %-12s %-5s %-12s %-12s %-14s %-30s %-20s %-20s %-30s", "dev_name", "sw_if_index", "id",
          "rx_ring_sz", "tx_ring_sz", "host_mac_addr", "host_namespace", "host_bridge", "host_ip4_addr", "host_ip6_addr");
...
api_vxlan_offload_rx(vat_main_t *vam)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            is_add = 0;
        else if (unformat(line_input, "hw %U", api_unformat_hw_if_index, vam, &hw_if_index))
            ;
        else if (unformat(line_input, "hw hw_if_index %u", &hw_if_index))
            ;
        else if (unformat(line_input, "rx %U", api_unformat_sw_if_index, vam, &rx_if_index))
            ;
...
api_vxlan_add_del_tunnel(vat_main_t *vam)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            ...
            ipv6_set = 1;
            dst_set  = 1;
        } else if (unformat(line_input, "group %U %U", unformat_ip4_address, &dst.ip4, api_unformat_sw_if_index, vam,
                            &mcast_sw_if_index)) {
            grp_set = dst_set = 1;
            ipv4_set          = 1;
            ...
            grp_set = dst_set = 1;
            ipv4_set          = 1;
        } else if (unformat(line_input, "group %U %U", unformat_ip6_address, &dst.ip6, api_unformat_sw_if_index, vam,
                            &mcast_sw_if_index)) {
            grp_set = dst_set = 1;
            ipv6_set          = 1;
            ...
            ;
        else if (unformat(line_input, "decap-next %U", unformat_vxlan_decap_next, &decap_next_index))
            ;
...
vl_api_vxlan_tunnel_details_t_handler(vl_api_vxlan_tunnel_details_t *mp)
{
    ...
    print(vam->ofp, "%11d%11d%24U%24U%14d%18d%13d%19d", ntohl(mp->sw_if_index), ntohl(mp->instance),
          format_ip46_address, &src, IP46_TYPE_ANY, format_ip46_address, &dst, IP46_TYPE_ANY, ntohl(mp->encap_vrf_id),
          ntohl(mp->decap_next_index), ntohl(mp->vni), ntohl(mp->mcast_sw_if_index));
}
...
vl_api_vxlan_tunnel_details_t_handler_json(vl_api_vxlan_tunnel_details_t *mp)
{
    ...
    vat_json_object_add_uint(node, "mcast_sw_if_index", ntohl(mp->mcast_sw_if_index));
}
...
api_vxlan_tunnel_dump(vat_main_t *vam)
{
    ...
    if (!vam->json_output) {
        print(vam->ofp, "%11s%11s%24s%24s%14s%18s%13s%19s", "sw_if_index", "instance", "src_address", "dst_address",
...
vl_api_geneve_tunnel_details_t_handler(vl_api_geneve_tunnel_details_t *mp)
{
    ...
    print(vam->ofp, "%11d%24U%24U%14d%18d%13d%19d", ntohl(mp->sw_if_index), format_ip46_address, &src, IP46_TYPE_ANY,
          format_ip46_address, &dst, IP46_TYPE_ANY, ntohl(mp->encap_vrf_id), ntohl(mp->decap_next_index),
          ntohl(mp->vni), ntohl(mp->mcast_sw_if_index));
}

static void
vl_api_geneve_tunnel_details_t_handler_json(vl_api_geneve_tunnel_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
...
api_geneve_tunnel_dump(vat_main_t *vam)
{
    ...
    if (!vam->json_output) {
        print(vam->ofp, "%11s%24s%24s%14s%18s%13s%19s", "sw_if_index", "local_address", "remote_address",
              "encap_vrf_id", "decap_next_index", "vni", "mcast_sw_if_index");
    }
...
static void
vl_api_gre_tunnel_details_t_handler(vl_api_gre_tunnel_details_t *mp)
{
    vat_main_t *vam    = &vat_main;
    ip46_address_t src = to_ip46(mp->is_ipv6, mp->src_address);
    ip46_address_t dst = to_ip46(mp->is_ipv6, mp->dst_address);

    print(vam->ofp, "%11d%11d%24U%24U%13d%14d%12d", ntohl(mp->sw_if_index), ntohl(mp->instance), format_ip46_address,
          &src, IP46_TYPE_ANY, format_ip46_address, &dst, IP46_TYPE_ANY, mp->tunnel_type, ntohl(mp->outer_fib_id),
          ntohl(mp->session_id));
}

static void
vl_api_gre_tunnel_details_t_handler_json(vl_api_gre_tunnel_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
    struct in_addr ip4;
...
api_gre_tunnel_dump(vat_main_t *vam)
{
    ...
    if (!vam->json_output) {
        print(vam->ofp, "%11s%11s%24s%24s%13s%14s%12s", "sw_if_index", "instance", "src_address", "dst_address",
...
static void
vl_api_sw_interface_vhost_user_details_t_handler(vl_api_sw_interface_vhost_user_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp, "%-25s %3" PRIu32 " %6" PRIu32 " %8x %6d %7d %s", (char *) mp->interface_name,
          ntohl(mp->sw_if_index), ntohl(mp->virtio_net_hdr_sz), clib_net_to_host_u64(mp->features), mp->is_server,
          ntohl(mp->num_regions), (char *) mp->sock_filename);
    print(vam->ofp, "    Status: '%s'", strerror(ntohl(mp->sock_errno)));
}

static void
vl_api_sw_interface_vhost_user_details_t_handler_json(vl_api_sw_interface_vhost_user_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
...
api_sw_interface_vhost_user_dump(vat_main_t *vam)
{
    ...
    vl_api_control_ping_t *mp_ping;
    int ret;
    print(vam->ofp, "Interface name            idx hdr_sz features server regions filename");
...
api_vxlan_gpe_add_del_tunnel(vat_main_t *vam)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            is_add = 0;
        else if (unformat(line_input, "local %U", unformat_ip4_address, &local4)) {
            local_set = 1;
            ipv4_set  = 1;
            ...
            remote_set = 1;
            ipv6_set   = 1;
        } else if (unformat(line_input, "group %U %U", unformat_ip4_address, &remote4, api_unformat_sw_if_index, vam,
                            &mcast_sw_if_index)) {
            grp_set = remote_set = 1;
            ipv4_set             = 1;
        } else if (unformat(line_input, "group %U", unformat_ip4_address, &remote4)) {
            grp_set = remote_set = 1;
            ipv4_set             = 1;
        } else if (unformat(line_input, "group %U %U", unformat_ip6_address, &remote6, api_unformat_sw_if_index, vam,
                            &mcast_sw_if_index)) {
            grp_set = remote_set = 1;
            ipv6_set             = 1;
        } else if (unformat(line_input, "group %U", unformat_ip6_address, &remote6)) {
            grp_set = remote_set = 1;
            ipv6_set             = 1;
        } else if (unformat(line_input, "mcast_sw_if_index %u", &mcast_sw_if_index))
            ;
...
static void
vl_api_vxlan_gpe_tunnel_details_t_handler(vl_api_vxlan_gpe_tunnel_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    ip46_address_t local  = to_ip46(mp->is_ipv6, mp->local);
    ip46_address_t remote = to_ip46(mp->is_ipv6, mp->remote);

    print(vam->ofp, "%11d%24U%24U%13d%12d%19d%14d%14d", ntohl(mp->sw_if_index), format_ip46_address, &local,
          IP46_TYPE_ANY, format_ip46_address, &remote, IP46_TYPE_ANY, ntohl(mp->vni), mp->protocol,
          ntohl(mp->mcast_sw_if_index), ntohl(mp->encap_vrf_id), ntohl(mp->decap_vrf_id));
}


static void
vl_api_vxlan_gpe_tunnel_details_t_handler_json(vl_api_vxlan_gpe_tunnel_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
    struct in_addr ip4;
...
api_vxlan_gpe_tunnel_dump(vat_main_t *vam)
{
    ...
    if (!vam->json_output) {
        print(vam->ofp, "%11s%24s%24s%13s%15s%19s%14s%14s", "sw_if_index", "local", "remote", "vni", "protocol",
              "mcast_sw_if_index", "encap_vrf_id", "decap_vrf_id");
    }
...
vl_api_l2_fib_table_details_t_handler(vl_api_l2_fib_table_details_t *mp)
{
    ...
    print(vam->ofp, "%3" PRIu32 "    %U    %3" PRIu32 "       %d       %d     %d", ntohl(mp->bd_id),
          format_ethernet_address, mp->mac, ntohl(mp->sw_if_index), mp->static_mac, mp->filter_mac, mp->bvi_mac);
}
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "del"))
            ...
            if (crypto_alg >= IPSEC_CRYPTO_N_ALG) {
                clib_warning("unsupported crypto-alg: '%U'", format_ipsec_crypto_alg, crypto_alg);
                return -99;
            }
            ...
            if (integ_alg >= IPSEC_INTEG_N_ALG) {
                clib_warning("unsupported integ-alg: '%U'", format_ipsec_integ_alg, integ_alg);
                return -99;
            }
...
api_ipsec_tunnel_if_add_del(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "del"))
            ...
            ;
        else if (unformat(i, "remote_crypto_key %U", unformat_hex_string, &rck))
            ;
...
vl_api_ipsec_sa_details_t_handler(vl_api_ipsec_sa_details_t *mp)
{
    ...
    print(vam->ofp,
          ...
          ntohl(mp->sa_id), ntohl(mp->sw_if_index), ntohl(mp->spi), mp->protocol, mp->crypto_alg, format_hex_bytes,
          mp->crypto_key, mp->crypto_key_len, mp->integ_alg, format_hex_bytes, mp->integ_key, mp->integ_key_len,
          mp->use_esn, mp->use_anti_replay, mp->is_tunnel, mp->is_tunnel_ip6,
          (mp->is_tunnel_ip6) ? format_ip6_address : format_ip4_address, mp->tunnel_src_addr,
          (mp->is_tunnel_ip6) ? format_ip6_address : format_ip4_address, mp->tunnel_dst_addr, ntohl(mp->salt),
          clib_net_to_host_u64(mp->seq_outbound), clib_net_to_host_u64(mp->last_seq_inbound),
          clib_net_to_host_u64(mp->replay_window), clib_net_to_host_u64(mp->total_data_size));
}
...
vl_api_ipsec_sa_details_t_handler_json(vl_api_ipsec_sa_details_t *mp)
{
    ...
    vat_json_object_add_bytes(node, "crypto_key", mp->crypto_key, mp->crypto_key_len);
    vat_json_object_add_bytes(node, "integ_key", mp->integ_key, mp->integ_key_len);
    if (mp->is_tunnel_ip6) {
    ...
    vat_json_object_add_uint(node, "replay_window", clib_net_to_host_u64(mp->replay_window));
    vat_json_object_add_uint(node, "total_data_size", clib_net_to_host_u64(mp->total_data_size));
}
...
api_ipsec_tunnel_if_set_key(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U", api_unformat_sw_if_index, vam, &sw_if_index))
            ;
        else if (unformat(i, "local crypto %U", unformat_ipsec_crypto_alg, &alg))
            key_type = IPSEC_IF_SET_KEY_TYPE_LOCAL_CRYPTO;
        else if (unformat(i, "remote crypto %U", unformat_ipsec_crypto_alg, &alg))
        ...
        else if (unformat(i, "remote integ %U", unformat_ipsec_integ_alg, &alg))
...
api_ikev2_set_responder(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U interface %d address %U", unformat_token, valid_chars, &name, &sw_if_index,
...
api_ikev2_set_ike_transforms(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U %d %d %d %d", unformat_token, valid_chars, &name, &crypto_alg, &crypto_key_size, &integ_alg,
                     &dh_group))
            vec_add1(name, 0);
        else {
            errmsg("parse error '%U'", format_unformat_error, i);
...
api_ikev2_set_sa_lifetime(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U %lu %u %u %lu", unformat_token, valid_chars, &name, &lifetime, &lifetime_jitter, &handover,
                     &lifetime_maxdata))
            vec_add1(name, 0);
        else {
            errmsg("parse error '%U'", format_unformat_error, i);
...
api_cop_interface_enable_disable(vat_main_t *vam)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(line_input, "enable"))
            enable_disable = 1;
        else if (unformat(line_input, "%U", api_unformat_sw_if_index, vam, &sw_if_index))
            ;
        else if (unformat(line_input, "sw_if_index %d", &sw_if_index))
            ;
        else
...
api_one_add_del_locator_set(vat_main_t *vam)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        } else if (unformat(input, "iface %U p %u w %u", api_unformat_sw_if_index, vam, &sw_if_index, &priority, &weight)) {
            locator.sw_if_index = htonl(sw_if_index);
            locator.priority    = priority;
            locator.weight      = weight;
            vec_add1(locators, locator);
...
api_one_add_del_locator(vat_main_t *vam)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del")) {
            ...
            locator_set_name_set = 1;
        } else if (unformat(input, "iface %U", api_unformat_sw_if_index, vam, &tmp_if_index)) {
...
api_one_add_del_adjacency(vat_main_t *vam)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del")) {
            ...
            is_add = 1;
        } else if (unformat(input, "reid %U/%d", unformat_ip4_address, &reid4, &len)) {
            reid_type = 0; /* ipv4 */
            reid_len  = len;
        } else if (unformat(input, "reid %U/%d", unformat_ip6_address, &reid6, &len)) {
            reid_type = 1; /* ipv6 */
            reid_len  = len;
        } else if (unformat(input, "reid %U", unformat_ethernet_address, reid_mac)) {
            reid_type = 2; /* mac */
        } else if (unformat(input, "leid %U/%d", unformat_ip4_address, &leid4, &len)) {
            leid_type = 0; /* ipv4 */
            leid_len  = len;
        } else if (unformat(input, "leid %U/%d", unformat_ip6_address, &leid6, &len)) {
            leid_type = 1; /* ipv6 */
            leid_len  = len;
        } else if (unformat(input, "leid %U", unformat_ethernet_address, leid_mac)) {
            leid_type = 2; /* mac */
        } else if (unformat(input, "vni %d", &vni)) {
            ;
        } else {
...
api_one_eid_table_dump(vat_main_t *vam)
{
    ...
    if (!vam->json_output) {
        print(vam->ofp, "%-35s%-20s%-30s%-20s%-20s%-10s%-20s", "EID", "type", "ls_index", "ttl", "authoritative",
              "key_id", "key");
    }
...
api_gpe_add_del_native_fwd_rpath(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "del"))
            is_add = 0;
        else if (unformat(i, "via %U %U", unformat_ip4_address, &ip4, api_unformat_sw_if_index, vam, &nh_sw_if_index)) {
            ip_set = 1;
            is_ip4 = 1;
        } else if (unformat(i, "via %U %U", unformat_ip6_address, &ip6, api_unformat_sw_if_index, vam, &nh_sw_if_index)) {
            ip_set = 1;
            is_ip4 = 0;
...
vl_api_af_packet_details_t_handler(vl_api_af_packet_details_t *mp)
{
    ...
    print(vam->ofp, "%-16s %d", mp->host_if_name, clib_net_to_host_u32(mp->sw_if_index));
}
...
api_policer_add_del(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "del"))
            ...
            ;
        else if (unformat(i, "rate_type %U", unformat_policer_rate_type, &rate_type))
            ;
        else if (unformat(i, "round_type %U", unformat_policer_round_type, &round_type))
            ;
        else if (unformat(i, "type %U", unformat_policer_type, &type))
            ;
        else if (unformat(i, "conform_action %U", unformat_policer_action_type, &conform_action))
            ;
        else if (unformat(i, "exceed_action %U", unformat_policer_action_type, &exceed_action))
            ;
        else if (unformat(i, "violate_action %U", unformat_policer_action_type, &violate_action))
            ;
...
vl_api_mpls_fib_path_print(vat_main_t *vam, vl_api_fib_path_t *fp)
{
    if (fp->afi == IP46_TYPE_IP6)
        print(vam->ofp,
              ...
              fp->afi, format_ip6_address, fp->next_hop);
    else if (fp->afi == IP46_TYPE_IP4)
        print(vam->ofp,
              "  weight %d, sw_if_index %d, is_local %d, is_drop %d, "
              "is_unreach %d, is_prohitbit %d, afi %d, next_hop %U",
              ntohl(fp->weight), ntohl(fp->sw_if_index), fp->is_local, fp->is_drop, fp->is_unreach, fp->is_prohibit,
              fp->afi, format_ip4_address, fp->next_hop);
}
...
vl_api_mpls_tunnel_details_t_handler(vl_api_mpls_tunnel_details_t *mp)
{
    ...
    print(vam->ofp, "[%d]: sw_if_index %d via:", ntohl(mp->mt_tunnel_index), ntohl(mp->mt_sw_if_index));
    fp = mp->mt_paths;
    for (i = 0; i < count; i++) {
...
vl_api_mpls_tunnel_details_t_handler_json(vl_api_mpls_tunnel_details_t *mp)
{
    ...
    vat_json_init_object(node);
    vat_json_object_add_uint(node, "tunnel_index", ntohl(mp->mt_tunnel_index));
...
vl_api_mpls_fib_details_t_handler(vl_api_mpls_fib_details_t *mp)
{
    ...
    print(vam->ofp, "table-id %d, label %u, ess_bit %u", ntohl(mp->table_id), ntohl(mp->label), mp->eos_bit);
    fp = mp->path;
    for (i = 0; i < count; i++) {
        vl_api_mpls_fib_path_print(vam, fp);
        fp++;
...
static void
vl_api_mpls_fib_details_t_handler_json(vl_api_mpls_fib_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    int count             = ntohl(mp->count);
    vat_json_node_t *node = NULL;
...
vl_api_ip_fib_details_t_handler(vl_api_ip_fib_details_t *mp)
{
    ...
    print(vam->ofp, "table-id %d, prefix %U/%d stats-index %d", ntohl(mp->table_id), format_ip4_address, mp->address,
    ...
    for (i = 0; i < count; i++) {
        if (fp->afi == IP46_TYPE_IP6)
            print(vam->ofp,
                  ...
                  "next_hop_table %d",
                  ntohl(fp->weight), ntohl(fp->sw_if_index), fp->is_local, fp->is_drop, fp->is_unreach, fp->is_prohibit,
                  fp->afi, format_ip6_address, fp->next_hop, ntohl(fp->table_id));
        else if (fp->afi == IP46_TYPE_IP4)
            print(vam->ofp,
                  "  weight %d, sw_if_index %d, is_local %d, is_drop %d, "
                  "is_unreach %d, is_prohitbit %d, afi %d, next_hop %U, "
                  "next_hop_table %d",
                  ntohl(fp->weight), ntohl(fp->sw_if_index), fp->is_local, fp->is_drop, fp->is_unreach, fp->is_prohibit,
                  fp->afi, format_ip4_address, fp->next_hop, ntohl(fp->table_id));
        fp++;
    }
}

static void
vl_api_ip_fib_details_t_handler_json(vl_api_ip_fib_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    int count             = ntohl(mp->count);
    vat_json_node_t *node = NULL;
...
vl_api_ip_neighbor_details_t_handler(vl_api_ip_neighbor_details_t *mp)
{
    ...
    print(vam->ofp, "%c %U %U", (mp->is_static) ? 'S' : 'D', format_ethernet_address, &mp->mac_address,
          (mp->is_ipv6) ? format_ip6_address : format_ip4_address, &mp->ip_address);
}

static void
vl_api_ip_neighbor_details_t_handler_json(vl_api_ip_neighbor_details_t *mp)
{
    ...
    vat_json_init_object(node);
    vat_json_object_add_string_copy(node, "flag", (mp->is_static) ? (u8 *) "static" : (u8 *) "dynamic");

    vat_json_object_add_string_copy(node, "link_layer", format(0, "%U", format_ethernet_address, &mp->mac_address));
...
vl_api_ip6_fib_details_t_handler(vl_api_ip6_fib_details_t *mp)
{
    ...
    print(vam->ofp, "table-id %d, prefix %U/%d stats-index %d", ntohl(mp->table_id), format_ip6_address, mp->address,
          mp->address_length, ntohl(mp->stats_index));
    fp = mp->path;
    for (i = 0; i < count; i++) {
        if (fp->afi == IP46_TYPE_IP6)
...
static void
vl_api_ip6_fib_details_t_handler_json(vl_api_ip6_fib_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    int count             = ntohl(mp->count);
    vat_json_node_t *node = NULL;
...
vl_api_ipfix_exporter_details_t_handler(vl_api_ipfix_exporter_details_t *mp)
{
    ...
    print(vam->ofp,
          ...
          format_ip4_address, mp->collector_address, ntohs(mp->collector_port), format_ip4_address, mp->src_address,
          ntohl(mp->vrf_id), ntohl(mp->path_mtu), ntohl(mp->template_interval), mp->udp_checksum);
...
static void
vl_api_ipfix_exporter_details_t_handler_json(vl_api_ipfix_exporter_details_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
    ...
    vat_json_init_object(&node);
    clib_memcpy(&collector_address, &mp->collector_address, sizeof(collector_address));
    vat_json_object_add_ip4(&node, "collector_address", collector_address);
    vat_json_object_add_uint(&node, "collector_port", ntohs(mp->collector_port));
    ...
    vat_json_object_add_uint(&node, "template_interval", ntohl(mp->template_interval));
...
static void
vl_api_ipfix_classify_stream_details_t_handler(vl_api_ipfix_classify_stream_details_t *mp)
{
    vat_main_t *vam = &vat_main;
    print(vam->ofp, "domain_id %d, src_port %d", ntohl(mp->domain_id), ntohs(mp->src_port));
    vam->retval       = 0;
    vam->result_ready = 1;
}

static void
vl_api_ipfix_classify_stream_details_t_handler_json(vl_api_ipfix_classify_stream_details_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
api_ipfix_classify_table_dump(vat_main_t *vam)
{
    ...
    if (!vam->json_output) {
        print(vam->ofp, "%15s%15s%20s", "table_id", "ip_version", "transport_protocol");
    }
...
static void
vl_api_ipfix_classify_table_details_t_handler(vl_api_ipfix_classify_table_details_t *mp)
{
    vat_main_t *vam = &vat_main;
    print(vam->ofp, "%15d%15d%20d", ntohl(mp->table_id), mp->ip_version, mp->transport_protocol);
}

static void
vl_api_ipfix_classify_table_details_t_handler_json(vl_api_ipfix_classify_table_details_t *mp)
{
    vat_json_node_t *node = NULL;
    vat_main_t *vam       = &vat_main;
    ...
    vat_json_object_add_uint(node, "transport_protocol", mp->transport_protocol);
}
...
api_sw_interface_span_enable_disable(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "src %U", api_unformat_sw_if_index, vam, &src_sw_if_index))
            ;
        else if (unformat(i, "src_sw_if_index %d", &src_sw_if_index))
            ;
        else if (unformat(i, "dst %U", api_unformat_sw_if_index, vam, &dst_sw_if_index))
            ;
...
static void
vl_api_sw_interface_span_details_t_handler(vl_api_sw_interface_span_details_t *mp)
{
    vat_main_t *vam      = &vat_main;
    ...
    /* *INDENT-ON* */
    print(vam->ofp, "%20s => %20s (%s) %s", sw_if_from_name, sw_if_to_name, states[mp->state],
          mp->is_l2 ? "l2" : "device");
}

static void
vl_api_sw_interface_span_details_t_handler_json(vl_api_sw_interface_span_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
    u8 *sw_if_from_name   = 0;
...
static void
vl_api_ipsec_gre_tunnel_details_t_handler(vl_api_ipsec_gre_tunnel_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp, "%11d%15U%15U%14d%14d", ntohl(mp->sw_if_index), format_ip4_address, &mp->src_address,
          format_ip4_address, &mp->dst_address, ntohl(mp->local_sa_id), ntohl(mp->remote_sa_id));
}

static void
vl_api_ipsec_gre_tunnel_details_t_handler_json(vl_api_ipsec_gre_tunnel_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
    struct in_addr ip4;
...
api_ipsec_gre_tunnel_dump(vat_main_t *vam)
{
    ...
    if (!vam->json_output) {
        print(vam->ofp, "%11s%15s%15s%14s%14s", "sw_if_index", "src_address", "dst_address", "local_sa_id",
              "remote_sa_id");
    }
...
vl_api_l2_xconnect_details_t_handler(vl_api_l2_xconnect_details_t *mp)
{
    ...
    print(vam->ofp, "%15d%15d", ntohl(mp->rx_sw_if_index), ntohl(mp->tx_sw_if_index));
}
...
vl_api_l2_xconnect_details_t_handler_json(vl_api_l2_xconnect_details_t *mp)
{
    ...
    vat_json_init_object(node);
    vat_json_object_add_uint(node, "rx_sw_if_index", ntohl(mp->rx_sw_if_index));
...
api_p2p_ethernet_add(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U", api_unformat_sw_if_index, vam, &parent_if_index))
            ...
            ;
        else if (unformat(i, "remote_mac %U", unformat_ethernet_address, remote_mac))
            mac_set++;
        else if (unformat(i, "sub_id %d", &sub_id))
            ;
        else {
...
api_tcp_configure_src_addresses(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U - %U", unformat_ip4_address, &v4first, unformat_ip4_address, &v4last)) {
            if (range_set) {
                errmsg("one range per message (range already set)");
                return -99;
            }
            range_set = 1;
        } else if (unformat(i, "%U - %U", unformat_ip6_address, &v6first, unformat_ip6_address, &v6last)) {
            if (range_set) {
                errmsg("one range per message (range already set)");
                return -99;
            }
...
static void
vl_api_app_namespace_add_del_reply_t_handler_json(vl_api_app_namespace_add_del_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    vat_json_node_t node;
...
vl_api_session_rules_details_t_handler(vl_api_session_rules_details_t *mp)
{
    ...
    if (mp->is_ip4) {
        print(vam->ofp, "appns %u tp %u scope %d %U/%d %d %U/%d %d action: %d tag: %s",
              clib_net_to_host_u32(mp->appns_index), mp->transport_proto, mp->scope, format_ip4_address, &mp->lcl_ip,
              mp->lcl_plen, clib_net_to_host_u16(mp->lcl_port), format_ip4_address, &mp->rmt_ip, mp->rmt_plen,
              clib_net_to_host_u16(mp->rmt_port), clib_net_to_host_u32(mp->action_index), mp->tag);
    } else {
        print(vam->ofp, "appns %u tp %u scope %d %U/%d %d %U/%d %d action: %d tag: %s",
              clib_net_to_host_u32(mp->appns_index), mp->transport_proto, mp->scope, format_ip6_address, &mp->lcl_ip,
              mp->lcl_plen, clib_net_to_host_u16(mp->lcl_port), format_ip6_address, &mp->rmt_ip, mp->rmt_plen,
              clib_net_to_host_u16(mp->rmt_port), clib_net_to_host_u32(mp->action_index), mp->tag);
    }
}

static void
vl_api_session_rules_details_t_handler_json(vl_api_session_rules_details_t *mp)
{
    vat_main_t *vam       = &vat_main;
    vat_json_node_t *node = NULL;
    struct in6_addr ip6;
    ...
    vat_json_object_add_uint(node, "appns_index", clib_net_to_host_u32(mp->appns_index));
    vat_json_object_add_uint(node, "transport_proto", mp->transport_proto);
    vat_json_object_add_uint(node, "scope", mp->scope);
    vat_json_object_add_uint(node, "action_index", clib_net_to_host_u32(mp->action_index));
    vat_json_object_add_uint(node, "lcl_port", clib_net_to_host_u16(mp->lcl_port));
    vat_json_object_add_uint(node, "rmt_port", clib_net_to_host_u16(mp->rmt_port));
...
api_session_rule_add_del(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "del"))
            ...
            ;
        else if (unformat(i, "%U/%d %d %U/%d %d", unformat_ip4_address, &lcl_ip4, &lcl_plen, &lcl_port,
                          unformat_ip4_address, &rmt_ip4, &rmt_plen, &rmt_port)) {
            is_ip4   = 1;
            conn_set = 1;
        } else if (unformat(i, "%U/%d %d %U/%d %d", unformat_ip6_address, &lcl_ip6, &lcl_plen, &lcl_port,
                            unformat_ip6_address, &rmt_ip6, &rmt_plen, &rmt_port)) {
            is_ip4   = 0;
            conn_set = 1;
        } else if (unformat(i, "action %d", &action))
            ;
...
#define foreach_vpe_api_msg                                                                                            \
    ...
    _(sw_interface_dump, "")                                                                                           \
    _(sw_interface_set_flags, "<intfc> | sw_if_index <id> admin-up | admin-down link-up | link down")                  \
    _(sw_interface_add_del_address, "<intfc> | sw_if_index <id> <ip4-address> | <ip6-address> [del] [del-all] ")       \
    _(sw_interface_set_rx_mode, "<intfc> | sw_if_index <id> [queue <id>] <polling | interrupt | adaptive>")            \
    _(sw_interface_set_rx_placement, "<intfc> | sw_if_index <id> [queue <id>] [worker <id> | main]")                   \
    _(sw_interface_rx_placement_dump, "[<intfc> | sw_if_index <id>]")                                                  \
    _(sw_interface_set_table, "<intfc> | sw_if_index <id> vrf <table-id> [ipv6]")                                      \
    _(sw_interface_set_mpls_enable, "<intfc> | sw_if_index [disable | dis]")                                           \
    _(sw_interface_set_vpath, "<intfc> | sw_if_index <id> enable | disable")                                           \
    _(sw_interface_set_vxlan_bypass, "<intfc> | sw_if_index <id> [ip4 | ip6] [enable | disable]")                      \
    _(sw_interface_set_geneve_bypass, "<intfc> | sw_if_index <id> [ip4 | ip6] [enable | disable]")                     \
    _(sw_interface_set_l2_xconnect, "rx <intfc> | rx_sw_if_index <id> tx <intfc> | tx_sw_if_index <id>\n"              \
                                    "enable | disable")                                                                \
    _(sw_interface_set_l2_bridge, "{<intfc> | sw_if_index <id>} bd_id <bridge-domain-id>\n"                            \
                                  "[shg <split-horizon-group>] [bvi]\n"                                                \
                                  "enable | disable")                                                                  \
    _(bridge_domain_set_mac_age, "bd_id <bridge-domain-id> mac-age 0-255")                                             \
    _(bridge_domain_add_del, "bd_id <bridge-domain-id> [flood 1|0] [uu-flood 1|0] [forward 1|0] [learn 1|0] "          \
                             "[arp-term 1|0] [mac-age 0-255] [bd-tag <text>] [del]\n")                                 \
    _(bridge_domain_dump, "[bd_id <bridge-domain-id>]\n")                                                              \
    _(l2fib_add_del, "mac <mac-addr> bd_id <bridge-domain-id> [del] | sw_if <intfc> | sw_if_index <id> [static] "      \
                     "[filter] [bvi] [count <nn>]\n")                                                                  \
    _(l2fib_flush_bd, "bd_id <bridge-domain-id>")                                                                      \
    _(l2fib_flush_int, "<intfc> | sw_if_index <id>")                                                                   \
    _(l2_flags, "sw_if <intfc> | sw_if_index <id> [learn] [forward] [uu-flood] [flood] [arp-term] [disable]\n")        \
    _(bridge_flags, "bd_id <bridge-domain-id> [learn] [forward] [uu-flood] [flood] [arp-term] [disable]\n")            \
    _(tap_connect, "tapname <name> mac <mac-addr> | random-mac [tag <string>]")                                        \
    _(tap_modify, "<vpp-if-name> | sw_if_index <id> tapname <name> mac <mac-addr> | random-mac")                       \
    _(tap_delete, "<vpp-if-name> | sw_if_index <id>")                                                                  \
    _(sw_interface_tap_dump, "")                                                                                       \
    _(tap_create_v2, "id <num> [hw-addr <mac-addr>] [host-ns <name>] [rx-ring-size <num> [tx-ring-size <num>]")        \
    _(tap_delete_v2, "<vpp-if-name> | sw_if_index <id>")                                                               \
    _(sw_interface_tap_v2_dump, "")                                                                                    \
    _(bond_create, "[hw-addr <mac-addr>] {round-robin | active-backup | "                                              \
                   "broadcast | {lacp | xor} [load-balance { l2 | l23 | l34 }]}")                                      \
    _(bond_delete, "<vpp-if-name> | sw_if_index <id>")                                                                 \
    _(bond_enslave, "sw_if_index <n> bond <sw_if_index> [is_passive] [is_long_timeout]")                               \
    _(bond_detach_slave, "sw_if_index <n>")                                                                            \
    _(sw_interface_bond_dump, "")                                                                                      \
    _(sw_interface_slave_dump, "<vpp-if-name> | sw_if_index <id>")                                                     \
    _(ip_table_add_del, "table <n> [ipv6] [add | del]\n")                                                              \
    _(ip_add_del_route, "<addr>/<mask> via <<addr>|<intfc>|sw_if_index <id>|via-label <n>>\n"                          \
    ...
    _(mpls_route_add_del, "<label> <eos> via <addr | next-hop-table <n> | via-label <n> |\n"                           \
    ...
    _(mpls_tunnel_add_del, "[add | del <intfc | sw_if_index <id>>] via <addr | via-label <n>>\n"                       \
                           "[<intfc> | sw_if_index <id> | next-hop-table <id>]\n"                                      \
                           "[l2-only]  [out-label <n>]")                                                               \
    _(sr_mpls_policy_add, "bsid <id> [weight <n>] [spray] next <sid> [next <sid>]")                                    \
    _(sr_mpls_policy_del, "bsid <id>")                                                                                 \
    _(bier_table_add_del, "<label> <sub-domain> <set> <bsl> [del]")                                                    \
    _(bier_route_add_del, "<bit-position> <sub-domain> <set> <bsl> via <addr> [table-id <n>]\n"                        \
                          "[<intfc> | sw_if_index <id>]"                                                               \
                          "[weight <n>] [del] [multipath]")                                                            \
    _(proxy_arp_add_del, "<lo-ip4-addr> - <hi-ip4-addr> [vrf <n>] [del]")                                              \
    _(proxy_arp_intfc_enable_disable, "<intfc> | sw_if_index <id> enable | disable")                                   \
    _(sw_interface_set_unnumbered, "<intfc> | sw_if_index <id> unnum_if_index <id> [del]")                             \
    _(ip_neighbor_add_del, "(<intfc> | sw_if_index <id>) dst <ip46-address> "                                          \
    ...
    _(dhcp_proxy_config, "svr <v46-address> src <v46-address>\n"                                                       \
                         "rx_vrf_id <nn> server_vrf_id <nn>  [del]")                                                   \
    _(dhcp_proxy_set_vss, "tbl_id <n> [fib_id <n> oui <n> | vpn_ascii_id <text>] [ipv6] [del]")                        \
    _(dhcp_proxy_dump, "ip6")                                                                                          \
    _(dhcp_client_config, "<intfc> | sw_if_index <id> [hostname <name>] [disable_event] [del]")                        \
    _(set_ip_flow_hash, "vrf <n> [src] [dst] [sport] [dport] [proto] [reverse] [ipv6]")                                \
    _(sw_interface_ip6_enable_disable, "<intfc> | sw_if_index <id> enable | disable")                                  \
    _(sw_interface_ip6_set_link_local_address, "<intfc> | sw_if_index <id> <ip6-address>/<mask-width>")                \
    _(ip6nd_proxy_add_del, "<intfc> | sw_if_index <id> <ip6-address>")                                                 \
    _(ip6nd_proxy_dump, "")                                                                                            \
    _(sw_interface_ip6nd_ra_prefix, "<intfc> | sw_if_index <id> <ip6-address>/<mask-width>\n"                          \
                                    "val_life <n> pref_life <n> [def] [noadv] [offl] [noauto]\n"                       \
                                    "[nolink] [isno]")                                                                 \
    _(sw_interface_ip6nd_ra_config, "<intfc> | sw_if_index <id> [maxint <n>] [minint <n>]\n"                           \
                                    "[life <n>] [count <n>] [interval <n>] [suppress]\n"                               \
                                    "[managed] [other] [ll] [send] [cease] [isno] [def]")                              \
    _(set_arp_neighbor_limit, "arp_nbr_limit <n> [ipv6]")                                                              \
    _(l2_patch_add_del, "rx <intfc> | rx_sw_if_index <id> tx <intfc> | tx_sw_if_index <id>\n"                          \
                        "enable | disable")                                                                            \
    _(sr_localsid_add_del, "(del) address <addr> next_hop <addr> behavior <beh>\n"                                     \
                           "fib-table <num> (end.psp) sw_if_index <num>")                                              \
    _(classify_add_del_table, "buckets <nn> [skip <n>] [match <n>] [memory_size <nn-bytes>]\n"                         \
    ...
    _(classify_add_del_session, "[hit-next|l2-hit-next|acl-hit-next|policer-hit-next] <name|nn>\n"                     \
    ...
    _(classify_set_interface_ip_table, "<intfc> | sw_if_index <nn> table <nn>")                                        \
    _(classify_set_interface_l2_tables, "<intfc> | sw_if_index <nn> [ip4-table <nn>] [ip6-table <nn>]\n"               \
                                        "  [other-table <nn>]")                                                        \
    _(get_node_index, "node <node-name")                                                                               \
    _(add_node_next, "node <node-name> next <next-node-name>")                                                         \
    _(l2tpv3_create_tunnel, "client_address <ip6-addr> our_address <ip6-addr>\n"                                       \
    ...
    _(l2tpv3_interface_enable_disable, "<intfc> | sw_if_index <nn> enable | disable")                                  \
    _(l2tpv3_set_lookup_key, "lookup_v6_src | lookup_v6_dst | lookup_session_id")                                      \
    _(sw_if_l2tpv3_tunnel_dump, "")                                                                                    \
    _(vxlan_offload_rx, "hw { <interface name> | hw_if_index <nn>} "                                                   \
                        "rx { <vxlan tunnel name> | sw_if_index <nn> } [del]")                                         \
    _(vxlan_add_del_tunnel, "src <ip-addr> { dst <ip-addr> | group <mcast-ip-addr>\n"                                  \
                            "{ <intfc> | mcast_sw_if_index <nn> } [instance <id>]}\n"                                  \
                            "vni <vni> [encap-vrf-id <nn>] [decap-next <l2|nn>] [del]")                                \
    _(geneve_add_del_tunnel, "src <ip-addr> { dst <ip-addr> | group <mcast-ip-addr>\n"                                 \
                             "{ <intfc> | mcast_sw_if_index <nn> } }\n"                                                \
                             "vni <vni> [encap-vrf-id <nn>] [decap-next <l2|nn>] [del]")                               \
    _(vxlan_tunnel_dump, "[<intfc> | sw_if_index <nn>]")                                                               \
    _(geneve_tunnel_dump, "[<intfc> | sw_if_index <nn>]")                                                              \
    _(gre_add_del_tunnel, "src <ip-addr> dst <ip-addr> [outer-fib-id <nn>] [instance <n>]\n"                           \
    ...
    _(l2_interface_vlan_tag_rewrite, "<intfc> | sw_if_index <nn> \n"                                                   \
                                     "[disable][push-[1|2]][pop-[1|2]][translate-1-[1|2]] \n"                          \
                                     "[translate-2-[1|2]] [push_dot1q 0] tag1 <nn> tag2 <nn>")                         \
    _(create_vhost_user_if, "socket <filename> [server] [renumber <dev_instance>] "                                    \
                            "[disable_mrg_rxbuf] [disable_indirect_desc] "                                             \
                            "[mac <mac_address>]")                                                                     \
    _(modify_vhost_user_if, "<intfc> | sw_if_index <nn> socket <filename>\n"                                           \
                            "[server] [renumber <dev_instance>]")                                                      \
    _(delete_vhost_user_if, "<intfc> | sw_if_index <nn>")                                                              \
    _(sw_interface_vhost_user_dump, "")                                                                                \
    _(show_version, "")                                                                                                \
    _(show_threads, "")                                                                                                \
    _(vxlan_gpe_add_del_tunnel, "local <addr> remote <addr>  | group <mcast-ip-addr>\n"                                \
                                "{ <intfc> | mcast_sw_if_index <nn> } }\n"                                             \
    ...
    _(interface_name_renumber, "<intfc> | sw_if_index <nn> new_show_dev_instance <nn>")                                \
    _(input_acl_set_interface, "<intfc> | sw_if_index <nn> [ip4-table <nn>] [ip6-table <nn>]\n"                        \
                               "  [l2-table <nn>] [del]")                                                              \
    ...
    _(policer_classify_set_interface, "<intfc> | sw_if_index <nn> [ip4-table <nn>] [ip6-table <nn>]\n"                 \
                                      "  [l2-table <nn>] [del]")                                                       \
    ...
    _(sw_interface_span_enable_disable,                                                                                \
      "[l2] [src <intfc> | src_sw_if_index <id>] [disable | [[dst <intfc> | dst_sw_if_index <id>] [both|rx|tx]]]")     \
    ...
    _(ip_source_and_port_range_check_add_del, "<ip-addr>/<mask> range <nn>-<nn> vrf <id>")                             \
    _(ip_source_and_port_range_check_interface_add_del,                                                                \
      "<intf> | sw_if_index <nn> [tcp-out-vrf <id>] [tcp-in-vrf <id>]"                                                 \
      "[udp-in-vrf <id>] [udp-out-vrf <id>]")                                                                          \
    _(ipsec_gre_add_del_tunnel, "src <addr> dst <addr> local_sa <sa-id> remote_sa <sa-id> [del]")                      \
    _(ipsec_gre_tunnel_dump, "[sw_if_index <nn>]")                                                                     \
    _(delete_subif, "<intfc> | sw_if_index <nn>")                                                                      \
    _(l2_interface_pbb_tag_rewrite, "<intfc> | sw_if_index <nn> \n"                                                    \
    ...
    _(flow_classify_set_interface, "<intfc> | sw_if_index <nn> [ip4-table <nn>] [ip6-table <nn>] [del]")               \
    ...
    _(output_acl_set_interface, "<intfc> | sw_if_index <nn> [ip4-table <nn>] [ip6-table <nn>]\n"                       \
                                "  [l2-table <nn>] [del]")                                                             \
...
{
#define _(N, n)                                                                                                        \
    vl_msg_api_set_handlers(VL_API_##N, #n, vl_api_##n##_t_handler_uni, vl_noop_handler, vl_api_##n##_t_endian,        \
