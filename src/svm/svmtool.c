...
repair(char *chroot_path, int crash_root_region)
{
    ...
    root_rp = (void *) mmap(uword_to_pointer(a->baseva, void *), a->size, PROT_READ | PROT_WRITE,
    ...
    if (pthread_mutex_trylock(&root_rp->mutex)) {
        clib_warning("root_rp->mutex LOCKED by pid %d, tag %d, cleared...", root_rp->mutex_owner_pid,
                     root_rp->mutex_owner_tag);
