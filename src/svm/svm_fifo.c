...
{
    return (ooo_segment_distance_from_tail(f, a) < ooo_segment_distance_from_tail(f, b));
}
...
{
    return ooo_segment_distance_from_tail(f, posa) - ooo_segment_distance_from_tail(f, posb);
}
...
format_ooo_segment(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%u, %u], len %u, next %d, prev %d", normalized_start, (normalized_start + seg->length) % f->nitems,
               seg->length, seg->next, seg->prev);
    return s;
}
...
svm_fifo_replay(u8 *s, svm_fifo_t *f, u8 no_read, u8 verbose)
{
    ...
    for (i = 0; i < trace_len; i++) {
        ...
        if (trace[i].action == 1) {
            if (verbose)
                s = format(s, "adding [%u, %u]:", trace[i].offset, (trace[i].offset + trace[i].len) % dummy_fifo->nitems);
            svm_fifo_enqueue_with_offset(dummy_fifo, trace[i].offset, trace[i].len, &data[offset]);
...
format_svm_fifo(u8 *s, va_list *args)
{
    ...
    s = format(s, " head %d tail %d segment manager %u\n", f->head, f->tail, f->segment_manager);

    if (verbose > 1)
        s = format(s, " vpp session %d thread %d app session %d thread %d\n", f->master_session_index,
                   f->master_thread_index, f->client_session_index, f->client_thread_index);
...
svm_fifo_create(u32 data_size_in_bytes)
{
    ...
    f                 = clib_mem_alloc_aligned_or_null(sizeof(*f) + rounded_data_size, CLIB_CACHE_LINE_BYTES);
    if (f == 0)
        return 0;
...
ooo_segment_add(svm_fifo_t *f, u32 offset, u32 length)
{
    ...
    while (s->next != OOO_SEGMENT_INVALID_INDEX && position_lt(f, s->start, normalized_position))
...
check_tail:
    ...
    if (position_gt(f, normalized_end_position, s_end_pos)) {
        ...
        while (it && position_leq(f, ooo_segment_end_pos(f, it), normalized_end_position)) {
...
CLIB_MARCH_FN(svm_fifo_enqueue_nowait, int, svm_fifo_t *f, u32 max_bytes, const u8 *copy_from_here)
{
    ...
    total_copy_bytes = (nitems - cursize) < max_bytes ? (nitems - cursize) : max_bytes;

    if (PREDICT_TRUE(copy_from_here != 0)) {
        /* Number of bytes in first copy segment */
        first_copy_bytes = ((nitems - f->tail) < total_copy_bytes) ? (nitems - f->tail) : total_copy_bytes;
        ...
        if (second_copy_bytes) {
            clib_memcpy(&f->data[f->tail], copy_from_here + first_copy_bytes, second_copy_bytes);
...
{
    return CLIB_MARCH_FN_SELECT(svm_fifo_enqueue_nowait)(f, max_bytes, copy_from_here);
}
#endif
...
CLIB_MARCH_FN(svm_fifo_enqueue_with_offset, int, svm_fifo_t *f, u32 offset, u32 required_bytes, u8 *copy_from_here)
{
    ...
    /* Number of bytes in first copy segment */
    first_copy_bytes =
        ((nitems - normalized_offset) < total_copy_bytes) ? (nitems - normalized_offset) : total_copy_bytes;
    ...
    if (second_copy_bytes) {
        ...
        clib_memcpy(&f->data[normalized_offset], copy_from_here + first_copy_bytes, second_copy_bytes);
    }
...
int
svm_fifo_enqueue_with_offset(svm_fifo_t *f, u32 offset, u32 required_bytes, u8 *copy_from_here)
{
    return CLIB_MARCH_FN_SELECT(svm_fifo_enqueue_with_offset)(f, offset, required_bytes, copy_from_here);
}
...
CLIB_MARCH_FN(svm_fifo_dequeue_nowait, int, svm_fifo_t *f, u32 max_bytes, u8 *copy_here)
{
    ...
    if (PREDICT_TRUE(copy_here != 0)) {
        /* Number of bytes in first copy segment */
        first_copy_bytes = ((nitems - f->head) < total_copy_bytes) ? (nitems - f->head) : total_copy_bytes;
        ...
        second_copy_bytes = total_copy_bytes - first_copy_bytes;
        if (second_copy_bytes) {
            clib_memcpy(copy_here + first_copy_bytes, &f->data[f->head], second_copy_bytes);
...
{
    return CLIB_MARCH_FN_SELECT(svm_fifo_dequeue_nowait)(f, max_bytes, copy_here);
}
#endif
...
CLIB_MARCH_FN(svm_fifo_peek, int, svm_fifo_t *f, u32 relative_offset, u32 max_bytes, u8 *copy_here)
{
    ...
    /* Number of bytes we're going to copy */
    total_copy_bytes = (cursize - relative_offset < max_bytes) ? cursize - relative_offset : max_bytes;

    if (PREDICT_TRUE(copy_here != 0)) {
        /* Number of bytes in first copy segment */
        first_copy_bytes = ((nitems - real_head) < total_copy_bytes) ? (nitems - real_head) : total_copy_bytes;
        clib_memcpy(copy_here, &f->data[real_head], first_copy_bytes);

        /* Number of bytes in second copy segment, if any */
        second_copy_bytes = total_copy_bytes - first_copy_bytes;
        if (second_copy_bytes) {
            clib_memcpy(copy_here + first_copy_bytes, &f->data[0], second_copy_bytes);
        }
    }
    return total_copy_bytes;
}
...
int
svm_fifo_peek(svm_fifo_t *f, u32 relative_offset, u32 max_bytes, u8 *copy_here)
{
    return CLIB_MARCH_FN_SELECT(svm_fifo_peek)(f, relative_offset, max_bytes, copy_here);
}
...
svm_fifo_dequeue_drop(svm_fifo_t *f, u32 max_bytes)
{
    ...
    /* Number of bytes in first copy segment */
    first_drop_bytes = ((nitems - f->head) < total_drop_bytes) ? (nitems - f->head) : total_drop_bytes;
