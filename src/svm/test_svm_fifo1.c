...
offset(int verbose)
{
    ...
    svm_fifo_enqueue_nowait(f, 4096 /* bytes to enqueue */, (u8 *) test_data);
    ...
    svm_fifo_enqueue_nowait(f, 4096 /* bytes to enqueue */, (u8 *) &test_data[1024]);
    ...
    for (i = 0; i < (3 * 1024); i++) {
        if (recovered_data[i] != test_data[i]) {
            clib_warning("[%d] expected %d recovered %d", i, test_data[i], recovered_data[i]);
...
slave(int verbose)
{
    ...
    for (i = 0; i < 1000; i++) {
        ...
        if (memcmp(retrieved_data, test_data, vec_len(retrieved_data)))
            return clib_error_return(0, "retrieved data incorrect, '%s'", retrieved_data);
    }
