...
static init_fn master_init_fns[SSVM_N_SEGMENT_TYPES] = {ssvm_master_init_shm, ssvm_master_init_memfd,
                                                        ssvm_master_init_private};
static init_fn slave_init_fns[SSVM_N_SEGMENT_TYPES]  = {ssvm_slave_init_shm, ssvm_slave_init_memfd,
...
#if USE_DLMALLOC == 0
    sh->heap = mheap_alloc_with_flags(((u8 *) sh) + page_size, ssvm->ssvm_size - page_size, mh_flags);
#else
    sh->heap = create_mspace_with_base(((u8 *) sh) + page_size, ssvm->ssvm_size - page_size, 1 /* locked */);
...
re_map_it:
    ...
    sh = ssvm->sh = (void *) mmap((void *) ssvm->requested_va, ssvm->ssvm_size, PROT_READ | PROT_WRITE,
...
ssvm_delete_shm(ssvm_private_t *ssvm)
{
    ...
    if (CLIB_DEBUG > 1)
        clib_warning("[%d] unlinking ssvm (%s) backing file '%s'", getpid(), ssvm->name, fn);
...
ssvm_master_init_memfd(ssvm_private_t *memfd)
{
    ...
    sh->heap = mheap_alloc_with_flags(((u8 *) sh) + page_size, memfd->ssvm_size - page_size, flags);
#else
    sh->heap = create_mspace_with_base(((u8 *) sh) + page_size, memfd->ssvm_size - page_size, 1 /* locked */);
    mspace_disable_expand(sh->heap);
#endif
    oldheap  = ssvm_push_heap(sh);
