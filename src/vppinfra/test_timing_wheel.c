...
test_timing_wheel_main(unformat_input_t *input)
{
    ...
    clib_warning("iter %wd, events %d, seed %u, %U", tm->n_iter, tm->n_events, tm->seed, format_timing_wheel,
                 &tm->timing_wheel, /* verbose */ 0);
...

    {
        ...
        for (iter = 0; iter < tm->n_iter || n_events_in_wheel > 0; iter++) {
            ...
            if (tm->time_per_status_update != 0 && clib_time_now(&tm->time) >= tm->time_next_status_update) {
                f64 ave = 0, rms = 0;
                ...
                clib_warning("%12wd iter done %10wd expired; ave. error %.4e +- %.4e, range %.4e %.4e", iter, n_expired,
                             ave, rms, min_error, max_error);
            }

            if (tm->total_iterate_time != 0 &&
                (clib_time_now(&tm->time) - tm->time_iterate_start >= tm->total_iterate_time))
                tm->n_iter = iter;
        ...
        clib_warning("%wd iter done %wd expired; ave. error %.4e +- %.4e, range %.4e %.4e", 1 + iter, n_expired,
                     ave_error, rms_error, min_error, max_error);

        {
            ...
            for (i = 0; i < vec_len(error_hist); i++) {
                if (error_hist[i] == 0)
                    continue;
                vec_add2(fs, f, 1);
                f->dt = (((i64) zvec_unsigned_to_signed(i) << w->log2_clocks_per_bin) * tm->time.seconds_per_clock);
            ...
            vec_foreach(f, fs)
            {
                ...
                fformat(stdout, "%12.4e %16.4f%% %16.4f%% %Ld\n", f->dt, f->fraction * 100, total_fraction * 100,
                        f->count);
            }
        }
