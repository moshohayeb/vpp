 ...
 *
 */
#define flowhash_foreach_entry(h, ei) for (ei = 1; ei < (h)->total_entries; ei++)
 ...
 */
#define flowhash_foreach_valid_entry(h, ei, now) flowhash_foreach_entry(h, ei) if (((now) <= (h)->entries[ei].timeout))
...
#define flowhash_prefetch(h, hash)                                                                                     \
    CLIB_PREFETCH(&(h)->entries[((hash) & (h)->fixed_entries_mask) + 1], sizeof((h)->entries[0]), LOAD)
 ...
 *
 */
static_always_inline FVT(flowhash) * FV(flowhash_alloc)(u32 fixed_entries, u32 collision_buckets)
{
    FVT(flowhash) * h;
    uword size;
    void *mem;
    ...
    entries = 1 + fixed_entries + collision_buckets * FLOWHASH_ENTRIES_PER_BUCKETS;
    size    = sizeof(*h) + sizeof(h->entries[0]) * entries + sizeof(h->free_buckets_indices[0]) * collision_buckets;
...
static void FV(__flowhash_get_chained)(FVT(flowhash) * h, FVT(flowhash_lkey) * k, u32 hash, u32 time_now, u32 *ei);
...
static_always_inline void FV(__flowhash_get_chained)(FVT(flowhash) * h, FVT(flowhash_lkey) * k, u32 hash, u32 time_now,
                                                     u32 *ei)
{
    ...
    if (h->entries[*ei].chained_entry_index == FLOWHASH_INVALID_ENTRY_INDEX) {
        ...
        if (h->free_buckets_position == 0) {
            ...
            *ei                                              = FLOWHASH_INVALID_ENTRY_INDEX;
            ...
            FV(flowhash_cpy_key)(&h->entries[FLOWHASH_INVALID_ENTRY_INDEX].key, k);
            return;
        }

        /* Forward link */
        h->entries[*ei].chained_entry_index = h->free_buckets_indices[h->free_buckets_position];

        /* Backward link (for garbage collection) */
        h->entries[h->free_buckets_indices[h->free_buckets_position]].chained_entry_index = *ei;
    ...
    u32 bi0 = h->entries[*ei].chained_entry_index + (hash >> (32 - FLOWHASH_ENTRIES_PER_BUCKETS_LOG));
    u32 bi1 = bi0 + 1;
    bi1     = (bi0 & (FLOWHASH_ENTRIES_PER_BUCKETS - 1)) ? bi1 : bi1 - FLOWHASH_ENTRIES_PER_BUCKETS;
...
static_always_inline void FV(flowhash_gc)(FVT(flowhash) * h, u32 time_now, u32 *freed_index, u32 *freed_len)
{
    ...
    ei = 2 + h->fixed_entries_mask + ((h->gc_counter + 2) & h->collision_buckets_mask) * FLOWHASH_ENTRIES_PER_BUCKETS;
    CLIB_PREFETCH(&h->entries[ei], sizeof(h->entries[0]), READ);

    /* prefetch one round in advance */
    ei = 2 + h->fixed_entries_mask + ((h->gc_counter + 1) & h->collision_buckets_mask) * FLOWHASH_ENTRIES_PER_BUCKETS;
    if (h->entries[ei].chained_entry_index != FLOWHASH_INVALID_ENTRY_INDEX) {
        CLIB_PREFETCH(&h->entries[ei], 4 * CLIB_CACHE_LINE_BYTES, READ);
    }

    /* do GC */
    ei = 2 + h->fixed_entries_mask + ((h->gc_counter) & h->collision_buckets_mask) * FLOWHASH_ENTRIES_PER_BUCKETS;
    if (h->entries[ei].chained_entry_index != FLOWHASH_INVALID_ENTRY_INDEX) {
        u8 found = 0;
        int i;
        ...
        if (!found) {
            ...
            h->free_buckets_position--;
            /* Reset forward link */
            h->entries[h->entries[ei].chained_entry_index].chained_entry_index = FLOWHASH_INVALID_ENTRY_INDEX;
            /* Reset back link */
            h->entries[ei].chained_entry_index = FLOWHASH_INVALID_ENTRY_INDEX;
            /* Free element */
