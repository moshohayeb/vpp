...
test_qhash_main(unformat_input_t *input)
{
    ...
    clib_warning("iter %d, seed %u, keys %d, max vector %d, ", tm->n_iter, tm->seed, tm->n_keys, tm->max_vector);
    ...
    for (iter = 0; iter < tm->n_iter; iter++) {
        ...
        while (j < n) {
            ...
            if (clib_bitmap_get(tm->keys_in_hash_bitmap, i) != is_set) {
                ...
                tm->keys_in_hash_bitmap = clib_bitmap_set(tm->keys_in_hash_bitmap, i, is_set);
                j++;
            }
        }

        {
            ...
            if (is_set) {
                t[0] = clib_time_now(&tm->time);
                qhash_set_multiple(tm->qhash, tm->lookup_keys, vec_len(tm->lookup_keys), tm->lookup_results);
                t[1] = clib_time_now(&tm->time);
                tm->set_time += t[1] - t[0];
                ...
                t[0] = clib_time_now(&tm->time);
                qhash_unset_multiple(tm->qhash, tm->lookup_keys, vec_len(tm->lookup_keys), tm->lookup_results);
                t[1] = clib_time_now(&tm->time);
...

        {
            ...
            t[0] = clib_time_now(&tm->time);
            qhash_get_multiple(tm->qhash, tm->keys, vec_len(tm->keys), tm->get_multiple_results);
            t[1] = clib_time_now(&tm->time);
            tm->get_time += t[1] - t[0];
        ...
        tm->overflow_fraction += ((f64) qhash_n_overflow(tm->qhash) / qhash_elts(tm->qhash));
        tm->ave_elts += qhash_elts(tm->qhash);
    }

    fformat(stderr, "%d iter %.6e overflow, %.4f ave. elts\n", tm->n_iter, tm->overflow_fraction / tm->n_iter,
    ...
    fformat(stderr, "get/set/unset clocks %.2e %.2e %.2e clib %.2e %.2e %.2e ratio %.2f %.2f %.2f\n",
            tm->get_time * tm->time.clocks_per_second, tm->set_time * tm->time.clocks_per_second,
            tm->unset_time * tm->time.clocks_per_second, tm->hash_get_time * tm->time.clocks_per_second,
            tm->hash_set_time * tm->time.clocks_per_second, tm->hash_unset_time * tm->time.clocks_per_second,
            tm->hash_get_time / tm->get_time, tm->hash_set_time / tm->set_time, tm->hash_unset_time / tm->unset_time);
