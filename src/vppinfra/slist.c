...
clib_error_t *
clib_slist_init(clib_slist_t *sp, f64 branching_factor, clib_slist_key_compare_function_t compare,
...
slist_search_internal(clib_slist_t *sp, void *key, int need_full_path)
{
    ...
    while (1) {
         ...
         */
        prefetch_elt = sp->elts + clib_slist_get_next_at_level(search_elt, level);

        CLIB_PREFETCH(prefetch_elt, CLIB_CACHE_LINE_BYTES, READ);

        /* Compare the key with the current element */
        comp_result = (search_elt == head_elt) ? 1 : sp->compare(key, search_elt->user_pool_index);
...
clib_slist_add(clib_slist_t *sp, void *key, u32 user_pool_index)
{
    ...
    search_result = slist_search_internal(sp, key, 0 /* don't need full path */);
    ...
    for (level = 0; level < vec_len(sp->path); level++) {
        ...
        prev_elt_next_index_this_level = clib_slist_get_next_at_level(prev_elt_this_level, level);

        clib_slist_set_next_at_level(new_elt, prev_elt_next_index_this_level, level);

        clib_slist_set_next_at_level(prev_elt_this_level, new_elt - sp->elts, level);
        sp->occupancy[level]++;
