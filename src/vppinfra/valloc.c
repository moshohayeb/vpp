    ...
    if (CLIB_DEBUG > 0 && index != ~0) {
        while (index != ~0) {
            ...
            ASSERT(template->baseva + template->size < ch->baseva ||
                   template->baseva + template->size >= (ch->baseva + ch->size));
            index = ch->next;
        }
        index = vam->first_index;
    }
...
*/
void
clib_valloc_init(clib_valloc_main_t *vam, clib_valloc_chunk_t *template, int need_lock)
{
...
*/
uword
clib_valloc_alloc(clib_valloc_main_t *vam, uword size, int os_out_of_memory_on_failure)
{
...
format_valloc(u8 *s, va_list *va)
{
    ...
    if (verbose) {
        ...
        while (index != ~0) {
            ...
            s = format(s, "[%d] base %llx size %llx (%lld) prev %d %s\n", index, ch->baseva, ch->size, ch->size,
                       ch->prev, (ch->flags & CLIB_VALLOC_BUSY) ? "busy" : "free");
