...
void
clib_timebase_init(clib_timebase_t *tb, i32 timezone_offset_in_hours, clib_timebase_daylight_time_t daylight_type)
{
...
format_clib_timebase_time(u8 *s, va_list *args)
{
    ...
    s = format(s, "%s, %u %s %u %u:%02u:%02u", day_names_epoch_order[cp->day_name_index], cp->day,
               month_short_names[cp->month], cp->year, cp->hour, cp->minute, cp->second);
    return (s);
}
...
unformat_clib_timebase_range_hms(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%u:%u:%u - %u:%u:%u", &start_hour, &start_minute, &start_second, &end_hour, &end_minute,
                 &end_second))
        ;
