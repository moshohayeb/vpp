...
void BV(clib_bihash_master_init_svm)(BVT(clib_bihash) * h, char *name, u32 nbuckets, u64 memory_size)
{
    uword bucket_size;
    u8 *mmap_addr;
    ...
    h->sh->alloc_lock_as_u64 = (u64) BV(clib_bihash_get_offset)(h, (void *) h->alloc_lock);
    freelist_vh              = BV(alloc_aligned)(h, sizeof(vec_header_t) + BIHASH_FREELIST_LENGTH * sizeof(u64));
    freelist_vh->len         = BIHASH_FREELIST_LENGTH;
    freelist_vh->dlmalloc_header_offset = 0xDEADBEEF;
    h->sh->freelists_as_u64             = (u64) BV(clib_bihash_get_offset)(h, freelist_vh->vector_data);
    h->freelists                        = (void *) (freelist_vh->vector_data);

    h->fmt_fn = NULL;
}

void BV(clib_bihash_slave_init_svm)(BVT(clib_bihash) * h, char *name, int fd)
{
    u8 *mmap_addr;
    u64 memory_size;
...
void BV(clib_bihash_set_kvp_format_fn)(BVT(clib_bihash) * h, format_function_t *fmt_fn)
{
    h->fmt_fn = fmt_fn;
}
...
static BVT(clib_bihash_value) * BV(split_and_rehash)(BVT(clib_bihash) * h, BVT(clib_bihash_value) * old_values,
    ...
    for (i = 0; i < length_in_kvs; i++) {
        ...
        for (j = 0; j < BIHASH_KVP_PER_PAGE; j++) {
            ...
            if (BV(clib_bihash_is_free)(&(new_v->kvp[j]))) {
                clib_memcpy(&(new_v->kvp[j]), &(old_values->kvp[i]), sizeof(new_v->kvp[j]));
                goto doublebreak;
            }
        }
...
static BVT(clib_bihash_value) * BV(split_and_rehash_linear)(BVT(clib_bihash) * h, BVT(clib_bihash_value) * old_values,
                                                            u32 old_log2_pages, u32 new_log2_pages)
{
    ...
    for (i = 0; i < old_length; i++) {
        ...
        for (; j < new_length; j++) {
            ...
            if (BV(clib_bihash_is_free)(&(new_values->kvp[j]))) {
                ...
                clib_memcpy(&(new_values->kvp[j]), &(old_values->kvp[i]), sizeof(new_values->kvp[j]));
                j++;
                goto doublebreak;
            }
        }
...
static inline int BV(clib_bihash_add_del_inline)(BVT(clib_bihash) * h, BVT(clib_bihash_kv) * add_v, int is_add,
    ...
    if (is_add) {
        ...
        for (i = 0; i < limit; i++) {
            if (BV(clib_bihash_is_free)(&(v->kvp[i]))) {
                 ...
                 */
                clib_memcpy(&(v->kvp[i].value), &add_v->value, sizeof(add_v->value));
...
int BV(clib_bihash_add_or_overwrite_stale)(BVT(clib_bihash) * h, BVT(clib_bihash_kv) * add_v,
...
u8 *BV(format_bihash)(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < h->nbuckets; i++) {
        ...
        for (j = 0; j < (1 << b->log2_pages); j++) {
            for (k = 0; k < BIHASH_KVP_PER_PAGE; k++) {
                ...
                if (verbose) {
                    if (h->fmt_fn) {
                        s = format(s, "    %d: %U\n", j * BIHASH_KVP_PER_PAGE + k, h->fmt_fn, &(v->kvp[k]));
                    } else {
    ...
    s          = format(s,
               ...
               alloc_arena(h), alloc_arena_next(h), used_bytes, used_bytes >> 20, alloc_arena_size(h),
