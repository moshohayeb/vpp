...
format_elf_section(u8 *s, va_list *args)
{
    ...
    if (!h)
        return format(s, "%=40s%=10s%=20s%=8s%=16s%=16s%=16s", "Name", "Index", "Type", "Size", "Align", "Address",
                      "File offset");

    s = format(s, "%-40s%10d%=20U%8Lx%16d%16Lx %Lx-%Lx", elf_section_name(em, es), es->index, format_elf_section_type,
               h->type, h->file_size, h->align, h->exec_address, h->file_offset, h->file_offset + h->file_size);
...
format_elf_segment(u8 *s, va_list *args)
{
    ...
    if (!h)
        return format(s, "%=16s%=16s%=16s%=16s", "Type", "Virt. Address", "Phys. Address", "Size");

    s = format(s, "%=16U%16Lx%16Lx%16Lx%16Lx", format_elf_segment_type, h->type, h->virtual_address,
...
format_elf_symbol(u8 *s, va_list *args)
{
    ...
    if (!sym)
        return format(s, "%=32s%=16s%=16s%=16s%=16s%=16s", "Symbol", "Size", "Value", "Type", "Visibility", "Section");

    s = format(s, "%-32s%16Ld%16Lx%=16U%=16U%U", elf_symbol_name(t, sym), sym->size, sym->value,
               format_elf_symbol_binding_and_type, sym->binding_and_type, format_elf_symbol_visibility, sym->visibility,
...
format_elf_main(u8 *s, va_list *args)
{
    ...
    s = format(s, "File header: machine: %U, file type/class %U/%U, data-encoding: %U, abi: %U version %d\n",
               format_elf_architecture, em->first_header.architecture, format_elf_file_type, em->first_header.file_type,
               format_elf_file_class, em->first_header.file_class, format_elf_data_encoding,
               em->first_header.data_encoding, format_elf_abi, em->first_header.abi, em->first_header.abi_version);
...

    {
        ...
        copy = 0;
        vec_foreach(h, em->sections) if (h->header.type != ~0) vec_add1(copy, h[0]);
...

    {
        ...
        copy = 0;
        vec_foreach(h, em->segments) if (h->header.type != ELF_SEGMENT_UNUSED && h->header.type != ~0)
            vec_add1(copy, h[0]);
    ...
    if ((verbose & FORMAT_ELF_MAIN_SYMBOLS) && vec_len(em->symbol_tables) > 0) {
        ...
        vec_foreach(t, em->symbol_tables)
        {
            ...
            vec_foreach(sym, t->symbols) s = format(s, "%U\n", format_elf_symbol, em, t, sym);
        }
    }
...
#undef _
    ...
    {
        if (sym64->name != 0)
            hash_set_mem(tab->symbol_by_name, tab->string_table + sym64->name, sym64 - tab->symbols);
    }
}
...
static void *
elf_get_section_contents_with_starting_address(elf_main_t *em, uword start_address, uword elt_size,
...
elf_parse_dynamic(elf_main_t *em)
{
    ...
    vec_foreach(e, em->dynamic_entries)
    {
        ...
        case ELF_DYNAMIC_ENTRY_VERSYM:
            em->versym = elf_get_section_contents_with_starting_address(em, e->data, sizeof(em->versym[0]),
        ...
        case ELF_DYNAMIC_ENTRY_VERSION_NEED:
            em->verneed = elf_get_section_contents_with_starting_address(em, e->data, sizeof(em->verneed[0]),
...
void
elf_create_section_with_contents(elf_main_t *em, char *section_name, elf64_section_header_t *header, void *contents,
