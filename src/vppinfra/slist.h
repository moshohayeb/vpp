...
typedef enum { CLIB_SLIST_MATCH = 0, CLIB_SLIST_NO_MATCH } clib_slist_search_result_t;
...
clib_slist_get_next_at_level(clib_slist_elt_t *elt, int level)
{
    if (elt->n.next0[0] & 1) {
        ...
        return (elt->n.next0[0] == (u32) ~0) ? elt->n.next0[0] : (elt->n.next0[0] >> 1);
    } else {
...
clib_slist_set_next_at_level(clib_slist_elt_t *elt, u32 index, int level)
{
    ...
    if (elt->n.next0[0] & 1) {
        old_level0_value[0] = (elt->n.next0[0] == (u32) ~0) ? elt->n.next0[0] : elt->n.next0[0] >> 1;
...
clib_error_t *clib_slist_init(clib_slist_t *sp, f64 branching_factor, clib_slist_key_compare_function_t compare,
                              format_function_t format_user_element);
