...
clib_mem_vm_ext_alloc(clib_mem_vm_alloc_t *a)
{
    ...
    if (a->flags & CLIB_MEM_VM_F_SHARED) {
        ...
        if (a->flags & CLIB_MEM_VM_F_HUGETLB) {
            ...
            if (mount("none", (char *) mount_dir, "hugetlbfs", 0, NULL)) {
                ...
                err = clib_error_return_unix(0, "mount hugetlb directory '%s'", mount_dir);
                goto error;
            }
    ...
    if (a->flags & CLIB_MEM_VM_F_HUGETLB_PREALLOC) {
        err = clib_sysfs_prealloc_hugepages(a->numa_node, 1 << (log2_page_size - 10), n_pages);
        if (err)
            goto error;
    }
    ...
    if (old_mpol != -1) {
        ...
        if (rv == -1 && a->numa_node != 0 && (a->flags & CLIB_MEM_VM_F_NUMA_FORCE) != 0) {
            err = clib_error_return_unix(0, "set_mempolicy");
            goto error;
        }
    }

    addr = mmap(uword_to_pointer(a->requested_va, void *), a->size, (PROT_READ | PROT_WRITE), mmap_flags, fd, 0);
    if (addr == MAP_FAILED) {
    ...
    if (old_mpol != -1 && set_mempolicy(old_mpol, old_mask, sizeof(old_mask) * 8 + 1) == -1) {
        err = clib_error_return_unix(0, "set_mempolicy");
        goto error;
    }
...
clib_mem_vm_ext_map(clib_mem_vm_map_t *a)
{
    ...
    addr =
        (void *) mmap(uword_to_pointer(a->requested_va, void *), a->size, PROT_READ | PROT_WRITE, mmap_flags, a->fd, 0);
