...
{
    return (version << (1 + CLIB_CUCKOO_USE_COUNT_BIT_WIDTH)) + (use_count << 1) + writer_flag;
}
...
always_inline clib_cuckoo_bucket_aux_t
clib_cuckoo_bucket_aux_set_use_count(clib_cuckoo_bucket_aux_t aux, int use_count)
{
...
always_inline clib_cuckoo_bucket_aux_t
clib_cuckoo_bucket_aux_set_writer_flag(clib_cuckoo_bucket_aux_t aux, int writer_flag)
{
    u64 version   = clib_cuckoo_bucket_aux_get_version(aux);
    int use_count = clib_cuckoo_bucket_aux_get_use_count(aux);
    return clib_cuckoo_bucket_aux_pack(version, use_count, writer_flag);
...
void CV(clib_cuckoo_init)(CVT(clib_cuckoo) * h, const char *name, u64 nbuckets,
...
always_inline clib_cuckoo_lookup_info_t CV(clib_cuckoo_calc_lookup)(CVT(clib_cuckoo_bucket) * buckets, u64 hash)
{
    ...
    CLIB_PREFETCH(vec_elt_at_index(buckets, lookup.bucket1), sizeof(*buckets), LOAD);
#endif
    u8 reduced_hash = clib_cuckoo_reduce_hash(hash);
    lookup.bucket2  = clib_cuckoo_get_other_bucket(nbuckets, lookup.bucket1, reduced_hash);
#if CLIB_CUCKOO_OPTIMIZE_PREFETCH
    CLIB_PREFETCH(vec_elt_at_index(buckets, lookup.bucket2), sizeof(*buckets), LOAD);
#endif
 ...
 */
always_inline int CV(clib_cuckoo_bucket_search)(CVT(clib_cuckoo_bucket) * b, CVT(clib_cuckoo_kv) * kvp, u8 reduced_hash)
{
...
always_inline int CV(clib_cuckoo_search_inline)(CVT(clib_cuckoo) * h, CVT(clib_cuckoo_kv) * kvp)
{
    clib_cuckoo_lookup_info_t lookup;
    int rv;
...
again:
    ...
    do {
        rv = CV(clib_cuckoo_bucket_search)(vec_elt_at_index(buckets, lookup.bucket1), kvp, lookup.reduced_hash);
    } while (PREDICT_FALSE(CLIB_CUCKOO_ERROR_AGAIN == rv));
    if (CLIB_CUCKOO_ERROR_SUCCESS == rv) {
        return CLIB_CUCKOO_ERROR_SUCCESS;
    }

    rv = CV(clib_cuckoo_bucket_search)(vec_elt_at_index(buckets, lookup.bucket2), kvp, lookup.reduced_hash);
