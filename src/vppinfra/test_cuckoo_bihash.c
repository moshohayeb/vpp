...
#define w_thread(x, guts)                                                                                              \
    ...
    {                                                                                                                  \
        ...
        do {                                                                                                           \
            for (counter = 0; counter < vec_len(tm->key_add_del_sequence); ++counter) {                                \
...
#define run_threads(x)                                                                                                 \
    do {                                                                                                               \
        ...
        tm->deadline = clib_cpu_time_now() + tm->runtime * tm->clib_time.clocks_per_second;                            \
        ...
        */                                                                                                             \
        ...
        before       = clib_time_now(&tm->clib_time);                                                                  \
        tm->deadline = clib_cpu_time_now() + tm->runtime * tm->clib_time.clocks_per_second;                            \
        if (0 != pthread_create(&tm->x##writer_thread, NULL, x##writer_thread, tm)) {                                  \
            perror("pthread_create()");                                                                                \
            abort();                                                                                                   \
        }                                                                                                              \
                                                                                                                       \
        for (i = 0; i < tm->nthreads; i++) {                                                                           \
            ...
            if (0 != pthread_create(&tm->x##reader_threads[i], NULL, x##reader_thread, &tm->rthread_data[i])) {        \
                perror("pthread_create()");                                                                            \
                abort();                                                                                               \
            }                                                                                                          \
        }                                                                                                              \
        ...
        if (delta > 0) {                                                                                               \
            ...
            fformat(stdout, #x "-> %.f searches per second\n", sps);                                                   \
                                                                                                                       ...
                                                                                                                       \
        fformat(stdout, #x "-> %wu adds, %wu dels, %lld searches in %.6f seconds\n", tm->nadds, tm->ndels,             \
...
test_cuckoo_bihash(test_main_t *tm)
{
    ...
    for (iter = 0; iter < tm->search_iter; ++iter) {
        ...
        fformat(stdout, "Bihash add/del speed is %.2f%% of cuckoo add/del speed\n", bops / cops * 100);
        fformat(stdout, "Bihash search speed is %.2f%% of cuckoo search speed\n", bsps / csps * 100);
    }
    return 0;
}
