...
unformat_srv6_localsid_sample(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "new_srv6_localsid %u", &table_id)) {
        ...
        ls_mem = clib_mem_alloc_aligned_at_offset(sizeof(srv6_localsid_sample_per_sid_memory_t), 0, 0, 1);
...
srv6_localsid_sample_init(vlib_main_t *vm)
{
    ...
    rv = sr_localsid_register_function(vm, srv6_localsid_name, keyword_str, def_str, params_str,
