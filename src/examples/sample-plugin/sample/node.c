...
VLIB_NODE_FN(sample_node)(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
        ...
        for (i = 0; i < frame->n_vectors; i++) {
            if (b[0]->flags & VLIB_BUFFER_IS_TRACED) {
                ...
                clib_memcpy(t->new_src_mac, en->src_address, sizeof(t->new_src_mac));
                clib_memcpy(t->new_dst_mac, en->dst_address, sizeof(t->new_dst_mac));
                b++;
            } else
                break;
        }
