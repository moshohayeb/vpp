...
static void
test_rx_callback(mc_main_t *mcm, mc_stream_t *stream, mc_peer_id_t peer_id, u32 buffer_index)
{
    if (SERIALIZE) {
        ...
        fformat(stdout, "RX from %U %U\n", stream->transport->format_peer_id, peer_id, format_hex_bytes, dp,
                tm->n_msg_bytes);
...
static u8 *
test_snapshot_callback(mc_main_t *mcm, u8 *data_vector, u32 last_global_sequence_processed)
{
    if (SERIALIZE) {
        serialize_main_t m;
...
static clib_error_t *
mc_test_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    f64 npkts = 10;
...
static clib_error_t *
mc_show_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *
mc_clear_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    mc_main_t *mcm = &mc_socket_main.mc_main;
...
mc_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    if (tm->n_packets_to_send > 0)
        vlib_process_signal_event(vm, mc_test_process_node.index, EVENT_SEND_DATA, (uword) tm->n_packets_to_send);
